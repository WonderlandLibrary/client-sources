...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public void enableShader(int id) {
		try {
			if (OpenGlHelper.shadersSupported && this.mc.field_175622_Z instanceof EntityPlayer) {
				if (this.theShaderGroup != null) {
					this.theShaderGroup.deleteShaderGroup();
				}
				this.loadShader(shaderResourceLocations[id]);
			}
		} catch (Exception e) {
	...
	private void func_175069_a(ResourceLocation p_175069_1_) {
		if (OpenGlHelper.isFramebufferEnabled()) {
			try {
				this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), this.resourceManager,
						this.mc.getFramebuffer(), p_175069_1_);
				this.theShaderGroup.createBindFramebuffers(this.mc.displayWidth, this.mc.displayHeight);
				this.field_175083_ad = true;
	...
	public void getMouseOver(float p_78473_1_) {
		...
		if (var2 != null && this.mc.theWorld != null) {
			...
			Vec3 var10 = null;
			float var11 = 1.0F;
			List var12 = this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(var2,
					var2.getEntityBoundingBox().addCoord(var8.xCoord * var3, var8.yCoord * var3, var8.zCoord * var3)
							.expand((double) var11, (double) var11, (double) var11));
			double var13 = var5;
	...
	private void setupViewBobbing(float p_78475_1_) {
		if (this.mc.func_175606_aa() instanceof EntityPlayer) {
			if (!ModuleUtils.getMod(NoBob.class).isToggled()) {
				...
				GlStateManager.rotate(Math.abs(MathHelper.cos(var4 * (float) Math.PI - 0.2F) * var5) * 5.0F, 1.0F, 0.0F,
						0.0F);
	...
	private void orientCamera(float p_78467_1_) {
		...
		if (var2 instanceof EntityLivingBase && ((EntityLivingBase) var2).isPlayerSleeping()) {
			...
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
					Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera,
							new Object[] { this.mc.theWorld, var27, partialTicks, var2 });
				...
				GlStateManager.rotate(
						var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * p_78467_1_ + 180.0F, 0.0F,
						-1.0F, 0.0F);
				GlStateManager.rotate(
			...
			double var28 = (double) (this.thirdPersonDistanceTemp
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				double roll = (double) (-MathHelper.sin(yaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(pitch / 180.0F * (float) Math.PI)) * var28;
				double event = (double) (MathHelper.cos(yaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(pitch / 180.0F * (float) Math.PI)) * var28;
				...
				for (int var20 = 0; var20 < 8; ++var20) {
					...
					MovingObjectPosition var24 = this.mc.theWorld.rayTraceBlocks(
							new Vec3(var4 + (double) var21, var6 + (double) var22, var8 + (double) var23),
							new Vec3(var4 - roll + (double) var21 + (double) var23, var6 - var18 + (double) var22,
									var8 - event + (double) var23));
		...
		if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				if (var2 instanceof EntityAnimal) {
					...
					yaw = block.prevRotationYawHead + (block.rotationYawHead - block.prevRotationYawHead) * p_78467_1_
							+ 180.0F;
				}

				Block var32 = ActiveRenderInfo.func_180786_a(this.mc.theWorld, var2, p_78467_1_);
				Object var33 = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor,
						new Object[] { this, var2, var32, Float.valueOf(p_78467_1_), Float.valueOf(yaw),
								Float.valueOf(pitch), Float.valueOf(var31) });
			...
			GlStateManager.rotate(var2.prevRotationPitch + (var2.rotationPitch - var2.prevRotationPitch) * p_78467_1_,
					1.0F, 0.0F, 0.0F);

			if (var2 instanceof EntityAnimal) {
				EntityAnimal var281 = (EntityAnimal) var2;
				GlStateManager.rotate(var281.prevRotationYawHead
						+ (var281.rotationYawHead - var281.prevRotationYawHead) * p_78467_1_ + 180.0F, 0.0F, 1.0F,
						0.0F);
			} else {
				GlStateManager.rotate(
						var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * p_78467_1_ + 180.0F, 0.0F,
	...
	public void renderHand(float p_78476_1_, int p_78476_2_) {
		if (!this.field_175078_W) {
			...
			if (!Config.isShaders() || !Shaders.isHandRendered) {
				...
				var4 = this.mc.func_175606_aa() instanceof EntityLivingBase
						&& ((EntityLivingBase) this.mc.func_175606_aa()).isPlayerSleeping();

				if (this.mc.gameSettings.thirdPersonView == 0 && !var4 && !this.mc.gameSettings.hideGUI
	...
	private void updateTorchFlicker() {
		this.field_175075_L = (float) ((double) this.field_175075_L
	...
	private void updateLightmap(float partialTicks) {
		if (this.lightmapUpdateNeeded) {
			...
			if (var2 != null) {
				if (Config.isCustomColors() && CustomColors.updateLightmap(var2, this.torchFlickerX,
				...
				for (int var3 = 0; var3 < 256; ++var3) {
					...
					float var6 = var2.provider.getLightBrightnessTable()[var3 % 16]
				...
				try {
					if (Reflector.ForgeHooksClient_drawScreen.exists()) {
						Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, new Object[] { this.mc.currentScreen,
								Integer.valueOf(var161), Integer.valueOf(var171), Float.valueOf(partialTicks) });
					} else {
					...
					var11.addCrashSectionCallable("Screen size", new Callable() {
						...
						public String call() {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d",
									new Object[] { Integer.valueOf(var131.getScaledWidth()),
											Integer.valueOf(var131.getScaledHeight()),
											Integer.valueOf(EntityRenderer.this.mc.displayWidth),
											Integer.valueOf(EntityRenderer.this.mc.displayHeight),
											Integer.valueOf(var131.getScaleFactor()) });
						}
					});
				...
				if (this.mc.objectMouseOver != null
					...
					if (this.mc.playerController.func_178889_l() == WorldSettings.GameType.SPECTATOR) {
						var2 = ReflectorForge.blockHasTileEntity(state)
								&& this.mc.theWorld.getTileEntity(var4) instanceof IInventory;
					} else {
	...
	private void func_175067_i(float p_175067_1_) {
		if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI && !this.mc.thePlayer.func_175140_cp()
	...
	private void func_175068_a(int pass, float partialTicks, long finishTimeNano) {
		...
		if (isShaders) {
			ShadersRender.setupTerrain(var5, var9, (double) partialTicks, var8, this.field_175084_ae++,
					this.mc.thePlayer.func_175149_v());
		} else {
			var5.func_174970_a(var9, (double) partialTicks, var8, this.field_175084_ae++,
					this.mc.thePlayer.func_175149_v());
		}
			...
			if (this.mc.objectMouseOver != null && var9.isInsideOfMaterial(Material.water) && var7) {
				...
				if ((!Reflector.ForgeHooksClient_onDrawBlockHighlight.exists()
						|| !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight,
								new Object[] { var5, var16, this.mc.objectMouseOver, Integer.valueOf(0),
										var16.getHeldItem(), Float.valueOf(partialTicks) }))
		...
		if (Reflector.ForgeHooksClient_dispatchRenderLast.exists()) {
			...
			Reflector.callVoid(Reflector.ForgeHooksClient_dispatchRenderLast,
					new Object[] { var5, Float.valueOf(partialTicks) });
		}

		this.mc.mcProfiler.endStartSection("hand");
		boolean handRendered = Reflector.callBoolean(Reflector.ForgeHooksClient_renderFirstPersonHand,
	...
	private void addRainParticles() {
		...
		if (var1 != 0.0F && Config.isRainSplash()) {
			...
			for (int var14 = 0; var14 < var13; ++var14) {
				BlockPos var15 = var3.func_175725_q(var4.add(this.random.nextInt(var5) - this.random.nextInt(var5), 0,
						this.random.nextInt(var5) - this.random.nextInt(var5)));
				...
				if (var15.getY() <= var4.getY() + var5 && var15.getY() >= var4.getY() - var5
					...
					if (var18.getMaterial() == Material.lava) {
						this.mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL,
								...
								(double) ((float) var15.getY() + 0.1F) - var18.getBlockBoundsMinY(),
								(double) ((float) var15.getZ() + var20), 0.0D, 0.0D, 0.0D, new int[0]);
						...
						this.mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP,
								(double) ((float) var17.getX() + var19),
								(double) ((float) var17.getY() + 0.1F) + var18.getBlockBoundsMaxY(),
								(double) ((float) var17.getZ() + var20), 0.0D, 0.0D, 0.0D, new int[0]);
					}
				}
			}
			...
			if (var12 > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				...
				if (var8 > (double) (var4.getY() + 1)
						&& var3.func_175725_q(var4).getY() > MathHelper.floor_float((float) var4.getY())) {
					...
					if (var26.canSpawnLightningBolt() || var26.getEnableSnow()) {
						...
						if (var28 != var29) {
							...
							if (var4.getWorldChunkManager().getTemperatureAtHeight(var32, var27) >= 0.15F) {
								...
								var33 = ((float) (this.rendererUpdateCount + var21 * var21 * 3121 + var21 * 45238971
								...
								var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, (double) var28,
										(double) ((float) var20 - var24) + 0.5D, (double) (0.0F * var30),
										(double) ((float) var28 * var30 / 4.0F + var33 * var30));
								var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, (double) var28,
										(double) ((float) var20 + var24) + 0.5D, (double) (1.0F * var30),
										(double) ((float) var28 * var30 / 4.0F + var33 * var30));
								var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, (double) var29,
										(double) ((float) var20 + var24) + 0.5D, (double) (1.0F * var30),
								...
								float var44 = this.random.nextFloat()
										+ var19 * 0.01F * (float) this.random.nextGaussian();
								float var35 = this.random.nextFloat()
								...
								var9.setTranslation(-var10 * 1.0D, -var12 * 1.0D, -var14 * 1.0D);
								var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, (double) var28,
										(double) ((float) var20 - var24) + 0.5D, (double) (0.0F * var30 + var44),
										(double) ((float) var28 * var30 / 4.0F + var33 * var30 + var35));
								var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, (double) var28,
										(double) ((float) var20 + var24) + 0.5D, (double) (1.0F * var30 + var44),
										(double) ((float) var28 * var30 / 4.0F + var33 * var30 + var35));
								var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, (double) var29,
										(double) ((float) var20 + var24) + 0.5D, (double) (1.0F * var30 + var44),
										(double) ((float) var29 * var30 / 4.0F + var33 * var30 + var35));
								var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, (double) var29,
										(double) ((float) var20 - var24) + 0.5D, (double) (0.0F * var30 + var44),
										(double) ((float) var29 * var30 / 4.0F + var33 * var30 + var35));
								var9.setTranslation(0.0D, 0.0D, 0.0D);
	...
	private void updateFogColor(float partialTicks) {
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			double var19 = -1.0D;
			Vec3 var20 = MathHelper.sin(var2.getCelestialAngleRadians(partialTicks)) > 0.0F
					? new Vec3(var19, 0.0D, 0.0D) : new Vec3(1.0D, 0.0D, 0.0D);
			...
			if (var13 > 0.0F) {
				float[] var21 = var2.provider.calcSunriseSunsetColors(var2.getCelestialAngle(partialTicks),
						partialTicks);
		...
		if (this.cloudFog) {
			...
			fogYFactor = CustomColors.getUnderwaterColor(this.mc.theWorld, this.mc.func_175606_aa().posX,
		...
		if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
			Object event1 = Reflector
					.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor,
							new Object[] { this, var3, var211, Float.valueOf(partialTicks),
									Float.valueOf(this.field_175080_Q), Float.valueOf(this.field_175082_R),
									Float.valueOf(this.field_175081_S) });
			Reflector.postForgeBusEvent(event1);
			this.field_175080_Q = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_red,
					this.field_175080_Q);
			this.field_175082_R = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_green,
					this.field_175082_R);
			this.field_175081_S = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_blue,
					this.field_175081_S);
		}
	...
	private void setupFog(int p_78468_1_, float partialTicks) {
		...
		GL11.glFog(GL11.GL_FOG_COLOR,
				this.setFogColorBuffer(this.field_175080_Q, this.field_175082_R, this.field_175081_S, 1.0F));
		GL11.glNormal3f(0.0F, -1.0F, 0.0F);
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		...
		if (Reflector.ForgeHooksClient_getFogDensity.exists()) {
			forgeFogDensity = Reflector.callFloat(Reflector.ForgeHooksClient_getFogDensity,
					new Object[] { this, var3, var5, Float.valueOf(partialTicks), Float.valueOf(0.1F) });
		}
		...
		if (forgeFogDensity >= 0.0F) {
			...
			if (var3 instanceof EntityLivingBase && ((EntityLivingBase) var3).isPotionActive(Potion.blindness)) {
				...
				if (Reflector.ForgeHooksClient_onFogRender.exists()) {
					Reflector.callVoid(Reflector.ForgeHooksClient_onFogRender, new Object[] { this, var3, var5,
							Float.valueOf(partialTicks), Integer.valueOf(p_78468_1_), Float.valueOf(var6) });
				}
			}
		}
	...
	private void frameInit() {
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				...
				ChatComponentText msg1 = new ChatComponentText(
			...
			if (now > this.lastErrorCheckTimeMs + 10000L) {
				...
				if (err != 0) {
					...
					ChatComponentText msg = new ChatComponentText(
							I18n.format("of.message.openglError", new Object[] { Integer.valueOf(err), text }));
