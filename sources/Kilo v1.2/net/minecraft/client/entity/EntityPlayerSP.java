...
public class EntityPlayerSP extends AbstractClientPlayer
{
	...
	public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            for(int i = 0; i < ModuleManager.enabledList().size(); i++) {
            	Module m = ModuleManager.enabledList().get(i);
            	m.onPlayerPreUpdate();
            }
            ...
            for(int i = ModuleManager.enabledList().size()-1; i >= 0; i--) {
            	Module m = ModuleManager.enabledList().get(i);
            	m.onPlayerPostUpdate();
            }
        }
    }
	...
	public void swingItem()
    {
    	if (this == mc.thePlayer) {
        	if (ModuleManager.get("noswing").active) {
        		return;
        	}
    	}
        super.swingItem();
    ...
    {
        if (this.noClip || ModuleManager.get("phase").active)
        {
            return false;
        }
        else
    ...
    {
    	if (!ModuleManager.get("blanksign").active) {
	...
	public void onLivingUpdate()
    {
        ...
        {
        	NoSlowDown nsd = (NoSlowDown)ModuleManager.get("noslowdown");
    		if (nsd.slowOnItemUse()) {
        ...
        {
        	if (!(ModuleManager.get("sprint").active && Util.makeBoolean(ModuleManager.get("sprint").getOptionValue("directional")) && (this.movementInput.moveForward != 0 || this.movementInput.moveStrafe != 0))) {
        		this.setSprinting(false);
        	}
        }
        ...
        if (this.capabilities.isFlying && this.isCurrentViewEntity())
        {
            ...
            {
                this.motionY -= (double)(this.capabilities.getFlySpeed() * 3.0F) * Util.makeFloat(ModuleManager.get("speed").active?ModuleManager.get("speed").getOptionValue("speed"):1);
            }
        ...
        if (this.isRidingHorse())
        {
            ...
            if (var1 && !this.movementInput.jump)
            {
                ...
                if (ModuleManager.get("horsejump").active) {
