...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public EntityPlayer(World worldIn, GameProfile gameProfileIn)
    {
        ...
        this.setLocationAndAngles(var3.getX() + 0.5D, var3.getY() + 1, var3.getZ() + 0.5D, 0.0F, 0.0F);
    ...
    {
        this.noClip = ModuleManager.get("freecam").active?true:this.isSpectator();
    ...
    protected void updateItemUse(ItemStack itemStackIn, int p_71010_2_)
    {
        ...
        if (itemStackIn.getItemUseAction() == EnumAction.EAT)
        {
            for (int var3 = 0; var3 < p_71010_2_; ++var3)
            {
                ...
                var7 = var7.addVector(this.posX, this.posY + this.getEyeHeight(), this.posZ);
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        {
            double var4 = this.posY - 0.30000001192092896D + this.getEyeHeight();
            ...
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (var2 > 0.0F || var4 > 0.0F)
                {
                    ...
                    if (var14)
                    {
                        if (var18 > 0)
                        {
                            targetEntity.addVelocity(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * var18 * 0.5F, 0.1D, MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * var18 * 0.5F);
                            this.motionX *= 0.6D;
                            this.motionZ *= 0.6D;
    ...
    public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation)
    {
        if (!this.worldObj.isRemote)
        {
            ...
            double var2 = 8.0D;
            double var4 = 5.0D;
            List var6 = this.worldObj.getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB(bedLocation.getX() - var2, bedLocation.getY() - var4, bedLocation.getZ() - var2, bedLocation.getX() + var2, bedLocation.getY() + var4, bedLocation.getZ() + var2));
        ...
        if (this.worldObj.isBlockLoaded(bedLocation))
        {
            ...
            this.setPosition(bedLocation.getX() + var3, bedLocation.getY() + 0.6875F, bedLocation.getZ() + var8);
        }
        else
        {
