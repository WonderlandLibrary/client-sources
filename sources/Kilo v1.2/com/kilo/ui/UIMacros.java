...
import com.kilo.ui.inter.slotlist.slot.MacroSlot;
...
public class UIMacros extends UI {
	...
	public void init() {
		...
		fW = 600;
		fH = 48+48+48+(48*Math.min(8, MacroManager.getSize()));

		sl = new SlotList(7f);
		
		inters.clear();
		inters.add(new IconButton(this, fX+(fW/2)-40, fY-(fH/2)+12, 24, 24, 0x00000000, Resources.iconClose[2]));
		inters.add(new Button(this, "Create Macro", fX-72-8-144, fY+(fH/2)-40, 144, 32, Fonts.ttfRoundedBold14, Colors.GREEN.c, null, 0));
		inters.add(new Button(this, "Edit Macro", fX-72, fY+(fH/2)-40, 144, 32, Fonts.ttfRoundedBold14, Colors.GREY.c, null, 0));
		inters.add(new Button(this, "Delete Macro", fX+72+8, fY+(fH/2)-40, 144, 32, Fonts.ttfRoundedBold14, Colors.RED.c, null, 0));
	}
	...
	public void update(int mx, int my) {
		...
		if (fH != 48+48+48+(48*Math.min(8, MacroManager.getSize()))) {
			init();
		}

		Macro activeMacro = null;
		for(Slot s : sl.slots) {
			if (s.active) {
				activeMacro = MacroManager.getMacro(((MacroSlot)s).index);
				break;
			}
		}
		((Button)inters.get(2)).enabled = activeMacro != null;
		((Button)inters.get(3)).enabled = activeMacro != null;
		...
		if (sl.slots.size() != MacroManager.getSize()) {
			sl.slots.clear();
			int i = 0;
			for(Macro w : MacroManager.getList()) {
				sl.slots.add(new MacroSlot(sl, MacroManager.getIndex(w), sl.x, sl.y, sl.w, 48, 0, i*48));
				i++;
			}
		}
	...
	public void interact(final Inter i) {
		...
		case 0:
			...
			break;
		case 1:
			changePopup(new UIPopupMacroCreate(this));
			break;
		case 2:
			changePopup(new UIPopupMacroEdit(this, activeMacro));
			break;
		case 3:
			if (activeMacro != null) {
				MacroManager.removeMacro(activeMacro);
				Config.saveMacros();
			}
			break;
		}
