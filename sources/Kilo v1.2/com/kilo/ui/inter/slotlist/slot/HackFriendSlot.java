...
public class HackFriendSlot extends Slot {

	private final Minecraft mc = Minecraft.getMinecraft();
	
	public int index;
	
	public HackFriendSlot(SlotList p, int i, float x, float y, float w, float h, float ox, float oy) {
	...
	public void render(float opacity) {
		...
		if (HackFriendManager.getHackFriend(index) != null) {
			HackFriend hf = HackFriendManager.getHackFriend(index);
			String name = hf.name;
			if (hf.nickname.length() > 0) {
				name = hf.nickname+" ("+hf.name+")";
			}
