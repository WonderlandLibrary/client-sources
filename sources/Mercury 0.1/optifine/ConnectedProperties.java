...
public class ConnectedProperties {
    ...
    public ConnectedProperties(Properties props, String path) {
        ConnectedParser cp2 = new ConnectedParser("ConnectedTextures");
        this.name = cp2.parseName(path);
        this.basePath = cp2.parseBasePath(path);
        this.matchBlocks = cp2.parseMatchBlocks(props.getProperty("matchBlocks"));
        this.metadatas = cp2.parseIntList(props.getProperty("metadata"));
        ...
        this.biomes = cp2.parseBiomes(props.getProperty("biomes"));
        this.minHeight = cp2.parseInt(props.getProperty("minHeight"), -1);
        this.maxHeight = cp2.parseInt(props.getProperty("maxHeight"), 1024);
        this.renderPass = cp2.parseInt(props.getProperty("renderPass"));
        this.innerSeams = ConnectedParser.parseBoolean(props.getProperty("innerSeams"));
        this.width = cp2.parseInt(props.getProperty("width"));
        this.height = cp2.parseInt(props.getProperty("height"));
        this.weights = cp2.parseIntList(props.getProperty("weights"));
    ...
    private String[] parseMatchTiles(String str) {
        ...
        for (int i2 = 0; i2 < names.length; ++i2) {
            String iconName = names[i2];
            if (iconName.endsWith(".png")) {
                iconName = iconName.substring(0, iconName.length() - 4);
            }
            names[i2] = iconName = TextureUtils.fixResourcePath(iconName, this.basePath);
        }
        return names;
    }
    ...
    private String[] parseTileNames(String str) {
        ...
        for (int names = 0; names < iconStrs.length; ++names) {
            String[] iconName;
            String i2 = iconStrs[names];
            if (i2.contains("-") && (iconName = Config.tokenize(i2, "-")).length == 2) {
                int pathBlocks = Config.parseInt(iconName[0], -1);
                int max = Config.parseInt(iconName[1], -1);
                if (pathBlocks >= 0 && max >= 0) {
                    if (pathBlocks <= max) {
                        for (int n2 = pathBlocks; n2 <= max; ++n2) {
                            list.add(String.valueOf(n2));
                        }
                        continue;
                    }
                    Config.warn("Invalid interval: " + i2 + ", when parsing: " + str);
                    continue;
                }
            }
            list.add(i2);
    ...
    public static IProperty getProperty(String key, Collection properties) {
        ...
        do {
            ...
            return null;
        } while (!key.equals((prop = (IProperty)it2.next()).getName()));
        return prop;
    }
    ...
    private int[] detectMatchBlockIds() {
        ...
        int id2 = Config.parseInt(var5, -1);
        if (id2 < 0) {
            arrn = null;
        } else {
            int[] arrn2 = new int[1];
        ...
        if (this.tiles != null && this.tiles.length > 0) {
            if (this.weights != null) {
                ...
                for (i2 = 0; i2 < this.weights.length; ++i2) {
                    this.sumWeights[i2] = var5 += this.weights[i2];
                }
    ...
    public int getMetadataMax() {
        ...
        if (this.matchBlocks != null) {
            for (int i2 = 0; i2 < this.matchBlocks.length; ++i2) {
                ...
                var4 = this.getMax(mb2.getMetadatas(), var4);
            }
        }
        return var4;
    }
