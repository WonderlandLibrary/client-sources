...
public class CustomSkyLayer {
    ...
    public CustomSkyLayer(Properties props, String defSource) {
        ConnectedParser cp2 = new ConnectedParser("CustomSky");
        ...
        this.days = cp2.parseRangeListInt(props.getProperty("days"));
        this.daysLoop = cp2.parseInt(props.getProperty("daysLoop"), 8);
    }
    ...
    private float[] parseAxis(String str, float[] defVal) {
        ...
        for (int ax2 = 0; ax2 < strs.length; ++ax2) {
            fs2[ax2] = Config.parseFloat(strs[ax2], Float.MIN_VALUE);
            if (fs2[ax2] == Float.MIN_VALUE) {
                Config.warn("Invalid axis: " + str);
                return defVal;
            }
            if (!(fs2[ax2] < -1.0f) && !(fs2[ax2] > 1.0f)) continue;
        ...
        float ay2 = fs2[1];
        float az2 = fs2[2];
        if (var9 * var9 + ay2 * ay2 + az2 * az2 < 1.0E-5f) {
            Config.warn("Invalid axis values: " + str);
            return defVal;
        }
        float[] as2 = new float[]{az2, ay2, -var9};
        return as2;
    }
    ...
    private void renderSide(Tessellator tess, int side) {
        WorldRenderer wr2 = tess.getWorldRenderer();
        double tx2 = (double)(side % 3) / 3.0;
        double ty2 = (double)(side / 3) / 2.0;
        wr2.startDrawingQuads();
        wr2.addVertexWithUV(-100.0, -100.0, -100.0, tx2, ty2);
        wr2.addVertexWithUV(-100.0, -100.0, 100.0, tx2, ty2 + 0.5);
        wr2.addVertexWithUV(100.0, -100.0, 100.0, tx2 + 0.3333333333333333, ty2 + 0.5);
        wr2.addVertexWithUV(100.0, -100.0, -100.0, tx2 + 0.3333333333333333, ty2);
        tess.draw();
    }
