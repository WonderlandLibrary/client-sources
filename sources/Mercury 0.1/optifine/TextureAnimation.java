...
public class TextureAnimation {
    ...
    public TextureAnimation(String texFrom, byte[] srcData, String texTo, ResourceLocation locTexTo, int dstX, int dstY, int frameWidth, int frameHeight, Properties props, int durDef) {
        ...
        for (int i2 = 0; i2 < this.frames.length; ++i2) {
            ...
            String indexStr = (String)props.get("tile." + i2);
            int index = Config.parseInt(indexStr, i2);
            String durationStr = (String)props.get("duration." + i2);
            int duration = Config.parseInt(durationStr, durationDef);
            this.frames[i2] = frm = new TextureAnimationFrame(index, duration);
        }
    }
