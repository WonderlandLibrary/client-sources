...
extends TreeSet {
    ...
    private Set getSubSet(NextTickListEntry entry, boolean autoCreate) {
        ...
        int cx2 = pos.getX() >> 4;
        int cz2 = pos.getZ() >> 4;
        return this.getSubSet(cx2, cz2, autoCreate);
    }

    private Set getSubSet(int cx2, int cz2, boolean autoCreate) {
        long key = ChunkCoordIntPair.chunkXZ2Int(cx2, cz2);
    ...
    public Iterator iterator() {
        ...
        for (int x2 = cMinX; x2 <= cMaxX; ++x2) {
            for (int z2 = cMinZ; z2 <= cMaxZ; ++z2) {
                Set set = this.getSubSet(x2, z2, false);
