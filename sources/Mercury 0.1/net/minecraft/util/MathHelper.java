...
public class MathHelper {
    ...
    static {
        ...
        for (i2 = 0; i2 < 65536; ++i2) {
            MathHelper.SIN_TABLE[i2] = (float)Math.sin((double)i2 * 3.141592653589793 * 2.0 / 65536.0);
        }
        int[] arrn = new int[32];
        arrn[1] = 1;
        arrn[2] = 28;
        ...
        for (i2 = 0; i2 < 4096; ++i2) {
            MathHelper.SIN_TABLE_FAST[i2] = (float)Math.sin(((float)i2 + 0.5f) / 4096.0f * 6.2831855f);
        }
        for (i2 = 0; i2 < 360; i2 += 90) {
            MathHelper.SIN_TABLE_FAST[(int)((float)i2 * 11.377778f) & 4095] = (float)Math.sin((float)i2 * 0.017453292f);
        }
    }
    ...
    public static long func_180187_c(int x2, int y2, int z2) {
        long var3 = (long)(x2 * 3129871) ^ (long)z2 * 116129781L ^ (long)y2;
