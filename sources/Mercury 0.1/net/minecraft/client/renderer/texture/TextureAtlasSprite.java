...
public class TextureAtlasSprite {
    ...
    private void func_180599_n() {
        ...
        if (var3 != (var5 = this.animationMetadata.getFrameIndex((this.frameCounter + 1) % (var4 = this.animationMetadata.getFrameCount() == 0 ? this.framesTextureData.size() : this.animationMetadata.getFrameCount()))) && var5 >= 0 && var5 < this.framesTextureData.size()) {
            ...
            for (int var8 = 0; var8 < var6.length; ++var8) {
                ...
                if (var8 >= var7.length || var7[var8].length != var6[var8].length) continue;
    ...
    public void func_180598_a(BufferedImage[] p_180598_1_, AnimationMetadataSection p_180598_2_) throws IOException {
        ...
        for (var6 = 0; var6 < p_180598_1_.length; ++var6) {
            BufferedImage i2 = p_180598_1_[var6];
            if (i2 == null) continue;
            if (var6 > 0 && (i2.getWidth() != var3 >> var6 || i2.getHeight() != var4 >> var6)) {
                throw new RuntimeException(String.format("Unable to load miplevel: %d, image is size: %dx%d, expected %dx%d", var6, i2.getWidth(), i2.getHeight(), var3 >> var6, var4 >> var6));
            }
            var5[var6] = new int[i2.getWidth() * i2.getHeight()];
            i2.getRGB(0, 0, i2.getWidth(), i2.getHeight(), var5[var6], 0, i2.getWidth());
        }
        if (p_180598_2_ == null) {
            if (var4 != var3) {
            ...
            if (p_180598_2_.getFrameCount() > 0) {
                ...
                while (data.hasNext()) {
                    int di2 = (Integer)data.next();
                    if (di2 >= var6) {
                        throw new RuntimeException("invalid frameindex " + di2);
                    }
                    this.allocateFrameTextureData(di2);
                    this.framesTextureData.set(di2, TextureAtlasSprite.getFrameTextureData(var5, var11, datas, di2));
                }
                this.animationMetadata = p_180598_2_;
            } else {
                ArrayList<AnimationFrame> var13 = Lists.newArrayList();
                for (int di3 = 0; di3 < var6; ++di3) {
                    this.framesTextureData.add(TextureAtlasSprite.getFrameTextureData(var5, var11, datas, di3));
                    var13.add(new AnimationFrame(di3, -1));
                }
        ...
        for (var11 = 0; var11 < this.framesTextureData.size(); ++var11) {
            ...
            if (var12 == null || this.iconName.startsWith("minecraft:blocks/leaves_")) continue;
            for (int di4 = 0; di4 < var12.length; ++di4) {
                int[] var14 = var12[di4];
