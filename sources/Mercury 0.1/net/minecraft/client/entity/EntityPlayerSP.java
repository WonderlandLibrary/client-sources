...
import us.amerikan.utils.PlayerUtils;
...
extends AbstractClientPlayer {
    ...
    public void func_175161_p() {
        ...
        MotionUpdateEvent event = new MotionUpdateEvent(EventType.PRE, this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, MotionUpdateEvent.State.PRE);
        ...
        if (this.func_175160_A()) {
            ...
            boolean bl2 = var14 = var9 != 0.0 || var11 != 0.0;
        ...
        EventManager.call(new MotionUpdateEvent(EventType.POST, this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, MotionUpdateEvent.State.POST));
    }
    ...
    public void sendChatMessage(String p_71165_1_) {
        if (amerikan.commandManager.execute(p_71165_1_) || p_71165_1_.startsWith(amerikan.instance.Chat_Prefix)) {
            return;
        }
    ...
    protected boolean pushOutOfBlocks(double x2, double y2, double z2) {
        ...
        double var10 = z2 - (double)var7.getZ();
    ...
    public boolean isSneaking() {
        boolean var1;
        boolean bl2 = var1 = this.movementInput != null ? this.movementInput.sneak : false;
    ...
    public void onLivingUpdate() {
        ...
        if (this.inPortal) {
            ...
            this.inPortal = false;
        } else if (this.isPotionActive(Potion.confusion) && EntityPlayerSP.getActivePotionEffect(Potion.confusion).getDuration() > 60) {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            if (amerikan.modulemanager.getModuleByClass(NoSlowDown.class).isEnabled()) {
                double speed;
                if (amerikan.setmgr.getSettingByName("Bypass-Mode").getValString().equalsIgnoreCase("AAC Bypass")) {
                    speed = 0.7111118767614212;
                    double strafe = Math.toRadians(Minecraft.thePlayer.movementInput.moveStrafe);
                    double forward = Math.toRadians(Minecraft.thePlayer.movementInput.moveForward);
                    Minecraft.thePlayer.movementInput.moveStrafe *= (float)(-Math.sin(strafe) * speed);
                    Minecraft.thePlayer.movementInput.moveForward *= (float)(Math.cos(forward) * speed);
                } else if (amerikan.setmgr.getSettingByName("Bypass-Mode").getValString().equalsIgnoreCase("AAC Fast")) {
                    speed = 0.80013137777125;
                    double strafe = Math.toRadians(Minecraft.thePlayer.movementInput.moveStrafe);
                    double forward = Math.toRadians(Minecraft.thePlayer.movementInput.moveForward);
                    Minecraft.thePlayer.movementInput.moveStrafe *= (float)(-Math.sin(strafe) * speed);
                    Minecraft.thePlayer.movementInput.moveForward *= (float)(Math.cos(forward) * speed);
                ...
                Minecraft.thePlayer.movementInput.moveStrafe *= 0.2f;
                Minecraft.thePlayer.movementInput.moveForward *= 0.2f;
            }
            Minecraft.thePlayer.sprintToggleTimer = 0;
        }
        ...
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, this.getEntityBoundingBox().minY + 0.5, this.posZ - (double)this.width * 0.35);
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, this.getEntityBoundingBox().minY + 0.5, this.posZ + (double)this.width * 0.35);
        boolean bl2 = var5 = (float)this.getFoodStats().getFoodLevel() > 6.0f || this.capabilities.allowFlying;
