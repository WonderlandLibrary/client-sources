...
import us.amerikan.utils.ColorUtils;
...
extends Gui {
    ...
    public void func_175180_a(float p_175180_1_) {
        ...
        if (!Minecraft.thePlayer.isPotionActive(Potion.confusion) && (var7 = Minecraft.thePlayer.prevTimeInPortal + (Minecraft.thePlayer.timeInPortal - Minecraft.thePlayer.prevTimeInPortal) * p_175180_1_) > 0.0f) {
        ...
        if (this.mc.playerController.enableEverythingIsScrewedUpMode()) {
            ...
            EventRender2D tupac = new EventRender2D();
            EventManager.call(tupac);
        }
    ...
    private void renderArrayList() {
        if (ModuleManager.getModuleByName("HUD").isEnabled()) {
            boolean index = false;
            ScaledResolution sr2 = new ScaledResolution(this.mc, this.mc.displayWidth, this.mc.displayHeight);
            sr2.getScaledHeight();
            sr2.getScaledWidth();
            final FontRenderer fr2 = this.mc.fontRendererObj;
            int i2 = 0;
            ArrayList<String> display = new ArrayList<String>();
            for (Module m2 : amerikan.modulemanager.getModules()) {
                if (!m2.isEnabled() || !m2.visible || m2.isCategory(Category.GUI)) continue;
                display.add(String.valueOf(m2.getDisplayname()));
            }
            Collections.sort(amerikan.modulemanager.getModules(), new Comparator<Module>(){

                @Override
                public int compare(Module mod1, Module mod2) {
                    if (fr2.getStringWidth(mod1.getDisplayname()) > fr2.getStringWidth(mod2.getDisplayname())) {
                        return -1;
                    }
                    return fr2.getStringWidth(mod1.getDisplayname()) < fr2.getStringWidth(mod2.getDisplayname());
                }
            });
            for (String s2 : display) {
                int x2 = 0;
                int mwidth = sr2.getScaledWidth() - fr2.getStringWidth(s2) - 2;
                int mheight = 10 * i2 + 2;
                fr2.func_175063_a(s2, mwidth, mheight, GuiIngame.rainbowEffekt(1L, 1.0f).hashCode());
                ++i2;
                ++x2;
            }
            display.clear();
    ...
    public void renderName() {
        if (ModuleManager.getModuleByName("HUD").isEnabled()) {
            int index = 0;
            long x2 = 0L;
            Minecraft.getMinecraft().fontRendererObj.drawString(String.valueOf(amerikan.Client_Name) + " " + amerikan.instance.Client_Version, 3, 5, ColorUtils.rainbowEffekt((long)index + x2 * 20000000L, 1.0f).getRGB());
        }
    }
    ...
    private void renderFPS() {
        if (ModuleManager.getModuleByName("Hud").isEnabled()) {
            ...
            Minecraft.getMinecraft().fontRendererObj.drawString("FPS: " + Minecraft.debugFPS, 3, 25, ColorUtils.rainbowEffekt((long)index + x2 * 20000000L, 1.0f).getRGB());
        }
    }
    ...
    private void renderUser() {
        if (ModuleManager.getModuleByName("Hud").isEnabled()) {
            int index = 0;
            long x2 = 0L;
            Minecraft.getMinecraft().fontRendererObj.drawString("User: " + Minecraft.thePlayer.getName(), 3, 15, ColorUtils.rainbowEffekt((long)index + x2 * 20000000L, 1.0f).getRGB());
        }
    }
    ...
    public void func_175176_b(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (var3 > 0) {
            int var9 = 182;
            int var10 = (int)(Minecraft.thePlayer.experience * (float)(var9 + 1));
    ...
    public void func_175185_b(ScaledResolution p_175185_1_) {
        ...
        String var2 = "";
        var2 = this.mc.theWorld.getTotalWorldTime() >= 120500L ? I18n.format("demo.demoExpired", new Object[0]) : I18n.format("demo.remainingTime", StringUtils.ticksToElapsedTime((int)(120500L - this.mc.theWorld.getTotalWorldTime())));
        int var3 = this.func_175179_f().getStringWidth(var2);
        this.func_175179_f().func_175063_a(var2, p_175185_1_.getScaledWidth() - var3 - 10, 5.0f, 16777215);
        this.mc.mcProfiler.endSection();
    }
    ...
    protected boolean func_175183_b() {
        ...
        if (this.mc.playerController.enableEverythingIsScrewedUpMode()) {
            ...
            return this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.mc.theWorld.getTileEntity(var1 = this.mc.objectMouseOver.func_178782_a()) instanceof IInventory;
        }
        return true;
    }
    ...
    private void func_180475_a(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        ArrayList<Score> var5 = Lists.newArrayList(Iterables.filter(var4, new Predicate(){
        ...
        ArrayList<Score> var21 = var5.size() > 15 ? Lists.newArrayList(Iterables.skip(var5, var4.size() - 15)) : var5;
    ...
    private void func_180477_d(ScaledResolution p_180477_1_) {
        if (this.mc.func_175606_aa() instanceof EntityPlayer) {
            ...
            boolean bl2 = var4 = this.field_175191_F > (long)this.updateCounter && (this.field_175191_F - (long)this.updateCounter) / 3L % 2L == 1L;
    ...
    private void renderBossHealth() {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            if (Config.isCustomColors()) {
                int index = 0;
                long x2 = 0L;
                bossTextColor = ColorUtils.rainbowEffekt((long)index + x2 * 20000000L, 1.0f).getRGB();
            }
            this.func_175179_f().func_175063_a(var8, var3 / 2 - this.func_175179_f().getStringWidth(var8) / 2, var7 - 10, bossTextColor);
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }
