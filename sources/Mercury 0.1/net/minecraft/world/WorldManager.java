...
implements IWorldAccess {
    ...
    public void playSound(String soundName, double x2, double y2, double z2, float volume, float pitch) {
        this.mcServer.getConfigurationManager().sendToAllNear(x2, y2, z2, volume > 1.0f ? (double)(16.0f * volume) : 16.0, this.theWorldServer.provider.getDimensionId(), new S29PacketSoundEffect(soundName, x2, y2, z2, volume, pitch));
    }
    ...
    public void playSoundToNearExcept(EntityPlayer except, String soundName, double x2, double y2, double z2, float volume, float pitch) {
        this.mcServer.getConfigurationManager().sendToAllNearExcept(except, x2, y2, z2, volume > 1.0f ? (double)(16.0f * volume) : 16.0, this.theWorldServer.provider.getDimensionId(), new S29PacketSoundEffect(soundName, x2, y2, z2, volume, pitch));
    }
