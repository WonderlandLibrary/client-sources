...
public abstract class SMCLog {
    ...
    extends Formatter {
        ...
        public String format(LogRecord record) {
            ...
            sb2.append("[");
            sb2.append("Shaders").append("]");
            if (record.getLevel() != SMCINFO) {
                sb2.append("[").append(record.getLevel()).append("]");
            }
            sb2.append(" ");
            sb2.append(record.getMessage()).append("\n");
            return sb2.toString();
        }
    }
    ...
    extends Logger {
        SMCLogger(String name) {
            ...
            SMCFormatter formatter = new SMCFormatter();
            ConsoleHandler handler = new ConsoleHandler();
            handler.setFormatter(formatter);
            handler.setLevel(Level.ALL);
            this.addHandler(handler);
            try {
                FileOutputStream e2 = new FileOutputStream("logs/shadersmod.log", false);
                StreamHandler handler1 = new StreamHandler(e2, formatter){
