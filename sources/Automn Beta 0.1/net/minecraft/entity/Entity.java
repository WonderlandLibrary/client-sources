...
import wtf.automn.events.impl.player.EventMoveFlying;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void setAngles(float yaw, float pitch)
    {
        ...
        /**
         * @Author Esound
         * adding option for silent Aim
         */
        if (this instanceof EntityPlayerSP) {
            YawPitchHelper yp = Automn.instance().yawPitchHelper();
            float f = yp.realPitch;
            float f1 = yp.realYaw;
            yp.realYaw = (float)((double)yp.realYaw + (double)yaw * 0.15D);
            yp.realPitch = (float)((double)yp.realPitch - (double)pitch * 0.15D);
            yp.realPitch = MathHelper.clamp_float(yp.realPitch, -90.0F, 90.0F);
            yp.realLastPitch += yp.realPitch - f;
            yp.realLastYaw += yp.realYaw - f1;

            float fyaw = yp.realYaw;
            while (this.rotationYaw - fyaw >= 180.0F) {
                fyaw += 360.0F;
            }
            while (this.rotationYaw - fyaw < -180.0F) {
                fyaw -= 360.0F;
            }
            float flyaw = yp.realLastYaw;
            while (this.rotationYaw - flyaw >= 180.0F) {
                flyaw += 360.0F;
            }
            while (this.rotationYaw - flyaw < -180.0F) {
                flyaw -= 360.0F;
            ...
            this.rotationYaw = fyaw;
            this.rotationPitch = yp.realPitch;
            this.prevRotationPitch = yp.realLastPitch;
            this.prevRotationYaw = flyaw;
    ...
    {
        /**
         * adding EventMove and modificate this method
         */
        EventMoveFlying eventMove = new EventMoveFlying(this.rotationYaw, friction);
