...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    public void renderEntities(final Entity renderViewEntity, final ICamera camera, final float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            if (this.isRenderEntityOutlines()) {
                ...
                this.mc.getFramebuffer().bind(false);
            ...
            for (final RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfosEntities) {
            ...
            for (final RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation1 : this.renderInfos) {
            ...
            synchronized (this.setTileEntities) {
                for (final TileEntity tileentity : this.setTileEntities)
    ...
    private void cleanupDamagedBlocks(final Iterator<DestroyBlockProgress> iteratorIn) {
        while (iteratorIn.hasNext()) {
            ...
            if (this.cloudTickCounter - i > 400) iteratorIn.remove();
        }
    }
    ...
    public void renderSky(final float partialTicks, final int pass) {
        ...
        if (this.mc.theWorld.provider.getDimensionId() == 1) this.renderSkyEnd();
            ...
            if (afloat != null && Config.isSunMoonEnabled()) {
                ...
                for (int l = 0; l <= 16; ++l) {
                    final float f18 = (float) l * (float) Math.PI * 2.0F / 16.0F;
            ...
            if (Config.isMoonTexture()) {
                ...
                final int k = i % 4;
                final int i1 = i / 4 % 2;
                final float f19 = (float) (k + 0) / 4.0F;
                final float f21 = (float) (i1 + 0) / 2.0F;
    ...
    public void renderWorldBorder(final Entity entityIn, final float partialTicks) {
        ...
        if (entityIn.posX >= worldborder.maxX() - d0 || entityIn.posX <= worldborder.minX() + d0 || entityIn.posZ >= worldborder.maxZ() - d0 || entityIn.posZ <= worldborder.minZ() + d0) {
            ...
            d1 = Math.pow(d1, 4.0D);
            final double d2 = entityIn.lastTickPosX + (entityIn.posX - entityIn.lastTickPosX) * (double) partialTicks;
            final double d3 = entityIn.lastTickPosY + (entityIn.posY - entityIn.lastTickPosY) * (double) partialTicks;
            final double d4 = entityIn.lastTickPosZ + (entityIn.posZ - entityIn.lastTickPosZ) * (double) partialTicks;
            GlStateManager.enableBlend();
            GlStateManager.tryBlendFuncSeparate(770, 1, 1, 0);
            this.renderEngine.bindTexture(locationForcefieldPng);
            GlStateManager.depthMask(false);
        ...
        switch (soundID) {
            ...
            case 1018:
                if (this.mc.getRenderViewEntity() != null) {
                    final double d0 = (double) pos.getX() - this.mc.getRenderViewEntity().posX;
                    final double d1 = (double) pos.getY() - this.mc.getRenderViewEntity().posY;
                    final double d2 = (double) pos.getZ() - this.mc.getRenderViewEntity().posZ;
    ...
    public void pauseChunkUpdates() {
        if (this.renderDispatcher != null) this.renderDispatcher.pauseChunkUpdates();
    }

    public void resumeChunkUpdates() {
        if (this.renderDispatcher != null) this.renderDispatcher.resumeChunkUpdates();
    }
