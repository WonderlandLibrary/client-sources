...
public class GuiPlayerTabOverlay extends Gui {
    ...
    public void renderPlayerlist(final int width, final Scoreboard scoreboardIn, final ScoreObjective scoreObjectiveIn) {
        ...
        if (this.header != null) {
            ...
            for (final String s : list1) l1 = Math.max(l1, this.mc.fontRendererObj.getStringWidth(s));
        }

        if (this.footer != null) {
            list2 = this.mc.fontRendererObj.listFormattedStringToWidth(this.footer.getFormattedText(), width - 50);

            for (final String s2 : list2) l1 = Math.max(l1, this.mc.fontRendererObj.getStringWidth(s2));
        }
        ...
        if (list1 != null) {
            ...
            ModuleBlur.drawBlurred(() -> drawRect(width / 2 - finalL / 2 - 1, finalK - 1, width / 2 + finalL / 2 + 1, finalK + finalList.size() * this.mc.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE), false);


            for (final String s3 : list1) {
                final int i2 = this.mc.fontRendererObj.getStringWidth(s3);
        ...
        final int finalL1 = l1;
        final int finalK1 = k1;
        final int finalI = i4;
        ModuleBlur.drawBlurred(() -> drawRect(width / 2 - finalL1 / 2 - 1, finalK1 - 1, width / 2 + finalL1 / 2 + 1, finalK1 + finalI * 9, Integer.MIN_VALUE), false);
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            final int finalJ = j2;
            final int finalJ1 = j2;
            ModuleBlur.drawBlurred(() -> drawRect(finalJ, k2, finalJ1 + i1, k2 + 8, 553648127), false);
            ...
            if (k4 < list.size()) {
                final NetworkPlayerInfo networkplayerinfo1 = (NetworkPlayerInfo) list.get(k4);
        ...
        if (list2 != null) {
            k1 = k1 + i4 * 9 + 1;
            final int finalL2 = l1;
            final int finalK2 = k1;
            final List<String> finalList1 = list2;
            ModuleBlur.drawBlurred(() -> drawRect(width / 2 - finalL2 / 2 - 1, finalK2 - 1, width / 2 + finalL2 / 2 + 1, finalK2 + finalList1.size() * this.mc.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE), false);


            for (final String s4 : list2) {
                final int j5 = this.mc.fontRendererObj.getStringWidth(s4);
    ...
    private void drawScoreboardValues(final ScoreObjective p_175247_1_, final int p_175247_2_, final String p_175247_3_, final int p_175247_4_, final int p_175247_5_, final NetworkPlayerInfo p_175247_6_) {
        ...
        if (p_175247_1_.getRenderType() == IScoreObjectiveCriteria.EnumRenderType.HEARTS) {
            ...
            if (this.lastTimeOpened == p_175247_6_.func_178855_p()) if (i < p_175247_6_.func_178835_l()) {
