...
public class Framebuffer {
    ...
    public void createBindFramebuffer(final int width, final int height) {
        if (!OpenGlHelper.isFramebufferEnabled()) {
            ...
            if (this.framebufferObject >= 0) this.deleteFramebuffer();
    ...
    public void createFramebuffer(final int width, final int height) {
        ...
        if (!OpenGlHelper.isFramebufferEnabled()) this.clear();
        else {
            this.framebufferObject = OpenGlHelper.glGenFramebuffers();
            this.texture = TextureUtil.glGenTextures();

            if (this.useDepth) this.depthBuffer = OpenGlHelper.glGenRenderbuffers();
            ...
            OpenGlHelper.glFramebufferTexture2D(OpenGlHelper.GL_FRAMEBUFFER, OpenGlHelper.GL_COLOR_ATTACHMENT0, 3553, this.texture, 0);
    ...
    public void checkFramebufferComplete() {
        ...
        if (i != OpenGlHelper.GL_FRAMEBUFFER_COMPLETE) if (i == OpenGlHelper.GL_FB_INCOMPLETE_ATTACHMENT)
    ...
    public void bindFramebufferTexture() {
        if (OpenGlHelper.isFramebufferEnabled()) GlStateManager.bindTexture(this.texture);
    }

    public void unbindFramebufferTexture() {
        if (OpenGlHelper.isFramebufferEnabled()) GlStateManager.bindTexture(0);
    }

    public void bind(final boolean p_147610_1_) {
        if (OpenGlHelper.isFramebufferEnabled()) {
            OpenGlHelper.glBindFramebuffer(OpenGlHelper.GL_FRAMEBUFFER, this.framebufferObject);

            if (p_147610_1_) GlStateManager.viewport(0, 0, this.framebufferWidth, this.framebufferHeight);
        }
    }

    public void unbindFramebuffer() {
        if (OpenGlHelper.isFramebufferEnabled()) OpenGlHelper.glBindFramebuffer(OpenGlHelper.GL_FRAMEBUFFER, 0);
    }
    ...
    public void checkSetup(final int displayWidth, final int displayHeight) {
        if (this.framebufferWidth != displayWidth || this.framebufferHeight != displayHeight) {
            this.deleteFramebuffer();
            this.createFramebuffer(displayWidth, displayHeight);
        }
    }
}
