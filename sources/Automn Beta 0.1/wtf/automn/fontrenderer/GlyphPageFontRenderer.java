...
public class GlyphPageFontRenderer implements BasicFontRenderer {
  ...
  public int drawCenteredScaledString(String string, float x, float y, float scale, int color) {
    ...
    i = drawCenteredString(string, x / scale, y / scale, color, true);
  ...
  private void doDraw(float f, GlyphPage glyphPage) {
    if (this.strikethroughStyle) {
      ...
      worldrenderer
        ...
        .endVertex();
      worldrenderer.pos(this.posX + f,
        this.posY + (float) (glyphPage.getMaxFontHeight() / 2), 0.0D).endVertex();
      worldrenderer.pos(this.posX + f,
        this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F, 0.0D).endVertex();
      worldrenderer.pos(this.posX,
        this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F, 0.0D).endVertex();
      tessellator.draw();
      GlStateManager.enableTexture2D();
    }
    ...
    if (this.underlineStyle) {
      ...
      worldrenderer1.pos(this.posX + (float) l,
        this.posY + (float) glyphPage.getMaxFontHeight(), 0.0D).endVertex();
      worldrenderer1
        .pos(this.posX + f, this.posY + (float) glyphPage.getMaxFontHeight(), 0.0D)
        .endVertex();
      worldrenderer1.pos(this.posX + f,
        this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F, 0.0D).endVertex();
      worldrenderer1.pos(this.posX + (float) l,
        this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F, 0.0D).endVertex();
      tessellator1.draw();
      GlStateManager.enableTexture2D();
...
//			return s + "\n" + this.wrapFormattedStringToWidth(s1, wrapWidth);
  ...
  public int drawScaledString(String string, float x, float y, float scale, int color) {
    ...
    GlStateManager.pushMatrix();
    GlStateManager.scale(scale, scale, 0);
    i = drawString(string, x / scale, y / scale, color);
    GlStateManager.popMatrix();
    return i;
  }
