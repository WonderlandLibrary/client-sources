...
import markgg.ui.compLight.CheckBoxLight;
import markgg.ui.compLight.ComboLight;
import markgg.ui.compLight.CompLight;
import markgg.ui.compLight.KeybindLight;
import markgg.ui.compLight.SliderLight;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.Gui;
import net.minecraft.client.gui.GuiScreen;
import net.minecraft.client.gui.ScaledResolution;

public class GUIMethodLight extends GuiScreen {
	...
	public ArrayList<CompLight> comps = new ArrayList<>();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		width = posX + 120 * 2;
		height = posY + 200;
		Gui.drawRect(posX, posY - 15, width, posY, new Color(255,255,255).getRGB());
		Gui.drawRect(posX, posY, width, height, new Color(220,220,220).getRGB());
		fontRendererObj.drawString("ClickGUI", (int)posX + 5, (int)posY - 11, 0xFF000000);
		int offset = 0;
		for (Category category : Category.values()) {
			Gui.drawRect(posX,posY  + offset + 1.7,posX + 60,posY + 15 + offset,category.equals(selectedCategory) ? primaryColor : new Color(255,255,255).getRGB());
			fontRendererObj.drawString(category.name,(int)posX + 5, (int)(posY + 4) + offset + (int)1.7, 0xFF000000);
			offset += 15;
		}
		offset = 0;
		for (Module m : Client.getModulesByCategory(selectedCategory)) {
			Gui.drawRect(posX + 65, posY + offset + 1.7, posX + 125, posY + 15 + offset,m.isEnabled() ? secondaryColor : new Color(255,255,255).getRGB());
			fontRendererObj.drawString(m.getName(),(int)posX + 67, (int)(posY + 4) + offset + (int)1.7, 0xFF000000);
			offset += 15;
		}
		for (CompLight comp : comps) {
					...
					if (Setting.getSettingsByMod(m) != null)
						for (Setting setting : Setting.getSettingsByMod(m)) {
							...
							if (setting instanceof ModeSetting) {
								comps.add(new ComboLight(200, sOffset, this, selectedModule, setting));
								sOffset += 15;
							}
							if (setting instanceof NumberSetting) {
								comps.add(new SliderLight(200, sOffset, this, selectedModule, setting));
								sOffset += 25;
							}
							if (setting instanceof BooleanSetting) {
								comps.add(new CheckBoxLight(200, sOffset, this, selectedModule, setting));
								sOffset += 15;
							} 
							if(setting instanceof KeybindSetting) {
								comps.add(new KeybindLight(200, sOffset, this, selectedModule, setting));
								sOffset += 15;
							}
						}
				}
