...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void func_175161_p()
    {
        ...
        if (this.func_175160_A())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (var13 && var14)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getX(), this.getEntityBoundingBox().minY, event.getZ(), event.getYaw(), event.getPitch(), event.isOnGround()));
                }
                else if (var13)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.getX(), this.getEntityBoundingBox().minY, event.getZ(), event.isOnGround()));
                }
                else if (var14)
                {
    ...
    {
    	//Raze
		List<Module> modules = Client.getAllModules();
