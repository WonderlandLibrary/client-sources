...
@ModuleInfo(name = "Aura", description = "Attacks nearby entities for you", category = Category.COMBAT)
public final class Aura extends Module {
    ...
    private final ModeSetting rotationMode = new ModeSetting("Rotation Mode", this, "Custom", "Custom", "Custom Simple", "Custom Advanced", "Smooth", "Sin Wave", "Up", "Down", "Derp", "None");
    ...
    public void onPacketSend(final PacketSendEvent event) {
        if (blockMode.getMode().equals("Hypixel")) {
        }
    }
...
//        }
        ...
        if (blockMode.getMode().equals("Hypixel")) {
            if (target == null) {
                mc.gameSettings.keyBindUseItem.setKeyPressed(Mouse.isButtonDown(1));
            }
        }
...

        {
            ...
            MovingObjectPosition movingObjectPosition = PlayerUtil.getMouseOver(serverYaw, serverPitch, (float) range.getValue());
    ...
    public void sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        if (!(mc.playerController.currentGameType == WorldSettings.GameType.SPECTATOR) && itemStackIn != null) {
    ...
    private float[] getRotations() {
        ...
        switch (this.rotationMode.getMode()) {
            ...
            case "Down": {
                pitch = (float) MathUtil.getRandom(89, 90);
                break;
            }

            case "Up": {
                pitch = (float) MathUtil.getRandom(-80, -81);
                break;
            }
    ...
    private List<EntityLivingBase> getTargets() {
        final List<EntityLivingBase> entities = mc.theWorld.loadedEntityList
                ...
                .filter(entity -> {
                    // DO NOT TOUCH THIS VALUE IT'S CALCULATED WITH MATH
    ...
    private void drawCircle(final Entity entity, final double rad, final int color, final boolean shade) {
        ...
        for (float i = 0; i < Math.PI * 2; i += Math.PI * 2 / 64.F) {
            ...
            c = ColorUtil.getRainbow(10, (int) Math.toDegrees(i), 0.5F, 0.85F, 1.0F);
