...
public class NahrFont {
    public enum FontType {NORMAL, EMBOSS_TOP, EMBOSS_BOTTOM, OUTLINE_THIN, SHADOW_THICK, SHADOW_THIN}
    ...
    private void drawer(String text, float x, float y, int color) {
        ...
        for (int i = 0; i < text.length(); i++)
            if (text.charAt(i) == '\247' && i + 1 < text.length()) {
                ...
                else if (oneMore == 'r') GlStateManager.color(red, green, blue, alpha);
                else if (oneMore == 'g') GlStateManager.color(0.3F, 0.7F, 1.0F, alpha);
                i++;
            } else {
    ...
    public String wrapFormattedStringToWidth(String s, float width) {
        ...
        if (s.length() <= wrapWidth) return s;
