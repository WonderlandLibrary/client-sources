...
/* 1842 */     float[] key = this.key;
/* 1843 */     short[] value = this.value;
/* 1844 */     int mask = newN - 1;
/* 1845 */     float[] newKey = new float[newN + 1];
/* 1846 */     short[] newValue = new short[newN + 1];
...
/* 1852 */       int pos; if (Float.floatToIntBits(key[i]) == 0) {
/* 1853 */         pos = newN;
/*      */       } else {
/* 1855 */         pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask;
/* 1856 */         while (Float.floatToIntBits(newKey[pos]) != 0)
/* 1857 */           pos = pos + 1 & mask; 
/*      */       } 
...
/*      */     try {
/* 1900 */       c = (Float2ShortLinkedOpenHashMap)super.clone();
...
/* 1908 */     c.key = (float[])this.key.clone();
/* 1909 */     c.value = (short[])this.value.clone();
...
/* 1926 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1927 */         i++; 
/* 1928 */       t = HashCommon.float2int(this.key[i]);
/* 1929 */       t ^= this.value[i];
/* 1930 */       h += t;
/* 1931 */       i++;
/*      */     } 
...
/* 1935 */       h += this.value[this.n]; 
/* 1936 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1939 */     float[] key = this.key;
/* 1940 */     short[] value = this.value;
/* 1941 */     MapIterator i = new MapIterator();
/* 1942 */     s.defaultWriteObject();
/* 1943 */     for (int j = this.size; j-- != 0; ) {
/* 1944 */       int e = i.nextEntry();
/* 1945 */       s.writeFloat(key[e]);
/* 1946 */       s.writeShort(value[e]);
/*      */     } 
/*      */   }
/*      */   
...
/* 1955 */     float[] key = this.key = new float[this.n + 1];
/* 1956 */     short[] value = this.value = new short[this.n + 1];
...
/* 1963 */       int pos; float k = s.readFloat();
/* 1964 */       short v = s.readShort();
/* 1965 */       if (Float.floatToIntBits(k) == 0) {
/* 1966 */         pos = this.n;
/* 1967 */         this.containsNullKey = true;
/*      */       } else {
/* 1969 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1970 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Syz\Downloads\Senura (1).jar!\i\\unimi\dsi\fastutil\floats\Float2ShortLinkedOpenHashMap.class
