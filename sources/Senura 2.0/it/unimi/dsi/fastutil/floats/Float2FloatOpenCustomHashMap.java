...
/* 1178 */     float[] key = this.key;
/* 1179 */     float[] value = this.value;
/* 1180 */     int mask = newN - 1;
/* 1181 */     float[] newKey = new float[newN + 1];
/* 1182 */     float[] newValue = new float[newN + 1];
/* 1183 */     int i = this.n;
/* 1184 */     for (int j = realSize(); j-- != 0; ) {
/* 1185 */       while (Float.floatToIntBits(key[--i]) == 0); int pos;
/* 1186 */       if (Float.floatToIntBits(newKey[
/* 1187 */             pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask]) != 0)
/* 1188 */         while (Float.floatToIntBits(newKey[pos = pos + 1 & mask]) != 0); 
...
/*      */     try {
/* 1214 */       c = (Float2FloatOpenCustomHashMap)super.clone();
...
/* 1222 */     c.key = (float[])this.key.clone();
...
/* 1240 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1241 */         i++; 
...
/* Location:              C:\Users\Syz\Downloads\Senura (1).jar!\i\\unimi\dsi\fastutil\floats\Float2FloatOpenCustomHashMap.class
