...
/*  118 */     long t = array[BigArrays.segment(first)][BigArrays.displacement(first)];
...
/*  168 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = array[BigArrays.segment(index)][BigArrays.displacement(index)] + 1L;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  182 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = array[BigArrays.segment(index)][BigArrays.displacement(index)] - 1L;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  342 */     long[][] base = new long[baseLength][];
/*  343 */     int residual = (int)(length & 0x7FFFFFFL);
/*  344 */     if (residual != 0) {
/*  345 */       for (int i = 0; i < baseLength - 1; i++)
/*  346 */         base[i] = new long[134217728]; 
/*  347 */       base[baseLength - 1] = new long[residual];
/*      */     } else {
/*  349 */       for (int i = 0; i < baseLength; i++)
/*  350 */         base[i] = new long[134217728]; 
/*  351 */     }  return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  371 */       return new long[][] { array }; 
/*  372 */     long[][] bigArray = newBigArray(array.length);
...
/*  430 */     long[][] base = Arrays.<long[]>copyOf(array, baseLength);
/*  431 */     int residual = (int)(length & 0x7FFFFFFL);
/*  432 */     if (residual != 0) {
/*  433 */       for (int i = valid; i < baseLength - 1; i++)
/*  434 */         base[i] = new long[134217728]; 
/*  435 */       base[baseLength - 1] = new long[residual];
/*      */     } else {
/*  437 */       for (int i = valid; i < baseLength; i++)
/*  438 */         base[i] = new long[134217728]; 
...
/*  560 */     long[][] base = Arrays.<long[]>copyOf(array, baseLength);
/*  561 */     int residual = (int)(length & 0x7FFFFFFL);
/*  562 */     if (residual != 0)
/*  563 */       base[baseLength - 1] = LongArrays.trim(base[baseLength - 1], residual); 
/*  564 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  628 */     long[][] base = (long[][])array.clone();
/*  629 */     for (int i = base.length; i-- != 0;)
/*  630 */       base[i] = (long[])array[i].clone(); 
/*  631 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  830 */       return LongBigArrays.equals(a, b);
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/*  936 */     int ab = Long.compare(BigArrays.get(x, a), BigArrays.get(x, b));
/*  937 */     int ac = Long.compare(BigArrays.get(x, a), BigArrays.get(x, c));
/*  938 */     int bc = Long.compare(BigArrays.get(x, b), BigArrays.get(x, c));
...
/* 1005 */       m = med3(x, l, m, n);
/*      */     } 
/* 1007 */     long v = BigArrays.get(x, m);
/*      */     
/* 1009 */     long a = from, b = a, c = to - 1L, d = c;
/*      */     
/*      */     int comparison;
/* 1012 */     while (b <= c && (comparison = Long.compare(BigArrays.get(x, b), v)) <= 0) {
/* 1013 */       if (comparison == 0)
/* 1014 */         swap(x, a++, b); 
/* 1015 */       b++;
/*      */     } 
/* 1017 */     while (c >= b && (comparison = Long.compare(BigArrays.get(x, c), v)) >= 0) {
...
/* 1084 */       long midVal = BigArrays.get(a, mid);
/* 1085 */       if (midVal < key) {
...
/* 1149 */       long midVal = BigArrays.get(a, mid);
...
/* 1248 */     int maxLevel = 7;
/* 1249 */     int stackSize = 1786;
/* 1250 */     long[] offsetStack = new long[1786];
/* 1251 */     int offsetPos = 0;
/* 1252 */     long[] lengthStack = new long[1786];
/* 1253 */     int lengthPos = 0;
/* 1254 */     int[] levelStack = new int[1786];
/* 1255 */     int levelPos = 0;
...
/* 1266 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1267 */       if (length < 40L) {
/* 1268 */         selectionSort(a, first, first + length);
/*      */         continue;
/*      */       } 
/* 1271 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
/* 1276 */       for (i = length; i-- != 0L;)
/* 1277 */         BigArrays.set(digit, i, (byte)(int)(BigArrays.get(a, first + i) >>> shift & 0xFFL ^ signMask)); 
...
/* 1302 */         long t = BigArrays.get(a, l1 + first);
...
/* 1405 */     int maxLevel = 15;
/* 1406 */     int stackSize = 3826;
/* 1407 */     long[] offsetStack = new long[3826];
/* 1408 */     int offsetPos = 0;
/* 1409 */     long[] lengthStack = new long[3826];
/* 1410 */     int lengthPos = 0;
/* 1411 */     int[] levelStack = new int[3826];
/* 1412 */     int levelPos = 0;
...
/* 1423 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1424 */       if (length < 40L) {
/* 1425 */         selectionSort(a, b, first, first + length);
/*      */         continue;
/*      */       } 
/* 1428 */       long[][] k = (level < 8) ? a : b;
/* 1429 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
/* 1434 */       for (i = length; i-- != 0L;)
/* 1435 */         BigArrays.set(digit, i, (byte)(int)(BigArrays.get(k, first + i) >>> shift & 0xFFL ^ signMask)); 
...
/* 1458 */         long t = BigArrays.get(a, l1 + first);
/* 1459 */         long u = BigArrays.get(b, l1 + first);
...
/*      */     } 
/* 1572 */     int layers = 2;
/* 1573 */     int maxLevel = 15;
/* 1574 */     int stackSize = 3826;
/* 1575 */     int stackPos = 0;
/* 1576 */     long[] offsetStack = new long[3826];
/* 1577 */     long[] lengthStack = new long[3826];
/* 1578 */     int[] levelStack = new int[3826];
...
/*      */ 
/*      */     
/* 1586 */     long[][] support = stable ? newBigArray(BigArrays.length(perm)) : null;
...
/* 1591 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1592 */       long[][] k = (level < 8) ? a : b;
/* 1593 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1598 */       for (long i = first + length; i-- != first;) {
/* 1599 */         count[(int)(BigArrays.get(k, BigArrays.get(perm, i)) >>> shift & 0xFFL ^ signMask)] = count[(int)(BigArrays.get(k, BigArrays.get(perm, i)) >>> shift & 0xFFL ^ signMask)] + 1L;
/*      */       }
/* 1601 */       int lastUsed = -1;
...
/* 1611 */                 (int)(BigArrays.get(k, BigArrays.get(perm, l)) >>> shift & 0xFFL ^ signMask)] = pos[(int)(BigArrays.get(k, BigArrays.get(perm, l)) >>> shift & 0xFFL ^ signMask)] - 1L, 
/* 1612 */               BigArrays.get(perm, l)); 
/* 1613 */         BigArrays.copy(support, 0L, perm, first, length);
/* 1614 */         p = first;
/* 1615 */         for (int m = 0; m < 256; m++) {
/* 1616 */           if (level < 15 && count[m] > 1L) {
...
/* 1674 */       long t = BigArrays.get(a, from + i);
...
/* Location:              C:\Users\Syz\Downloads\Senura (1).jar!\i\\unimi\dsi\fastutil\longs\LongBigArrays.class
