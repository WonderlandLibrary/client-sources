...
/*  118 */     short t = array[BigArrays.segment(first)][BigArrays.displacement(first)];
...
/*  137 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] + incr);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  154 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] * factor);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  182 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] - 1);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  342 */     short[][] base = new short[baseLength][];
/*  343 */     int residual = (int)(length & 0x7FFFFFFL);
/*  344 */     if (residual != 0) {
/*  345 */       for (int i = 0; i < baseLength - 1; i++)
/*  346 */         base[i] = new short[134217728]; 
/*  347 */       base[baseLength - 1] = new short[residual];
/*      */     } else {
/*  349 */       for (int i = 0; i < baseLength; i++)
/*  350 */         base[i] = new short[134217728]; 
/*  351 */     }  return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  371 */       return new short[][] { array }; 
/*  372 */     short[][] bigArray = newBigArray(array.length);
...
/*  430 */     short[][] base = Arrays.<short[]>copyOf(array, baseLength);
/*  431 */     int residual = (int)(length & 0x7FFFFFFL);
/*  432 */     if (residual != 0) {
/*  433 */       for (int i = valid; i < baseLength - 1; i++)
/*  434 */         base[i] = new short[134217728]; 
/*  435 */       base[baseLength - 1] = new short[residual];
/*      */     } else {
/*  437 */       for (int i = valid; i < baseLength; i++)
/*  438 */         base[i] = new short[134217728]; 
...
/*  560 */     short[][] base = Arrays.<short[]>copyOf(array, baseLength);
/*  561 */     int residual = (int)(length & 0x7FFFFFFL);
/*  562 */     if (residual != 0)
/*  563 */       base[baseLength - 1] = ShortArrays.trim(base[baseLength - 1], residual); 
/*  564 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  613 */     short[][] a = newBigArray(length);
...
/*  628 */     short[][] base = (short[][])array.clone();
/*  629 */     for (int i = base.length; i-- != 0;)
/*  630 */       base[i] = (short[])array[i].clone(); 
/*  631 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  830 */       return ShortBigArrays.equals(a, b);
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/*  936 */     int ab = Short.compare(BigArrays.get(x, a), BigArrays.get(x, b));
/*  937 */     int ac = Short.compare(BigArrays.get(x, a), BigArrays.get(x, c));
/*  938 */     int bc = Short.compare(BigArrays.get(x, b), BigArrays.get(x, c));
...
/* 1005 */       m = med3(x, l, m, n);
/*      */     } 
/* 1007 */     short v = BigArrays.get(x, m);
/*      */     
/* 1009 */     long a = from, b = a, c = to - 1L, d = c;
/*      */     
/*      */     int comparison;
/* 1012 */     while (b <= c && (comparison = Short.compare(BigArrays.get(x, b), v)) <= 0) {
/* 1013 */       if (comparison == 0)
/* 1014 */         swap(x, a++, b); 
/* 1015 */       b++;
/*      */     } 
/* 1017 */     while (c >= b && (comparison = Short.compare(BigArrays.get(x, c), v)) >= 0) {
...
/* 1084 */       short midVal = BigArrays.get(a, mid);
/* 1085 */       if (midVal < key) {
...
/* 1149 */       short midVal = BigArrays.get(a, mid);
...
/* 1266 */       int signMask = (level % 2 == 0) ? 128 : 0;
/* 1267 */       if (length < 40L) {
...
/* 1302 */         short t = BigArrays.get(a, l1 + first);
...
/* 1322 */           m = j; 
/* 1323 */       }  if (m != i) {
/* 1324 */         short t = BigArrays.get(a, i);
...
/* 1423 */       int signMask = (level % 2 == 0) ? 128 : 0;
/* 1424 */       if (length < 40L) {
/* 1425 */         selectionSort(a, b, first, first + length);
/*      */         continue;
/*      */       } 
/* 1428 */       short[][] k = (level < 2) ? a : b;
...
/* 1458 */         short t = BigArrays.get(a, l1 + first);
/* 1459 */         short u = BigArrays.get(b, l1 + first);
...
/* 1591 */       int signMask = (level % 2 == 0) ? 128 : 0;
/* 1592 */       short[][] k = (level < 2) ? a : b;
...
/* 1599 */         count[BigArrays.get(k, BigArrays.get(perm, i)) >>> shift & 0xFF ^ signMask] = count[BigArrays.get(k, BigArrays.get(perm, i)) >>> shift & 0xFF ^ signMask] + 1L;
/*      */       }
/* 1601 */       int lastUsed = -1;
...
/* 1611 */                 BigArrays.get(k, BigArrays.get(perm, l)) >>> shift & 0xFF ^ signMask] = pos[BigArrays.get(k, BigArrays.get(perm, l)) >>> shift & 0xFF ^ signMask] - 1L, 
...
/* 1674 */       short t = BigArrays.get(a, from + i);
...
/* 1693 */       short t = BigArrays.get(a, i);
...
/* Location:              C:\Users\Syz\Downloads\Senura (1).jar!\i\\unimi\dsi\fastutil\shorts\ShortBigArrays.class
