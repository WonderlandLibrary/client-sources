...
public final class TeleportCommand extends AbstractCommand {
   ...
   public void onUpdate(MotionUpdateEvent event) {
      if (this.gotonigga && !this.packet) {
         ...
         double distancex = -3.0D * Math.sin(storedangles);
         double distancez = 3.0D * Math.cos(storedangles);
         if (this.mc.thePlayer.ticksExisted % 3 == 0) {
            ...
            if (this.mc.thePlayer.getDistanceSq(this.x, this.mc.thePlayer.posY, this.z) >= 32.0D) {
               if (this.niggay) {
      ...
      if (this.packet) {
         if (this.gotonigga) {
            if (event.getPacket() instanceof C03PacketPlayer && !this.niggay) {
               ...
               this.mc.getNetHandler().addToSendQueueSilent(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, true));
