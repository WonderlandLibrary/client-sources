...
public class Lagometer {
			...
			if (gameSettings.ofLagometer) {
				...
				for (y60 = 0; y60 < timesFrame.length; ++y60) {
					...
					if (gcs[y60]) {
						...
						lumMem -= (float) renderTime(y60, timesServer[y60], y30 / 2, y30 / 2, y30 / 2, lumMem,
								tessellator);
						...
						lumMem -= (float) renderTime(y60, timesScheduledExecutables[y60], 0, 0, y30, lumMem,
								tessellator);
						float var10000 = lumMem
								- (float) renderTime(y60, timesTick[y60], 0, y30, y30, lumMem, tessellator);
					}
				}
	...
	private static long renderTime(int frameNum, long time, int r, int g, int b, float baseHeight,
			WorldRenderer tessellator) {
		...
		if (heightTime < 3L) {
			...
			tessellator.addVertex((double) ((float) frameNum + 0.5F), (double) (baseHeight - (float) heightTime + 0.5F),
					0.0D);
	...
	private static long renderTimeDivider(int frameStart, int frameEnd, long time, int r, int g, int b,
			float baseHeight, WorldRenderer tessellator) {
		long heightTime = time / 200000L;
		...
		if (heightTime < 3L) {
			...
			tessellator.func_178961_b(r, g, b, 255);
			tessellator.addVertex((double) ((float) frameStart + 0.5F),
					(double) (baseHeight - (float) heightTime + 0.5F), 0.0D);
			tessellator.addVertex((double) ((float) frameEnd + 0.5F), (double) (baseHeight - (float) heightTime + 0.5F),
					0.0D);
			return heightTime;
		}
	}
