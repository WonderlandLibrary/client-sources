...
public class BetterSnow {
	...
	public static void update() {
		modelSnowLayer = Config.getMinecraft().getBlockRendererDispatcher().func_175023_a()
				.func_178125_b(Blocks.snow_layer.getDefaultState());
	}
	...
	public static boolean shouldRender(IBlockAccess blockAccess, Block block, IBlockState blockState,
			BlockPos blockPos) {
	...
	private static boolean hasSnowNeighbours(IBlockAccess blockAccess, BlockPos pos) {
		...
		return blockAccess.getBlockState(pos.offsetNorth()).getBlock() != blockSnow
				&& blockAccess.getBlockState(pos.offsetSouth()).getBlock() != blockSnow
				&& blockAccess.getBlockState(pos.offsetWest()).getBlock() != blockSnow
				&& blockAccess.getBlockState(pos.offsetEast()).getBlock() != blockSnow ? false
						: blockAccess.getBlockState(pos.offsetDown()).getBlock().isOpaqueCube();
	}
	...
	private static boolean checkBlock(Block block, IBlockState blockState) {
		if (block.isFullCube()) {
			...
			return false;
				...
				&& (block instanceof BlockDoublePlant || block instanceof BlockFlower || block instanceof BlockMushroom
			...
			return true;
				...
				&& !(block instanceof BlockFlowerPot) && !(block instanceof BlockPane) && !(block instanceof BlockReed)
