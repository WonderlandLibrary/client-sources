...
public class CloudRenderer {
	...
	public boolean shouldUpdateGlList() {
		if (!this.updated) {
			...
			boolean belowCloudsPrev = this.cloudPlayerY + (double) rve.getEyeHeight() < 128.0D
					+ (double) (this.mc.gameSettings.ofCloudsHeight * 128.0F);
			boolean belowClouds = rve.prevPosY + (double) rve.getEyeHeight() < 128.0D
					+ (double) (this.mc.gameSettings.ofCloudsHeight * 128.0F);
	...
	public void renderGlList() {
		...
		double exactPlayerX = entityliving.prevPosX
				+ (entityliving.posX - entityliving.prevPosX) * (double) this.partialTicks;
		double exactPlayerY = entityliving.prevPosY
				+ (entityliving.posY - entityliving.prevPosY) * (double) this.partialTicks;
		double exactPlayerZ = entityliving.prevPosZ
				+ (entityliving.posZ - entityliving.prevPosZ) * (double) this.partialTicks;
