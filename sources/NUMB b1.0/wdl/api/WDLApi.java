...
public class WDLApi {
	 ...
	 * 
	 * @param pos
	 *            The position to save at.
	 * @param te
	 *            The TileEntity to save.
	 */
	public static void saveTileEntity(BlockPos pos, TileEntity te) {
		if (!WDLPluginChannels.canSaveTileEntities(pos.getX() << 16, pos.getZ() << 16)) {
			logger.warn("API attempted to call saveTileEntity when " + "saving TileEntities is not allowed!  Pos: "
					+ pos + ", te: " + te + ".  StackTrace: ");
			logStackTrace();
	...
	public static void addWDLMod(IWDLMod mod) {
		...
		if (!WDL.VERSION.equals(mod.getTargetedWDLVersion())) {
			throw new IllegalArgumentException("mod " + modName + " targets WDL " + mod.getTargetedWDLVersion()
					+ ", but you are running " + WDL.VERSION + "!");
		}
		if (wdlMods.containsKey(modName)) {
			throw new IllegalArgumentException("A mod by the name of '" + modName + "' is already registered by "
					+ wdlMods.get(modName) + " (tried to register " + mod + " over it)");
		}
	...
	public static String getModInfo(IWDLMod mod) {
		...
		try {
			...
			String path = new File(mod.getClass().getProtectionDomain().getCodeSource().getLocation().toURI())
					.getPath();
		...
		for (int i = 0; i < interfaces.length; i++) {
			info.append(i).append(": ").append(interfaces[i].getName()).append('\n');
		}
		info.append("Superclass: ").append(mod.getClass().getSuperclass().getName()).append('\n');
		...
		for (int i = 0; i < annotations.length; i++) {
			info.append(i).append(": ").append(annotations[i].toString()).append(" (")
					.append(annotations[i].annotationType().getName()).append(")\n");
		}
