...
public class WDLHooks {
	 ...
	 */
	public static Map<String, IGuiHooksListener> guiListeners = new HashMap<String, IGuiHooksListener>();
	/**
	 * All WDLMods that implement {@link IChatMessageListener}.
	 */
	public static Map<String, IChatMessageListener> chatMessageListeners = new HashMap<String, IChatMessageListener>();
	/**
	 * All WDLMods that implement {@link IPluginChannelListener}.
	 */
	public static Map<String, IPluginChannelListener> pluginChannelListeners = new HashMap<String, IPluginChannelListener>();
	/**
	 * All WDLMods that implement {@link IBlockEventListener}.
	 */
	public static Map<String, IBlockEventListener> blockEventListeners = new HashMap<String, IBlockEventListener>();
					...
					if (WDL.thePlayer.openContainer != WDL.windowContainer) {
						if (WDL.thePlayer.openContainer == WDL.thePlayer.inventoryContainer) {
							...
							if (WDL.lastEntity != null) {
								...
								for (Map.Entry<String, IGuiHooksListener> e : guiListeners.entrySet()) {
									if (handled) {
										break;
									}

									profiler.startSection(e.getKey());
									handled = e.getValue().onEntityGuiClosed(sender, entity, container);
									profiler.endSection();
								}
								...
								for (Map.Entry<String, IGuiHooksListener> e : guiListeners.entrySet()) {
									...
									profiler.startSection(e.getKey());
									handled = e.getValue().onBlockGuiClosed(sender, pos, container);
									profiler.endSection();
								}
	...
	/**
	 * Called when {@link NetHandlerPlayClient#handleChat(S02PacketChat)} is called.
	 * <br/>
	...
	public static void onNHPCHandleChat(NetHandlerPlayClient sender, S02PacketChat packet) {
		try {
			...
			for (Map.Entry<String, IChatMessageListener> e : chatMessageListeners.entrySet()) {
				profiler.startSection(e.getKey());
	...
	/**
	 * Called when {@link NetHandlerPlayClient#handleMaps(S34PacketMaps)} is called.
	 * <br/>
	 * Should be at the end of the method.
	 */
	...
	/**
	 * Called when
	 * {@link NetHandlerPlayClient#handleCustomPayload(S3FPacketCustomPayload)} is
	 * called. <br/>
	 * Should be at the end of the method.
	 */
	public static void onNHPCHandleCustomPayload(NetHandlerPlayClient sender, S3FPacketCustomPayload packet) {
		try {
			...
			for (Map.Entry<String, IPluginChannelListener> e : pluginChannelListeners.entrySet()) {
				profiler.startSection(e.getKey());
				e.getValue().onPluginChannelPacket(WDL.worldClient, channel, payload);
				profiler.endSection();
			}
	...
	public static void onNHPCHandleBlockAction(NetHandlerPlayClient sender, S24PacketBlockAction packet) {
		try {
			...
			for (Map.Entry<String, IBlockEventListener> e : blockEventListeners.entrySet()) {
				profiler.startSection(e.getKey());
				e.getValue().onBlockEvent(WDL.worldClient, pos, block, data1, data2);
				profiler.endSection();
			}
