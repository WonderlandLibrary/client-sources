...
public class GuiWDLPermissions extends GuiScreen {
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(100, width / 2 - 100, height - 29, "Done"));

		this.buttonList.add(new GuiButton(200, this.width / 2 - 155, 39, 100, 20, "Current perms"));
		if (WDLPluginChannels.canRequestPermissions()) {
			this.buttonList.add(new GuiButton(201, this.width / 2 - 50, 39, 100, 20, "Request perms"));
			this.buttonList.add(new GuiButton(202, this.width / 2 + 55, 39, 100, 20, "Chunk Overrides"));
		}

		reloadButton = new GuiButton(1, (this.width / 2) + 5, 18, 150, 20, "Reload permissions");
		...
		list.addLine("Can download: " + WDLPluginChannels.canDownloadInGeneral());
		...
		list.addLine("Can save entities: " + WDLPluginChannels.canSaveEntities());
		list.addLine("Can save tile entities: " + WDLPluginChannels.canSaveTileEntities());
		list.addLine("Can save containers: " + WDLPluginChannels.canSaveContainers());
		list.addLine("Received entity ranges: " + WDLPluginChannels.hasServerEntityRange() + " ("
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id == 1) {
			...
			try {
				...
				WDLMessages.chatMessageTranslated(WDLMessageTypes.ERROR, "wdl.messages.generalError.noUTF8");
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, "Permission info", this.width / 2, 8, 0xFFFFFF);

		if (!WDLPluginChannels.hasPermissions()) {
			this.drawCenteredString(this.fontRendererObj, "No permissions received; defaulting to everything enabled.",
					this.width / 2, (this.height - 32 - 23) / 2 + 23 - fontRendererObj.FONT_HEIGHT / 2, 0xFFFFFF);
		}
