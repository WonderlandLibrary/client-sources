 ...
 * 
 * It's composed of two halves, one that lists enabled extensions that can be
 * clicked, and the other that shows the details on the selected extension. The
 * two halves can be dragged up and down (which is why the logic is so complex
 * here; {@link GuiListExtended} was not designed for that).
 * 
 * @author Pokechu22
 */
public class GuiWDLExtensions extends GuiScreen {
	...
	private class ModList extends GuiListExtended {
		public ModList() {
			super(GuiWDLExtensions.this.mc, GuiWDLExtensions.this.width, bottomLocation, TOP_HEIGHT, bottomLocation,
					22);
		...
		private class ModEntry implements IGuiListEntry {
			...
			public ModEntry(IWDLMod mod) {
				...
				if (mod instanceof IWDLModDescripted) {
					String displayName = ((IWDLModDescripted) mod).getDisplayName();

					if (displayName != null && !displayName.isEmpty()) {
						name = displayName;
					}
				}
				this.modDesc = I18n.format("wdl.gui.extensions.modVersion", name, mod.getVersion());
				...
				if (mod instanceof IWDLModWithGui) {
					...
					button = new GuiButton(0, 0, 0, 80, 20, ((IWDLModWithGui) mod).getButtonName());
				}
			}
			...
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
				...
				int centerY = y + slotHeight / 2 - fontRendererObj.FONT_HEIGHT / 2;
	...
	private class ModDetailList extends TextList {
		public ModDetailList() {
			super(GuiWDLExtensions.this.mc, GuiWDLExtensions.this.width, GuiWDLExtensions.this.height - bottomLocation,
		...
		public void drawScreen(int mouseX, int mouseY, float partialTicks) {
			...
			drawCenteredString(fontRendererObj, I18n.format("wdl.gui.extensions.detailsCaption"),
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(0, width / 2 - 100, height - 29, I18n.format("gui.done")));
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, I18n.format("wdl.gui.extensions.title"), this.width / 2, 8,
				0xFFFFFF);
