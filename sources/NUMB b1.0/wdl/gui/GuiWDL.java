...
public class GuiWDL extends GuiScreen {
	...
	private class GuiWDLButtonList extends GuiListExtended {
		...
		private class ButtonEntry implements IGuiListEntry {
			 ...
			 * 
			 * @param key
			 *            The I18n key, which will have the base for this GUI prepended.
			 * @param toOpen
			 *            The gui screen to open when the button is clicked.
			 * @param needsPerms
			 *            Whether the player needs download permission to use this button.
			 */
			public ButtonEntry(String key, GuiScreen toOpen, boolean needsPerms) {
				this.button = new GuiButton(0, 0, 0, I18n.format("wdl.gui.wdl." + key + ".name"));
				this.toOpen = toOpen;
				if (needsPerms) {
		...
		private List<IGuiListEntry> entries = new ArrayList<IGuiListEntry>() {
			{
				...
				add(new ButtonEntry("worldOverrides", new GuiWDLWorld(GuiWDL.this), true));
				add(new ButtonEntry("generatorOverrides", new GuiWDLGenerator(GuiWDL.this), true));
				add(new ButtonEntry("playerOverrides", new GuiWDLPlayer(GuiWDL.this), true));
				add(new ButtonEntry("entityOptions", new GuiWDLEntities(GuiWDL.this), true));
				add(new ButtonEntry("backupOptions", new GuiWDLBackup(GuiWDL.this), true));
				add(new ButtonEntry("messageOptions", new GuiWDLMessages(GuiWDL.this), false));
				add(new ButtonEntry("permissionsInfo", new GuiWDLPermissions(GuiWDL.this), false));
				add(new ButtonEntry("about", new GuiWDLAbout(GuiWDL.this), false));
				if (WDLUpdateChecker.hasNewVersion()) {
					add(0, new ButtonEntry("updates.hasNew", new GuiWDLUpdates(GuiWDL.this), false));
				} else {
	...
	public void initGui() {
		if (WDL.isMultiworld && WDL.worldName.isEmpty()) {
			this.mc.displayGuiScreen(
					new GuiWDLMultiworldSelect(I18n.format("wdl.gui.multiworldSelect.title.changeOptions"),
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawString(this.fontRendererObj, name,
				this.worldname.xPosition - this.fontRendererObj.getStringWidth(name + " "), 26, 0xFFFFFF);
