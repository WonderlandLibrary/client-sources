...
public class GuiWDLMessages extends GuiScreen {
	...
	private class GuiMessageTypeList extends GuiListExtended {
		public GuiMessageTypeList() {
			super(GuiWDLMessages.this.mc, GuiWDLMessages.this.width, GuiWDLMessages.this.height, 39,
		...
		private class CategoryEntry implements IGuiListEntry {
			...
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
					boolean isSelected) {
				drawCenteredString(fontRendererObj, category.getDisplayName(), GuiWDLMessages.this.width / 2 - 40,
						y + slotHeight - mc.fontRendererObj.FONT_HEIGHT - 1, 0xFFFFFF);

				button.xPosition = GuiWDLMessages.this.width / 2 + 20;
				button.yPosition = y;

				button.displayString = I18n.format("wdl.gui.messages.group." + WDLMessages.isGroupEnabled(category));
		...
		private class MessageTypeEntry implements IGuiListEntry {
			...
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
				...
				button.displayString = I18n.format("wdl.gui.messages.message." + WDLMessages.isEnabled(type),
						type.getDisplayName());
		...
		private List<IGuiListEntry> entries = new ArrayList<IGuiListEntry>() {
			{
				...
				for (Map.Entry<MessageTypeCategory, Collection<IWDLMessageType>> e : map.entrySet()) {
	...
	public void initGui() {
		...
		resetButton = new GuiButton(101, (this.width / 2) + 5, 18, 150, 20, I18n.format("wdl.gui.messages.reset"));
		this.buttonList.add(resetButton);

		this.list = new GuiMessageTypeList();

		this.buttonList.add(new GuiButton(102, (this.width / 2) - 100, this.height - 29, I18n.format("gui.done")));
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 100) {
			...
			this.mc.displayGuiScreen(new GuiYesNo(this, I18n.format("wdl.gui.messages.reset.confirm.title"),
					I18n.format("wdl.gui.messages.reset.confirm.subtitle"), 101));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, I18n.format("wdl.gui.messages.message.title"), this.width / 2, 8,
				0xFFFFFF);
		...
		if (hoveredButtonDescription != null) {
			...
			Utils.drawGuiInfoBox(I18n.format("wdl.gui.messages.all.description"), width, height, 48);
		} else if (resetButton.isMouseOver()) {
			Utils.drawGuiInfoBox(I18n.format("wdl.gui.messages.reset.description"), width, height, 48);
		}
	}
	...
	private String getAllEnabledText() {
		return I18n.format("wdl.gui.messages.all." + WDLMessages.enableAllMessages);
	}
}
