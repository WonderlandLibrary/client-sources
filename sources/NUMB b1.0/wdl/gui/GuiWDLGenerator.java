...
public class GuiWDLGenerator extends GuiScreen {
	...
	public void initGui() {
		...
		this.seedField = new GuiTextField(40, this.fontRendererObj, this.width / 2 - (100 - seedWidth), y,
				200 - seedWidth, 18);
		this.seedField.setText(WDL.worldProps.getProperty("RandomSeed"));
		y += 22;
		this.generatorBtn = new GuiButton(1, this.width / 2 - 100, y, getGeneratorText());
		this.buttonList.add(this.generatorBtn);
		y += 22;
		this.generateStructuresBtn = new GuiButton(2, this.width / 2 - 100, y, getGenerateStructuresText());
		...
		this.buttonList.add(new GuiButton(100, this.width / 2 - 100, height - 29, I18n.format("gui.done")));
	}
		...
		if (button.enabled) {
			if (button.id == 1) {
				...
				if (WDL.worldProps.getProperty("MapGenerator", "").equals("flat")) {
					...
					this.mc.displayGuiScreen(new GuiCustomizeWorldScreen(new GuiCreateWorldProxy(),
							WDL.worldProps.getProperty("GeneratorOptions", "")));
				}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawString(this.fontRendererObj, seedText, this.width / 2 - 100, this.height / 4 - 10, 0xFFFFFF);
	...
	/**
	 * Updates whether the {@link #settingsPageBtn} is shown or hidden, and the text
	 * on it.
	 */
	...
	private String getGeneratorText() {
		return I18n.format("wdl.gui.generator.generator." + WDL.worldProps.getProperty("MapGenerator"));
	}

	private String getGenerateStructuresText() {
		return I18n.format("wdl.gui.generator.generateStructures." + WDL.worldProps.getProperty("MapFeatures"));
	}

	/**
	 * Fake implementation of {@link GuiCreateFlatWorld} that allows use of
	 * {@link GuiFlatPresets}. Doesn't actually do anything; just passed in to the
	 * constructor to forward the information we need and to switch back to the main
	 * GUI afterwards.
	 */
	 ...
	 * to the constructor to forward the information we need and to switch back to
	 * the main GUI afterwards.
	 */
