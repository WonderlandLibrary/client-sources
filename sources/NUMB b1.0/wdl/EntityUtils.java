...
public class EntityUtils {
	 ...
	 */
	public static final Multimap<String, String> entitiesByGroup = HashMultimap.<String, String>create();
	...
	static {
		try {
			...
			for (Map.Entry<String, Class<?>> e : EntityUtils.stringToClassMapping.entrySet()) {
			...
			Minecraft.getMinecraft().crashed(new CrashReport("WDL Mod: failed to set up entity ranges!", e));
			// Will never happen.
	 ...
	 */
	private static final List<ISpecialEntityHandler> specialEntityHandlers = new ArrayList<ISpecialEntityHandler>();
	/**
	 * List of all entities that have special variants and the mods that handle said
	 * special variants.
	 */
	private static final Multimap<String, ISpecialEntityHandler> extendedEntities = HashMultimap
			.<String, ISpecialEntityHandler>create();
	 ...
	 * @see IEntityAdder
	 */
	private static final List<IEntityAdder> entityAdders = new ArrayList<IEntityAdder>();
	/**
	 * New entities and the mods that handle them.
	 */
	private static final Map<String, IEntityAdder> addedEntities = new HashMap<String, IEntityAdder>();
	...
	public static void addSpecialEntityHandler(ISpecialEntityHandler handler) {
		...
		for (Map.Entry<String, String> e : handler.getSpecialEntities().entries()) {
			WDL.defaultProps.setProperty("Entity." + e.getValue() + ".Enabled", "true");
			int trackDistance = handler.getSpecialEntityTrackDistance(e.getValue());
			if (trackDistance < 0) {
				trackDistance = getDefaultEntityRange(e.getKey());
			}
			WDL.defaultProps.setProperty("Entity." + e.getValue() + ".TrackDistance", Integer.toString(trackDistance));

			String group = handler.getSpecialEntityCategory(e.getValue());
			entitiesByGroup.put(group, e.getValue());

			WDL.defaultProps.setProperty("EntityGroup." + group + ".Enabled", "true");
		}
	}
	...
	public static void addEntityAdder(IEntityAdder adder) {
		...
		for (String s : adder.getModEntities()) {
			...
			WDL.defaultProps.setProperty("Entity." + s + ".Enabled", "true");
			int trackDistance = adder.getDefaultEntityTrackDistance(s);
			WDL.defaultProps.setProperty("Entity." + s + ".TrackDistance", Integer.toString(trackDistance));
	...
	/**
	 * Gets the entity tracking range used by vanilla minecraft. <br/>
	 * Proper tracking ranges can be found in EntityTracker#trackEntity (the one
	 * that takes an Entity as a paremeter) -- it's the 2nd arg given to
	 * addEntityToTracker.
	 * 
	 * @param type
	 *            The vanilla minecraft entity string.
	 * @return
	 */
	...
	public static int getEntityTrackDistance(Entity e) {
		return getEntityTrackDistance(WDL.worldProps.getProperty("Entity.TrackDistanceMode"), e);
	}
	...
	public static int getEntityTrackDistance(String mode, Entity e) {
		if ("default".equals(mode)) {
			...
			int serverDistance = WDLPluginChannels.getEntityRange(getEntityType(e));
			...
			String prop = WDL.worldProps.getProperty("Entity." + getEntityType(e) + ".TrackDistance", "-1");
	...
	public static int getEntityTrackDistance(String type) {
		return getEntityTrackDistance(WDL.worldProps.getProperty("Entity.TrackDistanceMode"), type);
	}
	...
	public static int getEntityTrackDistance(String mode, String type) {
		if ("default".equals(mode)) {
			...
			int serverDistance = WDLPluginChannels.getEntityRange(mode);
	 ...
	 * 
	 * @param type
	 *            The type of the entity (from {@link #getEntityType(Entity)})
	 * @return
	 */
	public static boolean isEntityEnabled(String type) {
		boolean groupEnabled = WDL.worldProps.getProperty("EntityGroup." + getEntityGroup(type) + ".Enabled", "true")
				.equals("true");
	...
	/**
	 * Gets the track distance for the given entity, making a guess about whether to
	 * use spigot track distances based off of the server brand.
	 */
	...
	public static int getVanillaEntityRange(Class<?> c) {
		...
		if (EntityFishHook.class.isAssignableFrom(c) || EntityArrow.class.isAssignableFrom(c)
				|| EntitySmallFireball.class.isAssignableFrom(c) || EntityFireball.class.isAssignableFrom(c)
				|| EntitySnowball.class.isAssignableFrom(c) || EntityEnderPearl.class.isAssignableFrom(c)
				|| EntityEnderEye.class.isAssignableFrom(c) || EntityEgg.class.isAssignableFrom(c)
				|| EntityPotion.class.isAssignableFrom(c) || EntityExpBottle.class.isAssignableFrom(c)
				|| EntityFireworkRocket.class.isAssignableFrom(c) || EntityItem.class.isAssignableFrom(c)
				|| EntitySquid.class.isAssignableFrom(c)) {
			return 64;
		} else if (EntityMinecart.class.isAssignableFrom(c) || EntityBoat.class.isAssignableFrom(c)
				|| EntityWither.class.isAssignableFrom(c) || EntityBat.class.isAssignableFrom(c)
				|| IAnimals.class.isAssignableFrom(c)) {
			return 80;
		} else if (EntityDragon.class.isAssignableFrom(c) || EntityTNTPrimed.class.isAssignableFrom(c)
				|| EntityFallingBlock.class.isAssignableFrom(c) || EntityHanging.class.isAssignableFrom(c)
				|| EntityArmorStand.class.isAssignableFrom(c) || EntityXPOrb.class.isAssignableFrom(c)) {
			return 160;
	...
	/**
	 * Gets the entity range used by Spigot by default. Mostly a utility method for
	 * presets.
	 * 
	 * @param entity
	 * @return
	...
	public static int getDefaultSpigotEntityRange(Class<?> c) {
		...
		if (EntityMob.class.isAssignableFrom(c) || EntitySlime.class.isAssignableFrom(c)) {
			return monsterRange;
		} else if (EntityCreature.class.isAssignableFrom(c) || EntityAmbientCreature.class.isAssignableFrom(c)) {
			return animalRange;
		} else if (EntityItemFrame.class.isAssignableFrom(c) || EntityPainting.class.isAssignableFrom(c)
				|| EntityItem.class.isAssignableFrom(c) || EntityXPOrb.class.isAssignableFrom(c)) {
			return miscRange;
		} else {
			return otherRange;
		}
	 ...
	 * 
	 * If it isn't safe, it may cause a crash or another issue when attempting to
	 * save it.
	 * 
	 * TODO: This is basically a bandaid for a base issue with broken entities.
	 * Solve that if possible.
	 * 
	 * @return <code>null</code> if it is safe to save, or a message explaining why
	 *         the entity is NOT safe to save if so.
	 */
	public static IChatComponent isUnsafeToSaveEntity(Entity e) {
		try {
			e.getCustomNameTag();
		} catch (ClassCastException ex) {
			return new ChatComponentTranslation("wdl.unsafeReasons.badCustomName", ex.toString());
		}
