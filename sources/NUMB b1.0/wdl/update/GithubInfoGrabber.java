...
public class GithubInfoGrabber {
	 ...
	 */
	private static final String RELEASE_LIST_LOCATION = "https://api.github.com/repos/" + WDL.GITHUB_REPO
			+ "/releases?per_page=100";
	/**
	 * File for the release cache.
	 */
	private static final File CACHED_RELEASES_FILE = new File(Minecraft.getMinecraft().mcDataDir,
	...
	static {
		...
		USER_AGENT = String.format("WorldDownloader mod by Pokechu22 " + "(Minecraft %s; WDL %s) ", mcVersion,
				wdlVersion);
	}
	...
	public static JsonElement query(String path) throws Exception {
		...
		try {
			...
			if (connection.getResponseCode() == HttpsURLConnection.HTTP_NOT_MODIFIED) {
				...
				throw new Exception("Unexpected response while getting " + path + ": " + connection.getResponseCode()
