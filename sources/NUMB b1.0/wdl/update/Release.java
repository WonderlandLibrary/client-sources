...
/**
 * An individual GitHub release. <br/>
...
public class Release {
	 ...
	 * 
	 * <pre>
	 * [](# '{"someKey":"someValue"}')MainBodyText
	 * </pre>
	 * 
	 * , which visually only appears as "MainBodyText" since the link is 0
	 * characters long. (The quotes are normally used to make tooltips.)
	 */
	...
	public class HiddenInfo {
		...
		/**
		 * Additional versions that can be connected to, but aren't used by default.
		 */
		...
		/**
		 * Post announcing this new version (EG on the minecraftforums). May be null.
		 */
		...
		public String toString() {
			return "HiddenInfo [mainMinecraftVersion=" + mainMinecraftVersion + ", supportedMinecraftVersions="
					+ Arrays.toString(supportedMinecraftVersions) + ", loader=" + loader + ", post=" + post
	...
	public class HashData {
		...
		/**
		 * Valid values for the hash to be. Should be in all caps hexadecimal. This is
		 * an array because there is sometimes a case where there are two legal values
		 * &ndash; for instance, 1.8 had two editions, one with debug enabled and one
		 * with debug disabled (now replaced with the Messages gui).
		 */
		...
		@Override
		public String toString() {
			return "HashData [relativeTo=" + relativeTo + ", file=" + file + ", validHashes="
					+ Arrays.toString(validHashes) + "]";
		}
	...
	public Release(JsonObject object) {
		...
		if (hiddenJSONMatcher.find()) {
			...
			String hiddenJSONStr = markdownBody.substring(hiddenJSONMatcher.start(1), hiddenJSONMatcher.end(1));
	...
	@Override
	public String toString() {
		return "Release [URL=" + URL + ", tag=" + tag + ", title=" + title + ", date=" + date + ", prerelease="
				+ prerelease + ", markdownBody=" + markdownBody + ", textOnlyBody=" + textOnlyBody + ", hiddenInfo="
				+ hiddenInfo + "]";
	}
}
