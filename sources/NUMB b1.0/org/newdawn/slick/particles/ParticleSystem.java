...
public class ParticleSystem {
	...
	private class ParticlePool {
		...
		/**
		 * The list of particles left to be used, if this size() == 0 then the particle
		 * engine was too small for the effect
		 */
	...
	/**
	 * A map from emitter to a the particle pool holding the particles it uses void:
	 * this is now sorted by emitters to allow emitter specfic state to be set for
	 * each emitter. actually this is used to allow setting an individual blend mode
	 * for each emitter
	 */
	 ...
	 * 
	 * @param x
	 *            The x coordinate to render the particle system at (in the current
	 *            coordinate space)
	 * @param y
	 *            The y coordinate to render the particle system at (in the current
	 *            coordiante space)
	 */
	public void render(float x, float y) {
		...
		for (int emitterIdx = 0; emitterIdx < emitters.size(); emitterIdx++) {
			...
			// now get the particle pool for this emitter and render all particles that are
			// in use
	...
	/**
	 * Create a duplicate of this system. This would have been nicer as a different
	 * interface but may cause to much API change headache. Maybe next full version
	 * release it should be rethought.
	 * 
	 * TODO: Consider refactor at next point release
	 * 
	 * @return A copy of this particle system
	 * @throws SlickException
	 *             Indicates a failure during copy or a invalid particle system to
	 *             be duplicated
	 */
