...
public abstract class GameContainer implements GUIContext {
	...
	/**
	 * Indicate if we want to smooth deltas. This feature will report a delta based
	 * on the FPS not the time passed. This works well with vsync.
	 * 
	 * @param smoothDeltas
	...
	/**
	 * Enable shared OpenGL context. After calling this all containers created will
	 * shared a single parent context
	 * 
	 * @throws SlickException
	...
	/**
	 * Indicates if the game should be paused, i.e. if updates should be propogated
	 * through to the game.
	 * 
	 * @param paused
	 ...
	 * 
	 * Windows window icon must be 16x16 Windows alt-tab icon must be 24x24 or 32x32
	 * depending on Windows version (XP=32)
	 * 
	 * @param refs
	 *            The reference to the icon to be displayed
	 * @throws SlickException
	...
	/**
	 * Set the mouse cursor based on the contents of the image. Note that this will
	 * not take account of render state type changes to images (rotation and such).
	 * If these effects are required it is recommended that an offscreen buffer be
	 * used to produce an appropriate image. An offscreen buffer will always be used
	 * to produce the new cursor and as such this operation an be very expensive
	 * 
	 * @param image
	 ...
	 * 
	 * @throws SlickException
	 *             Indicates a failure to load the image or a failure to create the
	 *             hardware cursor
	 */
	...
	/**
	 * Indicate whether the display should be synced to the vertical refresh (stops
	 * tearing)
	 * 
	 * @param vsync
