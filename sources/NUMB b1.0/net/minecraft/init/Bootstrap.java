	...
	static void registerDispenserBehaviors() {
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Items.spawn_egg, new BehaviorDefaultDispenseItem() {
			...
			public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				Entity var10 = ItemMonsterPlacer.spawnCreature(source.getWorld(), stack.getMetadata(), var4, var6,
						var8);
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Items.boat, new BehaviorDefaultDispenseItem() {
			...
			public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				if (Material.water.equals(var12)) {
					...
					if (!Material.air.equals(var12) || !Material.water
							.equals(var4.getBlockState(var11.offsetDown()).getBlock().getMaterial())) {
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Items.bucket, new BehaviorDefaultDispenseItem() {
			...
			public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				if (Material.water.equals(var7) && var6 instanceof BlockLiquid
						&& ((Integer) var5.getValue(BlockLiquid.LEVEL)).intValue() == 0) {
					var8 = Items.water_bucket;
				} else {
					if (!Material.lava.equals(var7) || !(var6 instanceof BlockLiquid)
							|| ((Integer) var5.getValue(BlockLiquid.LEVEL)).intValue() != 0) {
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Items.flint_and_steel, new BehaviorDefaultDispenseItem() {
			...
			protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				if (var3.isAirBlock(var4)) {
					...
					Blocks.tnt.onBlockDestroyedByPlayer(var3, var4,
							Blocks.tnt.getDefaultState().withProperty(BlockTNT.field_176246_a, Boolean.valueOf(true)));
					var3.setBlockToAir(var4);
				} else {
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Item.getItemFromBlock(Blocks.tnt),
				new BehaviorDefaultDispenseItem() {
					...
					protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
						World var3 = source.getWorld();
						BlockPos var4 = source.getBlockPos()
								.offset(BlockDispenser.getFacing(source.getBlockMetadata()));
						EntityTNTPrimed var5 = new EntityTNTPrimed(var3, (double) var4.getX() + 0.5D,
								(double) var4.getY(), (double) var4.getZ() + 0.5D, (EntityLivingBase) null);
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Items.skull, new BehaviorDefaultDispenseItem() {
			...
			protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				if (var3.isAirBlock(var5) && var6.func_176415_b(var3, var5, stack)) {
					if (!var3.isRemote) {
						var3.setBlockState(var5,
								var6.getDefaultState().withProperty(BlockSkull.field_176418_a, EnumFacing.UP), 3);
		...
		BlockDispenser.dispenseBehaviorRegistry.putObject(Item.getItemFromBlock(Blocks.command_block),
				new BehaviorDefaultDispenseItem() {
					...
					protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
						...
						if (var3.isAirBlock(var4)) {
							if (!var3.isRemote) {
								IBlockState var5 = Blocks.command_block.getDefaultState()
										.withProperty(BlockCommandBlock.TRIGGERED_PROP, Boolean.valueOf(false));
