...
public class GuiContainerCreative extends InventoryEffectRenderer {
				...
				if (selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()) {
					...
					if (Container.getDragEvent(clickedButton) == 2) {
						for (var10 = 0; var10 < 9; ++var10) {
							this.mc.playerController.sendSlotPacket(this.inventorySlots.getSlot(45 + var10).getStack(),
									36 + var10);
						}
					} else if (slotIn != null) {
	...
	protected void renderToolTip(ItemStack itemIn, int x, int y) {
		if (selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex()) {
			...
			if (var5 == null && itemIn.getItem() == Items.enchanted_book) {
				...
				if (var6.size() == 1) {
					Enchantment var7 = Enchantment
							.func_180306_c(((Integer) var6.keySet().iterator().next()).intValue());
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if (var4.shouldHidePlayerInventory()) {
			this.drawTexturedModalRect(var9, var6 + (int) ((float) (var7 - var6 - 17) * this.currentScroll),
