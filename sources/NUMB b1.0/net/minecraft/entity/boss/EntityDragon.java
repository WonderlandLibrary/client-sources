...
public class EntityDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart, IMob {
	...
	public void onLivingUpdate() {
		...
		if (this.getHealth() <= 0.0F) {
			...
			this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double) var1,
					this.posY + 2.0D + (double) var2, this.posZ + (double) var3, 0.0D, 0.0D, 0.0D, new int[0]);
		} else {
			...
			if (this.worldObj.isRemote) {
				...
				Vec3 var15 = (new Vec3(this.targetX - this.posX, this.targetY - this.posY, this.targetZ - this.posZ))
						.normalize();
				var16 = (double) (-MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F));
				Vec3 var18 = (new Vec3((double) MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F),
						this.motionY, var16)).normalize();
				...
				float var20 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ) * 1.0F
						+ 1.0F;
				...
				this.randomYawVelocity = (float) ((double) this.randomYawVelocity
						+ var13 * (0.699999988079071D / var21 / (double) var20));
			...
			this.dragonPartBody.setLocationAndAngles(this.posX + (double) (var30 * 0.5F), this.posY,
			...
			this.dragonPartWing1.setLocationAndAngles(this.posX + (double) (var7 * 4.5F), this.posY + 2.0D,
					this.posZ + (double) (var30 * 4.5F), 0.0F, 0.0F);
			this.dragonPartWing2.onUpdate();
			this.dragonPartWing2.setLocationAndAngles(this.posX - (double) (var7 * 4.5F), this.posY + 2.0D,
					this.posZ - (double) (var30 * 4.5F), 0.0F, 0.0F);

			if (!this.worldObj.isRemote && this.hurtTime == 0) {
				this.collideWithEntities(this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartWing1
						.getEntityBoundingBox().expand(4.0D, 2.0D, 4.0D).offset(0.0D, -2.0D, 0.0D)));
				this.collideWithEntities(this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartWing2
						.getEntityBoundingBox().expand(4.0D, 2.0D, 4.0D).offset(0.0D, -2.0D, 0.0D)));
			...
			this.dragonPartHead.setLocationAndAngles(this.posX + (double) (var33 * 5.5F * var3),
					this.posY + (var9[1] - var31[1]) * 1.0D + (double) (var29 * 5.5F),
					this.posZ - (double) (var35 * 5.5F * var3), 0.0F, 0.0F);
			...
			for (int var32 = 0; var32 < 3; ++var32) {
				...
				float var37 = this.rotationYaw * (float) Math.PI / 180.0F
						+ this.simplifyAngle(var36[0] - var31[0]) * (float) Math.PI / 180.0F * 1.0F;
				float var38 = MathHelper.sin(var37);
				float var39 = MathHelper.cos(var37);
				float var40 = 1.5F;
				float var41 = (float) (var32 + 1) * 2.0F;
				var34.onUpdate();
				var34.setLocationAndAngles(this.posX - (double) ((var30 * var40 + var38 * var41) * var3),
						this.posY + (var36[1] - var31[1]) * 1.0D - (double) ((var41 + var40) * var29) + 1.5D,
						this.posZ + (double) ((var7 * var40 + var39 * var41) * var3), 0.0F, 0.0F);
			}
	...
	private void updateDragonEnderCrystal() {
		...
		if (this.rand.nextInt(10) == 0) {
			...
			List var2 = this.worldObj.getEntitiesWithinAABB(EntityEnderCrystal.class,
					this.getEntityBoundingBox().expand((double) var1, (double) var1, (double) var1));
	...
	private void collideWithEntities(List p_70970_1_) {
		double var2 = (this.dragonPartBody.getEntityBoundingBox().minX
				+ this.dragonPartBody.getEntityBoundingBox().maxX) / 2.0D;
		double var4 = (this.dragonPartBody.getEntityBoundingBox().minZ
				+ this.dragonPartBody.getEntityBoundingBox().maxZ) / 2.0D;
	...
	protected void onDeathUpdate() {
		...
		if (this.deathTicks >= 180 && this.deathTicks <= 200) {
			...
			this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, this.posX + (double) var1,
					this.posY + 2.0D + (double) var2, this.posZ + (double) var3, 0.0D, 0.0D, 0.0D, new int[0]);
		}
