...
public class BlockDoor extends Block {
	...
	protected BlockDoor(Material p_i45402_1_) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING_PROP, EnumFacing.NORTH)
				.withProperty(OPEN_PROP, Boolean.valueOf(false))
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (this.blockMaterial == Material.iron) {
			...
			if (var10.getBlock() != this) {
				...
				worldIn.playAuxSFXAtEntity(playerIn, ((Boolean) state.getValue(OPEN_PROP)).booleanValue() ? 1003 : 1006,
						pos, 0);
				return true;
			}
		}
	...
	public void func_176512_a(World worldIn, BlockPos p_176512_2_, boolean p_176512_3_) {
		...
		if (var4.getBlock() == this) {
			BlockPos var5 = var4.getValue(HALF_PROP) == BlockDoor.EnumDoorHalf.LOWER ? p_176512_2_
		...
		if (state.getValue(HALF_PROP) == BlockDoor.EnumDoorHalf.UPPER) {
			...
			if (var9) {
				...
				if ((var8 || neighborBlock.canProvidePower()) && neighborBlock != this
						&& var8 != ((Boolean) var7.getValue(POWERED_PROP)).booleanValue()) {
	...
	public IBlockState getStateFromMeta(int meta) {
		return (meta & 8) > 0
				? this.getDefaultState().withProperty(HALF_PROP, BlockDoor.EnumDoorHalf.UPPER)
						.withProperty(HINGEPOSITION_PROP,
								(meta & 1) > 0 ? BlockDoor.EnumHingePosition.RIGHT : BlockDoor.EnumHingePosition.LEFT)
						.withProperty(POWERED_PROP, Boolean.valueOf((meta & 2) > 0))
				: this.getDefaultState().withProperty(HALF_PROP, BlockDoor.EnumDoorHalf.LOWER)
						.withProperty(FACING_PROP, EnumFacing.getHorizontal(meta & 3).rotateYCCW())
						.withProperty(OPEN_PROP, Boolean.valueOf((meta & 4) > 0));
	}
