...
public final class ItemStack {
	 ...
	 * @param create
	 *            Whether a new, empty compound should be created if none is present
	 *            yet
	 */
	...
	public List getTooltip(EntityPlayer playerIn, boolean advanced) {
			...
			while (var19.hasNext()) {
				...
				if (var10 > 0.0D) {
					var3.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted(
							"attribute.modifier.plus." + var22.getOperation(),
							new Object[] { DECIMALFORMAT.format(var12),
									StatCollector.translateToLocal("attribute.name." + (String) var21.getKey()) }));
				} else if (var10 < 0.0D) {
					var12 *= -1.0D;
					var3.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted(
							"attribute.modifier.take." + var22.getOperation(),
							new Object[] { DECIMALFORMAT.format(var12),
									StatCollector.translateToLocal("attribute.name." + (String) var21.getKey()) }));
				}
			}
		...
		if (advanced) {
			...
			var3.add(EnumChatFormatting.DARK_GRAY
	...
	/**
	 * Gets the attribute modifiers for this ItemStack. Will check for an NBT tag
	 * list containing modifiers for the stack.
	 */
	public Multimap getAttributeModifiers() {
		...
		if (this.hasTagCompound() && this.stackTagCompound.hasKey("AttributeModifiers", 9)) {
			...
			for (int var3 = 0; var3 < var2.tagCount(); ++var3) {
				...
				if (var5 != null && var5.getID().getLeastSignificantBits() != 0L
	...
	public IChatComponent getChatComponent() {
		...
		if (this.item != null) {
			...
			var2.getChatStyle().setChatHoverEvent(
					new HoverEvent(HoverEvent.Action.SHOW_ITEM, new ChatComponentText(var3.toString())));
