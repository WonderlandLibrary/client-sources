...
public class NetHandlerLoginServer implements INetHandlerLoginServer, IUpdatePlayerListBox {
	...
	public void func_147326_c() {
		...
		String var1 = this.server.getConfigurationManager().allowUserToConnect(this.networkManager.getRemoteAddress(),
	...
	public void processLoginStart(C00PacketLoginStart packetIn) {
		...
		if (this.server.isServerInOnlineMode() && !this.networkManager.isLocalChannel()) {
			...
			this.networkManager.sendPacket(new S01PacketEncryptionRequest(this.serverId,
					this.server.getKeyPair().getPublic(), this.field_147330_e));
		} else {
		...
		if (!Arrays.equals(this.field_147330_e, packetIn.func_149299_b(var2))) {
			...
			(new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
				...
				public void run() {
					...
					try {
						String var2 = (new BigInteger(CryptManager.getServerIdHash(NetHandlerLoginServer.this.serverId,
