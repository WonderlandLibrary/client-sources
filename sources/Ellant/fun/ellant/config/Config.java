...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\ellant\\CFG"), name + ".json");
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        Ellant.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        Ellant.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : Ellant.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", Ellant.getInstance().getStyleManager().getCurrentStyle() == style);
