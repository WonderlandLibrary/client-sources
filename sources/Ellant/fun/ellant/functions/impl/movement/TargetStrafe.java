...
@FunctionRegister(name = "TargetStrafe", type = Category.MOVEMENT, desc = "Крутит вас вокруг цели")
    ...
    private void ft() {
        ...
        private void normalMode() {
            if (target != null && target.isAlive() && target.getHealth() > 0.0f) {
                ...
                double yaw = getYaw(mc.player, x, z);
                final double speed = strafeMovement.calculateSpeed(null, false, true, true, 1.0F);
                mc.player.setMotion(speed * -Math.sin(Math.toRadians(yaw)), mc.player.getMotion().y, speed * Math.cos(Math.toRadians(yaw)));
            }
        }
