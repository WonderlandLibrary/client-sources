...
public class Font implements IMinecraft {
    ...
    public void drawCenteredText(MatrixStack stack, String text, float x, float y, int color, float size) {
        ...
        RenderSystem.color4f((float) (color >> 16 & 255) / 255.0F, (float) (color >> 8 & 255) / 255.0F, (float) (color & 255) / 255.0F, 1.0F);
        // Рисуем текст
    ...
    public void drawCenteredText(MatrixStack stack, ITextComponent text, float x, float y, int color, float size, int use) {
        drawText(stack, text.getString(), x - getWidth(text.getString(), size) / 2f, y, color, size, 255);
    }
    ...
    public void drawGradientText(MatrixStack stack, String text, float x, float y, int startColor, int endColor, int fontSize, float shadowOffset) {
        float gradientStep = 1.0F / (float)text.length();
        float currentX = x;

        for(int i = 0; i < text.length(); ++i) {
            char character = text.charAt(i);
            int color = ColorUtils.interpolate(startColor, endColor, (float)i * gradientStep);
            Fonts.montserrat.drawText(stack, String.valueOf(character), currentX, y, color, (float)fontSize, shadowOffset);
            currentX += Fonts.montserrat.getWidth(String.valueOf(character), (float)fontSize) - 1.0F;
        }
