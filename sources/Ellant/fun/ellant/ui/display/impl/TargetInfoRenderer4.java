...
public class TargetInfoRenderer4 implements ElementRenderer {
    private final ResourceLocation head = new ResourceLocation("expensive/images/steavehead.png");
    ...
    public void render(EventDisplay eventDisplay) {
        ...
        if (entity != null) {
            ...
            // DisplayUtils.drawShadow(posX, posY, width, height - 15 , 10, style.getFirstColor().getRGB(), style.getSecondColor().getRGB());
            DisplayUtils.drawRoundedRect(posX, posY, width + 5, height - 10, 2, ColorUtils.rgba(255, 255, 255 , 200));
            DisplayUtils.drawImage(head, posX + 5 , posY  + 4, headSize, headSize, ColorUtils.rgb(255, 255, 255));
            Scissor.push();
            Scissor.setFromComponentCoordinates(testX, testY, testW - 6, testH);
            Fonts.sfui.drawText(eventDisplay.getMatrixStack(), entity.getName().getString(), posX + headSize + spacing + spacing, posY + spacing + 1 - 3, -1, 7);
            Fonts.sfMedium.drawText(eventDisplay.getMatrixStack(), "Здоровье: " , posX + headSize + spacing + spacing ,
                    posY + spacing + 1 + spacing + spacing- 5, ColorUtils.rgb(255, 0, 255), 6);
            Fonts.sfMedium.drawText(eventDisplay.getMatrixStack(), "" +  ((int) hp + (int) mc.player.getAbsorptionAmount()), posX + headSize + spacing + spacing  + 35,
                    posY + spacing + 1 + spacing + spacing - 4.8f, ColorUtils.rgb(255, 0, 255), 6);
            Scissor.unset();
            Scissor.pop();
            ...
            int height1 = 5;
            DisplayUtils.drawRoundedRect(posX + headSize + spacing + spacing, posY  - spacing * 2 - 3 + 35, (width  - 30), 3,  1, ColorUtils.rgba(32, 32, 32, 100));


            DisplayUtils.drawShadow(posX + headSize + spacing + spacing, posY - spacing * 2 - 3 + 35, (width - 30) * healthAnimation, 3, 8, ColorUtils.rgb(255, 0, 255));

            DisplayUtils.drawRoundedRect(posX + headSize + spacing + spacing, posY + - spacing * 2 - 3 + 35, (width - 30) * healthAnimation, 3, 1, ColorUtils.rgb(255, 0, 255));
