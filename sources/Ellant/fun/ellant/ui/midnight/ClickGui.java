package fun.ellant.ui.midnight;

import com.mojang.blaze3d.matrix.MatrixStack;
import fun.ellant.Ellant;
import fun.ellant.functions.api.Category;
import fun.ellant.functions.api.Function;
import fun.ellant.ui.midnight.component.impl.*;
import fun.ellant.ui.midnight.component.impl.Component;
...
import static fun.ellant.ui.midnight.component.impl.ModuleComponent.binding;
import static fun.ellant.utils.client.IMinecraft.mc;
...
public class ClickGui extends Screen {
    public ClickGui() {
        ...
        for (Style style : Ellant.getInstance().getStyleManager().getStyleList()) {
            this.theme.add(new ThemeComponent(style));
        }
        cfg.clear();
        for (String config : Ellant.getInstance().getConfigStorage().getConfigsByName()) {
            cfg.add(new ConfigComponent(Ellant.getInstance().getConfigStorage().findConfig(config)));
        }
    }
    ...
    void renderBackground(MatrixStack matrixStack, float x, float y, float width, float height, float leftPanel, int mouseX, int mouseY) {
        DisplayUtils.drawShadow(x, y, width, height, 6, new Color(50, 50, 50).getRGB());
        DisplayUtils.drawRoundedRect(x, y, width, height, new Vector4f(4, 4, 4, 4), new Color(22, 24, 28, 130).getRGB());
        DisplayUtils.drawRoundedRect(x, y, width, 64 / 2f, new Vector4f(6, 0, 6, 0), new Color(17, 18, 21, 130).getRGB());
        DisplayUtils.drawRectHorizontalW(x + leftPanel, y, 3, 64 / 2f, new Color(12, 13, 15, 150).getRGB(), new Color(12, 13, 15, 160).getRGB());
        DisplayUtils.drawRoundedRect(x, y, leftPanel, height, new Vector4f(6, 6, 0, 0), new Color(17, 18, 21).getRGB());
        Fonts.msBold[19].drawCenteredString(matrixStack, "Ellant | ALPHA", x + leftPanel / 2f, y + 14, -1);
    }
    ...
    void renderCategories(MatrixStack matrixStack, float x, float y, float width, float height, float leftPanel, int mouseX, int mouseY) {
        ...
        for (Category t : Category.values()) {
            ...
            if (t == current)
                DisplayUtils.drawRoundedRect(x + (leftPanel / 2f) * animation, y + 32.5f + t.ordinal() * heightCategory, leftPanel * (1 - animation), heightCategory, new Vector4f(20 * animation, 20 * animation, 20 * animation, 20 * animation), new Color(50, 50, 50, 120).getRGB());

            boolean hovered = MathUtil.isInRegion(mouseX, mouseY, x, y + 32.5f + t.ordinal() * heightCategory, leftPanel, heightCategory);
            DisplayUtils.drawRoundedRect(x + (leftPanel / 2f) * animation, y + 32.5f + t.ordinal() * heightCategory, leftPanel * (1 - animation), heightCategory, new Vector4f(20 * animation, 20 * animation, 20 * animation, 20 * animation), new Color(50, 50, 50, (int) (120 * (t.anim / 5f))).getRGB());
    ...
    void renderComponents(MatrixStack matrixStack, float x, float y, float width, float height, float leftPanel, int mouseX, int mouseY) {
        ...
        Scissor.unset();
        Scissor.pop();
        DisplayUtils.drawRoundedRect(x + leftPanel, y + 64 / 2f, width - leftPanel, height - 64 / 2f, new Vector4f(0, 0, 6, 6), new Color(22, 24, 28, ((int) (160 * animation))).getRGB());
    }
    ...
    void drawComponents(MatrixStack stack, int mouseX, int mouseY) {
        ...
        if (current == Category.CONFIGS || current == Category.THEME) {
            ...
            if (current == Category.CONFIGS) {
                ...
                Fonts.gilroy[14].drawCenteredString(stack, "Создать", x + width - 45 - 2 + (35 - 2) / 2f, y + 64 / 2F + 21.5F, -1);
                DisplayUtils.drawRoundedRect(x + width - 45 - 35 - 2, y + 64 / 2F + 15, 35 - 2, 32 / 2f, 4, new Color(22, 24, 28).getRGB());
                Fonts.gilroy[14].drawCenteredString(stack, "Обновить", x + width - 45 - 35 - 2 + (35 - 2) / 2f, y + 64 / 2F + 21.5F, -1);
            ...
            for (ConfigComponent component : config) {
                if (current != Category.CONFIGS) continue;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + width - 45 - 2, y + 64 / 2F + 15, 35 - 2, 32 / 2f)) {
            Ellant.getInstance().getConfigStorage().saveConfiguration(configName);
            configName = "";
            configTyping = false;
            cfg.clear();
            for (String config : Ellant.getInstance().getConfigStorage().getConfigsByName()) {
        ...
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + width - ((64 / 2f) * 2) / 2f - 1, y + (64 / 2f) / 2f - 5, 10, 10)) {
            try {
                Runtime.getRuntime().exec("explorer " + Ellant.getInstance().getConfigStorage().CONFIG_DIR);
            } catch (IOException e) {
                e.printStackTrace();
            }
