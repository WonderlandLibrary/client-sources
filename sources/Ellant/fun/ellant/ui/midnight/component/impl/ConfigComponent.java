...
import fun.ellant.config.Config;
...
public class ConfigComponent extends Component {
    ...
    private void drawButtons(MatrixStack matrixStack) {
        ...
        Fonts.gilroy[14].drawCenteredString(matrixStack, selected ? "Сохранить" : "Загрузить", x + width - 35 - 1.5F + 32F / 2F, y + 8, -1);
        Fonts.gilroy[14].drawCenteredString(matrixStack, "Удалить", x + width - 35 - 35 - 1.5F + 32F / 2F, y + 8, -1);
    }
    ...
    private void handleSaveLoadButtonClick() {
        if (!selected) {
            Ellant.getInstance().getConfigStorage().loadConfiguration(config.getFile().getName().replace(".json", ""));
            ClickGui.confign = config.getFile().getName();
        } else {
            Ellant.getInstance().getConfigStorage().saveConfiguration(config.getFile().getName().replace(".json", ""));
        }
    }
    ...
    private void handleDeleteButtonClick() {
        Ellant.getInstance().getConfigStorage().deleteConfig(config.getFile().getName().replace(".json", ""));
        parent.cfg.clear();
        for (String config : Ellant.getInstance().getConfigStorage().getConfigsByName()) {
            parent.cfg.add(new ConfigComponent(Ellant.getInstance().getConfigStorage().findConfig(config)));
        }
    }
