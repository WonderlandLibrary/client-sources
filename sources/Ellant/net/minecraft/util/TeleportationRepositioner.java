...
public class TeleportationRepositioner
{
     ...
     */
    public static Result findLargestRectangle(BlockPos centerPos, Direction.Axis axis1, int max1, Direction.Axis axis2, int max2, Predicate<BlockPos> posPredicate)
    {
        ...
        int k = i;
        IntBounds[] ateleportationrepositioner$intbounds = new IntBounds[i + 1 + j];
        ateleportationrepositioner$intbounds[i] = new IntBounds(distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos), direction2, max2), distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos), direction3, max2));
        int l = ateleportationrepositioner$intbounds[i].min;

        for (int i1 = 1; i1 <= i; ++i1)
        {
            IntBounds teleportationrepositioner$intbounds = ateleportationrepositioner$intbounds[k - (i1 - 1)];
            ateleportationrepositioner$intbounds[k - i1] = new IntBounds(distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos).move(direction, i1), direction2, teleportationrepositioner$intbounds.min), distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos).move(direction, i1), direction3, teleportationrepositioner$intbounds.max));
        }

        for (int l2 = 1; l2 <= j; ++l2)
        {
            IntBounds teleportationrepositioner$intbounds2 = ateleportationrepositioner$intbounds[k + l2 - 1];
            ateleportationrepositioner$intbounds[k + l2] = new IntBounds(distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos).move(direction1, l2), direction2, teleportationrepositioner$intbounds2.min), distanceInDirection(posPredicate, blockpos$mutable.setPos(centerPos).move(direction1, l2), direction3, teleportationrepositioner$intbounds2.max));
        }
        ...
        for (int l1 = l; l1 >= 0; --l1)
        {
            ...
            {
                IntBounds teleportationrepositioner$intbounds1 = ateleportationrepositioner$intbounds[i2];
            ...
            Pair<IntBounds, Integer> pair = largestRectInHeights(aint);
            IntBounds teleportationrepositioner$intbounds3 = pair.getFirst();
        ...
        return new Result(centerPos.func_241872_a(axis1, i3 - k).func_241872_a(axis2, j3 - l), j1, k1);
    }
    ...
    @VisibleForTesting
    static Pair<IntBounds, Integer> largestRectInHeights(int[] heights)
    {
        int i = 0;
        int j = 0;
        int k = 0;
