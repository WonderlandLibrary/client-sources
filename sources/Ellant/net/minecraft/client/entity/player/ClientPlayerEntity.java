...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Ellant.getInstance().playerOnServer) {
                Ellant.getInstance().playerOnServer = true;
            }

            Ellant.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Ellant.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Ellant.getInstance().getEventBus().post(eventMotion);
        ...
        Ellant.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Ellant.getInstance().getCommandDispatcher();
    ...
    public void closeScreen() {
        ...
        Ellant.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    public void setSprinting(boolean sprinting) {
        Ellant.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            Ellant.getInstance().getEventBus().post(noSlowEvent);
