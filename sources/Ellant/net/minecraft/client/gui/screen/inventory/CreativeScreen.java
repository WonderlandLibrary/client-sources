...
public class CreativeScreen extends DisplayEffectsScreen<CreativeScreen.CreativeContainer>
{
    ...
    {
        super(new CreativeContainer(player), player.inventory, StringTextComponent.EMPTY);
        ...
        else
        {
            ...
            else if (selectedTabIndex == ItemGroup.INVENTORY.getIndex())
            {
                ...
                else if (type == ClickType.THROW && slotIn != null && slotIn.getHasStack())
                {
                    ...
                    this.minecraft.playerController.sendSlotPacket(itemstack1, ((CreativeSlot)slotIn).slot.slotNumber);
                }
                ...
                {
                    this.minecraft.player.container.slotClick(slotIn == null ? slotId : ((CreativeSlot)slotIn).slot.slotNumber, mouseButton, type, this.minecraft.player);
    ...
    private void setCurrentCreativeTab(ItemGroup tab)
    {
        ...
        if (tab == ItemGroup.INVENTORY)
        {
            ...
            for (int l = 0; l < container.inventorySlots.size(); ++l)
            {
                ...
                Slot slot = new CreativeSlot(container.inventorySlots.get(l), l, i1, j1);
    ...
    public static class CreativeContainer extends Container
    {
        ...
        public CreativeContainer(PlayerEntity player)
        {
            ...
            for (int i = 0; i < 5; ++i)
            {
                for (int j = 0; j < 9; ++j)
                {
                    this.addSlot(new LockedSlot(CreativeScreen.TMP_INVENTORY, i * 9 + j, 9 + j * 18, 18 + i * 18));
                }
            }
