...
public class ScreenManager
{
    ...
    public static <T extends Container> void openScreen(@Nullable ContainerType<T> type, Minecraft mc, int windowId, ITextComponent title)
    {
        ...
        {
            IScreenFactory < T, ? > iscreenfactory = getFactory(type);
    ...
    {
        return (IScreenFactory < T, ? >)FACTORIES.get(type);
    }

    private static <M extends Container, U extends Screen & IHasContainer<M>> void registerFactory(ContainerType <? extends M > type, IScreenFactory<M, U> factory)
    {
        IScreenFactory <? , ? > iscreenfactory = FACTORIES.put(type, factory);
