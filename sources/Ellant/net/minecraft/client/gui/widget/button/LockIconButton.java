...
public class LockIconButton extends Button
{
    ...
    public LockIconButton(int x, int y, IPressable p_i51133_3_)
    {
    ...
    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (!this.active)
        {
            lockiconbutton$icon = this.locked ? Icon.LOCKED_DISABLED : Icon.UNLOCKED_DISABLED;
        }
        else if (this.isHovered())
        {
            lockiconbutton$icon = this.locked ? Icon.LOCKED_HOVER : Icon.UNLOCKED_HOVER;
        }
        else
        {
            lockiconbutton$icon = this.locked ? Icon.LOCKED : Icon.UNLOCKED;
        }
