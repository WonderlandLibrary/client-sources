...
import vestige.impl.module.render.ESP;
import vestige.impl.module.render.HUD;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected boolean setScoreTeamColor(T entityLivingBaseIn)
    {
        ...
        ESP esp = (ESP) Vestige.getInstance().getModuleManager().getModule(ESP.class);
        
        if(esp.isEnabled() && esp.mode.is("Minecraft Outline")) {
        	HUD hud = (HUD) Vestige.getInstance().getModuleManager().getModule(HUD.class);
        	
        	Color color = hud.getCurrentColor();
        	
        	GlStateManager.color(color.getRed() / 255.0F, color.getGreen() / 255.0F, color.getBlue() / 255.0F, 255.0F);
        } else {
