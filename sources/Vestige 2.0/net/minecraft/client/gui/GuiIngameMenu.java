    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        switch (button.id)
        {
            ...
            case 1:
                ...
                if (flag)
                {
                    this.mc.displayGuiScreen(Vestige.getInstance().getMainMenu());
                }
                else if (flag1)
                {
                    RealmsBridge realmsbridge = new RealmsBridge();
                    realmsbridge.switchToRealms(Vestige.getInstance().getMainMenu());
                }
                else
                {
                    this.mc.displayGuiScreen(new GuiMultiplayer(Vestige.getInstance().getMainMenu()));
                }
