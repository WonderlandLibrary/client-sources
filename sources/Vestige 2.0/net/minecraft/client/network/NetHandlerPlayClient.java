...
import vestige.impl.module.exploit.Disabler;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
    	//Vestige.getInstance().addChatMessage("Entity id : " + packetIn.getEntityID() + " | Player id : " + packetIn.getPlayer() + " | Position : " + packetIn.getX() + " " + packetIn.getY() + " " + packetIn.getZ());
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        ...
        Disabler disabler = (Disabler) Vestige.getInstance().getModuleManager().getModule(Disabler.class);
        if(disabler.isEnabled() && disabler.mode.is("PingSpoof") && disabler.s08.isEnabled()) {
        	new Thread() {
        		@Override
        		public void run() {
        			try {
						sleep((long) disabler.s08PingDelay.getCurrentValue());
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
        			teleport(packetIn, false);
        		}
        	}.start();
        } else if(disabler.isEnabled() && disabler.mode.is("Watchdog") /* && Minecraft.getMinecraft().thePlayer != null*/) {
        	new Thread() {
        		@Override
        		public void run() {        			
        			try {
						sleep(mc.thePlayer == null ? 1500 : mc.thePlayer.ticksExisted < 150 ? 1000 : 450);
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
        			teleport(packetIn, false);
        	...
        	this.teleport(packetIn, true);
        }
    }
    
    private void teleport(S08PacketPlayerPosLook packetIn, boolean unfunny) {
    	try {
    ...
    public void onDisconnect(IChatComponent reason)
    {
        ...
        {
            this.gameController.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(Vestige.getInstance().getMainMenu()), "disconnect.lost", reason));
        }
    }
