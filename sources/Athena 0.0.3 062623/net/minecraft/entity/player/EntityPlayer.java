...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void setItemInUse(final ItemStack stack, int duration) {
        if (stack != this.itemInUse) {
            duration = (Athena.INSTANCE.getEventBus().post(new PlayerInteractEvent(PlayerInteractEvent.Action.USE_ITEM, this.getPosition(), this.worldObj)) ? duration : -1);
            if (duration <= 0) {
                return;
            }
