            ...
            while (iterator1.hasNext()) {
                ...
                if (!classinheritancemultimap.isEmpty()) {
                    for (final Entity entity4 : classinheritancemultimap) {
                        if (SettingsPage.ENTITIES.contains(entity4.getClass())) {
                            continue;
                        }
                        final OptimizerMod mod = (OptimizerMod)Athena.INSTANCE.getModuleManager().get(OptimizerMod.class);
                        boolean newRenderE = false;
                        final int entDist = mod.noLagEntityDistance;
                        if (entDist != 64) {
                            newRenderE = true;
                        }
                        boolean doIt = newRenderE && entity4.getDistanceToEntity(this.mc.thePlayer) <= entDist;
                        doIt = (entity4 instanceof EntityPlayer || entity4 instanceof EntityEnderPearl || doIt);
                        if ((newRenderE && !doIt) || (flag && !Reflector.callBoolean(entity4, Reflector.ForgeEntity_shouldRenderInPass, i))) {
                            continue;
                        }
            ...
            for (final Object renderglobal$containerlocalrenderinformation2 : this.renderInfosTileEntities) {
                ...
                if (!list2.isEmpty()) {
                    for (final TileEntity tileentity : list2) {
                        ...
                        final OptimizerMod mod2 = (OptimizerMod)Athena.INSTANCE.getModuleManager().get(OptimizerMod.class);
                        final int dist = mod2.noLagBlockDistance;
                        final boolean rdy = dist != 64;
