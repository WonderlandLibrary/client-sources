...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        if (!Athena.INSTANCE.getEventBus().post(new RenderEntityEvent<Object>(entity, this, x, y, z))) {
            return;
        }
    ...
    protected boolean setBrightness(final T entitylivingbaseIn, final float partialTicks, final boolean combineTextures) {
        ...
        if (flag2) {
            final CustomHitColor mod = (CustomHitColor)Athena.INSTANCE.getModuleManager().get(CustomHitColor.class);
            if (Athena.INSTANCE.getModuleManager().get(CustomHitColor.class).isToggled()) {
                this.brightnessBuffer.put(mod.color.getRed() / 255.0f);
                this.brightnessBuffer.put(mod.color.getGreen() / 255.0f);
                this.brightnessBuffer.put(mod.color.getBlue() / 255.0f);
                this.brightnessBuffer.put(mod.color.getAlpha() / 255.0f);
            }
            else {
    ...
    public void renderName(final T entity, final double x, final double y, final double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            final Settings settings = (Settings)Athena.INSTANCE.getModuleManager().get(Settings.class);
