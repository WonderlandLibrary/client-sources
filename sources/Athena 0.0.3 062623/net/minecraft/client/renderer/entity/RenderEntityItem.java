...
public class RenderEntityItem extends Render<EntityItem>
{
    ...
    public void doRender(final EntityItem entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        ...
        int rotation = (int)((System.nanoTime() / 3000000L + entity.getEntityId() * 10000) % 360L);
        if (Athena.INSTANCE.getModuleManager().get(ItemPhysics.class).isToggled()) {
            if (entity.onGround) {
                rotation = 90;
            }
            GL11.glRotatef((float)(rotation / 2), 0.0f, 1.0f, 0.0f);
            GL11.glRotatef((float)rotation, 1.0f, 0.0f, 0.0f);
        }
        for (int j = 0; j < i; ++j) {
