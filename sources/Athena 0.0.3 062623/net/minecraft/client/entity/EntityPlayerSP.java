...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onLivingUpdate() {
        ...
        if (Athena.INSTANCE.getModuleManager().get(ToggleSprint.class).isToggled()) {
            ToggleSprint.update(this.mc, (MovementInputFromOptions)this.movementInput, this);
        }
        else {
        ...
        if (Athena.INSTANCE.getModuleManager().get(ToggleSprint.class).isToggled()) {
            final boolean isSprintDisabled = !ToggleSprint.optionToggleSprint;
            final boolean canDoubleTap = ToggleSprint.optionDoubleTap;
            ...
            if (isSprintDisabled) {
                if (ToggleSprint.optionDoubleTap && this.onGround && !flag3 && this.movementInput.moveForward >= f && !this.isSprinting() && flag4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
                ...
                final boolean state = ToggleSprint.sprint;
                if (flag4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && !ToggleSprint.sprintHeldAndReleased && ((canDoubleTap && !this.isSprinting()) || !canDoubleTap)) {
                    this.setSprinting(state);
                }
                if (canDoubleTap && !state && this.onGround && !flag3 && this.movementInput.moveForward >= f && !this.isSprinting() && flag4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
                    if (this.sprintToggleTimer == 0) {
                        this.sprintToggleTimer = 7;
                    }
                    else {
            ...
            if (this.isSprinting() && (this.movementInput.moveForward < f || this.mc.thePlayer.isCollidedHorizontally || !flag4 || this.isUsingItem() || this.isPotionActive(Potion.blindness))) {
                this.setSprinting(false);
                if (ToggleSprint.sprintHeldAndReleased || isSprintDisabled || ToggleSprint.sprintDoubleTapped || this.isRiding() || (!ToggleSprint.optionEnableFlyBoost && this.capabilities.isFlying)) {
                    ToggleSprint.UpdateSprint(false, false);
                }
            }
        }
        else if (this.onGround && !flag2 && !flag3 && this.movementInput.moveForward >= f && !this.isSprinting() && flag4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
            if (this.sprintToggleTimer <= 0 && !this.mc.gameSettings.keyBindSprint.isKeyDown()) {
                this.sprintToggleTimer = 7;
            }
            else {
        ...
        if (Athena.INSTANCE.getModuleManager().get(ToggleSprint.class).isToggled()) {
            if (ToggleSprint.optionEnableFlyBoost && this.capabilities.isFlying && ToggleSprint.sprint) {
                if (ToggleSprint.sprint && ToggleSprint.optionEnableFlyBoost) {
                    ...
                    if (this.movementInput.sneak) {
                        this.motionY -= 0.15 * ToggleSprint.flyboostspeedVertical;
                    }
            ...
            if (this.capabilities.allowFlying && !this.isJumping && this.movementInput.jump) {
