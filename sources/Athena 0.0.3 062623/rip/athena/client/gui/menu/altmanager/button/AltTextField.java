...
public class AltTextField extends Gui
{
    ...
    public AltTextField(final int componentId, final FontRenderer fontrendererObj, final int x, final int y, final int par5Width, final int par6Height, final int color) {
        this.text = "";
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            if (s.length() > 0) {
                ...
                j2 = FontManager.getProductSansRegular(20).drawString(s2, (float)l, (float)i1, this.color);
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                j2 = FontManager.getProductSansRegular(20).drawString(s.substring(j), (float)j2, (float)i1, this.color);
            }
            if (flag2) {
                if (flag3) {
                    Gui.drawRect(k2, i1 - 1, k2 + 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT, -3092272);
                }
                else {
                    FontManager.getProductSansRegular(20).drawString("_", (float)k2, (float)i1, this.color);
                }
            }
            if (k != j) {
                final int l2 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
                this.drawCursorVertical(k2, i1 - 1, l2 - 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT);
            }
        }
    }
