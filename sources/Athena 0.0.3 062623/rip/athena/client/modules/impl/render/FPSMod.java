...
public class FPSMod extends Module
{
    ...
    public FPSMod() {
        ...
        this.width = 56;
        this.height = 18;
        (this.hud = new HUDElement("fps", this.width, this.height) {
            @Override
    ...
    public void render() {
        if (FPSMod.mc.gameSettings.showDebugInfo) {
            return;
        }
        GL11.glPushMatrix();
        final String string = FPSMod.mc.debug.split(" fps")[0] + " FPS";
        ...
        if (this.customFont) {
            ...
            this.hud.setWidth(FPSMod.mc.fontRendererObj.getStringWidth(string) + 16);
            this.hud.setHeight(FPSMod.mc.fontRendererObj.FONT_HEIGHT + 9);
            if (this.isUsingStaticChroma) {
                ...
                FPSMod.mc.fontRendererObj.drawStringWithShadow(string, posX, posY + 3.0f, this.color.getRGB());
            }
        }
