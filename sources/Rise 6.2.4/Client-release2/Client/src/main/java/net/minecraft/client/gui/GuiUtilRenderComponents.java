...
public class GuiUtilRenderComponents {
    ...
    public static List<IChatComponent> splitText(final IChatComponent chatComponent, final int width, final FontRenderer fontRenderer, final boolean p_178908_3_, final boolean p_178908_4_) {
        int i = 0;
        IChatComponent ichatcomponent = new ChatComponentText("");
        final List<IChatComponent> list = Lists.newArrayList();
        final List<IChatComponent> list1 = Lists.newArrayList(chatComponent);
        ...
        for (int j = 0; j < list1.size(); ++j) {
            ...
            if (i + i1 > width) {
                ...
                if (s3 != null && !s3.isEmpty()) {
                    ...
                    if (l >= 0 && fontRenderer.width(s4.substring(0, l)) > 0) {
                        s2 = s4.substring(0, l);
