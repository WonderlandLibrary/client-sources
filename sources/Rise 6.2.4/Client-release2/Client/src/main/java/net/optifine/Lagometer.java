...
public class Lagometer {
    ...
    public static boolean updateMemoryAllocation() {
        ...
        if (j < memLast) {
            final double d0 = (double) memDiff / 1000000.0D;
            final double d1 = (double) memTimeDiffMs / 1000.0D;
            final int k = (int) (d0 / d1);
            ...
            if (gameSettings.ofLagometer || gameSettings.field_181657_aC) {
                ...
                for (int j = 0; j < timesFrame.length; ++j) {
                    ...
                    if (gcs[j]) {
                        ...
                        final float f2 = f - (float) renderTime(j, timesTick[j], 0, k, k, f, worldrenderer);
                    }
                }
                ...
                mc.fontRendererObj.draw("30", 2, k2 + 1, -8947849);
                mc.fontRendererObj.draw("30", 1, k2, -3881788);
                mc.fontRendererObj.draw("60", 2, j2 + 1, -8947849);
                mc.fontRendererObj.draw("60", 1, j2, -3881788);
                GlStateManager.matrixMode(5889);
                ...
                mc.fontRendererObj.draw(" " + memMbSec + " MB/s", l1, i2, k1);
    ...
    private static long renderTime(final int frameNum, final long time, final int r, final int g, final int b, final float baseHeight, final WorldRenderer tessellator) {
        ...
        if (i < 3L) {
            ...
            tessellator.pos((float) frameNum + 0.5F, baseHeight - (float) i + 0.5F, 0.0D).func_181669_b(r, g, b, 255).endVertex();
            tessellator.pos((float) frameNum + 0.5F, baseHeight + 0.5F, 0.0D).func_181669_b(r, g, b, 255).endVertex();
            return i;
        }
    }
    ...
    private static long renderTimeDivider(final int frameStart, final int frameEnd, final long time, final int r, final int g, final int b, final float baseHeight, final WorldRenderer tessellator) {
        ...
        if (i < 3L) {
            ...
            tessellator.pos((float) frameStart + 0.5F, baseHeight - (float) i + 0.5F, 0.0D).func_181669_b(r, g, b, 255).endVertex();
            tessellator.pos((float) frameEnd + 0.5F, baseHeight - (float) i + 0.5F, 0.0D).func_181669_b(r, g, b, 255).endVertex();
            return i;
        }
    }
