...
public class ServerListEntryLanDetected implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(final int slotIndex, final int x, final int y, final int listWidth, final int slotHeight, final int mouseX, final int mouseY, final boolean isSelected) {
        this.mc.fontRendererObj.draw(I18n.format("lanServer.title"), x + 32 + 3, y + 1, 16777215);
        this.mc.fontRendererObj.draw(this.field_148291_b.getServerMotd(), x + 32 + 3, y + 12, 8421504);

        if (this.mc.gameSettings.hideServerAddress) {
            this.mc.fontRendererObj.draw(I18n.format("selectServer.hiddenAddress"), x + 32 + 3, y + 12 + 11, 3158064);
        } else {
            this.mc.fontRendererObj.draw(this.field_148291_b.getServerIpPort(), x + 32 + 3, y + 12 + 11, 3158064);
        }
    }
