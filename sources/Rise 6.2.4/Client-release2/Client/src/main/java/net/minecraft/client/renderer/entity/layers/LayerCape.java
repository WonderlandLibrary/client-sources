...
public class LayerCape implements LayerRenderer<AbstractClientPlayer> {
    ...
    public void doRenderLayer(final AbstractClientPlayer entitylivingbaseIn, final float p_177141_2_, final float p_177141_3_, final float partialTicks, final float p_177141_5_, final float p_177141_6_, final float p_177141_7_, final float scale) {
        UserInfo vantageUser = IRCInfoComponent.usersMap.get(entitylivingbaseIn.getCommandSenderName());
        if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && vantageUser != null) {
            if (!vantageUser.hasCape()) {
                switch (vantageUser.getStatus()) {
                    case Developer:
                        this.playerRenderer.bindTexture(riseDev);
                        break;
                    case Admin:
                        this.playerRenderer.bindTexture(riseAdmin);
                        break;
                    case Gato:
                        this.playerRenderer.bindTexture(riseGato);
                        break;
                    default:
                        this.playerRenderer.bindTexture(riseUser);
                        break;
                }
            }
            else {
                if (!vantageUser.isDownloading()) {
                    CapeComponent.capesToDownload.put(vantageUser.getUsername(), vantageUser.getCustomCapeUrlArray());
                } else {
                    this.playerRenderer.bindTexture(vantageUser.getCapeResourceLocation());
                }
            }
