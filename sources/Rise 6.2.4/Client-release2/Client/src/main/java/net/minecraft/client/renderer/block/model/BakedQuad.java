...
public class BakedQuad implements IVertexProducer {
    ...
    private static int[] makeVertexDataSingle(final int[] p_makeVertexDataSingle_0_, final TextureAtlasSprite p_makeVertexDataSingle_1_) {
        ...
        for (int l = 0; l < 4; ++l) {
            final int i1 = l * k;
            final float f = Float.intBitsToFloat(aint[i1 + 4]);
            final float f1 = Float.intBitsToFloat(aint[i1 + 4 + 1]);
            final float f2 = p_makeVertexDataSingle_1_.toSingleU(f);
            final float f3 = p_makeVertexDataSingle_1_.toSingleV(f1);
    ...
    private static TextureAtlasSprite getSpriteByUv(final int[] p_getSpriteByUv_0_) {
        ...
        for (int j = 0; j < 4; ++j) {
            final int k = j * i;
            final float f4 = Float.intBitsToFloat(p_getSpriteByUv_0_[k + 4]);
            final float f5 = Float.intBitsToFloat(p_getSpriteByUv_0_[k + 4 + 1]);
            f = Math.min(f, f4);
            f1 = Math.min(f1, f5);
            f2 = Math.max(f2, f4);
            f3 = Math.max(f3, f5);
        ...
        final float f6 = (f + f2) / 2.0F;
        final float f7 = (f1 + f3) / 2.0F;
        final TextureAtlasSprite textureatlassprite = Minecraft.getMinecraft().getTextureMapBlocks().getIconByUV(f6, f7);
