...
public class PrereleaseDisclaimer extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.MAIN.get(24, Weight.REGULAR).drawCentered("Note: This is private prerelease software", sr.getScaledWidth() / 2D,
                sr.getScaledHeight() / 2D - 70, ColorUtil.withAlpha(Color.WHITE, (int) fadeAnimation.getValue()).getRGB());
        Fonts.MAIN.get(24, Weight.REGULAR).drawCentered("Features, interfaces and sequences are not final and can be expected to change at any time", sr.getScaledWidth() / 2D,
                sr.getScaledHeight() / 2D - 50, ColorUtil.withAlpha(Color.WHITE, (int) fadeAnimation.getValue()).getRGB());

        Fonts.MAIN.get(16, Weight.REGULAR).drawCentered("© Rise Client 2022. All Rights Reserved", sr.getScaledWidth() / 2D,
