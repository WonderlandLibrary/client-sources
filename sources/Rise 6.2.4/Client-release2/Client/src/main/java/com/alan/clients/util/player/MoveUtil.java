...
public class MoveUtil implements Accessor {
    ...
    public static javax.vecmath.Vector2d getMotion(double moveSpeed) {
        ...
        if (moveForward != 0.0D || moveStrafe != 0.0D) {
            ...
            double cos = Math.cos(Math.toRadians(rotationYaw + ConstantsManager.b));
            double sin = Math.sin(Math.toRadians(rotationYaw + ConstantsManager.b));
    ...
    public void setSpeedMoveEvent(final MoveEvent moveEvent, final double moveSpeed) {
        setSpeedMoveEvent(moveEvent, moveSpeed, mc.thePlayer.movementYaw, mc.thePlayer.movementInput.moveStrafe, mc.thePlayer.movementInput.moveForward);
    }
    ...
    public void setSpeedMoveEvent(final MoveEvent moveEvent, final double moveSpeed, final float pseudoYaw, final double pseudoStrafe, final double pseudoForward) {
        ...
        final double mx = Math.cos(Math.toRadians((yaw + ConstantsManager.b)));
        final double mz = Math.sin(Math.toRadians((yaw + ConstantsManager.b)));
    ...
    public double[] moveFlying(float strafe, float forward, final boolean onGround, final float yaw, final boolean sprinting) {
        ...
        if (f >= 1.0E-4F) {
            ...
            final float f1 = MathHelper.sin(yaw * (float) ConstantsManager.a / 180.0F);
            final float f2 = MathHelper.cos(yaw * (float) ConstantsManager.a / 180.0F);
    ...
    public Vector3d predictMovement(Entity target, Vector2f input, int amount, boolean sprinting) {
        double deltaX = (target.posX - target.lastTickPosX);
        double deltaZ = (target.posZ - target.lastTickPosZ);
        return predictMovement(new Vector3d(target.posX, target.posY, target.posZ), new Vector3d(deltaX, 0, deltaZ), target.rotationYaw, input, amount, sprinting);
    }

    public Vector3d predictMovement(Vector3d position, Vector3d delta, float yaw, Vector2f input) {
        return predictMovement(position, delta, yaw, input, 1, input.x != 0 || input.y != 0);
    }

    public Vector3d predictMovement(Vector3d position, Vector3d delta, float yaw, Vector2f input, int amount, boolean sprinting) {
        for (int i = 0; i <= amount; i++) {
            delta.z *= 0.6f * 0.91f;
            delta.x *= 0.6f * 0.91f;

            double[] moveFlying = MoveUtil.moveFlying(input.x, input.y,
                    true, yaw, sprinting);

            if (moveFlying == null) return position;

            delta.x += moveFlying[0];
            delta.y = 0;
            delta.z += moveFlying[1];
    ...
    public Vector3d predictedPosition() {
        EntityPlayerSP predictivePlayer = new EntityPlayerSP(mc, mc.theWorld, new NetHandlerPlayClient(mc, mc.currentScreen, new NetworkManager(mc.getNetHandler().getNetworkManager().direction), mc.thePlayer.getGameProfile()), mc.thePlayer.getStatFileWriter());

        mc.theWorld.addEntityToWorld(-99999, predictivePlayer);
        predictivePlayer.motionX = mc.thePlayer.motionX;
        predictivePlayer.motionY = mc.thePlayer.motionY;
        predictivePlayer.motionZ = mc.thePlayer.motionZ;
        predictivePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ);
        predictivePlayer.rotationYaw = predictivePlayer.movementYaw = mc.thePlayer.movementYaw;
        predictivePlayer.onGround = mc.thePlayer.onGround;
        predictivePlayer.setSprinting(mc.thePlayer.isSprinting());
        predictivePlayer.omniSprint = mc.thePlayer.omniSprint;

        predictivePlayer.movementInput = new MovementInputFromOptions(mc.gameSettings);

        predictivePlayer.movementInput.updatePlayerMoveState();
        predictivePlayer.moveForward = predictivePlayer.movementInput.moveForward;
        predictivePlayer.moveStrafing = predictivePlayer.movementInput.moveStrafe;

        predictivePlayer.onUpdate();
        mc.theWorld.removeEntityFromWorld(-99999);

        if (mc.gameSettings.keyBindJump.isKeyDown() && mc.thePlayer.onGround && mc.thePlayer.jumpTicks <= 1) {
            predictivePlayer.setPosition(predictivePlayer.posX, predictivePlayer.posY + MoveUtil.jumpMotion(), predictivePlayer.posZ);
        }

        return new Vector3d(predictivePlayer.posX, predictivePlayer.posY, predictivePlayer.posZ);
    }
}
