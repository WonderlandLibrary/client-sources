        ...
        for (double yPercent = 1; yPercent >= 0; yPercent -= 0.25 + Math.random() * 0.1) {
            for (double xPercent = 1; xPercent >= -0.5; xPercent -= 0.5) {
                for (double zPercent = 1; zPercent >= -0.5; zPercent -= 0.5) {
                    ...
                    MovingObjectPosition rayCastResult = RayCastUtil.rayCast(adaptiveRotations, range);
                    if (rayCastResult != null && rayCastResult.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) {
