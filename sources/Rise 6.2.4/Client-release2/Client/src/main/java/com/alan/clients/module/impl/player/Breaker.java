...
if(Client.INSTANCE.getModuleManager().get(FastBreak.class).isEnabled()) {
    PercentageComponent.setActive((float) (1.21*damage + ((double) (Client.INSTANCE.getModuleManager().get(FastBreak.class).speed.getValue())) / 100));
} else{
    PercentageComponent.setActive((float) (1.21*damage));
}
...
/*test*/
    ...
    public final Listener<MouseOverEvent> onMouseOver = event -> {
        ...
        MovingObjectPosition movingObjectPosition = PlayerUtil.block(blockPos).collisionRayTrace(mc.theWorld, blockPos, mc.thePlayer.getPositionEyes(1), forward);
