...
public final class ItemDamageComponent extends Component {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        if (active) {
            ...
            if (bowSlot != -1 && arrow()) {
                getComponent(Slot.class).setSlot(bowSlot);
                type = Type.BOW;
            } else if (rodSlot != -1) {
                getComponent(Slot.class).setSlot(rodSlot);
                type = Type.ROD;
            } else if (snowBall != -1) {
                getComponent(Slot.class).setSlot(snowBall);
                type = Type.PROJECTILES;
            } else if (egg != -1) {
                getComponent(Slot.class).setSlot(egg);
                type = Type.PROJECTILES;
            }
