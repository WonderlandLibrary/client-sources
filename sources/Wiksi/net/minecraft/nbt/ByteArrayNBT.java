...
public class ByteArrayNBT extends CollectionNBT<ByteNBT>
{
    ...
    {
        ITextComponent itextcomponent = (new StringTextComponent("B")).mergeStyle(SYNTAX_HIGHWiksi_NUMBER_TYPE);
        IFormattableTextComponent iformattabletextcomponent = (new StringTextComponent("[")).append(itextcomponent).appendString(";");

        for (int i = 0; i < this.data.length; ++i)
        {
            IFormattableTextComponent iformattabletextcomponent1 = (new StringTextComponent(String.valueOf((int)this.data[i]))).mergeStyle(SYNTAX_HIGHWiksi_NUMBER);
