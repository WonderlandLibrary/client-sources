...
public class CompoundNBT implements INBT
{
    ...
    protected static ITextComponent getNameComponent(String name)
    {
        ...
        {
            return (new StringTextComponent(name)).mergeStyle(SYNTAX_HIGHWiksi_KEY);
        }
        else
        {
            String s = StringNBT.quoteAndEscape(name);
            String s1 = s.substring(0, 1);
            ITextComponent itextcomponent = (new StringTextComponent(s.substring(1, s.length() - 1))).mergeStyle(SYNTAX_HIGHWiksi_KEY);
