...
public class IntArrayNBT extends CollectionNBT<IntNBT>
{
    ...
    {
        ITextComponent itextcomponent = (new StringTextComponent("I")).mergeStyle(SYNTAX_HIGHWiksi_NUMBER_TYPE);
        IFormattableTextComponent iformattabletextcomponent = (new StringTextComponent("[")).append(itextcomponent).appendString(";");

        for (int i = 0; i < this.intArray.length; ++i)
        {
            iformattabletextcomponent.appendString(" ").append((new StringTextComponent(String.valueOf(this.intArray[i]))).mergeStyle(SYNTAX_HIGHWiksi_NUMBER));
