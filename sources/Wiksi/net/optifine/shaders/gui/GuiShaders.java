...
public class GuiShaders extends GuiScreenOF
{
    ...
    public void init()
    {
        ...
        this.addButton(new GuiButtonEnumShaderOption(EnumShaderOption.OLD_Wiksi, k, 7 * i1 + l, i, j));
        ...
        if (guiElement.active)
        {
            if (guiElement instanceof GuiButtonEnumShaderOption)
            {
                ...
                switch (guibuttonenumshaderoption.getEnumShaderOption())
                {
                    ...
                    case OLD_Wiksi:
                        Shaders.configOldWiksi.nextValue(!rightClick);

                        if (hasShiftDown())
                        {
                            Shaders.configOldWiksi.resetValue();
                        }
