...
public class MacroCommand implements Command, MultiNamedCommand, CommandWithAdvice {
    ...
    public void execute(Parameters parameters) {
        ...
        switch (commandType) {
            ...
            default ->
                    throw new CommandException(TextFormatting.RED + "Команды:" + TextFormatting.GRAY + " add, remove, clear, list");
        }
    }
    ...
    private void removeMacro(Parameters parameters) {
        ...
        Wiksi.getInstance().getMacroManager().deleteMacro(macroName);
    ...
    private void clearMacros() {
        Wiksi.getInstance().getMacroManager().clearList();
        logger.log(TextFormatting.GREEN + "Все макросы были удалены.");
    }

    private void printMacrosList() {
        if (Wiksi.getInstance().getMacroManager().isEmpty()) {
            logger.log(TextFormatting.RED + "Список пустой");
            return;
        }
        Wiksi.getInstance().getMacroManager().macroList
