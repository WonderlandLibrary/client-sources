...
import src.Wiksi.functions.settings.impl.*;
...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\Wiksi\\configs"), name + ".cfg");
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        Wiksi.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        Wiksi.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : Wiksi.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", Wiksi.getInstance().getStyleManager().getCurrentStyle() == style);
