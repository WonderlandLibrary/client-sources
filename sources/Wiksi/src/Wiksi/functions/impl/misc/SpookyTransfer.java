...
@FunctionRegister(name = "SpookyTransfer", type = Category.Misc)
public class SpookyTransfer extends Function {
    ...
    private void onUpdate(EventUpdate updateEvent) {
        if (mc.player.ticksExisted < 300 && !isReadyToSell) {
            int ticksRemaining = 300 - mc.player.ticksExisted;
            int secondsRemaining = ticksRemaining / 10;
        ...
        if (sellCount >= itemCountToSell || allItemsToSell) {
            ...
            if (!connectedToServer) {
                if (changeServerStopWatch.isReached(250)) {
            ...
            if (mc.player.openContainer instanceof ChestContainer container) {
                ...
                for (int index = 0; index < lowerChestInventory.getSizeInventory(); ++index) {
                    if (stopWatch.isReached(400) && lowerChestInventory.getStackInSlot(index).getItem() != Items.AIR) {
