...
@FunctionRegister(name = "WorldParticle", type = Category.Render)
public class Snow extends Function {
    ...
    private void onDisplay(EventDisplay e) {
        ...
        for (Particle p : particles) {
            ...
            if (System.currentTimeMillis() - p.time > 12500) {
                particles.remove(p);
            }
            if (mc.player.getPositionVec().distanceTo(p.pos) > 350) {
                particles.remove(p);
            }
            if (isInView(p.pos)) {
                ...
                float size = 1 - ((System.currentTimeMillis() - p.time) / 12500f);
