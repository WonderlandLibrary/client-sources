...
default void addQuad(MatrixStack.Entry p_addQuad_1_, BakedQuad p_addQuad_2_, float[] p_addQuad_3_, float p_addQuad_4_, float p_addQuad_5_, float p_addQuad_6_, float p_addQuad_7_, int[] p_addQuad_8_, int p_addQuad_9_, boolean p_addQuad_10_)
    {
        ...
        for (int i1 = 0; i1 < k; ++i1)
        {
            ...
            if (FORGE)
            {
                k1 = this.applyBakedWiksi(p_addQuad_8_[i1], aint, j1);
            }
...
default int applyBakedWiksi(int p_applyBakedWiksi_1_, int[] p_applyBakedWiksi_2_, int p_applyBakedWiksi_3_)
    {
        int i = getLightOffset(0);
        int j = LightTexture.getLightBlock(p_applyBakedWiksi_2_[p_applyBakedWiksi_3_ + i]);
        int k = LightTexture.getLightSky(p_applyBakedWiksi_2_[p_applyBakedWiksi_3_ + i]);

        if (j == 0 && k == 0)
        {
            return p_applyBakedWiksi_1_;
        }
        else
        {
            int l = LightTexture.getLightBlock(p_applyBakedWiksi_1_);
            int i1 = LightTexture.getLightSky(p_applyBakedWiksi_1_);
            l = Math.max(l, j);
            i1 = Math.max(i1, k);
