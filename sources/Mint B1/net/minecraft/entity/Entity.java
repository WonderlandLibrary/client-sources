...
public abstract class Entity implements ICommandSender
{
    ...
    public MovingObjectPosition rayTraceCustom(final double blockReachDistance, final float partialTicks, final float yaw, final float pitch) {
        final Vec3 vec3 = this.getPositionEyes(partialTicks);
        final Vec3 vec31 = this.getLookCustom(partialTicks, yaw, pitch);
        final Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, false, true);
    }
    ...
    public Vec3 getLookCustom(float p_70676_1_, final float yaw, final float pitch)
    {
        if (p_70676_1_ == 1.0F)
        {
            return this.func_174806_f(pitch, yaw);
        }
        else
        {
            float var2 = this.prevRotationPitch + (pitch - this.prevRotationPitch) * p_70676_1_;
            float var3 = this.prevRotationYaw + (yaw - this.prevRotationYaw) * p_70676_1_;
            return this.func_174806_f(var2, var3);
        }
    }
