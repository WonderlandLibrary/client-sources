...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
        PacketThreadUtil.func_180031_a(packetIn, this, this.gameController);
        final double var2 = (double)packetIn.func_149051_d() / 32.0D;
        final double var4 = (double)packetIn.func_149050_e() / 32.0D;
        final double var6 = (double)packetIn.func_149049_f() / 32.0D;
    ...
    public void addSilentToSendQueue(Packet packet)
    {
    ...
    public void sendPacketNoEventDelayed(final Packet packet, final long delay) {
    	...
    	if(Minecraft.getMinecraft().getNetHandler().getNetworkManager()==null) return;
        try {
            new Thread(){
                ...
                public void run() {
                    try {
                        Thread.sleep(delay);
                        Minecraft.getMinecraft().getNetHandler().getNetworkManager().sendPacketWithoutEvent(packet);
                    }
                    catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    public void addPacketToQueueDelayed(final Packet p, final long delay) {
