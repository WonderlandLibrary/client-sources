...
import wtf.diablo.client.module.impl.render.ChestChamsModule;
...
public class TileEntityChestRenderer extends TileEntitySpecialRenderer<TileEntityChest>
{
    ...
    public void renderTileEntityAt(TileEntityChest te, double x, double y, double z, float partialTicks, int destroyStage)
    {
        ...
        if (te.adjacentChestZNeg == null && te.adjacentChestXNeg == null)
        {
            ...
            final ChestChamsModule chestChamsModule = Diablo.getInstance().getModuleRepository().getModuleInstance(ChestChamsModule.class);
            if (chestChamsModule.isEnabled()) {
                int color = chestChamsModule.getColorHidden().getRGB();
                int alpha = chestChamsModule.opacity();
                boolean material = chestChamsModule.isMaterial();
                GL11.glPushAttrib(GL11.GL_ALL_CLIENT_ATTRIB_BITS);
                GlStateManager.enableBlend();
                GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);

                if (!chestChamsModule.isTextureVisible()) {
                ...
                if (!chestChamsModule.isTextureVisible()) {
                    color = chestChamsModule.getColor().getRGB();
                    GL11.glColor4f((color >> 16 & 0xFF) / 255.0F, (color >> 8 & 0xFF) / 255.0F, (color & 0xFF) / 255.0F, Math.max(30 / 255F, alpha / 255F));
                }
