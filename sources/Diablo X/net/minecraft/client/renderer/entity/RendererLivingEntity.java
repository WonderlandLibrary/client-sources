...
import wtf.diablo.client.module.impl.render.ChamsModule;
import wtf.diablo.client.module.impl.render.ESPModule;
import wtf.diablo.client.module.impl.render.OutlineModule;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[]{entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)})) {
            ...
            try {
                ...
                final OutlineModule outline = Diablo.getInstance().getModuleRepository().getModuleInstance(OutlineModule.class);

                if (outline.isEnabled() && entity instanceof EntityPlayer && entity != Minecraft.getMinecraft().thePlayer) {
                    final double lineWidth = outline.getWidth().getValue();
                    final int color = outline.getColor().getValue().getRGB();
                    ...
                    GL11.glColor4f((color >> 16 & 0xFF) / 255.0F, (color >> 8 & 0xFF) / 255.0F, (color & 0xFF) / 255.0F, Math.max(30 / 255F, 1F));
                    this.renderModel(entity, f6, f5, f7, f2, f8, 0.0625F);
                    GL11.glPolygonOffset(1.0F, 2000000F);
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        ...
        if (flag || flag1) {
            ...
            final ChamsModule chamsModule = Diablo.getInstance().getModuleRepository().getModuleInstance(ChamsModule.class);

            if (chamsModule.isEnabled() && entitylivingbaseIn instanceof EntityPlayer) {

                //TODO: Add entity color
                //int color = ColorUtil.getEntityColor(entitylivingbaseIn,Diablo.INSTANCE.getModuleManager().getModule(ChamsModule.class).getColorHidden());
                int color = chamsModule.getColorHidden().getRGB();
                int alpha = chamsModule.opacity();
                boolean material = chamsModule.isMaterial();
                GL11.glPushAttrib(GL11.GL_ALL_CLIENT_ATTRIB_BITS);
                GlStateManager.enableBlend();
                GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);

                if (!chamsModule.isTextureVisible()) {
                    GlStateManager.disableTexture2D();
                }
                ...
                if (!chamsModule.isTextureVisible()) {
                    color = chamsModule.getColor().getRGB();
                    GL11.glColor4f((color >> 16 & 0xFF) / 255.0F, (color >> 8 & 0xFF) / 255.0F, (color & 0xFF) / 255.0F, Math.max(30 / 255F, alpha / 255F));
                }
    ...
    public void renderName(T entity, double x, double y, double z) {
        final ESPModule esp = Diablo.getInstance().getModuleRepository().getModuleInstance(ESPModule.class);
    ...
    private static final class RendererLivingEntity$1 {
        static final int[] field_178679_a = initializeVisibleArray();
        private static final String __OBFID = "CL_00002435";

        private static int[] initializeVisibleArray() {
            int[] array = new int[Team.EnumVisible.values().length];
            array[Team.EnumVisible.ALWAYS.ordinal()] = 1;
            array[Team.EnumVisible.NEVER.ordinal()] = 2;
            array[Team.EnumVisible.HIDE_FOR_OTHER_TEAMS.ordinal()] = 3;
            array[Team.EnumVisible.HIDE_FOR_OWN_TEAM.ordinal()] = 4;
            return array;
        }
    }
}
