...
import wtf.diablo.client.module.impl.render.CrosshairModule;
import wtf.diablo.client.module.impl.render.EffectsModule;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        final EffectsModule postProcessing = Diablo.getInstance().getModuleRepository().getModuleInstance(EffectsModule.class);
        ...
        final CrosshairModule crosshairModule = Diablo.getInstance().getModuleRepository().getModuleInstance(CrosshairModule.class);

        if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1 && !crosshairModule.isEnabled())
        {
        ...
        final OverlayEvent renderOverlayEvent = new OverlayEvent(scaledresolution);

        renderOverlayEvent.call();
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        final ArrayListModule arrayList = Diablo.getInstance().getModuleRepository().getModuleInstance(ArrayListModule.class);

        int j1 = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        int normal = arrayList.getY() + 15 + j1;
        ...
        for (Object score1 : arraylist1)
        {
            ...
            if (s2.equals("§c1") && Minecraft.getMinecraft().getCurrentServerData().serverIP.contains("hypixel.net")) {
                final String s4 = "www.diablo.wtf";

                int count = 0;
                int x = 0;
                for (char c : s4.toCharArray()) {
                    this.getFontRenderer().drawStringWithShadow(String.valueOf(c), j + x, l, ColorModule.getColor(250 * count));
                    x += this.getFontRenderer().getStringWidth(String.valueOf(c));
                    count++;
                }
            } else {
