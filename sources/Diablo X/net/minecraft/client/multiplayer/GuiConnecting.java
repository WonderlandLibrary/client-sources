...
public class GuiConnecting extends GuiScreen
{
    ...
    {
        final ServerConnectEvent event = new ServerConnectEvent(ip, port);
        event.call();
        ...
        (new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet())
        {
            public void run()
            {
                ...
                {
                    if (event.isCancelled()) {
                        mc.displayGuiScreen(new GuiDisconnected(previousGuiScreen, "connect.failed", new ChatComponentText(event.getReason())));
                    }
