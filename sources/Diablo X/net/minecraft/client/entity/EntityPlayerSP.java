...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        if(!this.onGround) ++ticksInAir; else ticksInAir = 0;
        ...
        if (this.isCurrentViewEntity()) {
            MotionEvent e = new MotionEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
            e.setEventType(EventTypeEnum.PRE);
            e.call();
            ...
            if (!e.isCancelled()) {
                ...
                if (this.ridingEntity == null) {
                    if (flag2 && flag3) {
                        this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(e.getPosX(), e.getPosY(), e.getPosZ(), e.getRotationYaw(), e.getRotationPitch(), e.isOnGround()));
                    } else if (flag2) {
                        this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(e.getPosX(), e.getPosY(), e.getPosZ(), e.isOnGround()));
                    } else if (flag3) {
                    ...
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, e.getRotationYaw(), e.getRotationPitch(), e.isOnGround()));
                    flag2 = false;
                }
            ...
            e.setEventType(EventTypeEnum.POST);
            e.call();
        }
    ...
    public void onLivingUpdate()
    {
        ...
        final SlowDownEvent e = new SlowDownEvent(SlowDownEvent.Mode.Item);
        e.call();

        if (this.isUsingItem() && !this.isRiding() && !e.isCancelled()) {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        Diablo.getInstance().getEventBus().call(event);
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        final MoveFlyingEvent eventMove = new MoveFlyingEvent(rotationYaw, friction, strafe, forward);
        Diablo.getInstance().getEventBus().call(eventMove);
        this.rotationYaw = eventMove.getYaw();
        this.rotationYaw = oldYaw;
        super.moveFlying(eventMove.getStrafe(), eventMove.getForward(), eventMove.getFriction());
    }
}