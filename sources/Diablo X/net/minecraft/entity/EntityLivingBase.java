...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
        float val = this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
        float value = 1;
        final AnimationsModule animation = Diablo.getInstance().getModuleRepository().getModuleInstance(AnimationsModule.class);

        if (animation.isEnabled()) {
            final double swing = animation.getSwordSlowdown();
            val /= (1 / swing);
        }
