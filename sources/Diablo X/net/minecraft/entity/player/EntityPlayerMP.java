...
public class EntityPlayerMP extends EntityPlayer implements ICrafting
{
    ...
    public MovingObjectPosition rayTrace(final double reach, final float yaw, final float lastYaw, final float pitch, final float lastPitch) {

        final float partialTicks = Minecraft.getMinecraft().getTimer().renderPartialTicks;

        Vec3 vec3 = this.getPositionEyes(partialTicks);

        Vec3 vec31 = this.getLook(partialTicks, yaw, lastYaw, pitch, lastPitch);
    ...
    public Vec3 getLook(float partialTicks, final float yaw, final float lastYaw, final float pitch, final float lastPitch)
    {
        ...
        {
            float intpP = lastPitch + (pitch - lastPitch) * partialTicks;
            float intpY = lastYaw + (yaw - lastYaw) * partialTicks;
            return this.getVectorForRotation(intpP, intpY);
        }
    }
}
