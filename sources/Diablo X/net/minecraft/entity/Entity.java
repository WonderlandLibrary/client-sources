...
import wtf.diablo.client.event.impl.player.motion.SafeWalkEvent;
import wtf.diablo.client.event.impl.player.motion.step.StepEvent;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            final SafeWalkEvent weedEventSafewalk = new SafeWalkEvent();

            if (this instanceof EntityPlayerSP) {
                Diablo.getInstance().getEventBus().call(weedEventSafewalk);
            }

            boolean flag = this.onGround && (this.isSneaking() || weedEventSafewalk.isCancelled()) && this instanceof EntityPlayer;
            if (flag)
            {
                double d6;
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                ...
                {
                    final StepConfirmEvent stepConfirmEvent = new StepConfirmEvent();
                    stepConfirmEvent.call();
                }
            }
    ...
    public float getDistanceToEntityFixed(Entity entityIn)
    {
        float f = (float)(this.posX - entityIn.posX);
        float f1 = (float)(this.posY - entityIn.posY);
        float f2 = (float)(this.posZ - entityIn.posZ);
        return MathHelper.sqrt_float(f * f + f1 * f1 + f2 * f2) - 0.5657F;
    }

    public final void vClip(final double y) {
        this.setPosition(this.posX, this.posY + y, this.posZ);
    }
}