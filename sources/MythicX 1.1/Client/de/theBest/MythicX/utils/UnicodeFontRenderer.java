...
public class UnicodeFontRenderer {
    ...
    private Font getFontByName(String name) throws IOException, FontFormatException {
        return getFontFromInput("/assets/minecraft/hero/fonts/" + name + ".ttf");
    }

    private Font getFontFromInput(String path) throws IOException, FontFormatException {
        return Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(UnicodeFontRenderer.class.getResourceAsStream(path)));
    }
    ...
    public int getStringWidth(String text) {
        ...
        for (int j = 0; j < EnumChatFormatting.getTextWithoutFormattingCodes(text).length(); ++j) {
            ...
            if (k < 0 && j < EnumChatFormatting.getTextWithoutFormattingCodes(text).length() - 1) {
                ++j;
