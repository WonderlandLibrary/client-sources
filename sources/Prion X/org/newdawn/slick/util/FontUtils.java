...
public class FontUtils
{
  ...
  public static final int drawString(Font font, String s, int alignment, int x, int y, int width, Color color)
  {
    ...
    } else if (alignment == 4)
    {
      ...
      return drawJustifiedSpaceSeparatedSubstrings(font, s, x, 
        y, calculateWidthOfJustifiedSpaceInPixels(font, 
        s, leftWidth));
    }
  ...
  private static int drawJustifiedSpaceSeparatedSubstrings(Font font, String s, int x, int y, int justifiedSpaceWidth)
  {
    ...
    while (curpos < s.length()) {
      ...
      resultingXCoordinate = resultingXCoordinate + (font.getWidth(substring) + justifiedSpaceWidth);
