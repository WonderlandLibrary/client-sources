...
public class TextureAtlasSprite
{
  ...
  public void updateAnimation()
  {
    ...
    if (tickCounter >= animationMetadata.getFrameTimeSingle(frameCounter))
    {
      ...
      {
        TextureUtil.uploadTextureMipmap((int[][])framesTextureData.get(var3), width, height, originX, originY, false, false);
      }
    }
  ...
  private void fixTransparentColor(int[] data)
  {
    if (data != null)
    {
      ...
      for (int redAvg = 0; redAvg < data.length; redAvg++)
      {
        ...
        if (blueAvg != 0)
        {
          int i = greenAvg >> 16 & 0xFF;
          int col = greenAvg >> 8 & 0xFF;
          int alpha = greenAvg & 0xFF;
          redSum += i;
          greenSum += col;
          blueSum += alpha;
          count += 1L;
