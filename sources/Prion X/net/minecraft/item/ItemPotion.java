...
public class ItemPotion extends Item
{
  ...
  public List getEffects(ItemStack p_77832_1_)
  {
    ...
    {
      ArrayList var7 = com.google.common.collect.Lists.newArrayList();
    ...
    if (stack.getMetadata() != 0)
    {
      ...
      if ((var5 != null) && (!var5.isEmpty()))
      {
        ...
        while (var16.hasNext())
        {
          ...
          {
            var9 = var9 + " " + StatCollector.translateToLocal(new StringBuilder("potion.potency.").append(var8.getAmplifier()).toString()).trim();
          }
      ...
      if (!var6.isEmpty())
      {
        ...
        while (var16.hasNext())
        {
          ...
          if (var19 > 0.0D)
          {
            tooltip.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted(new StringBuilder("attribute.modifier.plus.").append(var18.getOperation()).toString(), new Object[] { ItemStack.DECIMALFORMAT.format(var20), StatCollector.translateToLocal("attribute.name." + (String)var17.getKey()) }));
          }
          else if (var19 < 0.0D)
          {
            var20 *= -1.0D;
            tooltip.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted(new StringBuilder("attribute.modifier.take.").append(var18.getOperation()).toString(), new Object[] { ItemStack.DECIMALFORMAT.format(var20), StatCollector.translateToLocal("attribute.name." + (String)var17.getKey()) }));
          }
        }
      }
    }
