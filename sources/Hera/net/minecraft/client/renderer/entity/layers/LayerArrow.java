...
/* 27 */     if (i > 0) {
/*    */       
/* 29 */       EntityArrow entityArrow = new EntityArrow(entitylivingbaseIn.worldObj, entitylivingbaseIn.posX, entitylivingbaseIn.posY, entitylivingbaseIn.posZ);
...
/* 50 */         f1 *= -1.0F;
/* 51 */         f2 *= -1.0F;
/* 52 */         float f6 = MathHelper.sqrt_float(f * f + f2 * f2);
/* 53 */         ((Entity)entityArrow).prevRotationYaw = ((Entity)entityArrow).rotationYaw = (float)(Math.atan2(f, f2) * 180.0D / Math.PI);
/* 54 */         ((Entity)entityArrow).prevRotationPitch = ((Entity)entityArrow).rotationPitch = (float)(Math.atan2(f1, f6) * 180.0D / Math.PI);
/* 55 */         double d0 = 0.0D;
/* 56 */         double d1 = 0.0D;
/* 57 */         double d2 = 0.0D;
/* 58 */         this.field_177168_a.getRenderManager().renderEntityWithPosYaw((Entity)entityArrow, d0, d1, d2, 0.0F, partialTicks);
