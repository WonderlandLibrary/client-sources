...
/*   81 */   private final TreeSet<NextTickListEntry> pendingTickListEntriesTreeSet = new TreeSet<NextTickListEntry>();
...
/*   98 */   private ServerBlockEventList[] field_147490_S = new ServerBlockEventList[] { new ServerBlockEventList(null), new ServerBlockEventList(null) };
...
/*  126 */       this.mapStorage.setData(s, (WorldSavedData)this.villageCollectionObj);
/*      */     }
/*      */     else {
/*      */       
...
/*  140 */       this.mapStorage.setData("scoreboard", (WorldSavedData)scoreboardsavedata);
/*      */     } 
/*      */     
...
/*  254 */           i++; continue;
/*      */         } 
/*  256 */         if (entityplayer.isPlayerSleeping())
/*      */         {
/*  258 */           j++;
/*      */         }
/*      */       } 
...
/*      */           {
/*  374 */             addWeatherEffect((Entity)new EntityLightningBolt(this, blockpos.getX(), blockpos.getY(), blockpos.getZ()));
/*      */           }
/*      */         } 
/*      */         
...
/*  385 */           BlockPos blockpos1 = blockpos2.down();
/*      */           
...
/*  408 */           for (m = (arrayOfExtendedBlockStorage = chunk.getBlockStorageArray()).length, b = 0; b < m; ) { ExtendedBlockStorage extendedblockstorage = arrayOfExtendedBlockStorage[b];
/*      */             
...
/*  442 */     List<EntityLivingBase> list = getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb, new Predicate<EntityLivingBase>()
/*      */         {
...
/*      */       
/*  579 */       NextTickListEntry nextticklistentry = this.pendingTickListEntriesTreeSet.first();
/*      */       
...
/*      */       
/*  597 */       NextTickListEntry nextticklistentry1 = iterator.next();
/*  598 */       iterator.remove();
/*  599 */       int k = 0;
/*      */       
...
/*      */         
/*  662 */         NextTickListEntry nextticklistentry = iterator.next();
...
/*  692 */     if (!canSpawnAnimals() && (entityIn instanceof net.minecraft.entity.passive.EntityAnimal || entityIn instanceof net.minecraft.entity.passive.EntityWaterMob))
/*      */     {
/*  694 */       entityIn.setDead();
/*      */     }
/*      */     
/*  697 */     if (!canSpawnNPCs() && entityIn instanceof net.minecraft.entity.INpc)
/*      */     {
...
/*  722 */     return (IChunkProvider)this.theChunkProviderServer;
/*      */   }
/*      */ 
/*      */   
...
/*      */       
/*  731 */       TileEntity tileentity = this.loadedTileEntityList.get(i);
...
/*      */         }
/*  771 */         catch (Throwable throwable1) {}
/*      */ 
/*      */ 
/*      */ 
/*      */         
...
/*      */       
/*  984 */       this.mcServer.getConfigurationManager().sendToAllNear(entityIn.posX, entityIn.posY, entityIn.posZ, 512.0D, this.provider.getDimensionId(), (Packet)new S2CPacketSpawnGlobalEntity(entityIn));
/*  985 */       return true;
/*      */     } 
/*      */ 
/*      */     
...
/*  998 */     getEntityTracker().func_151248_b(entityIn, (Packet)new S19PacketEntityStatus(entityIn, state));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */       {
/* 1019 */         ((EntityPlayerMP)entityplayer).playerNetServerHandler.sendPacket((Packet)new S27PacketExplosion(x, y, z, strength, explosion.getAffectedBlockPositions(), explosion.getPlayerKnockbackMap().get(entityplayer)));
/*      */       }
/*      */     } 
/*      */     
...
/*      */         {
/* 1052 */           this.mcServer.getConfigurationManager().sendToAllNear(blockeventdata.getPosition().getX(), blockeventdata.getPosition().getY(), blockeventdata.getPosition().getZ(), 64.0D, this.provider.getDimensionId(), (Packet)new S24PacketBlockAction(blockeventdata.getPosition(), blockeventdata.getBlock(), blockeventdata.getEventID(), blockeventdata.getEventParameter()));
/*      */         }
/*      */       } 
/*      */       
...
/*      */     {
/* 1084 */       this.mcServer.getConfigurationManager().sendPacketToAllPlayersInDimension((Packet)new S2BPacketChangeGameState(7, this.rainingStrength), this.provider.getDimensionId());
/*      */     }
/*      */     
/* 1087 */     if (this.prevThunderingStrength != this.thunderingStrength)
/*      */     {
/* 1089 */       this.mcServer.getConfigurationManager().sendPacketToAllPlayersInDimension((Packet)new S2BPacketChangeGameState(8, this.thunderingStrength), this.provider.getDimensionId());
/*      */     }
/*      */     
...
/* 1094 */       if (flag) {
/*      */         
/* 1096 */         this.mcServer.getConfigurationManager().sendPacketToAllPlayers((Packet)new S2BPacketChangeGameState(2, 0.0F));
/*      */       }
/*      */       else {
/*      */         
/* 1100 */         this.mcServer.getConfigurationManager().sendPacketToAllPlayers((Packet)new S2BPacketChangeGameState(1, 0.0F));
/*      */       } 
/*      */       
/* 1103 */       this.mcServer.getConfigurationManager().sendPacketToAllPlayers((Packet)new S2BPacketChangeGameState(7, this.rainingStrength));
/* 1104 */       this.mcServer.getConfigurationManager().sendPacketToAllPlayers((Packet)new S2BPacketChangeGameState(8, this.thunderingStrength));
/*      */     } 
/*      */   }
/*      */ 
/*      */   
...
/* 1149 */     S2APacketParticles s2APacketParticles = new S2APacketParticles(particleType, longDistance, (float)xCoord, (float)yCoord, (float)zCoord, (float)xOffset, (float)yOffset, (float)zOffset, (float)particleSpeed, numberOfParticles, p_180505_18_);
/*      */     
...
/*      */       {
/* 1159 */         entityplayermp.playerNetServerHandler.sendPacket((Packet)s2APacketParticles);
/*      */       }
/*      */     } 
/*      */   }
/*      */ 
...
/* 1166 */     return this.entitiesByUuid.get(uuid);
/*      */   }
/*      */ 
/*      */   
