...
public class ElementComboBox extends Element {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		// The shadow box under the title.
		...
		if (comboextended) {
			...
			for (String sld : set.getOptions()) {
				...
				/*
				 * Ist das Element ausgewhlt, wenn ja dann markiere das Element
				 * in der ComboBox
				 */
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		if (mouseButton == 0) {
			...
			/*
			 * Also wenn die Box ausgefahren ist, dann wird fr jede mgliche
			 * Options berprft, ob die Maus auf diese zeigt, wenn ja dann global
			 * jeder weitere call an mouseClicked gestoppt und die Values werden
			 * aktualisiert
			 */
			if (!comboextended)
				return false;
			...
			for (String slcd : set.getOptions()) {
				if (mouseX >= x && mouseX <= x + width && mouseY >= ay && mouseY <= ay + FontUtil.getFontHeight() + 2) {
					if (Karaguc.instance.settingsManager.getSettingByName("Sound").getValBoolean())
	...
	/*
	 * Einfacher HoverCheck, bentigt damit die Combobox geffnet und geschlossen
	 * werden kann
	 */
