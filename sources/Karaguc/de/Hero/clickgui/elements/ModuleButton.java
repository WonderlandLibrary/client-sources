...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Karaguc.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Karaguc.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Karaguc.instance.clickGUI.closeAllSettings();
				this.extended = b;
