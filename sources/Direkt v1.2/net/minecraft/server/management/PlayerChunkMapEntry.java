...
public class PlayerChunkMapEntry {
	...
	/**
	 * Provide the chunk at the player's location. Can fail, returning false, if the player is a spectator floating outside of any pre-existing chunks, and the server is not configured to allow chunk generation for spectators.
	 */
	...
	/**
	 * Send packets to player for: - nearby tile entities - nearby entities that are leashed - nearby entities with
	 */
	...
	public void updateChunkInhabitedTime() {
		long i = this.field_187282_b.getWorldServer().getTotalWorldTime();
		...
		if (this.sentToPlayers) {
			...
			if (this.changes < 64) {
				...
				for (int i = 0; i < this.changes; ++i) {
					if (this.changedBlocks[i] == short1) { return; }
				}
	...
	public boolean hasPlayerMatchingInRange(double range, Predicate<EntityPlayerMP> predicate) {
		...
		for (int j = this.players.size(); i < j; ++i) {
			...
			if (predicate.apply(entityplayermp) && (this.pos.getDistanceSq(entityplayermp) < (range * range))) { return true; }
		}
