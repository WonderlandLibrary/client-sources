...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
	...
	public void update() {
		...
		if ((this.playerEntity.getLastActiveTime() > 0L) && (this.serverController.getMaxPlayerIdleMinutes() > 0)
				&& ((MinecraftServer.getCurrentTimeMillis() - this.playerEntity.getLastActiveTime()) > this.serverController.getMaxPlayerIdleMinutes() * 1000 * 60)) {
	...
	private static boolean isMovePlayerPacketInvalid(CPacketPlayer packetIn) {
		return Doubles.isFinite(packetIn.getX(0.0D)) && Doubles.isFinite(packetIn.getY(0.0D)) && Doubles.isFinite(packetIn.getZ(0.0D)) && Floats.isFinite(packetIn.getPitch(0.0F))
				&& Floats.isFinite(packetIn.getYaw(0.0F)) ? false : (Math.abs(packetIn.getX(0.0D)) <= 3.0E7D) && (Math.abs(packetIn.getX(0.0D)) <= 3.0E7D);
	}

	private static boolean isMoveVehiclePacketInvalid(CPacketVehicleMove packetIn) {
		return !Doubles.isFinite(packetIn.getX()) || !Doubles.isFinite(packetIn.getY()) || !Doubles.isFinite(packetIn.getZ()) || !Floats.isFinite(packetIn.getPitch())
			...
			if ((entity != this.playerEntity) && (entity.getControllingPassenger() == this.playerEntity) && (entity == this.lowestRiddenEnt)) {
				...
				if (((d10 - d9) > 100.0D) && (!this.serverController.isSinglePlayer() || !this.serverController.getServerOwner().equals(entity.getName()))) {
					LOGGER.warn("{} (vehicle of {}) moved too quickly! {},{},{}",
							new Object[] { entity.getName(), this.playerEntity.getName(), Double.valueOf(d6), Double.valueOf(d7), Double.valueOf(d8) });
				...
				this.vehicleFloating = (d11 >= -0.03125D) && !this.serverController.isFlightAllowed()
						&& !worldserver.checkBlockCollision(entity.getEntityBoundingBox().expandXyz(0.0625D).addCoord(0.0D, -0.55D, 0.0D));
			...
			if (!this.playerEntity.playerConqueredTheEnd) {
				...
				if (this.targetPos != null) {
					...
					if (this.playerEntity.isRiding()) {
						this.playerEntity.setPositionAndRotation(this.playerEntity.posX, this.playerEntity.posY, this.playerEntity.posZ, packetIn.getYaw(this.playerEntity.rotationYaw),
								packetIn.getPitch(this.playerEntity.rotationPitch));
						this.serverController.getPlayerList().serverUpdateMountedMovingPlayer(this.playerEntity);
					} else {
						...
						double d10 = (this.playerEntity.motionX * this.playerEntity.motionX) + (this.playerEntity.motionY * this.playerEntity.motionY)
						...
						if (!this.playerEntity.isInvulnerableDimensionChange()
								&& (!this.playerEntity.getServerWorld().getGameRules().getBoolean("disableElytraMovementCheck") || !this.playerEntity.isElytraFlying())) {
						...
						if (!this.playerEntity.isInvulnerableDimensionChange() && (d11 > 0.0625D) && !this.playerEntity.isPlayerSleeping() && !this.playerEntity.interactionManager.isCreative()
						...
						this.floating &= !this.playerEntity.isPotionActive(MobEffects.LEVITATION) && !this.playerEntity.isElytraFlying()
								&& !worldserver.checkBlockCollision(this.playerEntity.getEntityBoundingBox().expandXyz(0.0625D).addCoord(0.0D, -0.55D, 0.0D));
						this.playerEntity.onGround = packetIn.isOnGround();
						this.serverController.getPlayerList().serverUpdateMountedMovingPlayer(this.playerEntity);
	...
	public void processRightClickBlock(CPacketPlayerTryUseItemOnBlock packetIn) {
		...
		if ((blockpos.getY() < (this.serverController.getBuildLimit() - 1)) || ((enumfacing != EnumFacing.UP) && (blockpos.getY() < this.serverController.getBuildLimit()))) {
			if ((this.targetPos == null) && (this.playerEntity.getDistanceSq(blockpos.getX() + 0.5D, blockpos.getY() + 0.5D, blockpos.getZ() + 0.5D) < 64.0D)
					&& !this.serverController.isBlockProtected(worldserver, blockpos, this.playerEntity) && worldserver.getWorldBorder().contains(blockpos)) {
				this.playerEntity.interactionManager.processRightClickBlock(this.playerEntity, worldserver, itemstack, enumhand, blockpos, enumfacing, packetIn.getFacingX(), packetIn.getFacingY(),
						packetIn.getFacingZ());
			}
		} else {
		...
		if (this.playerEntity.isSpectator()) {
			...
			if (entity != null) {
				...
				if (entity.worldObj == this.playerEntity.worldObj) {
					...
					this.sendPacket(new SPacketRespawn(this.playerEntity.dimension, worldserver1.getDifficulty(), worldserver1.getWorldInfo().getTerrainType(),
							this.playerEntity.interactionManager.getGameType()));
	...
	public void sendPacket(final Packet<?> packetIn) {
		if (packetIn instanceof SPacketChat) {
			...
			if (entityplayer$enumchatvisibility == EntityPlayer.EnumChatVisibility.HIDDEN) { return; }

			if ((entityplayer$enumchatvisibility == EntityPlayer.EnumChatVisibility.SYSTEM) && !spacketchat.isSystem()) { return; }
		}
	...
	public void processConfirmTransaction(CPacketConfirmTransaction packetIn) {
		...
		if ((oshort != null) && (packetIn.getUid() == oshort.shortValue()) && (this.playerEntity.openContainer.windowId == packetIn.getWindowId())
	...
	public void processCustomPayload(CPacketCustomPayload packetIn) {
		...
		if ("MC|BEdit".equals(s)) {
			...
			try {
				...
				if (!ItemWritableBook.isNBTValid(itemstack.getTagCompound())) { throw new IOException("Invalid book tag!"); }
			...
			try {
				...
				if (!ItemWrittenBook.validBookTagContents(itemstack2.getTagCompound())) { throw new IOException("Invalid book tag!"); }

				ItemStack itemstack3 = this.playerEntity.getHeldItemMainhand();

				if (itemstack3 == null) { return; }
			...
			try {
				...
				if (commandblockbaselogic != null) {
					EnumFacing enumfacing = this.playerEntity.worldObj.getBlockState(blockpos1).getValue(BlockCommandBlock.FACING);
					...
					case SEQUENCE:
						...
						this.playerEntity.worldObj.setBlockState(blockpos1,
								iblockstate3.withProperty(BlockCommandBlock.FACING, enumfacing).withProperty(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(flag)), 2);
						break;

					case AUTO:
						IBlockState lvt_14_2_ = Blocks.REPEATING_COMMAND_BLOCK.getDefaultState();
						this.playerEntity.worldObj.setBlockState(blockpos1,
								lvt_14_2_.withProperty(BlockCommandBlock.FACING, enumfacing).withProperty(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(flag)), 2);
						break;

					case REDSTONE:
						IBlockState lvt_14_1_ = Blocks.COMMAND_BLOCK.getDefaultState();
						this.playerEntity.worldObj.setBlockState(blockpos1,
								lvt_14_1_.withProperty(BlockCommandBlock.FACING, enumfacing).withProperty(BlockCommandBlock.CONDITIONAL, Boolean.valueOf(flag)), 2);
					}
			...
			if (!this.playerEntity.func_189808_dh()) { return; }
			...
			try {
				...
				this.playerEntity.connection
						.sendPacket(new SPacketSetSlot(-2, this.playerEntity.inventory.currentItem, this.playerEntity.inventory.getStackInSlot(this.playerEntity.inventory.currentItem)));
