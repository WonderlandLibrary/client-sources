...
public class BlockChest extends BlockContainer {
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		...
		if (worldIn.getBlockState(blockpos).getBlock() == this) {
			if (this.isDoubleChest(worldIn, blockpos)) { return false; }

			++i;
		}

		if (worldIn.getBlockState(blockpos1).getBlock() == this) {
			if (this.isDoubleChest(worldIn, blockpos1)) { return false; }

			++i;
		}

		if (worldIn.getBlockState(blockpos2).getBlock() == this) {
			if (this.isDoubleChest(worldIn, blockpos2)) { return false; }

			++i;
		}

		if (worldIn.getBlockState(blockpos3).getBlock() == this) {
			if (this.isDoubleChest(worldIn, blockpos3)) { return false; }
	...
	private boolean isDoubleChest(World worldIn, BlockPos pos) {
		if (worldIn.getBlockState(pos).getBlock() != this) {
			...
			for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
				if (worldIn.getBlockState(pos.offset(enumfacing)).getBlock() == this) { return true; }
			}
	...
	public ILockableContainer getLockableContainer(World worldIn, BlockPos pos) {
		return this.func_189418_a(worldIn, pos, false);
	}
		...
		if (!(tileentity instanceof TileEntityChest)) {
			...
			if (!p_189418_3_ && this.isBlocked(p_189418_1_, p_189418_2_)) {
				...
				for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
					...
					if (block == this) {
						if (this.isBlocked(p_189418_1_, blockpos)) { return null; }
