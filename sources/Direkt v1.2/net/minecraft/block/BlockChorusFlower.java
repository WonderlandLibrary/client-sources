...
public class BlockChorusFlower extends Block {
	...
	private static boolean areAllNeighborsEmpty(World p_185604_0_, BlockPos p_185604_1_, EnumFacing p_185604_2_) {
		for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
			if ((enumfacing != p_185604_2_) && !p_185604_0_.isAirBlock(p_185604_1_.offset(enumfacing))) { return false; }
		}
		...
		if ((block != Blocks.CHORUS_PLANT) && (block != Blocks.END_STONE)) {
			if (iblockstate.getMaterial() == Material.AIR) {
				...
				for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
					...
					if (block1 == Blocks.CHORUS_PLANT) {
						++i;
					} else if (iblockstate1.getMaterial() != Material.AIR) { return false; }
				}
	...
	private static void growTreeRecursive(World worldIn, BlockPos p_185601_1_, Random rand, BlockPos p_185601_3_, int p_185601_4_, int p_185601_5_) {
		...
		for (int j = 0; j < i; ++j) {
			...
			if (!areAllNeighborsEmpty(worldIn, blockpos, (EnumFacing) null)) { return; }
		...
		if (p_185601_5_ < 4) {
			...
			for (int k = 0; k < l; ++k) {
				...
				if ((Math.abs(blockpos1.getX() - p_185601_3_.getX()) < p_185601_4_) && (Math.abs(blockpos1.getZ() - p_185601_3_.getZ()) < p_185601_4_) && worldIn.isAirBlock(blockpos1)
						&& worldIn.isAirBlock(blockpos1.down()) && areAllNeighborsEmpty(worldIn, blockpos1, enumfacing.getOpposite())) {
					flag = true;
