...
public class BlockPistonStructureHelper {
	...
	public boolean canMove() {
		...
		if (!BlockPistonBase.canPush(iblockstate, this.world, this.blockToMove, this.moveDirection, false)) {
			...
			for (int i = 0; i < this.toMove.size(); ++i) {
				...
				if ((this.world.getBlockState(blockpos).getBlock() == Blocks.SLIME_BLOCK) && !this.addBranchingBlocks(blockpos)) { return false; }
			}
		...
		if (iblockstate.getMaterial() == Material.AIR) {
			...
			if ((i + this.toMove.size()) > 12) {
				...
				while (block == Blocks.SLIME_BLOCK) {
					...
					if ((iblockstate.getMaterial() == Material.AIR) || !BlockPistonBase.canPush(iblockstate, this.world, blockpos, this.moveDirection, false) || blockpos.equals(this.pistonPos)) {
						break;
					}
				...
				while (true) {
					...
					if (k > -1) {
						...
						for (int l = 0; l <= (k + i1); ++l) {
							...
							if ((this.world.getBlockState(blockpos2).getBlock() == Blocks.SLIME_BLOCK) && !this.addBranchingBlocks(blockpos2)) { return false; }
						}
					...
					if (iblockstate.getMaterial() == Material.AIR) { return true; }

					if (!BlockPistonBase.canPush(iblockstate, this.world, blockpos1, this.moveDirection, true) || blockpos1.equals(this.pistonPos)) { return false; }
					...
					if (this.toMove.size() >= 12) { return false; }
	...
	private boolean addBranchingBlocks(BlockPos p_177250_1_) {
		for (EnumFacing enumfacing : EnumFacing.values()) {
			if ((enumfacing.getAxis() != this.moveDirection.getAxis()) && !this.addBlockLine(p_177250_1_.offset(enumfacing))) { return false; }
		}
