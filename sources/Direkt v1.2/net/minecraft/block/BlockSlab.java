...
public abstract class BlockSlab extends Block {
	...
	public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		...
		return this.isDouble() ? iblockstate
				: ((facing != EnumFacing.DOWN) && ((facing == EnumFacing.UP) || (hitY <= 0.5D)) ? iblockstate : iblockstate.withProperty(HALF, BlockSlab.EnumBlockHalf.TOP));
	}
	...
	public boolean shouldSideBeRendered(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
		if (this.isDouble()) {
			...
			return flag1 ? (side == EnumFacing.DOWN ? true : ((side == EnumFacing.UP) && super.shouldSideBeRendered(blockState, blockAccess, pos, side) ? true : !isHalfSlab(iblockstate) || !flag))
					: (side == EnumFacing.UP ? true : ((side == EnumFacing.DOWN) && super.shouldSideBeRendered(blockState, blockAccess, pos, side) ? true : !isHalfSlab(iblockstate) || flag));
		}
	}
