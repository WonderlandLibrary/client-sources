...
public class TileEntityBrewingStand extends TileEntityLockable implements ITickable, ISidedInventory {
	...
	public void update() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (!Arrays.equals(aboolean, this.filledSlots)) {
				...
				if (!(iblockstate.getBlock() instanceof BlockBrewingStand)) { return; }
	...
	/**
	 * Creates an array of boolean values, each value represents a potion input slot, value is true if the slot is not null.
	 */
		...
		if ((this.brewingItemStacks[3] != null) && (this.brewingItemStacks[3].stackSize > 0)) {
			...
			if (!PotionHelper.isReagent(itemstack)) {
				...
				for (int i = 0; i < 3; ++i) {
					...
					if ((itemstack1 != null) && PotionHelper.hasConversions(itemstack1, itemstack)) { return true; }
				}
