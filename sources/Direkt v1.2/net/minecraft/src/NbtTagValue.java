...
public class NbtTagValue {
	...
	public NbtTagValue(String p_i51_1_, String p_i51_2_) {
		String[] astring = Config.tokenize(p_i51_1_, ".");
		this.parents = Arrays.copyOfRange(astring, 0, astring.length - 1);
		this.name = astring[astring.length - 1];

		if (p_i51_2_.startsWith("pattern:")) {
			this.type = 1;
			p_i51_2_ = p_i51_2_.substring("pattern:".length());
		} else if (p_i51_2_.startsWith("ipattern:")) {
			this.type = 2;
			p_i51_2_ = p_i51_2_.substring("ipattern:".length()).toLowerCase();
		} else if (p_i51_2_.startsWith("regex:")) {
			this.type = 3;
			p_i51_2_ = p_i51_2_.substring("regex:".length());
		} else if (p_i51_2_.startsWith("iregex:")) {
			this.type = 4;
			p_i51_2_ = p_i51_2_.substring("iregex:".length()).toLowerCase();
		} else {
			this.type = 0;
		}

		p_i51_2_ = StringEscapeUtils.unescapeJava(p_i51_2_);
		this.value = p_i51_2_;
	}
	...
	private boolean matchesAnyChild(NBTBase p_matchesAnyChild_1_) {
		if (p_matchesAnyChild_1_ instanceof NBTTagCompound) {
			...
			for (String s : nbttagcompound.getKeySet()) {
				...
				if (this.matches(nbtbase)) { return true; }
			}
		}
		...
		if (p_matchesAnyChild_1_ instanceof NBTTagList) {
			...
			for (int j = 0; j < i; ++j) {
				...
				if (this.matches(nbtbase1)) { return true; }
			}
		}
