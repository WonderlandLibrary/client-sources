...
public class HttpPipeline {
	private static Map<String, HttpPipelineConnection> mapConnections = new HashMap();
	...
	public static void addRequest(HttpPipelineRequest p_addRequest_0_) {
		...
		for (HttpPipelineConnection httppipelineconnection = getConnection(httprequest.getHost(), httprequest.getPort(), httprequest.getProxy()); !httppipelineconnection
				.addRequest(p_addRequest_0_); httppipelineconnection = getConnection(httprequest.getHost(), httprequest.getPort(), httprequest.getProxy())) {
	...
	public static boolean hasActiveRequests() {
		for (HttpPipelineConnection httppipelineconnection : mapConnections.values()) {
			if (httppipelineconnection.hasActiveRequests()) { return true; }
		}
