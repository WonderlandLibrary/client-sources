...
public class SmartLeaves {
	...
	public static IBakedModel getLeavesModel(IBakedModel p_getLeavesModel_0_) {
		if (!Config.isTreesSmart()) {
			...
			List list = p_getLeavesModel_0_.getQuads((IBlockState) null, (EnumFacing) null, 0L);
					...
					: (list == generalQuadsCullBirch ? modelLeavesDoubleBirch
							: (list == generalQuadsCullDarkOak ? modelLeavesDoubleDarkOak
									: (list == generalQuadsCullJungle ? modelLeavesDoubleJungle
											: (list == generalQuadsCullOak ? modelLeavesDoubleOak : (list == generalQuadsCullSpruce ? modelLeavesDoubleSpruce : p_getLeavesModel_0_)))));
		}
	}
			...
			if (Config.getDefiningResourcePack(resourcelocation) != Config.getDefaultResourcePack()) {
				...
				if (Config.getDefiningResourcePack(resourcelocation1) != Config.getDefaultResourcePack()) {
					...
					if ((ibakedmodel != null) && (ibakedmodel != modelmanager.getMissingModel())) {
						List<BakedQuad> list = ibakedmodel.getQuads((IBlockState) null, (EnumFacing) null, 0L);
