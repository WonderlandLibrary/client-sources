...
public class CustomSkyLayer {
	...
	public CustomSkyLayer(Properties p_i24_1_, String p_i24_2_) {
		this.axis = DEFAULT_AXIS;
		this.days = null;
		this.daysLoop = 8;
		this.textureId = -1;
		ConnectedParser connectedparser = new ConnectedParser("CustomSky");
		this.source = p_i24_1_.getProperty("source", p_i24_2_);
		this.startFadeIn = this.parseTime(p_i24_1_.getProperty("startFadeIn"));
		this.endFadeIn = this.parseTime(p_i24_1_.getProperty("endFadeIn"));
		this.startFadeOut = this.parseTime(p_i24_1_.getProperty("startFadeOut"));
		this.endFadeOut = this.parseTime(p_i24_1_.getProperty("endFadeOut"));
		this.blend = Blender.parseBlend(p_i24_1_.getProperty("blend"));
		this.rotate = this.parseBoolean(p_i24_1_.getProperty("rotate"), true);
		this.speed = this.parseFloat(p_i24_1_.getProperty("speed"), 1.0F);
		this.axis = this.parseAxis(p_i24_1_.getProperty("axis"), DEFAULT_AXIS);
		this.days = connectedparser.parseRangeListInt(p_i24_1_.getProperty("days"));
		this.daysLoop = connectedparser.parseInt(p_i24_1_.getProperty("daysLoop"), 8);
	}
	...
	private void renderSide(Tessellator p_renderSide_1_, int p_renderSide_2_) {
		VertexBuffer vertexbuffer = p_renderSide_1_.getBuffer();
		double d0 = p_renderSide_2_ % 3 / 3.0D;
		double d1 = p_renderSide_2_ / 3 / 2.0D;
		vertexbuffer.begin(7, DefaultVertexFormats.POSITION_TEX);
		vertexbuffer.pos(-100.0D, -100.0D, -100.0D).tex(d0, d1).endVertex();
		vertexbuffer.pos(-100.0D, -100.0D, 100.0D).tex(d0, d1 + 0.5D).endVertex();
		vertexbuffer.pos(100.0D, -100.0D, 100.0D).tex(d0 + 0.3333333333333333D, d1 + 0.5D).endVertex();
		vertexbuffer.pos(100.0D, -100.0D, -100.0D).tex(d0 + 0.3333333333333333D, d1).endVertex();
	...
	public boolean isActive(World p_isActive_1_, int p_isActive_2_) {
		if (this.timeBetween(p_isActive_2_, this.endFadeOut, this.startFadeIn)) {
			...
			if (this.days != null) {
				...
				if (!this.days.isInRange(l)) { return false; }
			}
	...
	private boolean timeBetween(int p_timeBetween_1_, int p_timeBetween_2_, int p_timeBetween_3_) {
		return p_timeBetween_2_ <= p_timeBetween_3_ ? (p_timeBetween_1_ >= p_timeBetween_2_) && (p_timeBetween_1_ <= p_timeBetween_3_)
