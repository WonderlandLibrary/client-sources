...
public class CustomItems {
	...
	private static boolean use3Glint = true;
	...
	private static void updateIcons(TextureMap p_updateIcons_0_, IResourcePack p_updateIcons_1_) {
		String[] astring = ResUtils.collectFiles(p_updateIcons_1_, "mcpatcher/cit/", ".properties", (String[]) null);
			...
			public int compare(Object p_compare_1_, Object p_compare_2_) {
				...
				return customitemproperties.layer != customitemproperties1.layer ? customitemproperties.layer - customitemproperties1.layer
						: (customitemproperties.weight != customitemproperties1.weight ? customitemproperties1.weight - customitemproperties.weight
								: (!customitemproperties.basePath.equals(customitemproperties1.basePath) ? customitemproperties.basePath.compareTo(customitemproperties1.basePath)
	...
	private static int getPotionNameDamage(String p_getPotionNameDamage_0_) {
		...
		for (ResourceLocation resourcelocation : Potion.REGISTRY.getKeys()) {
			...
			if (s.equals(s1)) { return Potion.getIdFromPotion(potion); }
		}
	...
	private static List makePropertyList(CustomItemProperties[][] p_makePropertyList_0_) {
		...
		if (p_makePropertyList_0_ != null) {
			for (CustomItemProperties[] acustomitemproperties : p_makePropertyList_0_) {
				List list1 = null;
	...
	private static void addToItemList(CustomItemProperties p_addToItemList_0_, List p_addToItemList_1_) {
		if (p_addToItemList_0_.items != null) {
			for (int j : p_addToItemList_0_.items) {
				if (j <= 0) {
	...
	public static ResourceLocation getCustomElytraTexture(ItemStack p_getCustomElytraTexture_0_, ResourceLocation p_getCustomElytraTexture_1_) {
		if (itemProperties == null) {
			...
			return customitemproperties == null ? p_getCustomElytraTexture_1_ : (customitemproperties.textureLocation == null ? p_getCustomElytraTexture_1_ : customitemproperties.textureLocation);
		}
	}
	...
	private static CustomItemProperties getCustomItemProperties(ItemStack p_getCustomItemProperties_0_, int p_getCustomItemProperties_1_) {
		if (itemProperties == null) {
			...
			if ((i >= 0) && (i < itemProperties.length)) {
				...
				if (acustomitemproperties != null) {
					for (CustomItemProperties customitemproperties : acustomitemproperties) {
						if ((customitemproperties.type == p_getCustomItemProperties_1_)
								&& matchesProperties(customitemproperties, p_getCustomItemProperties_0_, (int[][]) null)) { return customitemproperties; }
					}
				}
			}
	...
	private static boolean matchesProperties(CustomItemProperties p_matchesProperties_0_, ItemStack p_matchesProperties_1_, int[][] p_matchesProperties_2_) {
		...
		if (p_matchesProperties_0_.damage != null) {
			...
			if (!p_matchesProperties_0_.damage.isInRange(i)) { return false; }
		}
		...
		if ((p_matchesProperties_0_.stackSize != null) && !p_matchesProperties_0_.stackSize.isInRange(p_matchesProperties_1_.stackSize)) {
			...
			if (p_matchesProperties_0_.nbtTagValues != null) {
				...
				for (NbtTagValue nbtTagValue2 : p_matchesProperties_0_.nbtTagValues) {
					NbtTagValue nbttagvalue = nbtTagValue2;

					if (!nbttagvalue.matches(nbttagcompound)) { return false; }
				}
			}
	...
	public static boolean renderCustomArmorEffect(EntityLivingBase p_renderCustomArmorEffect_0_, ItemStack p_renderCustomArmorEffect_1_, ModelBase p_renderCustomArmorEffect_2_,
			float p_renderCustomArmorEffect_3_, float p_renderCustomArmorEffect_4_, float p_renderCustomArmorEffect_5_, float p_renderCustomArmorEffect_6_, float p_renderCustomArmorEffect_7_,
			float p_renderCustomArmorEffect_8_, float p_renderCustomArmorEffect_9_) {
		if (enchantmentProperties == null) {
			return false;
	...
	public static boolean setUseGlint(boolean glint) {
		 use3Glint = glint;
		 return false;
	}
	public static boolean get3UseGlint() {
		return use3Glint;
	}
}
