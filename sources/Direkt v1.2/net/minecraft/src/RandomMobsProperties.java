...
public class RandomMobsProperties {
	...
	public RandomMobsProperties(String p_i61_1_, ResourceLocation[] p_i61_2_) {
		ConnectedParser connectedparser = new ConnectedParser("RandomMobs");
		this.name = connectedparser.parseName(p_i61_1_);
		this.basePath = connectedparser.parseBasePath(p_i61_1_);
		this.resourceLocations = p_i61_2_;
	}

	public RandomMobsProperties(Properties p_i62_1_, String p_i62_2_, ResourceLocation p_i62_3_) {
		ConnectedParser connectedparser = new ConnectedParser("RandomMobs");
		this.name = connectedparser.parseName(p_i62_2_);
		this.basePath = connectedparser.parseBasePath(p_i62_2_);
		this.rules = this.parseRules(p_i62_1_, p_i62_3_, connectedparser);
	}

	public ResourceLocation getTextureLocation(ResourceLocation p_getTextureLocation_1_, EntityLiving p_getTextureLocation_2_) {
		if (this.rules != null) {
			for (RandomMobsRule randommobsrule : this.rules) {
				if (randommobsrule.matches(p_getTextureLocation_2_)) { return randommobsrule.getTextureLocation(p_getTextureLocation_1_, p_getTextureLocation_2_.randomMobsId); }
			}
		}
	...
	public boolean isValid(String p_isValid_1_) {
		if ((this.resourceLocations == null) && (this.rules == null)) {
			...
			if (this.rules != null) {
				for (RandomMobsRule rule : this.rules) {
					RandomMobsRule randommobsrule = rule;

					if (!randommobsrule.isValid(p_isValid_1_)) { return false; }
				}
			}

			if (this.resourceLocations != null) {
				for (ResourceLocation resourceLocation2 : this.resourceLocations) {
