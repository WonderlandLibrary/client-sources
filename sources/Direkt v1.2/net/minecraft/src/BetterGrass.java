...
public class BetterGrass {
	...
	public static List getFaceQuads(IBlockAccess p_getFaceQuads_0_, IBlockState p_getFaceQuads_1_, BlockPos p_getFaceQuads_2_, EnumFacing p_getFaceQuads_3_, List p_getFaceQuads_4_) {
		if ((p_getFaceQuads_3_ != EnumFacing.UP) && (p_getFaceQuads_3_ != EnumFacing.DOWN)) {
			...
			return block instanceof BlockMycelium ? getFaceQuadsMycelium(p_getFaceQuads_0_, p_getFaceQuads_1_, p_getFaceQuads_2_, p_getFaceQuads_3_, p_getFaceQuads_4_)
					: (block instanceof BlockGrassPath ? getFaceQuadsGrassPath(p_getFaceQuads_0_, p_getFaceQuads_1_, p_getFaceQuads_2_, p_getFaceQuads_3_, p_getFaceQuads_4_)
							: (block instanceof BlockDirt ? getFaceQuadsDirt(p_getFaceQuads_0_, p_getFaceQuads_1_, p_getFaceQuads_2_, p_getFaceQuads_3_, p_getFaceQuads_4_)
									: (block instanceof BlockGrass ? getFaceQuadsGrass(p_getFaceQuads_0_, p_getFaceQuads_1_, p_getFaceQuads_2_, p_getFaceQuads_3_, p_getFaceQuads_4_)
											: p_getFaceQuads_4_)));
		} else {
	...
	private static List getFaceQuadsMycelium(IBlockAccess p_getFaceQuadsMycelium_0_, IBlockState p_getFaceQuadsMycelium_1_, BlockPos p_getFaceQuadsMycelium_2_, EnumFacing p_getFaceQuadsMycelium_3_,
			List p_getFaceQuadsMycelium_4_) {
		...
		if (Config.isBetterGrassFancy()) {
			if (flag) {
				if (betterMyceliumSnow && (getBlockAt(p_getFaceQuadsMycelium_2_, p_getFaceQuadsMycelium_3_, p_getFaceQuadsMycelium_0_) == Blocks.SNOW_LAYER)) { return modelCubeSnow
						.getQuads(p_getFaceQuadsMycelium_1_, p_getFaceQuadsMycelium_3_, 0L); }
			} else if (betterMycelium && (getBlockAt(p_getFaceQuadsMycelium_2_.down(), p_getFaceQuadsMycelium_3_, p_getFaceQuadsMycelium_0_) == Blocks.MYCELIUM)) { return modelCubeMycelium
					.getQuads(p_getFaceQuadsMycelium_1_, p_getFaceQuadsMycelium_3_, 0L); }
		} else if (flag) {
			if (betterMyceliumSnow) { return modelCubeSnow.getQuads(p_getFaceQuadsMycelium_1_, p_getFaceQuadsMycelium_3_, 0L); }
		} else if (betterMycelium) { return modelCubeMycelium.getQuads(p_getFaceQuadsMycelium_1_, p_getFaceQuadsMycelium_3_, 0L); }

		return p_getFaceQuadsMycelium_4_;
	}

	private static List getFaceQuadsGrassPath(IBlockAccess p_getFaceQuadsGrassPath_0_, IBlockState p_getFaceQuadsGrassPath_1_, BlockPos p_getFaceQuadsGrassPath_2_,
			EnumFacing p_getFaceQuadsGrassPath_3_, List p_getFaceQuadsGrassPath_4_) {
		return !betterGrassPath ? p_getFaceQuadsGrassPath_4_
				: (Config.isBetterGrassFancy()
						? (getBlockAt(p_getFaceQuadsGrassPath_2_.down(), p_getFaceQuadsGrassPath_3_, p_getFaceQuadsGrassPath_0_) == Blocks.GRASS_PATH
								? modelGrassPath.getQuads(p_getFaceQuadsGrassPath_1_, p_getFaceQuadsGrassPath_3_, 0L) : p_getFaceQuadsGrassPath_4_)
						: modelGrassPath.getQuads(p_getFaceQuadsGrassPath_1_, p_getFaceQuadsGrassPath_3_, 0L));
	}

	private static List getFaceQuadsDirt(IBlockAccess p_getFaceQuadsDirt_0_, IBlockState p_getFaceQuadsDirt_1_, BlockPos p_getFaceQuadsDirt_2_, EnumFacing p_getFaceQuadsDirt_3_,
			List p_getFaceQuadsDirt_4_) {
		...
		if (p_getFaceQuadsDirt_1_.getValue(BlockDirt.VARIANT) != BlockDirt.DirtType.PODZOL) {
			return block == Blocks.GRASS_PATH ? (betterGrassPath && (getBlockAt(p_getFaceQuadsDirt_2_, p_getFaceQuadsDirt_3_, p_getFaceQuadsDirt_0_) == Blocks.GRASS_PATH)
					? modelCubeGrassPath.getQuads(p_getFaceQuadsDirt_1_, p_getFaceQuadsDirt_3_, 0L) : p_getFaceQuadsDirt_4_) : p_getFaceQuadsDirt_4_;
		} else {
			boolean flag = (block == Blocks.SNOW) || (block == Blocks.SNOW_LAYER);

			if (Config.isBetterGrassFancy()) {
				if (flag) {
					if (betterPodzolSnow && (getBlockAt(p_getFaceQuadsDirt_2_, p_getFaceQuadsDirt_3_, p_getFaceQuadsDirt_0_) == Blocks.SNOW_LAYER)) { return modelCubeSnow
							.getQuads(p_getFaceQuadsDirt_1_, p_getFaceQuadsDirt_3_, 0L); }
					...
					if ((iblockstate.getBlock() == Blocks.DIRT)
							&& (iblockstate.getValue(BlockDirt.VARIANT) == BlockDirt.DirtType.PODZOL)) { return modelCubePodzol.getQuads(p_getFaceQuadsDirt_1_, p_getFaceQuadsDirt_3_, 0L); }
				}
			} else if (flag) {
				if (betterPodzolSnow) { return modelCubeSnow.getQuads(p_getFaceQuadsDirt_1_, p_getFaceQuadsDirt_3_, 0L); }
			} else if (betterPodzol) { return modelCubePodzol.getQuads(p_getFaceQuadsDirt_1_, p_getFaceQuadsDirt_3_, 0L); }

			return p_getFaceQuadsDirt_4_;
		}
	}

	private static List getFaceQuadsGrass(IBlockAccess p_getFaceQuadsGrass_0_, IBlockState p_getFaceQuadsGrass_1_, BlockPos p_getFaceQuadsGrass_2_, EnumFacing p_getFaceQuadsGrass_3_,
			List p_getFaceQuadsGrass_4_) {
		Block block = p_getFaceQuadsGrass_0_.getBlockState(p_getFaceQuadsGrass_2_.up()).getBlock();
		boolean flag = (block == Blocks.SNOW) || (block == Blocks.SNOW_LAYER);

		if (Config.isBetterGrassFancy()) {
			if (flag) {
				if (betterGrassSnow && (getBlockAt(p_getFaceQuadsGrass_2_, p_getFaceQuadsGrass_3_, p_getFaceQuadsGrass_0_) == Blocks.SNOW_LAYER)) { return modelCubeSnow
						.getQuads(p_getFaceQuadsGrass_1_, p_getFaceQuadsGrass_3_, 0L); }
			} else if (betterGrass && (getBlockAt(p_getFaceQuadsGrass_2_.down(), p_getFaceQuadsGrass_3_, p_getFaceQuadsGrass_0_) == Blocks.GRASS)) { return modelCubeGrass
					.getQuads(p_getFaceQuadsGrass_1_, p_getFaceQuadsGrass_3_, 0L); }
		} else if (flag) {
			if (betterGrassSnow) { return modelCubeSnow.getQuads(p_getFaceQuadsGrass_1_, p_getFaceQuadsGrass_3_, 0L); }
		} else if (betterGrass) { return modelCubeGrass.getQuads(p_getFaceQuadsGrass_1_, p_getFaceQuadsGrass_3_, 0L); }
