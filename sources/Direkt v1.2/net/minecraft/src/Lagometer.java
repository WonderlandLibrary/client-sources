...
public class Lagometer {
	...
	public static boolean updateMemoryAllocation() {
		...
		if (j < memLast) {
			double d0 = memDiff / 1000000.0D;
			double d1 = memTimeDiffMs / 1000.0D;
			int k = (int) (d0 / d1);
			...
			if (gameSettings.ofLagometer || gameSettings.showLagometer) {
				...
				for (int j = 0; j < timesFrame.length; ++j) {
					...
					if (gcs[j]) {
						...
						f = f - renderTime(j, timesServer[j], k / 2, k / 2, k / 2, f, vertexbuffer);
						f = f - renderTime(j, timesTerrain[j], 0, k, 0, f, vertexbuffer);
						f = f - renderTime(j, timesVisibility[j], k, k, 0, f, vertexbuffer);
						f = f - renderTime(j, timesChunkUpdate[j], k, 0, 0, f, vertexbuffer);
						f = f - renderTime(j, timesChunkUpload[j], k, 0, k, f, vertexbuffer);
						f = f - renderTime(j, timesScheduledExecutables[j], 0, 0, k, f, vertexbuffer);
						float f2 = f - renderTime(j, timesTick[j], 0, k, k, f, vertexbuffer);
					}
				}

				renderTimeDivider(0, timesFrame.length, 33333333L, 196, 196, 196, mc.displayHeight, vertexbuffer);
				renderTimeDivider(0, timesFrame.length, 16666666L, 196, 196, 196, mc.displayHeight, vertexbuffer);
				tessellator.draw();
				...
				GlStateManager.popMatrix();
				GlStateManager.enableTexture2D();
				float f1 = 1.0F - (float) ((System.currentTimeMillis() - memTimeStartMs) / 1000.0D);
	...
	private static long renderTime(int p_renderTime_0_, long p_renderTime_1_, int p_renderTime_3_, int p_renderTime_4_, int p_renderTime_5_, float p_renderTime_6_, VertexBuffer p_renderTime_7_) {
		...
		if (i < 3L) {
			...
			p_renderTime_7_.pos(p_renderTime_0_ + 0.5F, (p_renderTime_6_ - i) + 0.5F, 0.0D).color(p_renderTime_3_, p_renderTime_4_, p_renderTime_5_, 255).endVertex();
			p_renderTime_7_.pos(p_renderTime_0_ + 0.5F, p_renderTime_6_ + 0.5F, 0.0D).color(p_renderTime_3_, p_renderTime_4_, p_renderTime_5_, 255).endVertex();
			return i;
		}
	}
	...
	private static long renderTimeDivider(int p_renderTimeDivider_0_, int p_renderTimeDivider_1_, long p_renderTimeDivider_2_, int p_renderTimeDivider_4_, int p_renderTimeDivider_5_,
			int p_renderTimeDivider_6_, float p_renderTimeDivider_7_, VertexBuffer p_renderTimeDivider_8_) {
		long i = p_renderTimeDivider_2_ / 200000L;

		if (i < 3L) {
			return 0L;
		} else {
			p_renderTimeDivider_8_.pos(p_renderTimeDivider_0_ + 0.5F, (p_renderTimeDivider_7_ - i) + 0.5F, 0.0D).color(p_renderTimeDivider_4_, p_renderTimeDivider_5_, p_renderTimeDivider_6_, 255)
					.endVertex();
			p_renderTimeDivider_8_.pos(p_renderTimeDivider_1_ + 0.5F, (p_renderTimeDivider_7_ - i) + 0.5F, 0.0D).color(p_renderTimeDivider_4_, p_renderTimeDivider_5_, p_renderTimeDivider_6_, 255)
					.endVertex();
			return i;
		}
	}
