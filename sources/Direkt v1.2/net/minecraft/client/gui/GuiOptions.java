...
import us.dev.direkt.gui.font.GuiFontPicker;
	...
	public void initGui() {
		...
		for (GameSettings.Options gamesettings$options : SCREEN_OPTIONS) {
			if (gamesettings$options.getEnumFloat()) {
				this.buttonList.add(
						new GuiOptionSlider(gamesettings$options.returnEnumOrdinal(), ((this.width / 2) - 155) + ((i % 2) * 160), ((this.height / 6) - 12) + (24 * (i >> 1)), gamesettings$options));
			} else {
				GuiOptionButton guioptionbutton = new GuiOptionButton(gamesettings$options.returnEnumOrdinal(), ((this.width / 2) - 155) + ((i % 2) * 160), ((this.height / 6) - 12) + (24 * (i >> 1)),
		...
		if (this.mc.theWorld != null) {
			...
			this.buttonList.add(new GuiOptionButton(GameSettings.Options.REALMS_NOTIFICATIONS.returnEnumOrdinal(), ((this.width / 2) - 155) + ((i % 2) * 160),
					((this.height / 6) - 12) + (24 * (i >> 1)), GameSettings.Options.REALMS_NOTIFICATIONS, this.settings.getKeyBinding(GameSettings.Options.REALMS_NOTIFICATIONS)));
		}
		...
		//        TODO: Direkt: Font Selector Interface
        this.buttonList.add(new GuiButton(102, this.width / 2 - 155, this.height / 6 + 96 - 6, 150, 20, "Client Font..."));
		...
		if (button.enabled) {
			...
			if (button.id == 109) {
				this.mc.displayGuiScreen(
						new GuiYesNo(this, (new TextComponentTranslation("difficulty.lock.title", new Object[0])).getFormattedText(),
								(new TextComponentTranslation("difficulty.lock.question",
										new Object[] { new TextComponentTranslation(this.mc.theWorld.getWorldInfo().getDifficulty().getDifficultyResourceKey(), new Object[0]) })).getFormattedText(),
								109));
			}
            ...
            {
                this.mc.gameSettings.saveOptions();
//                TODO: Direkt: Font Selection Interface
                this.mc.displayGuiScreen(new GuiFontPicker(this));
