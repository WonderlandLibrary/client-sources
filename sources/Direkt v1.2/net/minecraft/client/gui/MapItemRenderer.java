		...
		private void render(boolean noOverlayRendering) {
			...
			for (Vec4b vec4b : this.mapData.mapDecorations.values()) {
				if (!noOverlayRendering || (vec4b.getType() == 1)) {
					...
					GlStateManager.translate(0.0F + (vec4b.getX() / 2.0F) + 64.0F, 0.0F + (vec4b.getY() / 2.0F) + 64.0F, -0.02F);
					GlStateManager.rotate(vec4b.getRotation() * 360 / 16.0F, 0.0F, 0.0F, 1.0F);
					...
					vertexbuffer.begin(7, DefaultVertexFormats.POSITION_TEX);
					float f5 = -0.001F;
					vertexbuffer.pos(-1.0D, 1.0D, k * -0.001F).tex(f1, f2).endVertex();
					vertexbuffer.pos(1.0D, 1.0D, k * -0.001F).tex(f3, f2).endVertex();
					vertexbuffer.pos(1.0D, -1.0D, k * -0.001F).tex(f3, f4).endVertex();
					vertexbuffer.pos(-1.0D, -1.0D, k * -0.001F).tex(f1, f4).endVertex();
					tessellator.draw();
