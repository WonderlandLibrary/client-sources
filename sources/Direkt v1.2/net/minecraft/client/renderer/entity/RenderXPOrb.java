...
public class RenderXPOrb extends Render<EntityXPOrb> {
	...
	public void doRender(EntityXPOrb entity, double x, double y, double z, float entityYaw, float partialTicks) {
		if (!this.renderOutlines) {
			...
			vertexbuffer.pos(-0.5D, -0.25D, 0.0D).tex(f, f3).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F).endVertex();
			vertexbuffer.pos(0.5D, -0.25D, 0.0D).tex(f1, f3).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F).endVertex();
			vertexbuffer.pos(0.5D, 0.75D, 0.0D).tex(f1, f2).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F).endVertex();
			vertexbuffer.pos(-0.5D, 0.75D, 0.0D).tex(f, f2).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F).endVertex();
			tessellator.draw();
