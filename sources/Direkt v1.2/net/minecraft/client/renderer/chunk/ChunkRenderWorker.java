	...
	protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException {
		...
		try {
			...
			if (blockpos1.add(8, 8, 8).distanceSq(blockpos) > 576.0D) {
				...
				if (!this.isChunkExisting(blockpos$mutableblockpos.func_189533_g(blockpos1).func_189534_c(EnumFacing.WEST, 16), world)
						|| !this.isChunkExisting(blockpos$mutableblockpos.func_189533_g(blockpos1).func_189534_c(EnumFacing.NORTH, 16), world)
						|| !this.isChunkExisting(blockpos$mutableblockpos.func_189533_g(blockpos1).func_189534_c(EnumFacing.EAST, 16), world)
						|| !this.isChunkExisting(blockpos$mutableblockpos.func_189533_g(blockpos1).func_189534_c(EnumFacing.SOUTH, 16), world)) { return; }
			}
			...
			if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.REBUILD_CHUNK) {
				for (BlockRenderLayer blockrenderlayer : BlockRenderLayer.values()) {
					if (lvt_7_2_.isLayerStarted(blockrenderlayer)) {
						lvt_8_2_.add(this.chunkRenderDispatcher.uploadChunk(blockrenderlayer, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(blockrenderlayer),
								generator.getRenderChunk(), lvt_7_2_, generator.getDistanceSq()));
					}
				}
			} else if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.RESORT_TRANSPARENCY) {
				lvt_8_2_.add(this.chunkRenderDispatcher.uploadChunk(BlockRenderLayer.TRANSLUCENT, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(BlockRenderLayer.TRANSLUCENT),
						generator.getRenderChunk(), lvt_7_2_, generator.getDistanceSq()));
			}
