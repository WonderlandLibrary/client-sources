...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (flag)
        {
            ...
            //TODO: Direkt: Hand not going off the screen thing
	...
	public void renderItemInFirstPerson(AbstractClientPlayer p_187457_1_, float p_187457_2_, float p_187457_3_, EnumHand p_187457_4_, float p_187457_5_, @Nullable ItemStack p_187457_6_,
			float p_187457_7_) {
		...
		if (p_187457_6_ == null) {
			...
			if (p_187457_1_.isHandActive() && (p_187457_1_.getItemInUseCount() > 0) && (p_187457_1_.getActiveHand() == p_187457_4_)) {
				...
				case BOW:
					...
					float f5 = p_187457_6_.getMaxItemUseDuration() - ((this.mc.thePlayer.getItemInUseCount() - p_187457_2_) + 1.0F);
					float f6 = f5 / 20.0F;
	...
	public void renderOverlays(float partialTicks) {
		...
		if (!this.mc.thePlayer.isSpectator()) {
			if (this.mc.thePlayer.isInsideOfMaterial(Material.WATER)
					&& !Reflector.callBoolean(Reflector.ForgeEventFactory_renderWaterOverlay, new Object[] { this.mc.thePlayer, Float.valueOf(partialTicks) })) {
	...
	/**
	 * Renders a texture that warps around based on the direction the player is looking. Texture needs to be bound before being called. Used for the water overlay.
	 */
	...
	private void renderFireInFirstPerson(float partialTicks) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			vertexbuffer.begin(7, DefaultVertexFormats.POSITION_TEX);
			vertexbuffer.pos(-0.5D, -0.5D, -0.5D).tex(f2, f4).endVertex();
			vertexbuffer.pos(0.5D, -0.5D, -0.5D).tex(f1, f4).endVertex();
			vertexbuffer.pos(0.5D, 0.5D, -0.5D).tex(f1, f3).endVertex();
			vertexbuffer.pos(-0.5D, 0.5D, -0.5D).tex(f2, f3).endVertex();
			tessellator.draw();
			GlStateManager.popMatrix();
		}
	...
	public void updateEquippedItem() {
		...
		if (entityplayersp.isRowingBoat()) {
			...
			if (Reflector.ForgeHooksClient_shouldCauseReequipAnimation.exists()) {
				boolean flag = Reflector.callBoolean(Reflector.ForgeHooksClient_shouldCauseReequipAnimation,
						new Object[] { this.itemStackMainHand, itemstack, Integer.valueOf(entityplayersp.inventory.currentItem) });
				...
				this.equippedProgressOffHand += MathHelper.clamp_float((!flag1 ? 1 : 0) - this.equippedProgressOffHand, -0.4F, 0.4F);
			} else {
				...
				this.equippedProgressOffHand += MathHelper.clamp_float((Objects.equal(this.itemStackOffHand, itemstack1) ? 1 : 0) - this.equippedProgressOffHand, -0.4F, 0.4F);
			}
		}
