...
public class PlayerControllerMP {
	...
	public boolean onPlayerDestroyBlock(BlockPos pos) {
		if (this.currentGameType.isAdventure()) {
			if (this.currentGameType == GameType.SPECTATOR) { return false; }
	...
	public boolean clickBlock(BlockPos loc, EnumFacing face) {
		if (this.currentGameType.isAdventure()) {
			...
			if (!this.mc.thePlayer.isAllowEdit()) {
				...
				if (!itemstack.canDestroy(this.mc.theWorld.getBlockState(loc).getBlock())) { return false; }
			}
		}
			...
			if (iblockstate.getMaterial() == Material.AIR) {
				...
				if ((this.stepSoundTickCounter % 4.0F) == 0.0F) {
					...
					this.mc.getSoundHandler()
							.playSound(new PositionedSoundRecord(soundtype.getHitSound(), SoundCategory.NEUTRAL, (soundtype.getVolume() + 1.0F) / 8.0F, soundtype.getPitch() * 0.5F, posBlock));
				}
	...
	public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, @Nullable ItemStack stack, BlockPos pos, EnumFacing facing, Vec3d vec, EnumHand hand) {
		this.syncCurrentPlayItem();
		float f = (float) (vec.xCoord - pos.getX());
		float f1 = (float) (vec.yCoord - pos.getY());
		float f2 = (float) (vec.zCoord - pos.getZ());
		boolean flag = false;
		...
		if (!this.mc.theWorld.getWorldBorder().contains(pos)) {
			...
			if (this.currentGameType != GameType.SPECTATOR) {
				...
				if ((!player.isSneaking() || ((player.getHeldItemMainhand() == null) && (player.getHeldItemOffhand() == null)))
						&& iblockstate.getBlock().onBlockActivated(worldIn, pos, iblockstate, player, hand, stack, facing, f, f1, f2)) {
					flag = true;
				}
