...
public class ModelBoat extends ModelBase implements IMultipassModel {
	...
	void renderPaddle(EntityBoat boat, int paddle, float scale, float limbSwing) {
		...
		modelrenderer.rotateAngleX = (float) MathHelper.denormalizeClamp(-1.0471975803375244D, -0.2617993950843811D, (MathHelper.sin(-f1) + 1.0F) / 2.0F);
		modelrenderer.rotateAngleY = (float) MathHelper.denormalizeClamp(-(Math.PI / 4D), (Math.PI / 4D), (MathHelper.sin(-f1 + 1.0F) + 1.0F) / 2.0F);
