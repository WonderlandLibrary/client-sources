...
public class CommandSetBlock extends CommandBase {
		...
		if (args.length < 4) {
			...
			if (!world.isBlockLoaded(blockpos)) {
				...
				if (args.length >= 6) {
					...
					} else if ("keep".equals(args[5]) && !world.isAirBlock(blockpos)) { throw new CommandException("commands.setblock.noChange", new Object[0]); }
				}
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
		return (args.length > 0) && (args.length <= 3) ? getTabCompletionCoordinate(args, 0, pos)
				: (args.length == 4 ? getListOfStringsMatchingLastWord(args, Block.REGISTRY.getKeys())
						: (args.length == 6 ? getListOfStringsMatchingLastWord(args, new String[] { "replace", "destroy", "keep" }) : Collections.<String> emptyList()));
	}
}
