						...
						for (int k = structureboundingbox.minY; k <= structureboundingbox.maxY; ++k) {
							for (int l = structureboundingbox.minX; l <= structureboundingbox.maxX; ++l) {
								...
								if (!flag || (iblockstate.getBlock() != Blocks.AIR)) {
									...
									if (flag1) { throw new CommandException("commands.compare.failed", new Object[0]); }
								}
							}
						}
					}
		...
		return (args.length > 0) && (args.length <= 3) ? getTabCompletionCoordinate(args, 0, pos)
				: ((args.length > 3) && (args.length <= 6) ? getTabCompletionCoordinate(args, 3, pos)
						: ((args.length > 6) && (args.length <= 9) ? getTabCompletionCoordinate(args, 6, pos)
								: (args.length == 10 ? getListOfStringsMatchingLastWord(args, new String[] { "masked", "all" }) : Collections.<String> emptyList())));
	}
}
