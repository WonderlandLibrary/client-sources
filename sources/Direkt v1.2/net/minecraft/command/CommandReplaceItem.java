...
public class CommandReplaceItem extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1) {
			...
			if ("entity".equals(args[0])) {
				...
				if (!"block".equals(args[0])) { throw new WrongUsageException("commands.replaceitem.usage", new Object[0]); }
			...
			if (flag) {
				if (args.length < 6) { throw new WrongUsageException("commands.replaceitem.block.usage", new Object[0]); }

				i = 4;
			} else {
				if (args.length < 4) { throw new WrongUsageException("commands.replaceitem.entity.usage", new Object[0]); }
			...
			try {
				...
				if (Block.getBlockFromName(args[i]) != Blocks.AIR) { throw numberinvalidexception; }
			...
			if (flag) {
				...
				if ((tileentity == null) || !(tileentity instanceof IInventory)) { throw new CommandException("commands.replaceitem.noContainer",
				...
				if (!entity.replaceItemInInventory(j,
						itemstack)) { throw new CommandException("commands.replaceitem.failed", new Object[] { s, Integer.valueOf(k), itemstack == null ? "Air" : itemstack.getTextComponent() }); }
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
		return args.length == 1 ? getListOfStringsMatchingLastWord(args, new String[] { "entity", "block" })
				: ((args.length == 2)
						&& "entity".equals(args[0])
								? getListOfStringsMatchingLastWord(args, server.getAllUsernames())
								: ((args.length >= 2) && (args.length <= 4)
										&& "block"
												.equals(args[0])
														? getTabCompletionCoordinate(args, 1, pos)
														: (((args.length != 3) || !"entity".equals(args[0])) && ((args.length != 5) || !"block".equals(args[0]))
																? (((args.length != 4) || !"entity".equals(args[0])) && ((args.length != 6) || !"block".equals(args[0]))
																		? Collections.<String> emptyList() : getListOfStringsMatchingLastWord(args, Item.REGISTRY.getKeys()))
