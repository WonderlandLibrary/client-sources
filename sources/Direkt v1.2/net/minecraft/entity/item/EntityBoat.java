...
public class EntityBoat extends Entity {
	...
	private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] { EntityDataManager.createKey(EntityBoat.class, DataSerializers.BOOLEAN),
	...
	/**
	 * How much the boat should glide given the slippery blocks it's currently gliding over. Halved every tick.
	 */
	...
	public void onUpdate() {
		...
		List<Entity> list = this.worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.20000000298023224D, -0.009999999776482582D, 0.20000000298023224D),
		...
		if (!list.isEmpty()) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (!entity.isPassenger(this)) {
					if (flag && (this.getPassengers().size() < 2) && !entity.isRiding() && (entity.width < this.width) && (entity instanceof EntityLivingBase) && !(entity instanceof EntityWaterMob)
	...
	public static enum Type {
		...
		public static EntityBoat.Type getTypeFromString(String nameIn) {
			for (int i = 0; i < values().length; ++i) {
				if (values()[i].getName().equals(nameIn)) { return values()[i]; }
			}
