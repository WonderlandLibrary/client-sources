...
public class EntityArmorStand extends EntityLivingBase {
	...
	public EnumActionResult applyPlayerInteraction(EntityPlayer player, Vec3d vec, @Nullable ItemStack stack, EnumHand hand) {
		if (this.hasMarker()) {
			...
			if (this.isDisabled(entityequipmentslot1) || this.isDisabled(entityequipmentslot)) {
				...
				if (this.isDisabled(entityequipmentslot)) { return EnumActionResult.FAIL; }
			}
	...
	private void playParticles() {
		if (this.worldObj instanceof WorldServer) {
			((WorldServer) this.worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY + (this.height / 1.5D), this.posZ, 10, this.width / 4.0F, this.height / 4.0F,
					this.width / 4.0F, 0.05D, new int[] { Block.getStateId(Blocks.PLANKS.getDefaultState()) });
		}
	}
