	...
	public void onUpdateEntity() {
		try {
			...
			if (this.foodStats.getFoodLevel() != this.lastFoodScore) {
				...
				this.updateScorePoints(IScoreCriteria.FOOD, MathHelper.ceiling_float_int(this.lastFoodScore));
			}

			if (this.getAir() != this.lastAirScore) {
				this.lastAirScore = this.getAir();
				this.updateScorePoints(IScoreCriteria.AIR, MathHelper.ceiling_float_int(this.lastAirScore));
			}

			if (this.getTotalArmorValue() != this.lastArmorScore) {
				this.lastArmorScore = this.getTotalArmorValue();
				this.updateScorePoints(IScoreCriteria.ARMOR, MathHelper.ceiling_float_int(this.lastArmorScore));
			}

			if (this.experienceTotal != this.lastExperienceScore) {
				this.lastExperienceScore = this.experienceTotal;
				this.updateScorePoints(IScoreCriteria.XP, MathHelper.ceiling_float_int(this.lastExperienceScore));
			}

			if (this.experienceLevel != this.lastLevelScore) {
				this.lastLevelScore = this.experienceLevel;
				this.updateScorePoints(IScoreCriteria.LEVEL, MathHelper.ceiling_float_int(this.lastLevelScore));
			}
		...
		if (this.isEntityInvulnerable(source)) {
			...
			if (!flag && (this.respawnInvulnerabilityTicks > 0) && (source != DamageSource.outOfWorld)) {
				...
				if (source instanceof EntityDamageSource) {
					...
					if ((entity instanceof EntityPlayer) && !this.canAttackPlayer((EntityPlayer) entity)) { return false; }

					if (entity instanceof EntityArrow) {
						EntityArrow entityarrow = (EntityArrow) entity;

						if ((entityarrow.shootingEntity instanceof EntityPlayer) && !this.canAttackPlayer((EntityPlayer) entityarrow.shootingEntity)) { return false; }
					}
				}
	...
	public void displayGUIChest(IInventory chestInventory) {
		if ((chestInventory instanceof ILootContainer) && (((ILootContainer) chestInventory).getLootTable() != null) && this.isSpectator()) {
			...
			if (chestInventory instanceof IInteractionObject) {
				this.connection
						.sendPacket(new SPacketOpenWindow(this.currentWindowId, ((IInteractionObject) chestInventory).getGuiID(), chestInventory.getDisplayName(), chestInventory.getSizeInventory()));
