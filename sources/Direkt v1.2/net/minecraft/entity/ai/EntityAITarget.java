			...
			if ((team != null) && (team1 == team)) {
				...
				if (this.taskOwner.getDistanceSqToEntity(entitylivingbase) > (d0 * d0)) {
					...
					if (this.shouldCheckSight) {
						...
						} else if (++this.targetUnseenTicks > this.unseenMemoryTicks) { return false; }
					}
	...
	public static boolean isSuitableTarget(EntityLiving attacker, EntityLivingBase target, boolean includeInvincibles, boolean checkSight) {
		if (target == null) {
			...
			if ((attacker instanceof IEntityOwnable) && (((IEntityOwnable) attacker).getOwnerId() != null)) {
				if ((target instanceof IEntityOwnable) && ((IEntityOwnable) attacker).getOwnerId().equals(target.getUniqueID())) { return false; }

				if (target == ((IEntityOwnable) attacker).getOwner()) { return false; }
			} else if ((target instanceof EntityPlayer) && !includeInvincibles && ((EntityPlayer) target).capabilities.disableDamage) { return false; }
