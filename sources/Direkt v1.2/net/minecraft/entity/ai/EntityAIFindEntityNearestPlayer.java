		...
		this.predicate = new Predicate<Entity>() {
			...
			public boolean apply(@Nullable Entity p_apply_1_) {
				if (!(p_apply_1_ instanceof EntityPlayer)) {
					...
					return p_apply_1_.getDistanceToEntity(EntityAIFindEntityNearestPlayer.this.entityLiving) > d0 ? false
		...
		if (entitylivingbase == null) {
			...
			if ((team != null) && (team1 == team)) {
				...
				return this.entityLiving.getDistanceSqToEntity(entitylivingbase) > (d0 * d0) ? false
						: !(entitylivingbase instanceof EntityPlayerMP) || !((EntityPlayerMP) entitylivingbase).interactionManager.isCreative();
			}
		}
	}
