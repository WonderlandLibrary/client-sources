...
public abstract class EntityArrow extends Entity implements IProjectile {
	...
	public void setAim(Entity p_184547_1_, float p_184547_2_, float p_184547_3_, float p_184547_4_, float p_184547_5_, float p_184547_6_) {
		...
		this.setThrowableHeading(f, f1, f2, p_184547_5_, p_184547_6_);
	...
	public void onUpdate() {
		...
		if (this.inGround) {
			...
			if (this.getIsCritical()) {
				for (int k = 0; k < 4; ++k) {
					this.worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + ((this.motionX * k) / 4.0D), this.posY + ((this.motionY * k) / 4.0D), this.posZ + ((this.motionZ * k) / 4.0D),
			...
			if (this.isInWater()) {
				for (int i = 0; i < 4; ++i) {
					float f3 = 0.25F;
					this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - (this.motionX * 0.25D), this.posY - (this.motionY * 0.25D), this.posZ - (this.motionZ * 0.25D),
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (entity != null) {
			...
			int i = MathHelper.ceiling_double_int(f * this.damage);
