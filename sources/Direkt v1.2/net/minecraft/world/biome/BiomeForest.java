...
public class BiomeForest extends Biome {
	...
	public WorldGenAbstractTree genBigTreeChance(Random rand) {
		return (this.type == BiomeForest.Type.ROOFED) && (rand.nextInt(3) > 0) ? ROOF_TREE
				: ((this.type != BiomeForest.Type.BIRCH) && (rand.nextInt(5) != 0) ? (rand.nextInt(10) == 0 ? BIG_TREE_FEATURE : TREE_FEATURE) : BIRCH_TREE);
	}

	@Override
	public BlockFlower.EnumFlowerType pickRandomFlower(Random rand, BlockPos pos) {
		if (this.type == BiomeForest.Type.FLOWER) {
			double d0 = MathHelper.clamp_double((1.0D + GRASS_COLOR_NOISE.getValue(pos.getX() / 48.0D, pos.getZ() / 48.0D)) / 2.0D, 0.0D, 0.9999D);
