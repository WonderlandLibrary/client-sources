...
public class StructureBoundingBox {
	 ...
	 */
	public static StructureBoundingBox getComponentToAddBoundingBox(int structureMinX, int structureMinY, int structureMinZ, int xMin, int yMin, int zMin, int xMax, int yMax, int zMax,
			EnumFacing facing) {
		switch (facing) {
		case NORTH:
			return new StructureBoundingBox(structureMinX + xMin, structureMinY + yMin, (structureMinZ - zMax) + 1 + zMin, ((structureMinX + xMax) - 1) + xMin, ((structureMinY + yMax) - 1) + yMin,
					structureMinZ + zMin);

		case SOUTH:
			return new StructureBoundingBox(structureMinX + xMin, structureMinY + yMin, structureMinZ + zMin, ((structureMinX + xMax) - 1) + xMin, ((structureMinY + yMax) - 1) + yMin,
					((structureMinZ + zMax) - 1) + zMin);

		case WEST:
			return new StructureBoundingBox((structureMinX - zMax) + 1 + zMin, structureMinY + yMin, structureMinZ + xMin, structureMinX + zMin, ((structureMinY + yMax) - 1) + yMin,
					((structureMinZ + xMax) - 1) + xMin);

		case EAST:
			return new StructureBoundingBox(structureMinX + zMin, structureMinY + yMin, structureMinZ + xMin, ((structureMinX + zMax) - 1) + zMin, ((structureMinY + yMax) - 1) + yMin,
					((structureMinZ + xMax) - 1) + xMin);
	...
	public boolean intersectsWith(StructureBoundingBox structurebb) {
		return (this.maxX >= structurebb.minX) && (this.minX <= structurebb.maxX) && (this.maxZ >= structurebb.minZ) && (this.minZ <= structurebb.maxZ) && (this.maxY >= structurebb.minY)
				&& (this.minY <= structurebb.maxY);
	}
