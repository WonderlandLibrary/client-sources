		...
		if (position.getY() <= 4) {
			...
			for (int k1 = 0; k1 < 16; ++k1) {
				for (int l2 = 0; l2 < 16; ++l2) {
					for (int k = 0; k < 8; ++k) {
						boolean flag = !aboolean[(((k1 * 16) + l2) * 8) + k] && (((k1 < 15) && aboolean[((((k1 + 1) * 16) + l2) * 8) + k]) || ((k1 > 0) && aboolean[((((k1 - 1) * 16) + l2) * 8) + k])
						...
						if (flag) {
							...
							if ((k < 4) && !material.isSolid() && (worldIn.getBlockState(position.add(k1, k, l2)).getBlock() != this.block)) { return false; }
						}
					}
				}
			}
