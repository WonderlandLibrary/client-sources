...
public class WorldGenSpikes extends WorldGenerator {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (this.spike == null) {
			...
			for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(new BlockPos(position.getX() - i, 0, position.getZ() - i),
					new BlockPos(position.getX() + i, this.spike.getHeight() + 10, position.getZ() + i))) {
				if ((blockpos$mutableblockpos.distanceSq(position.getX(), blockpos$mutableblockpos.getY(), position.getZ()) <= (i * i) + 1)
						&& (blockpos$mutableblockpos.getY() < this.spike.getHeight())) {
	...
	/**
	 * Sets the value that will be used in a call to entitycrystal.setBeamTarget. At the moment, WorldGenSpikes.setBeamTarget is only ever called with a value of (0, 128, 0)
	 */
