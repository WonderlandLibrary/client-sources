...
public class DragonFightManager {
	...
	private final BossInfoServer bossInfo = (BossInfoServer) (new BossInfoServer(new TextComponentTranslation("entity.EnderDragon.name", new Object[0]), BossInfo.Color.PINK,
			BossInfo.Overlay.PROGRESS)).setPlayEndBossMusic(true).setCreateFog(true);
	...
	public DragonFightManager(WorldServer worldIn, NBTTagCompound compound) {
		...
		this.portalPattern = FactoryBlockPattern.start().aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " })
				.aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " })
				.aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " })
				.aisle(new String[] { "  ###  ", " #   # ", "#     #", "#  #  #", "#     #", " #   # ", "  ###  " })
				.aisle(new String[] { "       ", "  ###  ", " ##### ", " ##### ", " ##### ", "  ###  ", "       " }).where('#', BlockWorldState.hasState(BlockMatcher.forBlock(Blocks.BEDROCK)))
				.build();
	}
		...
		for (int i = -8; i <= 8; ++i) {
			for (int j = -8; j <= 8; ++j) {
				...
				for (TileEntity tileentity : chunk.getTileEntityMap().values()) {
					if (tileentity instanceof TileEntityEndPortal) { return true; }
				}
			}
		}
	...
	private BlockPattern.PatternHelper findExitPortal() {
		...
		for (int l = k; l >= 0; --l) {
			BlockPattern.PatternHelper blockpattern$patternhelper1 = this.portalPattern.match(this.world,
					new BlockPos(WorldGenEndPodium.END_PODIUM_LOCATION.getX(), l, WorldGenEndPodium.END_PODIUM_LOCATION.getZ()));
	...
	private void generatePortal(boolean active) {
		...
		if (this.exitPortalLocation == null) {
			for (this.exitPortalLocation = this.world.getTopSolidOrLiquidBlock(WorldGenEndPodium.END_PODIUM_LOCATION)
					.down(); (this.world.getBlockState(this.exitPortalLocation).getBlock() == Blocks.BEDROCK)
							&& (this.exitPortalLocation.getY() > this.world.getSeaLevel()); this.exitPortalLocation = this.exitPortalLocation.down()) {
				;
			}
		}
