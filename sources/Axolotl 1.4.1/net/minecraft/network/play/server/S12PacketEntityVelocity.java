...
import axolotl.cheats.modules.impl.combat.Velocity;
import axolotl.cheats.modules.impl.movement.Flight;
...
public class S12PacketEntityVelocity implements Packet
{
    ...
    public void processPacket(INetHandlerPlayClient handler)
    {
        ...
        Velocity velo = ((Velocity)Axolotl.INSTANCE.moduleManager.getModule("Velocity"));
        if(velo.toggled) {
            switch (velo.mode.getMode()) {
                case "Cancel":
                    return;
                ...
                case "Jump":
                    if (Minecraft.getMinecraft().thePlayer.hurtTime > 0 && Minecraft.getMinecraft().thePlayer.hurtTime < 2)
                        Minecraft.getMinecraft().thePlayer.motionY = Minecraft.getMinecraft().thePlayer.onGround ? 0.42 : 0.21;
                    return;
        ...
        Flight flight = (Flight)Axolotl.INSTANCE.moduleManager.getModule("Flight");
        if(flight.toggled && flight.mode.getMode().equalsIgnoreCase("RedeSky")) {
            return;
        }
