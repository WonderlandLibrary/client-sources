...
import axolotl.cheats.modules.impl.world.ChestStealer;
...
public class GuiChest extends GuiContainer
{
    ...
    public void initGui() {
    	super.initGui();
    	if(Axolotl.INSTANCE.moduleManager.getModule("ChestStealer").toggled) {
        	new Thread(new Runnable() {
    			@Override
    			public void run() {
    				try {
                        ChestStealer chestStealer = ((ChestStealer)Axolotl.INSTANCE.moduleManager.getModule("ChestStealer"));
    					for(int i = 0; i < GuiChest.this.inventoryRows * 9; i++) {
    						Slot slot = (Slot) GuiChest.this.inventorySlots.inventorySlots.get(i);
    						if(slot.getStack() != null) {
                                if(chestStealer.stealSpeed.getNumberValue() != 0)Thread.sleep((long) (chestStealer.stealSpeed.getNumberValue()));
    							GuiChest.this.handleMouseClick(slot, slot.slotNumber, 0, 1);
    							GuiChest.this.handleMouseClick(slot, slot.slotNumber, 0, 6);
    						}
    					}
    					if(chestStealer.closeDelay.getNumberValue() != 0)Thread.sleep((long)chestStealer.closeDelay.getNumberValue());
