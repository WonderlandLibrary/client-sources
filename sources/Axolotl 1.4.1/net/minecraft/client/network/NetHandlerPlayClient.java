...
import axolotl.cheats.modules.impl.world.Disabler;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        ...
        Disabler disabler = (Disabler)Axolotl.INSTANCE.moduleManager.getModule("Disabler");
        if(disabler.isToggled() && disabler.mode.getMode().equalsIgnoreCase("Watchdog")) {
            this.netManager.sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(var2.posX, var2.getEntityBoundingBox().minY, var2.posZ, false));
        } else this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(var2.posX, var2.getEntityBoundingBox().minY, var2.posZ, var2.rotationYaw, var2.rotationPitch, false));
    ...
    {
    	if(PacketUtils.shouldSend(packetIn)) {
    		EventPacket event = new EventPacket(packetIn, EventType.PRE, 1);
        ...
        if(PacketUtils.shouldSend(packetIn)) {
            EventPacket event = new EventPacket(packetIn, EventType.POST, 1);
            Axolotl.INSTANCE.moduleManager.onEvent(event);
        }
    }
    ...
    public void handleChangeGameState(S2BPacketChangeGameState packetIn)
    {
        ...
        else if (var3 == 5)
        {
            ...
            if (var4 == 0.0F)
            {
                Axolotl.INSTANCE.sendMessage("You just got trolled by a demo screen. Don't worry, we're protected.");
    ...
    public void handleParticles(S2APacketParticles packetIn)
    {
        ...
        {
            int var20 = packetIn.func_149222_k();
            try {
                if(ac == null) {
                    ac = (AntiCrash)Axolotl.INSTANCE.moduleManager.getModule("AntiCrash");
                    maxParticles = (int)ac.packetMax.getNumberValue();
                }
                if(ac.toggled) var20 = Math.min(var20, maxParticles);
