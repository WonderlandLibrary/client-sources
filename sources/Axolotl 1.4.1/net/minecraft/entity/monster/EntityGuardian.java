    ...
    public void onLivingUpdate()
    {
        if (this.worldObj.isRemote)
        {
            ...
            if (this.func_175472_n() && this.isInWater())
            {
                ...
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width - var1.x * 1.5D, this.posY + this.rand.nextDouble() * (double)this.height - var1.y * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width - var1.z * 1.5D, 0.0D, 0.0D, 0.0D, new int[0]);
                }
            }
