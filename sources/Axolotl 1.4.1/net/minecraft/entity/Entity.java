...
import static axolotl.util.MotionUtil.mc;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            boolean sneak = this.isSneaking() || (scaffold.toggled && this == mc.thePlayer);

            boolean var19 = this.onGround && sneak && this instanceof EntityPlayer;
    ...
    {
        StrafeEvent strafeEvent = new StrafeEvent(strafe, forward, EventType.PRE);
        if (this == Minecraft.getMinecraft().thePlayer) {
            Axolotl.INSTANCE.moduleManager.onEvent(strafeEvent);
            strafe = strafeEvent.getStrafe();
            forward = strafeEvent.getForward();
            if (strafeEvent.getCancelled()) return;
        }
        ...
        if (var4 >= 1.0E-4F)
        {
            ...
            float var5 = MathHelper.sin(yaw * (float)Math.PI / 180.0F);
            float var6 = MathHelper.cos(yaw * (float)Math.PI / 180.0F);

            this.motionX += (double)(strafe * var6 - forward * var5);
            this.motionZ += (double)(forward * var6 + strafe * var5);
        }

        strafeEvent.eventType = EventType.POST;
    ...
    {
        float p = this.rotationPitch;
        float y = this.rotationYaw;

        Aura aura = (Aura)Axolotl.INSTANCE.moduleManager.getModule("Aura");
        if(aura.toggled) {
            p = aura.pitch;
            y = aura.yaw;
        }
    ...
    public MovingObjectPosition func_174822_a(double p_174822_1_, float p_174822_3_)
    {
        ...
        Vec3 var6 = var4.addVector(var5.x * p_174822_1_, var5.y * p_174822_1_, var5.z * p_174822_1_);
