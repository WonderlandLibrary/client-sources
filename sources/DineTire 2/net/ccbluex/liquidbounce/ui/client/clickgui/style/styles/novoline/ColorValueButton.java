...
public class ColorValueButton
    ...
    public void render(int mouseX, int mouseY) {
        float[] huee = {hue[0]};
        RenderUtils.drawRect2(x - 10, y - 4, x + 80, y + 11, new Color(0, 0, 0, 100).getRGB());
        for (int i = x - 7; i < x + 79; ++i) {
            ...
            RenderUtils.drawRect2(i - 1, y, i, y + 8, color.getRGB());
            huee[0] = huee[0] + 4.0f;
            if (!(huee[0] > 255.0f)) continue;
            huee[0] = huee[0] - 255.0f;
        }
        RenderUtils.drawRect2(position, y, position + 1, y + 8, -1);
        if (hue[0] > 255.0f) {
            hue[0] = hue[0] - 255.0f;
        }
    }
