...
class NameTags : Module() {
    private val healthValue = true
    private val pingValue = false
    private val distanceValue = false
    private val armorValue = true
    private val clearNamesValue = false
    private val fontValue = Fonts.font40
    private val borderValue = true
    private val scaleValue = 1.15F
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        val distanceText = if (distanceValue) "§7${mc.thePlayer.getDistanceToEntity(entity).roundToInt()}m " else ""
        val pingText = if (pingValue && entity is EntityPlayer) (if (ping > 200) "§c" else if (ping > 100) "§e" else "§a") + ping + "ms §7" else ""
        val healthText = if (healthValue) "§7§c " + entity.health.toInt() + " HP" else ""
        ...
        val scale = distance / 100F * scaleValue
        ...
        if (armorValue && entity is EntityPlayer) {
            for (index in 0..4) {
