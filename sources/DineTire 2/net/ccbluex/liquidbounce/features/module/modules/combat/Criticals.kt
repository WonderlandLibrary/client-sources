...
class Criticals : Module() {
    ...
    val modeValue = ListValue("Mode", arrayOf("AAC3.3.10", "AAC4.3.12"), "AAC3.3.10")
    val delayValue = 20
    private val hurtTimeValue = 7
    ...
    fun onAttack(event: AttackEvent) {
        if (event.targetEntity is EntityLivingBase) {
            ...
            if (!mc.thePlayer.onGround || mc.thePlayer.isOnLadder || mc.thePlayer.isInWeb || mc.thePlayer.isInWater ||
                    mc.thePlayer.isInLava || mc.thePlayer.ridingEntity != null || entity.hurtTime > hurtTimeValue ||
                    LiquidBounce.moduleManager[Fly::class.java]!!.state || !msTimer.hasTimePassed(delayValue.toLong()))
                return
            ...
            when (modeValue.get().toLowerCase()) {
                "aac3.3.10" -> mc.thePlayer.onCriticalHit(entity)
                "aac4.3.12" -> {
