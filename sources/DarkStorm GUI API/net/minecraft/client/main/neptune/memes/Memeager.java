...
import net.minecraft.client.main.neptune.memes.events.StopMeme;
 ...
 */
public final class Memeager {
	
	/**
	 * HashMap containing all the registered MethodData sorted on the event parameters of the methods.
	 */
	private static final Map<Class<? extends Memevnt>, List<MethodData>> REGISTRY_MAP = new HashMap<Class<? extends Memevnt>, List<MethodData>>();
     ...
     */
    public static void register(Object object, Class<? extends Memevnt> eventClass) {
        for (final Method method : object.getClass().getDeclaredMethods()) {
     ...
     */
    public static void unregister(Object object, Class<? extends Memevnt> eventClass) {
    ...
    private static void register(Method method, Object object) {
    	Class<? extends Memevnt> indexClass = (Class<? extends Memevnt>) method.getParameterTypes()[0];
    	//New MethodData from the Method we are registering.
    	final MethodData data = new MethodData(object, method, method.getAnnotation(Memetarget.class).value());
     ...
     */
    public static void removeEntry(Class<? extends Memevnt> indexClass) {
        Iterator<Map.Entry<Class<? extends Memevnt>, List<MethodData>>> mapIterator = REGISTRY_MAP.entrySet().iterator();
     ...
     */
    private static void sortListValue(Class<? extends Memevnt> indexClass) {
        List<MethodData> sortedList = new CopyOnWriteArrayList<MethodData>();

        for (final byte priority : Memeority.VALUE_ARRAY) {
     ...
     *
     * @see net.minecraft.client.main.neptune.memes.Memetarget
     */
    private static boolean isMethodBad(Method method) {
        return method.getParameterTypes().length != 1 || !method.isAnnotationPresent(Memetarget.class);
    }
     ...
     * @see net.minecraft.client.main.neptune.memes.Memetarget
     */
    private static boolean isMethodBad(Method method, Class<? extends Memevnt> eventClass) {
     ...
     */
    public static final Memevnt call(final Memevnt memevnt) {
        List<MethodData> dataList = REGISTRY_MAP.get(memevnt.getClass());

        if (dataList != null) {
            if (memevnt instanceof StopMeme) {
                StopMeme stoppable = (StopMeme) memevnt;
     ...
     * TODO: Error messages.
     */
    private static void invoke(MethodData data, Memevnt argument) {
        try {
