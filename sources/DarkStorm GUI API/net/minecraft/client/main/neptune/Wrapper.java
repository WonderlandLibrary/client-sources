...
import net.minecraft.client.network.*; //MEMES
...
public class Wrapper
{
    ...
    public static String funt = "Comfortaa";  //GUIS FONT
    ...
    public static void sendChatMessage(final String msg) {
        sendPacket((Packet)new C01PacketChatMessage(msg));
    }
        ...
        for (int x = MathHelper.floor_double(e.getEntityBoundingBox().minX); x < MathHelper.floor_double(e.getEntityBoundingBox().maxX) + 1; ++x) {
            for (int y = MathHelper.floor_double(e.getEntityBoundingBox().minY); y < MathHelper.floor_double(e.getEntityBoundingBox().maxY) + 1; ++y) {
                for (int z = MathHelper.floor_double(e.getEntityBoundingBox().minZ); z < MathHelper.floor_double(e.getEntityBoundingBox().maxZ) + 1; ++z) {
                    final Block block = getWorld().getBlockState(new BlockPos((double)x, (double)(y + offset), (double)z)).getBlock();
                    if (block != null && !(block instanceof BlockAir)) {
                        if (!(block instanceof BlockLiquid)) {
                            final AxisAlignedBB boundingBox = block.getCollisionBoundingBox((World)getWorld(), new BlockPos((double)x, (double)(y + offset), (double)z), getWorld().getBlockState(new BlockPos((double)x, (double)(y + offset), (double)z)));
    ...
    public static float getDistanceToGround(final Entity e) {
        ...
        while (a > 0.0f) {
            ...
            final Block block = getBlock(new BlockPos(e.posX, (double)(a - 1.0f), e.posZ));
    ...
    public static boolean isOnLiquid() {
        boolean onLiquid = false;
        if (getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.30000001192092896, 0.10000000149011612, 0.30000001192092896).getMaterial().isLiquid() && getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.30000001192092896, 0.10000000149011612, -0.30000001192092896).getMaterial().isLiquid()) {
            onLiquid = true;
        }
        return onLiquid;
    }
    ...
    public static boolean isInLiquid() {
        ...
        if (getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.30000001192092896, 0.0, 0.30000001192092896).getMaterial().isLiquid() || getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.30000001192092896, 0.0, -0.30000001192092896).getMaterial().isLiquid()) {
            inLiquid = true;
        }
        return inLiquid;
    }
    ...
    public static boolean isInFire() {
        ...
        if (getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.30000001192092896, 0.0, 0.30000001192092896).getMaterial() == Material.fire || getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.30000001192092896, 0.0, -0.30000001192092896).getMaterial() == Material.fire) {
            isInFire = true;
        }
        return isInFire;
    }
    ...
    public static String liquidCollision() {
        String colission = "";
        if (getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.3100000023841858, 0.0, 0.3100000023841858).getMaterial().isLiquid()) {
            colission = "Positive";
        }
        if (getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.3100000023841858, 0.0, -0.3100000023841858).getMaterial().isLiquid()) {
            colission = "Negative";
    ...
    public static boolean isBlockUnderPlayer(final Material material, final float height) {
        return getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.3100000023841858, height, 0.3100000023841858).getMaterial() == material && getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.3100000023841858, height, -0.3100000023841858).getMaterial() == material && getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, -0.3100000023841858, height, 0.3100000023841858).getMaterial() == material && getBlockAtPosC((EntityPlayer)Wrapper.mc.thePlayer, 0.3100000023841858, height, -0.3100000023841858).getMaterial() == material;
    }
