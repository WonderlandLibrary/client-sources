...
public abstract class Cmd {
	...
	public Cmd() {
		...
		this.name = this.getClass().<Info>getAnnotation(Info.class).name();
		this.help = this.getClass().<Info>getAnnotation(Info.class).help();
		this.syntax = this.getClass().<Info>getAnnotation(Info.class).syntax();
	}
	...
	protected final int[] argsToPos(final String... args) throws Error {
		int[] pos = new int[3];
		if (args.length == 3) {
			final int[] playerPos = { (int) Minecraft.getMinecraft().thePlayer.posX,
