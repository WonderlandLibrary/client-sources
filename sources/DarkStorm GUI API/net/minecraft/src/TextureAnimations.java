		...
		if (!(rp instanceof AbstractResourcePack)) {
			...
			if (tpFile == null) {
				...
				if (animPropNames == null) {
					...
					TextureAnimation[] var14 = (TextureAnimation[]) ((TextureAnimation[]) list
							.toArray(new TextureAnimation[list.size()]));
					return var14;
				}
			}
		...
		if (!dirAnim.exists()) {
			...
			if (propFiles == null) {
				...
				for (int props = 0; props < propFiles.length; ++props) {
					...
					if (!name.startsWith("custom_") && name.endsWith(".properties") && file.isFile()
							&& file.canRead()) {
