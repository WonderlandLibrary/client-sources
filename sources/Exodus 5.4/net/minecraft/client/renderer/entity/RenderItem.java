...
implements IResourceManagerReloadListener {
    ...
    public void renderItemIntoGUI(ItemStack itemStack, int n, int n2) {
        IBakedModel iBakedModel = this.itemModelMesher.getItemModel(itemStack);
        ...
        this.setupGuiTransform(n, n2, iBakedModel.isGui3d());
        iBakedModel.getItemCameraTransforms().applyTransform(ItemCameraTransforms.TransformType.GUI);
        this.renderItem(itemStack, iBakedModel);
    ...
    public void func_181564_a(ItemStack itemStack, ItemCameraTransforms.TransformType transformType) {
        if (itemStack != null) {
            IBakedModel iBakedModel = this.itemModelMesher.getItemModel(itemStack);
            this.renderItemModelTransform(itemStack, iBakedModel, transformType);
        }
    }
    ...
    private void renderQuad(WorldRenderer worldRenderer, BakedQuad bakedQuad, int n) {
        worldRenderer.addVertexData(bakedQuad.getVertexData());
        worldRenderer.putColor4(n);
        this.putQuadNormal(worldRenderer, bakedQuad);
    }

    protected void registerBlock(Block block, int n, String string) {
        this.registerItem(Item.getItemFromBlock(block), n, string);
    }

    private void registerBlock(Block block, String string) {
        this.registerBlock(block, 0, string);
    }

    private void func_181565_a(WorldRenderer worldRenderer, int n, int n2, int n3, int n4, int n5, int n6, int n7, int n8) {
    ...
    private void registerItem(Item item, String string) {
        this.registerItem(item, 0, string);
    }
    ...
    public void renderItem(ItemStack itemStack, IBakedModel iBakedModel) {
        if (itemStack != null) {
            ...
            if (iBakedModel.isBuiltInRenderer()) {
                ...
                GlStateManager.translate(-0.5f, -0.5f, -0.5f);
                this.renderModel(iBakedModel, itemStack);
                if (itemStack.hasEffect()) {
                    this.renderEffect(iBakedModel);
                }
            }
            GlStateManager.popMatrix();
        }
    ...
    public void renderItemModelForEntity(ItemStack itemStack, EntityLivingBase entityLivingBase, ItemCameraTransforms.TransformType transformType) {
        if (itemStack != null && entityLivingBase != null) {
            ...
            if (entityLivingBase instanceof EntityPlayer) {
                ...
                if (item == Items.fishing_rod && entityPlayer.fishEntity != null) {
                    ...
                    int n = itemStack.getMaxItemUseDuration() - entityPlayer.getItemInUseCount();
                    if (n >= 18) {
    ...
    private void preTransform(ItemStack itemStack) {
        IBakedModel iBakedModel = this.itemModelMesher.getItemModel(itemStack);
        Item item = itemStack.getItem();
        if (item != null) {
    ...
    private void setupGuiTransform(int n, int n2, boolean bl) {
    ...
    private void renderQuads(WorldRenderer worldRenderer, List<BakedQuad> list, int n, ItemStack itemStack) {
        boolean bl = n == -1 && itemStack != null;
        int n2 = 0;
        int n3 = list.size();
        while (n2 < n3) {
            BakedQuad bakedQuad = list.get(n2);
            int n4 = n;
            if (bl && bakedQuad.hasTintIndex()) {
                n4 = itemStack.getItem().getColorFromItemStack(itemStack, bakedQuad.getTintIndex());
    ...
    private boolean func_183005_a(ItemTransformVec3f itemTransformVec3f) {
        return itemTransformVec3f.scale.x < 0.0f ^ itemTransformVec3f.scale.y < 0.0f ^ itemTransformVec3f.scale.z < 0.0f;
    }

    protected void registerItem(Item item, int n, String string) {
        this.itemModelMesher.register(item, n, new ModelResourceLocation(string, "inventory"));
    }
    ...
    public void renderItemAndEffectIntoGUI(final ItemStack itemStack, int n, int n2) {
    ...
    public void renderItemAboveHead(final ItemStack itemStack, int n, int n2) {
        if (itemStack != null) {
            this.zLevel += 50.0f;
            try {
                this.renderItemIntoGUI(itemStack, n, n2);
    ...
    private void renderModel(IBakedModel iBakedModel, int n, ItemStack itemStack) {
        ...
        while (n3 < n2) {
            ...
            this.renderQuads(worldRenderer, iBakedModel.getFaceQuads(enumFacing), n, itemStack);
            ++n3;
        }
        this.renderQuads(worldRenderer, iBakedModel.getGeneralQuads(), n, itemStack);
        tessellator.draw();
    }

    public void renderItemOverlayIntoGUI(FontRenderer fontRenderer, ItemStack itemStack, int n, int n2, String string) {
        if (itemStack != null) {
            if (itemStack.stackSize != 1 || string != null) {
                String string2;
                String string3 = string2 = string == null ? String.valueOf(itemStack.stackSize) : string;
                if (string == null && itemStack.stackSize < 1) {
                    string2 = (Object)((Object)EnumChatFormatting.RED) + String.valueOf(itemStack.stackSize);
                }
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                GlStateManager.disableBlend();
                fontRenderer.drawStringWithShadow(string2, n + 19 - 2 - fontRenderer.getStringWidth(string2), n2 + 6 + 3, 0xFFFFFF);
                GlStateManager.enableLighting();
                GlStateManager.enableDepth();
            }
            if (itemStack.isItemDamaged()) {
                int n3 = (int)Math.round(13.0 - (double)itemStack.getItemDamage() * 13.0 / (double)itemStack.getMaxDamage());
    ...
    public void func_175039_a(boolean bl) {
        this.field_175058_l = bl;
    }
    ...
    private void renderModel(IBakedModel iBakedModel, int n) {
        this.renderModel(iBakedModel, n, null);
    }

    protected void renderItemModelTransform(ItemStack itemStack, IBakedModel iBakedModel, ItemCameraTransforms.TransformType transformType) {
        this.textureManager.bindTexture(TextureMap.locationBlocksTexture);
        this.textureManager.getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false, false);
