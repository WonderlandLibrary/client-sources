...
public enum EnumConnectionState {
    ...
    public Packet getPacket(EnumPacketDirection enumPacketDirection, int n) throws IllegalAccessException, InstantiationException {
        Class clazz = (Class)this.directionMaps.get((Object)enumPacketDirection).get((Object)n);
    ...
    public static EnumConnectionState getById(int n) {
        return n >= field_181136_e && n <= field_181137_f ? STATES_BY_ID[n - field_181136_e] : null;
    }
    ...
    static {
        ...
        while (n2 < n) {
            ...
            int n3 = enumConnectionState.getId();
            if (n3 < field_181136_e || n3 > field_181137_f) {
                throw new Error("Invalid protocol ID " + Integer.toString(n3));
            }
            EnumConnectionState.STATES_BY_ID[n3 - EnumConnectionState.field_181136_e] = enumConnectionState;
            for (EnumPacketDirection enumPacketDirection : enumConnectionState.directionMaps.keySet()) {
                for (Class clazz : enumConnectionState.directionMaps.get((Object)enumPacketDirection).values()) {
                    if (STATES_BY_CLASS.containsKey(clazz) && STATES_BY_CLASS.get(clazz) != enumConnectionState) {
                        throw new Error("Packet " + clazz + " is already assigned to protocol " + (Object)((Object)STATES_BY_CLASS.get(clazz)) + " - can't reassign to " + (Object)((Object)enumConnectionState));
                    }
                    try {
                        clazz.newInstance();
                    }
    ...
    public Integer getPacketId(EnumPacketDirection enumPacketDirection, Packet packet) {
        return (Integer)this.directionMaps.get((Object)enumPacketDirection).inverse().get(packet.getClass());
    }

    protected EnumConnectionState registerPacket(EnumPacketDirection enumPacketDirection, Class<? extends Packet> clazz) {
        HashBiMap hashBiMap = this.directionMaps.get((Object)enumPacketDirection);
        if (hashBiMap == null) {
            hashBiMap = HashBiMap.create();
            this.directionMaps.put(enumPacketDirection, (BiMap<Integer, Class<? extends Packet>>)hashBiMap);
        }
        if (hashBiMap.containsValue(clazz)) {
            String string = (Object)((Object)enumPacketDirection) + " packet " + clazz + " is already known to ID " + hashBiMap.inverse().get(clazz);
