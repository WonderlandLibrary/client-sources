...
implements IAdminCommand {
    ...
    @Override
    public void notifyOperators(ICommandSender iCommandSender, ICommand iCommand, int n, String string, Object ... objectArray) {
        boolean bl;
        boolean bl2 = true;
        MinecraftServer minecraftServer = MinecraftServer.getServer();
        if (!iCommandSender.sendCommandFeedback()) {
            bl2 = false;
        }
        ChatComponentTranslation chatComponentTranslation = new ChatComponentTranslation("chat.type.admin", iCommandSender.getName(), new ChatComponentTranslation(string, objectArray));
        ...
        if (bl2) {
            for (EntityPlayer entityPlayer : minecraftServer.getConfigurationManager().func_181057_v()) {
                boolean bl3;
                if (entityPlayer == iCommandSender || !minecraftServer.getConfigurationManager().canSendCommands(entityPlayer.getGameProfile()) || !iCommand.canCommandSenderUseCommand(iCommandSender)) continue;
                boolean bl4 = iCommandSender instanceof MinecraftServer && MinecraftServer.getServer().func_183002_r();
                boolean bl5 = bl3 = iCommandSender instanceof RConConsoleSource && MinecraftServer.getServer().func_181034_q();
                if (!bl4 && !bl3 && (iCommandSender instanceof RConConsoleSource || iCommandSender instanceof MinecraftServer)) continue;
                entityPlayer.addChatMessage(chatComponentTranslation);
            }
        }
        if (iCommandSender != minecraftServer && minecraftServer.worldServers[0].getGameRules().getBoolean("logAdminCommands")) {
            minecraftServer.addChatMessage(chatComponentTranslation);
        }
        boolean bl5 = minecraftServer.worldServers[0].getGameRules().getBoolean("sendCommandFeedback");
        if (iCommandSender instanceof CommandBlockLogic) {
            bl = ((CommandBlockLogic)iCommandSender).shouldTrackOutput();
        }
        if ((n & 1) != 1 && bl || iCommandSender instanceof MinecraftServer) {
            iCommandSender.addChatMessage(new ChatComponentTranslation(string, objectArray));
        }
    }
}
