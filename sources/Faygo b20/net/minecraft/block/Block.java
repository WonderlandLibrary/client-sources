...
import me.valk.event.events.blocks.EventBlockBoundingBox;
...
public class Block {
	...
	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		...
		if (Vital.getManagers().getModuleManager().getModuleFromClass(XrayMod.class).getState()) {
			return XrayMod.shouldRender(this, pos);
		}
	 ...
	 */
	  public void addCollisionBoxesToList(final World worldIn, final BlockPos pos, final IBlockState state, final AxisAlignedBB mask, final List list, final Entity collidingEntity) {
	        ...
	        final EventEntityCollision event = (EventEntityCollision)EventSystem.call(new EventEntityCollision(collidingEntity, new Location(pos.getX(), pos.getY(), pos.getZ()), var7, this));
	...
	public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
		EventBlockBoundingBox event = new EventBlockBoundingBox(
	...
	public static void spawnAsEntity(World worldIn, BlockPos pos, ItemStack stack) {
		if (!worldIn.isRemote && worldIn.getGameRules().getGameRuleBooleanValue("doTileDrops")) {
			...
			EntityItem var10 = new EntityItem(worldIn, (double) pos.getX() + var4, (double) pos.getY() + var6,
					(double) pos.getZ() + var8, stack);
		...
		if (!worldIn.isRemote) {
			while (amount > 0) {
				...
				worldIn.spawnEntityInWorld(new EntityXPOrb(worldIn, (double) pos.getX() + 0.5D,
						(double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D, var4));
			}
		}
	}
	...
	public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end) {
		...
		if (var11 == null) {
			...
			return new MovingObjectPosition(
					var11.addVector((double) pos.getX(), (double) pos.getY(), (double) pos.getZ()), var12, pos);
		}
	}
	...
	public static void registerBlocks() {
		...
		Block var1 = (new BlockPlanks()).setHardness(2.0F).setResistance(5.0F).setStepSound(soundTypeWood)
		...
		registerBlock(22, "lapis_block",
				(new BlockCompressed(MapColor.lapisColor)).setHardness(3.0F).setResistance(5.0F)
		...
		Block var4 = (new BlockMushroom()).setHardness(0.0F).setStepSound(soundTypeGrass)
				.setUnlocalizedName("mushroom");
		...
		registerBlock(53, "oak_stairs",
				(new BlockStairs(
						var1.getDefaultState().withProperty(BlockPlanks.VARIANT_PROP, BlockPlanks.EnumType.OAK)))
		...
		Block var6 = (new BlockFarmland()).setHardness(0.6F).setStepSound(soundTypeGravel)
		...
		Block var11 = (new BlockQuartz()).setStepSound(soundTypePiston).setHardness(0.8F)
		...
		Block var12 = (new BlockRedSandstone()).setStepSound(soundTypePiston).setHardness(0.8F)
