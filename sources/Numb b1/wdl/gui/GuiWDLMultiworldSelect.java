...
public class GuiWDLMultiworldSelect extends GuiTurningCameraBase {
	private class WorldGuiButton extends GuiButton {
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			...
			if (info != null && info == selectedMultiWorld) {
				drawRect(this.xPosition - 2, this.yPosition - 2, this.xPosition + width + 2,
						this.yPosition + height + 2, 0xFF007F00);
			}
	...
	/**
	 * Interface for a listener when the "cancel" or "use this world" buttons are
	 * clicked.
	 * 
	 * Note that implementations must <i>also</i> handle opening the correct next
	 * GUI.
	 */
	...
	public void initGui() {
		...
		this.cancelBtn = new GuiButton(-1, this.width / 2 - 155, this.height - 25, 150, 20, I18n.format("gui.cancel"));
		this.buttonList.add(this.cancelBtn);

		this.acceptBtn = new GuiButton(-2, this.width / 2 + 5, this.height - 25, 150, 20,
		...
		nextButton = new GuiButton(-5, this.width / 2 - offset + 25 + numWorldButtons * 155, y, 20, 20, ">");
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, title, this.width / 2, 8, 0xFFFFFF);

		this.drawCenteredString(this.fontRendererObj, I18n.format("wdl.gui.multiworldSelect.subtitle"), this.width / 2,
				18, 0xFF0000);
		...
		if (this.searchField.getText().isEmpty() && !this.searchField.isFocused()) {
			drawString(fontRendererObj, I18n.format("wdl.gui.multiworldSelect.search"), searchField.xPosition + 4,
	...
	/**
	 * Rebuilds the {@link #linkedWorldsFiltered} list after a change has occurred
	 * to the search or the {@link #linkedWorlds} list.
	 */
