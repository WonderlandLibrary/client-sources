...
public class GuiWDLEntities extends GuiScreen {
	private class GuiEntityList extends GuiListExtended {
		...
		private List<IGuiListEntry> entries = new ArrayList<IGuiListEntry>() {
			{
				try {
					...
					Map<String, Collection<String>> entities = EntityUtils.entitiesByGroup.asMap();
					for (Map.Entry<String, Collection<String>> e : entities.entrySet()) {
		...
		private class CategoryEntry implements GuiListExtended.IGuiListEntry {
			...
			public CategoryEntry(String group) {
				...
				this.groupEnabled = WDL.worldProps.getProperty("EntityGroup." + group + ".Enabled", "true")
						.equals("true");

				this.enableGroupButton = new GuiButton(0, 0, 0, 90, 18, getButtonText());
			}

			@Override
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
					boolean isSelected) {
				mc.fontRendererObj.drawString(this.group, (x + 110 / 2) - (this.labelWidth / 2),
						y + slotHeight - mc.fontRendererObj.FONT_HEIGHT - 1, 0xFFFFFF);
			...
			public boolean mousePressed(int slotIndex, int x, int y, int mouseEvent, int relativeX, int relativeY) {
				if (enableGroupButton.mousePressed(mc, x, y)) {
					...
					WDL.worldProps.setProperty("EntityGroup." + group + ".Enabled", Boolean.toString(groupEnabled));
					return true;
				}
				return false;
			}
		 ...
		 * 
		 * Based off of {@link net.minecraft.client.gui.GuiKeyBindingList.KeyEntry}.
		 */
		private class EntityEntry implements GuiListExtended.IGuiListEntry {
			...
			public EntityEntry(CategoryEntry category, String entity) {
				...
				entityEnabled = WDL.worldProps.getProperty("Entity." + entity + ".Enabled", "true").equals("true");
				range = EntityUtils.getEntityTrackDistance(entity);

				this.onOffButton = new GuiButton(0, 0, 0, 75, 18, getButtonText());
				this.onOffButton.enabled = category.isGroupEnabled();

				this.rangeSlider = new GuiSlider(1, 0, 0, 150, 18, "wdl.gui.entities.trackDistance", range, 256);
			...
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
				...
				mc.fontRendererObj.drawString(this.entity, center - largestWidth - 10,
						y + slotHeight / 2 - mc.fontRendererObj.FONT_HEIGHT / 2, 0xFFFFFF);
			...
			public boolean mousePressed(int slotIndex, int x, int y, int mouseEvent, int relativeX, int relativeY) {
				if (onOffButton.mousePressed(mc, x, y)) {
					...
					WDL.worldProps.setProperty("Entity." + entity + ".Enabled", Boolean.toString(entityEnabled));
					return true;
				}
				if (rangeSlider.mousePressed(mc, x, y)) {
					range = rangeSlider.getValue();

					WDL.worldProps.setProperty("Entity." + entity + ".TrackDistance", Integer.toString(range));
		...
		public GuiEntityList() {
			super(GuiWDLEntities.this.mc, GuiWDLEntities.this.width, GuiWDLEntities.this.height, 39,
	...
	@Override
	public void initGui() {
		this.buttonList.add(new GuiButton(200, this.width / 2 - 100, this.height - 29, "OK"));

		rangeModeButton = new GuiButton(100, this.width / 2 - 155, 18, 150, 20, getRangeModeText());
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, I18n.format("wdl.gui.entities.title"), this.width / 2, 8,
				0xFFFFFF);
