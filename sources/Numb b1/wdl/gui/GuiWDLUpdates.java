...
public class GuiWDLUpdates extends GuiScreen {
	...
	private class UpdateList extends GuiListExtended {
		public UpdateList() {
			super(GuiWDLUpdates.this.mc, GuiWDLUpdates.this.width, GuiWDLUpdates.this.height, TOP_MARGIN,
					GuiWDLUpdates.this.height - BOTTOM_MARGIN, (fontRendererObj.FONT_HEIGHT + 1) * 6 + 2);
		...
		private class VersionEntry implements IGuiListEntry {
			...
			public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
				...
				if (mouseX > x && mouseX < x + listWidth && mouseY > y && mouseY < y + slotHeight) {
					drawRect(x - 2, y - 2, x + listWidth - 3, y + slotHeight + 2, 0x1FFFFFFF);
				}
			}
			...
			public boolean mousePressed(int slotIndex, int x, int y, int mouseEvent, int relativeX, int relativeY) {
				if (relativeY > 0 && relativeY < slotHeight) {
					mc.displayGuiScreen(new GuiWDLSingleUpdate(GuiWDLUpdates.this, this.release));
		 ...
		 * 
		 * TODO: This is probably a bit laggy; cache this data? Right now it's being
		 * called each frame.
		 */
	...
	public void initGui() {
		...
		this.updateMinecraftVersionButton = new GuiButton(0, this.width / 2 - 155, 18, 150, 20,
				getUpdateMinecraftVersionText());
		this.buttonList.add(this.updateMinecraftVersionButton);
		this.updateAllowBetasButton = new GuiButton(1, this.width / 2 + 5, 18, 150, 20, getAllowBetasText());
		this.buttonList.add(this.updateAllowBetasButton);

		this.buttonList.add(new GuiButton(100, this.width / 2 - 100, this.height - 29, I18n.format("gui.done")));
	}
	...
	private String getUpdateMinecraftVersionText() {
		return I18n.format(
				"wdl.gui.updates.updateMinecraftVersion." + WDL.globalProps.getProperty("UpdateMinecraftVersion"),
				WDL.getMinecraftVersion());
	}

	private String getAllowBetasText() {
		return I18n.format("wdl.gui.updates.updateAllowBetas." + WDL.globalProps.getProperty("UpdateAllowBetas"));
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!WDLUpdateChecker.hasFinishedUpdateCheck()) {
			drawCenteredString(fontRendererObj, I18n.format("wdl.gui.updates.pleaseWait"), width / 2, height / 2,
					0xFFFFFF);
		} else if (WDLUpdateChecker.hasUpdateCheckFailed()) {
			String reason = WDLUpdateChecker.getUpdateCheckFailReason();

			drawCenteredString(fontRendererObj, I18n.format("wdl.gui.updates.checkFailed"), width / 2,
					height / 2 - fontRendererObj.FONT_HEIGHT / 2, 0xFF5555);
			drawCenteredString(fontRendererObj, I18n.format(reason), width / 2,
					height / 2 + fontRendererObj.FONT_HEIGHT / 2, 0xFF5555);
		}

		drawCenteredString(fontRendererObj, I18n.format("wdl.gui.updates.title"), width / 2, 8, 0xFFFFFF);

		super.drawScreen(mouseX, mouseY, partialTicks);

		if (updateMinecraftVersionButton.isMouseOver()) {
			Utils.drawGuiInfoBox(
					I18n.format("wdl.gui.updates.updateMinecraftVersion.description", WDL.getMinecraftVersion()), width,
					height, BOTTOM_MARGIN);
		} else if (updateAllowBetasButton.isMouseOver()) {
			Utils.drawGuiInfoBox(I18n.format("wdl.gui.updates.updateAllowBetas.description"), width, height,
					BOTTOM_MARGIN);
		}
	}
	...
	private class GuiWDLSingleUpdate extends GuiScreen {
		...
		@Override
		public void initGui() {
			this.buttonList.add(new GuiButton(0, this.width / 2 - 155, 18, 150, 20,
					I18n.format("wdl.gui.updates.update.viewOnline")));
			if (release.hiddenInfo != null) {
				this.buttonList.add(new GuiButton(1, this.width / 2 + 5, 18, 150, 20,
		...
		public void drawScreen(int mouseX, int mouseY, float partialTicks) {
			...
			this.drawCenteredString(this.fontRendererObj, buildReleaseTitle(release), this.width / 2, 8, 0xFFFFFF);
