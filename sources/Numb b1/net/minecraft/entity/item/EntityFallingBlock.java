...
public class EntityFallingBlock extends Entity {
	...
	public EntityFallingBlock(World worldIn, double p_i45848_2_, double p_i45848_4_, double p_i45848_6_,
			IBlockState p_i45848_8_) {
		super(worldIn);
	...
	public void onUpdate() {
		...
		if (var1.getMaterial() == Material.air) {
			...
			if (!this.worldObj.isRemote) {
				...
				if (this.onGround) {
					...
					if (this.worldObj.getBlockState(var2).getBlock() != Blocks.piston_extension) {
						...
						if (!this.field_145808_f
								&& this.worldObj.canBlockBePlaced(var1, var2, true, EnumFacing.UP, (Entity) null,
										(ItemStack) null)
								&& !BlockFalling.canFallInto(this.worldObj, var2.offsetDown())
								&& this.worldObj.setBlockState(var2, this.field_175132_d, 3)) {
				...
				} else if (this.fallTime > 100 && !this.worldObj.isRemote && (var2.getY() < 1 || var2.getY() > 256)
						|| this.fallTime > 600) {
		...
		if (this.hurtEntities) {
			...
			if (var4 > 0) {
				...
				while (var8.hasNext()) {
					...
					var9.attackEntityFrom(var7, (float) Math
							.min(MathHelper.floor_float((float) var4 * this.fallHurtAmount), this.fallHurtMax));
				}
				...
				if (var6 && (double) this.rand.nextFloat() < 0.05000000074505806D + (double) var4 * 0.05D) {
					...
					if (var10 > 2) {
						...
						this.field_175132_d = this.field_175132_d.withProperty(BlockAnvil.DAMAGE,
	...
	public void addEntityCrashInfo(CrashReportCategory category) {
		...
		if (this.field_175132_d != null) {
			...
			category.addCrashSection("Immitating block data",
					Integer.valueOf(var2.getMetaFromState(this.field_175132_d)));
		}
	}
