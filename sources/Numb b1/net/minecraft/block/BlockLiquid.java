...
public abstract class BlockLiquid extends Block {
	...
	protected int func_176362_e(IBlockAccess p_176362_1_, BlockPos p_176362_2_) {
		return p_176362_1_.getBlockState(p_176362_2_).getBlock().getMaterial() == this.blockMaterial
				? ((Integer) p_176362_1_.getBlockState(p_176362_2_).getValue(LEVEL)).intValue()
				: -1;
	}
	...
	public boolean isBlockSolid(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		...
		return var4 == this.blockMaterial ? false
				: (side == EnumFacing.UP ? true
						: (var4 == Material.ice ? false : super.isBlockSolid(worldIn, pos, side)));
	}
	...
	protected Vec3 func_180687_h(IBlockAccess p_180687_1_, BlockPos p_180687_2_) {
		...
		while (var5.hasNext()) {
			...
			if (var8 < 0) {
				if (!p_180687_1_.getBlockState(var7).getBlock().getMaterial().blocksMovement()) {
					...
					if (var8 >= 0) {
						var9 = var8 - (var4 - 8);
						var3 = var3.addVector((double) ((var7.getX() - p_180687_2_.getX()) * var9),
								(double) ((var7.getY() - p_180687_2_.getY()) * var9),
								(double) ((var7.getZ() - p_180687_2_.getZ()) * var9));
					}
				}
			} else if (var8 >= 0) {
				var9 = var8 - var4;
		...
		if (((Integer) p_180687_1_.getBlockState(p_180687_2_).getValue(LEVEL)).intValue() >= 8) {
			...
			while (var5.hasNext()) {
				...
				if (this.isBlockSolid(p_180687_1_, var7, var6)
						|| this.isBlockSolid(p_180687_1_, var7.offsetUp(), var6)) {
	...
	public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (this.blockMaterial == Material.water) {
			...
			if (var11 > 0 && var11 < 8) {
				...
				worldIn.spawnParticle(EnumParticleTypes.SUSPENDED, var5 + (double) rand.nextFloat(),
						var7 + (double) rand.nextFloat(), var9 + (double) rand.nextFloat(), 0.0D, 0.0D, 0.0D,
						new int[0]);
			}
		}
