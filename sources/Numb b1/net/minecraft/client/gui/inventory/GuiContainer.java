...
public abstract class GuiContainer extends GuiScreen {
	...
	private void drawSlot(Slot slotIn) {
		...
		if (slotIn == this.clickedSlot && this.draggedStack != null && this.isRightMouseClick && var4 != null) {
			...
			if (Container.canAddItemToSlot(slotIn, var7, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
				var4 = var7.copy();
				var5 = true;
				Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, var4,
	...
	private void updateDragSplitting() {
		...
		if (var1 != null && this.dragSplitting) {
			...
			for (Iterator var2 = this.dragSplittingSlots.iterator(); var2
					.hasNext(); this.dragSplittingRemnant -= var4.stackSize - var5) {
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		this.doubleClick = this.lastClickSlot == var5 && var6 - this.lastClickTime < 250L
		...
		if (mouseButton == 0 || mouseButton == 1 || var4) {
			...
			boolean var10 = mouseX < var8 || mouseY < var9 || mouseX >= var8 + this.xSize
					|| mouseY >= var9 + this.ySize;
			int var11 = -1;
	...
	protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
		...
		if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
			if (clickedMouseButton == 0 || clickedMouseButton == 1) {
				...
				} else if (this.draggedStack.stackSize > 1 && var6 != null
						&& Container.canAddItemToSlot(var6, this.draggedStack, false)) {
		...
		} else if (this.dragSplitting && var6 != null && var7 != null && var7.stackSize > this.dragSplittingSlots.size()
				&& Container.canAddItemToSlot(var6, var7, true) && var6.isItemValid(var7)
				&& this.inventorySlots.canDragIntoSlot(var6)) {
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		...
		if (this.doubleClick && var4 != null && state == 0
				&& this.inventorySlots.func_94530_a((ItemStack) null, var4)) {
			if (isShiftKeyDown()) {
				if (var4 != null && var4.inventory != null && this.shiftClickedSlot != null) {
					...
					while (var11.hasNext()) {
						...
						if (var10 != null && var10.canTakeStack(this.mc.thePlayer) && var10.getHasStack()
								...
								&& Container.canAddItemToSlot(var10, this.shiftClickedSlot, true)) {
			...
			if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
				...
				while (var11.hasNext()) {
					var10 = (Slot) var11.next();
					this.handleMouseClick(var10, var10.slotNumber, Container.func_94534_d(1, this.dragSplittingLimit),
							5);
				}
