...
public class ShaderManager {
	...
	public ShaderManager(IResourceManager resourceManager, String programName) throws JsonException {
		...
		try {
			...
			this.fragmentShaderLoader = ShaderLoader.loadShader(resourceManager, ShaderLoader.ShaderType.FRAGMENT,
					var28);
			...
			if (this.shaderSamplers.get(this.samplerNames.get(var1)) != null) {
				...
				if (var3 != -1) {
					...
					OpenGlHelper.glUniform1i(
							OpenGlHelper.glGetUniformLocation(this.program, (CharSequence) this.samplerNames.get(var1)),
							var1);
				}
			}
		}
	...
	private void setupUniforms() {
		...
		for (int var2 = 0; var1 < this.samplerNames.size(); ++var2) {
			...
			if (var4 == -1) {
				logger.warn("Shader " + this.programFilename + "could not find sampler named " + var3
