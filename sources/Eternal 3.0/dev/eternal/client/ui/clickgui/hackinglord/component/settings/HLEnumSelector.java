...
public class HLEnumSelector extends HLSetting {

  private final EnumSetting enumSetting;

  public HLEnumSelector(EnumSetting enumSetting) {
    super(enumSetting);
  ...
  public void drawSetting(double x, double y, int mouseX, int mouseY) {
    ...
    fr.drawString(String.format("%s: %s", enumSetting.name(), ((INameable) enumSetting.value()).getName()), x + 2, y + 3, -1);
  }

  @Override
  public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
    if (MouseUtils.isInArea(mouseX, mouseY, x, y, 85, getHeight())) {
      List<?> modeList = enumSetting.enumList();
      int index = modeList.indexOf(enumSetting.value()) + (mouseButton == 0 ? 1 : -1);
      enumSetting.value((INameable) (index > modeList.size() - 1 ? modeList.get(0) : index < 0 ? modeList.get(modeList.size() - 1) : modeList.get(index)));
      return true;
    }
    return false;
  }
