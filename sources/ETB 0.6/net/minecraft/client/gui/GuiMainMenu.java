...
import com.enjoytheban.ui.login.GuiAltManager;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	public GuiMainMenu() {
		...
		try {
			...
			if (getClass().getClassLoader().getClass().getName().hashCode() == -1258019345 + 1229) {
				Minecraft.getMinecraft().breaknigga = false;
			}
			if (!var2.isEmpty()) {
				do {
	...
	private void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_) {
		...
		this.buttonList.add(this.field_175372_K = new GuiButton(14, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2,
				I18n.format("Alt Login", new Object[0])));
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id == 0 && !mc.breaknigga) {
			this.mc.displayGuiScreen(new GuiOptions(this, this.mc.gameSettings));
		}

		if (button.id == 5 && !mc.breaknigga) {
			this.mc.displayGuiScreen(new GuiLanguage(this, this.mc.gameSettings, this.mc.getLanguageManager()));
		}

		if (button.id == 1 && !mc.breaknigga) {
			this.mc.displayGuiScreen(new GuiSelectWorld(this));
		}

		if (button.id == 2 && !mc.breaknigga) {
			this.mc.displayGuiScreen(new GuiMultiplayer(this));
		}
		if (button.id == 14 && this.field_175372_K.visible && !mc.breaknigga) {
			this.mc.displayGuiScreen(new GuiAltManager());
		}

		if (button.id == 4 && !mc.breaknigga) {
			this.mc.shutdown();
		}

		if (button.id == 11 && !mc.breaknigga) {
			this.mc.launchIntegratedServer("Demo_World", "Demo_World", DemoWorldServer.demoWorldSettings);
		}

		if (button.id == 12 && !mc.breaknigga) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		String var10 = Client.instance.name + " " + Client.instance.version;
		GlStateManager.popMatrix();
		GlStateManager.pushMatrix();
		...
		boolean blender = GL11.glIsEnabled(GL11.GL_BLEND);
		try {
			GL11.glPushMatrix();
			GL11.glEnable(GL11.GL_BLEND);
			GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
			GL11.glScaled(0.45, 0.45, 1.0);
			mc.fontRendererObj.drawStringWithShadow(AALAPI.getUsername(), 5, 2, new Color(255, 255, 255, 26).getRGB());
		...
		this.drawString(this.fontRendererObj, var10, 2, this.height - 10, new Color(102, 172, 255).getRGB());
		String var11 = AALAPI.getUsername().contains("debug-mode") ? "Protected by nothing" : "Protected by AAL";
		this.drawString(this.fontRendererObj, var11, this.width - this.fontRendererObj.getStringWidth(var11) - 2,
				this.height - 10, new Color(102, 172, 255).getRGB());
