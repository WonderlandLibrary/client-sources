...
import dev.star.module.impl.player.ChestStealer;
import dev.star.module.impl.player.InvManager;
...
import dev.star.utils.misc.RomanNumeralUtils;
...
public class HUDMod extends Module {
    ...
    private final ModeSetting watermarkMode = new ModeSetting("Watermark", "Starsense", "Star", "Plain Text", "Starsense", "Star1", "Logo", "None");
    ...
    public static final ModeSetting theme = Theme.getModeSetting("Theme", "Star");
    ...
    public HUDMod(UserLogin userLogin) {
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        if (e.isBloom()) {
            ...
            String intentInfo = Client.INSTANCE.user;
            switch (watermarkMode.getMode()) {
                case "Logo":
                    float WH = 110 / 2f;
                    float textWidth = BoldFont32.getStringWidth(finalName);

                    GL11.glEnable(GL11.GL_SCISSOR_TEST);
                    RenderUtil.scissor(10, 7, 13 + WH + textWidth + 5, WH);

                    BoldFont32.drawString(finalName, ((13 + WH) - textWidth) + (textWidth * fadeInText.getOutput().floatValue()), 8 + BoldFont32.getMiddleOfBox(WH), ColorUtil.applyOpacity( -1, fadeInText.getOutput().floatValue()));
                    GL11.glDisable(GL11.GL_SCISSOR_TEST);


                    GradientUtil.applyGradientCornerLR(27, 23, WH - 28, WH - 28, 1, clientColors.getSecond(), clientColors.getFirst(), () -> {
                        mc.getTextureManager().bindTexture(new ResourceLocation("Star/Images/watermarkBack.png"));
                ...
                case "Star1":
                    float xVal = 6f;
                    float yVal = 6f;
                    float versionWidth = Font16.getStringWidth(Client.INSTANCE.getVersion());
                    float versionX = xVal + BoldFont40.getStringWidth(finalName);
                    float width = version ? (versionX + versionWidth) - xVal : BoldFont40.getStringWidth(finalName);


                    RenderUtil.resetColor();
                    GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                        RenderUtil.setAlphaLimit(0);
                        BoldFont40.drawString(finalName, xVal, yVal, 0);
                        if (version) {
                            Font16.drawString(Client.INSTANCE.getVersion(), versionX, yVal, 0);
                        }
                    });
                ...
                case "Plain Text":
                    ...
                    break;
                case "Starsense":
                    String text = "§fstar§rsense§f" + " | " + Client.RELEASE.getName() + " (" + Client.VERSION + ") | " + (mc.isSingleplayer() ? "singleplayer" : mc.getCurrentServerData().serverIP);
                    float x = 4.5f, y = 4.5f;

                    Gui.drawRect2(x, y, Font16.getStringWidth(text) + 7, 18.5, Color.BLACK.getRGB());
                    break;
            }
        }
    }
    ...
    public void onRender2DEvent(Render2DEvent e) {
        ...
        switch (watermarkMode.getMode()) {
            case "Logo":
                ...
                BoldFont32.drawString(finalName, ((13 + WH) - textWidth) + (textWidth * fadeInText.getOutput().floatValue()), 8 + BoldFont32.getMiddleOfBox(WH), ColorUtil.applyOpacity(-1, .7f * fadeInText.getOutput().floatValue()));
                GL11.glDisable(GL11.GL_SCISSOR_TEST);
                ...
                RenderUtil.color(-1);
                GLUtil.startBlend();
                mc.getTextureManager().bindTexture(new ResourceLocation("Star/Images/watermarkT.png"));
                Gui.drawModalRectWithCustomSizedTexture(7, 7, 0, 0, WH, WH, WH, WH);
            ...
            case "Star1":
                ...
                GradientUtil.applyGradientHorizontal(xVal + spacing, yVal + spacing, width + spacing,
                        20, 1, darkerColors.getFirst(), darkerColors.getSecond(), () -> {
                            RenderUtil.setAlphaLimit(0);
                            BoldFont40.drawString(finalName1, xVal + spacing, yVal + spacing, 0);
                            if (version) {
                                Font16.drawString(Client.INSTANCE.getVersion(), versionX + (spacing / 2f), yVal + (spacing / 2f), 0);
                            }
                        });
                ...
                GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                    RenderUtil.setAlphaLimit(0);
                    BoldFont40.drawString(finalName1, xVal, yVal, 0);
                    if (version) {
                        Font16.drawString(Client.INSTANCE.getVersion(), versionX, yVal, 0);
                    }
                });
            ...
            case "Starsense":
                ...
                Gui.drawRect2(x, y, Font16.getStringWidth(text) + 7, 18.5, new Color(59, 57, 57).getRGB());

                Gui.drawRect2(x + 2.5, y + 2.5, Font16.getStringWidth(text) + 2, 13, new Color(23, 23, 23).getRGB());

                // Top small bar
                Gui.drawRect2(x + 1, y + 1, Font16.getStringWidth(text) + 5, .5, lineColor);

                // Bottom small bar
                Gui.drawRect2(x + 1, y + 17, Font16.getStringWidth(text) + 5, .5, lineColor);

                // Left bar
                Gui.drawRect2(x + 1, y + 1.5, .5, 16, lineColor);

                // Right Bar
                Gui.drawRect2((x + 1.5) + Font16.getStringWidth(text) + 4, y + 1.5, .5, 16, lineColor);

                // Lowly saturated rainbow bar
                GradientUtil.drawGradientLR(x + 2.5f, y + 14.5f, Font16.getStringWidth(text) + 2, 1, 1, clientColors.getFirst(), clientColors.getSecond());

                // Bottom of the rainbow bar
                Gui.drawRect2(x + 2.5, y + 16, Font16.getStringWidth(text) + 2, .5, lineColor);
                Font16.drawString(text, x + 4.5f, y + 5.5f, clientColors.getSecond().getRGB());
                break;
            case "Star":
                StringBuilder stringBuilder = new StringBuilder(name.replace("Star", "Star")).insert(1, "§7");
    ...
    private void drawBottomRight() {
        AbstractFontRenderer fr = customFont.isEnabled() ? Font20 : mc.fontRendererObj;
        ScaledResolution sr = new ScaledResolution(mc);
        ...
        text += " | " + Client.INSTANCE.user;
        text += " (" + Client.INSTANCE.uid + ")";
    ...
    private void drawInfo(Pair<Color, Color> clientColors) {
        ...
        //InfoStuff
        AbstractFontRenderer nameInfoFr = Font20;
        if (!customFont.isEnabled()) {
    ...
    private void drawArmor(ScaledResolution sr) {
        if (hudCustomization.getSetting("Armor HUD").isEnabled()) {
            ...
            // Draw armor and enchantment effects
            for (ItemStack itemStack : equipment) {
                armorPiece = itemStack;
                RenderHelper.enableGUIStandardItemLighting();
                x += 15;
                if (armorPiece == mc.thePlayer.getHeldItem()) {
                    // If you are holding an item, adjust the position so that it is a certain distance away from the armor
                    x -= 15;
                }
                ...
                // If the number of items is greater than 1, render the number of items
                if (armorPiece.stackSize > 1) {
                    String count = String.valueOf(armorPiece.stackSize);
                    mc.fontRendererObj.drawStringWithShadow(count,
                            -x + (float) sr.getScaledWidth() / 2 - 4 + 19 - 2 - mc.fontRendererObj.getStringWidth(count),
                ...
                // Draw enchantment effect
                RenderUtil.drawExhiEnchants(armorPiece, -x + (float) sr.getScaledWidth() / 2 - 4, sr.getScaledHeight() - (inWater ? 65 : 55) + s - 5 - (16 * GuiChat.openingAnimation.getOutput().floatValue()));
            }
        }
    }
