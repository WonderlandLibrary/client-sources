...
import dev.star.gui.clickguis.dropdown.components.settings.*;
...
import dev.star.utils.animations.impl.EaseInOutQuad;
import dev.star.utils.animations.impl.EaseOutSine;
...
public class ModuleRect implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        boolean hoveringText = HoveringUtil.isHovering(x + 5, y + Font18.getMiddleOfBox(height), Font18.getStringWidth(module.getName()), Font18.getHeight(), mouseX, mouseY);
        ...
        Pair<Color, Color> colors = theme.getColors().apply((color1, color2) -> {
            return Pair.of(ColorUtil.applyOpacity(color1, alpha), ColorUtil.applyOpacity(color2, 0.8f));
        });


        Color rectColor = new Color(15, 15, 15, (int) (150));
        Color textColor = ColorUtil.applyOpacity(Color.WHITE, alpha);

        //float textAlpha = .5f;
        Color moduleTextColor = ColorUtil.applyOpacity(textColor, .9F); //textAlpha + (.4f * toggleAnimation.getOutput().floatValue())
        ...
        if (module.isEnabled() || !toggleAnimation.isDone()) {
            RoundedUtil.drawGradientRound(x, y, width, height, 0, ColorUtil.applyOpacity(theme.getColors().getFirst(), toggleAnimation.getOutput().floatValue()), ColorUtil.applyOpacity(theme.getColors().getFirst(), toggleAnimation.getOutput().floatValue()), ColorUtil.applyOpacity(theme.getColors().getSecond(), toggleAnimation.getOutput().floatValue()), ColorUtil.applyOpacity(theme.getColors().getSecond(), toggleAnimation.getOutput().floatValue()));
        }

        RenderUtil.resetColor();

        Font20.drawString(module.getName(), x + width / 2F - Font20.getStringWidth(module.getName()) / 2F, y + Font20.getMiddleOfBox(height), moduleTextColor);


        Color settingRectColor = new Color(15, 15, 15, (int) (165));
