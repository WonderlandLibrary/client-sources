...
import me.nekoWare.client.event.events.PostPacketInEvent;
import me.nekoWare.proxymanager.ProxyAdapter;
...
//		Queues.newPriorityQueue();
    ...
    public void exceptionCaught(ChannelHandlerContext p_exceptionCaught_1_, Throwable p_exceptionCaught_2_) {
        ...
        this.closeChannel(new ChatComponentTranslation("disconnect.genericReason",
                new Object[]{"Internal Exception: " + p_exceptionCaught_2_}));
    }

    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) {
        if (this.channel.isOpen()) {
            PacketInEvent n = new PacketInEvent(p_channelRead0_2_);
            EventAPI.fire(n);

            if (n.isCancelled())
                return;
            try {
                n.getPacket().processPacket(this.packetListener);
            } catch (ThreadQuickExitException var4) {
                ;
            }
            PostPacketInEvent p = new PostPacketInEvent(p_channelRead0_2_);
            EventAPI.fire(p);
        }
    }
    ...
    public static NetworkManager provideLanClient(InetAddress p_150726_0_, int p_150726_1_) {
        if (Nekoware.INSTANCE.proxy != Proxy.NO_PROXY) {
            ...
            })).channelFactory(new ProxyAdapter(Nekoware.INSTANCE.proxy))).connect(p_150726_0_, p_150726_1_).syncUninterruptibly();
            return var2;
        }
        final NetworkManager var2 = new NetworkManager(EnumPacketDirection.CLIENTBOUND);
        ((Bootstrap) ((Bootstrap) ((Bootstrap) (new Bootstrap())
                .group((EventLoopGroup) CLIENT_NIO_EVENTLOOP.getValue())).handler(new ChannelInitializer() {
            private static final String __OBFID = "CL_00002312";
    ...
    public static class PacketHandler extends SimpleChannelInboundHandler {
        @Override
        public void channelRead0(ChannelHandlerContext ctx, Object msg) {
            try {
                PacketInEvent n = new PacketInEvent((Packet) msg);
                EventAPI.fire(n);
...
//    			System.out.println(msg);
    ...
    public static NetworkManager provideLocalClient(SocketAddress p_150722_0_) {
        ...
        ((Bootstrap) ((Bootstrap) ((Bootstrap) (new Bootstrap())
                .group((EventLoopGroup) CLIENT_LOCAL_EVENTLOOP.getValue())).handler(new ChannelInitializer() {
