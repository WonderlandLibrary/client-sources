...
import me.nekoWare.client.module.combat.TargetStrafe;
...
public class EntityPlayerSP extends AbstractClientPlayer {
		...
		if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			...
			if (this.isRiding()) {
				...
				this.sendQueue.addToSendQueue(new C0CPacketInput(this.moveStrafing, this.moveForward,
						this.movementInput.isJumping(), this.movementInput.isSneaking()));
			} else {
				this.func_175161_p();
			}
		}
	...
	public void moveEntity(double x, double y, double z) {
		MovementEvent e = new MovementEvent(x, y, z);
		EventAPI.fire(e);
		if (e.isCancelled())
			return;
	...
	public void func_175161_p() {
		UpdateEvent event = new UpdateEvent(posX, boundingBox.minY, posZ, rotationYaw, rotationPitch, onGround);
		EventAPI.fire(event);
		...
		if (this.func_175160_A()) {
			...
			if (this.ridingEntity == null) {
				if (var13 && var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getX(), event.getY(),
							event.getZ(), event.getYaw(), event.getPitch(), event.getGroundState()));
				} else if (var13) {
	...
	public boolean isSneaking() {
		boolean var1 = this.movementInput != null && this.movementInput.isSneaking();
	...
	public void updateEntityActionState() {
		...
		if (this.func_175160_A()) {
			this.moveStrafing = this.movementInput.getStrafe();
			this.moveForward = this.movementInput.getForward();
			this.isJumping = this.movementInput.isJumping();
	...
	public void onLivingUpdate() {
		...
		boolean var1 = this.movementInput.isJumping();
		boolean var2 = this.movementInput.isSneaking();
		float var3 = 0.8F;
		boolean var4 = this.movementInput.getForward() >= var3;
		this.movementInput.updatePlayerMoveState();
		if (this.isUsingItem() && !this.isRiding()) {
			this.movementInput.setStrafe(this.movementInput.getStrafe() * 0.2F);
			this.movementInput.setForward(this.movementInput.getForward() * 0.2F);
		...
		if (this.onGround && !var2 && !var4 && this.movementInput.getForward() >= var3 && !this.isSprinting() && var5
		...
		if (!this.isSprinting() && this.movementInput.getForward() >= var3 && var5 && !this.isUsingItem()
				&& !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.getIsKeyPressed()) {
			this.setSprinting(true);
		}

		if (this.isSprinting() && (this.movementInput.getForward() < var3 || this.isCollidedHorizontally || !var5)) {
			this.setSprinting(false);
		}
		...
		if (this.capabilities.allowFlying) {
			...
			} else if (!var1 && this.movementInput.isJumping()) {
		...
		if (this.isRidingHorse()) {
			...
			if (var1 && !this.movementInput.isJumping()) {
	...
	public boolean isMoving() {
		return (TargetStrafe.canStrafe() ? TargetStrafe.dir != 0 : false)
				|| (this.movementInput.getForward() != 0 || this.movementInput.getStrafe() != 0);
	}
