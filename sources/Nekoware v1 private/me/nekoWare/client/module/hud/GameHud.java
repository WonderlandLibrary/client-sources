package me.nekoWare.client.module.hud;
...
import me.nekoWare.client.util.render.ColorUtils;
...
//	if (mc.thePlayer != null) {
	...
	//	}
	mainFont.drawStringWithShadow(String.format(("Nekoware"),
			Nekoware.INSTANCE.getEdition(), Nekoware.INSTANCE.getVersion()), 5.0f, 5.0f, -1);
	...
	private final void drawModuleNames(ScaledResolution scaledResolution) {
		 ...
		 */
		int yPos = 0;
		for (Module mod : Nekoware.INSTANCE.getModuleManager().getModules()) {
			if (mod.isToggled()) {
	...
	private final void sortIfNeeded() {
		if (GameHud.sortingUpdate) {
			Nekoware.INSTANCE.getModuleManager().getModules().sort((mod, mod1) -> {
				final String name = mod.getName() + mod.getMode(true);
