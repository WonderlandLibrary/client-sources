...
import me.nekoWare.client.util.blocks.ScaffoldUtils;
import me.nekoWare.client.util.blocks.misc.BlockInfo;
	...
	public Consumer<UpdateEvent> sexoButthole = (event) -> {
		if (event.isPost()) {
			...
			if (tempSlot != -1) {
				...
				BlockPos blockBelow1 = new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 0.5, mc.thePlayer.posZ);
				...
				if (mc.theWorld.getBlockState(blockBelow1).getBlock() == Blocks.air) {
					...
					mc.thePlayer.rotationYawHead = rots[1];
			...
			if (this.data != null && this.timer.delay(900L) && this.slot != -1) {
				mc.thePlayer.inventory.currentItem = newSlot;
