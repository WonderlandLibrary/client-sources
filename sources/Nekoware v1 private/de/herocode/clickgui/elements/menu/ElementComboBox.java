...
public class ElementComboBox extends Element {
     ...
     * Konstrukor
     */
    public ElementComboBox(ModuleButton iparent, Option iset) {
        parent = iparent;
        set = iset;
        super.setup();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (comboextended) {
            ...
            for (String sld : set.getValues()) {
                 ...
                 */
                if (sld.equalsIgnoreCase(set.getValue())) {
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0) {
            ...
            double ay = y + 15;
            for (String slcd : set.getValues()) {
                if (mouseX >= x && mouseX <= x + width && mouseY >= ay && mouseY <= ay + FontUtil.getFontHeight() + 2) {
                    if (clickgui != null && clickgui.setmgr != null) set.setValue(slcd);
                    return true;
                }
                ay += FontUtil.getFontHeight() + 2;
            }
