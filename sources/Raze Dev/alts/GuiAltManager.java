...
public class GuiAltManager extends GuiScreen{
	...
	private String status = EnumChatFormatting.GRAY + "No alts selected"; public void actionPerformed(GuiButton button) throws IOException {
		...
		case 0:
			...
			if (!loginThread.getStatus().equals(EnumChatFormatting.YELLOW + "Attempting to log in") && !loginThread.getStatus().equals(EnumChatFormatting.RED + "Do not press back!" + EnumChatFormatting.YELLOW + " Logging in...")) {
				mc.displayGuiScreen(null);
				break;
			} 
		...
		case 2:
			...
			altManager = RazeClient.getAltManager();
		...
		case 5:
			...
			status = EnumChatFormatting.GRAY + "Selected random alt from list!";
			selectedAlt = randAlt;
			break;
		case 6:
			String generatedAlt = altGen.generateUsername();
			mc.session = new Session(generatedAlt, "", "", "legacy");
			status = EnumChatFormatting.GRAY + "Generated random alt!";
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		GL11.glEnable(3089);
		int y2 = 38;
		AltManager altManager2 = RazeClient.getAltManager();
	...
	protected void mouseClicked(int par1, int par2, int par3) throws IOException {
		...
		int y2 = 38 - offset;
		AltManager altManager = RazeClient.getAltManager();
		for (Alt alt2 : AltManager.registry) {
