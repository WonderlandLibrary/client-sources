...
import appu26j.mods.visuals.Perspective;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures)
    {
        ...
        else
        {
            ...
            float[] finalColors = new float[] {colors[0] / 255F, colors[1] / 255F, colors[2] / 255F};
            
            if (flag1)
            {
                if (damageTint.isEnabled())
                {
                    this.brightnessBuffer.put(finalColors[0]);
                    this.brightnessBuffer.put(finalColors[1]);
                    this.brightnessBuffer.put(finalColors[2]);
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            if (this.canRenderName(entity))
            {
                ...
                try
                {
                    aBoolean = networkPlayerInfo != null && entity instanceof EntityPlayer && EnumChatFormatting.getTextWithoutFormattingCodes(entity.getDisplayName().getUnformattedText()).contains(entity.getName()) && (Apple.CLIENT.getPeopleUsingAppleClient().stream().filter(id -> networkPlayerInfo.getGameProfile().getId().toString().replaceAll("-", "").equals(id)).findFirst().orElse(null) != null);
                }
                ...
                if (d0 < (double)(f * f))
                {
                    ...
                    if (entity.isSneaking())
                    {
                        ...
                        {
                            viewX = Minecraft.getMinecraft().gameSettings.thirdPersonView == 2 ? -Perspective.getRotationPitch() : Perspective.getRotationPitch();
                            ...
                            GlStateManager.rotate(-Perspective.getRotationYaw(), 0.0F, 1.0F, 0.0F);
                            ...
                            if (aBoolean)
                            {
                                ...
                                if (networkPlayerInfo.getGameProfile().getId().toString().replace("-", "").equals("cb6b6dd401d345abbbe646670c674b7b"))
                                {
                                    Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("icons/golden_apple.png"));
                                }
                            ...
                            if (damageIndicator.isEnabled() && entity instanceof EntityPlayer && s.contains(entity.getName()))
                            {
                                ...
                                float maxHearts = ((EntityLivingBase) entity).getMaxHealth();
                                ...
                                if (System.currentTimeMillis() <= (entity.time + 500))
                                {
                                    entity.flag = Minecraft.getMinecraft().thePlayer.ticksExisted % 5 == 0;
                                }
                                ...
                                for (i = 0; i < maxHearts / 2; i++)
                                {
                                    ...
                                    Gui.drawModalRectWithCustomSizedTexture((i * 8) - (maxHearts * 2), -10, entity.flag ? 25 : 16, 0, 9, 9, 256, 256);
                                }
    ...
    protected boolean canRenderName(T entity)
    {
        ...
        return Minecraft.isGuiEnabled() && (entity != this.renderManager.livingPlayer || (nameTags.isEnabled() && nameTags.getSetting("Show self nametag").getCheckBoxValue())) && !entity.isInvisibleToPlayer(entityplayersp) && entity.riddenByEntity == null;
    }
