...
public class Panel {
	...
	public Panel(int x,Category category, ClickGui clickGUI) {
		...
		int count = 0;
		for(ModuleBase mod : SRT.instance.moduleManager.getModules()) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		mc.fontRendererObj.drawStringWithShadow(category.name, (float)(x + 5), (float)(y + ((height - mc.fontRendererObj.FONT_HEIGHT) / 2)), -1);
		if(open) {
			int count = 0;
			double lButHeight = 0;
