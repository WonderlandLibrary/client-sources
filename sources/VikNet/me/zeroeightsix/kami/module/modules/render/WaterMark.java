...
public class WaterMark extends Module
{
    ...
    public WaterMark() {
        this.x = this.register(Settings.f("InfoX", 15.0f));
        this.y = this.register(Settings.f("InfoY", 220.0f));
        this.rainbow = this.register(Settings.b("Rainbow", false));
        this.smooth = this.register(Settings.b("Smooth Font", true));
        this.red = this.register((Setting<Integer>)Settings.integerBuilder("Red").withRange(0, 255).withValue(143).build());
    ...
    public void onRender() {
        ...
        if (this.smooth.getValue()) {
            this.cFontRenderer.drawStringWithShadow("VikNet\u00A72:^) " + "", this.x.getValue(), (yCount += 10.0f) - this.cFontRenderer.getHeight() - 1.0f, color);
        }
        else {
            Wrapper.getMinecraft().fontRenderer.drawStringWithShadow("VikNet\u00A72:^) " + "", (float)this.x.getValue(), (yCount += 10.0f) - Wrapper.getMinecraft().fontRenderer.FONT_HEIGHT, color);
