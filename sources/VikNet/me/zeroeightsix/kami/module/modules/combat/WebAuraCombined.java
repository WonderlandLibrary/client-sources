...
/**
 * heph autotrap made into webaura by vik
 */
@Module.Info(name = "WebAuraCombined", category = Module.Category.COMBAT)
public class WebAuraCombined extends Module {

    private Setting<Double> range = register(Settings.doubleBuilder("Range").withMinimum(3.5).withValue(6.0).withMaximum(32.0).build());
    ...
    private Setting<Cage> cage = register(Settings.e("Cage", Cage.FULL));
    ...
    public void onUpdate() {
        ...
        if (firstRun) {
            ...
            if (announceUsage.getValue()) {
                Command.sendChatMessage("[WebAuraCombined] " + ChatFormatting.GREEN.toString() + "On" + ChatFormatting.RESET.toString() + ", target: " + lastTickTargetName);
            }
        } else if (!lastTickTargetName.equals(closestTarget.getName())) {
            lastTickTargetName = closestTarget.getName();
            offsetStep = 0;
            if (announceUsage.getValue()) {
                Command.sendChatMessage("[WebAuraCombined] New target: " + lastTickTargetName);
            }
        }

        List<Vec3d> placeTargets = new ArrayList<>();

        if (cage.getValue().equals(Cage.FULL)) {
            Collections.addAll(placeTargets, Offsets.FULL);
        }

        if (cage.getValue().equals(Cage.FEET)) {
            Collections.addAll(placeTargets, Offsets.FEET);
        }

        if (cage.getValue().equals(Cage.BODY)) {
