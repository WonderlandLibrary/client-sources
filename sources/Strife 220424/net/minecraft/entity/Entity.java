...
import club.strifeclient.module.implementations.combat.HitBox;
...
public abstract class Entity implements ICommandSender {
	...
	protected void setSize(float width, float height) {
		if (width != this.width || height != this.height) {
			...
			this.width = width;
			this.height = height;
			this.setEntityBoundingBox(new AxisAlignedBB(this.getEntityBoundingBox().minX,
	...
	public void setPosition(double x, double y, double z) {
		...
		this.setEntityBoundingBox(new AxisAlignedBB(x - (double) f,
		                                            y,
		                                            z - (double) f,
		                                            x + (double) f,
		                                            y + (double) f1,
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			if (flag) {
				...
				for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this,
				...
				for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this,
				                                                            this.getEntityBoundingBox()
				                                                                .offset(0.0D, -1.0D, z)
				).isEmpty(); d5 = z) {
					if (z < d6 && z >= -d6) {
				...
				for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this,
				                                                                         this.getEntityBoundingBox()
				                                                                             .offset(x, -1.0D, z)
				).isEmpty(); d5 = z) {
					if (x < d6 && x >= -d6) {
			...
			if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
				...
				if (this.distanceWalkedOnStepModified > (float) this.nextStepDistance &&
					...
					if (this.isInWater()) {
						...
						this.playSound(this.getSwimSound(),
						               f,
			...
			try {
				...
				CrashReportCategory crashreportcategory = crashreport.makeCategory("Entity being checked for " +
				                                                                   "collision");
	...
	protected void playStepSound(BlockPos pos, Block blockIn) {
		...
		if (this.worldObj.getBlockState(pos.up()).getBlock() == Blocks.snow_layer) {
			...
			this.playSound(block$soundtype.getStepSound(),
			               block$soundtype.getVolume() * 0.15F,
	...
	public boolean handleWaterMovement() {
		if (this.worldObj.handleMaterialAcceleration(this.getEntityBoundingBox()
		                                                 .expand(0.0D, -0.4000000059604645D, 0.0D)
		                                                 .contract(0.001D, 0.001D, 0.001D), Material.water, this)) {
	...
	public void moveFlying(float strafe, float forward, float friction) {
		...
		if (this instanceof EntityPlayerSP) {
			StrafeEvent event = new StrafeEvent(strafe, forward, friction, rotationYaw);
	...
	public void moveToBlockPosAndAngles(BlockPos pos, float rotationYawIn, float rotationPitchIn) {
		this.setLocationAndAngles((double) pos.getX() + 0.5D,
		                          pos.getY(),
	...
	public double getDistanceSqFromVector(Vec3 vec) {
		double d0 = this.posX - vec.xCoord;
		double d1 = this.posY - vec.yCoord;
		double d2 = this.posZ - vec.zCoord;
		return d0 * d0 + d1 * d1 + d2 * d2;
	}
	...
	public double getDistanceHorizontal(double x, double z) {
		double d0 = this.posX - x;
		double d2 = this.posZ - z;
		return MathHelper.hypot_double(d0, d2);
	}

	public double getDistanceVertical(double y) {
		double d1 = this.posY - y;
	...
	public Vec3 getPositionEyes(float partialTicks) {
		if (partialTicks == 1.0F) {
			...
			double d1 =
					this.prevPosY + (this.posY - this.prevPosY) * (double) partialTicks + (double) this.getEyeHeight();
		...
		if (this.noClip) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				int k = MathHelper.floor_double(
						this.posX + (double) (((float) ((i >> 1) % 2) - 0.5F) * this.width * 0.8F));
				int l = MathHelper.floor_double(
						this.posZ + (double) (((float) ((i >> 2) % 2) - 0.5F) * this.width * 0.8F));
	...
	public void updateRidden() {
		if (this.ridingEntity.isDead) {
			...
			if (this.ridingEntity != null) {
				...
				for (this.entityRiderPitchDelta +=
						     this.ridingEntity.rotationPitch - this.ridingEntity.prevRotationPitch;
				     this.entityRiderYawDelta >= 180.0D;
		...
		if (entityIn == null) {
			if (this.ridingEntity != null) {
				this.setLocationAndAngles(this.ridingEntity.posX,
				                          this.ridingEntity.getEntityBoundingBox().minY +
				                          (double) this.ridingEntity.height,
	...
	public void setPositionAndRotation2(double x,
		...
		List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this,
		                                                                   this.getEntityBoundingBox()
		                                                                       .contract(0.03125D, 0.0D, 0.03125D)
		);
	...
	public float getCollisionBorderSize() {
		if (hitBox == null)
			hitBox = Client.INSTANCE.getModuleManager().getModule(HitBox.class);
		return hitBox.isEnabled() && this instanceof EntityPlayer ? hitBox.sizeSetting.getFloat() : 0.1F;
	}
	...
	public void setPortal(BlockPos pos) {
		if (this.timeUntilPortal > 0) {
			...
			if (!this.worldObj.isRemote && !pos.equals(this.lastPortalPos)) {
				...
				double d0 = blockpattern$patternhelper.getFinger().getAxis() ==
				            EnumFacing.Axis.X ? (double) blockpattern$patternhelper.getPos()
				                                                                   .getZ() :
						(double) blockpattern$patternhelper.getPos()
						                                   .getX();
				double d1 =
						blockpattern$patternhelper.getFinger().getAxis() == EnumFacing.Axis.X ? this.posZ : this.posX;
				d1 = Math.abs(MathHelper.func_181160_c(d1 - (double) (
						                                       blockpattern$patternhelper.getFinger().rotateY().getAxisDirection() ==
						                                       EnumFacing.AxisDirection.NEGATIVE ? 1 : 0),
				                                       d0,
	...
	public String toString() {
		return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]",
		                     ...
		                     Double.valueOf(this.posX),
		                     Double.valueOf(this.posY),
	...
	public void copyLocationAndAnglesFrom(Entity entityIn) {
		this.setLocationAndAngles(entityIn.posX,
		                          entityIn.posY,
		                          entityIn.posZ,
	...
	public void addEntityCrashInfo(CrashReportCategory category) {
		...
		category.addCrashSection("Entity's Momentum",
		                         String.format("%.2f, %.2f, %.2f",
		                                       Double.valueOf(this.motionX),
		                                       Double.valueOf(this.motionY),
	...
	public EnumFacing getHorizontalFacing() {
		return EnumFacing.getHorizontal(
				MathHelper.floor_double((double) (this.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3);
	}
	...
	public Vec3 getMotionVector() {
		return new Vec3(this.motionX, this.onGround ? 0 : this.motionY, this.motionZ);
	}
