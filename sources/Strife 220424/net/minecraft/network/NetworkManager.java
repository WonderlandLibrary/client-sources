...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                PacketInboundEvent event = new PacketInboundEvent(p_channelRead0_2_);
    ...
    public void sendPacket(Packet packetIn, boolean silent) {
        final PacketOutboundEvent event = new PacketOutboundEvent(packetIn);
        if(!silent)
            Client.INSTANCE.getEventBus().call(event);
        if (event.isCancelled()) return;
        if (this.isChannelOpen()) {
            ...
            try {
                this.outboundPacketsQueue.add(new NetworkManager.InboundHandlerTuplePacketListener(event.packet, (GenericFutureListener[]) null));
            } finally {
    ...
    public void sendPacket(Packet packetIn, GenericFutureListener<? extends Future<? super Void>> listener, GenericFutureListener<? extends Future<? super Void>>... listeners) {
        ...
        if (this.isChannelOpen()) {
            ...
            try {
                this.outboundPacketsQueue.add(new NetworkManager.InboundHandlerTuplePacketListener(event.packet, ArrayUtils.add(listeners, 0, listener)));
            } finally {
                this.readWriteLock.writeLock().unlock();
            }
        }
