...
import club.strifeclient.event.implementations.system.MouseEvent;
import club.strifeclient.event.implementations.world.LoadWorldEvent;
import club.strifeclient.module.implementations.player.FastUse;
import club.strifeclient.module.implementations.visual.Ambience;
...
import club.strifeclient.ui.implementations.ingame.GuiIngameHook;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	public Minecraft(GameConfiguration gameConfig) {
		...
		this.mcDefaultResourcePack = new DefaultResourcePack(
			(new ResourceIndex(gameConfig.folderInfo.assetsDir,
	...
	public void run() {
		...
		do {
			try {
				...
				CrashReport crashreport1 = this.addGraphicsAndWorldToCrashReport(
					new CrashReport("Unexpected error", throwable1));
				this.freeMemory();
	...
	private void startGame() throws LWJGLException {
		...
		if (this.gameSettings.language != null) {
			...
			this.fontRendererObj.setBidiFlag(
				this.mcLanguageManager.isCurrentLanguageBidirectional());
		}
		...
		AchievementList.openInventory.setStatStringFormatter(str -> {
			try {
				return String.format(str, GameSettings.getKeyDisplayString(
		...
		this.renderEngine.loadTickableTexture(TextureMap.locationBlocksTexture,
			this.textureMapBlocks);
...
//        UltralightEngine.INSTANCE.initEngine();
	...
	private void registerMetadataSerializers() {
		...
		this.metadataSerializer_.registerMetadataSectionType(
			new AnimationMetadataSectionSerializer(), AnimationMetadataSection.class);
		this.metadataSerializer_.registerMetadataSectionType(new PackMetadataSectionSerializer(),
			PackMetadataSection.class);
		this.metadataSerializer_.registerMetadataSectionType(
			new LanguageMetadataSectionSerializer(), LanguageMetadataSection.class);
	}
	...
	private static ByteBuffer readImage(InputStream in, IntBuffer x, IntBuffer y,
		IntBuffer channels) throws IOException {
	...
	public void setIcon(final InputStream icon16, final InputStream icon32) throws IOException {
		MemoryStack memoryStack = MemoryStack.stackPush();
		IntBuffer intBuffer = memoryStack.mallocInt(1);
		IntBuffer intBuffer2 = memoryStack.mallocInt(1);
		IntBuffer intBuffer3 = memoryStack.mallocInt(1);
		GLFWImage.Buffer buffer = GLFWImage.mallocStack(2, memoryStack);
		ByteBuffer byteBuffer = readImage(icon16, intBuffer, intBuffer2, intBuffer3);
		if (byteBuffer == null) {
		...
		ByteBuffer byteBuffer2 = readImage(icon32, intBuffer, intBuffer2, intBuffer3);
		if (byteBuffer2 == null) {
			throw new IllegalStateException(
				"Could not load icon: " + STBImage.stbi_failure_reason());
		}
		...
		buffer.position(0);
		GLFW.glfwSetWindowIcon(Display.Window.handle, buffer);
	...
	public void setWindowIcon() {
		...
		if (util$enumos != Util.EnumOS.OSX) {
			...
			try {
				inputstream = this.mcDefaultResourcePack.getInputStreamAssets(new ResourceLocation(
					"icons/icon_16x16.png"));
				inputstream1 = this.mcDefaultResourcePack.getInputStreamAssets(new ResourceLocation(
					"icons/icon_32x32.png"));

				if (inputstream != null && inputstream1 != null) {
					Display.setIcon(inputstream, inputstream1);
				}
	...
	public boolean isUnicode() {
		return this.mcLanguageManager.isCurrentLocaleUnicode()
			|| this.gameSettings.forceUnicodeFont;
	}

	public void refreshResources() {
		List<IResourcePack> list = Lists.newArrayList(this.defaultResourcePacks);

		for (ResourcePackRepository.Entry resourcepackrepository$entry :
			this.mcResourcePackRepository.getRepositoryEntries()) {
	...
	private ByteBuffer readImageToBuffer(InputStream imageStream) throws IOException {
		...
		int[] aint = bufferedimage.getRGB(0,
			0,
		...
		if (!set.contains(displaymode) && Util.getOSType() == Util.EnumOS.OSX) {
			...
			for (DisplayMode displaymode1 : macDisplayModes) {
				...
				if (!flag) {
					...
					} while (displaymode3.getBitsPerPixel() != 32 ||
	...
	private void drawSplashScreen(TextureManager textureManagerInstance) {
		...
		try {
			...
			this.mojangLogo = textureManagerInstance.getDynamicTextureLocation("logo",
				new DynamicTexture(ImageIO.read(
					inputstream))
			);
		...
		worldrenderer.pos(0.0D, this.displayHeight, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255, 255)
			.endVertex();
		worldrenderer.pos(this.displayWidth, this.displayHeight, 0.0D)
			.tex(0.0D, 0.0D)
			.color(255, 255, 255, 255)
			.endVertex();
		worldrenderer.pos(this.displayWidth, 0.0D, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255, 255)
			.endVertex();
		...
		int j = 256;
		int k = 256;
		this.draw((scaledresolution.getScaledWidth() - j) / 2,
			(scaledresolution.getScaledHeight() - k) / 2,
			0,
			0,
			j,
			k,
	...
	public void draw(int posX,
		...
		worldrenderer.pos(posX, posY + height, 0.0D)
			.tex((float) texU * f, (float) (texV + height) * f1).color(red,
				green,
				blue,
				alpha
			).endVertex();
		worldrenderer.pos(posX + width, posY + height, 0.0D).tex((float) (texU + width) * f,
			(float) (texV + height) * f1
		).color(red, green, blue, alpha).endVertex();
		worldrenderer.pos(posX + width, posY, 0.0D)
			.tex((float) (texU + width) * f, (float) texV * f1).color(red,
				green,
				blue,
				alpha
			).endVertex();
		worldrenderer.pos(posX, posY, 0.0D)
			.tex((float) texU * f, (float) texV * f1)
	...
	private void runGameLoop() throws IOException {
		...
		++this.fpsCounter;
		this.isGamePaused =
			this.isSingleplayer() && this.currentScreen != null
		...
		while (getSystemTime() >= this.debugUpdateTime + 1000L) {
			...
			this.debug = String.format("%d fps (%d chunk update%s) T: %s%s%s%s%s",
				...
				(float) this.gameSettings.limitFramerate ==
					GameSettings.Options.FRAMERATE_LIMIT.getValueMax() ? "inf" :
	...
	public int getLimitFramerate() {
		return this.theWorld == null && this.currentScreen != null ? 120
			: this.gameSettings.limitFramerate;
	}

	public boolean isFramerateLimitBelowMax() {
		return (float) this.getLimitFramerate()
			< GameSettings.Options.FRAMERATE_LIMIT.getValueMax();
	}
	...
	private void updateDebugProfilerName(int keyCount) {
		...
		if (list != null && !list.isEmpty()) {
			...
			if (keyCount == 0) {
				...
				if (keyCount < list.size() && !list.get(keyCount).field_76331_c.equals(
					"unspecified")) {
	...
	private void displayDebugInfo() {
		if (this.mcProfiler.profilingEnabled) {
			...
			GlStateManager.loadIdentity();
			GlStateManager.ortho(0.0D, this.displayWidth, this.displayHeight, 0.0D, 1000.0D,
				3000.0D);
			GlStateManager.matrixMode(5888);
			GlStateManager.loadIdentity();
			GlStateManager.translate(0.0F, 0.0F, -2000.0F);
			...
			worldrenderer.pos((float) j - (float) i * 1.1F, (float) k - (float) i * 0.6F - 16.0F,
				0.0D).color(200,
				0,
				0,
				0
			...
			for (int l = 0; l < list.size(); ++l) {
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					float f = (float) (
						(d0 + profiler$result1.field_76332_a * (double) j2 / (double) i1) * Math.PI
							* 2.0D /
							100.0D);
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					float f3 = (float) (
						(d0 + profiler$result1.field_76332_a * (double) i3 / (double) i1) * Math.PI
							* 2.0D /
							100.0D);
					...
					worldrenderer.pos((float) j + f4, (float) k - f5, 0.0D)
						...
						.endVertex();
					worldrenderer.pos((float) j + f4, (float) k - f5 + 10.0F, 0.0D).color(k1 >> 1,
						l1 >> 1,
						i2 >> 1,
						255
					).endVertex();
			...
			int l2 = 16777215;
			this.fontRendererObj.drawStringWithShadow(s, (float) (j - i), (float) (k - i / 2 - 16),
				l2);
			...
			for (int k2 = 0; k2 < list.size(); ++k2) {
				...
				this.fontRendererObj.drawStringWithShadow(
					s1 = decimalformat.format(profiler$result2.field_76332_a) +
						"%",
					(float) (j + i - 50 -
						this.fontRendererObj.getStringWidth(s1)),
					(float) (k + i / 2 + k2 * 8 + 20),
					profiler$result2.getColor()
				);
				this.fontRendererObj.drawStringWithShadow(
		...
		if (this.leftClickCounter <= 0 && !this.thePlayer.isUsingItem()) {
			if (leftClick && this.objectMouseOver != null &&
				...
				if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air
					&&
					this.playerController.onPlayerDamageBlock(blockpos,
	...
	public void leftClickMouse() {
		final MouseEvent mouseEvent = new MouseEvent(0, this.objectMouseOver);
		Client.INSTANCE.getEventBus().call(mouseEvent);
		if (this.leftClickCounter <= 0) {
			this.thePlayer.swingItem();

			if (mouseEvent.objectMouseOver == null) {
				...
				switch (mouseEvent.objectMouseOver.typeOfHit) {
					case ENTITY:
						this.playerController.attackEntity(this.thePlayer,
							mouseEvent.objectMouseOver.entityHit);
						break;

					case BLOCK:
						BlockPos blockpos = mouseEvent.objectMouseOver.getBlockPos();

						if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial()
							!= Material.air) {
							this.playerController.clickBlock(blockpos,
								mouseEvent.objectMouseOver.sideHit);
							break;
						}
	...
	public void rightClickMouse() {
		final MouseEvent mouseEvent = new MouseEvent(1, this.objectMouseOver);
		Client.INSTANCE.getEventBus().call(mouseEvent);
		if (!this.playerController.getIsHittingBlock()) {
			if (fastUse == null) {
				fastUse = Client.INSTANCE.getModuleManager().getModule(FastUse.class);
			}
			if (fastUse.isEnabled()) {
				boolean holdingBlock =
					thePlayer.getHeldItem() != null && thePlayer.getHeldItem()
						.getItem() instanceof ItemBlock;
				boolean notHoldingBlock = fastUse.onlyBlocksSetting.getValue() && !holdingBlock;
				this.rightClickDelayTimer = notHoldingBlock ? 4 : fastUse.delaySetting.getInt();
			} else {
				...
				switch (mouseEvent.objectMouseOver.typeOfHit) {
					...
					case BLOCK:
						...
						if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial()
							...
							if (this.playerController.onPlayerRightClick(this.thePlayer,
								this.theWorld,
								itemstack,
								blockpos,
								mouseEvent.objectMouseOver.sideHit,
								mouseEvent.objectMouseOver.hitVec
							)) {
								flag = false;
								this.thePlayer.swingItem();
							}
			...
			if (flag) {
				...
				if (itemstack1 != null && this.playerController.sendUseItem(this.thePlayer,
					this.theWorld,
					itemstack1
				)) {
	...
	public void toggleFullscreen() {
		try {
			...
			if (this.fullscreen) {
				...
				GLFW.glfwSetWindowMonitor(Display.getWindow(),
				...
				int lastWidth = displayWidth, lastHeight = displayHeight;
				...
				GLFW.glfwSetWindowMonitor(Display.getWindow(),
					0,
					lastWidth / 2 - displayWidth / 2,
					lastHeight / 2 - displayHeight / 2,
					displayWidth,
					displayHeight,
					gameSettings.limitFramerate
				);
	...
	private void resize(int width, int height) {
		...
		if (this.currentScreen != null) {
			ScaledResolution scaledresolution = new ScaledResolution(this);
			this.currentScreen.onResize(this, scaledresolution.getScaledWidth(),
				scaledresolution.getScaledHeight());
		}
	...
	public void runTick() throws IOException {
		if (ambience == null) {
			ambience = Client.INSTANCE.getModuleManager().getModule(Ambience.class);
		}
		if (motionBlur == null) {
			motionBlur = Client.INSTANCE.getModuleManager().getModule(MotionBlur.class);
		}
		ambience.onTick();
		motionBlur.onTick();
		...
		if (this.currentScreen != null) {
			try {
				...
				crashreportcategory.addCrashSectionCallable("Screen name",
					() -> Minecraft.this.currentScreen.getClass()
						.getCanonicalName()
				);
			...
			if (this.currentScreen != null) {
				try {
					...
					CrashReport crashreport1 = CrashReport.makeCrashReport(throwable,
						"Ticking screen");
					CrashReportCategory crashreportcategory1 = crashreport1.makeCategory(
						"Affected screen");
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
						...
						if (this.thePlayer.isSpectator()) {
							...
							if (this.ingameGUI.getSpectatorGui().func_175262_a()) {
								...
								float f = MathHelper.clamp_float(
									this.thePlayer.capabilities.getFlySpeed() + (float) j * 0.005F,
									0.0F, 0.2F);
			...
			while (Keyboard.next()) {
				int k = Keyboard.getEventKey() == 0 ? Keyboard.getEventCharacter() + 256
					: Keyboard.getEventKey();
				KeyBinding.setKeyBindState(k, Keyboard.getEventKeyState());

				if (Keyboard.getEventKeyState()) {
					Client.INSTANCE.getEventBus().call(new KeyEvent(k));
					KeyBinding.onTick(k);
				}
				...
				if (this.debugCrashKeyPressTime > 0L) {
					if (getSystemTime() - this.debugCrashKeyPressTime >= 6000L) {
						throw new ReportedException(
							new CrashReport("Manually triggered debug crash",
								new Throwable()));
					}
					...
					if (this.currentScreen != null) {
						...
						if (k == 48 && Keyboard.isKeyDown(61)) {
							this.renderManager.setDebugBoundingBox(
								!this.renderManager.isDebugBoundingBox());
						}
			...
			while (this.gameSettings.keyBindInventory.isPressed()) {
				if (this.playerController.isRidingHorse()) {
					...
					this.getNetHandler()
						.addToSendQueue(new C16PacketClientStatus(
...
//            if (this.currentScreen == null && Keyboard.isKeyDown(Keyboard.getKeyIndex(ChatUtil.COMMAND_PREFIX)))
//                this.displayGuiScreen(new GuiChat(ChatUtil.COMMAND_PREFIX));
		...
		if (this.theWorld != null) {
			if (!this.isGamePaused) {
				this.theWorld.setAllowedSpawnTypes(
					this.theWorld.getDifficulty() != EnumDifficulty.PEACEFUL, true);

				try {
					this.theWorld.tick();
				} catch (Throwable throwable2) {
					CrashReport crashreport2 = CrashReport.makeCrashReport(throwable2,
						"Exception in world tick");

					if (this.theWorld == null) {
						CrashReportCategory crashreportcategory2 = crashreport2.makeCategory(
							"Affected level");
	...
	public void launchIntegratedServer(String folderName, String worldName,
		...
		try {
			this.theIntegratedServer = new IntegratedServer(this, folderName, worldName,
				worldSettingsIn);
		...
		this.displayGuiScreen(null);
		SocketAddress socketaddress = this.theIntegratedServer.getNetworkSystem()
			.addLocalEndpoint();
		NetworkManager networkmanager = NetworkManager.provideLocalClient(socketaddress);
		networkmanager.setNetHandler(new NetHandlerLoginClient(networkmanager, this, null));
		networkmanager.sendPacket(
			new C00Handshake(47, socketaddress.toString(), 0, EnumConnectionState.LOGIN));
	...
	public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
		...
		Client.INSTANCE.getEventBus().call(new LoadWorldEvent());
		System.gc();
		this.systemTime = 0L;
	}
	...
	public void setDimensionAndSpawnPlayer(int dimension) {
		...
		this.thePlayer = this.playerController.func_178892_a(this.theWorld,
			this.thePlayer ==
				null ? new StatFileWriter() :
				this.thePlayer.getStatFileWriter()
		);
		this.thePlayer.getDataWatcher()
			.updateWatchedObjectsFromList(entityplayersp.getDataWatcher().getAllWatched());
	...
	public void middleClickMouse() {
		final MouseEvent mouseEvent = new MouseEvent(2, this.objectMouseOver);
		Client.INSTANCE.getEventBus().call(mouseEvent);
		if (mouseEvent.objectMouseOver != null) {
			...
			if (mouseEvent.objectMouseOver.typeOfHit
				...
				Block block1 =
					item instanceof ItemBlock && !block.isFlowerPot() ? Block.getBlockFromItem(item)
						:
							block;
				...
				if (mouseEvent.objectMouseOver.typeOfHit
					...
					mouseEvent.objectMouseOver.entityHit == null || !flag) {
					return;
				}

				if (mouseEvent.objectMouseOver.entityHit instanceof EntityPainting) {
					item = Items.painting;
				} else if (mouseEvent.objectMouseOver.entityHit instanceof EntityLeashKnot) {
					item = Items.lead;
				} else if (mouseEvent.objectMouseOver.entityHit instanceof EntityItemFrame) {
					EntityItemFrame entityitemframe = (EntityItemFrame) mouseEvent.objectMouseOver.entityHit;
				...
				} else if (mouseEvent.objectMouseOver.entityHit instanceof EntityMinecart) {
					EntityMinecart entityminecart = (EntityMinecart) mouseEvent.objectMouseOver.entityHit;
				...
				} else if (mouseEvent.objectMouseOver.entityHit instanceof EntityBoat) {
					item = Items.boat;
				} else if (mouseEvent.objectMouseOver.entityHit instanceof EntityArmorStand) {
					item = Items.armor_stand;
				} else {
					item = Items.spawn_egg;
					i = EntityList.getEntityID(mouseEvent.objectMouseOver.entityHit);
					flag1 = true;
			...
			if (flag) {
				int j = this.thePlayer.inventoryContainer.inventorySlots.size() - 9
					+ inventoryplayer.currentItem;
				this.playerController.sendSlotPacket(
					inventoryplayer.getStackInSlot(inventoryplayer.currentItem), j);
			}
		}
	}
	...
	public CrashReport addGraphicsAndWorldToCrashReport(CrashReport theCrash) {
		theCrash.getCategory()
			.addCrashSectionCallable("Launched Version", () -> Minecraft.this.launchedVersion);
		...
		theCrash.getCategory().addCrashSectionCallable("Is Modded", () -> {
			...
			return !s.equals("vanilla") ? "Definitely; Client brand changed to '" + s + "'" : (
				Minecraft.class.getSigners() ==
					null ? "Very likely; Jar signature invalidated"
					: "Probably not. Jar signature remains and client " +
						"brand is untouched.");
		});
		...
		theCrash.getCategory().addCrashSectionCallable("Current Language",
			() -> Minecraft.this.mcLanguageManager.getCurrentLanguage()
				.toString()
		);
		theCrash.getCategory().addCrashSectionCallable("Profiler Position",
			() -> Minecraft.this.mcProfiler.profilingEnabled ?
				Minecraft.this.mcProfiler.getNameOfLastSection() : "N/A " +
				"(disabled)"
		);
	...
	public void addServerStatsToSnooper(PlayerUsageSnooper playerSnooper) {
		...
		playerSnooper.addClientStat("run_time",
			(MinecraftServer.getCurrentTimeMillis() -
				playerSnooper.getMinecraftStartTimeMillis()) / 60L * 1000L
		);
		...
		if (this.theIntegratedServer != null
			&& this.theIntegratedServer.getPlayerUsageSnooper() != null) {
	...
	private String getCurrentAction() {
		return this.theIntegratedServer !=
			null ? (this.theIntegratedServer.getPublic() ? "hosting_lan" : "singleplayer") : (
			this.currentServerData !=
				null ? (this.currentServerData.isOnLAN() ? "playing_lan" : "multiplayer")
				: "out_of_game");
	}
	...
	public static int getGLMaximumTextureSize() {
		for (int i = 16384; i > 0; i >>= 1) {
			GL11.glTexImage2D(GL11.GL_PROXY_TEXTURE_2D,
				0,
				GL11.GL_RGBA,
				i,
				i,
			...
			int j = GL11.glGetTexLevelParameteri(GL11.GL_PROXY_TEXTURE_2D, 0,
	...
	public PropertyMap getProfileProperties() {
		if (this.profileProperties.isEmpty()) {
			GameProfile gameprofile = this.getSessionService()
				.fillProfileProperties(this.session.getProfile(), false);
	...
	public MusicTicker.MusicType getAmbientMusicType() {
		return this.thePlayer !=
			null ? (this.thePlayer.worldObj.provider instanceof WorldProviderHell
			...
			(this.thePlayer.worldObj.provider instanceof WorldProviderEnd ? (
				...
				this.thePlayer.capabilities.isCreativeMode &&
					this.thePlayer.capabilities.allowFlying ? MusicTicker.MusicType.CREATIVE
					: MusicTicker.MusicType.GAME))) : MusicTicker.MusicType.MENU;
	}
	...
	public void dispatchKeypresses() {
		...
		if (i != 0 && !Keyboard.isRepeatEvent()) {
			if (!(this.currentScreen instanceof GuiControls) ||
				...
				if (Keyboard.getEventKeyState()) {
					if (i == clickGUI.getKeybind() && !clickGUI.isEnabled()
						&& clickGUI.getSettingByName("Mode").getValue()
						.equals(ClickGUI.ClickGuiMode.IMGUI)) {
					...
					if (i == this.gameSettings.keyBindFullscreen.getKeyCode()) {
						...
						this.ingameGUI.getChatGUI()
							.printChatMessage(ScreenShotHelper.saveScreenshot(this.mcDataDir,
								this.displayWidth,
								this.displayHeight,
								this.framebufferMc
							));
	...
	public <V> ListenableFuture<V> addScheduledTask(Callable<V> callableToSchedule) {
		...
		if (!this.isCallingFromMinecraftThread()) {
			ListenableFutureTask<V> listenablefuturetask = ListenableFutureTask.create(
				callableToSchedule);
	...
	// Credit: Moropheles Client (I developed it too <3)

	private int guiScale;
	private boolean lastFullScreenState;

	public ScaledResolution getScaledResolution() {
		if (this.scaledResolution == null || this.guiScale != this.gameSettings.guiScale ||
			this.lastFullScreenState != this.fullscreen) {
			this.scaledResolution = new ScaledResolution(this);
			this.guiScale = this.gameSettings.particleSetting;
			this.lastFullScreenState = this.fullscreen;
		}
