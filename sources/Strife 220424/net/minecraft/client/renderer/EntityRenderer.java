...
import club.strifeclient.module.implementations.combat.Reach;
import club.strifeclient.module.implementations.visual.CameraClip;
import club.strifeclient.module.implementations.visual.ClickGUI;
import club.strifeclient.module.implementations.visual.NoHurtCam;
...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private static final ResourceLocation[] shaderResourceLocations = new ResourceLocation[] {new ResourceLocation(
	...
	public void loadShader(ResourceLocation resourceLocationIn) {
		if (OpenGlHelper.isFramebufferEnabled()) {
			try {
				this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(),
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			if (reach == null) reach = Client.INSTANCE.getModuleManager().getModule(Reach.class);
			if (reach.isEnabled()) {
				d1 = reach.reachSetting.getDouble();
				MovingObjectPosition movingObjectPosition = entity.rayTrace(d1, partialTicks);
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			float x = Math.min(1, (System.currentTimeMillis() - start) / 500f);
			animation = -1 * Math.pow(x - 1, 12) + 1;
			if (!flag) f *= animation;
			else f /= animation;
			...
			return Reflector.ForgeHooksClient_getFOVModifier.exists() ?
					Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier,
					                    this,
					                    entity,
					                    block,
					                    partialTicks,
	...
	private void hurtCameraEffect(float partialTicks) {
		if (noHurtCam == null) noHurtCam = Client.INSTANCE.getModuleManager().getModule(NoHurtCam.class);
		if (noHurtCam.isEnabled()) return;
	...
	private void orientCamera(float partialTicks) {
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				double d4 = (double) (-MathHelper.sin(f1 / 180.0F * (float) Math.PI) * MathHelper.cos(
						f2 / 180.0F * (float) Math.PI)) * d3;
				double d5 = (double) (MathHelper.cos(f1 / 180.0F * (float) Math.PI) * MathHelper.cos(
						f2 / 180.0F * (float) Math.PI)) * d3;
				...
				for (int i = 0; i < 8; ++i) {
					...
					f4 = f4 * 0.1F;
					f5 = f5 * 0.1F;
					MovingObjectPosition movingobjectposition = this.mc.theWorld.rayTraceBlocks(new Vec3(
							                                                                            d0 + (double) f3, d1 + (double) f4, d2 + (double) f5),
					                                                                            new Vec3(d0 - d4 +
					                                                                                     (double) f3 +
					                                                                                     (double) f5,
					                                                                                     d1 - d6 +
					...
					if (movingobjectposition != null) {
						...
						if (cameraClip == null)
							cameraClip = Client.INSTANCE.getModuleManager().getModule(CameraClip.class);

						if (!cameraClip.isEnabled()) if (d7 < d3) {
							d3 = d7;
						}
					}
				}
		...
		if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
			...
			if (entity instanceof EntityAnimal) {
				...
				GlStateManager.rotate(entityanimal.prevRotationYawHead +
				                      (entityanimal.rotationYawHead - entityanimal.prevRotationYawHead) * partialTicks +
	...
	public void renderHand(float p_renderHand_1_,
	                       int p_renderHand_2_,
		...
		if (!this.debugView) {
			...
			if (p_renderHand_3_) {
				...
				boolean flag1 = !ReflectorForge.renderFirstPersonHand(this.mc.renderGlobal,
				                                                      p_renderHand_1_,
				                                                      p_renderHand_2_
				);
	...
	private void updateLightmap(float partialTicks) {
		if (this.lightmapUpdateNeeded) {
			...
			if (world != null) {
				if (Config.isCustomColors() && CustomColors.updateLightmap(world,
				                                                           this.torchFlickerX,
				                                                           this.lightmapColors,
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime) {
		...
		if (!flag && this.mc.gameSettings.pauseOnLostFocus && (!this.mc.gameSettings.touchscreen || !Mouse.isButtonDown(
				1))) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.theWorld != null) {
				...
				if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
					...
					this.mc.ingameGUI.renderGameOverlay(partialTicks, scaledresolution);
			...
			if (this.mc.currentScreen != null) {
				...
				try {
					...
					crashreportcategory.addCrashSectionCallable("Screen name",
					                                            () -> EntityRenderer.this.mc.currentScreen.getClass()
					                                                                                      .getCanonicalName()
					);
		...
		this.mc.mcProfiler.startSection("ImGui Render");
		if (this.clickGUI == null) clickGUI = Client.INSTANCE.getModuleManager().getModule(ClickGUI.class);
		this.mc.mcProfiler.endSection();
	}
	...
	private void renderWorldDirections(float partialTicks) {
		if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI &&
			...
			RenderGlobal.drawOutlinedBoundingBox(new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.005D, 1.0E-4D, 1.0E-4D),
			                                     255,
			                                     0,
			                                     0,
			                                     255
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		if (entity.posY + (double) entity.getEyeHeight() <
		...
		if (flag) {
			ShadersRender.setupTerrain(renderglobal,
			                           entity,
			                           partialTicks,
			                           icamera,
			                           this.frameCount++,
			...
			renderglobal.setupTerrain(entity,
			                          partialTicks,
			                          icamera,
			                          this.frameCount++,
			                          this.mc.thePlayer.isSpectator()
		...
		this.mc.getTextureManager().getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false,
		                                                                                       this.mc.gameSettings.mipmapLevels >
		                                                                                       0
		);
		...
		if (!renderglobal.damagedBlocks.isEmpty()) {
			...
			this.mc.getTextureManager().getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false, false);
			renderglobal.drawBlockDamageTexture(Tessellator.getInstance(),
		...
		Client.INSTANCE.getEventBus().call(new Render3DEvent());
	...
	private void addRainParticles() {
		...
		if (f != 0.0F && Config.isRainSplash()) {
			...
			for (int l = 0; l < k; ++l) {
				BlockPos blockpos1 = world.getPrecipitationHeight(blockpos.add(
			...
			if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				...
				if (d1 > (double) (blockpos.getY() + 1) &&
				    world.getPrecipitationHeight(blockpos).getY() > MathHelper.floor_float((float) blockpos.getY())) {
					...
					if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
						...
						if (k2 != l2) {
							...
							if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
								...
								double d5 = ((double) (
										this.rendererUpdateCount + l1 * l1 * 3121 + l1 * 45238971 + k1 * k1 * 418711 +
										k1 * 13761 & 31) + (double) partialTicks) / 32.0D *
								...
								int k3 = j3 >> 16 & 65535;
								int l3 = j3 & 65535;
								worldrenderer.pos((double) l1 - d3 + 0.5D, k2, (double) k1 - d4 + 0.5D).tex(0.0D,
								                                                                            (double) k2 *
								                                                                            0.25D + d5
								).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, k2, (double) k1 + d4 + 0.5D).tex(1.0D,
								                                                                            (double) k2 *
								                                                                            0.25D + d5
								).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, l2, (double) k1 + d4 + 0.5D).tex(1.0D,
								                                                                            (double) l2 *
								                                                                            0.25D + d5
								).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
								worldrenderer.pos((double) l1 - d3 + 0.5D, l2, (double) k1 - d4 + 0.5D).tex(0.0D,
								                                                                            (double) l2 *
								                                                                            0.25D + d5
								).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
							} else {
								...
								int j4 = i4 >> 16 & 65535;
								int k4 = i4 & 65535;
								worldrenderer.pos((double) l1 - d3 + 0.5D, k2, (double) k1 - d4 + 0.5D).tex(0.0D + d9,
								                                                                            (double) k2 *
								                                                                            0.25D + d8 +
								                                                                            d10
								).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, k2, (double) k1 + d4 + 0.5D).tex(1.0D + d9,
								                                                                            (double) k2 *
								                                                                            0.25D + d8 +
								                                                                            d10
								).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, l2, (double) k1 + d4 + 0.5D).tex(1.0D + d9,
								                                                                            (double) l2 *
								                                                                            0.25D + d8 +
								                                                                            d10
								).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 - d3 + 0.5D, l2, (double) k1 - d4 + 0.5D).tex(0.0D + d9,
								                                                                            (double) l2 *
								                                                                            0.25D + d8 +
								                                                                            d10
								).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
	...
	private void updateFogColor(float partialTicks) {
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			double d0 = -1.0D;
			Vec3 vec32 = MathHelper.sin(world.getCelestialAngleRadians(partialTicks)) > 0.0F ? new Vec3(d0,
			                                                                                            0.0D,
			                                                                                            0.0D
		...
		if (this.cloudFog) {
			...
			Vec3 vec35 = CustomColors.getUnderwaterColor(this.mc.theWorld,
			...
			Vec3 vec34 = CustomColors.getUnderlavaColor(this.mc.theWorld,
			                                            this.mc.getRenderViewEntity().posX,
			                                            this.mc.getRenderViewEntity().posY + 1.0D,
			                                            this.mc.getRenderViewEntity().posZ
			);
		...
		if (this.bossColorModifier > 0.0F) {
			float f14 =
					this.bossColorModifierPrev + (this.bossColorModifier - this.bossColorModifierPrev) * partialTicks;
		...
		if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
			...
			Reflector.postForgeBusEvent(object);
			this.fogColorRed = Reflector.getFieldValueFloat(object,
			                                                Reflector.EntityViewRenderEvent_FogColors_red,
			                                                this.fogColorRed
			);
			this.fogColorGreen = Reflector.getFieldValueFloat(object,
			                                                  Reflector.EntityViewRenderEvent_FogColors_green,
			                                                  this.fogColorGreen
			);
			this.fogColorBlue = Reflector.getFieldValueFloat(object,
	...
	private void frameInit() {
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				...
				ChatComponentText chatcomponenttext = new ChatComponentText(I18n.format("of.message.newVersion",
				                                                                        "\u00a7n" + s1 + "\u00a7r"
				));
				chatcomponenttext.setChatStyle((new ChatStyle()).setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL,
				                                                                                  "https://optifine" +
				                                                                                  ".net/downloads"
				)));
	...
	private void frameFinish() {
		if (this.mc.theWorld != null && Config.isShowGlErrors() && TimedEvent.isActive("CheckGlErrorFrameFinish",
		                                                                               10000L
		)) {
	...
	private void checkLoadVisibleChunks(Entity p_checkLoadVisibleChunks_1_,
		...
		if (this.loadVisibleChunks) {
			...
			this.loadAllVisibleChunks(p_checkLoadVisibleChunks_1_,
			                          p_checkLoadVisibleChunks_2_,
	...
	private void loadAllVisibleChunks(Entity p_loadAllVisibleChunks_1_,
		...
		try {
			...
			while (true) {
				...
				for (int j1 = 0; j1 < 100; ++j1) {
					...
					renderglobal.setupTerrain(p_loadAllVisibleChunks_1_,
