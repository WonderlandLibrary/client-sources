package com.ohare.client.gui.account.gui.impl;

import com.ohare.client.Client;
import com.ohare.client.gui.account.gui.thread.AccountLoginThread;
import com.ohare.client.utils.thealtening.TheAltening;
import com.ohare.client.utils.thealtening.domain.AlteningAlt;
...
public class GuiAlteningLogin extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            case -1:
                ...
                Client.INSTANCE.getAccountManager().setAlteningKey(key.getText());
                Client.INSTANCE.getAccountManager().setLastAlteningAlt(token.getText());
            ...
            case 2:
                ...
                break;
            case 3:
                if (key.getText().isEmpty() || Client.INSTANCE.getAccountManager().getLastAlteningAlt() == null) return;
                Client.INSTANCE.getAccountManager().setAlteningKey(key.getText());
                thread = new AccountLoginThread(Client.INSTANCE.getAccountManager().getLastAlteningAlt().replaceAll(" ", ""), "ohareware");
                thread.run();
                Client.INSTANCE.getAccountManager().save();
                break;
        }
    ...
    public void initGui() {
        ...
        if (Client.INSTANCE.getAccountManager().getAlteningKey() != null)
            key.setText(Client.INSTANCE.getAccountManager().getAlteningKey());
