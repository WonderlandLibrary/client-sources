...
import com.ohare.client.event.events.player.PushEvent;
import com.ohare.client.event.events.player.SlowdownEvent;
import com.ohare.client.event.events.player.UpdateEvent;
import com.ohare.client.event.events.render.InsideBlockRenderEvent;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                final UpdateEvent updateEvent = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround, true);
                updateEvent.dispatch();
                if (updateEvent.isCanceled()) {
                    new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround).dispatch();
                    return;
                }
    ...
    public void onUpdateWalkingPlayer() {
        final UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround, true);
        event.dispatch();
        if (event.isCanceled()) {
            new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround).dispatch();
            return;
    ...
    public boolean isEntityInsideOpaqueBlock() {
        ...
        insideBlockRenderEvent.dispatch();
        return insideBlockRenderEvent.isCanceled() ? false : super.isEntityInsideOpaqueBlock();
    }
    ...
    public void onLivingUpdate() {
        ...
        final SlowdownEvent event = new SlowdownEvent(SlowdownEvent.Type.Item);
        event.dispatch();
        if (this.isUsingItem() && !this.isRiding() && !event.isCanceled()) {
    ...
    public boolean isInLiquid() {
        for (int x = MathHelper.floor_double(getEntityBoundingBox().minX); x < MathHelper.floor_double(getEntityBoundingBox().maxX) + 1; ++x) {
            for (int z = MathHelper.floor_double(getEntityBoundingBox().minZ); z < MathHelper.floor_double(getEntityBoundingBox().maxZ) + 1; ++z) {
                final BlockPos pos = new BlockPos(x, (int) getEntityBoundingBox().minY, z);
    ...
    public void moveEntity(double x, double y, double z) {
        final MotionEvent event = new MotionEvent(x, y, z);
        event.dispatch();
