...
import com.ohare.client.module.modules.other.Animate;
...
public abstract class GuiContainer extends GuiScreen
{
	...
	public void initGui()
    {
        ...
        this.buttonList.add(new GuiButton(420,new ScaledResolution(mc).getScaledWidth() / 2 - 60, 3, 110, 20, "Disable Aura"));
        x = y = 0;
    }
    ...
    {
        Animate animate = (Animate) Client.INSTANCE.getModuleManager().getModule("animate");
        interpolate(this.width,this.height, animate.speed.getValue());
        ...
        if(animate.isEnabled()){
            double xmod = this.width/2-(x/2);
            double ymod = this.height/2-(y/2);

            GlStateManager.translate(xmod,ymod, 0);
            GlStateManager.scale(x/this.width, y/this.height, 1);
        }
    ...
    private void interpolate(float targetX, float targetY, double speed) {
        ...
        x = RenderUtil.calculateCompensation(targetX, x, delta, deltaX);
        y = RenderUtil.calculateCompensation(targetY, y, delta, deltaY);
    }
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        for (Object aButtonList : this.buttonList) {
            ...
            if (var52.mousePressed(this.mc, mouseX, mouseY) && var52.id == 420) {
                if(Client.INSTANCE.getModuleManager().getModule("killaura").isEnabled()) {
                    Client.INSTANCE.getModuleManager().getModule("killaura").toggle();
                }
            }
        }
