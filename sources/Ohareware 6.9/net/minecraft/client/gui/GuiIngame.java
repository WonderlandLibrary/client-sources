...
import com.ohare.client.event.events.render.CrosshairEvent;
import com.ohare.client.event.events.render.Render2DEvent;
import com.ohare.client.module.modules.other.AutoMatchJoin;
...
public class GuiIngame extends Gui {
    ...
    public static boolean enabledsb;
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            float f3 = this.recordPlayingUpFor - partialTicks;
        ...
        if (this.field_175195_w > 0) {
            ...
            float f4 = this.field_175195_w - partialTicks;
            int l1 = 255;

            if (this.field_175195_w > this.field_175193_B + this.field_175192_A) {
                float f1 = this.field_175199_z + this.field_175192_A + this.field_175193_B - f4;
                l1 = (int) (f1 * 255.0F / this.field_175199_z);
            }

            if (this.field_175195_w <= this.field_175193_B) {
                l1 = (int) (f4 * 255.0F / this.field_175193_B);
            }
        ...
        if (scoreobjective1 != null && Client.INSTANCE.getHudCompManager().getHudComp("scoreboardcomp").isEnabled()) {
        ...
        final Render2DEvent event = new Render2DEvent(scaledresolution, partialTicks);
        event.dispatch();
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (i > 0) {
            short short1 = 182;
            int k = (int) (this.mc.thePlayer.experience * (short1 + 1));
    ...
    public void renderDemo(ScaledResolution p_175185_1_) {
        ...
        this.getFontRenderer().drawStringWithShadow(s, p_175185_1_.getScaledWidth() - i - 10, 5.0F, 16777215);
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        if (arraylist1.isEmpty()) return;
        ScorePlayerTeam scoreplayerteam11 = scoreboard.getPlayersTeam(((Score) arraylist1.get(0)).getPlayerName());
        String s1gay = ScorePlayerTeam.formatPlayerName(scoreplayerteam11, ((Score) arraylist1.get(0)).getPlayerName());
        int largestString = getFontRenderer().getStringWidth(s1gay);
        for (int i2 = 0; i < arraylist1.size(); i2++) {
            ScorePlayerTeam scoreplayerteam1gay = scoreboard.getPlayersTeam(((Score) arraylist1.get(i)).getPlayerName());
            String sgay = ScorePlayerTeam.formatPlayerName(scoreplayerteam11, ((Score) arraylist1.get(i)).getPlayerName());
            if (getFontRenderer().getStringWidth(sgay) > largestString) {
                largestString = getFontRenderer().getStringWidth(sgay);
            }
        }
        float j1 = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        float k1 = (float) Client.INSTANCE.getHudCompManager().getHudComp("scoreboardcomp").getY() + height + 8;
        byte b0 = 3;
        float j = (float) Client.INSTANCE.getHudCompManager().getHudComp("scoreboardcomp").getX() - width - 28 + (float) Client.INSTANCE.getHudCompManager().getHudComp("scoreboardcomp").getWidth();
        width = largestString;
        height = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        int k = 0;
        for (Object score1 : arraylist1) {
            ...
            float l = k1 - k * this.getFontRenderer().FONT_HEIGHT;
            int i1 = b0 + 2;
            drawRect(j - 2, l, j + largestString + 28, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            AutoMatchJoin autoMatchJoin = (AutoMatchJoin) Client.INSTANCE.getModuleManager().getModule("AutoMatchJoin");
            if (autoMatchJoin.isEnabled() && autoMatchJoin.autoDetect.isEnabled()) {
                ...
                autoMatchJoin.teams = s1.toLowerCase().contains("teams");
            }
            ...
            if (k == arraylist1.size()) {
                ...
                drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, j + largestString + 28, l - 1, 1610612736);
                drawRect(j - 2, l - 1, j + largestString + 28, l, 1342177280);
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if (entityplayer.isInsideOfMaterial(Material.water)) {
                ...
                int j8 = MathHelper.ceiling_double_int((i6 - 2) * 10.0D / 300.0D);
                int k6 = MathHelper.ceiling_double_int(i6 * 10.0D / 300.0D) - j8;
