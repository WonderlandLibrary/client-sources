...
import com.ohare.client.module.modules.combat.KillAura;
...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float p_78440_1_) {
        ...
        if (this.itemToRender != null) { //KillAura.autoblock.isEnabled()
            final boolean fakeautoblock = (Client.INSTANCE.getModuleManager().getModule("killaura")).isEnabled() && ((KillAura) Client.INSTANCE.getModuleManager().getModule("killaura")).isAutoBlocking();
            if (this.itemToRender.getItem() instanceof ItemMap) {
                this.renderItemMap(var3, var5, var2, var4);
            } else if (var3.getItemInUseCount() > 0 || fakeautoblock) {

                EnumAction enumaction = fakeautoblock ? EnumAction.BLOCK :
            ...
            if (Client.INSTANCE.getModuleManager().getModuleClass(Chams.class).isEnabled() && chams.hands.isEnabled() && chams.colored.isEnabled()) {
                GL11.glDisable(3008);
                GL11.glDisable(3553);
                GL11.glDisable(2896);
                GL11.glEnable(3042);
                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0f, 240.0f);
                GL11.glColor4f(chams.rainbow.isEnabled() ? rgba[0]:chams.visiblered.getValue(),chams.rainbow.isEnabled() ? rgba[1]:chams.visiblegreen.getValue(),chams.rainbow.isEnabled() ? rgba[2]:chams.visibleblue.getValue(),chams.alpha.getValue());
