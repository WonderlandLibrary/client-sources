...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase)
        {
            ...
            float f = Client.INSTANCE.getModuleManager().getModule("antihurtcam").isEnabled() ? 0:entitylivingbase.hurtTime - partialTicks;
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            if (this.mc.gameSettings.smoothCamera)
            {
                ...
                this.mc.thePlayer.setAngles(f2, f3 * b0);
            }
            else
            {
                this.smoothCamYaw = 0.0F;
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.theWorld != null)
            {
                ...
                long k = Math.max(1000000000 / i / 4 - j, 0L);
        ...
        if (f5 > 0.0F)
        {
            ...
            for (int i1 = k - b0; i1 <= k + b0; ++i1)
            {
                for (int j1 = i - b0; j1 <= i + b0; ++j1)
                {
                    ...
                    double d3 = this.rainXCoords[k1] * 0.5D;
                    double d4 = this.rainYCoords[k1] * 0.5D;
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow())
                    {
                        ...
                        if (i2 != j2)
                        {
                            ...
                            if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F)
                            {
                                ...
                                double d6 = j1 + 0.5F - entity.posX;
                                double d7 = i1 + 0.5F - entity.posZ;
                                float f2 = MathHelper.sqrt_double(d6 * d6 + d7 * d7) / b0;
                                ...
                                int i3 = l2 >> 16 & 65535;
                                int j3 = l2 & 65535;
                                worldrenderer.pos(j1 - d3 + 0.5D, i2, i1 - d4 + 0.5D).tex(0.0D, i2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(i3, j3).endVertex();
                                worldrenderer.pos(j1 + d3 + 0.5D, i2, i1 + d4 + 0.5D).tex(1.0D, i2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(i3, j3).endVertex();
                                worldrenderer.pos(j1 + d3 + 0.5D, j2, i1 + d4 + 0.5D).tex(1.0D, j2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(i3, j3).endVertex();
                                worldrenderer.pos(j1 - d3 + 0.5D, j2, i1 - d4 + 0.5D).tex(0.0D, j2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(i3, j3).endVertex();
                            }
                            else
                            {
                                if (b1 != 1)
                                ...
                                double d11 = j1 + 0.5F - entity.posX;
                                double d12 = i1 + 0.5F - entity.posZ;
                                float f6 = MathHelper.sqrt_double(d11 * d11 + d12 * d12) / b0;
                                float f4 = ((1.0F - f6 * f6) * 0.3F + 0.5F) * f5;
                                blockpos$mutableblockpos.func_181079_c(j1, k2, i1);
                                int k3 = (worldclient.getCombinedLight(blockpos$mutableblockpos, 0) * 3 + 15728880) / 4;
                                int l3 = k3 >> 16 & 65535;
                                int i4 = k3 & 65535;
                                worldrenderer.pos(j1 - d3 + 0.5D, i2, i1 - d4 + 0.5D).tex(0.0D + d9, i2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(l3, i4).endVertex();
                                worldrenderer.pos(j1 + d3 + 0.5D, i2, i1 + d4 + 0.5D).tex(1.0D + d9, i2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(l3, i4).endVertex();
                                worldrenderer.pos(j1 + d3 + 0.5D, j2, i1 + d4 + 0.5D).tex(1.0D + d9, j2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(l3, i4).endVertex();
                                worldrenderer.pos(j1 - d3 + 0.5D, j2, i1 - d4 + 0.5D).tex(0.0D + d9, j2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(l3, i4).endVertex();
                            }
                        }
                    }
                }
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        {
            float f8 = EnchantmentHelper.getRespiration(entity) * 0.2F;
        ...
        double d1 = (entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * partialTicks) * d2;
