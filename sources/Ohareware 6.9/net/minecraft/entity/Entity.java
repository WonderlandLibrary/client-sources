...
import com.ohare.client.event.events.player.SafewalkEvent;
import com.ohare.client.event.events.player.SlowdownEvent;
import com.ohare.client.event.events.player.StepEvent;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            double d4 = y;
            double d5 = z;
            final SafewalkEvent event = new SafewalkEvent();
            event.dispatch();
            boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer;

            if (flag || (event.isCanceled() && this instanceof EntityPlayer)) {
                double d6;
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                final StepEvent entityStepEvent = new StepEvent(this, true);
                entityStepEvent.dispatch();
                if (entityStepEvent.isCanceled())
                    return;
                double d11 = x;
                double d7 = y;
                double d8 = z;
            ...
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                this.distanceWalkedModified = (float) (this.distanceWalkedModified
                        + MathHelper.sqrt_double(d12 * d12 + d14 * d14) * 0.6D);
                ...
                if (this.distanceWalkedOnStepModified > this.nextStepDistance
                    ...
                    final SlowdownEvent event1 = new SlowdownEvent(SlowdownEvent.Type.Water);
                    event1.dispatch();
                    if (this.isInWater() && !event1.isCanceled()) {
    ...
    protected void createRunningParticles()
    {
        ...
        {
            this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextFloat() - 0.5D) * this.width, this.getEntityBoundingBox().minY + 0.1D, this.posZ + (this.rand.nextFloat() - 0.5D) * this.width, -this.motionX * 4.0D, 1.5D, -this.motionZ * 4.0D, new int[] {Block.getStateId(iblockstate)});
        }
    }
