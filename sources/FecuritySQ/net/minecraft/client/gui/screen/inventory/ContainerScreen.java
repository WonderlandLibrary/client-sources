...
import ru.FecuritySQ.module.общее.ItemScroller;
...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T>
{
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1)
        {
            ...
            {
                if (FecuritySQ.get().getModule(ItemScroller.class).isEnabled() && isButtonDown
                        && InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), Minecraft.getInstance().gameSettings.keyBindSneak.keyCode.getKeyCode())
                        && Minecraft.getInstance().currentScreen != null && this.counter.hasTimeElapsed(80)) {
