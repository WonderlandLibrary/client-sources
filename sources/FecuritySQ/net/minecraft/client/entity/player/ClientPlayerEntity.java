...
import ru.FecuritySQ.event.EventManager;
...
import ru.FecuritySQ.module.передвижение.NoSlowDown;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity
{
    ...
    public void tick()
    {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ())))
        {
            ...
            {
                IBaritone FLEX = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) this);
                if (FLEX != null) {
                    FLEX.getGameEventHandler().onPlayerUpdate(new PlayerUpdateEvent(EventState.POST));
                }
    ...
    {
        WalkingUpdateEvent eventWalkingUpdate = new WalkingUpdateEvent(getPosX(), getPosY(), getPosZ(), rotationYaw, rotationPitch, onGround);

        EventManager.call(eventWalkingUpdate);
        ...
        {
            double d4 = eventWalkingUpdate.getPosX() - this.lastReportedPosX;
            double d0 = eventWalkingUpdate.getPosY() - this.lastReportedPosY;
            double d1 = eventWalkingUpdate.getPosZ() - this.lastReportedPosZ;
            double d2 = (double)(eventWalkingUpdate.getYaw() - this.lastReportedYaw);
            double d3 = (double)(eventWalkingUpdate.getPitch() - this.lastReportedPitch);
            ...
            if (this.isPassenger())
            {
                ...
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(vector3d.x, -999.0D, vector3d.z, eventWalkingUpdate.getYaw(), eventWalkingUpdate.getPitch(), eventWalkingUpdate.isGround()));
                flag1 = false;
            }
            else if (flag1 && flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(eventWalkingUpdate.getPosX(), eventWalkingUpdate.getPosY(), eventWalkingUpdate.getPosZ(), eventWalkingUpdate.getYaw(), eventWalkingUpdate.getPitch(), eventWalkingUpdate.isGround()));
            }
            else if (flag1)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionPacket(eventWalkingUpdate.getPosX(), eventWalkingUpdate.getPosY(), eventWalkingUpdate.getPosZ(), eventWalkingUpdate.isGround()));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.RotationPacket(eventWalkingUpdate.getYaw(), eventWalkingUpdate.getPitch(), eventWalkingUpdate.isGround()));
            }
            else if (this.prevOnGround != eventWalkingUpdate.isGround())
            {
                this.connection.sendPacket(new CPlayerPacket(eventWalkingUpdate.isGround()));
            }

            if (flag1)
            {
                this.lastReportedPosX = eventWalkingUpdate.getPosX();
                this.lastReportedPosY = eventWalkingUpdate.getPosY();
                this.lastReportedPosZ = eventWalkingUpdate.getPosZ();
                this.positionUpdateTicks = 0;
            }

            if (flag2)
            {
                this.lastReportedYaw = eventWalkingUpdate.getYaw();
                this.lastReportedPitch = eventWalkingUpdate.getPitch();
            }

            this.prevOnGround = eventWalkingUpdate.isGround();
    ...
    public void sendChatMessage(String message)
    {
        ...
        boolean execute = FecuritySQ.get().getCommandManager().execute(message);
        if(!execute){
    ...
    public void livingTick()
    {
        ...
        if (this.isHandActive() &&  !this.isPassenger() && !FecuritySQ.get().getModule(NoSlowDown.class).isEnabled()) {
        ...
        {
            if (this.sprintToggleTimer <= 0 && !isKeyDown(this.mc.gameSettings.keyBindSprint))
            {
        ...
        if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS) && isKeyDown(this.mc.gameSettings.keyBindSprint))
        {
            this.setSprinting(true);
        }
