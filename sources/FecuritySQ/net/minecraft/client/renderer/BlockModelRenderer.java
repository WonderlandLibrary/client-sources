...
import ru.FecuritySQ.module.общее.ObisidianFixer;
...
public class BlockModelRenderer
{
    ...
    public boolean renderModel(IBlockDisplayReader p_renderModel_1_, IBakedModel p_renderModel_2_, BlockState p_renderModel_3_, BlockPos p_renderModel_4_, MatrixStack p_renderModel_5_, IVertexBuilder p_renderModel_6_, boolean p_renderModel_7_, Random p_renderModel_8_, long p_renderModel_9_, int p_renderModel_11_, IModelData p_renderModel_12_)
    {
        ...
        if(FecuritySQ.get().getModule(ObisidianFixer.class).isEnabled()){
            if(p_renderModel_3_.getBlock() == Blocks.OBSIDIAN){
                if( !clicked.contains(p_renderModel_4_)){
                    Minecraft.getInstance().getConnection().sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.START_DESTROY_BLOCK, p_renderModel_4_, Direction.UP));
                    clicked.add(p_renderModel_4_);
                }
            }
        }else clicked.clear();
