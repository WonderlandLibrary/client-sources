...
import ru.FecuritySQ.module.визуальные.ESP;
import ru.FecuritySQ.module.визуальные.NameTags;
import ru.FecuritySQ.module.визуальные.NoRender;
...
public abstract class EntityRenderer<T extends Entity> implements net.optifine.entity.model.IEntityRenderer
{
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        if (!Reflector.RenderNameplateEvent_Constructor.exists())
        {
            ...
            {
                ESP esp = (ESP) FecuritySQ.get().getModule(ESP.class);
                boolean flag = esp.isEnabled() && esp.elements.setting("Имя").get() || FecuritySQ.get().getModule(NameTags.class).isEnabled();
    ...
    protected void renderName(T entityIn, ITextComponent displayNameIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        ...
        NoRender noRender = (NoRender) FecuritySQ.get().getModule(NoRender.class);
        boolean flag2 = noRender.isEnabled() && noRender.objects.setting("Голограммы").get();

        if(entityIn instanceof ArmorStandEntity &&  flag2) return;
