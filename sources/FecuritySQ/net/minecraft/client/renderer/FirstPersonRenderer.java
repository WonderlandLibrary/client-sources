...
import ru.FecuritySQ.module.визуальные.SwingAnimations;
...
public class FirstPersonRenderer
{
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn))
        {
            ...
            SwingAnimations swingAnimations = (SwingAnimations) FecuritySQ.get().getModule(SwingAnimations.class);

            boolean flag = handIn == Hand.MAIN_HAND;
            HandSide handside = flag ? player.getPrimaryHand() : player.getPrimaryHand().opposite();
            matrixStackIn.push();

            if(swingAnimations.isEnabled()){
                if(handIn == Hand.OFF_HAND) swingAnimations.translate(matrixStackIn, swingAnimations.leftx.current, swingAnimations.lefty.current, swingAnimations.leftz.current, -1.0f);
                if(handIn == Hand.MAIN_HAND) swingAnimations.translate(matrixStackIn, swingAnimations.rightx.current, swingAnimations.righty.current, swingAnimations.rightz.current, -1.0f);
            }
            ...
            {
                if (swingAnimations.isEnabled()) {
                     swingAnimations.translate(matrixStackIn, 0F, 0F, swingAnimations.leftz.current + swingAnimations.rightz.current / 2, 0f);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    float swingprogress = mc.player.getSwingProgress(partialTicks);
                    float smooth = swingprogress * 0.8F - swingprogress * swingprogress * 0.8F;


                    boolean isEnabled = swingAnimations.isEnabled();

                    if(isEnabled) {
                        switch (swingAnimations.mode.current()) {
                            case "Стандарт":
                                ...
                                break;
                            case "Тапы":
                                GlStateManager.translatef(.56f, -0.52f, -0.71999997f);
                                GlStateManager.translatef(0, equippedProgress * -0.15f, 0.0f);
                                GlStateManager.rotatef(45.0f, 0.0f, 1.0f, 0.0f);
                                GlStateManager.rotatef(smooth * -90.0f, 0.0f, 1.0f, 0.0f);
                                GlStateManager.scaled(0.37f, 0.37f, 0.37f);
                                GlStateManager.translatef(-0.5f, 1.0f, 0.0f);
                                ...
                                break;
                            case "Блок":
                                transformFirstPersonItem(equippedProgress / 10, swingprogress);
                                translate();
                                break;
                            case "Сворачивание":
                                transformFirstPersonItem(0, 0);
                                translate();
                                GlStateManager.rotatef(f7 * 4, 1, 0, 0);
                                break;
                            case "Шлепки":
                                ...
                                float var4 = MathHelper.sin(MathHelper.sqrt(0) * (float) Math.PI);
                                ...
                                GlStateManager.rotatef(-80.0f, 1.0f, 0.0f, 0.0f);
                                GlStateManager.rotatef(60.0f, 0.0f, 1.0f, 0.0f);
                                int alpha = (int) Math.min(255L,
                                        (System.currentTimeMillis() % 255L > 127L
                                                ? Math.abs(Math.abs(System.currentTimeMillis()) % 255L - 255L)
                                                : System.currentTimeMillis() % 255L));
                                float f5 = (double) f7 > 0.5 ? 1.0f - f7 : f7;
                                ...
                                GlStateManager.translated(0.0, 0.0, -0.5);
                                GlStateManager.rotatef(this.mc.player.isSwingInProgress
                                        ? (float) (-alpha) / 8
                                        : 0, 1.0f, -0.0f, 1.0f);
                                GlStateManager.translated(0.0, 0.0, 0.5);
                                break;
                            case "Сдвиг":
                                GlStateManager.translatef(.46f, -.3f, -.71999997f);
                                GlStateManager.rotatef(45, 0, 1, 0);
                                GlStateManager.rotatef(sin1 * -20, 0, 1, 0);
                                GlStateManager.rotatef(sin2 * -20, 0, 0, 1);
                                GlStateManager.rotatef(sin2 * -40, 1, 0, 0);
                                GlStateManager.scaled(.5f, .5f, .5f);
                                GlStateManager.translatef(-.5f, .2f, 0);
                                GlStateManager.rotatef(30, 0, 1, 0);
                                GlStateManager.rotatef(-80, 1, 0, 0);
                                GlStateManager.rotatef(60, 0, 1, 0);
                                break;
                            case "Ломание":
                                GlStateManager.translatef(.46f, -.3f, -.71999997f);
                                GlStateManager.rotatef(45, 0, 1, 0);
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress) {
        ...
        GlStateManager.translatef(0.56f, -0.44F, -0.71999997f);
