...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener
{
    ...
    private String getWindowTitle() {
        ...
        String title = "FecuritySQ - vk.com/FecuritySQ";

        ClientPlayNetHandler clientplaynethandler = this.getConnection();

        if(clientplaynethandler != null && currentServerData != null){
            title = "FecuritySQ - vk.com/FecuritySQ * playing on " + currentServerData.serverIP;
        }
            ...
            if (!this.player.isRowingBoat())
            {
                ...
                for (Hand hand : Hand.values())
                {
                    ...
                    if (this.objectMouseOver != null)
                    {
                        switch (this.objectMouseOver.getType())
                        {
                            case ENTITY:
                                ...
                                EventInteractEntity eventInteractEntity = new EventInteractEntity(entity);

                                EventManager.call(eventInteractEntity);

                                if(!eventInteractEntity.cancel){
                            ...
                            case BLOCK:
                                ...
                                EventRightClickBlock eventRightClickBlock = new EventRightClickBlock(blockraytraceresult.getPos());

                                EventManager.call(eventRightClickBlock);

                                if(eventRightClickBlock.cancel){

                                    if (!itemstack.isEmpty() && !eventRightClickBlock.cancelUsingT2o) {
                    ...
                    if (!itemstack.isEmpty())
                    {
                        ...
                        EventItemFoodRightClick itemEat = new EventItemFoodRightClick(itemstack);
                        EventManager.call(itemEat);
