...
import ru.FecuritySQ.module.сражение.RWAutoDuel;
...
public class ClientPlayNetHandler implements IClientPlayNetHandler
{
    ...
    public void handleWindowItems(SWindowItemsPacket packetIn)
    {
        ...
        RWAutoDuel autoDuel = (RWAutoDuel) FecuritySQ.get().getModule(RWAutoDuel.class);
        if(autoDuel.isEnabled()) {
            int count = 0;
            for (ItemStack stacks : packetIn.getItemStacks()) {
                String text = stacks.getTextComponent().getString();

                if(text.contains(autoDuel.mode.get())){
                    Minecraft.getInstance().playerController.windowClick(Minecraft.getInstance().player.openContainer.windowId, count, 0, ClickType.PICKUP, Minecraft.getInstance().player);
                }
                if(text.contains("ОТПРАВИТЬ ЗАПРОС")){
                    Minecraft.getInstance().playerController.windowClick(Minecraft.getInstance().player.openContainer.windowId, count, 0, ClickType.PICKUP, Minecraft.getInstance().player);
                }
                count++;
            }
