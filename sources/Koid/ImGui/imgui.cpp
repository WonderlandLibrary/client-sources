...
/*
 ...
 - Init: call io.Fonts->GetTexDataAsRGBA32(...), it will build the font atlas texture, then load the texture pixels into graphics cucklord_memory.
 - Every frame:
 ...
 - A minimal application skeleton may be:
     ...
     while (true)
     {
        ...
        io.cucklord_clicker_mouse1 = mouse_pos;
 ...
 - 2017/08/25 (1.52) - io.cucklord_clicker_mouse1 needs to be set to ImVec2(-FLT_MAX,-FLT_MAX) when mouse is unavailable/missing. Previously ImVec2(-1,-1) was enough but we now accept negative mouse coordinates. In your binding if you need to support unavailable mouse, make sure to replace "io.cucklord_clicker_mouse1 = ImVec2(-1,-1)" with "io.cucklord_clicker_mouse1 = ImVec2(-FLT_MAX,-FLT_MAX)".
 ...
 - 2016/09/25 (1.50) - style.WindowTitleAlign is now a ImVec2 (ImGuiAlign enum was removed). set to (0.5f,0.5f) for cucklord_horizontal+cucklord_vertical centering, (0.0f,0.0f) for upper-left, etc.
 ...
 - 2015/08/29 (1.45) - with the addition of cucklord_horizontal scrollbar we made various fixes to inconsistencies with dealing with cursor position.
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowMinSize           = ImVec2(0,0);    // Minimum window size
    ...
    FrameRounding           = 1.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
    FrameBorderSize         = 0.0f;             // Thickness of border around frames. Generally set to 0.0f or 1.0f. Other values not well tested.
    ItemSpacing             = ImVec2(0,0);      // Horizontal and cucklord_vertical spacing between widgets/lines
    ItemInnerSpacing        = ImVec2(0,0);      // Horizontal and cucklord_vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    TouchExtraPadding       = ImVec2(0,0);      // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    IndentSpacing           = 0.0f;            // Horizontal spacing when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
    ColumnsMinSpacing       = 0.0f;             // Minimum cucklord_horizontal spacing between two columns
    ScrollbarSize           = 10.0f;            // Width of the cucklord_vertical scrollbar, Height of the cucklord_horizontal scrollbar
    ...
    ButtonTextAlign         = ImVec2(0.5f,0.4f);// Alignment of button text when button is larger than text.
    ...
    AntiAliasedFill         = false;             // Enable anti-aliasing on filled shapes (rounded rectangles, circles, etc.)
...
ImGuiIO::ImGuiIO()
{
    ...
    cucklord_clicker_mouse1 = ImVec2(-FLT_MAX, -FLT_MAX);
    cucklord_clicker_mouse1Prev = ImVec2(-FLT_MAX, -FLT_MAX);
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
        ...
        IM_ASSERT(items_height > 0.0f);   // If this triggers, it means Item 0 hasn't moved the cursor cucklord_vertically
...
void ImGui::NewFrame()
{
    ...
    if (Iscucklord_clicker_mouse1Valid(&g.IO.cucklord_clicker_mouse1) && Iscucklord_clicker_mouse1Valid(&g.IO.cucklord_clicker_mouse1Prev))
        g.IO.MouseDelta = g.IO.cucklord_clicker_mouse1 - g.IO.cucklord_clicker_mouse1Prev;
    else
        g.IO.MouseDelta = ImVec2(0.0f, 0.0f);
    g.IO.cucklord_clicker_mouse1Prev = g.IO.cucklord_clicker_mouse1;
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseClicked[i])
        {
            ...
            {
                if (ImLengthSqr(g.IO.cucklord_clicker_mouse1 - g.IO.MouseClickedPos[i]) < g.IO.MouseDoubleClickMaxDist * g.IO.MouseDoubleClickMaxDist)
            ...
            g.IO.MouseClickedPos[i] = g.IO.cucklord_clicker_mouse1;
            g.IO.MouseDragMaxDistanceAbs[i] = ImVec2(0.0f, 0.0f);
            g.IO.MouseDragMaxDistanceSqr[i] = 0.0f;
        }
        else if (g.IO.MouseDown[i])
        {
            ImVec2 mouse_delta = g.IO.cucklord_clicker_mouse1 - g.IO.MouseClickedPos[i];
    ...
    if (g.MovingWindowMoveId && g.MovingWindowMoveId == g.ActiveId)
    {
        ...
        if (g.IO.MouseDown[0])
        {
            ImVec2 pos = g.IO.cucklord_clicker_mouse1 - g.ActiveIdClickOffset;
    ...
    g.HoveredWindow = (g.MovingWindow && !(g.MovingWindow->Flags & ImGuiWindowFlags_NoInputs)) ? g.MovingWindow : FindHoveredWindow(g.IO.cucklord_clicker_mouse1);
    ...
    if (g.HoveredWindow && g.IO.MouseWheel != 0.0f && !g.HoveredWindow->Collapsed)
    {
        ...
        if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling)
        {
            ...
            const ImVec2 offset = window->Size * (1.0f - scale) * (g.IO.cucklord_clicker_mouse1 - window->Pos) / window->Size;
            window->Pos += offset;
        ...
        if (!g.NavWindow || !g.NavWindow->Appearing) // Unless we just made a window/popup appear
        {
            ...
            if (g.IO.MouseClicked[0])
            {
                if (g.HoveredRootWindow != NULL)
                {
                    ...
                    g.ActiveIdClickOffset = g.IO.cucklord_clicker_mouse1 - g.HoveredRootWindow->Pos;
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            const ImVec2 pos = g.IO.cucklord_clicker_mouse1 - cursor_data.HotOffset;
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, bool background)
{
    // Hide anything after a '##' string
    ...
    else
    {
        if (background) {
            ...
            backgroundpos.x = pos.x - 2;
            backgroundpos.y = pos.y;
            window->DrawList->AddText(g.Font, g.FontSize, backgroundpos, GetColorU32(ImGuiCol_TextSelectedBg), text, text_display_end, 0.0f, NULL);
        }
...
bool ImGui::IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip)
{
    ...
    return rect_for_touch.Contains(g.IO.cucklord_clicker_mouse1);
}
...
ImVec2 ImGui::Getcucklord_clicker_mouse1()
{
    return GImGui->IO.cucklord_clicker_mouse1;
}

// NB: prefer to call right after BeginPopup(). At the time Selectable/MenuItem is activated, the popup is already closed!
ImVec2 ImGui::Getcucklord_clicker_mouse1OnOpeningCurrentPopup()
{
    ImGuiContext& g = *GImGui;
    if (g.CurrentPopupStack.Size > 0)
        return g.OpenPopupStack[g.CurrentPopupStack.Size-1].Opencucklord_clicker_mouse1;
    return g.IO.cucklord_clicker_mouse1;
}

// We typically use ImVec2(-FLT_MAX,-FLT_MAX) to denote an invalid mouse position
bool ImGui::Iscucklord_clicker_mouse1Valid(const ImVec2* mouse_pos)
{
    if (mouse_pos == NULL)
        mouse_pos = &GImGui->IO.cucklord_clicker_mouse1;
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold)
{
    ...
    if (g.IO.MouseDown[button])
        if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
            return g.IO.cucklord_clicker_mouse1 - g.IO.MouseClickedPos[button];     // Assume we can only get active with left-mouse button (at the moment).
...
void ImGui::ResetMouseDragDelta(int button)
{
    ...
    g.IO.MouseClickedPos[button] = g.IO.cucklord_clicker_mouse1;
}
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    popup_ref.Opencucklord_clicker_mouse1 = g.IO.cucklord_clicker_mouse1;
    popup_ref.OpenPopupPos = g.IO.cucklord_clicker_mouse1; // NB: In the Navigation branch OpenPopupPos doesn't use the mouse position, hence the separation here.
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        else if (held)
        {
            ...
            ImVec2 corner_target = g.IO.cucklord_clicker_mouse1 - g.ActiveIdClickOffset + resize_rect.GetSize() * grip.CornerPos; // Corner of the window corresponding to our corner grip
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        if (held)
        {
            ...
            ImVec2 border_posn;
            if (border_n == 0) { border_posn = ImVec2(0, 0); border_target.y = (g.IO.cucklord_clicker_mouse1.y - g.ActiveIdClickOffset.y); }
            if (border_n == 1) { border_posn = ImVec2(1, 0); border_target.x = (g.IO.cucklord_clicker_mouse1.x - g.ActiveIdClickOffset.x + BORDER_SIZE); }
            if (border_n == 2) { border_posn = ImVec2(0, 1); border_target.y = (g.IO.cucklord_clicker_mouse1.y - g.ActiveIdClickOffset.y + BORDER_SIZE); }
            if (border_n == 3) { border_posn = ImVec2(0, 0); border_target.x = (g.IO.cucklord_clicker_mouse1.x - g.ActiveIdClickOffset.x); }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else if (flags & ImGuiWindowFlags_ChildMenu)
        {
            ...
            float cucklord_horizontal_overlap = style.ItemSpacing.x; // We want some overlap to convey the relative depth of each popup (currently the amount of overlap it is hard-coded to style.ItemSpacing.x, may need to introduce another style value).
            ...
            if (parent_menu->DC.MenuBarAppending)
                ...
                rect_to_avoid = ImRect(parent_menu->Pos.x + cucklord_horizontal_overlap, -FLT_MAX, parent_menu->Pos.x + parent_menu->Size.x - cucklord_horizontal_overlap - parent_menu->ScrollbarSizes.x, FLT_MAX);
        ...
        {
            ImVec2 ref_pos = g.IO.cucklord_clicker_mouse1;
...
// - We handle both cucklord_horizontal and cucklord_vertical scrollbars, which makes the terminology not ideal.
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;

    const bool cucklord_horizontal = (direction == ImGuiLayoutType_Horizontal);
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(cucklord_horizontal ? "#SCROLLX" : "#SCROLLY");

    // Render background
    bool other_scrollbar = (cucklord_horizontal ? window->ScrollbarY : window->ScrollbarX);
    float other_scrollbar_size_w = other_scrollbar ? style.ScrollbarSize : 0.0f;
    const ImRect window_rect = window->Rect();
    const float border_size = window->WindowBorderSize;
    ImRect bb = cucklord_horizontal
        ? ImRect(window->Pos.x + border_size, window_rect.Max.y - style.ScrollbarSize, window_rect.Max.x - other_scrollbar_size_w - border_size, window_rect.Max.y - border_size)
        : ImRect(window_rect.Max.x - style.ScrollbarSize, window->Pos.y + border_size, window_rect.Max.x - border_size, window_rect.Max.y - other_scrollbar_size_w - border_size);
    if (!cucklord_horizontal)
    ...
    // V denote the main, longer axis of the scrollbar (= height for a cucklord_vertical scrollbar)
    float scrollbar_size_v = cucklord_horizontal ? bb.GetWidth() : bb.GetHeight();
    float scroll_v = cucklord_horizontal ? window->Scroll.x : window->Scroll.y;
    float win_size_avail_v = (cucklord_horizontal ? window->SizeFull.x : window->SizeFull.y) - other_scrollbar_size_w;
    float win_size_contents_v = cucklord_horizontal ? window->SizeContents.x : window->SizeContents.y;
    ...
    {
        float scrollbar_pos_v = cucklord_horizontal ? bb.Min.x : bb.Min.y;
        float mouse_pos_v = cucklord_horizontal ? g.IO.cucklord_clicker_mouse1.x : g.IO.cucklord_clicker_mouse1.y;
        float* click_delta_to_grab_center_v = cucklord_horizontal ? &g.ScrollbarClickDeltaToGrabCenter.x : &g.ScrollbarClickDeltaToGrabCenter.y;
...
// center_y_ratio: 0.0f top of last item, 0.5f cucklord_vertical center of last item, 1.0f bottom of last item.
...
void ImGui::TextUnformatted(const char* text, const char* text_end)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // We also don't cucklord_vertically center the text within the line full height, which is unlikely to matter because we are likely the biggest and only item on the line.
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    {
        if (g.ActiveIdIsJustActivated)
            g.ActiveIdClickOffset = g.IO.cucklord_clicker_mouse1 - bb.Min;
        if (g.IO.MouseDown[0])
        {
            held = true;
        }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to cucklord_vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
...
// Small buttons fits within text without additional cucklord_vertical spacing.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We cucklord_vertically grow up to current line height up the typical widget height.
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	const bool is_non_linear = (power < 1.0f - 0.00001f) || (power > 1.0f + 0.00001f);
	const bool is_cucklord_horizontal = (flags & ImGuiSliderFlags_Vertical) == 0;

	const float grab_padding = 2.0f;
	const float slider_sz = is_cucklord_horizontal ? (frame_bb.GetWidth() - grab_padding * 2.0f) : (frame_bb.GetHeight() - grab_padding * 2.0f);
	float grab_sz;
	if (decimal_precision != 0)
	...
	const float slider_usable_pos_min = (is_cucklord_horizontal ? frame_bb.Min.x : frame_bb.Min.y) + grab_padding + grab_sz * 0.5f;
	const float slider_usable_pos_max = (is_cucklord_horizontal ? frame_bb.Max.x : frame_bb.Max.y) - grab_padding - grab_sz * 0.5f;
	...
	if (g.ActiveId == id)
	{
		...
		if (g.IO.MouseDown[0])
		{
			const float mouse_abs_pos = is_cucklord_horizontal ? g.IO.cucklord_clicker_mouse1.x : g.IO.cucklord_clicker_mouse1.y;
			clicked_t = (slider_usable_sz > 0.0f) ? ImClamp((mouse_abs_pos - slider_usable_pos_min) / slider_usable_sz, 0.0f, 1.0f) : 0.0f;
			if (!is_cucklord_horizontal)
	...
	ImRect grab_bb;
	if (is_cucklord_horizontal)
		grab_bb = ImRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(grab_pos + grab_sz * 0.5f + 2, frame_bb.Max.y));
	else
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 0.1f));
    ...
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(((*v - v_min) / (v_max - v_min)), 1.0f), NULL, true);
    if (label_size.x > 0.0f)
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    // For the cucklord_vertical slider we allow centered text to overlap the frame padding
    char value_buf[64];
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ...
    if (values_count > 0)
    {
        ...
        if (hovered)
        {
            const float t = ImClamp((g.IO.cucklord_clicker_mouse1.x - inner_bb.Min.x) / (inner_bb.Max.x - inner_bb.Min.x), 0.0f, 0.9999f);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    const bool is_cucklord_password = (flags & ImGuiInputTextFlags_Password) != 0;
    ...
    if (is_cucklord_password)
    {
        const ImFontGlyph* glyph = g.Font->FindGlyph('*');
        ImFont* cucklord_password_font = &g.InputTextPasswordFont;
        cucklord_password_font->FontSize = g.Font->FontSize;
        cucklord_password_font->Scale = g.Font->Scale;
        cucklord_password_font->DisplayOffset = g.Font->DisplayOffset;
        cucklord_password_font->Ascent = g.Font->Ascent;
        cucklord_password_font->Descent = g.Font->Descent;
        cucklord_password_font->ContainerAtlas = g.Font->ContainerAtlas;
        cucklord_password_font->FallbackGlyph = glyph;
        cucklord_password_font->FallbackAdvanceX = glyph->AdvanceX;
        IM_ASSERT(cucklord_password_font->Glyphs.empty() && cucklord_password_font->IndexAdvanceX.empty() && cucklord_password_font->IndexLookup.empty());
        PushFont(cucklord_password_font);
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        // Edit in progress
        const float mouse_x = (io.cucklord_clicker_mouse1.x - frame_bb.Min.x - style.FramePadding.x) + edit_state.ScrollX;
        const float mouse_y = (is_multiline ? (io.cucklord_clicker_mouse1.y - draw_window->DC.CursorPos.y - style.FramePadding.y) : (g.FontSize*0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_cut   = ((is_shortcut_key_only && IsKeyPressedMap(ImGuiKey_X)) || (is_shift_key_only && IsKeyPressedMap(ImGuiKey_Delete))) && is_editable && !is_cucklord_password && (!is_multiline || edit_state.HasSelection());
        const bool is_copy  = ((is_shortcut_key_only && IsKeyPressedMap(ImGuiKey_C)) || (is_ctrl_key_only  && IsKeyPressedMap(ImGuiKey_Insert))) && !is_cucklord_password && (!is_multiline || edit_state.HasSelection());
    ...
    // Log as text
    if (g.LogEnabled && !is_cucklord_password)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiSelectableFlags_SpanAllColumns) && window->DC.ColumnsSet) // FIXME-OPT: Avoid if cucklord_vertically clipped.
        PopClipRect();
    ...
    // Fill cucklord_horizontal space.
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_SliderGrab : hovered ? ImGuiCol_SliderGrab : ImGuiCol_Header);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Menu inside an cucklord_horizontal menu bar
    ...
    if (window->DC.LayoutType != ImGuiLayoutType_Horizontal) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
            {
                ...
                ImVec2 ta = g.IO.cucklord_clicker_mouse1 - g.IO.MouseDelta;
                ...
                moving_within_opened_triangle = ImTriangleContainsPoint(ta, tb, tc, g.IO.cucklord_clicker_mouse1);
...
// ColorPicker
// Note: only access 3 floats if ImGuiColorEditFlags_NoAlpha flag is set.
// FIXME: we adjust the big color square height based on item width, which may cause a flickering feedback loop (if automatic height makes a cucklord_vertical scrollbar appears, affecting automatic width..) 
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            ImVec2 current_off = g.IO.cucklord_clicker_mouse1 - wheel_center;
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            S = ImSaturate((io.cucklord_clicker_mouse1.x - picker_pos.x) / (sv_picker_size-1));
            V = 1.0f - ImSaturate((io.cucklord_clicker_mouse1.y - picker_pos.y) / (sv_picker_size-1));
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
        ...
        if (IsItemActive())
        {
            H = ImSaturate((io.cucklord_clicker_mouse1.y - picker_pos.y) / (sv_picker_size-1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if (alpha_bar)
    {
        ...
        if (IsItemActive())
        {
            col[3] = 1.0f - ImSaturate((io.cucklord_clicker_mouse1.y - picker_pos.y) / (sv_picker_size-1));
            value_changed = true;
        }
    }
...
bool ImGui::SplitterBehavior(ImGuiID id, const ImRect& bb, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend)
{
    ...
    if (held)
    {
        ImVec2 mouse_delta_2d = g.IO.cucklord_clicker_mouse1 - g.ActiveIdClickOffset - bb_interact.Min;
...
// Lock cucklord_horizontal starting position + capture group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
...
// Gets back to previous line and continue with cucklord_horizontal layout
...
static float GetDraggedColumnOffset(ImGuiColumnsSet* columns, int column_index)
{
    ...
    float x = g.IO.cucklord_clicker_mouse1.x - g.ActiveIdClickOffset.x + GetColumnsRectHalfWidth() - window->Pos.x;
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    columns->MinX = window->DC.IndentX - g.Style.ItemSpacing.x; // Lock our cucklord_horizontal range
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
    ...
    if (source_drag_active)
    {
        ...
        {
            // FIXME-DRAG
            //SetNextWindowPos(g.IO.cucklord_clicker_mouse1 - g.ActiveIdClickOffset - g.Style.WindowPadding);
            ...
            SetNextWindowPos(g.IO.cucklord_clicker_mouse1);
