...
typedef int ImGuiLayoutType;        // enum: cucklord_horizontal or cucklord_vertical             // enum ImGuiLayoutType_
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_AlignTextBaseLine      = 1 << 9,   // cucklord_vertically align button to match text baseline - ButtonEx() only // FIXME: Should be removed and handled by SmallButton(), not possible currently because of DC.CursorPosPrevLine
...
struct ImGuiPopupRef
{
    ...
    ImVec2              OpenPopupPos;   // Set on OpenPopup(), preferred popup position (typically == Opencucklord_clicker_mouse1 when using mouse)
    ImVec2              Opencucklord_clicker_mouse1;   // Set on OpenPopup(), copy of mouse position at the time of opening popup
};
...
namespace ImGui
{
    ...
    IMGUI_API void          RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align = ImVec2(0, 0), const ImRect* clip_rect = NULL, bool background = false);
