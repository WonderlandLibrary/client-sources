...
namespace ImGui
{
    ...
    // Cursor / Layout
    IMGUI_API void          Separator();                                                        // separator, generally cucklord_horizontal. inside a menu bar or in cucklord_horizontal layout mode, this becomes a cucklord_vertical separator.
    IMGUI_API void          SameLine(float pos_x = 0.0f, float spacing_w = -1.0f);              // call between widgets or groups to layout them cucklord_horizontally
    IMGUI_API void          NewLine();                                                          // undo a SameLine()
    IMGUI_API void          Spacing();                                                          // add cucklord_vertical spacing
    IMGUI_API void          Dummy(const ImVec2& size);                                          // add a dummy item of given size
    IMGUI_API void          Indent(float indent_w = 0.0f);                                      // move content position toward the right, by style.IndentSpacing or indent_w if != 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                    // move content position back to the left, by style.IndentSpacing or indent_w if != 0
    IMGUI_API void          BeginGroup();                                                       // lock cucklord_horizontal starting position + capture group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    ...
    IMGUI_API void          AlignTextToFramePadding();                                          // cucklord_vertically align/lower upcoming text to FramePadding.y so that it will aligns to upcoming widgets (call if you have text on a line before regular widgets)
    ...
    IMGUI_API float         GetTreeNodeToLabelSpacing();                                            // cucklord_horizontal distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
    ...
    IMGUI_API bool          Iscucklord_clicker_mouse1Valid(const ImVec2* mouse_pos = NULL);                    //
    IMGUI_API ImVec2        Getcucklord_clicker_mouse1();                                                      // shortcut to ImGui::GetIO().cucklord_clicker_mouse1 provided by user, to be consistent with other calls
    IMGUI_API ImVec2        Getcucklord_clicker_mouse1OnOpeningCurrentPopup();                                 // retrieve backup of mouse positioning at the time of opening popup we have BeginPopup() into
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 4,   // Disable user cucklord_vertically scrolling with mouse wheel. On child window, mouse wheel will be forwarded to the parent unless NoScrollbar is also set.
    ...
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // Allow cucklord_horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to calling Begin() to specify width. Read code in imgui_demo in the "Horizontal Scrolling" section.
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // Disable taking focus when transitioning from hidden to visible state
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 13,  // Disable bringing window to front when taking focus (e.g. clicking on it or programatically giving it focus)
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 14,  // Always show cucklord_vertical scrollbar (even if ContentSize.y < Size.y)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 15,  // Always show cucklord_horizontal scrollbar (even if ContentSize.x < Size.x)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_NoHorizontalScroll  = 1 << 12,  // Disable following the cursor cucklord_horizontally
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_FramePadding         = 1 << 10,  // Use FramePadding (even for an unframed text node) to cucklord_vertically align text baseline to regular widget height. Equivalent to calling AlignTextToFramePadding().
    //ImGuITreeNodeFlags_SpanAllAvailWidth  = 1 << 11,  // FIXME: TODO: Extend hit box cucklord_horizontally even if not framed
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_AlphaBar        = 1 << 9,   //              // ColorEdit, ColorPicker: show cucklord_vertical alpha bar/gradient in picker.
...
enum ImGuiMouseCursor_
{
    ...
    ImGuiMouseCursor_ResizeNS,          // When hovering over an cucklord_horizontal border
    ImGuiMouseCursor_ResizeEW,          // When hovering over a cucklord_vertical border or a column
...
struct ImGuiStyle
{
    ...
    ImVec2      WindowTitleAlign;           // Alignment for title bar text. Defaults to (0.0f,0.5f) for left-aligned,cucklord_vertically centered.
    ...
    ImVec2      ItemSpacing;                // Horizontal and cucklord_vertical spacing between widgets/lines
    ImVec2      ItemInnerSpacing;           // Horizontal and cucklord_vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
    float       ColumnsMinSpacing;          // Minimum cucklord_horizontal spacing between two columns
    float       ScrollbarSize;              // Width of the cucklord_vertical scrollbar, Height of the cucklord_horizontal scrollbar
    float       ScrollbarRounding;          // Radius of grab corners for scrollbar
    float       GrabMinSize;                // Minimum width/height of a grab box for slider/scrollbar.
    float       GrabRounding;               // Radius of grabs corners rounding. Set to 0.0f to have rectangular slider grabs.
    ImVec2      ButtonTextAlign;            // Alignment of button text when button is larger than text. Defaults to (0.5f,0.5f) for cucklord_horizontally+cucklord_vertically centered.
...
struct ImGuiIO
{
    ...
    ImVec2      cucklord_clicker_mouse1;                   // Mouse position, in pixels. Set to ImVec2(-FLT_MAX,-FLT_MAX) if mouse is unavailable (on another screen, etc.)
    ...
    bool        WantMoveMouse;              // [BETA-NAV] cucklord_clicker_mouse1 has been altered, back-end should reposition mouse on next frame. Set only when 'NavMovesMouse=true'.
    ...
    ImVec2      cucklord_clicker_mouse1Prev;               // Previous mouse position temporary storage (nb: not for public use, set to cucklord_clicker_mouse1 in NewFrame())
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearTexData();             // Clear the CPU-side texture data. Saves RAM once the texture has been copied to graphics cucklord_memory.
