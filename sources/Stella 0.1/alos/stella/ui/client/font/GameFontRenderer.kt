...
import alos.stella.event.events.TextEvent
import alos.stella.utils.ClassUtils
...
class GameFontRenderer(font: Font) : FontRenderer(Minecraft.getMinecraft().gameSettings,
    ...
    override fun drawString(text: String, x: Float, y: Float, color: Int, shadow: Boolean): Int {
        ...
        Stella.eventManager.callEvent(event)
        ...
        if (text.contains("§")) {
            ...
            parts.forEachIndexed { index, part ->
                ...
                if (index == 0) {
                    ...
                    if (strikeThrough)
                        DrawUtils.drawLine(width / 2.0 + 1, currentFont.height / 3.0,
                                (width + currentFont.getStringWidth(words)) / 2.0 + 1, currentFont.height / 3.0,
                                FONT_HEIGHT / 16F)

                    if (underline)
                        DrawUtils.drawLine(width / 2.0 + 1, currentFont.height / 2.0,
