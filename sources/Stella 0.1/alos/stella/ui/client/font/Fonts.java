...
import alos.stella.utils.misc.HttpUtils;
...
public class Fonts {
    ...
    public static void loadFonts() {
        ...
        try {
            ...
            final File fontsFile = new File(Stella.fileManager.fontsDir, "fonts.json");
    ...
    private static void downloadFonts() {
        try {
            final File outputFile = new File(Stella.fileManager.fontsDir, "roboto.zip");
            final File sfuiFile = new File(Stella.fileManager.fontsDir, "sfui.ttf");
            final File prodSansFile = new File(Stella.fileManager.fontsDir, "Roboto-Medium.ttf");
            final File prodBoldFile = new File(Stella.fileManager.fontsDir, "Roboto-Bold.ttf");
            final File tahomaFile = new File(Stella.fileManager.fontsDir, "TahomaBold.ttf");
            final File tahomaReFile = new File(Stella.fileManager.fontsDir, "Tahoma.ttf");
            final File bangersFile = new File(Stella.fileManager.fontsDir, "Bangers-Regular.ttf");
            ...
            if (!outputFile.exists() || !sfuiFile.exists() || !prodSansFile.exists() || !prodBoldFile.exists() || !tahomaFile.exists() || !tahomaReFile.exists() || !bangersFile.exists()) {
                ...
                extractZip(outputFile.getPath(), Stella.fileManager.fontsDir.getPath());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static Font getFont(final String fontName, final int size) {
        try {
            final InputStream inputStream = new FileInputStream(new File(Stella.fileManager.fontsDir, fontName));
