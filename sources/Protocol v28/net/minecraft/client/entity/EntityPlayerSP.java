...
import events.EventPostPlayerUpdate;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdate() {
		if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			...
			if (this.isRiding()) {
				EventPreMotionUpdates preMotion = new EventPreMotionUpdates(this.rotationYaw, this.rotationPitch, this.posY);
	...
	public void sendChatMessage(String message) {
		if (message.startsWith("@")) {
			final IrcManager irc = Protocol.irc;
			if (irc.isConnected()) {
				irc.sendMessage("#Innocent", "[P] " + message.replace("@", ""));
				irc.getLines().add(new IrcLine(irc.getChatLine(true), "[P] " + message.replace("@", ""), irc.getName(), false));
				irc.setUnreadMessages(true);
			} else {
				Wrapper.tellPlayer("§7[§3IRC§7] Not Connected To IRC");
			}
			return;
		}
		if (Protocol.commands.isToggled()) {
			if (message.startsWith(".")){
				CommandManager.get().runCommand(message.replace(".", ""));
				return;
			}
		}
	...
	protected boolean pushOutOfBlocks(double x, double y, double z) {
		...
		if (this.noClip || blockPush.isCancelled()) {
			return false;
		} else {
	...
	public void onLivingUpdate() {
		EventPrePlayerUpdate preMotion = new EventPrePlayerUpdate(this.rotationYaw, this.rotationPitch, this.posY);
		EventManager.call(preMotion);
		if (preMotion.isCancelled()) {
			return;
		}
		...
		EventPostPlayerUpdate postMotion = new EventPostPlayerUpdate();
		EventManager.call(postMotion);
	}
}
