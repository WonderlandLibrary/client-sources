...
import dev.luvbeeq.baritone.api.utils.NotificationHelper;
import dev.luvbeeq.baritone.api.utils.SettingsUtil;
import dev.luvbeeq.baritone.api.utils.TypeUtils;
import dev.luvbeeq.baritone.api.utils.gui.BaritoneToast;
...
public final class Settings {
    ...
    /**
     * Blocks that dev.intave.baritone will be allowed to break even with allowBreak set to false
     */
    ...
    /**
     * Disable dev.intave.baritone's auto-tool at runtime, but still assume that another mod will provide auto tool functionality
     * <p>
     ...
     */
    public final Setting<Double> blockBreakAdditionalPenalty = new Setting<>(1D);

    /**
     * Additional penalty for hitting the space bar (ascend, pillar, or parkour) because it uses hunger
     */
    public final Setting<Double> jumpPenalty = new Setting<>(1D);

    /**
     * Walking on water uses up hunger really quick, so penalize it
     */
    public final Setting<Double> walkOnWaterOnePenalty = new Setting<>(1D);
    ...
    /**
     * blocks that dev.intave.baritone shouldn't break, but can if it needs to.
     */
     ...
     */
    public final Setting<Double> avoidBreakingMultiplier = new Setting<>(.5);
     ...
     */
    public final Setting<Float> blockReachDistance = new Setting<>(3.0f);

    /**
     * How many degrees to randomize the pitch and yaw every tick. Set to 0 to disable
     */
    public final Setting<Double> randomLooking = new Setting<>(0.0d);
     ...
     */
    public final Setting<Double> costHeuristic = new Setting<>(4.6);
     ...
     */
    public final Setting<Double> backtrackCostFavoringCoefficient = new Setting<>(1.0);
     ...
     */
    public final Setting<Integer> movementTimeoutTicks = new Setting<>(10);
    ...
    /**
     * allows dev.intave.baritone to save bed waypoints when interacting with beds
     */
    public final Setting<Boolean> doBedWaypoints = new Setting<>(true);

    /**
     * allows dev.intave.baritone to save death waypoints
     */
     ...
     */
    public final Setting<Float> pathRenderLineWidthPixels = new Setting<>(1.5F);

    /**
     * Line width of the goal when rendered, in pixels
     */
    public final Setting<Float> goalRenderLineWidthPixels = new Setting<>(1.5F);

    /**
     * Start fading out the path at 20 movements ahead, and stop rendering it entirely 30 movements ahead.
     * Improves FPS.
     */
    public final Setting<Boolean> fadePath = new Setting<>(true);
     ...
     */
    public final Setting<Integer> itemSaverThreshold = new Setting<>(1);
     ...
     */
    public final Setting<Boolean> walkWhileBreaking = new Setting<>(false);
    ...
    /**
     * This will only allow dev.intave.baritone to mine exposed ores, can be used to stop ore obfuscators on servers that use them.
     */
     ...
     */
    public final Setting<Boolean> replantNetherWart = new Setting<>(true);
     ...
     */
    public final Setting<Boolean> buildOnlySelection = new Setting<>(true);
    ...
    /**
     * Build in map art mode, which makes dev.intave.baritone only care about the top block in each column
     */
     ...
     * Thanks Louca
     */
    public final Setting<Long> mineDropLoiterDurationMSThanksLouca = new Setting<>(500L);
     ...
     * Line width of the goal when rendered, in pixels
     */
    public final Setting<Float> selectionLineWidth = new Setting<>(1.5F);
     ...
     */
    public final Setting<Boolean> desktopNotifications = new Setting<>(true);
