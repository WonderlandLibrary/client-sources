...
import dev.luvbeeq.baritone.utils.PathingCommandContext;
import dev.luvbeeq.baritone.utils.schematic.MapArtSchematic;
import dev.luvbeeq.baritone.utils.schematic.SchematicSystem;
import dev.luvbeeq.baritone.utils.schematic.SelectionSchematic;
import dev.luvbeeq.baritone.utils.schematic.format.defaults.LitematicaSchematic;
import dev.luvbeeq.baritone.utils.schematic.litematica.LitematicaHelper;
import dev.luvbeeq.baritone.utils.schematic.schematica.SchematicaHelper;
...
import static dev.luvbeeq.baritone.api.pathing.movement.ActionCosts.COST_INF;
...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    private Optional<Placement> possibleToPlace(BlockState toPlace, int x, int y, int z, BlockStateInterface bsi) {
        for (Direction against : Direction.values()) {
            ...
            for (Vector3d placementMultiplier : aabbSideMultipliers(against)) {
                ...
                if (result.getType() == RayTraceResult.Type.BLOCK && ((BlockRayTraceResult) result).getPos().equals(placeAgainstPos) && ((BlockRayTraceResult) result).getFace() == against.getOpposite()) {
    ...
    private List<BlockState> approxPlaceable(int size) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            result.add(Objects.requireNonNullElseGet(itemState, Blocks.AIR::getDefaultState));
            // </toxic cloud>
        }
        return result;
    }
