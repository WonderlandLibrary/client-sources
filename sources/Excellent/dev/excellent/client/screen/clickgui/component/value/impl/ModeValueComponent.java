...
public class ModeValueComponent extends ValueComponent {
    public ModeValueComponent(Value<?> value, float width) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        font12.draw(matrixStack, modeValue.getName(), x + padding, y + padding, ColorUtil.getColor(215, 255, 255, (int) Mathf.clamp(5, 255, (alphaAnimation.getValue() * 255))));

        float size = 8;

        float offset = 0;
        for (Mode<?> mode : modeValue.getModes()) {
            boolean selected = mode.getName().equals(modeValue.getValue().getName());
            ...
            font12.draw(matrixStack, mode.getName(), x + padding + size + 2.5F, yPos + 0.5F, selected ? ColorUtil.getColor(255, 255, 255, (int) Mathf.clamp(5, 255, (alphaAnimation.getValue() * 255))) : ColorUtil.getColor(200, 200, 200, (int) Mathf.clamp(5, 255, (alphaAnimation.getValue() * 255))));
            offset += size + 4F;
        }
