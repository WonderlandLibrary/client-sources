...
public class PlayerInventory implements IInventory, INameable {
    ...
    public void changeCurrentItem(double direction) {
        Minecraft.getInstance().gameRenderer.whellZooming += direction;
        if (direction > 0)
            Minecraft.getInstance().gameRenderer.whellZooming *= 1.1F;
        else Minecraft.getInstance().gameRenderer.whellZooming /= 1.1F;
        if (direction > 0.0D) {
            direction = 1.0D;
        }
