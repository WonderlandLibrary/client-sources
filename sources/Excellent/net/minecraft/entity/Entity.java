...
import dev.excellent.client.module.impl.misc.SeeInvisibles;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public void setPacketCoordinates(double x, double y, double z) {
        this.setPacketCoordinates(new Vector3d(x, y, z));
    }

    public void setPacketCoordinates(Vector3d packetCoordinates) {
        this.field_242272_av = packetCoordinates;
    }
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                MoveEvent move = new MoveEvent(fromPosition, predictedPosition, pos, onGround, collidedHorizontally, collidedVertically, bb);
                Excellent.getInst().getEventBus().handle(move);
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                Excellent.getInst().getEventBus().handle(post);
            }
    ...
    private Vector3d getAllowedMovement(Vector3d vec, boolean hor, boolean ver) {
        ...
        if (flag3 && (flag || flag2)) {
            if (stepHeight > 0.0F) {
                Vector3d vector3d1 = collideBoundingBoxHeuristically(this, new Vector3d(vec.x, stepHeight, vec.z), axisalignedbb, this.world, iselectioncontext, reuseablestream, hor, ver);
                Vector3d vector3d2 = collideBoundingBoxHeuristically(this, new Vector3d(0.0D, stepHeight, 0.0D), axisalignedbb.expand(vec.x, 0.0D, vec.z), this.world, iselectioncontext, reuseablestream, hor, ver);
    ...
    public void moveRelative(float friction, Vector3d relative) {
        ...
        if (this instanceof ClientPlayerEntity clientPlayer && BaritoneAPI.getProvider().getBaritoneForPlayer(clientPlayer) != null) {
            ...
            BaritoneAPI.getProvider().getBaritoneForPlayer(clientPlayer).getGameEventHandler().onPlayerRotationMove(motionUpdateRotationEvent);
    ...
    private static Vector3d getAbsoluteMotion(Vector3d relative, float friction, float facing) {
        double d0 = relative.lengthSquared();

        if (d0 < 1.0E-7D) {
            return Vector3d.ZERO;
        } else {
            Vector3d vector3d = (d0 > 1.0D ? relative.normalize() : relative).scale(friction);
    ...
    public Vector3d getLookVec() {
        ...
        if (this instanceof ClientPlayerEntity) {
            LookVecEvent event = new LookVecEvent(yaw, pitch);
            Excellent.getInst().getEventBus().handle(event);
            yaw = event.getYaw();
    ...
    public boolean isInvisibleToPlayer(PlayerEntity player) {
        if (SeeInvisibles.singleton.get().isEnabled() && !(this instanceof ArmorStandEntity))
            return false;
