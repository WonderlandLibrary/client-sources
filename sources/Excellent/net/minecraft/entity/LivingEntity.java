...
import dev.excellent.api.event.impl.player.LookVecEvent;
...
public abstract class LivingEntity extends Entity {
    ...
    public boolean isPotionActive(Effect potionIn) {
        if ((potionIn == Effects.NAUSEA || potionIn == Effects.BLINDNESS) && NoRender.singleton.get().isEnabled() && NoRender.singleton.get().getElements().isEnabled("Плохие эффекты"))
            return false;
    ...
    public boolean onLivingFall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            if (this instanceof ClientPlayerEntity) {
                Excellent.getInst().getEventBus().handle(new DamageEvent(DamageEvent.DamageType.FALL));
            }
            return true;
        } else {
            return flag;
    ...
    protected void jump() {
        ...
        float yaw = BaritoneUtils.overrideYaw(this, jumpRotationEvent);
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                if (this instanceof ClientPlayerEntity) {
                    LookVecEvent.Elytra event = new LookVecEvent.Elytra(pitch);
                    Excellent.getInst().getEventBus().handle(event);
    ...
    public boolean canBePushed() {
        if (NoPush.singleton.get().isEnabled() && NoPush.singleton.get().elements.isEnabled("Игроков") && this instanceof ClientPlayerEntity) {
            return false;
        }
