...
import dev.excellent.api.event.impl.other.BlockBreakEvent;
import dev.excellent.api.event.impl.player.AttackEvent;
import dev.excellent.api.event.impl.player.EntityInteractEvent;
import dev.excellent.client.module.impl.misc.NoInteract;
import dev.luvbeeq.baritone.utils.accessor.IPlayerControllerMP;
...
public class PlayerController implements IPlayerControllerMP {
            ...
            if (!this.mc.player.getHeldItemMainhand().getItem().canPlayerBreakBlockWhileHolding(blockstate, world, pos, this.mc.player)) {
                ...
                if ((block instanceof CommandBlockBlock || block instanceof StructureBlock || block instanceof JigsawBlock) && !this.mc.player.canUseCommandBlock()) {
                    ...
                    if (flag) {
                        ...
                        Excellent.getInst().getEventBus().handle(new BlockBreakEvent(pos));
                    }
    ...
    public ActionResultType rightClickBlock(ClientPlayerEntity clientPlayer, ClientWorld world, Hand hand, BlockRayTraceResult rayTraceResult) {
        this.syncCurrentPlayItem();
        BlockPos blockpos = rayTraceResult.getPos();

        EntityInteractEvent event = new EntityInteractEvent(clientPlayer);
        Excellent.getInst().getEventBus().handle(event);
        if (event.isCancelled()) return ActionResultType.PASS;

        if (!this.mc.world.getWorldBorder().contains(blockpos)) {
            return ActionResultType.FAIL;
        } else {
            ItemStack itemstack = clientPlayer.getHeldItem(hand);

            if (this.currentGameType == GameType.SPECTATOR) {
                this.connection.sendPacket(new CPlayerTryUseItemOnBlockPacket(hand, rayTraceResult));
                return ActionResultType.SUCCESS;
            } else {
                boolean flag = !clientPlayer.getHeldItemMainhand().isEmpty() || !clientPlayer.getHeldItemOffhand().isEmpty();
                boolean flag1 = clientPlayer.isSecondaryUseActive() && flag;
                NoInteract noInteract = NoInteract.getSingleton().get();

                boolean flag2 = noInteract.isEnabled() && noInteract.getMode().is("Все Блоки") && itemstack.getItem() instanceof BlockItem || noInteract.isEnabled() &&
                        noInteract.getBlocks().contains(world.getBlockState(rayTraceResult.getPos()).getBlockId());

                if (!flag2) {
                    if (!flag1) {
                        ActionResultType actionresulttype = world.getBlockState(blockpos).onBlockActivated(world, clientPlayer, hand, rayTraceResult);
                ...
                if (!itemstack.isEmpty() && !clientPlayer.getCooldownTracker().hasCooldown(itemstack.getItem())) {
                    ItemUseContext itemusecontext = new ItemUseContext(clientPlayer, hand, rayTraceResult);
    ...
    public ClientPlayerEntity createPlayer(ClientWorld worldIn, StatisticsManager statsManager, ClientRecipeBook recipes, boolean sneak, boolean sprint) {
        return new ClientPlayerEntity(this.mc, worldIn, this.connection, statsManager, recipes, sneak, sprint);
    }
    ...
    public void attackEntity(PlayerEntity playerIn, Entity targetEntity) {
        ...
        Excellent.getInst().getEventBus().handle(event);
        if (targetEntity.equals(playerIn) || event.isCancelled() || Excellent.getInst().getFriendManager().isFriend(TextFormatting.getTextWithoutFormattingCodes(targetEntity.getName().getString())))
            return;
        this.syncCurrentPlayItem();
    ...
    public ActionResultType interactWithEntity(PlayerEntity player, Entity target, Hand hand) {
        ...
        EntityInteractEvent event = new EntityInteractEvent(target);
        Excellent.getInst().getEventBus().handle(event);
        if (target.equals(player) || event.isCancelled()) return ActionResultType.PASS;
...
//        ChatUtil.addText("window: " + windowId + ", slot: " + slotId + ", mouse: " + mouseButton + ", type: " + type);
//        if (player.openContainer instanceof ChestContainer wrapper) {
//            ChatUtil.addText("size: " + wrapper.getLowerChestInventory().getSizeInventory());
//        }
