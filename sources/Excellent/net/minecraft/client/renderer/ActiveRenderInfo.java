...
import dev.excellent.client.module.impl.player.CameraClip;
...
public class ActiveRenderInfo {
    ...
    RotationEvent event = new RotationEvent(0, 0, 0);
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks) {
        ...
        this.setDirection(event.yaw, event.pitch);
        ...
        if (thirdPersonIn) {
            if (thirdPersonReverseIn) {
                this.setDirection(event.yaw + 180.0F, -event.pitch);
            }
            double startingDistance;
            if (CameraClip.singleton.get().isEnabled()) {
                startingDistance = CameraClip.singleton.get().cameraDistance.getValue().doubleValue();
            } else {
                startingDistance = 4.0D;
            }
            this.movePosition(-this.calcCameraDistance(startingDistance), 0.0D, 0.0D);
    ...
    private double calcCameraDistance(double startingDistance) {
        for (int i = 0; i < 8; ++i) {
            ...
            Excellent.getInst().getEventBus().handle(cameraClipEvent);
    ...
    protected void setDirection(float yaw, float pitch) {
        this.pitch = pitch;
        this.yaw = yaw;
