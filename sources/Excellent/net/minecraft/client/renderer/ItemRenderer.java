...
public class ItemRenderer implements IResourceManagerReloadListener {
    ...
    protected void renderItemModelIntoGUI(ItemStack stack, float x, float y, IBakedModel bakedmodel) {
        renderItemGui = true;
        RenderSystem.pushMatrix();
        this.textureManager.bindTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE);
        this.textureManager.getTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE).setBlurMipmapDirect(false, false);
    ...
    public void renderItemAndEffectIntoGuiWithoutEntity(ItemStack stack, float x, float y) {
        this.renderItemIntoGUI(null, stack, x, y);
    }

    public void renderItemAndEffectIntoGUI(LivingEntity entityIn, ItemStack itemIn, float x, float y) {
        this.renderItemIntoGUI(entityIn, itemIn, x, y);
    }

    private void renderItemIntoGUI(@Nullable LivingEntity livingEntity, ItemStack stack, float x, float y) {
        if (!stack.isEmpty()) {
            this.zLevel += 50.0F;
            ...
            try {
                ...
                crashreportcategory.addDetail("Registry Name", () ->
                        String.valueOf(Reflector.call(stack.getItem(), Reflector.ForgeRegistryEntry_getRegistryName)));
                crashreportcategory.addDetail("Item Damage", () ->
                        String.valueOf(stack.getDamage()));
                crashreportcategory.addDetail("Item NBT", () ->
                        String.valueOf(stack.getTag()));
                crashreportcategory.addDetail("Item Foil", () ->
                        String.valueOf(stack.hasEffect()));
