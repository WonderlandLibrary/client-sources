...
import dev.excellent.api.event.impl.funs.CrystalRemodellingEvent;
...
public class EnderCrystalRenderer extends EntityRenderer<EnderCrystalEntity> {
   ...
   public void render(EnderCrystalEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
      CrystalRemodellingEvent event = new CrystalRemodellingEvent();
      Excellent.getInst().getEventBus().handle(event);
      boolean remodel = event.isCancelled();
      matrixStackIn.push();
      ...
      matrixStackIn.translate(0.0D, -0.5D * (remodel ? -.5 : 1), 0.0D);
      ...
      if (!remodel) {
         ...
         matrixStackIn.translate(0.0D, 1.5F + f / 2.0F, 0.0D);
