...
import dev.excellent.client.command.impl.PanicCommand;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(entity, vector3d, vector3d2, axisalignedbb, (ent) -> {
                return ent.canBeRaytracing() && !ent.isSpectator() && ent.canBeCollidedWith();
            }, d1);
    ...
    AnimationUtil zoomAnim = AnimationUtil.create(1, 1, .1F);
    public double whellZooming = 0;

    private float getUpdatedSmoothZooming(boolean zoomActive) {
        boolean canSmoothZoom = !PanicCommand.stopClient;
        float zoomValue = (zoomActive ? (canSmoothZoom ? 3 : 4) : 1);
        if (whellZooming > 3500) whellZooming = 3500;
        else if (whellZooming < 0) whellZooming = 0;
        if (whellZooming != 0 && !zoomActive) whellZooming = 0;
        float to = zoomValue + (float) whellZooming / 2.F;
        // to = to > 1500 ? 1500 : to;
        zoomAnim.setTo(to);
        zoomAnim.setSpeed(zoomActive && canSmoothZoom ? .1F : .2F);
        return canSmoothZoom ? zoomAnim.getAnim() : zoomValue;
    }
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        if (noRender.isEnabled() && noRender.getElements().isEnabled("Тряска камеры")) return;
        ...
        if (!this.debugView) {
            ...
            if (p_renderHand_4_) {
                ...
                if (this.mc.gameSettings.getPointOfView().func_243192_a() && !flag && !this.mc.gameSettings.hideGUI && this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR) {
                    ...
                    this.resetProjectionMatrix(this.getProjectionMatrix(p_renderHand_2_, p_renderHand_3_, true));
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (renderWorldIn && this.mc.world != null) {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    IRenderAccess.dragProfiler.start();
                    Excellent.getInst().getComponentManager().get(DragComponent.class).render();
                    IRenderAccess.dragProfiler.stop();
                    IRenderAccess.dragProfiler.reset();

                    IShader.render2DBlur();
                    RenderFactory.renderAllTasks();

                    IRenderAccess.render2dProfiler.start();
                    Excellent.getInst().getEventBus().handle(new Render2DEvent(matrixstack, activeRender, mainwindow, partialTicks));
                    IRenderAccess.render2dProfiler.stop();
                    IRenderAccess.render2dProfiler.reset();

                    ScaleMath.scalePost();
                    this.mc.ingameGUI.renderIngameGui(matrixstack, partialTicks);
                    ScaleMath.scalePre();
                    if (this.mc.currentScreen instanceof ClickGuiScreen && ClickGui.singleton.get().isEnabled()) {
                        ClickGui.singleton.get().BLUR_SHADER.draw(4, 3, BlurShader.RenderType.DISPLAY, -2);
                    }
    ...
    public CustomFramebuffer buffer = new CustomFramebuffer(true);
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        if (FogBlur.singleton.get().isEnabled()) {
            buffer.setup(true);
            this.mc.worldRenderer.updateCameraAndRender(matrixStackIn, partialTicks, finishTimeNano, flag1, activerenderinfo, this, this.lightmapTexture, matrix4f);
            buffer.stop();
        }
        ...
        Render3DLastPostEvent lastPostEvent = new Render3DLastPostEvent(mc.worldRenderer, matrixStackIn, matrix4f, activerenderinfo, partialTicks, finishTimeNano);
        Excellent.getInst().getEventBus().handle(lastPostEvent);
        Render3DLastEvent lastEvent = new Render3DLastEvent(mc.worldRenderer, matrixStackIn, matrix4f, activerenderinfo, partialTicks, finishTimeNano);
        Excellent.getInst().getEventBus().handle(lastEvent);
        if (FogBlur.singleton.get().isEnabled()) {
            DepthRenderer.INSTANCE.doRender();
        }
        IShader.render3DBlur();
        ...
        if (this.renderHand && !Shaders.isShadowPass) {
            ...
            if (flag) {
                ...
                if (!GlassHands.singleton.get().isEnabled() || GlassHands.singleton.get().isEnabled() && !GlassHands.singleton.get().blur.getValue()) {
        ...
        matrixStackIn.rotate(Vector3f.XP.rotationDegrees(activerenderinfo.getPitch()));
        matrixStackIn.rotate(Vector3f.YP.rotationDegrees(activerenderinfo.getYaw() + 180.0F));
        Render3DEvent event = new Render3DEvent(mc.worldRenderer, matrixStackIn, matrix4f, activerenderinfo, partialTicks, finishTimeNano);
    ...
    private void frameInit() {
        ...
        if (this.updatedWorld != world) {
            ...
            Excellent.getInst().getEventBus().handle(new WorldChangeEvent());
        }
        ...
        if (this.mc.currentScreen != null && this.mc.currentScreen.getClass() == ChatScreen.class) {
            this.mc.displayScreen(new GuiChatOF((ChatScreen) this.mc.currentScreen));
        }
    }
