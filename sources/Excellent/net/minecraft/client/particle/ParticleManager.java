...
public class ParticleManager implements IFutureReloadListener {
    ...
    public ParticleManager(ClientWorld world, TextureManager textureManager) {
        cppInit(world, textureManager);
    }

    @Protect(Protect.Type.VIRTUALIZATION)
    @Native
    private void cppInit(ClientWorld world, TextureManager textureManager) {
        byType = Maps.newIdentityHashMap();
        particleEmitters = Queues.newArrayDeque();
        rand = new Random();
        factories = new HashMap<>();
        queue = Queues.newArrayDeque();
        sprites = Maps.newHashMap();
        atlas = new AtlasTexture(AtlasTexture.LOCATION_PARTICLES_TEXTURE);
