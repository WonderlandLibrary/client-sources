...
public class ChatScreen extends Screen implements IWindow {
    ...
    private ThemeWidget getThemeWidget() {
        return Excellent.getInst().getThemeWidget();
    }

    protected void init() {
        if (mc.firstChatInit) {
            getThemeWidget().getPosition().set((scaled().x / 2F) - (getThemeWidget().getDrag().size.x / 2F), 10);
            mc.firstChatInit = false;
        }
        getThemeWidget().init();
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        getThemeWidget().keyPressed(keyCode, scanCode, modifiers);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (getThemeWidget().isHovered()) return super.mouseScrolled(mouse.x, mouse.y, delta);
        if (delta > 1.0D) {
            delta = 1.0D;
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        Vector2d mouse = ScaleMath.getMouse(mouseX, mouseY);
        getThemeWidget().mouseClicked(mouse.x, mouse.y, button);
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        getThemeWidget().mouseReleased(mouseX, mouseY, button);
        return super.mouseReleased(mouseX, mouseY, button);
    }

    @Override
    public boolean charTyped(char codePoint, int modifiers) {
        getThemeWidget().charTyped(codePoint, modifiers);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        Vector2d mouse = ScaleMath.getMouse(mouseX, mouseY);
        ScaleMath.scalePre();
        getThemeWidget().render(matrixStack, (int) mouse.x, (int) mouse.y, partialTicks);
        ScaleMath.scalePost();
