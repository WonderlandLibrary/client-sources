...
import dev.excellent.client.module.impl.player.ItemScroller;
import dev.excellent.client.module.impl.render.JumpCircles;
...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    public int titleX;
    public int titleY;
    public int playerInventoryTitleX;
    ...
    public final float animationDuration = 350.0f;
    public final float startExpandY = -50f;

    public float getYAnimationTranslate() {
        float timePC = Mathf.clamp01((System.currentTimeMillis() - initTime) / animationDuration);
        float animationByPC = 1.F - (float) JumpCircles.easeOutBack(timePC);
        return startExpandY * animationByPC;
    }
    ...
    protected void init() {
        ...
        AutoBuy autoBuy = AutoBuy.singleton.get();
        if (autoBuy.isEnabled() && autoBuy.isAuction(title.getString())) {
            String autoBuyText = "AutoBuy: " + (autoBuy.isActive() ? TextFormatting.GREEN + "активен" : TextFormatting.RED + "нeактивен");
            int btnOffset = 15;
            this.addButton(new Button(width / 2 - ((btnOffset + font.getStringWidth(autoBuyText) + btnOffset) / 2), guiTop - 25, (btnOffset + font.getStringWidth(autoBuyText) + btnOffset), 20, new StringTextComponent(autoBuyText), (button) -> {
                if (minecraft.player != null && minecraft.playerController != null) {
                    autoBuy.setActive(!autoBuy.isActive());
                    String btnText = "AutoBuy: " + (autoBuy.isActive() ? TextFormatting.GREEN + "активен" : TextFormatting.RED + "нeактивен");
                    button.setMessage(new StringTextComponent(btnText));
                    button.x = width / 2 - ((btnOffset + font.getStringWidth(btnText) + btnOffset) / 2);
                    button.setWidth(btnOffset + font.getStringWidth(btnText) + btnOffset);
                }
            }));
        }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        float yExtend = getYAnimationTranslate();
        int i = this.guiLeft;
        int j = this.guiTop;
        if (yExtend != 0) GL11.glTranslated(0, yExtend, 0);
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1) {
            ...
            if (!stack.isEmpty() && i1 < 43 && this.getContainer() instanceof ChestContainer && AutoBuy.singleton.get().isAuction(title.getString())) {
                int price = AutoBuy.singleton.get().getPrice(stack) * stack.getCount();
                if (price < minPrice) {
                    minPrice = price;
                    minPriceSlot = i1;
                }
            ...
            if (this.isSlotSelected(slot, mouseX, mouseY) && slot.isEnabled()) {
                if (ItemScroller.singleton.get().isEnabled()) {
                    if (GLFW.glfwGetMouseButton(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_MOUSE_BUTTON_LEFT) == GLFW.GLFW_PRESS
                        ...
                        if (timerUtil.hasReached(ItemScroller.singleton.get().getDelay().getValue().longValue()) && slot.getHasStack()) {
        ...
        if (minPriceSlot != -1 && AutoBuy.singleton.get().isAuction(title.getString())) {
            Slot minSlot = this.container.inventorySlots.get(minPriceSlot);
            RectUtil.drawRect(matrixStack, guiLeft + minSlot.xPos, guiTop + minSlot.yPos, guiLeft + minSlot.xPos + 16, guiTop + minSlot.yPos + 16, Excellent.getInst().getTheme().getClientColor(0));
        }
        if (yExtend != 0) GL11.glTranslated(0, -yExtend, 0);
    }
        ...
        if (super.keyPressed(keyCode, scanCode, modifiers)) {
            ...
            if (this.hoveredSlot != null && this.hoveredSlot.getHasStack()) {
                if (this.minecraft.gameSettings.keyBindPickBlock.matchesKey(keyCode, scanCode)) {
                    ...
                    if (ItemScroller.singleton.get().isEnabled() && hasShiftDown() && hasControlDown()) {
