...
import dev.excellent.client.module.impl.render.Crosshair;
...
import dev.luvbeeq.shader.KawaseBlur;
...
public class IngameGui extends AbstractGui {
    ...
    @Getter
    private PlayerTabOverlayGui overlayPlayerList;
    ...
    private Map<ChatType, List<IChatListener>> chatListeners;
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        if (FogBlur.singleton.get().isEnabled()) {
            KawaseBlur.INSTANCE.updateBlur(2, 4);
        }
        ...
        if (this.mc.playerController.getCurrentGameType() == GameType.SPECTATOR) {
            ...
            RenderHotbarEvent hotbarEvent = new RenderHotbarEvent(matrixStack, partialTicks);
            Excellent.getInst().getEventBus().handle(hotbarEvent);
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            if (this.mc.gameSettings.heldItemTooltips && (FreeCam.singleton.get().isEnabled() || this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR)) {
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            if (this.overlayPlayerList.isVisible() || this.overlayPlayerList.getAnimation().getValue() != 0) {
                ScaleMath.scalePre();
                this.overlayPlayerList.func_238523_a_(matrixStack, (int) ScaleMath.getMouse(mc.getMainWindow().getScaledWidth(), 0).x, scoreboard, scoreobjective1);
                ScaleMath.scalePost();
            }
        }
    ...
    private void func_238456_d_(MatrixStack p_238456_1_) {
        ...
        if (gamesettings.getPointOfView().func_243192_a() && (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR || this.isTargetNamedMenuProvider(this.mc.objectMouseOver))) {
            if (!Crosshair.singleton.get().isEnabled() && gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug() && !gamesettings.reducedDebugInfo) {
                RenderSystem.pushMatrix();
                ...
                RenderSystem.popMatrix();
            } else if (!Crosshair.singleton.get().isEnabled()) {
                RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.ONE_MINUS_DST_COLOR, GlStateManager.DestFactor.ONE_MINUS_SRC_COLOR, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                int i = 15;
                this.blit(p_238456_1_, (this.scaledWidth - 15) / 2F, (this.scaledHeight - 15) / 2F, 0, 0, 15, 15);
...
//                }
    ...
    private void func_238447_a_(MatrixStack p_238447_1_, ScoreObjective p_238447_2_) {
        ...
        List<Score> list = collection.stream().filter((p_lambda$renderScoreboard$1_0_) ->
                p_lambda$renderScoreboard$1_0_.getPlayerName() != null && !p_lambda$renderScoreboard$1_0_.getPlayerName().startsWith("#")).collect(Collectors.toList());
        ...
        ScoreBoardEvent scoreBoardEvent = new ScoreBoardEvent(list1);
        Excellent.getInst().getEventBus().handle(scoreBoardEvent);

        NoRender noRender = NoRender.singleton.get();
        if (noRender.isEnabled() && noRender.getElements().isEnabled("Скорборд")) return;
        ...
        for (Pair<Score, ITextComponent> pair : list1) {
            ...
            // default itextcomponent2
            if (NameProtect.singleton.get().isEnabled()) {
                ITextComponent newComponent = new StringTextComponent("");
                if (!itextcomponent2.getSiblings().isEmpty()) {
                    for (ITextComponent iTextComponent : itextcomponent2.getSiblings()) {
                        if (iTextComponent.getString().contains(mc.player.getNameClear())) {
                            int k5 = 0;
                            for (ITextComponent lineComponent : iTextComponent.getSiblings()) {
                                if (lineComponent.getString().contains(mc.player.getNameClear())) {
                                    newComponent.getSiblings()
                                            .add(new StringTextComponent(iTextComponent.getSiblings().get(k5).getString()
                                                    .replaceAll(mc.player.getNameClear(), NameProtect.singleton.get().name.getValue()))
                                                    .mergeStyle(iTextComponent.getSiblings().get(k5).getStyle()));
                                } else {
                    ...
                    newComponent = newComponent.deepCopy().appendString(itextcomponent2.getString().replaceAll(mc.player.getNameClear(), NameProtect.singleton.get().name.getValue()));
                }
                this.getFontRenderer().func_243248_b(p_238447_1_, newComponent, (float) l2, (float) k1, -1);
            } else {
...
//            this.getFontRenderer().drawString(p_238447_1_, s, (float) (l1 - this.getFontRenderer().getStringWidth(s)), (float) k1, -1);
    ...
    public PlayerEntity getRenderViewPlayer() {
    ...
    public void func_238450_a_(ChatType p_238450_1_, ITextComponent p_238450_2_, UUID p_238450_3_) {
        if (!this.mc.cannotSendChatMessages(p_238450_3_) && (!this.mc.gameSettings.hideMatchedNames || !this.mc.cannotSendChatMessages(this.func_244795_b(p_238450_2_)))) {
