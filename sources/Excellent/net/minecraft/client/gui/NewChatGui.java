...
public class NewChatGui extends AbstractGui {
    ...
    private void func_238493_a_(ITextComponent p_238493_1_, int p_238493_2_, int p_238493_3_, boolean p_238493_4_) {
        ...
        list = RenderComponentsUtil.func_238505_a_(p_238493_1_, i, this.mc.fontRenderer);
        if (Excellent.getInst().getFriendManager() != null) {

            for (Friend friend : Excellent.getInst().getFriendManager()) {
                if (NameProtect.singleton.get().isEnabled() && mc.player != null && p_238493_1_.getString().contains(friend.getName())) {
                    ITextComponent chatComponent = p_238493_1_;
                    ...
                    list = RenderComponentsUtil.func_238505_a_(newComponent, i, this.mc.fontRenderer);
                }
            }
        }

        if (NameProtect.singleton.get().isEnabled() && mc.player != null && p_238493_1_.getString().contains(mc.player.getNameClear())) {
            ITextComponent chatComponent = p_238493_1_;
            ITextComponent newComponent = new StringTextComponent("");
            if (!chatComponent.getSiblings().isEmpty()) {
                int ixx = 0;
            ...
            list = RenderComponentsUtil.func_238505_a_(newComponent, i, this.mc.fontRenderer);
        } else if (Excellent.getInst().getFriendManager().stream().noneMatch(x -> TextFormatting.getTextWithoutFormattingCodes(p_238493_1_.getString()).toLowerCase().contains(x.getName()))) {
            list = RenderComponentsUtil.func_238505_a_(p_238493_1_, i, this.mc.fontRenderer);
        }
        boolean flag = this.getChatOpen();
