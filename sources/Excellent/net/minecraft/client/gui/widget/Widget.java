...
public abstract class Widget extends AbstractGui implements IRenderable, IGuiEventListener {
    ...
    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        btnDarkness = Interpolator.lerp(btnDarkness, isHovered ? 0.75F : 0.25F, 0.05);
        Themes theme = Excellent.getInst().getThemeManager().getTheme();
        float darkness = this.active ? btnDarkness : 0.2F;
        int color1 = ColorUtil.multDark(theme.getClientColor(0), darkness);
        int color2 = ColorUtil.multDark(theme.getClientColor(90), darkness);
        int color3 = ColorUtil.multDark(theme.getClientColor(180), darkness);
        int color4 = ColorUtil.multDark(theme.getClientColor(270), darkness);
        RectUtil.drawRoundedRectShadowed(matrixStack, this.x + 2, this.y + 2, this.x + this.width - 2, this.y + this.height - 2, 2, 5, color1, color2, color3, color4, true, true, true, true);
        RectUtil.drawRoundedRectShadowed(matrixStack, this.x + 2, this.y + 2, this.x + this.width - 2, this.y + this.height - 2, 2, 5, color1, color2, color3, color4, true, true, false, true);
        this.renderBg(matrixStack, minecraft, mouseX, mouseY);
        int j = this.active ? 16777215 : 10526880;
        Fonts.INTER_MEDIUM.get(16).drawCenter(matrixStack, this.getMessage().getString(), this.x + this.width / 2F, this.y + (this.height - 8) / 2F - 1, j | MathHelper.ceil(this.alpha * 255.0F) << 24);
    }
