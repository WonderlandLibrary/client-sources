    ...
    public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
        if (windowPointer == this.mc.getMainWindow().getHandle()) {
            ...
            if (this.mc.currentScreen == null || this.mc.currentScreen.passEvents) {
                ...
                if (action == 0 && key != -1) {
                    Excellent.getInst().getEventBus().handle(new KeyboardReleaseEvent(key, mc.currentScreen));
                }
                if (action == 1 && key != -1) {
                    Excellent.getInst().getEventBus().handle(new KeyboardPressEvent(key, mc.currentScreen));
                }
                if (action == 0) {
