...
public class MouseHelper {
    ...
    private void mouseButtonCallback(long handle, int button, int action, int mods) {
        if (handle == this.minecraft.getMainWindow().getHandle()) {
            ...
            if (GLFW.GLFW_PRESS == action && minecraft.currentScreen == null) {
                MouseInputEvent mouseInputEvent = new MouseInputEvent(i);
                Excellent.getInst().getEventBus().handle(mouseInputEvent);
            }
    ...
    public void updatePlayerLook() {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused()) {
            ...
            if (FreeCam.singleton.get().isEnabled() && FreeCam.singleton.get().getPlayer() != null) {
                FreeCam.singleton.get().getPlayer().rotateTowards(d2, d3 * (double) i);
            } else {
                if (this.minecraft.player != null) {
                    LookEvent event = new LookEvent(d2, d3 * (double) i);
                    Excellent.getInst().getEventBus().handle(event);
                    if (!event.isCancelled()) this.minecraft.player.rotateTowards(d2, d3 * (double) i);
                }
            }
        } else {
            this.xVelocity = 0.0D;
    ...
    public void grabMouse() {
        if (this.minecraft.isGameFocused()) {
            if (!this.mouseGrabbed) {
                ...
                this.mouseX = (double) this.minecraft.getMainWindow().getWidth() / 2D;
                this.mouseY = (double) this.minecraft.getMainWindow().getHeight() / 2D;
