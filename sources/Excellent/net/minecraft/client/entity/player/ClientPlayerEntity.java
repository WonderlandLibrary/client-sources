...
import dev.excellent.client.module.impl.player.NoPush;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    // EXCELLENT START
    public Vector3d tempMoveVec = Vector3d.ZERO;

    public Vector3d getTempMoveVec() {
        return tempMoveVec;
    }
    // EXCELLENT END
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            Excellent.getInst().getEventBus().handle(new UpdateEvent());
            super.tick();
            Excellent.getInst().getEventBus().handle(new PostUpdateEvent());
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Excellent.getInst().getEventBus().handle(actionEvent);
        ...
        if (actionEvent.isSprintState() != this.serverSprintState) {
            CEntityActionPacket.Action centityactionpacket$action = actionEvent.isSprintState() ? CEntityActionPacket.Action.START_SPRINTING : CEntityActionPacket.Action.STOP_SPRINTING;
            this.connection.sendPacket(new CEntityActionPacket(this, centityactionpacket$action));
            this.serverSprintState = actionEvent.isSprintState();
        }
        ...
        if (event.getPostMotion() != null) {
            event.getPostMotion().run();
        }
        event.setPostMotion(null);
    }
    ...
    public void sendChatMessage(String message) {
        ...
        if (baritone != null) {
            baritone.getGameEventHandler().onSendChatMessage(eventBaritone);
        }
    ...
    public void closeScreen() {
        InvCloseEvent event = new InvCloseEvent(this.openContainer.windowId);
        Excellent.getInst().getEventBus().handle(event);
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        if (NoPush.singleton.get().isEnabled() && NoPush.singleton.get().elements.isEnabled("Блоков")) {
            return false;
        }
        ...
        return !this.world.func_242405_a(this, axisalignedbb1, (state, pos2) ->
                state.isSuffocating(this.world, pos2));
    }
    ...
    public void livingTick() {
        ...
        boolean flag4 = (float) this.getFoodStats().getFoodLevel() > 6.0F || BaritoneUtils.isAllowFlying(abilities, this);
        ...
        if ((this.onGround || this.canSwim()) && !flag1 && !flag2 && this.isUsingSwimmingAnimation() && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS)) {
            if (this.sprintToggleTimer <= 0 && !BaritoneUtils.isKeyDown(this.mc.gameSettings.keyBindSprint, this)) {
    ...
    public void updateRidden() {
        IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(this);
        if (baritone != null && baritone.getLookBehavior() instanceof LookBehavior lookBehavior) {
            lookBehavior.pig();
        }
        super.updateRidden();
        this.rowingBoat = false;
