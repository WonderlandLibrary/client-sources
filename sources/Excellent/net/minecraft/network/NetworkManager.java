...
import dev.luvbeeq.via.netty.handler.ViaDecoder;
import dev.luvbeeq.via.netty.handler.ViaEncoder;
...
public class NetworkManager extends SimpleChannelInboundHandler<IPacket<?>> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, IPacket<?> packetIn) throws Exception {
        if (this.channel.isOpen()) {
            try {
                PacketEvent packetEvent = new PacketEvent(PacketEvent.Action.RECEIVE, packetIn);
                Excellent.getInst().getEventBus().handle(packetEvent);
                ...
                if (this.direction == PacketDirection.CLIENTBOUND) {
                    for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
                        if (ibaritone.getPlayerContext().player() != null && ibaritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                            ibaritone.getGameEventHandler().onReceivePacket(new dev.luvbeeq.baritone.api.event.events.PacketEvent(this, EventState.PRE, packetIn));
                        }
                    }
                }
                processPacket(packetIn, this.packetListener);
                if (this.direction == PacketDirection.CLIENTBOUND) {
                    for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
                        if (ibaritone.getPlayerContext().player() != null && ibaritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                            ibaritone.getGameEventHandler().onReceivePacket(new dev.luvbeeq.baritone.api.event.events.PacketEvent(this, EventState.POST, packetIn));
                        }
                    }
                }
    ...
    public void sendPacket(IPacket<?> packetIn, @Nullable GenericFutureListener<? extends Future<? super Void>> p_201058_2_) {
        PacketEvent packetEvent = new PacketEvent(PacketEvent.Action.SENT, packetIn);
        Excellent.getInst().getEventBus().handle(packetEvent);
     ...
     */
    private void dispatchPacket(IPacket<?> packetIn, @Nullable GenericFutureListener<? extends Future<? super Void>> futureListeners) {
        if (this.direction == PacketDirection.CLIENTBOUND) {
            for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
                if (ibaritone.getPlayerContext().player() != null && ibaritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                    ibaritone.getGameEventHandler().onSendPacket(new dev.luvbeeq.baritone.api.event.events.PacketEvent(this, EventState.PRE, packetIn));
                }
            }
        }

        ProtocolType protocoltype = ProtocolType.getFromPacket(packetIn);
        ...
        if (this.direction == PacketDirection.CLIENTBOUND) {
            for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
                if (ibaritone.getPlayerContext().player() != null && ibaritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                    ibaritone.getGameEventHandler().onSendPacket(new dev.luvbeeq.baritone.api.event.events.PacketEvent(this, EventState.POST, packetIn));
                }
            }
        }
    }
        ...
        (new Bootstrap()).group(lazyvalue.getValue()).handler(new ChannelInitializer<>() {
            protected void initChannel(Channel channel) {
                ...
                if (channel instanceof SocketChannel) {
                    ...
                    if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() != Excellent.getInst().getViaMCP().getNATIVE_VERSION()) {
                        channel.pipeline().addBefore("encoder", ViaPipeLine.VIA_ENCODER_HANDLER_NAME, new ViaEncoder(user));
                        channel.pipeline().addBefore("decoder", ViaPipeLine.VIA_DECODER_HANDLER_NAME, new ViaDecoder(user));
                    }
                }
