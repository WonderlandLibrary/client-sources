...
public class ComposterBlock extends Block implements ISidedInventoryProvider {
    ...
    {
        for (int i = 0; i < 8; ++i) {
            shapes[i] = VoxelShapes.combineAndSimplify(OUT_SHAPE, Block.makeCuboidShape(2.0D, Math.max(2, 1 + i * 2), 2.0D, 14.0D, 16.0D, 14.0D), IBooleanFunction.ONLY_FIRST);
        }
    ...
    public static void playEvent(World world, BlockPos pos, boolean success) {
        ...
        world.playSound(pos.getX(), pos.getY(), pos.getZ(), success ? SoundEvents.BLOCK_COMPOSTER_FILL_SUCCESS : SoundEvents.BLOCK_COMPOSTER_FILL, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
