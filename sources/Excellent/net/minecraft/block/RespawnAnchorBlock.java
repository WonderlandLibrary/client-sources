...
public class RespawnAnchorBlock extends Block {
    ...
    private void triggerExplosion(BlockState state, World world, final BlockPos pos) {
        ...
        world.createExplosion(null, DamageSource.func_233546_a_(), explosioncontext, (double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D, 5.0F, true, Explosion.Mode.DESTROY);
    }
    ...
    public static int getChargeScale(BlockState state, int scale) {
        return MathHelper.floor((float) (state.get(CHARGES)) / 4.0F * (float) scale);
    }
