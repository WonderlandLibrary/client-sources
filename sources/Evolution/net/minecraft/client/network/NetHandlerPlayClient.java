...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleBlockChange(SPacketBlockChange packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        try {
            if (this.world.getBlockState(packetIn.getBlockPosition()).getBlock() == Blocks.AIR
                    && packetIn.getBlockState().getBlock() != Blocks.AIR) {
                IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(Minecraft.getMinecraft().player);
                if (baritone.getMineProcess().isActive() && baritone.getMineProcess() instanceof MineProcess && (Main.m.getModule(wtf.evolution.module.impl.Player.Baritone.class).state && ((wtf.evolution.module.impl.Player.Baritone) Main.m.getModule(Baritone.class)).antirg.get())) {
                    MineProcess mp = (MineProcess) baritone.getMineProcess();
                    mp.rescan = true;
                    for (int i = 0; i < 255; i++) {
                        mp.blacklist.add(new BlockPos(packetIn.getBlockPosition().getX(), i,
                                packetIn.getBlockPosition().getZ()));
                    }
                    ...
                    if (Minecraft.getMinecraft().objectMouseOver != null
                        ...
                        for (int i = 0; i < 255; i++) {
                            mp.blacklist.add(new BlockPos(Minecraft.getMinecraft().objectMouseOver.getBlockPos().getX(),
                                    i, Minecraft.getMinecraft().objectMouseOver.getBlockPos().getZ()));
                        }
                    }
                }
            }
        ...
        for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
            ...
            if (player != null && player.connection == (NetHandlerPlayClient) (Object) this) {
                ibaritone.getGameEventHandler()
                        .onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL,
                                packetIn.getBlockPosition().getX() >> 4, packetIn.getBlockPosition().getZ() >> 4));
            }
        }
    }
