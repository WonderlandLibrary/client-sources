...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    private void init() throws LWJGLException, IOException
    {
        ...
        wtf.evolution.Main main = new wtf.evolution.Main();
        main.load();
                    ...
                    case BLOCK:
                        ...
                        if (this.world.getBlockState(blockpos).getMaterial() != Material.AIR)
                        {
                            BaritoneAPI.getProvider().getPrimaryBaritone().getGameEventHandler()
                                    .onBlockInteract(new BlockInteractEvent(blockpos, BlockInteractEvent.Type.START_BREAK));
                    ...
                    if (this.objectMouseOver != null)
                    {
                        switch (this.objectMouseOver.typeOfHit)
                        {
                            case ENTITY:
                                ...
                                InteractEvent eiwe = new InteractEvent();
                            ...
                            case BLOCK:
                                ...
                                if (this.world.getBlockState(blockpos).getMaterial() != Material.AIR)
                                {
                                    ...
                                    if (enumactionresult == EnumActionResult.SUCCESS)
                                    {
                                        BaritoneAPI.getProvider().getPrimaryBaritone().getGameEventHandler()
                                                .onBlockInteract(
                                                        new BlockInteractEvent(blockpos, BlockInteractEvent.Type.USE));
    ...
    {
        if (Main.protectedd)
            new Thread(() -> Display.setTitle(RandomStringUtils.random(16))).run();
    ...
    public float deltaTime() {
        return getDebugFPS() > 0F  ? (1.0000F / getDebugFPS()) : 1F;
    }
    ...
    public void loadWorld(@Nullable WorldClient worldClientIn, String loadingMessage)
    {
        ...
        try {
            WorldData data = (WorldData) BaritoneAPI.getProvider().getPrimaryBaritone().getWorldProvider()
                    .getCurrentWorld();
            if (data != null && data.cache != null) {
                data.cache.cachedRegions.clear();
                data.cache.toPackMap.clear();
                data.cache.toPackQueue.clear();
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
