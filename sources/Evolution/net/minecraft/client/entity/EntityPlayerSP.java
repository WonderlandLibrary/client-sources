...
private float prePitch;
    private void onUpdateWalkingPlayer() {
        ...
        if (Main.m.getModule(Freecam.class).state) return;
    ...
    {
        {
            ChatEvent eventBaritone = new ChatEvent(message);
            ...
            if (eventBaritone.isCancelled()) {
                return;
            }
        }

        for (Command command : Main.d.cmds) {
    ...
    {
        if (Main.m.getModule(NoPush.class).state) return false;
        if (Main.m.getModule(Freecam.class).state) return false;
        if (this.noClip)
        {
            return false;
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= 0.8F && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS))
        {
            ...
            {
                if (Main.m.getModule(Sprint.class).state && ((Sprint) Main.m.getModule(Sprint.class)).allmode.get()) this.setSprinting(true);
