...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn)
    {
        ...
        list = list.subList(0, Math.min(list.size(), Main.m.getModule("ExtraTab").state ? Integer.MAX_VALUE : 80));
        int l3 = list.size();
        int i4 = l3;
        int j4;
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            int k2 = k1 + i5 * 9;
            List<String> strings = Bot.bots.stream().map(bot -> bot.getBot().getName()).collect(Collectors.toList());
            drawRect(j2, k2, j2 + i1, k2 + 8, (mc.player == mc.world.getPlayerEntityByUUID(list.get(k4).getGameProfile().getId()) ? new Color(0, 157, 255, 128).getRGB() :  strings.contains(list.get(k4).getGameProfile().getName()) ? new Color(54, 252, 139, 128).getRGB() : 553648127));
            ...
            if (k4 < list.size())
            {
                ...
                {
                    this.mc.fontRenderer.drawStringWithShadow(TextFormatting.ITALIC + s4, (float)j2 + 1, (float)k2, -1862270977);
                }
                else
                {
                    this.mc.fontRenderer.drawStringWithShadow(s4, (float)j2 + 1, (float)k2, -1);
                }
    ...
    protected void drawPing(int p_175245_1_, int p_175245_2_, int p_175245_3_, NetworkPlayerInfo networkPlayerInfoIn)
    {
        ...
        Fonts.pix.drawStringWithShadow(p, (float)(p_175245_2_ +  p_175245_1_ - 2 - Fonts.pix.getStringWidth(p)), (float)p_175245_3_ + 5.5f, clr);
        this.zLevel -= 100.0F;
    }
