    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1)
        {
            ...
            if (this.isMouseOverSlot(slot, mouseX, mouseY) && slot.isEnabled()) {
                ItemScroller scroller = (ItemScroller) Main.m.getModule(ItemScroller.class);
                if (scroller.state && Mouse.isButtonDown(0) && Keyboard.isKeyDown(this.mc.gameSettings.keyBindSneak.getKeyCode())
                        && timer.hasReached(scroller.delay.get())) {
                ...
                Fonts.pix.drawCenteredStringWithShadow(String.valueOf(slot.slotNumber), j1 + 16 / 2f, k1 + 16 / 2f, new Color(255, 255, 255, 255                                                                                                                                ).getRGB());
