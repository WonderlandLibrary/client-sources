...
import wtf.evolution.helpers.animation.impl.*;
...
import static net.minecraft.client.gui.GuiNewChat.animatedYText;
import static net.minecraft.client.gui.GuiNewChat.scrollPos;
...
public class GuiIngame extends Gui
{
    ...
    {
        if (Main.protectedd)
            GL11.glScaled(1, 1 - Math.abs(Math.sin(System.nanoTime() / 1000000000f)), 1);
        animatedYText = wtf.evolution.helpers.math.MathHelper.interpolate(scrollPos, animatedYText, 0.05);
        ...
        {
            ScaleUtil.scale_pre();
            Timer t = (Timer) Main.m.getModule(Timer.class);
            if (t.smart.get()) {
                t.animWidth = (float) wtf.evolution.helpers.math.MathHelper.interpolate((100 - (t.ticks * 4)), t.animWidth, 0.05f);
                t.animWidth = MathHelper.clamp(t.animWidth, 0, 100);

                RenderUtil.drawRectWH((float) (ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2 - 51), ScaleUtil.calc(scaledresolution.getScaledHeight()) - 70, 102, 7, new Color(1, 1, 1, 50).getRGB());
                RenderUtil.drawRectWH(ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f - 50, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 70 + 1, t.animWidth, 5, t.color.get());
                RenderUtil.drawBlurredShadow(ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f - 50, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 70, t.animWidth, 5, 20, new Color(t.color.get()));
                Fonts.RUB12.drawCenteredString((int) Math.round(t.animWidth) + "%", ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 67.5f, -1);
            }
        ...
        a = wtf.evolution.helpers.math.MathHelper.interpolate((mc.gameSettings.keyBindPlayerList.isKeyDown() ? 1 : 0), a, 0.1);
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            RenderUtil.drawRectWH(i - 91, sr.getScaledHeight() - 22, 182, 22, new Color(0,0,0,128).getRGB());
            StencilUtil.readStencilBuffer(0);
            RenderUtil.drawBlurredShadow(i - 91, sr.getScaledHeight() - 22, 182, 22, 10, new Color(0,0,0, 200));
