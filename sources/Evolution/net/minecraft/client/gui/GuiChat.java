...
public class GuiChat extends GuiScreen implements ITabCompleter
{
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        KillAura k = (KillAura) Main.m.getModule(KillAura.class);
        DragManager.draggables.values().forEach(dragging -> {
            if (Main.m.getModule(TargetHud.class).state) {
                if (dragging.module == k) {
                    dragging.onClick(mouseX, mouseY);
                }
            }
            if (dragging.module.state) {
                dragging.onClick(mouseX, mouseY);
            }
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        DragManager.draggables.values().forEach(dragging -> {
            if (Main.m.getModule(TargetHud.class).state) {
                if (dragging.module == k) {
                    dragging.onRelease(mouseX, mouseY);
                }
            }
            if (dragging.module.state) {
                dragging.onRelease(mouseX, mouseY);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
            ...
            if (Main.m.getModule(TargetHud.class).state) {
                if (dragging.module == k) {
                    if (dragging.dragging) {
                        dragging.xPos = (mouseX - dragging.startX);
                        dragging.yPos = (mouseY - dragging.startY);
                    }
                    dragging.xPos = MathHelper.clamp(dragging.xPos, 0, width - dragging.getWidth());
                    dragging.yPos = MathHelper.clamp(dragging.yPos, 0, height - dragging.getHeight());
                }
            }
            if (dragging.module.state) {
                ...
                dragging.xPos = MathHelper.clamp(dragging.xPos, 0, width - dragging.getWidth());
                dragging.yPos = MathHelper.clamp(dragging.yPos, 0, height - dragging.getHeight());
                //Fonts.pix.drawCenteredStringWithShadow(dragging.name, dragging.xPos + dragging.getWidth() / 2, dragging.yPos + dragging.getHeight() / 2, -1);
            }
        ...
        if (Main.m.getModule(PasswordHider.class).state) {
            RenderUtil.blur(() -> {
                if ((inputField.getText().startsWith("/l") && inputField.getText().split(" ")[0].length() <= 6) || (inputField.getText().startsWith("/reg") && inputField.getText().split(" ")[0].length() <= 9)) {

                    RenderUtil.drawRect(fontRenderer.getStringWidth(inputField.getText().split(" ")[0]) + 6, this.height - 14, fontRenderer.getStringWidth(inputField.getText().split(" ")[0]) + 6 + fontRenderer.getStringWidth(inputField.getText().substring(inputField.getText().split(" ")[0].length())), this.height - 2, Integer.MIN_VALUE);
                }
            }, 10);
        }
