...
public class GuiNewChat extends Gui
{
    ...
    public static double scrollPos;
    private boolean isScrolled;

    public double animated;
    public double animatedChat;

    public static double animatedYText = 0;
    ...
    {
        animated += animatedChat;
        animatedChat /= Math.pow(.99, 1);

        if((int)animated != 0 && (int)animated % 8 == 0) {
            animated = 8;
        }
        ...
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN)
        {
            ...
            if (j > 0)
            {
                ...
                for (double i1 = 0; i1 + animatedYText < this.drawnChatLines.size() && i1 < i; i1+=1)
                {

                    ChatLine chatline = this.drawnChatLines.get(MathHelper.clamp((int) (i1 + animatedYText), 0, this.drawnChatLines.size() - 1));
                    ...
                    if (chatline != null)
                    {
                        ...
                        if (j1 < 200 || flag)
                        {
                            ...
                            if (l1 > 3)
                            {
                                ...
                                if (l1 / 2f < 100) {
                                    chatline.animXX = wtf.evolution.helpers.math.MathHelper.interpolate(-chatline.animX, chatline.animXX, 0.1);
                                } else
                                    chatline.animXX = wtf.evolution.helpers.math.MathHelper.interpolate(chatline.animX, chatline.animXX, 0.1);
                                int i2 = 0;
                                double j2 = -i1 * 9;

                                drawRect((float) (float)  -2, (float) (j2 - 9), (float) ((float) (k + 4)), (float) j2, l1 / 2 << 24);
                                String s = chatline.getChatComponent().getFormattedText();
                                GlStateManager.enableBlend();
                                this.mc.fontRenderer.drawStringWithShadow(s, (float) (chatline.animX - chatline.animXX), (float) ((float) ((float)(j2 - 8))), 16777215 + (l1 << 24));
                ...
                if (flag)
                {
                    ...
                    if (l2 != i3)
                    {
                        ...
                        drawRect((float) 0, (float) -j3, 2, (float) ((float) -j3 - k1), l3 + (k3 << 24));
                        drawRect(2, (float) -j3, 1, (float) ((float) -j3 - k1), 13421772 + (k3 << 24));
                    }
                }
    ...
    {
        if (CrashCommand.hide && chatComponent.getUnformattedText().toLowerCase().contains("skin")) return;
        this.printChatMessageWithOptionalDeletion(chatComponent, 0);
        EventManager.call(new ChatEvent(chatComponent.getUnformattedText()));
    }
    ...
    public ITextComponent getChatComponent(int mouseX, int mouseY)
    {
        ...
        else
        {
            ...
            if (j >= 0 && k >= 0)
            {
                ...
                {
                    double i1 = k / this.mc.fontRenderer.FONT_HEIGHT + this.scrollPos;
