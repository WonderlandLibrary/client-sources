...
public class EntityRenderer implements IResourceManagerReloadListener
{
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (!Main.m.getModule(CameraClip.class).state) {
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    if (Main.m.getModule(WorldColor.class).state) {
                        int j = 255;
                        int k = new Color(((WorldColor) Main.m.getModule(WorldColor.class)).color.get()).getRed();
                        int l = new Color(((WorldColor) Main.m.getModule(WorldColor.class)).color.get()).getGreen();
                        int i1 = new Color(((WorldColor) Main.m.getModule(WorldColor.class)).color.get()).getBlue();
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        if (Main.m.getModule(FogColor.class).state) {
            Color rgb = new Color(((FogColor) Main.m.getModule(FogColor.class)).fogColor.color);
