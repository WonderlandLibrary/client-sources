...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                EnchantmentColor c = (EnchantmentColor) Main.m.getModule(EnchantmentColor.class);
                if (c.state) {
                    this.renderModel(model, c.enchantColor.get());
                } else {
                ...
                if (c.state) {
                    Color cc = new Color(c.enchantColor.get());
                    GlStateManager.color(cc.getRed() / 255F,
                            cc.getGreen() / 255F,
                            cc.getBlue() / 255F,
                            cc.getAlpha() / 255F);
                } else {
     ...
     */
    private void draw(BufferBuilder renderer, double x, double y, int width, int height, int red, int green, int blue, int alpha)
    {
