...
public class ItemRenderer
{
    ...
    {
        EventManager.call(new EventRenderHand(p_187456_3_));
    ...
    {
        EventManager.call(new EventRenderHand(hand));
    ...
    public void renderOverlays(float partialTicks)
    {
        ...
        if (this.mc.player.isEntityInsideOpaqueBlock())
        {
            ...
            if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE)
            {
                ...
                {
                    if (!Main.m.getModule(NoRender.class).state && !((NoRender) Main.m.getModule(NoRender.class)).listSetting.selected.contains("Block")) {
    ...
    {
        if (Main.m.getModule(NoRender.class).state && ((NoRender)Main.m.getModule(NoRender.class)).listSetting.selected.contains("Fire")) return;
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
