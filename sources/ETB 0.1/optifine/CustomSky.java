...
public class CustomSky
{
    ...
    while (worldCount < wsls.length)
    {
      ...
      while (sls < 1000)
      {
        ...
        try
        {
          ...
          if (sl.isValid(path))
          {
            ResourceLocation locSource = new ResourceLocation(source);
  ...
  public static void renderSky(World world, net.minecraft.client.renderer.texture.TextureManager re, float celestialAngle, float rainBrightness)
  {
    if (worldSkyLayers != null)
    {
      if (getGameSettingsrenderDistanceChunks >= 8)
      {
        int dimId = provider.getDimensionId();
