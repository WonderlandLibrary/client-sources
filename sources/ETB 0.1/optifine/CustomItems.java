...
public class CustomItems
{
  ...
  public static void updateIcons(TextureMap textureMap) { itemProperties = null;
  ...
  private static void updateIcons(TextureMap textureMap, IResourcePack rp)
  {
    ...
    if (mapAutoProperties.size() > 0)
    {
      ...
      String[] enchantmentList = (String[])itemList.toArray(new String[itemList.size()]);
    ...
    for (int var17 = 0; var17 < itemProperties.length; var17++)
    {
      CustomItemProperties[] var18 = itemProperties[var17];
    ...
    {
      CustomItemProperties[] var18 = enchantmentProperties[var17];
  ...
  private static Comparator getPropertiesComparator()
  {
    Comparator comp = new Comparator()
    {
      public int compare(Object o1, Object o2)
      {
        ...
        return !basePath.equals(basePath) ? basePath.compareTo(basePath) : weight != weight ? weight - weight : layer != layer ? layer - layer : name.compareTo(name);
      }
    };
    return comp;
  }
  ...
  private static Map getMapPotionIds()
  {
    ...
    {
      mapPotionIds = new java.util.LinkedHashMap();
  ...
  private static ResourceLocation getCustomArmorLocation(ItemStack itemStack, int layer, String overlay)
  {
    ...
    ResourceLocation loc = (ResourceLocation)mapTextureLocations.get(key);
    return loc;
  }
    ...
    if ((itemId >= 0) && (itemId < itemProperties.length))
    {
      ...
      if (cips != null)
      {
        for (int i = 0; i < cips.length; i++)
        {
          ...
          if ((type == type) && (matchesProperties(cip, itemStack, null)))
          {
            return cip;
          }
        }
  ...
  private static boolean matchesProperties(CustomItemProperties cip, ItemStack itemStack, int[][] enchantmentIdLevels)
  {
    ...
    if (damage != null)
    {
      ...
      if (damagePercent)
      {
        ...
        idLevels = (int)(idLevels * 100 / nbt);
      }
      
      if (!damage.isInRange(idLevels))
      {
        return false;
      }
    }
    
    if ((stackSize != null) && (!stackSize.isInRange(stackSize)))
    {
      return false;
    }
    ...
    if (enchantmentIds != null)
    {
      ...
      for (int i = 0; i < var8.length; i++)
      {
        ...
        if (enchantmentIds.isInRange(ntv))
        {
          var9 = true;
          break;
        }
    ...
    if (enchantmentLevels != null)
    {
      ...
      for (int i = 0; i < var8.length; i++)
      {
        ...
        if (enchantmentLevels.isInRange(ntv))
        {
          var9 = true;
          break;
        }
    ...
    if (nbtTagValues != null)
    {
      ...
      {
        NbtTagValue var11 = nbtTagValues[i];
      ...
      if ((id >= 0) && (id < enchantmentProperties.length))
      {
        ...
        if (cips != null)
        {
          for (int p = 0; p < cips.length; p++)
          {
            ...
            if ((layersRendered.add(Integer.valueOf(id))) && (matchesProperties(cip, itemStack, idLevels)) && (textureLocation != null))
            {
              ...
              float offset = speed * (float)(Minecraft.getSystemTime() % 3000L) / 3000.0F / 8.0F;
