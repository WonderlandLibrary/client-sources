...
public final class Display
{
    ...
    synchronized (GlobalLock.lock) {
      ...
      try
      {
        ...
        try {
          createWindow();
          try {
            context = new ContextGL(peer_info, attribs, shared_drawable != null ? ((DrawableGL)shared_drawable).getContext() : null);
            try {
              makeCurrentAndSetSwapInterval();
              initContext();
            } catch (LWJGLException e) {
  ...
  static boolean getPrivilegedBoolean(String property_name)
  {
    ((Boolean)AccessController.doPrivileged(new PrivilegedAction() {
      public Boolean run() {
        return Boolean.valueOf(Boolean.getBoolean(val$property_name));
      }
    })).booleanValue();
  }
  
  static String getPrivilegedString(String property_name)
  {
    (String)AccessController.doPrivileged(new PrivilegedAction() {
      public String run() {
        return System.getProperty(val$property_name);
      }
    });
  }
