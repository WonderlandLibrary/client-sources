...
public final class RenderTexture
{
  ...
  public RenderTexture(boolean useRGB, boolean useRGBA, boolean useDepth, boolean isRectangle, int target, int mipmaps)
  {
    ...
    if (useRGB) {
      pixelFormatCaps.put(isRectangle ? 8352 : 8304);
      pixelFormatCaps.put(1);
      
      pBufferAttribs.put(8306);
      pBufferAttribs.put(8309);
    } else if (useRGBA) {
      pixelFormatCaps.put(isRectangle ? 8353 : 8305);
      pixelFormatCaps.put(1);
      
      pBufferAttribs.put(8306);
      pBufferAttribs.put(8310);
    }
    
    if (useDepth) {
      pixelFormatCaps.put(isRectangle ? 8356 : 8355);
      pixelFormatCaps.put(1);
      
      pBufferAttribs.put(8357);
      pBufferAttribs.put(8358);
    }
    
    pBufferAttribs.put(8307);
    pBufferAttribs.put(target);
    
    if (mipmaps != 0) {
      pBufferAttribs.put(8308);
