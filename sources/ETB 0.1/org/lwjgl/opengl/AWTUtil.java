...
final class AWTUtil
{
    ...
    try {
      (Robot)AccessController.doPrivileged(new PrivilegedExceptionAction() {
        public Robot run() throws Exception {
          return new Robot(val$component.getGraphicsConfiguration().getDevice());
        }
      });
  ...
  public static Point getCursorPosition(Component component)
  {
    try
    {
      ...
      if (pointer_location != null) {
        ...
        pointer_location.translate(-x, -y);
        pointer_location.move(x, transformY(component, y));
  ...
  public static void setCursorPosition(Component component, Robot robot, int x, int y) {
    if (robot != null) {
      try {
        Point location = component.getLocationOnScreen();
        int transformed_x = x + x;
        int transformed_y = y + transformY(component, y);
