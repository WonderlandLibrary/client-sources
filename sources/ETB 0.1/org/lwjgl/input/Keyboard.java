...
public class Keyboard
{
  ...
  private static final String[] keyName = new String['Ā'];
  ...
  private static KeyEvent current_event = new KeyEvent(null);
  

  private static KeyEvent tmp_event = new KeyEvent(null);
  ...
  public static int getNumKeyboardEvents()
  {
    synchronized (OpenGLPackageAccess.global_lock) {
      ...
      int num_events = 0;
      while ((readNext(tmp_event)) && ((!tmp_eventrepeat) || (repeat_enabled)))
        num_events++;
  ...
  public static boolean next()
  {
    synchronized (OpenGLPackageAccess.global_lock) {
      ...
      boolean result;
      while (((result = readNext(current_event))) && (current_eventrepeat) && (!repeat_enabled)) {}
  ...
  private static boolean readNext(KeyEvent event) {
    if (readBuffer.hasRemaining()) {
      key = (readBuffer.getInt() & 0xFF);
      state = (readBuffer.get() != 0);
      character = readBuffer.getInt();
      nanos = readBuffer.getLong();
      repeat = (readBuffer.get() == 1);
      return true;
    }
    return false;
  }
  ...
  {
    synchronized (OpenGLPackageAccess.global_lock) {
      return (char)current_eventcharacter;
    }
  }
