...
public final class MemoryUtil
{
  ...
  private static class CharSequenceNT implements CharSequence
  {
    ...
    {
      return index == source.length() ? '\000' : source.charAt(index);
    }
  ...
  private static Accessor loadAccessor(String className)
    throws Exception { return (Accessor)Class.forName(className).newInstance(); }
  
  static abstract interface Accessor { public abstract long getAddress(Buffer paramBuffer);
  }
  
  private static class AccessorJNI implements MemoryUtil.Accessor { private AccessorJNI() {}
    
    public long getAddress(Buffer buffer) { return BufferUtils.getBufferAddress(buffer); }
  }
