...
class Poly
{
  ...
  static void fromBytes(short[] r, byte[] a)
  {
    ...
    {
      int j = 7 * i;
      int a0 = a[(j + 0)] & 0xFF;int a1 = a[(j + 1)] & 0xFF;int a2 = a[(j + 2)] & 0xFF;int a3 = a[(j + 3)] & 0xFF;
      int a4 = a[(j + 4)] & 0xFF;int a5 = a[(j + 5)] & 0xFF;int a6 = a[(j + 6)] & 0xFF;
      
      int k = 4 * i;
      r[(k + 0)] = ((short)(a0 | (a1 & 0x3F) << 8));
      r[(k + 1)] = ((short)(a1 >>> 6 | a2 << 2 | (a3 & 0xF) << 10));
      r[(k + 2)] = ((short)(a3 >>> 4 | a4 << 4 | (a5 & 0x3) << 12));
  ...
  static void getNoise(short[] r, byte[] seed, byte nonce)
  {
    ...
    for (int i = 0; i < 1024; i++)
    {
      int t = org.spongycastle.util.Pack.bigEndianToInt(buf, i * 4);
  ...
  static void pointWise(short[] x, short[] y, short[] z)
  {
    for (int i = 0; i < 1024; i++)
    {
      int xi = x[i] & 0xFFFF;int yi = y[i] & 0xFFFF;
