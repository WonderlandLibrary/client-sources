...
class Tnaf
{
  ...
  {
    BigInteger s1 = u.multiply(u);
    

    BigInteger s2 = u.multiply(v);
    

    BigInteger s3 = v.multiply(v).shiftLeft(1);
    BigInteger norm;
    if (mu == 1)
    {
  ...
  public static ZTauElement round(SimpleBigDecimal lambda0, SimpleBigDecimal lambda1, byte mu)
  {
    ...
    SimpleBigDecimal check2; if (mu == 1)
    {
      SimpleBigDecimal check1 = eta0.subtract(threeEta1);
  ...
  public static ZTauElement partModReduction(BigInteger k, int m, byte a, BigInteger[] s, byte mu, byte c)
  {
    ...
    BigInteger r0 = k.subtract(d0.multiply(u)).subtract(
      BigInteger.valueOf(2L).multiply(s[1]).multiply(v));
    

    BigInteger r1 = s[1].multiply(u).subtract(s[0].multiply(v));
