  ...
  extends SerpentEngineBase
{
  ...
  protected void encryptBlock(byte[] input, int inOff, byte[] output, int outOff)
  {
    ...
    Pack.intToBigEndian(wKey[''] ^ X3, output, outOff);
    Pack.intToBigEndian(wKey[''] ^ X2, output, outOff + 4);
    Pack.intToBigEndian(wKey[''] ^ X1, output, outOff + 8);
    Pack.intToBigEndian(wKey[''] ^ X0, output, outOff + 12);
  }
  ...
  {
    X3 = (wKey[''] ^ Pack.bigEndianToInt(input, inOff));
    X2 = (wKey[''] ^ Pack.bigEndianToInt(input, inOff + 4));
    X1 = (wKey[''] ^ Pack.bigEndianToInt(input, inOff + 8));
    X0 = (wKey[''] ^ Pack.bigEndianToInt(input, inOff + 12));
