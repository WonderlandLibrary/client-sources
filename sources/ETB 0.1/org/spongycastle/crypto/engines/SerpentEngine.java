  ...
  extends SerpentEngineBase
{
  ...
  protected void encryptBlock(byte[] input, int inOff, byte[] output, int outOff)
  {
    ...
    Pack.intToLittleEndian(wKey[''] ^ X0, output, outOff);
    Pack.intToLittleEndian(wKey[''] ^ X1, output, outOff + 4);
    Pack.intToLittleEndian(wKey[''] ^ X2, output, outOff + 8);
    Pack.intToLittleEndian(wKey[''] ^ X3, output, outOff + 12);
  }
  ...
  {
    X0 = (wKey[''] ^ Pack.littleEndianToInt(input, inOff));
    X1 = (wKey[''] ^ Pack.littleEndianToInt(input, inOff + 4));
    X2 = (wKey[''] ^ Pack.littleEndianToInt(input, inOff + 8));
    X3 = (wKey[''] ^ Pack.littleEndianToInt(input, inOff + 12));
