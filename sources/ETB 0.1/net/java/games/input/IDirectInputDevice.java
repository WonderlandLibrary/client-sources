...
final class IDirectInputDevice
{
    ...
    throws IOException
  {
    ...
    axes_in_relative_mode = ((all_relative) && (has_axis));
    ...
    setBufferSize(32);
    acquire();
    device_state = new int[objects.size()];
  }
  ...
  private final Rumbler createPeriodicRumbler(DIDeviceObject[] axes, long[] directions, DIEffectInfo info) throws IOException {
    ...
    long effect_address = nCreatePeriodicEffect(address, info.getGUID(), 17, -1, 0, 10000, -1, 0, axis_ids, directions, 0, 0, 0, 0, 10000, 0, 0, 50000, 0);
  ...
  private final DIDeviceObject lookupObjectByGUID(int guid_id) { for (int i = 0; i < objects.size(); i++) {
  ...
  private final void acquire() throws IOException { int res = nAcquire(address);
  ...
  private final void unacquire() throws IOException { int res = nUnacquire(address);
    if ((res != 0) && (res != 1))
  ...
  private final boolean getDeviceData(DataQueue queue) throws IOException { int res = nGetDeviceData(address, 0, queue, queue.getElements(), queue.position(), queue.remaining());
    if ((res != 0) && (res != 1)) {
      if (res == -2147024868) {
        acquire();
        return false;
  ...
  private final void getDeviceState(int[] device_state) throws IOException { int res = nGetDeviceState(address, device_state);
    if (res != 0) {
      if (res == -2147024868) {
  ...
  public final String getProductName() { return product_name; }
  ...
  private final void enumEffects() throws IOException {
    int res = nEnumEffects(address, 0);
    if (res != 0)
  ...
  private final void enumObjects() throws IOException {
    int res = nEnumObjects(address, 31);
    if (res != 0)
  ...
  public final synchronized long[] getRangeProperty(int object_identifier) throws IOException { checkReleased();
  ...
  public final synchronized int getDeadzoneProperty(int object_identifier) throws IOException { checkReleased();
  ...
  public final synchronized void setCooperativeLevel(int flags) throws IOException { checkReleased();
  ...
  public final synchronized void release() { if (!released) {
      released = true;
