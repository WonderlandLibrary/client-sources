...
import yung.purity.api.events.EventChat;
import yung.purity.api.events.EventMove;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
  ...
  {
    EventUpdate e = (EventUpdate)EventBus.getDefault().call(
      new EventUpdate(getEntityBoundingBoxminY, rotationYaw, rotationPitch, onGround));
    
    if (e.isCancelled()) {
      EventBus.getDefault().call(new EventUpdate());
      return;
    }
    ...
    if (func_175160_A())
    {
      ...
      double var9 = e.getYaw() - field_175164_bL;
      double var11 = e.getPitch() - field_175165_bM;
      ...
      if (ridingEntity == null)
      {
        if ((var13) && (var14))
        {
          sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(posX, e.getY(), posZ, e.getYaw(), e.getPitch(), e.isOnGround()));
        }
        else if (var13)
        {
          sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(posX, e.getY(), posZ, e.isOnGround()));
        }
        else if (var14)
        {
          sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(e.getYaw(), e.getPitch(), e.isOnGround()));
        }
        else
        {
      ...
      {
        sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(motionX, -999.0D, motionZ, e.getYaw(), e.getPitch(), e.isOnGround()));
        var13 = false;
      }
  ...
  {
    EventMove e = (EventMove)EventBus.getDefault().call(new EventMove(x, y, z));
  ...
  {
    EventChat e = new EventChat(p_71165_1_);
    EventBus.getDefault().call(e);
    if (!e.isCancelled()) {
  ...
  public void onLivingUpdate()
  {
    ...
    if ((isUsingItem()) && (!isRiding()) && (!Client.instance.getModuleManager().getModuleByClass(NoSlow.class).isEnabled()))
    {
