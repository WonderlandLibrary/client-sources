...
public class AtaniServerListEntryNormal implements AtaniGuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        FontStorage.getInstance().findFont("Roboto", 19).drawString(this.field_148301_e.serverName, x + 32 + 3, y + 1, 16777215);
        List<String> list = FontStorage.getInstance().findFont("Roboto", 19).listFormattedStringToWidth(this.field_148301_e.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(list.size(), 2); ++i)
        {
            FontStorage.getInstance().findFont("Roboto", 19).drawString((String)list.get(i), x + 32 + 3, y + 12 + FontStorage.getInstance().findFont("Roboto", 19).FONT_HEIGHT * i, 8421504);
        }
