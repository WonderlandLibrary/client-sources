...
import dev.vertic.util.animation.impl.EaseInOutQuad;
import dev.vertic.util.animation.impl.SmoothStepAnimation;
import dev.vertic.util.math.Animate;
...
public class GuiTextField extends Gui implements Utils {
    ...
    /* Cursor's blinking animation (opacity value) */
    int cursorOP = 0;
    EaseBackIn cursorOpacityThing = new EaseBackIn(400, 1, 2f);
    ...
    public void drawAltManagerBox(double cornerRadius, Color backgroundColor, Color outlineColor, int cursorOpacity) {
        DrawUtil.roundedRect(xPosition, yPosition, width, height, cornerRadius, backgroundColor);

        if (isFocused) {
            DrawUtil.roundedOutline(xPosition, yPosition, width, height, cornerRadius, 1, outlineColor);
            DrawUtil.rect(xPosition + 10 + font24.getStringWidth(text) + 1, yPosition + (font24.getHeight() / 1), 1, font24.getHeight(), true, new Color(255, 255, 255, cursorOpacity));
        } else if (text.isEmpty()) {
            font24.drawCenteredStringWithShadow("Username...", xPosition + (width / 2), yPosition + (height / 2) - (font24.getHeight() / 2), new Color(200, 200, 200).getRGB());
        }
        font24.drawStringWithShadow(cursorOpacity + "", 5, 5, -1);
        font24.drawStringWithShadow(text, xPosition + 10, yPosition + (height / 2) - (font24.getHeight() / 2), -1);
