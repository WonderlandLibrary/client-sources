...
import Reality.Realii.event.events.world.EventMoveFix;
...
import Reality.Realii.mods.modules.movement.SafeWalk;
...
public abstract class Entity implements ICommandSender
{
    ...
    public static int ticksSinceVelocity, ticksSincePlayerVelocity;
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer || ModuleManager.getModuleByClass(SafeWalk.class).isEnabled()  || ModuleManager.getModuleByClass(Scaffold.class).isEnabled() &&  Scaffold.SafeWalk.getValue().equals("On") && this instanceof EntityPlayer;;
    ...
    {
        EventMoveFix nigga = new EventMoveFix(strafe,forward,friction,this.rotationYaw,this.rotationPitch);
        if(this instanceof EntityPlayerSP) {
            EventBus.getInstance().call(nigga);
        }
        if(nigga.isCancelled()) {
            return;
        }
        strafe = nigga.getStrafe();
        forward = nigga.getForward();
        friction = nigga.getFriction();
        ...
        if (f >= 1.0E-4F)
        {
            ...
            strafe = strafe * f;
            forward = forward * f;
            float f1 = MathHelper.sin(nigga.getYaw() * (float)Math.PI / 180.0F);
            float f2 = MathHelper.cos(nigga.getYaw() * (float)Math.PI / 180.0F);
