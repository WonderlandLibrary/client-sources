...
public class GuiIngame extends Gui {
     ...
     */
    public static long healthUpdateCounter = 0L;
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (Client.instance.getModuleManager().getModuleByClass(HUD.class).isEnabled() && ((HUD) Client.instance.getModuleManager().getModuleByClass(HUD.class)).mod.getValue().equals("OverWatch")) {
            ...
            float ff3 = mc.thePlayer.prevCameraPitch + (mc.thePlayer.cameraPitch - mc.thePlayer.prevCameraPitch) * partialTicks;
            ...
            GlStateManager.translate((moveX - mouseX) / 4f, (moveY - mouseY) / 4f, 0);

            GlStateManager.translate(Math.abs(MathHelper.cos(ff1 * (float) Math.PI - 0.2F) * ff2) * 13.0F, Math.abs(MathHelper.cos(ff1 * (float) Math.PI - 0.2F) * ff2) * 10.0F, 0.0F);
//        GlStateManager.rotate(MathHelper.sin(ff1 * (float) Math.PI) * ff2 * 1.0F, 0.0F, 0.0F, 1.0F);
//        GlStateManager.rotate(Math.abs(MathHelper.cos(ff1 * (float) Math.PI - 0.2F) * ff2) * 2.0F, 1.0F, 0.0F, 0.0F);
//        GlStateManager.rotate(ff3, 1.0F, 0.0F, 0.0F);
            if (moveX != mouseX) {
                moveX += (mouseX - moveX) / 40f;
            }
            if (moveY != mouseY) {
                moveY += (mouseY - moveY) / 40f;
            }
//        if (moveY != mouseY) {
//            moveY += (mouseY - moveX) / 20f;
//        }
        ...
        if (this.field_175195_w > 0) {
            ...
            if (mc.isSingleplayer()) {
                ...
                Gui.drawRect(sr.getScaledWidth() / 2 - mc.fontRendererObj.getStringWidth("SinglePlayer isn't supported. Some modules don't work correctly!") / 2 - 2, 40, sr.getScaledWidth() / 2 + mc.fontRendererObj.getStringWidth("SinglePlayer isn't supported. Some modules don't work correctly!") / 2 + 2, 42 + mc.fontRendererObj.FONT_HEIGHT, new Color(0, 0, 0, 200).getRGB());
                mc.fontRendererObj.drawCenteredString("SinglePlayer isn't supported. Some modules don't work correctly!", sr.getScaledWidth() / 2, 40, new Color(255, 255, 0).getRGB());
            }
        }
        ...
        for (Object score1 : arraylist1) {
            ...
            if (k == arraylist1.size()) {
            	  ...
            	  int i45 = 0;
             	  ...
             	  Color color3 = Render2.interpolateColorsBackAndForth(7, 3 + (i45 * 20), colors.getFirst(), colors.getSecond(), false);
                String s3 = "Reality.nigger";
