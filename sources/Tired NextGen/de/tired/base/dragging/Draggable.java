...
import de.tired.util.math.vector.Vec3;
...
@Getter
@Setter
public class Draggable implements IHook {
    ...
    public boolean resizeCompatible, didDragPreviously;
    ...
    private Points touchPoints = null;

    public Draggable(String name, final Module module, float startX, float startY, boolean resizeCompatible) {
        this.startX = startX;
        this.startY = startY;
        this.module = module;
        this.name = name;
    ...
    public void onDragAndDraw(final int mouseX, final int mouseY) {
        ...
        if (alphaAnimation.getValue() > 0)
            ShaderManager.shaderBy(RoundedRectOutlineShader.class).drawRound(xPosition, yPosition, objectWidth, objectHeight + 4, 3, 2, new Color(255, 255, 255, (int) alphaAnimation.getValue()));
        if (alphaAnimationText.getValue() > 62 && !dragging) {
            FontManager.raleWay20.drawString(name, xPosition + 4, yPosition + objectHeight + 8, new Color(255, 255, 255, (int) alphaAnimationText.getValue()).getRGB());
        }
        ...
        for (Map.Entry<String, Draggable> entry : DragHandler.draggableHashMap.entrySet()) {
            ...
            if (dragging && draggable.module.isState() && draggable != this) {
                ...
                if (diffLeft < maxDifference) { //Left from rect
                    ...
                    RenderUtil.instance.line(myVector, theirVector, Color.WHITE.getRGB());
                }
    ...
    private void moveSelectedRenderBy(int offsetX, int offsetY) {
        ScreenPosition pos = screenPosition;
    ...
    enum Points {
        RIGHT_OVER, LEFT_OVER, RIGHT_UNDER, LEFT_UNDER, LEFT_MIDDLE, RIGHT_MIDDLE
    }
