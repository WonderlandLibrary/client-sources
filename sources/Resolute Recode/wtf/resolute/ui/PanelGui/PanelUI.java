...
public class PanelUI extends Screen implements IMinecraft {

    private final List<Panel> panels = new ArrayList<>();
    @Getter
    private static Animation animation = new Animation();

    public PanelUI(ITextComponent titleIn) {
        super(titleIn);
        for (Categories category : Categories.values()) {
            if (category == Categories.Theme) continue;
            panels.add(new Panel(category));
        }
        panels.add(new PanelTheme(Categories.Theme));
    }
