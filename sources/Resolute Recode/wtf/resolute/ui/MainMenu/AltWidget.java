...
public class AltWidget implements IMinecraft {
    ...
    public void render(MatrixStack stack, int x, int y) {
        ...
        DisplayUtils.drawRoundedRect(this.x, this.y, width, height, new Vector4f(4, 4, 4, 4), DisplayUtils.reAlphaInt(new Color(33, 32, 34).getRGB(), 210));
        Scissor.push();
        Scissor.setFromComponentCoordinates(this.x, this.y, width - 16, height);
        Fonts.sfMedium.drawText(stack, mc.session.getUsername(), this.x + 6, this.y + 6, -1, 7);
        Scissor.unset();
        Scissor.pop();
        ...
        if (open) {
            ...
            for (Alt alt : alts) {
                ...
                Fonts.sfMedium.drawText(stack, alt.name, this.x + 10, this.y + 26 + i * 17 + 4 + scroll, -1, 6);
                i++;
            }
            ...
            Fonts.sfMedium.drawText(stack, textToDraw + (typing ? (System.currentTimeMillis() % 1000 > 500 ? "_" : "") : ""),
            ...
            DisplayUtils.drawRoundedRect(this.x + 5 + 2, this.y + 26 + i * 17 + 2 + scroll,
                    Fonts.sfMedium.getWidth(textToDraw + (typing ? (System.currentTimeMillis() % 1000 > 500 ? "_" : "") : ""),
                            6) + 7, 15 - 4, 2, ColorUtils.rgba(30, 30, 36, 64));
            Fonts.sfMedium.drawText(stack, "+", this.x + width - 18,
    ...
    public void onChar(char typed) {
        if (typing) {
            if (Fonts.sfMedium.getWidth(altName, 6f) < 145 - 50) {
                altName += typed;
            }
        }
    }
