...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\resolute\\configs"), name + ".cfg");
    }

    public void loadConfig(JsonObject jsonObject, String configuration) {
        if (jsonObject == null) {
            return;
        }
    ...
    private void loadStyleSettings(JsonObject stylesObject) {
        for (Style style : Managed.STYLE_MANAGER.styles) {
            if (stylesObject.has(style.name)) {
                ...
                if (selected) {
                    Managed.STYLE_MANAGER.setCurrentStyle(style);
                }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        ResoluteInfo.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        ResoluteInfo.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        ...
        for (wtf.resolute.ui.styled.Style style : Managed.STYLE_MANAGER.styles) {
            ...
            styleObject.addProperty("selected", Managed.STYLE_MANAGER.getCurrentStyle() == style);
