...
public class Brain<E extends LivingEntity>
{
    ...
    public static <E extends LivingEntity> Codec<Brain<E>> getBrainCodec(final Collection <? extends MemoryModuleType<? >> memoryTypes, final Collection <? extends SensorType <? extends Sensor <? super E >>> sensorTypes)
    {
        ...
        mutableobject.setValue((new MapCodec<Brain<E>>()
        {
            ...
            {
                MutableObject < DataResult < Builder < MemoryCodec<? >>> > mutableobject1 = new MutableObject<>(DataResult.success(ImmutableList.builder()));
                p_decode_2_.entries().forEach((inputPair) ->
                {
                    ...
                    DataResult <? extends MemoryCodec<? >> dataresult1 = dataresult.flatMap((memoryType) -> {
                ...
                ImmutableList < MemoryCodec<? >> immutablelist = mutableobject1.getValue().resultOrPartial(Brain.LOGGER::error).map(Builder::build).orElseGet(ImmutableList::of);
            ...
            private <T, U> DataResult<MemoryCodec<U>> decodeMemory(MemoryModuleType<U> memoryType, DynamicOps<T> ops, T input)
            {
                ...
                {
                    return new MemoryCodec<>(memoryType, Optional.of(memory));
                });
            }
    ...
    private Stream < MemoryCodec<? >> createMemoryCodecs()
    {
        ...
        {
            return MemoryCodec.createCodec(entry.getKey(), entry.getValue());
        });
    }
    ...
    static final class MemoryCodec<U>
    {
        ...
        {
            return new MemoryCodec<>(memoryType, (Optional <? extends Memory<U >>) memory);
        }
