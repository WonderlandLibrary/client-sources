...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!ResoluteInfo.getInstance().playerOnServer) {
                ResoluteInfo.getInstance().playerOnServer = true;
            }

            ResoluteInfo.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        ResoluteInfo.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            ResoluteInfo.getInstance().getEventBus().post(eventMotion);
        ...
        ResoluteInfo.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = ResoluteInfo.getInstance().getCommandDispatcher();
    ...
    public void closeScreen() {
        ...
        ResoluteInfo.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    public void setSprinting(boolean sprinting) {
        ResoluteInfo.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            ResoluteInfo.getInstance().getEventBus().post(noSlowEvent);
