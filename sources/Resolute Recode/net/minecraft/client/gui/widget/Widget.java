...
public abstract class Widget extends AbstractGui implements IRenderable, IGuiEventListener {
    ...
    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        DisplayUtils.drawRoundedRect(this.x, this.y, this.width, this.height, 3,DisplayUtils.reAlphaInt(new Color(33, 32, 34).getRGB(), 210));

        this.animation = AnimationMath.lerp(this.animation, isHovered() ? 1.0F : 0.0F, 10.0F);
        // Fonts.sfMedium.drawText(matrixStack, getMessage().getString(), (this.x + this.width / 2.0F), (this.y + this.height / 2.0F - Fonts.sfMedium.getHeight(7) / 2.0F + 2.0F), -1,7);
        ...
        if (selfDestruct.unhooked) { //если анхук включен рендерим обычные кнопки
