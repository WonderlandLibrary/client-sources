...
import wtf.resolute.ui.MainMenu.LastConnectionComponent;
import wtf.resolute.ui.MainMenu.MainScreen;
...
public class IngameMenuScreen extends Screen {
    ...
    private void addButtons() {
        ...
        {
            Util.getOSType().openURI("https://discord.gg/P2nVH6z8JQ");
        }));
        boolean isLocalHost = this.minecraft.isSingleplayer();

        this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 96 + -16, 98, 20, isLocalHost ? new TranslationTextComponent("menu.shareToLan") : new StringTextComponent("Reconnect"), (button2) -> {
            if (isLocalHost) {
                this.minecraft.displayGuiScreen(new ShareToLanScreen(this));
            } else {
                this.minecraft.world.sendQuittingDisconnectingPacket();
                this.minecraft.displayGuiScreen(new ConnectingScreen(new MainScreen(), this.minecraft, LastConnectionComponent.ip, LastConnectionComponent.port));
            }
        }));
        ...
        this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 72 + -16, 98, 20, new TranslationTextComponent("menu.reportBugs"), (button2) ->
        {
            this.minecraft.displayGuiScreen(new ConfirmOpenLinkScreen((open) -> {
                if (open) {
                    Util.getOSType().openURI("https://discord.gg/ySd5qVYcfs");
                }

                this.minecraft.displayGuiScreen(this);
            }, "https://discord.gg/ySd5qVYcfs", true));
        }));
