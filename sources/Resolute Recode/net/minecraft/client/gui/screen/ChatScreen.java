...
public class ChatScreen extends Screen {
    ...
    protected void init() {
        ...
        if (!selfDestruct.unhooked) {
            this.addButton(button = new Button(mainWindow.getScaledWidth() - 152, mainWindow.getScaledHeight() - 35, 150, 20, new StringTextComponent("Скрыть пароль: " + hide), (button) ->
            {
                hide = !hide;
            }));
        }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (button != null) {
            ...
            PANIC selfDestruct = functionRegistry.getSelfDestruct();
            if (!selfDestruct.unhooked) {
                button.setMessage(new StringTextComponent("Скрыть пароль: " + hide));
            }
        }
