...
public class ChestScreen extends ContainerScreen<ChestContainer> implements IHasContainer<ChestContainer>, IMinecraft {
    ...
    protected void init() {
        ...
        if (!selfDestruct.unhooked && ResoluteInfo.userData.getUid() < 2) {
            if (title.getString().contains("Аукционы") || title.getString().contains("Поиск")) {
                this.addButton(button = new Button(width / 2 - 50, height / 2 - 150, 100, 20, new StringTextComponent("AutoBuy: " + ResoluteInfo.getInstance().getActivationLogic().getCurrentState()), (button) -> {
                    ResoluteInfo.getInstance().getActivationLogic().toggleState();
    ...
    public void closeScreen() {
        if (title.getString().contains("Аукционы") || title.getString().contains("Поиск") && ResoluteInfo.userData.getUid() < 2) {
            ResoluteInfo.getInstance().getActivationLogic().setCurrentState(ActivationLogic.State.INACTIVE);
        }
        super.closeScreen();
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (button != null && ResoluteInfo.userData.getUid() < 2)
            button.setMessage(new StringTextComponent("AutoBuy: " + ResoluteInfo.getInstance().getActivationLogic().getCurrentState()));
