...
public class Main {
    ...
    public static void main(String[] p_main_0_) {
        ...
        OptionSpec<String> optionspec10 = optionparser.accepts("username").withRequiredArg().defaultsTo("fgst_" + Util.milliTime() % 1000L);
        ...
        Thread thread = new Thread("Client Shutdown Thread") {
            public void run() {
                if (ResoluteInfo.getInstance().playerOnServer) {
                    try {
                        ResoluteInfo.getInstance().shutDown();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    ResoluteInfo.getInstance().playerOnServer = false;
                }
