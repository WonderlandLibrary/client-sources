...
public class HeadLayer<T extends LivingEntity, M extends EntityModel<T> & IHasHead> extends LayerRenderer<T, M>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        boolean ChinaHat = ResoluteInfo.getInstance().getFunctionRegistry().getChinaHat().isState();
        if (ChinaHat && entitylivingbaseIn instanceof PlayerEntity player && ((player instanceof ClientPlayerEntity)) || FriendStorage.isFriend(TextFormatting.getTextWithoutFormattingCodes(player.getName().getString()))) {
            boolean babyboy = ResoluteInfo.getInstance().getFunctionRegistry().getBabyBoy().isState();
            ...
            matrixStackIn.push();
            {
                ...
                buffer.begin(GL_TRIANGLE_FAN, POSITION_COLOR);
                {
                    buffer.pos(matrixStackIn.getLast().getMatrix(), 0, .3f, 0)
                            .color(ColorUtils.setAlpha(HUD.getColor(0, 1), 150)).endVertex();
                ...
                buffer.begin(GL_LINE_LOOP, POSITION_COLOR);
                {
                    for (int i = 0, size = 360; i <= size; i++) {
                        buffer.pos(matrixStackIn.getLast().getMatrix(),
                                        -MathHelper.sin(i * PI2 / size) * width, 0,
                                        MathHelper.cos(i * PI2 / size) * width)
                                .color(ColorUtils.setAlpha(HUD.getColor(i, 1), 100)).endVertex();
                    }
                }
                depthMask(false);
                tessellator.draw();
