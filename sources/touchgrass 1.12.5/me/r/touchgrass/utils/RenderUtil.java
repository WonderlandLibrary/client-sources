...
public class RenderUtil {
    ...
    public static void drawPotionEffectsMC() {
        ...
        for (Object var4 : Minecraft.getMinecraft().thePlayer.getActivePotionEffects()) {
            ...
            boolean hotbar = touchgrass.getClient().moduleManager.getModule(Hotbar.class).isEnabled();
            boolean tephra = touchgrass.getClient().settingsManager.getSettingByName("Style").getMode().equalsIgnoreCase("Tephra");
            if(hotbar) {
        ...
        if(touchgrass.getClient().moduleManager.getModule(NameTags.class).isEnabled()) {
            if((entityLivingBaseIn.getEntityId() != -3 ) && !(entityLivingBaseIn.isInvisible()) && entityLivingBaseIn instanceof EntityPlayer)
            {
                if (touchgrass.getClient().moduleManager.getModule(NameTags.class).isEnabled()) {
                    ...
                    if (touchgrass.getClient().settingsManager.getSettingByName("Items").isEnabled() && entityLivingBaseIn != Minecraft.getMinecraft().thePlayer) {
