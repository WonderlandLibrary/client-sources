...
public class New implements Style {
    ...
    public static void loadSettings() {
        if(!lmao) {
            ...
            touchgrass.getClient().settingsManager.getSettingByName("Outline").setState(true);
            lmao = true;
        }
    }
    ...
    public static void sortModules() {
        ...
        if (lengthSort) {
            touchgrass.getClient().moduleManager.getModules().sort((m1, m2) -> Integer.compare(FontHelper.sf_l.getStringWidth(m2.getName()), FontHelper.sf_l.getStringWidth(m1.getName())));
        } else {
    ...
    public void drawArrayList() {
        ...
        for (int i = 0; i < touchgrass.getClient().moduleManager.getEnabledMods().size(); i++) {
            ...
            boolean outline = touchgrass.getClient().settingsManager.getSettingByName(arraylist, "Outline").isEnabled();

            int mheight = (count * 11 + i) + 1;

            double rectX = (sr.getScaledWidth() - mod.getSlideTTF() - 5);
            double rectX2 = rectX + FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i)).getName()) + 3.0D;
            double rectY = (1 + i * 12);
            ...
            if (outline && background) {
                ...
                if (i == touchgrass.getClient().moduleManager.getEnabledMods().size() - 1) {
                ...
                if (i != touchgrass.getClient().moduleManager.getEnabledMods().size() - 1) {
                    double modwidth = (FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i)).getName()));
                    double mwidthNext = (FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i + 1)).getName()));
                    double difference = modwidth - mwidthNext;
                    if (modwidth < mwidthNext) {
                        if ((touchgrass.getClient().moduleManager.getEnabledMods().get(i + 1)).getSlideTTF() < FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i + 1)).getName()) + 3) {
                            if ((touchgrass.getClient().moduleManager.getEnabledMods().get(i)).getSlideTTF() >= FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i)).getName()) + 3) {
                                rectX = rectX - (touchgrass.getClient().moduleManager.getEnabledMods().get(i + 1)).getSlideTTF() + FontHelper.sf_l.getStringWidth((touchgrass.getClient().moduleManager.getEnabledMods().get(i)).getName()) - difference + 2.0D;
                            }
                        }
                    }
    ...
    public void drawInfo() {
        ...
        Module info = touchgrass.getClient().moduleManager.getModule(Info.class);
        boolean lines = touchgrass.getClient().settingsManager.getSettingByName(info, "XYZ Style").getMode().equalsIgnoreCase("1-Line");

        if(touchgrass.getClient().settingsManager.getSettingByName(info, "Alignment").getMode().equalsIgnoreCase("right")) {
    ...
    public void drawPotionEffects() {
        ...
        for (Object var4 : Minecraft.getMinecraft().thePlayer.getActivePotionEffects()) {
            ...
            Info info = new Info();
            boolean infoIsRight = touchgrass.getClient().settingsManager.getSettingByName(info, "Alignment").getMode().equalsIgnoreCase("Right");
            boolean infoEnabled = touchgrass.getClient().moduleManager.getModule(Info.class).isEnabled();
    ...
    public void drawWatermark() {
        Module watermarkModule = touchgrass.getClient().moduleManager.getModule(Watermark.class);
        boolean background = touchgrass.getClient().settingsManager.getSettingByName(watermarkModule, "Background").isEnabled();
        boolean time = touchgrass.getClient().settingsManager.getSettingByName(watermarkModule, "Time").isEnabled();
        ...
        if (time) {
            ...
            String watermark = touchgrass.version + " §7(" + currenttime + ")" + (touchgrass.getClient().outdated ? " §7(Outdated)" : "");
            ...
            FontHelper.sf_l2.drawStringWithShadow("TG", 2, -1, new Color(19,109,21));
            FontHelper.sf_l.drawStringWithShadow("", 13, 12, Color.white);
            FontHelper.sf_l.drawStringWithShadow(watermark, 22, 5, Color.white);

        } else {

            String watermark_no_time = touchgrass.version + (touchgrass.getClient().outdated ? " §7(Outdated)" : "");
