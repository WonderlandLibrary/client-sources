...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
    	if(mc.thePlayer != null && mc.theWorld != null) this.isInLiquid = BlockUtils.isLiquidBlock(mc.theWorld.getBlockState(new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ)).getBlock());
    ...
    public void sendChatMessage(String p_71165_1_)
    {
    	...
    	if(event.isCancelled())
    		return;
    	this.sendQueue.addToSendQueue(new C01PacketChatMessage(event.getMessage() + (Watermark.Watermark ? " " + Watermark.watermarkText : "")));
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !NoSlowdown.noSlowdown)
        {
        ...
        if (NoSlowdown.noSlowdown) {
        	if (!this.isSprinting() && this.movementInput.moveForward >= var3 && var5 && this.mc.gameSettings.keyBindSprint.getIsKeyPressed())
            {
                this.setSprinting(true);
            }
        } else {
