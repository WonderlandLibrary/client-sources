...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback
{
    ...
    public void initGui()
    {
    	if(TabGUI.openTabGUI) {
	    	if(mc.session.getUsername().length() > 16) {
	    		mc.displayGuiScreen(new GuiError(new GuiAltManager(new GuiMultiplayer(new MainMenu())), "Your username is above 16 characters!", "Change"));
	    		return;
	    	}
	    	else if(mc.session.getUsername().length() < 3) {
	    		mc.displayGuiScreen(new GuiError(new GuiAltManager(new GuiMultiplayer(new MainMenu())), "Your username is under 3 characters!", "Change"));
	    		return;
	    	}
	    	for(int i=0;i<mc.session.getUsername().length();i++) {
	    		String thisPart = mc.session.getUsername().split("")[i];
	    		String validCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_";
	    		
	    		if(!validCharacters.contains(thisPart)) {
	    			mc.displayGuiScreen(new GuiError(new GuiAltManager(new GuiMultiplayer(new MainMenu())), "Your username contains invalid characters!", "Change"));
	        		return;
	    		}
	    	}
    	}
