...
import best.azura.client.impl.events.EventStep;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z, EventMove move) {
        if (move.isNoClip()) {
            this.noClip = move.isNoClip();
            this.setEntityBoundingBox(this.getEntityBoundingBox().offset(x, y, z));
            this.resetPositionToBB();
        } else {
            this.worldObj.theProfiler.startSection("move");
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = ((this.onGround && this.isSneaking()) || move.isSafeWalk()) && this instanceof EntityPlayer;
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                this.setEntityBoundingBox(axisalignedbb);
                y = this.stepHeight;
                Client.INSTANCE.getEventBus().call(new EventStep());
                List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(d3, y, d5));
                AxisAlignedBB axisalignedbb4 = this.getEntityBoundingBox();
                AxisAlignedBB axisalignedbb5 = axisalignedbb4.addCoord(d3, 0.0D, d5);
                double d9 = y;
            ...
            if (this.canTriggerWalking() && (!flag || move.isSafeWalk()) && this.ridingEntity == null) {
                double d12 = this.posX - d0;
                double d13 = this.posY - d1;
                double d14 = this.posZ - d2;
    ...
    public Vec3 getLook(float partialTicks) {
        if (this instanceof EntityPlayerSP) {
            return this.getLook(partialTicks);
        }
    ...
    public AxisAlignedBB getEntityBoundingBox() {
        if (this.entityId < 0)
            return AxisAlignedBB.fromBounds(this.posX, this.posY, this.posZ, this.posX, this.posY + 0.1, this.posZ);
