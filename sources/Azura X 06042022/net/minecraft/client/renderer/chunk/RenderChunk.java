...
public class RenderChunk {
	...
	public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
		...
		if (!this.isChunkRegionEmpty(blockpos)) {
			...
			for (EnumWorldBlockLayer enumWorldBlockLayer : ENUM_WORLD_BLOCK_LAYERS) {
				...
				if (compiledchunk.isLayerStarted(enumWorldBlockLayer)) {
					if (Config.isShaders()) {
						SVertexBuilder.calcNormalChunkLayer(generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(enumWorldBlockLayer));
					}

					WorldRenderer worldrenderer1 = generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(enumWorldBlockLayer);
					this.postRenderBlocks(enumWorldBlockLayer, x, y, z, worldrenderer1, compiledchunk);

					if (worldrenderer1.animatedSprites != null) {
						compiledchunk.setAnimatedSprites(enumWorldBlockLayer, (BitSet) worldrenderer1.animatedSprites.clone());
					}
				} else {
					compiledchunk.setAnimatedSprites(enumWorldBlockLayer, null);
				}
			}
