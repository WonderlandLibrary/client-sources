...
public abstract class Render<T extends Entity> implements IEntityRenderer
{
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        ...
        if (Client.INSTANCE.getModuleManager().getModuleByClass(IRCModule.class).isEnabled()) {
            if (Client.INSTANCE.getIrcConnector().getIrcCache().getIrcUserHashMap().entrySet().stream().anyMatch(stringIRCUserEntry -> stringIRCUserEntry.getValue().getMinecraftName().equalsIgnoreCase(entityIn.getName()))) {

                User user = Client.INSTANCE.getIrcConnector().getIrcCache().getIrcUserHashMap().entrySet().stream().filter(stringIRCUserEntry -> stringIRCUserEntry.getValue().getMinecraftName().equalsIgnoreCase(entityIn.getName())).findAny().get().getValue();

                str = str.replace(entityIn.getName(), user.getIrcRank().getPrefix() + " " + user.getUsername() + " " + ChatFormatting.PREFIX_CODE + "7(" + entityIn.getName() + ")" + ChatFormatting.PREFIX_CODE + "r");
            }
        }
