...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			if (flag) {
				if (!Config.zoomMode) {
					if (Camera.smoothZoom.getObject() && Client.INSTANCE.getModuleManager().getModule(Camera.class).isEnabled() || (!Client.INSTANCE.getModuleManager().getModule(Camera.class).isEnabled())) {
				...
				boolean zoomMod = Client.INSTANCE.getModuleManager().getModule(Camera.class).isEnabled();
				f /= zoomMod ? Camera.modifier.getObject() : 4.0F;
	...
	private void hurtCameraEffect(float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
			...
			Flight flight = (Flight) Client.INSTANCE.getModuleManager().getModule(Flight.class);
			if ((camera != null && camera.isEnabled() && Camera.noHurtCam.getObject()) || (flight != null && flight.isEnabled() && Flight.silentDamage.getObject()))
				return;
	...
	private void setupViewBobbing(float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			float f1 = -(entityplayer.distanceWalkedModified + f  * partialTicks) / mc.timer.getSilentTimerAmplification();
			float f2 = entityplayer.prevCameraYaw + (entityplayer.cameraYaw - entityplayer.prevCameraYaw) * partialTicks / mc.timer.getSilentTimerAmplification();
			float f3 = entityplayer.prevCameraPitch + (entityplayer.cameraPitch - entityplayer.prevCameraPitch) * partialTicks / mc.timer.getSilentTimerAmplification();
			...
			if (this.mc.gameSettings.fovSetting) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					if (movingobjectposition != null) {
						...
						final Camera camera = (Camera) Client.INSTANCE.getModuleManager().getModule(Camera.class);
						if (d7 < d3 && !(camera.isEnabled() && camera.noClip.getObject())) {
							d3 = d7;
						}
					}
				}
			...
			if (p_renderHand_3_) {
				...
				if (flag1 && this.mc.gameSettings.showDebugInfo == 0 && !flag && !this.mc.gameSettings.thirdPersonView && !this.mc.playerController.isSpectator()) {
					...
					if (Config.isShaders()) {
						...
						Client.INSTANCE.getEventBus().call(eventRenderHand);
						this.itemRenderer.renderItemInFirstPerson(p_renderHand_1_);
						eventRenderHand.setState("Post");
						Client.INSTANCE.getEventBus().call(eventRenderHand);
					}
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.theWorld != null) {
				...
				Client.INSTANCE.getEventBus().call(new EventRender3DPost());
			...
			Client.INSTANCE.getEventBus().call(new EventRenderScreen());
			GlStateManager.popMatrix();
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		float lastCameraPitch = mc.thePlayer.cameraPitch;
		float lastCameraYaw = mc.thePlayer.cameraYaw;
		...
		mc.thePlayer.cameraPitch = lastCameraPitch;
		mc.thePlayer.cameraYaw = lastCameraYaw;
