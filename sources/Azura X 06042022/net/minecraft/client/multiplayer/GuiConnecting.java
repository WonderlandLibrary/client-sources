...
public class GuiConnecting extends GuiScreen {
	...
	private void connect(final String ip, final int port) {
		...
		ServerUtil.lastData = new ServerData("", ip + ":" + port, true);
		ServerUtil.lastIP = ip;
		ServerUtil.lastPort = port;
		ServerUtil.joinedTime = System.currentTimeMillis();
		new Thread(() -> ServerUtil.lastResolvedIP = Resolver.resolveIP(ip, port), "IP Resolver thread").start();
			...
			public void run() {
				...
				try {
					...
					GuiConnecting.this.mc.displayGuiScreen(new GuiDisconnected(GuiConnecting.this.previousGuiScreen,
							"connect.failed", new ChatComponentTranslation("disconnect.genericReason", "Unknown host")));
	...
	public void updateScreen() {
		DiscordRPCImpl.updateNewPresence(
				...
				"Connecting to " + ServerUtil.lastIP
		);
