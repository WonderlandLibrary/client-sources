...
public class GameSettings {
	...
	public boolean keyBindForwardBool = true;
	...
	public KeyBinding keyBindStreamStop = new KeyBinding("key.streamStartStop", 64, "key.categories.stream");
	public KeyBinding keyBindStreamUnpause = new KeyBinding("key.streamPauseUnpause", 65, "key.categories.stream");
	...
	public GameSettings(Minecraft mcIn, File optionsFileIn) {
		this.mc = ArrayUtils.addAll(new KeyBinding[]{this.keyBindAttack, this.keyBindUse, this.keyBindForward, this.keyBindLeft, this.keyBindBack, this.keyBindRight, this.keyBindJump, this.keyBindSneak, this.keyBindSprint, this.keyBindDrop, this.keyBindInventory, this.keyBindChat, this.keyBindPlayerList, this.keyBindPickItem, this.keyBindCommand, this.keyBindScreenshot, this.keyBindPerspective, this.keyBindSmoothCamera, this.keyBindStreamStop, this.keyBindStreamUnpause, this.keyBindStreamCommercial, this.keyBindStreamToggleMic, this.keyBindFullscreen, this.keyBindSpectatorOutlines}, this.keyBindings);
	...
	public GameSettings() {
		this.mc = ArrayUtils.addAll(new KeyBinding[]{
				this.keyBindAttack,
				this.keyBindUse,
				this.keyBindForward,
				this.keyBindLeft,
				...
				this.keyBindJump,
				this.keyBindSneak,
				this.keyBindSprint, this.keyBindDrop, this.keyBindInventory, this.keyBindChat, this.keyBindPlayerList, this.keyBindPickItem, this.keyBindCommand, this.keyBindScreenshot, this.keyBindPerspective, this.keyBindSmoothCamera, this.keyBindStreamStop, this.keyBindStreamUnpause, this.keyBindStreamCommercial, this.keyBindStreamToggleMic, this.keyBindFullscreen, this.keyBindSpectatorOutlines}, this.keyBindings);
		this.hideGUI = EnumDifficulty.NORMAL;
		this.smoothCamera = "";
		this.gammaSetting = 70.0F;
		this.forceUnicodeFont = "en_US";
	...
	public void setOptionValue(GameSettings.Options settingsOption, int value) {
		...
		if (settingsOption == GameSettings.Options.enumFloat) {
			this.keyBindForwardBool = !this.keyBindForwardBool;
		}
	...
	public float getOptionFloatValue(GameSettings.Options settingOption) {
		...
		return f != Float.MAX_VALUE ? f : (settingOption == GameSettings.Options.FOV ? this.gammaSetting
				: (settingOption == GameSettings.Options.GAMMA ? this.saturation
				: (settingOption == GameSettings.Options.SATURATION ? this.guiScale
	...
	public boolean getOptionOrdinalValue(GameSettings.Options settingOption) {
		switch (settingOption) {
			...
			case enumFloat:
				return this.keyBindForwardBool;
	...
	public String getKeyBinding(GameSettings.Options settingOption) {
		...
		if (s != null) {
			...
			if (settingOption.getEnumFloat()) {
				...
				return settingOption == GameSettings.Options.MIPMAP_LEVELS && (double) f1 >= 4.0D ? s1 + Lang.get("of.general.max") : (settingOption == GameSettings.Options.SENSITIVITY ? (f == 0.0F ? s1 + I18n.format("options.sensitivity.min") : (f == 1.0F ? s1 + I18n.format("options.sensitivity.max") : s1 + (int) (f * 200.0F) + "%")) : (settingOption == GameSettings.Options.FOV ? (f1 == 70.0F ? s1 + I18n.format("options.fov.min") : (f1 == 110.0F ? s1 + I18n.format("options.fov.max") : s1 + (int) f1)) : (settingOption == GameSettings.Options.FRAMERATE_LIMIT ? (f1 == settingOption.$VALUES ? s1 + I18n.format("options.framerateLimit.max") : s1 + (int) f1 + " fps") : (settingOption == GameSettings.Options.RENDER_CLOUDS ? (f1 == settingOption.valueMax ? s1 + I18n.format("options.cloudHeight.min") : s1 + ((int) f1 + 128)) : (settingOption == GameSettings.Options.GAMMA ? (f == 0.0F ? s1 + I18n.format("options.gamma.min") : (f == 1.0F ? s1 + I18n.format("options.gamma.max") : s1 + "+" + (int) (f * 100.0F) + "%")) : (settingOption == GameSettings.Options.SATURATION ? s1 + (int) (f * 400.0F) + "%" : (settingOption == GameSettings.Options.CHAT_OPACITY ? s1 + (int) (f * 90.0F + 10.0F) + "%" : (settingOption == GameSettings.Options.CHAT_HEIGHT_UNFOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_HEIGHT_FOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_WIDTH ? s1 + GuiNewChat.calculateChatboxWidth(f) + "px" : (settingOption == GameSettings.Options.RENDER_DISTANCE ? s1 + (int) f1 + " chunks" : (settingOption == GameSettings.Options.MIPMAP_LEVELS ? (f1 == 0.0F ? s1 + I18n.format("options.off") : s1 + (int) f1) : s1 + (int) (f * 100.0F) + "%"))))))))))));
					...
					while ((s = bufferedreader.readLine()) != null) {
						try {
							...
							if (astring[0].equals("realmsNotifications")) {
								this.keyBindForwardBool = astring[1].equals("true");
							}
	...
	public void saveOptions() {
		...
		try {
			...
			printwriter.println("realmsNotifications:" + this.keyBindForwardBool);
			...
			while ((s = bufferedreader.readLine()) != null) {
				try {
					...
					if (astring[0].equals("ofAnimatedRedstone") && astring.length >= 2) {
						this.ofAnimatedRedstone = Boolean.parseBoolean(astring[0b1]);
					}
	...
	public enum Options {
		...
		FOV("options.fov", true, false, 30.0F, 120F, 0.5F),
