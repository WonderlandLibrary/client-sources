...
import best.azura.client.impl.events.EventKey;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public void run() {
        ...
        try {
            ...
            try {
                ...
                System.err.println("FAILED TO WRITE CRASH LOG");
                throwable.printStackTrace();
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        this.displayGuiScreen(new MainLogin());
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("Loading Minecraft 1.8.9...");
    ...
    private void runGameLoop() throws IOException {
        ...
        if (Client.INSTANCE.getModuleManager().getModule(FPSBooster.class).isEnabled())
            this.mcProfiler.profilingEnabled = false;
    ...
    public void clickMouse() {
        final EventClickMouse eventClickMouse = new EventClickMouse(0, this.objectMouseOver);
        Client.INSTANCE.getEventBus().call(eventClickMouse);
        if (eventClickMouse.isCancelled()) return;
        if (this.leftClickCounter <= 0) {
            AttackOrder.sendConditionalSwing(this.objectMouseOver);

            if (eventClickMouse.getObjectMouseOver() == null) {
                ...
                switch (eventClickMouse.getObjectMouseOver().typeOfHit) {
                    case ENTITY:
                        AttackOrder.sendFixedAttack(this.thePlayer, this.objectMouseOver.entityHit);
                        break;

                    case BLOCK:
                        BlockPos blockpos = eventClickMouse.getObjectMouseOver().getBlockPos();

                        if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air) {
                            this.playerController.clickBlock(blockpos, eventClickMouse.getObjectMouseOver().sideHit);
                            break;
                        }
                ...
                if (ViaMCP.getInstance().getVersion() != ViaMCP.PROTOCOL_VERSION) this.thePlayer.swingItem();
            }
        }
    }
    ...
    public void rightClickMouse() {
        final EventClickMouse eventClickMouse = new EventClickMouse(1, this.objectMouseOver);
        Client.INSTANCE.getEventBus().call(eventClickMouse);
        if (eventClickMouse.isCancelled()) return;
        if (!this.playerController.getIsHittingBlock()) {
            ...
            if (eventClickMouse.getObjectMouseOver() == null) {
                ...
                switch (eventClickMouse.getObjectMouseOver().typeOfHit) {
                    case ENTITY:
                        if (this.playerController.isPlayerRightClickingOnEntity(this.thePlayer, eventClickMouse.getObjectMouseOver().entityHit, eventClickMouse.getObjectMouseOver())) {
                            flag = false;
                        } else if (this.playerController.interactWithEntitySendPacket(this.thePlayer, eventClickMouse.getObjectMouseOver().entityHit)) {
                            flag = false;
                        }
                    ...
                    case BLOCK:
                        ...
                        if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air) {
                            ...
                            if (this.playerController.onPlayerRightClick(this.thePlayer, this.theWorld, itemstack, blockpos, eventClickMouse.getObjectMouseOver().sideHit, eventClickMouse.getObjectMouseOver().hitVec)) {
                                flag = false;
                                this.thePlayer.swingItem();
                            }
    ...
    public void toggleFullscreen() {
        try {
            ...
            if (this.fullscreen) {
                this.updateDisplayMode();
                if (ClientModule.borderlessFullscreen.getObject()) {
            ...
            if (!ClientModule.borderlessFullscreen.getObject()) Display.setFullscreen(this.fullscreen);
            Display.setVSyncEnabled(this.gameSettings.enableVsync);
            this.updateDisplay();
        } catch (Exception exception) {
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        Client.INSTANCE.getEventBus().call(new EventKey(k));
    ...
    private void middleClickMouse() {
        final EventClickMouse eventClickMouse = new EventClickMouse(2, this.objectMouseOver);
        Client.INSTANCE.getEventBus().call(eventClickMouse);
        if (eventClickMouse.isCancelled()) return;
        if (eventClickMouse.getObjectMouseOver() != null) {
            ...
            if (eventClickMouse.getObjectMouseOver().typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                BlockPos blockpos = eventClickMouse.getObjectMouseOver().getBlockPos();
                ...
                if (eventClickMouse.getObjectMouseOver().typeOfHit != MovingObjectPosition.MovingObjectType.ENTITY || eventClickMouse.getObjectMouseOver().entityHit == null || !flag) {
                    return;
                }

                if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityPainting) {
                    item = Items.painting;
                } else if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityLeashKnot) {
                    item = Items.lead;
                } else if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityItemFrame) {
                    EntityItemFrame entityitemframe = (EntityItemFrame) eventClickMouse.getObjectMouseOver().entityHit;
                ...
                } else if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityMinecart) {
                    EntityMinecart entityminecart = (EntityMinecart) eventClickMouse.getObjectMouseOver().entityHit;
                ...
                } else if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityBoat) {
                    item = Items.boat;
                } else if (eventClickMouse.getObjectMouseOver().entityHit instanceof EntityArmorStand) {
                    item = Items.armor_stand;
                } else {
                    item = Items.spawn_egg;
                    i = EntityList.getEntityID(eventClickMouse.getObjectMouseOver().entityHit);
                    flag1 = true;
    ...
    public void setSession(Session session) {
        String oldName = this.session != null ? this.session.getUsername() : "";
        this.session = session;
        Client.INSTANCE.getEventBus().call(new EventNameChange(oldName, session.getUsername()));
    }
            ...
            if (!(this.currentScreen instanceof GuiControls) || ((GuiControls) this.currentScreen).time <= getSystemTime() - 20L) {
                if (Keyboard.getEventKeyState()) {
                    if (i == this.gameSettings.keyBindFullscreen.getKeyCode()) {
                        this.toggleFullscreen();
                    } else if (i == this.gameSettings.keyBindScreenshot.getKeyCode() && Keyboard.getEventCharacter() != '<') { // '<' is never a key that you can use for screenshotting
