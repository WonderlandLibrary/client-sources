...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    public final void drawScreenNoOverride(int mouseX, int mouseY, float partialTicks)
    {
        for (int i = 0; i < this.buttonList.size(); ++i)
        {
            this.buttonList.get(i).drawButton(this.mc, mouseX, mouseY);
        ...
        if (!textLines.isEmpty())
        {
            ...
            for (String s : textLines)
            {
                ...
                if (customMinecraft.toolTip.getObject() && customMinecraft.isEnabled()) {
                    j = Client.INSTANCE.getFontRenderer().getStringWidth(s) / 2;
                } else
                    j = this.fontRendererObj.getStringWidth(s);
            ...
            for (int k1 = 0; k1 < textLines.size(); ++k1)
            {
                ...
                if (customMinecraft.toolTip.getObject() && customMinecraft.isEnabled()) {
                    ...
                    Client.INSTANCE.getFontRenderer().drawStringWithShadow(s1, (float)l1 * 2, (float)i2 * 2 - 3, -1);
                    GlStateManager.scale(2, 2, 2);
                } else this.fontRendererObj.drawStringWithShadow(s1, (float)l1, (float)i2, -1);
    ...
    protected final void mouseClickedNoOverride(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        if (mouseButton == 0)
        {
            for (int i = 0; i < this.buttonList.size(); ++i)
