...
public class GuiInventory extends InventoryEffectRenderer
{
    ...
    public static void drawEntityOnScreen(int posX, int posY, int scale, float mouseX, float mouseY, EntityLivingBase ent)
    {
        ...
        if (ent instanceof EntityPlayerSP) {
            EntityPlayerSP p = (EntityPlayerSP) ent;
            f = p.renderYawOffset;
            f1 = p.rotationYaw;
            f2 = p.rotationPitchHead;
            f3 = p.prevRotationYawHead;
            f4 = p.rotationYawHead;
            p.renderYawOffset = (float)Math.atan(mouseX / 40.0F) * 20.0F;
            p.rotationYaw = (float)Math.atan(mouseX / 40.0F) * 40.0F;
            p.rotationPitchHead = -((float)Math.atan(mouseY / 40.0F)) * 20.0F;
            p.rotationYawHead = p.rotationYaw;
            p.prevRotationYawHead = p.rotationYaw;
        } else {
        ...
        if (ent instanceof EntityPlayerSP) {
            ...
            p.renderYawOffset = f;
            p.rotationYaw = f1;
            p.rotationPitchHead = f2;
            p.prevRotationYawHead = f3;
            p.rotationYawHead = f4;
        } else {
