...
public class Gui
{
     ...
     */
    public void drawTexturedModalRect(int xCord, int yCord, TextureAtlasSprite textureSprite, int widthIn, int heightIn)
    {
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos((xCord), (yCord + heightIn), this.zLevel).tex(textureSprite.getMinU(), textureSprite.getMaxV()).endVertex();
        worldrenderer.pos((xCord + widthIn), (yCord + heightIn), this.zLevel).tex(textureSprite.getMaxU(), textureSprite.getMaxV()).endVertex();
        worldrenderer.pos((xCord + widthIn), (yCord), this.zLevel).tex(textureSprite.getMaxU(), textureSprite.getMinV()).endVertex();
        worldrenderer.pos((xCord), (yCord), this.zLevel).tex(textureSprite.getMinU(), textureSprite.getMinV()).endVertex();
        tessellator.draw();
    }
    ...
    {
        drawScaled(x, y, u, v, width, height, width, height, textureWidth, textureHeight);
    }
    ...
    {
        drawScaled(x, y, u, v, uWidth, vHeight, width, height, tileWidth, tileHeight);
    }

    private static void drawScaled(int x, int y, float u, float v, float uWidth, float vHeight, int width, int height, float tileWidth, float tileHeight) {
