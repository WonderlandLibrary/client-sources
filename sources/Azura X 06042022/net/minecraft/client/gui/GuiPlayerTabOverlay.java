...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn)
    {
        ...
        if (Client.INSTANCE.getModuleManager().getModuleByClass(IRCModule.class).isEnabled()) {
            if (Client.INSTANCE.getIrcConnector().getIrcCache().getIrcUserHashMap().entrySet().stream().anyMatch(stringIRCUserEntry -> stringIRCUserEntry.getValue().getMinecraftName().equalsIgnoreCase(networkPlayerInfoIn.getGameProfile().getName()))) {

                User user = Client.INSTANCE.getIrcConnector().getIrcCache().getIrcUserHashMap().entrySet().stream().filter(stringIRCUserEntry -> stringIRCUserEntry.getValue().getMinecraftName().equalsIgnoreCase(networkPlayerInfoIn.getGameProfile().getName())).findAny().get().getValue();

                try {
                    if (user != null && user.getIrcRank() != null && networkPlayerInfoIn.getGameProfile() != null && networkPlayerInfoIn.getGameProfile().getName() != null && !networkPlayerInfoIn.getGameProfile().getName().isEmpty() && name != null && !name.isEmpty()) {
                        name = name.replace(networkPlayerInfoIn.getGameProfile().getName(), user.getIrcRank().getPrefix() + " " + user.getUsername() + " " + user.getFormattedIGN());
                    }
