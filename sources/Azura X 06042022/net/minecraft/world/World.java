...
import best.azura.client.impl.module.impl.render.Ambience;
...
public abstract class World implements IBlockAccess
{
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate)
    {
        ...
        if (!forceUpdate || this.isAreaLoaded(i - k, 0, j - k, i + k, 0, j + k, true))
        {
            ...
            final boolean skip = Client.INSTANCE.getModuleManager().getModule(FPSBooster.class).isEnabled() &&
                    FPSBooster.noTickInvis.getObject() && entityIn.isInvisible();
    ...
    protected void updateWeather()
    {
        if (!this.provider.getHasNoSky())
        {
            if (!this.isRemote)
            {
                ...
                final Ambience ambience = (Ambience) Client.INSTANCE.getModuleManager().getModule(Ambience.class);
                //this.worldInfo.setRaining(ambience.weather.getObject().equals("Rain"));
    ...
    protected void setActivePlayerChunksAndCheckLight()
    {
        ...
        for (EntityPlayer playerEntity : this.playerEntities) {
            int j = MathHelper.floor_double((playerEntity).posX / 16.0D);
            int k = MathHelper.floor_double((playerEntity).posZ / 16.0D);
