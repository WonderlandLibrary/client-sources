 ...
 */
package me.AveReborn.util.fontRenderer.CFont;

import java.awt.Font;
import java.util.ArrayList;
import java.util.List;
import me.AveReborn.mod.ModManager;
import me.AveReborn.util.fontRenderer.CFont.CFont;
...
extends CFont {
    ...
    public float drawCenteredStringWithShadow(String text, double x2, int y2, int color) {
    ...
    public float drawString(String text, double x2, double y2, int color, boolean shadow) {
        if (text.contains(Minecraft.getMinecraft().session.getUsername()) && ModManager.getModByName("NameProtect").isEnabled()) {
            Minecraft.getMinecraft();
            text.replace(Minecraft.thePlayer.getName(), "AveClient");
        }
        x2 -= 1.0;
        if (color == 553648127) {
            color = 16777215;
