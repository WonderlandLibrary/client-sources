...
extends DynamicMethod {
    ...
    static MethodHandle getInvocation(MethodHandle target, MethodType callSiteType, LinkerServices linkerServices) {
        ...
        if (argsLen == fixParamsLen) {
            MethodHandle matchedMethod = varArgs ? MethodHandles.insertArguments(fixTarget, fixParamsLen, Array.newInstance(methodType.parameterType(fixParamsLen).getComponentType(), 0)) : fixTarget;
        ...
        if (argsLen == paramsLen) {
            ...
            if (varArgType.isAssignableFrom(callSiteLastArgType)) {
                return SingleDynamicMethod.createConvertingInvocation(filteredTarget, linkerServices, callSiteType).asVarargsCollector(callSiteLastArgType);
            }
            ...
            MethodHandle arrayConvertingInvocation = SingleDynamicMethod.createConvertingInvocation(MethodHandles.filterArguments(fixTarget, fixParamsLen, linkerServices.getTypeConverter(callSiteLastArgType, varArgType)), linkerServices, callSiteType);
