...
implements Loggable {
    ...
    GuardedInvocation findGetMethod(FindProperty find, ScriptObject receiver, CallSiteDescriptor desc) {
        ...
        synchronized (globalConstants) {
            ...
            MethodHandle mh = isOptimistic ? (JSType.getAccessorTypeIndex(cmh.type().returnType()) <= JSType.getAccessorTypeIndex(retType) ? Lookup.MH.asType(cmh, cmh.type().changeReturnType(retType)) : Lookup.MH.dropArguments(Lookup.MH.insertArguments(JSType.THROW_UNWARRANTED.methodHandle(), 0, c, programPoint), 0, Object.class)) : Lookup.filterReturnType(cmh, retType);
