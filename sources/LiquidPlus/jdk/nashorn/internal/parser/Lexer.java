...
extends Scanner {
    ...
    private String valueOfString(int start, int length, boolean strict2) throws RuntimeException {
        int savePosition = this.position;
        int end = start + length;
        this.reset(start);
        StringBuilder sb = new StringBuilder(length);
        while (this.position < end) {
            if (this.ch0 == '\\') {
                ...
                switch (next) {
                    ...
                    case '6': 
                    case '7': {
                        if (strict2 && (next != '0' || this.ch0 >= '0' && this.ch0 <= '9')) {
    ...
    Object getValueOf(long token, boolean strict2) {
        ...
        switch (Token.descType(token)) {
            ...
            case ESCSTRING: {
                return this.valueOfString(start, len, strict2);
            }
            case IDENT: {
