...
import com.github.benmanes.caffeine.cache.PSWR;
import java.lang.ref.ReferenceQueue;

final class PSWRMW<K, V>
extends PSWR<K, V> {
    int queueType;
    int weight;
    int policyWeight;
    ...
    public Node<K, V> newNode(K key, ReferenceQueue<K> keyReferenceQueue, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PSWRMW<K, V>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
    }

    @Override
    public Node<K, V> newNode(Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PSWRMW<K, V>(keyReference, value, valueReferenceQueue, weight, now);
    }
}
