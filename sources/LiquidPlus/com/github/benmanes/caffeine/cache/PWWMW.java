...
import com.github.benmanes.caffeine.cache.PWW;
import java.lang.ref.ReferenceQueue;

final class PWWMW<K, V>
extends PWW<K, V> {
    int queueType;
    int weight;
    int policyWeight;
    ...
    public Node<K, V> newNode(K key, ReferenceQueue<K> keyReferenceQueue, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PWWMW<K, V>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
    }

    @Override
    public Node<K, V> newNode(Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PWWMW<K, V>(keyReference, value, valueReferenceQueue, weight, now);
    }
}
