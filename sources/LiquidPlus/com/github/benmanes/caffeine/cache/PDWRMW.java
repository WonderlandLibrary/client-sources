...
import com.github.benmanes.caffeine.cache.PDWR;
import java.lang.ref.ReferenceQueue;

final class PDWRMW<K, V>
extends PDWR<K, V> {
    int queueType;
    int weight;
    int policyWeight;
    ...
    public Node<K, V> newNode(K key, ReferenceQueue<K> keyReferenceQueue, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PDWRMW<K, V>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
    }

    @Override
    public Node<K, V> newNode(Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
        return new PDWRMW<K, V>(keyReference, value, valueReferenceQueue, weight, now);
    }
}
