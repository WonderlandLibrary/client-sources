...
extends ItemRewriterBase<T> {
    ...
    public @Nullable Block handleBlock(int blockId, int data) {
        MappedLegacyBlockItem settings2 = (MappedLegacyBlockItem)this.replacementData.get(blockId);
        if (settings2 == null || !settings2.isBlock()) {
            return null;
        }
        Block block = settings2.getBlock();
        if (block.getData() == -1) {
    ...
    protected void handleChunk(Chunk chunk) {
        ...
        for (CompoundTag tag : chunk.getBlockEntities()) {
            ...
            tags.put(pos, tag);
            if (pos.getY() < 0 || pos.getY() > 255 || (section = chunk.getSections()[pos.getY() >> 4]) == null || (settings2 = (MappedLegacyBlockItem)this.replacementData.get(btype = (block = section.getFlatBlock(pos.getX(), pos.getY() & 0xF, pos.getZ())) >> 4)) == null || !settings2.hasBlockEntityHandler()) continue;
        ...
        for (int i = 0; i < chunk.getSections().length; ++i) {
            ...
            for (int j = 0; j < section.getPaletteSize(); ++j) {
                ...
                if (hasBlockEntityHandler || (settings3 = (MappedLegacyBlockItem)this.replacementData.get(btype)) == null || !settings3.hasBlockEntityHandler()) continue;
            ...
            for (int x = 0; x < 16; ++x) {
                for (int y = 0; y < 16; ++y) {
                    for (int z = 0; z < 16; ++z) {
                        ...
                        int btype = block >> 4;
                        int meta = block & 0xF;
                        settings2 = (MappedLegacyBlockItem)this.replacementData.get(btype);
