...
extends MemberCodeGen {
    ...
    public boolean isParamListName(ASTList args2) {
        if (this.paramTypeList != null && args2 != null && args2.tail() == null) {
    ...
    void compileInvokeSpecial(ASTree target, int methodIndex, String descriptor, ASTList args2) throws CompileError {
        target.accept(this);
        int nargs = this.getMethodArgsLength(args2);
        this.atMethodArgs(args2, new int[nargs], new int[nargs], new String[nargs]);
    ...
    public static int compileParameterList(Bytecode code, CtClass[] params, int regno) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (params[i].isPrimitive()) {
                ...
                regno += s;
                args2[0] = pt;
                code.addInvokespecial(wrapper, "<init>", Descriptor.ofMethod(CtClass.voidType, args2));
            } else {
                code.addAload(regno);
                ++regno;
            }
