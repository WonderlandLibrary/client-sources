...
extends AbstractExaminer<Stream<String>> {
    ...
    private static Stream<String> enclose(Stream<String> lines, String open2, String close) {
        return MultiLineStringExaminer.enclose(lines.collect(Collectors.toList()), open2, close);
    }

    private static Stream<String> enclose(List<String> lines, String open2, String close) {
        if (lines.isEmpty()) {
            return Stream.of(open2 + close);
        }
        return Stream.of(Stream.of(open2), MultiLineStringExaminer.indent(lines.stream()), Stream.of(close)).reduce(Stream.empty(), Stream::concat);
    }
