...
extends Type<DataPalette> {
    ...
    public DataPalette read(ByteBuf buffer) throws Exception {
        ...
        if ((values2 = new long[Type.VAR_INT.readPrimitive(buffer)]).length > 0) {
            ...
            if (values2.length != expectedLength) {
                throw new IllegalStateException("Palette data length (" + values2.length + ") does not match expected length (" + expectedLength + ")! bitsPerValue=" + bitsPerValue + ", originalBitsPerValue=" + originalBitsPerValue);
            }
            for (int i = 0; i < values2.length; ++i) {
                values2[i] = buffer.readLong();
            }
