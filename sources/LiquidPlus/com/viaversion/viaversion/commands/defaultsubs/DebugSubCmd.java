    ...
    public boolean execute(ViaCommandSender sender, String[] args2) {
        ...
        if (args2.length == 1) {
            ...
            if (args2[0].equalsIgnoreCase("add")) {
                ...
                DebugSubCmd.sendMessage(sender, "&6Added packet type %s to debug logging", args2[1]);
                return true;
            }
            if (args2[0].equalsIgnoreCase("remove")) {
                debug.removePacketTypeNameToLog(args2[1].toUpperCase(Locale.ROOT));
                DebugSubCmd.sendMessage(sender, "&6Removed packet type %s from debug logging", args2[1]);
                return true;
            }
        }
        return false;
