...
extends BaseVersionProvider {
    ...
    public int getClosestServerProtocol(UserConnection user) throws Exception {
        ...
        ArrayList sorted2 = new ArrayList(list);
        Collections.sort(sorted2);
        ProtocolInfo info = user.getProtocolInfo();
        if (sorted2.contains(info.getProtocolVersion())) {
            return info.getProtocolVersion();
        }
        if (info.getProtocolVersion() < (Integer)sorted2.get(0)) {
            return BungeeVersionProvider.getLowestSupportedVersion();
        }
        for (Integer protocol : Lists.reverse(sorted2)) {
