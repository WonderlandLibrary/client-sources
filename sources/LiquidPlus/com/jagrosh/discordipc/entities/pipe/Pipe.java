...
public abstract class Pipe {
    ...
    public static Pipe openPipe(IPCClient ipcClient, long clientId, HashMap<String, Callback> callbacks, DiscordBuild ... preferredOrder) throws NoDiscordClientException {
        ...
        Pipe pipe = null;
        Pipe[] open2 = new Pipe[DiscordBuild.values().length];
        for (i = 0; i < 10; ++i) {
            try {
                ...
                open2[pipe.build.ordinal()] = pipe;
                open2[DiscordBuild.ANY.ordinal()] = pipe;
                pipe.build = null;
                pipe = null;
                continue;
            }
        ...
        if (pipe == null) {
            for (i = 1; i < preferredOrder.length; ++i) {
                ...
                if (open2[cb.ordinal()] == null) continue;
                pipe = open2[cb.ordinal()];
                open2[cb.ordinal()] = null;
                if (cb == DiscordBuild.ANY) {
                    for (int k = 0; k < open2.length; ++k) {
                        if (open2[k] != pipe) continue;
        ...
        for (i = 0; i < open2.length; ++i) {
            if (i == DiscordBuild.ANY.ordinal() || open2[i] == null) continue;
            try {
                open2[i].close();
                continue;
            }
