...
extends Style {
    ...
    private Color modifyAlpha(Color col, int alpha2) {
        return new Color(col.getRed(), col.getGreen(), col.getBlue(), alpha2);
    }

    @Override
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        RenderUtils.drawGradientSideways((float)panel.getX() - 3.0f, panel.getY(), (float)panel.getX() + (float)panel.getWidth() + 3.0f, (float)panel.getY() + 19.0f, this.modifyAlpha(ClickGUI.generateColor(), 120).getRGB(), this.modifyAlpha(ClickGUI.generateColor().darker().darker(), 120).getRGB());
        ...
        Fonts.Poppins.func_78276_b("\u00a7f" + panel.getName(), (int)((float)panel.getX() - (textWidth - 100.0f) / 2.0f), panel.getY() + 7, Integer.MAX_VALUE);
    }

    @Override
    public void drawDescription(int mouseX, int mouseY, String text) {
        int textWidth = Fonts.Poppins.func_78256_a(text);
        RenderUtils.drawRect((float)(mouseX + 9), (float)mouseY, (float)(mouseX + textWidth + 14), (float)(mouseY + Fonts.Poppins.field_78288_b + 3), this.modifyAlpha(ClickGUI.generateColor(), 120).getRGB());
        GlStateManager.func_179117_G();
