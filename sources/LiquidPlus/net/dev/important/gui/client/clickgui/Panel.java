...
extends MinecraftInstance {
    ...
    public Panel(String name, int x, int y, int width, int height, boolean open2) {
        this.name = name;
    ...
    public void drawScreen(int mouseX, int mouseY, float button) {
        ...
        int maxElements = (Integer)((ClickGUI)Objects.requireNonNull(Client.moduleManager.getModule(ClickGUI.class))).maxElementsValue.get();
        if (this.drag) {
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 1 && this.isHovering(mouseX, mouseY)) {
            this.open = !this.open;
            mc.func_147118_V().func_147682_a((ISound)PositionedSoundRecord.func_147674_a((ResourceLocation)new ResourceLocation("random.bow"), (float)1.0f));
            return;
        }
        for (Element element : this.elements) {
    ...
    private int getElementsHeight() {
        ...
        int count = 0;
        for (Element element : this.elements) {
            if (count >= (Integer)((ClickGUI)Objects.requireNonNull(Client.moduleManager.getModule(ClickGUI.class))).maxElementsValue.get()) continue;
    ...
    boolean isHovering(int mouseX, int mouseY) {
        float textWidth = (float)Panel.mc.field_71466_p.func_78256_a(StringUtils.func_76338_a((String)this.name)) - 100.0f;
        return (float)mouseX >= (float)this.x - textWidth / 2.0f - 19.0f && (float)mouseX <= (float)this.x - textWidth / 2.0f + (float)Panel.mc.field_71466_p.func_78256_a(StringUtils.func_76338_a((String)this.name)) + 19.0f && mouseY >= this.y && mouseY <= this.y + this.height - (this.open ? 2 : 0);
    }
}
