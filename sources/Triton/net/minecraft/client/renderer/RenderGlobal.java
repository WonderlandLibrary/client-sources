...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener
{
        ...
        else
        {
            ...
            while (var321.hasNext())
            {
                ...
                while (((Iterator)var27).hasNext())
                {
                    ...
                    if (var34)
                    {
                        boolean teClass = this.mc.getRenderViewEntity() instanceof EntityLivingBase ? ((EntityLivingBase)this.mc.getRenderViewEntity()).isPlayerSleeping() : false;

                        if (var30 == this.mc.getRenderViewEntity() && this.mc.gameSettings.thirdPersonView == 0 && !teClass || var30.posY >= 0.0D && var30.posY < 256.0D && !this.theWorld.isBlockLoaded(new BlockPos(var30)))
                        {
                            continue;
                        }
            ...
            if (this.mc.theWorld.provider.isSurfaceWorld())
            {
                ...
                else
                {
                    ...
                    if (this.isFancyGlListClouds || this.cloudTickCounter >= this.cloudTickCounterGlList + 20)
                    {
                        ...
                        dc = this.mc.getRenderViewEntity().prevPosX + (this.mc.getRenderViewEntity().posX - this.mc.getRenderViewEntity().prevPosX) * (double)p_180447_1_ + exactPlayerZ1 * 0.029999999329447746D;
                        double cdx = this.mc.getRenderViewEntity().prevPosZ + (this.mc.getRenderViewEntity().posZ - this.mc.getRenderViewEntity().prevPosZ) * (double)p_180447_1_;
                    ...
                    Entity entityliving1 = this.mc.getRenderViewEntity();
    ...
    private void func_180445_c(float p_180445_1_, int p_180445_2_)
    {
        ...
        for (int var29 = 0; var29 < 2; ++var29)
        {
            ...
            Entity var43 = this.mc.getRenderViewEntity();
