...
public final class ConfigManager extends Manager<Config>
{
    ...
    public void load() {
        if (!ConfigManager.cfg_dir.exists()) {
            ConfigManager.cfg_dir.mkdirs();
        }
        if (ConfigManager.cfg_dir != null) {
            final File[] listFiles;
            final File[] files = listFiles = ConfigManager.cfg_dir.listFiles(f -> !f.isDirectory() && FilenameUtils.getExtension(f.getName()).equals("json"));
    ...
    public Config findConfig(final String configName) {
        ...
        if (new File(ConfigManager.cfg_dir, configName + ".json").exists()) {
    ...
    static {
        cfg_dir = new File(Fluger.name, "configs");
