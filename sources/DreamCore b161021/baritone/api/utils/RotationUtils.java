...
public final class RotationUtils {
    ...
    private static Rotation calcRotationFromVec3d(Vec3d orig, Vec3d dest) {
        ...
        double yaw = MathHelper.atan2(delta[0], -delta[2]) + MathematicHelper.randomizeFloat(-1, 1);
        double dist = Math.sqrt(delta[0] * delta[0] + delta[2] * delta[2]);
        double pitch = MathHelper.atan2(delta[1], dist) + MathematicHelper.randomizeFloat(-1, 1);
        return new Rotation(
    ...
    public static Optional<Rotation> reachable(EntityPlayerSP entity, BlockPos pos, double blockReachDistance, boolean wouldSneak) {
        ...
        for (Vec3d sideOffset : BLOCK_SIDE_MULTIPLIERS) {
            ...
            possibleRotation = reachableOffset(entity, pos, new Vec3d(pos).add(new Vec3d(xDiff, yDiff, zDiff)), blockReachDistance, wouldSneak);
            if (possibleRotation.isPresent()) {
                return possibleRotation;
            }
        }
