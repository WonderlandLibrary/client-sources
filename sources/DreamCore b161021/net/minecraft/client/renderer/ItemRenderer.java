...
import org.dreamcore.client.event.events.impl.player.EventTransformSideFirstPerson;
import org.dreamcore.client.feature.impl.combat.KillAura;
import org.dreamcore.client.feature.impl.visual.Animations;
import org.dreamcore.client.feature.impl.visual.Chams;
...
public class ItemRenderer {
    ...
    private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_) {
        ...
        if (dreamcore.instance.featureManager.getFeatureByClass(Chams.class).getState() && Chams.chamsMode.currentMode.equals("Fill")) {
            GL11.glPushMatrix();
            ...
            if (!Chams.clientColor.getBoolValue()) {
                org.dreamcore.client.helpers.render.RenderHelper.setColor(Chams.colorChams.getColorValue());
            } else {
                org.dreamcore.client.helpers.render.RenderHelper.setColor(ClientHelper.getClientColor().getRGB());
            }
    ...
    private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_) {
        ...
        if(dreamcore.instance.featureManager.getFeatureByClass(Animations.class).getState() && Animations.itemAnimation.getBoolValue()) {
        ...
        if (dreamcore.instance.featureManager.getFeatureByClass(Animations.class).getState()) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.isEmpty()) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    if (dreamcore.instance.featureManager.getFeatureByClass(Animations.class).getState() && Animations.animation.getBoolValue() && dreamcore.instance.featureManager.getFeatureByClass(KillAura.class).getState() && KillAura.target != null && mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getNumberValue()) {
                        if (dreamcore.instance.featureManager.getFeatureByClass(Animations.class).getState() && Animations.animation.getBoolValue() && dreamcore.instance.featureManager.getFeatureByClass(KillAura.class).getState() && KillAura.target != null && mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getNumberValue()) {
                            if (enumhandside != EnumHandSide.LEFT) {
                                if (mode.equalsIgnoreCase("dreamcore")) {
                                    this.transformFirstPersonItem(equipProgress / 3, swingprogress);
                                    this.translate();
                                    ...
                                    this.transformFirstPersonItem(equipProgress / Animations.smooth.getNumberValue(), swingprogress);
                                    this.translate();
                                }
                            } else {
    ...
    private void renderFireInFirstPerson() {
        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.fire.getBoolValue()) {
            return;
        }
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
