...
import org.dreamcore.client.feature.impl.visual.EnchantmentColor;
...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase> {
    ...
    private void renderArmorLayer(EntityLivingBase entityLivingBaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale, EntityEquipmentSlot slotIn) {
        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.armor.getBoolValue()) {
            return;
        }
            ...
            if (itemarmor.getEquipmentSlot() == slotIn) {
                ...
                if (Reflector.ForgeHooksClient_getArmorTexture.exists()) {
                    ...
                    if (!this.skipRenderGlint && itemstack.hasEffect() && (!Config.isCustomItems() || !CustomItems.renderCustomArmorEffect(entityLivingBaseIn, itemstack, t, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale))) {
                        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.glintEffect.getBoolValue())
                            return;
    ...
    public static void renderEnchantedGlint(RenderLivingBase<?> p_188364_0_, EntityLivingBase p_188364_1_, ModelBase model, float p_188364_3_, float p_188364_4_, float p_188364_5_, float p_188364_6_, float p_188364_7_, float p_188364_8_, float p_188364_9_) {
        if (!Config.isShaders() || !Shaders.isShadowPass) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (dreamcore.instance.featureManager.getFeatureByClass(EnchantmentColor.class).getState()) {
                    switch (EnchantmentColor.colorMode.currentMode) {
                        case "Rainbow":
                            Color rainbow = PaletteHelper.rainbow((int) f, 1, 1);
                            GlStateManager.color(rainbow.getRed(), rainbow.getGreen(), rainbow.getBlue(), rainbow.getAlpha());
                            break;
                        case "Custom":
                            Color colorCustom = new Color(EnchantmentColor.customColor.getColorValue());
                            GlStateManager.color(colorCustom.getRed(), colorCustom.getGreen(), colorCustom.getBlue(), colorCustom.getAlpha());
                            break;
                        case "Client":
                            GlStateManager.color(ClientHelper.getClientColor().getRed(), ClientHelper.getClientColor().getGreen(), ClientHelper.getClientColor().getBlue(), ClientHelper.getClientColor().getAlpha());
                            break;
                    }
                }
