...
import org.dreamcore.client.feature.impl.ghost.Reach;
import org.dreamcore.client.feature.impl.player.NoInteract;
import org.dreamcore.client.feature.impl.visual.*;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            for (Entity entity1 : list) {
                if (entity1 instanceof EntityArmorStand && dreamcore.instance.featureManager.getFeatureByClass(NoInteract.class).getState() && NoInteract.armorStands.getBoolValue()) {
                    return;
                }
            ...
            float reachDist = dreamcore.instance.featureManager.getFeatureByClass(Reach.class).getState() ? Reach.reachValue.getNumberValue() : 3;
            if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > reachDist) {
                this.pointedEntity = null;
    ...
    private float zoomAnimation = 1.0F;
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting)
    {
        if (this.debugView) {
            ...
            if (flag) {
                ...
                if (zoomAnimation < 5) {
                    zoomAnimation += 0.03 * Minecraft.frameTime / 6;
                }
            } else {
                if (zoomAnimation > 1) {
                    zoomAnimation -= 0.03 * Minecraft.frameTime / 6;
            ...
            return Reflector.ForgeHooksClient_getFOVModifier.exists()
                    ? Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier, this, entity, iblockstate,
                    partialTicks, f)
                    : f;
        }
    }
    ...
    {
        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.hurt.getBoolValue()) {
            return;
        }
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        {
            double d3 = dreamcore.instance.featureManager.getFeatureByClass(PersonViewer.class).getState() ? PersonViewer.fovModifier.getNumberValue() : (double)(this.thirdPersonDistancePrev + (4.0F - this.thirdPersonDistancePrev) * partialTicks);
            if (this.mc.gameSettings.debugCamEnable) {
                GlStateManager.translate(0.0F, 0.0F, (float) (-d3));
            } else {
                float f1;
                float f2;
                if (dreamcore.instance.featureManager.getFeatureByClass(PersonViewer.class).getState()) {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if(!(dreamcore.instance.featureManager.getFeatureByClass(CameraNoClip.class).getState()))
                    {
    ...
    private void updateLightmap(float partialTicks)
    {
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                if (dreamcore.instance.featureManager.getFeatureByClass(WorldColor.class).getState()) {
                    int color = WorldColor.worldColor.getColorValue();
                    int r = color >> 16 & 0xFF;
                    int g = color >> 8 & 0xFF;
                    int b = color & 0xFF;
                    int a = color >> 24 & 0xFF;
    ...
    private void updateFogColor(float partialTicks)
    {
        World world = this.mc.world;
        Entity entity = this.mc.getRenderViewEntity();
        float f = dreamcore.instance.featureManager.getFeatureByClass(FogColor.class).getState() ? FogColor.distance.getNumberValue() * 10 : 0.25F + 0.75F * (float)this.mc.gameSettings.renderDistanceChunks / 32.0F;
    ...
    {
        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.fog.getBoolValue()) {
            return;
        }
        ...
        else
        {
            ...
            {
                float dist = dreamcore.instance.featureManager.getFeatureByClass(FogColor.class).getState() ? -FogColor.distance.getNumberValue() : Config.getFogStart();
    ...
    {
        if (dreamcore.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.totem.getBoolValue()) {
            return;
        }
