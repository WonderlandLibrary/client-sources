package org.dreamcore.client.ui.clickgui.component.impl;
...
import org.dreamcore.client.ui.clickgui.Panel;
import org.dreamcore.client.ui.clickgui.component.Component;
import org.dreamcore.client.ui.clickgui.component.ExpandableComponent;
import org.dreamcore.client.ui.clickgui.component.PropertyComponent;
...
public class ColorPickerComponent extends ExpandableComponent implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        Gui.drawRect(x, y, x + width, y + height, new Color(20, 20, 20, 160).getRGB());
        mc.fontRenderer.drawStringWithShadow(getName(), x + 2, y + height / 2F - 3, textColor);
        float left = x + width - 13;
        float top = y + height / 2.0F - 2;
        float right = x + width - 2;
        float bottom = y + height / 2.0F + 2;

        RectHelper.drawRect(left - 0.5F, top - 0.5F, right + 0.5F,
                bottom + 0.5F, 0xFF000000);
        drawCheckeredBackground(left, top, right, bottom);
        RectHelper.drawRect(left, top, right, bottom, colorSetting.getColorValue());

        if (isExpanded()) {
            // Draw Background
            Gui.drawRect(x + Panel.X_ITEM_OFFSET, y + height,
                    x + width - Panel.X_ITEM_OFFSET, y + getHeightWithExpand(),
            ...
            // Alpha Slider
            {
                ...
                RenderHelper.setColor(0xFF000000);
            ...
            {
                float xOff = COLOR_PICKER_HEIGHT + 14;
                float sLeft = x + xOff + 7;
                float sTop = y + height + 2;
                float sRight = sLeft + width - xOff - 7;
                float sBottom = y + height + (COLOR_PICKER_HEIGHT / 2.0F) + 8;

                RectHelper.drawRect(sLeft, sTop, sRight, sBottom + 31, 0xFF000000);

                drawCheckeredBackground(sLeft + 0.5F, sTop + 0.5F, sRight - 0.5F, sBottom + 30F);

                RectHelper.drawRect(sLeft + 3, sTop + 4, sRight - 3, sBottom + 28, colorSetting.getColorValue());
            }
        }
    }
