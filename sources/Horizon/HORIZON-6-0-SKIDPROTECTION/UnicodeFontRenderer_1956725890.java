...
public class UnicodeFontRenderer_1956725890 extends FontRenderer
{
    private UnicodeFont Ø;
    
    public UnicodeFontRenderer_1956725890(final String fontPath, final int size) {
        super(Minecraft.áŒŠà().ŠÄ, new ResourceLocation_1975012498("textures/font/ascii.png"), Minecraft.áŒŠà().¥à(), false);
        try {
            this.Ø = new UnicodeFont(fontPath, size, false, false);
        }
        catch (SlickException e) {
            this.Ø = new UnicodeFont(new Font("Arial", 0, size));
        }
        this.Ø.Â();
    ...
    @Override
    public int HorizonCode_Horizon_È(String string, int x, int y, final int color) {
        if (string == null) {
            return 0;
        }
        string = StringUtils.HorizonCode_Horizon_È(string);
        GL11.glPushMatrix();
