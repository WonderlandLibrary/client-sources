...
import io.github.liticane.clients.feature.module.impl.visual.Animations;
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0 || (Client.INSTANCE.getModuleManager().get(Aura.class).autoblock.isToggled() && (Client.INSTANCE.getModuleManager().get(Aura.class).autoblockm.is("test") || Client.INSTANCE.getModuleManager().get(Aura.class).autoblockm.is("Fake")) &&  MoveUtil.getHeldItem() instanceof ItemSword && Aura.target!=null))
                {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            if(Client.INSTANCE.getModuleManager().get(Animations.class).isToggled()) {
                                switch (Client.INSTANCE.getModuleManager().get(Animations.class).animations.getMode()) {
                                    case "Old":
                                        GlStateManager.popMatrix();
                                    ...
                                    case "Spin":
                                        ...
                                        doBlockTransformations();
                                        GlStateManager.rotate(-(System.currentTimeMillis() / 2 % 360), 0.0F, 1.0F, 1.5F);
