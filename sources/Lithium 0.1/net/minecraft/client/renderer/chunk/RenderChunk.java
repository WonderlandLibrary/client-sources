...
public class RenderChunk
{
    ...
    private EnumMap<EnumFacing, BlockPosition> mapEnumFacing = null;
    private BlockPosition[] positionOffsets16 = new BlockPosition[EnumFacing.VALUES.length];
    ...
    public RenderChunk(World worldIn, RenderGlobal renderGlobalIn, BlockPosition blockPositionIn, int indexIn)
    {
        this.world = worldIn;
        this.renderGlobal = renderGlobalIn;
        this.index = indexIn;

        if (!blockPositionIn.equals(this.getPosition()))
        {
            this.setPosition(blockPositionIn);
        }
    ...
    public void setPosition(BlockPosition pos)
    {
    ...
    public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator)
    {
        ...
        int i = 1;
        BlockPosition blockpos = new BlockPosition(this.position);
        BlockPosition blockpos1 = blockpos.add(15, 15, 15);
        ...
        if (!this.isChunkRegionEmpty(blockpos))
        {
            ...
            for (Object blockposm0 : BlockPositionM.getAllInBoxMutable(blockpos, blockpos1))
            {
                BlockPositionM blockposm = (BlockPositionM) blockposm0;
                ...
                {
                    TileEntity tileentity = chunkcacheof.getTileEntity(new BlockPosition(blockposm));
    ...
    private void preRenderBlocks(WorldRenderer worldRendererIn, BlockPosition pos)
    {
    ...
    public BlockPosition getBlockPosOffset16(EnumFacing p_181701_1_)
    {
        return this.getPositionOffset16(p_181701_1_);
    }

    public BlockPosition getPositionOffset16(EnumFacing p_getPositionOffset16_1_)
    {
        int i = p_getPositionOffset16_1_.getIndex();
        BlockPosition blockpos = this.positionOffsets16[i];
    ...
    protected RegionRenderCache createRegionRenderCache(World p_createRegionRenderCache_1_, BlockPosition p_createRegionRenderCache_2_, BlockPosition p_createRegionRenderCache_3_, int p_createRegionRenderCache_4_)
    {
    ...
    private ChunkCacheOF makeChunkCacheOF(BlockPosition p_makeChunkCacheOF_1_)
    {
        BlockPosition blockpos = p_makeChunkCacheOF_1_.add(-1, -1, -1);
        BlockPosition blockpos1 = p_makeChunkCacheOF_1_.add(16, 16, 16);
    ...
    public RenderChunk getRenderChunkOffset16(ViewFrustum p_getRenderChunkOffset16_1_, EnumFacing p_getRenderChunkOffset16_2_)
    {
        if (!this.renderChunksOffset16Updated)
        {
            for (int i = 0; i < EnumFacing.VALUES.length; ++i)
            {
                ...
                BlockPosition blockpos = this.getBlockPosOffset16(enumfacing);
    ...
    private Chunk getChunk(BlockPosition p_getChunk_1_)
    {
    ...
    private boolean isChunkRegionEmpty(BlockPosition p_isChunkRegionEmpty_1_)
    {
    ...
    public AabbFrame getBoundingBoxParent()
    {
        if (this.boundingBoxParent == null)
        {
            ...
            if (i1 != i || j1 != j || k1 != k)
            {
                AabbFrame aabbframe = this.renderGlobal.getRenderChunk(new BlockPosition(i1, j1, k1)).getBoundingBoxParent();
