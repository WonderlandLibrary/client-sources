...
extends Entity {
    public boolean hasNewVersionMoves;
    public boolean isLay;
    public boolean isNewSneak;
    public boolean noRenderArms = false;
    public NewPhisicsFixes newPhisicsFixes = new NewPhisicsFixes();
    ...
    public AnimationUtils smoothHealth = new AnimationUtils(20.0f, 20.0f, 0.1f);
    ...
    protected int ticksSinceLastSwingFixed;
    ...
    public static boolean isSunRiseDamaged = false;
    public static boolean isMatrixDamaged = false;
    public static boolean isNcpDamaged = false;
    public static TimerHelper fallTickers = new TimerHelper();
    public static float getElytraSpeed = 0.0f;
    ...
    public EntityLivingBase(World worldIn) {
        ...
        this.smoothHealth = new AnimationUtils(this.getHealth(), this.getHealth(), 0.1f);
    ...
    public boolean isChild() {
        SchoolBoy forMod = SchoolBoy.get;
        if (this.child) {
            return true;
        }
        if (forMod.actived) {
            String targets = forMod.currentMode("Targets");
            if (targets.equalsIgnoreCase("FullAll")) {
                return this != null;
            }
            if (targets.equalsIgnoreCase("All")) {
                return this instanceof EntityOtherPlayerMP;
            }
            if (targets.equalsIgnoreCase("Self&All")) {
                return this instanceof EntityPlayer;
            }
            if (targets.equalsIgnoreCase("Self")) {
                return this instanceof EntityPlayerSP || FreeCam.fakePlayer != null && this == FreeCam.fakePlayer;
            }
            if (targets.equalsIgnoreCase("Friends") || targets.equalsIgnoreCase("Self&Friends") || targets.equalsIgnoreCase("Self")) {
                for (Friend friend : Client.friendManager.getFriends()) {
                    if (!this.getName().equalsIgnoreCase(friend.getName())) continue;
                    return targets.equalsIgnoreCase("Friends") || targets.equalsIgnoreCase("Self&Friends");
                }
                return (this instanceof EntityPlayerSP || FreeCam.fakePlayer != null && this == FreeCam.fakePlayer) && (targets.equalsIgnoreCase("Self&Friends") || targets.equalsIgnoreCase("Self"));
            }
            return false;
        }
        return false;
    ...
    public float getDeathAlpha() {
        if (this != null && this.getHealth() == 0.0f) {
            return 1.0f - (float)this.deathTime / 19.0f;
        }
        return 1.0f;
    }
    ...
    public final float getHealth() {
        float hp = this.getMaxHealth();
        try {
            hp = this.dataManager.get(HEALTH).floatValue();
    ...
    public final float getSmoothHealth() {
        return this.smoothHealth.getAnim();
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        boolean flag2;
        if (source.isFireDamage() && source.getEntity() == Minecraft.player) {
    ...
    private boolean func_190628_d(DamageSource p_190628_1_) {
        ...
        for (EnumHand enumhand : EnumHand.values()) {
            ...
            if (itemstack1.getItem() != Items.TOTEM) continue;
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            if (LongJump.get.actived) {
                LongJump.isFallDamage = true;
                LongJump.doSpeed = false;
            }
    ...
    public final float getMaxHealth() {
        return this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH) == null ? 20.0f : (float)this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getAttributeValue();
    }
    ...
    private int getArmSwingAnimationEnd() {
        return ViewModel.getSwingSpeed(this, this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : 6);
    }

    public void getSwing(EnumHand hand) {
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (!(id == 2 || flag || flag1 || flag2)) {
            ...
            if (flag2) {
                ...
                if (Minecraft.player != null && damagesource.getEntity() == Minecraft.player) {
                    isSunRiseDamaged = false;
                    isMatrixDamaged = false;
                    isNcpDamaged = false;
                    fallTickers.reset();
                ...
                damagesource = flag1 ? DamageSource.drown : DamageSource.generic;
            }
            if ((soundevent = this.getHurtSound(damagesource)) != null) {
    ...
    protected void jump() {
        double boost;
        HighJump highjump = (HighJump)Client.moduleManager.getModule("HighJump");
        boolean custolLevel = highjump.actived && highjump.JumpMode.currentMode.equalsIgnoreCase("Normal") && highjump.GroundJump.bValue;
        this.motionY = boost = custolLevel ? (double)highjump.JumpPulse.fValue : (double)this.getJumpUpwardsMotion();
        if (this.isPotionActive(MobEffects.JUMP_BOOST)) {
            this.motionY += (double)((float)(this.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1f);
        }
        if (this.isSprinting()) {
            EventRotationJump e = new EventRotationJump(this.rotationYaw);
            if (this instanceof EntityPlayerSP) {
                e.call();
            }
            float f = (this instanceof EntityPlayerSP ? e.getYaw() : this.rotationYaw) * ((float)Math.PI / 180);
    ...
    protected void handleJumpWater() {
        this.motionY += this.hasNewVersionMoves ? (double)0.04f : (double)0.04f;
    }
    ...
    protected float getWaterSlowDown() {
        return (float)WaterSpeed.getWaterSlowDownMultiply(0.8f, this);
    }
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                    if (this.isElytraFlying()) {
                        ...
                        EventElytraVector eventElytra = new EventElytraVector(this.getLookVec(), this, this.rotationYaw, this.rotationPitch);
                        eventElytra.call();
                        Vec3d vec3d = eventElytra.getVector();
                        float f = eventElytra.getPitch() * ((float)Math.PI / 180);
                        ...
                        this.rotationYaw = eventElytra.getYaw();
                        this.rotationPitch = eventElytra.getPitch();
                        this.moveEntity(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
                        this.rotationYaw = prevYaw;
                        this.rotationPitch = prevPitch;
                        getElytraSpeed = (float)Math.sqrt(this.motionX * this.motionX + this.motionZ * this.motionZ);
                        ...
                        this.moveEntity(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
                        if ((this.isCollidedHorizontally || this.isJumping() && this.hasNewVersionMoves) && this.isOnLadder()) {
                            this.motionY = 0.2;
                        }
                ...
                boolean newVersion = this.hasNewVersionMoves;
                boolean fullSwim = this.isLay;
                double d0 = this.posY;
                ...
                if (newVersion) {
                    ...
                    if (f3 > 0.0f) {
                        ...
                        f2 = (float)((double)f2 + (WaterSpeed.AIMoveWaterSpeedMultiply(this.getAIMoveSpeed(), this) - (double)f2) * (double)f3 / 3.0);
                    }
                } else {
                    if (f3 > 3.0f) {
                        f3 = 3.0f;
                ...
                this.moveEntity(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
                boolean ladderFlag = this.isCollidedHorizontally && this.isOnLadder();
                if (ladderFlag) {
                    this.motionY = 0.2;
                }
                ...
                if (!ladderFlag && this.isSneaking()) {
                    this.motionY -= 0.04;
                }
                if (!this.hasNoGravity()) {
                    this.motionY -= 0.02;
                }
                if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX, this.motionY + (newVersion ? (double)0.6f : (double)0.6f) - this.posY + d0, this.motionZ)) {
                    this.motionY = newVersion ? (double)0.3f : (double)0.3f;
                }
            }
        }
    ...
    public void onUpdate() {
        this.smoothHealth.to = this.getHealth();
        this.hasNewVersionMoves = NewPhisicsFixes.isNewVersion() && this instanceof EntityPlayer;
        if (this.hasNewVersionMoves) {
            this.newPhisicsFixes.updateLayOrShift(this);
        } else if (this.isLay || this.isNewSneak || this.hasNewVersionMoves) {
            this.isLay = false;
            this.isNewSneak = false;
            this.hasNewVersionMoves = false;
        }
        super.onUpdate();
    ...
    public boolean EntityPlayer(Entity entityIn) {
        return this.world.rayTraceBlocks(new Vec3d(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3d(entityIn.posX, entityIn.posY + (double)entityIn.getEyeHeight(), entityIn.posZ), false, true, false) == null;
    }
    ...
    public Vec3d getLook(float partialTicks) {
        if (partialTicks == 1.0f) {
            return EntityLivingBase.getVectorForRotation(this.rotationPitchHead, this.rotationYawHead);
        }
    ...
    protected void updateItemUse(ItemStack stack, int eatingParticleCount) {
        if (!stack.func_190926_b() && this.isHandActive() && stack.getItemUseAction() == EnumAction.EAT) {
            if (!NoRender.get.actived || !NoRender.get.EatParticles.bValue) {
    ...
    public void stopActiveHand() {
        if (AutoApple.get.actived && AutoApple.get.canEat()) {
            return;
        }
        if (!this.activeItemStack.func_190926_b()) {
