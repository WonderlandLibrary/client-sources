...
import net.minecraft.cape.CapeHolder;
...
implements CapeHolder {
    public List<Trails.Track> tracks = new ArrayList<Trails.Track>();
    ...
    public final List<Trails.AkrienTr> trail = new CopyOnWriteArrayList<Trails.AkrienTr>();
    ...
    private final AnimationUtils extendEye = new AnimationUtils(1.8f, 1.8f, 0.08f);
    ...
    public float getDigSpeed(IBlockState state) {
        ...
        if (this.isPotionActive(MobEffects.MINING_FATIGUE)) {
            f *= (switch (this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) {
                case 0 -> 0.3f;
                case 1 -> 0.09f;
                case 2 -> 0.0027f;
                default -> 8.1E-4f;
    ...
    @Override
    public void jump() {
        if (Speed.snowGround && Speed.get.actived && MoveMeHelp.getSpeed() > 0.5) {
            return;
        }
        if (JesusSpeed.get.actived && (JesusSpeed.get.JesusMode.currentMode.equalsIgnoreCase("MatrixSolid3") || JesusSpeed.get.currentMode("Jesus Mode").equalsIgnoreCase("Matrix6.9.2")) && JesusSpeed.isJesused) {
            return;
        }
        if (TPExploit.get.actived || NoClip.get.actived && NoClip.isNoClip(Minecraft.player)) {
            return;
        }
        if (Scaffold.get.actived && Scaffold.get.currentBooleanValue("TowerMotion") && Scaffold.get.currentMode("Tower").equalsIgnoreCase("Strict") || ScaffWalk.get.actived && ScaffWalk.get.Elevator.currentMode.equalsIgnoreCase("Strict") && !MoveMeHelp.moveKeysPressed()) {
            return;
        }
        super.jump();
    ...
    public boolean isPushedByWater() {
        WaterSpeed.isFlowingWater = true;
        if (Velocity.get.actived && Velocity.get.NoWaterPush.bValue || FreeCam.get.actived) {
            return false;
        }
        return !this.capabilities.isFlying;
    }
    ...
    public float getEyeHeight() {
        if (this.hasNewVersionMoves) {
            ...
            if (this.isPlayerSleeping()) {
                newEye = 0.1f;
            } else if (this.isLay || this.isElytraFlying()) {
                newEye = 0.4f;
            } else if (this.isNewSneak || this.isSneaking()) {
                newEye = 1.25f;
            }
            if (SchoolBoy.isSetEyeHeightReduce(this)) {
                newEye /= 1.65f;
            }
            if (this instanceof EntityPlayerSP) {
                this.extendEye.to = newEye;
                return this.extendEye.getAnim();
            }
            return newEye;
        }
        float f = 1.62f;
        if (this.isPlayerSleeping()) {
            ...
            if ((this.isElytraFlying() || this.height == 0.6f) && !ElytraBoost.get.actived) {
                f = 0.4f;
            }
        } else {
            f -= 0.08f;
