...
public class MathHelper {
    ...
    public static float clampAngleFloat180(float angle) {
    ...
    public static float clampAngleFloat90(float angle) {
        if ((angle %= 90.0f) > 90.0f) {
            angle -= 180.0f;
        }
        if (angle < -90.0f) {
            angle += 180.0f;
    ...
    static {
        ...
        for (int k = 0; k < 360; k += 90) {
            MathHelper.SIN_TABLE_FAST[(int)((float)k * 11.377778f) & 0xFFF] = (float)Math.sin((float)k * ((float)Math.PI / 180));
        }
