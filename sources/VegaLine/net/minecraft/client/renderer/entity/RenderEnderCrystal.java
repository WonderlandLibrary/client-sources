...
extends Render<EntityEnderCrystal> {
    ...
    public void doRender(EntityEnderCrystal entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        float f1S = f1;
        Runnable renderModel = () -> this.modelEnderCrystalNoBase.render(entity, 0.0f, f * 3.0f, f1S * 0.2f, 0.0f, 0.0f, 0.0625f);
        for (Module m : Client.moduleManager.modules) {
            if (!m.actived) continue;
            m.preRenderLivingBase(entity, renderModel, false);
        }
        if (entity.shouldShowBottom()) {
            this.modelEnderCrystal.render(entity, 0.0f, f * 3.0f, f1S * 0.2f, 0.0f, 0.0f, 0.0625f);
        } else {
            renderModel.run();
        }
        for (Module m : Client.moduleManager.modules) {
            if (!m.actived) continue;
            m.postRenderLivingBase(entity, renderModel, false);
        }
        if (this.renderOutlines) {
