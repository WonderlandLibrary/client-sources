...
extends Render<EntityItem> {
    ...
    private final boolean isPhisics() {
        return WorldRender.get.isItemPhisics && !Panic.stop;
    }
    ...
    public void doRender(EntityItem entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!this.isPhisics()) {
            ...
            ItemStack itemstack = entity.getItem();
            double d = this.rotation = RenderEntityItem.mc.inGameHasFocus ? (double)(System.nanoTime() - this.tick) / 3000000.0 * 0.4 * RenderEntityItem.mc.timer.speed : 0.0;
            if (itemstack != null) {
                ...
                if (!(entity == null || Double.isNaN(entity.getAge()) || Double.isNaN(entity.getAir()) || Double.isNaN(entity.getEntityId()) || entity.getPosition() == null || entity.onGround)) {
                    ...
                    entity.rotationPitch = m3 | m1 == Material.WATER | m2 == Material.WATER | m4 ? (entity.rotationPitch = entity.rotationPitch + (float)(this.rotation / 4.0)) : (entity.rotationPitch = entity.rotationPitch + (float)(this.rotation * 2.0));
                }
                GlStateManager.translate(0.0, entity.fix + (itemstack.getItem() instanceof ItemBlock ? 0.1 : 0.0), 0.0);
                GL11.glRotatef(entity.rotationYaw, 0.0f, 1.0f, 0.0f);
                GL11.glRotatef(entity.rotationPitch + 90.0f + (float)(!(itemstack.getItem() instanceof ItemTool) && !(itemstack.getItem() instanceof ItemSword) ? 0 : 90), 1.0f, 0.0f, 0.0f);
                GL11.glRotatef(!(itemstack.getItem() instanceof ItemSword) ? 0 : 45, 0.0f, 0.0f, 1.0f);
                for (int l = 0; l < k; ++l) {
                    if (ibakedmodel.isAmbientOcclusion()) {
                        GlStateManager.pushMatrix();
                        GlStateManager.scale(0.3f, 0.3f, 0.3f);
                        this.renderItem.renderItem(itemstack, ibakedmodel);
                        GlStateManager.popMatrix();
                        continue;
                    }
                    GlStateManager.pushMatrix();
