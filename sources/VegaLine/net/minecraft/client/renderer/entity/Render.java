...
implements IEntityRenderer {
    ...
    protected boolean bindEntityTexture(T entity) {
        ...
        resourcelocation = WorldRender.get.updatedResourceSkin(resourcelocation, (Entity)entity);
        if (entity == TargetHUD.curTarget && resourcelocation != Minecraft.player.getLocationSkin()) {
            TargetHUD.skin = resourcelocation;
            if (TargetHUD.curTarget != null) {
                TargetHUD.OldSkin = resourcelocation;
            }
        }
        if (this.locationTextureCustom != null) {
            resourcelocation = this.locationTextureCustom;
        }
        if (entity == FreeCam.fakePlayer) {
            resourcelocation = Minecraft.player.getLocationSkin();
        }
    ...
    public void doRenderShadowAndFire(Entity entityIn, double x, double y, double z, float yaw, float partialTicks) {
        if (this.renderManager.options != null) {
            ...
            if (!(!entityIn.canRenderOnFire() || entityIn instanceof EntityPlayer && ((EntityPlayer)entityIn).isSpectator() || NoRender.get.actived && NoRender.get.FireOnEntity.bValue)) {
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
        ...
        if (d0 <= (double)(maxDistance * maxDistance)) {
            ...
            boolean isChild = entityIn instanceof EntityLivingBase && (base = (EntityLivingBase)entityIn).isChild();
            float f2 = ((Entity)entityIn).height / (isChild ? 1.8f : 1.0f) + 0.3f - (flag ? 0.25f : 0.0f);
