...
public class BufferBuilder {
    ...
    public void putColorMultiplier(float red, float green, float blue, int vertexIndex) {
        ...
        if (!this.noColor) {
            ...
            if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
                ...
                j &= 0xFF000000;
                j = j | i1 << 16 | l << 8 | k;
                j = ColorUtils.swapAlpha(j, (float)ColorUtils.getAlphaFromColor(j) * Block.blockModelAlphaMul());
            } else {
    ...
    public BufferBuilder pos(Vec3d vec) {
    ...
    public BufferBuilder pos(double x, double y) {
