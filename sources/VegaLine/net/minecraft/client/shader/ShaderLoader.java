...
public class ShaderLoader {
    ...
    private ShaderLoader(ShaderType type2, int shaderId, String filename) {
     ...
     */
    public static ShaderLoader loadShader(IResourceManager resourceManager, ShaderType type2, String filename) throws IOException {
        ShaderLoader shaderloader = type2.getLoadedShaders().get(filename);
        if (shaderloader == null) {
            ResourceLocation resourcelocation = new ResourceLocation("shaders/program/" + filename + type2.getShaderExtension());
            ...
            try {
                ...
                int i = OpenGlHelper.glCreateShader(type2.getShaderMode());
                ...
                if (OpenGlHelper.glGetShaderi(i, OpenGlHelper.GL_COMPILE_STATUS) == 0) {
                    ...
                    JsonException jsonexception = new JsonException("Couldn't compile " + type2.getShaderName() + " program: " + s);
                    jsonexception.setFilenameAndFlush(resourcelocation.getResourcePath());
                    throw jsonexception;
                }
                shaderloader = new ShaderLoader(type2, i, filename);
                type2.getLoadedShaders().put(filename, shaderloader);
            } finally {
