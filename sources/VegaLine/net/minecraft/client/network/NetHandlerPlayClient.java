...
import ru.govno.client.event.events.EventCollectItem;
...
implements INetHandlerPlayClient {
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
        ...
        Vec3d velocity = new Vec3d((double)packetIn.getMotionX() / 8000.0, (double)packetIn.getMotionY() / 8000.0, (double)packetIn.getMotionZ() / 8000.0);
        if (entity == null) {
            return;
        }
        if (entity instanceof EntityPlayerSP && !Panic.stop && Velocity.get.actived && Velocity.get.OnKnockBack.bValue && !Velocity.pass) {
            Velocity.abuseVelocity(entity, velocity);
            return;
        }
        entity.setVelocity(velocity.xCoord, velocity.yCoord, velocity.zCoord);
    }

    @Override
    public void handleEntityMetadata(SPacketEntityMetadata packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        if (this.clientWorldController.getEntityByID(packetIn.getEntityId()) == null) {
            return;
        }
        Entity entity = this.clientWorldController.getEntityByID(packetIn.getEntityId());
    ...
    public void handleSpawnPlayer(SPacketSpawnPlayer packetIn) {
        ...
        if (this.getPlayerInfo(packetIn.getUniqueId()) == null || this.getPlayerInfo(packetIn.getUniqueId()).getGameProfile() == null) {
            return;
        }
    ...
    public void handlePlayerPosLook(SPacketPlayerPosLook packetIn) {
        ...
        boolean bl = hasPearlPerFlag = entityplayer instanceof EntityPlayerSP && Bypass.get.canFixPearlFlag() && MoveMeHelp.getCuttingSpeed() <= 0.25 && Minecraft.getMinecraft().world.getLoadedEntityList().stream().filter(entity -> entity instanceof EntityEnderPearl).filter(Objects::nonNull).anyMatch(pearl -> pearl.getDistanceToVec3d(new Vec3d(packetIn.getX(), packetIn.getY(), packetIn.getZ())) < 2.40112);
        if (packetIn.getFlags().contains((Object)SPacketPlayerPosLook.EnumFlags.X)) {
            d0 += entityplayer.posX;
        } else if (!hasPearlPerFlag) {
        ...
        if (Bypass.get.canCancelServerRots()) {
            Bypass.get.callServerRotsSpoof(f, f1);
        ...
        if (!Panic.stop && hasPearlPerFlag) {
            Minecraft mc = Minecraft.getMinecraft();
            Minecraft.player.motionX = -0.13 + 0.26 * Math.random();
            Minecraft.player.motionX = MathUtils.getDifferenceOf(Minecraft.player.motionX, 0.0) < 0.12 ? (Minecraft.player.motionX > 0.0 ? 0.12 : -0.12) : Minecraft.player.motionX;
            Minecraft.player.motionZ = -0.13 + 0.26 * Math.random();
            double d = MathUtils.getDifferenceOf(Minecraft.player.motionZ, 0.0) < 0.12 ? (Minecraft.player.motionZ > 0.0 ? 0.12 : -0.12) : (Minecraft.player.motionZ = Minecraft.player.motionZ);
        }
        if (!Panic.stop && Minecraft.player != null && Minecraft.getMinecraft().world != null && Minecraft.player == Minecraft.player) {
            Minecraft.getMinecraft().world.getLoadedEntityList().forEach(ents -> {
                if (ents != null && ents instanceof EntityEnderPearl) {
                    EntityEnderPearl pearl = (EntityEnderPearl)ents;
                    Vec3d vec3d = new Vec3d(packetIn.getX(), packetIn.getY(), packetIn.getZ());
                    if (pearl.getDistanceToVec3d(vec3d) < 3.40112) {
                        EntityLivingBase.isMatrixDamaged = false;
                        EntityLivingBase.isNcpDamaged = false;
                        EntityLivingBase.isSunRiseDamaged = false;
                        EntityLivingBase.fallTickers.reset();
                    }
                }
            });
        }
    ...
    public void sendPacket(Packet<?> packetIn) {
        EventSendPacket DICK = new EventSendPacket(packetIn);
        DICK.call();
        if (packetIn != null && !DICK.isCancelled()) {
            this.netManager.sendPacket(packetIn);
        }
    }

    public void preSendPacket(Packet<?> packetIn) {
        if (packetIn != null) {
            this.netManager.sendPacket(packetIn);
        }
    }
    ...
    public void handleCollectItem(SPacketCollectItem packetIn) {
        ...
        if (entity instanceof EntityItem) {
            ...
            EventCollectItem COLLECT_EVENT = new EventCollectItem(EItem, entitylivingbase);
            COLLECT_EVENT.call();
            if (COLLECT_EVENT.isCancelled()) {
                return;
            }
        }
        if (entity instanceof EntityItem) {
            EItem = (EntityItem)entity;
            Hud.get.onCollect(EItem, entitylivingbase);
        }
        if (entity != null) {
    ...
    public void handleSetPassengers(SPacketSetPassengers packetIn) {
        ...
        if (entity == null) {
            ...
            for (int i : packetIn.getPassengerIds()) {
                ...
                if (entity1 != Minecraft.player || flag) continue;
    ...
    public void handleEntityStatus(SPacketEntityStatus packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 21) {
                ...
                if (packetIn instanceof SPacketEntityStatus && (packet = packetIn).getOpCode() == 35 && packet.getEntity(Minecraft.getMinecraft().world) == Minecraft.player && OffHand.get.actived) {
                    OffHand.scaleAnim.setAnim(0.6f);
                    OffHand.scaleAnim.to = 1.05f;
                    OffHand.popAnim.setAnim(1.0f);
                    OffHand.totemBackward = true;
                    OffHand.timer4.reset();
                    if (WorldRender.get.isActived() && WorldRender.get.ClientCamera.bValue) {
                        WorldRender.get.fovMultiplier.setAnim(WorldRender.get.fovMultiplier.anim + 0.4f);
                    }
                }
                int i = 40;
                ...
                if (!(entity != Minecraft.player || NoRender.get.actived && NoRender.get.TotemOverlay.bValue)) {
                    ItemStack stack = null;
                    stack = Minecraft.player.getHeldItemOffhand().getItem() == Items.TOTEM ? Minecraft.player.getHeldItemOffhand() : (Minecraft.player.getHeldItemMainhand().getItem() == Items.TOTEM ? Minecraft.player.getHeldItemMainhand() : new ItemStack(Items.TOTEM, 1));
                    this.gameController.entityRenderer.func_190565_a(stack);
                }
            } else {
    ...
    public void handleExplosion(SPacketExplosion packetIn) {
        ...
        if (Minecraft.player instanceof EntityPlayerSP && !Panic.stop && Velocity.get != null && Velocity.get.actived && Velocity.get.OnKnockBack.bValue) {
            Velocity.abuseVelocity(Minecraft.player, new Vec3d(packetIn.getMotionX(), packetIn.getMotionY(), packetIn.getMotionZ()));
        } else {
            Minecraft.player.motionX += (double)packetIn.getMotionX();
            Minecraft.player.motionY += (double)packetIn.getMotionY();
            Minecraft.player.motionZ += (double)packetIn.getMotionZ();
        }
    }
    ...
    public void handleChangeGameState(SPacketChangeGameState packetIn) {
        ...
        if (i == 1) {
            ...
            if (j == 0) {
                ...
                this.gameController.displayGuiScreen(new GuiWinGame(true, () -> Minecraft.player.connection.sendPacket(new CPacketClientStatus(CPacketClientStatus.State.PERFORM_RESPAWN))));
            }
        } else if (i == 5) {
    ...
    public void handleCombatEvent(SPacketCombatEvent packetIn) {
        Entity entity;
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        if (packetIn.eventType == SPacketCombatEvent.Event.ENTITY_DIED && (entity = this.clientWorldController.getEntityByID(packetIn.playerId)) == Minecraft.player) {
    ...
    public void handlePlayerAbilities(SPacketPlayerAbilities packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        EntityPlayerSP entityplayer1 = Minecraft.player;
    ...
    public void handleResourcePack(SPacketResourcePackSend packetIn) {
        ...
        if (this.validateResourcePackUrl(s)) {
            if (s.startsWith("level://")) {
                ...
                String s2 = URLDecoder.decode((String)s.substring("level://".length()), (Charset)StandardCharsets.UTF_8);
    ...
    public void handleCustomPayload(SPacketCustomPayload packetIn) {
        ...
        if ("MC|TrList".equals(packetIn.getChannelName())) {
            ...
            try {
                ...
                GuiScreen guiscreen = this.gameController.currentScreen;
                if (guiscreen == null || !(guiscreen instanceof GuiMerchant) || k != Minecraft.player.openContainer.windowId) return;
            ...
            Minecraft.player.setServerBrand(packetIn.getBufferData().readStringFromBuffer(Short.MAX_VALUE));
            return;
        } else if ("MC|BOpen".equals(packetIn.getChannelName())) {
            ItemStack itemstack;
            EnumHand enumhand = packetIn.getBufferData().readEnumValue(EnumHand.class);
            ItemStack itemStack = itemstack = enumhand == EnumHand.OFF_HAND ? Minecraft.player.getHeldItemOffhand() : Minecraft.player.getHeldItemMainhand();
    ...
    public void handleScoreboardObjective(SPacketScoreboardObjective packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Scoreboard scoreboard = this.clientWorldController.getScoreboard();
        if (packetIn.getAction() == 0 && scoreboard != null) {
            ...
            if (packetIn.getAction() == 1 && scoreobjective1 != null) {
                scoreboard.removeObjective(scoreobjective1);
            } else if (packetIn.getAction() == 2 && scoreobjective1 != null) {
    ...
    public void func_194307_a(SPacketPlaceGhostRecipe p_194307_1_) {
        ...
        if (container.windowId == p_194307_1_.func_194313_b() && container.getCanCraft(Minecraft.player) && this.gameController.currentScreen instanceof IRecipeShownListener) {
