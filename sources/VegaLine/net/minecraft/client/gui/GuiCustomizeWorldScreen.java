...
GuiPageButtonList.GuiResponder {
    ...
    public void initGui() {
        ...
        if (this.list != null) {
            i = this.list.getPage();
            j = (int)this.list.getAmountScrolled();
        }
        ...
        this.nextPage = this.addButton(new GuiButton(303, width - 100, 5, 80, 20, I18n.format("createWorld.customize.custom.next", new Object[0])));
        this.defaults = this.addButton(new GuiButton(304, width / 2 - 187, height - 27, 90, 20, I18n.format("createWorld.customize.custom.defaults", new Object[0])));
        this.randomize = this.addButton(new GuiButton(301, width / 2 - 92, height - 27, 90, 20, I18n.format("createWorld.customize.custom.randomize", new Object[0])));
        this.presets = this.addButton(new GuiButton(305, width / 2 + 3, height - 27, 90, 20, I18n.format("createWorld.customize.custom.presets", new Object[0])));
        this.done = this.addButton(new GuiButton(300, width / 2 + 98, height - 27, 90, 20, I18n.format("gui.done", new Object[0])));
        this.defaults.enabled = this.settingsModified;
        this.confirm = new GuiButton(306, width / 2 - 55, 160, 50, 20, I18n.format("gui.yes", new Object[0]));
        this.confirm.visible = false;
        this.buttonList.add(this.confirm);
        this.cancel = new GuiButton(307, width / 2 + 5, 160, 50, 20, I18n.format("gui.no", new Object[0]));
    ...
    private void createPagedList() {
        ...
        this.list = new GuiPageButtonList(this.mc, width, height, 32, height - 32, 25, this, aguipagebuttonlist$guilistentry, aguipagebuttonlist$guilistentry1, aguipagebuttonlist$guilistentry2, aguipagebuttonlist$guilistentry3);
        for (int i = 0; i < 4; ++i) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawCenteredString(this.fontRendererObj, this.title, width / 2, 2, 0xFFFFFF);
        this.drawCenteredString(this.fontRendererObj, this.subtitle, width / 2, 12, 0xFFFFFF);
        this.drawCenteredString(this.fontRendererObj, this.pageTitle, width / 2, 22, 0xFFFFFF);
        super.drawScreen(mouseX, mouseY, partialTicks);
        if (this.confirmMode != 0) {
            GuiCustomizeWorldScreen.drawRect(0, 0.0, (double)width, (double)height, Integer.MIN_VALUE);
            ...
            bufferbuilder.pos(width / 2 - 90, 185.0, 0.0).tex(0.0, 2.65625).color(64, 64, 64, 64).endVertex();
            bufferbuilder.pos(width / 2 + 90, 185.0, 0.0).tex(5.625, 2.65625).color(64, 64, 64, 64).endVertex();
            bufferbuilder.pos(width / 2 + 90, 100.0, 0.0).tex(5.625, 0.0).color(64, 64, 64, 64).endVertex();
            bufferbuilder.pos(width / 2 - 90, 100.0, 0.0).tex(0.0, 0.0).color(64, 64, 64, 64).endVertex();
            tessellator.draw();
