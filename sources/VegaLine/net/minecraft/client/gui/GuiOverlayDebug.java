...
extends Gui {
    ...
    protected void renderDebugInfoLeft() {
        ...
        if (Panic.stop || !ComfortUi.get.isBetterDebugF3()) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                int l = 2;
                int i1 = 2 + j * i;
                GuiOverlayDebug.drawRect(1, (double)(i1 - 1), (double)(2 + k + 1), (double)(i1 + j - 1), -1873784752);
            ...
            list.add("For help: press F3 + Q");
            GL11.glPushMatrix();
            GL11.glTranslated((float)(-sr.getScaledWidth() / 4) * GuiIngame.sizer, -GuiIngame.sizer * 64.0f, 0.0);
            for (int i = 0; i < list.size(); ++i) {
                String s = list.get(i);
                if (Strings.isNullOrEmpty(s)) continue;
                int j = 8;
                int k = Fonts.mntsb_15.getStringWidth(s);
                int l = 2;
                int i1 = 2 + j * i;
                int color = 0xE0E0E0;
                if (s.startsWith("XYZ:")) {
                    color = 1748624;
                    RenderUtils.drawGradientSideways(1.0, i1 - 1, 2 + k + 1, i1 + j - 1, ColorUtils.getColor(155, 255, 225, 70), ColorUtils.getColor(0, 0, 125, 0));
                } else {
                    RenderUtils.drawGradientSideways(1.0, i1 - 1, 2 + k + 1, i1 + j - 1, ColorUtils.getColor(210, 210, 225, 110), 0);
                }
                Fonts.mntsb_12.drawString(s, 2.0, i1 + 2, color);
            }
            GL11.glPopMatrix();
        }
    }
    ...
    protected void renderDebugInfoRight(ScaledResolution scaledRes) {
        ...
        if (Panic.stop || !ComfortUi.get.isBetterDebugF3()) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                int j = this.fontRenderer.FONT_HEIGHT;
                int k = this.fontRenderer.getStringWidth(s);
                int l = sr.getScaledWidth() - 2 - k;
                int i1 = 2 + j * i;
                GuiOverlayDebug.drawRect(l - 1, (double)(i1 - 1), (double)(l + k + 1), (double)(i1 + j - 1), -1873784752);
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (Strings.isNullOrEmpty(s)) continue;
                int j = 8;
                int k = Fonts.mntsb_12.getStringWidth(s);
                int l = scaledRes.getScaledWidth() - 2 - k;
                int i1 = 2 + j * i;
                RenderUtils.drawGradientSideways(l - 1, i1 - 1, l + k + 1, i1 + j - 1, 0, ColorUtils.getColor(210, 210, 225, 110));
                Fonts.mntsb_12.drawString(s, l, i1 + 2, 0xE0E0E0);
            }
            GL11.glPopMatrix();
        }
    }
    ...
    protected List<String> call() {
        ...
        if (this.mc.isReducedDebug()) {
            return Lists.newArrayList(ClientBrandRetriever.getDebugVersionString(), this.mc.debug, this.mc.renderGlobal.getDebugInfoRenders(), this.mc.renderGlobal.getDebugInfoEntities(), "P: " + this.mc.effectRenderer.getStatistics() + ". T: " + this.mc.world.getDebugLoadedEntities(), this.mc.world.getProviderName(), "", String.format("Chunk-relative: %d %d %d", blockpos.getX() & 0xF, blockpos.getY() & 0xF, blockpos.getZ() & 0xF));
        }
        ...
        if (this.mc.world != null) {
            ...
            if (this.mc.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256) {
                if (!chunk.isEmpty()) {
                    ...
                    if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null && (entityplayermp = this.mc.getIntegratedServer().getPlayerList().getPlayerByUUID(Minecraft.player.getUniqueID())) != null) {
    ...
    protected <T extends Comparable<T>> List<String> getDebugInfoRight() {
        ...
        if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK && this.mc.objectMouseOver.getBlockPos() != null) {
            ...
            for (Map.Entry entry : iblockstate.getProperties().entrySet()) {
                ...
                Object s = iproperty.getName(t);
                if (Boolean.TRUE.equals(t)) {
                    s = TextFormatting.GREEN + (String)s;
                } else if (Boolean.FALSE.equals(t)) {
                    s = TextFormatting.RED + (String)s;
                }
                list.add(iproperty.getName() + ": " + (String)s);
            }
        }
        return list;
    }
