...
implements ISoundEventListener {
    ...
    public void renderSubtitles(ScaledResolution resolution) {
        ...
        if (this.enabled && !this.subtitles.isEmpty()) {
            ...
            Vec3d vec3d1 = new Vec3d(0.0, 0.0, -1.0).rotatePitch(-Minecraft.player.rotationPitch * ((float)Math.PI / 180)).rotateYaw(-Minecraft.player.rotationYaw * ((float)Math.PI / 180));
            Vec3d vec3d2 = new Vec3d(0.0, 1.0, 0.0).rotatePitch(-Minecraft.player.rotationPitch * ((float)Math.PI / 180)).rotateYaw(-Minecraft.player.rotationYaw * ((float)Math.PI / 180));
            ...
            for (Subtitle guisubtitleoverlay$subtitle1 : this.subtitles) {
                ...
                int l1 = MathHelper.floor(MathHelper.clampedLerp(255.0, 75.0, (float)(Minecraft.getSystemTime() - guisubtitleoverlay$subtitle1.getStartTime()) / 3000.0f));
                ...
                GuiSubtitleOverlay.drawRect(-l - 1, (double)(-j1 - 1), (double)(l + 1), (double)(j1 + 1), -872415232);
                GlStateManager.enableBlend();
                if (!flag) {
                    if (d0 > 0.0) {
