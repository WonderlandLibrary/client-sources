    ...
    public static void addPotionTooltip(ItemStack itemIn, List<String> lores, float durationFactor) {
        ...
        if (list.isEmpty()) {
            ...
            for (PotionEffect potionEffect : list) {
                Object s1 = I18n.translateToLocal(potionEffect.getEffectName()).trim();
                ...
                if (potionEffect.getAmplifier() > 0) {
                    s1 = (String)s1 + " " + I18n.translateToLocal("potion.potency." + potionEffect.getAmplifier()).trim();
                }
                if (potionEffect.getDuration() > 20) {
                    s1 = (String)s1 + " (" + Potion.getPotionDurationString(potionEffect, durationFactor) + ")";
                }
                if (potion.isBadEffect()) {
                    lores.add(TextFormatting.RED + (String)s1);
                    continue;
                }
                lores.add(TextFormatting.BLUE + (String)s1);
            }
        }
        if (!list1.isEmpty()) {
            lores.add("");
            ...
            for (Tuple tuple : list1) {
                ...
                if (!(d0 < 0.0)) continue;
                lores.add(TextFormatting.RED + I18n.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1 *= -1.0), I18n.translateToLocal("attribute.name." + (String)tuple.getFirst())));
            }
        }
    }
}
