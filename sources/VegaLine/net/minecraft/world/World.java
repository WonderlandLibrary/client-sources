...
implements IBlockAccess {
    ...
    public boolean isObsidOrBdBlock(BlockPos pos) {
        return this.getBlockState(pos).getBlock() == Blocks.OBSIDIAN || this.getBlockState(pos).getBlock() == Blocks.BEDROCK;
    }
    ...
    public int getLight(BlockPos pos, boolean checkNeighbors) {
        int scaleW;
        int n = scaleW = this.getWorldBorder() == null ? 30000000 : this.getWorldBorder().getSize();
        if (pos.getX() >= -scaleW && pos.getZ() >= -scaleW && pos.getX() < scaleW && pos.getZ() < scaleW) {
    ...
    public int getHeight(int x, int z) {
        int scaleW;
        int n = scaleW = this.getWorldBorder() == null ? 30000000 : this.getWorldBorder().getSize();
        int i = x >= -scaleW && z >= -scaleW && x < scaleW && z < scaleW ? (this.isChunkLoaded(x >> 4, z >> 4, true) ? this.getChunkFromChunkCoords(x >> 4, z >> 4).getHeightValue(x & 0xF, z & 0xF) : 0) : this.getSeaLevel() + 1;
        return i;
    }
    ...
    public int getChunksLowestHorizon(int x, int z) {
        int scaleW;
        int n = scaleW = this.getWorldBorder() == null ? 30000000 : this.getWorldBorder().getSize();
        if (x >= -scaleW && z >= -scaleW && x < scaleW && z < scaleW) {
    ...
    public int getLightFromNeighborsFor(EnumSkyBlock type2, BlockPos pos) {
        if (!this.provider.func_191066_m() && type2 == EnumSkyBlock.SKY) {
            return 0;
        }
        if (pos.getY() < 0) {
        ...
        if (!this.isValid(pos)) {
            return type2.defaultLightValue;
        }
        if (!this.isBlockLoaded(pos)) {
            return type2.defaultLightValue;
        }
        if (this.getBlockState(pos).useNeighborBrightness()) {
            int i1 = this.getLightFor(type2, pos.up());
            int i = this.getLightFor(type2, pos.east());
            int j = this.getLightFor(type2, pos.west());
            int k = this.getLightFor(type2, pos.south());
            int l = this.getLightFor(type2, pos.north());
            if (i > i1) {
                i1 = i;
            }
            if (j > i1) {
        ...
        Chunk chunk = this.getChunkFromBlockCoords(pos);
        return chunk.getLightFor(type2, pos);
    }

    public int getLightFor(EnumSkyBlock type2, BlockPos pos) {
        if (pos.getY() < 0) {
            pos = new BlockPos(pos.getX(), 0, pos.getZ());
        }
        if (!this.isValid(pos)) {
    ...
    public void setLightFor(EnumSkyBlock type2, BlockPos pos, int lightValue) {
        if (this.isValid(pos) && this.isBlockLoaded(pos)) {
            Chunk chunk = this.getChunkFromBlockCoords(pos);
            chunk.setLightFor(type2, pos, lightValue);
    ...
    private void spawnParticle(int particleID, boolean ignoreRange, double xCood, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, int ... parameters) {
        for (int i = 0; i < this.eventListeners.size(); ++i) {
            if (particleID == 7 && Minecraft.player != null && Minecraft.player.getDistance(xCood, yCoord + 3.0, zCoord) > 6.0) continue;
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        ...
        if (forceUpdate && entityIn.addedToChunk) {
            ...
            if (entityIn.isRiding()) {
                ...
                entityIn.onUpdate();
                if (!Panic.stop && entityIn instanceof EntityLivingBase) {
                    EntityLivingBase base = (EntityLivingBase)entityIn;
                    if (prevPosX != entityIn.posX || prevPosY != entityIn.posY || prevPosZ != entityIn.posZ) {
                        if (JumpCircles.get.actived) {
                            JumpCircles.onEntityMove(base, new Vec3d(prevPosX, prevPosY, prevPosZ));
                        }
                        ((DashTrail)DashTrail.dash).onEntityMove(base, new Vec3d(prevPosX, prevPosY, prevPosZ));
                    }
                }
            }
        }
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos) {
        EventLightingCheck event = new EventLightingCheck(lightType, pos);
        event.call();
    ...
    public float getRainStrength(float delta) {
        return WorldRender.get.weatherReStrengh(this.prevRainingStrength + (this.rainingStrength - this.prevRainingStrength) * delta);
    }
    ...
    public void playEvent(int type2, BlockPos pos, int data) {
        this.playEvent(null, type2, pos, data);
    }

    public void playEvent(@Nullable EntityPlayer player, int type2, BlockPos pos, int data) {
        try {
            for (int j2 = 0; j2 < this.eventListeners.size(); ++j2) {
                this.eventListeners.get(j2).playEvent(player, type2, pos, data);
            }
            ...
            crashreportcategory3.addCrashSection("Event type", type2);
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private boolean func_191504_a2(@Nullable Entity p_191504_1_, AxisAlignedBB p_191504_2_, boolean p_191504_3_, @Nullable List<AxisAlignedBB> p_191504_4_) {
        int i = MathHelper.floor(p_191504_2_.minX) - 1;
        int j = MathHelper.ceil(p_191504_2_.maxX) + 1;
        int k = MathHelper.floor(p_191504_2_.minY) - 1;
        int l = MathHelper.ceil(p_191504_2_.maxY) + 1;
