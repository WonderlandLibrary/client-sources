...
implements IBlockAccess {
    ...
    public int getCombinedLight(BlockPos pos, int lightValue) {
        int i = MathUtils.clamp(this.getLightForExt(EnumSkyBlock.SKY, pos), 0, Integer.MAX_VALUE);
        int j = MathUtils.clamp(this.getLightForExt(EnumSkyBlock.BLOCK, pos), 0, Integer.MAX_VALUE);
        if (j < lightValue) {
            j = lightValue;
        }
        return i << 20 | j << 4;
    ...
    private int getLightForExt(EnumSkyBlock type2, BlockPos pos) {
        if (type2 == EnumSkyBlock.SKY && !this.worldObj.provider.func_191066_m()) {
            return 0;
        }
        if (pos.getY() >= 0 && pos.getY() < 256) {
            if (this.getBlockState(pos).useNeighborBrightness()) {
                ...
                for (EnumFacing enumfacing : EnumFacing.values()) {
                    int k = this.getLightFor(type2, pos.offset(enumfacing));
                    if (k > l) {
                        l = k;
                    }
                    if (l < 15) continue;
            ...
            int i = MathUtils.clamp((pos.getX() >> 4) - this.chunkX, 0, Integer.MAX_VALUE);
            int j = MathUtils.clamp((pos.getZ() >> 4) - this.chunkZ, 0, Integer.MAX_VALUE);
            return this.chunkArray[i][j].getLightFor(type2, pos);
        }
        return type2.defaultLightValue > 0 ? type2.defaultLightValue : 0;
    }
