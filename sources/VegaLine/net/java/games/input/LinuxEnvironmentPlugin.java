...
implements Plugin {
    ...
    private static final Controller createJoystickFromDevice(LinuxEventDevice device, Component[] components, Controller.Type type2) throws IOException {
        LinuxAbstractController joystick = new LinuxAbstractController(device, components, new Controller[0], device.getRumblers(), type2);
        return joystick;
    }
    ...
    private static final Controller createControllerFromDevice(LinuxEventDevice device) throws IOException {
        ...
        Controller.Type type2 = device.getType();
        if (type2 == Controller.Type.MOUSE) {
            return LinuxEnvironmentPlugin.createMouseFromDevice(device, components);
        }
        if (type2 == Controller.Type.KEYBOARD) {
            return LinuxEnvironmentPlugin.createKeyboardFromDevice(device, components);
        }
        if (type2 == Controller.Type.STICK || type2 == Controller.Type.GAMEPAD) {
            return LinuxEnvironmentPlugin.createJoystickFromDevice(device, components, type2);
        }
        return null;
    }
