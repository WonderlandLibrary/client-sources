...
public class ExpressionParser {
    ...
    private IExpression makeFunction(EnumFunctionType type2, Deque<Token> deque) throws ParseException {
        if (type2.getCountArguments() == 0) {
            return ExpressionParser.makeFunction(type2, new IExpression[0]);
        }
        Token token = deque.poll();
        ...
        return ExpressionParser.makeFunction(type2, aiexpression);
    }
    ...
    private static IExpression makeFunction(EnumFunctionType type2, IExpression[] exprs) throws ParseException {
        if (type2.getCountArguments() != exprs.length) {
            throw new ParseException("Invalid number of arguments: " + exprs.length + ", should be: " + type2.getCountArguments() + ", function: " + type2.getName());
        }
        return new Function(type2, exprs);
    }
