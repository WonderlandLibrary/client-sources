...
implements ChannelFactory<T> {
    ...
    static ServerSocketChannelUDT newAcceptorChannelUDT(TypeUDT type2) {
        try {
            return SelectorProviderUDT.from((TypeUDT)type2).openServerSocketChannel();
        } catch (IOException e) {
            throw new ChannelException("failed to open a server socket channel", e);
        }
    }

    static SocketChannelUDT newConnectorChannelUDT(TypeUDT type2) {
        try {
            return SelectorProviderUDT.from((TypeUDT)type2).openSocketChannel();
        } catch (IOException e) {
            throw new ChannelException("failed to open a socket channel", e);
        }
    }

    static RendezvousChannelUDT newRendezvousChannelUDT(TypeUDT type2) {
        try {
            return SelectorProviderUDT.from((TypeUDT)type2).openRendezvousChannel();
        } catch (IOException e) {
    ...
    private NioUdtProvider(TypeUDT type2, KindUDT kind) {
        this.type = type2;
        this.kind = kind;
    }
