...
public class SpdyFrameDecoder {
            ...
            switch (this.state) {
                case READ_COMMON_HEADER: {
                    ...
                    if (control) {
                        ...
                        type2 = SpdyCodecUtil.getUnsignedShort(buffer, frameOffset + 2);
                        this.streamId = 0;
                    } else {
                    ...
                    if (!SpdyFrameDecoder.isValidFrameHeader(this.streamId, type2, this.flags, this.length)) {
                        this.state = State.FRAME_ERROR;
                        this.delegate.readFrameError("Invalid Frame Error");
                        continue block16;
                    }
                    this.state = SpdyFrameDecoder.getNextState(type2, this.length);
                    continue block16;
                }
    ...
    private static State getNextState(int type2, int length) {
        switch (type2) {
            case 0: {
    ...
    private static boolean isValidFrameHeader(int streamId, int type2, byte flags, int length) {
        switch (type2) {
            case 0: {
                return streamId != 0;
            }
