...
implements DnsRecordDecoder {
    ...
    public final DnsQuestion decodeQuestion(ByteBuf in) throws Exception {
        ...
        return new DefaultDnsQuestion(name, type2, qClass);
    }
    ...
    public final <T extends DnsRecord> T decodeRecord(ByteBuf in) throws Exception {
        ...
        DnsRecord record = this.decodeRecord(name, type2, aClass, ttl, in, offset, length);
        in.readerIndex(offset + length);
        return (T)record;
    }

    protected DnsRecord decodeRecord(String name, DnsRecordType type2, int dnsClass, long timeToLive, ByteBuf in, int offset, int length) throws Exception {
        ...
        return new DefaultDnsRawRecord(name, type2, dnsClass, timeToLive, in.retainedDuplicate().setIndex(offset, offset + length));
    }
