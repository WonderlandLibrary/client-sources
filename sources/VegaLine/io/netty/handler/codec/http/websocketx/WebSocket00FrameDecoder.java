...
implements WebSocketFrameDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        WebSocketFrame frame = (type2 & 0x80) == 128 ? this.decodeBinaryFrame(ctx, type2, in) : this.decodeTextFrame(ctx, in);
        if (frame != null) {
            out.add(frame);
        }
    }

    private WebSocketFrame decodeBinaryFrame(ChannelHandlerContext ctx, byte type2, ByteBuf buffer) {
        byte b;
        long frameSize = 0L;
