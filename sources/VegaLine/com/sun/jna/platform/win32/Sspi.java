...
extends StdCallLibrary {
    ...
    extends Structure {
        ...
        public SecBufferDesc(int type2, byte[] token) {
            SecBuffer.ByReference secBuffer = new SecBuffer.ByReference(type2, token);
            this.pBuffers = (SecBuffer.ByReference[])secBuffer.toArray(1);
            this.allocateMemory();
        }

        public SecBufferDesc(int type2, int tokenSize) {
            SecBuffer.ByReference secBuffer = new SecBuffer.ByReference(type2, tokenSize);
            this.pBuffers = (SecBuffer.ByReference[])secBuffer.toArray(1);
            this.allocateMemory();
        }
    ...
    extends Structure {
        ...
        public SecBuffer(int type2, int size) {
            ...
            this.BufferType = new NativeLong((long)type2);
            this.allocateMemory();
        }
