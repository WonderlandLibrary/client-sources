...
extends WeakReference<Callback> {
    ...
    private static Callback getCallback(Class<?> type2, Pointer p, boolean direct) {
        ...
        synchronized (map2) {
            ...
            if (ref != null) {
                cb = ref.get();
                if (cb != null && !type2.isAssignableFrom(cb.getClass())) {
                    throw new IllegalStateException("Pointer " + p + " already mapped to " + cb + ".\nNative code may be re-using a default function pointer, in which case you may need to use a common Callback class wherever the function pointer is reused.");
                }
                return cb;
            }
            int ctype = AltCallingConvention.class.isAssignableFrom(type2) ? 63 : 0;
            HashMap<String, Object> foptions = new HashMap<String, Object>(Native.getLibraryOptions(type2));
            ...
            cb = (Callback)Proxy.newProxyInstance(type2.getClassLoader(), new Class[]{type2}, h);
            map.remove(cb);
    ...
    static Class<?> findCallbackClass(Class<?> type2) {
        if (!Callback.class.isAssignableFrom(type2)) {
            throw new IllegalArgumentException(type2.getName() + " is not derived from com.sun.jna.Callback");
        }
        if (type2.isInterface()) {
            return type2;
        }
        ...
        if (Callback.class.isAssignableFrom(type2.getSuperclass())) {
        ...
        private Object invokeCallback(Object[] args) {
            ...
            for (int i = 0; i < args.length; ++i) {
                ...
                if (this.fromNative[i] != null) {
                    CallbackParameterContext context = new CallbackParameterContext(type2, this.callbackMethod, args, i);
                    callbackArgs[i] = this.fromNative[i].fromNative(arg, context);
                    continue;
                }
                callbackArgs[i] = this.convertArgument(arg, type2);
            }
            Object result = null;
