...
public final class InternalLocaleBuilder {
    ...
    public InternalLocaleBuilder setUnicodeLocaleKeyword(String key, String type2) throws LocaleSyntaxException {
        ...
        if (type2 == null) {
            ...
            if (type2.length() != 0) {
                String tp = type2.replaceAll("_", "-");
                StringTokenIterator itr = new StringTokenIterator(tp, "-");
                while (!itr.isDone()) {
                    String s = itr.current();
                    if (!UnicodeLocaleExtension.isTypeSubtag(s)) {
                        throw new LocaleSyntaxException("Ill-formed Unicode locale keyword type: " + type2, itr.currentStart());
                    }
                    itr.next();
                }
            }
            ...
            this._ukeywords.put(cikey, type2);
        }
        return this;
    }
        ...
        while (!itr.isDone()) {
            ...
            if (key != null) {
                if (UnicodeLocaleExtension.isKey(itr.current())) {
                    ...
                    String string = type2 = typeStart == -1 ? "" : subtags.substring(typeStart, typeEnd);
                    if (this._ukeywords == null) {
                        this._ukeywords = new HashMap(4);
                    }
