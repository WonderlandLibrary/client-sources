...
implements Serializable {
    ...
    private static String ldmlTypeToBCP47(String key, String type2) {
        UResourceBundle keyTypeData = UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", "keyTypeData", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
        UResourceBundle typeMap = keyTypeData.get("typeMap");
        key = AsciiUtil.toLowerString(key);
        UResourceBundle typeMapForKey = null;
        String bcpType = null;
        String typeResKey = key.equals("timezone") ? type2.replace('/', ':') : type2;
        try {
        ...
        if (bcpType == null) {
            int typeLen = type2.length();
            if (typeLen >= 3 && typeLen <= 8 && LanguageTag.isExtensionSubtag(type2)) {
                return type2;
            }
            return null;
        }
    ...
    public static final class Builder {
        ...
        public Builder setUnicodeLocaleKeyword(String key, String type2) {
            try {
                this._locbld.setUnicodeLocaleKeyword(key, type2);
            } catch (LocaleSyntaxException e) {
                throw new IllformedLocaleException(e.getMessage(), e.getErrorIndex());
            }
            return this;
