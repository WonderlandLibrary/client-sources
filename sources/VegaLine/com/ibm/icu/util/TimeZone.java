...
Freezable<TimeZone> {
    ...
    public static TimeZone getTimeZone(String ID2, int type2) {
        return TimeZone.getTimeZone(ID2, type2, false);
    }

    private static synchronized TimeZone getTimeZone(String ID2, int type2, boolean frozen) {
        TimeZone result;
        if (type2 == 1) {
    ...
    public static synchronized void setDefaultTimeZoneType(int type2) {
        if (type2 != 0 && type2 != 1) {
    ...
    static {
        String type2 = ICUConfig.get(TZIMPL_CONFIG_KEY, TZIMPL_CONFIG_ICU);
        if (type2.equalsIgnoreCase(TZIMPL_CONFIG_JDK)) {
            TZ_IMPL = 1;
        }
    }
