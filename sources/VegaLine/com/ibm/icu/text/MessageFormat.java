...
extends UFormat {
    ...
    private int nextTopLevelArgStart(int partIndex) {
        ...
        do {
            if ((type2 = this.msgPattern.getPartType(++partIndex)) != MessagePattern.Part.Type.ARG_START) continue;
            return partIndex;
        } while (type2 != MessagePattern.Part.Type.MSG_LIMIT);
        return -1;
    }
    ...
    private void parse(int msgStart, String source, ParsePosition pos, Object[] args, Map<String, Object> argsMap) {
        ...
        while (true) {
            ...
            MessagePattern.Part.Type type2 = part.getType();
            ...
            if (type2 == MessagePattern.Part.Type.SKIP_SYNTAX || type2 == MessagePattern.Part.Type.INSERT_CHAR) {
                prevIndex = part.getLimit();
            } else {
                assert (type2 == MessagePattern.Part.Type.ARG_START) : "Unexpected Part " + part + " in parsed message.";
    ...
    private void format(int msgStart, double pluralNumber, Object[] args, Map<String, Object> argsMap, AppendableWrapper dest, FieldPosition fp) {
        ...
        while (true) {
            ...
            } else if (type2 == MessagePattern.Part.Type.ARG_START) {
                Object arg;
                int argLimit = this.msgPattern.getLimitPartIndex(i);
                MessagePattern.ArgType argType = part.getArgType();
                part = this.msgPattern.getPart(++i);
    ...
    private void formatComplexSubMessage(int msgStart, double pluralNumber, Object[] args, Map<String, Object> argsMap, AppendableWrapper dest) {
        ...
        while (true) {
            ...
            if (type2 == MessagePattern.Part.Type.REPLACE_NUMBER || type2 == MessagePattern.Part.Type.SKIP_SYNTAX) {
                if (sb == null) {
            ...
            if (type2 != MessagePattern.Part.Type.ARG_START) continue;
            if (sb == null) {
                sb = new StringBuilder();
            }
            sb.append(msgString, prevIndex, index);
    ...
    private String getLiteralStringUntilNextArgument(int from) {
        ...
        while (true) {
            ...
            assert (type2 == MessagePattern.Part.Type.SKIP_SYNTAX || type2 == MessagePattern.Part.Type.INSERT_CHAR) : "Unexpected Part " + part + " in parsed message.";
            prevIndex = part.getLimit();
            ++i;
        }
    }
    ...
    private static int findChoiceSubMessage(MessagePattern pattern, int partIndex, double number) {
        ...
        do {
            ...
            if (++partIndex >= count || (type2 = (part = pattern.getPart(partIndex++)).getType()) == MessagePattern.Part.Type.ARG_LIMIT) break;
            assert (type2.hasNumericValue());
    ...
    private Format createAppropriateFormat(String type2, String style) {
        UFormat newFormat = null;
        int subformatType = MessageFormat.findKeyword(type2, typeList);
        block3 : switch (subformatType) {
            ...
            default: {
                throw new IllegalArgumentException("Unknown format type \"" + type2 + "\"");
            }
        }
        return newFormat;
    }
    ...
    implements PluralFormat.PluralSelector {
        ...
        public PluralSelectorProvider(ULocale loc, PluralRules.PluralType type2) {
            this.locale = loc;
            this.type = type2;
        }
