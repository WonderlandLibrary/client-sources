...
extends UFormat {
    ...
    public PluralFormat(ULocale ulocale, PluralRules.PluralType type2) {
        this.init(null, type2, ulocale);
    }
    ...
    public PluralFormat(ULocale ulocale, PluralRules.PluralType type2, String pattern) {
        this.init(null, type2, ulocale);
        this.applyPattern(pattern);
    }

    private void init(PluralRules rules, PluralRules.PluralType type2, ULocale locale) {
        this.ulocale = locale;
        this.pluralRules = rules == null ? PluralRules.forLocale(this.ulocale, type2) : rules;
        this.resetPattern();
    ...
    static int findSubMessage(MessagePattern pattern, int partIndex, PluralSelector selector, double number) {
        ...
        do {
            ...
            if (type2 == MessagePattern.Part.Type.ARG_LIMIT) break;
            assert (type2 == MessagePattern.Part.Type.ARG_SELECTOR);
    ...
    public final String format(double number) {
        ...
        while (true) {
            ...
            if (type2 == MessagePattern.Part.Type.REPLACE_NUMBER || type2 == MessagePattern.Part.Type.SKIP_SYNTAX && this.msgPattern.jdkAposMode()) {
                if (result == null) {
