...
Freezable<MessagePattern> {
    ...
    public double getNumericValue(Part part) {
        Part.Type type2 = part.type;
        if (type2 == Part.Type.ARG_INT) {
            return part.value;
        }
        if (type2 == Part.Type.ARG_DOUBLE) {
    ...
    private void addPart(Part.Type type2, int index, int length, int value) {
        this.parts.add(new Part(type2, index, length, value));
    }

    private void addLimitPart(int start, Part.Type type2, int index, int length, int value) {
        this.parts.get(start).limitPartIndex = this.parts.size();
        this.addPart(type2, index, length, value);
    }
