...
extends NumberFormat {
    ...
    private final boolean subparse(String text, ParsePosition parsePosition, DigitList digits, boolean[] status, Currency[] currency, String negPrefix, String negSuffix, String posPrefix, String posSuffix, int type2) {
        ...
        int posMatch = this.compareAffix(text, position, false, true, posPrefix, type2, currency);
        int negMatch = this.compareAffix(text, position, true, true, negPrefix, type2, currency);
        ...
        if (posMatch >= 0) {
            posMatch = this.compareAffix(text, position, false, false, posSuffix, type2, currency);
        }
        if (negMatch >= 0) {
            negMatch = this.compareAffix(text, position, true, false, negSuffix, type2, currency);
        }
        if (posMatch >= 0 && negMatch >= 0) {
            if (posMatch > negMatch) {
                negMatch = -1;
    ...
    private int compareAffix(String text, int pos, boolean isNegative, boolean isPrefix, String affixPat, int type2, Currency[] currency) {
        if (currency != null || this.currencyChoice != null || this.currencySignCount > 0) {
            return this.compareComplexAffix(affixPat, text, pos, type2, currency);
        }
        if (isPrefix) {
    ...
    private int compareComplexAffix(String affixPat, String text, int pos, int type2, Currency[] currency) {
        int start = pos;
        int i = 0;
        block6: while (i < affixPat.length() && pos >= 0) {
            ...
            switch (c) {
                case '\u00a4': {
                    ...
                    if ((iso = Currency.parse(uloc, text, type2, ppos = new ParsePosition(pos))) != null) {
                        if (currency != null) {
...
lbl199:
    ...
    private static final class AffixForCurrency {
        ...
        public AffixForCurrency(String negPrefix, String negSuffix, String posPrefix, String posSuffix, int type2) {
