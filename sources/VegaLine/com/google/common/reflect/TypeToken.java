...
implements Serializable {
    ...
    public final boolean isSupertypeOf(TypeToken<?> type2) {
        return type2.isSubtypeOf(this.getType());
    }

    public final boolean isSupertypeOf(Type type2) {
        return TypeToken.of(type2).isSubtypeOf(this.getType());
    }

    public final boolean isSubtypeOf(TypeToken<?> type2) {
        return this.isSubtypeOf(type2.getType());
    }
    ...
    private boolean isOwnedBySubtypeOf(Type supertype) {
        for (TypeToken type2 : this.getTypes()) {
            Type ownerType = type2.getOwnerTypeIfPresent();
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        ...
        Type type2 = ownerType = cls.isMemberClass() && !Modifier.isStatic(cls.getModifiers()) ? TypeToken.toGenericType(cls.getEnclosingClass()).runtimeType : null;
        if (typeParams.length > 0 || ownerType != null && ownerType != cls.getEnclosingClass()) {
            TypeToken<?> type3 = TypeToken.of(Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams));
            return type3;
        }
    ...
    private static abstract class TypeCollector<K> {
        ...
        ImmutableList<K> collectTypes(Iterable<? extends K> types) {
            ...
            for (K type2 : types) {
                this.collectTypes(type2, map);
            }
            return TypeCollector.sortKeysByValue(map, Ordering.natural().reverse());
        }

        @CanIgnoreReturnValue
        private int collectTypes(K type2, Map<? super K, Integer> map) {
            Integer existing = map.get(type2);
            if (existing != null) {
                return existing;
            }
    ...
    private static class Bounds {
        ...
        boolean isSupertypeOf(Type subtype) {
            TypeToken<?> type2 = TypeToken.of(subtype);
            for (Type bound : this.bounds) {
                if (type2.isSubtypeOf(bound) != this.target) continue;
                return this.target;
            }
            return !this.target;
        }
