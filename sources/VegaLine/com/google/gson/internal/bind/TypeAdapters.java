...
public final class TypeAdapters {
    ...
    public static <TT> TypeAdapterFactory newFactory(final TypeToken<TT> type2, final TypeAdapter<TT> typeAdapter) {
        return new TypeAdapterFactory(){

            @Override
            public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
                return typeToken.equals(type2) ? typeAdapter : null;
            }
        };
    }

    public static <TT> TypeAdapterFactory newFactory(final Class<TT> type2, final TypeAdapter<TT> typeAdapter) {
        return new TypeAdapterFactory(){

            @Override
            public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
                return typeToken.getRawType() == type2 ? typeAdapter : null;
            }

            public String toString() {
                return "Factory[type=" + type2.getName() + ",adapter=" + typeAdapter + "]";
            }
        };
    }
