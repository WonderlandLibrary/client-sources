...
extends EntityRewriter<ClientboundPackets1_12_1, Protocol1_13To1_12_2> {
    ...
    protected void handleMetadata(int entityId, EntityType type2, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        ...
        if (type2 == Entity1_13Types.EntityType.ENDERMAN && metadata.id() == 12) {
        ...
        if (type2 == Entity1_13Types.EntityType.WOLF && metadata.id() == 17) {
            metadata.setValue(15 - (Integer)metadata.getValue());
        }
        if (type2.isOrHasParent(Entity1_13Types.EntityType.ZOMBIE) && metadata.id() > 14) {
            metadata.setId(metadata.id() + 1);
        }
        if (type2.isOrHasParent(Entity1_13Types.EntityType.MINECART_ABSTRACT) && metadata.id() == 9) {
        ...
        if (type2 == Entity1_13Types.EntityType.AREA_EFFECT_CLOUD) {
            if (metadata.id() == 9) {
                ...
                Particle particle2 = ParticleRewriter.rewriteParticle(particleId, new Integer[]{parameter1, parameter2});
                if (particle2 != null && particle2.getId() != -1) {
                    metadatas.add(new Metadata(9, Types1_13.META_TYPES.particleType, particle2));
                }
            }
            if (metadata.id() >= 9) {
    ...
    public EntityType typeFromId(int type2) {
        return Entity1_13Types.getTypeFromId(type2, false);
    }

    @Override
    public EntityType objectTypeFromId(int type2) {
        return Entity1_13Types.getTypeFromId(type2, true);
    }
}
