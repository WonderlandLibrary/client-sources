...
extends EntityRewriter<ClientboundPackets1_13, Protocol1_14To1_13_2> {
    ...
    @Override
    protected void handleMetadata(int entityId, EntityType type2, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        float v;
        int data;
        ...
        if (metadata.id() == 8 && type2.isOrHasParent(Entity1_14Types.LIVINGENTITY) && Float.isNaN(v = ((Number)metadata.getValue()).floatValue()) && Via.getConfig().is1_14HealthNaNFix()) {
            metadata.setValue(Float.valueOf(1.0f));
        }
        if (metadata.id() > 11 && type2.isOrHasParent(Entity1_14Types.LIVINGENTITY)) {
            metadata.setId(metadata.id() + 1);
        }
        if (type2.isOrHasParent(Entity1_14Types.ABSTRACT_INSENTIENT) && metadata.id() == 13) {
        ...
        if (type2.isOrHasParent(Entity1_14Types.PLAYER)) {
        ...
        } else if (type2.isOrHasParent(Entity1_14Types.ZOMBIE)) {
            if (metadata.id() == 16) {
        ...
        if (type2.isOrHasParent(Entity1_14Types.MINECART_ABSTRACT)) {
            if (metadata.id() == 10) {
                data = (Integer)metadata.getValue();
                metadata.setValue(((Protocol1_14To1_13_2)this.protocol).getMappingData().getNewBlockStateId(data));
            }
        } else if (type2.is((EntityType)Entity1_14Types.HORSE)) {
            if (metadata.id() == 18) {
                metadatas.remove(metadata);
        ...
        } else if (type2.is((EntityType)Entity1_14Types.VILLAGER)) {
            if (metadata.id() == 15) {
        ...
        } else if (type2.is((EntityType)Entity1_14Types.ZOMBIE_VILLAGER)) {
            if (metadata.id() == 18) {
                metadata.setTypeAndValue(Types1_14.META_TYPES.villagerDatatType, new VillagerData(2, MetadataRewriter1_14To1_13_2.getNewProfessionId((Integer)metadata.getValue()), 0));
            }
        } else if (type2.isOrHasParent(Entity1_14Types.ABSTRACT_ARROW)) {
            if (metadata.id() >= 9) {
                metadata.setId(metadata.id() + 1);
            }
        } else if (type2.is((EntityType)Entity1_14Types.FIREWORK_ROCKET)) {
            if (metadata.id() == 8) {
        ...
        } else if (type2.isOrHasParent(Entity1_14Types.ABSTRACT_SKELETON) && metadata.id() == 14) {
            tracker.setInsentientData(entityId, (byte)(tracker.getInsentientData(entityId) & 0xFFFFFFFB | ((Boolean)metadata.getValue() != false ? 4 : 0)));
            metadatas.remove(metadata);
            metadatas.add(new Metadata(13, Types1_14.META_TYPES.byteType, tracker.getInsentientData(entityId)));
        }
        if (type2.isOrHasParent(Entity1_14Types.ABSTRACT_ILLAGER_BASE) && metadata.id() == 14) {
        ...
        if ((type2.is((EntityType)Entity1_14Types.WITCH) || type2.is((EntityType)Entity1_14Types.RAVAGER) || type2.isOrHasParent(Entity1_14Types.ABSTRACT_ILLAGER_BASE)) && metadata.id() >= 14) {
            metadata.setId(metadata.id() + 1);
        }
    }
