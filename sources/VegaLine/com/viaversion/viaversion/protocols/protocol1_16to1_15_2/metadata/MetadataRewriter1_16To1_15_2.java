...
extends EntityRewriter<ClientboundPackets1_15, Protocol1_16To1_15_2> {
    ...
    public void handleMetadata(int entityId, EntityType type2, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        ...
        if (type2.isOrHasParent(Entity1_16Types.MINECART_ABSTRACT) && metadata.id() == 10) {
            data = (Integer)metadata.getValue();
            metadata.setValue(((Protocol1_16To1_15_2)this.protocol).getMappingData().getNewBlockStateId(data));
        }
        if (type2.isOrHasParent(Entity1_16Types.ABSTRACT_ARROW)) {
            if (metadata.id() == 8) {
        ...
        if (type2 == Entity1_16Types.WOLF && metadata.id() == 16) {
    ...
    public EntityType typeFromId(int type2) {
        return Entity1_16Types.getTypeFromId(type2);
    }
}
