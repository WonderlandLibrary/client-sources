...
extends EntityRewriter<ClientboundPackets1_14_4, Protocol1_15To1_14_4> {
    ...
    public void handleMetadata(int entityId, EntityType type2, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        ...
        if (type2.isOrHasParent(Entity1_15Types.MINECART_ABSTRACT) && metadata.id() == 10) {
            data = (Integer)metadata.getValue();
            metadata.setValue(((Protocol1_15To1_14_4)this.protocol).getMappingData().getNewBlockStateId(data));
        }
        if (metadata.id() > 11 && type2.isOrHasParent(Entity1_15Types.LIVINGENTITY)) {
            metadata.setId(metadata.id() + 1);
        }
        if (type2.isOrHasParent(Entity1_15Types.WOLF)) {
            if (metadata.id() == 18) {
    ...
    public EntityType typeFromId(int type2) {
        return Entity1_15Types.getTypeFromId(type2);
    }
}
