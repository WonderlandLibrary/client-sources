...
extends EntityRewriter<ClientboundPackets1_8, Protocol1_9To1_8> {
    ...
    protected void handleMetadata(int entityId, EntityType type2, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        MetaIndex metaIndex = MetaIndex.searchIndex(type2, metadata.id());
        if (metaIndex == null) {
        ...
        switch (metaIndex.getNewType()) {
            case Byte: {
                ...
                if (metaIndex != MetaIndex.ENTITY_STATUS || type2 != Entity1_10Types.EntityType.PLAYER) break;
                Byte val = 0;
    ...
    public EntityType typeFromId(int type2) {
        return Entity1_10Types.getTypeFromId(type2, false);
    }

    @Override
    public EntityType objectTypeFromId(int type2) {
        return Entity1_10Types.getTypeFromId(type2, true);
    }
}
