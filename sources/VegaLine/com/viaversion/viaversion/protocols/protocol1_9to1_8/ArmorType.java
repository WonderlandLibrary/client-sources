...
public enum ArmorType {
    ...
    private ArmorType(int armorPoints, int id, String type2) {
    ...
    public static ArmorType findById(int id) {
        ArmorType type2 = armor.get(id);
        return type2 == null ? NONE : type2;
    }

    public static ArmorType findByType(String type2) {
        for (ArmorType a : ArmorType.values()) {
            if (!a.getType().equals(type2)) continue;
            return a;
        }
        return NONE;
    }
    ...
    public static boolean isArmor(String type2) {
        for (ArmorType a : ArmorType.values()) {
            if (!a.getType().equals(type2)) continue;
            return true;
        }
