...
extends Type<Particle> {
    ...
    protected ParticleReader blockHandler() {
        return (buf, particle2) -> particle2.getArguments().add(new Particle.ParticleData(Type.VAR_INT, Type.VAR_INT.readPrimitive(buf)));
    }

    protected ParticleReader itemHandler(Type<Item> itemType) {
        return (buf, particle2) -> particle2.getArguments().add(new Particle.ParticleData(itemType, itemType.read(buf)));
    }

    protected ParticleReader dustHandler() {
        return (buf, particle2) -> {
            particle2.getArguments().add(new Particle.ParticleData(Type.FLOAT, Float.valueOf(Type.FLOAT.readPrimitive(buf))));
            particle2.getArguments().add(new Particle.ParticleData(Type.FLOAT, Float.valueOf(Type.FLOAT.readPrimitive(buf))));
            particle2.getArguments().add(new Particle.ParticleData(Type.FLOAT, Float.valueOf(Type.FLOAT.readPrimitive(buf))));
            particle2.getArguments().add(new Particle.ParticleData(Type.FLOAT, Float.valueOf(Type.FLOAT.readPrimitive(buf))));
        };
    ...
    protected ParticleReader vibrationHandler(Type<Position> positionType) {
        return (buf, particle2) -> {
            particle2.getArguments().add(new Particle.ParticleData(positionType, positionType.read(buf)));
            ...
            if (resourceLocation.equals("block")) {
                ...
                particle2.getArguments().add(new Particle.ParticleData(Type.VAR_INT, Type.VAR_INT.readPrimitive(buf)));
            } else {
