...
implements ListBinaryTag {
    ...
    public ListBinaryTag add(Iterable<? extends BinaryTag> tagsToAdd) {
        ...
        BinaryTagType<?> type2 = ListBinaryTagImpl.mustBeSameType(tagsToAdd);
        return this.edit(tags -> {
    ...
    static BinaryTagType<?> mustBeSameType(Iterable<? extends BinaryTag> tags) {
        BinaryTagType<? extends BinaryTag> type2 = null;
        for (BinaryTag binaryTag : tags) {
            if (type2 == null) {
                type2 = binaryTag.type();
                continue;
            }
            ListBinaryTagImpl.mustBeSameType(binaryTag, type2);
        }
        return type2;
    }

    static void mustBeSameType(BinaryTag tag, BinaryTagType<? extends BinaryTag> type2) {
        if (tag.type() != type2) {
            throw new IllegalArgumentException(String.format("Trying to add tag of type %s to list of %s", tag.type(), type2));
        }
    }
