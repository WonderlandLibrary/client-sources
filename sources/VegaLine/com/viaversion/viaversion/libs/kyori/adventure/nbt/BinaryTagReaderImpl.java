...
implements BinaryTagIO.Reader {
    ...
    public CompoundBinaryTag read(@NotNull DataInput input) throws IOException {
        ...
        BinaryTagType<BinaryTag> type2 = BinaryTagType.binaryTagType(input.readByte());
        BinaryTagReaderImpl.requireCompound(type2);
    ...
    private static void requireCompound(BinaryTagType<? extends BinaryTag> type2) throws IOException {
        if (type2 != BinaryTagTypes.COMPOUND) {
            throw new IOException(String.format("Expected root tag to be a %s, was %s", BinaryTagTypes.COMPOUND, type2));
        }
    }
}
