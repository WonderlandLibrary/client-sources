...
extends EntityRewriterBase<C, T> {
    ...
    protected @Nullable EntityData getObjectData(ObjectType type2) {
        return this.objectTypes.get(type2);
    }
    ...
    protected PacketHandler getMobSpawnRewriter(Type<List<Metadata>> metaType) {
        return wrapper -> {
            ...
            EntityType type2 = this.tracker(wrapper.user()).entityType(entityId);
            List metadata = (List)wrapper.get(metaType, 0);
            this.handleMetadata(entityId, metadata, wrapper.user());
            EntityData entityData = this.entityDataForType(type2);
            if (entityData != null) {
    ...
    protected PacketHandler getObjectRewriter(Function<Byte, ObjectType> objectGetter) {
        return wrapper -> {
            ObjectType type2 = (ObjectType)objectGetter.apply(wrapper.get(Type.BYTE, 0));
            if (type2 == null) {
                ViaBackwards.getPlatform().getLogger().warning("Could not find Entity Type" + wrapper.get(Type.BYTE, 0));
                return;
            }
            EntityData data = this.getObjectData(type2);
            if (data != null) {
    ...
    protected void addTrackedEntity(PacketWrapper wrapper, int entityId, EntityType type2) throws Exception {
        this.tracker(wrapper.user()).addEntity(entityId, type2);
    }
}
