...
class GLChecks {
    ...
    static int calculateImageStorage(Buffer buffer, int format, int type2, int width, int height, int depth) {
        return LWJGLUtil.CHECKS ? GLChecks.calculateImageStorage(format, type2, width, height, depth) >> BufferUtils.getElementSizeExponent(buffer) : 0;
    }

    static int calculateTexImage1DStorage(Buffer buffer, int format, int type2, int width) {
        return LWJGLUtil.CHECKS ? GLChecks.calculateTexImage1DStorage(format, type2, width) >> BufferUtils.getElementSizeExponent(buffer) : 0;
    }

    static int calculateTexImage2DStorage(Buffer buffer, int format, int type2, int width, int height) {
        return LWJGLUtil.CHECKS ? GLChecks.calculateTexImage2DStorage(format, type2, width, height) >> BufferUtils.getElementSizeExponent(buffer) : 0;
    }

    static int calculateTexImage3DStorage(Buffer buffer, int format, int type2, int width, int height, int depth) {
        return LWJGLUtil.CHECKS ? GLChecks.calculateTexImage3DStorage(format, type2, width, height, depth) >> BufferUtils.getElementSizeExponent(buffer) : 0;
    }

    private static int calculateImageStorage(int format, int type2, int width, int height, int depth) {
        return GLChecks.calculateBytesPerPixel(format, type2) * width * height * depth;
    }

    private static int calculateTexImage1DStorage(int format, int type2, int width) {
        return GLChecks.calculateBytesPerPixel(format, type2) * width;
    }

    private static int calculateTexImage2DStorage(int format, int type2, int width, int height) {
        return GLChecks.calculateTexImage1DStorage(format, type2, width) * height;
    }

    private static int calculateTexImage3DStorage(int format, int type2, int width, int height, int depth) {
        return GLChecks.calculateTexImage2DStorage(format, type2, width, height) * depth;
    }

    private static int calculateBytesPerPixel(int format, int type2) {
        int epp;
        int bpe;
        switch (type2) {
            case 5120: 
    ...
    static int calculateBytesPerCharCode(int type2) {
        switch (type2) {
            case 5121: 
            case 37018: {
                return 1;
        ...
        throw new IllegalArgumentException("Unsupported charcode type: " + type2);
    }
