...
public class MipMap
extends Util {
    public static int gluBuild2DMipmaps(int target, int components, int width, int height, int format, int type2, ByteBuffer data) {
        ByteBuffer image;
        int h;
        if (width < 1 || height < 1) {
            return 100901;
        }
        int bpp = MipMap.bytesPerPixel(format, type2);
        if (bpp == 0) {
            return 100900;
        }
        ...
        if (w != width || h != height) {
            ...
            int error = MipMap.gluScaleImage(format, width, height, type2, data, w, h, type2, image);
            if (error != 0) {
                retVal = error;
                done = true;
            }
        ...
        while (!done) {
            ...
            GL11.glTexImage2D(target, level, components, w, h, 0, format, type2, image);
            if (w == 1 && h == 1) break;
            int newW = w < 2 ? 1 : w >> 1;
            ...
            int error = MipMap.gluScaleImage(format, w, h, type2, image, newW, newH, type2, newImage);
            if (error != 0) {
                retVal = error;
                done = true;
            }
