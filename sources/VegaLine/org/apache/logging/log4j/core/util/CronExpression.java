...
public final class CronExpression {
    ...
    protected int storeExpressionVals(int pos, String s, int type2) throws ParseException {
        int val;
        int incr = 0;
        ...
        if (!(c < 'A' || c > 'Z' || s.equals("L") || s.equals("LW") || s.matches("^L-[0-9]*[W]?"))) {
            ...
            this.addToSet(sval, eval, incr, type2);
            return i + 3;
        }
        if (c == '?') {
            int val2;
            ...
            if (type2 == 5 && !this.lastdayOfMonth && (val2 = this.daysOfMonth.last().intValue()) == 98) {
                throw new ParseException("'?' can only be specfied for Day-of-Month -OR- Day-of-Week.", i);
            }
            this.addToSet(98, -1, 0, type2);
            return i;
        }
        if (c == '*' || c == '/') {
            if (c == '*' && i + 1 >= s.length()) {
                this.addToSet(99, -1, incr, type2);
                return i + 1;
            }
        ...
        if (c == 'L') {
            ...
            if (type2 == 5) {
                this.addToSet(7, 7, 0, type2);
            }
            if (type2 == 3 && s.length() > i) {
                c = s.charAt(i);
                if (c == '-') {
        ...
        if (c >= '0' && c <= '9') {
            ...
            if (++i < s.length()) {
                ...
                i = this.checkNext(i, s, val, type2);
                return i;
            }
        } else {
            throw new ParseException("Unexpected character: " + c, i);
        }
        this.addToSet(val, -1, -1, type2);
        return i;
    }

    protected int checkNext(int pos, String s, int val, int type2) throws ParseException {
        int end = -1;
        int i = pos;
        if (i >= s.length()) {
            this.addToSet(val, end, -1, type2);
            return i;
        }
        char c = s.charAt(pos);
        if (c == 'L') {
            ...
            this.lastdayOfWeek = true;
            TreeSet<Integer> set = this.getSet(type2);
            set.add(val);
            return ++i;
        }
        if (c == 'W') {
        ...
        if (c == '-') {
            ...
            if (i < s.length() && (c = s.charAt(i)) == '/') {
                ...
                if (++i >= s.length()) {
                    this.addToSet(val, end, v2, type2);
                    return i;
                }
                c = s.charAt(i);
                if (c >= '0' && c <= '9') {
                    ...
                    int v3 = vs.value;
                    this.addToSet(val, end, v3, type2);
                    i = vs.pos;
                    return i;
                }
                this.addToSet(val, end, v2, type2);
    ...
    protected void addToSet(int val, int end, int incr, int type2) throws ParseException {
        TreeSet<Integer> set = this.getSet(type2);
        if (type2 == 0 || type2 == 1) {
        ...
        } else if (type2 == 5 && (val == 0 || val > 7 || end > 7) && val != 99 && val != 98) {
            throw new ParseException("Day-of-Week values must be between 1 and 7", -1);
        }
        ...
        for (int i = startAt; i <= stopAt; i += incr) {
            ...
            int i2 = i % max;
            if (i2 == 0 && (type2 == 4 || type2 == 5 || type2 == 3)) {
                i2 = max;
            }
            set.add(i2);
        }
    ...
    TreeSet<Integer> getSet(int type2) {
        switch (type2) {
            case 0: {
                return this.seconds;
            }
