    ...
    private ChangeSetResults perform(ArchiveEntryIterator entryIterator, ArchiveOutputStream out) throws IOException {
        ...
        while (entryIterator.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                int type2 = change.type();
                String name = entry.getName();
                if (type2 == 1 && name != null) {
                ...
                if (type2 != 4 || name == null || !name.startsWith(change.targetFile() + "/")) continue;
                copy = false;
                results.deleted(name);
                break;
            }
    ...
    private boolean isDeletedLater(Set<Change> workingSet, ArchiveEntry entry) {
        ...
        if (!workingSet.isEmpty()) {
            for (Change change : workingSet) {
                ...
                if (type2 == 1 && source.equals(target)) {
                    return true;
                }
                if (type2 != 4 || !source.startsWith(target + "/")) continue;
                return true;
            }
        }
        return false;
