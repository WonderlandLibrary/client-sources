...
extends CompressorInputStream {
    ...
    private void readNextBlock() throws IOException {
        ...
        int type2 = this.readOneByte();
        if (type2 == -1) {
            this.endReached = true;
        } else if (type2 == 255) {
            this.in.unread(type2);
            this.pushedBackBytes(1L);
            this.readStreamIdentifier();
            this.readNextBlock();
        } else if (type2 == 254 || type2 > 127 && type2 <= 253) {
            this.skipBlock();
            this.readNextBlock();
        } else {
            if (type2 >= 2 && type2 <= 127) {
                throw new IOException("unskippable chunk with type " + type2 + " (hex " + Integer.toHexString(type2) + ")" + " detected.");
            }
            if (type2 == 1) {
                ...
                throw new IOException("unknown chunk type " + type2 + " detected.");
            }
        }
    }
