...
public class ExceptionUtils {
    ...
    public static int indexOfType(Throwable throwable, Class<?> type2) {
        return ExceptionUtils.indexOf(throwable, type2, 0, true);
    }

    public static int indexOfType(Throwable throwable, Class<?> type2, int fromIndex) {
        return ExceptionUtils.indexOf(throwable, type2, fromIndex, true);
    }

    private static int indexOf(Throwable throwable, Class<?> type2, int fromIndex, boolean subclass) {
        Throwable[] throwables;
        if (throwable == null || type2 == null) {
            return -1;
        }
        if (fromIndex < 0) {
            fromIndex = 0;
        ...
        if (subclass) {
            for (int i = fromIndex; i < throwables.length; ++i) {
                if (!type2.isAssignableFrom(throwables[i].getClass())) continue;
                return i;
            }
        } else {
            for (int i = fromIndex; i < throwables.length; ++i) {
                if (!type2.equals(throwables[i].getClass())) continue;
                return i;
            }
        }
        return -1;
    ...
    public static boolean hasCause(Throwable chain, Class<? extends Throwable> type2) {
        if (chain instanceof UndeclaredThrowableException) {
            chain = chain.getCause();
        }
        return type2.isInstance(chain);
    }
}
