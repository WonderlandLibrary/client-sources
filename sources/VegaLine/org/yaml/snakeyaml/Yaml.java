...
public class Yaml {
    ...
    public <T> T loadAs(Reader io, Class<? super T> type2) {
        return (T)this.loadFromReader(new StreamReader(io), type2);
    }

    public <T> T loadAs(String yaml, Class<? super T> type2) {
        return (T)this.loadFromReader(new StreamReader(yaml), type2);
    }

    public <T> T loadAs(InputStream input, Class<? super T> type2) {
        return (T)this.loadFromReader(new StreamReader(new UnicodeReader(input)), type2);
    }

    private Object loadFromReader(StreamReader sreader, Class<?> type2) {
        Composer composer = new Composer(new ParserImpl(sreader, this.loadingConfig), this.resolver, this.loadingConfig);
        this.constructor.setComposer(composer);
        return this.constructor.getSingleData(type2);
    }
