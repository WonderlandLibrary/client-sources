...
Cloneable {
    ...
    private void setActualComparator() {
        this.actualComparator = this.storedComparator == null || this.storedComparator instanceof CharComparator ? (CharComparator)this.storedComparator : new CharComparator(){
            ...
            public int compare(char k1, char k2) {
                return Char2ShortAVLTreeMap.this.storedComparator.compare(Character.valueOf(k1), Character.valueOf(k2));
            }

            @Override
            public int compare(Character ok1, Character ok2) {
                return Char2ShortAVLTreeMap.this.storedComparator.compare(ok1, ok2);
            }
        };
    }
    ...
    public Short put(Character ok, Short ov) {
        short oldValue = this.put(ok.charValue(), (short)ov);
    ...
    public Short remove(Object ok) {
        short oldValue = this.remove(((Character)ok).charValue());
        ...
        if (this.entries == null) {
            this.entries = new AbstractObjectSortedSet<Char2ShortMap.Entry>(){
                final Comparator<? super Char2ShortMap.Entry> comparator = new Comparator<Char2ShortMap.Entry>(){
                    ...
                    @Override
                    public int compare(Char2ShortMap.Entry x, Char2ShortMap.Entry y) {
                        return Char2ShortAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
                    }
                };
    ...
    implements Serializable {
        ...
        public ObjectSortedSet<Char2ShortMap.Entry> char2ShortEntrySet() {
            if (this.entries == null) {
                this.entries = new AbstractObjectSortedSet<Char2ShortMap.Entry>(){
                    ...
                    @Override
                    public Comparator<? super Char2ShortMap.Entry> comparator() {
                        return Char2ShortAVLTreeMap.this.entrySet().comparator();
                    }
        ...
        public Short put(Character ok, Short ov) {
            short oldValue = this.put(ok.charValue(), (short)ov);
            return Char2ShortAVLTreeMap.this.modified ? null : Short.valueOf(oldValue);
        }
        ...
        public Short remove(Object ok) {
            short oldValue = this.remove(((Character)ok).charValue());
            return Char2ShortAVLTreeMap.this.modified ? Short.valueOf(oldValue) : null;
        }
