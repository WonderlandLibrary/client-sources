...
Cloneable {
    ...
    private void setActualComparator() {
        this.actualComparator = this.storedComparator == null || this.storedComparator instanceof CharComparator ? (CharComparator)this.storedComparator : new CharComparator(){
            ...
            public int compare(char k1, char k2) {
                return Char2CharRBTreeMap.this.storedComparator.compare(Character.valueOf(k1), Character.valueOf(k2));
            }

            @Override
            public int compare(Character ok1, Character ok2) {
                return Char2CharRBTreeMap.this.storedComparator.compare(ok1, ok2);
            }
        };
    }
    ...
    public Character put(Character ok, Character ov) {
        char oldValue = this.put(ok.charValue(), ov.charValue());
    ...
    public Character remove(Object ok) {
        char oldValue = this.remove(((Character)ok).charValue());
        ...
        if (this.entries == null) {
            this.entries = new AbstractObjectSortedSet<Char2CharMap.Entry>(){
                final Comparator<? super Char2CharMap.Entry> comparator = new Comparator<Char2CharMap.Entry>(){
                    ...
                    @Override
                    public int compare(Char2CharMap.Entry x, Char2CharMap.Entry y) {
    ...
    implements Serializable {
        ...
        public Character put(Character ok, Character ov) {
            char oldValue = this.put(ok.charValue(), ov.charValue());
            return Char2CharRBTreeMap.this.modified ? null : Character.valueOf(oldValue);
        }
        ...
        public Character remove(Object ok) {
            char oldValue = this.remove(((Character)ok).charValue());
            return Char2CharRBTreeMap.this.modified ? Character.valueOf(oldValue) : null;
        }
