...
Cloneable {
    ...
    private void setActualComparator() {
        this.actualComparator = this.storedComparator == null || this.storedComparator instanceof FloatComparator ? (FloatComparator)this.storedComparator : new FloatComparator(){
            ...
            public int compare(float k1, float k2) {
                return Float2ByteRBTreeMap.this.storedComparator.compare(Float.valueOf(k1), Float.valueOf(k2));
            }

            @Override
            public int compare(Float ok1, Float ok2) {
                return Float2ByteRBTreeMap.this.storedComparator.compare(ok1, ok2);
            }
        };
    }
    ...
    public Byte put(Float ok, Byte ov) {
        byte oldValue = this.put(ok.floatValue(), (byte)ov);
    ...
    public Byte remove(Object ok) {
        byte oldValue = this.remove(((Float)ok).floatValue());
        ...
        if (this.entries == null) {
            this.entries = new AbstractObjectSortedSet<Float2ByteMap.Entry>(){
                final Comparator<? super Float2ByteMap.Entry> comparator = new Comparator<Float2ByteMap.Entry>(){
                    ...
                    @Override
                    public int compare(Float2ByteMap.Entry x, Float2ByteMap.Entry y) {
    ...
    implements Serializable {
        ...
        public Byte put(Float ok, Byte ov) {
            byte oldValue = this.put(ok.floatValue(), (byte)ov);
            return Float2ByteRBTreeMap.this.modified ? null : Byte.valueOf(oldValue);
        }
        ...
        public Byte remove(Object ok) {
            byte oldValue = this.remove(((Float)ok).floatValue());
            return Float2ByteRBTreeMap.this.modified ? Byte.valueOf(oldValue) : null;
        }
