...
public class GunAura extends Module
{
    ...
    public boolean isValidTarget(final Entity entity) {
        ...
        final KillAura aura = (KillAura)Icarus.getModuleManager().getModuleByName("killaura");
        ...
        if (entity instanceof EntityPlayer) {
            valid = (entity != null && GunAura.mc.thePlayer.getDistanceToEntity(entity) <= this.reach.getValue() && entity != GunAura.mc.thePlayer && entity.isEntityAlive() && !Icarus.getFriendManager().isFriend(entity.getName()));
        }
        return valid;
    }
    ...
    public EntityLivingBase getCursorEntity() {
        ...
        for (final Object o : GunAura.mc.theWorld.loadedEntityList) {
            if (o instanceof Entity) {
                ...
                if (!(targetEntity instanceof EntityPlayer) || targetEntity == GunAura.mc.thePlayer || !GunAura.mc.thePlayer.canEntityBeSeen(targetEntity) || Icarus.getFriendManager().isFriend(targetEntity.getName()) || ((EntityLivingBase)targetEntity).deathTime > 0) {
                    continue;
                }
                if (poorEntity == null) {
