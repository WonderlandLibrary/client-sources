...
public class BowAimbot extends Module
{
    ...
    public boolean isValidTarget(final Entity entity) {
        ...
        final Teams teams = (Teams)Icarus.getModuleManager().getModuleByName("teams");
        ...
        if (entity instanceof EntityPlayer && this.players.getValue()) {
            valid = (entity != null && BowAimbot.mc.thePlayer.getDistanceToEntity(entity) <= this.reach.getValue() && entity != BowAimbot.mc.thePlayer && entity.isEntityAlive() && !Icarus.getFriendManager().isFriend(entity.getName()));
        }
    ...
    public EntityLivingBase getCursorEntity() {
        ...
        for (final Object o : BowAimbot.mc.theWorld.loadedEntityList) {
            if (o instanceof Entity) {
                ...
                if (!(targetEntity instanceof EntityPlayer) || targetEntity == BowAimbot.mc.thePlayer || !BowAimbot.mc.thePlayer.canEntityBeSeen(targetEntity) || Icarus.getFriendManager().isFriend(targetEntity.getName()) || ((EntityLivingBase)targetEntity).deathTime > 0) {
                    continue;
                }
                if (poorEntity == null) {
    ...
    public void onEvent(final Event event) {
        ...
        if (event instanceof PreMotion) {
            ...
            if (BowAimbot.mc.thePlayer != null && entity != null && this.isValidTarget(entity) && BowAimbot.mc.thePlayer.isUsingItem() && BowAimbot.mc.thePlayer.getCurrentEquippedItem().getItem() != null && BowAimbot.mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemBow) {
                ...
                final double xDistance = entity.posX - BowAimbot.mc.thePlayer.posX + (entity.posX - entity.lastTickPosX) * ((float)(Icarus.getModuleManager().getModuleByName("instantuse").isEnabled() ? 1.0 : bowVelocity) * 10.0f);
                final double zDistance = entity.posZ - BowAimbot.mc.thePlayer.posZ + (entity.posZ - entity.lastTickPosZ) * ((float)(Icarus.getModuleManager().getModuleByName("instantuse").isEnabled() ? 1.0 : bowVelocity) * 10.0f);
                ...
                final float trajectoryTheta90 = (float)(Math.atan2(zDistance, xDistance) * 180.0 / 3.141592653589793) - 90.0f;
                final float bowTrajectory = -this.getTrajectoryAngleSolutionLow((float)trajectoryXZ, (float)eyeDistance, (float)(Icarus.getModuleManager().getModuleByName("instantuse").isEnabled() ? 1.0 : bowVelocity));
            ...
            if (sent.getPacket() instanceof C03PacketPlayer) {
                ...
                if (BowAimbot.mc.thePlayer != null && entity != null && this.isValidTarget(entity) && BowAimbot.mc.thePlayer.isUsingItem() && BowAimbot.mc.thePlayer.getCurrentEquippedItem().getItem() != null && BowAimbot.mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemBow) {
                    ...
                    final HealingBot healingBot2 = (HealingBot)Icarus.getModuleManager().getModuleByName("healingbot");
