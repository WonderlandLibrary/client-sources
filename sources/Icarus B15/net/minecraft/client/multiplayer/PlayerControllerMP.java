...
public class PlayerControllerMP
{
  ...
  {
    BlockBreaking event = new BlockBreaking(BlockBreaking.EnumBlock.DESTROY, 
      p_178888_1_, p_178888_2_);
  ...
  {
    BlockBreaking event = new BlockBreaking(BlockBreaking.EnumBlock.CLICK, 
      p_180511_1_, p_180511_2_);
    Icarus.getEventManager().hook(event);
    if (this.currentGameType.isAdventure())
    {
      if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
      ...
      if (!this.mc.thePlayer.func_175142_cm())
      {
        Block var3 = this.mc.theWorld.getBlockState(p_180511_1_).getBlock();
        ItemStack var4 = this.mc.thePlayer.getCurrentEquippedItem();
        if (var4 == null) {
          return false;
        }
    ...
    else if ((!this.isHittingBlock) || (!func_178893_a(p_180511_1_)))
    {
  ...
  {
    BlockBreaking event = new BlockBreaking(BlockBreaking.EnumBlock.DAMAGE, 
      p_180512_1_, p_180512_2_);
    Icarus.getEventManager().hook(event);
    syncCurrentPlayItem();
    ...
    if (func_178893_a(p_180512_1_))
    {
      ...
      this.curBlockDamageMP += var3.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, p_180512_1_) * event.getMutliplier();
  ...
  {
    ReachDistance event = new ReachDistance(this.currentGameType.isCreative() ? 5.0F : 4.5F);
    Icarus.getEventManager().hook(event);
  ...
  {
    Icarus.getEventManager().hook(new Attacking(targetEntity));
  ...
  public void windowClickPacket(int windowId, int slotId, int p_78753_3_, int p_78753_4_, EntityPlayer playerIn)
  {
    short var6 = playerIn.openContainer.getNextTransactionID(playerIn.inventory);
    this.netClientHandler.addToSendQueue(new C0EPacketClickWindow(windowId, slotId, p_78753_3_, p_78753_4_, this.mc.thePlayer.getCurrentEquippedItem(), var6));
  }
