...
public class TNTTimerMod extends Mod {
	public TNTTimerMod() {
		super("TNT Timer", ModCategory.MODS, "silentclient/icons/mods/tnttimer.png");
	}
    ...
    public static void doRender(RenderTNTPrimed tntRenderer, EntityTNTPrimed tntPrimed, double x, double y, double z, float partialTicks) {
        int fuseTimer = Server.isHypixel() ? tntPrimed.fuse - 28 : tntPrimed.fuse;
        ...
        if (fuseTimer >= 1) {
            ...
            if (distance <= 4096.0D) {
                ...
                float green = Math.min((float) fuseTimer / (Server.isHypixel() ? 52.0F : 80.0F), 1.0F);
