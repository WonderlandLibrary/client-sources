...
public class ItemRenderer
{
    ...
    {
        float scale = Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).scale.get().floatValue();
        float xPos = Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).xPos.get().floatValue();
        float yPos = Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).yPos.get().floatValue();
        if(mc.thePlayer.isUsingItem())
            GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
        else
            GlStateManager.translate(xPos, -yPos, -0.71999997F);
    ...
    private static int ticksForAstoflo = 0;

    private void astofloSpin(float swingProgress) {
        float f1 = MathHelper.sin((float) (MathHelper.sqrt_float(swingProgress) * Math.PI));
        ticksForAstoflo++;
        GlStateManager.translate(-0.0F, -0.2F, -0.6F);
        GlStateManager.rotate(-ticksForAstoflo * 0.07F * 30.0F, 0.0F, 0.0F, -1.0F);
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case DRINK:
                            if(Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).isEnabled()
                                    && Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).mode.compare("1.7")){
                        ...
                        case BLOCK:
                            ...
                            if(!Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).isEnabled()){
                                ...
                                switch(Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).mode.get()){
                                    case "Sigma": {
                                    ...
                                    case "Astolfo": {
                                        astofloSpin(mc.thePlayer.getSwingProgress(f1));
                                        this.doBlockTransformations();
                                        break;
                                    }
                                }
                ...
                {
                    if(Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).isEnabled()
                        && Raze.INSTANCE.MANAGER_REGISTRY.MODULE_REGISTRY.getModule(Animations.class).swing.compare("Smooth")){
