...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void func_175161_p() {
		...
		if (this.func_175160_A()) {
			EventMotionUpdate emu = new EventMotionUpdate(this.posX, this.getEntityBoundingBox().minY, this.posZ,
					this.rotationYaw, this.rotationPitch, this.onGround);
			EventManager.call(emu);
			int XD = 0;

			double var3 = emu.getPosX() - this.field_175172_bI;
			double var5 = emu.getPosY() - this.field_175166_bJ;
			double var7 = emu.getPosZ() - this.field_175167_bK;
			double var9 = (double) (emu.getYaw() - this.field_175164_bL);
			double var11 = (double) (emu.getPitch() - this.field_175165_bM);
			boolean var13 = var3 * var3 + var5 * var5 + var7 * var7 > 9.0E-4D || this.field_175168_bP >= 20;
			boolean var14 = var9 != 0.0D || var11 != 0.0D;

			if (this.ridingEntity == null) {
				if (var13 && var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(emu.getPosX(),
							emu.getPosY(), emu.getPosZ(), emu.getYaw(), emu.getPitch(), emu.isOnGround()));
				} else if (var13) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(emu.getPosX(),
							emu.getPosY(), emu.getPosZ(), emu.getYaw(), emu.getPitch(), emu.isOnGround()));
				} else if (var14) {
			...
			if (var14) {
				this.field_175164_bL = emu.getYaw();
				this.field_175165_bM = emu.getPitch();
			}
			EventMotionUpdate emu2 = new EventMotionUpdate();
			EventManager.call(emu2);
		}
	}
	...
	public void sendChatMessage(String m) {
		if (m.startsWith("-")) {
			for (Command comm : ClientBase.INSTANCE.cmds.cmds) {
				String args[] = m.replaceFirst("-", "").split(" ");
				if (m.startsWith("-" + comm.name)) {
					try {
						comm.run(args);
					} catch (Exception e) {
						ClientBase.INSTANCE.chat("Unknown command :(");
					}
				}
			}
			return;
	...
	public void onLivingUpdate() {
		...
		// if
		// (!ClientBase.INSTANCE.getModuleManager().getModule(NoSlow.class).isEnabled())
		// {
		...
		if (!this.isSprinting() && this.movementInput.moveForward >= var3 && var5 && !this.isUsingItem()
	...
	public double getFlyMotion() {
		return this.movementInput.jump ? 5 : this.movementInput.sneak ? -5 : -0.039;
	}

	public double depresscito() {
		return -0.0501;
	}
	...
	public void moveEntity(double x, double y, double z) {
		EventRawMove e = new EventRawMove(x, y, z);
		EventManager.call(e);
		super.moveEntity(e.getX(), e.getY(), e.getZ());
	}

	public void createAndSendC04(double posX, double posY, double posZ, boolean b) {
		this.sendQueue.addToSendQueue(new C04PacketPlayerPosition(posX, posY, posZ, b));
	}

	public void upC04(double d, boolean b) {
		this.createAndSendC04(posX, posY + d, posZ, b);
	}
}
