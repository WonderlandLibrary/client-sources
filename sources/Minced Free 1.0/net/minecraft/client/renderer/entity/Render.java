...
public abstract class Render<T extends Entity> implements IEntityRenderer {
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
        if (Celestial.instance.featureManager.getFeature(ESP.class).isEnabled() && ESP.nametag.getBoolValue() || entityIn.getEntityId() == -20) {
            return;
        }
        ...
        if (d0 <= (maxDistance * maxDistance)) {
            ...
            int i = 0;
            if (Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled() && str.contains(entityIn.getName()) && !(entityIn instanceof EntityArmorStand))
                str = NameUtils.getName();
