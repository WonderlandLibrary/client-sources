...
import ru.smertnix.celestial.command.impl.TpCommand;
...
import ru.smertnix.celestial.event.events.impl.render.EventRenderScoreboard;
...
public class GuiIngame extends Gui {
    ...
    public static int grief;
    /**
     * ChatGUI instance that retains all previous chat data
     */
    private final GuiNewChat persistantChatGUI;
    private int updateCounter;
    String name = NameUtils.name;
    /**
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (Timer.smart.getCurrentValue()) {
        	...
        	if (!Celestial.instance.featureManager.getFeature(Timer.class).isEnabled()) {
        		if (Timer.active && Timer.ticks > 0) {
                 ...
                 if (!MovementUtils.isMoving())
                	 if (mc.player.ticksExisted % 4 == 0) {
               		...
               		if (Speed.speedMode.getOptions().equalsIgnoreCase("ReallyWorld2")) {
               			if (!Celestial.instance.featureManager.getFeature(Speed.class).isEnabled()) {
                   			mc.timer.timerSpeed = 1;
                   		}
               		} else {
               			mc.timer.timerSpeed = 1;
        	...
        	RenderUtils.drawBlurredShadow((scaledresolution.getScaledWidth() / 2) + 20 - 60, scaledresolution.getScaledHeight() - 101 - 10 + 20, 80, 15, 40, Timer.ticks > 40 ? new Color(130, 40, 40, 100) : new Color(40, 40, 40, 100));
            RoundedUtil.drawRound((scaledresolution.getScaledWidth() / 2) + 24 - 38, scaledresolution.getScaledHeight() - 101 + 17, 28, 9, 1, new Color(40, 40, 40));
            RoundedUtil.drawRound((scaledresolution.getScaledWidth() / 2) + 20 - 50, scaledresolution.getScaledHeight() - 101 + 5, 60, 8, 1, new Color(40, 40, 40));
            RoundedUtil.drawGradientHorizontal((scaledresolution.getScaledWidth() / 2) + 20 - 50, scaledresolution.getScaledHeight() - 101 + 5, (60 - (Timer.ticks)), 8, 1, ClientHelper.getClientColor(), new Color(255, 255, 255));
            mc.mntsb_13.drawCenteredStringWithShadow(MathematicHelper.round(100 - ((int)Timer.ticks * 2),0) + "%", (float) (scaledresolution.getScaledWidth() / 2) + 20 - 18.5f, (float) (scaledresolution.getScaledHeight() - 101 + 3.5f + 4.5f), Color.GRAY.getRGB());
            mc.mntsb_13.drawCenteredStringWithShadow( MathematicHelper.round(100 - ((int)Timer.ticks * 2),0) + "%", (float) (scaledresolution.getScaledWidth() / 2) + 20 - 19, (float) (scaledresolution.getScaledHeight() - 101 + 3 + 4.5f), -1);
            mc.mntsb_13.drawCenteredStringWithShadow("Timer", (float) (scaledresolution.getScaledWidth() / 2) + 20  - 20, (float) (scaledresolution.getScaledHeight() - 101 + 20), -1);
           
            
        }
        BlockPos entity = new BlockPos(Integer.parseInt(String.valueOf(GPSCommand.x)), 0, Integer.parseInt(String.valueOf(GPSCommand.z)));
        float an = (float) (RotationHelper.getRotations(entity.getX(), 0, entity.getZ())[0] - AnimationHelper.Interpolate(mc.player.rotationYaw, mc.player.prevRotationYaw, 1.0D));
       
        if (GPSCommand.mode != null && GPSCommand.mode.equalsIgnoreCase("on") && entity.getX() != 0 && entity.getZ() != 0) {
            GL11.glPushMatrix();
            int x = scaledresolution.getScaledWidth() / 2;
            int y = scaledresolution.getScaledHeight() / 2;
            int x2 = scaledresolution.getScaledWidth() / 2 - 23;
            int y2 = scaledresolution.getScaledHeight() / 2 + 15;
            mc.mntsb_16.drawCenteredString(MathematicHelper.round((int) mc.player.getDistance(entity.getX(), mc.player.posY, entity.getZ()), 0) + "m", x2 + 25, y2, -1);
	        mc.mntsb_16.drawCenteredString("Way", x2 + 25, y2 + 10, -1);
            GL11.glTranslatef((float) x, (float) y, 0.0F);
            GL11.glRotatef(an % 360.0F + 180.0F, 0.0F, 0.0F, 1.0F);
            GL11.glTranslatef((float) (-x), (float) (-y), 0.0F);
    ...
    private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_) {
        ...
        if (gamesettings.thirdPersonView == 0) {
            ...
            if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug() && !gamesettings.reducedDebugInfo) {
                ...
                GlStateManager.enableAlpha();
                if (!Celestial.instance.featureManager.getFeature(Crosshair.class).isEnabled()) {
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks) {
    	 EntityRenderer.a = (float) Interpolator.LINEAR.interpolate(EntityRenderer.a, this.mc.gameSettings.thirdPersonView > 0 ? this.mc.gameSettings.thirdPersonView == 2 ? 0.9f : 1    : 0, 0.2f);
    	...
    	if (TpCommand.ct) {
   		 ...
   		 String[] coord = a2.split(",");
   		 int[] cd = {(int) Double.parseDouble(coord[0].replaceAll(" ", "")), (int) Double.parseDouble(coord[1].replaceAll(" ", "")), (int) Double.parseDouble(coord[2].replaceAll(" ", ""))};
   		 System.out.println(cd[0] + " " + cd[1] + " " + cd[2]);
   		 endX = (float) cd[0];
   		 endY = (float) cd[1];
   		 endZ = (float) cd[2];
    	...
    	ClickGuiScreen.anim = (float) Interpolator.LINEAR.interpolate(ClickGuiScreen.anim, ClickGuiScreen.a, 0.4f);
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int k = 91;
            this.zLevel = -90.0F;
            anim = AnimationHelper.animation((float) anim, mc.currentScreen instanceof GuiChat ? 1 : 0, (float) (10 * Celestial.deltaTime()));
            this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22-14 * anim, 0, 0, 182, 22);
            this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1-14 * anim, 0, 22, 24, 22);

            if (!itemstack.func_190926_b()) {
                if (enumhandside == EnumHandSide.LEFT) {
                    this.drawTexturedModalRect(i - 91 - 29, sr.getScaledHeight() - 23-14 * anim, 24, 22, 29, 24);
                } else {
                    this.drawTexturedModalRect(i + 91, sr.getScaledHeight() - 23-14 * anim, 53, 22, 29, 24);
                }
            }
            ...
            for (int l = 0; l < 9; ++l) {
                int i1 = i - 90 + l * 20 + 2;
                int j1 = (int) (sr.getScaledHeight() - 16 - 3-14 * anim);
            ...
            if (!itemstack.func_190926_b()) {
                ...
                int l1 = (int) (sr.getScaledHeight() - 16 - 3-14 * anim);
            ...
            if (this.mc.gameSettings.attackIndicator == 2) {
                ...
                if (f1 < 1.0F) {
                    int i2 = (int) (sr.getScaledHeight() - 20-14 * anim);
                    int j2 = i + 91 + 6;
    ...
    public void renderHorseJumpBar(ScaledResolution scaledRes, int x) {
        ...
        int i = 182;
        int j = (int) (f * 183.0F);
        int k = (int) (scaledRes.getScaledHeight() - 32 + 3-14 * anim);
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        ...
        int i = this.mc.player.xpBarCap();
            ...
            int l = (int) (scaledRes.getScaledHeight() - 32-14 * anim);

        this.mc.mcProfiler.endSection();

        if (this.mc.player.experienceLevel > 0) {
        	mc.mntsb_18.drawCenteredStringWithShadow((int) mc.player.experienceLevel + "", scaledRes.getScaledWidth() / 2, l, CustomColors.getExpBarTextColor(8453920));
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int j1 = (int) (scaledRes.getScaledHeight() - 33-14 * anim);
        ...
        if (!AttackAura.targetHud.getBoolValue()) {
        	return;
        }
        DraggableTargetHUD dth = (DraggableTargetHUD) Celestial.instance.draggableHUD.getDraggableComponentByClass(DraggableTargetHUD.class);
        float x = dth.getX();
        float y = dth.getY();
        dth.setWidth(130);
        dth.setHeight(37);
        if (AttackAura.target == null) {
            if (mc.player != null && mc.currentScreen instanceof GuiChat) {
                curTarget = mc.player;
                scale = AnimationHelper.animation((float) scale, (float) 1, (float) (6 * Celestial.deltaTime()));
            } else {
                scale = AnimationHelper.animation((float) scale, (float) 0, (float) (6 * Celestial.deltaTime()));
            }
        } else {
            curTarget = AttackAura.target;
            scale = AnimationHelper.animation((float) scale, (float) 1, (float) (6 * Celestial.deltaTime()));
        }
        String name = NameUtils.name;
        EntityLivingBase target = AttackAura.target;
        if (curTarget != null) {
            if (curTarget instanceof EntityPlayer) {
                try {
                    ...
                    GL11.glTranslated(-(x + 36), -(y + 26), 0);
                        RenderUtils.drawShadow(8, 1, () -> {
                            RenderUtils.drawSmoothRect(x, y, x + dth.getWidth(), y + dth.getHeight(), new Color(30, 30, 30, 255).getRGB());
                        });
                    RenderUtils.drawSmoothRect(x, y, x + dth.getWidth(), y + dth.getHeight() + 0.5f, new Color(30, 30, 30, 255).getRGB());
                    boolean friend2 = false;
                    for (Friend friend : Celestial.instance.friendManager.getFriends()) {
                        if (curTarget.getName().equals(friend.getName())) {
                        	friend2 = true;
                        }
                    }
                    mc.mntsb_16.drawString(friend2 ? Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.streamer.getBoolValue() ? name : curTarget.getName() : Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.streamer.getBoolValue() ? name : curTarget.getName(), x + 39, y + 5, -1);
                    double healthWid = (curTarget.getHealth() / curTarget.getMaxHealth() * 82);
                    healthWid = MathHelper.clamp(healthWid, 0.0D, 90.0D);
                    healthBarWidth = AnimationHelper.animation((float) healthBarWidth, (float) healthWid, (float) (10 * Celestial.deltaTime()));



                    RoundedUtil.drawGradientHorizontal(x + 40, y + 15, (float) healthBarWidth, 6, 0, ClientHelper.getClientColor(), new Color(-1));

                    mc.mntsb_14.drawStringWithShadow((int)(healthBarWidth / 82 * 100) + ".0%", x + 67.5f, y + 16, -1);
                    ...
                    for (NetworkPlayerInfo targetHead : mc.player.connection.getPlayerInfoMap()) {
                        try {
                            if (mc.world.getPlayerEntityByUUID(targetHead.getGameProfile().getId()) == curTarget) {
                                ...
                                Gui.drawScaledCustomSizeModalRect((int) x, y, 8.0f, 8.0f, 8, 8, 37, 37, 64.0f, 64.0f);
                                GL11.glPopMatrix();
                    ...
                    ItemStack heldItemStack = curTarget.getHeldItem(EnumHand.OFF_HAND);
                    for (int i = -1; i < 5; ++i) {
                    	if (i == -1) {
                    	      list.add(heldItemStack);
                    	} else {
                    		ItemStack getEquipmentInSlot = ((EntityPlayer) curTarget).getEquipmentInSlot(i);
                    ...
                    for (ItemStack itemStack : list) {
                    	GlStateManager.pushMatrix();
                        GlStateManager.translate(x + n10 + 187 - 85, y + 88 - 65, 0);
                        GlStateManager.scale(0.83,0.83,0);
                        RenderHelper.enableGUIStandardItemLighting();
                        mc.getRenderItem().renderItemIntoGUI(itemStack, (int) 0,0);
                        mc.getRenderItem().renderItemOverlays(mc.mntsb_18, itemStack,0,0);
                        RenderHelper.disableStandardItemLighting();
                        n10 += 14;
                        GlStateManager.popMatrix();
                    }
                ...
                if (mc.player != null && mc.currentScreen instanceof GuiChat) {
                    ...
                    texture.bind(dth.getX() - 73 + dth.getWidth() / 2 + 10, dth.getY() -  68/ 2 + dth.getHeight() + 5);
                }
            }
        }
    }
