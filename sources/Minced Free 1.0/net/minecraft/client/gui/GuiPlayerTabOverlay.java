...
public class GuiPlayerTabOverlay extends Gui {
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
        ...
        if (this.mc.gameSettings.keyBindPlayerList.isKeyDown()) {
            if (addition < 1.0F) addition += 1.5F * Celestial.deltaTime();
        } else {
            if (addition > 0.0F) addition -= 1.5F * Celestial.deltaTime();
        }
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                if (Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled()) {
                    if (NameProtect.streamer.getBoolValue() && !s4.contains(mc.player.getName())) {
                    	s4 = NameUtils.getName();
                    }
                    s4 = s4.replace(this.mc.player.getName(), NameUtils.getName());
                }
