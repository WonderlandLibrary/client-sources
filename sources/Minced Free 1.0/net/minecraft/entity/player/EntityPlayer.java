...
import ru.smertnix.celestial.command.impl.FakeNameCommand;
import ru.smertnix.celestial.event.EventManager;
import ru.smertnix.celestial.event.events.impl.player.EventUpdateLiving;
import ru.smertnix.celestial.feature.impl.misc.Disabler;
import ru.smertnix.celestial.feature.impl.movement.Flight;
import ru.smertnix.celestial.feature.impl.movement.AutoSprint;
import ru.smertnix.celestial.feature.impl.player.FreeCamera;
import ru.smertnix.celestial.feature.impl.player.NameProtect;
import ru.smertnix.celestial.feature.impl.player.NoClip;
...
public abstract class EntityPlayer extends EntityLivingBase implements CapeHolder {
    ...
    public void onUpdate() {
        this.noClip = Celestial.instance.featureManager.getFeature(NoClip.class).isEnabled() || Celestial.instance.featureManager.getFeature(FreeCamera.class).isEnabled() || this.isSpectator();
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            ...
                            if (Celestial.instance.featureManager.getFeature(AutoSprint.class).isEnabled() && AutoSprint.keep.getBoolValue()) {
                                this.motionX *= 1;
                                this.motionZ *= 1;
                                this.setSprinting(MovementUtils.isMoving());
                            } else {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
    ...
    public String getName() {
        String str = this.gameProfile.getName();
        if (Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled()) {
                str = (Celestial.instance.featureManager.getFeature(NameProtect.class).isEnabled() && (this == (Minecraft.getMinecraft()).player)) ? NameUtils.getName() : this.gameProfile.getName();
        }
        return str;
    }
    ...
    public float getEyeHeight() {
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (this.isElytraFlying() || this.height == 0.6F) {
                if (Flight.flyMode.currentMode.equals("ReallyWorld") || Flight.flyMode.currentMode.equals("ReallyWorld2")) {
                    f = 1.62F;
                } else {
                    f = 0.4F;
                }
