...
public class CustomFont
{
    ...
    public CustomFont(final Minecraft minecraft, final Object obj, final int i, final int j, final int k) {
        ...
        for (int j2 = j; j2 < k; ++j2) {
            g.drawString(new StringBuilder().append((char)j2).toString(), l, i2 + g.getFontMetrics().getAscent());
            this.xPos[j2 - j] = l;
            this.yPos[j2 - j] = i2 - this.metrics.getMaxDescent();
            l += this.metrics.stringWidth(new StringBuilder().append((char)j2).toString()) + 2;
            ...
            for (int l = 0; l < s.length(); ++l) {
                ...
                if (c1 == '\n') {
                    ...
                    if (c1 == '§') {
                        ...
                        i = (int)(i + this.metrics.getStringBounds(new StringBuilder().append(c1).toString(), null).getWidth());
                    }
                }
            }
        }
    ...
    private Rectangle getBounds(final String s) {
        ...
        for (int l = 0; l < s.length(); ++l) {
            ...
            if (c == '\\') {
                ...
                k += this.metrics.stringWidth(new StringBuilder().append(c).toString());
            }
        }
        if (k > i) {
            i = k;
    ...
    private void drawChar(final Gui gui, final char c, final double i, final double j) {
        final Rectangle2D rectangle2d = this.metrics.getStringBounds(new StringBuilder().append(c).toString(), null);
    }
}
