...
public class RenderUtil
{
  ...
  public static void scissorBox(int x, int y, int xend, int yend)
  {
    int width = xend - x;
    int height = yend - y;
    ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft(), getMinecraftdisplayWidth, getMinecraftdisplayHeight);
    int factor = sr.getScaleFactor();
    int bottomY = getMinecraftcurrentScreen.height - yend;
  ...
  public static Point calculateMouseLocation() {
    ...
    while ((scaleFactor < scale) && (displayWidth / (scaleFactor + 1) >= 320) && (displayHeight / (scaleFactor + 1) >= 240))
      scaleFactor++;
    return new Point(Mouse.getX() / scaleFactor, displayHeight / scaleFactor - Mouse.getY() / scaleFactor - 1);
  }
}
