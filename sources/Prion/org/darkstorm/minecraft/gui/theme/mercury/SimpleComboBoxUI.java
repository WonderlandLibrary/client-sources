...
public class SimpleComboBoxUI extends org.darkstorm.minecraft.gui.theme.AbstractComponentUI<ComboBox>
{
  private final MercuryTheme theme;
  
  SimpleComboBoxUI(MercuryTheme theme)
  {
    ...
    if ((x <= component.getWidth()) && 
      ...
      for (int i = 0; i < elements.length; i++) {
        if (i != component.getSelectedIndex())
        {
          if (y >= offset)
          {
            if (y <= offset + theme.getFontRenderer().FONT_HEIGHT) {
