...
public class CrashReportCategory
{
  ...
  {
    children.add(new Entry(sectionName, value));
  }
  ...
  public boolean firstTwoElementsOfStackTraceMatch(StackTraceElement s1, StackTraceElement s2)
  {
    if ((stackTrace.length != 0) && (s1 != null))
    {
      ...
      {
        if ((s2 != null ? 1 : 0) != (stackTrace.length > 1 ? 1 : 0))
        {
          return false;
        }
        if ((s2 != null) && (!stackTrace[1].equals(s2)))
        {
          return false;
        }
  ...
  {
    int var4 = Block.getIdFromBlock(blockIn);
    category.addCrashSectionCallable("Block type", new Callable()
    {
      ...
      {
        try {
          return String.format("ID #%d (%s // %s)", new Object[] { Integer.valueOf(val$var4), blockIn.getUnlocalizedName(), blockIn.getClass().getCanonicalName() });
        }
    ...
    category.addCrashSectionCallable("Block data value", new Callable()
    {
      ...
      public String call() {
        if (val$blockData < 0)
        {
          return "Unknown? (Got " + val$blockData + ")";
        }
        

        String var1 = String.format("%4s", new Object[] { Integer.toBinaryString(val$blockData) }).replace(" ", "0");
        return String.format("%1$d / 0x%1$X / 0b%2$s", new Object[] { Integer.valueOf(val$blockData), var1 });
      }
    ...
    category.addCrashSectionCallable("Block location", new Callable()
    {
      ...
      public String call() {
        return CrashReportCategory.getCoordinateInfo(CrashReportCategory.this);
      }
    });
  }
