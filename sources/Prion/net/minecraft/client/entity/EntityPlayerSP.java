...
public class EntityPlayerSP extends AbstractClientPlayer
{
  ...
  {
    getValuesprepitch = mc.thePlayer.rotationPitch;
    Float temppitch = Float.valueOf(mc.thePlayer.rotationPitch);
    for (Module m : ModuleManager.moduleList) {
      if (m.isEnabled()) {
        m.onPreMotionUpdate();
      }
    }
    mc.thePlayer.rotationPitch = getValuesprepitch;
    ...
    if (func_175160_A())
    {
      ...
      mc.thePlayer.rotationPitch = temppitch.floatValue();
      for (Module m : ModuleManager.moduleList) {
        if (m.isEnabled()) {
  ...
  {
    Module.outboundmessage = p_71165_1_;
    getValuesoutboundmessage = p_71165_1_;
    for (Module m : ModuleManager.moduleList) {
      if (m.isEnabled()) {
        m.onChatSend();
      }
    }
    p_71165_1_ = getValuesoutboundmessage;
    if (p_71165_1_.startsWith(".")) {
      String cmdname = p_71165_1_.substring(1);
      String[] message = cmdname.split(" ");
      String[] args = (String[])Arrays.copyOfRange(message, 1, message.length);
      boolean triggered = false;
      for (Command cmd : CommandManager.commandList) {
        if (message[0].equalsIgnoreCase(cmd.getName())) {
          cmd.execute(args);
          triggered = true;
        }
      }
      if (!triggered) {
        me.hexxed.mercury.util.Util.sendInfo("Couldn't find the command §b" + message[0]);
      }
      return;
    }
  ...
  public void onLivingUpdate()
  {
    ...
    if (inPortal)
    {
      ...
      {
        mc.getSoundHandler().playSound(PositionedSoundRecord.createPositionedSoundRecord(new ResourceLocation("portal.trigger"), rand.nextFloat() * 0.4F + 0.8F));
      }
    ...
    if ((!ModuleManager.getModByName("NoSlow").isEnabled()) && 
      (isUsingItem()) && (!isRiding()))
    {
    ...
    if ((!ModuleManager.getModByName("Phase").isEnabled()) && (!ModuleManager.getModByClassName("Vclip").isEnabled())) {
    ...
    if ((!isSprinting()) && (movementInput.moveForward >= var3) && (var5) && (!isUsingItem()) && (!isPotionActive(Potion.blindness)) && (mc.gameSettings.keyBindSprint.getIsKeyPressed()))
    {
      setSprinting(true);
    }
    
    if (((!ModuleManager.getModByName("Sprint").isEnabled()) || (!getValuesbwsprint) || ((!mc.gameSettings.keyBindForward.pressed) && (!mc.gameSettings.keyBindBack.pressed) && (!mc.gameSettings.keyBindLeft.pressed) && (!mc.gameSettings.keyBindRight.pressed))) && (isSprinting()) && ((movementInput.moveForward < var3) || (isCollidedHorizontally) || (!var5)))
    {
      setSprinting(false);
    }
