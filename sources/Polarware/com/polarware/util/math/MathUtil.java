package com.polarware.util.math;
...
public class MathUtil {
    ...
    /**
     * Generates less predictable random number between 2 numbers using median of these numbers.
     * Should be used to bypass autoclicker checks.
     *
     * @param min minimal number
     * @param max maximal number
     * @return random between both numbers
    ...
    public double getRandomMedian(double min, double max) {
        double median = (min + max) / 2;
        double k = ThreadLocalRandom.current().nextDouble();
        if (k < 0.5) {
            return ThreadLocalRandom.current().nextDouble(min, median);
        } else {
            return ThreadLocalRandom.current().nextDouble(median, max);
        }

    }
    public double getRandomMedian(long minTime, long maxTime) {
        double result;
        double median = (double) (minTime + maxTime) / 2;
        double k = ThreadLocalRandom.current().nextDouble();
        if (k < 0.5) {
            result = median + ThreadLocalRandom.current().nextDouble() * 100;
            if (result > maxTime) {
                result = maxTime;
            }
        } else {
