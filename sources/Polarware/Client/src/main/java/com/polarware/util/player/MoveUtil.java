package com.polarware.util.player;
...
public class MoveUtil implements InstanceAccess {
    ...
    public static final double[] ALL_LEGIT_JUMP_MOTIONS = new double[]{
            0.41999998688697815,
            0.33319999363422365,
            0.24813599859094576,
            0.16477328182606651,
    ...
    public double getAllowedHorizontalDistance() {
        ...
        if (useBaseModifiers) {
            ...
            final ScaffoldModule scaffoldModule = Client.INSTANCE.getModuleManager().get(ScaffoldModule.class);

            if (mc.thePlayer.isPotionActive(Potion.moveSpeed) && mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).duration > 0 && !(scaffoldModule.isEnabled() && scaffoldModule.ignoreSpeed.getValue())) {
    ...
    public static void hClip(final double offset) {
        float direction = (float) direction(mc.thePlayer.rotationYaw, mc.thePlayer.moveForward, mc.thePlayer.moveStrafing);
        mc.thePlayer.setPosition(mc.thePlayer.posX + -MathHelper.sin(direction) * offset, mc.thePlayer.posY, mc.thePlayer.posZ + MathHelper.cos(direction) * offset);
    }
}
