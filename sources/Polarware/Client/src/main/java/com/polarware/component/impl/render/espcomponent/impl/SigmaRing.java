package com.polarware.component.impl.render.espcomponent.impl;
...
public class SigmaRing extends ESP implements InstanceAccess {
    ...
    public void render3D() {
        ...
        for (float i = 0; i <= Math.PI * 2 + ((Math.PI * 2) / 32.F); i += (Math.PI * 2) / 32.F) {
            ...
            RenderUtil.color(ColorUtil.withAlpha(color, (int) (255)));
        ...
        for (float i = 0; i <= Math.PI * 2 + (Math.PI * 2) / 32.F; i += (Math.PI * 2) / 32.F) {
            ...
            RenderUtil.color(ColorUtil.withAlpha(color, 255));
