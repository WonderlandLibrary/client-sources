...
public class ItemRenderer {
     ...
     */
    public static float animationProgressio = 0;
    public void renderItemInFirstPerson(final float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            animationProgressio = 1.0F - (this.prevEquippedProgress + (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
            ...
            if (this.itemToRender != null) {
                ...
                final RenderItemEvent event = new RenderItemEvent(enumaction, useItem, animationProgressio, partialTicks, swingProgress, itemToRender);
                Client.INSTANCE.getEventBus().handle(event);
                enumaction = event.getEnumAction();
                useItem = event.isUseItem();
                animationProgressio = event.getAnimationProgression();
                swingProgress = event.getSwingProgress();

                if (this.itemToRender.getItem() instanceof ItemMap) {
                    this.renderItemMap(abstractclientplayer, f2, animationProgressio, swingProgress);
                } else if (useItem) {
                    if (!event.isCancelled()) {
                        switch (enumaction) {
                            case NONE:
                                this.transformFirstPersonItem(animationProgressio, 0.0F);
                                break;
                    ...
                    this.transformFirstPersonItem(animationProgressio, swingProgress);
                }

                this.renderItem(abstractclientplayer, this.itemToRender, ItemCameraTransforms.TransformType.FIRST_PERSON);
            } else if (!abstractclientplayer.isInvisible()) {
                this.func_178095_a(abstractclientplayer, animationProgressio, swingProgress);
            }
