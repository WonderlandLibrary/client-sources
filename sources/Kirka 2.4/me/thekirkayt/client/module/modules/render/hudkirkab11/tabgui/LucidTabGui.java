 ...
 */
package me.thekirkayt.client.module.modules.render.hudkirkab11.tabgui;
...
import me.thekirkayt.client.module.modules.render.hudkirkab11.tabgui.TabGui;
...
public class LucidTabGui
extends TabGui {
    private static final int NO_COLOR = -11184641;
    private static final int INSIDE_COLOR = -16777216;
    ...
    public LucidTabGui(String name, boolean value, Module module) {
    ...
    public boolean onRender(Render2DEvent event) {
        if (super.onRender(event)) {
            int i;
            String name;
            LucidTabGui.updateBars();
            RenderUtils.rectangle(2.0, 14.0, 35 + baseCategoryWidth, baseCategoryHeight, -16777216);
            RenderUtils.rectangle(1.5, categoryPosition, 35 + baseCategoryWidth, categoryPosition + 14, -11184641);
            int yPos = 15;
            int yPosBottom = 29;
            for (i = 0; i < Module.Category.values().length; ++i) {
                Module.Category category = Module.Category.values()[i];
                if (category == Module.Category.Bypass) continue;
                ...
                if (categoryTab == i) {
                    ...
                    ClientUtils.clientFont().drawStringWithShadow(name, baseCategoryWidth / ClientUtils.clientFont().getStringWidth(name) / 2 + 5, yPos + 3, categoryTab == i ? -1 : -6052957);
                }
                yPos += 14;
                yPosBottom += 14;
            }
            if (section == Section.MODS) {
                RenderUtils.rectangle(baseCategoryWidth + 40, categoryPosition - 1, baseCategoryWidth + 55 + baseModWidth, categoryPosition + LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size() * 14 + 1, -16777216);
                RenderUtils.rectangle(baseCategoryWidth + 40, modPosition, baseCategoryWidth + baseModWidth + 55, modPosition + 14, -11184641);
                yPos = categoryPosition;
                yPosBottom = categoryPosition + 14;
                for (i = 0; i < LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size(); ++i) {
                    Module mod = LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).get(i);
                    name = mod.getDisplayName();
                    if (modTab == i) {
                        ClientUtils.clientFont().drawStringWithShadow(name, baseCategoryWidth + 48 + baseModWidth / 2 - ClientUtils.clientFont().getStringWidth(name) / 2 + 3, yPos + 3, modTab == i ? (mod.isEnabled() ? -1 : -3026479) : (mod.isEnabled() ? -11184641 : -6052957));
                    } else {
                        ClientUtils.clientFont().drawStringWithShadow(name, baseCategoryWidth + 45 + baseModWidth / 2 - ClientUtils.clientFont().getStringWidth(name) / 2 + 3, yPos + 3, modTab == i ? (mod.isEnabled() ? -1 : -3026479) : (mod.isEnabled() ? -11184641 : -6052957));
                    }
                    yPos += 14;
                    yPosBottom += 14;
                }
        ...
        block17 : {
            ...
            block18 : {
                ...
                switch (key) {
                    case 205: {
                        int highestWidth = 0;
                        for (Module module : LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab])) {
                            String name = StringUtils.capitalize((String)module.getDisplayName().toLowerCase());
                            int stringWidth = ClientUtils.clientFont().getStringWidth(name);
                            highestWidth = Math.max(stringWidth, highestWidth);
                        }
                        baseModWidth = highestWidth + 6;
                        baseModHeight = LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size() * 14 + 2;
                    ...
                    case 200: {
                        ...
                        transitionQuickly = true;
                        categoryTargetPosition = 15 + (Module.Category.values().length - 2) * 14;
            ...
            if (section == Section.MODS) {
                switch (key) {
                    ...
                    case 205: {
                        Module mod = LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).get(modTab);
                        mod.toggle();
                        break;
                    }
                    case 208: {
                        modTargetPosition += 14;
                        if (++modTab <= LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size() - 1) break;
                        transitionQuickly = true;
                    ...
                    case 200: {
                        ...
                        if (--modTab >= 0) break;
                        transitionQuickly = true;
                        modTargetPosition = categoryTargetPosition + (LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size() - 1) * 14;
                        modTab = LucidTabGui.getModsInCategory(Module.Category.values()[categoryTab]).size() - 1;
                    }
                }
            }
        }
