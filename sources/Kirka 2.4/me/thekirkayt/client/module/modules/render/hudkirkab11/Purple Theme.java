...
public class Purple Theme
extends Theme {
    public Purple Theme(String string, boolean bl, Module module) {
    ...
    public boolean onRender(Render2DEvent render2DEvent) {
        if (super.onRender(render2DEvent)) {
            ...
            ClientUtils.clientFont().drawStringWithShadow(String.format("\u00a7d\u00a7lK\u00a77\u00a7lirka \u00a7d\u00a7lb\u00a77\u00a7l11", string), 2.0, 2.0, 16733695);
            ClientUtils.clientFont().drawStringWithShadow(String.format("", Math.round(ClientUtils.x()), Math.round(ClientUtils.y()), Math.round(ClientUtils.z())), 2.0, 14.0, 16733695);
            int n = 2;
            for (Module module : ModuleManager.getModulesForRender()) {
                if (!module.drawDisplayName(render2DEvent.getWidth() - ClientUtils.clientFont().getStringWidth(String.format("%s" + (module.getSuffix().length() > 0 ? " [%s]" : ""), module.getDisplayName(), module.getSuffix())) - 2, n, 16733695)) continue;
                n += 10;
            }
        }
