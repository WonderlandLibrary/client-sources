...
import Squad.Squad;
import Squad.Modules.Movement.ItemPhysics;
...
public class RenderEntityItem extends Render
{
    ...
    private int func_177077_a(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_)
    {
        ...
        else
        {
            ...
            float var15 = 0.0f;
            if(Squad.moduleManager.getModule(ItemPhysics.class).toggled){
            	var15 = -0.2f;
            }
            if(!Squad.moduleManager.getModule(ItemPhysics.class).toggled){
            	 var15 = MathHelper.sin(((float)itemIn.getAge() + p_177077_8_) / 10.0F + itemIn.hoverStart) * 0.1F + 0.1F;
            }
            ...
            if ((var12) || ((this.renderManager.options != null) && (this.renderManager.options.fancyGraphics))) {
              if (Squad.moduleManager.getModule(ItemPhysics.class).toggled)
              {
                float var16 = ((itemIn.getAge() + p_177077_8_) / 20.0F + itemIn.hoverStart) * 254.6479F;
                if (itemIn.onGround) {
                  GlStateManager.rotate(90.0F, 5.0F, 0.0F, 0.0F);
                } else {
                  GlStateManager.rotate(var16, 5.0F, 0.0F, 0.0F);
                }
              }
            }
            if (!Squad.moduleManager.getModule(ItemPhysics.class).toggled)
            {
              float var16 = ((itemIn.getAge() + p_177077_8_) / 20.0F + itemIn.hoverStart) * 57.295776F;
