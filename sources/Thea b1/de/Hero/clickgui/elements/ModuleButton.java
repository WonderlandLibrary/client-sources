...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Duckware.setmgr.getSettingsByMod(imod) != null)
			for (Setting s : Duckware.setmgr.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (mod.isToggled()) {
			Gui.drawRect((int) (x - 2), (int) y, (int) (x + width + 2), (int) (y + height + 1), new Color(200, 140, 244, 255).getRGB());
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Duckware.clickgui.closeAllSettings();
				this.extended = b;
	...
	public boolean keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (listening) {
			if (keyCode != Keyboard.KEY_ESCAPE) {
				Duckware.moduleManager.getModuleByName(String.valueOf(mod)).setKeyBind(keyCode);
			} else {
				Duckware.moduleManager.getModuleByName(String.valueOf(mod)).setKeyBind(Keyboard.KEY_NONE);
			}
			listening = false;
			return true;
		}
