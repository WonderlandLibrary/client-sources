...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                if (entity instanceof EntityPlayerSP) {
                    final boolean killaura = Duckware.moduleManager.getModuleByName("Killaura").isEnabled() && Killaura.target != null;
                    boolean scaffold = Duckware.moduleManager.getModuleByName("Scaffold").isEnabled();
                    if (killaura || scaffold)
    ...
    private boolean setScoreTeamColor(T entityLivingBaseIn) {
        float f1, f2, f;
        if (entityLivingBaseIn.hurtTime > 0) {
            ...
            Color color = new Color(0, 140, 244, 255);
            f1 = color.getRed() / 255f;
            f2 = color.getGreen() / 255f;
            f = color.getBlue() / 255f;
        }
