...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            try {
                boolean safewalk = Duckware.moduleManager.getModuleByName("Scaffold").isEnabled() && !Scaffold.legit;
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        Movefix movefix = new Movefix (this.rotationYaw, friction);
        EventManager.call(movefix);
        if (movefix.isCancellable()){
            return;
        }
        ...
        if (f >= 1.0E-4F) {
            ...
            f = movefix.getFriction() / f;
            strafe = strafe * f;
            forward = forward * f;
            float f1 = MathHelper.sin(movefix.getYaw() * (float) Math.PI / 180.0F);
            float f2 = MathHelper.cos(movefix.getYaw() * (float) Math.PI / 180.0F);
    ...
    protected final Vec3 getVectorForRotation(float pitch, float yaw) {
        boolean scaffold = Duckware.moduleManager.getModuleByName("Scaffold").isEnabled();
        float f1 = 0;
        float f2 = 0;
        float f3 = 0;
        float f = 0;
        try {
            f = scaffold ? MathHelper.cos(-EventPreMotion.getYaw() * 0.017453292F - (float) Math.PI) : MathHelper.cos(-yaw * 0.017453292F - (float) Math.PI);
            f1 = scaffold ? MathHelper.sin(-EventPreMotion.getYaw() * 0.017453292F - (float) Math.PI) : MathHelper.sin(-yaw * 0.017453292F - (float) Math.PI);
            f2 = scaffold ? -MathHelper.cos(-EventPreMotion.getPitch() * 0.017453292F) : -MathHelper.cos(-pitch * 0.017453292F);
            f3 = scaffold ? MathHelper.sin(-EventPreMotion.getPitch() * 0.017453292F) : MathHelper.sin(-pitch * 0.017453292F);
