   ...
   public List getTooltip(EntityPlayer playerIn, boolean advanced) {
      ...
      if (this.hasTagCompound()) {
         ...
         if (this.stackTagCompound.hasKey("display", 10)) {
            ...
            if (var16.hasKey("color", 3)) {
               if (advanced) {
                  var3.add("ColorCommand: #" + Integer.toHexString(var16.getInteger("color")).toUpperCase());
               } else {
      ...
      if (!var17.isEmpty() && (var14 & 2) == 0) {
         ...
         while(var19.hasNext()) {
            ...
            if (var10 > 0.0D) {
               var3.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + var22.getOperation(), DECIMALFORMAT.format(var12), StatCollector.translateToLocal("attribute.name." + (String)var21.getKey())));
            } else if (var10 < 0.0D) {
               var12 *= -1.0D;
               var3.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + var22.getOperation(), DECIMALFORMAT.format(var12), StatCollector.translateToLocal("attribute.name." + (String)var21.getKey())));
            }
         }
      }
