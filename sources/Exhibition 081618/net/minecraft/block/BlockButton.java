...
public abstract class BlockButton extends Block {
   ...
   public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
      return worldIn.getBlockState(pos.offset(facing.getOpposite())).getBlock().isNormalCube() ? this.getDefaultState().withProperty(FACING_PROP, facing).withProperty(POWERED_PROP, false) : this.getDefaultState().withProperty(FACING_PROP, EnumFacing.DOWN).withProperty(POWERED_PROP, false);
   }
   ...
   public IBlockState getStateFromMeta(int meta) {
      ...
      return this.getDefaultState().withProperty(FACING_PROP, var2).withProperty(POWERED_PROP, (meta & 8) > 0);
   }
