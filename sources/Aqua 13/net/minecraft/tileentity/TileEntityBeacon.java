...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, IInventory {
   ...
   private void addEffectsToPlayers() {
      if (this.isComplete && this.levels > 0 && !this.worldObj.isRemote && this.primaryEffect > 0) {
         ...
         int k = this.pos.getY();
         int l = this.pos.getZ();
         AxisAlignedBB axisalignedbb = new AxisAlignedBB((double)j, (double)k, (double)l, (double)(j + 1), (double)(k + 1), (double)(l + 1))
            .expand(d0, d0, d0)
            .addCoord(0.0, (double)this.worldObj.getHeight(), 0.0);
      ...
      if (!this.worldObj.isRemote && this.levels == 4 && i < this.levels) {
         for(EntityPlayer entityplayer : this.worldObj
            .getEntitiesWithinAABB(
               EntityPlayer.class, new AxisAlignedBB((double)j, (double)k, (double)l, (double)j, (double)(k - 4), (double)l).expand(10.0, 5.0, 10.0)
            )) {
   ...
   private int func_183001_h(int p_183001_1_) {
      if (p_183001_1_ >= 0 && p_183001_1_ < Potion.potionTypes.length && Potion.potionTypes[p_183001_1_] != null) {
         ...
         return potion != Potion.moveSpeed
               && potion != Potion.digSpeed
               && potion != Potion.resistance
               && potion != Potion.jump
               && potion != Potion.damageBoost
   ...
   public ItemStack decrStackSize(int index, int count) {
      if (index != 0 || this.payment == null) {
         return null;
      } else if (count >= this.payment.stackSize) {
