...
public abstract class Entity implements ICommandSender {
   ...
   public void moveFlying(float strafe, float forward, float friction) {
      ...
      if (f >= 1.0E-4F) {
         ...
         try {
            boolean killAura = Aqua.moduleManager.getModuleByName("Killaura").isToggled()
               && Killaura.target != null
               && Aqua.setmgr.getSetting("KillauraCorrectMovement").isState();
            boolean bedAura = Aqua.moduleManager.getModuleByName("BedAura").isToggled()
                  && BedAura.rotating
                  && Aqua.setmgr.getSetting("BedAuraMoveFixMode").getCurrentMode().equalsIgnoreCase("Normal")
                  && Aqua.setmgr.getSetting("BedAuraCorrectMovement").isState()
               || Aqua.moduleManager.getModuleByName("BedAura").isToggled()
                  && BedAura.rotating
            ...
            if (!killAura && !bedAura) {
               ...
               float f1 = MathHelper.sin(RotationUtil.yaw * (float) Math.PI / 180.0F);
               float f2 = MathHelper.cos(RotationUtil.yaw * (float) Math.PI / 180.0F);
               this.motionX += (double)(strafe * f2 - forward * f1);
               this.motionZ += (double)(forward * f2 + strafe * f1);
            }
   ...
   protected final Vec3 getVectorForRotation(float pitch, float yaw) {
      boolean build = Aqua.moduleManager.getModuleByName("Scaffold").isToggled()
         || Aqua.moduleManager.getModuleByName("BedAura").isToggled() && BedAura.rotating
         || Aqua.moduleManager.getModuleByName("Longjump").isToggled()
         || Aqua.moduleManager.getModuleByName("Killaura").isToggled() && Killaura.target != null && Aqua.setmgr.getSetting("KillauraLegitAttack").isState();
      float f = MathHelper.cos(
         build ? -this.getRotationYawHead() * (float) (Math.PI / 180.0) - (float) Math.PI : -yaw * (float) (Math.PI / 180.0) - (float) Math.PI
      );
      float f1 = MathHelper.sin(
         build ? -this.getRotationYawHead() * (float) (Math.PI / 180.0) - (float) Math.PI : -yaw * (float) (Math.PI / 180.0) - (float) Math.PI
      );
      float f2 = -MathHelper.cos(build ? -this.rotationPitchHead * (float) (Math.PI / 180.0) : -pitch * (float) (Math.PI / 180.0));
      float f3 = MathHelper.sin(build ? -this.rotationPitchHead * (float) (Math.PI / 180.0) : -pitch * (float) (Math.PI / 180.0));
      ...
      if (this.timeUntilPortal > 0) {
         ...
         if (!this.worldObj.isRemote && !pos.equals(this.lastPortalPos)) {
            ...
            double d0 = blockpattern$patternhelper.getFinger().getAxis() == EnumFacing.Axis.X
               ? (double)blockpattern$patternhelper.getPos().getZ()
            ...
            double d2 = MathHelper.func_181160_c(
               this.posY - 1.0,
               (double)blockpattern$patternhelper.getPos().getY(),
               (double)(blockpattern$patternhelper.getPos().getY() - blockpattern$patternhelper.func_181119_e())
            );
