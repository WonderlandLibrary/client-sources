...
public class EntityArmorStand extends EntityLivingBase {
      ...
      if (list != null && !list.isEmpty()) {
         for(int i = 0; i < list.size(); ++i) {
            ...
            if (entity instanceof EntityMinecart
               ...
               && this.getDistanceSqToEntity(entity) <= 0.2) {
   ...
   public boolean interactAt(EntityPlayer player, Vec3 targetVec3) {
      if (this.hasMarker()) {
         ...
         double d3 = flag1 ? targetVec3.yCoord * 2.0 : targetVec3.yCoord;
         if (d3 >= 0.1 && d3 < 0.1 + (flag1 ? 0.8 : 0.45) && this.contents[1] != null) {
            j = 1;
         } else if (d3 >= 0.9 + (flag1 ? 0.3 : 0.0) && d3 < 0.9 + (flag1 ? 1.0 : 0.7) && this.contents[3] != null) {
            j = 3;
         } else if (d3 >= 0.4 && d3 < 0.4 + (flag1 ? 1.0 : 0.8) && this.contents[2] != null) {
            j = 2;
