...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
   ...
   public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
      if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
         ...
         try {
            ...
            float f7 = !Aqua.moduleManager.getModuleByName("Scaffold").isToggled()
                  ...
                  && (!Aqua.moduleManager.getModuleByName("Killaura").isToggled() || Killaura.target == null)
               ? entity.prevRotationPitch + 0.0F * partialTicks
               : entity.rotationPitchHead;
   ...
   protected boolean setScoreTeamColor(T entityLivingBaseIn) {
      ...
      if (Aqua.setmgr.getSetting("ESPVanillaTeamColor").isState()) {
         GlStateManager.color(f1, f2, f, (float)Aqua.setmgr.getSetting("ESPShaderAlpha").getCurrentNumber() / 255.0F);
      } else {
         int[] rgb = Aqua.setmgr.getSetting("ESPAstolfoColors").isState()
            ? ESP.getRGB(ESP.SkyRainbow(20, 1.0F, 0.5F).getRGB())
            : (
               Aqua.setmgr.getSetting("ESPRainbow").isState()
                  ? ESP.getRGB(ESP.rainbowESP(0))
                  : (
                     Aqua.setmgr.getSetting("ESPClientColor").isState()
                        ? ESP.getRGB(Aqua.setmgr.getSetting("HUDColor").getColor())
                        : ESP.getRGB(Aqua.setmgr.getSetting("ESPColor").getColor())
                  )
            );
         Color color = !Aqua.setmgr.getSetting("ESPFade").isState()
         ...
         GlStateManager.color(
            ...
            (float)Aqua.setmgr.getSetting("ESPShaderAlpha").getCurrentNumber() / 255.0F
         );
      }
   ...
   protected void renderModel(
      T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float scaleFactor
   ) {
