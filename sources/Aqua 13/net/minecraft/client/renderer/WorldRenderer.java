...
public class WorldRenderer {
   ...
   private void growBuffer(int p_181670_1_) {
      if (p_181670_1_ > this.rawIntBuffer.remaining()) {
         ...
         this.rawShortBuffer = this.byteBuffer.asShortBuffer();
         ((Buffer)this.rawShortBuffer).position(l << 1);
   ...
   public void putColorMultiplierRgba(
      float p_putColorMultiplierRgba_1_,
      float p_putColorMultiplierRgba_2_,
      float p_putColorMultiplierRgba_3_,
      float p_putColorMultiplierRgba_4_,
      int p_putColorMultiplierRgba_5_
   ) {
   ...
   public void quadsToTriangles() {
      if (this.drawMode == 7) {
         ...
         for(int k = 0; k < this.vertexCount; k += 4) {
            ((Buffer)this.byteBuffer).limit((k + 3) * i);
            ((Buffer)this.byteBuffer).position(k * i);
            this.byteBufferTriangles.put(this.byteBuffer);
            ((Buffer)this.byteBuffer).limit((k + 1) * i);
            ((Buffer)this.byteBuffer).position(k * i);
            this.byteBufferTriangles.put(this.byteBuffer);
            ((Buffer)this.byteBuffer).limit((k + 2 + 2) * i);
            ((Buffer)this.byteBuffer).position((k + 2) * i);
            this.byteBufferTriangles.put(this.byteBuffer);
         }

         ((Buffer)this.byteBuffer).limit(j);
         ((Buffer)this.byteBuffer).rewind();
