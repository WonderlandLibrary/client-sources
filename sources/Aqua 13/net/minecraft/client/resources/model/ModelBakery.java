...
public class ModelBakery {
      ...
      for(ModelResourceLocation modelresourcelocation : p_177591_1_) {
         try {
            ...
            try {
               ...
               LOGGER.warn("Unable to load variant: " + modelresourcelocation.getVariant() + " from " + modelresourcelocation, (Throwable)var6);
            }
         } catch (Exception var7) {
            LOGGER.warn("Unable to load definition " + modelresourcelocation, (Throwable)var7);
         }
      }
   }
         ...
         for(ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
            ...
            if (this.models.get(resourcelocation) == null) {
               try {
                  ...
                  LOGGER.warn("Unable to load block model: '" + resourcelocation + "' for variant: '" + modelresourcelocation + "'", (Throwable)var7);
               }
            }
         }
      }
         ...
         for(String s : this.getVariantNames(item)) {
            ...
            if (this.models.get(resourcelocation) == null) {
               try {
                  ...
                  LOGGER.warn(
                     "Unable to load item model: '" + resourcelocation + "' for item: '" + Item.itemRegistry.getNameForObject(item) + "'", (Throwable)var7
                  );
               }
            }
         }
   ...
   private void bakeBlockModels() {
         ...
         for(ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
            ...
            if (modelblock != null && modelblock.isResolved()) {
               ...
               weightedbakedmodel$builder.add(
                  this.bakeModel(modelblock, modelblockdefinition$variant.getRotation(), modelblockdefinition$variant.isUvLocked()),
      ...
      for(Entry<String, ResourceLocation> entry : this.itemLocations.entrySet()) {
         ...
         } else if (this.isCustomRenderer(modelblock1)) {
         ...
         for(EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
            ...
            if (blockpartface.cullFace != null && flag) {
               simplebakedmodel$builder.addFaceQuad(
                  p_bakeModel_2_.rotate(blockpartface.cullFace),
                  this.makeBakedQuad(blockpart, blockpartface, textureatlassprite1, enumfacing, p_bakeModel_2_, p_bakeModel_3_)
               );
            } else {
   ...
   protected BakedQuad makeBakedQuad(
      BlockPart p_makeBakedQuad_1_,
      BlockPartFace p_makeBakedQuad_2_,
      TextureAtlasSprite p_makeBakedQuad_3_,
      EnumFacing p_makeBakedQuad_4_,
      ITransformation p_makeBakedQuad_5_,
      boolean p_makeBakedQuad_6_
   ) {
      return this.faceBakery
         .makeBakedQuad(
            p_makeBakedQuad_1_.positionFrom,
            p_makeBakedQuad_1_.positionTo,
            p_makeBakedQuad_2_,
            p_makeBakedQuad_3_,
            p_makeBakedQuad_4_,
            p_makeBakedQuad_5_,
            p_makeBakedQuad_1_.partRotation,
            p_makeBakedQuad_6_,
            p_makeBakedQuad_1_.shade
         );
   }
