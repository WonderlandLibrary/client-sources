   ...
   public void draw(int posX, int posY, int mouseX, int mouseY) {
      ...
      for(Module module : Aqua.moduleManager.modules) {
         if (module.getCategory() == this.category) {
            ...
            drawRect(this.x + 2, this.y + add, this.x + this.width - 1, this.y + add + 17, new Color(40, 40, 40).getRGB());
            if (module.isToggled()) {
               Gui.drawRect(this.x + 4, this.y + add, this.x + this.width - 3, this.y + add + 15, this.ACTIVATED_TEXT_COLOR);
            }

            Aqua.INSTANCE.novoline.drawStringWithShadow(module.getName(), (float)this.x + (float)this.width / 2.0F - 43.0F, (float)(this.y + add + 2), -1);
            int modX = this.x + 2;
            int modY = this.y + add;
