...
public class XuluSlider {
	...
	public XuluSlider(XuluBWindow window, Value value, int x, int y) {
		this.window = window;
	...
	public XuluSlider(XuluBWindow window, Value value, int x, int y, float maxValue) {
		this.window = window;
		this.sliderValue = value;
		this.xPos = x;
		this.yPos = y;
	...
	public XuluSlider(XuluBWindow window, Value value, int x, int y, float minValue, float maxValue, boolean shouldRound) {
		this.window = window;
		this.sliderValue = value;
		this.xPos = x;
		this.yPos = y;
	...
	public void draw(int x) {
		...
		Fonts.roboto18.drawStringWithShadow(sliderValue.getName() + ": " + format.format(sliderValue.getValue()), xPos + 1 + window.dragX, yPos - 3 + window.dragY, 0xFFFFFF);

		RenderUtils.drawBetterBorderedRect(xPos + window.dragX, yPos + 9 + window.dragY, xPos + this.drawSliderWidth + window.dragX + 1, yPos + 17 + window.dragY, 0.5F, 0xFF000000, 0xFF383b42);
		RenderUtils.drawBetterBorderedRect(xPos + window.dragX + 1, yPos + 10 + window.dragY, xPos + 5 + window.dragX + (int)dragX, yPos + 16 + window.dragY, 0.5F, 0xFF000000, Wrapper.getRainbowClass().getRainbow(6000, -15 * 4));
		RenderUtils.drawBetterBorderedRect(xPos + 2 + window.dragX + (int)dragX, yPos + 10 + window.dragY, xPos + 5 + window.dragX + (int)dragX, yPos + 16 + window.dragY, 0.5F, 0xFFfffff, Wrapper.getRainbowClass().getRainbow(6000, -15 * 4));
		//ff3232
