...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   private static final Map playerInfoMap = Maps.newHashMap();
   public int currentServerMaxPlayers = 20;
   private boolean field_147308_k = false;
   private final Random avRandomizer = new Random();
   public static float lastYaw;
   public static float lastPitch;
   public static ArrayList invQue = new ArrayList();
   public static C16PacketClientStatus openPacket;
   // $FF: synthetic field
   ...
   public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
      ...
      if(packetIn.getType() == 10) {
         ...
         Entity aentity = this.clientWorldController.getEntityByID(packetIn.func_149009_m());
         if(aentity instanceof EntityPlayer) {
            entity = new EntityFishHook(this.clientWorldController, d0, d1, d2, (EntityPlayer)aentity);
         }
      ...
      if(entity != null) {
         ...
         if(var12 != null) {
            int entity2 = packetIn.getEntityID() - ((Entity)entity).getEntityId();

            for(int j = 0; j < var12.length; ++j) {
               var12[j].setEntityId(var12[j].getEntityId() + entity2);
            }
         }
         ...
         if(packetIn.func_149009_m() > 0) {
            if(packetIn.getType() == 60) {
               ...
               if(var13 instanceof EntityLivingBase && entity instanceof EntityArrow) {
                  ((EntityArrow)entity).shootingEntity = var13;
               }
            }
   ...
   public void handleSpawnExperienceOrb(S11PacketSpawnExperienceOrb packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      EntityXPOrb entity = new EntityXPOrb(this.clientWorldController, (double)packetIn.getX() / 32.0D, (double)packetIn.getY() / 32.0D, (double)packetIn.getZ() / 32.0D, packetIn.getXPValue());
   ...
   public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
      ...
      if(entity != null) {
         ...
         if(entity == Minecraft.getMinecraft().thePlayer && Violence.getPacketHandler().handle("velocity", new Object[]{Integer.valueOf(packetIn.getMotionX()), Integer.valueOf(packetIn.getMotionY()), Integer.valueOf(packetIn.getMotionZ())})) {
            setVelocity = false;
         }
   ...
   public void handleMultiBlockChange(S22PacketMultiBlockChange packetIn) {
      ...
      int var4 = (var5 = packetIn.getChangedBlocks()).length;

      for(int var3 = 0; var3 < var4; ++var3) {
         S22PacketMultiBlockChange.BlockUpdateData s22packetmultiblockchange$blockupdatedata = var5[var3];
   ...
   public void addToSendQueue(Packet p_147297_1_) {
      if(p_147297_1_ instanceof C03PacketPlayer) {
         p_147297_1_ = this.handleC03Packet(p_147297_1_);
      }
   ...
   public void handleCollectItem(S0DPacketCollectItem packetIn) {
      ...
      Object entitylivingbase = (EntityLivingBase)this.clientWorldController.getEntityByID(packetIn.getEntityID());
      ...
      if(entity != null) {
         ...
         this.gameController.effectRenderer.addEffect(new EntityPickupFX(this.clientWorldController, entity, (Entity)entitylivingbase, 0.5F));
   ...
   public void handleSpawnMob(S0FPacketSpawnMob packetIn) {
      ...
      if(aentity != null) {
         int list = packetIn.getEntityID() - entitylivingbase.getEntityId();

         for(int j = 0; j < aentity.length; ++j) {
            aentity[j].setEntityId(aentity[j].getEntityId() + list);
         }
      }
      ...
      if(var14 != null) {
         entitylivingbase.getDataWatcher().updateWatchedObjectsFromList(var14);
      }
   ...
   public void handleEntityAttach(S1BPacketEntityAttach packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      Object entity = this.clientWorldController.getEntityByID(packetIn.getEntityId());
   ...
   public void handleOpenWindow(S2DPacketOpenWindow packetIn) {
      ...
      if("minecraft:container".equals(packetIn.getGuiId())) {
         ...
         Entity containerlocalmenu = this.clientWorldController.getEntityByID(packetIn.getEntityId());
         if(containerlocalmenu instanceof EntityHorse) {
            entityplayersp.displayGUIHorse((EntityHorse)containerlocalmenu, new AnimalChest(packetIn.getWindowTitle(), packetIn.getSlotCount()));
            entityplayersp.openContainer.windowId = packetIn.getWindowId();
         }
         ...
         ContainerLocalMenu containerlocalmenu1 = new ContainerLocalMenu(packetIn.getGuiId(), packetIn.getWindowTitle(), packetIn.getSlotCount());
         entityplayersp.displayGUIChest(containerlocalmenu1);
         entityplayersp.openContainer.windowId = packetIn.getWindowId();
      }
   ...
   public void handleSetSlot(S2FPacketSetSlot packetIn) {
      ...
      if(packetIn.func_149175_c() == -1) {
         ...
         if(this.gameController.currentScreen instanceof GuiContainerCreative) {
            GuiContainerCreative itemstack = (GuiContainerCreative)this.gameController.currentScreen;
            flag = itemstack.getSelectedTabIndex() != CreativeTabs.tabInventory.getTabIndex();
         }

         if(packetIn.func_149175_c() == 0 && packetIn.func_149173_d() >= 36 && packetIn.func_149173_d() < 45) {
            ItemStack itemstack1 = entityplayer.inventoryContainer.getSlot(packetIn.func_149173_d()).getStack();
            if(packetIn.func_149174_e() != null && (itemstack1 == null || itemstack1.stackSize < packetIn.func_149174_e().stackSize)) {
   ...
   public Packet handleC03Packet(Packet packet) {
      if(!String.valueOf(RotationHandler.server_yaw).equalsIgnoreCase("nan") && !String.valueOf(RotationHandler.server_pitch).equalsIgnoreCase("nan")) {
         boolean useNext = RotationHandler.server_yaw != Minecraft.getMinecraft().thePlayer.rotationYaw || RotationHandler.server_pitch != Minecraft.getMinecraft().thePlayer.rotationPitch;
         float nextYaw = RotationHandler.server_yaw;
         float nextPitch = RotationHandler.server_pitch;
         if(packet instanceof C03PacketPlayer.C04PacketPlayerPosition) {
            C03PacketPlayer.C04PacketPlayerPosition pack = (C03PacketPlayer.C04PacketPlayerPosition)packet;
            if(useNext) {
               if(nextYaw != lastYaw || nextPitch != lastPitch) {
                  packet = new C03PacketPlayer.C06PacketPlayerPosLook(pack.getPositionX(), pack.getPositionY(), pack.getPositionZ(), nextYaw, nextPitch, ((C03PacketPlayer.C04PacketPlayerPosition)packet).isOnGround());
                  ((C03PacketPlayer.C06PacketPlayerPosLook)packet).rotating = true;
                  lastPitch = nextPitch;
                  lastYaw = nextYaw;
               }
            } else if(Math.abs(lastYaw - Minecraft.getMinecraft().thePlayer.rotationYaw) > 1.0F || Math.abs(lastPitch - Minecraft.getMinecraft().thePlayer.rotationPitch) > 1.0F) {
               packet = new C03PacketPlayer.C06PacketPlayerPosLook(pack.getPositionX(), pack.getPositionY(), pack.getPositionZ(), Minecraft.getMinecraft().thePlayer.rotationYaw, Minecraft.getMinecraft().thePlayer.rotationPitch, ((C03PacketPlayer.C04PacketPlayerPosition)packet).isOnGround());
               lastPitch = Minecraft.getMinecraft().thePlayer.rotationPitch;
               lastYaw = Minecraft.getMinecraft().thePlayer.rotationYaw;
            }
         } else if(packet instanceof C03PacketPlayer.C05PacketPlayerLook) {
            C03PacketPlayer.C05PacketPlayerLook pack1 = (C03PacketPlayer.C05PacketPlayerLook)packet;
            if(useNext) {
               pack1.yaw = nextYaw;
               pack1.pitch = nextPitch;
            }

            if(pack1.yaw == lastYaw && pack1.pitch == lastPitch) {
               packet = new C03PacketPlayer(pack1.isOnGround());
            } else {
               lastPitch = ((C03PacketPlayer.C05PacketPlayerLook)packet).getPitch();
               lastYaw = ((C03PacketPlayer.C05PacketPlayerLook)packet).getYaw();
               packet = pack1;
            }
         } else if(packet instanceof C03PacketPlayer.C06PacketPlayerPosLook) {
            C03PacketPlayer.C06PacketPlayerPosLook pack2 = (C03PacketPlayer.C06PacketPlayerPosLook)packet;
            if(useNext) {
               pack2.yaw = nextYaw;
               pack2.pitch = nextPitch;
            }

            if(pack2.yaw == lastYaw && pack2.pitch == lastPitch) {
               packet = new C03PacketPlayer.C04PacketPlayerPosition(pack2.getPositionX(), pack2.getPositionY(), pack2.getPositionZ(), pack2.isOnGround());
            } else {
               lastPitch = ((C03PacketPlayer.C06PacketPlayerPosLook)packet).getPitch();
               lastYaw = ((C03PacketPlayer.C06PacketPlayerPosLook)packet).getYaw();
               packet = pack2;
            }
         } else if(useNext && (nextPitch != lastPitch || nextYaw != lastYaw)) {
            packet = new C03PacketPlayer.C05PacketPlayerLook(nextYaw, nextPitch, ((C03PacketPlayer)packet).isOnGround());
            lastPitch = nextPitch;
            lastYaw = nextYaw;
         }
   ...
   public void handleSignEditorOpen(S36PacketSignEditorOpen packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      Object tileentity = this.clientWorldController.getTileEntity(packetIn.getSignPosition());
   ...
   public void handleStatistics(S37PacketStatistics packetIn) {
      ...
      StatBase statbase;
      int i;
      for(Iterator var4 = packetIn.func_148974_c().entrySet().iterator(); var4.hasNext(); this.gameController.thePlayer.getStatFileWriter().unlockAchievement(this.gameController.thePlayer, statbase, i)) {
   ...
   public void handleCombatEvent(S42PacketCombatEvent packetIn) {
      ...
      if(packetIn.eventType == S42PacketCombatEvent.Event.END_COMBAT) {
         long entity1 = (long)(1000 * packetIn.field_179772_d / 20);
         MetadataCombat metadatacombat = new MetadataCombat(this.gameController.thePlayer, entitylivingbase);
         this.gameController.getTwitchStream().func_176026_a(metadatacombat, 0L - entity1, 0L);
      } else if(packetIn.eventType == S42PacketCombatEvent.Event.ENTITY_DIED) {
         Entity entity11 = this.clientWorldController.getEntityByID(packetIn.field_179774_b);
         if(entity11 instanceof EntityPlayer) {
            MetadataPlayerDeath metadataplayerdeath = new MetadataPlayerDeath((EntityPlayer)entity11, entitylivingbase);
   ...
   public void handleTitle(S45PacketTitle packetIn) {
      ...
      switch($SWITCH_TABLE$net$minecraft$network$play$server$S45PacketTitle$Type()[s45packettitle$type.ordinal()]) {
      case 1:
         s = s2;
         break;
      case 2:
   ...
   public void handlePlayerListItem(S38PacketPlayerListItem packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      Iterator var3 = packetIn.func_179767_a().iterator();
      ...
      while(var3.hasNext()) {
         ...
         if(packetIn.func_179768_b() == S38PacketPlayerListItem.Action.REMOVE_PLAYER) {
            ...
            NetworkPlayerInfo networkplayerinfo = (NetworkPlayerInfo)playerInfoMap.get(s38packetplayerlistitem$addplayerdata.getProfile().getId());
            ...
            if(networkplayerinfo != null) {
               switch($SWITCH_TABLE$net$minecraft$network$play$server$S38PacketPlayerListItem$Action()[packetIn.func_179768_b().ordinal()]) {
               case 1:
   ...
   public void handleSoundEffect(S29PacketSoundEffect packetIn) {
      ...
      while(var3.hasNext()) {
         ...
         if(e instanceof EntityLivingBase) {
            ...
            if(distX < 1.0D && distY <= 0.5D && distZ < 1.0D) {
               AntiBots.getArrayList().add(e);
            }
         }
      }
   ...
   public void handleCustomPayload(S3FPacketCustomPayload packetIn) {
      ...
      if("MC|TrList".equals(packetIn.getChannelName())) {
         PacketBuffer itemstack = packetIn.getBufferData();

         try {
            int ioexception = itemstack.readInt();
            GuiScreen guiscreen = this.gameController.currentScreen;
            if(guiscreen != null && guiscreen instanceof GuiMerchant && ioexception == this.gameController.thePlayer.openContainer.windowId) {
               IMerchant imerchant = ((GuiMerchant)guiscreen).getMerchant();
               MerchantRecipeList merchantrecipelist = MerchantRecipeList.readFromBuf(itemstack);
         ...
         ItemStack itemstack1 = this.gameController.thePlayer.getCurrentEquippedItem();
         if(itemstack1 != null && itemstack1.getItem() == Items.written_book) {
            this.gameController.displayGuiScreen(new GuiScreenBook(this.gameController.thePlayer, itemstack1, false));
         }
      }
   ...
   public void handleTeams(S3EPacketTeams packetIn) {
      ...
      if(packetIn.func_149307_h() == 0 || packetIn.func_149307_h() == 2) {
         ...
         Team.EnumVisible s1 = Team.EnumVisible.func_178824_a(packetIn.func_179814_i());
         if(s1 != null) {
            scoreplayerteam.setNameTagVisibility(s1);
         }
      }
      ...
      if(packetIn.func_149307_h() == 0 || packetIn.func_149307_h() == 3) {
         var5 = packetIn.func_149310_g().iterator();

         while(var5.hasNext()) {
            s11 = (String)var5.next();
            scoreboard.addPlayerToTeam(s11, packetIn.func_149312_c());
         }
      }
      ...
      if(packetIn.func_149307_h() == 4) {
         ...
         while(var5.hasNext()) {
            s11 = (String)var5.next();
            scoreboard.removePlayerFromTeam(s11, scoreplayerteam);
         }
      }
   ...
   public void handleParticles(S2APacketParticles packetIn) {
      ...
      if(packetIn.getParticleCount() == 0) {
         double i = (double)(packetIn.getParticleSpeed() * packetIn.getXOffset());
         ...
         try {
            this.clientWorldController.spawnParticle(packetIn.getParticleType(), packetIn.isLongDistance(), packetIn.getXCoordinate(), packetIn.getYCoordinate(), packetIn.getZCoordinate(), i, d2, d4, packetIn.getParticleArgs());
   ...
   public void handleEntityProperties(S20PacketEntityProperties packetIn) {
      ...
      if(entity != null) {
         ...
         Iterator var5 = packetIn.func_149441_d().iterator();

         while(var5.hasNext()) {
            S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot = (S20PacketEntityProperties.Snapshot)var5.next();
            ...
            Iterator var8 = s20packetentityproperties$snapshot.func_151408_c().iterator();
   ...
   public static Collection getPlayerInfoMap() {
   ...
   public NetworkPlayerInfo getPlayerInfo(String p_175104_1_) {
      Iterator var3 = playerInfoMap.values().iterator();

      while(var3.hasNext()) {
         NetworkPlayerInfo networkplayerinfo = (NetworkPlayerInfo)var3.next();
