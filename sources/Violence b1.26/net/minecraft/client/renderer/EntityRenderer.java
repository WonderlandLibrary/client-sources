...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private float getFOVModifier(float partialTicks, boolean p_78481_2_) {
		if (this.debugView) {
			...
			if (this.mc.currentScreen == null) {
				GameSettings block = this.mc.gameSettings;
			...
			if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHealth() <= 0.0F) {
				float block1 = (float) ((EntityLivingBase) entity).deathTime + partialTicks;
				f /= (1.0F - 500.0F / (block1 + 500.0F)) * 2.0F + 1.0F;
			}

			Block block2 = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
			if (block2.getMaterial() == Material.water) {
				f = f * 60.0F / 70.0F;
			}

			if (Module.getByName("FovChanger").isToggled()) {
				f = 90.0F + (float) VSetting.getByName("Fov", Module.getByName("FovChanger")).getCurrent() * 20.0F;
			}
	...
	private void hurtCameraEffect(float partialTicks) {
		if (!Module.getByName("NoHurtTime").isToggled()) {
	...
	private void setupViewBobbing(float partialTicks) {
		if (!Module.getByName("NoBob").isToggled()) {
	...
	private void frameInit() {
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				String chatcomponenttext1 = "HD_U".replace("HD_U", "HD Ultra").replace("L", "Light");
				String s1 = chatcomponenttext1 + " " + Config.getNewRelease();
			...
			if (Config.isNotify64BitJava()) {
				...
				this.mc.ingameGUI.getChatGUI().printChatMessage(chatcomponenttext11);
			}
		}
