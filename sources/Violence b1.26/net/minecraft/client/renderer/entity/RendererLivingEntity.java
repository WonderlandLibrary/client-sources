...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
		...
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			try {
				...
				if (entity == Minecraft.getMinecraft().thePlayer
						&& VSetting.getByName("Rotate Model", Module.getByName("Model")).isToggled()
						&& Minecraft.getMinecraft().currentScreen == null) {
					f1 = this.interpolateRotation(RotationHandler.server_yaw, RotationHandler.server_yaw, partialTicks);
				} else {
	...
	protected boolean setScoreTeamColor(EntityLivingBase entityLivingBaseIn) {
		...
		if (entityLivingBaseIn.hurtTime <= 0) {
			GlStateManager.color(0.39215687F, 0.5882353F, 1.0F);
		} else {
			GlStateManager.color(1.0F, 0.0F, 0.0F);
		}

		if (FriendManager.getAliasOf(entityLivingBaseIn.getName()) != null) {
			GlStateManager.color(0.39215687F, 1.9607843F, 1.0F);
		}

		if (VSetting.getByName("Team Color", Module.getByName("ESP")).isToggled()) {
			int i = 16777215;
			if (entityLivingBaseIn instanceof EntityPlayer) {
				...
				if (f1 != null) {
					...
					if (f2.length() >= 2) {
						i = this.getFontRendererFromRenderManager().getColorCode(f2.charAt(1));
					}
				}
			}
