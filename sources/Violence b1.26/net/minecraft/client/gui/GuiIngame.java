...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		IngameRenderer.onRender();
		GlStateManager.popMatrix();
		if (VSetting.getByName("Custom Hotbar", Module.getByName("Hud")).isToggled()) {
			IngameRenderer.renderHotbar();
		}
		...
		if (!this.mc.thePlayer.isPotionActive(Potion.confusion)) {
			float i2 = this.mc.thePlayer.prevTimeInPortal
					+ (this.mc.thePlayer.timeInPortal - this.mc.thePlayer.prevTimeInPortal) * partialTicks;
			if (i2 > 0.0F) {
				this.func_180474_b(i2, scaledresolution);
			}
		}

		if (this.mc.playerController.isSpectator()) {
			this.spectatorGui.renderTooltip(scaledresolution, partialTicks);
		} else if (!VSetting.getByName("Custom Hotbar", Module.getByName("Hud")).isToggled()) {
		...
		if (scoreobjective1 != null && !Module.getByName("NoScoreboard").isToggled()) {
