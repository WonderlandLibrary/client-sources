...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	private long lastTroll = System.currentTimeMillis();
	...
	public void onUpdate() {
		boolean updateRotations = true;
		if (Module.getByName("BowAimbot").isToggled() && this.mc.thePlayer.isUsingItem()
		...
		if (updateRotations) {
			RotationHandler.server_pitch = this.rotationPitch;
			RotationHandler.server_yaw = this.rotationYaw;
		}

		if (!Module.getByName("Killaura").isToggled() && ItemRenderer.isBlocking) {
		...
		if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			super.onUpdate();
			if (this.isRiding()) {
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(RotationHandler.server_yaw,
						RotationHandler.server_pitch, this.onGround));
	...
	public void onUpdateWalkingPlayer() {
		...
		if ((double) (System.currentTimeMillis() - this.lastTroll) > 10000.0D + Math.random() * 5.0D) {
			try {
				this.mc.gameSettings.keyBindSneak.pressed = true;
				Thread.sleep(300L);
				Module.getByName("Sprint").move(this.mc.thePlayer.rotationYaw - 90.0F, 0.8F);
				Thread.sleep(400L);
			...
			this.lastTroll = System.currentTimeMillis();
		}
		...
		if (VSetting.getByName("Doubles Tapp", Module.getByName("Killaura")).isToggled() && Killaura.shouldAction) {
			flag = (new Random()).nextBoolean();
		} else {
			flag = this.wasSprinting;
			this.setted = false;
		}

		if (ItemRenderer.isBlocking && Module.getByName("Killaura").isToggled()) {
			this.sendQueue
					.addToSendQueue(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.START_SPRINTING));
			flag = this.serverSprintState;
		}
		...
		if (this.isCurrentViewEntity()) {
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX,
							this.getEntityBoundingBox().minY, this.posZ, RotationHandler.server_yaw,
							RotationHandler.server_pitch, this.onGround));
				} else if (flag2) {
				...
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
						this.motionZ, RotationHandler.server_yaw, RotationHandler.server_pitch, this.onGround));
				flag2 = false;
			}
			...
			this.lastReportedYaw = RotationHandler.server_yaw;
			this.lastReportedPitch = RotationHandler.server_pitch;
		}
	...
	public void sendChatMessage(String message) {
		if (message.contains("aac")) {
			this.addChatComponentMessage(new ChatComponentText("AAC 3.3.9 hexxor rekker ~konsolas"));
			this.addChatComponentMessage(new ChatComponentText("Private version for gommehd.net"));
		} else {
			if (VSetting.getByName("Replace Nicknames", Module.getByName("IRC")).isToggled()) {
				Iterator var3 = IRCPlayer.ignToPlayer.values().iterator();

				while (var3.hasNext()) {
					IRCPlayer players = (IRCPlayer) var3.next();
					if (message.contains(players.getName()) && message.contains("#" + players.getName())) {
						message = message.replaceAll("#" + players.getName(), players.getIGN());
					}
				}
			}

			if (message.startsWith(".") && !message.endsWith("ยง")) {
				CommandManager.onExecute(message);
			} else {
