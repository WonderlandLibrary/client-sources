...
import client.module.impl.render.Animations;
...
public class ItemRenderer
{
    ...
    {
        final Animations animations = Client.INSTANCE.getModuleManager().get(Animations.class);
        final boolean b = animations.isEnabled();
        final float scale = b ? animations.getScale().getValue().floatValue() : 0.4F;
        GlStateManager.translate(b ? animations.getX().getValue().floatValue() : 0.56F, b ? animations.getY().getValue().floatValue() : -0.52F, b ? animations.getZ().getValue().floatValue() : -0.71999997F);
        GlStateManager.translate(0.0F, equipProgress * -0.6F, 0.0F);
        GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            if (animations.isEnabled()) {
                                switch (animations.getBlockAnimation().getValue().getName()) {
                                    case "Vanilla": {
                                        this.transformFirstPersonItem(f, 0.0F);
                                    ...
                                    case "ket": {
                                        ...
                                        doItemUsedTransformations(f);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case "Test": {
                                        transformFirstPersonItem(f / 2, 0);
                                        GlStateManager.translate(0, 0.3F, 0);
                                        GlStateManager.rotate(-(MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI)) * 31, 1, 0, 2);
                                        GlStateManager.rotate(-(MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI)) * 33, 1.5F, ((MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI)) / 1.1F), 0);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case "Test2": {
                                        transformFirstPersonItem(f / 1.5F, 0);
                                        doBlockTransformations();
                                        GlStateManager.translate(-0.05F, 0.3F, 0.3F);
                                        double d = Math.sin(Math.sqrt(f1) * Math.PI);
                                        GlStateManager.rotate((float) (-d * 140), 8, 0, 8);
                                        GlStateManager.rotate((float) (d * 90), 8, 0, 8);
                                        break;
                                    }
                                    case "Virtue": {
                ...
                {
                    if (animations.isEnabled()) {
                        switch (animations.getSwingAnimation().getValue().getName()) {
                            case "Vanilla": {
