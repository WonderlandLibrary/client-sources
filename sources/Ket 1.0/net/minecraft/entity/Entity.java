...
import client.event.impl.other.PostStrafeEvent;
...
public abstract class Entity implements ICommandSender
{
    ...
    {
        final MoveEvent moveEvent = new MoveEvent(x, y, z, onGround && isSneaking() && this instanceof EntityPlayer);
        if (this instanceof EntityPlayerSP) Client.INSTANCE.getEventBus().handle(moveEvent);
        ...
        else
        {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = moveEvent.isSafeWalk();
            ...
            if (this instanceof EntityPlayerSP && flag1 && (d3 != x || d5 != z)) Client.INSTANCE.getEventBus().handle(stepEvent);
            if (stepEvent.getHeight() > 0.0F && flag1 && (d3 != x || d5 != z) && !stepEvent.isCancelled())
            {
                double d11 = x;
                double d7 = y;
                double d8 = z;
    ...
    {
        final StrafeEvent event = new StrafeEvent(strafe, forward, friction, rotationYaw);
        if (this instanceof EntityPlayerSP) Client.INSTANCE.getEventBus().handle(event);
        ...
        if (this instanceof EntityPlayerSP) Client.INSTANCE.getEventBus().handle(new PostStrafeEvent());
    }
