...
public class BridgeAssist extends Module {
    ...
    private final float[] godbridgePos = {75.6f, -315, -225, -135, -45, 0, 45, 135, 225, 315 , -20 , 0};
    private final float[] moonwalkPos = {79.6f, -340, -290, -250, -200, -160, -110, -70, -20, 0, 20, 70, 110, 160, 200, 250, 290, 340};
    private final float[] breezilyPos = {79.9f, -360, -270, -180, -90, 0, 90, 180, 270, 360 , 20 , 90};
    ...
    private boolean moonwalkDirection = false; // false for right, true for left
    ...
    public void onBlockPlaced(BlockEvent.PlaceEvent event) {
        if (!Utils.Player.isPlayerInGame() || !isEnabled()) {
            return;
        }

        if (Utils.Modes.BridgeMode.values()[(int) (assistMode.getInput() - 1.0D)] == Utils.Modes.BridgeMode.GODBRIDGE) {
            mc.thePlayer.rotationYaw += 1; // Adjust this value as needed
            mc.thePlayer.rotationPitch -= 0; // Adjust this value as needed
            if (Utils.Modes.BridgeMode.values()[(int) (assistMode.getMax() + 2.0D)] == Utils.Modes.BridgeMode.BREEZILY) {
