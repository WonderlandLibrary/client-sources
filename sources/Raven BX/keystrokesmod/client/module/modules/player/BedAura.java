                  ...
                  for(int z = -ra; z <= ra; ++z) {
                     if (Utils.Player.isPlayerInGame()) {
                        ...
                        if (BedAura.this.m == p) {
                           ...
                           // Draw a blue box around the bed (add your logic here)
                           Module.mc.theWorld.markBlockRangeForRenderUpdate(p, p);

                           BedAura.this.mi(p);
                           BedAura.this.m = p;

                           // Break the nearest block to the player if facing the bed
                           EnumFacing facing = Utils.Player.getFacing();
                           if (facing == EnumFacing.NORTH && Module.mc.thePlayer.getDistanceSq(p) <= 1.0D) {
                              BlockPos blockToBreak = p.offset(EnumFacing.NORTH);
                              BedAura.this.mi(blockToBreak);
                           }
