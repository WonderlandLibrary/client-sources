...
public class SafeWalk extends Module {
   ...
   // Add the "moon walk" option.
   public static TickSetting moonWalk;
   ...
   public SafeWalk() {
      ...
      this.registerSetting(moonWalk = new TickSetting("Moon Walk", false)); // Register the "moon walk" option.
   }
   ...
   public void p(PlayerTickEvent e) {
      ...
      if (moonWalk.isToggled()) {
         // Set Shift time to 0 when moon walk is enabled.
         shiftTime.setValueMin(0);
         shiftTime.setValueMax(0);
         this.setShift(false); // Always set shift to true (moon walk).
      } else if (doShift.isToggled()) {
         ...
         if (mc.thePlayer.onGround) {
            if (Utils.Player.playerOverAir()) {
               ...
               this.setShift(true);
               shouldBridge = true;
            } else if (mc.thePlayer.isSneaking() && !Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode()) && onHold.isToggled()) { // if player is sneaking and shiftDown and holdSetting turned on
               isShifting = false;
               shouldBridge = false;
               this.setShift(false);
   ...
   private void setShift(boolean sh) {
         KeyBinding.setKeyBindState(mc.gameSettings.keyBindSneak.getKeyCode(), sh); // Handle regular sneak setting.
      }
