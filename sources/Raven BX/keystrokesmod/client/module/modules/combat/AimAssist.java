...
public class AimAssist extends Module {
   ...
   public static SliderSetting horizontalSensitivity;
   public static SliderSetting verticalSensitivity;
   public static TickSetting aimLockEnabled; // New setting for aim lock
   public static ArrayList<Entity> friends = new ArrayList<>();
   private boolean isStrafing = false; // Variable to keep track of strafing
   ...
   public AimAssist() {
      ...
      this.registerSetting(horizontalSensitivity = new SliderSetting("Horizontal Sensitivity", 1.0D, 0.1D, 5.0D, 0.1D));
      this.registerSetting(verticalSensitivity = new SliderSetting("Vertical Sensitivity", 1.0D, 0.1D, 5.0D, 0.1D));
      ...
      this.registerSetting(blatantMode = new TickSetting("Locked", false));
      this.registerSetting(aimLockEnabled = new TickSetting("Strafe Lock", false));
   }

   public void update() {
      if (!Utils.Player.isPlayerInGame()) {
         isStrafing = false; // Reset the strafing flag when not in-game
         return;
      }
      ...
      if (mc.thePlayer != null && (mc.gameSettings.keyBindLeft.isKeyDown() || mc.gameSettings.keyBindRight.isKeyDown())) {
         isStrafing = true;
      } else {
         isStrafing = false;
      }
      ...
      //this is so good ;) my fav module of all time
      ...
      if (!weaponOnly.isToggled() || Utils.Player.isPlayerHoldingWeapon()) {
         ...
         if ((clickAim.isToggled() && Utils.Client.autoClickerClicking()) || (Mouse.isButtonDown(0) && autoClicker != null && !autoClicker.isEnabled()) || !clickAim.isToggled()) {
            ...
            if (en != null) {
               ...
               if (aimLockEnabled.isToggled() && isStrafing) { // Check for aim lock and strafing
                  // Calculate the aim lock height (1.5 blocks above the enemy):
                  double aimLockHeight = en.posY + 1.5;

                  // Adjust the player's rotation to lock onto the aimLockHeight:
                  double playerYaw = Math.atan2(aimLockHeight - mc.thePlayer.posY, en.posZ - mc.thePlayer.posZ) * 180 / Math.PI - 90;
                  double playerPitch = Math.atan2(en.posX - mc.thePlayer.posX, Math.sqrt((en.posZ - mc.thePlayer.posZ) * (en.posZ - mc.thePlayer.posZ) + (en.posX - mc.thePlayer.posX) * (en.posX - mc.thePlayer.posX))) * 180 / Math.PI;
                  mc.thePlayer.rotationYaw = (float) playerYaw;
                  mc.thePlayer.rotationPitch = (float) playerPitch;

                  // you can also send the server-side look packet to maintain accuracy.
                   //mc.playerController.sendPacket(new CPacketPlayer.Rotation(mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch, mc.thePlayer.onGround));
                   //mc.playerController.sendPacket(new CPacketPlayer.Position(mc.thePlayer.posX, aimLockHeight, mc.thePlayer.posZ, mc.thePlayer.onGround));
               } else if (blatantMode.isToggled()) {
