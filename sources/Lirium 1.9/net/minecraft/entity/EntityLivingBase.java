...
public abstract class EntityLivingBase extends Entity {
    ...
    private static final UUID SPRINTING_SPEED_BOOST_ID = FastUUID.parseUUID("662A6B8D-DA3E-4C1C-8813-96EA6097278D");
    ...
    public void onEntityUpdate() {
        if (!FakeEntityUtil.clientSideEntities.contains(this))
        ...
        if (this.hurtTime > 0 && !FakeEntityUtil.clientSideEntities.contains(this)) {
            --this.hurtTime;
        }
    ...
    private int getArmSwingAnimationEnd() {
        final int speed = this.isPotionActive(MobEffects.HASTE) ? (6 - (1 + this.getActivePotionEffect(MobEffects.HASTE).getAmplifier())) : (this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : 6);
        final AnimationEndEvent event = new AnimationEndEvent(speed);
        Client.INSTANCE.getEventBus().call(event);
        return event.speed;
    }

    public void swingArm(EnumHand hand) {
        if (FakeEntityUtil.clientSideEntities.contains(this)) return;
    ...
    public float getSwingProgress(float partialTickTime) {
        if(FakeEntityUtil.clientSideEntities.contains(this)) return swingProgress;
