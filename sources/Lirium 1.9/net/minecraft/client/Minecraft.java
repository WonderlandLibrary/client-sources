...
import de.lirium.base.texturefix.TextureFix;
...
import me.felix.splash.SplashRenderer;
...
public class Minecraft implements IThreadListener, ISnooperInfo {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        this.sessionService = (new YggdrasilAuthenticationService(this.proxy, FastUUID.toString(UUID.randomUUID()))).createMinecraftSessionService();
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        splashFramebuffer = FramebufferHelper.doFrameBuffer(splashFramebuffer);


        splashRenderer = new SplashRenderer(splashFramebuffer);
        ...
        splashRenderer.changeSplash("Skin Manager", 20);
        Client.INSTANCE.hook();
        splashRenderer.changeSplash("Client Hooking", 30);
        this.saveLoader = new AnvilSaveConverter(new File(this.mcDataDir, "saves"), this.dataFixer);
        splashRenderer.changeSplash("Save loader", 34);
        this.mcSoundHandler = new SoundHandler(this.mcResourceManager, this.gameSettings);
        splashRenderer.changeSplash("Sound Hooking", 37);
        this.mcResourceManager.registerReloadListener(this.mcSoundHandler);
        splashRenderer.changeSplash("Resource Hooking", 40);
        this.mcMusicTicker = new MusicTicker(this);
        splashRenderer.changeSplash("Music Hooking", 45);
        ...
        splashRenderer.getSplashProcess().updateStage("Font Hooking", 50);
        ...
        splashRenderer.getSplashProcess().updateStage("Mouse Hooking", 55);
        ...
        splashRenderer.getSplashProcess().updateStage("Texture Hooking", 60);
        ...
        splashRenderer.getSplashProcess().updateStage("Model Hooking", 65);
        this.blockColors = BlockColors.init();
        this.itemColors = ItemColors.init(this.blockColors);
        this.renderItem = new RenderItem(this.renderEngine, this.modelManager, this.itemColors);
        splashRenderer.getSplashProcess().updateStage("Item render Hooking", 70);
        this.renderManager = new RenderManager(this.renderEngine, this.renderItem);
        splashRenderer.getSplashProcess().updateStage("Handling render manager", 75);
        this.itemRenderer = new ItemRenderer(this);
        splashRenderer.getSplashProcess().updateStage("Handling item Renderer", 80);
        this.mcResourceManager.registerReloadListener(this.renderItem);
        this.entityRenderer = new EntityRenderer(this, this.mcResourceManager);
        this.mcResourceManager.registerReloadListener(this.entityRenderer);
        splashRenderer.getSplashProcess().updateStage("Handled resource manager", 85);
        ...
        splashRenderer.getSplashProcess().updateStage("Set up viewport", 90);
        this.effectRenderer = new ParticleManager(this.world, this.renderEngine);
        this.checkGLError("Post startup");
        splashRenderer.getSplashProcess().updateStage("Post startup", 93);
        this.ingameGUI = new GuiIngame(this);
        splashRenderer.changeSplash("Ingame Hooking", 97);

        new TextureFix().runFix();

        splashRenderer.changeSplash("Fixxing Textures", 99);
        ...
        splashRenderer.changeSplash("Done.", 100);
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle(Client.NAME + " v" + Client.VERSION + " " + Client.CODE_NAME + " Edition | Based on Minecraft 1.12.2");
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            try {
                inputstream = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("lirium/icon/16x16.png"));
                inputstream1 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("lirium/icon/32x32.png"));
    ...
    public void refreshResources() {
        ...
        for (ResourcePackRepository.Entry resourcepackrepository$entry : this.mcResourcePackRepository.getRepositoryEntries()) {
            final IResourcePack resourcePack = resourcepackrepository$entry.getResourcePack();
            if (resourcepackrepository$entry.getPackFormat() < 3) {
                if (resourcePack instanceof FileResourcePack) {
                    final FileResourcePack fileResourcePack = (FileResourcePack) resourcePack;
                    fileResourcePack.legacy = true;
                }
            }
    ...
    private void runGameLoop() throws IOException {
        ...
        final GameLoopEvent gameLoopEvent = new GameLoopEvent(true);
        Client.INSTANCE.getEventBus().call(gameLoopEvent);

        for (int j = 0; j < (gameLoopEvent.capTicks ? Math.min(10, this.timer.elapsedTicks) : this.timer.elapsedTicks); ++j) {
            this.runTick();
        }

        for (int j = 0; j < Math.min(10, this.particleTimer.elapsedTicks); j++) {
        ...
        if (!this.skipRenderWorld) {
            ...
            this.entityRenderer.updateCameraAndRender(this.isGamePaused ? this.field_193996_ah : this.timer.partialTicks, i, this.particleTimer.partialTicks);
            this.mcProfiler.endStartSection("toasts");
            final ToastsRenderEvent toastsRenderEvent = new ToastsRenderEvent();
            Client.INSTANCE.getEventBus().call(toastsRenderEvent);
            if (!toastsRenderEvent.isCancelled())
                this.toast.render(new ScaledResolution(this));
            this.mcProfiler.endSection();
        }
    ...
    private void callResizeFunction() {
    ...
    public void runTick() throws IOException {
        Client.INSTANCE.getEventBus().call(new TickEvent());
        ...
        Client.INSTANCE.getEventBus().call(new GuiHandleEvent());
    ...
    private void runTickKeyboard() throws IOException {
        while (Keyboard.next()) {
            ...
            if (flag) {
                ...
                Client.INSTANCE.getEventBus().call(new KeyEvent(i));
    ...
    private void processKeyBinds() {
        ...
        Client.INSTANCE.getEventBus().call(new AttackEvent());
    ...
    private void runTickMouse() throws IOException {
        while (Mouse.next()) {
            ...
            if (Mouse.getEventButtonState()) {
                ...
                Client.INSTANCE.getEventBus().call(new KeyEvent(i - 100));
    ...
    public void launchIntegratedServer(String folderName, String worldName, @Nullable WorldSettings worldSettingsIn) {
        ...
        try {
            YggdrasilAuthenticationService yggdrasilauthenticationservice = new YggdrasilAuthenticationService(this.proxy, FastUUID.toString(UUID.randomUUID()));
        ...
        ServerUtil.joinedTime = System.currentTimeMillis();
        ServerUtil.lastIP = "";
        ServerUtil.lastPort = 0;
        ServerUtil.lastData = null;
        ServerUtil.lastResolvedIP = null;
    ...
    public void loadWorld(@Nullable WorldClient worldClientIn, String loadingMessage) {
        ...
        if (worldClientIn != null) {
            if (!this.integratedServerIsRunning) {
                AuthenticationService authenticationservice = new YggdrasilAuthenticationService(this.proxy, FastUUID.toString(UUID.randomUUID()));
