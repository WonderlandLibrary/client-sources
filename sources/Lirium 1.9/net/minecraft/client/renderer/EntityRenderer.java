...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            final MouseOverEvent mouseOverEvent = new MouseOverEvent(3.0, d0);
            Client.INSTANCE.getEventBus().call(mouseOverEvent);

            d0 = mouseOverEvent.blockReach;
            ...
            if (this.mc.playerController.extendedReach()) {
                d1 = 6.0D;
                d0 = d1;
            } else if (d0 > mouseOverEvent.maxRange) {
                flag = true;
            }
            ...
            List<Entity> list = this.mc.world.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec3d1.xCoord * d0, vec3d1.yCoord * d0, vec3d1.zCoord * d0).expand(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>() {
                public boolean apply(@Nullable Entity p_apply_1_) {
                    return p_apply_1_ != null && p_apply_1_.canBeCollidedWith() && !FakeEntityUtil.clientSideEntities.contains(p_apply_1_);
                }
            }));
            double d2 = d1;
            ...
            if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > mouseOverEvent.maxRange) {
                this.pointedEntity = null;
    ...
    private void orientCamera(float partialTicks) {
        ...
        final OrientCameraEvent cameraEvent = new OrientCameraEvent();
        Client.INSTANCE.getEventBus().call(cameraEvent);
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            ...
            final OrientCameraRotateEvent rotateEvent = new OrientCameraRotateEvent(entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks + 180.0F, entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks);
            Client.INSTANCE.getEventBus().call(rotateEvent);
            GlStateManager.rotate(rotateEvent.pitch, 1.0F, 0.0F, 0.0F);
            ...
            if (entity instanceof EntityAnimal) {
                ...
                GlStateManager.rotate(rotateEvent.yaw, 0.0F, 1.0F, 0.0F);
            }
        }
    ...
    private void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                ...
                final LightMapEvent lightMapEvent = new LightMapEvent(1F, 1F, 1F, false);
                Client.INSTANCE.getEventBus().call(lightMapEvent);
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    if(lightMapEvent.overwrite) {
                        f8 = lightMapEvent.red;
                        f9 = lightMapEvent.green;
                        f10 = lightMapEvent.blue;
                    }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime, float particlePartialTicks) {
        this.frameInit();
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            final RotateEvent rotateEvent = new RotateEvent(this.mc.player.rotationYaw, this.mc.player.rotationPitch, this.mc.mouseHelper.deltaX, this.mc.mouseHelper.deltaY, false);
            Client.INSTANCE.getEventBus().call(rotateEvent);
            if (!rotateEvent.stopRotate) {
            ...
            final RotationEvent rotationEvent = new RotationEvent(mc.player.rotationYaw, mc.player.rotationPitch);
            Client.INSTANCE.getEventBus().call(rotationEvent);
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.world == null) {
                ...
                this.renderWorld(partialTicks, particlePartialTicks, System.nanoTime() + l);
         ...
         /*   animation.animate(ScreenShotHelper.shotScreenshot ? 1 : 0, 0.25, true);

            animation.update();

            Gui.drawRect(0, 0, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), new Color(1,1,1, animation.getValue()).getRGB());

            if(animation.getValue() > 0.9999)
                ScreenShotHelper.shotScreenshot = false;*/
        }
    ...
    public void renderWorld(float partialTicks, float particlePartialTicks, long finishTimeNano) {
        ...
        if (this.mc.gameSettings.anaglyph) {
            ...
            this.renderWorldPass(0, partialTicks, particlePartialTicks, finishTimeNano);
            anaglyphField = 1;
            GlStateManager.colorMask(true, false, false, false);
            this.renderWorldPass(1, partialTicks, particlePartialTicks, finishTimeNano);
            GlStateManager.colorMask(true, true, true, false);
        } else {
            this.renderWorldPass(2, partialTicks, particlePartialTicks, finishTimeNano);
        }

        this.mc.mcProfiler.endSection();
    }

    private void renderWorldPass(int pass, float partialTicks, float particlePartialTicks, long finishTimeNano) {
        ...
        if (!this.debugView) {
            ...
            particlemanager.renderLitParticles(entity, particlePartialTicks);
            RenderHelper.disableStandardItemLighting();
            this.setupFog(0, partialTicks);
            ...
            particlemanager.renderParticles(entity, partialTicks, particlePartialTicks);
        ...
        Client.INSTANCE.getEventBus().call(new Render3DEvent(partialTicks));
