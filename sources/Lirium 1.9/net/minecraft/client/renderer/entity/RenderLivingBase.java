...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (FakeEntityUtil.clientSideEntities.contains(entity)) partialTicks = 1F;
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, partialTicks, x, y, z)) {
            if (animateModelLiving && !FakeEntityUtil.clientSideEntities.contains(entity)) {
                entity.limbSwingAmount = 1.0F;
            }

            if(FakeEntityUtil.clientSideEntities.contains(entity) && this.renderOutlines)
                return;

            GlStateManager.pushMatrix();
            GlStateManager.disableCull();
            if (!FakeEntityUtil.clientSideEntities.contains(entity))
            ...
            try {
                ...
                if (entity == Minecraft.getMinecraft().player) {
                    ...
                    prevYaw = RotationUtil.prevYaw;
                    pitch = RotationUtil.pitch;
                    prevPitch = RotationUtil.prevPitch;
                }

                float f = this.interpolateRotation(entity.prevRenderYawOffset, entity.renderYawOffset, partialTicks);
                float f1 = this.interpolateRotation(prevYaw, yaw, partialTicks);
                float f2 = f1 - f;
                ...
                float f7 = prevPitch + (pitch - prevPitch) * partialTicks;
                ...
                if (this.renderOutlines) {
                    final OutlineColorEvent colorEvent = new OutlineColorEvent(this.getTeamColor(entity), entity);
                    Client.INSTANCE.getEventBus().call(colorEvent);
                    boolean flag1 = this.setScoreTeamColor(entity);
                    GlStateManager.enableColorMaterial();
                    GlStateManager.enableOutlineMode(colorEvent.color);
