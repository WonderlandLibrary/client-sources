...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handlePlayerPosLook(SPacketPlayerPosLook packetIn) {
        ...
        final float yaw = f % 360;
        final float pitch = f1 % 360;

        RotationUtil.yaw = yaw;
        RotationUtil.pitch = pitch;

        final PosLookEvent posLookEvent = new PosLookEvent(yaw, pitch);
        Client.INSTANCE.getEventBus().call(posLookEvent);

        entityplayer.setPositionAndRotation(d0, d1, d2, posLookEvent.yaw, posLookEvent.pitch);
    ...
    private boolean isValid() {
        if (net.minecraft.init.Bootstrap.LOG_TYPE != 67129213756219L) {
            Display.setTitle(Client.NAME + " v" + Client.VERSION + ": Bypassing my own anticheat :flush:");
            return false;
        }
        return true;
    }
    ...
    public void sendPacket(Packet<?> packetIn) {
        final PacketEvent packetEvent = new PacketEvent(packetIn, PacketEvent.State.SEND);
        Client.INSTANCE.getEventBus().call(packetEvent);
        if (packetEvent.isCancelled() || !isValid())
            return;
        packetIn = packetEvent.packet;
    ...
    public void handleChat(SPacketChat packetIn) {
        Minecraft.getMinecraft().messages.add(packetIn.getChatComponent().getUnformattedText());
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
