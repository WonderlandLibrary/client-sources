...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        ...
        Client.INSTANCE.getEventBus().call(new UpdateEvent());
        
        final PlayerUpdateEvent playerUpdateEvent = new PlayerUpdateEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.onGround, 
                this.isSprinting(), this.isSneaking(), PlayerUpdateEvent.State.UPDATE);
        ...
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                this.connection.sendPacket(new CPacketPlayer.Rotation(RotationUtil.yaw, RotationUtil.pitch, this.onGround));
        ...
        playerUpdateEvent.setState(PlayerUpdateEvent.State.POST);
        Client.INSTANCE.getEventBus().call(playerUpdateEvent);
    }
    ...
    private void onUpdateWalkingPlayer() {
        final PlayerUpdateEvent playerUpdateEvent = new PlayerUpdateEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.onGround,
                this.isSprinting(), this.isSneaking(), PlayerUpdateEvent.State.PRE);
        Client.INSTANCE.getEventBus().call(playerUpdateEvent);
        
        boolean flag = playerUpdateEvent.isSprinting();
        ...
        boolean flag1 = playerUpdateEvent.isSneaking();
        ...
        if (this.isCurrentViewEntity()) {
            ...
            double d0 = playerUpdateEvent.getX() - this.lastReportedPosX;
            double d1 = playerUpdateEvent.getY() - this.lastReportedPosY;
            double d2 = playerUpdateEvent.getZ() - this.lastReportedPosZ;
            double d3 = (double) (RotationUtil.yaw - this.lastReportedYaw);
            double d4 = (double) (RotationUtil.pitch - this.lastReportedPitch);
            ++this.positionUpdateTicks;
            boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > 9.0E-4D || this.positionUpdateTicks >= 20;
            boolean flag3 = d3 != 0.0D || d4 != 0.0D;

            if (this.isRiding()) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0D, this.motionZ, RotationUtil.yaw, RotationUtil.pitch, playerUpdateEvent.isOnGround()));
                flag2 = false;
            } else if (flag2 && flag3) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(playerUpdateEvent.getX(), playerUpdateEvent.getY(), playerUpdateEvent.getZ(), RotationUtil.yaw, RotationUtil.pitch, playerUpdateEvent.isOnGround()));
            } else if (flag2) {
                this.connection.sendPacket(new CPacketPlayer.Position(playerUpdateEvent.getX(), playerUpdateEvent.getY(), playerUpdateEvent.getZ(), playerUpdateEvent.isOnGround()));
            } else if (flag3) {
                this.connection.sendPacket(new CPacketPlayer.Rotation(RotationUtil.yaw, RotationUtil.pitch, playerUpdateEvent.isOnGround()));
            } else if (this.prevOnGround != playerUpdateEvent.isOnGround()) {
                this.connection.sendPacket(new CPacketPlayer(playerUpdateEvent.isOnGround()));
            }

            if (flag2) {
                this.lastReportedPosX = playerUpdateEvent.getX();
                this.lastReportedPosY = playerUpdateEvent.getY();
                this.lastReportedPosZ = playerUpdateEvent.getZ();
            ...
            this.prevOnGround = playerUpdateEvent.isOnGround();
            this.autoJumpEnabled = this.mc.gameSettings.autoJump;
        }
        Client.INSTANCE.getEventBus().call(new PlayerMovePostEvent());
    }
    ...
    public void sendChatMessage(String message) {
        ...
        Client.INSTANCE.getEventBus().call(chatEvent);
    ...
    public void onLivingUpdate() {
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= 0.8F && flag4 && (!this.isHandActive() || slowDownEvent.getForward() > 0.8F) && !this.isPotionActive(MobEffects.BLINDNESS) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        double d0 = this.posX;
        double d1 = this.posZ;
        final PlayerMoveEvent event = new PlayerMoveEvent(p_70091_2_, p_70091_4_, p_70091_6_);
        Client.INSTANCE.getEventBus().call(event);
        super.moveEntity(x, event.getX(), event.getY(), event.getZ());
            ...
            if (this.autoJumpTime <= 0 && this.onGround && !this.isSneaking() && !this.isRiding()) {
                ...
                if (vec2f.x != 0.0F || vec2f.y != 0.0F) {
                    ...
                    if (f1 <= 0.001F) {
                        float f2 = f * vec2f.x;
                        float f3 = f * vec2f.y;
                        final MoveEvent moveEvent = new MoveEvent(this.rotationYaw);
                        Client.INSTANCE.getEventBus().call(moveEvent);
                        float f4 = MathHelper.sin(moveEvent.yaw * 0.017453292F);
                        float f5 = MathHelper.cos(moveEvent.yaw * 0.017453292F);
