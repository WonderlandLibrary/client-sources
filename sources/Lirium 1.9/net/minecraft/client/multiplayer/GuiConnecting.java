...
import me.errordev.http.resolver.Resolver;
...
public class GuiConnecting extends GuiScreen
{
    ...
    {
        final AtomicReference<String> ipString = new AtomicReference<>(ip);
        final AtomicInteger portInteger = new AtomicInteger(port);
        if (Client.INSTANCE.getProfileManager().get() != null && Client.INSTANCE.getProfileManager().get().getName().equalsIgnoreCase("Cubecraft")) {
            ipString.set("play.cubecraft.net");
            portInteger.set(25565);
        }
        ServerUtil.lastData = new ServerData("", ipString.get() + ":" + port, true);
        ServerUtil.lastIP = ipString.get();
        ServerUtil.lastPort = portInteger.get();
        ServerUtil.joinedTime = System.currentTimeMillis();
        new Thread(() -> ServerUtil.lastResolvedIP = Resolver.resolveIP(ipString.get(), portInteger.get()), "IP Resolver thread").start();
        LOGGER.info("Connecting to {}, {}", ipString.get(), portInteger.get());
        (new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet())
        {
            public void run()
            {
                ...
                try
                {
                    ...
                    inetaddress = InetAddress.getByName(ipString.get());
                    GuiConnecting.this.networkManager = NetworkManager.createNetworkManagerAndConnect(inetaddress, portInteger.get(), GuiConnecting.this.mc.gameSettings.isUsingNativeTransport());
                    GuiConnecting.this.networkManager.setNetHandler(new NetHandlerLoginClient(GuiConnecting.this.networkManager, GuiConnecting.this.mc, GuiConnecting.this.previousGuiScreen));
                    GuiConnecting.this.networkManager.sendPacket(new C00Handshake(ipString.get(), portInteger.get(), EnumConnectionState.LOGIN));
                ...
                catch (Exception exception)
                {
                    ...
                    if (inetaddress != null)
                    {
                        String s1 = inetaddress + ":" + portInteger.get();
                        s = s.replaceAll(s1, "");
                    }
