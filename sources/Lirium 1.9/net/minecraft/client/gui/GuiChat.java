...
public class GuiChat extends GuiScreen implements ITabCompleter
{
    ...
    final Predicate<Triple<Integer, Integer, Draggable>> predicate = e -> {
        final int mouseX = e.first(), mouseY = e.second();
        final Draggable draggable = e.third();
        assert draggable != null;
        if((draggable.getModule() == null || draggable.getModule().isEnabled()) && draggable.isHovering(mouseX, mouseY, draggable.getXPosition(), draggable.getYPosition(), draggable.getObjectWidth(), draggable.getObjectHeight()))
            return true;
        return false;
    };
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        final Draggable draggable = DragHandler.overlappingHelper.getInFront(mouseX, mouseY, predicate);
        if(draggable != null) {
            if (mouseButton == 0) {
                DragHandler.overlappingHelper.resort(draggable);
                draggable.onClick(mouseX, mouseY, mouseButton);
            }
        }
    ...
    {
        if (DragHandler.overlappingHelper.elements.size() > 0) {
            DragHandler.overlappingHelper.elements.forEach(draggable -> {
                if (draggable.getModule() == null || draggable.getModule().isEnabled()) {
                    draggable.onDragAndDraw(mouseX, mouseY);
                }
            });
        }
