...
public class Bootstrap {
    ...
    public static long LOG_TYPE = 67129213756219L;
    ...
    @BCompiler(aot = BCompiler.AOT.AGGRESSIVE)
    static void registerDispenserBehaviors() throws Exception {
        final RuntimeMXBean runtimeMxBean = ManagementFactory.getRuntimeMXBean();

        boolean invalid = true;
        for (String argument : runtimeMxBean.getInputArguments()) {
            if (argument.startsWith("-Dcl")) {
                invalid = false;
                break;
            }
        }
        if (!System.getProperty("java.class.path").contains("idea_rt.jar") && invalid) {
            alreadyRegistered = false;
        } else if (!runtimeMxBean.getInputArguments().contains("-Xverify:none") && !System.getProperty("java.class.path").contains("idea_rt.jar")) {
            alreadyRegistered = false;
        } else {
    ...
    public static void register() {
        if (!alreadyRegistered) {
            ...
            //Why in the name of fuck does this even exist - Error
            //Like seriously, why would you decide to redirect System.out and System.err to the Log4J Logger
            //redirectOutputToLog();
