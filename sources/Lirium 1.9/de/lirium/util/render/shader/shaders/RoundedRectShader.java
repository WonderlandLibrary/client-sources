...
public class RoundedRectShader implements IMinecraft {

    public ShaderProgram shaderProgram = new ShaderProgram("vertex/vertex.vsh", "/fragment/rounded.glsl", ShaderType.GLSL);
    ...
    public void drawRound(float x, float y, float width, float height, float radius, Color color) {
        ...
        shaderProgram.setUniformf("color", color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha() / 255f);
