...
public class NameTags extends Module {
    public NameTags() {
        super("NameTags", Category.RENDER, "Shows Players NameTags");
    }
...
//stay mad john
    ...
    private void renderNametag(EntityPlayer player, double x, double y, double z) {
        ...
        GL11.glDisable(2929);
        int width = fontRendererIn.getStringWidth(name) / 2;

        drawBorderedRect(-width - 2, 10, width + 1, 20, 0,
                Friends.isFriend(name) ? new Color(0, 130, 130).getRGB() : 0xff373737, -1);


        fontRendererIn.drawString(name, -width, 11, Friends.isFriend(name) ? new Color(0, 255, 255).getRGB() :  new Color(0, 0, 255).getRGB());
    ...
    private void renderEnchantText(EntityPlayer player, ItemStack stack, int x, int y) {
        ...
        if (stack.getItem() instanceof ItemArmor || stack.getItem() instanceof ItemSword
            ...
            fontRendererIn.drawStringWithShadow(stack.getMaxDamage() - stack.getItemDamage() + "\u00A74", x * 2 + 8, y + 26,
                        -1);
        }
        ...
        for (int index = 0; index < enchants.tagCount(); ++index) {
            ...
            if (enc != null) {
                ...
                fontRendererIn.drawStringWithShadow(encName, x * 2 + 13, yCount, -1);
