...
public class WaypointCommand extends Command {
    ...
    public void onCommand(String command, String[] args) throws Exception {
        if (args[0].equalsIgnoreCase("add")) {
            if (args.length == 6) {
                AuroraMod.getInstance().waypointManager.addWaypoint(new Waypoint(
                        args[1],
            ...
            AuroraMod.getInstance().waypointManager.delWaypoint(AuroraMod.getInstance().waypointManager.getWaypointByName(args[1]));
        } else if (args[0].equalsIgnoreCase("list")) {
            TextComponentString tcs = new TextComponentString("");
            AuroraMod.getInstance().waypointManager.getWaypoints().forEach(w -> {
