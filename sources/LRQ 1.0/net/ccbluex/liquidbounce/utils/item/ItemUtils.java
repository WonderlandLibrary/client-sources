...
extends MinecraftInstance {
    public static IItemStack createItem(String itemArguments) {
        try {
            ...
            int i = 1;
            int j = 0;
            for (int mode = 0; mode <= Math.min(12, itemArguments.length() - 2) && (item = functions.getObjectFromItemRegistry(resourcelocation = classProvider.createResourceLocation((args = itemArguments.substring(mode).split(Pattern.quote(" ")))[0]))) == null; ++mode) {
            }
            if (item == null) {
                return null;
            }
