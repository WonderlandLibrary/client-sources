        ...
        switch (ChestAura$WhenMappings.$EnumSwitchMapping$0[event.getEventState().ordinal()]) {
            case 1: {
                ...
                while (iterator.hasNext()) {
                    ...
                    it = element$iv$iv = iterator.next();
                    boolean bl3 = false;
                    boolean bl4 = ((Boolean)throughWallsValue.get()).booleanValue() ? true : (movingObjectPosition = theWorld.rayTraceBlocks(eyesPos, BlockExtensionKt.getVec(blockPos = (WBlockPos)it.getKey()), false, true, false)) != null && ((Object)movingObjectPosition.getBlockPos()).equals(blockPos);
                    if (!bl4) continue;
                    destination$iv$iv.put(element$iv$iv.getKey(), element$iv$iv.getValue());
                }
