...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        MotionFlyingEvent motionFlyingEvent = new MotionFlyingEvent(strafe, forward, friction, MotionFlyingEvent.State.PRE);
        XIV.getInstance().getListenerManager().call(motionFlyingEvent);
        strafe = motionFlyingEvent.getStrafe();
        forward = motionFlyingEvent.getForward();
        friction = motionFlyingEvent.getFriction();
        if (!motionFlyingEvent.isCancelled())
            super.moveFlying(strafe, forward, friction);
        motionFlyingEvent = new MotionFlyingEvent(strafe, forward, friction, MotionFlyingEvent.State.POST);
        XIV.getInstance().getListenerManager().call(motionFlyingEvent);
    }
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        x = motionEvent.getMotionX();
        y = motionEvent.getMotionY();
        z = motionEvent.getMotionZ();
        super.moveEntity(x, y, z);
        motionEvent = new MotionEvent(x, y, z, MotionEvent.State.POST);
        XIV.getInstance().getListenerManager().call(motionEvent);
    }
    ...
    public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            MotionUpdateEvent pre = new MotionUpdateEvent(MotionUpdateEvent.State.PRE, rotationYaw, rotationPitch, posX, posY, posZ, onGround, field_175171_bO);
            ...
            MotionUpdateEvent post = new MotionUpdateEvent(MotionUpdateEvent.State.POST, rotationYaw, rotationPitch, posX, posY, posZ, onGround, field_175171_bO);
