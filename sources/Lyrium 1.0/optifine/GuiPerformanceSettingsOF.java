...
public class GuiPerformanceSettingsOF extends GuiScreen
{
    ...
    public void initGui() {
        ...
        for (int i = 0; i < GuiPerformanceSettingsOF.enumOptions.length; ++i) {
            final GameSettings.Options gamesettingsoptions = GuiPerformanceSettingsOF.enumOptions[i];
            final int j = this.width / 2 - 155 + i % 2 * 160;
            final int k = this.height / 6 + 21 * (i / 2) - 12;
            if (!gamesettingsoptions.getEnumFloat()) {
                this.buttonList.add(new GuiOptionButtonOF(gamesettingsoptions.returnEnumOrdinal(), j, k, gamesettingsoptions, this.settings.getKeyBinding(gamesettingsoptions)));
            }
            else {
                this.buttonList.add(new GuiOptionSliderOF(gamesettingsoptions.returnEnumOrdinal(), j, k, gamesettingsoptions));
            }
        }
