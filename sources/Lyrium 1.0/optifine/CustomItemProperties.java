...
public class CustomItemProperties
{
    ...
    private static IBakedModel bakeModel(final TextureMap p_bakeModel_0_, final ModelBlock p_bakeModel_1_, final boolean p_bakeModel_2_) {
        ...
        final SimpleBakedModel.Builder simplebakedmodelbuilder = new SimpleBakedModel.Builder(p_bakeModel_1_).setTexture(textureatlassprite);
        for (final BlockPart blockpart : p_bakeModel_1_.getElements()) {
            for (final EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
                ...
                if (blockpartface.cullFace == null) {
                    simplebakedmodelbuilder.addGeneralQuad(bakedquad);
                }
                else {
                    simplebakedmodelbuilder.addFaceQuad(modelrotation.rotateFace(blockpartface.cullFace), bakedquad);
                }
            }
        }
        return simplebakedmodelbuilder.makeBakedModel();
    }
