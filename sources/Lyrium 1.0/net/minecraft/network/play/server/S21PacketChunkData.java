...
public class S21PacketChunkData implements Packet<INetHandlerPlayClient>
{
    ...
    public static Extracted func_179756_a(final Chunk p_179756_0_, final boolean p_179756_1_, final boolean p_179756_2_, final int p_179756_3_) {
        ...
        final Extracted s21packetchunkdataextracted = new Extracted();
        ...
        for (int i = 0; i < aextendedblockstorage.length; ++i) {
            ...
            if (extendedblockstorage != null && (!p_179756_1_ || !extendedblockstorage.isEmpty()) && (p_179756_3_ & 1 << i) != 0x0) {
                final Extracted extracted = s21packetchunkdataextracted;
                extracted.dataSize |= 1 << i;
                list.add(extendedblockstorage);
            }
        }
        s21packetchunkdataextracted.data = new byte[func_180737_a(Integer.bitCount(s21packetchunkdataextracted.dataSize), p_179756_2_, p_179756_1_)];
        int j = 0;
        for (final ExtendedBlockStorage extendedblockstorage2 : list) {
            final char[] data;
            final char[] achar = data = extendedblockstorage2.getData();
            for (final char c0 : data) {
                s21packetchunkdataextracted.data[j++] = (byte)(c0 & '\u00ff');
                s21packetchunkdataextracted.data[j++] = (byte)(c0 >> 8 & 0xFF);
            }
        }
        for (final ExtendedBlockStorage extendedblockstorage3 : list) {
            j = func_179757_a(extendedblockstorage3.getBlocklightArray().getData(), s21packetchunkdataextracted.data, j);
        }
        if (p_179756_2_) {
            for (final ExtendedBlockStorage extendedblockstorage4 : list) {
                j = func_179757_a(extendedblockstorage4.getSkylightArray().getData(), s21packetchunkdataextracted.data, j);
            }
        }
        if (p_179756_1_) {
            func_179757_a(p_179756_0_.getBiomeArray(), s21packetchunkdataextracted.data, j);
        }
        return s21packetchunkdataextracted;
    }
