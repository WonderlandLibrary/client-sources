...
public class S38PacketPlayerListItem implements Packet<INetHandlerPlayClient>
{
    ...
    public void readPacketData(final PacketBuffer buf) throws IOException {
        ...
        for (int i = buf.readVarIntFromBuffer(), j = 0; j < i; ++j) {
            ...
            int k = 0;
            WorldSettings.GameType worldsettingsgametype = null;
            ...
            switch (this.action) {
                case ADD_PLAYER: {
                    ...
                    worldsettingsgametype = WorldSettings.GameType.getByID(buf.readVarIntFromBuffer());
            ...
            this.players.add(new AddPlayerData(gameprofile, k, worldsettingsgametype, ichatcomponent));
        }
    }
    ...
    public void writePacketData(final PacketBuffer buf) throws IOException {
        ...
        for (final AddPlayerData s38packetplayerlistitemaddplayerdata : this.players) {
            switch (this.action) {
                case ADD_PLAYER: {
                    buf.writeUuid(s38packetplayerlistitemaddplayerdata.getProfile().getId());
                    buf.writeString(s38packetplayerlistitemaddplayerdata.getProfile().getName());
                    buf.writeVarIntToBuffer(s38packetplayerlistitemaddplayerdata.getProfile().getProperties().size());
                    for (final Property property : ((ForwardingMultimap<K, Property>)s38packetplayerlistitemaddplayerdata.getProfile().getProperties()).values()) {
                    ...
                    buf.writeVarIntToBuffer(s38packetplayerlistitemaddplayerdata.getGameMode().getID());
                    buf.writeVarIntToBuffer(s38packetplayerlistitemaddplayerdata.getPing());
                    if (s38packetplayerlistitemaddplayerdata.getDisplayName() == null) {
                        buf.writeBoolean(false);
                        continue;
                    }
                    buf.writeBoolean(true);
                    buf.writeChatComponent(s38packetplayerlistitemaddplayerdata.getDisplayName());
                    continue;
                }
                case UPDATE_GAME_MODE: {
                    buf.writeUuid(s38packetplayerlistitemaddplayerdata.getProfile().getId());
