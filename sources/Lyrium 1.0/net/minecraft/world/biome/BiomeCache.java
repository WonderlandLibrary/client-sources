...
public class BiomeCache
{
    ...
    public Block getBiomeCacheBlock(int x, int z) {
        ...
        Block biomecacheblock = (Block)this.cacheMap.getValueByKey(i);
        if (biomecacheblock == null) {
            biomecacheblock = new Block(x, z);
            this.cacheMap.add(i, biomecacheblock);
    ...
    public void cleanupCache() {
        ...
        if (j > 7500L || j < 0L) {
            ...
            for (int k = 0; k < this.cache.size(); ++k) {
                final Block biomecacheblock = this.cache.get(k);
                final long l = i - biomecacheblock.lastAccessTime;
                if (l > 30000L || l < 0L) {
                    this.cache.remove(k--);
                    final long i2 = ((long)biomecacheblock.xPosition & 0xFFFFFFFFL) | ((long)biomecacheblock.zPosition & 0xFFFFFFFFL) << 32;
