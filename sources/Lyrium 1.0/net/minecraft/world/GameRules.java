...
public class GameRules
{
    ...
    public void setOrCreateGameRule(final String key, final String ruleValue) {
        final Value gamerulesvalue = this.theGameRules.get(key);
        if (gamerulesvalue != null) {
            gamerulesvalue.setValue(ruleValue);
        }
        else {
    ...
    public String getString(final String name) {
        final Value gamerulesvalue = this.theGameRules.get(name);
        return (gamerulesvalue != null) ? gamerulesvalue.getString() : "";
    }
    
    public boolean getBoolean(final String name) {
        final Value gamerulesvalue = this.theGameRules.get(name);
        return gamerulesvalue != null && gamerulesvalue.getBoolean();
    }
    
    public int getInt(final String name) {
        final Value gamerulesvalue = this.theGameRules.get(name);
        return (gamerulesvalue != null) ? gamerulesvalue.getInt() : 0;
    }
    
    public NBTTagCompound writeToNBT() {
        final NBTTagCompound nbttagcompound = new NBTTagCompound();
        for (final Object s : this.theGameRules.keySet()) {
            final Value gamerulesvalue = this.theGameRules.get(s);
            nbttagcompound.setString((String)s, gamerulesvalue.getString());
        }
    ...
    public boolean areSameType(final String key, final ValueType otherValue) {
        final Value gamerulesvalue = this.theGameRules.get(key);
        return gamerulesvalue != null && (gamerulesvalue.getType() == otherValue || otherValue == ValueType.ANY_VALUE);
    }
    ...
    public enum ValueType
    {
        ...
        private ValueType(final String p_i19_3_, final int p_i19_4_) {
        }
