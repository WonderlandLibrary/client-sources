...
public class MapData extends WorldSavedData
{
    ...
    public void updateVisiblePlayers(final EntityPlayer player, final ItemStack mapStack) {
        if (!this.playersHashMap.containsKey(player)) {
            final MapInfo mapdatamapinfo = new MapInfo(player);
            this.playersHashMap.put(player, mapdatamapinfo);
            this.playersArrayList.add(mapdatamapinfo);
        }
        if (!player.inventory.hasItemStack(mapStack)) {
            this.mapDecorations.remove(player.getName());
        }
        for (int i = 0; i < this.playersArrayList.size(); ++i) {
            final MapInfo mapdatamapinfo2 = this.playersArrayList.get(i);
            if (!mapdatamapinfo2.entityplayerObj.isDead && (mapdatamapinfo2.entityplayerObj.inventory.hasItemStack(mapStack) || mapStack.isOnItemFrame())) {
                if (!mapStack.isOnItemFrame() && mapdatamapinfo2.entityplayerObj.dimension == this.dimension) {
                    this.updateDecorations(0, mapdatamapinfo2.entityplayerObj.worldObj, mapdatamapinfo2.entityplayerObj.getName(), mapdatamapinfo2.entityplayerObj.posX, mapdatamapinfo2.entityplayerObj.posZ, mapdatamapinfo2.entityplayerObj.rotationYaw);
                }
            }
            else {
                this.playersHashMap.remove(mapdatamapinfo2.entityplayerObj);
                this.playersArrayList.remove(mapdatamapinfo2);
            }
        }
    ...
    public Packet getMapPacket(final ItemStack mapStack, final World worldIn, final EntityPlayer player) {
        final MapInfo mapdatamapinfo = this.playersHashMap.get(player);
        return (mapdatamapinfo == null) ? null : mapdatamapinfo.getPacket(mapStack);
    }
    
    public void updateMapData(final int x, final int y) {
        super.markDirty();
        for (final MapInfo mapdatamapinfo : this.playersArrayList) {
            mapdatamapinfo.update(x, y);
        }
    }
    
    public MapInfo getMapInfo(final EntityPlayer player) {
        MapInfo mapdatamapinfo = this.playersHashMap.get(player);
        if (mapdatamapinfo == null) {
            mapdatamapinfo = new MapInfo(player);
            this.playersHashMap.put(player, mapdatamapinfo);
            this.playersArrayList.add(mapdatamapinfo);
        }
        return mapdatamapinfo;
    }
