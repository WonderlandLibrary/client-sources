...
public class Chunk
{
    ...
    private boolean func_150811_f(final int x, final int z) {
        ...
        boolean flag = false;
        boolean flag2 = false;
        final BlockPos.MutableBlockPos blockposmutableblockpos = new BlockPos.MutableBlockPos((this.xPosition << 4) + x, 0, (this.zPosition << 4) + z);
        for (int j = i + 16 - 1; j > this.worldObj.func_181545_F() || (j > 0 && !flag2); --j) {
            blockposmutableblockpos.func_181079_c(blockposmutableblockpos.getX(), j, blockposmutableblockpos.getZ());
            final int k = this.getBlockLightOpacity(blockposmutableblockpos);
            if (k == 255 && blockposmutableblockpos.getY() < this.worldObj.func_181545_F()) {
                flag2 = true;
            }
            if (!flag && k > 0) {
                flag = true;
            }
            else if (flag && k == 0 && !this.worldObj.checkLight(blockposmutableblockpos)) {
                return false;
            }
        }
        for (int l = blockposmutableblockpos.getY(); l > 0; --l) {
            blockposmutableblockpos.func_181079_c(blockposmutableblockpos.getX(), l, blockposmutableblockpos.getZ());
            if (this.getBlock(blockposmutableblockpos).getLightValue() > 0) {
                this.worldObj.checkLight(blockposmutableblockpos);
            }
        }
        return true;
    }
