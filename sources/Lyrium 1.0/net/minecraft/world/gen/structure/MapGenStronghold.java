...
public class MapGenStronghold extends MapGenStructure
{
    ...
    protected StructureStart getStructureStart(final int chunkX, final int chunkZ) {
        Start mapgenstrongholdstart;
        for (mapgenstrongholdstart = new Start(this.worldObj, this.rand, chunkX, chunkZ); mapgenstrongholdstart.getComponents().isEmpty() || mapgenstrongholdstart.getComponents().get(0).strongholdPortalRoom == null; mapgenstrongholdstart = new Start(this.worldObj, this.rand, chunkX, chunkZ)) {}
        return mapgenstrongholdstart;
    }
    ...
    public static class Start extends StructureStart
    {
        ...
        public Start(final World worldIn, final Random p_i2067_2_, final int p_i2067_3_, final int p_i2067_4_) {
            ...
            final StructureStrongholdPieces.Stairs2 structurestrongholdpiecesstairs2 = new StructureStrongholdPieces.Stairs2(0, p_i2067_2_, (p_i2067_3_ << 4) + 2, (p_i2067_4_ << 4) + 2);
            this.components.add(structurestrongholdpiecesstairs2);
            structurestrongholdpiecesstairs2.buildComponent(structurestrongholdpiecesstairs2, this.components, p_i2067_2_);
            final List<StructureComponent> list = structurestrongholdpiecesstairs2.field_75026_c;
            while (!list.isEmpty()) {
                final int i = p_i2067_2_.nextInt(list.size());
                final StructureComponent structurecomponent = list.remove(i);
                structurecomponent.buildComponent(structurestrongholdpiecesstairs2, this.components, p_i2067_2_);
            }
