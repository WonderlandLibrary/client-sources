...
public class WorldGenBigTree extends WorldGenAbstractTree
{
    ...
    void func_175937_a(final BlockPos p_175937_1_, final BlockPos p_175937_2_, final Block p_175937_3_) {
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            final BlockLog.EnumAxis blocklogenumaxis = this.func_175938_b(p_175937_1_, blockpos2);
            this.setBlockAndNotifyAdequately(this.world, blockpos2, p_175937_3_.getDefaultState().withProperty(BlockLog.LOG_AXIS, blocklogenumaxis));
        }
    }
    ...
    private BlockLog.EnumAxis func_175938_b(final BlockPos p_175938_1_, final BlockPos p_175938_2_) {
        BlockLog.EnumAxis blocklogenumaxis = BlockLog.EnumAxis.Y;
        ...
        if (k > 0) {
            if (i == k) {
                blocklogenumaxis = BlockLog.EnumAxis.X;
            }
            else if (j == k) {
                blocklogenumaxis = BlockLog.EnumAxis.Z;
            }
        }
        return blocklogenumaxis;
    }
    
    void generateLeaves() {
        for (final FoliageCoordinates worldgenbigtreefoliagecoordinates : this.field_175948_j) {
            this.generateLeafNode(worldgenbigtreefoliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases() {
        for (final FoliageCoordinates worldgenbigtreefoliagecoordinates : this.field_175948_j) {
            final int i = worldgenbigtreefoliagecoordinates.func_177999_q();
            final BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());
            if (!blockpos.equals(worldgenbigtreefoliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY())) {
                this.func_175937_a(blockpos, worldgenbigtreefoliagecoordinates, Blocks.log);
            }
        }
    }
