...
public class WalkNodeProcessor extends NodeProcessor
{
    ...
    public PathPoint getPathPointTo(final Entity entityIn) {
        ...
        if (this.canSwim && entityIn.isInWater()) {
            ...
            final BlockPos.MutableBlockPos blockposmutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor_double(entityIn.posX), i, MathHelper.floor_double(entityIn.posZ));
            for (Block block = this.blockaccess.getBlockState(blockposmutableblockpos).getBlock(); block == Blocks.flowing_water || block == Blocks.water; block = this.blockaccess.getBlockState(blockposmutableblockpos).getBlock()) {
                ++i;
                blockposmutableblockpos.func_181079_c(MathHelper.floor_double(entityIn.posX), i, MathHelper.floor_double(entityIn.posZ));
            }
        ...
        for (int i = x; i < x + sizeX; ++i) {
            for (int j = y; j < y + sizeY; ++j) {
                for (int k = z; k < z + sizeZ; ++k) {
                    blockposmutableblockpos.func_181079_c(i, j, k);
                    final Block block = blockaccessIn.getBlockState(blockposmutableblockpos).getBlock();
                    if (block.getMaterial() != Material.air) {
                        ...
                        if (entityIn.worldObj.getBlockState(blockposmutableblockpos).getBlock() instanceof BlockRailBase) {
                        ...
                        else if (!block.isPassable(blockaccessIn, blockposmutableblockpos) && (!breakDoors || !(block instanceof BlockDoor) || block.getMaterial() != Material.wood)) {
