...
public class Teleporter
{
    ...
    public boolean placeInExistingPortal(final Entity entityIn, final float rotationYaw) {
        ...
        if (this.destinationCoordinateCache.containsItem(l)) {
            final PortalPosition teleporterportalposition = (PortalPosition)this.destinationCoordinateCache.getValueByKey(l);
            d0 = 0.0;
            blockpos = teleporterportalposition;
            teleporterportalposition.lastUpdateTime = this.worldServerInstance.getTotalWorldTime();
            flag = false;
        }
        else {
        ...
        if (d0 >= 0.0) {
            ...
            final BlockPattern.PatternHelper blockpatternpatternhelper = Blocks.portal.func_181089_f(this.worldServerInstance, blockpos);
            final boolean flag2 = blockpatternpatternhelper.getFinger().rotateY().getAxisDirection() == EnumFacing.AxisDirection.NEGATIVE;
            double d6 = (blockpatternpatternhelper.getFinger().getAxis() == EnumFacing.Axis.X) ? blockpatternpatternhelper.func_181117_a().getZ() : ((double)blockpatternpatternhelper.func_181117_a().getX());
            d4 = blockpatternpatternhelper.func_181117_a().getY() + 1 - entityIn.func_181014_aG().yCoord * blockpatternpatternhelper.func_181119_e();
            if (flag2) {
                ++d6;
            }
            if (blockpatternpatternhelper.getFinger().getAxis() == EnumFacing.Axis.X) {
                d5 = d6 + (1.0 - entityIn.func_181014_aG().xCoord) * blockpatternpatternhelper.func_181118_d() * blockpatternpatternhelper.getFinger().rotateY().getAxisDirection().getOffset();
            }
            else {
                d3 = d6 + (1.0 - entityIn.func_181014_aG().xCoord) * blockpatternpatternhelper.func_181118_d() * blockpatternpatternhelper.getFinger().rotateY().getAxisDirection().getOffset();
            }
            float f = 0.0f;
            float f2 = 0.0f;
            float f3 = 0.0f;
            float f4 = 0.0f;
            if (blockpatternpatternhelper.getFinger().getOpposite() == entityIn.func_181012_aH()) {
                f = 1.0f;
                f2 = 1.0f;
            }
            else if (blockpatternpatternhelper.getFinger().getOpposite() == entityIn.func_181012_aH().getOpposite()) {
                f = -1.0f;
                f2 = -1.0f;
            }
            else if (blockpatternpatternhelper.getFinger().getOpposite() == entityIn.func_181012_aH().rotateY()) {
                f3 = 1.0f;
                f4 = -1.0f;
            }
            else {
            ...
            entityIn.setLocationAndAngles(d3, d4, d5, entityIn.rotationYaw = rotationYaw - entityIn.func_181012_aH().getOpposite().getHorizontalIndex() * 90 + blockpatternpatternhelper.getFinger().getHorizontalIndex() * 90, entityIn.rotationPitch);
            return true;
        }
        return false;
    }
        ...
        for (int j3 = j - i; j3 <= j + i; ++j3) {
            ...
            Label_0461:
                for (int j4 = this.worldServerInstance.getActualHeight() - 1; j4 >= 0; --j4) {
                    if (this.worldServerInstance.isAirBlock(blockposmutableblockpos.func_181079_c(j3, j4, l3))) {
                        while (j4 > 0 && this.worldServerInstance.isAirBlock(blockposmutableblockpos.func_181079_c(j3, j4 - 1, l3))) {
                            --j4;
                        }
                            ...
                            for (int j5 = 0; j5 < 3; ++j5) {
                                for (int k4 = 0; k4 < 4; ++k4) {
                                    for (int l5 = -1; l5 < 4; ++l5) {
                                        ...
                                        blockposmutableblockpos.func_181079_c(i5, j6, k5);
                                        if (l5 < 0 && !this.worldServerInstance.getBlockState(blockposmutableblockpos).getBlock().getMaterial().isSolid()) {
                                            continue Label_0461;
                                        }
                                        if (l5 >= 0 && !this.worldServerInstance.isAirBlock(blockposmutableblockpos)) {
                                            continue Label_0461;
                                        }
                                    }
                                }
            ...
            for (int l6 = j - i; l6 <= j + i; ++l6) {
                ...
                Label_0833:
                    for (int i6 = this.worldServerInstance.getActualHeight() - 1; i6 >= 0; --i6) {
                        if (this.worldServerInstance.isAirBlock(blockposmutableblockpos.func_181079_c(l6, i6, j7))) {
                            while (i6 > 0 && this.worldServerInstance.isAirBlock(blockposmutableblockpos.func_181079_c(l6, i6 - 1, j7))) {
                                --i6;
                            }
                            for (int k6 = i3; k6 < i3 + 2; ++k6) {
                                ...
                                for (int j10 = 0; j10 < 4; ++j10) {
                                    for (int j11 = -1; j11 < 4; ++j11) {
                                        ...
                                        blockposmutableblockpos.func_181079_c(j12, i7, j13);
                                        if (j11 < 0 && !this.worldServerInstance.getBlockState(blockposmutableblockpos).getBlock().getMaterial().isSolid()) {
                                            continue Label_0833;
                                        }
                                        if (j11 >= 0 && !this.worldServerInstance.isAirBlock(blockposmutableblockpos)) {
                                            continue Label_0833;
                                        }
                                    }
                                }
    ...
    public void removeStalePortalLocations(final long worldTime) {
        if (worldTime % 100L == 0L) {
            ...
            while (iterator.hasNext()) {
                ...
                final PortalPosition teleporterportalposition = (PortalPosition)this.destinationCoordinateCache.getValueByKey(olong);
                if (teleporterportalposition == null || teleporterportalposition.lastUpdateTime < i) {
                    iterator.remove();
