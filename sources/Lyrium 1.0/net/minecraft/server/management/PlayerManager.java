...
public class PlayerManager
{
    ...
    public void updatePlayerInstances() {
        ...
        if (i - this.previousTotalWorldTime > 8000L) {
            ...
            for (int j = 0; j < this.playerInstanceList.size(); ++j) {
                final PlayerInstance playermanagerplayerinstance = this.playerInstanceList.get(j);
                playermanagerplayerinstance.onUpdate();
                playermanagerplayerinstance.processChunk();
            }
        }
        else {
            for (int k = 0; k < this.playerInstancesToUpdate.size(); ++k) {
                final PlayerInstance playermanagerplayerinstance2 = this.playerInstancesToUpdate.get(k);
                playermanagerplayerinstance2.onUpdate();
            }
        }
    ...
    private PlayerInstance getPlayerInstance(final int chunkX, final int chunkZ, final boolean createIfAbsent) {
        final long i = chunkX + 2147483647L | chunkZ + 2147483647L << 32;
        PlayerInstance playermanagerplayerinstance = (PlayerInstance)this.playerInstances.getValueByKey(i);
        if (playermanagerplayerinstance == null && createIfAbsent) {
            playermanagerplayerinstance = new PlayerInstance(chunkX, chunkZ);
            this.playerInstances.add(i, playermanagerplayerinstance);
            this.playerInstanceList.add(playermanagerplayerinstance);
        }
        return playermanagerplayerinstance;
    }
    
    public void markBlockForUpdate(final BlockPos pos) {
        final int i = pos.getX() >> 4;
        final int j = pos.getZ() >> 4;
        final PlayerInstance playermanagerplayerinstance = this.getPlayerInstance(i, j, false);
        if (playermanagerplayerinstance != null) {
            playermanagerplayerinstance.flagChunkForUpdate(pos.getX() & 0xF, pos.getY(), pos.getZ() & 0xF);
        }
    }
    ...
    public void removePlayer(final EntityPlayerMP player) {
        ...
        for (int k = i - this.playerViewRadius; k <= i + this.playerViewRadius; ++k) {
            for (int l = j - this.playerViewRadius; l <= j + this.playerViewRadius; ++l) {
                final PlayerInstance playermanagerplayerinstance = this.getPlayerInstance(k, l, false);
                if (playermanagerplayerinstance != null) {
                    playermanagerplayerinstance.removePlayer(player);
                }
            }
        }
                ...
                for (int l2 = i - i2; l2 <= i + i2; ++l2) {
                    for (int i3 = j - i2; i3 <= j + i2; ++i3) {
                        ...
                        if (!this.overlaps(l2 - j2, i3 - k2, i, j, i2)) {
                            final PlayerInstance playermanagerplayerinstance = this.getPlayerInstance(l2 - j2, i3 - k2, false);
                            if (playermanagerplayerinstance != null) {
                                playermanagerplayerinstance.removePlayer(player);
                            }
                        }
    ...
    public boolean isPlayerWatchingChunk(final EntityPlayerMP player, final int chunkX, final int chunkZ) {
        final PlayerInstance playermanagerplayerinstance = this.getPlayerInstance(chunkX, chunkZ, false);
        return playermanagerplayerinstance != null && playermanagerplayerinstance.playersWatchingChunk.contains(player) && !player.loadedChunks.contains(playermanagerplayerinstance.chunkCoords);
    }
                ...
                if (i > 0) {
                    for (int j2 = j - radius; j2 <= j + radius; ++j2) {
                        for (int k2 = k - radius; k2 <= k + radius; ++k2) {
                            final PlayerInstance playermanagerplayerinstance = this.getPlayerInstance(j2, k2, true);
                            if (!playermanagerplayerinstance.playersWatchingChunk.contains(entityplayermp)) {
                                playermanagerplayerinstance.addPlayer(entityplayermp);
                            }
                        }
                    }
                }
