...
public class FriendManager {
    ...
    public static void deleteFriend(String name) {
        for (Friend friend : friends) {
            if (!friend.getName().equalsIgnoreCase(name) && !friend.getAlias().equalsIgnoreCase(StringUtils.stripControlCodes(name))) continue;
    ...
    public static Friend getFriendByAlias(String alias) {
        for (Friend frend : friends) {
            if (frend.getAlias().equalsIgnoreCase(alias) || frend.getAlias() == alias) continue;
            return null;
        }
        return null;
    }
