...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (this.mc.currentScreen == null) {
                flag = mc.gameSettings.isKeyDown(this.mc.gameSettings.ofKeyBindZoom);
            }
        ...
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    final int colorValue = worldColorFeature.colorSetting.getColor();
                    float f8 = worldColorFeature.isEnabled() ? new Color(colorValue).getRed() / 255.0f : f4 + f3;
                    float f9 = worldColorFeature.isEnabled() ? new Color(colorValue).getGreen() / 255.0f : f5 + f6;
                    float f10 = worldColorFeature.isEnabled() ? new Color(colorValue).getBlue() / 255.0f : f2 + f7;
                    f8 = f8 * 0.96f + 0.03f;
                    f9 = f9 * 0.96f + 0.03f;
                    f10 = f10 * 0.96f + 0.03f;
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        Render3DEvent event3D = new Render3DEvent(partialTicks);
        ClientMain.INSTANCE.eventBus.call(event3D);
