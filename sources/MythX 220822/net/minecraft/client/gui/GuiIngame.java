...
import dev.myth.features.visual.ScoreboardFeature;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        ClientMain.INSTANCE.eventBus.call(new Render2DEvent(scaledresolution));
        ...
        final ScoreboardFeature scoreboardModule = ClientMain.INSTANCE.manager.getManager(FeatureManager.class).getFeature(ScoreboardFeature.class);
        ...
        if (scoreobjective1 != null) {
            GlStateManager.pushMatrix();
            if (scoreboardModule.isEnabled())
                GlStateManager.translate(-scoreboardModule.xPosition.getValue(), scoreboardModule.yPosition.getValue(), 1.0);
            if (!scoreboardModule.isEnabled() || !scoreboardModule.hide.getValue())
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes, final boolean rect) {
        ...
        ScoreboardFeature scoreboardModule = ClientMain.INSTANCE.manager.getManager(FeatureManager.class).getFeature(ScoreboardFeature.class);
        ...
        for (Score score1 : collection) {
            ...
            if (j == 1 && mc.getCurrentServerData() != null && StringUtil.removeFormatting(s1).toLowerCase().contains(mc.getCurrentServerData().serverIP.toLowerCase())) {
                mc.fontRendererObj.drawStringWithShadow("vyra.xyz", l1, k, ColorUtil.getClientColor((float) (height * 20)));
            } else {
