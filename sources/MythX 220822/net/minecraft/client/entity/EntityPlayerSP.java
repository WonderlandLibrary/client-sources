...
import dev.myth.features.combat.TargetStrafeFeature;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            UpdateEvent event = new UpdateEvent(EventState.UPDATE, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, this.isSprinting(), this.isSneaking());
            ClientMain.INSTANCE.eventBus.call(event);
    ...
    public UpdateEvent currentEvent;

    public void onUpdateWalkingPlayer() {
        UpdateEvent event = new UpdateEvent(EventState.PRE, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, this.isSprinting(), this.isSneaking());
        event.setLastYaw(this.lastReportedYaw);
        event.setLastPitch(this.lastReportedPitch);
        event.setLastY(this.lastReportedPosY);
        event.setLastX(this.lastReportedPosX);
        event.setLastZ(this.lastReportedPosZ);
        ClientMain.INSTANCE.eventBus.call(event);
    ...
    public boolean isClientMessage(final String msg) {
        if (msg.startsWith(".")) {
            ClientMain.INSTANCE.manager.getManager(CommandManager.class).call(msg.substring(1));
            return false;
        }
        return true;
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && (!this.isUsingItem() || noSlow) && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        MoveFlyingEvent event = new MoveFlyingEvent(this.rotationYaw, strafe, forward, friction);
        ClientMain.INSTANCE.eventBus.call(event);
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        TargetStrafeFeature targetStrafeModule = ClientMain.INSTANCE.manager.getManager(FeatureManager.class).getFeature(TargetStrafeFeature.class);
        if (targetStrafeModule.isEnabled()) {
            event = targetStrafeModule.editMovement(event.getX(), event.getY(), event.getZ());
        }
