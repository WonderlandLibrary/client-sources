...
import digital.bq.module.ModuleManager;
import digital.bq.module.implement.Combat.KillAura;
import digital.bq.module.implement.Render.BlockAnimation;
import digital.bq.utility.AnimationUtils;
...
public class ItemRenderer {
	 ...
	 */
	// Client
	public void renderBlock(float swingProgress, float equippedProgress) {


		float var15;

		// 检测动画是否开启
		if (!ModuleManager.animationMod.isEnabled()) {
			this.transformFirstPersonItem(equippedProgress, 0.0f);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Slide")) {
			...
			// signature
			GlStateManager.rotate(-f1 * 70.0F / 2.0F, -8.0F, -0.0F, 9.0F);
			GlStateManager.rotate(-f1 * 70.0F, 1.0F, -0.4F, -0.0F);
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Sigma")) {
			this.transformFirstPersonItem(equippedProgress, 0.0f);
		...
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Push")) {
			...
			GlStateManager.scale(0.4F, 0.4F, 0.4F);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Push1")) {
			...
			GlStateManager.scale(0.4F, 0.4F, 0.4F);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Reverse1")) {
			this.transformFirstPersonItem(equippedProgress, swingProgress);
			this.doBlockTransformations();
			GL11.glTranslated(1.0D, -0.1D, 0.5D);
			GL11.glTranslatef(-1.0F, this.mc.thePlayer.isSneaking() ? -0.2F : -0.3F, 0.1F);
		} else if (BlockAnimation.swordAnimation.isCurrentMode("1.7")) {
			GlStateManager.translate(0, 0, 0);
			this.transformFirstPersonItem(equippedProgress, swingProgress);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Exhi 1.7")) {
			GlStateManager.translate(0, 0.18F, 0);
			this.transformFirstPersonItem(equippedProgress / 2.0f, swingProgress);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Vanilla")) {
			this.transformFirstPersonItem(equippedProgress, 0.0f);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Fixed")) {
			this.doItemUsedTransformations(swingProgress);
			this.transformFirstPersonItem(equippedProgress, 0.0f);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Reverse")) {
			this.doItemUsedTransformations(swingProgress);
			this.transformFirstPersonItem(equippedProgress, swingProgress);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Strange")) {
			GlStateManager.translate(-0.16F, -0F, -0.411999997F);
			GlStateManager.translate(-0.04F, 0.3F, 0F);
			this.transformFirstPersonItem(equippedProgress, swingProgress);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Spin")) {
			GlStateManager.translate(0.56F, -0.35F, -0.71999997F);
			GlStateManager.translate(0.0F, equippedProgress * -0.6F, 0.0F);
			GlStateManager.rotate(AnimationUtils.getRotateDirection(), 0.0F, 1.0F, 0.0F);
			/**
			float f = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI);
			float f1 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
			GlStateManager.rotate(f * -20.0F, 0.0F, 1.0F, 0.0F);
			...
			GlStateManager.rotate(f1 * -80.0F, 1.0F, 0.0F, 0.0F);
			 */
			GlStateManager.rotate(60.0F, 1.0F, 0.0F, 20.0F);
			GlStateManager.scale(0.34F, 0.34F, 0.34F);
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Screw")) {
			GlStateManager.translate(1.5F, -1.3F, -2F);
			GlStateManager.rotate(-150.0F, 0.0F, 1.0F, 0.0F);
			GlStateManager.rotate(-64F, 0.0F, 1.0F, 0.0F);
			GlStateManager.rotate(AnimationUtils.getRotateDirection() , 0.7F, 1.0F, 1.0F);
			GlStateManager.rotate(64F, 1.0F, 1.0F, 0.0F);
			GlStateManager.scale(0.8F, 0.8F, 0.8F);
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Poke")) {
			this.transformFirstPersonItem(equippedProgress, 1.0f);//原装
			GlStateManager.rotate(-50.0F, 0.0F, 1.0F, 0.0F);//倾斜剑，更有3D感
			GlStateManager.translate(0.56F, 0.2F, -0.11999997F);//修一波位置
			float f = -0.4F * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
			float f1 = 0.2F * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI * 2.0F);
			float f2 = -0.2F * MathHelper.sin(swingProgress * (float) Math.PI);
			GL11.glTranslatef(f, f1, f2);//额，GlStateManager里面没找到，只能直接调用GL11
			this.doBlockTransformations();//Block!
		}
		else if (BlockAnimation.swordAnimation.isCurrentMode("Swong")) {
			...
			GL11.glRotated(-var15 * 25.0F, var15 / 4.9, 0, 2.0F);
			GL11.glRotated(-var15 * 30.0F, 4.0F, var15 * 1.7, 0F);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Exhi Swang")) {
			this.transformFirstPersonItem(equippedProgress / 2.0F, swingProgress);
			var15 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927F);
			...
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Exhi Swong")) {
			this.transformFirstPersonItem(equippedProgress / 2.0F, 0.0f);
			var15 = MathHelper.sin((MathHelper.sqrt_float(swingProgress) * 3.1415927F));
			GlStateManager.translate(0.0F, 0.3F, -0.0F);
			GlStateManager.rotate(-var15 * 30.0F, 1, 0, 2.0F);
			GlStateManager.rotate(-var15 * 44.0F, 1.5F, (float) (var15 / 1.2), 0F);
			this.doBlockTransformations();
		} else if (BlockAnimation.swordAnimation.isCurrentMode("Exhi Swank")) {
			this.transformFirstPersonItem(equippedProgress / 2.0F, swingProgress);
			var15 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927F);
			...
			this.transformFirstPersonItem(equippedProgress / 2.0f, -0.2F);
			var15 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * swingProgress * 3.1415927F);
		...
		// 检测动画是否开启
		if (ModuleManager.animationMod.isEnabled()) {
			GlStateManager.translate(BlockAnimation.x.getValue(), BlockAnimation.y.getValue(), BlockAnimation.z.getValue());
		}
	}
		...
		if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
			...
			if (this.itemToRender != null) {
				...
				if (this.itemToRender.getItem() == Items.filled_map) {
					this.renderItemMap(abstractclientplayer, f2, equippedProgress, swingProgress);
				} else if (abstractclientplayer.getItemInUseCount() > 0) {
					enumaction = this.itemToRender.getItemUseAction();
					...
					case BLOCK:
						renderBlock(swingProgress, equippedProgress);
						break;
				...
				} else if ((Lycoris.everythingBlock.getValueState() && mc.gameSettings.keyBindUseItem.pressed) || (KillAura.isRenderBlocked() && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)) {
					enumaction = EnumAction.BLOCK;
					renderBlock(swingProgress, equippedProgress);
				} else {
					if (!ModuleManager.animationMod.isEnabled() || !BlockAnimation.swingAnimation.getValue()) {
						this.doItemUsedTransformations(swingProgress);
					}
				...
				if (enumaction == EnumAction.NONE && ModuleManager.animationMod.isEnabled()) {
					GlStateManager.translate(BlockAnimation.xArm.getValue(), BlockAnimation.yArm.getValue(), BlockAnimation.zArm.getValue());
				}
