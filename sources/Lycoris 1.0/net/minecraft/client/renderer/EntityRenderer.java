...
import digital.bq.event.WorldRenderEvent;
import digital.bq.module.ModuleManager;
import digital.bq.module.implement.Combat.Reach;
...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			if (this.mc.playerController.extendedReach()) {
				d0 = 6.0D;
				d1 = 6.0D;
			} else if (ModuleManager.reachMod.isEnabled()) {
				// Client: Reach
				d0 = d1 = Reach.range.getValue();
			} else {
				if (d0 > 3.0D) {
					flag = true;
				}
	...
	private void updateFovModifierHand() {
		...
		// Client
		if (ModuleManager.noFovMod.isEnabled()) {
			this.fovModifierHand = ModuleManager.noFovMod.fov.getValue().floatValue();
		}
	}
	 ...
	 * or not
	 * 
	 * @param useFOVSetting If true the FOV set in the settings will be use in the
	 *                      calculation
	 */
		...
		if (this.debugView) {
			...
			return Reflector.ForgeHooksClient_getFOVModifier.exists()
					? Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier,
							new Object[] { this, entity, block, Float.valueOf(partialTicks), Float.valueOf(f) })
					: f;
		}
	}
	...
	private void hurtCameraEffect(float partialTicks) {
		...
		// Client: NoHurtCam
		if (ModuleManager.noHurtCamMod.isEnabled())
			return;
	...
	private void setupViewBobbing(float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			if (!Lycoris.RealBobbing.getValue()) {
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					if (movingobjectposition != null) {
						...
						// Client: ViewClip
						if (d7 < d3 && !ModuleManager.viewClipMod.isEnabled()) {
							d3 = d7;
						}
					}
				}
	...
	private void updateLightmap(float partialTicks) {
		if (this.lightmapUpdateNeeded) {
			...
			if (world != null) {
				...
				// Client
				if (ModuleManager.fullBrightMod.isEnabled() || ModuleManager.xRayMod.isEnabled()) {
					for (int i = 0; i < 256; ++i) {
						...
						public String call() throws Exception {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d",
									new Object[] { Integer.valueOf(scaledresolution.getScaledWidth()),
											...
											Float.valueOf(scaledresolution.getScaleFactor()) });
						}
					});
