...
import digital.bq.module.implement.Render.EnchantEffect;
...
public class RenderItem implements IResourceManagerReloadListener {
	...
	private void renderEffect(IBakedModel model) {
		if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
			if (!Config.isShaders() || !Shaders.isShadowPass) {
				...
				// Client EnchantEffect
				int color = -8372020;

				if (ModuleManager.enchantEffectMod.isEnabled())
					color = EnchantEffect.enchantEffectColours.getColorInt();
	...
	public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, String text) {
		if (stack != null) {
			if (stack.stackSize != 1 || text != null) {
				...
				GlStateManager.disableBlend();
				FontManager.small.drawStringWithSuperShadow(s, (float) (xPosition + 19 - 3 - FontManager.small.getStringWidth(s)),
						(float) (yPosition + 6 + 1), ColorUtils.WHITE.c);
				GlStateManager.enableLighting();
	...
	public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, float xPosition, float yPosition, String text, int color) {
		if (stack != null) {
			if (stack.stackSize != 1 || text != null) {
				String s = text == null ? String.valueOf(stack.stackSize) : text;
				...
				GlStateManager.disableDepth();
				GlStateManager.disableBlend();
				fr.drawStringWithLightShadow(s, (float) (xPosition + 19 - 3.5f - fr.getStringWidth(s)), (float) (yPosition + 6 + 2), color);
				GlStateManager.enableLighting();
				GlStateManager.enableDepth();
				GlStateManager.enableBlend();
			}
	 ...
	 * @param alpha    Alpha component of the color
	 */
	private void draw(WorldRenderer renderer, float x, float y, float width, float height, int red, int green, int blue,
					  int alpha) {
		renderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
		renderer.pos((double) (x + 0), (double) (y + 0), 0.0D).color(red, green, blue, alpha).endVertex();
		renderer.pos((double) (x + 0), (double) (y + height), 0.0D).color(red, green, blue, alpha).endVertex();
