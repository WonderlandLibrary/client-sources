...
import digital.bq.event.EventRendererLivingEntity;
import digital.bq.event.EventRotationAnimation;
import digital.bq.event.NameTagRenderEvent;
import digital.bq.module.ModuleManager;
import digital.bq.module.implement.Combat.AntiBots;
import digital.bq.module.implement.Render.AntiInvis;
import digital.bq.module.implement.Render.ESP;
import digital.bq.utility.WorldRenderUtils;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			try {
				...
				EventRotationAnimation event = new EventRotationAnimation(entity, f, f1, f2, f7, partialTicks);
				...
				if (entity instanceof EntityPlayer) {
					EventRendererLivingEntity evt = new EventRendererLivingEntity(entity, EventType.PRE, f6, f5, f8, f2, f7, f, 0.0625f);
					EventManager.call(evt);
				...
				if (this.renderOutlines) {
					...
					if (ModuleManager.espMod.isEnabled() && ESP.mode.isCurrentMode("Outline") && ESP.isValidForESP(entity)) {
						this.renderModel(entity, f6, f5, f8, f2, f7, 0.0625F);
			...
			EventRendererLivingEntity evt = new EventRendererLivingEntity(entity, EventType.POST);
			EventManager.call(evt);
	...
	public void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        boolean isVisible = !entitylivingbaseIn.isInvisible();
        boolean flag1 = !isVisible && !entitylivingbaseIn.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer);

        if (isVisible || flag1 || ModuleManager.antiInvisMod.isEnabled()) {
            if (!this.bindEntityTexture(entitylivingbaseIn)) {
                return;
            }

            if (flag1 || (ModuleManager.antiInvisMod.isEnabled() && AntiInvis.trancperent.getValue())) {
                if(entitylivingbaseIn.isInvisible()) {
            	GlStateManager.pushMatrix();
                GlStateManager.color(1.0F, 1.0F, 1.0F, ModuleManager.antiInvisMod.transAlpha.getValue()/100);
                GlStateManager.depthMask((ModuleManager.antiInvisMod.transAlpha.getValue()/100)>=0.75f);
	...
	public void renderName(T entity, double x, double y, double z) {
		if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists()
			...
			if (this.canRenderName(entity)) {
				if (ModuleManager.nameTagsMod.isEnabled() && ModuleManager.nameTagsMod.isValid(entity)) {
					return;
				}
				...
				if (d0 < (double) (f * f)) {
					...
					if (entity.isSneaking()) {
						NameTagRenderEvent event = new NameTagRenderEvent(entity, x, y, z);
	                    EventManager.call(event);
						...
						float i = fontrenderer.getStringWidth(s) / 2;
						...
						DrawTextEvent drawTextEvent = new DrawTextEvent(s);
                        EventManager.call(drawTextEvent);
                        s = drawTextEvent.text;
						
						fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, -1.5f, 553648127);
						GlStateManager.enableLighting();
						GlStateManager.disableBlend();
						GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
						GlStateManager.popMatrix();
