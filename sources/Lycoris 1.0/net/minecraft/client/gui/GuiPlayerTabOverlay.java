...
public class GuiPlayerTabOverlay extends Gui {
	...
	public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
		if (ModuleManager.noRenderMod.isEnabled() && NoRender.tablist.getValue())
            return;
		...
		for (NetworkPlayerInfo networkplayerinfo : list) {
	        ...
	        DrawTextEvent event = new DrawTextEvent(playername);
            EventManager.call(event);
			int k = this.mc.fontRendererObj.getStringWidth(playername);
            int k1 = this.mc.fontRendererObj.getStringWidth(event.text);
            int k2 = mc.fontRendererObj.getStringWidth(this.getPlayerName(networkplayerinfo));
			...
			IRCUser user = IRCUser.getIRCUserByIGN(networkplayerinfo.getGameProfile().getName());

			if (user != null) {
				clientTag = "\247" + user.rank.charAt(0) + "[" + user.rank.substring(1) + "|" + user.username + "] ";
			}

			k = k + this.mc.fontRendererObj.getStringWidth(clientTag); //取最大长度
			i = Math.max(i, k);
		...
		float target = isBeingRendered ? (Hud.waterMarkMode.isCurrentMode("Lycoris Sense") ? 22f : 10) : -(((list.size() / j4) * 10) + (list1 != null ? list1.size() * 10 : 0) + (list2 != null ? list2.size() * 10 : 0));
		this.animation = AnimationUtils.getAnimationState(this.animation, target, Math.max(10, (Math.abs(this.animation - target)) * 35) * 0.3f);

		if(Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
			if (this.hue > 255.0F) {
				this.hue = 0.0F;
			}
			...
			float bottom = ((list.size() / j4 + (list.size() % j4)) * mc.fontRendererObj.FONT_HEIGHT + 1) + (list1 != null ? list1.size() * mc.fontRendererObj.FONT_HEIGHT + 1 : 0) + (list2 != null ? list2.size() * mc.fontRendererObj.FONT_HEIGHT + 4 : 0) + 1;
		...
		if (list1 != null) {
			...
			if(!Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
				drawRect(width / 2 - l1 / 2 - 2, k1 - 1, width / 2 + l1 / 2 + 2, k1 + list1.size() * this.mc.fontRendererObj.FONT_HEIGHT, 0xcc40444b);
			}
		...
		if(!Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
			drawRect(width / 2 - l1 / 2 - 2, k1 - 1, width / 2 + l1 / 2 + 2, k1 + i4 * 9, 0xcc40444b);
		}
		...
		for (int k4 = 0; k4 < l3; ++k4) {
			...
			float k2 = k1 + i5 * 9;
			drawRect(j2, k2, j2 + i1, k2 + 9, Hud.waterMarkMode.isCurrentMode("Lycoris Sense") ? 0xff40444b : ColorUtils.getColor(60, 150));
			...
			if (k4 < list.size()) {
				...
				IRCUser user = IRCUser.getIRCUserByIGN(networkplayerinfo1.getGameProfile().getName());

				if (user != null) {
					clientTag = "\247" + user.rank.charAt(0) + "[" + user.rank.substring(1) + "|" + user.username + "] \247r";
				}


				s1 = clientTag + event.text;
                s1 = s1.replaceAll("\u00a7k", Lycoris.noobfname.getValueState() ? "" : "\u00a7k");
		...
		if (list2 != null) {
			k1 = k1 + i4 * 9 + 1;
			if(!Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
				drawRect(width / 2 - l1 / 2 - 2, k1 - 1, width / 2 + l1 / 2 + 2, k1 + list2.size() * this.mc.fontRendererObj.FONT_HEIGHT + 2, 0xcc40444b);
			}
		...
		if(!Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
			drawRect(width / 2 - l1 / 2 - 2, k1 + 1, width / 2 + l1 / 2 + 2,
					k1 + 2, 0xff4286f5);
		}
	}

	protected void drawPing(int p_175245_1_, int p_175245_2_, float p_175245_3_, NetworkPlayerInfo networkPlayerInfoIn) {
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
	...
	private void drawScoreboardValues(ScoreObjective p_175247_1_, float p_175247_2_, String p_175247_3_, int p_175247_4_,
		...
		if (p_175247_1_.getRenderType() == IScoreObjectiveCriteria.EnumRenderType.HEARTS) {
			...
			if (j > 0) {
				...
				if (f > 3.0F) {
					for (int l = j; l < k; ++l) {
						this.drawTexturedModalRect((float) p_175247_4_ + (float) l * f, p_175247_2_,
								flag ? 25 : 16, 0, 9, 9);
					}

					for (int j1 = 0; j1 < j; ++j1) {
						this.drawTexturedModalRect((float) p_175247_4_ + (float) j1 * f, p_175247_2_,
								flag ? 25 : 16, 0, 9, 9);
			...
			this.mc.fontRendererObj.drawStringWithShadow(s1,
					(float) (p_175247_5_ - this.mc.fontRendererObj.getStringWidth(s1)), p_175247_2_ + 0.5f, 16777215);
		}
	}
