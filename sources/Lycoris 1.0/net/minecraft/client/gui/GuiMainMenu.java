...
import digital.bq.addon.AddonManager;
import digital.bq.gui.altMgr.GuiAltMgr;
import digital.bq.gui.clickgui.classic.GuiRenderUtils;
import digital.bq.gui.clickgui.classic.RenderUtil;
import digital.bq.gui.fontRenderer.FontManager;
import digital.bq.gui.loginui.GuiLogin;
import digital.bq.gui.plugingui.GuiPluginMgr;
import digital.bq.irc.IRCClient;
import digital.bq.utility.AnimationUtils;
import digital.bq.utility.ColorUtils;
import digital.bq.utility.MathUtils;
import digital.bq.utility.shader.shaders.ShaderBlob;
...
public class GuiMainMenu extends GuiScreen {
	...
	RoundButton addonButton = new RoundButton(new GuiPluginMgr(this), "Lycoris Addon", "E", 0, 0);
	...
	public void initGui() {
		...
		addonButton = new RoundButton(new GuiPluginMgr(this), "Lycoris Addon", "E", -.5f, -.5f);
		adding = false;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		try {
			if (Lycoris.irc == null || IRCClient.loggedPacket == null) {
			...
			if (!Lycoris.background.getValue()) {
				//B L O B
				if (Lycoris.INSTANCE.blobShader == null)
					Lycoris.INSTANCE.blobShader = new ShaderBlob();
				Lycoris.INSTANCE.blobShader.renderShader(this.width, this.height);
			} else {
			...
			// 信息显示
			FontManager.roboto15.drawStringWithAlpha(Lycoris.NAME + " b" + Lycoris.VERSION + " " + Lycoris.status, 10, height - 35,
			...
			try {
				if (AddonManager.loaded && Lycoris.INSTANCE.api != null) {
					...
					int apiSize = Lycoris.INSTANCE.api.getAddonManager().getEnabledLycorisAddonList().size();
					String apiString = "Enabled " + EnumChatFormatting.GREEN + apiSize + EnumChatFormatting.WHITE + " Addon" + (apiSize > 1 ? "s" : "") + " (" + Lycoris.INSTANCE.api.getAddonManager().getLycorisAddonList().size() + " Loaded)";
			...
			if (!drawAPI) {
				String apiString = "Lycoris API Loading...";
				FontManager.roboto15.drawStringWithAlpha(apiString, width - FontManager.roboto15.getStringWidth(apiString) - 9, height - 25, ColorUtils.WHITE.c, 0.6f);
			}

			FontManager.roboto15.drawStringWithAlpha(Lycoris.CREDIT,
					width - FontManager.robotoL15.getStringWidth(Lycoris.CREDIT) - 12, height - 15, ColorUtils.WHITE.c,
					0.6f);
