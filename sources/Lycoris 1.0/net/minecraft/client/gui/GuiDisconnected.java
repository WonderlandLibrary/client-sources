...
import digital.bq.gui.altMgr.Alt;
import digital.bq.gui.altMgr.GuiAltMgr;
import digital.bq.utility.DelayTimer;
...
public class GuiDisconnected extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (Lycoris.currentAlt != null && GuiAltMgr.alts.stream().anyMatch(alt -> !alt.isCracked()
                && alt.getEmail().equals(Lycoris.currentAlt[0]) && alt.getPassword().equals(Lycoris.currentAlt[1])))
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 1338) {
            if (Lycoris.AUTO_RECONNECT && !this.timer.hasPassed(5000)) {
                Lycoris.AUTO_RECONNECT = false;
                randomAlt.displayString = "Reconnect with a Random Alt";
                return;
            }

            Lycoris.AUTO_RECONNECT = true;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        // Client
        if (doReconnect) {
            this.mc.displayGuiScreen(new GuiConnecting(null, this.mc, Lycoris.INSTANCE.lastServer));
            return;
        }

        this.drawDefaultBackground();

        long durationInMillis = sessionEndTime - Lycoris.playTimeStart;
        ...
        if (Lycoris.AUTO_RECONNECT && this.randomAlt.enabled) {
