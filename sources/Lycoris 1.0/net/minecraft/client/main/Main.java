...
import digital.bq.utility.LoginUtils;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        OptionSpec<String> alt = optionparser.accepts("alt").withRequiredArg();
        ...
        OptionSpec<String> flux_username = optionparser.accepts("fluxusername").withRequiredArg().defaultsTo("", new String[0]);
        OptionSpec<String> flux_password = optionparser.accepts("fluxpassword").withRequiredArg().defaultsTo("", new String[0]);

        OptionSpec<String> flux_locale = optionparser.accepts("locale").withRequiredArg().defaultsTo(Locale.getDefault().getCountry(), new String[0]);

        OptionSpec<String> optionspec19 = optionparser.nonOptions();
        OptionSet optionset = optionparser.parse(args);

        if (!optionset.valueOf(flux_username).equals("") && !optionset.valueOf(flux_password).equals("")) {
            GuiLogin.account = new String[]{optionset.valueOf(flux_username), optionset.valueOf(flux_password)};
        }

        IRCClient.isChina = optionset.valuesOf(flux_locale).toString().toLowerCase().contains("cn");
        ...
        //Client : start
        if (alt != null && alt.value(optionset) != null && alt.value(optionset).contains(":")) {
            String altRaw = alt.value(optionset);

            session = LoginUtils.getSession(altRaw.split(":")[0], altRaw.split(":")[1]);
