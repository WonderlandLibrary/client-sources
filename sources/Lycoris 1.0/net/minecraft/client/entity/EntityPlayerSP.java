...
import digital.bq.event.*;
import digital.bq.module.ModuleManager;
import digital.bq.utility.PlayerUtils;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdateWalkingPlayer() {
		PreUpdateEvent eventPre = new PreUpdateEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ,
				this.rotationYaw, this.rotationPitch, this.onGround);
		EventManager.call(eventPre);

		if (eventPre.isCancelled()) {
			PostUpdateEvent eventPost = new PostUpdateEvent(eventPre.getYaw(), eventPre.getPitch());
		...
		if (this.isCurrentViewEntity()) {
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(eventPre.getX(), eventPre.getY(),
							eventPre.getZ(), eventPre.getYaw(), eventPre.getPitch(), eventPre.isOnGround()));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(eventPre.getX(), eventPre.getY(),
							eventPre.getZ(), eventPre.isOnGround()));
				} else if (flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(eventPre.getYaw(),
							eventPre.getPitch(), eventPre.isOnGround()));
				} else {
				...
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
						this.motionZ, eventPre.getYaw(), eventPre.getPitch(), eventPre.isOnGround()));
				flag2 = false;
			}
	...
	public void sendChatMessage(String message) {
		...
		if (ModuleManager.chatBypassMod.isEnabled() && !message.startsWith("/")) {
			StringBuilder msg = new StringBuilder();
			for (char character : event.message.toCharArray()) {
				msg.append(character + "\u061d");
			}
			event.message = msg.toString().replaceFirst("%", "");
		}
	...
	public void onLivingUpdate() {
		if (this.sprintingTicksLeft > 0) {
			...
			if (this.sprintingTicksLeft == 0) {
				if (!ModuleManager.sprintMod.isEnabled()) {
		...
		if (this.inPortal) {
			...
			if (this.mc.currentScreen != null && !this.mc.currentScreen.doesGuiPauseGame()
					&& !ModuleManager.portalmenuMod.isEnabled()) {
				this.mc.displayGuiScreen((GuiScreen) null);
			}
		...
		// Client: NoSlow (Blocking)
		if (this.isUsingItem() && !this.isRiding() && (!ModuleManager.noSlowMod.isEnabled())) {
		...
		// Sprint
		if (this.isSprinting() && !ModuleManager.sprintMod.isEnabled() && (this.movementInput.moveForward < f || this.isCollidedHorizontally || !flag3)) {
			this.setSprinting(false);
		}

		//TODO Client (Event)
		EventManager.call(new PlayerAbilitiesEvent(EventType.PRE));
		...
		//TODO Client (Event)
		EventManager.call(new PlayerAbilitiesEvent(EventType.POST));
