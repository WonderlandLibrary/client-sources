...
import digital.bq.event.BBSetEvent;
import digital.bq.module.ModuleManager;
import digital.bq.module.implement.Render.XRay;
...
public class Block {
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        //Client
        if (ModuleManager.xRayMod.isEnabled() && !XRay.caveFinder.getValueState()) {
            if (ModuleManager.blockEspMod.isEnabled() && ModuleManager.blockEspMod.closeFinder.result != null && ModuleManager.blockEspMod.farFinder.result != null) {
                if (ModuleManager.blockEspMod.closeFinder.result.contains(pos) || ModuleManager.blockEspMod.farFinder.result.contains(pos)) {
                    return XRay.isOreBlock(this);
                }
            } else {
                return XRay.isOreBlock(this);
            }
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask,
        ...
        if (collidingEntity == Minecraft.getMinecraft().thePlayer) {
            BBSetEvent event = new BBSetEvent(this, pos, this.getCollisionBoundingBox(worldIn, pos, state));
    ...
    public EnumWorldBlockLayer getBlockLayer() {
        ...
        if(ModuleManager.xRayMod.isEnabled()) {
            return XRay.isOreBlock(this) ? EnumWorldBlockLayer.SOLID : EnumWorldBlockLayer.TRANSLUCENT;
        }
    ...
    public static boolean isBeaconBase(World worldObj, BlockPos blockPos) {
        int id = Block.getIdFromBlock(worldObj.getBlockState(blockPos).getBlock());
        return id == 41 || id == 42 || id == 57 || id == 133;
    }
}
