...
import digital.bq.event.JumpEvent;
import digital.bq.event.LivingUpdateEvent;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public boolean isOnGround(double height) {
        if (!Minecraft.getMinecraft().theWorld.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, -height, 0.0D)).isEmpty()) {
            return true;
        } else {
            return false;
        }
	...
	public void readEntityFromNBT(NBTTagCompound tagCompund) {
		...
		if (tagCompund.hasKey("Attributes", 9) && this.worldObj != null && !this.worldObj.isRemote) {
			SharedMonsterAttributes.setAttributeModifiers(this.getAttributeMap(),
					tagCompund.getTagList("Attributes", 10));
		}
	...
	public final float getHealth() {
		float health = this.dataWatcher.getWatchableObjectFloat(6);
		if (hurtTime == maxHurtTime || lastHealth == -1) lastHealth = health;
		return health;
	}
	...
	private int getArmSwingAnimationEnd() {
		final boolean animation = Lycoris.INSTANCE.getModuleManager().getModuleByClass(BlockAnimation.class).isEnabled();
		final float slowValue = BlockAnimation.swingSpeed.getValue();
	...
	public boolean canBlockBeSeen(BlockPos blockPos) {
		return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ),
				new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ())) == null;
	}
	...
	public Vec3 getLook(float partialTicks) {
		if (Lycoris.mouseDelay.getValueState() && this instanceof EntityPlayerSP) {
