...
import digital.bq.event.MoveEvent;
import digital.bq.event.StepEvent;
import digital.bq.module.ModuleManager;
import digital.bq.module.implement.Combat.HitBox;
...
import digital.bq.module.implement.Movement.Speed;
...
public abstract class Entity implements ICommandSender {
	...
	public void moveEntity(double x, double y, double z) {
		...
		if (this == Minecraft.getMinecraft().thePlayer) {
			event = new MoveEvent(null, x, y, z);
		...
		if (this.noClip) {
			...
			// Client Step
			StepEvent stepEvent = new StepEvent(true, stepHeight);
			if (this == Minecraft.getMinecraft().thePlayer)
				EventManager.call(stepEvent);

			double d11 = x;
			double d7 = y;
			double d8 = z;
			if ((x * x + z * z >= x * x + z * z) && stepEvent.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z)) {
				...
				if (d11 * d11 + d8 * d8 >= x * x + z * z) {
					...
					EventManager.call(new StepEvent(false, stepHeight, 1 + y));
				}
			}
	...
	public boolean isOverOfMaterial(Material materialOver) {
		if(isInsideOfMaterial(materialOver)) return false;
		double d0 = this.posY;
		BlockPos blockpos = new BlockPos(this.posX, d0, this.posZ);
		IBlockState iblockstate = this.worldObj.getBlockState(blockpos);
		Block block = iblockstate.getBlock();
		
		if (block.getMaterial() == materialOver) {
			float f = BlockLiquid.getLiquidHeightPercent(iblockstate.getBlock().getMetaFromState(iblockstate))
					- 0.11111111F;
			float f1 = (float) (blockpos.getY() + 1) - f;
			boolean flag = d0 < (double) f1;
    ...
    public float getCollisionBorderSize() {
        if (this instanceof EntityPlayer && ModuleManager.hitBoxMod.isEnabled()) {
