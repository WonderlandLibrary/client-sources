...
import digital.bq.utility.viaversion.handler.CommonTransformer;
import digital.bq.utility.viaversion.handler.VRDecodeHandler;
import digital.bq.utility.viaversion.handler.VREncodeHandler;
...
import digital.bq.event.PacketReceiveEvent;
import digital.bq.event.PacketSendEvent;
	...
	public void sendPacketNoEvent(Packet packet) {
		if (channel != null && channel.isOpen()) {
			...
			outboundPacketsQueue
					.add(new NetworkManager.InboundHandlerTuplePacketListener(packet, (GenericFutureListener[]) null));
		}
	}
		...
		((Bootstrap) ((Bootstrap) ((Bootstrap) (new Bootstrap()).group((EventLoopGroup) lazyloadbase.getValue()))
				.handler(new ChannelInitializer<Channel>() {
					protected void initChannel(Channel p_initChannel_1_) throws Exception {
						...
						if (GuiProxy.address != null) {
							p_initChannel_1_.pipeline().addFirst(new Socks5ProxyHandler(GuiProxy.address, null, null));
						}
	...
	public void setCompressionTreshold(int treshold) {
		if (treshold >= 0) {
			if (this.channel.pipeline().get("decompress") instanceof NettyCompressionDecoder) {
				...
				this.channel.pipeline().addBefore(CommonTransformer.HANDLER_DECODER_NAME, "decompress", new NettyCompressionDecoder(treshold));
			}

			if (this.channel.pipeline().get("compress") instanceof NettyCompressionEncoder) {
				((NettyCompressionEncoder) this.channel.pipeline().get("decompress")).setCompressionTreshold(treshold);
			} else {
				this.channel.pipeline().addBefore(CommonTransformer.HANDLER_ENCODER_NAME, "compress", new NettyCompressionEncoder(treshold));
			}
		} else {
			if (this.channel.pipeline().get("decompress") instanceof NettyCompressionDecoder) {
