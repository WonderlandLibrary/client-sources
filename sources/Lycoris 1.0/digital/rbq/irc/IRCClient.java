...
import digital.rbq.event.ChatSendEvent;
import digital.rbq.event.RespawnEvent;
import digital.rbq.gui.loginui.PopupDialog;
import digital.rbq.gui.loginui.GuiLogin;
import digital.rbq.gui.hud.notification.Notification;
import digital.rbq.gui.hud.notification.NotificationManager;
import digital.rbq.irc.client.event.IRCEventManagement;
import digital.rbq.irc.client.event.IRCListener;
import digital.rbq.irc.client.network.ClientNetwork;
import digital.rbq.irc.protocol.packet.LoggedPacket;
import digital.rbq.module.ModuleManager;
import digital.rbq.utility.Base58;
...
public class IRCClient implements IRCListener {
    ...
    public void init() {
        ...
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(Objects.requireNonNull(this.getClass().getClassLoader().getResourceAsStream("LycorisAntiLeak"))));
            String line = null;
            ...
            this.clientNetwork.setValue(isChina ? "china.hvh.asia" : "global.hvh.asia", 5780, username, password, text, Lycoris.VERSION, String.valueOf(allQQ));
        ...
        if (loggedPacket.getMessage().equals("")) {
            ...
            try {
                ...
                if (hasOffline) {
                    hasOffline = false;
                    if (!hasClientInit) {
                        ChatUtils.sendOutputMessage("[Lycoris] \2477Connected and Verified Successfully!");
                        hasClientInit = true;
                        Lycoris.INSTANCE.start();

                        String version = "\247" + loggedPacket.getVersion();

                        Lycoris.status = version;
                        Lycoris.DEBUG_MODE = version.contains("Development");

                        if (Lycoris.DEBUG_MODE) {
                            ModuleManager.registerModule(ModuleManager.debugMod);
                        }

                        Minecraft.getMinecraft().displayGuiScreen(null);
                    } else {
                        ChatUtils.sendOutputMessage("[Lycoris] \2477Reconnected Successfully!");
                        if (Minecraft.getMinecraft().currentScreen instanceof GuiLogin)
                            Minecraft.getMinecraft().displayGuiScreen(null);
                    }
                }
    ...
    public void onCommandEvent(String s) {
        ...
        if (args[0].equalsIgnoreCase("sudo")) {
            ...
            Minecraft.getMinecraft().getNetHandler().getNetworkManager().closeChannel(new ChatComponentText("\247cDisconnected by Lycoris Staff\n\n\247r" + s.substring(11)));
        }
    }
