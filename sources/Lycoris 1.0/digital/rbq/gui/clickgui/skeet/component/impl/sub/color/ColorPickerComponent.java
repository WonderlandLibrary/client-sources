package digital.rbq.gui.clickgui.skeet.component.impl.sub.color;
...
import digital.rbq.gui.clickgui.skeet.component.ButtonComponent;
import digital.rbq.gui.clickgui.skeet.component.Component;
import digital.rbq.gui.clickgui.skeet.component.ExpandableComponent;
import digital.rbq.gui.clickgui.skeet.component.PredicateComponent;
...
public abstract class ColorPickerComponent extends ButtonComponent implements PredicateComponent, ExpandableComponent {
    ...
    private static void drawCheckeredBackground(float x, float y, float x2, float y2) {
        Gui.drawRect(x, y, x2, y2, SkeetClickGUI.getColor(16777215));
        for(boolean offset = false; y < y2; ++y) {
            for(float x1 = x + (float)((offset = !offset) ? 1 : 0); x1 < x2; x1 += 2.0F) {
                if (x1 <= x2 - 1.0F) {
                    Gui.drawRect(x1, y, x1 + 1.0F, y + 1.0F, SkeetClickGUI.getColor(8421504));
                }
            }
        }
    }
    ...
    public void drawComponent(LockedResolution resolution, int mouseX, int mouseY) {
        ...
        if (this.isExpanded()) {
            ...
            Gui.drawRect(expandedX + 0.5f, expandedY + 0.5f, (expandedX + expandedWidth) - 0.5f, (expandedY + expandedHeight) - 0.5f, SkeetClickGUI.getColor(0x39393B));
            Gui.drawRect(expandedX + 1.0f, expandedY + 1.0f, expandedX + expandedWidth - 1.0f, expandedY + expandedHeight - 1.0f, SkeetClickGUI.getColor(0x232323));
            ...
            Gui.drawRect(colorPickerLeft - 0.5f, colorPickerTop - 0.5f, colorPickerRight + 0.5f, colorPickerBottom + 0.5f, SkeetClickGUI.getColor(0));
