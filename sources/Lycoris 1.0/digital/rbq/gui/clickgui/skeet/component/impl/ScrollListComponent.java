package digital.rbq.gui.clickgui.skeet.component.impl;
...
import digital.rbq.gui.clickgui.skeet.MaskUtils;
import digital.rbq.gui.clickgui.skeet.component.Component;
import digital.rbq.gui.clickgui.skeet.component.ScrollableListComponent;
...
public class ScrollListComponent extends ScrollableListComponent {
    ...
    public void drawComponent(LockedResolution resolution, int mouseX, int mouseY) {
        ...
        if(SkeetClickGUI.needUpdateValue) {
            this.elements = this.allElements.get();
            this.scrollAni = 0;
            SkeetClickGUI.needUpdateValue = false;
        } else if (SkeetClickGUI.alpha > 20) {
            for(String element : this.elements) {
                ...
                FontUtils fr = isHovered || selected.equals(element) ? SkeetClickGUI.GROUP_BOX_HEADER_RENDERER : SkeetClickGUI.FONT_RENDERER;

                if(isHovered) {
                    RenderUtil.drawRect(this.getX() + .5f, startY, this.getX() + this.getWidth() - .5f, startY + 10f,0xff1a1a1a);
                }

                fr.drawLimitedString(element, this.getX() + 4, startY + 1.5f, color, 300);
                startY += 10f;
                yShouldbe += 10f;
            }
