...
public class Gradient implements Theme {
    ...
    static float getYIndex(float newWidth, float newHeight, float yIndex, String renderName) {
        ...
        int color = ColorUtils.fadeBetween(Hud.arraylistColor1.getColorInt(), Hud.arraylistColor2.getColorInt(), (int) yIndex * (Hud.fadeDirection.isCurrentMode("Up") ? 7  : -7));

        if (Hud.colorbar.getValue())
            WorldRenderUtils.drawRects2(WorldRenderUtils.getScaledResolution().getScaledWidth() - 1.5, yIndex + (10 - Hud.offset.getValue()) , WorldRenderUtils.getScaledResolution().getScaledWidth(), yIndex + 10, color);
    ...
    public void render(float newWidth, float newHeight) {
        ...
        for (Module module : Lycoris.INSTANCE.getModuleManager().getModulesRender(Hud.isMinecraftFont ? mc.fontRendererObj : FontManager.normal2)) {
            ...
            yIndex = Gradient.getYIndex(newWidth, newHeight, yIndex, renderName);
        }
        GL11.glPopMatrix();
    }
    ...
    public void renderWatermark() {
        int color = ColorUtils.fadeBetween(Hud.arraylistColor1.getColorInt(), Hud.arraylistColor2.getColorInt());
