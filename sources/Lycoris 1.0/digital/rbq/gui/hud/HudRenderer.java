...
import digital.rbq.gui.clickgui.classic.BlurBuffer;
...
import digital.rbq.gui.hud.Themes.*;
import digital.rbq.gui.hud.notification.NotificationManager;
import digital.rbq.gui.hud.window.HudWindow;
import digital.rbq.irc.IRCClient;
import digital.rbq.irc.ReconnectThread;
...
import digital.rbq.module.implement.World.AutoGG;
import digital.rbq.module.implement.World.Scaffold;
import digital.rbq.utility.*;
...
public class HudRenderer extends GuiIngame {
    ...
    public void renderGameOverlay(float particalTicks) {
        ...
        int color = 0;
        Lycoris.rainbow = java.awt.Color.getHSBColor(index, Hud.arraylistColor1.getColorHSB()[1], Hud.arraylistColor1.getColorHSB()[2]);

        color = Hud.positionColour.getColorInt();
        if (Hud.Theme.isCurrentMode("Rainbow") || Hud.Theme.isCurrentMode("Rainbow2"))
            color = Lycoris.rainbow.getRGB();
        ...
        if (IRCClient.hasOffline) {
            notice = String.format((IRCClient.isChina ? "Lycoris与服务器断开连接，已尝试重新连接%d次！" : "Lycoris has disconnected from the IRC server! Reconnected %d times."), ReconnectThread.times);
        }
        ...
        if (Hud.waterMarkMode.isCurrentMode("Lycoris Sense")) {
            ...
            String ping;
            ping = ServerUtils.INSTANCE.isOnHypixel() ? String.valueOf(Lycoris.INSTANCE.curPing) : info != null ? info.getResponseTime() + "" : "0";

            String str = "flux";
            String str2 = "sense";
            String str3 = " | " + IRCClient.loggedPacket.getRealUsername() + " | " + (mc.isSingleplayer() ? "localhost:25565" : ServerUtils.INSTANCE.isOnHypixel() ? "mc.hypixel.net:25565" : !mc.getCurrentServerData().serverIP.contains(":") ? mc.getCurrentServerData().serverIP + ":25565" : mc.getCurrentServerData().serverIP) + " | " + Minecraft.getDebugFPS() + "fps | " + ping + "ms | " + (Math.round(Lycoris.INSTANCE.lastTPS * 10) / 10d) + "ticks";
            ...
            FontManager.vision30.drawString(Lycoris.NAME.toUpperCase(), 20f, 2f, 0xffffffff);
        }
        ...
        if (!mc.isSingleplayer()) {
            if (Lycoris.laggTimer.getElapsedTime() > 1000) {
        ...
        Lycoris.INSTANCE.getHudWindowMgr().
