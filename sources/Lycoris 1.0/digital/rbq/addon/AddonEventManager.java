...
import digital.rbq.addon.api.event.events.Event2DRender;
import digital.rbq.addon.api.event.events.Event3DRender;
...
public class AddonEventManager {
    ...
    private void onPre(PreUpdateEvent e) {
        ...
        for (AddonModule module : LycorisAPI.FLUX_API.getModuleManager().getAddonModules()) {
            try {
                if (module.getStage()) module.onPreUpdate(apiEvent);
            } catch (Throwable exception) {
                exception.printStackTrace();
                ChatUtils.sendErrorToPlayer(String.format("[Lycoris API] [%s] Error: %s", module.getName(), exception.getMessage()));
            }
        }
