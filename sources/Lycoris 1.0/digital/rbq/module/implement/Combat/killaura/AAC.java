   ...
   /* private float getRotationToEntityYaw(final Entity entity) {
    ...
    public boolean isValid(final EntityLivingBase entity, final boolean attack) {
     ...
     //       return false;
        ...
        //friend
        if (entity instanceof EntityPlayer && Lycoris.INSTANCE.getFriendManager().isFriend(entity.getName()))
            return false;

        //team
        if (Lycoris.teams.getValue() && entity instanceof EntityPlayer && FriendManager.isTeam((EntityPlayer) entity))
            return false;
