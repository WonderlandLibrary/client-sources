...
public class GuiIngame extends Gui
{
    ...
    {
        ScaledResolution scaledresolution = ScaledResolution.fetchResolution(this.mc);
        ...
        if(hud.getBooleanSetting("Pri Pri").isEnabled()){
            for(Entity entity : hud.entityPosition.keySet()){
                Vector4f pos = hud.entityPosition.get(entity);
                float x = pos.getX(), y = pos.getY(), right = pos.getZ(), bottom = pos.getW();
//                StencilUtil.initStencilToWrite();
//                Gui.drawRect2(x, y, right, bottom, Color.black.getRGB());
//                StencilUtil.readStencilBuffer(1);
//                RenderUtil.drawImage(new ResourceLocation("straight/images/randomchild_nobg.png"), x, y - 30, right - x, bottom - y);
//                StencilUtil.uninitStencilBuffer();
                RenderUtil.drawImage(new ResourceLocation("straight/images/killemenew.png"), x, y, right - x, bottom - y);
            }
        }

//        PostProcessing postProcessing = Client.INSTANCE.getModuleManager().getModule(PostProcessing.class);
//        postProcessing.funny();
//
//        Client.INSTANCE.getPubSubEventBus().publish(new Render2DEvent(partialTicks, scaledresolution));
    ...
    {
        Hud hud = Client.INSTANCE.getModuleManager().getModule(Hud.class);
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer && !hud.components.isEnabled("Hotbar"))
        {
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    {
        Hud hud = Client.INSTANCE.getModuleManager().getModule(Hud.class);
        if(hud.components.isEnabled("hotbar"))
            return;
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            if(streamerMode.isEnabled() && streamerMode.getBooleanSetting("Scramble GameID").isEnabled())
                s1 = s1.replace(EnumChatFormatting.DARK_GRAY.toString(), EnumChatFormatting.OBFUSCATED.toString());
    ...
    private void renderPlayerStats(ScaledResolution scaledRes)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            int k1 = scaledRes.getScaledHeight() - (hud.components.isEnabled("hotbar") ? 38 : 39);
