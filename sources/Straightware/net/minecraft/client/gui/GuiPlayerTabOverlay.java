...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public String getUsername(String input) {
        for(String key : Client.INSTANCE.getPlayerList().keySet()) {
//            System.out.println(key);
            if(input.contains(key)){
                input = input.replace(key, String.format("%s %s(%s%s%s)%s", key, EnumChatFormatting.GRAY, EnumChatFormatting.DARK_PURPLE,
                        Client.INSTANCE.getPlayerList().get(key), EnumChatFormatting.GRAY, EnumChatFormatting.RESET));
            }
        }
        return input;
    }
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn)
    {
        ...
        StreamerMode streamerMode = Client.INSTANCE.getModuleManager().getModule(StreamerMode.class);
        if(streamerMode.isEnabled()) {
            if(networkPlayerInfoIn.getDisplayName() != null) {
                return networkPlayerInfoIn.getDisplayName().getFormattedText().replace(mc.session.getUsername(), streamerMode.getReplacedName() + " " + getUsername(name));
            }else {
                name = networkPlayerInfoIn.getGameProfile().getName().replace(mc.session.getUsername(), streamerMode.getReplacedName() + " " + getUsername(name));
                return ScorePlayerTeam.formatPlayerName(networkPlayerInfoIn.getPlayerTeam(), name);
            }
        }

        if(networkPlayerInfoIn.getDisplayName() != null){
            name = networkPlayerInfoIn.getDisplayName().getFormattedText();

            if(FriendCommand.isFriend(networkPlayerInfoIn.getGameProfile().getName())){
                return name.replace(networkPlayerInfoIn.getGameProfile().getName(), EnumChatFormatting.DARK_AQUA + networkPlayerInfoIn.getGameProfile().getName());
            }

            return getUsername(name);
        }else{
            name = networkPlayerInfoIn.getGameProfile().getName();

            if(FriendCommand.isFriend(name)){
                return name.replace(networkPlayerInfoIn.getGameProfile().getName(), EnumChatFormatting.DARK_AQUA + networkPlayerInfoIn.getGameProfile().getName());
            }

            return ScorePlayerTeam.formatPlayerName(networkPlayerInfoIn.getPlayerTeam(), getUsername(networkPlayerInfoIn.getGameProfile().getName()));
        }
    }
