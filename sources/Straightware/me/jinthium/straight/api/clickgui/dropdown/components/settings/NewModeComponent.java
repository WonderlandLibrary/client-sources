...
public class NewModeComponent extends SettingComponent<NewModeSetting> {
    ...
    public NewModeComponent(NewModeSetting modeSetting) {
        super(modeSetting);
        normalCount = 2;
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        NewModeSetting modeSetting = getSetting();
        ...
        normalFont16.drawString(modeSetting.getCurrentMode().getName(), boxX + boxWidth / 2 - normalFont16.getStringWidth(modeSetting.getCurrentMode().getName()) / 2, boxY + normalFont16.getMiddleOfBox(boxHeight) + 2, textColor);
        ...
        if (opened || !openAnimation.isDone()) {
            ...
            float modeHeight = (modeSetting.getValues().size() - 1) * rectHeight;
            ...
            for (ModuleMode<?> mode : modeSetting.getValues()) {
                ...
                if(hoveringMode) {
                    this.hoveringMode = mode.getName();
                }


                if(mode.getName().equals(this.hoveringMode)) {
                    selectionBoxY.animate(rectCount * rectHeight, 17);
                }

                RenderUtil.resetColor();
                normalFont16.drawString(mode.getName(), modeX + boxWidth / 2 - normalFont16.getStringWidth(mode.getName()) / 2,
