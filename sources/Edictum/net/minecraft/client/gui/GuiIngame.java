...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (scoreobjective1 != null&&Meanings.sc)
        {
...
public void draw_radar_2()
{
    ...
    while (i$.hasNext())
    {
        ...
        {
            int pdisx = (int)Math.round(this.mc.thePlayer.posX);
            int pdisz = (int)Math.round(this.mc.thePlayer.posZ);
            int disx = (int)Math.round(entity.posX);
            int disz = (int)Math.round(entity.posZ);
            int posX = pdisx - disx;
            int posZ = pdisz - disz;
            boolean flag = Math.hypot((double)posX, (double)posZ) < 50.0D;

            if (flag && entity instanceof EntityPlayer)
            {
                GuiIngame var10000 = this.mc.ingameGUI;
                drawRect(posX - 1, posZ - 1, posX + 1, posZ + 1, -18467567);
            }
        }
    }
}
