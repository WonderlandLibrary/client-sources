...
import club.dortware.client.proxy.util.MinecraftProxyHandler;
...
public class NetworkManager extends SimpleChannelInboundHandler
{
    ...
    public static NetworkManager provideLanClient(InetAddress serverAddress, int serverPort)
    {
        ...
        if (!canUseProxies()) {
            //TOR proxy running on the JVM
//            proxy = new Proxy(Proxy.Type.SOCKS, new InetSocketAddress("127.0.0.1", 57441));
        ...
        bootstrap.group(new OioEventLoopGroup()).handler(new ChannelInitializer<OioSocketChannel>() {
        ...
        }).channelFactory(new MinecraftProxyHandler(proxy)).connect(serverAddress, serverPort).syncUninterruptibly();
