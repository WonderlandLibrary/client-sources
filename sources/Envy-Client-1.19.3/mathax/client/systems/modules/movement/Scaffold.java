...
//We need to add 180 Rotations Mode
public class Scaffold extends Module {
    ...
    private final Setting<towerMode> towerModeSetting = sgGeneral.add(new EnumSetting.Builder<towerMode>()
        .name("Tower Mode")
        .description("What Tower method should be used")
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (towerModeSetting.get() == towerMode.Vanilla) {
            if (mc.options.jumpKey.isPressed() && !mc.options.sneakKey.isPressed()) mc.player.setVelocity(0, 0.42f, 0);
        }
        if (towerModeSetting.get() == towerMode.MMC) {
            //untested
            double distance = PlayerUtils.distanceTo(bp.down().down());
            if (mc.options.jumpKey.isPressed() && distance == 1.4) mc.player.setVelocity(0, 0.42f, 0);
        }
        if (towerModeSetting.get() == towerMode.TenacityNCP) {
            //untested
            if (mc.options.jumpKey.isPressed()) {
                if (!PlayerUtils.isMoving() || MoveUtilV.getSpeed() < 0.16) {
                    if (mc.player.isOnGround()) {
                        mc.player.setVelocity(mc.player.getX(), 0.42f, mc.player.getZ());
        ...
        if (towerModeSetting.get() == towerMode.NONE) {
            //do nothing
        }
    ...
    public enum towerMode {
        Vanilla("Vanilla"),
        MMC("MMC"),
        TenacityNCP("Tenacity NCP"),
        NONE("None");

        private final String title;

        towerMode(String title) {
            this.title = title;
        }
    }
}
