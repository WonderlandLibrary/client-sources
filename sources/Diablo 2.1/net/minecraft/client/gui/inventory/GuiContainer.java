...
import wtf.diablo.module.impl.Render.Gui;
...
public abstract class GuiContainer extends GuiScreen
{
    ...
    {
        /*
        if (inventorySlots.getInventory().nam.getDisplayName().getUnformattedText().contains("Chest") || container.getLowerChestInventory().getDisplayName().getUnformattedText().contains("Storage")) {
        ...
        double percentage = AnimationUtil.getPercentage(animationDuration, openTime) / 100;

        switch (Gui.guiContainerAnimation.getMode()){
            case "Scale":
                if (percentage < 1) {
                    GlStateManager.translate(width / 2, height / 2, 0);
                    GlStateManager.scale(percentage, percentage, 0);
                    GlStateManager.translate(-(width / 2), -(height / 2), 0);
                }
            break;
            case "Slide":
                ...
                if(percentage < 1)
                    GlStateManager.translate(0,(guiTop + 170)+ (-AnimationUtil.getDoubleFromPercentage(percentage * 100,(this.guiTop + 170))),0);
                break;
        }
        ...
        /*
        if(Diablo.getInstance().getModuleManager().getModuleByName("Gui").isToggled())
        if(Gui.guiContainerAnimation.getMode().equalsIgnoreCase("Scale")) {
            double percentage = AnimationUtil.getPercentage(animationDuration, openTime) / 100;
            if (percentage < 1) {
                GlStateManager.translate(-(width / 2), -(height / 2), 0);
                GlStateManager.scale(-percentage, -percentage, 0);
