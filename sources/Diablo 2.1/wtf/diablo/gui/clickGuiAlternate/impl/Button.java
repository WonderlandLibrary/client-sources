...
import wtf.diablo.gui.clickGuiAlternate.impl.settings.*;
...
public class Button {
    ...
    public double drawScreen(int mouseX, int mouseY, float partialTicks, double offset) {
        ...
        int bgColor = new Color(0x832F2F2F, true).getRGB();

        if (RenderUtil.isHovered(mouseX, mouseY, parent.x, y, parent.x + parent.width, y + height))
            bgColor = new Color(0x83252525, true).getRGB();
        if (mod.isToggled())
            bgColor = new Color(0x83212121, true).getRGB();

        Gui.drawRect(parent.x, y, parent.x + parent.width, y + height, bgColor);
        Fonts.SFReg18.drawStringWithShadow(mod.getName(), parent.x + 3, y + ((height - Fonts.SFReg18.getHeight()) / 2f), mod.isToggled() ? -1 : 0x80FFFFFF);
