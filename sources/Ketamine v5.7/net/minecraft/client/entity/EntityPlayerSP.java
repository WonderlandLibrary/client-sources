...
import io.github.nevalackin.client.event.player.UseSlowDownEvent;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public UpdatePositionEvent lastEvent;
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        KetamineClient.getInstance().getEventBus().post(moveEvent);
    ...
    public float getLastTickYaw() {
        return this.lastEvent != null ? this.lastEvent.getLastTickYaw() : this.prevRotationYaw;
    }
    ...
    public float getLastTickPitch() {
        return this.lastEvent != null ? this.lastEvent.getLastTickPitch() : this.prevRotationPitch;
    }
    ...
    public void onUpdatePlayer() {
        ...
        if (!riding) {
            // FIND ME :: SendSprintStateEvent

            final SendSprintStateEvent sprintEvent = new SendSprintStateEvent(isSprinting());
            KetamineClient.getInstance().getEventBus().post(sprintEvent);
            final boolean clientSprintState = sprintEvent.isSprintState();
        ...
        if (this.isCurrentViewEntity()) {
            // FIND ME :: UpdatePositionEvent

            final UpdatePositionEvent updatePositionEvent = new UpdatePositionEvent(
                this.posX, this.getEntityBoundingBox().minY, this.posZ,
                this.lastReportedPosX, this.lastReportedPosY, this.lastReportedPosZ,
                this.rotationYaw, this.rotationPitch,
                this.lastReportedYaw, this.lastReportedPitch,
                this.onGround,
                this.lastEvent != null ? this.lastEvent.isOnGround() : this.mc.thePlayer.onGround);
            KetamineClient.getInstance().getEventBus().post(updatePositionEvent);
            this.lastEvent = updatePositionEvent;
            ...
            final boolean updateXYZ = xDif * xDif + yDif * yDif + zDif * zDif > 9.0E-4D || this.positionUpdateTicks >= 20;
            final boolean updateYawPitch = yawDif != 0.0D || pitchDif != 0.0D;
    ...
    public void onLivingUpdate() {
        ...
        // FIND ME :: UseSlowDownEvent

        final UseSlowDownEvent event = new UseSlowDownEvent();
