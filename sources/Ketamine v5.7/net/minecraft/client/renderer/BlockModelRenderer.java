...
import io.github.nevalackin.client.event.render.world.BlockSideRenderEvent;
import io.github.nevalackin.client.event.render.world.GetBlockLightLevelEvent;
...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        ...
        if (Minecraft.isAmbientOcclusionEnabled() && modelIn.isAmbientOcclusion()) {
            final GetBlockLightLevelEvent event = new GetBlockLightLevelEvent(blockStateIn.getBlock().getLightValue());
            KetamineClient.getInstance().getEventBus().post(event);
            flag = event.getLightLevel() == 0;
        } else {
            flag = false;
        }
    ...
    public boolean renderModelStandard(IBlockAccess blockAccessIn, IBakedModel modelIn, Block blockIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        ...
        for (EnumFacing enumfacing : EnumFacing.values()) {
            ...
            if (!list.isEmpty()) {
                ...
                {
                    if (checkSides) {
                        // FIND ME :: BlockRenderEvent
                        final BlockSideRenderEvent event = new BlockSideRenderEvent(blockIn, blockIn::shouldSideBeRendered);
                        KetamineClient.getInstance().getEventBus().post(event);
                        final BlockSideRenderEvent.Callback sideCheckCallback = event.getCallback();

                        if (!sideCheckCallback.shouldRenderSide(blockAccessIn, blockpos, enumfacing))
                            break drawBlockSide;
                    }

                    int i = blockIn.getMixedBrightnessForBlock(blockAccessIn, blockpos);

                    this.renderModelStandardQuads(blockAccessIn, blockIn, blockPosIn,
                                                  enumfacing, i, false,
