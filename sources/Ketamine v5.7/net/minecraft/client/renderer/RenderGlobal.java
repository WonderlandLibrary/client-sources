...
import io.github.nevalackin.client.event.render.world.DrawSelectedBoundingBoxEvent;
                ...
                if (!classinheritancemultimap.isEmpty()) {
                    ...
                    while (true) {
                        ...
                        while (true) {
                            ...
                            if ((entity2 != this.mc.getRenderViewEntity() || this.mc.gameSettings.getThirdPersonView() != 0 || flag3) && (entity2.posY < 0.0D || entity2.posY >= 256.0D || this.theWorld.isBlockLoaded(new BlockPos(entity2)))) {
                                ++this.countEntitiesRendered;
    ...
    public void drawSelectionBox(EntityPlayer player, MovingObjectPosition movingObjectPositionIn, int p_72731_3_, float partialTicks) {
        if (p_72731_3_ == 0 && movingObjectPositionIn.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            if (block.getMaterial() != Material.air && this.theWorld.getWorldBorder().contains(blockpos)) {
                ...
                final DrawSelectedBoundingBoxEvent event = new DrawSelectedBoundingBoxEvent();
                KetamineClient.getInstance().getEventBus().post(event);

                DrawUtil.glColour(event.getColour());
                DrawUtil.glDrawBoundingBox(block.getSelectedBoundingBox(this.theWorld, blockpos)
                                    .expand(0.002f, 0.002f, 0.002f).offset(-d0, -d1, -d2), event.getOutlineWidth(), event.isFilled());
            }
