...
import io.github.nevalackin.client.event.player.PlayerHealthUpdateEvent;
import io.github.nevalackin.client.event.player.PlayerPositionUpdateEvent;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn) {
        ...
        final List<DataWatcher.WatchableObject> watchableObjects = packetIn.func_149376_c();

        if (entity != null && watchableObjects != null) {
            if (entity instanceof EntityPlayer) {
                final EntityPlayer player = (EntityPlayer) entity;
                for (DataWatcher.WatchableObject object : watchableObjects) {
                    if (object.getDataValueId() == 6 // Health object ID = 6
                        && object.getObjectType() == 3) { // Float type ID = 3
                        final float health = (float) object.getObject();
                        float dif = player.getHealth() - health;
                        if (dif >= 0.0f && health != 10.0f) {
                            KetamineClient.getInstance().getEventBus().post(new PlayerHealthUpdateEvent(player, health, player.getHealth()));
                        }
                        break;
                    }
                }
            ...
            entity.getDataWatcher().updateWatchedObjectsFromList(watchableObjects);
        }
    }
    ...
    public void handleEntityMovement(S14PacketEntity packetIn) {
        ...
        if (entity != null) {
            final byte x = packetIn.func_149062_c();
            final byte y = packetIn.func_149061_d();
            final byte z = packetIn.func_149064_e();
            entity.serverPosX += x;
            entity.serverPosY += y;
            entity.serverPosZ += z;
            double d0 = (double) entity.serverPosX / 32.0D;
            ...
            // FIND ME :: PlayerPositionEvent
            if (entity instanceof EntityOtherPlayerMP) {
                final PlayerPositionUpdateEvent event = new PlayerPositionUpdateEvent((EntityPlayer) entity, new Vec3(x / 32.0, y / 32.0, z / 32.0));
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        // FIND ME :: ServerSetPosLookEvent

        final ServerSetPosLookEvent event = new ServerSetPosLookEvent(packetIn.getX(), packetIn.getY(), packetIn.getZ(),
        ...
        double d0 = event.getPosX();
        double d1 = event.getPosY();
        double d2 = event.getPosZ();
        ...
        this.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, event.getSendYaw(), event.getSendPitch(), false));
    ...
    public void onDisconnect(IChatComponent reason) {
        ...
        KetamineClient.getInstance().getEventBus().post(new WatchdogBannedEvent(reason));
    ...
    public void sendPacketDirect(Packet<?> p_147297_1_) {
        this.netManager.sendPacket(p_147297_1_);
    }

    public void sendPacket(Packet<?> p_147297_1_) {
        // FIND ME :: SendPacketEvent

        final SendPacketEvent event = new SendPacketEvent(p_147297_1_);
        KetamineClient.getInstance().getEventBus().post(event);
        if (event.isCancelled()) return;
        this.sendPacketDirect(event.getPacket());
    }
    ...
    public void handleChat(S02PacketChat packetIn) {
        ...
        // FIND ME :: Removed records optimization
...
//        if (packetIn.getActionNumber() < 0)
//            System.out.printf("Received transaction @ %s, ID: %s.\n", System.currentTimeMillis(), packetIn.getActionNumber());
    ...
    public void handleCombatEvent(S42PacketCombatEvent packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        // FIND ME :: Optimized handle combat event
    }
    ...
    public void handleParticles(S2APacketParticles packetIn) {
        ...
        if (packetIn.getParticleCount() <= 1) {
