...
public abstract class EntityLivingBase extends Entity
{
    ...
    // FIND ME :: Fields used for target hud animations
    ...
    private int getArmSwingAnimationEnd() {
        // Get potion effects
        final PotionEffect hasteEffect = this.getActivePotionEffect(Potion.digSpeed);
        final PotionEffect miningFatigueEffect = this.getActivePotionEffect(Potion.digSlowdown);
        // Get base modifier
        final GetArmSwingModEvent event = new GetArmSwingModEvent(1.f);
        KetamineClient.getInstance().getEventBus().post(event);
        final float mod = event.getModifier();
        // Calculate swing animation duration
        int base = (int) Math.ceil(6 * mod);
        if (hasteEffect != null) base -= (1 + hasteEffect.getAmplifier());
        if (miningFatigueEffect != null) base += (1 + miningFatigueEffect.getAmplifier()) * 2;
        return base;
    }
