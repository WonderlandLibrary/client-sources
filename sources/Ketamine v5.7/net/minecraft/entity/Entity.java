...
import io.github.nevalackin.client.event.player.SafeWalkEvent;
import io.github.nevalackin.client.event.player.StepEvent;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            // FIND ME :: SafeWalkEvent logic

            final SafeWalkEvent shouldSafeWalkEvent = new SafeWalkEvent();

            if (this instanceof EntityPlayerSP) {
                KetamineClient.getInstance().getEventBus().post(shouldSafeWalkEvent);
            }

            boolean flag = this.onGround && (this.isSneaking() || shouldSafeWalkEvent.isCancelled()) && this instanceof EntityPlayer;
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                // FIND ME :: StepEvent Pre

                final StepEvent event = new StepEvent(this.stepHeight);
                ...
                // FIND ME :: StepEvent Post

                if (this instanceof EntityPlayerSP) {
                    double blockHeight = event.getStepHeight() + y;
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4F) {
            ...
            if (this instanceof EntityPlayerSP) {
                final MoveFlyingEvent event = new MoveFlyingEvent(yaw);
                KetamineClient.getInstance().getEventBus().post(event);
                yaw = event.getYaw();
            }
