...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            // FIND ME :: ReceivePacketEvent

            final ReceivePacketEvent receivePacketEvent = new ReceivePacketEvent(p_channelRead0_2_);
            KetamineClient.getInstance().getEventBus().post(receivePacketEvent);
            if (receivePacketEvent.isCancelled()) return;
