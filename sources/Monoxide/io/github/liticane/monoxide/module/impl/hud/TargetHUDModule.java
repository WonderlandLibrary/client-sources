...
import io.github.liticane.monoxide.util.render.animation.simple.SimpleAnimation;
...
@ModuleData(name = "TargetHUD", description = "Draws a little box with the targets info", category = ModuleCategory.HUD)
public class TargetHUDModule extends Module implements ColorPalette {

    public final ModeValue targethudMode = new ModeValue("Mode", this, new String[]{"Atani Simple", "Atani Modern", "Atani Modern 2", "Atani Golden", "Augustus 2.6", "Xave", "Ryu", "Fatality", "Icarus", "Atani CS:GO", "Koks", "Astolfo"});
    public final BooleanValue followTargetHUD = new BooleanValue("Follow Target HUD", this, false);
    private final NumberValue<Integer> red = new NumberValue<>("Red", this, 255, 0, 255, 0, new Supplier[]{() -> targethudMode.getValue().equalsIgnoreCase("Atani Modern")});
    private final NumberValue<Integer> green = new NumberValue<>("Green", this, 255, 0, 255, 0, new Supplier[]{() -> targethudMode.getValue().equalsIgnoreCase("Atani Modern")});
    private final NumberValue<Integer> blue = new NumberValue<>("Blue", this, 255, 0, 255, 0, new Supplier[]{() -> targethudMode.getValue().equalsIgnoreCase("Atani Modern")});
    ...
    public void onRender2D(Render2DEvent render2DEvent) {
        if(ModuleManager.getInstance().getModule(KillAuraModule.class).isEnabled() && KillAuraModule.currentTarget != null && KillAuraModule.currentTarget instanceof EntityPlayer) {
            EntityLivingBase target = KillAuraModule.currentTarget;
            ...
            FontRenderer small = FontStorage.getInstance().findFont("SF UI", 17);
            ...
            switch (this.targethudMode.getValue()) {
                ...
                case "Fatality": {
                    ...
                    FontRenderer medium20 = FontStorage.getInstance().findFont("SF UI", 19);
                ...
                case "Ryu": {
                    ...
                    RenderableShaders.render(true, false, () -> {
                        FontRenderer medium21 = FontStorage.getInstance().findFont("SF UI", 21);
                        FontRenderer medium17 = FontStorage.getInstance().findFont("SF UI", 17);
                    ...
                    FontRenderer medium21 = FontStorage.getInstance().findFont("SF UI Medium", 21);
                    FontRenderer medium17 = FontStorage.getInstance().findFont("SF UI Medium", 17);
                    float health = MathUtils.clamp(target.getHealth(), 0, 20);
