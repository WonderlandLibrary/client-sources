...
import io.github.liticane.monoxide.util.render.StencilUtil;
...
public class AtaniServerListEntryNormal implements AtaniGuiListExtended.IGuiListEntry
{
    ...
    private final AtaniMultiPlayerMenu field_148303_c;
    ...
    protected AtaniServerListEntryNormal(AtaniMultiPlayerMenu p_i45048_1_, ServerData p_i45048_2_)
    {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        if (!this.field_148301_e.field_78841_f)
        {
            ...
            field_148302_b.submit(new Runnable()
            {
                public void run()
                {
                    try
                    {
                        AtaniServerListEntryNormal.this.field_148303_c.getOldServerPinger().ping(AtaniServerListEntryNormal.this.field_148301_e);
                    }
                    catch (UnknownHostException var2)
                    {
                        AtaniServerListEntryNormal.this.field_148301_e.pingToServer = -1L;
                        AtaniServerListEntryNormal.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_RED + "Can\'t resolve hostname";
                    }
                    catch (Exception var3)
                    {
                        AtaniServerListEntryNormal.this.field_148301_e.pingToServer = -1L;
                        AtaniServerListEntryNormal.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_RED + "Can\'t connect to server.";
                    }
                }
            });
        }
        ...
        FontStorage.getInstance().findFont("SF UI", 19).drawString(this.field_148301_e.serverName, x + 32 + 3, y + 1, 16777215);
        List<String> list = FontStorage.getInstance().findFont("SF UI", 19).listFormattedStringToWidth(this.field_148301_e.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(list.size(), 2); ++i)
        {
            FontStorage.getInstance().findFont("SF UI", 19).drawString(list.get(i), x + 32 + 3, y + 12 + FontStorage.getInstance().findFont("SF UI", 19).FONT_HEIGHT * i, 8421504);
        }

        String s2 = flag2 ? EnumChatFormatting.DARK_RED + this.field_148301_e.gameVersion : this.field_148301_e.populationInfo;
        int j = FontStorage.getInstance().findFont("SF UI", 19).getStringWidthInt(s2);
        FontStorage.getInstance().findFont("SF UI", 19).drawString(s2, x + listWidth - j - 15 - 2, y + 1, 8421504);
        int k = 0;
        String s = null;
        int l;
        String s1;
