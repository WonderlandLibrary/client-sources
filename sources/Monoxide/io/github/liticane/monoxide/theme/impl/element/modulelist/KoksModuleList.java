...
import io.github.liticane.monoxide.util.java.MapUtil;
...
public class KoksModuleList extends ModuleListElement {
    ...
    public void onDraw(ScaledResolution scaledResolution, float partialTicks, AtomicFloat leftY, AtomicFloat rightY, LinkedHashMap<Module, DecelerateAnimation> moduleHashMap) {
        final FontRenderer roboto18 = FontStorage.getInstance().findFont("SF UI", 18);
            ...
            if (!moduleHashMap.get(module).finished(Direction.BACKWARDS)) {
                ...
                if (i == 0) {
                    ...
                    for (double left = l; left < r; left += distance / displayName.length()) {
                        ...
                        Gui.drawRect((int) left, (int) (y - depth - posY), (int) (nextStep), y - posY, getRainbow(100 * (lIndex + (moduleHashMap.size() - i) + 1), 6000, 0.6f, 1).getRGB());
                        lIndex++;
                    }
                }
                ...
                if (i + 1 < modules.size()) {
                    ...
                    final float difference = roboto18.getStringWidthInt(displayName) - roboto18.getStringWidthInt(nextModule.getName());
                    Gui.drawRect((int) (((float) (scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) - size - depth)), (int) (y + height + 1.5F - posY), (int) ((scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) + difference - size), (int) bottom, getRainbow(100 * ((moduleHashMap.size() - i) + 1), 6000, 0.6f, 1).getRGB());
                } else {
                    int lIndex = 1;
                    for (double left = l; left < r; left += distance / displayName.length()) {
                        Gui.drawRect((int) left, (int) (y + height + 1.5f - posY), (int) (left + distance / displayName.length()), (int) bottom, getRainbow(100 * ((lIndex + 1) + (moduleHashMap.size() - i) + 1), 6000, 0.6f, 1).getRGB());
                        lIndex++;
                    }
                }

                Gui.drawRect((int) (scaledResolution.getScaledWidth() - posX + size), y - posY, (int) (scaledResolution.getScaledWidth() - posX + size + depth), (int) (y + height + 2.5F - posY), getRainbow(100 * (displayName.length() + (moduleHashMap.size() - i) + 2), 6000, 0.6f, 1).getRGB());

                Gui.drawRect((int) ((scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) - size), y - posY - (i == 0 ? 0 : 1), (int) (scaledResolution.getScaledWidth() - posX + size), (int) (y + (roboto18.FONT_HEIGHT + addY) + 2 - 1 - posY), Integer.MIN_VALUE);
                Gui.drawRect((int) ((scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) - size - depth), y - posY - 1, (int) ((scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) - size), (int) (y + height + 2.5F - posY), getRainbow(100 * ((moduleHashMap.size() - i) + 1), 6000, 0.6f, 1).getRGB());

                for (int c = 0; c < displayName.length(); c++) {
                    final char character = displayName.charAt(c);
                    roboto18.drawStringWithShadow(character + "", (float) (scaledResolution.getScaledWidth() - roboto18.getStringWidthInt(displayName) - 1 - posX) + roboto18.getStringWidthInt(displayName.substring(0, c)), y + 1.5f - posY + 1, getRainbow(100 * (c + (moduleHashMap.size() - i) + 1), 6000, 0.6f, 1).getRGB());
                }
