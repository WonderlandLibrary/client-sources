...
import io.github.liticane.monoxide.module.impl.render.AntiBlindModule;
...
public abstract class Entity implements ICommandSender {
    ...
    public void onEntityUpdate() {
        ...
        if (this.worldObj.isRemote) {
            this.fire = 0;
        } else if (this.fire > 0 && !ModuleManager.getInstance().getModule(AntiBlindModule.class).isEnabled()) {
    ...
    protected void setOnFireFromLava() {
        if (!this.isImmuneToFire && !ModuleManager.getInstance().getModule(AntiBlindModule.class).isEnabled()) {
    ...
    public void setFire(int seconds) {
        if (!ModuleManager.getInstance().getModule(AntiBlindModule.class).isEnabled()) {
            int i = seconds * 20;
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (this.worldObj.isFlammableWithin(this.getEntityBoundingBox().contract(0.001D, 0.001D, 0.001D))
                    && !ModuleManager.getInstance().getModule(AntiBlindModule.class).isEnabled()) {
    ...
    public boolean isWet() {
        return ModuleManager.getInstance().getModule(AntiBlindModule.class).isEnabled() || this.inWater ||
    ...
    public float getCollisionBorderSize() {
        return (ModuleManager.getInstance().getModule(ViewModelModule.class).isEnabled() ? 0.0F : 0.1F);
    }
    ...
    public String toString() {
        return String.format("%s[\'%s\'/%d, l=\'%s\', x=%.2f, y=%.2f, z=%.2f]", new Object[]{this.getClass().getSimpleName(), this.getCommandSenderName(), this.entityId, this.worldObj == null ? "~NULL~" : this.worldObj.getWorldInfo().getWorldName(), this.posX, this.posY, this.posZ});
    }
