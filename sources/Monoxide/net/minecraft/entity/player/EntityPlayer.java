...
import io.github.liticane.monoxide.listener.event.minecraft.player.combat.KnockbackModifierEvent;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (cause != null) {
            ...
            if(this == Minecraft.getMinecraft().thePlayer)
                yaw = PlayerHandler.yaw;
            this.motionX = -MathHelper.cos((this.attackedAtYaw + yaw) * (float) Math.PI / 180.0F) * 0.1F;
            this.motionZ = -MathHelper.sin((this.attackedAtYaw + yaw) * (float) Math.PI / 180.0F) * 0.1F;
        } else {
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                entityitem.motionX = -MathHelper.sin(yaw / 180.0F * (float) Math.PI) * MathHelper.cos(pitch / 180.0F * (float) Math.PI) * f2;
                entityitem.motionZ = MathHelper.cos(yaw / 180.0F * (float) Math.PI) * MathHelper.cos(pitch / 180.0F * (float) Math.PI) * f2;
                entityitem.motionY = -MathHelper.sin(pitch / 180.0F * (float) Math.PI) * f2 + 0.1F;
