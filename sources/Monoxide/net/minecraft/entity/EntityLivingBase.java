...
import io.github.liticane.monoxide.module.impl.misc.ViaVersionFixModule;
...
import io.github.liticane.monoxide.module.impl.movement.NoJumpDelayModule;
...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        double swingSpeed = ModuleManager.getInstance().getModule(HitAnimationsModule.class).isEnabled()
                ? 1.0 / ModuleManager.getInstance().getModule(HitAnimationsModule.class).swingSpeed.getValue()
                : 1.0;
     ...
     */
    protected void jump() {
        PlayerJumpEvent jumpEvent = new PlayerJumpEvent(this.rotationYaw, this.getJumpUpwardsMotion())
                .publishItself();
    ...
    public void onLivingUpdate() {
        ...
        if (Math.abs(this.motionX) < (ModuleManager.getInstance().getModule(ViaVersionFixModule.class).isEnabled() ? 0.003D : 0.005D)) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < (ModuleManager.getInstance().getModule(ViaVersionFixModule.class).isEnabled() ? 0.003D : 0.005D)) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < (ModuleManager.getInstance().getModule(ViaVersionFixModule.class).isEnabled() ? 0.003D : 0.005D)) {
            this.motionZ = 0.0D;
        }
        ...
        if (this.isJumping) {
            if (this.isInWater()) {
                ...
                this.jump();
                if (!ModuleManager.getInstance().getModule(NoJumpDelayModule.class).isEnabled()) {
                    this.jumpTicks = 10;
                }
            }
        } else {
