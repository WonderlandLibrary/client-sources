...
import io.github.liticane.monoxide.module.impl.render.AmbienceModule;
...
import io.github.liticane.monoxide.listener.event.minecraft.render.PerspectiveEvent;
import io.github.liticane.monoxide.listener.event.minecraft.render.Render3DEvent;
import io.github.liticane.monoxide.listener.event.minecraft.render.player.EntityRendererEvent;
import io.github.liticane.monoxide.listener.event.minecraft.render.player.RendererEvent;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (renderViewEntity != null && this.mc.theWorld != null) {
            final RayTraceRangeEvent rayTraceRangeEvent = new RayTraceRangeEvent(3.0f).publishItself();
            ...
            Vec3 vec3 = null;
            final float n4 = 1.0f;
            final List<Entity> entitiesInAABBexcluding = this.mc.theWorld.getEntitiesInAABBexcluding(renderViewEntity, renderViewEntity.getEntityBoundingBox().addCoord(look.xCoord * n3, look.yCoord * n3, look.zCoord * n3).expand(n4, n4, n4), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>()
            {
            ...
            for (Object pointedEntityObj : entitiesInAABBexcluding) {
                Entity pointedEntity = (Entity) pointedEntityObj;
                if (pointedEntity.getCommandSenderName().contains("fake!")) {
                    continue;
                }
                ...
                if (expand.isVecInside(positionEyes)) {
                    ...
                    if (Reflector.ForgeEntity_canRiderInteract.exists()) {
                        callBoolean = Reflector.callBoolean(pointedEntity, Reflector.ForgeEntity_canRiderInteract, new Object[0]);
                    }
                    if (pointedEntity == renderViewEntity.ridingEntity && !callBoolean) {
                        if (n5 != 0.0) {
                            continue;
                        }
                        this.pointedEntity = pointedEntity;
    ...
    private float getFOVModifier(float partialTicks, boolean p_78481_2_)
    {
        ...
        else
        {
            ...
            return Reflector.ForgeHooksClient_getFOVModifier.exists() ? Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier, new Object[] {this, entity, block, partialTicks, f}): f;
        }
    }
    ...
    public void orientCamera(float partialTicks)
    {
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, new Object[] {this, entity, block1, partialTicks, f6, f7, f8});
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                try
                {
                    ...
                    {
                        Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, new Object[] {this.mc.currentScreen, k1, l1, p_181560_1_});
                    }
                    else
                    {
                ...
                catch (Throwable throwable)
                {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen size", new Callable<String>()
                    {
                        public String call() throws Exception
                        {
                            return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", new Object[] {scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), EntityRenderer.this.mc.displayWidth, EntityRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor()});
                        }
                    });
    ...
    private void renderWorldPass(int pass, float partialTicks, float particleTicks, long finishTimeNano)
    {
        ...
        if (flag1 && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.water))
        {
            ...
            if ((!Reflector.ForgeHooksClient_onDrawBlockHighlight.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, new Object[] {renderglobal, entityplayer1, this.mc.objectMouseOver, 0, entityplayer1.getHeldItem(), partialTicks})) && !this.mc.gameSettings.hideGUI)
            {
    ...
    private void addRainParticles()
    {
        ...
        if (f != 0.0F && Config.isRainSplash() && !AmbienceModule.snow) {
            {
    ...
    protected void renderRainSnow(float partialTicks)
    {
        if (Reflector.ForgeWorldProvider_getWeatherRenderer.exists())
        {
            ...
            if (object != null)
            {
                Reflector.callVoid(object, Reflector.IRenderHandler_render, new Object[] {partialTicks, this.mc.theWorld, this.mc});
                return;
            }
        }
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1)
                {
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow())
                    {
                        ...
                        if (k2 != l2)
                        {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F && !AmbienceModule.snow) {
                                if (j1 != 0)
                                {
                                    if (j1 >= 0)
                                    {
                                ...
                                int k3 = j3 >> 16 & 65535;
                                int l3 = j3 & 65535;
                                worldrenderer.pos((double)l1 - d3 + 0.5D, k2, (double)k1 - d4 + 0.5D).tex(0.0D, (double)k2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).endVertex();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, k2, (double)k1 + d4 + 0.5D).tex(1.0D, (double)k2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).endVertex();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, l2, (double)k1 + d4 + 0.5D).tex(1.0D, (double)l2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).endVertex();
                                worldrenderer.pos((double)l1 - d3 + 0.5D, l2, (double)k1 - d4 + 0.5D).tex(0.0D, (double)l2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).endVertex();
                            }
                            else
                            {
                                if (j1 != 1)
                                ...
                                int j4 = i4 >> 16 & 65535;
                                int k4 = i4 & 65535;
                                worldrenderer.pos((double)l1 - d3 + 0.5D, k2, (double)k1 - d4 + 0.5D).tex(0.0D + d9, (double)k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).endVertex();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, k2, (double)k1 + d4 + 0.5D).tex(1.0D + d9, (double)k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).endVertex();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, l2, (double)k1 + d4 + 0.5D).tex(1.0D + d9, (double)l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).endVertex();
                                worldrenderer.pos((double)l1 - d3 + 0.5D, l2, (double)k1 - d4 + 0.5D).tex(0.0D + d9, (double)l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).endVertex();
                            }
                        }
                    }
                }
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        {
            Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor, new Object[] {this, entity, block, partialTicks, this.fogColorRed, this.fogColorGreen, this.fogColorBlue});
            Reflector.postForgeBusEvent(object);
    ...
    private void setupFog(int p_78468_1_, float partialTicks)
    {
        ...
        {
            f = Reflector.callFloat(Reflector.ForgeHooksClient_getFogDensity, new Object[] {this, entity, block, partialTicks, 0.1F});
        }
        ...
        else
        {
            ...
            {
                Reflector.callVoid(Reflector.ForgeHooksClient_onFogRender, new Object[] {this, entity, block, partialTicks, p_78468_1_, f3});
            }
        }
