...
import club.bluezenith.module.modules.movement.LongJump;
import club.bluezenith.module.modules.movement.NoSlowDown;
import club.bluezenith.module.modules.movement.Sprint;
import club.bluezenith.module.modules.movement.flight.OldVerus;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdateWalkingPlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            this.playerPosition = new BlockPos(this.posX, this.posY, this.posZ);
            ...
            //Rotations rt = (Rotations) BlueZenith.getBlueZenith().getModuleManager().getModule(Rotations.class);
    ...
    public void onLivingUpdate() {
        if(this == Minecraft.getMinecraft().thePlayer){
            BlueZenith.getBlueZenith().postEvent(new UpdateEvent());
        }
        ...
        if (this.isUsingItem() && !this.isRiding()) { //if ((this.isUsingItem() || ((Aura) BlueZenith.getBlueZenith().getModuleManager().getModule(Aura.class)).blockStatus) && !this.isRiding()) {
            BlueZenith.getBlueZenith().postEvent(event);
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && !(this.isUsingItem() && !BlueZenith.getBlueZenith().getModuleManager().getModule(NoSlowDown.class).getState()) && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }
        final Flight fly = (Flight) BlueZenith.getBlueZenith().getModuleManager().getModule(Flight.class);
        final LongJump longJump = (LongJump) BlueZenith.getBlueZenith().getModuleManager().getModule(LongJump.class);
        final Scaffold scaff = (Scaffold) BlueZenith.getBlueZenith().getModuleManager().getModule(Scaffold.class);
        final Sprint sprint = (Sprint) BlueZenith.getBlueZenith().getModuleManager().getModule(Sprint.class);

        if(fly == null) return;
        final boolean flyT = fly.getState() && (((OldVerus) fly.oldVerusMode).moveSpeed[2] > ((OldVerus)fly.oldVerusMode).f || fly.mode.is("Verus Fast"));
        final boolean longJumpT = false;//todo fixlongJump.getState() && longJump.canStopSprinting();
        final boolean scaffoldT = scaff.getState() && scaff.sprintMode.is("Boost") && scaff.isPlacing || scaff.getState() && !scaff.sprintMode.isVisible();
        final boolean sprintT = (!sprint.getState() && this.movementInput.moveForward < f) || (sprint.getValue("Mode").get().equals("Legit") && this.movementInput.moveForward < f);

        if (this.isSprinting() && (!MovementUtil.areMovementKeysPressed() || this.isCollidedHorizontally || !flag3 || flyT || longJumpT || scaffoldT || sprintT)) {
            if(event.reducer != 1.0F || !mc.thePlayer.isUsingItem())
            this.setSprinting(false);
        }
