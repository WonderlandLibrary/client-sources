...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
     ...
     */
    public void initGui() {
        //RPCManager.updateRPC("Multiplayer Menu", "");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawCenteredString(this.fontRendererObj, I18n.format("multiplayer.title"), this.width / 2, 12, 16777215);

        String ipStatus = "§7§oIP Status: ";
        final IpTracker tracker = BlueZenith.getBlueZenith().getNewAltManagerGUI().ipTracker;

        if(tracker.failedToCheckIP()) {
            ipStatus += "§c§oFailed to check";
        } else if(tracker.isIpBanned()) {
            ipStatus += "§c§oUsed";
        } else {
            ipStatus += "§a§oUnused";
        }

        if(tracker.secondsLeftUntilNextCheck == 0) {
            ipStatus += " §8§o(Checking)";
        } else ipStatus += " §8§o(" + tracker.secondsLeftUntilNextCheck + "s)";

        this.fontRendererObj.drawString(ipStatus, this.width - this.fontRendererObj.getStringWidthF(ipStatus) - 1, 2, -1, true);
