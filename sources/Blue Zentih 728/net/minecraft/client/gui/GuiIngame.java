...
import club.bluezenith.module.modules.render.AntiBlind;
...
import static club.bluezenith.util.font.FontUtil.drawGradientWithShadow;
import static club.bluezenith.util.render.RenderUtil.hollowRect;
import static com.google.common.collect.Lists.newArrayList;
import static java.lang.Math.abs;
import static java.util.stream.Collectors.toList;
import static net.minecraft.scoreboard.ScorePlayerTeam.formatPlayerName;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null)) {
            if (!tabEnabled) {
                this.overlayPlayerList.anim.setEase(Easing.CUBIC_OUT);
                this.overlayPlayerList.anim.setReversed(false);
                this.overlayPlayerList.anim.setValue(0);
                this.overlayPlayerList.anim.setSpeed(4);
                this.overlayPlayerList.anim.reset();
                tabEnabled = true;
            }
            ...
            if (tabEnabled) {
                this.overlayPlayerList.anim.setReversed(true);
                this.overlayPlayerList.anim.setEase(Easing.CUBIC_IN);
                tabEnabled = false;
            }
            if (this.overlayPlayerList.anim.getValue() < 0.25) {
        ...
        this.mc.mcProfiler.startSection("bluezenith");

        Render2DEvent event = new Render2DEvent(partialTicks, scaledresolution);
        BlueZenith.getBlueZenith().postEvent(event);
        BlueZenith.getBlueZenith().getDraggableRenderer().draw(event);
    ...
    private float translateTooltip = 0;
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            final boolean customHotbar = HUD.elements.getOptionState("Custom hotbar"),
                          smoothHotbar = HUD.elements.getOptionState("Animate tooltip");

            for (int j = 0; j < 9; ++j) {
                float x = ((float)sr.getScaledWidth() / 2) - 90 + j * 20 + 0.5f;
                int y = sr.getScaledHeight() - 16 - 5;

                if(customHotbar)
                  RenderUtil.rect(x, y, x + 19, y + 20, new Color(30, 30, 30, 180));
            }

            /*RenderUtil.blur(halfWidth - 91, sr.getScaledHeight() - 22, halfWidth - 91 + 182, sr.getScaledHeight() - 22 + 22);
            Gui.drawRect(halfWidth - 91, sr.getScaledHeight() - 22, halfWidth - 91 + 182, sr.getScaledHeight() - 22 + 22, new Color(30, 30, 30, 180).getRGB());*/
            ...
            if(!customHotbar)
               this.drawTexturedModalRect(halfWidth - 91, sr.getScaledHeight() - 22, 0, 0, 182, 22);

            if(player.inventory != null) {
                int slotX = player.inventory.currentItem * 20;

                if(smoothHotbar) {
                    float diff = abs(slotX - translateTooltip) * MathHelper.clamp(RenderUtil.delta * 0.02f, 0, 1);

                    if(slotX > translateTooltip) {
                        translateTooltip += diff;
                    }
                    if(slotX < translateTooltip) {
                        translateTooltip -= diff;
                    }
                ...
                if(!customHotbar)
                  this.drawTexturedModalRect(halfWidth - 91 - 1 + (smoothHotbar ? translateTooltip : slotX), sr.getScaledHeight() - 22 - 1, 0, 22, 24, 22);
                //Gui.drawRect(halfWidth - 91 - 1 + (doSmoothHotbar ? translateTooltip : slotX), sr.getScaledHeight() - 22 - 1, halfWidth - 91 - 1 + (doSmoothHotbar ? translateTooltip : slotX) + 24, sr.getScaledHeight() - 22 - 1 + 22, HUD.module.getColor(0));

                if(customHotbar)
                 hollowRect(halfWidth - 90 + (smoothHotbar ? translateTooltip : slotX), sr.getScaledHeight() - 21, halfWidth - 70 + (smoothHotbar ? translateTooltip : slotX), sr.getScaledHeight() - 22 - 1 + 22, 1.5f, HUD.module.getColor(0));
            ...
            for (int j = 0; j < 9; ++j) {
                ...
                this.renderHotbarItem(j, k, l, partialTicks, player);
            }
    ...
    private void renderScoreboard(ScoreObjective scoreObjective, ScaledResolution resolution) {
        final HUD hud = HUD.module;
        if(hud.getState() && !HUD.elements.getOptionState("Scoreboard")) return;

        final Scoreboard scoreboard = scoreObjective.getScoreboard();

        //sorts score either by points or by string width, if the point amount is equal
        Collection<Score> scores = scoreboard.getSortedScores(scoreObjective);

        final List<Score> validScores = scores.stream()
                .filter(score ->
        ...
        if (validScores.size() > 15) {
            scores = newArrayList(Iterables.skip(validScores, scores.size() - 15));
        } else {
            scores = validScores;
        }

        final FontRenderer fontRenderer = hud.getState() && HUD.elements.getOptionState("Scoreboard font") ? hud.font.get() : this.getFontRenderer();

        //scoreObjective.getDisplayName() returns the scoreboard title
        int scoreboardWidth = fontRenderer.getStringWidth(scoreObjective.getDisplayName());

        for (Score score : scores) {
            final ScorePlayerTeam team = scoreboard.getPlayersTeam(score.getPlayerName()); //usually just another scoreboard entry
            final String entry = formatPlayerName(team, score.getPlayerName()) + 2; //formatted entry plus "2" (probably just for extra width)

            //this way we can get the optimal width of the scoreboard
            scoreboardWidth = Math.max(scoreboardWidth, fontRenderer.getStringWidth(entry));
        }

        final int overallHeight = scores.size() * fontRenderer.FONT_HEIGHT;
        final int endY = (resolution.getScaledHeight() / 2 + overallHeight / 3) - (hud.getState() ? hud.scoreboardHeight.get().intValue() : 0);

        final int rightOffset = 3; //how much pixels away from the right side of the screen
        int startX = resolution.getScaledWidth() - scoreboardWidth - rightOffset;

        int drawnScores = 0;

        final int startY = endY - scores.size() * fontRenderer.FONT_HEIGHT;

        final int endX = resolution.getScaledWidth() - rightOffset + 2;

        RenderUtil.rect(startX - 2, startY - fontRenderer.FONT_HEIGHT, endX, endY, 1342177280);

        if(fontRenderer != this.getFontRenderer())
            startX -= 1F;
        for (Score score1 : scores) {
            ++drawnScores;
            ScorePlayerTeam team = scoreboard.getPlayersTeam(score1.getPlayerName());

            String entry = formatPlayerName(team, score1.getPlayerName());

            final int textY = endY - drawnScores * fontRenderer.FONT_HEIGHT;

            final Matcher linkMatcher = LINK_PATTERN.matcher(entry);
            //if the entry is likely to be a server ip / some other link
            if(linkMatcher.find()) {
                entry = "bluezenith.club";

                drawGradientWithShadow((index) -> new Color(HUD.module.getColor(index)),
                        fontRenderer,
                        entry,
                        startX, textY
                );
            } else {
                //color before: 553648127 (idk why it has low alpha, doesn't do shit anyway)
                fontRenderer.drawString(entry, startX, textY, -1, true);
            }

            //if the score being drawn is actually the title,
            //draw it centered
            if (drawnScores == scores.size()) {
                final String title = scoreObjective.getDisplayName();

                fontRenderer.drawString(
                        title,
                        startX + scoreboardWidth / 2F - fontRenderer.getStringWidthF(title) / 2F,
                        textY - fontRenderer.FONT_HEIGHT,
                        -1, //color before: 553648127 (read above)
                        true
                );
            }
        }
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            float f2 = f1;
            int k2 = player.getTotalArmorValue();
            int l2 = -1;
    ...
    private void renderBossHealth() {
        if(HUD.module.getState() && !HUD.elements.getOptionState("Bossbar")) return;
    ...
    private void renderPumpkinOverlay(ScaledResolution p_180476_1_) {
        if(BlueZenith.getBlueZenith().getModuleManager().getModule(AntiBlind.class).getState() && ((AntiBlind) BlueZenith.getBlueZenith().getModuleManager().getModule(AntiBlind.class)).noPumpkin.get()){
            return;
        }
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
        if (!Config.isVignetteEnabled()) {
            ...
            double d0 = Math.min(worldborder.getResizeSpeed() * (double) worldborder.getWarningTime() * 1000.0D, abs(worldborder.getTargetSize() - worldborder.getDiameter()));
