...
import static club.bluezenith.util.render.ShaderSources.mainMenuBackground;
import static club.bluezenith.util.render.ShaderSources.vertexShader;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    private static long initializationTime = currentTimeMillis();
    ...
    private void setupShader() {
        if(BlueZenith.getBlueZenith().backgroundShader == null || shouldReloadShader()) {
            initializationTime = currentTimeMillis();
            BlueZenith.getBlueZenith().backgroundShader = new GLShader(vertexShader, mainMenuBackground) {
                @Override
                public void updateUniforms() {
                    glUniform1f(this.getUniformLocation("time"), (currentTimeMillis() - initializationTime) / 1000F);
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (keyCode == 1) {
            if(this instanceof ClickGui) {
                final ClickGui gui = (ClickGui) this;
                if(gui.shouldDrawSpotlight) {
                    gui.shouldDrawSpotlight = false;
    ...
    protected boolean shouldReloadShader() { //should reload if the shader has been going on for an hour to prevent it lookin weird af
        return (currentTimeMillis() - initializationTime) / 1000L > 7200;
    }
    ...
    public void sendChatMessage(String msg, boolean addToChat) {
        ...
        BlueZenith.getBlueZenith().getCommandManager().dispatch(event);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton == 0) {
            final GuiButton[] buttons = this.buttonList.toArray(new GuiButton[0]);
    ...
    /**
     * Draws either a gradient over the background screen (when it exists) or a shader.
     */
    public void drawDefaultBackground(boolean useBlur) {
        if(Preferences.useBackgroundShader && mc.theWorld == null && BlueZenith.getBlueZenith().backgroundShader != null) {
            BlueZenith.getBlueZenith().backgroundShader.useAuto();
            if(Preferences.useBlurInMenus && !(mc.currentScreen instanceof GuiMainMenu) && useBlur) //prevent double blur
        ...
        if(mc.theWorld == null) {
            final GuiMainMenu guiMainMenu = ((GuiMainMenu) BlueZenith.getBlueZenith().getMainMenu());
            if (guiMainMenu.getBackgroundAlpha() > 0.05F) { //do not draw the background if the alpha is too low
                rect(0, 0, width, height, new Color(0, 0, 0, guiMainMenu.getBackgroundAlpha() / 255F)); //draw the dark background
            }
        }
    }
    ...
    public void drawWorldBackground(int tint) {
        if (this.mc.theWorld != null) {
            ...
            drawGradientRect(0,0, this.width, this.height, new Color(0, 0, 69).getRGB(), ColorUtil.pulseBlue(10));
          /*  drawGradientRect(0, 0, this.width, this.height, new Color(0, 0, 69).getRGB(), ColorUtil.getMainColor().getRGB());
            drawGradientRect(0,0, this.width, this.height, new Color(0, 0, 69).getRGB(), ColorUtil.pulse(10));*/
