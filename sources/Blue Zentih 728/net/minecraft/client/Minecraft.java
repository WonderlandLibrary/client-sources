...
import net.minecraft.moved.sussy.ConstantSugoma;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        this.isDemo = false;//gameConfig.gameInfo.isDemo;
     ...
     */
    public static volatile boolean blockThread = true;
    public static volatile boolean error, erroredOnce;

    private static String getUID(String msg) {//todo
        return "1";
        /*final String output = showInputDialog(null, msg, "Blue Zenith", INFORMATION_MESSAGE);
        if(!output.matches("[0-9]{1,4}")) {
            return getUID("Input a valid UID");
        } else return output;*/
    }

    private static Thread asyncMessage(String msg, int type) {
        final JFrame panel = new JFrame();
        panel.setAlwaysOnTop(true);
        Thread t = new Thread(() -> showMessageDialog(panel, msg, "Blue Zenith", type));
        t.start();
        return t;
    }

    private static void syncMessage(String msg, int type) {
        final JFrame panel = new JFrame();
        panel.setAlwaysOnTop(true);
        showMessageDialog(panel, msg, "Blue Zenith", type);
    }
 ...
 //       this.debug = String.valueOf(RConConsoleSource.a(5777));
        ...
        Display.setTitle("bluezenith.club | waiting for login");
        final String UID = getUID("Input your UID");
        Display.setTitle("bluezenith.club | logging in..");
        actualUID = Integer.parseInt(UID);
        final Thread loginNotification = asyncMessage("Loading, please wait (this may take some time)", INFORMATION_MESSAGE);
        new Thread(() -> {
            while(blockThread) {
                if(AuthenticationInfo.customMessage != null) {
                    error = true;
                    erroredOnce = true;
                    syncMessage(AuthenticationInfo.customMessage, ERROR_MESSAGE);
                    error = false;
                    blockThread = false;
                }
            }
        ...
        try {
            SimpleLoader.startFirstStage(Long.parseLong(UID));
        } catch (Exception e) {
            if(e instanceof UnknownHostException) {
                syncMessage("Could not contact the authentication server. Try again later.", ERROR_MESSAGE);
            } else {
                syncMessage("Couldn't load the client due to an unexpected error", ERROR_MESSAGE);
            }
            e.printStackTrace();
        }
        loginNotification.interrupt();//todo
        while(error);
        if(erroredOnce && false) {
            System.exit(9);
        }
        this.ingameGUI = new GuiIngame(this);

        this.displayGuiScreen(BlueZenith.getBlueZenith().getMainMenu());
        ...
        try {
            ...
            final double a = new ConstantSugoma().the();
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("bluezenith.club | loading");
        //Display.setTitle("BlueZenith b"+BlueZenith.version+" (1.8.9) DEV BUILD");
    ...
    public ResourcePackRepository copyRepo(File newDir) {
        return new ResourcePackRepository(newDir, new File(this.mcDataDir, "server-resource-packs"), this.mcDefaultResourcePack, this.metadataSerializer_, this.gameSettings);
    }
    ...
    private void drawSplashScreen(TextureManager textureManagerInstance) {
        ...
        worldrenderer.pos(0.0D, this.displayHeight, 0.0D).func_181673_a(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
        worldrenderer.pos(this.displayWidth, this.displayHeight, 0.0D).func_181673_a(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
        worldrenderer.pos(this.displayWidth, 0.0D, 0.0D).func_181673_a(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
        worldrenderer.pos(0.0D, 0.0D, 0.0D).func_181673_a(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
        tessellator.draw();
    ...
    public void func_181536_a(int p_181536_1_, int p_181536_2_, int p_181536_3_, int p_181536_4_, int p_181536_5_, int p_181536_6_, int p_181536_7_, int p_181536_8_, int p_181536_9_, int p_181536_10_) {
        ...
        worldrenderer.begin(7, DefaultVertexFormats.field_181709_i);
        worldrenderer.pos(p_181536_1_, p_181536_2_ + p_181536_6_, 0.0D).func_181673_a((float) p_181536_3_ * f, (float) (p_181536_4_ + p_181536_6_) * f1).color(p_181536_7_, p_181536_8_, p_181536_9_, p_181536_10_).endVertex();
        worldrenderer.pos(p_181536_1_ + p_181536_5_, p_181536_2_ + p_181536_6_, 0.0D).func_181673_a((float) (p_181536_3_ + p_181536_5_) * f, (float) (p_181536_4_ + p_181536_6_) * f1).color(p_181536_7_, p_181536_8_, p_181536_9_, p_181536_10_).endVertex();
        worldrenderer.pos(p_181536_1_ + p_181536_5_, p_181536_2_, 0.0D).func_181673_a((float) (p_181536_3_ + p_181536_5_) * f, (float) p_181536_4_ * f1).color(p_181536_7_, p_181536_8_, p_181536_9_, p_181536_10_).endVertex();
        worldrenderer.pos(p_181536_1_, p_181536_2_, 0.0D).func_181673_a((float) p_181536_3_ * f, (float) p_181536_4_ * f1).color(p_181536_7_, p_181536_8_, p_181536_9_, p_181536_10_).endVertex();
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        if (this.currentScreen != null) {
            ...
            if(guiScreenIn == null && this.currentScreen.hasParentScreen()) {
                guiScreenIn = this.currentScreen.getParentScreen();
            }
        }

        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = BlueZenith.getBlueZenith().getMainMenu();
        ...
        if (guiScreenIn != null) {
            boolean flag = false;
            if(BlueZenith.getBlueZenith() != null && BlueZenith.getBlueZenith().getModuleManager() != null) {
                final ChestStealer stealer = BlueZenith.getBlueZenith().getModuleManager().getAndCast(ChestStealer.class);
                if(guiScreenIn instanceof GuiChest && stealer.getState() && !stealer.silentMode.is("Normal") && stealer.isEligible(guiScreenIn)) {
                    flag = true;
                }
            }
            if(!flag)
    ...
    private void resize(int width, int height) {
        ...
        if (this.currentScreen != null) {
            resolution = new ScaledResolution(this);
            this.currentScreen.onResize(this, resolution.getScaledWidth(), resolution.getScaledHeight());
        }

        this.loadingScreen = new LoadingScreenRenderer(this);
        this.updateFramebufferSize();

       /* todo slay this  if(BlueZenith.getBlueZenith() != null && BlueZenith.getBlueZenith().getDraggableRenderer() != null) {
            if(resolution == null)
                resolution = new ScaledResolution(this);

            BlueZenith.getBlueZenith().getDraggableRenderer().screenResize((float) resolution.getScaledWidth_double(), (float) resolution.getScaledHeight_double());
        }*/
    }
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    BlueZenith.getBlueZenith().postEvent(new KeyPressEvent(k));
                    ...
                    if (this.currentScreen != null) {
                        ...
                        final boolean flag = Keyboard.isKeyDown(61);
                        switch (k) {
                            case 1:
                            ...
                            case 30:
                                if(flag) this.renderGlobal.loadRenderers();
                            break;
                            ...
                            case 48:
                                if(flag) this.renderManager.setDebugBoundingBox(!this.renderManager.isDebugBoundingBox());
                            break;
            ...
            if (this.thePlayer.isUsingItem()) {
                ...
                if (!this.gameSettings.keyBindUseItem.isKeyDown() && (!aura.getState() || !((BooleanValue) aura.getValue("Autoblock")).get())) {
...
/* idk if this is necessary
    ...
    public void launchIntegratedServer(String folderName, String worldName, WorldSettings worldSettingsIn) {
        ...
        if (worldSettingsIn == null && worldinfo != null) {
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        if (worldClientIn != null) {
            ...
            BlueZenith.getBlueZenith().postEvent(new SpawnPlayerEvent(thePlayer));
        } else {
