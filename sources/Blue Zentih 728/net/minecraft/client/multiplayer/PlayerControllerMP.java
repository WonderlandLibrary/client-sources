...
import club.bluezenith.module.modules.exploit.FastBreak;
...
public class PlayerControllerMP
{
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing)
    {
        ...
        else if (this.isHittingPosition(posBlock))
        {
            ...
            else
            {
                ...
                final FastBreak fastbreak = BlueZenith.getBlueZenith().getModuleManager().getAndCast(FastBreak.class);
                if (fastbreak.getState()) {
                    if (this.curBlockDamageMP >= ((FloatValue)fastbreak.getValue("Damage")).get()) {
                        this.isHittingBlock = false;
                        ...
                        this.curBlockDamageMP = 0.0F;
                        this.stepSoundTickCounter = 0.0F;
                        this.blockHitDelay = ((IntegerValue)fastbreak.getValue("Break delay")).get();
                    }
                }
                else {
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec)
    {
        ...
        else
        {
            ...
            final RightClickBlockEvent event = new RightClickBlockEvent(hitPos, side.getIndex(), player.inventory.getCurrentItem(), new Vec3(f, f1, f2));
            BlueZenith.getBlueZenith().postEvent(event);
            final ItemStack item = mc.thePlayer.getHeldItem();
            if(ViaMCP.getInstance().getVersion() == 47 || item == null || mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK
                    && item != null
...
//
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn)
    {
        ...
        {
            this.syncCurrentPlayItem();
            final NoSlowDown noslow = BlueZenith.getBlueZenith().getModuleManager().getAndCast(NoSlowDown.class);
            if (!(noslow.getState() && noslow.extendedModeValue.get().getKey().equals("NCP")) || !(playerIn.inventory.getCurrentItem() != null && playerIn.inventory.getCurrentItem().getItem() instanceof ItemSword)) {
    ...
    public void onStoppedUsingItem(EntityPlayer playerIn)
    {
        ...
        /*final NoSlowDown noslow = BlueZenith.getBlueZenith().getModuleManager().getAndCast(NoSlowDown.class);
        if (!(noslow.getState() && noslow.mode.is("NCP")) || !(playerIn.inventory.getCurrentItem().getItem() instanceof ItemSword)) {
            this.netClientHandler.addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
        }*/
