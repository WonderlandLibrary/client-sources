...
public class GuiConnecting extends GuiScreen {
    ...
    public void connect(final String ip, final int port) {
        ...
        PacketUtil.data = new ServerData("", ip + ":" + port, false);
        //RPCManager.updateRPC("Playing Multiplayer", "Connecting to " + ip);
        (new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet()) {
            public void run() {
                ...
                try {
                    ...
                    networkManager = NetworkManager.createInstance(inetaddress, port, mc.gameSettings.useNativeTransport());
                    networkManager.setNetHandler(new NetHandlerLoginClient(networkManager, mc, previousGuiScreen));
                    networkManager.sendPacket(new C00Handshake(47, ip, port, EnumConnectionState.LOGIN)); //todo 1.9 version -> 107
                    networkManager.sendPacket(new C00PacketLoginStart(mc.getSession().getProfile()));
                    BlueZenith.getBlueZenith().setCurrentServerIP(ip);
                    //RPCManager.updateRPC("Playing Multiplayer", "IP: " + ip);
                    ...
                    BlueZenith.getBlueZenith().setCurrentServerIP(null);
