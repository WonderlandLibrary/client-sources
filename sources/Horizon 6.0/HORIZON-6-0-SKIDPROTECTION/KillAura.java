...
@ModInfo(Ø­áŒŠá = Category.COMBAT, Ý = 16777215, Â = "Da behst Module.", HorizonCode_Horizon_È = "KillAura")
public class KillAura extends Mod
{
    ...
    private ArrayList<EntityLivingBase> ˆà;
    private ArrayList<EntityLivingBase> ¥Æ;
    private TimeHelper Ø­à;
    private TimeHelper µÕ;
    private TimeHelper Æ;
    static EntityLivingBase £à;
    private Random Šáƒ;
    private float Ï­Ðƒà;
    private float áŒŠà;
    public static long µà;
    
    static {
        KillAura.Ý = true;
        KillAura.Ø­áŒŠá = true;
        KillAura.Âµá€ = true;
        KillAura.à = false;
        KillAura.Ø = 3.5;
        KillAura.á = true;
    ...
    public KillAura() {
        ...
        this.ÂµÈ = true;
        this.Å = null;
        this.ˆà = new ArrayList<EntityLivingBase>();
        this.¥Æ = new ArrayList<EntityLivingBase>();
        this.Ø­à = new TimeHelper();
        this.µÕ = new TimeHelper();
    ...
    public void HorizonCode_Horizon_È(final EventUpdate event) {
        ...
        if (Horizon.áƒ) {
            KillAura.µà = this.HorizonCode_Horizon_È(100, 140);
        }
        else {
            KillAura.µà = Horizon.á€;
        }
        if (this.Å == null || !this.Â(this.Å)) {
            ...
            for (final EntityLivingBase var13 : this.¥Æ) {
            ...
            if (highestWeightedTarget != null) {
                ...
                for (int var14 = 0; var14 < 2; ++var14) {
                    ...
                    for (final EntityLivingBase el2 : this.¥Æ) {
                        if (this.Ý(el2) && !this.ˆà.contains(el2) && this.Ø­áŒŠá(el2) > var15) {
                            var15 = this.Ø­áŒŠá(el2);
                            smHighestWeightedTarget = el2;
                        }
                    }
                    this.ˆà.add(smHighestWeightedTarget);
                }
                if (this.ˆà.isEmpty()) {
                    KillAura.µà -= 40L;
                }
        ...
        if (this.Å != null) {
            final boolean var17 = this.Ø­à.Â(KillAura.µà);
            if (var17) {
    ...
    public void Â(final EventUpdate evt) {
        ...
        if (this.Å != null && this.Ý(this.Å) && this.Ø­à.Â(KillAura.µà)) {
            this.Ø­à.Ø­áŒŠá();
            if (KillAura.ˆÏ­ && this.Â.á.áŒŠá() != null && this.Â.á.áŒŠá().HorizonCode_Horizon_È() instanceof ItemSword) {
            ...
            if (this.áˆºÑ¢Õ) {
                for (final EntityLivingBase e : this.ˆà) {
                    if (KillAura.á) {
                        this.Â.á.b_();
                    }
                    else {
    ...
    public void HorizonCode_Horizon_È(final EventPacketSend event) {
        ...
        if (event.Ý() instanceof C03PacketPlayer) {
            ...
            if (packet.Ø()) {
                this.Ï­Ðƒà = packet.Ø­áŒŠá();
                this.áŒŠà = packet.Âµá€();
            }
        }
    }
    ...
    public boolean HorizonCode_Horizon_È(final EntityLivingBase el) {
        ...
        return playerChecks ? (this.Ó ? var13 : KillAura.Ø­áŒŠá) : (mobChecks ? KillAura.Ý : (animalChecks && KillAura.Âµá€));
    }
    
    public boolean Â(final EntityLivingBase p) {
        return !this.Â.á.equals(p) && this.Ý((Entity)p) < KillAura.Ø * KillAura.Ø && !FriendManager.HorizonCode_Horizon_È.containsKey(p.v_()) && p.Œ() && !this.Âµá€(p) && (!(p instanceof EntityPlayer) || p.Œ > 40 || !this.ÂµÈ) && this.HorizonCode_Horizon_È(p) && !p.Ï­Ó();
    }
    ...
    public boolean Ý(final EntityLivingBase p) {
        final double adjRange = this.Â.á.ÇªÂµÕ() ? (KillAura.Ø - 0.2) : KillAura.Ø;
        ...
        if (this.Ï­Ðƒà != this.Â.á.É || this.áŒŠà != this.Â.á.áƒ) {
            maxAngle = 45.0;
        }
        boolean angleCheck = distCombined < maxAngle;
        angleCheck = (angleCheck || this.Æ.Â((this.¥Æ.size() <= 5) ? 30L : 60L));
        if (angleCheck) {
            this.Æ.Ø­áŒŠá();
        }
        return this.Â(p) && this.Â.á.Âµá€(p) < adjRange * adjRange && (p != KillAura.£à || this.¥Æ.size() <= 1 || this.µÕ.Â(200L)) && p.µà <= 4 && (angleCheck || this.¥Æ.size() <= 1 || !this.áŒŠÆ);
    }
    ...
    public int HorizonCode_Horizon_È(final int min, final int max) {
        return this.Šáƒ.nextInt(max - min + 1) + max;
    }
    ...
    public void HorizonCode_Horizon_È(final double d, final double d1, final double d2, final EntityLivingBase ep, final double e, final double f) {
        if (!(ep instanceof EntityPlayerSP) && !ep.ˆáŠ) {
            ...
            if (Horizon.Âµá€.equalsIgnoreCase("rainbow")) {
                color = ColorUtil.HorizonCode_Horizon_È(200000000L, 1.0f).getRGB();
            ...
            if (Horizon.Âµá€.equalsIgnoreCase("rainbow")) {
                color = ColorUtil.HorizonCode_Horizon_È(color, 0.9);
                OGLManager.Ø­áŒŠá(color);
            }
            else {
