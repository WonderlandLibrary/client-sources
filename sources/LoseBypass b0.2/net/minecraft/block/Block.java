...
public class Block {
    ...
    public static Block getBlockFromItem(Item itemIn) {
        if (!(itemIn instanceof ItemBlock)) return null;
        Block block = ((ItemBlock)itemIn).getBlock();
        return block;
    }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        if (state.getPropertyNames().isEmpty()) return 0;
    ...
    public boolean isBlockNormalCube() {
        if (!this.blockMaterial.blocksMovement()) return false;
        if (!this.isFullCube()) return false;
        return true;
    }

    public boolean isNormalCube() {
        if (!this.blockMaterial.isOpaque()) return false;
        if (!this.isFullCube()) return false;
        if (this.canProvidePower()) return false;
        return true;
    }
    ...
    protected Block setHardness(float hardness) {
        ...
        if (!(this.blockResistance < hardness * 5.0f)) return this;
    ...
    public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos) {
        ...
        if (i != 0) return i;
        if (!(block instanceof BlockSlab)) return i;
        pos = pos.down();
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        if (side == EnumFacing.DOWN && this.minY > 0.0) {
            return true;
        }
        if (side == EnumFacing.UP && this.maxY < 1.0) {
            return true;
        }
        if (side == EnumFacing.NORTH && this.minZ > 0.0) {
            return true;
        }
        if (side == EnumFacing.SOUTH && this.maxZ < 1.0) {
            return true;
        }
        if (side == EnumFacing.WEST && this.minX > 0.0) {
            return true;
        }
        if (side == EnumFacing.EAST && this.maxX < 1.0) {
            return true;
        }
        if (worldIn.getBlockState(pos).getBlock().isOpaqueCube()) return false;
        return true;
    }
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity) {
        ...
        if (!mask.intersectsWith(axisalignedbb)) return;
    ...
    public float getPlayerRelativeBlockHardness(EntityPlayer playerIn, World worldIn, BlockPos pos) {
        float f;
        float f2 = this.getBlockHardness(worldIn, pos);
        if (f2 < 0.0f) {
            return 0.0f;
        }
        if (!playerIn.canHarvestBlock(this)) {
            f = playerIn.getToolDigEfficiency(this) / f2 / 100.0f;
            return f;
        }
        f = playerIn.getToolDigEfficiency(this) / f2 / 30.0f;
        return f;
    }
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end) {
        ...
        if (vec36 != vec35) return new MovingObjectPosition(vec36.addVector(pos.getX(), pos.getY(), pos.getZ()), enumfacing, pos);
    ...
    private boolean isVecInsideYZBounds(Vec3 point) {
        ...
        if (!(point.yCoord >= this.minY)) return false;
        if (!(point.yCoord <= this.maxY)) return false;
        if (!(point.zCoord >= this.minZ)) return false;
        if (!(point.zCoord <= this.maxZ)) return false;
        return true;
    }
    ...
    private boolean isVecInsideXZBounds(Vec3 point) {
        ...
        if (!(point.xCoord >= this.minX)) return false;
        if (!(point.xCoord <= this.maxX)) return false;
        if (!(point.zCoord >= this.minZ)) return false;
        if (!(point.zCoord <= this.maxZ)) return false;
        return true;
    }
    ...
    protected boolean canSilkHarvest() {
        if (!this.isFullCube()) return false;
        if (this.isBlockContainer) return false;
        return true;
    }
    ...
    protected ItemStack createStackedBlock(IBlockState state) {
        ...
        if (item == null) return new ItemStack(item, 1, i);
        if (!item.getHasSubtypes()) return new ItemStack(item, 1, i);
    ...
    public boolean func_181623_g() {
        if (this.blockMaterial.isSolid()) return false;
        if (this.blockMaterial.isLiquid()) return false;
        return true;
    }
    ...
    public float getAmbientOcclusionLightValue() {
        if (!this.isBlockNormalCube()) return 1.0f;
        return 0.2f;
    }
    ...
    public static boolean isEqualTo(Block blockIn, Block other) {
        ...
        boolean bl = blockIn.isAssociatedBlock(other);
        return bl;
    }
    ...
    public static void registerBlocks() {
        ...
        block1: while (iterator.hasNext()) {
            Block block14 = (Block)iterator.next();
            Iterator iterator2 = block14.getBlockState().getValidStates().iterator();
            while (true) {
                if (!iterator2.hasNext()) continue block1;
                IBlockState iblockstate = (IBlockState)iterator2.next();
