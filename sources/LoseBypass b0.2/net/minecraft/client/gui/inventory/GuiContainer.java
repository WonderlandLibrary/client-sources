...
extends GuiScreen {
    ...
    private void updateDragSplitting() {
        ...
        if (!this.dragSplitting) return;
        this.dragSplittingRemnant = itemstack.stackSize;
        Iterator<Slot> iterator = this.dragSplittingSlots.iterator();
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        ...
        if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
            if (clickedMouseButton != 0) {
                if (clickedMouseButton != 1) return;
            }
            if (this.draggedStack == null) {
                if (slot == this.clickedSlot) return;
                if (this.clickedSlot.getStack() == null) return;
                this.draggedStack = this.clickedSlot.getStack().copy();
                return;
            }
            if (this.draggedStack.stackSize <= 1) return;
            if (slot == null) return;
            if (!Container.canAddItemToSlot(slot, this.draggedStack, false)) return;
            long i = Minecraft.getSystemTime();
            if (this.currentDragTargetSlot == slot) {
                if (i - this.dragItemDropDelay <= 500L) return;
        ...
        if (slot == null) return;
        if (itemstack == null) return;
        if (itemstack.stackSize <= this.dragSplittingSlots.size()) return;
        if (!Container.canAddItemToSlot(slot, itemstack, true)) return;
        if (!slot.isItemValid(itemstack)) return;
        if (!this.inventorySlots.canDragIntoSlot(slot)) return;
    ...
    protected boolean isPointInRegion(int left, int top, int right, int bottom, int pointX, int pointY) {
        int i = this.guiLeft;
        int j = this.guiTop;
        if ((pointX -= i) < left - 1) return false;
        if (pointX >= left + right + 1) return false;
        if ((pointY -= j) < top - 1) return false;
        if (pointY >= top + bottom + 1) return false;
        return true;
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if (!this.theSlot.getHasStack()) return;
        if (keyCode == this.mc.gameSettings.keyBindPickBlock.getKeyCode()) {
            this.handleMouseClick(this.theSlot, this.theSlot.slotNumber, 0, 3);
            return;
        }
        if (keyCode != this.mc.gameSettings.keyBindDrop.getKeyCode()) return;
        this.handleMouseClick(this.theSlot, this.theSlot.slotNumber, GuiContainer.isCtrlKeyDown() ? 1 : 0, 4);
    }

    protected boolean checkHotbarKeys(int keyCode) {
        if (this.mc.thePlayer.inventory.getItemStack() != null) return false;
        if (this.theSlot == null) return false;
        int i = 0;
        while (i < 9) {
    ...
    public void updateScreen() {
        ...
        if (this.mc.thePlayer.isEntityAlive()) {
            if (!this.mc.thePlayer.isDead) return;
        }
        this.mc.thePlayer.closeScreen();
    }
}
