...
extends Gui {
    ...
    public void addToSentMessages(String p_146239_1_) {
        if (!this.sentMessages.isEmpty()) {
            if (this.sentMessages.get(this.sentMessages.size() - 1).equals(p_146239_1_)) return;
        }
    ...
    public IChatComponent getChatComponent(int p_146236_1_, int p_146236_2_) {
        ...
        if (j > MathHelper.floor_float((float)this.getChatWidth() / this.getChatScale())) return null;
        if (k >= this.mc.fontRendererObj.FONT_HEIGHT * l + l) return null;
        int i1 = k / this.mc.fontRendererObj.FONT_HEIGHT + this.scrollPos;
        if (i1 < 0) return null;
        if (i1 >= this.field_146253_i.size()) return null;
        ChatLine chatline = this.field_146253_i.get(i1);
        int j1 = 0;
        Iterator iterator = chatline.getChatComponent().iterator();
        do {
            if (!iterator.hasNext()) return null;
        } while (!((ichatcomponent = (IChatComponent)iterator.next()) instanceof ChatComponentText) || (j1 += this.mc.fontRendererObj.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText)ichatcomponent).getChatComponentText_TextValue(), false))) <= j);
    ...
    public void deleteChatLine(int p_146242_1_) {
        ...
        } while ((chatline1 = iterator.next()).getChatLineID() != p_146242_1_);
        iterator.remove();
    }
    ...
    public int getChatHeight() {
        float f;
        if (this.getChatOpen()) {
            f = this.mc.gameSettings.chatHeightFocused;
            return GuiNewChat.calculateChatboxHeight(f);
        }
        f = this.mc.gameSettings.chatHeightUnfocused;
        return GuiNewChat.calculateChatboxHeight(f);
    }
