...
extends Entity {
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        if (this.activePotionsMap.isEmpty()) return;
    ...
    protected void updatePotionEffects() {
        ...
        if (!this.isInvisible()) {
            ...
            boolean bl = flag = this.rand.nextInt(15) == 0;
        }
        if (flag1) {
    ...
    public void addPotionEffect(PotionEffect potioneffectIn) {
        if (!this.isPotionApplicable(potioneffectIn)) return;
    ...
    public boolean isPotionApplicable(PotionEffect potioneffectIn) {
        if (this.getCreatureAttribute() != EnumCreatureAttribute.UNDEAD) return true;
        int i = potioneffectIn.getPotionID();
        if (i == Potion.regeneration.id) return false;
        if (i != Potion.poison.id) return true;
        return false;
    }

    public boolean isEntityUndead() {
        if (this.getCreatureAttribute() != EnumCreatureAttribute.UNDEAD) return false;
        return true;
    }
    ...
    public void knockBack(Entity entityIn, float p_70653_2_, double p_70653_3_, double p_70653_5_) {
        if (!(this.rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue())) return;
        ...
        if (!(this.motionY > (double)0.4f)) return;
        this.motionY = 0.4f;
    }
    ...
    public boolean isOnLadder() {
        ...
        if (block != Blocks.ladder) {
            if (block != Blocks.vine) return false;
        }
        if (!(this instanceof EntityPlayer)) return true;
        if (((EntityPlayer)this).isSpectator()) return false;
        return true;
    }
    ...
    protected String getFallSoundString(int damageValue) {
        if (damageValue <= 4) return "game.neutral.hurt.fall.small";
    ...
    public EntityLivingBase func_94060_bK() {
        ...
        if (this.entityLivingToAttack == null) return null;
    ...
    private int getArmSwingAnimationEnd() {
        ...
        if (this.isPotionActive(Potion.digSpeed)) {
            n = 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1;
            return n;
        }
        if (!this.isPotionActive(Potion.digSlowdown)) return 6;
        n = 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2;
        return n;
    }

    public void swingItem() {
        if (this.isSwingInProgress && this.swingProgressInt < this.getArmSwingAnimationEnd() / 2) {
            if (this.swingProgressInt >= 0) return;
        }
    ...
    public BaseAttributeMap getAttributeMap() {
        if (this.attributeMap != null) return this.attributeMap;
    ...
    protected float getSoundPitch() {
        float f;
        if (this.isChild()) {
            f = (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2f + 1.5f;
            return f;
        }
        f = (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2f + 1.0f;
        return f;
    }
    ...
    public void onUpdate() {
        ...
        while (true) {
            if (!(this.rotationYawHead - this.prevRotationYawHead >= 180.0f)) {
                this.worldObj.theProfiler.endSection();
    ...
    public void onItemPickup(Entity p_71001_1_, int p_71001_2_) {
        ...
        if (!(p_71001_1_ instanceof EntityXPOrb)) return;
        entitytracker.sendToAllTrackingEntity(p_71001_1_, new S0DPacketCollectItem(p_71001_1_.getEntityId(), this.getEntityId()));
    }

    public boolean canEntityBeSeen(Entity entityIn) {
        if (this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3(entityIn.posX, entityIn.posY + (double)entityIn.getEyeHeight(), entityIn.posZ)) != null) return false;
        return true;
    }
    ...
    public float getSwingProgress(float partialTickTime) {
        ...
        if (!(f < 0.0f)) return this.prevSwingProgress + f * partialTickTime;
        f += 1.0f;
    ...
    public boolean isOnTeam(Team p_142012_1_) {
        ...
        boolean bl = this.getTeam().isSameTeam(p_142012_1_);
        return bl;
    }
