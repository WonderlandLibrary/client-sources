...
implements ICommandSender {
    ...
    public boolean equals(Object p_equals_1_) {
        ...
        if (((Entity)p_equals_1_).entityId != this.entityId) return false;
        return true;
    }
    ...
    public void setFire(int seconds) {
        int i = seconds * 20;
        if (this.fire >= (i = EnchantmentProtection.getFireTimeForEntity(this, i))) return;
        this.fire = i;
    }
    ...
    private boolean isLiquidPresentInAABB(AxisAlignedBB bb) {
        if (!this.worldObj.getCollidingBoundingBoxes(this, bb).isEmpty()) return false;
        if (this.worldObj.isAnyLiquid(bb)) return false;
        return true;
    }
    ...
    protected void doBlockCollisions() {
        ...
        if (!this.worldObj.isAreaLoaded(blockpos, blockpos1)) return;
        int i = blockpos.getX();
        block2: while (i <= blockpos1.getX()) {
            int j = blockpos.getY();
            while (true) {
                if (j <= blockpos1.getY()) {
                } else {
    ...
    public boolean isSilent() {
        if (this.dataWatcher.getWatchableObjectByte(4) != 1) return false;
        return true;
    }
    ...
    public boolean isWet() {
        ...
        if (this.worldObj.canLightningStrike(new BlockPos(this.posX, this.posY, this.posZ))) return true;
        if (this.worldObj.canLightningStrike(new BlockPos(this.posX, this.posY + (double)this.height, this.posZ))) return true;
        return false;
    }
    ...
    public boolean isInsideOfMaterial(Material materialIn) {
        ...
        IBlockState iblockstate = this.worldObj.getBlockState(blockpos);
        Block block = iblockstate.getBlock();
        if (block.getMaterial() != materialIn) return false;
        ...
        if (!flag && this instanceof EntityPlayer) {
            return false;
        }
        boolean bl2 = flag;
        return bl2;
    ...
    public int getBrightnessForRender(float partialTicks) {
        ...
        if (!this.worldObj.isBlockLoaded(blockpos)) return 0;
        int n = this.worldObj.getCombinedLight(blockpos, 0);
        return n;
    }

    public float getBrightness(float partialTicks) {
        BlockPos blockpos = new BlockPos(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ);
        if (!this.worldObj.isBlockLoaded(blockpos)) return 0.0f;
        float f = this.worldObj.getLightBrightness(blockpos);
        return f;
    }
    ...
    public void applyEntityCollision(Entity entityIn) {
        if (entityIn.riddenByEntity == this) return;
        if (entityIn.ridingEntity == this) return;
        if (entityIn.noClip) return;
        if (this.noClip) return;
        double d0 = entityIn.posX - this.posX;
        double d1 = entityIn.posZ - this.posZ;
        double d2 = MathHelper.abs_max(d0, d1);
        if (!(d2 >= (double)0.01f)) return;
    ...
    public void writeToNBT(NBTTagCompound tagCompund) {
        try {
            ...
            if (!this.ridingEntity.writeMountToNBT(nbttagcompound)) return;
    ...
    public void readFromNBT(NBTTagCompound tagCompund) {
        try {
            ...
            if (!this.shouldSetPosAfterLoading()) return;
            this.setPosition(this.posX, this.posY, this.posZ);
            return;
        }
        catch (Throwable throwable) {
    ...
    public EntityItem entityDropItem(ItemStack itemStackIn, float offsetY) {
        ...
        if (itemStackIn.getItem() == null) return null;
    ...
    public boolean isBurning() {
        ...
        if (this.isImmuneToFire) return false;
        if (this.fire > 0) return true;
    ...
    protected boolean getFlag(int flag) {
        if ((this.dataWatcher.getWatchableObjectByte(0) & 1 << flag) == 0) return false;
        return true;
    }
    ...
    public String getName() {
        ...
        if (s != null) return StatCollector.translateToLocal("entity." + s + ".name");
        s = "generic";
    ...
    public boolean hasCustomName() {
        if (this.dataWatcher.getWatchableObjectString(2).length() <= 0) return false;
        return true;
    }
    ...
    public boolean getAlwaysRenderNameTag() {
        if (this.dataWatcher.getWatchableObjectByte(3) != 1) return false;
        return true;
    }
