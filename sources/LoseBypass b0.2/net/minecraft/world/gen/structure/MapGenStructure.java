...
extends MapGenBase {
    ...
    protected final void recursiveGenerate(World worldIn, final int chunkX, final int chunkZ, int p_180701_4_, int p_180701_5_, ChunkPrimer chunkPrimerIn) {
        ...
        if (this.structureMap.containsKey(ChunkCoordIntPair.chunkXZ2Int(chunkX, chunkZ))) return;
        this.rand.nextInt();
        try {
    ...
    public boolean func_175796_a(World worldIn, BlockPos pos) {
        ...
        } while (!(structurestart = iterator.next()).isSizeableStructure() || !structurestart.getBoundingBox().isVecInside(pos));
        return true;
    }
