...
public class PlayerManager {
    ...
    public void updatePlayerInstances() {
        ...
        if (worldprovider.canRespawnHere()) return;
    ...
    public boolean hasPlayerInstance(int chunkX, int chunkZ) {
        ...
        if (this.playerInstances.getValueByKey(i) == null) return false;
        return true;
    }
    ...
    private PlayerInstance getPlayerInstance(int chunkX, int chunkZ, boolean createIfAbsent) {
        ...
        if (playermanager$playerinstance != null) return playermanager$playerinstance;
        if (!createIfAbsent) return playermanager$playerinstance;
    ...
    public void markBlockForUpdate(BlockPos pos) {
        ...
        if (playermanager$playerinstance == null) return;
    ...
    private boolean overlaps(int x1, int z1, int x2, int z2, int radius) {
        int i = x1 - x2;
        int j = z1 - z2;
        if (i < -radius) return false;
        if (i > radius) return false;
        if (j < -radius) return false;
        if (j > radius) return false;
        return true;
    }
    ...
    public boolean isPlayerWatchingChunk(EntityPlayerMP player, int chunkX, int chunkZ) {
        ...
        if (playermanager$playerinstance == null) return false;
        if (!playermanager$playerinstance.playersWatchingChunk.contains(player)) return false;
        if (player.loadedChunks.contains(playermanager$playerinstance.chunkCoords)) return false;
        return true;
    }
    ...
    public void setPlayerViewRadius(int radius) {
        if ((radius = MathHelper.clamp_int(radius, 3, 32)) == this.playerViewRadius) return;
        i = radius - this.playerViewRadius;
        var3_3 = Lists.newArrayList(this.players).iterator();
        block0: while (true) {
            if (!var3_3.hasNext()) {
                this.playerViewRadius = radius;
                return;
            }
            entityplayermp = var3_3.next();
            j = (int)entityplayermp.posX >> 4;
            k = (int)entityplayermp.posZ >> 4;
            if (i > 0) {
                j1 = j - radius;
                while (true) {
                    if (j1 > j + radius) continue block0;
                    for (k1 = k - radius; k1 <= k + radius; ++k1) {
                        playermanager$playerinstance = this.getPlayerInstance(j1, k1, true);
                        if (PlayerInstance.access$100(playermanager$playerinstance).contains(entityplayermp)) continue;
                        playermanager$playerinstance.addPlayer(entityplayermp);
                    }
                    ++j1;
                }
            }
            l = j - this.playerViewRadius;
            while (true) {
                if (l > j + this.playerViewRadius) ** break;
    ...
    class PlayerInstance {
        ...
        public void removePlayer(EntityPlayerMP player) {
            if (!this.playersWatchingChunk.contains(player)) return;
            ...
            if (!this.playersWatchingChunk.isEmpty()) return;
        ...
        public void flagChunkForUpdate(int x, int y, int z) {
            ...
            if (this.numBlocksToUpdate >= 64) return;
            short short1 = (short)(x << 12 | z << 8 | y);
            int i = 0;
            while (true) {
                if (i >= this.numBlocksToUpdate) {
        ...
        public void onUpdate() {
            if (this.numBlocksToUpdate == 0) return;
