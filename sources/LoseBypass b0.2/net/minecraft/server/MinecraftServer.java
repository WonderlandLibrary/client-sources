...
IPlayerUsage {
    ...
    protected void convertMapIfNeeded(String worldNameIn) {
        if (!this.getActiveAnvilConverter().isOldMapFormat(worldNameIn)) return;
    ...
    protected void initialWorldChunkLoad() {
        ...
        for (int l1 = -192; l1 <= 192 && this.isServerRunning(); l1 += 16) {
            for (int i2 = -192; i2 <= 192 && this.isServerRunning(); ++i1, i2 += 16) {
    ...
    protected void saveAllWorlds(boolean dontLog) {
        if (this.worldIsBeingDeleted) return;
    ...
    public void stopServer() {
        ...
        if (!this.usageSnooper.isSnooperRunning()) return;
    ...
    public WorldServer worldServerForDimension(int dimension) {
        ...
        if (dimension == -1) {
            worldServer = this.worldServers[1];
            return worldServer;
        }
        if (dimension == 1) {
            worldServer = this.worldServers[2];
            return worldServer;
        }
        worldServer = this.worldServers[0];
        return worldServer;
    }
    ...
    public CrashReport addServerInfoToCrashReport(CrashReport report) {
        report.getCategory().addCrashSectionCallable("Profiler Position", new Callable<String>(){
            ...
            public String call() throws Exception {
                if (!MinecraftServer.this.theProfiler.profilingEnabled) return "N/A (disabled)";
                String string = MinecraftServer.this.theProfiler.getNameOfLastSection();
                return string;
            }
        });
        if (this.serverConfigManager == null) return report;
    ...
    public boolean isAnvilFileSet() {
        if (this.anvilFile == null) return false;
        return true;
    }
    ...
    public boolean isSinglePlayer() {
        if (this.serverOwner == null) return false;
        return true;
    }
    ...
    public boolean isCallingFromMinecraftThread() {
        if (Thread.currentThread() != this.serverThread) return false;
        return true;
    }
