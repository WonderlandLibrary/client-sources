...
extends SettingComponent {
    ...
    Runnable submitText = new Runnable(){
        ...
        public void run() {
            try {
                IntSlider.this.setting.setValue(MathUtil.clamp(Integer.parseInt(IntSlider.this.value.getText()), (int)IntSlider.this.setting.getMin(), (int)IntSlider.this.setting.getMax()));
                return;
            }
            catch (NumberFormatException e) {
                IntSlider.this.value.setText(IntSlider.this.setting.getValueString());
            }
        }
    };
    ...
    public IntSlider(IntSetting setting) {
        ...
        this.target = this.progress = ((double)((Integer)setting.getValue()).intValue() - setting.getMin()) / (setting.getMax() - setting.getMin()) * 180.0;
    ...
    public int drawComponent(int posX, int posY, double deltaTime, int click, int mouseX, int mouseY) {
        ...
        if (this.sliding) {
            if (!Mouse.isButtonDown((int)0)) {
                ...
                value += 0.5;
                value -= value % 1.0;
                this.setting.setValue((int)((double)((int)value) + this.setting.getMin()));
            }
        }
        this.target = ((double)((Integer)this.setting.getValue()).intValue() - this.setting.getMin()) / (this.setting.getMax() - this.setting.getMin()) * 180.0;
