...
public class BlockUtils extends UtilityClass {
    ...
    public static int findBlockHotbar() {
        for (int i = 36; i < 45; ++i) {
            ItemBlock itemBlock;
            Block block;
            ItemStack itemStack = BlockUtils.mc.thePlayer.inventoryContainer.getSlot(i).getStack();
            if (itemStack == null || !(itemStack.getItem() instanceof ItemBlock) || itemStack.stackSize <= 0 || !(block = (itemBlock = (ItemBlock) itemStack.getItem()).getBlock()).isFullCube() || BlackList.contains(block)) {
                continue;
            }
            return i;
        }
