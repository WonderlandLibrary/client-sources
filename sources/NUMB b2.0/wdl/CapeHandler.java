...
public class CapeHandler {
	...
	/**
	 * Number of times a player's cape has failed; if they have failed too many
	 * times skip them.
	 */
	...
	/**
	 * Number of times the a player can fail to have a cape set up before they are
	 * skipped..
	 */
	...
	static {
		...
		capes.put(UUID.fromString("f6c068f1-0738-4b41-bdb2-69d81d2b0f1c"),
				new ResourceLocation("wdl", "textures/cape_dev.png"));
		// loki
		capes.put(UUID.fromString("c8d4705f-88d9-4871-accf-8c9f29564816"),
				new ResourceLocation("textures/capes/AppleCape.png"));
	...
	private static void setupPlayer(AbstractClientPlayer player) {
		try {
			NetworkPlayerInfo info = ReflectionUtils.stealAndGetField((AbstractClientPlayer) player,
					AbstractClientPlayer.class, NetworkPlayerInfo.class);
	...
	private static void incrementFailure(EntityPlayer player) {
		if (playerFailures.containsKey(player)) {
			...
			if (numFailures > MAX_PLAYER_FAILURES) {
				...
				logger.warn("[WDL] Failed to set up cape for " + player + " too many times (" + numFailures
						+ "); skipping them");
			}
		} else {
