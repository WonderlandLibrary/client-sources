...
public class WDLMessages {
	...
	private static class MessageRegistration {
		 ...
		 *            The name to use.
		 * @param type
		 *            The type bound to this registration.
		 * @param category
		 *            The category.
		 */
		...
		public String toString() {
			return "MessageRegistration [name=" + name + ", type=" + type + ", category=" + category + "]";
		}
	...
	/**
	 * If <code>false</code>, all messages are disabled. Otherwise, per- message
	 * settings are used.
	 */
	...
	public static boolean isEnabled(IWDLMessageType type) {
		...
		if (!WDL.baseProps.containsKey("Messages." + r.name)) {
			if (WDL.baseProps.containsKey("Debug." + r.name)) {
				...
				WDL.baseProps.put("Messages." + r.name, WDL.baseProps.remove("Debug." + r.name));
			} else {
	...
	public static ListMultimap<MessageTypeCategory, IWDLMessageType> getTypes() {
		ListMultimap<MessageTypeCategory, IWDLMessageType> returned = LinkedListMultimap.create();
	...
	public static void resetEnabledToDefaults() {
		...
		for (MessageRegistration r : registrations) {
			WDL.baseProps.setProperty("MessageGroup." + r.category.internalName,
					WDL.globalProps.getProperty("MessageGroup." + r.category.internalName, "true"));
	 ...
	 * @param args
	 *            The arguments to pass to the {@link ChatComponentTranslation}. A
	 *            limited amount of processing is performed: {@link Entity}s will be
	 *            converted properly with a tooltip like the one generated by
	...
	public static void chatMessageTranslated(IWDLMessageType type, String translationKey, Object... args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i] instanceof Entity) {
				...
				if (customName != null) {
					component = new ChatComponentTranslation("wdl.messages.entityTypeAndCustomName", entityType,
							customName);
				} else {
				...
				HoverEvent event = new HoverEvent(Action.SHOW_TEXT, new ChatComponentText(exceptionAsString));
	...
	public static void chatMessage(IWDLMessageType type, IChatComponent message) {
		...
		header.getChatStyle().setChatHoverEvent(new HoverEvent(Action.SHOW_TEXT, tooltip));
