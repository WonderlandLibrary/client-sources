...
class Utils {
	 ...
	 *            The amount of space to put below the bottom of the info box.
	 */
	public static void drawGuiInfoBox(String text, int infoBoxWidth, int infoBoxHeight, int guiWidth, int guiHeight,
			int bottomPadding) {
		if (text == null) {
			return;
		}
	...
	/**
	 * Converts a string into a list of lines that are each shorter than the given
	 * width. Takes \n into consideration.
	 * 
	 * @param s
	...
	/**
	 * Draws the border used by list GUIs. <br/>
	 * Based off of
	 * {@link net.minecraft.client.gui.GuiSlot#drawScreen(int, int, float)}.
	 * 
	 * Note that there is an additional 4-pixel padding on the margins for the
	 * gradient.
	 * 
	 * @param topMargin
	 *            Amount of space to give for the upper box.
	 * @param bottomMargin
	 *            Amount of space to give for the lower box.
	 * @param top
	 *            Where to start drawing (usually, 0)
	 * @param left
	 *            Where to start drawing (usually, 0)
	 * @param bottom
	 *            Where to stop drawing (usually, height).
	 * @param right
	 *            Where to stop drawing (usually, width)
	 */
	public static void drawListBackground(int topMargin, int bottomMargin, int top, int left, int bottom, int right) {
		...
		wr.addVertexWithUV(0, bottom, 0, 0 / textureSize, bottom / textureSize);
		wr.addVertexWithUV(right, bottom, 0, right / textureSize, bottom / textureSize);
		wr.addVertexWithUV(right, top, 0, right / textureSize, top / textureSize);
		wr.addVertexWithUV(left, top, 0, left / textureSize, top / textureSize);
		t.draw();
	...
	public static void drawBorder(int topMargin, int bottomMargin, int top, int left, int bottom, int right) {
		...
		wr.addVertexWithUV(left, upperBoxEnd, 0.0D, 0.0D, upperBoxEnd / textureSize);
		wr.addVertexWithUV(right, upperBoxEnd, 0.0D, right / textureSize, upperBoxEnd / textureSize);
		wr.func_178974_a(0x404040, 255);
		wr.addVertexWithUV(right, top, 0.0D, right / textureSize, top / textureSize);
		...
		wr.addVertexWithUV(right, bottom, 0.0D, right / textureSize, bottom / textureSize);
		wr.func_178974_a(0x404040, 255);
		wr.addVertexWithUV(right, lowerBoxStart, 0.0D, right / textureSize, lowerBoxStart / textureSize);
		wr.addVertexWithUV(left, lowerBoxStart, 0.0D, 0.0D, lowerBoxStart / textureSize);
		t.draw();
	 ...
	 * 
	 * @param mouseX
	 *            The mouse's current (scaled) x.
	 * @param mouseY
	 *            The mouse's current (scaled) y.
	 * @param textBox
	 *            The text box.
	...
	public static boolean isMouseOverTextBox(int mouseX, int mouseY, GuiTextField textBox) {
		...
		return scaledX >= 0 && scaledX < textBox.getWidth() && scaledY >= 0 && scaledY < height;
	}
...
class GuiSlider extends GuiButton {
	...
	protected void mouseDragged(Minecraft mc, int mouseX, int mouseY) {
		if (this.visible) {
			...
			if (this.enabled) {
				this.drawTexturedModalRect(this.xPosition + (int) (this.sliderValue * (this.width - 8)), this.yPosition,
						0, 66, 4, 20);
...
class LinkEntry extends TextEntry {
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
		...
		if (relativeX >= 0 && relativeX <= textWidth && relativeY >= 0 && relativeY <= slotHeight) {
			int drawX = mouseX - 2;
			if (drawX + linkWidth + 4 > listWidth + x) {
				drawX = listWidth + x - (4 + linkWidth);
			}
			Gui.drawRect(drawX, mouseY - 2, drawX + linkWidth + 4, mouseY + mc.fontRendererObj.FONT_HEIGHT + 2,
					0x80000000);
