...
public class GuiWDLPermissionRequest extends GuiScreen {
	...
	public void initGui() {
		...
		for (Map.Entry<String, String> request : WDLPluginChannels.getRequests().entrySet()) {
			list.addLine("Requesting '" + request.getKey() + "' to be '" + request.getValue() + "'.");
		}

		this.requestField = new GuiTextField(0, fontRendererObj, width / 2 - 155, 18, 150, 20);
			...
			if (request.contains("=")) {
				...
				if (requestData.length == 2) {
					...
					if (isValid && keyCode == Keyboard.KEY_RETURN) {
						...
						list.addLine("Requesting '" + key + "' to be '" + value + "'.");
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, "Permission request", this.width / 2, 8, 0xFFFFFF);
