...
/**
 * GUI screen shown while the world is being saved. <br/>
 * Based off of vanilla minecraft's
 * {@link net.minecraft.client.gui.GuiScreenWorking GuiScreenWorking}.
 */
public class GuiWDLSaveProgress extends GuiTurningCameraBase implements IBackupProgressMonitor {
	 ...
	 *            The title.
	 * @param taskCount
	 *            The total number of major tasks that there will be.
	 */
	 ...
	 * 
	 * @param message
	 *            The message -- should be something like "saving chunk at x,z"; the
	 *            current position and maximum and the percent are automatically
	 *            appended after it.
	 */
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (this.doneWorking) {
			...
			if (majorTaskCount > 1) {
				majorTaskInfo = I18n.format("wdl.gui.saveProgress.progressInfo", majorTaskMessage, majorTaskNumber,
						majorTaskCount);
			}
			String minorTaskInfo = minorTaskMessage;
			if (minorTaskMaximum > 1) {
				majorTaskInfo = I18n.format("wdl.gui.saveProgress.progressInfo", minorTaskMessage, minorTaskProgress,
						minorTaskMaximum);
			}

			this.drawCenteredString(this.fontRendererObj, this.title, this.width / 2, 8, 0xFFFFFF);

			this.drawCenteredString(this.fontRendererObj, majorTaskInfo, this.width / 2, 100, 0xFFFFFF);

			if (minorTaskMaximum > 0) {
				this.drawProgressBar(110, 84, 89, (majorTaskNumber * minorTaskMaximum) + minorTaskProgress,
						(majorTaskCount + 1) * minorTaskMaximum);
			} else {
				this.drawProgressBar(110, 84, 89, majorTaskNumber, majorTaskCount);
			}

			this.drawCenteredString(this.fontRendererObj, minorTaskInfo, this.width / 2, 130, 0xFFFFFF);
	...
	/**
	 * Draws a progress bar on the screen. (A lot of things are always kept the same
	 * and thus aren't arguments, such as x-position being the center of the
	 * screen).
	 * 
	 * @param y
