...
public class GuiWDLOverwriteChanges extends GuiTurningCameraBase implements IBackupProgressMonitor {
	private class BackupThread extends Thread {
		...
		public void run() {
			try {
				String backupName = WDL.getWorldFolderName(WDL.worldName) + "_" + folderDateFormat.format(new Date())
						+ "_user" + (zip ? ".zip" : "");

				if (zip) {
					backupData = I18n.format("wdl.gui.overwriteChanges.backingUp.zip", backupName);
				} else {
					backupData = I18n.format("wdl.gui.overwriteChanges.backingUp.folder", backupName);
				}
	...
	public void initGui() {
		...
		backupAsZipButton = new GuiButton(0, x, y, I18n.format("wdl.gui.overwriteChanges.asZip.name"));
		this.buttonList.add(backupAsZipButton);
		y += 22;
		backupAsFolderButton = new GuiButton(1, x, y, I18n.format("wdl.gui.overwriteChanges.asFolder.name"));
		this.buttonList.add(backupAsFolderButton);
		y += 22;
		downloadNowButton = new GuiButton(2, x, y, I18n.format("wdl.gui.overwriteChanges.startNow.name"));
		this.buttonList.add(downloadNowButton);
		y += 22;
		cancelButton = new GuiButton(3, x, y, I18n.format("wdl.gui.overwriteChanges.cancel.name"));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (this.backingUp) {
			...
			drawCenteredString(fontRendererObj, backingUpTitle, width / 2, height / 4 - 40, 0xFFFFFF);
			drawCenteredString(fontRendererObj, backupData, width / 2, height / 4 - 10, 0xFFFFFF);
			if (backupFile != null) {
				String text = I18n.format("wdl.gui.overwriteChanges.backingUp.progress", backupCurrent, backupCount,
						backupFile);
				drawCenteredString(fontRendererObj, text, width / 2, height / 4 + 10, 0xFFFFFF);
			}
		} else {
			...
			drawCenteredString(fontRendererObj, footer, width / 2, height - 8 - fontRendererObj.FONT_HEIGHT, 0xFFFFFF);

			drawRect(infoBoxX - 5, infoBoxY - 5, infoBoxX + infoBoxWidth + 5, infoBoxY + infoBoxHeight + 5, 0xB0000000);

			drawCenteredString(fontRendererObj, captionTitle, width / 2, infoBoxY, 0xFFFFFF);
			drawCenteredString(fontRendererObj, captionSubtitle, width / 2, infoBoxY + fontRendererObj.FONT_HEIGHT,
					0xFFFFFF);
