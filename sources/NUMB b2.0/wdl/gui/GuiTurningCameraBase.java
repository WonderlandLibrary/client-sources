...
/**
 * Base GUI with the player in the background turning slowly to show the entire
 * world.
 */
public abstract class GuiTurningCameraBase extends GuiScreen {
	...
	/**
	 * The previous render view entity (the entity which Minecraft uses for the
	 * camera)
	 */
	...
	public void initGui() {
		if (!initializedCamera) {
			this.cam = new EntityPlayerSP(WDL.minecraft, WDL.worldClient, WDL.thePlayer.sendQueue,
					WDL.thePlayer.getStatFileWriter());
			this.cam.setLocationAndAngles(WDL.thePlayer.posX, WDL.thePlayer.posY - WDL.thePlayer.getYOffset(),
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.yaw = (float) (this.yaw
				+ rotationSpeed * (1.0D + 0.699999988079071D * Math.cos((this.yaw + 45.0F) / 45.0D * Math.PI)));
