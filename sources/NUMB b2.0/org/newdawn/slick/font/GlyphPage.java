...
public class GlyphPage {
	 ...
	 * Loaded glyphs are removed from the list.
	 * 
	 ...
	 *            This is the maximum number of glyphs to load from the list. Set to
	 *            -1 to attempt to load all the glyphs.
	...
	public int loadGlyphs(List glyphs, int maxGlyphsToLoad) throws SlickException {
		if (rowHeight != 0 && maxGlyphsToLoad == -1) {
			// If this page has glyphs and we are not loading incrementally, return zero if
		...
		for (Iterator iter = getIterator(glyphs); iter.hasNext();) {
			...
			if (i == maxGlyphsToLoad) {
				// If loading incrementally, flip orderAscending so it won't change, since we'll
				// probably load the rest next time.
		...
		// Every other batch of glyphs added to a page are sorted the opposite way to
		// attempt to keep same size glyps together.
		orderAscending = !orderAscending;
