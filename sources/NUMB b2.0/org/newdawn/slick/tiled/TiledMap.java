...
public class TiledMap {
	...
	/**
	 * Get a property given to the map. Note that this method will not perform well
	 * and should not be used as part of the default code path in the game loop.
	 * 
	 * @param propertyName
	 ...
	 * @param def
	 *            The default value to return
	 * @return The value assigned to the property on the layer (or the default value
	 *         if none is supplied)
	 */
	 ...
	 * @param def
	 *            The default value to return
	 * @return The value assigned to the property on the tile (or the default value
	 *         if none is supplied)
	 */
	 ...
	 * @param lineByLine
	 *            True if we should render line by line, i.e. giving us a chance to
	 *            render something else between lines (@see
	 ...
	 * @param layer
	 *            if this is null all layers are rendered, if not only the selected
	 *            layer is renderered
	 * @param lineByLine
	 *            True if we should render line by line, i.e. giving us a chance to
	 *            render something else between lines (@see
	 *            {@link #renderedLine(int, int, int)}
	 * 
	 *            TODO: [Isometric map] Render stuff between lines, concept of line
	 *            differs from ortho maps
	 */
	protected void renderIsometricMap(int x, int y, int sx, int sy, int width, int height, Layer layer,
		...
		while (!allProcessed) {
			...
			int min = 0;
			if (height > width)
				min = (startLineTileY < width - 1) ? startLineTileY
						: (width - currentTileX < height) ? width - currentTileX - 1 : width - 1;
			else
				min = (startLineTileY < height - 1) ? startLineTileY
	 ...
	 * @return The value of the property with the given name or def, if there is no
