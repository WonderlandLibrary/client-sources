...
public class WorldServer extends World implements IThreadListener {
	...
	private static final List bonusChestContent = Lists
	...
	public World init() {
		...
		if (this.worldInfo.func_176134_F() > 0L) {
			this.getWorldBorder().setTransition(this.worldInfo.func_176137_E(), this.worldInfo.func_176132_G(),
					this.worldInfo.func_176134_F());
		} else {
	...
	public void tick() {
		...
		if (this.getGameRules().getGameRuleBooleanValue("doMobSpawning")
			...
			this.field_175742_R.findChunksForSpawning(this, this.spawnHostileMobs, this.spawnPeacefulMobs,
	...
	public BiomeGenBase.SpawnListEntry func_175734_a(EnumCreatureType p_175734_1_, BlockPos p_175734_2_) {
		...
		return var3 != null && !var3.isEmpty()
				? (BiomeGenBase.SpawnListEntry) WeightedRandom.getRandomItem(this.rand, var3)
				: null;
	}

	public boolean func_175732_a(EnumCreatureType p_175732_1_, BiomeGenBase.SpawnListEntry p_175732_2_,
			...
			for (Iterator var3 = this.activeChunkSet.iterator(); var3.hasNext(); this.theProfiler.endSection()) {
				...
				if (this.rand.nextInt(100000) == 0 && this.isRaining() && this.isThundering()) {
					...
					if (this.func_175727_C(var9)) {
						this.addWeatherEffect(new EntityLightningBolt(this, (double) var9.getX(), (double) var9.getY(),
								(double) var9.getZ()));
					}
				}
	...
	protected BlockPos func_175736_a(BlockPos p_175736_1_) {
		...
		AxisAlignedBB var3 = (new AxisAlignedBB(var2, new BlockPos(var2.getX(), this.getHeight(), var2.getZ())))
		...
		List var4 = this.func_175647_a(EntityLivingBase.class, var3, new Predicate() {
			...
			public boolean func_180242_a(EntityLivingBase p_180242_1_) {
				return p_180242_1_ != null && p_180242_1_.isEntityAlive()
						&& WorldServer.this.isAgainstSky(p_180242_1_.getPosition());
			}
	...
	public void func_175654_a(BlockPos p_175654_1_, Block p_175654_2_, int p_175654_3_, int p_175654_4_) {
		...
		if (this.scheduledUpdatesAreImmediate && p_175654_2_.getMaterial() != Material.air) {
			if (p_175654_2_.requiresUpdates()) {
				...
				if (this.isAreaLoaded(var5.field_180282_a.add(-var6, -var6, -var6),
						var5.field_180282_a.add(var6, var6, var6))) {
		...
		if (this.worldInfo.getTerrainType() == WorldType.DEBUG_WORLD) {
			...
			if (var2 != this.pendingTickListEntriesHashSet.size()) {
				...
				while (var11.hasNext()) {
					...
					if (this.isAreaLoaded(var4.field_180282_a.add(-var5, -var5, -var5),
							var4.field_180282_a.add(var5, var5, var5))) {
						IBlockState var6 = this.getBlockState(var4.field_180282_a);

						if (var6.getBlock().getMaterial() != Material.air
								&& Block.isEqualTo(var6.getBlock(), var4.func_151351_a())) {
							try {
	...
	public List func_175712_a(StructureBoundingBox p_175712_1_, boolean p_175712_2_) {
		...
		for (int var4 = 0; var4 < 2; ++var4) {
			...
			while (var5.hasNext()) {
				...
				if (var7.getX() >= p_175712_1_.minX && var7.getX() < p_175712_1_.maxX && var7.getZ() >= p_175712_1_.minZ
						&& var7.getZ() < p_175712_1_.maxZ) {
					if (p_175712_2_) {
	...
	public List func_147486_a(int p_147486_1_, int p_147486_2_, int p_147486_3_, int p_147486_4_, int p_147486_5_,
		...
		for (int var8 = 0; var8 < this.loadedTileEntityList.size(); ++var8) {
			...
			if (var10.getX() >= p_147486_1_ && var10.getY() >= p_147486_2_ && var10.getZ() >= p_147486_3_
					&& var10.getX() < p_147486_4_ && var10.getY() < p_147486_5_ && var10.getZ() < p_147486_6_) {
				var7.add(var9);
			}
		}
	...
	public boolean isBlockModifiable(EntityPlayer p_175660_1_, BlockPos p_175660_2_) {
		return !this.mcServer.isBlockProtected(this, p_175660_2_, p_175660_1_)
				&& this.getWorldBorder().contains(p_175660_2_);
	}
	...
	public boolean addWeatherEffect(Entity p_72942_1_) {
		if (super.addWeatherEffect(p_72942_1_)) {
			this.mcServer.getConfigurationManager().sendToAllNear(p_72942_1_.posX, p_72942_1_.posY, p_72942_1_.posZ,
					512.0D, this.provider.getDimensionId(), new S2CPacketSpawnGlobalEntity(p_72942_1_));
			return true;
		} else {
			return false;
		}
			...
			if (var13.getDistanceSq(p_72885_2_, p_72885_4_, p_72885_6_) < 4096.0D) {
				((EntityPlayerMP) var13).playerNetServerHandler
						.sendPacket(new S27PacketExplosion(p_72885_2_, p_72885_4_, p_72885_6_, p_72885_8_,
								var11.func_180343_e(), (Vec3) var11.func_77277_b().get(var13)));
			}
		}
		...
		while (!this.field_147490_S[this.blockEventCacheIndex].isEmpty()) {
			...
			while (var2.hasNext()) {
				...
				if (this.func_147485_a(var3)) {
					this.mcServer.getConfigurationManager().sendToAllNear((double) var3.func_180328_a().getX(),
							(double) var3.func_180328_a().getY(), (double) var3.func_180328_a().getZ(), 64.0D,
							this.provider.getDimensionId(), new S24PacketBlockAction(var3.func_180328_a(),
	...
	private boolean func_147485_a(BlockEventData p_147485_1_) {
		...
		return var2.getBlock() == p_147485_1_.getBlock() ? var2.getBlock().onBlockEventReceived(this,
				p_147485_1_.func_180328_a(), var2, p_147485_1_.getEventID(), p_147485_1_.getEventParameter()) : false;
	}
	...
	public void func_175739_a(EnumParticleTypes p_175739_1_, double p_175739_2_, double p_175739_4_, double p_175739_6_,
			int p_175739_8_, double p_175739_9_, double p_175739_11_, double p_175739_13_, double p_175739_15_,
			int... p_175739_17_) {
		this.func_180505_a(p_175739_1_, false, p_175739_2_, p_175739_4_, p_175739_6_, p_175739_8_, p_175739_9_,
				p_175739_11_, p_175739_13_, p_175739_15_, p_175739_17_);
	}

	public void func_180505_a(EnumParticleTypes p_180505_1_, boolean p_180505_2_, double p_180505_3_,
			double p_180505_5_, double p_180505_7_, int p_180505_9_, double p_180505_10_, double p_180505_12_,
			double p_180505_14_, double p_180505_16_, int... p_180505_18_) {
		S2APacketParticles var19 = new S2APacketParticles(p_180505_1_, p_180505_2_, (float) p_180505_3_,
				(float) p_180505_5_, (float) p_180505_7_, (float) p_180505_10_, (float) p_180505_12_,
				(float) p_180505_14_, (float) p_180505_16_, p_180505_9_, p_180505_18_);
