...
public enum EnumConnectionState {
	...
	protected EnumConnectionState registerPacket(EnumPacketDirection direction, Class packetClass) {
		...
		if (((BiMap) var3).containsValue(packetClass)) {
			String var4 = direction + " packet " + packetClass + " is already known to ID "
					+ ((BiMap) var3).inverse().get(packetClass);
	...
	static {
		...
		for (int var2 = 0; var2 < var1; ++var2) {
			...
			while (var4.hasNext()) {
				...
				for (Iterator var6 = ((BiMap) var3.directionMaps.get(var5)).values().iterator(); var6
						.hasNext(); STATES_BY_CLASS.put(var7, var3)) {
					var7 = (Class) var6.next();

					if (STATES_BY_CLASS.containsKey(var7) && STATES_BY_CLASS.get(var7) != var3) {
						throw new Error("Packet " + var7 + " is already assigned to protocol "
								+ STATES_BY_CLASS.get(var7) + " - can\'t reassign to " + var3);
					}
