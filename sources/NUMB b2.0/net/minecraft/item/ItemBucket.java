		...
		if (this.isFull == Blocks.air) {
			...
			if (!worldIn.isAirBlock(p_180616_2_) && !var4) {
				...
				if (worldIn.provider.func_177500_n() && this.isFull == Blocks.flowing_water) {
					...
					worldIn.playSoundEffect((double) ((float) var5 + 0.5F), (double) ((float) var6 + 0.5F),
							(double) ((float) var7 + 0.5F), "random.fizz", 0.5F,
							2.6F + (worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.8F);

					for (int var8 = 0; var8 < 8; ++var8) {
						worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, (double) var5 + Math.random(),
								(double) var6 + Math.random(), (double) var7 + Math.random(), 0.0D, 0.0D, 0.0D,
								new int[0]);
					}
				} else {
