...
public class BlockSnow extends Block {
	...
	public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
		...
		return new AxisAlignedBB((double) pos.getX() + this.minX, (double) pos.getY() + this.minY,
				...
				(double) ((float) pos.getY() + (float) var4 * var5), (double) pos.getZ() + this.maxZ);
	}
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		...
		return var4 != Blocks.ice && var4 != Blocks.packed_ice ? (var4.getMaterial() == Material.leaves ? true
				: (var4 == this && ((Integer) var3.getValue(LAYERS_PROP)).intValue() == 7 ? true
	...
	public void harvestBlock(World worldIn, EntityPlayer playerIn, BlockPos pos, IBlockState state, TileEntity te) {
		spawnAsEntity(worldIn, pos,
				new ItemStack(Items.snowball, ((Integer) state.getValue(LAYERS_PROP)).intValue() + 1, 0));
