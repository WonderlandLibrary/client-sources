...
public class Shaders {
			...
			if (!skipPrint) {
				if (errorCode == 1286) {
					...
					System.err.format("GL error 0x%04X: %s (Fb status 0x%04X) at %s\n",
							new Object[] { Integer.valueOf(errorCode), GLU.gluErrorString(errorCode),
	...
	public static void init() {
		...
		if (!isShaderPackInitialized) {
			...
			if (usedDrawBuffers > maxDrawBuffers) {
				printChatAndLogError("[Shaders] Error: Not enough draw buffers, needed: " + usedDrawBuffers
						+ ", available: " + maxDrawBuffers);
			}
			...
			for (var12 = 0; var12 < 33; ++var12) {
				...
				for (var13 = var12; programsID[var13] == 0
						&& programBackups[var13] != var13; var13 = programBackups[var13]) {
					;
				}
						...
						if (!line.matches("#version .*")) {
							if (line.matches("uniform [ _a-zA-Z0-9]+ shadow;.*")) {
								...
								usedColorBuffers = 8;
									...
									&& line.matches("uniform [ _a-zA-Z0-9]+ centerDepthSmooth;.*")) {
								...
								if (line.matches("/\\* SHADOWRES:[0-9]+ \\*/.*")) {
									...
									shadowMapIsOrtho = true;
										...
										"[ \t]*const[ \t]*float[ \t]*shadowDistance[ \t]*=[ \t]*-?[0-9.]+f?;.*")) {
									e = line.split("(=[ \t]*|;)");
	...
	private static void setupFrameBuffer() {
		...
		for (status = 0; status < usedColorBuffers; ++status) {
			...
			GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, gbuffersFormat[status], renderWidth, renderHeight, 0, GL12.GL_BGRA,
		...
		if (usedShadowDepthBuffers != 0) {
			...
			for (status = 0; status < usedShadowColorBuffers; ++status) {
				...
				GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA, shadowMapWidth, shadowMapHeight, 0, GL12.GL_BGRA,
						GL12.GL_UNSIGNED_INT_8_8_8_8_REV, (ByteBuffer) null);
				EXTFramebufferObject.glFramebufferTexture2DEXT(36160, 36064 + status, 3553,
						sfbColorTextures.get(status), 0);
				checkGLError("FT sc");
			}
	...
	public static void beginRender(Minecraft minecraft, float partialTicks, long finishTimeNano) {
		...
		float i = (float) Math.exp(Math.log(0.5D) * (double) renderViewEntity
				/ (double) (wetness < rainStrength ? drynessHalfLife : wetnessHalfLife));
	...
	public static void setCameraShadow(float partialTicks) {
		...
		float angleInterval = shadowAngleInterval > 0.0F ? angle % shadowAngleInterval - shadowAngleInterval * 0.5F
				: 0.0F;
	...
	public static void renderCompositeFinal() {
		if (!isShadowPass) {
			...
			for (maskR = 0; maskR < usedColorBuffers; ++maskR) {
				EXTFramebufferObject.glFramebufferTexture2DEXT(36160, 36064 + maskR, 3553, dfbColorTexturesA[8 + maskR],
						0);
			}
	...
	public static void readCenterDepth() {
		if (!isShadowPass && centerDepthSmoothEnabled) {
			...
			float fadeFactor = (float) Math
					.exp(Math.log(0.5D) * (double) fadeScalar / (double) centerDepthSmoothHalflife);
	...
	static {
		...
		gbufferFormatPattern = Pattern
				.compile("[ \t]*const[ \t]*int[ \t]*(\\w+)Format[ \t]*=[ \t]*([RGBA81632FUI_SNORM]*)[ \t]*;.*");
		gbufferMipmapEnabledPattern = Pattern
				.compile("[ \t]*const[ \t]*bool[ \t]*(\\w+)MipmapEnabled[ \t]*=[ \t]*true[ \t]*;.*");
		formatNames = new String[] { "R8", "RG8", "RGB8", "RGBA8", "R8_SNORM", "RG8_SNORM", "RGB8_SNORM", "RGBA8_SNORM",
				"R16", "RG16", "RGB16", "RGBA16", "R16_SNORM", "RG16_SNORM", "RGB16_SNORM", "RGBA16_SNORM", "R32F",
				"RG32F", "RGB32F", "RGBA32F", "R32I", "RG32I", "RGB32I", "RGBA32I", "R32UI", "RG32UI", "RGB32UI",
				"RGBA32UI" };
		formatIds = new int[] { 33321, 33323, 32849, 32856, 36756, 36757, 36758, 36759, 33322, 33324, 32852, 32859,
				36760, 36761, 36762, 36763, 33326, 33328, 34837, 34836, 33333, 33339, 36227, 36226, 33334, 33340, 36209,
				36208 };
