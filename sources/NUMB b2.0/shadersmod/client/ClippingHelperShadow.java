...
public class ClippingHelperShadow extends ClippingHelper {
	...
	public boolean isBoxInFrustum(double x1, double y1, double z1, double x2, double y2, double z2) {
		for (int index = 0; index < this.shadowClipPlaneCount; ++index) {
			...
			if (this.dot4(plane, x1, y1, z1) <= 0.0D && this.dot4(plane, x2, y1, z1) <= 0.0D
					&& this.dot4(plane, x1, y2, z1) <= 0.0D && this.dot4(plane, x2, y2, z1) <= 0.0D
					&& this.dot4(plane, x1, y1, z2) <= 0.0D && this.dot4(plane, x2, y1, z2) <= 0.0D
					&& this.dot4(plane, x1, y2, z2) <= 0.0D && this.dot4(plane, x2, y2, z2) <= 0.0D) {
				return false;
			}
		}
	...
	private void makeShadowPlane(float[] shadowPlane, float[] positivePlane, float[] negativePlane, float[] vecSun) {
		...
		float disSN = this.distance(shadowPlane[0], shadowPlane[1], shadowPlane[2], negativePlane[0] * dotSN,
				negativePlane[1] * dotSN, negativePlane[2] * dotSN);
		float disPN = this.distance(positivePlane[0], positivePlane[1], positivePlane[2], negativePlane[0] * dotPN,
				negativePlane[1] * dotPN, negativePlane[2] * dotPN);
		float k1 = disSN / disPN;
		float dotSP = (float) this.dot3(shadowPlane, positivePlane);
		float disSP = this.distance(shadowPlane[0], shadowPlane[1], shadowPlane[2], positivePlane[0] * dotSP,
				positivePlane[1] * dotSP, positivePlane[2] * dotSP);
		float disNP = this.distance(negativePlane[0], negativePlane[1], negativePlane[2], positivePlane[0] * dotPN,
	...
	public void init() {
		...
		this.assignPlane(this.frustum[0], matMP[3] - matMP[0], matMP[7] - matMP[4], matMP[11] - matMP[8],
				matMP[15] - matMP[12]);
		this.assignPlane(this.frustum[1], matMP[3] + matMP[0], matMP[7] + matMP[4], matMP[11] + matMP[8],
				matMP[15] + matMP[12]);
		this.assignPlane(this.frustum[2], matMP[3] + matMP[1], matMP[7] + matMP[5], matMP[11] + matMP[9],
				matMP[15] + matMP[13]);
		this.assignPlane(this.frustum[3], matMP[3] - matMP[1], matMP[7] - matMP[5], matMP[11] - matMP[9],
				matMP[15] - matMP[13]);
		this.assignPlane(this.frustum[4], matMP[3] - matMP[2], matMP[7] - matMP[6], matMP[11] - matMP[10],
				matMP[15] - matMP[14]);
		this.assignPlane(this.frustum[5], matMP[3] + matMP[2], matMP[7] + matMP[6], matMP[11] + matMP[10],
				matMP[15] + matMP[14]);
			...
			if (test0 > 0.0F) {
				if (test2 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[0],
							this.frustum[2], vecSun);
				}

				if (test3 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[0],
							this.frustum[3], vecSun);
				}

				if (test4 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[0],
							this.frustum[4], vecSun);
				}

				if (test5 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[0],
							this.frustum[5], vecSun);
				}
			}
		}
			...
			if (test2 > 0.0F) {
				if (test0 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[2],
							this.frustum[0], vecSun);
				}

				if (test1 < 0.0F) {
					this.makeShadowPlane(this.shadowClipPlanes[this.shadowClipPlaneCount++], this.frustum[2],
							this.frustum[1], vecSun);
				}
