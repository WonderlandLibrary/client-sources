...
public class CustomColormap implements CustomColors.IColorizer {
	...
	private void readColors() {
		try {
			...
			if (!widthOk || !heightOk) {
				dbg("Non-standard palette size: " + imgWidth + "x" + imgHeight + ", should be: " + this.width + "x"
						+ this.height + ", path: " + e);
			}
	...
	public int getColor(BiomeGenBase biome, BlockPos blockPos) {
		return this.format == 0 ? this.getColorVanilla(biome, blockPos)
				: (this.format == 1 ? this.getColorGrid(biome, blockPos) : this.color);
	}
