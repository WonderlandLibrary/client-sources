...
public class GuiAnimationSettingsOF extends GuiScreen {
	...
	public void initGui() {
		...
		for (int i = 0; i < enumOptions.length; ++i) {
			...
			if (!opt.getEnumFloat()) {
				this.buttonList.add(
						new GuiOptionButtonOF(opt.returnEnumOrdinal(), x, y, opt, this.settings.getKeyBinding(opt)));
			} else {
				this.buttonList.add(new GuiOptionSliderOF(opt.returnEnumOrdinal(), x, y, opt));
			}
		}

		this.buttonList.add(new GuiButton(210, this.width / 2 - 155, this.height / 6 + 168 + 11, 70, 20,
