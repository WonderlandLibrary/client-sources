...
extends Gui {
    ...
    public void printChatMessage(IChatComponent chatComponent) {
        ...
        if (chatComponent.getUnformattedText().contains(Minecraft.getMinecraft().getSession().getUsername()) && (msg = chatComponent.getUnformattedText()).contains("by") && (msg = msg.replace(Minecraft.getMinecraft().getSession().getUsername(), "#")).split("by")[1].contains("#")) {
            ++Wrapper.getMonsoon().getModuleManager().getModule(SessionInfo.class).kills;
        }
        if (chatComponent.getFormattedText().contains("\u00a7aYour new API key is \u00a7r\u00a7b")) {
            String[] original = chatComponent.getFormattedText().split(" ");
            String complete = original[5].replace("\u00a7r\u00a7b", "").trim().replace("\u00a7r", "");
            Wrapper.getModule(AutoHypixel.class).setApiKey(complete);
        }
    }
