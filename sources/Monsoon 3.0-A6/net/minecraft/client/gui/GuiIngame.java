...
import wtf.monsoon.api.util.render.KawesaUtil;
...
extends Gui {
    ...
    Animation openAnimation = new Animation(() -> Float.valueOf(800.0f), false, () -> Easing.CUBIC_IN_OUT);
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                this.getFontRenderer().drawString(this.recordPlaying, (float)(-this.getFontRenderer().getStringWidth(this.recordPlaying) / 2), -4.0f, l + (l1 << 24 & 0xFF000000));
                GlStateManager.disableBlend();
        ...
        EventRender2D eventRender2D = new EventRender2D(scaledresolution, partialTicks, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight());
        float scaleFactor = 1.0f / ((float)scaledresolution.getScaleFactor() * 0.5f);
        RenderUtil.scale(0.0f, 0.0f, new float[]{scaleFactor, scaleFactor}, () -> {
            BlurUtil.blur_shader_2.bindFramebuffer(false);
            if (Wrapper.getModule(HUD.class).blur.getValue().booleanValue()) {
                BlurUtil.blur(Wrapper.getModule(HUD.class).blurStrength.getValue().intValue(), 1.0f);
                KawesaUtil.preBlur();
                Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(module -> module instanceof HUDModule).forEach(module -> {
                    HUDModule hudModule = (HUDModule)module;
                    RenderUtil.scaleXY(hudModule.getX() + hudModule.getWidth() / 2.0f, hudModule.getY() + hudModule.getHeight() / 2.0f, module.getToggleHudAnimation(), hudModule::blur);
                });
                BlurUtil.postBlur(Wrapper.getModule(HUD.class).blurStrength.getValue().intValue(), 1.0f);
            }
            this.mc.getFramebuffer().bindFramebuffer(false);
            Wrapper.getEventBus().post(eventRender2D);
            Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(module -> module instanceof HUDModule).forEach(module -> {
                HUDModule h = (HUDModule)module;
                module.getToggleHudAnimation().setState(module.isEnabled());
                if (!(this.mc.currentScreen instanceof GuiChat)) {
                    h.getHoverAnimation().setState(false);
                    h.getLinearHoverAnimation().setState(false);
                }
                RenderUtil.scaleXY(h.getX() + h.getWidth() / 2.0f, h.getY() + h.getHeight() / 2.0f, module.getToggleHudAnimation(), () -> {
                    h.render();
                    RoundedUtils.outline(h.getX(), h.getY(), h.getWidth(), h.getHeight(), 10.0f, -2.0f + (float)h.getLinearHoverAnimation().getAnimationFactor() * 3.0f, Color.WHITE);
                });
            });
            Wrapper.getNotifManager().render();
            Wrapper.getFont().drawString("cock", -400.0f, -400.0f, Color.WHITE, false);
        });
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        EventRenderHotbar.Pre pre = new EventRenderHotbar.Pre(new ScaledResolution(this.mc));
        Wrapper.getEventBus().post(pre);
        if (pre.isCancelled()) {
            return;
        }
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0f;
            EventRenderHotbar erh = new EventRenderHotbar(sr);
            if (Wrapper.getModule(Hotbar.class).isEnabled()) {
                Wrapper.getEventBus().post(erh);
            } else {
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (this.mc.thePlayer.experienceLevel > 0) {
            ...
            boolean j1 = false;
            this.getFontRenderer().drawString(s, (float)(l1 + 1), (float)i1, 0);
            this.getFontRenderer().drawString(s, (float)(l1 - 1), (float)i1, 0);
            this.getFontRenderer().drawString(s, (float)l1, (float)(i1 + 1), 0);
            this.getFontRenderer().drawString(s, (float)l1, (float)(i1 - 1), 0);
            this.getFontRenderer().drawString(s, (float)l1, (float)i1, k1);
            this.mc.mcProfiler.endSection();
        }
    }
    ...
    protected boolean showCrosshair() {
        if (this.mc.gameSettings.thirdPersonView > 0 || Wrapper.getModule(CrosshairCustomizer.class).isEnabled()) {
            return false;
        }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        ArrayList collection = scoreboard.getSortedScores(p_180475_1_);
        ...
        EventRenderScoreboard event = new EventRenderScoreboard(scoreboard, p_180475_2_, p_180475_2_.getScaledHeight() / 2 + collection.size() * this.getFontRenderer().FONT_HEIGHT / 2);
        ...
        for (Score score1 : collection) {
            ...
            this.getFontRenderer().drawString(s1, (float)l1, (float)k, 0x20FFFFFF);
            this.getFontRenderer().drawString(s2, (float)(l - this.getFontRenderer().getStringWidth(s2)), (float)k, 0x20FFFFFF);
            ...
            this.getFontRenderer().drawString(s3, (float)(l1 + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2), (float)(k - this.getFontRenderer().FONT_HEIGHT), 0x20FFFFFF);
        }
    }
    ...
    private void renderBossHealth() {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            this.getFontRenderer().drawStringWithShadow(s, (float)(i / 2 - this.getFontRenderer().getStringWidth(s) / 2), (float)(i1 - 10), 0xFFFFFF);
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }
    ...
    private void renderPumpkinOverlay(ScaledResolution p_180476_1_) {
        ...
        worldrenderer.pos(0.0, p_180476_1_.getScaledHeight(), -90.0).func_181673_a(0.0, 1.0).end();
        worldrenderer.pos(p_180476_1_.getScaledWidth(), p_180476_1_.getScaledHeight(), -90.0).func_181673_a(1.0, 1.0).end();
        worldrenderer.pos(p_180476_1_.getScaledWidth(), 0.0, -90.0).func_181673_a(1.0, 0.0).end();
        worldrenderer.pos(0.0, 0.0, -90.0).func_181673_a(0.0, 0.0).end();
        tessellator.draw();
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
        if (!Config.isVignetteEnabled()) {
            ...
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            worldrenderer.begin(7, DefaultVertexFormats.field_181707_g);
            worldrenderer.pos(0.0, p_180480_2_.getScaledHeight(), -90.0).func_181673_a(0.0, 1.0).end();
            worldrenderer.pos(p_180480_2_.getScaledWidth(), p_180480_2_.getScaledHeight(), -90.0).func_181673_a(1.0, 1.0).end();
            worldrenderer.pos(p_180480_2_.getScaledWidth(), 0.0, -90.0).func_181673_a(1.0, 0.0).end();
            worldrenderer.pos(0.0, 0.0, -90.0).func_181673_a(0.0, 0.0).end();
            tessellator.draw();
            GlStateManager.depthMask(true);
            GlStateManager.enableDepth();
    ...
    private void func_180474_b(float p_180474_1_, ScaledResolution p_180474_2_) {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.field_181707_g);
        worldrenderer.pos(0.0, p_180474_2_.getScaledHeight(), -90.0).func_181673_a(f, f3).end();
        worldrenderer.pos(p_180474_2_.getScaledWidth(), p_180474_2_.getScaledHeight(), -90.0).func_181673_a(f2, f3).end();
        worldrenderer.pos(p_180474_2_.getScaledWidth(), 0.0, -90.0).func_181673_a(f2, f1).end();
        worldrenderer.pos(0.0, 0.0, -90.0).func_181673_a(f, f1).end();
        tessellator.draw();
        GlStateManager.depthMask(true);
        GlStateManager.enableDepth();
        GlStateManager.enableAlpha();
