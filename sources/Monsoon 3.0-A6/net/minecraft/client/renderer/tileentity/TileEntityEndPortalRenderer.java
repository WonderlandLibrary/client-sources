    ...
    public void renderTileEntityAt(TileEntityEndPortal te, double x, double y, double z, float partialTicks, int destroyStage) {
        if (!Config.isShaders() || !ShadersRender.renderEndPortal(te, x, y, z, partialTicks, destroyStage, 0.75f)) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                worldrenderer.pos(x, y + (double)f3, z).func_181666_a(f11, f12, f13, 1.0f).end();
                worldrenderer.pos(x, y + (double)f3, z + 1.0).func_181666_a(f11, f12, f13, 1.0f).end();
                worldrenderer.pos(x + 1.0, y + (double)f3, z + 1.0).func_181666_a(f11, f12, f13, 1.0f).end();
                worldrenderer.pos(x + 1.0, y + (double)f3, z).func_181666_a(f11, f12, f13, 1.0f).end();
                tessellator.draw();
                GlStateManager.popMatrix();
