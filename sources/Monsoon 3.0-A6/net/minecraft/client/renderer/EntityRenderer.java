...
implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        Reach reachModule = Wrapper.getModule(Reach.class);
        HitBox hitboxModule = Wrapper.getModule(HitBox.class);
        Entity entity = this.mc.getRenderViewEntity();
        if (entity != null && this.mc.theWorld != null) {
            double reachToFlag;
            this.mc.mcProfiler.startSection("pick");
            this.mc.pointedEntity = null;
            double reach = reachModule.isEnabled() ? reachModule.blockReach : (double)this.mc.playerController.getBlockReachDistance();
            this.mc.objectMouseOver = entity.rayTrace(reach, partialTicks);
            double blockOrSmth = reach;
            Vec3 vec3 = entity.getPositionEyes(partialTicks);
            boolean flag = false;
            double d = reachToFlag = reachModule.isEnabled() ? (double)Math.round(reachModule.hitReach * 100.0) / 100.0 : 3.0;
            if (this.mc.playerController.extendedReach()) {
                reach = 6.0;
                blockOrSmth = 6.0;
            } else if (reach > reachToFlag) {
                flag = true;
            }
            if (this.mc.objectMouseOver != null) {
                blockOrSmth = this.mc.objectMouseOver.hitVec.distanceTo(vec3);
            }
            Vec3 vec31 = entity.getLook(partialTicks);
            Vec3 vec32 = vec3.addVector(vec31.x * reach, vec31.y * reach, vec31.z * reach);
            this.pointedEntity = null;
            Vec3 vec33 = null;
            float toExpand = 1.0f;
            List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.x * reach, vec31.y * reach, vec31.z * reach).expand(toExpand, toExpand, toExpand), (Predicate<? super Entity>)Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            double d2 = blockOrSmth;
            for (Entity entity1 : list) {
                double d3;
                float f1 = entity1.getCollisionBorderSize() + (hitboxModule.isEnabled() ? hitboxModule.expand.getValue().floatValue() / 10.0f : 0.0f);
            ...
            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > reachToFlag) {
                this.pointedEntity = null;
            ...
            if (this.pointedEntity != null && (d2 < blockOrSmth || this.mc.objectMouseOver == null)) {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (Wrapper.getModule(NoRender.class).isEnabled() && Wrapper.getModule(NoRender.class).hurtCam.getValue().booleanValue()) {
            return;
        }
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    float f4 = (i >> 1 & 1) * 2 - 1;
                    float f5 = (i >> 2 & 1) * 2 - 1;
                    MovingObjectPosition movingObjectPosition = this.mc.theWorld.rayTraceBlocks(new Vec3(d0 + (double)(f3 *= 0.1f), d1 + (double)(f4 *= 0.1f), d2 + (double)(f5 *= 0.1f)), new Vec3(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5));
                }
                if (this.mc.gameSettings.thirdPersonView == 2) {
    ...
    public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_) {
        if (!this.debugView) {
            ...
            float fov = this.getFOVModifier(p_renderHand_1_, false);
            EventFOVUpdate event = new EventFOVUpdate(fov);
            Wrapper.getEventBus().post(event);
            Project.gluPerspective((float)event.getNewFOV(), (float)((float)this.mc.displayWidth / (float)this.mc.displayHeight), (float)0.05f, (float)(this.farPlaneDistance * 2.0f));
            GlStateManager.matrixMode(5888);
            GlStateManager.loadIdentity();
            if (this.mc.gameSettings.anaglyph) {
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (Wrapper.getModule(Blur.class) != null && Wrapper.getModule(Blur.class).isEnabled() && (Wrapper.getModule(Blur.class).chat.getValue().booleanValue() || !(this.mc.currentScreen instanceof GuiChat))) {
                BlurUtil.alternateBlur(0.0f, -5.0f, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight() + 5, (int)((double)(2.0f * Wrapper.getModule(Blur.class).guiBlurStrength.getValue().floatValue()) * this.mc.getBlurAnimation().getAnimationFactor()));
            }
            if (BlurUtil.framebuffer != null) {
                BlurUtil.framebuffer.unbindFramebufferTexture();
            }
            RoundedUtils.gradient(0.0f, 0.0f, this.mc.displayWidth, this.mc.displayHeight, 0.0f, (float)(0.5 * this.mc.getGradientAnimation().getAnimationFactor()), ColorUtil.fadeBetween(10, 180, new Color(0, 238, 255, 255), new Color(135, 56, 232, 255)), ColorUtil.fadeBetween(10, 90, new Color(0, 238, 255, 255), new Color(135, 56, 232, 255)), ColorUtil.fadeBetween(10, 270, new Color(0, 238, 255, 255), new Color(135, 56, 232, 255)), ColorUtil.fadeBetween(10, 0, new Color(0, 238, 255, 255), new Color(135, 56, 232, 255)));
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    if (Screen.class.isAssignableFrom(this.mc.currentScreen.getClass())) {
                        Wrapper.getGlgui().render(this.mc.currentScreen);
                        Wrapper.getGlgui().mouseMove(Mouse.getX(), Mouse.getY());
                    }
                }
    ...
    private void renderWorldDirections(float partialTicks) {
        if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI && !this.mc.thePlayer.hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo) {
            ...
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0, 0.0, 0.0, 0.005, 1.0E-4, 1.0E-4), 255, 0, 0, 255);
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0, 0.0, 0.0, 1.0E-4, 1.0E-4, 0.005), 0, 0, 255, 255);
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0, 0.0, 0.0, 1.0E-4, 0.0033, 1.0E-4), 0, 255, 0, 255);
            GlStateManager.popMatrix();
            ...
            for (int k1 = k - i1; k1 <= k + i1; ++k1) {
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15f) {
                        ...
                        int k3 = j3 >> 16 & 0xFFFF;
                        int l3 = j3 & 0xFFFF;
                        worldrenderer.pos((double)l1 - d3 + 0.5, k2, (double)k1 - d4 + 0.5).func_181673_a(0.0, (double)k2 * 0.25 + d5).func_181666_a(1.0f, 1.0f, 1.0f, f3).func_181671_a(k3, l3).end();
                        worldrenderer.pos((double)l1 + d3 + 0.5, k2, (double)k1 + d4 + 0.5).func_181673_a(1.0, (double)k2 * 0.25 + d5).func_181666_a(1.0f, 1.0f, 1.0f, f3).func_181671_a(k3, l3).end();
                        worldrenderer.pos((double)l1 + d3 + 0.5, l2, (double)k1 + d4 + 0.5).func_181673_a(1.0, (double)l2 * 0.25 + d5).func_181666_a(1.0f, 1.0f, 1.0f, f3).func_181671_a(k3, l3).end();
                        worldrenderer.pos((double)l1 - d3 + 0.5, l2, (double)k1 - d4 + 0.5).func_181673_a(0.0, (double)l2 * 0.25 + d5).func_181666_a(1.0f, 1.0f, 1.0f, f3).func_181671_a(k3, l3).end();
                        continue;
                    }
                    if (j1 != 1) {
                        if (j1 >= 0) {
                    ...
                    int j4 = i4 >> 16 & 0xFFFF;
                    int k4 = i4 & 0xFFFF;
                    worldrenderer.pos((double)l1 - d3 + 0.5, k2, (double)k1 - d4 + 0.5).func_181673_a(0.0 + d9, (double)k2 * 0.25 + d8 + d10).func_181666_a(1.0f, 1.0f, 1.0f, f4).func_181671_a(j4, k4).end();
                    worldrenderer.pos((double)l1 + d3 + 0.5, k2, (double)k1 + d4 + 0.5).func_181673_a(1.0 + d9, (double)k2 * 0.25 + d8 + d10).func_181666_a(1.0f, 1.0f, 1.0f, f4).func_181671_a(j4, k4).end();
                    worldrenderer.pos((double)l1 + d3 + 0.5, l2, (double)k1 + d4 + 0.5).func_181673_a(1.0 + d9, (double)l2 * 0.25 + d8 + d10).func_181666_a(1.0f, 1.0f, 1.0f, f4).func_181671_a(j4, k4).end();
                    worldrenderer.pos((double)l1 - d3 + 0.5, l2, (double)k1 - d4 + 0.5).func_181673_a(0.0 + d9, (double)l2 * 0.25 + d8 + d10).func_181666_a(1.0f, 1.0f, 1.0f, f4).func_181671_a(j4, k4).end();
                }
            }
            if (j1 >= 0) {
                tessellator.draw();
