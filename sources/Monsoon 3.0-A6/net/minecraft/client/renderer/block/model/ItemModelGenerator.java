...
public class ItemModelGenerator {
    ...
    public ModelBlock makeItemModel(TextureMap textureMapIn, ModelBlock blockModel) {
        ...
        return new ModelBlock(list, (Map<String, String>)map, false, false, blockModel.func_181682_g());
    }

    private List<BlockPart> func_178394_a(int layer, String key, TextureAtlasSprite sprite) {
        HashMap map = Maps.newHashMap();
        map.put(EnumFacing.SOUTH, new BlockPartFace(null, layer, key, new BlockFaceUV(new float[]{0.0f, 0.0f, 16.0f, 16.0f}, 0)));
        map.put(EnumFacing.NORTH, new BlockPartFace(null, layer, key, new BlockFaceUV(new float[]{16.0f, 0.0f, 0.0f, 16.0f}, 0)));
        ArrayList list = Lists.newArrayList();
        list.add(new BlockPart(new Vector3f(0.0f, 0.0f, 7.5f), new Vector3f(16.0f, 16.0f, 8.5f), map, null, true));
        list.addAll(this.func_178397_a(sprite, key, layer));
        return list;
    }

    private List<BlockPart> func_178397_a(TextureAtlasSprite sprite, String key, int layer) {
        float f = sprite.getIconWidth();
        float f1 = sprite.getIconHeight();
        ArrayList list = Lists.newArrayList();
        for (Span itemmodelgenerator$span : this.func_178393_a(sprite)) {
            float f2 = 0.0f;
            float f3 = 0.0f;
            float f4 = 0.0f;
            float f5 = 0.0f;
            ...
            f5 = 16.0f - f5;
            HashMap map = Maps.newHashMap();
            map.put(itemmodelgenerator$spanfacing.getFacing(), new BlockPartFace(null, layer, key, new BlockFaceUV(new float[]{f6 *= f10, f8 *= f11, f7 *= f10, f9 *= f11}, 0)));
            switch (itemmodelgenerator$spanfacing) {
                case UP: {
