...
public abstract class PathNavigate {
    ...
    public void onUpdateNavigation() {
        ...
        if (!this.noPath()) {
            ...
            if (!this.noPath() && (vec32 = this.currentPath.getPosition(this.theEntity)) != null) {
                AxisAlignedBB axisalignedbb1 = new AxisAlignedBB(vec32.x, vec32.y, vec32.z, vec32.x, vec32.y, vec32.z).expand(0.5, 0.5, 0.5);
    ...
    protected void pathFollow() {
        ...
        for (int j = this.currentPath.getCurrentPathIndex(); j < this.currentPath.getCurrentPathLength(); ++j) {
            if (this.currentPath.getPathPointFromIndex((int)j).yCoord == (int)vec3.y) continue;
            i = j;
            break;
        }
