...
extends Drawable {
    ...
    public void draw(float mouseX, float mouseY, int mouseDelta) {
        ...
        RenderUtil.rect(this.getX() + this.getWidth() / 2.0f - 4.0f - Math.max(20.0f, (float)Wrapper.getFontUtil().productSans.getStringWidth(obfPass) / 2.0f - 4.0f) * animVal, this.getY() + this.getHeight() - 5.0f, 8.0f + Math.max(20.0f, (float)Wrapper.getFontUtil().productSans.getStringWidth(obfPass) / 2.0f - 4.0f) * 2.0f * animVal, 1.0f, ColorUtil.interpolate(background, fade1, this.focused.getAnimationFactor() / 2.0));
        Wrapper.getFontUtil().productSans.drawCenteredString(obfPass, this.getX() + this.getWidth() / 2.0f, this.getY() + this.getHeight() / 2.0f - (float)Wrapper.getFontUtil().productSans.getHeight() / 2.0f - 1.0f - animVal + 2.0f, ColorUtil.interpolate(new Color(0x3A3A3A), new Color(0x9D9D9D), this.focused.getAnimationFactor()), false);
    }
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (this.focused.getState()) {
            ...
            if (this.password.length() != 0 && keyCode == 14) {
                this.password = this.password.substring(0, this.password.length() - 1);
            }
            if (GuiScreen.isCtrlKeyDown() && keyCode == 47) {
                try {
                    this.password = (String)Toolkit.getDefaultToolkit().getSystemClipboard().getData(DataFlavor.stringFlavor);
                }
