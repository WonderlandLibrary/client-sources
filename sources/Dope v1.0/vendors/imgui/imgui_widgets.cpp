...
static std::map<ImGuiID, float> button_times{};
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	// Render
	const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : ImGuiCol_Button);
	RenderNavHighlight(bb, id);
	RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);

	constexpr auto easeInOutSine = [](float x)
	{
		return (float)-(cos(IM_PI * x) - 1.f) / 2.f;
	};

	const auto easeOutExpo = [](float x)
	{
		return 1.f - pow(1.f - x, 3.f);
	};

	{
		if (button_times.find(id) == button_times.end())
			button_times.emplace(id, 0.f);

		auto& actualTime = button_times.at(id);

		const bool held = g.ActiveId == id;

		if ((hovered || held) && actualTime < 1.f)
			actualTime += 6.f * g.IO.DeltaTime;
		if (!hovered && !held && actualTime > 0.f)
			actualTime -= 6.f * g.IO.DeltaTime;

		if (actualTime > 0.f)
			RenderFrame(ImVec2{ bb.Min.x, bb.Min.y }, { bb.Min.x + (size.x * easeInOutSine(actualTime)), bb.Min.y + (size.y * easeInOutSine(actualTime)) }, GetColorU32(ImGuiCol_ButtonHovered), false, style.FrameRounding);
	}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
	...
	window->DrawList->AddRectFilled(ImVec2{ bb_frame.Max.x, bb_frame.Min.y }, ImVec2{ bb_frame.Max.x - 3.f, bb_frame.Max.y }, bg_col, window->WindowRounding, flags);
	ImRect grab_rect;
	if (axis == ImGuiAxis_X)
	...
	window->DrawList->AddRectFilled(ImVec2{ bb_frame.Max.x, grab_rect.Min.y - 2.f }, ImVec2{ bb_frame.Max.x - 3.f, grab_rect.Max.y }, GetColorU32(ImGuiCol_CheckMark), 0.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect total_bb(pos, pos + ImVec2(GetFrameHeight() + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
	...
	ImU32 col_bg, col_bg2;
	if (ImGui::IsItemHovered())
	{
		col_bg = ImGui::GetColorU32(ImLerp(ImColor(5, 17, 38, 255), ImVec4(0.01f, 0.09f, 0.17f, 1.f), t));
		col_bg2 = ImGui::GetColorU32(ImLerp(ImColor(200, 200, 200, 255), ImVec4(0.f, 0.55f, 0.95f, 1.f), t));
	}
	else
	{
		col_bg = ImGui::GetColorU32(ImLerp(ImColor(5, 17, 38, 255), ImVec4(0.01f, 0.09f, 0.17f, 1.f), t));
	...
	draw_list->AddRectFilled(ImVec2(p.x, p.y + 4), ImVec2((p.x - 6) + width, p.y + height - 4), GetColorU32(ImVec4(ImColor(26, 28, 30))), height * 0.5f);
	draw_list->AddCircleFilled(ImVec2((p.x - 3) + radius + t * (width - radius * 2.0f), p.y + radius), radius - 2.5f, *v ? GetColorU32(ImGuiCol_CheckMark) : GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.85f)));
	
    if (label_size.x > 0.0f)
		RenderText(ImVec2((p.x + width) + style.ItemInnerSpacing.x, (p.y + 4) - style.FramePadding.y), label);

	return pressed;
}

static std::map<ImGuiID, float> check_times{};
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
	...
	{
		if (checkhover_times.find(id) == checkhover_times.end())
			checkhover_times.emplace(id, 0.f);

		auto& actualTime = checkhover_times.at(id);

		if (hovered && actualTime < 1.f)
			actualTime += 0.05f;
		if (!hovered && actualTime > 0.f)
			actualTime -= 0.05f;

		if (actualTime > 0.f)
		{
			RenderFrame(ImVec2{ check_bb.Min.x, check_bb.Max.y - (square_sz * easeInOutSine(actualTime)) }, check_bb.Max, GetColorU32(ImVec4(ImColor(181, 198, 240, 10))), false, style.FrameRounding);
		}

		if (label_size.x > 0.0f)
		{
			if (actualTime >= 0.65f) {
				PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f * easeInOutSine(actualTime)));
			}
			else {
				PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.65f));
			}
	...
	//{
	//	if (check_times.find(id) == check_times.end())
	//		check_times.emplace(id, 0.f);

	//	auto& actualTime = check_times.at(id);
	...
	//		else
	//		{
	//			if ((*v && actualTime < 1.f) || (!*v && actualTime >= 1.f))
	//				actualTime += 0.05f;

	//			const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
	//			RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
	//			//RenderFrame(ImVec2{ check_bb.Max.x - (square_sz * easeInOutSine(actualTime)), check_bb.Max.y - (square_sz * easeInOutSine(actualTime)) }, check_bb.Max, GetColorU32(ImGuiCol_CheckMark), false, style.FrameRounding);
	//		}
	...
	//			RenderFrame(check_bb.Min - ImVec2{ pad, pad }, check_bb.Max + ImVec2{ pad, pad }, GetColorU32(ImGuiCol_CheckMark), false, style.FrameRounding);
	//		}*/
	//	}
	//}

    if (*v) {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 4.f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), square_sz - pad * 2.0f, square_sz - pad * 2.0f);
    }
	return pressed;
}
...
static std::map<ImGuiID, float> sliderhover_times{};
static std::map<ImGuiID, float> sliderchange_times{};
static std::map<ImGuiID, float> sliderchange_poses{};
static std::map<ImGuiID, float> sliderchange_newPoses{};
static std::map<ImGuiID, bool> sliderchange_changings{};
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImVec2 label_pos = GetCursorPos();
	SetCursorPos({ 10.f, label_pos.y + 20.f });
	...
	if (!temp_input_is_active)
	{
		const bool focus_requested = temp_input_allowed && (g.CurrentItemFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
		const bool clicked = (hovered && g.IO.MouseClicked[0]);
	...
	//hover anim
	{
		if (sliderhover_times.find(id) == sliderhover_times.end())
			sliderhover_times.emplace(id, 0.f);

		auto& actualTime = sliderhover_times.at(id);
		auto& actualTimeTxt = sliderhover_times.at(id);
        actualTimeTxt = actualTimeTxt < 0.65f ? 0.65f : actualTimeTxt;

		const bool held = g.ActiveId == id;

		if ((hovered || held) && actualTime < 1.f)
			actualTime += 1.f * g.IO.DeltaTime;
		if (!hovered && !held && actualTime > 0.f)
			actualTime -= 1.f * g.IO.DeltaTime;

        if ((hovered || held) && actualTimeTxt < 1.f)
            actualTimeTxt += 0.25f * g.IO.DeltaTime;
        if (!hovered && !held && actualTimeTxt > 0.65f)
            actualTimeTxt -= 0.25f * g.IO.DeltaTime;

		ImVec4 hovercol = ImColor(32, 35, 38);
		...
		if (label_size.x > 0.0f)
		{
            if (actualTimeTxt >= 0.65f) {
                PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f * easeInOutSine(actualTimeTxt)));
            }
            else {
                PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.65f));
            }
            ...
            RenderText(ImVec2(frame_bb.Max.x - 2.f - frame_bb.Max.x + frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y - 25.f), label);
			if (actualTime > 0.f) {
                if (actualTime > 1.f)
                    actualTime = 1.f;

				RenderText(ImVec2((frame_bb.Max.x - frame_bb.Max.x + frame_bb.Min.x) + w - CalcTextSize(value_buf, NULL, true).x - 5.f, frame_bb.Max.y + 25.f - (67.f * easeInOutSine(actualTime))), value_buf);
			}
	...
	// Draw frame
	RenderNavHighlight(frame_bb, id);
	RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2{ 2.f, 0.f }, GetColorU32(ImGuiCol_FrameBg), true, g.Style.FrameRounding);
	...
	{
		if (sliderchange_times.find(id) == sliderchange_times.end())
			sliderchange_times.emplace(id, 0.f);
		if (sliderchange_poses.find(id) == sliderchange_poses.end())
			sliderchange_poses.emplace(id, 0.0f);
		if (sliderchange_newPoses.find(id) == sliderchange_newPoses.end())
			sliderchange_newPoses.emplace(id, 0.0f);

		auto& actualTime = sliderchange_times.at(id);
		auto& actualPos = sliderchange_poses.at(id);
		auto& actualNewPos = sliderchange_newPoses.at(id);

		if (sliderchange_changings.find(id) == sliderchange_changings.end())
		{
			actualTime = 0.f;
			actualNewPos = grab_bb.Max.x;
			actualPos = (actualNewPos - actualPos);
            sliderchange_changings.emplace(id, false);
		}

		auto& actualChanging = sliderchange_changings.at(id);

		if (value_changed || actualChanging)
		{
			actualTime = 0.f;
			actualNewPos = grab_bb.Max.x;
			actualChanging = hovered || value_changed;
		}

		if (actualChanging && actualTime < 1.f)
		{
			actualTime += g.IO.DeltaTime * 6.f;
			actualPos += (actualNewPos - actualPos) * easeOutExpo(actualTime);
		}
		else if (actualTime >= 1.f)
		{
		...
		if (grab_bb.Max.x > grab_bb.Min.x)
			window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 2.f, 2.f }, ImVec2{ actualChanging ? actualPos : grab_bb.Max.x, frame_bb.Max.y - 2.f }, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
	}
	...
	//char value_buf[64];
	//const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
	//RenderTextClipped(ImVec2(frame_bb.Min.x + 70, frame_bb.Min.y - 25), ImVec2(frame_bb.Max.x + 70, frame_bb.Max.y - 25), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderTextClipped({ text_min.x, text_min.y }, { text_max.x, text_max.y }, label, NULL, & label_size, style.SelectableTextAlign, & bb);
    if (selected)
        PopStyleColor(1);
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	while (clipper.Step())
		for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
		{
			...
			bool item_selected_cpy = item_selected;
			if (item_selected_cpy)
				PushStyleColor(ImGuiCol_Text, ImVec4(0.f, 167.f / 255.f, 255.f / 255.f, 1.f));
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (selected)
            RenderCheckMark(window->DrawList, pos + ImVec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetColorU32(ImGuiCol_Text), g.FontSize  * 0.866f,  g.FontSize * 0.866f);
    }
