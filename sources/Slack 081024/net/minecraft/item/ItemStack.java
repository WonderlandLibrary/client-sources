    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced)
    {
            ...
            if (this.stackTagCompound.hasKey("display", 10))
            {
                ...
                if (nbttagcompound.getTagId("Lore") == 9)
                {
                    ...
                    if (nbttaglist1.tagCount() > 0)
                    {
                        ...
                        {
                            list.add(ChatFormatting.DARK_PURPLE + "" + ChatFormatting.ITALIC + nbttaglist1.getStringTagAt(j1));
                        }
                    }
                }
            }
        ...
        if (!multimap.isEmpty() && (i1 & 2) == 0)
        {
            ...
            for (Entry<String, AttributeModifier> entry : multimap.entries())
            {
                ...
                if (d0 > 0.0D)
                {
                    list.add(ChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier.getOperation(), new Object[] {DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + entry.getKey())}));
                }
                else if (d0 < 0.0D)
                {
                    d1 = d1 * -1.0D;
                    list.add(ChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier.getOperation(), new Object[] {DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + entry.getKey())}));
                }
            }
        }
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (i1 & 8) == 0)
        {
            ...
            if (nbttaglist2.tagCount() > 0)
            {
                ...
                for (int k1 = 0; k1 < nbttaglist2.tagCount(); ++k1)
                {
                    ...
                    if (block != null)
                    {
                        list.add(ChatFormatting.DARK_GRAY + block.getLocalizedName());
                    }
                    else
                    {
        ...
        if (advanced)
        {
            ...
            list.add(ChatFormatting.DARK_GRAY + Item.itemRegistry.getNameForObject(this.item).toString());
