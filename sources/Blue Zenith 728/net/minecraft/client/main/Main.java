...
public class Main {
    ...
    public static void main(String[] p_main_0_) {
        try {
        /*try {
            System.out.println("doing thing 1");
            NativeLoader.loadNatives();
            System.out.println("did thing 1");
        }
        catch (IOException e) {
            System.out.println("did not do thing 1");
        }*/
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(Minecraft::stopIntegratedServer, "Minecraft Shutdown Thread"));
            Thread.currentThread().setName("Minecraft");
            (new Minecraft(gameconfiguration)).run();
            startupTime = System.currentTimeMillis();
        } catch (NoClassDefFoundError exception) {
            boolean hasNoverify = false;
            try {
                final RuntimeMXBean runtimeMxBean = ManagementFactory.getRuntimeMXBean();
                final List<String> arguments = runtimeMxBean.getInputArguments();
                for (String argument : arguments) {
                    if(argument.toLowerCase().contains("-noverify") || argument.toLowerCase().contains("-xverify:none")) {
                        hasNoverify = true;
                        break;
                    }
                }
            ...
            System.err.println("Something went wrong while starting the game. Is the jar corrupted?");
            if(!hasNoverify)
                System.err.println("Looks like you didn't add -noverify to JVM arguments. That might have been the error.");
        }
    }
