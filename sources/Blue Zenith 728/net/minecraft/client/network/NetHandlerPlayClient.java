...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
	...
	public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
		...
		if (packetIn != null && packetIn.getPlayer() != null
				&&
				this.getPlayerInfo(packetIn.getPlayer()).getGameProfile() != null) {
			entityotherplayermp = new EntityOtherPlayerMP(this.gameController.theWorld, this.getPlayerInfo(packetIn.getPlayer()).getGameProfile());
		} else {
			entityotherplayermp = new EntityOtherPlayerMP(this.gameController.theWorld, null);
		}
	...
	public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
		//if (BlueZenith.getBlueZenith().getModuleManager().getModule(Disabler.class).getState()) return;
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		...
		entityplayer.setPositionAndRotation(d0, d1, d2, f, f1); //findme watchdog disabler

		Disabler disabler;

		if ((disabler = getBlueZenith().getModuleManager().getAndCast(Disabler.class)).getState()
				&& disabler.mode.is("Watchdog")
				&& ServerUtils.hypixel) {
			if(entityplayer.ticksExisted < 200 && doneLoadingTerrain) return;
			...
			//ClientUtils.fancyMessage("quite funny");
		}
		else {
	...
	public void onDisconnect(IChatComponent reason) {
		...
		if (this.guiScreenServer != null) {
			if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
				...
				getBlueZenith().postEvent(new DisconnectEvent(reason));
				this.gameController.displayGuiScreen(new GuiDisconnected(this.guiScreenServer, "disconnect.lost", reason));
			}
		} else {
			getBlueZenith().postEvent(new DisconnectEvent(reason));
			this.gameController.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(getBlueZenith().getMainMenu()), "disconnect.lost", reason));
		}
	}
	...
	public void handleResourcePack(S48PacketResourcePackSend packetIn) {
		...
		if (s.startsWith("level://")) {
			if (packetIn.getURL().contains("..")) {
				getBlueZenith().getNotificationPublisher().postWarning(getBlueZenith().getName(), "The server has attempted a file exploit.", 2500);
				return;
			}
