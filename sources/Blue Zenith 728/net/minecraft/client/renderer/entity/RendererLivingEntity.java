...
import club.bluezenith.module.modules.render.AmongUs;
import club.bluezenith.module.modules.render.Camera;
import club.bluezenith.module.modules.render.Chams;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    private boolean initializedAmongUs, shouldInitialize;
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if(shouldInitialize && !initializedAmongUs) {
            if(this.mainModel instanceof ModelPlayer) {
                new AmongUs(mainModel);
                initializedAmongUs = true;
            }
        }
        final boolean amongUs = entity instanceof EntityPlayer && AmongUs.state();
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                if(entity == Minecraft.getMinecraft().thePlayer && getBlueZenith().getModuleManager().getModule(Camera.class).getState() && Camera.rotations.get()){
                    f = this.interpolateRotation(Camera.prevYaw, Camera.yaw, partialTicks);
                    f1 = this.interpolateRotation(Camera.prevYaw, Camera.yaw, partialTicks);
                    f7 = Camera.prevPitch + (Camera.pitch - Camera.prevPitch) * partialTicks;
                }
                float f2 = f1 - f;
                ...
                if(!amongUs || !(entity instanceof EntityPlayer))
                this.mainModel.setRotationAngles(f6, f5, f8, f2, f7, 0.0625F, entity);
                else AmongUs.instance.setAngles(f6, f5, f8, f2, f7, 0.0625F, entity);
                ...
                if (CustomEntityModels.isActive()) {
                    if(this.renderEntity == Minecraft.getMinecraft().thePlayer)
                        new AmongUs(null);
    ...
    protected void renderModel(T entityLivingBaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        firstPass:
        {
            final boolean amongUs = entityLivingBaseIn instanceof EntityPlayer && AmongUs.state();

            Chams chams = (Chams) getBlueZenith().getModuleManager().getModule(Chams.class);
            boolean flag = !entityLivingBaseIn.isInvisible();
            boolean flag1 = !flag && !entityLivingBaseIn.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer);
            boolean chamsState = chams.getState() && TargetHelper.isTarget(entityLivingBaseIn);

            if (flag || flag1 || (chamsState && TargetHelper.isTarget(entityLivingBaseIn) && entityLivingBaseIn != Minecraft.getMinecraft().thePlayer)) {
                ...
                if (chamsState) {
                    ...
                    final Color r = chams.throughWallsColor.get();
                    GL11.glColor4f(r.getRed() / 255f, r.getGreen() / 255f, r.getBlue() / 255f, alpha);
                    if(!amongUs)
                        this.mainModel.render(entityLivingBaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
                    else AmongUs.instance.render(p_77036_7_);
                    if (chams.a2.get() != 0) {
                        final Color r2 = chams.visibleColor.get();
                        GL11.glColor4f(r2.getRed() / 255f, r2.getGreen() / 255f, r2.getBlue() / 255f, alpha2);
    ...
    protected void renderLayers(T entitylivingbaseIn, float p_177093_2_, float p_177093_3_, float partialTicks, float p_177093_5_, float p_177093_6_, float p_177093_7_, float p_177093_8_) {
        for (LayerRenderer<T> layerrenderer : this.layerRenderers) {
            final boolean amongUsPlayer = AmongUs.state() && entitylivingbaseIn instanceof EntityPlayer;
            if(amongUsPlayer && (layerrenderer instanceof LayerBipedArmor || layerrenderer instanceof LayerCape))
                continue;

            if(amongUsPlayer && layerrenderer instanceof LayerHeldItem) {
                GlStateManager.pushMatrix();
