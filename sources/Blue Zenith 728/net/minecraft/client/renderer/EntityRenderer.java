...
import club.bluezenith.events.impl.TranslateCameraEvent;
import club.bluezenith.module.modules.player.ChestStealer;
import club.bluezenith.module.modules.render.AntiBlind;
import club.bluezenith.module.modules.render.Atmosphere;
import club.bluezenith.module.modules.render.Camera;
import club.bluezenith.module.modules.render.WorldColor;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private float targetFOV = 0, currentFOV;
    /**
    ...
    private float getFOVModifier(float partialTicks, boolean p_78481_2_) {
        if (this.debugView) {
            ...
            final boolean smoothZoomCamera = Camera.smoothZoomCamera.get();
            final boolean smoothZoom = Camera.smoothZoom.get();
            final float zoomSpeed = Camera.zoomSpeed.get() / 1000F;

            if (flag) {
                if (!Config.zoomMode) {
                    Config.zoomMode = true;
                    this.mc.gameSettings.smoothCamera = smoothZoomCamera;
                    this.mc.renderGlobal.displayListEntitiesDirty = true;
                }

                targetFOV = f / 4.0F;

                f = currentFOV = smoothZoom ? animate(targetFOV, currentFOV, zoomSpeed) : targetFOV;///= 4.0F; //findme smooth zoom

                if(smoothZoom && abs(currentFOV - targetFOV) < 3) f = targetFOV; //skip animation if the diff is too little to prevent it from buggin
            ...
            if(!flag && f - currentFOV > 0.03 && smoothZoom)
            f = currentFOV = animate(f, currentFOV, zoomSpeed / 2);
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (movingobjectposition != null) {
                        ...
                        if (d7 < d3 && (!Camera.cameraClip.get() || !getBlueZenith().getModuleManager().getModule(Camera.class).getState())) {
                            d3 = d7;
                        }
                    }
                }
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        final TranslateCameraEvent event = new TranslateCameraEvent();
        BlueZenith.getBlueZenith().postEvent(event);

        //player.prevPosY + ((player.posY - player.prevPosY) * partialTicks) - lj.startY

        if(!event.cancelled) {
            GlStateManager.translate(
                    interpolatePos(player.prevPosX, player.posX, event.x, partialTicks),
                    interpolatePos(player.prevPosY, player.posY, event.y, partialTicks),
                    interpolatePos(player.prevPosZ, player.posZ, event.z, partialTicks)
            );
        }
    }

    private static double interpolatePos(double prevPos, double currentPos, double eventPos, float partialTicks) {
        if(eventPos == 0) return 0;
        return prevPos + ((currentPos - prevPos) * partialTicks) - eventPos;
    }
    ...
    private void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            final WorldColor worldColor = getBlueZenith().getModuleManager().getAndCast(WorldColor.class);
            if (world != null) {
                if (!worldColor.getState() && Config.isCustomColors() && CustomColors.updateLightmap(world, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision), partialTicks)) {
                ...
                if(worldColor.getState()) {
                    final Color color = worldColor.color.get();
                    final int lightmapColor = color.getAlpha() << 24 | color.getRed() << 16 | color.getGreen() << 8 | color.getBlue();
                    for (int i0 = 0; i0 < 256; i0++) {
                        lightmapColors[i0] = lightmapColor;
                    }
    ...
    public void updateCameraAndRender(float partialTicks, long p_181560_2_) {
        ...
        final boolean flag1 = getBlueZenith() != null && getBlueZenith().getModuleManager() != null;
        ChestStealer stealer = null;
        if(flag1)
            stealer = getBlueZenith().getModuleManager().getAndCast(ChestStealer.class);

        if ((this.mc.inGameHasFocus && flag || (flag && mc.currentScreen instanceof GuiChest && stealer.getState() && !stealer.silentMode.is("Normal")))) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    } else if(flag1 && !stealer.getState() || flag1 && !stealer.silentMode.is("Silent") || !(mc.currentScreen instanceof GuiChest) || flag && !stealer.isEligible(mc.currentScreen)){ //silent cheststealer
        ...
        if(getBlueZenith() != null) {
            getBlueZenith().getNotificationPublisher().render();
        }
    }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        getBlueZenith().postEvent(new Render3DEvent(partialTicks));
    ...
    private void addRainParticles() {
        ...
        if (f != 0.0F && Config.isRainSplash()) {
            final Atmosphere atmosphere = getBlueZenith().getModuleManager().getAndCast(Atmosphere.class);
            if(atmosphere.getState() && (atmosphere.weather.is("Server") || atmosphere.weather.is("Snow") || atmosphere.weather.is("Snowstorm")))
                return;
    ...
    protected void renderRainSnow(float partialTicks) {
        ...
        if (f5 > 0.0F) {
            ...
            int intensity = 0;
            final Atmosphere atmosphere = getBlueZenith().getModuleManager().getAndCast(Atmosphere.class);
            final boolean shouldSnow = atmosphere.getState() && atmosphere.weather.is("Snow");
            for (int k1 = k - i1; k1 <= k + i1; ++k1) {
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow() || true) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F && !shouldSnow) {
                                if (j1 != 0) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
                                ...
                                if(atmosphere.getState() && atmosphere.weather.is("Rain")) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
    ...
    private void setupFog(int p_78468_1_, float partialTicks) {
        AntiBlind antiBlind = (AntiBlind) getBlueZenith().getModuleManager().getModule(AntiBlind.class);
        this.fogStandard = false;
        Entity entity = this.mc.getRenderViewEntity();

        final Color c = Camera.fogColor.get();
        if(p_78468_1_ != -1 && !cloudFog) {
            if(Camera.module.getState() && Camera.customFog.get()) {
                GL11.glFog(GL11.GL_FOG_COLOR, this.setFogColorBuffer(clamp(c.getRed(), 0, 255) / 255F, clamp(c.getGreen(), 0, 255) / 255F, clamp(c.getBlue(), 0, 255) / 255F));
            } else {
                GL11.glFog(GL11.GL_FOG_COLOR, this.setFogColorBuffer(this.fogColorRed, this.fogColorGreen, this.fogColorBlue));
            }
        } else GL11.glFog(GL11.GL_FOG_COLOR, this.setFogColorBuffer(this.fogColorRed, this.fogColorGreen, this.fogColorBlue));
        GL11.glNormal3f(0.0F, -1.0F, 0.0F);
        if(p_78468_1_ != -1)
        ...
        if (f >= 0.0F) {
            ...
            if (p_78468_1_ == -1) {
                ...
                final Camera camera = Camera.module;
                if(camera == null || !camera.getState() || !Camera.customFog.get()) {
                    GlStateManager.setFogStart(f3 * Config.getFogStart());
                } else {
                    GlStateManager.setFogStart(f3 * Camera.fogDistance.get());
                }
                GlStateManager.setFogEnd(f3);
