...
import club.bluezenith.module.modules.render.Animations;
...
public class ItemRenderer {
    ...
    /**
     * How far the current item has been equipped (0 non equipped and 1 fully up)
     */
    ...
    public void renderItem(EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        if(BlueZenith.getBlueZenith().targetHudEntity == entityIn)
            return;
    ...
    private void renderHand(AbstractClientPlayer clientPlayer, float p_178095_2_, float p_178095_3_) {
        ...
        Render<AbstractClientPlayer> render = this.renderManager.getEntityRenderObject(this.mc.thePlayer);
        GlStateManager.disableCull();
        Chams chams = (Chams) BlueZenith.getBlueZenith().getModuleManager().getModule(Chams.class);
        if(chams.getState()){
            GL11.glPushMatrix();
            ...
            float alpha = clientPlayer.isInvisible() ? 0.2f : chams.a2.get() / 255f;
            final Color r2 = chams.visibleColor.get();
            GL11.glColor4f(r2.getRed() / 255f, r2.getGreen() / 255f, r2.getBlue() / 255f, alpha);
            RenderPlayer renderplayer = (RenderPlayer) render;
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        Aura aura = (Aura) BlueZenith.getBlueZenith().getModuleManager().getModule(Aura.class);
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                } else if (abstractclientplayer.getItemInUseCount() > 0 || (aura.getState() && aura.shouldVisuallyBlock())) {
                    EnumAction enumaction = aura.getState() && aura.shouldVisuallyBlock() ? EnumAction.BLOCK : this.itemToRender.getItemUseAction();
                    ...
                    onItemTranslate(f, f1);
                }

                this.renderItem(abstractclientplayer, this.itemToRender, ItemCameraTransforms.TransformType.FIRST_PERSON);
            } else if (!abstractclientplayer.isInvisible() || BlueZenith.getBlueZenith().getModuleManager().getModule(Chams.class).getState()) {
    ...
    public void onItemTranslate(float equipProgress, float swingProgress) {
        Animations a = Animations.module;
        if(a.getState()) {
    ...
    public void onBlockTranslate(float equipProgress, float swingProgress) {
        ...
        ++b;
        if(a.getState()) {
            if(a.anim.is("Rotating")) {
                GL11.glRotated(b, 0, 0, 1);
                GL11.glTranslated(-0.1, 0, 0);
                this.transformFirstPersonItem(1, 0.4f);
            } else if(true/*!a.useCustom.get()*/) {
                switch (a.anim.get()) {
                    case "Bruh":
                    case "Spin":
                        GlStateManager.translate(a.itemPosX.get() / 100, a.itemPosY.get() / 100, a.itemPosZ.get() / 100);
                        ...
                        GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
                        GlStateManager.translate(0.0F, (a.anim.is("Spin") ? 0 : equipProgress) * -0.6F, 0.0F);
                        GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                        float f = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI);
                        float f1 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI) * 0.2f;
                        if(a.anim.is("Spin"))
                    ...
                    case "Exhibition":
                        ...
                        GL11.glRotated(-MathHelper.sin((float) (MathHelper.sqrt_float(swingProgress) * Math.PI)) * 30.0F, MathHelper.sin((float) (MathHelper.sqrt_float(swingProgress) * Math.PI)) / 2, 0.0F, 9.0F);
                    ...
                    case "Swank":
                        ...
                        float var14 = MathHelper.sin((float) ((double)MathHelper.sqrt_float(swingProgress) * 3.141592653589793D));
                ...
                /*if(a.custom != null) {
                    this.transformFirstPersonItem(equipProgress, swingProgress);
                    a.custom.onBlockTranslate(swingProgress, equipProgress);
                    this.doBlockTransformations();
                } else {
    ...
    public void renderOverlays(float partialTicks) {
        ...
        if (!this.mc.thePlayer.isSpectator()) {
            ...
            if (this.mc.thePlayer.isBurning() && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderFireOverlay, this.mc.thePlayer, partialTicks)) {
                if(BlueZenith.getBlueZenith().getModuleManager().getModule(Camera.class).getState() && !Camera.noFire.get()) return;
     ...
     * before being called. Used for the water overlay. Args: PartialTickTime
     */
