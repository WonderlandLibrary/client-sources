...
import static net.minecraft.client.renderer.OpenGlHelper.isFramebufferEnabled;
...
public class ScreenShotHelper
{
    ...
    public static IChatComponent saveScreenshot(File gameDirectory, String screenshotName, int width, int height, Framebuffer buffer)
    {
        try
        {
            final File screenshotFolder = new File(gameDirectory, "screenshots");
            screenshotFolder.mkdir();

            final int previousGuiScale = Config.getGameSettings().guiScale;
            final ScaledResolution scaledresolution = new ScaledResolution(mc);
            final int scaleFactor = scaledresolution.getScaleFactor();
            final int screenshotSize = Config.getScreenshotSize();
            final boolean canUseFramebuffer = isFramebufferEnabled() && screenshotSize > 1;
            ...
            if (canUseFramebuffer)
            {
                ...
                mc.entityRenderer.updateCameraAndRender(Config.renderPartialTicks, System.nanoTime());
            }
            ...
            final int screenPixels = width * height;

            if (pixelBuffer == null || pixelBuffer.capacity() < screenPixels)
            {
                pixelBuffer = BufferUtils.createIntBuffer(screenPixels);
                pixelValues = new int[screenPixels];
            }
            ...
            {
                file2 = getTimestampedPNGFileForDirectory(screenshotFolder);
            }
            else
            {
                file2 = new File(screenshotFolder, screenshotName);
            }
