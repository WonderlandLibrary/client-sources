...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    public void exceptionCaught(ChannelHandlerContext p_exceptionCaught_1_, Throwable p_exceptionCaught_2_) {
        ...
        this.closeChannel(new ChatComponentText("Disconnected"));
    }

    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) {

        PacketEvent event = new PacketEvent(p_channelRead0_2_, EnumPacketDirection.SERVERBOUND);
        BlueZenith.getBlueZenith().postEvent(event);
        if(event.cancelled) {
            return;
        }
        p_channelRead0_2_ = event.packet;
    ...
    public void sendPacket(Packet packetIn) {
        PacketEvent event = new PacketEvent(packetIn, EnumPacketDirection.CLIENTBOUND);
        BlueZenith.getBlueZenith().postEvent(event);
        if(event.cancelled) {
            return;
        }
    ...
    private Consumer<IChatComponent> closeChannelListener;
    ...
    public void closeChannel(IChatComponent message) {
        if (this.channel.isOpen()) {
            ...
            if(this.closeChannelListener != null) {
                this.closeChannelListener.accept(message);
                this.closeChannelListener = null;
            }
        }
    }

    public NetworkManager setCloseChannelListener(Consumer<IChatComponent> closeChannelListener) {
        this.closeChannelListener = closeChannelListener;
        return this;
    }
    ...
    public static NetworkManager createInstance(InetAddress serverAddress, int serverPort, boolean useNativeTransport) {
        ...
        }).channel(oclass).connect(serverAddress, serverPort).syncUninterruptibly();
