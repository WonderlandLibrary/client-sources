...
public class ModifiableAttributeInstance implements IAttributeInstance
{
    ...
    private double computeValue()
    {
        ...
        final AttributeModifier[] arr = this.func_180375_b(0).toArray(new AttributeModifier[0]);
        for (AttributeModifier attributemodifier : arr)
        {
            d0 += attributemodifier.getAmount();
        }

        double d1 = d0;
        final AttributeModifier[] arr0 = this.func_180375_b(1).toArray(new AttributeModifier[0]);
        for (AttributeModifier attributemodifier1 : arr0)
        {
            d1 += d0 * attributemodifier1.getAmount();
        }
        final AttributeModifier[] arr1 = this.func_180375_b(2).toArray(new AttributeModifier[0]);
        for (AttributeModifier attributemodifier2 : arr1)
        {
    ...
    {
        final AttributeModifier[] modifiers = this.getModifiersByOperation(p_180375_1_).toArray(new AttributeModifier[0]);
        Set<AttributeModifier> set = Sets.newHashSet(modifiers);
