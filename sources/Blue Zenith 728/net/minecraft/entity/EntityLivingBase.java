...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        if(this == Minecraft.getMinecraft().thePlayer){
            //Rotations r = (Rotations) BlueZenith.getBlueZenith().getModuleManager().getModule(Rotations.class);
    ...
    private int getArmSwingAnimationEnd() {
        Animations anim = Animations.module;
        Aura aura = (Aura) BlueZenith.getBlueZenith().getModuleManager().getModule(Aura.class);
        if(this == Minecraft.getMinecraft().thePlayer && (anim.slowSwingBlock.get() || (!Minecraft.getMinecraft().thePlayer.isBlocking() && !aura.shouldVisuallyBlock())) && anim.getState() && anim.slowSwing.get()){
     ...
     */
    protected void jump() {
        final Speed speed = (Speed) BlueZenith.getBlueZenith().getModuleManager().getModule(Speed.class);
        ...
        if (this == mc.thePlayer && this.isSprinting() && speed.getState() && speed.getValue("Mode").get().equals("Watchdog")) {
            ...
            if (mc.thePlayer.isPotionActive(Potion.moveSpeed)) {
                this.motionX -= MathHelper.sin(f) * (((FloatValue) speed.getValue("JumpBoostBase")).get() + (((FloatValue) speed.getValue("JumpBoostMult")).get() * (mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1)));
                this.motionZ += MathHelper.cos(f) * (((FloatValue) speed.getValue("JumpBoostBase")).get() + (((FloatValue) speed.getValue("JumpBoostMult")).get() * (mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1)));
            }
            else {
                this.motionX -= MathHelper.sin(f) * ((FloatValue) speed.getValue("JumpBoostBase")).get();
                this.motionZ += MathHelper.cos(f) * ((FloatValue) speed.getValue("JumpBoostBase")).get();
            }
        ...
        if (this.isServerWorld()) {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    ...
                    if (this == mc.thePlayer && this.onGround && speed.getState() && ((ModeValue) speed.getValue("Mode")).get().equals("Watchdog")) {
                        PotionEffect sus = mc.thePlayer.getActivePotionEffect(Potion.getPotionFromResourceLocation("speed"));
                        if (sus != null && sus.getAmplifier() == 2) {
                            f4 = ((FloatValue)(speed.getValue("Slipperiness speed 2"))).get();
                        }
                        else {
                            f4 = ((FloatValue)(speed.getValue("Slipperiness"))).get();
                        }
                    }
                    ...
                    final boolean flag = !this.onGround && this.motionY > 0.1d;
                    this.moveEntity(this.motionX * (flag ? 1.08F : 1.0F) , this.motionY, this.motionZ * (flag ? 1.08F : 1.0F));
