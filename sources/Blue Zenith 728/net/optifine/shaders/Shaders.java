...
public class Shaders
{
    ...
    public static void loadConfig()
    {
        ...
        //[REMOVED_LOG] SMCLog.info("Load shaders configuration.");
    ...
    {
        //[REMOVED_LOG] SMCLog.info("Save shaders configuration.");
    ...
    public static void loadShaderPack()
    {
        ...
        {
            //[REMOVED_LOG] SMCLog.info("Shaders can not be loaded, Antialiasing is enabled: " + Config.getAntialiasingLevel() + "x");
            flag2 = true;
        }

        if (Config.isAnisotropicFiltering())
        {
            //[REMOVED_LOG] SMCLog.info("Shaders can not be loaded, Anisotropic Filtering is enabled: " + Config.getAnisotropicFilterLevel() + "x");
            flag2 = true;
        }

        if (Config.isFastRender())
        {
            //[REMOVED_LOG] SMCLog.info("Shaders can not be loaded, Fast Render is enabled.");
            flag2 = true;
        }
        ...
        if (shaderPackLoaded)
        {
            //[REMOVED_LOG] SMCLog.info("Loaded shaderpack: " + getShaderPackName());
        }
        else
        {
            //[REMOVED_LOG] SMCLog.info("No shaderpack loaded.");
        ...
        for (Object e: set)
        {
            ...
            if (s1.startsWith(s))
            {
                ...
                else
                {
                    ...
                    {
                        //[REMOVED_LOG] SMCLog.info("Custom texture: " + s1 + " = " + s3);
    ...
    public static void printIntBuffer(String title, IntBuffer buf)
    {
        ...
        //[REMOVED_LOG] SMCLog.info(stringbuilder.toString());
    }
    ...
    public static void startup(Minecraft mcs)
    {
        ...
        //[REMOVED_LOG] SMCLog.info("OpenGL Version: " + glVersionString);
        //[REMOVED_LOG] SMCLog.info("Vendor:  " + glVendorString);
        //[REMOVED_LOG] SMCLog.info("Renderer: " + glRendererString);
        //[REMOVED_LOG] SMCLog.info("Capabilities: " + (capabilities.OpenGL20 ? " 2.0 " : " - ") + (capabilities.OpenGL21 ? " 2.1 " : " - ") + (capabilities.OpenGL30 ? " 3.0 " : " - ") + (capabilities.OpenGL32 ? " 3.2 " : " - ") + (capabilities.OpenGL40 ? " 4.0 " : " - "));
        //[REMOVED_LOG] SMCLog.info("GL_MAX_DRAW_BUFFERS: " + GL11.glGetInteger(GL20.GL_MAX_DRAW_BUFFERS));
        //[REMOVED_LOG] SMCLog.info("GL_MAX_COLOR_ATTACHMENTS_EXT: " + GL11.glGetInteger(36063));
        //[REMOVED_LOG] SMCLog.info("GL_MAX_TEXTURE_IMAGE_UNITS: " + GL11.glGetInteger(GL20.GL_MAX_TEXTURE_IMAGE_UNITS));
        ...
        if (!isShaderPackInitialized)
        {
            ...
            for (int k = 0; k < ProgramsAll.length; ++k)
            {
                ...
                if (program.getProgramStage() != ProgramStage.NONE)
                {
                    ...
                    if (!flag1)
                    {
                        //[REMOVED_LOG] SMCLog.info("Program disabled: " + s2);
                        s1 = "<disabled>";
                        s2 = s + s1;
                    }
                    ...
                    if (j > 0)
                    {
                        //[REMOVED_LOG] SMCLog.info("Program loaded: " + s2);
                    }
            ...
            //[REMOVED_LOG] SMCLog.info("usedColorBuffers: " + usedColorBuffers);
            //[REMOVED_LOG] SMCLog.info("usedDepthBuffers: " + usedDepthBuffers);
            //[REMOVED_LOG] SMCLog.info("usedShadowColorBuffers: " + usedShadowColorBuffers);
            //[REMOVED_LOG] SMCLog.info("usedShadowDepthBuffers: " + usedShadowDepthBuffers);
            //[REMOVED_LOG] SMCLog.info("usedColorAttachs: " + usedColorAttachs);
            //[REMOVED_LOG] SMCLog.info("usedDrawBuffers: " + usedDrawBuffers);
    ...
    {
        //[REMOVED_LOG] SMCLog.info("Reset model renderers");
        ++countResetDisplayLists;
        //[REMOVED_LOG] SMCLog.info("Reset world renderers");
                        ...
                        if (macrostate.processLine(s))
                        {
                            ...
                            if (shaderline != null)
                            {
                                ...
                                if (shaderline.isConstInt("countInstances"))
                                {
                                    ...
                                    //[REMOVED_LOG] SMCLog.info("countInstances: " + program.getCountInstances());
                                }
                            }
                        }
                    }
                                    ...
                                    if (!shaderline.isConstFloat("shadowMapFov") && !shaderline.isProperty("SHADOWFOV"))
                                    {
                                        if (!shaderline.isConstFloat("shadowDistance") && !shaderline.isProperty("SHADOWHPL"))
                                        {
                                            ...
                                            else if (shaderline.isConstBool("shadowcolor0Nearest", "shadowColor0Nearest", "shadowColor0MinMagNearest", true))
                                            {
                                                ...
                                                //[REMOVED_LOG] SMCLog.info("shadowcolor0Nearest");
                                            }
                                            else if (shaderline.isConstBool("shadowcolor1Nearest", "shadowColor1Nearest", "shadowColor1MinMagNearest", true))
                                            {
                                                ...
                                                //[REMOVED_LOG] SMCLog.info("shadowcolor1Nearest");
                                            }
                                            else if (!shaderline.isConstFloat("wetnessHalflife") && !shaderline.isProperty("WETNESSHL"))
                                            {
                                                if (!shaderline.isConstFloat("drynessHalflife") && !shaderline.isProperty("DRYNESSHL"))
                                                {
                                                    ...
                                                    else if (shaderline.isConstInt("superSamplingLevel"))
                                                    {
                                                        ...
                                                        if (i1 > 1)
                                                        {
                                                            //[REMOVED_LOG] SMCLog.info("Super sampling level: " + i1 + "x");
                                                    ...
                                                    else if (shaderline.isConstInt("noiseTextureResolution"))
                                                    {
                                                        ...
                                                        noiseTextureEnabled = true;
                                                        //[REMOVED_LOG] SMCLog.info("Noise texture enabled");
                                                        //[REMOVED_LOG] SMCLog.info("Noise texture resolution: " + noiseTextureResolution);
                                                    }
                                                    else if (shaderline.isConstIntSuffix("Format"))
                                                    {
                                                        ...
                                                        {
                                                            gbuffersFormat[i2] = l;
                                                            //[REMOVED_LOG] SMCLog.info("%s format: %s", new Object[] {s5, s7});
                                                        }
                                                    }
                                                    else if (shaderline.isConstBoolSuffix("Clear", false))
                                                    {
                                                        if (ShaderParser.isComposite(filename) || ShaderParser.isDeferred(filename))
                                                        {
                                                            ...
                                                            if (k1 >= 0)
                                                            {
                                                                ...
                                                                //[REMOVED_LOG] SMCLog.info("%s clear disabled", new Object[] {s4});
                                                            }
                                                        }
                                                    }
                                                        ...
                                                        if (ShaderParser.isComposite(filename) || ShaderParser.isDeferred(filename))
                                                        {
                                                            ...
                                                            if (j1 >= 0)
                                                            {
                                                                ...
                                                                if (vector4f != null)
                                                                {
                                                                    ...
                                                                    //[REMOVED_LOG] SMCLog.info("%s clear color: %s %s %s %s", new Object[] {s3, Float.valueOf(vector4f.getX()), Float.valueOf(vector4f.getY()), Float.valueOf(vector4f.getZ()), Float.valueOf(vector4f.getW())});
                                                                }
                                                                else
                                                                {
                                                    ...
                                                    else if (shaderline.isConstBoolSuffix("MipmapEnabled", true))
                                                    {
                                                        if (ShaderParser.isComposite(filename) || ShaderParser.isDeferred(filename) || ShaderParser.isFinal(filename))
                                                        {
                                                            ...
                                                            if (j >= 0)
                                                            {
                                                                ...
                                                                //[REMOVED_LOG] SMCLog.info("%s mipmap enabled", new Object[] {s2});
                                                            }
                                                        }
                                                    }
                                                ...
                                                else
                                                {
                                                    ...
                                                    //[REMOVED_LOG] SMCLog.info("Dryness halflife: " + drynessHalfLife);
                                                }
                                            }
                                            else
                                            {
                                                wetnessHalfLife = shaderline.getValueFloat();
                                                //[REMOVED_LOG] SMCLog.info("Wetness halflife: " + wetnessHalfLife);
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            shadowMapIsOrtho = true;
                                            //[REMOVED_LOG] SMCLog.info("Shadow map distance: " + shadowMapHalfPlane);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        //[REMOVED_LOG] SMCLog.info("Shadow map field of view: " + shadowMapFOV);
                                    }
                                }
                                else
                                {
                                    ...
                                    //[REMOVED_LOG] SMCLog.info("Shadow map resolution: " + spShadowMapWidth);
                                }
                            }
                        }
                    }
    ...
    private static boolean printLogInfo(int obj, String name)
    {
        ...
        if (i > 1)
        {
            ...
            s = StrUtils.trim(s, " \n\r\t");
            //[REMOVED_LOG] SMCLog.info("Info log: " + name + "\n" + s);
            return false;
        }
        else
        {
    ...
    private static boolean printShaderLogInfo(int shader, String name, List<String> listFiles)
    {
        ...
        else
        {
            for (int j = 0; j < listFiles.size(); ++j)
            {
                ...
                //[REMOVED_LOG] SMCLog.info("File: " + (j + 1) + " = " + s);
            }

            String s1 = GL20.glGetShaderInfoLog(shader, i);
            s1 = StrUtils.trim(s1, " \n\r\t");
            //[REMOVED_LOG] SMCLog.info("Shader info log: " + name + "\n" + s1);
            return false;
        }
    }
    ...
    public static void uninit()
    {
        if (isShaderPackInitialized)
        {
            ...
            //[REMOVED_LOG] SMCLog.info("Uninit");
    ...
    private static void setupFrameBuffer()
    {
        ...
        if (i1 == 36058)
        {
            ...
            if (i1 == 36053)
            {
                //[REMOVED_LOG] SMCLog.info("complete");
            }
        }
        ...
        {
            //[REMOVED_LOG] SMCLog.info("Framebuffer created.");
        }
    }
    ...
    private static void setupShadowFrameBuffer()
    {
        if (usedShadowDepthBuffers != 0)
        {
            ...
            {
                //[REMOVED_LOG] SMCLog.info("Shadow framebuffer created.");
            }
        }
    }
