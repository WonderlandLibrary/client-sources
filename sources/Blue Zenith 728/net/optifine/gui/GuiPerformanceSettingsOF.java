...
public class GuiPerformanceSettingsOF extends GuiScreen
{
    ...
    protected void actionPerformed(GuiButton guibutton)
    {
        if (guibutton.enabled)
        {
            ...
            {
                final GameSettings.Options options = ((GuiOptionButton)guibutton).returnEnumOptions();
                if(options.returnEnumOrdinal() != GameSettings.Options.FAST_RENDER.returnEnumOrdinal())
                {
                    this.settings.setOptionValue(((GuiOptionButton) guibutton).returnEnumOptions(), 1);
                    guibutton.displayString = this.settings.getKeyBinding(GameSettings.Options.getEnumOptions(guibutton.id));
                } else BlueZenith.getBlueZenith().getNotificationPublisher().postError("Fast Render", "This option is currently unsupported.", 3500);
            }
