...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry {
   ...
   public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
         ...
         field_148302_b.submit(new Runnable() {
            public void run() {
               try {
                  ...
                  ServerListEntryNormal.this.field_148301_e.serverMOTD = ChatFormatting.DARK_RED + "Can't resolve hostname";
               } catch (Exception var3) {
                  ServerListEntryNormal.this.field_148301_e.pingToServer = -1L;
                  ServerListEntryNormal.this.field_148301_e.serverMOTD = ChatFormatting.DARK_RED + "Can't connect to server.";
               }
      ...
      this.mc.MCfontRenderer.drawString(this.field_148301_e.serverName, x + 32 + 3, y + 1, 16777215);
      List<String> list = this.mc.MCfontRenderer.listFormattedStringToWidth(this.field_148301_e.serverMOTD, listWidth - 32 - 2);

      for(int i = 0; i < Math.min(list.size(), 2); ++i) {
         this.mc.MCfontRenderer.drawString((String)list.get(i), x + 32 + 3, y + 12 + this.mc.MCfontRenderer.FONT_HEIGHT * i, 8421504);
      }

      String s2 = flag2 ? ChatFormatting.DARK_RED + this.field_148301_e.gameVersion : this.field_148301_e.populationInfo;
      int j = this.mc.MCfontRenderer.getStringWidth(s2);
      this.mc.MCfontRenderer.drawString(s2, x + listWidth - j - 15 - 2, y + 1, 8421504);
      int k = 0;
      String s = null;
      int l;
      String s1;
