...
public abstract class EntityLiving extends EntityLivingBase {
   ...
   public void readEntityFromNBT(NBTTagCompound tagCompund) {
      ...
      if (tagCompund.hasKey("Equipment", 9)) {
         nbttaglist1 = tagCompund.getTagList("Equipment", 10);

         for(j = 0; j < this.equipment.length; ++j) {
            this.equipment[j] = ItemStack.loadItemStackFromNBT(nbttaglist1.getCompoundTagAt(j));
         }
      }

      if (tagCompund.hasKey("DropChances", 9)) {
         nbttaglist1 = tagCompund.getTagList("DropChances", 5);
   ...
   public void onLivingUpdate() {
      ...
      if (!this.worldObj.isRemote && this.canPickUpLoot() && !this.dead && this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing")) {
         Iterator var1 = this.worldObj.getEntitiesWithinAABB(EntityItem.class, this.getEntityBoundingBox().expand(1.0D, 0.0D, 1.0D)).iterator();
   ...
   private void recreateLeash() {
      if (this.isLeashed && this.leashNBTTag != null) {
         if (this.leashNBTTag.hasKey("UUIDMost", 4) && this.leashNBTTag.hasKey("UUIDLeast", 4)) {
            ...
            Iterator var5 = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(10.0D, 10.0D, 10.0D)).iterator();

            while(var5.hasNext()) {
               EntityLivingBase entitylivingbase = (EntityLivingBase)var5.next();
