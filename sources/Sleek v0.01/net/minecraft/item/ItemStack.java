...
public final class ItemStack {
   ...
   public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
            ...
            if (nbttagcompound.getTagId("Lore") == 9) {
               ...
               if (nbttaglist3.tagCount() > 0) {
                  for(l1 = 0; l1 < nbttaglist3.tagCount(); ++l1) {
                     list.add(ChatFormatting.DARK_PURPLE + "" + ChatFormatting.ITALIC + nbttaglist3.getStringTagAt(l1));
                  }
               }
            }
         }
      ...
      if (!multimap.isEmpty() && (i1 & 2) == 0) {
         ...
         while(var19.hasNext()) {
            Entry<String, AttributeModifier> entry = (Entry)var19.next();
            ...
            if (d0 > 0.0D) {
               list.add(ChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier.getOperation(), DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())));
            } else if (d0 < 0.0D) {
               d1 *= -1.0D;
               list.add(ChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier.getOperation(), DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())));
            }
         }
      }

      if (this.hasTagCompound() && this.getTagCompound().getBoolean("Unbreakable") && (i1 & 4) == 0) {
         list.add(ChatFormatting.BLUE + StatCollector.translateToLocal("item.unbreakable"));
      }
      ...
      if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (i1 & 8) == 0) {
         ...
         if (nbttaglist3.tagCount() > 0) {
            list.add("");
            list.add(ChatFormatting.GRAY + StatCollector.translateToLocal("item.canBreak"));

            for(l1 = 0; l1 < nbttaglist3.tagCount(); ++l1) {
               block1 = Block.getBlockFromName(nbttaglist3.getStringTagAt(l1));
               if (block1 != null) {
                  list.add(ChatFormatting.DARK_GRAY + block1.getLocalizedName());
               } else {
                  list.add(ChatFormatting.DARK_GRAY + "missingno");
               }
            }
         }
      }
      ...
      if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanPlaceOn", 9) && (i1 & 16) == 0) {
         ...
         if (nbttaglist3.tagCount() > 0) {
            list.add("");
            list.add(ChatFormatting.GRAY + StatCollector.translateToLocal("item.canPlace"));
      ...
      if (advanced) {
         ...
         list.add(ChatFormatting.DARK_GRAY + ((ResourceLocation)Item.itemRegistry.getNameForObject(this.item)).toString());
         if (this.hasTagCompound()) {
            list.add(ChatFormatting.DARK_GRAY + "NBT: " + this.getTagCompound().getKeySet().size() + " tag(s)");
         }
      }
