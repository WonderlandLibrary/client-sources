...
public class ItemEnderEye extends Item {
   ...
   public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
      ...
      if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && iblockstate.getBlock() == Blocks.end_portal_frame && !(Boolean)iblockstate.getValue(BlockEndPortalFrame.EYE)) {
         if (!worldIn.isRemote) {
            ...
            for(int k = -2; k <= 2; ++k) {
               ...
               if (iblockstate1.getBlock() == Blocks.end_portal_frame) {
                  if (!(Boolean)iblockstate1.getValue(BlockEndPortalFrame.EYE)) {
                     flag = false;
                     break;
                  }
            ...
            if (flag && j == l + 2) {
               ...
               int k1;
               for(k1 = l; k1 <= j; ++k1) {
                  BlockPos blockpos2 = blockpos.offset(enumfacing1, k1);
                  IBlockState iblockstate3 = worldIn.getBlockState(blockpos2);
                  if (iblockstate3.getBlock() != Blocks.end_portal_frame || !(Boolean)iblockstate3.getValue(BlockEndPortalFrame.EYE)) {
                     flag = false;
                     break;
                  }
               }
               ...
               int i2;
               BlockPos blockpos4;
               for(k1 = l - 1; k1 <= j + 1; k1 += 4) {
                  blockpos = pos.offset(enumfacing1, k1);

                  for(i2 = 1; i2 <= 3; ++i2) {
                     blockpos4 = blockpos.offset(enumfacing, i2);
                     IBlockState iblockstate2 = worldIn.getBlockState(blockpos4);
                     if (iblockstate2.getBlock() != Blocks.end_portal_frame || !(Boolean)iblockstate2.getValue(BlockEndPortalFrame.EYE)) {
                        flag = false;
                        break;
                     }
                  }
