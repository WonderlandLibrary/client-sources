package com.sun.jna.z.a.e.a.a.a.e;

import com.sun.jna.z.a.e.a.a.a.a.*;
import java.awt.*;
import org.lwjgl.opengl.*;
import com.sun.jna.z.a.*;

public abstract class b<T extends k> implements d
{
    protected final Class<T> a;
    protected Color b;
    protected Color c;
    public static int d;
    
    public b(final Class<T> a) {
        this.a = a;
    }
    
    @Override
    public void a(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        boolean b2 = false;
        Label_0065: {
            Label_0050: {
                boolean b;
                try {
                    b = (b2 = this.a.isInstance(a));
                    if (a2 != 0) {
                        break Label_0065;
                    }
                    if (!b) {
                        break Label_0050;
                    }
                    break Label_0050;
                }
                catch (NullPointerException ex16) {
                    throw ex16;
                }
                try {
                    if (!b) {
                        throw new IllegalArgumentException();
                    }
                }
                catch (NullPointerException ex17) {
                    throw ex17;
                }
            }
            b2 = a.p();
        }
        if (!b2) {
            return;
        }
        this.b(this.a.cast(a));
    }
    
    protected abstract void b(final T p0);
    
    @Override
    public Rectangle a(final l a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (!l.class.isAssignableFrom(this.a)) {
                                                                        throw new UnsupportedOperationException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex16) {
                                                                    throw ex16;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex17) {
                                                                throw ex17;
                                                            }
                                                        }
                                                        catch (NullPointerException ex18) {
                                                            throw ex18;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex19) {
                                                        throw ex19;
                                                    }
                                                }
                                                catch (NullPointerException ex20) {
                                                    throw ex20;
                                                }
                                            }
                                            catch (IllegalArgumentException ex21) {
                                                throw ex21;
                                            }
                                        }
                                        catch (NullPointerException ex22) {
                                            throw ex22;
                                        }
                                    }
                                    catch (IllegalArgumentException ex23) {
                                        throw ex23;
                                    }
                                }
                                catch (NullPointerException ex24) {
                                    throw ex24;
                                }
                            }
                            catch (IllegalArgumentException ex25) {
                                throw ex25;
                            }
                        }
                        catch (NullPointerException ex26) {
                            throw ex26;
                        }
                    }
                    catch (IllegalArgumentException ex27) {
                        throw ex27;
                    }
                }
                catch (NullPointerException ex28) {
                    throw ex28;
                }
            }
            catch (IllegalArgumentException ex29) {
                throw ex29;
            }
        }
        catch (NullPointerException ex30) {
            throw ex30;
        }
        Label_0085: {
            try {
                final b b = this;
                if (a2 != 0) {
                    return b.c(this.a.cast(a));
                }
                final Class<T> clazz = this.a;
                final l l = a;
                final boolean b2 = clazz.isInstance(l);
                if (!b2) {
                    break Label_0085;
                }
                break Label_0085;
            }
            catch (NullPointerException ex31) {
                throw ex31;
            }
            try {
                final Class<T> clazz = this.a;
                final l l = a;
                final boolean b2 = clazz.isInstance(l);
                if (!b2) {
                    throw new IllegalArgumentException();
                }
            }
            catch (NullPointerException ex32) {
                throw ex32;
            }
        }
        final b b = this;
        return b.c(this.a.cast(a));
    }
    
    protected Rectangle c(final T a) {
        return new Rectangle(new Point(0, 0), a.i());
    }
    
    @Override
    public Dimension b(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        Label_0050: {
            try {
                final b b = this;
                if (a2 != 0) {
                    return b.d(this.a.cast(a));
                }
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    break Label_0050;
                }
                break Label_0050;
            }
            catch (NullPointerException ex16) {
                throw ex16;
            }
            try {
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    throw new IllegalArgumentException();
                }
            }
            catch (NullPointerException ex17) {
                throw ex17;
            }
        }
        final b b = this;
        return b.d(this.a.cast(a));
    }
    
    protected abstract Dimension d(final T p0);
    
    protected void a(final k a, final boolean a) {
        final int d = com.sun.jna.z.a.e.a.a.a.e.b.d;
        a.k();
        final int a2 = d;
        double n6 = 0.0;
        int n7 = 0;
        Label_0317: {
            Label_0316: {
                Label_0313: {
                    Label_0236: {
                        Label_0235: {
                            Label_0232: {
                                Label_0164: {
                                    k a3 = null;
                                    while (a3 != null) {
                                        double n4 = 0.0;
                                        int n5 = 0;
                                        Label_0124: {
                                            Label_0123: {
                                                Label_0120: {
                                                    Label_0042: {
                                                        Label_0041: {
                                                            Label_0038: {
                                                                Label_0030: {
                                                                    try {
                                                                        final int n = a ? 1 : 0;
                                                                        final boolean b = a;
                                                                        final int n2 = a ? 1 : 0;
                                                                        if (a2 != 0) {
                                                                            break Label_0164;
                                                                        }
                                                                        final int n3 = a2;
                                                                        if (n3 == 0) {
                                                                            break Label_0030;
                                                                        }
                                                                        break Label_0038;
                                                                    }
                                                                    catch (NullPointerException ex) {
                                                                        throw ex;
                                                                    }
                                                                    try {
                                                                        final int n3 = a2;
                                                                        if (n3 != 0) {
                                                                            break Label_0038;
                                                                        }
                                                                        if (!a) {
                                                                            break Label_0041;
                                                                        }
                                                                    }
                                                                    catch (NullPointerException ex2) {
                                                                        throw ex2;
                                                                    }
                                                                }
                                                                final int n2 = -1;
                                                            }
                                                            break Label_0042;
                                                        }
                                                        final int n2 = 1;
                                                        try {
                                                            try {
                                                                try {
                                                                    try {
                                                                        try {
                                                                            try {
                                                                                try {
                                                                                    try {
                                                                                        try {
                                                                                            try {
                                                                                                try {
                                                                                                    try {
                                                                                                        try {
                                                                                                            try {
                                                                                                                try {
                                                                                                                    n4 = n2 * a3.d();
                                                                                                                    n5 = (a ? 1 : 0);
                                                                                                                    if (a2 != 0) {
                                                                                                                        break Label_0120;
                                                                                                                    }
                                                                                                                    if (!a) {
                                                                                                                        break Label_0123;
                                                                                                                    }
                                                                                                                }
                                                                                                                catch (NullPointerException ex3) {
                                                                                                                    throw ex3;
                                                                                                                }
                                                                                                            }
                                                                                                            catch (IllegalArgumentException ex4) {
                                                                                                                throw ex4;
                                                                                                            }
                                                                                                        }
                                                                                                        catch (NullPointerException ex5) {
                                                                                                            throw ex5;
                                                                                                        }
                                                                                                    }
                                                                                                    catch (IllegalArgumentException ex6) {
                                                                                                        throw ex6;
                                                                                                    }
                                                                                                }
                                                                                                catch (NullPointerException ex7) {
                                                                                                    throw ex7;
                                                                                                }
                                                                                            }
                                                                                            catch (IllegalArgumentException ex8) {
                                                                                                throw ex8;
                                                                                            }
                                                                                        }
                                                                                        catch (NullPointerException ex9) {
                                                                                            throw ex9;
                                                                                        }
                                                                                    }
                                                                                    catch (IllegalArgumentException ex10) {
                                                                                        throw ex10;
                                                                                    }
                                                                                }
                                                                                catch (NullPointerException ex11) {
                                                                                    throw ex11;
                                                                                }
                                                                            }
                                                                            catch (IllegalArgumentException ex12) {
                                                                                throw ex12;
                                                                            }
                                                                        }
                                                                        catch (NullPointerException ex13) {
                                                                            throw ex13;
                                                                        }
                                                                    }
                                                                    catch (IllegalArgumentException ex14) {
                                                                        throw ex14;
                                                                    }
                                                                }
                                                                catch (NullPointerException ex15) {
                                                                    throw ex15;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex16) {
                                                                throw ex16;
                                                            }
                                                        }
                                                        catch (NullPointerException ex17) {
                                                            throw ex17;
                                                        }
                                                    }
                                                    n5 = -1;
                                                }
                                                break Label_0124;
                                            }
                                            n5 = 1;
                                        }
                                        GL11.glTranslated(n4, (double)(n5 * a3.e()), 0.0);
                                        a3 = a3.k();
                                        if (a2 != 0) {
                                            int a4 = i.g;
                                            i.g = ++a4;
                                            break;
                                        }
                                    }
                                    final int n = a ? 1 : 0;
                                    final boolean b = a;
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    try {
                                                                        try {
                                                                            try {
                                                                                try {
                                                                                    try {
                                                                                        try {
                                                                                            try {
                                                                                                if (a2 != 0) {
                                                                                                    break Label_0232;
                                                                                                }
                                                                                                if (!b) {
                                                                                                    break Label_0235;
                                                                                                }
                                                                                            }
                                                                                            catch (NullPointerException ex18) {
                                                                                                throw ex18;
                                                                                            }
                                                                                        }
                                                                                        catch (IllegalArgumentException ex19) {
                                                                                            throw ex19;
                                                                                        }
                                                                                    }
                                                                                    catch (NullPointerException ex20) {
                                                                                        throw ex20;
                                                                                    }
                                                                                }
                                                                                catch (IllegalArgumentException ex21) {
                                                                                    throw ex21;
                                                                                }
                                                                            }
                                                                            catch (NullPointerException ex22) {
                                                                                throw ex22;
                                                                            }
                                                                        }
                                                                        catch (IllegalArgumentException ex23) {
                                                                            throw ex23;
                                                                        }
                                                                    }
                                                                    catch (NullPointerException ex24) {
                                                                        throw ex24;
                                                                    }
                                                                }
                                                                catch (IllegalArgumentException ex25) {
                                                                    throw ex25;
                                                                }
                                                            }
                                                            catch (NullPointerException ex26) {
                                                                throw ex26;
                                                            }
                                                        }
                                                        catch (IllegalArgumentException ex27) {
                                                            throw ex27;
                                                        }
                                                    }
                                                    catch (NullPointerException ex28) {
                                                        throw ex28;
                                                    }
                                                }
                                                catch (IllegalArgumentException ex29) {
                                                    throw ex29;
                                                }
                                            }
                                            catch (NullPointerException ex30) {
                                                throw ex30;
                                            }
                                        }
                                        catch (IllegalArgumentException ex31) {
                                            throw ex31;
                                        }
                                    }
                                    catch (NullPointerException ex32) {
                                        throw ex32;
                                    }
                                }
                                final int n = -1;
                            }
                            break Label_0236;
                        }
                        final int n = 1;
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    try {
                                                                        try {
                                                                            try {
                                                                                try {
                                                                                    n6 = n * a.d();
                                                                                    n7 = (a ? 1 : 0);
                                                                                    if (a2 != 0) {
                                                                                        break Label_0313;
                                                                                    }
                                                                                    if (!a) {
                                                                                        break Label_0316;
                                                                                    }
                                                                                }
                                                                                catch (NullPointerException ex33) {
                                                                                    throw ex33;
                                                                                }
                                                                            }
                                                                            catch (IllegalArgumentException ex34) {
                                                                                throw ex34;
                                                                            }
                                                                        }
                                                                        catch (NullPointerException ex35) {
                                                                            throw ex35;
                                                                        }
                                                                    }
                                                                    catch (IllegalArgumentException ex36) {
                                                                        throw ex36;
                                                                    }
                                                                }
                                                                catch (NullPointerException ex37) {
                                                                    throw ex37;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex38) {
                                                                throw ex38;
                                                            }
                                                        }
                                                        catch (NullPointerException ex39) {
                                                            throw ex39;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex40) {
                                                        throw ex40;
                                                    }
                                                }
                                                catch (NullPointerException ex41) {
                                                    throw ex41;
                                                }
                                            }
                                            catch (IllegalArgumentException ex42) {
                                                throw ex42;
                                            }
                                        }
                                        catch (NullPointerException ex43) {
                                            throw ex43;
                                        }
                                    }
                                    catch (IllegalArgumentException ex44) {
                                        throw ex44;
                                    }
                                }
                                catch (NullPointerException ex45) {
                                    throw ex45;
                                }
                            }
                            catch (IllegalArgumentException ex46) {
                                throw ex46;
                            }
                        }
                        catch (NullPointerException ex47) {
                            throw ex47;
                        }
                    }
                    n7 = -1;
                }
                break Label_0317;
            }
            n7 = 1;
        }
        GL11.glTranslated(n6, (double)(n7 * a.e()), 0.0);
    }
    
    @Override
    public Color c(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        Label_0050: {
            try {
                final b b = this;
                if (a2 != 0) {
                    return b.e(this.a.cast(a));
                }
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    break Label_0050;
                }
                break Label_0050;
            }
            catch (NullPointerException ex16) {
                throw ex16;
            }
            try {
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    throw new IllegalArgumentException();
                }
            }
            catch (NullPointerException ex17) {
                throw ex17;
            }
        }
        final b b = this;
        return b.e(this.a.cast(a));
    }
    
    protected Color e(final T a) {
        return this.c;
    }
    
    @Override
    public Color d(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        Label_0050: {
            try {
                final b b = this;
                if (a2 != 0) {
                    return b.f(this.a.cast(a));
                }
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    break Label_0050;
                }
                break Label_0050;
            }
            catch (NullPointerException ex16) {
                throw ex16;
            }
            try {
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    throw new IllegalArgumentException();
                }
            }
            catch (NullPointerException ex17) {
                throw ex17;
            }
        }
        final b b = this;
        return b.f(this.a.cast(a));
    }
    
    protected Color f(final T a) {
        return this.b;
    }
    
    @Override
    public Rectangle[] e(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        Label_0050: {
            try {
                final b b = this;
                if (a2 != 0) {
                    return b.g(this.a.cast(a));
                }
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    break Label_0050;
                }
                break Label_0050;
            }
            catch (NullPointerException ex16) {
                throw ex16;
            }
            try {
                final Class<T> clazz = this.a;
                final k k = a;
                final boolean b2 = clazz.isInstance(k);
                if (!b2) {
                    throw new IllegalArgumentException();
                }
            }
            catch (NullPointerException ex17) {
                throw ex17;
            }
        }
        final b b = this;
        return b.g(this.a.cast(a));
    }
    
    protected Rectangle[] g(final T a) {
        return new Rectangle[0];
    }
    
    @Override
    public void a(final k a, final Point a, final int a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        b b = null;
        Label_0062: {
            Label_0052: {
                try {
                    b = this;
                    if (a2 != 0) {
                        break Label_0062;
                    }
                    final Class<T> clazz = this.a;
                    final k k = a;
                    final boolean b2 = clazz.isInstance(k);
                    if (!b2) {
                        break Label_0052;
                    }
                    break Label_0052;
                }
                catch (NullPointerException ex16) {
                    throw ex16;
                }
                try {
                    final Class<T> clazz = this.a;
                    final k k = a;
                    final boolean b2 = clazz.isInstance(k);
                    if (!b2) {
                        throw new IllegalArgumentException();
                    }
                }
                catch (NullPointerException ex17) {
                    throw ex17;
                }
            }
            b = this;
        }
        b.b(this.a.cast(a), a, a);
    }
    
    protected void b(final T a, final Point a, final int a) {
    }
    
    @Override
    public void f(final k a) {
        final int a2 = com.sun.jna.z.a.e.a.a.a.e.b.d;
        try {
            try {
                try {
                    try {
                        try {
                            try {
                                try {
                                    try {
                                        try {
                                            try {
                                                try {
                                                    try {
                                                        try {
                                                            try {
                                                                try {
                                                                    if (a == null) {
                                                                        throw new NullPointerException();
                                                                    }
                                                                }
                                                                catch (NullPointerException ex) {
                                                                    throw ex;
                                                                }
                                                            }
                                                            catch (IllegalArgumentException ex2) {
                                                                throw ex2;
                                                            }
                                                        }
                                                        catch (NullPointerException ex3) {
                                                            throw ex3;
                                                        }
                                                    }
                                                    catch (IllegalArgumentException ex4) {
                                                        throw ex4;
                                                    }
                                                }
                                                catch (NullPointerException ex5) {
                                                    throw ex5;
                                                }
                                            }
                                            catch (IllegalArgumentException ex6) {
                                                throw ex6;
                                            }
                                        }
                                        catch (NullPointerException ex7) {
                                            throw ex7;
                                        }
                                    }
                                    catch (IllegalArgumentException ex8) {
                                        throw ex8;
                                    }
                                }
                                catch (NullPointerException ex9) {
                                    throw ex9;
                                }
                            }
                            catch (IllegalArgumentException ex10) {
                                throw ex10;
                            }
                        }
                        catch (NullPointerException ex11) {
                            throw ex11;
                        }
                    }
                    catch (IllegalArgumentException ex12) {
                        throw ex12;
                    }
                }
                catch (NullPointerException ex13) {
                    throw ex13;
                }
            }
            catch (IllegalArgumentException ex14) {
                throw ex14;
            }
        }
        catch (NullPointerException ex15) {
            throw ex15;
        }
        b b = null;
        Label_0060: {
            Label_0050: {
                try {
                    b = this;
                    if (a2 != 0) {
                        break Label_0060;
                    }
                    final Class<T> clazz = this.a;
                    final k k = a;
                    final boolean b2 = clazz.isInstance(k);
                    if (!b2) {
                        break Label_0050;
                    }
                    break Label_0050;
                }
                catch (NullPointerException ex16) {
                    throw ex16;
                }
                try {
                    final Class<T> clazz = this.a;
                    final k k = a;
                    final boolean b2 = clazz.isInstance(k);
                    if (!b2) {
                        throw new IllegalArgumentException();
                    }
                }
                catch (NullPointerException ex17) {
                    throw ex17;
                }
            }
            b = this;
        }
        b.h(this.a.cast(a));
    }
    
    protected void h(final T a) {
    }
}
