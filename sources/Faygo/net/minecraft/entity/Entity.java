...
import me.valk.event.events.entity.EventEntityStep;
...
public abstract class Entity implements ICommandSender {
    ...
    /**
     * MotionBlur entity even if it is outside the camera frustum. Only true in
	...
	if (width != this.width || height != this.height) {
	    ...
	    this.width = width;
	    this.height = height;
	    this.func_174826_a(new AxisAlignedBB(this.getEntityBoundingBox().minX, this.getEntityBoundingBox().minY,
	...
	this.func_174826_a(new AxisAlignedBB(x - (double) var7, y, z - (double) var7, x + (double) var7,
		y + (double) var8, z + (double) var7));
    }
	...
	return this.worldObj.getCollidingBoundingBoxes(this, p_174809_1_).isEmpty()
		&& !this.worldObj.isAnyLiquid(p_174809_1_);
    }
	...
	if (this.noClip) {
	    ...
	    double var15 = y;
	    double var17 = z;
        EventSafewalk eventSafewalk = (EventSafewalk) EventSystem.call(new EventSafewalk());
        boolean var19 = this.onGround && this instanceof EntityPlayer && (isSneaking() || eventSafewalk.isCancelled());

	    if (var19) {
		double var20;

		for (var20 = 0.05D; x != 0.0D && this.worldObj
	    ...
	    for (Iterator var22 = var53.iterator(); var22
		    .hasNext(); y = var23.calculateYOffset(this.getEntityBoundingBox(), y)) {
	    ...
	    for (var55 = var53.iterator(); var55
		    .hasNext(); x = var24.calculateXOffset(this.getEntityBoundingBox(), x)) {
	    ...
	    for (var55 = var53.iterator(); var55
		    .hasNext(); z = var24.calculateZOffset(this.getEntityBoundingBox(), z)) {
		var24 = (AxisAlignedBB) var55.next();
	    }

	    this.func_174826_a(this.getEntityBoundingBox().offset(0.0D, 0.0D, z));

	    EventEntityStep event = new EventEntityStep(this, this.stepHeight, EventType.PRE);
	    EventSystem.call(event);

	    if (event.getStepHeight() > 0.0F && var54 && (var13 != x || var17 != z)) {
		double var56 = x;
		double var25 = y;
		double var27 = z;
		...
		List var30 = this.worldObj.getCollidingBoundingBoxes(this,
			this.getEntityBoundingBox().addCoord(var13, y, var17));
		...
		for (Iterator var50 = var30.iterator(); var50
			.hasNext(); y = var51.calculateYOffset(this.getEntityBoundingBox(), y)) {
		...
		if (var56 * var56 + var27 * var27 >= x * x + z * z) {
		    ...
		    EventSystem.call(new EventEntityStep(this, event.getStepHeight(), EventType.POST));
		}
	    }
		...
		this.distanceWalkedModified = (float) ((double) this.distanceWalkedModified
			+ (double) MathHelper.sqrt_double(var61 * var61 + var66 * var66) * 0.6D);
		this.distanceWalkedOnStepModified = (float) ((double) this.distanceWalkedOnStepModified
			+ (double) MathHelper.sqrt_double(var61 * var61 + var64 * var64 + var66 * var66) * 0.6D);

		if (this.distanceWalkedOnStepModified > (float) this.nextStepDistance
			&& var60.getMaterial() != Material.air) {
	...
	BlockPos var1 = new BlockPos(this.getEntityBoundingBox().minX + 0.001D,
		this.getEntityBoundingBox().minY + 0.001D, this.getEntityBoundingBox().minZ + 0.001D);
	BlockPos var2 = new BlockPos(this.getEntityBoundingBox().maxX - 0.001D,
	...
	return this.inWater || this.worldObj.func_175727_C(new BlockPos(this.posX, this.posY, this.posZ))
		|| this.worldObj.func_175727_C(new BlockPos(this.posX, this.posY + (double) this.height, this.posZ));
    }
	...
	for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
	    ...
	    this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double) var4,
		    (double) (var2 + 1.0F), this.posZ + (double) var5, this.motionX,
	...
	for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
	    var4 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
	    var5 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
	    this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double) var4,
		    (double) (var2 + 1.0F), this.posZ + (double) var5, this.motionX, this.motionY, this.motionZ,
		    new int[0]);
	}
    }
	...
	if (var6.getRenderType() != -1) {
	    this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK,
		    ...
		    1.5D, -this.motionZ * 4.0D, new int[] { Block.getStateId(var5) });
	}
    }
	...
	this.setLocationAndAngles((double) p_174828_1_.getX() + 0.5D, (double) p_174828_1_.getY(),
		(double) p_174828_1_.getZ() + 0.5D, p_174828_2_, p_174828_3_);
    }
	...
	if (p_174824_1_ == 1.0F) {
	    ...
	    double var4 = this.prevPosY + (this.posY - this.prevPosY) * (double) p_174824_1_
	...
	if (itemStackIn.stackSize != 0 && itemStackIn.getItem() != null) {
	    EntityItem var3 = new EntityItem(this.worldObj, this.posX, this.posY + (double) offsetY, this.posZ,
		    itemStackIn);
	...
	if (this.noClip) {
		...
		if (this.worldObj.getBlockState(new BlockPos(var2, var4 + (double) this.getEyeHeight(), var6))
			.getBlock().isVisuallyOpaque()) {
		    return true;
		}
	    }
	...
	List var11 = this.worldObj.getCollidingBoundingBoxes(this,
    ...
    public float getCollisionBorderSize() {
    	if(Vital.getManagers().getModuleManager().getModuleFromClass(GCheatAura.class).getState()) {
    		return 0.5f;
    	}
	return 0.1F;
    }
    ...
    public Vec3 getPositionEyes(final float partialTicks) {
		...
		final double var2 = this.prevPosX + (this.posX - this.prevPosX) * partialTicks;
		final double var3 = this.prevPosY + (this.posY - this.prevPosY) * partialTicks + this.getEyeHeight();
		final double var4 = this.prevPosZ + (this.posZ - this.prevPosZ) * partialTicks;
