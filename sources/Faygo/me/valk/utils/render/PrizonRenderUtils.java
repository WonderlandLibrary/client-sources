...
public class PrizonRenderUtils {
		...
		Label_0237: {
			if (renderEntity instanceof EntityLivingBase) {
				...
				if (entity instanceof EntityPlayer) {
					Vital.getVital();
					if (Vital.getManagers().getFriendManager().hasFriend(entity.getName())) {
	...
	public static void waypoint(final WayPoint p, final double x, final double y) {
		final EntityPlayerSP player = Minecraft.getMinecraft().thePlayer;
		final double pX = p.pos.getX();
		final double pY = p.pos.getY();
		final double pZ = p.pos.getZ();
		final double dist = round(player.getDistance(pX, pY, pZ), 1);
		String color = "§f";
		if(dist < 50) {
			color = "§a";
		}
		...
		final String str = (String.valueOf(p.name) + " §7[" + color + dist + "§7]");
		final float center = -(Minecraft.getMinecraft().fontRendererObj.getStringWidth(str) / 2);
		GL11.glPushMatrix();
		GL11.glScaled( 0.8, 0.8, 0.8);
		drawBorderedRect(center - 6.0f, (float) y - 16.0f,
				center + Minecraft.getMinecraft().fontRendererObj.getStringWidth(str) + 6.0f, (float) y , 1.0,
				-16777216, -1612107);
		drawstring(str, center, (float) y * 2.0f - 12.0f, -65536);
		GL11.glPopMatrix();
	}
}