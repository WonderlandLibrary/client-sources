...
import me.valk.event.events.player.EventPlayerChat;
import me.valk.event.events.player.EventPlayerInteractEntity;
...
import me.valk.event.events.screen.EventOpaqueBlock;
...
public class Player extends EntityPlayerSP {
	...
	public Location getLastPosition() {
		return new Location(this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ);
	}
	...
	public static boolean isUnderBlock() {
		if (Wrapper.getPlayer() == null)
			return false;
		;
		int y = (int) Wrapper.getPlayer().getLocation().add(0, 2, 0).getY();
		for (int x = MathHelper.floor_double(Wrapper.getPlayer().boundingBox.minX); x < MathHelper
				.floor_double(Wrapper.getPlayer().boundingBox.maxX) + 1; x++) {
			for (int z = MathHelper.floor_double(Wrapper.getPlayer().boundingBox.minZ); z < MathHelper
					.floor_double(Wrapper.getPlayer().boundingBox.maxZ) + 1; z++) {
	...
	public void func_175161_p() {
		EventMotion event = (EventMotion) EventSystem.call(new EventMotion(this.getLocation(), this.onGround,
		...
		EventSystem.call(new EventMotion(this.getLocation(), this.onGround, this.rotationYaw, this.rotationPitch,
				EventType.POST));
	}
	...
	public void sendChatMessage(String p_71165_1_) {
		...
		if(message.startsWith(".")){
			...
			for(Command command : Vital.getManagers().getCommandManager().getContents()){
