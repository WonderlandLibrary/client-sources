...
public class ElementComboBox extends Element {
	...
	public double privheight;
	
	public ElementComboBox(ModuleButton iparent, ModeSetting iset) {
		parent = iparent;
		set = iset;
		super.setup();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		 ...
		 */
		Gui.drawRect(x, y, x + width, y + privheight, 0xff1a1a1a);
		privheight = comboextended ? set.modes.size() * (FontUtil.getFontHeight() + 2) + 15 : 15;
		FontUtil.drawTotalCenteredString(set.name, x + width / 2, y + 15/2, 0xffffffff);
		int clr1 = color;
		int clr2 = temp.getRGB();

		Gui.drawRect(x, y + 14, x + width, y + 15, 0x77000000);
		if (comboextended) {
			Gui.drawRect(x, y + 15, x + width, y + privheight, 0xaa121212);
			double ay = y + 15;
			for (String sld : set.modes) {
				 ...
				 */
				if (sld.equalsIgnoreCase(set.name)) {
