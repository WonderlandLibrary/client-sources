...
import me.xatzdevelopments.events.listeners.EventSafeWalk;
...
public class Scaffold2 extends Module {
    ...
    private final FTimer timerTower = new FTimer();
    ...
    int currentItem;
    double groundy = 0;
    FTimer timerMotion = new FTimer(), boostCap = new FTimer();
    private int slot;

    public Scaffold2() {
        super("Scaffold", Keyboard.KEY_G, Category.WORLD,"Places blocks under the player");
    ...
    public void onEnable() {
        ...
        if (toggleFly.isEnabled())
            Objects.requireNonNull(Xatz.getModuleByName("Fly")).setToggled(false);
        super.onEnable();
    }
    ...
    public void onEvent(Event ev) {
        ...
        if (ev instanceof EventUpdate) {
            if (mc.thePlayer.swingProgress <= 0.0f && !mc.thePlayer.isEating() && !Objects.requireNonNull(Xatz.getModuleByName("KillAura")).isEnabled()) {
        ...
        if (e.isPre()) {
            ...
            if ((mc.gameSettings.keyBindJump.isPressed()) && (mc.thePlayer.moveForward == 0.0F) && (mc.thePlayer.moveStrafing == 0.0F)) {
        ...
        if ((localBlockData != null)) {
            if (e.isPre()) {
                ...
                if (!this.timerMotion.hasTimeElapsed((long) (delay.getValue() + Math.random() * 4), true)
                        && isNotCollidingBelow(0.01) && !mc.gameSettings.keyBindJump.isPressed()) {
                    return;
                }
