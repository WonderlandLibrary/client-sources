...
import me.xatzdevelopments.events.listeners.EventChatmessage;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void sendChatMessage(String message)
    {
		...
		if (message.startsWith("@")) {
			...
			if(Xatz.getModuleByName("IRC").isEnabled()) {
			if (irc.isConnected()) {
				irc.sendMessage("#Xatz-Imp3rIum69",  message.replace("@", ""));
				irc.getLines().add(new IrcLine(irc.getChatLine(true), message.replace("@", ""), irc.getName(), false));
				irc.setUnreadMessages(true);
			  }
			} else {
				Xatz.tellPlayer("§7[§3IRC§7] Not Connected To IRC. Please enable the IRC module to communicate in the irc chat");
			}
			return;
		}
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding())
        {
                ...
                final EventUsingItem e = new EventUsingItem();
                e.setType(EventType.PRE);
                Xatz.onEvent(e);
                if (!e.isCancelled()) {
