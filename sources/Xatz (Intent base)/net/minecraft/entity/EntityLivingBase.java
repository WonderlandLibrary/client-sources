...
import me.xatzdevelopments.events.listeners.EventJumping;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
    	double speed = Xatz.getModuleByName("Camera").getNumberSetting("Swing Speed").getValue();
        return this.isPotionActive(Potion.digSpeed) ? (int)speed - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? (int)speed + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : (int)speed);
    }
     ...
     */
    protected void jump(boolean half)
    {
    	EventJumping e = new EventJumping(this.rotationYaw);
    	e.setType(EventType.PRE);
    	Xatz.onEvent(e);
    	
    	if(half) {
        this.motionY = (double)this.func_175134_bD() / 1.7;
    	} else {
