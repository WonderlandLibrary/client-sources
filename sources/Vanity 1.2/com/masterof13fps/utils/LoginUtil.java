            ...
            Thread loginToken = new Thread("loginToken") {
                public void run() {
                    try {
                        ...
                        Minecraft.mc().session = new Session(service.getSelectedProfile().getName(), service.getSelectedProfile().getId().toString(), service.getAuthenticatedToken(), "LEGACY");
                        this.stop();
                    } catch (Exception e) {
    ...
    public void loginCracked(String name) {
        Thread loginCracked = new Thread("loginCracked") {
            public void run() {
                ...
                Minecraft.mc().session = new Session(name, "", "", "LEGACY");
                this.stop();
            }
        };
        loginCracked.start();
