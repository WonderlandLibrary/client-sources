...
extends Render<T> {
    ...
    public static float chamsRed;
    public static float chamsGreen;
    public static float chamsBlue;
    public static float chamsAlpha;
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                this.mainModel.setRotationAngles(f6, f5, f7, f2, f8, f4, (Entity)entity);
                if (this.renderOutlines) {
                    boolean flag1 = this.setScoreTeamColor((EntityLivingBase)entity);
                    this.renderModel(entity, f6, f5, f7, f2, f8, f4);
                    if (flag1) {
                    ...
                    if (!(entity instanceof EntityPlayer) || !((EntityPlayer)entity).isSpectator()) {
                        this.renderLayers(entity, f6, f5, partialTicks, f7, f2, f8, f4);
                    }
                }
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        ...
        if (flag || flag1) {
            ...
            if (mc.gameSettings.renderDistanceChunks < 4) {
                maxDistance = (mc.gameSettings.renderDistanceChunks - 1) * 16;
            }
            if (flag1) {
                GlStateManager.pushMatrix();
            ...
            Color color = null;
            color = new Color((int)chamsRed, (int)chamsGreen, (int)chamsBlue, (int)chamsAlpha);
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
        ...
        boolean enabled = November.INSTANCE.getModuleManager().getModule("Chams").isEnabled();
        ...
        boolean bl = isModuleEnabled = entitylivingbaseIn instanceof EntityPlayer && entitylivingbaseIn != Minecraft.getMinecraft().thePlayer;
        if (isModuleEnabled) {
            isModuleEnabled = enabled;
        }
        if (!(flag || flag1 || isModuleEnabled)) {
            return false;
        }
        ...
        } else if (isModuleEnabled && ((EntityLivingBase)entitylivingbaseIn).hurtTime == 0) {
            boolean lightning = GL11.glIsEnabled((int)2896);
            GlStateManager.disableLighting();
            this.brightnessBuffer.put(chamsRed);
            this.brightnessBuffer.put(chamsGreen);
            this.brightnessBuffer.put(chamsBlue);
            this.brightnessBuffer.put(chamsAlpha);
            if (Config.isShaders()) {
                Shaders.setEntityColor(chamsRed, chamsGreen, chamsBlue, chamsAlpha);
            }
            if (lightning) {
                GlStateManager.enableLighting();
            }
