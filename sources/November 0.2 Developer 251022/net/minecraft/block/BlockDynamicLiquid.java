...
extends BlockLiquid {
    ...
    private int func_176374_a(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (Object enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (distance >= 4 || (j = this.func_176374_a(worldIn, blockpos, distance + 1, ((EnumFacing)enumfacing).getOpposite())) >= i) continue;
            i = j;
        }
        return i;
    }
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos) {
        ...
        for (Object enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            int j = this.isBlocked(worldIn, blockpos.down(), worldIn.getBlockState(blockpos.down())) ? this.func_176374_a(worldIn, blockpos, 1, ((EnumFacing)enumfacing).getOpposite()) : 0;
            if (j < i) {
                set.clear();
            }
            if (j > i) continue;
