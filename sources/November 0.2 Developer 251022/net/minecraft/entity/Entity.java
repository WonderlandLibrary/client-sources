...
implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = this.onGround && (this.isSneaking() || Scaffold.safewalk.isEnabled() && November.INSTANCE.moduleManager.getModule("Scaffold").isEnabled()) && this instanceof EntityPlayer;
            this.safe = false;
            if (Minecraft.getMinecraft().thePlayer != null) {
                try {
                    if (November.INSTANCE.getModuleManager().getModule("Scaffold").isEnabled() && Scaffold.safewalk.isEnabled() && !Minecraft.getMinecraft().gameSettings.keyBindSneak.isKeyDown() && Minecraft.getMinecraft().thePlayer.fallDistance < 2.0f) {
                        this.safe = true;
                    }
                }
                catch (Exception e) {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            EventStrafe event = new EventStrafe(forward, strafe, friction);
            November.INSTANCE.getBus().post((Object)event);
    ...
    public float getDistanceToEntityRender(Entity entityIn) {
        ...
        float x = (float)(entityIn.lastTickPosX + (entityIn.posX - entityIn.lastTickPosX) * (double)mc.timer.renderPartialTicks - mc.getRenderManager().renderPosX);
        float y = (float)(entityIn.lastTickPosY + (entityIn.posY - entityIn.lastTickPosY) * (double)mc.timer.renderPartialTicks - mc.getRenderManager().renderPosY);
        float z = (float)(entityIn.lastTickPosZ + (entityIn.posZ - entityIn.lastTickPosZ) * (double)mc.timer.renderPartialTicks - mc.getRenderManager().renderPosZ);
