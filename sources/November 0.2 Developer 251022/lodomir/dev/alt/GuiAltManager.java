...
import lodomir.dev.alt.GuiAddAlt;
import lodomir.dev.alt.GuiAltLogin;
import lodomir.dev.alt.GuiRenameAlt;
...
extends GuiScreen {
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        GuiAltManager.drawCenteredString(fontRendererObj, sb2.append(AltManager.registry.size()).append(" alts").toString(), width / 2, 10, -1);
        GuiAltManager.drawCenteredString(this.fontRendererObj, this.loginThread == null ? this.status : this.loginThread.getStatus(), width / 2, 20, -1);
        GL11.glPushMatrix();
        this.prepareScissorBox(0.0f, 33.0f, width, height - 50);
        GL11.glEnable((int)3089);
        int y2 = 38;
        AltManager altManager2 = November.INSTANCE.altManager;
        for (Alt alt2 : AltManager.registry) {
            ...
            if (!(alt2 == this.selectedAlt ? (!this.isMouseOverAlt(par1, par2, y2 - this.offset) || !Mouse.isButtonDown((int)0)) && this.isMouseOverAlt(par1, par2, y2 - this.offset) : this.isMouseOverAlt(par1, par2, y2 - this.offset) && Mouse.isButtonDown((int)0) || this.isMouseOverAlt(par1, par2, y2 - this.offset))) {
                // empty if block
            }
            GuiAltManager.drawCenteredString(this.fontRendererObj, name, width / 2, y2 - this.offset, -1);
            GuiAltManager.drawCenteredString(this.fontRendererObj, pass, width / 2, y2 - this.offset + 10, 0x555555);
            y2 += 26;
        }
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(7, width - 100, 0, 100, 20, "Microsoft"));
        this.login.enabled = false;
        this.remove.enabled = false;
        this.rename.enabled = false;
    }
