  ...
  extends CommandBase
{
  ...
  {
    if (args.length < 7) {
      throw new WrongUsageException("orders.fill.usage", new Object[0]);
    }
    ...
    if (var9 > 32768) {
      throw new CommandException("orders.fill.tooManyBlocks", new Object[] { Integer.valueOf(var9), Integer.valueOf(32768) });
    }
    if ((var7.getY() >= 0) && (var8.getY() < 256))
    {
      ...
      for (int var11 = var7.getZ(); var11 < var8.getZ() + 16; var11 += 16) {
        for (int var12 = var7.getX(); var12 < var8.getX() + 16; var12 += 16) {
          if (!var10.isBlockLoaded(new BlockPos(var12, var8.getY() - var7.getY(), var11))) {
            throw new CommandException("orders.fill.outOfWorld", new Object[0]);
          }
        }
      }
      ...
      if ((args.length >= 10) && (var5.hasTileEntity()))
      {
        ...
        {
          throw new CommandException("orders.fill.tagError", new Object[] { var21.getMessage() });
        }
      }
      ...
      for (int var14 = var7.getZ(); var14 <= var8.getZ(); var14++) {
        for (int var15 = var7.getY(); var15 <= var8.getY(); var15++) {
          for (int var16 = var7.getX(); var16 <= var8.getX(); var16++)
          {
            ...
            IBlockState var19 = var5.getStateFromMeta(var6);
      ...
      if (var9 <= 0) {
        throw new CommandException("orders.fill.failed", new Object[0]);
      }
      sender.func_174794_a(CommandResultStats.Type.AFFECTED_BLOCKS, var9);
      notifyOperators(sender, this, "orders.fill.success", new Object[] { Integer.valueOf(var9) });
    }
    else
    {
      throw new CommandException("orders.fill.outOfWorld", new Object[0]);
  ...
  {
    return args.length == 9 ? getListOfStringsMatchingLastWord(args, new String[] { "replace", "destroy", "keep", "hollow", "outline" }) : args.length == 7 ? func_175762_a(args, Block.blockRegistry.getKeys()) : (args.length > 3) && (args.length <= 6) ? func_175771_a(args, 3, pos) : (args.length > 0) && (args.length <= 3) ? func_175771_a(args, 0, pos) : null;
  }
}
