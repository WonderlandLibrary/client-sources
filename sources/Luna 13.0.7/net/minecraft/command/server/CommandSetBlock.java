  ...
  extends CommandBase
{
  ...
  {
    if (args.length < 4) {
      throw new WrongUsageException("orders.setblock.usage", new Object[0]);
    }
    ...
    if (!var6.isBlockLoaded(var3)) {
      throw new CommandException("orders.setblock.outOfWorld", new Object[0]);
    }
    ...
    if ((args.length >= 7) && (var4.hasTileEntity()))
    {
      ...
      {
        throw new CommandException("orders.setblock.tagError", new Object[] { var12.getMessage() });
      }
    }
    if (args.length >= 6) {
      if (args[5].equals("destroy"))
      {
        ...
        if (var4 == Blocks.air) {
          notifyOperators(sender, this, "orders.setblock.success", new Object[0]);
        }
      }
      else if ((args[5].equals("keep")) && (!var6.isAirBlock(var3)))
      {
        throw new CommandException("orders.setblock.noChange", new Object[0]);
      }
    }
  ...
  {
    return args.length == 6 ? getListOfStringsMatchingLastWord(args, new String[] { "replace", "destroy", "keep" }) : args.length == 4 ? func_175762_a(args, Block.blockRegistry.getKeys()) : (args.length > 0) && (args.length <= 3) ? func_175771_a(args, 0, pos) : null;
  }
}
