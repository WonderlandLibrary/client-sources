...
public class OffHandCrystal extends Module {
   ...
   private void SwitchOffHandIfNeed(Item var1) {
      Item var3 = Items.field_185158_cP;
   ...
   private float calculateDamageAABB(BlockPos var1) {
      List var2 = (List)mc.field_71441_e.func_72839_b((Entity)null, new AxisAlignedBB(var1)).stream().filter(OffHandCrystal::lambda$calculateDamageAABB$1).collect(Collectors.toList());
      float var3 = 0.0F;
   ...
   private boolean crystalCheck() {
      ...
      while(var5.hasNext()) {
         ...
         var3 += var6;
         if (this.ecpMode.getValue() == OffHandCrystal.ECPMode.TOTEMATHEALTH) {
            var2 = mc.field_71439_g.func_110143_aJ() + mc.field_71439_g.func_110139_bj() - var6 <= (float)(Integer)this.health.getValue();
         } else if (this.ecpMode.getValue() == OffHandCrystal.ECPMode.TOTEMATONE) {
            var2 = mc.field_71439_g.func_110143_aJ() + mc.field_71439_g.func_110139_bj() - var6 <= 1.0F;
         } else if (this.ecpMode.getValue() == OffHandCrystal.ECPMode.TOTEMATZERO) {
            var2 = mc.field_71439_g.func_110143_aJ() + mc.field_71439_g.func_110139_bj() - var6 < 1.0F;
         }
   ...
   public OffHandCrystal() {
      this.ecpMode = this.register(SettingsManager.e("ECP Mode", OffHandCrystal.ECPMode.TOTEMATHEALTH));
      this.totemOnElytra = this.register(SettingsManager.b("Force Totem on Elytra", true));
      this.crystalCooldown = this.register(SettingsManager.i("Return to Crystal Delay", 5));
      this.moving = false;
   ...
   private void lambda$new$0(EventPlayerUpdate var1) {
      if (!(mc.field_71462_r instanceof GuiContainer)) {
         if (this.shouldTotem() || (Boolean)this.totemOnElytra.getValue() && mc.field_71439_g.func_184613_cA() || mc.field_71439_g.field_70143_R >= (Float)this.fallDistance.getValue() && !mc.field_71439_g.func_184613_cA()) {
            ...
            this.SwitchOffHandIfNeed(Items.field_185158_cP);
            this.cooldown = (Integer)this.crystalCooldown.getValue();
         }
      }
   }
   ...
   private static enum ECPMode {
      ...
      private static final OffHandCrystal.ECPMode[] $VALUES = new OffHandCrystal.ECPMode[]{TOTEMATONE, TOTEMATZERO, TOTEMATHEALTH};
      TOTEMATZERO;
   }
}
