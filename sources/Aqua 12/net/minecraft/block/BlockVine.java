...
public class BlockVine extends Block
{
    ...
    private boolean recheckGrownSides(final World worldIn, final BlockPos pos, IBlockState state) {
        ...
        for (final EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (state.getValue((IProperty<Boolean>)propertybool) && !this.canPlaceOn(worldIn.getBlockState(pos.offset(enumfacing)).getBlock())) {
            ...
            if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos1)) {
                if (!flag) {
                    ...
                    for (final EnumFacing enumfacing2 : EnumFacing.Plane.HORIZONTAL) {
                        if (rand.nextBoolean() || !this.canPlaceOn(worldIn.getBlockState(blockpos1.offset(enumfacing2)).getBlock())) {
                ...
                if (block2.blockMaterial == Material.air) {
                    ...
                    for (final EnumFacing enumfacing5 : EnumFacing.Plane.HORIZONTAL) {
                        if (rand.nextBoolean()) {
                            iblockstate4 = iblockstate4.withProperty((IProperty<Comparable>)getPropertyFor(enumfacing5), false);
                        }
                    }
