...
public class ServerCommandManager extends CommandHandler implements IAdminCommand
{
    ...
    public void notifyOperators(final ICommandSender sender, final ICommand command, final int flags, final String msgFormat, final Object... msgParams) {
        ...
        if (flag) {
            for (final EntityPlayer entityplayer : minecraftserver.getConfigurationManager().getPlayerList()) {
                if (entityplayer != sender && minecraftserver.getConfigurationManager().canSendCommands(entityplayer.getGameProfile()) && command.canCommandSenderUseCommand(sender)) {
                    final boolean flag2 = sender instanceof MinecraftServer && MinecraftServer.getServer().shouldBroadcastConsoleToOps();
                    final boolean flag3 = sender instanceof RConConsoleSource && MinecraftServer.getServer().shouldBroadcastRconToOps();
