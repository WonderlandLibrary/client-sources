...
public class EntitySkeleton extends EntityMob implements IRangedAttackMob
{
    ...
    protected void dropFewItems(final boolean wasRecentlyHit, final int lootingModifier) {
        if (this.getSkeletonType() == 1) {
            for (int i = this.rand.nextInt(3 + lootingModifier) - 1, j = 0; j < i; ++j) {
                this.dropItem(Items.coal, 1);
            }
        }
        else {
            for (int k = this.rand.nextInt(3 + lootingModifier), i2 = 0; i2 < k; ++i2) {
                this.dropItem(Items.arrow, 1);
            }
        }
        for (int l = this.rand.nextInt(3 + lootingModifier), j2 = 0; j2 < l; ++j2) {
    ...
    public void attackEntityWithRangedAttack(final EntityLivingBase target, final float p_82196_2_) {
        final EntityArrow entityarrow = new EntityArrow(this.worldObj, this, target, 1.6f, (float)(14 - this.worldObj.getDifficulty().getDifficultyId() * 4));
