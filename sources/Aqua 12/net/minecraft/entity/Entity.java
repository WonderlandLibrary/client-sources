...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveFlying(float strafe, float forward, final float friction) {
        ...
        if (f >= 1.0E-4f) {
            ...
            forward *= f;
            try {
                final boolean killAura = Aqua.moduleManager.getModuleByName("Killaura").isToggled() && Killaura.target != null && Aqua.setmgr.getSetting("KillauraCorrectMovement").isState();
                final boolean bedAura = (Aqua.moduleManager.getModuleByName("BedAura").isToggled() && BedAura.rotating && Aqua.setmgr.getSetting("BedAuraMoveFixMode").getCurrentMode().equalsIgnoreCase("Normal") && Aqua.setmgr.getSetting("BedAuraCorrectMovement").isState()) || (Aqua.moduleManager.getModuleByName("BedAura").isToggled() && BedAura.rotating && Aqua.setmgr.getSetting("BedAuraMoveFixMode").getCurrentMode().equalsIgnoreCase("Silent") && Aqua.setmgr.getSetting("BedAuraCorrectMovement").isState());
                if (killAura || bedAura) {
                    final float f2 = MathHelper.sin(RotationUtil.yaw * 3.1415927f / 180.0f);
                    final float f3 = MathHelper.cos(RotationUtil.yaw * 3.1415927f / 180.0f);
    ...
    protected final Vec3 getVectorForRotation(final float pitch, final float yaw) {
        final boolean build = Aqua.moduleManager.getModuleByName("Scaffold").isToggled() || (Aqua.moduleManager.getModuleByName("BedAura").isToggled() && BedAura.rotating) || Aqua.moduleManager.getModuleByName("Longjump").isToggled() || (Aqua.moduleManager.getModuleByName("Killaura").isToggled() && Killaura.target != null && Aqua.setmgr.getSetting("KillauraLegitAttack").isState());
        final float f = MathHelper.cos(build ? (-this.getRotationYawHead() * 0.017453292f - 3.1415927f) : (-yaw * 0.017453292f - 3.1415927f));
        final float f2 = MathHelper.sin(build ? (-this.getRotationYawHead() * 0.017453292f - 3.1415927f) : (-yaw * 0.017453292f - 3.1415927f));
        final float f3 = -MathHelper.cos(build ? (-this.rotationPitchHead * 0.017453292f) : (-pitch * 0.017453292f));
        final float f4 = MathHelper.sin(build ? (-this.rotationPitchHead * 0.017453292f) : (-pitch * 0.017453292f));
