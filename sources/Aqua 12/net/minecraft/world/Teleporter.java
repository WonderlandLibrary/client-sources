...
public class Teleporter
{
    ...
    public boolean placeInExistingPortal(final Entity entityIn, final float rotationYaw) {
        ...
        if (d0 >= 0.0) {
            ...
            double d6 = (blockpattern$patternhelper.getFinger().getAxis() == EnumFacing.Axis.X) ? blockpattern$patternhelper.getPos().getZ() : ((double)blockpattern$patternhelper.getPos().getX());
            d4 = blockpattern$patternhelper.getPos().getY() + 1 - entityIn.func_181014_aG().yCoord * blockpattern$patternhelper.func_181119_e();
            if (flag2) {
                ++d6;
            }
            ...
            entityIn.setLocationAndAngles(d3, d4, d5, entityIn.rotationYaw = rotationYaw - entityIn.getTeleportDirection().getOpposite().getHorizontalIndex() * 90 + blockpattern$patternhelper.getFinger().getHorizontalIndex() * 90, entityIn.rotationPitch);
            return true;
        }
        return false;
    }
    ...
    public boolean makePortal(final Entity entityIn) {
        ...
        double d0 = -1.0;
        final int j = MathHelper.floor_double(entityIn.posX);
        final int k = MathHelper.floor_double(entityIn.posY);
        final int l = MathHelper.floor_double(entityIn.posZ);
        int i2 = j;
        int j2 = k;
        int k2 = l;
        int l2 = 0;
        ...
        for (int j3 = j - i; j3 <= j + i; ++j3) {
            ...
            Label_0461:
                for (int j4 = this.worldServerInstance.getActualHeight() - 1; j4 >= 0; --j4) {
                    if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(j3, j4, l3))) {
                        while (j4 > 0 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(j3, j4 - 1, l3))) {
                            --j4;
                        }
                            ...
                            for (int j5 = 0; j5 < 3; ++j5) {
                                for (int k4 = 0; k4 < 4; ++k4) {
                                    for (int l5 = -1; l5 < 4; ++l5) {
                                        ...
                                        blockpos$mutableblockpos.set(i5, j6, k5);
            ...
            for (int l6 = j - i; l6 <= j + i; ++l6) {
                ...
                Label_0833:
                    for (int i6 = this.worldServerInstance.getActualHeight() - 1; i6 >= 0; --i6) {
                        if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(l6, i6, j7))) {
                            while (i6 > 0 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(l6, i6 - 1, j7))) {
                                --i6;
                            }
                            for (int k6 = i3; k6 < i3 + 2; ++k6) {
                                ...
                                for (int j10 = 0; j10 < 4; ++j10) {
                                    for (int j11 = -1; j11 < 4; ++j11) {
                                        ...
                                        blockpos$mutableblockpos.set(j12, i7, j13);
