...
public class ModelBakery
{
        ...
        for (final ModelResourceLocation modelresourcelocation : p_177591_1_) {
            try {
                ...
                try {
                    ...
                    ModelBakery.LOGGER.warn("Unable to load variant: " + modelresourcelocation.getVariant() + " from " + modelresourcelocation, exception);
                }
            }
            catch (Exception exception2) {
                ModelBakery.LOGGER.warn("Unable to load definition " + modelresourcelocation, exception2);
            }
        }
    }
    ...
    private void registerVariantNames() {
        ...
        for (final Map.Entry<RegistryDelegate<Item>, Set<String>> entry : ModelBakery.customVariantNames.entrySet()) {
            this.variantNames.put(entry.getKey().get(), (List<String>)Lists.newArrayList((Iterator<?>)entry.getValue().iterator()));
        }
        CustomItems.update();
    ...
    private void loadModels() {
        ...
        while (!deque.isEmpty()) {
            ...
            try {
                ...
                ModelBakery.LOGGER.warn("In parent chain: " + ModelBakery.JOINER.join(this.getParentPath(resourcelocation3)) + "; unable to load model: '" + resourcelocation3 + "'");
            }
    ...
    public static void addVariantName(final Item p_addVariantName_0_, final String... p_addVariantName_1_) {
        ...
        if (ModelBakery.customVariantNames.containsKey(registrydelegate)) {
            ModelBakery.customVariantNames.get(registrydelegate).addAll(Lists.newArrayList(p_addVariantName_1_));
        }
        else {
            ModelBakery.customVariantNames.put(registrydelegate, Sets.newHashSet(p_addVariantName_1_));
        }
    }
    ...
    public static <T extends ResourceLocation> void registerItemVariants(final Item p_registerItemVariants_0_, final T... p_registerItemVariants_1_) {
        ...
        if (!ModelBakery.customVariantNames.containsKey(registrydelegate)) {
            ModelBakery.customVariantNames.put(registrydelegate, (Set<String>)Sets.newHashSet());
        }
