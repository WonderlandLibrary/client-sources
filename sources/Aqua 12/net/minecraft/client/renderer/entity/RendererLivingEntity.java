...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected boolean setScoreTeamColor(final T entityLivingBaseIn) {
        ...
        GlStateManager.setActiveTexture(OpenGlHelper.defaultTexUnit);
        if (Aqua.setmgr.getSetting("ESPVanillaTeamColor").isState()) {
            GlStateManager.color(f1, f2, f3, (float)Aqua.setmgr.getSetting("ESPShaderAlpha").getCurrentNumber() / 255.0f);
        }
        else {
            final int[] rgb = Aqua.setmgr.getSetting("ESPAstolfoColors").isState() ? ESP.getRGB(ESP.SkyRainbow(20, 1.0f, 0.5f).getRGB()) : (Aqua.setmgr.getSetting("ESPRainbow").isState() ? ESP.getRGB(ESP.rainbowESP(0)) : (Aqua.setmgr.getSetting("ESPClientColor").isState() ? ESP.getRGB(Aqua.setmgr.getSetting("HUDColor").getColor()) : ESP.getRGB(Aqua.setmgr.getSetting("ESPColor").getColor())));
            final Color color = Aqua.setmgr.getSetting("ESPFade").isState() ? ColorUtils.getColorAlpha(Arraylist.getGradientOffset(new Color(Aqua.setmgr.getSetting("HUDColor").getColor()), new Color(Aqua.setmgr.getSetting("ArraylistColor").getColor()), 15.0), 255) : new Color(rgb[0], rgb[1], rgb[2]);
            GlStateManager.color(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, (float)Aqua.setmgr.getSetting("ESPShaderAlpha").getCurrentNumber() / 255.0f);
        }
