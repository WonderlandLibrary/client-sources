...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(final float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            Vec3 vec6 = null;
            final float f = 1.0f;
            final List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec4.xCoord * d0, vec4.yCoord * d0, vec4.zCoord * d0).expand(1.0, 1.0, 1.0), Predicates.and((Predicate<? super Entity>)EntitySelectors.NOT_SPECTATING, (Predicate<? super Entity>)new Predicate<Entity>() {
                @Override
    ...
    private void hurtCameraEffect(final float partialTicks) {
        if (!Aqua.moduleManager.getModuleByName("NoHurtCam").isToggled() && this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
    ...
    public void setupCameraTransform(final float partialTicks, final int pass) {
        ...
        if (1.0 != 1.0) {
            ...
            final double x = 1.0;
            this.getClass();
            GlStateManager.scale(x, 1.0, 1.0);
        }
    ...
    private void updateLightmap(final float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            for (int i = 0; i < 256; ++i) {
                ...
                final int color2 = Aqua.setmgr.getSetting("WorldColorColor").getColor();
                float f9 = Aqua.moduleManager.getModuleByName("WorldColor").isToggled() ? (new Color(color2).getRed() / 255.0f) : (f5 + f4);
                float f10 = Aqua.moduleManager.getModuleByName("WorldColor").isToggled() ? (new Color(color2).getGreen() / 255.0f) : (f6 + f7);
                float f11 = Aqua.moduleManager.getModuleByName("WorldColor").isToggled() ? (new Color(color2).getBlue() / 255.0f) : (f3 + f8);
                f9 = f9 * 0.96f + 0.03f;
                f10 = f10 * 0.96f + 0.03f;
                f11 = f11 * 0.96f + 0.03f;
    ...
    private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
        ...
        Aqua.INSTANCE.onEvent(new EventRenderNameTags(partialTicks));
        GlStateManager.shadeModel(7424);
        GlStateManager.depthMask(true);
        GlStateManager.enableCull();
        GlStateManager.disableBlend();
        ...
        Aqua.INSTANCE.onEvent(new EventRender3D(partialTicks));
    ...
    private void addRainParticles() {
        ...
        if (f != 0.0f && Config.isRainSplash()) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (blockpos2.getY() <= blockpos.getY() + 10 && blockpos2.getY() >= blockpos.getY() - 10 && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos2) >= 0.15f) {
    ...
    private void updateFogColor(final float partialTicks) {
        ...
        if (this.mc.gameSettings.renderDistanceChunks >= 4) {
            final double d0 = -1.0;
            final Vec3 vec5 = (MathHelper.sin(world.getCelestialAngleRadians(partialTicks)) > 0.0f) ? new Vec3(-1.0, 0.0, 0.0) : new Vec3(1.0, 0.0, 0.0);
    ...
    private void setupFog(final int startCoords, final float partialTicks) {
        ...
        if (f >= 0.0f) {
            ...
            if (Reflector.ForgeHooksClient_onFogRender.exists()) {
                Reflector.callVoid(Reflector.ForgeHooksClient_onFogRender, this, entity, block, partialTicks, startCoords, f5);
            }
        }
    ...
    private void checkLoadVisibleChunks(final Entity p_checkLoadVisibleChunks_1_, final float p_checkLoadVisibleChunks_2_, final ICamera p_checkLoadVisibleChunks_3_, final boolean p_checkLoadVisibleChunks_4_) {
        ...
        if (Keyboard.isKeyDown(61) && Keyboard.isKeyDown(38)) {
            ...
            this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(chatcomponenttext, 201435902);
        }
    }
