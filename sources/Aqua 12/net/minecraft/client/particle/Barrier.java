...
public class Barrier extends EntityFX
{
    ...
    public void renderParticle(final WorldRenderer worldRendererIn, final Entity entityIn, final float partialTicks, final float rotationX, final float rotationZ, final float rotationYZ, final float rotationXY, final float rotationXZ) {
        ...
        worldRendererIn.pos(f6 - rotationX * 0.5f - rotationXY * 0.5f, f7 - rotationZ * 0.5f, f8 - rotationYZ * 0.5f - rotationXZ * 0.5f).tex(f2, f4).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        worldRendererIn.pos(f6 - rotationX * 0.5f + rotationXY * 0.5f, f7 + rotationZ * 0.5f, f8 - rotationYZ * 0.5f + rotationXZ * 0.5f).tex(f2, f3).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        worldRendererIn.pos(f6 + rotationX * 0.5f + rotationXY * 0.5f, f7 + rotationZ * 0.5f, f8 + rotationYZ * 0.5f + rotationXZ * 0.5f).tex(f, f3).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        worldRendererIn.pos(f6 + rotationX * 0.5f - rotationXY * 0.5f, f7 - rotationZ * 0.5f, f8 + rotationYZ * 0.5f - rotationXZ * 0.5f).tex(f, f4).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
    }
