...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public void draw(final int posX, final int posY, final int texU, final int texV, final int width, final int height, final int red, final int green, final int blue, final int alpha) {
        ...
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldrenderer.pos(posX, posY + height, 0.0).tex(texU * 0.00390625f, (texV + height) * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX + width, posY + height, 0.0).tex((texU + width) * 0.00390625f, (texV + height) * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX + width, posY, 0.0).tex((texU + width) * 0.00390625f, texV * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX, posY, 0.0).tex(texU * 0.00390625f, texV * 0.00390625f).color(red, green, blue, alpha).endVertex();
    ...
    private void runGameLoop() throws IOException {
        final long currentTime = getSystemTime();
    ...
    private void displayDebugInfo(final long elapsedTicksTime) {
        if (this.mcProfiler.profilingEnabled) {
            ...
            worldrenderer.pos(j - 176.0f, k - 96.0f - 16.0f, 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j - 176.0f, k + 320, 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j + 176.0f, k + 320, 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j + 176.0f, k - 96.0f - 16.0f, 0.0).color(200, 0, 0, 0).endVertex();
            tessellator.draw();
            ...
            this.fontRendererObj.drawStringWithShadow(s = decimalformat.format(profiler$result.field_76330_b) + "%", (float)(j + 160 - this.fontRendererObj.getStringWidth(s)), (float)(k - 80 - 16), 16777215);
            for (int k3 = 0; k3 < list.size(); ++k3) {
                ...
                this.fontRendererObj.drawStringWithShadow(s2 = decimalformat.format(profiler$result3.field_76332_a) + "%", (float)(j + 160 - 50 - this.fontRendererObj.getStringWidth(s2)), (float)(k + 80 + k3 * 8 + 20), profiler$result3.getColor());
                this.fontRendererObj.drawStringWithShadow(s2 = decimalformat.format(profiler$result3.field_76330_b) + "%", (float)(j + 160 - this.fontRendererObj.getStringWidth(s2)), (float)(k + 80 + k3 * 8 + 20), profiler$result3.getColor());
            }
        }
    }
    ...
    public void runTick() throws IOException {
        if (this.thePlayer != null) {
            ...
            Aqua.INSTANCE.onEvent(eventTick);
        }
        Aqua.INSTANCE.onEvent(new EventClick());
        ...
        if (this.thePlayer != null && this.theWorld != null) {
            Aqua.INSTANCE.onEvent(new EventMouseOver());
        }
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        for (final Module m : Aqua.moduleManager.getModules()) {
                            if (k == m.getKeyBind()) {
                                m.toggle();
                            }
                        }
            ...
            final EventClickMouse eventCLickMouse = new EventClickMouse(this.thePlayer.inventory.currentItem);
            final int oldSlot = this.thePlayer.inventory.currentItem;
            Aqua.INSTANCE.onEvent(eventCLickMouse);
            this.thePlayer.inventory.currentItem = eventCLickMouse.getSlot();
