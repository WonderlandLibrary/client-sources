...
{
    public static final Ordering<NetworkPlayerInfo> field_175252_a;
    ...
    public static String getIrcPrefix(final String name, final GameProfile pr) {
        try {
            final IrcPlayer ircPlayer = IrcPlayer.getByIngameName(pr.getName());
            if (ircPlayer != null) {
                final String client = ThemeScreen.themeHero ? "Hero" : "Aqua";
    ...
    public void renderPlayerlist(final int width, final Scoreboard scoreboardIn, final ScoreObjective scoreObjectiveIn) {
        ...
        for (final NetworkPlayerInfo networkplayerinfo : list) {
            final String prefix = getIrcPrefix(this.getPlayerName(networkplayerinfo), networkplayerinfo.getGameProfile());
        ...
        if (list2 != null) {
            final List<String> finalList = list2;
            final int finalK = k2;
            final int finalL1 = l4;
            final int n;
            final int n2;
            Shadow.drawGlow(() -> Gui.drawRect(width / 2 - n / 2 - 1, n2 - 1, width / 2 + n / 2 + 1, n2 + finalList.size() * FontRenderer.FONT_HEIGHT, Integer.MIN_VALUE), false);
            Gui.drawRect(width / 2 - l4 / 2 - 1, k2 - 1, width / 2 + l4 / 2 + 1, k2 + list2.size() * FontRenderer.FONT_HEIGHT, Integer.MIN_VALUE);
            for (final String s3 : list2) {
                final int i4 = this.mc.fontRendererObj.getStringWidth(s3);
                final int finalK2 = k2;
                Shadow.drawGlow(() -> this.mc.fontRendererObj.drawStringWithShadow(s3, (float)(width / 2 - i4 / 2), (float)finalK2, -1), false);
                this.mc.fontRendererObj.drawStringWithShadow(s3, (float)(width / 2 - i4 / 2), (float)k2, -1);
                k2 += FontRenderer.FONT_HEIGHT;
            }
            ++k2;
        }
        final int finalL2 = l4;
        final int finalK3 = k2;
        final int finalI = i2;
        final int n3;
        final int n4;
        Shadow.drawGlow(() -> Gui.drawRect(width / 2 - n3 / 2 - 1, n4 - 1, width / 2 + n3 / 2 + 1, n4 + finalI * 9, Integer.MIN_VALUE), false);
        ...
        for (int k3 = 0; k3 < l3; ++k3) {
            ...
            if (k3 < list.size()) {
                ...
                s4 = getIrcPrefix(s4, gameprofile) + s4;
                if (flag) {
        ...
        if (list3 != null) {
            k2 = k2 + i2 * 9 + 1;
            Gui.drawRect(width / 2 - l4 / 2 - 1, k2 - 1, width / 2 + l4 / 2 + 1, k2 + list3.size() * FontRenderer.FONT_HEIGHT, Integer.MIN_VALUE);
    ...
    static {
        field_175252_a = Ordering.from((Comparator<NetworkPlayerInfo>)new PlayerComparator());
    }
