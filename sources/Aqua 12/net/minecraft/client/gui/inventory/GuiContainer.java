...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void initGui() {
        if (Aqua.moduleManager.getModuleByName("GuiElements").isToggled()) {
            try {
                ...
                GuiContainer.resourceLocation = Minecraft.getMinecraft().getRenderManager().renderEngine.getDynamicTextureLocation("name", new DynamicTexture(bi));
            }
            catch (Exception ex) {}
        }
        super.initGui();
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        final double xmod = GuiContainer.width / 2.0f - this.translate.getX() / 2.0f;
        final double ymod = GuiContainer.height / 2.0f - this.translate.getY() / 2.0f;
        GlStateManager.translate(xmod, ymod, 0.0);
        GlStateManager.scale(this.translate.getX() / GuiContainer.width, this.translate.getY() / GuiContainer.height, 1.0f);
