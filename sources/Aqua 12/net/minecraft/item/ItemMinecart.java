    ...
    public boolean onItemUse(final ItemStack stack, final EntityPlayer playerIn, final World worldIn, final BlockPos pos, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        if (BlockRailBase.isRailBlock(iblockstate)) {
            if (!worldIn.isRemote) {
                ...
                final EntityMinecart entityminecart = EntityMinecart.getMinecart(worldIn, pos.getX() + 0.5, pos.getY() + 0.0625 + d0, pos.getZ() + 0.5, this.minecartType);
    ...
    static {
        dispenserMinecartBehavior = new BehaviorDefaultDispenseItem() {
            ...
            public ItemStack dispenseStack(final IBlockSource source, final ItemStack stack) {
                ...
                final EntityMinecart entityminecart = EntityMinecart.getMinecart(world, d0, d2 + d4, d3, ((ItemMinecart)stack.getItem()).minecartType);
                if (stack.hasDisplayName()) {
                    entityminecart.setCustomNameTag(stack.getDisplayName());
                }
