...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    @Override
    protected void channelRead0(final ChannelHandlerContext context, final Packet packet) throws Exception {
        final EventReceivedPacket eventReceivedPacket = new EventReceivedPacket(packet);
        eventReceivedPacket.setType(EventType.PRE);
        Aqua.INSTANCE.onEvent(eventReceivedPacket);
        if (eventReceivedPacket.isCancelled()) {
            return;
        }
        final EventBacktrack eventBacktrack = new EventBacktrack(EventBacktrack.Action.RECEIVE, packet, this.packetListener, this.direction);
        eventBacktrack.setType(EventType.PRE);
        Aqua.INSTANCE.onEvent(eventBacktrack);
        final EventPacket eventPacket = new EventPacket(EventPacket.Action.RECEIVE, packet, this.packetListener, this.direction);
        ...
        eventPacket.setType(EventType.PRE);
        final EventTimerDisabler eventTimerDisabler = new EventTimerDisabler(EventTimerDisabler.Action.SEND, packet, null, null);
        eventPacket.setType(EventType.PRE);
    ...
    public void sendPacket(final Packet packet, final GenericFutureListener<? extends Future<? super Void>> listener, final GenericFutureListener<? extends Future<? super Void>>... listeners) {
        if (this.isChannelOpen()) {
            this.flushOutboundQueue();
            ...
            this.readWriteLock.writeLock().lock();
            try {
                this.outboundPacketsQueue.add(new InboundHandlerTuplePacketListener(packet, (GenericFutureListener<? extends Future<? super Void>>[])ArrayUtils.add(listeners, 0, listener)));
            }
            finally {
                this.readWriteLock.writeLock().unlock();
            }
