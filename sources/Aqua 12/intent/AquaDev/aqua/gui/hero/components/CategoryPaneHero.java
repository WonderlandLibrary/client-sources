...
public class CategoryPaneHero extends Gui
{
    ...
    public CategoryPaneHero(final int x, final int y, final int width, final int height, final Category category, final ClickguiScreenHero novoline) {
    ...
    public void draw(final int posX, final int posY, final int mouseX, final int mouseY) {
        ...
        RenderUtil.drawTriangleFilledReversed(this.x + this.width / 2.0f + 27.0f, (float)(this.y + 16), 5.0f, 5.0f, -1);
        ...
        GL11.glEnable(3089);
        RenderUtil.scissor(this.x, this.y + this.height, this.width - 8, maxPanelHeight);
        allModHeight += this.height;
        int settingsHeight = 0;
        for (final Module module3 : Aqua.moduleManager.getModulesOrderedRoboto(this.category, false, Aqua.INSTANCE.robotoPanel)) {
            if (module3.getCategory() == this.category) {
                final int finalAdd2 = add;
                if (Aqua.moduleManager.getModuleByName("Blur").isToggled()) {}
                if (Aqua.moduleManager.getModuleByName("Shadow").isToggled()) {}
                Gui.drawRect(this.x + 10, this.y + add + 1, this.x + this.width - 8, this.y + add + 12, new Color(35, 35, 35, 195).getRGB());
                final int finalAdd3 = add;
                if (module3.isToggled()) {}
                Aqua.INSTANCE.robotoPanel.drawCenteredString(module3.getName(), this.x + this.width / 2.0f, (float)(this.y + add + 2), module3.isToggled() ? Color.decode("#00FF45").getRGB() : -1);
                final int modX = this.x + 2;
                final int modY = this.y + add;
                final int modWidth = this.width - 4;
                final int modHeight = 11;
