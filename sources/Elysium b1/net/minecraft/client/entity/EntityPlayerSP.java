...
import dev.elysium.client.mods.impl.player.KineticSneak;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        ...
        {
            super.onUpdate();
            Elysium.getInstance().targets.clear();
    ...
    {
        EventMotion e = new EventMotion(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
        e.setType(Event.EventType.PRE);
        e.post();
        ...
        boolean flag1 = this.isSneaking() || Elysium.getInstance().getModManager().getModByName("KineticSneak").toggled && !mc.thePlayer.isMoving();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            e.setType(Event.EventType.POST);
            e.post();
        }
    }
    ...
    public void sendChatMessage(String message)
    {
        ...
        event.setType(Event.EventType.PRE);
        Elysium.getInstance().getCmdManager().handleChat(event);
        if(event.post().isCancelled()) return;
    ...
    public boolean isSneaking() {
        boolean flag = !this.isMoving() && Elysium.getInstance().getModManager().getModByName("KineticSneak").toggled && ((KineticSneak)Elysium.getInstance().getModManager().getModByName("KineticSneak")).clientSide.isEnabled() || this.movementInput != null ? this.movementInput.sneak : false;
    ...
    public void onLivingUpdate()
    {
        ...
        boolean nsEnabled = Elysium.getInstance().getModManager().getModByName("NoSlowDown").toggled;

        if (this.isUsingItem() && !this.isRiding() && !nsEnabled)
        {
        ...
        if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= f && !this.isSprinting() && flag3 && (!this.isUsingItem() || nsEnabled) && !this.isPotionActive(Potion.blindness))
        {
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && (!this.isUsingItem() || nsEnabled) && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown())
        {
            this.setSprinting(true);
        }
