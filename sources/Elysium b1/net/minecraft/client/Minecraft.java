...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    {
        if (this.leftClickCounter <= 0 || Elysium.getInstance().getModManager().getModByName("NoClickDelay").toggled)
        {
            this.thePlayer.swingItem();
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                EventKeyTyped event = new EventKeyTyped(k);
                event.post();

                k = event.getKeyCode();

                if(Keyboard.getEventKeyState())
                    Elysium.getInstance().getModManager().HandleKeypress(event);
