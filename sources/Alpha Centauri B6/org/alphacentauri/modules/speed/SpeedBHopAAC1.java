...
public class SpeedBHopAAC1 extends Speed {
      ...
      if(event instanceof EventTick) {
         ...
         if(player.hasMovementInput()) {
            if(player.onGround) {
               ...
               MoveUtils.setSpeed((double)((float)Math.sqrt(player.motionX * player.motionX + player.motionZ * player.motionZ) * ((double)player.fallDistance > 0.4D?1.0F:1.01F)));
            }
         } else {
            player.motionX = 0.0D;
            player.motionZ = 0.0D;
