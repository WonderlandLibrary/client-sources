...
public class GuiIngameMenu extends GuiScreen {
   ...
   private void doReconnect() {
      if(AC.getMC().getLastServerData() != null) {
         boolean flag = this.mc.isIntegratedServerRunning();
         boolean flag1 = this.mc.func_181540_al();
         this.mc.theWorld.sendQuittingDisconnectingPacket();
         this.mc.loadWorld((WorldClient)null);
         if(flag) {
            this.mc.displayGuiScreen(new GuiConnecting(new GuiMultiplayer(new GuiMainMenu()), AC.getMC(), AC.getMC().getLastServerData()));
         } else if(flag1) {
            RealmsBridge realmsbridge = new RealmsBridge();
            realmsbridge.switchToRealms(new GuiMainMenu());
         } else {
   ...
   public void initGui() {
      ...
      if(!AC.isGhost()) {
         this.buttonList.add(new GuiButton(1337, this.width / 2 - 100, this.height / 4 + 72 + i, "Alpha Centauri"));
         this.buttonList.add(new GuiButton(69, this.width / 2 - 100, this.height / 4 + 145 + i, "Reconnect"));
      }
