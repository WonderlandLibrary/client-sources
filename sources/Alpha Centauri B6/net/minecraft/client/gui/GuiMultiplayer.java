...
import org.alphacentauri.gui.screens.GuiSetProxyMP;
...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
   ...
   private ThreadLanServerFind lanServerDetector;
   ...
   protected void actionPerformed(GuiButton button) throws IOException {
      if(button.enabled) {
         IGuiListEntry guilistextended$iguilistentry = this.serverListSelector.func_148193_k() < 0?null:this.serverListSelector.getListEntry(this.serverListSelector.func_148193_k());
         if(button.id == 2 && guilistextended$iguilistentry instanceof ServerListEntryNormal) {
            ...
            AC.getMC().displayGuiScreen(new GuiSetProxyMP(this));
         }
      }
   ...
   protected void keyTyped(char typedChar, int keyCode) throws IOException {
      ...
      IGuiListEntry guilistextended$iguilistentry = i < 0?null:this.serverListSelector.getListEntry(i);
      if(keyCode == 63) {
         this.refreshServerList();
      } else if(i >= 0) {
         if(keyCode == 200) {
   ...
   public void updateScreen() {
      ...
      if(this.lanServerList.getWasUpdated()) {
         List<LanServer> list = this.lanServerList.getLanServers();
   ...
   public void initGui() {
      ...
      if(!this.initialized) {
         ...
         try {
            this.lanServerDetector = new ThreadLanServerFind(this.lanServerList);
   ...
   public void connectToSelected() {
      ...
      if(guilistextended$iguilistentry instanceof ServerListEntryNormal) {
         ...
         LanServer lanserverdetector$lanserver = ((ServerListEntryLanDetected)guilistextended$iguilistentry).getLanServer();
   ...
   private void refreshServerList() {
      for(int i = 0; i < this.serverListSelector.getSize(); ++i) {
         IGuiListEntry listEntry1 = this.serverListSelector.getListEntry(i);
         if(listEntry1 instanceof ServerListEntryNormal) {
            ServerListEntryNormal listEntry = (ServerListEntryNormal)listEntry1;
            listEntry.field_148301_e.field_78841_f = false;
         }
      }
   ...
   public void selectServer(int index) {
      ...
      IGuiListEntry guilistextended$iguilistentry = index < 0?null:this.serverListSelector.getListEntry(index);
   ...
   public void createButtons() {
      ...
      if(!AC.isGhost()) {
         this.buttonList.add(new GuiButton(1337, 0, 0, 100, 20, "Proxy"));
      }
