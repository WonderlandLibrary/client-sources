   ...
   private static IBakedModel getModelDoubleFace(IBakedModel p_getModelDoubleFace_0_) {
      if(p_getModelDoubleFace_0_ == null) {
         ...
         for(int k = 0; k < aenumfacing.length; ++k) {
            ...
            BakedQuad bakedquad1 = new BakedQuad((int[])((int[])bakedquad.getVertexData().clone()), bakedquad.getTintIndex(), bakedquad.getFace(), bakedquad.getSprite());
            int[] aint = bakedquad1.getVertexData();
            int[] aint1 = (int[])((int[])aint.clone());
            int j = aint.length / 4;
