...
import none.module.modules.world.SafeWalk;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void onEntityUpdate()
    {
        ...
        if (this instanceof EntityPlayerSP && Client.instance.moduleManager.noSlowdown.isEnabled() && PlayerUtil.isInLiquid()) {
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (this instanceof EntityPlayer) {
	            flag = this.onGround && (isSneaking() ||
	            		(Client.instance.moduleManager.scaffold.isEnabled()
	            				&& Scaffold.safewalk.getObject()));
	            
	            if (this.isCollidedVertically && Client.instance.moduleManager.scaffold.isEnabled() && Scaffold.scaffoldmode.getSelected().equalsIgnoreCase("Cubecraft")) {
	            	flag = true;
	            }
	            
	            if (Client.instance.moduleManager.safeWalk.isEnabled()) {
	            	if (SafeWalk.airsafe.getObject()) {
	            		flag = true;
	            	}else if (!SafeWalk.airsafe.getObject() && onGround) {
	            		flag = true;
	            	}
	            }
	            
//	            if (Scaffold.downside.getObject() && Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && Client.instance.moduleManager.getModulesbycls(Scaffold.class).isEnabled()) {
//	            	flag = false;
//	            }
            }
            ...
            if (flag)
            {
                ...
                if (Scaffold.downside.getObject() && Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && Client.instance.moduleManager.scaffold.isEnabled()) {
	            	yoffset = -2D;
	            }
                
                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, yoffset, 0.0D)).isEmpty(); d3 = x)
                {
                    if (x < d6 && x >= -d6)
                    {
                        x = 0.0D;
                ...
                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, yoffset, z)).isEmpty(); d5 = z)
                {
                    if (z < d6 && z >= -d6)
                    {
                        z = 0.0D;
                ...
                for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, yoffset, z)).isEmpty(); d5 = z)
                {
                    if (x < d6 && x >= -d6)
                    {
                        x = 0.0D;
            ...
            if (this == Minecraft.getMinecraft().thePlayer) {
	            if (Client.instance.moduleManager.fly.isEnabled() && Fly.bobbing.getObject()) {
	            	this.onGround = true;
	            }
            }
    ...
    public void moveFlying(float strafe, float forward, float friction)
    {
        ...
        if (this instanceof EntityPlayerSP) {
        	StrafeEvent event = (StrafeEvent)EventSystem.getInstance(StrafeEvent.class);
        	event.fire(strafe, forward, friction);
    ...
    public void readFromNBT(NBTTagCompound tagCompund)
    {
        try
        {
            ...
            this.setRotationPitchHead(this.rotationPitch);
    ...
    public Vec3 getPositionVector2()
    {
        return new Vec3(Math.floor(this.posX), Math.floor(this.posY), Math.floor(this.posZ));
    }
