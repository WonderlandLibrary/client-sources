...
import none.module.modules.player.NoSlowdown;
import none.module.modules.world.CommandSystem;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdate() {
		if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			...
			if (this.isRiding()) {
//				EventPreMotionUpdate e = (EventPreMotionUpdate) EventSystem.getInstance(EventPreMotionUpdate.class);
//				e.fire(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ, this.isSneaking(),
...
//				this.connection.sendPacket(
//						new C03PacketPlayer.C05PacketPlayerLook(e.getYaw(), e.getPitch(), this.onGround));
				this.connection.sendPacket(new C0CPacketInput(this.moveStrafing, this.moveForward,
...
//				e.fire();
	...
	public void onUpdateWalkingPlayer() {
		...
		if (flag != this.serverSprintState) {
			if (flag) {
				this.connection
						.sendPacket(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.START_SPRINTING));
			} else {
				this.connection
						.sendPacket(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.STOP_SPRINTING));
			}
		...
		if (flag1 != this.serverSneakState) {
			if (flag1) {
				this.connection
						.sendPacket(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.START_SNEAKING));
			} else {
		...
		if (this.isCurrentViewEntity()) {
			...
			boolean flag3 = (d3 != 0.0D || d4 != 0.0D) && e.isRotate();

			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.connection.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(e.getX(), e.getY(), e.getZ(),
					...
					this.connection.sendPacket(new C03PacketPlayer(e.isOnGround()));
				}
			} else {
				this.connection.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
		...
		e.fire();
		if (Client.instance.moduleManager.derp.isEnabled()
				&& Derp.SideMode.getSelected().equalsIgnoreCase("Client")) {
			e.PREVYAW = Derp.rotations[0];
			e.PREVPITCH = Derp.rotations[1];
		}
	}
	...
	public void sendChatMessage(String message) {
		if (Client.instance.moduleManager.commandSystem.isEnabled()) {
			sendChatMessageNoEvent(message);
		} else if (EventChat.onSendChatMessage(message)) {
	...
	protected void sendHorseJump() {
		this.connection.sendPacket(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.RIDING_JUMP,
	...
	public void onLivingUpdate() {
		EventTick eventTick = (EventTick) EventSystem.getInstance(EventTick.class);
		eventTick.fire();
		...
		if (this.inPortal) {
			...
			this.inPortal = false;
		} else if (this.isPotionActive(MobEffects.confusion)
				&& this.getActivePotionEffect(MobEffects.confusion).getDuration() > 60) {
		...
		if (this.isUsingItem() && !this.isRiding()) {
			if (!Client.instance.moduleManager.noSlowdown.isEnabled()) {
				this.movementInput.moveStrafe *= 0.2F;
				this.movementInput.moveForward *= 0.2F;
				this.sprintToggleTimer = 0;
			} else {
				Module noSlowDown = Client.instance.moduleManager.noSlowdown;
				if (NoSlowdown.noslowmode.getSelected() != null
						&& NoSlowdown.noslowmode.getSelected().equalsIgnoreCase("AAC")) {
					...
					if (mc.thePlayer.inventory.getCurrentItem() != null) {
						if (!(mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemSword)) {
							if (!(mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemBow)) {
								double slow = (71 - mc.thePlayer.getItemInUseCount()) * 0.008;
						...
						} else if (mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemSword) {
							if (Killaura.targeter == null) {
								this.movementInput.moveStrafe *= 0.6f;
								this.movementInput.moveForward *= 0.6f;
							} else {
                				this.movementInput.moveStrafe *= 0.27f;
	                    		this.movementInput.moveForward *= 0.27f;
							}
						}
					}
				}
		...
		if (this.onGround) {
			if (Killaura.targeter != null && Killaura.isblocking && Killaura.autoblocktypes.getSelected().equalsIgnoreCase("AAC")) {
				this.movementInput.moveStrafe *= 0.4f;
				this.movementInput.moveForward *= 0.4f;
			}
		}
		...
		if (!BlockUtils.isOnLiquid(0.1)) {
			if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= f && !this.isSprinting() && flag3
					&& !this.isUsingItem() && !this.isPotionActive(MobEffects.blindness)) {
