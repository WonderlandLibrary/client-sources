...
import none.Client;
import none.module.modules.render.ItemPhysics;
...
public class RenderEntityItem extends Render<EntityItem>
{
    ...
    private int func_177077_a(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_)
    {
        ...
        else
        {
            ...
            if (Client.instance.moduleManager.itemPhysics.isEnabled()) {            	
            	GlStateManager.translate((float)p_177077_2_, (float)p_177077_4_ + 0.047F, (float)p_177077_6_);
            }else {
            	GlStateManager.translate((float)p_177077_2_, (float)p_177077_4_ + f1 + 0.25F * f2, (float)p_177077_6_);
            }
            
            float[] rot = Utils.getNeededRotationsForEntity(itemIn);
            if (Client.instance.moduleManager.itemPhysics.isEnabled()) {
	            if (flag || this.renderManager.options != null)
	            {
	                float f3 = (((float)itemIn.getAge() + p_177077_8_) / 10.0F + itemIn.hoverStart) * (180F / (float)Math.PI);
	                if (!itemIn.onGround) {
	                	GlStateManager.rotate(f3, 1.0F, 1.0F, 1.0F);
	                }else {
	                	GlStateManager.rotate(itemIn.hoverStart * (180F / (float)Math.PI), 0.0F, 1.0F, 0.0F);
//	                	GlStateManager.rotate(-rot[0], 0.0F, 1.0F, 0.0F);
...
//	                GlStateManager.rotate(f3, 0.0F, 1.0F, 0.0F);
	                GlStateManager.rotate(-rot[0], 0.0F, 1.0F, 0.0F);
	                GlStateManager.rotate(rot[1], 1.0F, 0.0F, 0.0F);
	            }
            }
            if (!flag)
            {
