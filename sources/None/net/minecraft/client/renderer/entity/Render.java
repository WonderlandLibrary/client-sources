...
public abstract class Render<T extends Entity>
{
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        ...
        if (d0 <= (double)(maxDistance * maxDistance))
        {
            ...
            int renderColor = 553648127;
            if (entityIn instanceof EntityPlayerSP) {
            	if (Client.ISAwakeNgineXE) {
            		renderColor = ClientColor.rainbow.getObject() ? ClientColor.rainbow(100) : ClientColor.getColor();
            ...
            if (entityIn instanceof EntityPlayer) {
            	if (Client.nameList.contains(((EntityPlayer)entityIn).getName())) {
            		renderColor = ClientColor.rainbow(100);
            	}
            }
            ...
            GlStateManager.depthMask(true);
            fontrenderer.drawString(str, -fontrenderer.getStringWidth(str) / 2, b0, renderColor);
            GlStateManager.enableLighting();
            GlStateManager.disableBlend();
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            GlStateManager.popMatrix();
