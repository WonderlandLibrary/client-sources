...
import none.module.modules.combat.BlockInfinite;
import none.module.modules.combat.Reach;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        {
            double reach = Client.instance.moduleManager.reach.isEnabled() && Reach.reachmode.getSelected().equalsIgnoreCase("Extended")? Reach.range.getDouble() : 3.0D;
            this.mc.mcProfiler.startSection("pick");
            this.mc.pointedEntity = null;
            double d0 = Client.instance.moduleManager.reach.isEnabled() && Reach.reachmode.getSelected().equalsIgnoreCase("Extended") ? reach : (double)this.mc.playerController.getBlockReachDistance();
            if (Client.instance.moduleManager.blockInfinite.isEnabled()) {
            	d0 = 100;
            }
...
//                d0 = d0;
    ...
    {
    	if (Client.instance.moduleManager.noHurtCam.isEnabled()) return;
...
//                        	d3 = d7;
    ...
    private void updateLightmap(float partialTicks)
    {
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (worldclient != null)
            {
                if (Config.isCustomColors() && CustomColors.updateLightmap(worldclient, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(MobEffects.nightVision)))
                {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Event3D event3d = (Event3D)EventSystem.getInstance(Event3D.class);
        event3d.fire(partialTicks);
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        else if (block.getMaterial() == Material.water)
        {
            ...
            if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.waterBreathing))
            {
                f8 = f8 * 0.3F + 0.6F;
            }
