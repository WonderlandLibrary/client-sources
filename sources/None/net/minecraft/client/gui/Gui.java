...
public class Gui
{
    ...
    public static void drawRectRGB(float left, float top, float right, float bottom, int color)
    {

        for (float i = left; i < right; i+=2) {
            for (float j = top; j < bottom; j+=2) {
                drawRect(i, j, i + 2, j + 2, ClientColor.rainbow((int)((i+j) * HUDEditer.ArrayListSpeed.getInteger())));
            }
        }
    }
    
    public static void drawRectRGB(float left, float top, float right, float bottom)
    {

        for (float i = left; i < right; i+=2) {
            for (float j = top; j < bottom; j+=2) {
                drawRect(i, j, i + 2, j + 2, ClientColor.rainbow((int)((i+j) * (Client.Starting ? HUDEditer.ArrayListSpeed.getInteger() : 100))));
            }
        }
    }
    
    public static void drawOutineRect(float left, float top, float right, float bottom, int width, int incolor, int outcolor) {
    	//Left
    	drawRect(left, top, left + width, bottom, outcolor);
    	//Right
    	drawRect(right - width, top, right, bottom, outcolor);
    	//Up
    	drawRect(left, top, right, top + width, outcolor);
    	//Down
    	drawRect(left, bottom - width, right, bottom, outcolor);
    	
    	//Center
    	drawRect(left + width, top + width, right - width, bottom - width, incolor);
    }
    
    public static void drawOutlineRGB(float left, float top, float right, float bottom, int width, int incolor) {
    	//Left
    	drawRectRGB(left, top, left + width, bottom);
    	//Right
    	drawRectRGB(right - width, top, right, bottom);
    	//Up
    	drawRectRGB(left, top, right, top + width);
    	//Down
    	drawRectRGB(left, bottom - width, right, bottom);
