...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    {
        if (this instanceof GuiMainMenu) {
		    	for (int i = 0; i < this.buttonList.size(); ++i)
		        {
		    		GuiButton guibutton = this.buttonList.get(i);
		    		if (GuiMainMenu.unlocked) {
		    			if (guibutton.id == -2) {
		    				guibutton.displayString = GuiMainMenu.unlocked ? "Lock" : "UnLock";
		    			}
		    			((GuiButton)this.buttonList.get(i)).drawButton(this.mc, mouseX, mouseY);
		    		}else if (((GuiButton)this.buttonList.get(i)).id == -2) {
		    			if (guibutton.id == -2) {
		    				guibutton.displayString = GuiMainMenu.unlocked ? "Lock" : "UnLock";
		    			}
		    			((GuiButton)this.buttonList.get(i)).drawButton(this.mc, mouseX, mouseY);
    ...
    protected void drawHoveringText(List<String> textLines, int x, int y)
    {
    	if (!textLines.isEmpty())
        {
            ...
            for (int k1 = 0; k1 < textLines.size(); ++k1)
            {
                ...
                this.fontRendererObj.drawString(s1, (float)l1, (float)i2, -1);
