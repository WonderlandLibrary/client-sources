...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable
{
    ...
    public void processPlayerDigging(C07PacketPlayerDigging packetIn)
    {
        ...
        switch (packetIn.getStatus())
        {
            ...
            case STOP_DESTROY_BLOCK:
                ...
                double d3 = this.playerEntity.getDistance(d0, d1, d2);
                double reach = Reach.range.getDouble();
    ...
    public void processPlayerBlockPlacement(C08PacketPlayerBlockPlacement packetIn)
    {
        ...
        {
            if (this.hasMoved && this.playerEntity.getDistance((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.5D, (double)blockpos.getZ() + 0.5D) < reach && !this.serverController.isBlockProtected(worldserver, blockpos, this.playerEntity) && worldserver.getWorldBorder().contains(blockpos))
            {
    ...
    public void processUseEntity(C02PacketUseEntity packetIn)
    {
        ...
        if (entity != null)
        {
            ...
            if (this.playerEntity.getDistanceToEntity(entity) <= (Client.instance.moduleManager.reach.isEnabled() && Reach.reachmode.getSelected().equalsIgnoreCase("Extended") ? reach : d0))
            {
