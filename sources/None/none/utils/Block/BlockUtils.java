...
public class BlockUtils {
    ...
    public static void updateTool(BlockPos pos) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if ((itemStack.getStrVsBlock(state) > strength)) {
                strength = itemStack.getStrVsBlock(state);
                bestItemIndex = i;
            }
        }
        if (bestItemIndex != -1) {
    ...
    public static boolean isInsideBlock(BlockPos pos){
    	Block block = mc.theWorld.getBlockState(pos).getBlock();
    	Block block2 = mc.theWorld.getBlockState(pos.add(0, 1, 0)).getBlock();
