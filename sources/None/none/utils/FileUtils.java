...
public final class FileUtils {
    ...
    private static File getConfigDir() {
        final File file = new File(Minecraft.getMinecraft().mcDataDir, Client.instance.name);
        if (!file.exists()) {
            file.mkdir();
        }
        return file;
    ...
    private static File getConfigFolder() {
        final File Sigma = new File(Minecraft.getMinecraft().mcDataDir, Client.instance.name);
