...
public class AddOrSave extends Slot{
	...
	public AddOrSave(int posX, int posY, int w, int h) {
		this.posX = posX;
		this.posY = posY;
		this.w = w;
		this.h = h;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		fontRenderer.drawString("AddOrSave", (float)((posX + posX + w) / 2) - (float)(fontRenderer.getStringWidth("AddOrSave") / 2), (float)((posY + posY + h) / 2) - (float)(fontRenderer.getHeight("AddOrSave") / 2), Color.WHITE.getRGB());
		if (recivetextgui != null) {
				...
				if (!recivetextgui.getText().isEmpty()) {
					...
					for (String str : list) {
						if (str.equalsIgnoreCase(recivetextgui.getText())) {							
							Client.instance.fileManager.SaveConfig(str);
	...
	public void keyTyped(char typedChar, int keyCode) {
		if (recivetextgui != null) {
			if (keyCode == Keyboard.KEY_RETURN) {
				if (!recivetextgui.getText().isEmpty()) {
					Client.instance.fileManager.SaveConfig(recivetextgui.getText());
					Client.instance.clickgui.UpdateConfigDisplay();
				}
				recivetextgui.setFocused(false);
				recivetextgui.setVisible(false);
