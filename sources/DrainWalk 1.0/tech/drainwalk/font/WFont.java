...
public class WFont {
    ...
    protected final BufferBuilder bufferbuilder = tessellator.getBuffer();
    ...
    public WFont(Font font, boolean antiAlias, boolean fractionalMetrics) {
        this.font = font;
    ...
    public static Font readFontFromFile(String fontName, float size) {
        try {
            Font font = Font.createFont(Font.PLAIN, Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("drainwalk/fonts/" + fontName)).getInputStream());
...
//	}
    protected void drawQuad(float x, float y, float width, float height, float srcX, float srcY, float srcWidth,
        ...
        bufferbuilder.pos(x + width, y, 0).tex(renderSRCX + renderSRCWidth, renderSRCY).endVertex();
        bufferbuilder.pos(x, y, 0).tex(renderSRCX, renderSRCY).endVertex();
        bufferbuilder.pos(x, y + height, 0).tex(renderSRCX, renderSRCY + renderSRCHeight).endVertex();
        bufferbuilder.pos(x, y + height, 0).tex(renderSRCX, renderSRCY + renderSRCHeight).endVertex();
        bufferbuilder.pos(x + width, y + height, 0).tex(renderSRCX + renderSRCWidth, renderSRCY + renderSRCHeight).endVertex();
        bufferbuilder.pos(x + width, y, 0).tex(renderSRCX + renderSRCWidth, renderSRCY).endVertex();
    }
