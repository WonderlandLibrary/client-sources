...
import net.augustus.events.EventWeb;
...
public abstract class Entity implements ICommandSender {
   ...
   protected void setSize(float width, float height) {
      if (width != this.width || height != this.height) {
         ...
         if (this.width > f && !this.firstUpdate && !this.worldObj.isRemote) {
            this.moveEntity((double)(f - this.width), 0.0, (double)(f - this.width));
         }
      }
   }
   ...
   public void setAngles(float yaw, float pitch) {
      float f = Augustus.getInstance().getYawPitchHelper().realPitch;
      float f1 = Augustus.getInstance().getYawPitchHelper().realYaw;
      Augustus.getInstance().getYawPitchHelper().realYaw = (float)((double)Augustus.getInstance().getYawPitchHelper().realYaw + (double)yaw * 0.15);
      Augustus.getInstance().getYawPitchHelper().realPitch = (float)((double)Augustus.getInstance().getYawPitchHelper().realPitch - (double)pitch * 0.15);
      Augustus.getInstance().getYawPitchHelper().realPitch = MathHelper.clamp_float(Augustus.getInstance().getYawPitchHelper().realPitch, -90.0F, 90.0F);
      YawPitchHelper var10000 = Augustus.getInstance().getYawPitchHelper();
      var10000.realLastPitch += Augustus.getInstance().getYawPitchHelper().realPitch - f;
      var10000 = Augustus.getInstance().getYawPitchHelper();
      var10000.realLastYaw += Augustus.getInstance().getYawPitchHelper().realYaw - f1;
      this.rotationYaw = Augustus.getInstance().getYawPitchHelper().realYaw;
      this.rotationPitch = Augustus.getInstance().getYawPitchHelper().realPitch;
      this.prevRotationPitch = Augustus.getInstance().getYawPitchHelper().realLastPitch;
      this.prevRotationYaw = Augustus.getInstance().getYawPitchHelper().realLastYaw;
   }
   ...
   public void moveEntity(double x, double y, double z) {
      EventSaveWalk eventSaveWalk = new EventSaveWalk(x, y, z, this);
      if (this instanceof EntityPlayerSP) {
         EventHandler.call(eventSaveWalk);
         x = eventSaveWalk.getMotionX();
         y = eventSaveWalk.getMotionY();
         z = eventSaveWalk.getMotionZ();
         if (eventSaveWalk.isCanceled()) {
            return;
         }
      }
      ...
      if (this.noClip) {
         ...
         if (this.isInWeb) {
            if (Minecraft.getMinecraft().thePlayer == this) {
               EventWeb eventWeb = new EventWeb(0.25, 0.05F, 0.25);
               EventHandler.call(eventWeb);
               this.isInWeb = false;
               x *= eventWeb.getX();
               y *= eventWeb.getY();
               z *= eventWeb.getZ();
         ...
         if (this instanceof EntityPlayerSP && eventSaveWalk.isDisableSneak()) {
            sneak = false;
         }

         boolean flag = (this.onGround && sneak || eventSaveWalk.isSaveWalk()) && this instanceof EntityPlayer;
         if (flag) {
            double d6;
         ...
         EventPreStep eventPreStep = new EventPreStep(this.stepHeight);
         if (this instanceof EntityPlayerSP) {
            EventHandler.call(eventPreStep);
         }

         if (eventPreStep.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z)) {
            double d11 = x;
            double d7 = y;
            double d8 = z;
            AxisAlignedBB axisalignedbb3 = this.getEntityBoundingBox();
            this.setEntityBoundingBox(axisalignedbb);
            y = (double)eventPreStep.getStepHeight();
            ...
            if (this instanceof EntityPlayerSP) {
               EventPostStep eventPostStep = new EventPostStep();
               EventHandler.call(eventPostStep);
            }
         }
         ...
         if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
            ...
            this.distanceWalkedOnStepModified = (float)(
               (double)this.distanceWalkedOnStepModified + (double)MathHelper.sqrt_double(d12 * d12 + d13 * d13 + d14 * d14) * 0.6
            );
            if (this.distanceWalkedOnStepModified > (float)this.nextStepDistance && block1.getMaterial() != Material.air) {
               this.nextStepDistance = (int)this.distanceWalkedOnStepModified + 1;
               if (this.isInWater()) {
                  float f = MathHelper.sqrt_double(this.motionX * this.motionX * 0.2F + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.2F)
                     * 0.35F;
                  if (f > 1.0F) {
                     f = 1.0F;
                  }
   ...
   protected void doBlockCollisions() {
      BlockPos blockpos = new BlockPos(
         this.getEntityBoundingBox().minX + 0.001, this.getEntityBoundingBox().minY + 0.001, this.getEntityBoundingBox().minZ + 0.001
      );
      BlockPos blockpos1 = new BlockPos(
         this.getEntityBoundingBox().maxX - 0.001, this.getEntityBoundingBox().maxY - 0.001, this.getEntityBoundingBox().maxZ - 0.001
      );
   ...
   protected void resetHeight() {
      float f = MathHelper.sqrt_double(this.motionX * this.motionX * 0.2F + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.2F) * 0.2F;
      if (f > 1.0F) {
         f = 1.0F;
      }
      ...
      for(int j = 0; (float)j < 1.0F + this.width * 20.0F; ++j) {
         ...
         this.worldObj
            .spawnParticle(
               EnumParticleTypes.WATER_SPLASH, this.posX + (double)f4, (double)(f1 + 1.0F), this.posZ + (double)f5, this.motionX, this.motionY, this.motionZ
            );
      }
   }
   ...
   public void moveFlying(float strafe, float forward, float friction) {
      EventMove eventMove = new EventMove(this.rotationYaw, friction);
      EventHandler.call(eventMove);
      if (!eventMove.isCanceled()) {
         ...
         if (f >= 1.0E-4F) {
            ...
            f = eventMove.getFriction() / f;
            strafe *= f;
            forward *= f;
            float f1 = MathHelper.sin(eventMove.getYaw() * (float) Math.PI / 180.0F);
            float f2 = MathHelper.cos(eventMove.getYaw() * (float) Math.PI / 180.0F);
   ...
   public MovingObjectPosition customRayTrace(double blockReachDistance, float partialTicks, float yaw, float pitch) {
      Vec3 vec3 = this.getPositionEyes(partialTicks);
      Vec3 vec31 = this.getCustomLook(partialTicks, yaw, pitch);
      Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
      return this.worldObj.rayTraceBlocks(vec3, vec32, false, false, true);
   }
   ...
   public float getCollisionBorderSize() {
      return ViaMCP.getInstance().getVersion() != 47 && GuiProtocolSelector.active ? 0.0F : 0.1F;
   }
         ...
         if (!this.worldObj.isRemote && !p_181015_1_.equals(this.field_181016_an)) {
            ...
            d1 = Math.abs(
               MathHelper.func_181160_c(
                  d1 - (double)(blockpattern$patternhelper.getFinger().rotateY().getAxisDirection() == EnumFacing.AxisDirection.NEGATIVE ? 1 : 0),
                  d0,
                  d0 - (double)blockpattern$patternhelper.func_181118_d()
               )
            );
            double d2 = MathHelper.func_181160_c(
               this.posY - 1.0,
   ...
   public void addEntityCrashInfo(CrashReportCategory category) {
      ...
      category.addCrashSection(
         ...
         CrashReportCategory.getCoordinateInfo(
            (double)MathHelper.floor_double(this.posX), (double)MathHelper.floor_double(this.posY), (double)MathHelper.floor_double(this.posZ)
         )
      );
