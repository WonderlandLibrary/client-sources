...
public abstract class EntityPlayer extends EntityLivingBase {
   private String protectedName = Minecraft.getMinecraft().nameProtectList.getRandomName();
   ...
   public void onUpdate() {
      this.noClip = mm.phase.isToggled() || this.isSpectator();
      if (this.isSpectator()) {
         this.onGround = false;
      }
      ...
      if (this instanceof EntityPlayerSP) {
         ++((EntityPlayerSP)this).ticksSinceLastSwing;
      }
   ...
   public void onLivingUpdate() {
      ...
      if (this.isSprinting() || mm.sprint.isToggled() && mm.sprint.allSprint) {
   ...
   public EntityItem dropOneItem(boolean dropAll) {
      return this.dropItem(
         this.inventory
            .decrStackSize(this.inventory.currentItem, dropAll && this.inventory.getCurrentItem() != null ? this.inventory.getCurrentItem().stackSize : 1),
         false,
         true
      );
   }
      ...
      if (droppedItem == null) {
         ...
         if (dropAround) {
            ...
            entityitem.motionX = (double)(
               -MathHelper.sin(this.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * f2
            );
            entityitem.motionZ = (double)(
               MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * f2
            );
   ...
   public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
      if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
         ...
         if (this instanceof EntityPlayerSP) {
            ((EntityPlayerSP)this).resetCooldown();
         }
         ...
         if (f > 0.0F || f1 > 0.0F) {
            ...
            if (flag2) {
               if (i > 0) {
                  ...
                  this.motionX *= eventAttackSlowdown.getSlowDown();
                  this.motionZ *= eventAttackSlowdown.getSlowDown();
                  this.setSprinting(eventAttackSlowdown.isSprint());
                  ...
                  this.motionX *= Augustus.getInstance().getModuleManager().velocity.XZValueIntave.getValue();
                  this.motionZ *= Augustus.getInstance().getModuleManager().velocity.XZValueIntave.getValue();
   ...
   public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
      if (!this.worldObj.isRemote) {
         ...
         double d0 = 8.0;
         double d1 = 5.0;
         List<EntityMob> list = this.worldObj
            .getEntitiesWithinAABB(
               EntityMob.class,
               new AxisAlignedBB(
                  (double)bedLocation.getX() - d0,
                  (double)bedLocation.getY() - d1,
                  (double)bedLocation.getZ() - d0,
                  (double)bedLocation.getX() + d0,
                  (double)bedLocation.getY() + d1,
                  (double)bedLocation.getZ() + d0
               )
   ...
   @Override
   public void jump() {
      if(!mm.blockFly.isTower2() || mm.blockFly.towerMode.getSelected().equals("None") || !mm.blockFly.isToggled()) {
         super.jump();
   ...
   public IChatComponent getDisplayName() {
      if (Augustus.getInstance().getModuleManager().protector.isToggled()) {
         IChatComponent ichatcomponent = new ChatComponentText(ScorePlayerTeam.formatPlayerName(this.getTeam(), this.getProtectedName()));
         ichatcomponent.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/msg " + this.getProtectedName() + " "));
         ichatcomponent.getChatStyle().setChatHoverEvent(this.getHoverEvent());
         ichatcomponent.getChatStyle().setInsertion(this.getProtectedName());
   ...
   public void setSpeedOnGround(float speedOnGround) {
