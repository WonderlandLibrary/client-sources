...
public abstract class EntityLivingBase extends Entity implements MM {
   ...
   private static final AttributeModifier sprintingSpeedBoostModifier = new AttributeModifier(sprintingSpeedBoostModifierUUID, "Sprinting speed boost", 0.3F, 2)
      .setSaved(false);
   ...
   private int getArmSwingAnimationEnd() {
      TimerRange tickbase = mm.timerRange;
      ...
      if (mm.blockAnimation.isToggled()) {
         i = (int)((double)i / mm.blockAnimation.speed.getValue());
      }
   ...
   public void swingItem() {
      if (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2 || this.swingProgressInt < 0) {
         EventSwingItemClientSide eventSwingItemClientSide = new EventSwingItemClientSide();
   ...
   protected void jump() {
      if (this == Minecraft.getMinecraft().thePlayer) {
         ...
         if (this.isSprinting() || mm.sprint.allSprint && mm.sprint.isToggled()) {
            Minecraft mc = Minecraft.getMinecraft();
            if (mm.sprint.allSprint && mm.sprint.isToggled()) {
            ...
            float f = eventJump.getYaw() * (float) (Math.PI / 180.0);
            if (!mm.speed.isToggled() || !mm.speed.mode.getSelected().equals("Matrix")) {
               this.motionX -= (double)(MathHelper.sin(f) * 0.2F);
               this.motionZ += (double)(MathHelper.cos(f) * 0.2F);
            } else if (mc.thePlayer.movementInput.moveForward > 0.0F && mc.thePlayer.movementInput.moveStrafe != 0.0F) {
               this.motionX -= (double)(MathHelper.sin(f) * 0.22F);
               this.motionZ += (double)(MathHelper.cos(f) * 0.22F);
            } else {
               this.motionX -= (double)(MathHelper.sin(f) * 0.202F);
               this.motionZ += (double)(MathHelper.cos(f) * 0.202F);
            }
         }
      } else {
         this.motionY = (double)this.getJumpUpwardsMotion();
         ...
         if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
            if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
               ...
               if (this.isCollidedHorizontally && this.isOnLadder()) {
                  EventLadder eventLadder = new EventLadder(0.2);
   ...
   public void onUpdate() {
      ...
      if (this instanceof EntityPlayerSP) {
         YawPitchHelper yp = Augustus.getInstance().getYawPitchHelper();

         while(yp.realYaw - yp.realLastYaw >= 180.0F) {
            yp.realLastYaw += 360.0F;
         }

         while(yp.realPitch - yp.realLastPitch >= 180.0F) {
            yp.realLastPitch += 360.0F;
         }
      }
   ...
   protected void collideWithNearbyEntities() {
      List<Entity> list = this.worldObj
         .getEntitiesInAABBexcluding(
            this, this.getEntityBoundingBox().expand(0.2F, 0.0, 0.2F), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>() {
   ...
   public boolean canPosBeSeen(Vec3 vec3) {
      return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), vec3) == null;
   }
   ...
   public Vec3 getLook(float partialTicks) {
      if (this instanceof EntityPlayerSP && mm.fixes.isToggled() && mm.fixes.mouseDelayFix.getBoolean()) {
