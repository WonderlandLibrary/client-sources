...
public class PlayerControllerMP implements MM {
   ...
   public boolean clickBlock(BlockPos loc, EnumFacing face, ItemStack itemstack) {
      if (this.currentGameType.isAdventure()) {
         if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
            return false;
         }
   ...
   public void resetBlockRemoving(boolean b) {
      if (b) {
         this.netClientHandler
            .addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.ABORT_DESTROY_BLOCK, this.currentBlock, EnumFacing.DOWN));
         this.curBlockDamageMP = 0.0F;
   ...
   public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
      ...
      if (this.blockHitDelay > 0) {
         ...
         if (block.getMaterial() == Material.air) {
            ...
            if (!mm.fastBreak.isToggled() || !(this.curBlockDamageMP >= 0.0F)) {
               this.curBlockDamageMP += block.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock);
            } else if (mm.fastBreak.instant.getBoolean()) {
               this.curBlockDamageMP = 2.0F;
            } else {
               this.curBlockDamageMP = (float)(
                  (double)this.curBlockDamageMP
                     + (double)block.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock)
                        * mm.fastBreak.multiplier.getValue()
               );
            }
               ...
               this.mc
                  ...
                  .playSound(
                     new PositionedSoundRecord(
                        ...
                        (float)posBlock.getX() + 0.5F,
                        (float)posBlock.getY() + 0.5F,
   ...
   public boolean onPlayerDamageBlockCustom(BlockPos posBlock, EnumFacing directionFacing, int id) {
      this.syncCurrentPlayItem();
      if (this.blockHitDelay > 0) {
         --this.blockHitDelay;
         return true;
   ...
   public void syncCurrentPlayItem() {
      ...
      /*
      if (mm.newScaffold.isToggled() && mm.newScaffold.silentMode.getSelected().equals("Spoof")) {
         i = mm.newScaffold.getSlotID();
      }

      if (mm.scaffoldWalk.isToggled() && mm.scaffoldWalk.silentMode.getSelected().equals("Spoof")) {
         i = mm.scaffoldWalk.getSlotID();
      }
      */
      if (mm.autoTool.isToggled() && mm.autoTool.getSlot() >= 0) {
         i = mm.autoTool.getSlot();
      }

      if (mm.fucker.isToggled() && mm.fucker.b != null) {
         i = mm.fucker.getSlotID();
      }

      if (mm.blockFly.isToggled() && mm.blockFly.silentMode.getSelected().equals("Spoof")) {
         i = mm.blockFly.getSlotID();
      }
   ...
   public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
      if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
         ...
         if (itemstack != null && (itemstack != itemStackIn || itemstack != null && itemstack.stackSize != i)) {
   ...
   public boolean func_178894_a(EntityPlayer p_178894_1_, Entity p_178894_2_, MovingObjectPosition p_178894_3_) {
      this.syncCurrentPlayItem();
      Vec3 vec3 = new Vec3(
         p_178894_3_.hitVec.xCoord - p_178894_2_.posX, p_178894_3_.hitVec.yCoord - p_178894_2_.posY, p_178894_3_.hitVec.zCoord - p_178894_2_.posZ
      );
