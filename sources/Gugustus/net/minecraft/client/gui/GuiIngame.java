...
import net.augustus.events.EventPreRender2D;
...
public class GuiIngame extends Gui implements MM {
   ...
   public void renderGameOverlay(float partialTicks) {
      ...
      EventManager.call(new EventPreRender2D());

      Shaders shaders = mm.shaders;
      shaders.blurScreen();

      EventManager.call(new EventRender2D(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()));
      ...
      if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1) {
         ...
         if (mm.crossHair.isToggled()) {
            GuiIngameHook.drawCustomCrossHair();
         } else {
      ...
      if (this.recordPlayingUpFor > 0) {
         ...
         if (k1 > 8) {
            ...
            this.getFontRenderer()
               .drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2, -4, i1 + (k1 << 24 & 0xFF000000));
            GlStateManager.disableBlend();
      ...
      if (this.field_175195_w > 0) {
         ...
         if (l1 > 8) {
            ...
            this.getFontRenderer()
               .drawString(this.field_175201_x, (float)(-this.getFontRenderer().getStringWidth(this.field_175201_x) / 2), -10.0F, 16777215 | j2, true);
            GlStateManager.popMatrix();
            GlStateManager.pushMatrix();
            GlStateManager.scale(2.0F, 2.0F, 2.0F);
            this.getFontRenderer()
               .drawString(this.field_175200_y, (float)(-this.getFontRenderer().getStringWidth(this.field_175200_y) / 2), 5.0F, 16777215 | j2, true);
            GlStateManager.popMatrix();
            GlStateManager.disableBlend();
            GlStateManager.popMatrix();
         }
   ...
   protected void renderTooltip(ScaledResolution sr, float partialTicks) {
      if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
         ...
         float f = this.zLevel;
         this.zLevel = -90.0F;
         if (mm.hud.isToggled() && mm.hud.hotBar.getBoolean()) {

         	if(mm.hud.hotbarblur.getBoolean()) {
         		BlurUtil.blur(0.0f, sr.getScaledHeight() - 21.5f, sr.getScaledWidth(), sr.getScaledHeight());
         	}
         	
            RenderUtil.drawFloatRect(
               0.0F, (float)sr.getScaledHeight(), (float)sr.getScaledWidth(), (float)(sr.getScaledHeight() - 22), new Color(0, 0, 0, 185).getRGB()
            );
            RenderUtil.drawFloatRect(
               (float)((double)i - 90.3 + (double)(entityplayer.inventory.currentItem * 20)),
               (float)(sr.getScaledHeight() - 22),
               (float)((double)i - 68.9 + (double)(entityplayer.inventory.currentItem * 20)),
               (float)sr.getScaledHeight(),
               new Color(222, 222, 222, 185).getRGB()
            );
            RenderUtil.drawFloatRect(
               (float)(sr.getScaledWidth() - 5),
               (float)(sr.getScaledHeight() - 22),
               (float)sr.getScaledWidth(),
               (float)sr.getScaledHeight(),
               new Color(255, 255, 255, 200).getRGB()
            );
            GuiIngameHook.drawCustomHotBar();
         } else {
   ...
   private void renderScoreboard(ScoreObjective scoreObjective, ScaledResolution sr) {
      if (!mm.scoreboard.remove.getBoolean() || !mm.scoreboard.isToggled()) {
         ...
         int k1 = sr.getScaledHeight() / 2 + j1 / 3;
         if (mm.scoreboard.isToggled()) {
            if (mm.scoreboard.stick.getBoolean()) {
               k1 = (int)((float)j1 + GuiIngameHook.lastArrayListY + 2.0F);
            } else {
               k1 = (int)((double)(sr.getScaledHeight() - j1) * (mm.scoreboard.yCord.getValue() / 100.0)) + j1;
            }
         }

         byte b0 = 3;
         int j = mm.scoreboard.isToggled()
            ? (int)((double)(sr.getScaledWidth() - i - b0) * (mm.scoreboard.xCord.getValue() / 100.0))
            : sr.getScaledWidth() - i - b0;
         int k = 0;
         ...
         for(Object score1 : arraylist1) {
             ...
             i1 = mm.scoreboard.isToggled() ? sr.getScaledWidth() - (sr.getScaledWidth() - i - b0 - (int)((double)(sr.getScaledWidth() - i - b0) * (mm.scoreboard.xCord.getValue() / 100.0))) : sr.getScaledWidth() - b0 + 2;
         }
         if(mm.scoreboard.blur.getBoolean()) {
    		 BlurUtil.blur(j - 2, k1 - k * this.getFontRenderer().FONT_HEIGHT - this.getFontRenderer().FONT_HEIGHT - 2 + 3, i1, k1 + 2 - 3);
    	 }
         if(mm.scoreboard.rounded.getBoolean()) {
        	 RectUtils.drawRoundedRect(j - 2, k1 - k * this.getFontRenderer().FONT_HEIGHT - this.getFontRenderer().FONT_HEIGHT - 2, i1, k1 + 2, 5, 1342177280);
         }
         k = 0;
         l = 0;
         i1 = 0;
   ...
   private void renderPumpkinOverlay(ScaledResolution p_180476_1_) {
      ...
      worldrenderer.pos(0.0, (double)p_180476_1_.getScaledHeight(), -90.0).tex(0.0, 1.0).endVertex();
      worldrenderer.pos((double)p_180476_1_.getScaledWidth(), (double)p_180476_1_.getScaledHeight(), -90.0).tex(1.0, 1.0).endVertex();
      worldrenderer.pos((double)p_180476_1_.getScaledWidth(), 0.0, -90.0).tex(1.0, 0.0).endVertex();
   ...
   private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
      if (!Config.isVignetteEnabled()) {
         ...
         WorldRenderer worldrenderer = tessellator.getWorldRenderer();
         worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
         worldrenderer.pos(0.0, (double)p_180480_2_.getScaledHeight(), -90.0).tex(0.0, 1.0).endVertex();
         worldrenderer.pos((double)p_180480_2_.getScaledWidth(), (double)p_180480_2_.getScaledHeight(), -90.0).tex(1.0, 1.0).endVertex();
         worldrenderer.pos((double)p_180480_2_.getScaledWidth(), 0.0, -90.0).tex(1.0, 0.0).endVertex();
         worldrenderer.pos(0.0, 0.0, -90.0).tex(0.0, 0.0).endVertex();
         tessellator.draw();
         GlStateManager.depthMask(true);
         GlStateManager.enableDepth();
   ...
   private void func_180474_b(float p_180474_1_, ScaledResolution p_180474_2_) {
      ...
      WorldRenderer worldrenderer = tessellator.getWorldRenderer();
      worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
      worldrenderer.pos(0.0, (double)p_180474_2_.getScaledHeight(), -90.0).tex((double)f, (double)f3).endVertex();
      worldrenderer.pos((double)p_180474_2_.getScaledWidth(), (double)p_180474_2_.getScaledHeight(), -90.0).tex((double)f2, (double)f3).endVertex();
      worldrenderer.pos((double)p_180474_2_.getScaledWidth(), 0.0, -90.0).tex((double)f2, (double)f1).endVertex();
