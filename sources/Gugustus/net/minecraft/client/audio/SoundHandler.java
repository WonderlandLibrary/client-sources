         ...
         try {
            for(IResource iresource : resourceManager.getAllResources(new ResourceLocation(s, "sounds.json"))) {
               try {
                  ...
                  logger.warn("Invalid sounds.json", (Throwable)var9);
               }
            }
         ...
         switch(soundlist$soundentry.getSoundEntryType()) {
            case FILE:
               ...
               try {
                  ...
                  logger.warn("Could not load sound file " + resourcelocation1 + ", cannot add it to event " + location, (Throwable)var19);
                  continue;
               } finally {
                  IOUtils.closeQuietly(inputstream);
               }

               lvt_10_1_ = new SoundEventAccessor(
                  new SoundPoolEntry(
                     resourcelocation1,
                     (double)soundlist$soundentry.getSoundEntryPitch(),
                     (double)soundlist$soundentry.getSoundEntryVolume(),
                     soundlist$soundentry.isStreaming()
                  ),
