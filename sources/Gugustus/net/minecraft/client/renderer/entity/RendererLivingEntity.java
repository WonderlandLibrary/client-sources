...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
   ...
   public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer(LayerRenderer<T> layer) {
	      return this.layerRenderers.add((LayerRenderer<T>) layer);
   }
   
   public <V extends EntityLivingBase, U extends LayerRenderer> boolean addLayer2(LayerRenderer layer) {
   ...
   public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
      if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
         || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
         ...
         if (Reflector.ForgeEntity_shouldRiderSit.exists()) {
            this.mainModel.isRiding = entity.isRiding()
               ...
               && Reflector.callBoolean(entity.ridingEntity, Reflector.ForgeEntity_shouldRiderSit);
         }
   ...
   protected boolean setScoreTeamColor(EntityLivingBase entityLivingBaseIn) {
      ...
      GlStateManager.setActiveTexture(OpenGlHelper.defaultTexUnit);
      float f3 = 1.0F;
      if (mm.esp.isToggled() && !mm.esp.teamColor.getBoolean()) {
         f1 = mm.esp.rainbow.getBoolean()
            ? (float)mm.esp.getRainbowUtil().getColor().getRed() / 255.0F
            : (float)mm.esp.outlineColor.getColor().getRed() / 255.0F;
         f2 = mm.esp.rainbow.getBoolean()
            ? (float)mm.esp.getRainbowUtil().getColor().getGreen() / 255.0F
            : (float)mm.esp.outlineColor.getColor().getGreen() / 255.0F;
         f = mm.esp.rainbow.getBoolean()
            ? (float)mm.esp.getRainbowUtil().getColor().getBlue() / 255.0F
            : (float)mm.esp.outlineColor.getColor().getBlue() / 255.0F;
         f3 = mm.esp.rainbow.getBoolean()
            ? (float)mm.esp.getRainbowUtil().getColor().getAlpha() / 255.0F
            : (float)mm.esp.outlineColor.getColor().getAlpha() / 255.0F;
      }

      GlStateManager.color(f1, f2, f, f3);
   ...
   public void renderName(T entity, double x, double y, double z) {
      if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists()
         ...
         if (this.canRenderName(entity)) {
            ...
            boolean b = entity instanceof EntityPlayer && mm.nameTags.isToggled();
            if (d0 < (double)(f * f) || b) {
               String s = entity.getDisplayName().getFormattedText();
               if (entity instanceof EntityPlayer && MidClick.friends.contains(entity.getName())) {
                  s = "[Friend] " + entity.getDisplayName().getFormattedText();
               }
               
               float f1 = 0.02666667F;
               GlStateManager.alphaFunc(516, 0.1F);
               if (entity.isSneaking() && !Augustus.getInstance().getModuleManager().nameTags.isToggled()) {
