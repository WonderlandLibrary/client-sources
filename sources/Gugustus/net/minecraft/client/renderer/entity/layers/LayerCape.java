...
public class LayerCape implements LayerRenderer {
   ...
   public static TimeHelper animationTimeHelper = new TimeHelper();
   int frameCount = 0;
   ...
   public void doRenderLayer(
      ...
      if ((entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE)) && entitylivingbaseIn instanceof EntityPlayerSP) {
    	  if(MM.mm.cape.isToggled()) {
      		int sel = -1, frames = -1;
      		switch(MM.mm.cape.cape.getSelected()) {
      		case "Mountain":
      			sel = 1;
      			frames = 9;
      			break;
      		...
      		if(sel == -1 || frames == -1)
      			return;
      		String path = "client/capes/" + sel + (frames < 10 ? "/frame_0" : "/frame_");
      		if(animationTimeHelper.reached((long)(MM.mm.cape.speed.getValue() * 1))) {
      			if(frameCount >= frames)
      				frameCount = 0;
      			frameCount++;
      			animationTimeHelper.reset();
      		}
      		path = path + frameCount + ".png";
//      		System.out.println(path);
      		entitylivingbaseIn.setLocationOfCape(new ResourceLocation(path));
      	}
