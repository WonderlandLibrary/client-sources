...
import net.augustus.events.EventMouseMoving;
import net.augustus.events.EventRayCast;
import net.augustus.events.EventRayCastPost;
...
public class EntityRenderer implements IResourceManagerReloadListener, MM {
   private float lastYawBefore;
   private float lastPitchBefore;
   ...
   public void loadShader(ResourceLocation resourceLocationIn) {
      if (OpenGlHelper.isFramebufferEnabled()) {
         try {
            ...
            logger.warn("Failed to load shader: " + resourceLocationIn, (Throwable)var3);
            this.shaderIndex = shaderCount;
            this.useShader = false;
         } catch (JsonSyntaxException var4) {
            logger.warn("Failed to load shader: " + resourceLocationIn, (Throwable)var4);
            this.shaderIndex = shaderCount;
            this.useShader = false;
         }
      }
   ...
   public void getMouseOver(float partialTicks) {
      if (mm.killAura.isToggled() && mm.killAura.target != null) {
         ...
         if (entity != null && this.mc.theWorld != null) {
            ...
            d0 = Math.max(d0, mm.killAura.range);
            ...
            d0 = Math.max(d0, mm.killAura.range);
            d1 = Math.max(d1, mm.killAura.range);
            ...
            List list = this.mc
               ...
               .getEntitiesInAABBexcluding(
                  ...
                  Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRenderer11(this))
               );
            double d2 = d1;
            ...
            for(int i = 0; i < list.size(); ++i) {
               ...
               if (ViaMCP.getInstance().getVersion() != 47 && GuiProtocolSelector.active) {
                  f1 = 0.0F;
               }
            ...
            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > mm.killAura.range) {
               this.pointedEntity = null;
         ...
         if (entity != null && this.mc.theWorld != null) {
            ...
            EventRayCastPost eventRayCastPost = new EventRayCastPost(partialTicks);
            EventHandler.call(eventRayCastPost);
            this.mc.mcProfiler.endSection();
         }
      }

      EventRayCast eventRayCast = new EventRayCast(partialTicks);
      EventHandler.call(eventRayCast);
   }
   ...
   private void hurtCameraEffect(float partialTicks) {
      if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
    	  if(mm.hurtcam.isToggled() && mm.hurtcam.remove.getBoolean())
    		  return;
      ...
      if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
         ...
         if (entity instanceof EntityAnimal) {
            ...
            GlStateManager.rotate(
               entityanimal1.prevRotationYawHead + (entityanimal1.rotationYawHead - entityanimal1.prevRotationYawHead) * partialTicks + 180.0F,
               0.0F,
               1.0F,
               0.0F
            );
      ...
      if (this.lightmapUpdateNeeded) {
         ...
         if (worldclient != null) {
            if (Config.isCustomColors()
               && CustomColors.updateLightmap(worldclient, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision))) {
   ...
   public void func_181560_a(float p_181560_1_, long p_181560_2_) {
      ...
      if (this.mc.inGameHasFocus && flag) {
         ...
         float f = this.mc.gameSettings.mouseSensitivity * 0.6F + 0.2F;
         float f1 = f * f * f * 8.0F;
         EventMouseMoving eventMouseMoving = new EventMouseMoving(this.mc.mouseHelper.deltaX, this.mc.mouseHelper.deltaY);
         EventHandler.call(eventMouseMoving);
         float f2 = (float)eventMouseMoving.getDeltaX() * f1;
         float f3 = (float)eventMouseMoving.getDeltaY() * f1;
         byte b0 = 1;
      ...
      if (!this.mc.skipRenderWorld) {
         ...
         if (this.mc.currentScreen != null) {
            ...
            try {
               ...
               crashreportcategory.addCrashSectionCallable("Screen name", new EntityRenderer12(this));
         ...
         if (this.mc.getRenderViewEntity() != null) {
            this.mc.getRenderViewEntity().prevRotationYaw = this.lastYawBefore;
            this.mc.getRenderViewEntity().prevRotationPitch = this.lastPitchBefore;
            this.mc.getRenderViewEntity().rotationYaw = this.yawBefore;
            this.mc.getRenderViewEntity().rotationPitch = this.pitchBefore;
         }
      }
   ...
   public void renderWorld(float partialTicks, long finishTimeNano) {
      ...
      this.lastYawBefore = this.mc.getRenderViewEntity().prevRotationYaw;
      this.lastPitchBefore = this.mc.getRenderViewEntity().prevRotationPitch;
      this.yawBefore = this.mc.getRenderViewEntity().rotationYaw;
      this.pitchBefore = this.mc.getRenderViewEntity().rotationPitch;
      this.mc.getRenderViewEntity().rotationYaw = Augustus.getInstance().getYawPitchHelper().realYaw;
      this.mc.getRenderViewEntity().rotationPitch = Augustus.getInstance().getYawPitchHelper().realPitch;
      this.mc.getRenderViewEntity().prevRotationYaw = Augustus.getInstance().getYawPitchHelper().realLastYaw;
      this.mc.getRenderViewEntity().prevRotationPitch = Augustus.getInstance().getYawPitchHelper().realLastPitch;
      if (Config.isShaders()) {
   ...
   private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
      ...
      GlStateManager.enableBlend();
      EventRender3D eventRender3D = new EventRender3D();
      EventHandler.call(eventRender3D);
      GlStateManager.disableBlend();
   ...
   private void addRainParticles() {
      ...
      if (f != 0.0F && Config.isRainSplash()) {
         ...
         for(int k = 0; k < j; ++k) {
            BlockPos blockpos1 = worldclient.getPrecipitationHeight(
               blockpos.add(this.random.nextInt(b0) - this.random.nextInt(b0), 0, this.random.nextInt(b0) - this.random.nextInt(b0))
            );
            ...
            if (blockpos1.getY() <= blockpos.getY() + b0
               && blockpos1.getY() >= blockpos.getY() - b0
               ...
               if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
                  ...
                  if (i2 != j2) {
                     ...
                     if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F) {
                        ...
                        double d5 = (
                              (double)(this.rendererUpdateCount + j1 * j1 * 3121 + j1 * 45238971 + i1 * i1 * 418711 + i1 * 13761 & 31) + (double)partialTicks
                           )
                           / 32.0
                        ...
                        int i3 = l2 >> 16 & 65535;
                        int j3 = l2 & 65535;
                        worldrenderer.pos((double)j1 - d3 + 0.5, (double)i2, (double)i1 - d4 + 0.5)
                           .tex(0.0, (double)i2 * 0.25 + d5)
                           .color(1.0F, 1.0F, 1.0F, f3)
                           .lightmap(i3, j3)
                           .endVertex();
                        worldrenderer.pos((double)j1 + d3 + 0.5, (double)i2, (double)i1 + d4 + 0.5)
                           .tex(1.0, (double)i2 * 0.25 + d5)
                           .color(1.0F, 1.0F, 1.0F, f3)
                           .lightmap(i3, j3)
                           .endVertex();
                        worldrenderer.pos((double)j1 + d3 + 0.5, (double)j2, (double)i1 + d4 + 0.5)
                           .tex(1.0, (double)j2 * 0.25 + d5)
                           .color(1.0F, 1.0F, 1.0F, f3)
                           .lightmap(i3, j3)
                           .endVertex();
                        worldrenderer.pos((double)j1 - d3 + 0.5, (double)j2, (double)i1 - d4 + 0.5)
                           .tex(0.0, (double)j2 * 0.25 + d5)
                           .color(1.0F, 1.0F, 1.0F, f3)
                           .lightmap(i3, j3)
                           .endVertex();
                     } else {
                        ...
                        int i4 = k3 & 65535;
                        worldrenderer.pos((double)j1 - d3 + 0.5, (double)i2, (double)i1 - d4 + 0.5)
                           .tex(0.0 + d9, (double)i2 * 0.25 + d8 + d10)
                           .color(1.0F, 1.0F, 1.0F, f4)
                           .lightmap(l3, i4)
                           .endVertex();
                        worldrenderer.pos((double)j1 + d3 + 0.5, (double)i2, (double)i1 + d4 + 0.5)
                           .tex(1.0 + d9, (double)i2 * 0.25 + d8 + d10)
                           .color(1.0F, 1.0F, 1.0F, f4)
                           .lightmap(l3, i4)
                           .endVertex();
                        worldrenderer.pos((double)j1 + d3 + 0.5, (double)j2, (double)i1 + d4 + 0.5)
                           .tex(1.0 + d9, (double)j2 * 0.25 + d8 + d10)
                           .color(1.0F, 1.0F, 1.0F, f4)
                           .lightmap(l3, i4)
                           .endVertex();
                        worldrenderer.pos((double)j1 - d3 + 0.5, (double)j2, (double)i1 - d4 + 0.5)
                           .tex(0.0 + d9, (double)j2 * 0.25 + d8 + d10)
                           .color(1.0F, 1.0F, 1.0F, f4)
                           .lightmap(l3, i4)
                           .endVertex();
                     }
   ...
   private void updateFogColor(float partialTicks) {
      ...
      //fog modifier
        if(mm.fogModifier.isToggled()) {
      	  this.fogColorRed += (float) (f1 - this.fogColorRed + mm.fogModifier.fogR.getValue()) * f;
      	  this.fogColorGreen += (float) (f2 -this.fogColorGreen + mm.fogModifier.fogG.getValue()) * f;
      	  this.fogColorBlue += (float) (f3 -this.fogColorBlue + mm.fogModifier.fogB.getValue()) * f;
        }else {
      ...
      if (this.cloudFog) {
         ...
         Vec3 vec34 = CustomColors.getUnderwaterColor(
            this.mc.theWorld, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0, this.mc.getRenderViewEntity().posZ
         );
         if (vec34 != null) {
      ...
      if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
         Object object = Reflector.newInstance(
            Reflector.EntityViewRenderEvent_FogColors_Constructor, this, entity, block, partialTicks, this.fogColorRed, this.fogColorGreen, this.fogColorBlue
         );
         Reflector.postForgeBusEvent(object);
