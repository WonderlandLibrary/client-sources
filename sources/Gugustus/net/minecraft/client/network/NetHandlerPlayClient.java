...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
      ...
      double z = packetIn.getX();
      double y = packetIn.getY();
      double x = packetIn.getZ();
      ...
      entityplayer.setPositionAndRotation(z, y, x, yaw, pitch);
      this.netManager
         .sendPacket(
   ...
   public void addToSendQueue(Packet packet) {
      ...
      EventHandler.call(eventSendPacket);
      if (!eventSendPacket.isCanceled()) {
         this.netManager.sendPacket(eventSendPacket.getPacket());
      }
   }
   ...
   public void handleAnimation(S0BPacketAnimation packetIn) {
      ...
      if (entity != null) {
         ...
         } else if (packetIn.getAnimationType() == 5 && !Augustus.getInstance().getModuleManager().attackEffects.isToggled()) {
   ...
   public void handleExplosion(S27PacketExplosion packetIn) {
      final int threshold = Short.MAX_VALUE * 4;

      if (packetIn.func_149149_c() >= threshold
              || packetIn.func_149144_d() > threshold
              || packetIn.func_149147_e() > threshold) {
         //NotificationsManager.addNotification(new Notification("Blocked Crack using a big explosion lol.", Notification.Type.Alert));
         GeneralNotifyManager.addNotification("Blocked Crash using a big explosion lol.", NotificationType.Alert);
   ...
   public void handlePlayerListItem(S38PacketPlayerListItem packetIn) {
      ...
      for(S38PacketPlayerListItem.AddPlayerData s38packetplayerlistitem$addplayerdata : packetIn.getPlayers()) {
   ...
   public boolean validateResourcePackUrl(String url, final String hash) {
      try {
         ...
         if (isLevelProtocol && (url.contains("..") || !url.endsWith("/resources.zip"))) {
            ...
            if (player != null) {
               PlayerUtil.sendChat("Just blocked a illegal exploit to check for file: " + url);
            }
         ...
         for(S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot : packetIn.func_149441_d()) {
            ...
            if (iattributeinstance == null) {
               iattributeinstance = baseattributemap.registerAttribute(
                  new RangedAttribute((IAttribute)null, s20packetentityproperties$snapshot.func_151409_a(), 0.0, Double.MIN_NORMAL, Double.MAX_VALUE)
               );
            }
   ...
   public Map<UUID, NetworkPlayerInfo> getPlayerMapInfo() {
