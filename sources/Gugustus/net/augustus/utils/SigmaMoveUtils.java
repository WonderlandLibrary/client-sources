...
public class SigmaMoveUtils {
    ...
    public static Block getBlockAtPosC(double x, double y, double z) {
        ...
        return (Minecraft.getMinecraft()).theWorld.getBlockState(new BlockPos(((EntityPlayer)entityPlayerSP).posX + x, ((EntityPlayer)entityPlayerSP).posY + y, ((EntityPlayer)entityPlayerSP).posZ + z)).getBlock();
    }
    ...
    public static float getDistanceToGround(Entity e) {
        ...
        for (float a = (float)e.posY; a > 0.0F; a--) {
            int[] stairs = {
                    ...
                    163, 164, 180 };
            int[] exemptIds = {
                    6, 27, 28, 30, 31, 32, 37, 38, 39, 40,
                    50, 51, 55, 59, 63, 65, 66, 68, 69, 70,
                    72, 75, 76, 77, 83, 92, 93, 94, 104, 105,
                    106, 115, 119, 131, 132, 143, 147, 148, 149, 150,
                    157, 171, 175, 176, 177 };
            ...
            if (!(block instanceof net.minecraft.block.BlockAir)) {
                ...
                int arrayOfInt1[], j = (arrayOfInt1 = stairs).length;
                int i;
                for (i = 0; i < j; i++) {
    ...
    public static boolean isBlockAboveHead() {
        ...
        return !mc.theWorld.getCollidingBoundingBoxes((Entity)mc.thePlayer, bb).isEmpty();
    }
    ...
    public static boolean isCollidedH(double dist) {
        ...
        if (!mc.theWorld.getCollidingBoundingBoxes((Entity)mc.thePlayer, bb.offset(0.3D + dist, 0.0D, 0.0D)).isEmpty())
            return true;
        if (!mc.theWorld.getCollidingBoundingBoxes((Entity)mc.thePlayer, bb.offset(-0.3D - dist, 0.0D, 0.0D)).isEmpty())
            return true;
        if (!mc.theWorld.getCollidingBoundingBoxes((Entity)mc.thePlayer, bb.offset(0.0D, 0.0D, 0.3D + dist)).isEmpty())
            return true;
        if (!mc.theWorld.getCollidingBoundingBoxes((Entity)mc.thePlayer, bb.offset(0.0D, 0.0D, -0.3D - dist)).isEmpty())
            return true;
        return false;
