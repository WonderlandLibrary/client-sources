package net.augustus.utils.skid.lorious.font;
...
public class FontRenderer extends CFont
{
    ...
    public int drawStringWithShadow(final String text, final double x2, final float y2, final int color, final boolean fade, final Color first, final Color second) {
        final float shadowWidth = this.drawString(text, x2 + 0.8999999761581421, y2 + 0.5, color, true, 8.3f, false, first, second);
        return (int)Math.max(shadowWidth, this.drawString(text, x2, y2, color, false, 8.3f, fade, first, second));
    }

    public int drawString(final String text, final double x2, final double y2, final int color, final boolean fade, final Color first, final Color second) {
        return (int)this.drawString(text, x2, y2, color, false, 8.3f, fade, first, second);
    }

    public int drawString(final String text, final double x2, final double y2, final int color, final boolean fade, final boolean shadow, final Color first, final Color second) {
        if (shadow) {
            return this.drawStringWithShadow(text, x2, (float)y2, color, fade, first, second);
        }
        return (int)this.drawString(text, x2, y2, color, false, 8.3f, fade, first, second);
    }

    public int drawPassword(final String text, final double x2, final float y2, final int color) {
        return (int)this.drawString(text.replaceAll(".", "."), x2, y2, color, false, 8.0f, false, new Color(0, 0, 0), new Color(0, 0, 0));
    }
    ...
    public float drawCenteredString(final String text, final float x2, final float y2, final int color, final boolean fade, final Color first, final Color second) {
        return (float)this.drawString(text, x2 - (float)(this.getStringWidth(text) / 2.0), y2, color, fade, first, second);
    }
    ...
    public float drawCenteredStringWithShadow(final String text, final float x2, final float y2, final int color, final boolean fade, final Color first, final Color second) {
        return (float)this.drawStringWithShadow(text, x2 - (float)(this.getStringWidth(text) / 2.0), y2, color, fade, first, second);
    }

    public float drawString(final String text, double x, double y, int color, final boolean shadow, final float kerning, final boolean fade, final Color first, final Color second) {
        --x;
        if (text == null) {
            return 0.0f;
        }
        ...
        for (int index = 0; index < text.length(); ++index) {
            ...
            if (fade) {
                final int clr = ColorUtils.getGradientOffset(first, second, Math.abs(System.currentTimeMillis() / 16L) / 50.0 + index * 0.25).getRGB();
                ColorUtils.color(clr);
            }
            if (character == FontInvokerFix.COLOR_INVOKER) {
                int colorIndex = 21;
                try {
    ...
    public double getStringWidth(final String text, final float kerning) {
        ...
        for (int index = 0; index < text.length(); ++index) {
            ...
            if (c == FontInvokerFix.COLOR_INVOKER) {
        ...
        if (this.getStringWidth(text) > width) {
            ...
            for (final String word : words) {
                for (int innerIndex = 0; innerIndex < word.toCharArray().length; ++innerIndex) {
                    ...
                    if (c == FontInvokerFix.COLOR_INVOKER && innerIndex < word.toCharArray().length - 1) {
    ...
    public List<String> formatString(final String string, final double width) {
        ...
        for (int index = 0; index < chars.length; ++index) {
            ...
            if (c == FontInvokerFix.COLOR_INVOKER && index < chars.length - 1) {
