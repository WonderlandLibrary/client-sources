...
public class GuiProtocolSelector extends GuiScreen {
   ...
   public void initGui() {
      super.initGui();
      this.string = active ? "Activated" : "Deactivated";
      ...
      this.list = new GuiProtocolSelector.SlotList(this.mc, this.width, this.height, 32, this.height - 32, 10);
   }
   ...
   class SlotList extends GuiSlot {
      ...
      protected void drawSlot(int i, int i1, int i2, int i3, int i4, int i5) {
         GuiProtocolSelector.this.drawCenteredString(
            ...
            (
                  ViaMCP.getInstance().getVersion() == ProtocolCollection.values()[i].getVersion().getVersion()
                     ? EnumChatFormatting.GREEN.toString() + EnumChatFormatting.BOLD
                     : EnumChatFormatting.GRAY.toString()
               )
               + ProtocolCollection.getProtocolById(ProtocolCollection.values()[i].getVersion().getVersion()).getName(),
            this.width / 2,
            i2 + 2,
            -1
         );
         ...
         GuiProtocolSelector.this.drawCenteredString(
            this.mc.fontRendererObj,
            "Ver: " + ProtocolCollection.getProtocolById(ProtocolCollection.values()[i].getVersion().getVersion()).getVersion(),
            this.width,
            (i2 + 2) * 2 + 20,
            -1
         );
