...
public class AltWidget implements IMinecraft {
    ...
    public void render(MatrixStack stack, int x, int y) {
        ...
        float height = Math.min(20 + (open ? 999 + (alts.size() + 1) * (17) : 0), 100);


        DisplayUtils.drawRoundedRect(this.x, this.y, width, height, new Vector4f(0, 0, 0, 0), ColorUtils.rgba(56, 71, 97, 128));
        Scissor.push();
        Scissor.setFromComponentCoordinates(this.x, this.y, width - 16, height);
        Fonts.sfbold.drawText(stack, mc.session.getUsername(), this.x + 6, this.y + 6, -1, 7);
        Scissor.unset();
        Scissor.pop();
        Fonts.icons.drawText(stack, open ? "C" : "B", this.x + width - 6 - Fonts.icons.getWidth(open ? "C" : "B", 7), this.y + 6.5f, -1, 7);
        if (open) {
            DisplayUtils.drawRectHorizontalW(this.x, this.y + 20, width, 5f, ColorUtils.rgba(97, 101, 137, 0), ColorUtils.rgba(97, 101, 137, 64));
            DisplayUtils.drawRectW(this.x, this.y + 20, width, 0.5f, ColorUtils.rgba(97, 101, 137, 255));
            Scissor.push();
            Scissor.setFromComponentCoordinates(this.x, this.y + 20, width, 100f - 20);
            float i = 0;
            for (Alt alt : alts) {
                DisplayUtils.drawRoundedRect(this.x + 5, this.y + 26 + i * 17 + scroll, width - 10, 15, 0, mc.session.getUsername().equals(alt.name) ? ColorUtils.rgba(96, 101, 137, 128) : ColorUtils.rgba(96, 101, 137, 64));
                Fonts.sfbold.drawText(stack, alt.name, this.x + 10, this.y + 26 + i * 17 + 4 + scroll, -1, 6);
                i++;
            }
            ...
            Fonts.sfbold.drawText(stack, textToDraw + (typing ? (System.currentTimeMillis() % 1000 > 500 ? "_" : "") : ""),
            ...
            DisplayUtils.drawRoundedRect(this.x + 5 + 2, this.y + 26 + i * 17 + 2 + scroll,
                    Fonts.sfbold.getWidth(textToDraw + (typing ? (System.currentTimeMillis() % 1000 > 500 ? "_" : "") : ""),
                            6) + 7, 15 - 4, 0, ColorUtils.rgba(30, 30, 36, 64));
            Fonts.sfbold.drawText(stack, "+", this.x + width - 18,
    ...
    public void onChar(char typed) {
        if (typing) {
            if (Fonts.sfbold.getWidth(altName, 6f) < 145 - 50) {
                altName += typed;
            }
        }
    }
