...
public class CrystalAura extends Module {
    ...
    public SettingBoolean ghostMode = register(new SettingBoolean("Fast Mode",this,false));
    ...
    public BlockPos get_best_block() {
        ...
        for (EntityPlayer target : mc.world.playerEntities) {
            ...
            for (BlockPos block : blocks) {
                ...
                if (!CrystalUtils.canSeeBlock(block) && mc.player.getDistance(block.getX(), block.getY(), block.getZ()) > placeRange.getValue()) {
                    continue;
                }
                ...
                double target_damage = CrystalUtils.calculateDamage(block.getX() + 0.5D, block.getY() + 1.0D, block.getZ() + 0.5D, target);

                if (target_damage < minimum_damage)
                    continue;
                double self_damage = CrystalUtils.calculateDamage(block.getX() + 0.5D, block.getY() + 1.0D, block.getZ() + 0.5D, mc.player);
