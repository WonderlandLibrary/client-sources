...
public class WorldUtil {
    ...
    public static HoleInfo isHole(BlockPos centreBlock, boolean onlyOneWide, boolean ignoreDown) {
        ...
        HashMap<BlockOffset, BlockSafety> unsafeSides = WorldUtil.getUnsafeSides(centreBlock);

        if (unsafeSides.containsKey(WorldUtil.BlockOffset.DOWN)) {
            if (unsafeSides.remove(WorldUtil.BlockOffset.DOWN, WorldUtil.BlockSafety.BREAKABLE)) {
                if (!ignoreDown) {
        ...
        unsafeSides.entrySet().removeIf(entry -> entry.getValue() == WorldUtil.BlockSafety.RESISTANT);
    ...
    private static HoleInfo isDoubleHole(HoleInfo info, BlockPos centreBlock, BlockOffset weakSide) {
        ...
        HashMap<WorldUtil.BlockOffset, WorldUtil.BlockSafety> unsafeSides = WorldUtil.getUnsafeSides(unsafePos);
        ...
        if (unsafeSides.containsKey(WorldUtil.BlockOffset.DOWN)) {
            ...
            unsafeSides.remove(WorldUtil.BlockOffset.DOWN);
        }
