...
public class NameTags extends Module {
   ...
   private int getNametagColor(EntityPlayer player) {
      ...
      int color = -1;
      if(Client.getFriendManager().isFriend(player.getName())) {
         color = -16724788;
         ...
         color = -1020415;
      } else if((mc.thePlayer.isSneaking() || Client.getModuleManager().getModuleByName("sneak") != null && Client.getModuleManager().getModuleByName("sneak").isEnabled()) && !player.canEntityBeSeen(mc.thePlayer)) {
         color = -13447886;
   ...
   private String getNametagName(EntityPlayer player) {
      ...
      if(Client.getFriendManager().isFriend(StringUtils.stripControlCodes(player.getName()))) {
         ...
         name = StringUtils.stripControlCodes(Client.getFriendManager().replaceNames(name, false));
      }
   ...
   protected void renderNametag(EntityPlayer player, double x, double y, double z) {
      ...
      GL11.glDisable(3553);
      float var17 = (float)(mc.fontRendererObj.getStringWidth(StringUtils.stripControlCodes(Client.getFriendManager().replaceNames(name, true))) / 2);
      GL11.glPushMatrix();
      GL11.glPopMatrix();
      GL11.glEnable(3553);
