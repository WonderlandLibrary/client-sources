...
import me.jinthium.straight.impl.modules.visual.ItemTransformer;
...
public class ItemRenderer
{
    ...
    {
        boolean animations = Client.INSTANCE.getModuleManager().getModule(ItemTransformer.class).isEnabled();
        double x = animations ? .56 + (ItemTransformer.x.getValue() * .01) : .56;
        double y = animations ? .52 - (ItemTransformer.y.getValue() * .01) : .52;
        double size = animations ? .40 + (ItemTransformer.size.getValue() * .01) : .40;
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0 || killAura.blockAnimation)
                {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK -> {
                            if (Client.INSTANCE.getModuleManager().getModule(ItemTransformer.class).isEnabled()) {
                                switch (ItemTransformer.mode.getMode()) {
                                    case "Stella" -> {
                                    ...
                                    case "Swong" -> {
                                        ...
                                        GlStateManager.rotate(f * 30.0F / 2.0F, -f, -0.0F, 9.0F);
                                        GlStateManager.rotate(f * 40.0F, 1.0F, -f / 2.0F, -0.0F);
                ...
                {
                    if (!ItemTransformer.smallSwing.isEnabled()) {
