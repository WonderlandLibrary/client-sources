...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public double getBPS() {
        if(this==null || this.ticksExisted < 1) {
            return 0;
        }
        return getDistance(this.lastTickPosX,this.lastTickPosZ) * (20 * Minecraft.getMinecraft().timer.timerSpeed);
    }
            ...
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        if (i > 0)
                        {
                            targetEntity.addVelocity((double)(-MathHelper.sin(this.movementYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));

                            KeepSprintEvent keepSprintEvent = new KeepSprintEvent(0.6D, false);
                            Client.INSTANCE.getPubSubEventBus().publish(keepSprintEvent);

                            if(!keepSprintEvent.isCancelled()) {
                                this.motionX *= keepSprintEvent.getSlowDown();
                                this.motionZ *= keepSprintEvent.getSlowDown();
                                this.setSprinting(keepSprintEvent.isSprint());
                            }
                        }
    ...
    public int getPing() {
        Iterator index = Minecraft.getMinecraft().getNetHandler().playerInfoMap.values().iterator();;
        while(index.hasNext()) {
            NetworkPlayerInfo net_e = (NetworkPlayerInfo)index.next();
            if(net_e.getGameProfile().getId().equals(this.getUniqueID())) return net_e.getResponseTime();
        }
        return 0;
    }
