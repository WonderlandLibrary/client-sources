...
public class MultipleBoolComponent extends SettingComponent<MultiBoolSetting> {
    ...
    public MultipleBoolComponent(MultiBoolSetting modeSetting) {
        super(modeSetting);
        normalCount = 2;
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        MultiBoolSetting modeSetting = getSetting();
        ...
        normalFont14.drawString(modeSetting.getName(), boxX + boxWidth / 2 - normalFont14.getStringWidth(modeSetting.getName()) / 2, y + 5, textColor);

        normalFont16.drawString("...", boxX + boxWidth / 2 -
                normalFont16.getStringWidth("...") / 2, boxY + normalFont16.getMiddleOfBox(boxHeight) + 2, textColor);
        ...
        if (opened || !openAnimation.isDone()) {
            ...
            float modeHeight = (modeSetting.getBoolSettings().size()) * rectHeight;
            ...
            for (BooleanSetting booleanSetting : modeSetting.getBoolSettings()) {
                boolean hoveringMode = HoveringUtil.isHovering(modeX, modeY + rectCount * rectHeight, boxWidth, rectHeight, mouseX, mouseY);
                if(hoveringMode) {
                    this.hoveringMode = booleanSetting.getName();
                }


                if(booleanSetting.getName().equals(this.hoveringMode)) {
                ...
                if(booleanSetting.isEnabled()){
                    RoundedUtil.drawRound(modeX + 1.5f, modeY + 1.5f + rectCount * rectHeight
                            ,boxWidth - 3, rectHeight - 3,
                            2, ColorUtil.applyOpacity(hoveringMode ? clientColors.getSecond().brighter() : clientColors.getSecond(), openAnim * (hoveringMode ? selectionBox.getOutput().floatValue() : openAnimation.getOutput().floatValue())));
                }
                RenderUtil.resetColor();
                normalFont16.drawString(booleanSetting.getName(), modeX + boxWidth / 2 - normalFont16.getStringWidth(booleanSetting.getName()) / 2,
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if(opened) {
            ...
            for (BooleanSetting mode : modeSetting.getBoolSettings()) {
//                if(mode.equals(modeSetting.getCurrentMode())) continue;
                boolean hoveringMode = HoveringUtil.isHovering(modeX, modeY + rectCount * rectHeight, boxWidth, rectHeight, mouseX, mouseY);
