...
import me.jinthium.straight.impl.event.render.RenderLivingEntityEvent;
...
public class EntityCulling extends Module {
    ...
    private static final NumberSetting cullingDelay = new NumberSetting("Culling Delay", 2, 1, 3, 1);
    private static final ModeSetting cullingMode = new ModeSetting("Culling Mode", "Grouped", "Grouped", "Custom");
    private static final NumberSetting entityCullingDis = new NumberSetting("Culling Distance", 45, 10, 150, 1);
    private static final NumberSetting mobCullingDis = new NumberSetting("Mob Cull Distance", 40, 10, 150, 1);
    private static final NumberSetting playerCullingDis = new NumberSetting("Player Cull Distance", 45, 10, 150, 1);
    private static final NumberSetting passiveCullingDis = new NumberSetting("Passive Cull Distance", 30, 10, 150, 1);

    public EntityCulling() {
        super("Entity Culling", 0, Category.MISC);
    ...
    public static boolean renderItem(Entity stack) {
        if (!Client.INSTANCE.getModuleManager().getModule(EntityCulling.class).isEnabled()) return false;
    ...
    @Callback
    final EventCallback<RenderLivingEntityEvent> renderLivingEntityEventEventCallback = event -> {
        if (event.isPost() || !shouldPerformCulling) return;
...
//        EntityLivingBase entity = event.getEntity();
    ...
    @Callback
    final EventCallback<RenderTickEvent> playerUpdateEventCallback = event -> {
        if (event.isPre()) {
    ...
    final EventCallback<TickEvent> tickEventEventCallback = event -> {
        if (event.isPost() || this.destroyTimer++ < 120) {
            return;
        }
