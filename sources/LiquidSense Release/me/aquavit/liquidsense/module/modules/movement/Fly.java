...
public class Fly extends Module {
    ...
    private final FloatValue blinkAttackPacket = new FloatValue("ZoomAttack-Packet", 4F, 2F, 20F);
    ...
    public void onEnable() {
        ...
        switch(mode.toLowerCase()) {
            ...
            case "zoom":
                ...
                double fall = 3.0125; //add 0.0125 to ensure we get the fall dmg
                while (fall > 0) {
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.0624986421, mc.thePlayer.posZ, false));
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.0625      , mc.thePlayer.posZ, false));
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.0624986421, mc.thePlayer.posZ, false));
    ...
    public void onMotion(final MotionEvent event) {
        if(modeValue.get().equalsIgnoreCase("zoom")) {
        ...
        if(modeValue.get().equalsIgnoreCase("boosthypixel")) {
            switch(event.getEventState()) {
                case PRE:
                    ...
                    if (hypixelTimer.hasTimePassed(2)) {
                        mc.thePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY + 1.0999E-5, mc.thePlayer.posZ);
                        hypixelTimer.reset();
                    }
    ...
    public void onPacket(PacketEvent event) {
        ...
        if(packet instanceof C03PacketPlayer) {
            ...
            if (mode.equalsIgnoreCase("Hypixel") || mode.equalsIgnoreCase("BoostHypixel") || mode.equalsIgnoreCase("Zoom"))
                packetPlayer.onGround = true;
        }
        ...
        if(modeValue.get().equalsIgnoreCase("Zoom"))
        {
            ...
            if(blinkAttack.get() && packet instanceof C02PacketUseEntity && packetList.size() >= blinkAttackPacket.get()) {
                noPacketModify = true;
                PlayerCapabilities playerCapabilities = new PlayerCapabilities();
                playerCapabilities.allowFlying = true;
                playerCapabilities.isFlying = true;
    ...
    public void onMove(final MoveEvent event) {
        switch(modeValue.get().toLowerCase()) {
            ...
            case "zoom":
                ...
                final double amplif = 1 + (mc.thePlayer.isPotionActive(Potion.moveSpeed) ? 0.2 *
                        (mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1) : 0);
                final double base = 0.29D * amplif;

                switch (boostHypixelState) {
                    case 1:
                        moveSpeed = (mc.thePlayer.isPotionActive(Potion.moveSpeed) ? 1.56 : 2.034) * base;
                        boostHypixelState = 2;
                        break;
                    case 2:
                        moveSpeed *= 2.16D;
                        ...
                        break;
                    case 3:
                        moveSpeed = lastDistance - (mc.thePlayer.ticksExisted % 2 == 0 ? 0.0103D : 0.0123D) * (lastDistance - base);
                ...
                final double ya = MovementUtils.getDirection();
                event.setX(-Math.sin(ya) * moveSpeed);
                event.setZ(Math.cos(ya) * moveSpeed);
                mc.thePlayer.motionX = event.getX();
                mc.thePlayer.motionZ = event.getZ();
                break;
            case "freehypixel":
    ...
    public void onBB(final BlockBBEvent event) {
        ...
        if (event.getBlock() instanceof BlockAir && (mode.equalsIgnoreCase("Hypixel") ||
                mode.equalsIgnoreCase("BoostHypixel") || mode.equalsIgnoreCase("Zoom") ||mode.equalsIgnoreCase("Rewinside") ||
    ...
    public void onJump(final JumpEvent e) {
        ...
        if (mode.equalsIgnoreCase("Hypixel") || mode.equalsIgnoreCase("BoostHypixel") ||
                mode.equalsIgnoreCase("Zoom") ||mode.equalsIgnoreCase("Rewinside") || (mode.equalsIgnoreCase("Mineplex") && mc.thePlayer.inventory.getCurrentItem() == null))
            e.cancelEvent();
    }
