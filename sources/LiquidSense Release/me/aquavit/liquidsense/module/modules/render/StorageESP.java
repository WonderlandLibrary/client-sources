    ...
    public void onRender3D(Render3DEvent event) {
        try {
            ...
            for (final TileEntity tileEntity : mc.theWorld.loadedTileEntityList) {
                ...
                if (chestValue.get() && tileEntity instanceof TileEntityChest && !ChestAura.clickedBlocks.contains(tileEntity.getPos()))
                    color = new Color(0, 66, 255);

                if (enderChestValue.get() && tileEntity instanceof TileEntityEnderChest && !ChestAura.clickedBlocks.contains(tileEntity.getPos()))
                    color = Color.MAGENTA;
