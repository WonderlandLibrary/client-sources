...
import igbt.astolfy.module.visuals.Animations;
...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (this.itemToRender != null)
        {
            ...
            else if (entityplayersp.getItemInUseCount() > 0 || Killaura.currentTarget != null && Astolfy.i.moduleManager.getModuleByName("Killaura").isToggled() && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)
            {
                EnumAction enumaction = this.itemToRender.getItemUseAction();
                if(Animations.mode.getCurrentValue().equalsIgnoreCase("Test"))
                	f1 -= 0.5;
                switch (ItemRenderer.ItemRenderer$1.field_178094_a[enumaction.ordinal()])
                {
                    ...
                    case 4:
                    	if(Animations.mode.getCurrentValue().equalsIgnoreCase("Smooth")) {
                    		GlStateManager.translate(-0.15, 0, -0.4);
                    		GlStateManager.rotate(-20,0,1,0);
                    		f-=0.2;
                    		f1-=1.2;
                    	}
                        if(Animations.mode.getCurrentValue().equalsIgnoreCase("Test"))
                        	this.transformFirstPersonItem((float)(f1 + 0.2), f1);
                        else
                            this.transformFirstPersonItem(f, f1);
                        this.func_178103_d();
                        break;
                    ...
                    case 5:
                        ...
                        this.transformFirstPersonItem(f, (float) (f1 + 0.7));
            ...
            double amount = Animations.scale.getValue();
            GlStateManager.translate(0,Animations.y.getValue() - 0.5, Animations.x.getValue() - .5);
            GlStateManager.scale(amount, amount, amount);
            GlStateManager.translate(0,-(Animations.y.getValue() - 0.5), -(Animations.x.getValue() - .5));
            if(entityplayersp.getItemInUseCount() > 0 || Killaura.currentTarget != null && Astolfy.moduleManager.getModuleByName("Killaura").isToggled())
                if(Animations.mode.getCurrentValue().equalsIgnoreCase("Spin")) {
                    if (spinAmount > 720) {
                        spinAmount = 0;
                    } else
                        spinAmount++;
                    GlStateManager.rotate((float) spinAmount, 0, 1, 0);
                    GlStateManager.rotate((float) spinAmount /2, 1, 0, 0);
                }
