package igbt.astolfy.ui.ClickGUI.dropDownLight.impl;
...
import igbt.astolfy.ui.ClickGUI.dropDownLight.impl.set.*;
...
public class Button {
	...
	public double drawScreen(int mouseX, int mouseY, float partialTicks, double plusplus) {
		...
		Gui.drawRect(panel.x + 1,panel.y + y + plusplus, panel.x + panel.width - 1, panel.y + y + height + plusplus, mod.isToggled() ? !isHovered(mouseX, mouseY) ? panel.category.color.darker().darker().getRGB() : panel.category.color.darker().darker().darker().getRGB() : isHovered(mouseX, mouseY) ?
				ClickGui.getSecondaryColor().darker().getRGB() : ClickGui.getSecondaryColor().getRGB());
		mc.customFont.drawString(binding ? "Binding..." : mod.getName(), panel.x + 5, panel.y + y + ((height - mc.customFont.getHeight()) / 2) + plusplus, 0);
		setHeight = 0;
		sussy = plusplus;
		if(extended) {
			int count = 0;
			...
			if(settings.size() > 0) {
				RenderUtils.drawDrop(panel.x, panel.y + height + plusplus + y, panel.x + panel.width, 5);
				RenderUtils.drawDrop(panel.x, panel.y + height + plusplus + setHeight + y, panel.x + panel.width, -5);
			}
		}
		return setHeight;
	}
