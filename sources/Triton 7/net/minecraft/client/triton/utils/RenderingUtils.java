...
public final class RenderingUtils {
	...
	public static void drawSearchBlock(Block block, BlockPos blockPos, Render3DEvent event) {
		...
		RenderGlobal.drawOutlinedBoundingBox(
				(AxisAlignedBB) block.getSelectedBoundingBox((World) Minecraft.getMinecraft().theWorld, blockPos)
	...
	public static void boundingBox(Entity entity, double x, double y, double z, int color, int colorIn) {
		...
		AxisAlignedBB var12 = new AxisAlignedBB(var11.minX - entity.posX + x, var11.minY - entity.posY + y,
	...
	public static void drawBeacon(BlockPos pos, int color, int colorIn, float partialTicks) {
		...
		GL11.glLineWidth((float) 1.0f);
		AxisAlignedBB var11 = new AxisAlignedBB((double) (pos.getX() + 1), (double) pos.getY(),
				(double) (pos.getZ() + 1), (double) pos.getX(), (double) (pos.getY() + 200), (double) pos.getZ());
		AxisAlignedBB var12 = new AxisAlignedBB(var11.minX - x, var11.minY - y, var11.minZ - z, var11.maxX - x,
				var11.maxY - y, var11.maxZ - z);
		if (color != 0) {
			GlStateManager.disableDepth();
			RenderingUtils.filledBox(var12, colorIn, true);
			RenderingUtils.disableLighting();
	...
	public static void drawPotionStatus(ScaledResolution sr) {
		...
		for (final PotionEffect effect : (Collection<PotionEffect>) ClientUtils.mc().thePlayer
			...
			ClientUtils.clientFont().drawStringWithShadow(PType,
					sr.getScaledWidth() - (ClientUtils.clientFont().getStringWidth(PType)),
					sr.getScaledHeight() - ClientUtils.clientFont().FONT_HEIGHT + pY, potion.getLiquidColor());
			pY -= 9;
		}
	}
	...
	public static void drawStuffStatus(ScaledResolution scaledRes) {
		...
		boolean onwater = ClientUtils.mc().thePlayer.isEntityAlive()
				&& ClientUtils.mc().thePlayer.isInsideOfMaterial(Material.water);
		int split = -3;
		...
		for (ItemStack errything : stuff) {
			...
			ClientUtils.mc().getRenderItem().func_180450_b(errything, split + scaledRes.getScaledWidth() / 2 - 4,
					scaledRes.getScaledHeight() - (onwater ? 65 : 55));
			ClientUtils.mc().getRenderItem().func_175030_a(ClientUtils.mc().fontRendererObj, errything,
					split + scaledRes.getScaledWidth() / 2 - 4, scaledRes.getScaledHeight() - (onwater ? 65 : 55));
	...
	public static void renderEnchantText(ItemStack stack, int x, int y) {
		...
		if ((stack.getItem() instanceof ItemArmor)) {
			...
			if (pLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "p" + pLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (tLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "t" + tLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (uLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "u" + uLevel, x * 2, encY, 16777215);
				encY += 7;
			}
		}
		if ((stack.getItem() instanceof ItemBow)) {
			...
			int uLevel = EnchantmentHelper.getEnchantmentLevel(Enchantment.unbreaking.effectId, stack);
			if (sLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "d" + sLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (kLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "k" + kLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (fLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "f" + fLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (uLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "u" + uLevel, x * 2, encY, 16777215);
		...
		if ((stack.getItem() instanceof ItemSword)) {
			...
			int uLevel = EnchantmentHelper.getEnchantmentLevel(Enchantment.unbreaking.effectId, stack);
			if (sLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "s" + sLevel, x * 2, encY, 16777215);
				encY += 7;
			}
			if (kLevel > 0) {
				ClientUtils.clientFont().drawStringWithShadow("§" + "f" + "k" + kLevel, x * 2, encY, 16777215);
