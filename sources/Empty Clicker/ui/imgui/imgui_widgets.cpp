...
bool ImGui::Checkbox( const char *label, bool *v )
{
	...
	RenderFrame( ( check_bb.Min ), ( check_bb.Max - ImVec2( 6, 6 ) ), GetColorU32( ( held && hovered ) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg ), true, style.FrameRounding );
	...
	if ( mixed_value )
	{
		...
		window->DrawList->AddRectFilled( check_bb.Min + ImVec2( 2, 2 ) + pad, check_bb.Max - ImVec2( 8, 8 ) - pad, check_col, style.FrameRounding );
	}
	else if ( *v )
	{
		// -> i made another type of checkbox
		// const float pad = ImMax( 1.0f, IM_FLOOR( square_sz / 5.0f ) );
		// RenderCheckMark( window->DrawList, check_bb.Min + ImVec2( 2, 2 ), check_col, square_sz - pad * 2.3f );

		window->DrawList->AddRectFilled( check_bb.Min + ImVec2( 2, 2 ), check_bb.Max - ImVec2( 8, 8 ), check_col, style.FrameRounding );
	}

	ImVec2 label_pos = ImVec2( check_bb.Max.x - 2.0f + style.ItemInnerSpacing.x, check_bb.Min.y - 4.0f + style.FramePadding.y );
