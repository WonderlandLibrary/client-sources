  ...
  extends AbstractComponentUI<Frame>
{
  ...
  SimpleFrameUI(SimpleTheme theme)
  {
    ...
    background = new Color(0, 83, 102, 255);
  }
  ...
  protected void renderComponent(Frame component)
  {
    ...
    {
      height = (fontHeight + 4);
      RenderUtil.setColor(new Color(0, 83, 102, 255));
      GL11.glBegin(7);
    ...
    {
      RenderUtil.setColor(new Color(0, 52, 64, 255));
      GL11.glBegin(7);
      GL11.glVertex2d(0.0D, 0.0D);
      GL11.glVertex2d(width, 0.0D);
      GL11.glVertex2d(width, height);
    ...
    boolean[] checks = { component.isClosable(), component.isPinnable(), component.isMinimizable() };
    boolean[] overlays = { false, component.isPinned(), component.isMinimized() };
    for (int i = 0; i < checks.length; i++)
    {
      ...
      if ((x >= offset - fontHeight) && (x <= offset) && (y >= 2) && (y <= fontHeight + 2))
      {
    ...
    GL11.glEnd();
    GL11.glEnable(3553);
    theme.getFontRenderer().drawStringWithShadow(component.getTitle(), 2.0F, 2.0F, 
  ...
  protected void handleComponentInteraction(Frame component, Point location, int button)
  {
    ...
    {
      if ((x >= offset - textHeight) && (x <= offset) && (y >= 2) && 
        (y <= textHeight + 2))
      {
        component.close();
        return;
    ...
    if ((x >= 0) && (x <= offset) && (y >= 0) && (y <= textHeight + 4))
    {
