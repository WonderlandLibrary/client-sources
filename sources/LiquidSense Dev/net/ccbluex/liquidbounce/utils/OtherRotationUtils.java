...
public final class OtherRotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static float[] getRotations(EntityLivingBase ent) {
        ...
        return getVecRotation(getEyesPos(), enemyCoords);
    }

    public static float[] getVecRotation(Vec3 origin, Vec3 position) {
        Vec3 difference = position.subtract(origin);
        double distance = difference.lengthVector();
        final double diffXZ = Math.sqrt(difference.xCoord * difference.xCoord + difference.zCoord * difference.zCoord);
        float yaw = ((float)Math.toDegrees(Math.atan2(difference.zCoord, difference.xCoord)) - 90.0f);
        float pitch = (float)(-(Math.atan2(difference.yCoord, diffXZ) * 180.0D / 3.141592653589793D));
    ...
    public static VecRotation searchCenter(final AxisAlignedBB bb, final boolean outborder, final boolean random, final boolean predict, final boolean throughWalls,final float distance) {
        ...
        for(double xSearch = 0.4D; xSearch < 0.6D; xSearch += 0.1D) {
            for (double ySearch = 0.3D; ySearch < 1D; ySearch += 0.1D) {
                for (double zSearch = 0.4D; zSearch < 0.6D; zSearch += 0.1D) {
    ...
    public static VecRotation searchCenter(final AxisAlignedBB bb) {
        double ySearch;
        ...
        double  z = bb.minZ + (bb.maxZ - bb.minZ) * (entityonr ? 0.25: 0.5);
              for(double xSearch = 0.1D; xSearch < 0.9D; xSearch += 0.1D) {
              for (ySearch = 0.2D; ySearch < 0.8D; ySearch += 0.2D) {
              for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
    ...
    public void onTick(final TickEvent event) {
        ...
        if(random.nextGaussian() > 0.6D) x = Math.random();
        if(random.nextGaussian() > 0.6D) y = Math.random();
        if(random.nextGaussian() > 0.6D) z = Math.random();
    }
    ...
    public static void setTargetRotation(final Rotation rotation, final int keepLength) {
        ...
        OtherRotationUtils.keepLength = keepLength;
    }

    public static void setTRotation(final Rotation rotation, final int keepLength) {
        if(Double.isNaN(rotation.getYaw()) || Double.isNaN(rotation.getPitch())
                || rotation.getPitch() > 90 || rotation.getPitch() < -90)
            return;
