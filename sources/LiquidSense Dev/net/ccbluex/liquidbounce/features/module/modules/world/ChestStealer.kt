...
class ChestStealer : Module() {
    ...
    private val slientstealer = BoolValue("SlientStealer",false)
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        if (slientstealer.get() && screen.inventorySlots.windowId == contentReceived && autoCloseTimer.hasTimePassed(nextCloseDelay)) {
            mc.thePlayer.closeScreen()
            nextCloseDelay = TimeUtils.randomDelay(autoCloseMinDelayValue.get(), autoCloseMaxDelayValue.get())
        }
    }

    fun getTrue(): BoolValue {
        return slientstealer
    }
