...
object Fucker : Module() {
    ...
    private val actionValue = ListValue("Action", arrayOf("Destroy", "Use","New"), "Destroy")
    ...
    fun onUpdate(event: UpdateEvent) {
        if (noHitValue.get()) {
            val aura = LiquidBounce.moduleManager.getModule(Aura::class.java) as Aura
        ...
        when {
            // Destory block
            actionValue.get().equals("destroy", true) || actionValue.get().equals("new", true) || surroundings -> {
                // Auto Tool
                ...
                if (instantValue.get()) {
                    ...
                    if(actionValue.get().equals("new", true)){
                        mc.netHandler.addToSendQueue(C07PacketPlayerDigging(C07PacketPlayerDigging.Action.START_DESTROY_BLOCK,
                                currentPos, EnumFacing.DOWN))
                    }
