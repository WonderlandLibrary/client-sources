...
class Step : Module() {
    ...
    private val timeValue = FloatValue("MotionTime", 0.42F, 0.1F, 1.0F)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (stepY == BigDecimal(stepY).setScale(3, RoundingMode.HALF_DOWN).toDouble())
        ...
        if (thePlayer.entityBoundingBox.minY - stepY > 0.5) { // Check if full block step
            ...
            when {
                mode.equals("NCP", ignoreCase = true) || mode.equals("AAC", ignoreCase = true) -> {
                    fakeJump()
                    mc.timer.timerSpeed = timeValue.get()
