...
class InventoryCleaner : Module() {
    ...
    private val maxWaterBucketsValue = object : IntegerValue("MaxWaterBuckets", 3, 0, 10) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            if (newValue < minimum) {
                set(minimum)
            }
    ...
    private val maxLavaBucketsValue = object : IntegerValue("MaxLavaBuckets", 3, 0, 10) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            if (newValue < minimum) {
                set(minimum)
            }
    ...
    private val items = arrayOf("None", "Ignore", "Sword", "Bow", "Pickaxe", "Axe", "Food", "Block", "Water", "Gapple", "Pearl", "SpeedPotion", "HealPotion")
        ...
        return try {
            ...
            if (item is ItemSword || item is ItemTool) {
                ...
                items(0, 45).none { (_, stack) -> itemStack != stack && stack.unlocalizedName == "item.compass" }
                    ...
                    (item is ItemBlock && getBlockStacksAmount() <= maxBlockStacksValue.get() && !itemStack.unlocalizedName.contains("flower")) || //TODO
    ...
    private fun getBlocksAmount(): Int {
        ...
        for (i in 9..44) {
            ...
            if (itemStack != null && itemStack.item is ItemBlock && !itemStack.unlocalizedName.contains("flower")) amount += itemStack.stackSize
        }
        return amount
    }
        ...
        when (type.toLowerCase()) {
            ...
            "healpotion" -> {
                mc.thePlayer.inventory.mainInventory.forEachIndexed { index, stack ->
                    ...
                    if (item is ItemPotion && !type(index).equals("HealPotion", ignoreCase = true)) {
                        val replaceCurr = slotStack == null || slotStack.item !is ItemPotion
                        for (potionEffect in item.getEffects(stack))
                            if (potionEffect.potionID == Potion.heal.id || potionEffect.potionID == Potion.regeneration.id)
                                return if (replaceCurr) index else null
                    }
                }
            }
            ...
            "speedpotion" -> {
                mc.thePlayer.inventory.mainInventory.forEachIndexed { index, stack ->
                    ...
                    if (item is ItemPotion && !type(index).equals("SpeedPotion", ignoreCase = true)) {
                        val replaceCurr = slotStack == null || slotStack.item !is ItemPotion
                        for (potionEffect in item.getEffects(stack))
