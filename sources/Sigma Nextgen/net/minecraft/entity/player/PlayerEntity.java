...
import info.sigmaclient.sigma.modules.movement.SafeWalk;
...
public abstract class PlayerEntity extends LivingEntity
{
    ...
    public ItemEntity dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        else
        {
            if (this.world.isRemote)
            {
//                if(!ViaFixManager.viaFixManager.negativeTimerFix.isEnable() && !ViaFixManager.viaFixManager.v117PlaceFix.isEnable())
    ...
    {
        if ((!this.abilities.isFlying && (mover == MoverType.SELF || mover == MoverType.PLAYER) && this.isStayingOnGroundSurface() && this.func_242375_q()) || SigmaNG.getSigmaNG().moduleManager.getModule(SafeWalk.class).enabled)
        {
            double d0 = vec.x;
            double d1 = vec.z;
            double d2 = 0.05D;
