...
import info.sigmaclient.sigma.modules.render.Fullbright;
...
public abstract class LivingEntity extends Entity
{
    ...
    protected void playEquipSound(ItemStack stack)
    {
        if (!stack.isEmpty())
        {
            ...
            }else if(item == Items.SHIELD && OldHitting.renderOldHitting()){
                return;
            }
    ...
    {
        if(potionIn == Effects.NIGHT_VISION && this instanceof ClientPlayerEntity){
            if(SigmaNG.getSigmaNG().moduleManager.getModule(Fullbright.class).enabled){
                if(((Fullbright)SigmaNG.getSigmaNG().moduleManager.getModule(Fullbright.class)).booleanValue.isEnable()){
                    return false;
                }
            }
        }
    ...
    {
        if(OldHitting.renderOldHitting() && OldHitting.modify.isEnable()){
            return OldHitting.speed.getValue().intValue();
        }
    ...
    public void handleStatusUpdate(byte id)
    {
        switch (id)
        {
            ...
            case 29:
                if(!OldHitting.renderOldHitting())
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_)
    {
        ...
        if(this instanceof ClientPlayerEntity){
//            if(mc.player.isWearing(PlayerModelPart.CAPE)) {
//            if(mc.player.getLocationOfCape() != null)
//                yaw = ((ClientPlayerEntity) this).lastReportedYaw;
//            }
        }
            ...
            if (!flag || this.onGround && !(d7 > d8))
            {
                if (!this.isInLava() || this.onGround && !(d7 > d8))
                {
                    if ((this.onGround || flag && d7 <= d8) && this.jumpTicks == 0)
                    {
                        this.jump();
                        this.jumpTicks = 10;
                        if(SigmaNG.getSigmaNG().moduleManager.getModule(AutoSprint.class).enabled && AutoSprint.noJumpDelay.isEnable() && this instanceof ClientPlayerEntity){
                            this.jumpTicks = 0;
                        }
                    }
                }
