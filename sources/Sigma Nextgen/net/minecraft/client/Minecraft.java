...
import info.sigmaclient.sigma.Sigma;
import info.sigmaclient.sigma.modules.misc.FakeLag;
import info.sigmaclient.sigma.sigma5.SelfDestructManager;
import info.sigmaclient.sigma.sigma5.jelloportal.florianmichael.viamcp.fixes.AttackOrder;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener
{
    ...
    public static final boolean IS_RUNNING_ON_LINUX = Util.getOSType() == Util.OS.LINUX;
...
//    @Nullable
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        if(!SelfDestructManager.destruct){
            this.setLoadingGui(new Sigma5LoadProgressGui(this, this.resourceManager.reloadResources(Util.getServerExecutor(), this, RESOURCE_RELOAD_INIT_TASK, list), (throwable) ->
            {
    ...
    {
        StringBuilder stringbuilder = new StringBuilder(Sigma.getWindowTitle());

        if (this.isModdedClient() && SelfDestructManager.destruct)
        {
            stringbuilder.append("*");
        }

        stringbuilder.append(" ");
        stringbuilder.append("1.16.4");
...
//                        this.player.swingArm(Hand.MAIN_HAND);
    ...
    {
        EventManager.call(new MouseClickEvent(true));
...
//                this.player.swingArm(Hand.MAIN_HAND);
    ...
    {
        MouseClickEvent clickEvent = new MouseClickEvent(false);
        EventManager.call(clickEvent);
        if (!this.playerController.getIsHittingBlock())
        {
            this.rightClickDelayTimer = 4;
            if (clickEvent.rightClickDelay != -1) {
                this.rightClickDelayTimer = clickEvent.rightClickDelay;
            }
    ...
    public void runTick()
    {
        EventManager.call(new WindowUpdateEvent());
        ...
        boolean pause = FakeLag.publicFreeze && ((FakeLag)SigmaNG.getSigmaNG().moduleManager.getModule(FakeLag.class)).betterAnimation.isEnable();
    ...
    {
        EventManager.call(new WorldEvent());
    ...
    {
        return (entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER) && (!SigmaNG.getSigmaNG().inGameGuiBlur || SelfDestructManager.destruct);
    }
    public boolean isEntityGlowing2(Entity entity)
    {
    ...
    public boolean isF3Enabled() {
        return this.gameSettings.showDebugInfo;
    }
