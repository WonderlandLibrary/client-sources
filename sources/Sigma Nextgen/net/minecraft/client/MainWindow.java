...
public final class MainWindow implements AutoCloseable
{
    ...
    public int 䄟姮䎰ꪕ鱀洝(final int n, final boolean b) {
        int i;
        for (i = 1; i != n; ++i) {
            if (i >= this.framebufferWidth) {
                break;
            }
            if (i >= this.framebufferHeight) {
                break;
            }
            if (this.framebufferWidth / (i + 1) < 320) {
                break;
            }
            if (this.framebufferHeight / (i + 1) < 240) {
                break;
            }
        }
        if (b) {
    ...
    private void onWindowSizeUpdate(long windowPointer, int windowWidthIn, int windowHeightIn)
    {
        ...
        if (windowWidthIn != 0 && windowHeightIn != 0) {
//            SigmaNG.lineWidth = (float)Math.max(getFramebufferWidth() / windowHeightIn, getFramebufferHeight() / windowHeightIn);
        }
    }
