...
//                mc.player.capeRotateY
    ...
    {
        if(SigmaNG.getSigmaNG().commandManager.onChat(message)) return;
    ...
    /**
     * returns true if this is an PlayerEntitySP, or the logged in player.
     */
    ...
    public void livingTick()
    {
        ...
        if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && (!this.isHandActive() || NoSlow.isNeedNoslow()) && !this.isPotionActive(Effects.BLINDNESS) && this.mc.gameSettings.keyBindSprint.isKeyDown())
        {
            this.setSprinting(true);
        }
    ...
    private void handlePortalTeleportation()
    {
        ...
        if (this.inPortal)
        {
            if (this.mc.currentScreen != null && !this.mc.currentScreen.isPauseScreen() && !SigmaNG.getSigmaNG().moduleManager.getModule(PortalGui.class).enabled)
            {
    ...
    {
        MoveEvent e = new MoveEvent(pos.x, pos.y, pos.z);
        EventManager.call(e);
        double d0 = this.getPosX();
        double d1 = this.getPosZ();
        super.move(typeIn, new Vector3d(e.getX(), e.getY() ,e.getZ()));
