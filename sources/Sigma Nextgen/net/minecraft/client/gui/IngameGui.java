...
import info.sigmaclient.sigma.modules.gui.NoScoreBoard;
...
import static info.sigmaclient.sigma.gui.hud.JelloArrayList.lasty;
...
public class IngameGui extends AbstractGui
{
    ...
    {
//        if(SigmaNG.getSigmaNG().moduleManager.getModule(ActiveMods.class).enabled) return;
        ...
        if (!collection.isEmpty())
        {
            ...
            while (true)
            {
                ...
                if (effectinstance.isShowIcon())
                {
                    ...
                    if(SigmaNG.getSigmaNG().moduleManager.getModule(ActiveMods.class).enabled){
                        k -= 100;
                    }
                    int l = 1;
                    ...
                    list.add(() ->
                    {
                        ...
                        if(!SelfDestructManager.destruct){
                            ...
                            RenderUtils.drawRect(j1 + 3, k1 + 3 + 18, j1 + 3 + 18 * Math.min((finalEffectinstance.getDuration() / (20 * 60f)), 1), k1 + 3 + (int)(20), -1);
                            StencilUtil.readStencilBuffer(1);
                            this.mc.getTextureManager().bindTexture(textureatlassprite.getAtlasTexture().getTextureLocation());
                            RenderSystem.color4f(1.0F, 1.0F, 1.0F, f1);
                            blit(matrixStack, j1 + 3 - 20, k1 + 3 - 20, this.getBlitOffset(), 80, 100, textureatlassprite);
                            StencilUtil.uninitStencilBuffer();
                            RenderUtils.drawRect(j1 + 3, k1 + 3 + 18, j1 + 3 + 18 * Math.min((finalEffectinstance.getDuration() / (20 * 60f)), 1), k1 + 3 + (int)(20), new Color(255, 255, 255, 100).getRGB());
                        }
                    });
    ...
    protected void renderHotbar(float partialTicks, MatrixStack matrixStack)
    {
        ...
        if (playerentity != null)
        {
            ...
            if (!itemstack.isEmpty() && !OldHitting.renderOldHitting())
            {
    ...
    {
        if(SigmaNG.SigmaNG.moduleManager.getModule(NoScoreBoard.class).enabled) return;
    ...
    public void renderHotbarItemCustom(int x, int y, float partialTicks, PlayerEntity player, ItemStack stack, float scale) {
        if (!stack.isEmpty()) {
            if (scale != 1) {
                RenderSystem.pushMatrix();
                RenderSystem.translatef((float) (x + 4), (float) (y + 4), 0.0F);
