...
import info.sigmaclient.sigma.modules.misc.GamePlay;
...
public class NewChatGui extends AbstractGui
{
     ...
     */
    public void clearChatMessages2()
    {
        ArrayList<String> needDeleteMessage = new ArrayList<>();
        for(String str : this.sentMessages){
            if(SelfDestructManager.isInVaildSend(str)){
                needDeleteMessage.add(str);
            }
        }
        this.sentMessages.removeAll(needDeleteMessage);
        ArrayList<Integer> integers = new ArrayList<>();
        this.chatLines.removeIf((p_lambda$deleteChatLine$1_1_) ->
        {
            boolean id = SelfDestructManager.isInVaildMessage(p_lambda$deleteChatLine$1_1_.getLineString().getString());
            if(id){
                integers.add(p_lambda$deleteChatLine$1_1_.getChatLineID());
            }
            return false;
        });
        ...
        this.field_238489_i_.removeIf((p_lambda$deleteChatLine$1_1_) ->
        {
            return SelfDestructManager.isInVaildMessage(p_lambda$deleteChatLine$1_1_.getString());
        });
    }
    /**
     * Clears the chat.
    ...
    {
        if(SigmaNG.getSigmaNG().moduleManager.getModule(GamePlay.class).enabled){
            GamePlay.onChat(chatComponent.getUnformattedComponentText());
        }
        this.func_238493_a_(chatComponent, chatLineId, this.mc.ingameGUI.getTicks(), false);
        if(SelfDestructManager.isInVaildMessage(chatComponent.getString())) return;
