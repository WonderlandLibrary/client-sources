...
import info.sigmaclient.sigma.modules.player.Freeinventory;
...
public class PlayerController
{
    ...
    public ActionResultType processRightClickBlock(ClientPlayerEntity p_217292_1_, ClientWorld p_217292_2_,
            BlockPos pos, Direction face, Vector3d hitvec
            ,

                                          Hand p_217292_3_)
    {
        BlockRayTraceResult p_217292_4_ =
                new BlockRayTraceResult(hitvec, face, pos, false);
        this.syncCurrentPlayItem();
        BlockPos blockpos = p_217292_4_.getPos();
    ...
    public void attackEntity(PlayerEntity playerIn, Entity targetEntity)
    {
        if(playerIn instanceof ClientPlayerEntity) {
            EventManager.call(new AttackEvent(targetEntity).PRE());
        }
        this.syncCurrentPlayItem();
        ...
        if(playerIn instanceof ClientPlayerEntity) {
            EventManager.call(new AttackEvent(targetEntity).POST());
        }
    }
     ...
     */
    public ItemStack windowClickFixed(int windowId, int slotId, int mouseButton, ClickType type, PlayerEntity player, boolean n)
    {
        ItemStack itemstack = null;
        if(slotId >= 0){
            itemstack = player.openContainer.getSlot(slotId).getStack().copy();
        }
        short short1 = player.openContainer.getNextTransactionID(player.inventory);
        ItemStack is = player.openContainer.slotClick(slotId, mouseButton, type, player);
        lab:
        {
            if ((itemstack != null && ViaLoadingBase.getInstance().getTargetVersion().isOlderThanOrEqualTo(ProtocolVersion.v1_8) || n) && type != ClickType.SWAP) {
                break lab;
            }
            itemstack = is;
        }
        ...
        if(n)
            mc.getConnection().sendPacket(new CConfirmTransactionPacket(windowId, short1, true));
        return itemstack;
    }
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, PlayerEntity player) {
        ItemStack itemstack = null;
        if (!SigmaNG.getSigmaNG().moduleManager.getModule(Freeinventory.class).enabled) {
            if (slotId >= 0) {
                itemstack = player.openContainer.getSlot(slotId).getStack().copy();
            }
            short short1 = player.openContainer.getNextTransactionID(player.inventory);
            ItemStack is = player.openContainer.slotClick(slotId, mouseButton, type, player);
            if (itemstack != null && ViaLoadingBase.getInstance().getTargetVersion().isOlderThanOrEqualTo(ProtocolVersion.v1_8) && type == ClickType.SWAP) {
                itemstack = is;
            }
            if (itemstack == null) {
                itemstack = is;
...
//        mc.getConnection().sendPacket(new CConfirmTransactionPacket(windowId, short1, true));
    ...
    public void sendDiggingPacket(CPlayerDiggingPacket.Action action, BlockPos pos, Direction dir)
    {
