...
public class SlimeRenderer extends MobRenderer<SlimeEntity, SlimeModel<SlimeEntity>>
{
    ...
    protected void preRenderCallback(SlimeEntity LivingEntityIn, MatrixStack matrixStackIn, float partialTickTime)
    {
        float f = 0.999F;
        matrixStackIn.scale(0.999F, 0.999F, 0.999F);
        matrixStackIn.translate(0.0D, (double)0.001F, 0.0D);
        float f1 = (float)LivingEntityIn.getSlimeSize();
        float f2 = MathHelper.lerp(partialTickTime, LivingEntityIn.prevSquishFactor, LivingEntityIn.squishFactor) / (f1 * 0.5F + 1.0F);
