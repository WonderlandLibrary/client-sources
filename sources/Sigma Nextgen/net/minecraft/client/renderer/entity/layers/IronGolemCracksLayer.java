...
public class IronGolemCracksLayer extends LayerRenderer<IronGolemEntity, IronGolemModel<IronGolemEntity>>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, IronGolemEntity LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        if (!LivingEntityIn.isInvisible())
        {
            IronGolemEntity.Cracks irongolementity$cracks = LivingEntityIn.func_226512_l_();
            ...
            if (irongolementity$cracks != IronGolemEntity.Cracks.NONE)
            {
                ...
                renderCutoutModel(this.getEntityModel(), resourcelocation, matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, 1.0F, 1.0F, 1.0F);
            }
        }
    }
}
