...
public class FoxHeldItemLayer extends LayerRenderer<FoxEntity, FoxModel<FoxEntity>>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, FoxEntity LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        boolean flag = LivingEntityIn.isSleeping();
        boolean flag1 = LivingEntityIn.isChild();
        matrixStackIn.push();
        ...
        float f1 = LivingEntityIn.func_213475_v(partialTicks);
        ...
        ItemStack itemstack = LivingEntityIn.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
        Minecraft.getInstance().getFirstPersonRenderer().renderItemSide(LivingEntityIn, itemstack, ItemCameraTransforms.TransformType.GROUND, false, matrixStackIn, bufferIn, packedLightIn);
        matrixStackIn.pop();
    }
}
