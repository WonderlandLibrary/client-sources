...
public class VillagerLevelPendantLayer<T extends LivingEntity & IVillagerDataHolder, M extends EntityModel<T> & IHeadToggle> extends LayerRenderer<T, M> implements IResourceManagerReloadListener
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        {
            VillagerData villagerdata = LivingEntityIn.getVillagerData();
            ...
            renderCutoutModel(m, resourcelocation, matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, 1.0F, 1.0F, 1.0F);
            m.func_217146_a(true);

            if (villagerprofession != VillagerProfession.NONE && !LivingEntityIn.isChild())
            {
                ResourceLocation resourcelocation1 = this.func_215351_a("profession", Registry.VILLAGER_PROFESSION.getKey(villagerprofession));
                renderCutoutModel(m, resourcelocation1, matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, 1.0F, 1.0F, 1.0F);
                ...
                if (villagerprofession != VillagerProfession.NITWIT)
                {
                    ...
                    renderCutoutModel(m, resourcelocation2, matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, 1.0F, 1.0F, 1.0F);
                }
            }
        }
    }
