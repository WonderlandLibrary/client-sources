...
public class HorseMarkingsLayer extends LayerRenderer<HorseEntity, HorseModel<HorseEntity>>
{
    ...
    {
        ResourceLocation resourcelocation = field_239405_a_.get(LivingEntityIn.func_234240_eM_());

        if (resourcelocation != null && !LivingEntityIn.isInvisible())
        {
            IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.getEntityTranslucent(resourcelocation));
            this.getEntityModel().render(matrixStackIn, ivertexbuilder, packedLightIn, LivingRenderer.getPackedOverlay(LivingEntityIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}
