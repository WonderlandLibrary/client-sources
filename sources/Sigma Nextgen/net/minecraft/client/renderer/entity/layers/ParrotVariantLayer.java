...
public class ParrotVariantLayer<T extends PlayerEntity> extends LayerRenderer<T, PlayerModel<T>>
{
    ...
    {
        this.renderParrot(matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, limbSwing, limbSwingAmount, netHeadYaw, headPitch, true);
        this.renderParrot(matrixStackIn, bufferIn, packedLightIn, LivingEntityIn, limbSwing, limbSwingAmount, netHeadYaw, headPitch, false);
    }

    private void renderParrot(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T LivingEntityIn, float limbSwing, float limbSwingAmount, float netHeadYaw, float headPitch, boolean leftShoulderIn)
    {
        CompoundNBT compoundnbt = leftShoulderIn ? LivingEntityIn.getLeftShoulderEntity() : LivingEntityIn.getRightShoulderEntity();
        ...
        }).ifPresent((p_lambda$renderParrot$1_11_) ->
        {
            ...
            if (LivingEntityIn instanceof AbstractClientPlayerEntity)
            {
                AbstractClientPlayerEntity abstractclientplayerentity = (AbstractClientPlayerEntity)LivingEntityIn;
            ...
            matrixStackIn.push();
            matrixStackIn.translate(leftShoulderIn ? (double)0.4F : (double) - 0.4F, LivingEntityIn.isCrouching() ? (double) - 1.3F : -1.5D, 0.0D);
            ...
            this.parrotModel.renderOnShoulder(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, limbSwing, limbSwingAmount, netHeadYaw, headPitch, LivingEntityIn.ticksExisted);
            matrixStackIn.pop();
