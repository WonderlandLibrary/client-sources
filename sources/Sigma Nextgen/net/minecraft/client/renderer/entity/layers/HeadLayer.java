...
public class HeadLayer<T extends LivingEntity, M extends EntityModel<T> & IHasHead> extends LayerRenderer<T, M>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        if (!itemstack.isEmpty())
        {
            ...
            boolean flag = LivingEntityIn instanceof VillagerEntity || LivingEntityIn instanceof ZombieVillagerEntity;

            if (LivingEntityIn.isChild() && !(LivingEntityIn instanceof VillagerEntity))
            {
                float f = 2.0F;
                float f1 = 1.4F;
            ...
            else if (!(item instanceof ArmorItem) || ((ArmorItem)item).getEquipmentSlot() != EquipmentSlotType.HEAD)
            {
                ...
                Minecraft.getInstance().getFirstPersonRenderer().renderItemSide(LivingEntityIn, itemstack, ItemCameraTransforms.TransformType.HEAD, false, matrixStackIn, bufferIn, packedLightIn);
            }
