...
public abstract class EnergyLayer<T extends Entity & IChargeableMob, M extends EntityModel<T>> extends LayerRenderer<T, M>
{
    ...
    {
        if (LivingEntityIn.isCharged())
        {
            float f = (float)LivingEntityIn.ticksExisted + partialTicks;
            EntityModel<T> entitymodel = this.func_225635_b_();
            entitymodel.setLivingAnimations(LivingEntityIn, limbSwing, limbSwingAmount, partialTicks);
            ...
            entitymodel.setRotationAngles(LivingEntityIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
