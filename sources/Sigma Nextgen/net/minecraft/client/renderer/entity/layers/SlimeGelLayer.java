...
public class SlimeGelLayer<T extends LivingEntity> extends LayerRenderer<T, SlimeModel<T>>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        if (!LivingEntityIn.isInvisible())
        {
            ...
            this.slimeModel.setLivingAnimations(LivingEntityIn, limbSwing, limbSwingAmount, partialTicks);
            this.slimeModel.setRotationAngles(LivingEntityIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
            IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.getEntityTranslucent(this.getEntityTexture(LivingEntityIn)));
            this.slimeModel.render(matrixStackIn, ivertexbuilder, packedLightIn, LivingRenderer.getPackedOverlay(LivingEntityIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}
