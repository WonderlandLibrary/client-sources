...
import info.sigmaclient.sigma.event.render.RenderModelEvent;
...
//                    酋埙鼒呓䡸㐈2,
//                    n3,
//                    ၝ䣓贞Ⱋ牰醧.ᜄ岋鼒蛊瀳W(f, 柿樽롤揩뫤ᢻ.뗴Ⱋ杭瀳顸뼢, 柿樽롤揩뫤ᢻ.頉待郝셴䈔鏟),
//                    f,
//                    柿樽롤揩뫤ᢻ
//            );
...
//            }
            ...
            matrixStackIn.translate(0.0D, (double) - 1.5010000467300415, 0.0D);
            float f9 = 0.0F;
            float f5 = 0.0F;
            ...
            Runnable drawModel = Runnables.doNothing();
            if (rendertype != null)
            {
                ...
                drawModel = () -> this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, 1.0F, 1.0F, 1.0F, flag2 ? 0.15F : 1.0F);
            }

            if (!entityIn.isSpectator())
            {
                if(!ShadowESP.cannotRenderName()) {
            ...
            float finalF1 = f9;
            float finalF2 = f2;
            RenderModelEvent renderModelEvent = new RenderModelEvent(entityIn,
                    drawModel,
                    () -> {
                if (!entityIn.isSpectator())
                        for (LayerRenderer<T, M> layerrenderer : this.layerRenderers)
                        {
                            layerrenderer.render(matrixStackIn, bufferIn, packedLightIn, entityIn, finalF, finalF1, partialTicks, f8, finalF2, f7);
                        }
                    });
    ...
    protected float getDeathMaxRotation(T LivingEntityIn)
    {
        return 90.0F;
    }
    ...
    protected void preRenderCallback(T LivingEntityIn, MatrixStack matrixStackIn, float partialTickTime)
    {
    }
