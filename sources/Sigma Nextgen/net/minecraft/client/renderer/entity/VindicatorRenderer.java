...
public class VindicatorRenderer extends IllagerRenderer<VindicatorEntity>
{
    ...
    public VindicatorRenderer(EntityRendererManager renderManagerIn)
    {
        ...
        {
            public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, VindicatorEntity LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
            {
                if (LivingEntityIn.isAggressive())
                {
