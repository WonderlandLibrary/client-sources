...
public class PlayerRenderer extends LivingRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>
{
    ...
    private void setModelVisibilities(AbstractClientPlayerEntity clientPlayer)
    {
        ...
        else
        {
            ...
            if(clientPlayer instanceof ClientPlayerEntity) {
                if (OldHitting.isBlocking() && !OldHitting.always.isEnable()) {
                    playermodel.rightArmPose = BipedModel.ArmPose.BLOCK;
                    playermodel.leftArmPose = BipedModel.ArmPose.EMPTY;
                }
            }else if(OldHitting.renderOldHitting() && clientPlayer.isHandActive() && clientPlayer.getHeldItem(Hand.OFF_HAND).getItem() instanceof ShieldItem){
                playermodel.rightArmPose = BipedModel.ArmPose.BLOCK;
                playermodel.leftArmPose = BipedModel.ArmPose.EMPTY;
            }
        }
    ...
    protected void preRenderCallback(AbstractClientPlayerEntity LivingEntityIn, MatrixStack matrixStackIn, float partialTickTime)
    {
        float f = 0.9375F;
