...
import info.sigmaclient.sigma.modules.render.LowFire;
...
public class OverlayRenderer
{
    ...
    private static void renderFire(Minecraft minecraftIn, MatrixStack matrixStackIn)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR_TEX);
            float alpha = 0.9F;
            if(SigmaNG.getSigmaNG().moduleManager.getModule(LowFire.class).enabled){
                alpha = 0.3f;
            }
            bufferbuilder.pos(matrix4f, -0.5F, -0.5F, -0.5F).color(1.0F, 1.0F, 1.0F, alpha).tex(f8, f10).endVertex();
            bufferbuilder.pos(matrix4f, 0.5F, -0.5F, -0.5F).color(1.0F, 1.0F, 1.0F, alpha).tex(f7, f10).endVertex();
            bufferbuilder.pos(matrix4f, 0.5F, 0.5F, -0.5F).color(1.0F, 1.0F, 1.0F, alpha).tex(f7, f9).endVertex();
            bufferbuilder.pos(matrix4f, -0.5F, 0.5F, -0.5F).color(1.0F, 1.0F, 1.0F, alpha).tex(f8, f9).endVertex();
            bufferbuilder.finishDrawing();
            WorldVertexBufferUploader.draw(bufferbuilder);
            matrixStackIn.pop();
        }
