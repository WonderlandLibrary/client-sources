...
public class FirstPersonRenderer
{
    ...
    {
        if(OldHitting.renderOldHitting() && itemStackIn.getItem() instanceof ShieldItem) return;
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            matrixStackIn.push();
            if(OldHitting.renderFPOldHitting()){
                if(flag) {
                    OldHitting.doBlockingAnimMainPre(matrixStackIn, stack);
                }else{
                    OldHitting.doBlockingAnimOffPre(matrixStackIn, stack);
                }
            }
            if (stack.isEmpty()) {
                ...
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    ...
                    switch (stack.getUseAction()) {
                        ...
                        case BLOCK:
                            if (OldHitting.renderOldHitting()) break;
                            this.transformSideFirstPerson(matrixStackIn, handside, equippedProgress);
                            break;
                ...
                } else if (OldHitting.isBlocking() && handside == HandSide.RIGHT) {
                    OldHitting.doBlockingAnim(swingProgress, matrixStackIn, 0, stack);
                    ...
                    int l = flag3 ? 1 : -1;
                    if (!OldHitting.renderFPOldHitting() || !OldHitting.mode.is("OSwing")) {
                ...
                if(OldHitting.renderFPOldHitting()){
                    if(flag) {
                        OldHitting.doBlockingAnimMainPost(matrixStackIn, stack);
                    }
                }
