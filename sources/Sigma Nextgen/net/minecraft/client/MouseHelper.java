...
import info.sigmaclient.sigma.modules.combat.Aimbot;
...
public class MouseHelper
{
    ...
    public void updatePlayerLook()
    {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused())
        {
            ...
            if(SigmaNG.getSigmaNG().moduleManager.getModule(Aimbot.class).enabled) {
                float[] f = ((Aimbot) SigmaNG.getSigmaNG().moduleManager.getModule(Aimbot.class)).rot;
                if(f != null){
                    double d22 = RotationUtils.getAngleDifference(f[0], Minecraft.getInstance().player.rotationYaw);
                    double d23 = f[1] - Minecraft.getInstance().player.rotationPitch;
                    if(Math.abs(d22) > 0)
                        this.xVelocity = d22;
                    if(Math.abs(d23) > 0)
                        this.yVelocity = d23;
