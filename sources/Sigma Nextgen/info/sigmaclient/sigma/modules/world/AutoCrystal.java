...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        super("AutoCrystal", Category.World, "Auto crystal");
     registerValue(range);
     registerValue(mcps);
     registerValue(cps);
        ...
        for(int x = -4; x <= 4; x ++)
            for(int y = -3; y <= 3; y ++)
                for(int z = -4; z <= 4; z ++){
                    ...
                    if(bb != Blocks.BEDROCK && bb != Blocks.OBSIDIAN){
                        continue;
                    }
                    if(!(bb2 instanceof AirBlock) || !(bb3 instanceof AirBlock)){
                        continue;
                    }
...
//                    if(r.getType() == RayTraceResult.Type.ENTITY)
//                        continue;
                    ...
                    Direction placeFace = calcFace(bp);
                    boolean canPlace = true;

                    if(!canPlace || placeFace == null) continue;

                    float d = damageFromPos(nn.add(0, 1.7, 0), target);
                    // no damage
                    if(d == 0) continue;
                    float d2 = damageFromPos(nn.add(0, 1.7, 0), mc.player);
...
//            RenderUtils.drawCircle(lastPos, new Color(-1));
    ...
    public int getCrystalSlot(){
        for(int i=0;i<9;i++){
            ...
            if(is.getItem() instanceof EnderCrystalItem || is.getTranslationKey().contains("crystal")){
                return i;
            }
        }
        return -1;
        ...
        if(event.isPre()){
            if(antiTrap.isEnable()){
                ...
                if (entityEnderCrystals.size() > 0) {
                    ...
                    event.yaw = calcRotation.getYaw();
                    event.pitch = calcRotation.getPitch();
                    if(entityEnderCrystal.getDistance(mc.player) <= gitRange.getValue().floatValue() + 0.7f) {
            ...
            if(lastPos2 != null) {
                ...
                if (entityEnderCrystals.size() > 0) {
                    EnderCrystalEntity entityEnderCrystal = entityEnderCrystals.get(0);
                    if(entityEnderCrystal.getDistance(mc.player) <= gitRange.getValue().floatValue()) {
                        Rotation calcRotation = RotationUtils.NCPRotation(entityEnderCrystal);
                        event.yaw = calcRotation.getYaw();
                        event.pitch = calcRotation.getPitch();
                        mc.playerController.attackEntity(mc.player, entityEnderCrystal);
            ...
            if(timerUtil.hasTimeElapsed(calc_cps)) {
                ...
                float[] calcRotation = scaffoldRots(
                        ...
                        180,
                        false);
                event.yaw = calcRotation[0];
                event.pitch = calcRotation[1];
    ...
    public class AutoTrap extends Module {
        ...
        public ScaffoldUtils.BlockCache getBlockCache(Entity e,BlockPos pos, int range) {
            ...
            d -= d2;
            bestDamage.add(d);
            damageMap.put(d, new AutoCrystal.SimpleCrystalPos(vec3d.add(-1, 0.5, 0), Direction.UP));
            ...
            d -= d2;
            bestDamage.add(d);
            damageMap.put(d, new AutoCrystal.SimpleCrystalPos(vec3d.add(0, 0.5, 1), Direction.UP));
