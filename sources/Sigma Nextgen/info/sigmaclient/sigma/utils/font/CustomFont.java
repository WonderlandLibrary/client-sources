...
public class CustomFont implements AbstractFontRenderer {
    ...
    public int drawCenteredString(String name, float x2, float y, Color color, boolean gradient){
        if(!gradient) {
            drawCenteredString(name, x2, y, color);
            return 0;
        }
        x2 -= this.getStringWidth(name) / 2f;
        float x = 0;
        for(int i = 0; i < name.length();i ++){
            String c = name.substring(i, i + 1);
            drawString(c, x2 + x, y, ColorChanger.getColor((int) (x * 2), 10).getRGB());
            x += getStringWidth(c);
        }
        return (int) x;
    }
    ...
    public int drawStringChroma(String name, float x2, float y){
        float x = 0;
        for(int i = 0; i < name.length();i ++){
            String c = name.substring(i, i + 1);
            drawString(c, x2 + x, y, ColorChanger.getColor((int) (x * 2), 10).getRGB());
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '§') {
                ...
                if (colorIndex < 16) {
                    ...
                    RenderUtils.color(colorCode[colorIndex], alpha);
                } else {
                    switch (colorIndex) {
                        case 17:
                            if (hasBoldFont()) {
                ...
                x += MathHelper.roundToHalf(currentData.chars[character].width - KERNING);
            }
        }
