...
public class ShaderUtil {
    ...
    public ShaderUtil(String fragmentShaderLoc, String vertexShaderLoc) {
        ...
        try {
            ...
            switch (fragmentShaderLoc) {
                case "jelloBlur":
                    fragmentShaderID = createShader(new ByteArrayInputStream(jelloBlur.getBytes()), GL_FRAGMENT_SHADER);
                    break;
                case "roundRect2":
                    fragmentShaderID = createShader(new ByteArrayInputStream(roundRect2.getBytes()), GL_FRAGMENT_SHADER);
                    break;
                case "gradientGlow":
                    fragmentShaderID = createShader(new ByteArrayInputStream(gradientGlow.getBytes()), GL_FRAGMENT_SHADER);
                    break;
                case "roundRect":
                    fragmentShaderID = createShader(new ByteArrayInputStream(roundRect.getBytes()), GL_FRAGMENT_SHADER);
                    break;
                case "kawaseUpGlow":
                ...
                case "kawaseUpBloom":
                    ...
                    break;
                case "outline":
                    fragmentShaderID = createShader(new ByteArrayInputStream(outlineShader.getBytes()), GL_FRAGMENT_SHADER);
                    break;
    ...
    public ShaderUtil(int ignoredProgramID) {
        int program = glCreateProgram();

        int vertexShaderID = createShader(new ByteArrayInputStream(kawaseUp.getBytes()), GL_VERTEX_SHADER);
        glAttachShader(program, vertexShaderID);
    ...
    public ShaderUtil(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "sigmang/shaders/vertex.vsh");
    }
    ...
    private String jelloBlur = "#version 110\n" +
            "\n" +
            ...
            "}\n";
    private String outlineShader = "#version 120\n" +
            "\n" +
            ...
            "\n" +
            "        innerColor += (colorCurrent1 + colorCurrent2);\n" +
            "    }\n" +
            "\n" +
...
//            "}\n";
    ...
    private String kawaseUp = "#version 120\n" +
            ...
            "    vec4 blurred = vec4(0.0);\n" +
            "    for(float r = -Radius; r <= Radius; r += 1.0) {\n" +
            "       blurred += texture2D(inTexture, gl_TexCoord[0].st + vec2(0, halfpixel.y) * r);\n" +
            "       blurred += texture2D(inTexture, gl_TexCoord[0].st + vec2(-halfpixel.x, 0) * r);\n" +
            "   }\n" +
            "   gl_FragColor = vec4(blurred.rgb / (Radius * 2.0 * 2.0 + 1.0), mix(1.0, texture2D(textureToCheck, gl_TexCoord[0].st).a, check));\n" +
            "}\n";
    ...
    private String kawaseDown = "#version 120\n" +
            ...
            "    vec2 uv = vec2(gl_FragCoord.xy / iResolution);\n" +
            "    vec4 sum = texture2D(inTexture, gl_TexCoord[0].st);\n" +
            "    sum += texture2D(inTexture, uv);\n" +
            "    gl_FragColor = vec4(sum.rgb * .5, 1.0);\n" +
//            "    gl_FragColor = texture2D(inTexture, gl_TexCoord[0].st);\n" +
    ...
    private String gradientMask = "#version 120\n" +
            ...
            "    float texColorAlpha = texture2D(tex, gl_TexCoord[0].st).a * alpha;\n" +
            "    gl_FragColor = vec4(createGradient(coords, color1, color2, color3, color4), texColorAlpha * alpha);\n" +
            "}";

    private String gradientGlow = "#version 120\n" +
            "\n" +
            "uniform vec2 location, rectSize;\n" +
            "uniform sampler2D tex;\n" +
            "uniform vec3 color1, color2, color3, color4;\n" +
            ...
            "void main() {\n" +
            "    vec2 coords = (gl_FragCoord.xy - location) / rectSize;\n" +
            "    float distance = roundedBoxSDF(gl_FragCoord.xy - location - (rectSize / 2.0), rectSize / 2.0, radius);\n" +
            "    float smoothedAlpha = (1.0 - smoothstep(-soft, soft, distance));\n" +
            "    gl_FragColor = vec4(createGradient(coords, color1, color2, color3, color4), smoothedAlpha);\n" +
            "}";

    private String roundRect = "#version 120\n" +
            "\n" +
            "uniform vec2 location, rectSize;\n" +
            "uniform sampler2D tex;\n" +
            "uniform vec3 color1;\n" +
            ...
            "void main() {\n" +
            "    vec2 coords = (gl_FragCoord.xy - location) / rectSize;\n" +
            "    float distance = roundedBoxSDF(gl_FragCoord.xy - location - (rectSize / 2.0), rectSize / 2.0, radius2);\n" +
            "    float smoothedAlpha = 1 - smoothstep(-1.0, 1.0, distance);\n" +
            "    gl_FragColor = vec4(color1, smoothedAlpha * alpha);\n" +
            "}";

    private String roundRect2 = "#version 120\n" +
            "\n" +
            "uniform vec2 location, rectSize;\n" +
            "uniform sampler2D tex;\n" +
            "uniform vec3 color1, color2, color3, color4;\n" +
            ...
            "    float distance = roundedBoxSDF(gl_FragCoord.xy - location - (rectSize / 2.0), rectSize / 2.0, radius2);\n" +
            "    float smoothedAlpha = 1 - smoothstep(-1.0, 1.0, distance);\n" +
            "    gl_FragColor = vec4(createGradient(coords, color1, color2, color3, color4), smoothedAlpha * alpha);\n" +
            "}";
