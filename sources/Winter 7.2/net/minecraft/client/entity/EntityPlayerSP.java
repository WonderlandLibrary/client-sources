...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
      UpdateEvent event = (UpdateEvent)EventSystem.call(new UpdateEvent(this.posY, this.rotationYaw, this.rotationPitch, this.onGround, true));
      if (!event.isCancelled())
      {
        ...
        if (func_175160_A())
        {
          ...
          {
            if ((var13) && (var14)) {
              this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, event.getY(), this.posZ, event.getYaw(), event.getPitch(), event.ground()));
            } else if (var13) {
              this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, event.getY(), this.posZ, event.ground()));
            } else if (var14) {
              this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event.getYaw(), event.getPitch(), event.ground()));
            } else {
              this.sendQueue.addToSendQueue(new C03PacketPlayer(event.ground()));
            }
          }
          else
          {
            this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, event.getYaw(), event.getPitch(), event.ground()));
            var13 = false;
          }
      ...
      EventSystem.call(new UpdateEvent(this.posY, this.rotationYaw, this.rotationPitch, this.onGround, false));
      if (Hud.cur != null) {
        Hud.cur.transition();
      }
    }
    ...
    {
        if (this.noClip && Client.isEnabled("Phase") && Client.isEnabled("Freecam"))
        {
            return false;
        }
        else
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !Client.getManager().getMod("NoSlowdown").isEnabled())
        {
    ...
    {
      MoveEvent event = (MoveEvent)EventSystem.call(new MoveEvent(x, y, z));
      if (event.isCancelled()) {
        return;
      }
