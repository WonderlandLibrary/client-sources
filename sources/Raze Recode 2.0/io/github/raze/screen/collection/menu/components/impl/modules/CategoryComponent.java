...
public class CategoryComponent extends BaseComponent {
    ...
    public CategoryComponent(ModuleCategory category, double x, double y, double width, double height) {
        ...
        List<AbstractModule> modules = Raze.INSTANCE.managerRegistry.moduleRegistry.getModulesByCategory(category);

        for (int index = 0; index < modules.size(); index += 1) {
            AbstractModule module = modules.get(index);
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        CFontUtil.Jello_Light_24.getRenderer().drawString(
                category.getName(),
                getX() + getHeight() / 2 - CFontUtil.Jello_Medium_24.getRenderer().getHeight() / 2.0D,
                getY() + getHeight() / 2 - CFontUtil.Jello_Medium_24.getRenderer().getHeight() / 2.0D,
