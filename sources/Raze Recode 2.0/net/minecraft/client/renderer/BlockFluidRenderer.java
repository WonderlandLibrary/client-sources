...
public class BlockFluidRenderer
{
    ...
    public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPosition blockPositionIn, WorldRenderer worldRendererIn)
    {
        boolean flag2;
        ...
        try
        {
            if (Config.isShaders())
            {
                SVertexBuilder.pushEntity(blockStateIn, blockPositionIn, blockAccess, worldRendererIn);
            }

            BlockLiquid blockliquid = (BlockLiquid)blockStateIn.getBlock();
            blockliquid.setBlockBoundsBasedOnState(blockAccess, blockPositionIn);
            ...
            int i = CustomColors.getFluidColor(blockAccess, blockStateIn, blockPositionIn, renderenv);
            float f = (float)(i >> 16 & 255) / 255.0F;
            float f1 = (float)(i >> 8 & 255) / 255.0F;
            float f2 = (float)(i & 255) / 255.0F;
            boolean flag = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.up(), EnumFacing.UP);
            boolean flag1 = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.down(), EnumFacing.DOWN);
            boolean[] aboolean = renderenv.getBorderFlags();
            aboolean[0] = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.north(), EnumFacing.NORTH);
            aboolean[1] = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.south(), EnumFacing.SOUTH);
            aboolean[2] = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.west(), EnumFacing.WEST);
            aboolean[3] = blockliquid.shouldSideBeRendered(blockAccess, blockPositionIn.east(), EnumFacing.EAST);
            ...
            if (flag || flag1 || aboolean[0] || aboolean[1] || aboolean[2] || aboolean[3])
            {
                ...
                float f7 = this.getFluidHeight(blockAccess, blockPositionIn, material);
                float f8 = this.getFluidHeight(blockAccess, blockPositionIn.south(), material);
                float f9 = this.getFluidHeight(blockAccess, blockPositionIn.east().south(), material);
                float f10 = this.getFluidHeight(blockAccess, blockPositionIn.east(), material);
                double d0 = (double) blockPositionIn.getX();
                double d1 = (double) blockPositionIn.getY();
                double d2 = (double) blockPositionIn.getZ();
                float f11 = 0.001F;
                ...
                if (flag)
                {
                    ...
                    float f12 = (float)BlockLiquid.getFlowDirection(blockAccess, blockPositionIn, material);
                    ...
                    int k2 = blockliquid.getMixedBrightnessForBlock(blockAccess, blockPositionIn);
                    int l2 = k2 >> 16 & 65535;
                    int i3 = k2 & 65535;
                    float f24 = f4 * f;
                    float f25 = f4 * f1;
                    ...
                    if (blockliquid.shouldRenderSides(blockAccess, blockPositionIn.up()))
                    {
                        worldRendererIn.pos(d0 + 0.0D, d1 + (double)f7, d2 + 0.0D).color(f24, f25, f26, 1.0F).tex((double)f13, (double)f17).lightmap(l2, i3).endVertex();
                        worldRendererIn.pos(d0 + 1.0D, d1 + (double)f10, d2 + 0.0D).color(f24, f25, f26, 1.0F).tex((double)f16, (double)f20).lightmap(l2, i3).endVertex();
                        worldRendererIn.pos(d0 + 1.0D, d1 + (double)f9, d2 + 1.0D).color(f24, f25, f26, 1.0F).tex((double)f15, (double)f19).lightmap(l2, i3).endVertex();
                ...
                if (flag1)
                {
                    ...
                    int l1 = blockliquid.getMixedBrightnessForBlock(blockAccess, blockPositionIn.down());
                    int i2 = l1 >> 16 & 65535;
                    int j2 = l1 & 65535;
                ...
                for (int i1 = 0; i1 < 4; ++i1)
                {
                    ...
                    BlockPosition blockpos = blockPositionIn.add(j1, 0, k1);
    ...
    private float getFluidHeight(IBlockAccess blockAccess, BlockPosition blockPositionIn, Material blockMaterial)
    {
        int i = 0;
        float f = 0.0F;
        ...
        for (int j = 0; j < 4; ++j)
        {
            BlockPosition blockpos = blockPositionIn.add(-(j & 1), 0, -(j >> 1 & 1));
