...
import io.github.raze.modules.collection.visual.Animations;

import io.github.raze.screen.collection.main.UIMainMenu;
...
import io.github.raze.version.fixes.AttackOrder;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public static InputStream convertToInputStream(BufferedImage image) throws IOException {
    ...
    private void runGameLoop() throws IOException
    {
        ...
        ++this.fpsCounter;
        this.isGamePaused = this.isSingleplayer() && this.currentScreen != null && this.currentScreen.pausegame() && !this.theIntegratedServer.getPublic();
    ...
    private void displayDebugInfo(long elapsedTicksTime)
    {
        if (this.mcProfiler.profilingEnabled)
        {
            ...
            for (int k2 = 0; k2 < list.size(); ++k2)
            {
                ...
                this.fontRenderer.drawStringWithShadow(s1, (float)(j - i), (float)(k + i / 2 + k2 * 8 + 20), profiler$result2.getColor());
                this.fontRenderer.drawStringWithShadow(s1 = decimalformat.format(profiler$result2.field_76332_a) + "%", (float)(j + i - 50 - this.fontRenderer.getStringWidth(s1)), (float)(k + i / 2 + k2 * 8 + 20), profiler$result2.getColor());
                this.fontRenderer.drawStringWithShadow(s1 = decimalformat.format(profiler$result2.field_76330_b) + "%", (float)(j + i - this.fontRenderer.getStringWidth(s1)), (float)(k + i / 2 + k2 * 8 + 20), profiler$result2.getColor());
            }
        }
    }
    ...
    private void sendClickBlockToController(boolean leftClick)
    {
        ...
        if(Raze.INSTANCE.managerRegistry.moduleRegistry.getByClass(Animations.class).isEnabled()){
            if (this.leftClickCounter <= 0)
            {
                if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    BlockPosition blockpos = this.objectMouseOver.getBlockPosition();
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    if (keyCode == 62 && this.entityRenderer != null)
                    {
                    ...
                    else
                    {
                        ...
                        EventKeyboard eventKeyboard = new EventKeyboard(keyCode);
                        eventKeyboard.call();

                        if (eventKeyboard.isCancelled()) {
                            return;
                        }
