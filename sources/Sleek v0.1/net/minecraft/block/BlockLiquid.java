...
public abstract class BlockLiquid extends Block {
   ...
   protected int getLevel(IBlockAccess worldIn, BlockPos pos) {
      return worldIn.getBlockState(pos).getBlock().getMaterial() == this.blockMaterial ? (Integer)worldIn.getBlockState(pos).getValue(LEVEL) : -1;
   }
   ...
   protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos) {
      ...
      BlockPos blockpos1;
      while(var5.hasNext()) {
         enumfacing1 = (EnumFacing)var5.next();
         blockpos1 = pos.offset(enumfacing1);
      ...
      if ((Integer)worldIn.getBlockState(pos).getValue(LEVEL) >= 8) {
         ...
         do {
            ...
            enumfacing1 = (EnumFacing)var5.next();
            blockpos1 = pos.offset(enumfacing1);
         } while(!this.isBlockSolid(worldIn, blockpos1, enumfacing1) && !this.isBlockSolid(worldIn, blockpos1.up(), enumfacing1));
   ...
   public static double getFlowDirection(IBlockAccess worldIn, BlockPos pos, Material materialIn) {
      ...
      return vec3.xCoord == 0.0D && vec3.zCoord == 0.0D ? -1000.0D : MathHelper.func_181159_b(vec3.zCoord, vec3.xCoord) - 1.5707963267948966D;
   }
