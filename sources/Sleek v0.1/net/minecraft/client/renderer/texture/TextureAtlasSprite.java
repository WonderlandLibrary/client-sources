...
public class TextureAtlasSprite {
   ...
   public void loadSprite(BufferedImage[] images, AnimationMetadataSection meta) throws IOException {
      ...
      int k1;
      for(k1 = 0; k1 < images.length; ++k1) {
         BufferedImage bufferedimage = images[k1];
         if (bufferedimage != null) {
            if (this.width >> k1 != bufferedimage.getWidth()) {
               bufferedimage = TextureUtils.scaleImage(bufferedimage, this.width >> k1);
            }

            if (k1 > 0 && (bufferedimage.getWidth() != i >> k1 || bufferedimage.getHeight() != j >> k1)) {
               throw new RuntimeException(String.format("Unable to load miplevel: %d, image is size: %dx%d, expected %dx%d", k1, bufferedimage.getWidth(), bufferedimage.getHeight(), i >> k1, j >> k1));
            }

            aint[k1] = new int[bufferedimage.getWidth() * bufferedimage.getHeight()];
            bufferedimage.getRGB(0, 0, bufferedimage.getWidth(), bufferedimage.getHeight(), aint[k1], 0, bufferedimage.getWidth());
         }
      }
      ...
      if (meta == null) {
         ...
         if (meta.getFrameCount() > 0) {
            ...
            while(iterator.hasNext()) {
               ...
               this.framesTextureData.set(i1, getFrameTextureData(aint, l1, i2, i1));
            }
            ...
            for(i1 = 0; i1 < k1; ++i1) {
               this.framesTextureData.add(getFrameTextureData(aint, l1, i2, i1));
      ...
      if (!this.isShadersSprite) {
         ...
         for(k1 = 0; k1 < this.framesTextureData.size(); ++k1) {
