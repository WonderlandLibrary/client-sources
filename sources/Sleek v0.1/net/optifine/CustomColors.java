...
public class CustomColors {
   ...
   private static void addToList(CustomColormap cm, List lists, int id) {
      while(id >= lists.size()) {
         lists.add((Object)null);
      }

      List list = (List)lists.get(id);
      if (list == null) {
         list = new ArrayList();
         ((List)list).set(id, list);
      }
   ...
   public static int getFluidColor(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, RenderEnv renderEnv) {
      ...
      return customcolors$icolorizer == null ? block.colorMultiplier(blockAccess, blockPos, 0) : (Config.isSmoothBiomes() && !((CustomColors.IColorizer)customcolors$icolorizer).isColorConstant() ? getSmoothColorMultiplier(blockState, blockAccess, blockPos, (CustomColors.IColorizer)customcolors$icolorizer, renderEnv.getColorizerBlockPosM()) : ((CustomColors.IColorizer)customcolors$icolorizer).getColor(blockState, blockAccess, blockPos));
   }
