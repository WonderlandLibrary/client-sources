...
public class HoverEvent
{
    ...
    public static class Action<T>
    {
        ...
        public static final Action<ItemHover> SHOW_ITEM = new Action<>("show_item", true, (element) ->
        {
            return ItemHover.deserialize(element);
        }, (hover) ->
        {
            return hover.serialize();
        }, (component) ->
        {
            return ItemHover.deserialize(component);
        });
        public static final Action<EntityHover> SHOW_ENTITY = new Action<>("show_entity", true, EntityHover::deserialize, EntityHover::serialize, EntityHover::deserialize);
        private static final Map<String, Action> NAME_MAPPING = Stream.of(SHOW_TEXT, SHOW_ITEM, SHOW_ENTITY).collect(ImmutableMap.toImmutableMap(Action::getCanonicalName, (action) ->
        {
            return action;
        }));
    ...
    public static class ItemHover
    {
        ...
        private static ItemHover deserialize(JsonElement element)
        {
            ...
            {
                return new ItemHover(Registry.ITEM.getOrDefault(new ResourceLocation(element.getAsString())), 1, (CompoundNBT)null);
            }
            else
            {
                ...
                return new ItemHover(item, i, (CompoundNBT)null);
            }
        }
