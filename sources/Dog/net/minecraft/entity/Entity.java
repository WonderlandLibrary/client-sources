...
import fr.dog.module.impl.movement.SafeWalk;
...
public abstract class Entity implements ICommandSender {
    ...
    public void setAngles(float yaw, float pitch) {
        ...
        if(freelook.isEnabled()){
            float f = freelook.getRenderRotationPitch();
            float f1 = freelook.getRenderRotationYaw();
            freelook.setRenderRotationYaw((float) ((double) freelook.getRenderRotationYaw() + (double) yaw * 0.15D));
            freelook.setRenderRotationPitch((float) ((double) freelook.getRenderRotationPitch() - (double) pitch * 0.15D));
            freelook.setRenderRotationPitch(MathHelper.clamp_float(freelook.getRenderRotationPitch(), -90.0F, 90.0F));
            freelook.setLastRenderRotationPitch(freelook.getLastRenderRotationPitch() + freelook.getRenderRotationPitch() - f);
            freelook.setLastRenderRotationYaw(freelook.getLastRenderRotationYaw() + freelook.getRenderRotationYaw() - f1);
        }else{
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = (this.onGround && this.isSneaking() && this instanceof EntityPlayer) || (this instanceof EntityPlayer && Dog.getInstance().getModuleManager().getModule(SafeWalk.class).doSafeWalk());
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        float square = strafe * strafe + forward * forward;

        if (square >= 1.0E-4F) {
            square = MathHelper.sqrt_float(square);

            if (square < 1.0F) {
                square = 1.0F;
            }

            square = friction / square;
            strafe = strafe * square;
    ...
    public MovingObjectPosition rayTraceCustom(double blockReachDistance, float yaw, float pitch) {
        ...
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, true, true);
    }
