...
import fr.dog.event.impl.player.PlayerPostNetworkTickEvent;
import fr.dog.event.impl.player.move.MovementEvent;
import fr.dog.event.impl.player.move.SlowDownEvent;
import fr.dog.module.impl.movement.NoSlowdown;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            Dog.getInstance().getEventBus().post(new PlayerTickEvent());
            ...
            Dog.getInstance().getEventBus().post(new PlayerPostNetworkTickEvent());
        }
    }
    ...
    public Vec3 getLook(float partialTicks) {
        return !RotationComponent.active && !RotationComponent.smoothReset ? super.getLook(partialTicks) : this.getVectorForRotation(RotationComponent.rotationPitch, RotationComponent.rotationYaw);
    }
    ...
    public void onUpdateWalkingPlayer() {
        ...
        final PlayerNetworkTickEvent event = new PlayerNetworkTickEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
        Dog.getInstance().getEventBus().post(event);
    ...
    public void sendChatMessage(String message) {
        final InputChatEvent chatEvent = new InputChatEvent(message);
        Dog.getInstance().getEventBus().post(chatEvent);

        if (chatEvent.isCancelled()) {
            return;
        }

        this.sendQueue.addToSendQueue(new C01PacketChatMessage(chatEvent.getMessage()));
    }
    ...
    public void onLivingUpdate() {
        ...
        {
            SlowDownEvent slowDown = new SlowDownEvent(0.2f, 0.2f);
            Dog.getInstance().getEventBus().post(slowDown);

            this.movementInput.moveStrafe *= slowDown.getStrafe();
            this.movementInput.moveForward *= slowDown.getForward();
        ...
        if(Dog.getInstance().getModuleManager().getModule(NoSlowdown.class).isEnabled()){
            isUsingItem = false;
        }
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && !isUsingItem && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }
