...
import fr.dog.module.impl.render.Animation;
...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            Animation animation = Dog.getInstance().getModuleManager().getModule(Animation.class);

            float f1 = abstractclientplayer.getSwingProgress(partialTicks);

            if(animation.isEnabled()){
                f1 = abstractclientplayer.getSwingProgress(partialTicks) * Animation.animationSpeed.getValue();
            }
            ...
            if (this.itemToRender != null) {
                ...
                } else if (abstractclientplayer.getItemInUseCount() > 0 || (KillAuraModule.shouldBlockRender && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)) {
                    EnumAction enumaction = this.itemToRender.getItemUseAction();
                    float f15 = MathHelper.sin(f1 * f1 * 3.1415927F);
                        ...
                        case BLOCK:
                            ...
                            switch (Animation.mode.getValue()) {
                                ...
                                case "Smooth":
                                    ...
                                    GlStateManager.rotate(f15 * 30.0F / 2.0F, -f15, -0.0F, 9.0F);
                                    GlStateManager.rotate(f15 * 40.0F, 1.0F, -f15 / 2.0F, -0.0F);
                                    this.doBlockTransformations();
                                ...
                                case "Exhi":
                                    ...
                                    GlStateManager.rotate(-funny * 12.0f, 1.2f, 0, 9.0F);
                                    GlStateManager.rotate(-funny * 29.0F, 1.2F, (funny / 0.8f), 0F);
                                    this.doBlockTransformations();
                                    break;
                                case "Spin":
                                    this.transformFirstPersonItem(f, 0.0f);

                                    float time = (float) (System.currentTimeMillis() % 1000L) / 40f;
                                    GlStateManager.translate(0.0F, 0.3F, -0.0F);
                                    GlStateManager.rotate(time * 29.0F, 1, 0, 2.0F);
                                    break;
                                case "Spin2":
                                    this.transformFirstPersonItem(f, 0.0f);

                                    float time2 = (float) (System.currentTimeMillis() % 1000L) / 40f;
                                    GlStateManager.translate(0.0F, 0.15F, -0.0F);
                                    GlStateManager.rotate(-time2 * 29.0F, -0.5f, 0.6f, 4.0F);
                                    GlStateManager.rotate(time2 * 29.0F, 0.0f, -0.6f, 4.0F);
                                    break;
                            }
                            break;
    ...
    private void renderFireInFirstPerson(float partialTicks) {
        ...
        if (Dog.getInstance().getModuleManager().getModule(Camera.class).nofire.getValue()) {
            return;
        }
