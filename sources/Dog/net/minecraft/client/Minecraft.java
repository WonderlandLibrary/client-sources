...
import dev.liticane.transpiler.NotNative;
import fr.dog.Dog;
import fr.dog.command.impl.KeyConfigCommand;
import fr.dog.event.impl.player.interact.LegitClickEvent;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    @NotNative
    public void run(String uid, String additionalFeatures) {
        this.running = true;

        try {
            this.startGame(uid, additionalFeatures);
    ...
    private void startGame(String uid, String additionalFeatures) throws LWJGLException {
        ...
        try {
            ProtectedLaunch.init(uid,additionalFeatures);
        } catch (Throwable t) {
            System.out.println("Client Integrity Issue 0x01");
        ...
        if(System.getProperty("os.name").toLowerCase().contains("win")) {
            try {
                String wallpaperPath = Advapi32Util.registryGetStringValue(
                        WinReg.HKEY_CURRENT_USER,
                        "Control Panel\\Desktop",
                        "WallPaper"
                );
                ImageManager.menuImages.replace("wallpaper", new ImageObject(new File(wallpaperPath)));
            } catch (Exception ignored) { /* */ }
        }

        ImageManager.loadMenuImages();
        ImageManager.loadInGameImages();
    ...
    //uncaped to 120 so idk if it changes for the obf
    @NotNative
    ...
    private void displayDebugInfo() {
        if (this.mcProfiler.profilingEnabled) {
            ...
            List<Profiler.Result> list = this.mcProfiler.getProfilingData(debugProfilerName);
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            for(Module m : Dog.getInstance().getModuleManager().getObjects()) {
                if (m.getOnlyOnKeyHold().getValue() && m.getCategory() == ModuleCategory.RENDER) {
                    m.setEnabled(Keyboard.isKeyDown(m.getKeyBind()));
                }
            }
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        Dog.getInstance().getCommandManager().getCommand(KeyConfigCommand.class).configKeybind.onKeyPressed(k);
                        //Dog.getInstance().getModuleManager().getObjects().stream().filter(module -> module.getKeyBind() == k).forEach(Module::toggle);
            ...
            Dog.getInstance().getEventBus().post(new LegitClickEvent());
        ...
        if (theWorld != null && thePlayer != null)
            Dog.getInstance().getEventBus().post(new TickEvent());
    ...
    public static ScaledResolution getScaledResolution() {
        return new ScaledResolution(theMinecraft);
    }
