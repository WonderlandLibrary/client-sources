...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        HUD hud = Dog.getInstance().getModuleManager().getModule(HUD.class);
        if(hud.blur.getValue())
            RenderUtil.drawBlur(this::drawBlur);

        Dog.getInstance().getEventBus().post(new Render2DEvent(partialTicks));
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);

        if(hud.glow.getValue())
    ...
    private void drawGlow(){
        Dog.getInstance().getEventBus().post(new RenderGlow());
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    }

    private void drawBlur(){
        Dog.getInstance().getEventBus().post(new RenderBlur());
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            if (HUD.scoreboardOffset > sr.getScaledHeight() / 3.5)
                k += HUD.scoreboardOffset - sr.getScaledHeight() / 3.5;
