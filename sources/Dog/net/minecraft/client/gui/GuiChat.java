...
public class GuiChat extends GuiScreen {
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton == 0) {
            ...
            for (Module m : Dog.getInstance().getModuleManager().getObjects()) {
                if (m.isDraggable() && m.isHovered() && m.isEnabled()) {
                    m.setDragged(true, MouseUtil.getMouseX(), MouseUtil.getMouseY());
                    break;
                }
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        //configWin.mouseReleased();
        for (Module m : Dog.getInstance().getModuleManager().getObjects()) {
            if (m.isDraggable()) m.setDragged(false);
        }
    }
