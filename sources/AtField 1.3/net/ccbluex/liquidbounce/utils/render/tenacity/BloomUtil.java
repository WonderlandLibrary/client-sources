...
public class BloomUtil {
    ...
    public static void bloom(RenderCallback renderCallback) {
        if (disableBloom) {
            return;
        }
        renders.add(renderCallback);
    }

    public static void onRenderGameOverlay(ScaledResolution scaledResolution, Framebuffer framebuffer) {
        if (BloomUtil.framebuffer == null) {
            return;
        }
        BloomUtil.framebuffer.func_147610_a(false);
        for (RenderCallback renderCallback : renders) {
        ...
        BloomUtil.framebuffer.func_147614_f();
        framebuffer.func_147610_a(false);
    }

    public static void onFrameBufferResize(int n, int n2) {
        if (framebuffer != null) {
            framebuffer.func_147608_a();
        }
        framebuffer = new Framebuffer(n, n2, false);
    }

    public static void drawAndBloom(RenderCallback renderCallback) {
        renderCallback.render();
        if (disableBloom) {
            return;
        }
