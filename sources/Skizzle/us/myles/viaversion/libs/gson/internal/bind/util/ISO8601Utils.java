...
public class ISO8601Utils {
    ...
    public static String format(Date date, boolean millis, TimeZone tz) {
        ...
        int capacity = 19;
        capacity += millis ? 4 : 0;
        StringBuilder formatted = new StringBuilder(capacity += tz.getRawOffset() == 0 ? 1 : 6);
        ISO8601Utils.padInt(formatted, calendar.get(1), 4);
        formatted.append('-');
        ISO8601Utils.padInt(formatted, calendar.get(2) + 1, 2);
        formatted.append('-');
        ISO8601Utils.padInt(formatted, calendar.get(5), 2);
        formatted.append('T');
        ISO8601Utils.padInt(formatted, calendar.get(11), 2);
        formatted.append(':');
        ISO8601Utils.padInt(formatted, calendar.get(12), 2);
        formatted.append(':');
        ISO8601Utils.padInt(formatted, calendar.get(13), 2);
        if (millis) {
            formatted.append('.');
            ISO8601Utils.padInt(formatted, calendar.get(14), 3);
        }
        if ((offset = tz.getOffset(calendar.getTimeInMillis())) != 0) {
            int hours = Math.abs(offset / 60000 / 60);
            int minutes = Math.abs(offset / 60000 % 60);
            formatted.append(offset < 0 ? (char)'-' : '+');
            ISO8601Utils.padInt(formatted, hours, 2);
            formatted.append(':');
            ISO8601Utils.padInt(formatted, minutes, 2);
        } else {
            formatted.append('Z');
        }
