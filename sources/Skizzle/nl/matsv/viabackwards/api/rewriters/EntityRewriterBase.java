...
extends Rewriter<T> {
    private final Map<EntityType, EntityData> entityTypes = new HashMap<EntityType, EntityData>();
    private final List<MetaHandlerSettings> metaHandlers = new ArrayList<MetaHandlerSettings>();
    ...
    protected EntityData mapEntity(EntityType oldType, EntityType replacement) {
        Preconditions.checkArgument((oldType.getClass() == replacement.getClass() ? 1 : 0) != 0);
        int mappedReplacementId = this.getOldEntityId(replacement.getId());
    ...
    public void mapEntityDirect(EntityType oldType, EntityType newType) {
        Preconditions.checkArgument((oldType.getClass() != newType.getClass() ? 1 : 0) != 0);
    ...
    protected MetaStorage handleMeta(UserConnection user, int entityId, MetaStorage storage) throws Exception {
        ...
        EntityTracker.StoredEntity storedEntity = this.getEntityTracker(user).getEntity(entityId);
        ...
        for (MetaHandlerSettings settings : this.metaHandlers) {
            ArrayList<Metadata> newData = new ArrayList<Metadata>();
            for (Metadata meta : storage.getMetaDataList()) {
                ...
                try {
                    ...
                    if (event == null || event.getExtraData() == null) continue;
                    newData.addAll(event.getExtraData());
                }
                catch (Exception e) {
                    Logger log = ViaBackwards.getPlatform().getLogger();
                    log.warning("Unable to handle metadata " + meta + " for entity type " + type);
                    log.warning(storage.getMetaDataList().stream().sorted(Comparator.comparingInt(Metadata::getId)).map(Metadata::toString).collect(Collectors.joining("\n", "Full metadata list: ", "")));
                    e.printStackTrace();
                }
            }
        ...
        if (data != null && (entityData = this.getEntityData(type)) != null && entityData.getMobName() != null && (data.getValue() == null || data.getValue().toString().isEmpty()) && data.getMetaType().getTypeID() == this.displayNameMetaType.getTypeID()) {
            ...
            if (ViaBackwards.getConfig().alwaysShowOriginalMobName()) {
                ...
                storage.add(new Metadata(this.displayVisibilityIndex, this.displayVisibilityMetaType, true));
            }
        }
        return storage;
    }
    ...
    protected void registerExtraTracker(ClientboundPacketType packetType, final EntityType entityType, final Type intType) {
        ((Protocol)this.getProtocol()).registerOutgoing((ClientboundPacketType)packetType, new PacketRemapper(){
            ...
            public void registerMap() {
                this.map(intType);
                this.handler(wrapper -> EntityRewriterBase.this.addTrackedEntity(wrapper, (Integer)wrapper.get(intType, 0), entityType));
            }
        });
    }

    protected void registerExtraTracker(ClientboundPacketType packetType, EntityType entityType) {
        this.registerExtraTracker(packetType, entityType, Type.VAR_INT);
    }
    ...
    protected PacketHandler getTrackerHandler(Type intType, int typeIndex) {
        return wrapper -> {
            ...
            this.addTrackedEntity(wrapper, wrapper.get(Type.VAR_INT, 0), this.getTypeFromId(id.intValue()));
        };
    }
    ...
    protected PacketHandler getTrackerHandler(EntityType entityType, Type intType) {
        return wrapper -> this.addTrackedEntity(wrapper, (Integer)wrapper.get(intType, 0), entityType);
    }
    ...
    public EntityTracker.ProtocolEntityTracker getEntityTracker(UserConnection user) {
        return user.get(EntityTracker.class).get((BackwardsProtocol)this.getProtocol());
    }
