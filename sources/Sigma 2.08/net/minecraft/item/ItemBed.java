		...
		if (worldIn.isRemote) {
			...
			if (playerIn.func_175151_a(pos, side, stack) && playerIn.func_175151_a(var14, side, stack)) {
				if (var16 && var17 && World.doesBlockHaveSolidTopSurface(worldIn, pos.offsetDown()) && World.doesBlockHaveSolidTopSurface(worldIn, var14.offsetDown())) {
					...
					IBlockState var19 = Blocks.bed.getDefaultState().withProperty(BlockBed.OCCUPIED_PROP, Boolean.valueOf(false)).withProperty(BlockDirectional.AGE, var13).withProperty(BlockBed.PART_PROP, BlockBed.EnumPartType.FOOT);
