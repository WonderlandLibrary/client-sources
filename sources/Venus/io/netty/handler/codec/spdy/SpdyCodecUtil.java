...
final class SpdyCodecUtil {
    ...
    static int getUnsignedShort(ByteBuf byteBuf, int n) {
        return (byteBuf.getByte(n) & 0xFF) << 8 | byteBuf.getByte(n + 1) & 0xFF;
    }

    static int getUnsignedMedium(ByteBuf byteBuf, int n) {
        return (byteBuf.getByte(n) & 0xFF) << 16 | (byteBuf.getByte(n + 1) & 0xFF) << 8 | byteBuf.getByte(n + 2) & 0xFF;
    }

    static int getUnsignedInt(ByteBuf byteBuf, int n) {
        return (byteBuf.getByte(n) & 0x7F) << 24 | (byteBuf.getByte(n + 1) & 0xFF) << 16 | (byteBuf.getByte(n + 2) & 0xFF) << 8 | byteBuf.getByte(n + 3) & 0xFF;
    }

    static int getSignedInt(ByteBuf byteBuf, int n) {
        return (byteBuf.getByte(n) & 0xFF) << 24 | (byteBuf.getByte(n + 1) & 0xFF) << 16 | (byteBuf.getByte(n + 2) & 0xFF) << 8 | byteBuf.getByte(n + 3) & 0xFF;
    }

    static boolean isServerId(int n) {
        return n % 2 == 0;
    }
    ...
    static void validateHeaderValue(CharSequence charSequence) {
        ...
        for (int i = 0; i < charSequence.length(); ++i) {
            char c = charSequence.charAt(i);
            if (c != '\u0000') continue;
            throw new IllegalArgumentException("value contains null character: " + charSequence);
        }
    }
}
