...
public final class BiomeMappings {
    ...
    private static void add(int n, String string) {
        LEGACY_BIOMES.put(string, n);
    }

    public static int toLegacyBiome(String string) {
        int n = MODERN_TO_LEGACY_ID.getInt(Key.stripMinecraftNamespace(string));
        if (n == -1) {
            if (!Via.getConfig().isSuppressConversionWarnings()) {
                ViaBackwards.getPlatform().getLogger().warning("Biome with id " + string + " has no legacy biome mapping (custom datapack?)");
            }
            return 0;
        }
        return n;
    ...
    static {
        ...
        for (Object2IntMap.Entry object : LEGACY_BIOMES.object2IntEntrySet()) {
            MODERN_TO_LEGACY_ID.put((String)object.getKey(), object.getIntValue());
        }
        JsonObject jsonObject = VBMappingDataLoader.loadFromDataDir("biome-mappings.json");
        for (Map.Entry<String, JsonElement> entry : jsonObject.entrySet()) {
            int n = LEGACY_BIOMES.getInt(entry.getValue().getAsString());
            if (n == -1) {
                ViaBackwards.getPlatform().getLogger().warning("Unknown legacy biome: " + entry.getValue().getAsString());
                continue;
            }
            MODERN_TO_LEGACY_ID.put(entry.getKey(), n);
        }
    }
}
