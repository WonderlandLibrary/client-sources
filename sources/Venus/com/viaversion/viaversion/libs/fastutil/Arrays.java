...
public class Arrays {
    ...
    static final boolean $assertionsDisabled = !Arrays.class.desiredAssertionStatus();
    ...
    public static void ensureOffsetLength(int n, int n2, int n3) {
        ...
        if (n3 > n - n2) {
            throw new ArrayIndexOutOfBoundsException("Last index (" + ((long)n2 + (long)n3) + ") is greater than array length (" + n + ")");
        }
    }
    ...
    public static void parallelQuickSort(int n, int n2, IntComparator intComparator, Swapper swapper) {
        ForkJoinPool forkJoinPool = Arrays.getPool();
        if (n2 - n < 8192 || forkJoinPool.getParallelism() == 1) {
            Arrays.quickSort(n, n2, intComparator, swapper);
        } else {
