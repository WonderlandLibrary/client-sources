...
Hash {
    ...
    public IntLinkedOpenHashSet(int n, float f) {
        if (f <= 0.0f || f >= 1.0f) {
    ...
    public IntLinkedOpenHashSet(Iterator<?> iterator2, float f) {
        this(IntIterators.asIntIterator(iterator2), f);
    }

    public IntLinkedOpenHashSet(Iterator<?> iterator2) {
        this(IntIterators.asIntIterator(iterator2));
    }

    public IntLinkedOpenHashSet(int[] nArray, int n, int n2, float f) {
        this(n2 < 0 ? 0 : n2, f);
    ...
    public IntLinkedOpenHashSet(int[] nArray, int n, int n2) {
        this(nArray, n, n2, 0.75f);
    }

    public IntLinkedOpenHashSet(int[] nArray, float f) {
        this(nArray, 0, nArray.length, f);
    }

    public IntLinkedOpenHashSet(int[] nArray) {
        this(nArray, 0.75f);
    }
    ...
    public static IntLinkedOpenHashSet of(int n) {
        IntLinkedOpenHashSet intLinkedOpenHashSet = new IntLinkedOpenHashSet(1, 0.75f);
        intLinkedOpenHashSet.add(n);
        return intLinkedOpenHashSet;
    }

    public static IntLinkedOpenHashSet of(int n, int n2) {
        IntLinkedOpenHashSet intLinkedOpenHashSet = new IntLinkedOpenHashSet(2, 0.75f);
        intLinkedOpenHashSet.add(n);
        if (!intLinkedOpenHashSet.add(n2)) {
            throw new IllegalArgumentException("Duplicate element: " + n2);
        }
        return intLinkedOpenHashSet;
    }

    public static IntLinkedOpenHashSet of(int n, int n2, int n3) {
        IntLinkedOpenHashSet intLinkedOpenHashSet = new IntLinkedOpenHashSet(3, 0.75f);
        intLinkedOpenHashSet.add(n);
        if (!intLinkedOpenHashSet.add(n2)) {
            throw new IllegalArgumentException("Duplicate element: " + n2);
        }
        if (!intLinkedOpenHashSet.add(n3)) {
            throw new IllegalArgumentException("Duplicate element: " + n3);
        }
        return intLinkedOpenHashSet;
    }

    public static IntLinkedOpenHashSet of(int ... nArray) {
        IntLinkedOpenHashSet intLinkedOpenHashSet = new IntLinkedOpenHashSet(nArray.length, 0.75f);
        for (int n : nArray) {
            if (intLinkedOpenHashSet.add(n)) continue;
    ...
    public static IntLinkedOpenHashSet toSetWithExpectedSize(IntStream intStream, int n) {
        if (n <= 16) {
            return IntLinkedOpenHashSet.toSet(intStream);
        }
        return intStream.collect(new IntCollections.SizeDecreasingSupplier<IntLinkedOpenHashSet>(n, IntLinkedOpenHashSet::lambda$toSetWithExpectedSize$0), IntLinkedOpenHashSet::add, IntLinkedOpenHashSet::addAll);
    }
    ...
    public void forEach(IntConsumer intConsumer) {
        ...
        while (n != -1) {
            int n2 = n;
            n = (int)this.link[n2];
            intConsumer.accept(this.key[n2]);
        }
    }
    ...
    private static IntLinkedOpenHashSet lambda$toSetWithExpectedSize$0(int n) {
    ...
    implements IntListIterator {
        ...
        SetIterator(IntLinkedOpenHashSet intLinkedOpenHashSet) {
            this.this$0 = intLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            this.index = -1;
            this.next = intLinkedOpenHashSet.first;
            this.index = 0;
        }

        SetIterator(IntLinkedOpenHashSet intLinkedOpenHashSet, int n) {
            this.this$0 = intLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            ...
            if (n == 0) {
                if (intLinkedOpenHashSet.containsNull) {
                    this.next = (int)intLinkedOpenHashSet.link[intLinkedOpenHashSet.n];
                    this.prev = intLinkedOpenHashSet.n;
                    return;
                }
                throw new NoSuchElementException("The key " + n + " does not belong to this set.");
            }
            if (intLinkedOpenHashSet.key[intLinkedOpenHashSet.last] == n) {
                this.prev = intLinkedOpenHashSet.last;
                this.index = intLinkedOpenHashSet.size;
                return;
            }
            int[] nArray = intLinkedOpenHashSet.key;
            int n2 = HashCommon.mix(n) & intLinkedOpenHashSet.mask;
            while (nArray[n2] != 0) {
                if (nArray[n2] == n) {
                    this.next = (int)intLinkedOpenHashSet.link[n2];
                    this.prev = n2;
                    return;
                }
                n2 = n2 + 1 & intLinkedOpenHashSet.mask;
            }
            throw new NoSuchElementException("The key " + n + " does not belong to this set.");
        }
        ...
        public void forEachRemaining(IntConsumer intConsumer) {
            ...
            long[] lArray = this.this$0.link;
            while (this.next != -1) {
                this.curr = this.next;
                this.next = (int)lArray[this.curr];
                this.prev = this.curr;
                if (this.index >= 0) {
                    ++this.index;
                }
                intConsumer.accept(nArray[this.curr]);
            }
        }
