...
public class Entity1_13Types {
    public static EntityType getTypeFromId(int n, boolean bl) {
        Optional<EntityType> optional = bl ? ObjectType.getPCEntity(n) : EntityType.findById(n);
        if (!optional.isPresent()) {
            Via.getPlatform().getLogger().severe("Could not find 1.13 type id " + n + " isObject=" + bl);
    ...
    public static enum ObjectType implements com.viaversion.viaversion.api.minecraft.entities.ObjectType
    {
        ...
        private ObjectType(int n2, EntityType entityType) {
            this.id = n2;
        ...
        public static Optional<ObjectType> findById(int n) {
            if (n == -1) {
                return Optional.empty();
            }
            return Optional.ofNullable(TYPES.get(n));
        }

        public static Optional<EntityType> getPCEntity(int n) {
            Optional<ObjectType> optional = ObjectType.findById(n);
            if (!optional.isPresent()) {
                return Optional.empty();
            }
            return Optional.of(optional.get().type);
        }

        public static Optional<ObjectType> fromEntityType(EntityType entityType) {
            for (ObjectType objectType : ObjectType.values()) {
                if (objectType.type != entityType) continue;
    ...
    public static enum EntityType implements com.viaversion.viaversion.api.minecraft.entities.EntityType
    {
        ...
        private EntityType(int n2, EntityType entityType) {
            this.id = n2;
        ...
        public static Optional<EntityType> findById(int n) {
            if (n == -1) {
                return Optional.empty();
            }
            return Optional.ofNullable(TYPES.get(n));
        ...
        static {
            ...
            for (EntityType entityType : EntityType.values()) {
                TYPES.put(entityType.id, entityType);
            }
        }
    }
}
