...
public class SpawnEggRewriter {
    ...
    private static void registerSpawnEgg(String string) {
        spawnEggs.put(string, spawnEggs.size());
    }

    public static int getSpawnEggId(String string) {
        if (!spawnEggs.containsKey(string)) {
            return 1;
        }
        return 0x17F0000 | (Integer)spawnEggs.get(string) & 0xFFFF;
    }

    public static Optional<String> getEntityId(int n) {
        if (n >> 16 != 383) {
            return Optional.empty();
        }
        return Optional.ofNullable(spawnEggs.inverse().get(n & 0xFFFF));
    }
