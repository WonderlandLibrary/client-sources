...
implements CharIndirectPriorityQueue {
    ...
    public CharArrayIndirectPriorityQueue(char[] cArray, int n, CharComparator charComparator) {
        if (n > 0) {
    ...
    public CharArrayIndirectPriorityQueue(char[] cArray, int n) {
        this(cArray, n, null);
    }

    public CharArrayIndirectPriorityQueue(char[] cArray, CharComparator charComparator) {
        this(cArray, cArray.length, charComparator);
    }

    public CharArrayIndirectPriorityQueue(char[] cArray) {
        this(cArray, cArray.length, null);
    }

    public CharArrayIndirectPriorityQueue(char[] cArray, int[] nArray, int n, CharComparator charComparator) {
        this(cArray, 0, charComparator);
        this.array = nArray;
        this.size = n;
    }

    public CharArrayIndirectPriorityQueue(char[] cArray, int[] nArray, CharComparator charComparator) {
        this(cArray, nArray, nArray.length, charComparator);
    }

    public CharArrayIndirectPriorityQueue(char[] cArray, int[] nArray, int n) {
        this(cArray, nArray, n, null);
    }

    public CharArrayIndirectPriorityQueue(char[] cArray, int[] nArray) {
    ...
    private int findFirst() {
        ...
        int n = this.size;
        int n2 = --n;
        char c = this.refArray[this.array[n2]];
        if (this.c == null) {
            while (n-- != 0) {
                if (this.refArray[this.array[n]] >= c) continue;
                n2 = n;
                c = this.refArray[this.array[n2]];
            }
        } else {
            while (n-- != 0) {
                if (this.c.compare(this.refArray[this.array[n]], c) >= 0) continue;
                n2 = n;
                c = this.refArray[this.array[n2]];
            }
        }
    ...
    private int findLast() {
        ...
        if (this.c == null) {
            while (n-- != 0) {
                if (c >= this.refArray[this.array[n]]) continue;
                n2 = n;
                c = this.refArray[this.array[n2]];
            }
        } else {
            while (n-- != 0) {
                if (this.c.compare(c, this.refArray[this.array[n]]) >= 0) continue;
                n2 = n;
                c = this.refArray[this.array[n2]];
            }
        }
    ...
    public int front(int[] nArray) {
        char c = this.refArray[this.array[this.findFirst()]];
        int n = this.size;
        int n2 = 0;
        while (n-- != 0) {
            if (c != this.refArray[this.array[n]]) continue;
