...
Hash {
    ...
    public ShortLinkedOpenHashSet(Collection<? extends Short> collection, float f) {
    ...
    public ShortLinkedOpenHashSet(ShortCollection shortCollection, float f) {
        this(shortCollection.size(), f);
        this.addAll(shortCollection);
    }

    public ShortLinkedOpenHashSet(ShortCollection shortCollection) {
        this(shortCollection, 0.75f);
    }

    public ShortLinkedOpenHashSet(ShortIterator shortIterator, float f) {
        this(16, f);
    ...
    public ShortLinkedOpenHashSet(ShortIterator shortIterator) {
        this(shortIterator, 0.75f);
    }

    public ShortLinkedOpenHashSet(Iterator<?> iterator2, float f) {
        this(ShortIterators.asShortIterator(iterator2), f);
    }

    public ShortLinkedOpenHashSet(Iterator<?> iterator2) {
        this(ShortIterators.asShortIterator(iterator2));
    }

    public ShortLinkedOpenHashSet(short[] sArray, int n, int n2, float f) {
        this(n2 < 0 ? 0 : n2, f);
    ...
    public ShortLinkedOpenHashSet(short[] sArray, int n, int n2) {
        this(sArray, n, n2, 0.75f);
    }

    public ShortLinkedOpenHashSet(short[] sArray, float f) {
        this(sArray, 0, sArray.length, f);
    }

    public ShortLinkedOpenHashSet(short[] sArray) {
        this(sArray, 0.75f);
    }
    ...
    public ShortLinkedOpenHashSet clone() {
        ShortLinkedOpenHashSet shortLinkedOpenHashSet;
        try {
            shortLinkedOpenHashSet = (ShortLinkedOpenHashSet)super.clone();
        } catch (CloneNotSupportedException cloneNotSupportedException) {
            throw new InternalError();
        }
        shortLinkedOpenHashSet.key = (short[])this.key.clone();
        shortLinkedOpenHashSet.containsNull = this.containsNull;
        shortLinkedOpenHashSet.link = (long[])this.link.clone();
        return shortLinkedOpenHashSet;
    }
    ...
    implements ShortListIterator {
        ...
        int curr;
        int index;
        final ShortLinkedOpenHashSet this$0;

        SetIterator(ShortLinkedOpenHashSet shortLinkedOpenHashSet) {
            this.this$0 = shortLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            this.index = -1;
            this.next = shortLinkedOpenHashSet.first;
            this.index = 0;
        }

        SetIterator(ShortLinkedOpenHashSet shortLinkedOpenHashSet, short s) {
            this.this$0 = shortLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            this.index = -1;
            if (s == 0) {
                if (shortLinkedOpenHashSet.containsNull) {
                    this.next = (int)shortLinkedOpenHashSet.link[shortLinkedOpenHashSet.n];
                    this.prev = shortLinkedOpenHashSet.n;
                    return;
                }
                throw new NoSuchElementException("The key " + s + " does not belong to this set.");
            }
            if (shortLinkedOpenHashSet.key[shortLinkedOpenHashSet.last] == s) {
                this.prev = shortLinkedOpenHashSet.last;
                this.index = shortLinkedOpenHashSet.size;
                return;
            }
            short[] sArray = shortLinkedOpenHashSet.key;
            int n = HashCommon.mix(s) & shortLinkedOpenHashSet.mask;
            while (sArray[n] != 0) {
                if (sArray[n] == s) {
                    this.next = (int)shortLinkedOpenHashSet.link[n];
                    this.prev = n;
                    return;
                }
                n = n + 1 & shortLinkedOpenHashSet.mask;
            }
            throw new NoSuchElementException("The key " + s + " does not belong to this set.");
        }
