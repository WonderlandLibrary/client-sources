...
Hash {
    ...
    public FloatOpenHashSet(int n, float f) {
        if (f <= 0.0f || f > 1.0f) {
    ...
    public FloatOpenHashSet(Collection<? extends Float> collection, float f) {
    ...
    public FloatOpenHashSet(FloatCollection floatCollection, float f) {
        this(floatCollection.size(), f);
        this.addAll(floatCollection);
    }

    public FloatOpenHashSet(FloatCollection floatCollection) {
        this(floatCollection, 0.75f);
    }

    public FloatOpenHashSet(FloatIterator floatIterator, float f) {
        this(16, f);
    ...
    public FloatOpenHashSet(FloatIterator floatIterator) {
        this(floatIterator, 0.75f);
    }

    public FloatOpenHashSet(Iterator<?> iterator2, float f) {
        this(FloatIterators.asFloatIterator(iterator2), f);
    }

    public FloatOpenHashSet(Iterator<?> iterator2) {
        this(FloatIterators.asFloatIterator(iterator2));
    }

    public FloatOpenHashSet(float[] fArray, int n, int n2, float f) {
        this(n2 < 0 ? 0 : n2, f);
    ...
    public FloatOpenHashSet(float[] fArray, int n, int n2) {
        this(fArray, n, n2, 0.75f);
    }

    public FloatOpenHashSet(float[] fArray, float f) {
        this(fArray, 0, fArray.length, f);
    }

    public FloatOpenHashSet(float[] fArray) {
        this(fArray, 0.75f);
    }
    ...
    public FloatOpenHashSet clone() {
        FloatOpenHashSet floatOpenHashSet;
        try {
            floatOpenHashSet = (FloatOpenHashSet)super.clone();
        } catch (CloneNotSupportedException cloneNotSupportedException) {
            throw new InternalError();
        }
        floatOpenHashSet.key = (float[])this.key.clone();
        floatOpenHashSet.containsNull = this.containsNull;
    ...
    implements FloatIterator {
        ...
        final FloatOpenHashSet this$0;

        private SetIterator(FloatOpenHashSet floatOpenHashSet) {
            this.this$0 = floatOpenHashSet;
        ...
        SetIterator(FloatOpenHashSet floatOpenHashSet, 1 var2_2) {
            this(floatOpenHashSet);
        }
    }
}
