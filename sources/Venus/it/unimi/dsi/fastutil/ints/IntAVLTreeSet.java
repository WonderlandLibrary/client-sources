...
IntSortedSet {
    ...
    static final boolean $assertionsDisabled = !IntAVLTreeSet.class.desiredAssertionStatus();
    ...
    public IntAVLTreeSet(Iterator<?> iterator2) {
        this(IntIterators.asIntIterator(iterator2));
    }

    public IntAVLTreeSet(int[] nArray, int n, int n2, Comparator<? super Integer> comparator) {
        this(comparator);
    ...
    public IntAVLTreeSet(int[] nArray, int n, int n2) {
        this(nArray, n, n2, null);
    }

    public IntAVLTreeSet(int[] nArray) {
        this();
        int n = nArray.length;
        while (n-- != 0) {
            this.add(nArray[n]);
    ...
    public IntAVLTreeSet(int[] nArray, Comparator<? super Integer> comparator) {
        this(comparator);
        int n = nArray.length;
        while (n-- != 0) {
            this.add(nArray[n]);
    ...
    public boolean add(int n) {
        if (this.tree == null) {
            ...
            while (true) {
                int n3;
                if ((n3 = this.compare(n, entry.key)) == 0) {
                    return true;
                }
                if (entry.balance() != 0) {
                    n2 = 0;
            ...
            while (entry != entry5) {
                ...
                entry = this.dirPath[n2++] ? entry.right : entry.left;
            }
            if (entry3.balance() == -2) {
    ...
    public Object clone() {
        IntAVLTreeSet intAVLTreeSet;
        try {
            intAVLTreeSet = (IntAVLTreeSet)super.clone();
        } catch (CloneNotSupportedException cloneNotSupportedException) {
            throw new InternalError();
        }
        intAVLTreeSet.allocatePaths();
        if (this.count != 0) {
            ...
            while (true) {
                ...
                if (!entry3.pred()) {
                    ...
                    while (entry3.succ()) {
                        ...
                        if (entry3 == null) {
                            entry4.right = null;
                            intAVLTreeSet.firstEntry = intAVLTreeSet.tree = entry2.left;
                            while (intAVLTreeSet.firstEntry.left != null) {
                                intAVLTreeSet.firstEntry = intAVLTreeSet.firstEntry.left;
                            }
                            intAVLTreeSet.lastEntry = intAVLTreeSet.tree;
                            while (intAVLTreeSet.lastEntry.right != null) {
                                intAVLTreeSet.lastEntry = intAVLTreeSet.lastEntry.right;
                            }
    ...
    IntSortedSet {
        ...
        boolean bottom;
        boolean top;
        final IntAVLTreeSet this$0;

        public Subset(IntAVLTreeSet intAVLTreeSet, int n, boolean bl, int n2, boolean bl2) {
            this.this$0 = intAVLTreeSet;
            if (!bl && !bl2 && intAVLTreeSet.compare(n, n2) > 0) {
    ...
    implements IntListIterator {
        ...
        SetIterator(IntAVLTreeSet intAVLTreeSet) {
            this.this$0 = intAVLTreeSet;
            this.index = 0;
            this.next = intAVLTreeSet.firstEntry;
        }

        SetIterator(IntAVLTreeSet intAVLTreeSet, int n) {
            this.this$0 = intAVLTreeSet;
            this.index = 0;
            this.next = intAVLTreeSet.locateKey(n);
            if (this.next != null) {
                if (intAVLTreeSet.compare(this.next.key, n) <= 0) {
                    this.prev = this.next;
                    this.next = this.next.next();
                } else {
                    this.prev = this.next.prev();
