...
Map<K, Double> {
    ...
    default public Double put(K k, Double d) {
        return Reference2DoubleFunction.super.put(k, d);
    }

    @Override
    @Deprecated
    default public Double get(Object object) {
        return Reference2DoubleFunction.super.get(object);
    }

    @Override
    @Deprecated
    default public Double remove(Object object) {
        return Reference2DoubleFunction.super.remove(object);
    }
    ...
    default public double computeDoubleIfAbsentPartial(K k, Reference2DoubleFunction<? super K> reference2DoubleFunction) {
        Objects.requireNonNull(reference2DoubleFunction);
        double d = this.getDouble(k);
        double d2 = this.defaultReturnValue();
        if (d != d2 || this.containsKey(k)) {
            return d;
        }
        if (!reference2DoubleFunction.containsKey(k)) {
            return d2;
        }
        double d3 = reference2DoubleFunction.getDouble(k);
        this.put(k, d3);
        return d3;
    }
