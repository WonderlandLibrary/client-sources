...
Hash {
    ...
    public ReferenceLinkedOpenHashSet(Collection<? extends K> collection, float f) {
        this(collection.size(), f);
        this.addAll(collection);
    }

    public ReferenceLinkedOpenHashSet(Collection<? extends K> collection) {
        this(collection, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(ReferenceCollection<? extends K> referenceCollection, float f) {
        this(referenceCollection.size(), f);
        this.addAll(referenceCollection);
    }

    public ReferenceLinkedOpenHashSet(ReferenceCollection<? extends K> referenceCollection) {
        this(referenceCollection, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(Iterator<? extends K> iterator2, float f) {
        this(16, f);
    ...
    public ReferenceLinkedOpenHashSet(Iterator<? extends K> iterator2) {
        this(iterator2, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(K[] KArray, int n, int n2, float f) {
        this(n2 < 0 ? 0 : n2, f);
    ...
    public ReferenceLinkedOpenHashSet(K[] KArray, int n, int n2) {
        this(KArray, n, n2, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(K[] KArray, float f) {
        this(KArray, 0, KArray.length, f);
    }

    public ReferenceLinkedOpenHashSet(K[] KArray) {
        this(KArray, 0.75f);
    }
    ...
    public ReferenceLinkedOpenHashSet<K> clone() {
        ReferenceLinkedOpenHashSet referenceLinkedOpenHashSet;
        try {
            referenceLinkedOpenHashSet = (ReferenceLinkedOpenHashSet)super.clone();
        } catch (CloneNotSupportedException cloneNotSupportedException) {
            throw new InternalError();
        }
        referenceLinkedOpenHashSet.key = (Object[])this.key.clone();
        referenceLinkedOpenHashSet.containsNull = this.containsNull;
        referenceLinkedOpenHashSet.link = (long[])this.link.clone();
    ...
    public int hashCode() {
        ...
        while (n2-- != 0) {
            ...
            if (this != this.key[n3]) {
                n += System.identityHashCode(this.key[n3]);
            }
            ++n3;
        }
        return n;
    ...
    implements ObjectListIterator<K> {
        ...
        int curr;
        int index;
        final ReferenceLinkedOpenHashSet this$0;

        SetIterator(ReferenceLinkedOpenHashSet referenceLinkedOpenHashSet) {
            this.this$0 = referenceLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            this.index = -1;
            this.next = referenceLinkedOpenHashSet.first;
            this.index = 0;
        }

        SetIterator(ReferenceLinkedOpenHashSet referenceLinkedOpenHashSet, K k) {
            this.this$0 = referenceLinkedOpenHashSet;
            this.prev = -1;
            this.next = -1;
            this.curr = -1;
            this.index = -1;
            if (k == null) {
                if (referenceLinkedOpenHashSet.containsNull) {
                    this.next = (int)referenceLinkedOpenHashSet.link[referenceLinkedOpenHashSet.n];
                    this.prev = referenceLinkedOpenHashSet.n;
                    return;
                }
                throw new NoSuchElementException("The key " + k + " does not belong to this set.");
            }
            if (referenceLinkedOpenHashSet.key[referenceLinkedOpenHashSet.last] == k) {
                this.prev = referenceLinkedOpenHashSet.last;
                this.index = referenceLinkedOpenHashSet.size;
                return;
            }
            K[] KArray = referenceLinkedOpenHashSet.key;
            int n = HashCommon.mix(System.identityHashCode(k)) & referenceLinkedOpenHashSet.mask;
            while (KArray[n] != null) {
                if (KArray[n] == k) {
                    this.next = (int)referenceLinkedOpenHashSet.link[n];
                    this.prev = n;
                    return;
                }
                n = n + 1 & referenceLinkedOpenHashSet.mask;
            }
            throw new NoSuchElementException("The key " + k + " does not belong to this set.");
        }
