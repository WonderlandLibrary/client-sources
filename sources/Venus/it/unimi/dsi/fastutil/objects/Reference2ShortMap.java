...
Map<K, Short> {
    ...
    default public Short put(K k, Short s) {
        return Reference2ShortFunction.super.put(k, s);
    }

    @Override
    @Deprecated
    default public Short get(Object object) {
        return Reference2ShortFunction.super.get(object);
    }

    @Override
    @Deprecated
    default public Short remove(Object object) {
        return Reference2ShortFunction.super.remove(object);
    }
    ...
    default public short computeShortIfAbsentPartial(K k, Reference2ShortFunction<? super K> reference2ShortFunction) {
        Objects.requireNonNull(reference2ShortFunction);
        short s = this.getShort(k);
        short s2 = this.defaultReturnValue();
        if (s != s2 || this.containsKey(k)) {
            return s;
        }
        if (!reference2ShortFunction.containsKey(k)) {
            return s2;
        }
        short s3 = reference2ShortFunction.getShort(k);
        this.put(k, s3);
        return s3;
    }
