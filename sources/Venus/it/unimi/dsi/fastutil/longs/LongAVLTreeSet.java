...
LongSortedSet {
    ...
    static final boolean $assertionsDisabled = !LongAVLTreeSet.class.desiredAssertionStatus();
    ...
    public LongAVLTreeSet(Comparator<? super Long> comparator) {
        this();
        this.storedComparator = comparator;
        this.setActualComparator();
    }

    public LongAVLTreeSet(Collection<? extends Long> collection) {
        this();
        this.addAll(collection);
    }

    public LongAVLTreeSet(SortedSet<Long> sortedSet) {
        this(sortedSet.comparator());
        this.addAll(sortedSet);
    }

    public LongAVLTreeSet(LongCollection longCollection) {
        this();
        this.addAll(longCollection);
    }

    public LongAVLTreeSet(LongSortedSet longSortedSet) {
        this(longSortedSet.comparator());
        this.addAll(longSortedSet);
    }

    public LongAVLTreeSet(LongIterator longIterator) {
        this.allocatePaths();
    ...
    public LongAVLTreeSet(Iterator<?> iterator2) {
        this(LongIterators.asLongIterator(iterator2));
    }

    public LongAVLTreeSet(long[] lArray, int n, int n2, Comparator<? super Long> comparator) {
        this(comparator);
    ...
    public LongAVLTreeSet(long[] lArray, int n, int n2) {
        this(lArray, n, n2, null);
    }

    public LongAVLTreeSet(long[] lArray) {
        this();
        int n = lArray.length;
        while (n-- != 0) {
            this.add(lArray[n]);
    ...
    public LongAVLTreeSet(long[] lArray, Comparator<? super Long> comparator) {
        this(comparator);
        int n = lArray.length;
        while (n-- != 0) {
            this.add(lArray[n]);
    ...
    public boolean add(long l) {
        if (this.tree == null) {
            ...
            while (true) {
                int n2;
                if ((n2 = this.compare(l, entry.key)) == 0) {
                    return true;
                }
                if (entry.balance() != 0) {
                    n = 0;
    ...
    public Object clone() {
        LongAVLTreeSet longAVLTreeSet;
        try {
            longAVLTreeSet = (LongAVLTreeSet)super.clone();
        } catch (CloneNotSupportedException cloneNotSupportedException) {
            throw new InternalError();
        }
        longAVLTreeSet.allocatePaths();
        if (this.count != 0) {
            ...
            while (true) {
                ...
                if (!entry3.pred()) {
                    ...
                    while (entry3.succ()) {
                        ...
                        if (entry3 == null) {
                            entry4.right = null;
                            longAVLTreeSet.firstEntry = longAVLTreeSet.tree = entry2.left;
                            while (longAVLTreeSet.firstEntry.left != null) {
                                longAVLTreeSet.firstEntry = longAVLTreeSet.firstEntry.left;
                            }
                            longAVLTreeSet.lastEntry = longAVLTreeSet.tree;
                            while (longAVLTreeSet.lastEntry.right != null) {
                                longAVLTreeSet.lastEntry = longAVLTreeSet.lastEntry.right;
                            }
    ...
    LongSortedSet {
        ...
        boolean bottom;
        boolean top;
        final LongAVLTreeSet this$0;

        public Subset(LongAVLTreeSet longAVLTreeSet, long l, boolean bl, long l2, boolean bl2) {
            this.this$0 = longAVLTreeSet;
            if (!bl && !bl2 && longAVLTreeSet.compare(l, l2) > 0) {
    ...
    implements LongListIterator {
        ...
        SetIterator(LongAVLTreeSet longAVLTreeSet) {
            this.this$0 = longAVLTreeSet;
            this.index = 0;
            this.next = longAVLTreeSet.firstEntry;
        }

        SetIterator(LongAVLTreeSet longAVLTreeSet, long l) {
            this.this$0 = longAVLTreeSet;
            this.index = 0;
            this.next = longAVLTreeSet.locateKey(l);
            if (this.next != null) {
                if (longAVLTreeSet.compare(this.next.key, l) <= 0) {
                    this.prev = this.next;
                    this.next = this.next.next();
                } else {
                    this.prev = this.next.prev();
