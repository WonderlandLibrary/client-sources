...
extends DataFix {
    public TeamDisplayName(Schema schema, boolean bl) {
        super(schema, bl);
    }
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhere("TeamDisplayNameFix", type, TeamDisplayName::lambda$makeRule$4);
    }

    private static Function lambda$makeRule$4(DynamicOps dynamicOps) {
        return TeamDisplayName::lambda$makeRule$3;
    }

    private static Pair lambda$makeRule$3(Pair pair) {
        return pair.mapSecond(TeamDisplayName::lambda$makeRule$2);
    }

    private static Dynamic lambda$makeRule$2(Dynamic dynamic) {
        return dynamic.update("DisplayName", arg_0 -> TeamDisplayName.lambda$makeRule$1(dynamic, arg_0));
    }

    private static Dynamic lambda$makeRule$1(Dynamic dynamic, Dynamic dynamic2) {
        return DataFixUtils.orElse(dynamic2.asString().map(TeamDisplayName::lambda$makeRule$0).map(dynamic::createString).result(), dynamic2);
    }
