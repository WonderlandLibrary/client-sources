...
extends DataFix {
    private static final Int2ObjectMap<String> ID_MAP = DataFixUtils.make(new Int2ObjectOpenHashMap(), ItemIntIDToString::lambda$static$0);

    public ItemIntIDToString(Schema schema, boolean bl) {
        super(schema, bl);
    }

    public static String getItem(int n) {
        return (String)ID_MAP.get(n);
    }
    ...
    public TypeRewriteRule makeRule() {
        ...
        Type<Pair<String, String>> type2 = DSL.named(TypeReferences.ITEM_NAME.typeName(), NamespacedSchema.func_233457_a_());
        OpticFinder<Either<Integer, Pair<String, String>>> opticFinder = DSL.fieldFinder("id", type);
        return this.fixTypeEverywhereTyped("ItemIdFix", this.getInputSchema().getType(TypeReferences.ITEM_STACK), this.getOutputSchema().getType(TypeReferences.ITEM_STACK), arg_0 -> ItemIntIDToString.lambda$makeRule$4(opticFinder, type2, arg_0));
    }

    private static Typed lambda$makeRule$4(OpticFinder opticFinder, Type type, Typed typed) {
        return typed.update(opticFinder, type, ItemIntIDToString::lambda$makeRule$3);
    }

    private static Pair lambda$makeRule$3(Either either) {
        return either.map(ItemIntIDToString::lambda$makeRule$1, ItemIntIDToString::lambda$makeRule$2);
    }
    ...
    private static Pair lambda$makeRule$1(Integer n) {
        return Pair.of(TypeReferences.ITEM_NAME.typeName(), ItemIntIDToString.getItem(n));
    }
