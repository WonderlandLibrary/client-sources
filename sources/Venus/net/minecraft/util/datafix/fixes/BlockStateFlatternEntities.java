...
extends DataFix {
    private static final Map<String, Integer> MAP = DataFixUtils.make(Maps.newHashMap(), BlockStateFlatternEntities::lambda$static$0);

    public BlockStateFlatternEntities(Schema schema, boolean bl) {
        super(schema, bl);
    }
    ...
    public TypeRewriteRule makeRule() {
        ...
        Function<Typed, Typed> function = this::lambda$makeRule$1;
        Function<Typed, Typed> function2 = this::lambda$makeRule$2;
        ...
        Function<Typed, Typed> function3 = arg_0 -> BlockStateFlatternEntities.lambda$makeRule$3(type, arg_0);
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(TypeReferences.ENTITY), schema2.getType(TypeReferences.ENTITY), arg_0 -> this.lambda$makeRule$5(function2, function3, function, arg_0));
    }
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        ...
        return typed.update(type.finder(), type2, arg_0 -> BlockStateFlatternEntities.lambda$updateFallingBlock$10(dynamic, arg_0)).set(DSL.remainderFinder(), dynamic.remove("Data").remove("TileID").remove("Tile"));
    }

    private Typed<?> updateBlockToBlockState(Typed<?> typed, String string, String string2, String string3) {
        Tag.TagType<Pair<String, Either<Integer, String>>> tagType = DSL.field(string, DSL.named(TypeReferences.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.func_233457_a_())));
        Tag.TagType<Pair<String, Dynamic<?>>> tagType2 = DSL.field(string3, DSL.named(TypeReferences.BLOCK_STATE.typeName(), DSL.remainderType()));
        Dynamic<?> dynamic = typed.getOrCreate(DSL.remainderFinder());
        return typed.update(tagType.finder(), tagType2, arg_0 -> BlockStateFlatternEntities.lambda$updateBlockToBlockState$12(dynamic, string2, arg_0)).set(DSL.remainderFinder(), dynamic.remove(string2));
    }
    ...
    private static Pair lambda$updateBlockToBlockState$12(Dynamic dynamic, String string, Pair pair) {
        int n = ((Either)pair.getSecond()).map(BlockStateFlatternEntities::lambda$updateBlockToBlockState$11, BlockStateFlatternEntities::getBlockId);
        int n2 = dynamic.get(string).asInt(0) & 0xF;
        return Pair.of(TypeReferences.BLOCK_STATE.typeName(), BlockStateFlatteningMap.getFixedNBTForID(n << 4 | n2));
    }

    private static Integer lambda$updateBlockToBlockState$11(Integer n) {
        return n;
    }

    private static Either lambda$updateFallingBlock$10(Dynamic dynamic, Either either) {
        int n = either.map(BlockStateFlatternEntities::lambda$updateFallingBlock$7, arg_0 -> BlockStateFlatternEntities.lambda$updateFallingBlock$9(dynamic, arg_0));
        int n2 = dynamic.get("Data").asInt(0) & 0xF;
        return Either.left(Pair.of(TypeReferences.BLOCK_STATE.typeName(), BlockStateFlatteningMap.getFixedNBTForID(n << 4 | n2)));
    }

    private static Integer lambda$updateFallingBlock$9(Dynamic dynamic, Unit unit) {
        Optional<Number> optional = dynamic.get("TileID").asNumber().result();
        return optional.map(Number::intValue).orElseGet(() -> BlockStateFlatternEntities.lambda$updateFallingBlock$8(dynamic));
    }

    private static Integer lambda$updateFallingBlock$8(Dynamic dynamic) {
        return dynamic.get("Tile").asByte((byte)0) & 0xFF;
    }

    private static Integer lambda$updateFallingBlock$7(Pair pair) {
        return ((Either)pair.getSecond()).map(BlockStateFlatternEntities::lambda$updateFallingBlock$6, BlockStateFlatternEntities::getBlockId);
    }

    private static Integer lambda$updateFallingBlock$6(Integer n) {
        return n;
    }

    private Typed lambda$makeRule$5(Function function, Function function2, Function function3, Typed typed) {
        typed = this.updateEntity(typed, "minecraft:falling_block", this::updateFallingBlock);
        typed = this.updateEntity(typed, "minecraft:enderman", this::lambda$makeRule$4);
        typed = this.updateEntity(typed, "minecraft:arrow", function);
        typed = this.updateEntity(typed, "minecraft:spectral_arrow", function);
        ...
        typed = this.updateEntity(typed, "minecraft:commandblock_minecart", function3);
        typed = this.updateEntity(typed, "minecraft:minecart", function3);
        typed = this.updateEntity(typed, "minecraft:chest_minecart", function3);
        typed = this.updateEntity(typed, "minecraft:furnace_minecart", function3);
        typed = this.updateEntity(typed, "minecraft:tnt_minecart", function3);
        typed = this.updateEntity(typed, "minecraft:hopper_minecart", function3);
        return this.updateEntity(typed, "minecraft:spawner_minecart", function3);
    }
