...
extends DataFix {
    private static final Int2ObjectMap<String> field_199186_a = DataFixUtils.make(new Int2ObjectOpenHashMap(), LWJGL3KeyOptions::lambda$static$0);

    public LWJGL3KeyOptions(Schema schema, boolean bl) {
        super(schema, bl);
    }

    @Override
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(TypeReferences.OPTIONS), LWJGL3KeyOptions::lambda$makeRule$4);
    }

    private static Typed lambda$makeRule$4(Typed typed) {
        return typed.update(DSL.remainderFinder(), LWJGL3KeyOptions::lambda$makeRule$3);
    }

    private static Dynamic lambda$makeRule$3(Dynamic dynamic) {
        return dynamic.getMapValues().map(arg_0 -> LWJGL3KeyOptions.lambda$makeRule$2(dynamic, arg_0)).result().orElse(dynamic);
    }

    private static Dynamic lambda$makeRule$2(Dynamic dynamic, Map map) {
        return dynamic.createMap(map.entrySet().stream().map(LWJGL3KeyOptions::lambda$makeRule$1).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond)));
    }

    private static Pair lambda$makeRule$1(Map.Entry entry) {
        if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
            ...
            if (n < 0) {
                ...
                return Pair.of((Dynamic)entry.getKey(), ((Dynamic)entry.getValue()).createString((String)object));
            }
            String string = field_199186_a.getOrDefault(n, "key.unknown");
