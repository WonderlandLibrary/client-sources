...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ChunkStructuresTemplateRenameFix", type, arg_0 -> this.lambda$makeRule$2(opticFinder, arg_0));
    }

    private Dynamic<?> func_233153_a_(Dynamic<?> dynamic, Dynamic<?> dynamic2) {
        Pair<String, ImmutableMap<String, String>> pair;
        String string = dynamic.get("id").asString("");
        if (field_207912_a.containsKey(string) && (pair = field_207912_a.get(string)).getFirst().equals(dynamic2.get("id").asString(""))) {
    ...
    private Typed lambda$makeRule$2(OpticFinder opticFinder, Typed typed) {
        return typed.updateTyped(opticFinder, arg_0 -> this.lambda$makeRule$1(typed, arg_0));
    }

    private Typed lambda$makeRule$1(Typed typed, Typed typed2) {
        return typed2.update(DSL.remainderFinder(), arg_0 -> this.lambda$makeRule$0(typed, arg_0));
    }

    private Dynamic lambda$makeRule$0(Typed typed, Dynamic dynamic) {
        return this.func_233153_a_(typed.get(DSL.remainderFinder()), dynamic);
    }
}
