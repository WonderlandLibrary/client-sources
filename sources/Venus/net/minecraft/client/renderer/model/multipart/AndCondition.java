...
implements ICondition {
    ...
    public Predicate<BlockState> getPredicate(StateContainer<Block, BlockState> stateContainer) {
        List list = Streams.stream(this.conditions).map(arg_0 -> AndCondition.lambda$getPredicate$0(stateContainer, arg_0)).collect(Collectors.toList());
        return arg_0 -> AndCondition.lambda$getPredicate$2(list, arg_0);
    }

    private static boolean lambda$getPredicate$2(List list, BlockState blockState) {
        return list.stream().allMatch(arg_0 -> AndCondition.lambda$getPredicate$1(blockState, arg_0));
    }
