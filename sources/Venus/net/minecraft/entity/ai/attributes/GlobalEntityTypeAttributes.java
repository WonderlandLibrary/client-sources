...
public class GlobalEntityTypeAttributes {
    ...
    public static AttributeModifierMap getAttributesForEntity(EntityType<? extends LivingEntity> entityType) {
        return VANILLA_ATTRIBUTES.get(entityType);
    }
    ...
    public static void validateEntityAttributes() {
        Registry.ENTITY_TYPE.stream().filter(GlobalEntityTypeAttributes::lambda$validateEntityAttributes$0).filter(GlobalEntityTypeAttributes::lambda$validateEntityAttributes$1).map(Registry.ENTITY_TYPE::getKey).forEach(GlobalEntityTypeAttributes::lambda$validateEntityAttributes$2);
    }

    private static void lambda$validateEntityAttributes$2(ResourceLocation resourceLocation) {
        if (SharedConstants.developmentMode) {
            throw new IllegalStateException("Entity " + resourceLocation + " has no attributes");
        }
        LOGGER.error("Entity {} has no attributes", (Object)resourceLocation);
    }

    private static boolean lambda$validateEntityAttributes$1(EntityType entityType) {
        return !GlobalEntityTypeAttributes.doesEntityHaveAttributes(entityType);
    }

    private static boolean lambda$validateEntityAttributes$0(EntityType entityType) {
