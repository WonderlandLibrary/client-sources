...
extends AbstractCriterionTrigger<Instance> {
    ...
    public Instance deserializeTrigger(JsonObject jsonObject, EntityPredicate.AndPredicate andPredicate, ConditionArrayParser conditionArrayParser) {
        ...
        if (block != null) {
            statePropertiesPredicate.forEachNotPresent(block.getStateContainer(), arg_0 -> SlideDownBlockTrigger.lambda$deserializeTrigger$0(block, arg_0));
        }
    ...
    private static Block deserializeBlock(JsonObject jsonObject) {
        if (jsonObject.has("block")) {
            ...
            return Registry.BLOCK.getOptional(resourceLocation).orElseThrow(() -> SlideDownBlockTrigger.lambda$deserializeBlock$1(resourceLocation));
        }
        return null;
    }

    public void test(ServerPlayerEntity serverPlayerEntity, BlockState blockState) {
        this.triggerListeners(serverPlayerEntity, arg_0 -> SlideDownBlockTrigger.lambda$test$2(blockState, arg_0));
    }
    ...
    private static boolean lambda$test$2(BlockState blockState, Instance instance) {
