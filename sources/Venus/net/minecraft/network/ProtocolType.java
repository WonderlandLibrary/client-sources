...
public enum ProtocolType {
    ...
    private ProtocolType(int n2, PacketRegistry packetRegistry) {
        this.id = n2;
        this.field_229711_h_ = packetRegistry.field_229722_a_;
    }

    @Nullable
    public Integer getPacketId(PacketDirection packetDirection, IPacket<?> iPacket) {
        return this.field_229711_h_.get((Object)packetDirection).func_229720_a_(iPacket.getClass());
    }

    @Nullable
    public IPacket<?> getPacket(PacketDirection packetDirection, int n) {
        return this.field_229711_h_.get((Object)packetDirection).func_229718_a_(n);
    }
    ...
    @Nullable
    public static ProtocolType getById(int n) {
        return n >= -1 && n <= 2 ? STATES_BY_ID[n - -1] : null;
    }

    public static ProtocolType getFromPacket(IPacket<?> iPacket) {
        return STATES_BY_CLASS.get(iPacket.getClass());
    }

    private static void lambda$static$1(ProtocolType protocolType, PacketDirection packetDirection, PacketList packetList) {
        packetList.func_229717_a_().forEach(arg_0 -> ProtocolType.lambda$static$0(protocolType, arg_0));
    }

    private static void lambda$static$0(ProtocolType protocolType, Class clazz) {
        if (STATES_BY_CLASS.containsKey(clazz) && STATES_BY_CLASS.get(clazz) != protocolType) {
            throw new IllegalStateException("Packet " + clazz + " is already assigned to protocol " + (Object)((Object)STATES_BY_CLASS.get(clazz)) + " - can't reassign to " + protocolType);
        }
        STATES_BY_CLASS.put(clazz, protocolType);
    }
    ...
    static {
        ...
        for (ProtocolType protocolType : ProtocolType.values()) {
            int n = protocolType.getId();
            if (n < -1 || n > 2) {
                throw new Error("Invalid protocol ID " + Integer.toString(n));
            }
            ProtocolType.STATES_BY_ID[n - -1] = protocolType;
            protocolType.field_229711_h_.forEach((arg_0, arg_1) -> ProtocolType.lambda$static$1(protocolType, arg_0, arg_1));
        }
    }
    ...
    static class PacketRegistry {
        ...
        public <T extends INetHandler> PacketRegistry func_229724_a_(PacketDirection packetDirection, PacketList<T> packetList) {
            this.field_229722_a_.put(packetDirection, packetList);
            return this;
        }
    }

    static class PacketList<T extends INetHandler> {
        private final Object2IntMap<Class<? extends IPacket<T>>> field_229715_a_ = Util.make(new Object2IntOpenHashMap(), PacketList::lambda$new$0);
        private final List<Supplier<? extends IPacket<T>>> field_229716_b_ = Lists.newArrayList();

        private PacketList() {
        }

        public <P extends IPacket<T>> PacketList<T> func_229721_a_(Class<P> clazz, Supplier<P> supplier) {
            int n = this.field_229716_b_.size();
            int n2 = this.field_229715_a_.put((Class<IPacket<T>>)clazz, n);
            if (n2 != -1) {
                String string = "Packet " + clazz + " is already registered to ID " + n2;
                LogManager.getLogger().fatal(string);
                throw new IllegalArgumentException(string);
            }
            this.field_229716_b_.add(supplier);
            return this;
        }

        @Nullable
        public Integer func_229720_a_(Class<?> clazz) {
            int n = this.field_229715_a_.getInt(clazz);
            return n == -1 ? null : Integer.valueOf(n);
        }

        @Nullable
        public IPacket<?> func_229718_a_(int n) {
            Supplier<IPacket<T>> supplier = this.field_229716_b_.get(n);
        ...
        private static void lambda$new$0(Object2IntOpenHashMap object2IntOpenHashMap) {
