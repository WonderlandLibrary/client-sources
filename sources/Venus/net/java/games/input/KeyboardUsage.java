...
implements Usage {
    ...
    public static final KeyboardUsage map(int n) {
        if (n < 0 || n >= map.length) {
            return null;
        }
        return map[n];
    ...
    private KeyboardUsage(int n) {
        this(Component.Identifier.Key.UNKNOWN, n);
    }

    private KeyboardUsage(Component.Identifier.Key key, int n) {
        this.identifier = key;
        this.usage = n;
        KeyboardUsage.map[n] = this;
    }
