...
public class OldDortwareMode extends Mode {

  public OldDortwareMode(IToggleable owner, String name) {
    super(owner, name);
  }
  ...
  public void handleRender2D(EventPostRenderGui eventPostRenderGui) {
    ...
    client.moduleManager().stream()
        ...
        .sorted(this::compare)
        .forEach(name -> {
          normal.drawStringWithShadow(name, x - normal.getWidth(name), (float) y.get(), hud.colorSetting().getColor());
          y.set(y.get() + normal.getHeight(name) + 1);
        });
  }
