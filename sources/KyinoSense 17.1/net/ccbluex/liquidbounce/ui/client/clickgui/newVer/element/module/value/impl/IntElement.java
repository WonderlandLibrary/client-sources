...
extends ValueElement<Integer> {
    ...
    public float drawElement(int mouseX, int mouseY, float x, float y, float width, @NotNull Color bgColor, @NotNull Color accentColor) {
        ...
        float valueDisplay = 30.0f + (float)Fonts.font40.func_78256_a(this.savedValue.getMaximum() + this.savedValue.getSuffix());
        ...
        if (this.dragged) {
            this.savedValue.set(RangesKt.coerceIn((int)((float)this.savedValue.getMinimum() + (float)(this.savedValue.getMaximum() - this.savedValue.getMinimum()) / sliderWidth * ((float)mouseX - startPoint)), this.savedValue.getMinimum(), this.savedValue.getMaximum()));
        }
        int currLength = Fonts.font40.func_78256_a(((Number)this.savedValue.get()).intValue() + this.savedValue.getSuffix());
        ...
        Fonts.font40.drawString(((Number)this.savedValue.get()).intValue() + this.savedValue.getSuffix(), x + width + 6.0f - valueDisplay, y + 10.0f - (float)Fonts.font40.field_78288_b / 2.0f + 2.0f, -1);
    ...
    public void onClick(int mouseX, int mouseY, float x, float y, float width) {
        ...
        if (MouseUtils.mouseWithinBounds(mouseX, mouseY, x + width - 5.0f - valueDisplay, y + 2.0f, x + width + 3.0f - valueDisplay, y + 18.0f)) {
            this.savedValue.set(RangesKt.coerceIn(((Number)this.savedValue.get()).intValue() - 1, this.savedValue.getMinimum(), this.savedValue.getMaximum()));
        }
