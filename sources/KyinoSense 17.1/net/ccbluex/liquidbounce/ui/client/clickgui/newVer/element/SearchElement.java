...
public final class SearchElement {
    ...
    public final boolean drawBox(int mouseX, int mouseY, @NotNull Color accentColor) {
        ...
        RenderUtils.drawImage2(IconManager.INSTANCE.getSearch(), this.xPos + this.width - 15.0f, this.yPos + 5.0f, 10, 10);
    ...
    public final void drawPanel(int mX, int mY, float x, float y, float w, float h, int wheel, @NotNull List<CategoryElement> ces, @NotNull Color accentColor) {
        ...
        for (CategoryElement ce : ces) {
            for (ModuleElement me : ce.getModuleElements()) {
                ...
                Intrinsics.checkExpressionValueIsNotNull(string2, "searchBox.text");
                if (!StringsKt.startsWith(string, string2, true)) continue;
        ...
        Fonts.fontLarge.drawString("Search", x + 10.0f, y + 10.0f, -1);
        Fonts.fontSmall.drawString("Search", x - 170.0f, y - 12.0f, -1);
        RenderUtils.drawImage2(IconManager.INSTANCE.getBack(), x - 190.0f, y - 15.0f, 10, 10);
        ...
        for (CategoryElement ce : ces) {
            for (ModuleElement me : ce.getModuleElements()) {
                ...
                Intrinsics.checkExpressionValueIsNotNull(string3, "searchBox.text");
                if (!StringsKt.startsWith(string, string3, true)) continue;
    ...
    private final void handleScrolling(int wheel, float height) {
        ...
        this.scrollHeight = this.lastHeight > height - 60.0f ? RangesKt.coerceIn(this.scrollHeight, -this.lastHeight + height - (float)60, 0.0f) : 0.0f;
    ...
    private final void drawScroll(float x, float y, float width, float height) {
        if (this.lastHeight > height - 60.0f) {
            ...
            RenderUtils.originalRoundedRect(x + width - 6.0f, y + 5.0f + multiply, x + width - 4.0f, y + 5.0f + (height - 60.0f) * ((height - 60.0f) / this.lastHeight) + multiply, 1.0f, (int)0x50FFFFFFL);
        }
    }
    ...
    public final boolean handleTyping(char typedChar, int keyCode, float x, float y, float w, float h, @NotNull List<CategoryElement> ces) {
        ...
        for (CategoryElement ce : ces) {
            for (ModuleElement me : ce.getModuleElements()) {
                ...
                String string2 = this.searchBox.func_146179_b();
                Intrinsics.checkExpressionValueIsNotNull(string2, "searchBox.text");
                if (!StringsKt.startsWith(string, string2, true) || !me.handleKeyTyped(typedChar, keyCode)) continue;
                return true;
            }
        }
        return false;
