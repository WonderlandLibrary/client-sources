...
public class UiUtils {
    ...
    public static void shadowedRect(double x, double y, double x2, double y2, int width, float r, float g, float b, float al) {
        for (int i = width; i >= 0; --i) {
            RenderUtils.drawRect((float)x - (float)i, (float)y - (float)i, (float)x2 + (float)i, (float)y2 + (float)i, new Color(r, g, b, (float)(width - i) / (float)width * al).getRGB());
        }
    }

    public static void shadowedRect(double x, double y, double x2, double y2, int width, Color color) {
        UiUtils.shadowedRect(x, y, x2, y2, width, (float)color.getRed() / 255.0f, (float)color.getGreen() / 255.0f, (float)color.getBlue() / 255.0f, (float)color.getAlpha() / 255.0f);
    }

    public static void shadowedRect(double x, double y, double x2, double y2, int width, int color) {
        Color c = new Color(color);
        UiUtils.shadowedRect(x, y, x2, y2, width, c);
    }
