...
extends ArrayData {
    ...
    public GuardedInvocation findFastSetIndexMethod(Class<? extends ArrayData> clazz, CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE && this.hasRoomFor(index = ((Integer)(args2 = request.getArguments())[args2.length - 2]).intValue()) && (setElement = this.getElementSetter((Class<?>)elementType)) != null) {
            MethodHandle getArray = ScriptObject.GET_ARRAY.methodHandle();
