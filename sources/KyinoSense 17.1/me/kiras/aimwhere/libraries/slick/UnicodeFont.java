...
implements Font {
    ...
    public UnicodeFont(String ttfFileRef, HieroSettings settings2) throws SlickException {
        this.ttfFileRef = ttfFileRef;
        java.awt.Font font = UnicodeFont.createFont(ttfFileRef);
        this.initializeFont(font, settings2.getFontSize(), settings2.isBold(), settings2.isItalic());
        this.loadSettings(settings2);
    }
    ...
    public UnicodeFont(java.awt.Font font, HieroSettings settings2) {
        this.initializeFont(font, settings2.getFontSize(), settings2.isBold(), settings2.isItalic());
        this.loadSettings(settings2);
    }
    ...
    private void loadSettings(HieroSettings settings2) {
        this.paddingTop = settings2.getPaddingTop();
        this.paddingLeft = settings2.getPaddingLeft();
        this.paddingBottom = settings2.getPaddingBottom();
        this.paddingRight = settings2.getPaddingRight();
        this.paddingAdvanceX = settings2.getPaddingAdvanceX();
        this.paddingAdvanceY = settings2.getPaddingAdvanceY();
        this.glyphPageWidth = settings2.getGlyphPageWidth();
        this.glyphPageHeight = settings2.getGlyphPageHeight();
        this.effects.addAll(settings2.getEffects());
    }
    ...
    public boolean loadGlyphs(int maxGlyphsToLoad) throws SlickException {
        ...
        for (Object object : this.glyphPages) {
    ...
    public void clearGlyphs() {
        ...
        Iterator<GlyphPage> iterator2 = this.glyphPages.iterator();
        while (iterator2.hasNext()) {
            GlyphPage glyphPage;
            GlyphPage page = glyphPage = iterator2.next();
            try {
    ...
    public void drawDisplayList(float x, float y, String text, Color color, int startIndex, int endIndex) {
        ...
        for (int glyphIndex = 0; glyphIndex < n; ++glyphIndex) {
            ...
            if ((image2 = glyph.getImage()) == null && this.missingGlyph != null && glyph.isMissing()) {
                image2 = this.missingGlyph.getImage();
            }
            if (image2 != null) {
                Texture texture = image2.getTexture();
                ...
                image2.drawEmbedded(bounds.x + extraX, bounds.y + extraY, image2.getWidth(), image2.getHeight());
            }
            if (glyphIndex >= 0) {
