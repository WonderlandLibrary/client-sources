...
import net.minecraft.entity.EntityLivingBase.1;
...
public abstract class EntityLivingBase extends Entity {
      ...
      if(this.isServerWorld()) {
         if(!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
            if(!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
               ...
               float noSlowDown = ((EventSlowdown)(new EventSlowdown(EventSlowdown$Cause.Lava)).fire()).getNoSlowDown();
               this.moveEntity(this.motionX, this.motionY, this.motionZ);
               this.motionX *= 0.5D + 0.5D * (double)noSlowDown;
               this.motionY *= 0.5D + 0.5D * (double)noSlowDown;
               this.motionZ *= 0.5D + 0.5D * (double)noSlowDown;
               this.motionY -= 0.02D;
            ...
            this.moveEntity(this.motionX, this.motionY, this.motionZ);
            float noSlowDown = ((EventSlowdown)(new EventSlowdown(EventSlowdown$Cause.Water)).fire()).getNoSlowDown();
            this.motionX *= (double)f1 + (double)((1.0F - f1) * noSlowDown);
            this.motionY *= 0.800000011920929D + 0.19999998807907104D * (double)noSlowDown;
            this.motionZ *= (double)f1 + (double)((1.0F - f1) * noSlowDown);
            this.motionY -= 0.02D;
   ...
   public boolean isAboveLadder() {
      ...
      int j = MathHelper.floor_double(this.getEntityBoundingBox().minY - 1.0D);
   ...
   public boolean isEndOfLadder() {
      int i = MathHelper.floor_double(this.posX);
      int j = MathHelper.floor_double(this.getEntityBoundingBox().maxY);
      int k = MathHelper.floor_double(this.posZ);
   ...
   protected void collideWithNearbyEntities() {
      List<Entity> list = this.worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.20000000298023224D, 0.0D, 0.20000000298023224D), Predicates.and(EntitySelectors.NOT_SPECTATING, new 1(this)));
      if(!list.isEmpty()) {
   ...
   protected void jump() {
      ...
      if(this.isSprinting()) {
         ...
         if(this instanceof EntityPlayerSP) {
            ...
            if(isMoving && AC.getModuleManager().get(ModuleSprint.class).isEnabled() && ((Boolean)((ModuleSprint)AC.getModuleManager().get(ModuleSprint.class)).multi_jump.value).booleanValue()) {
         ...
         if(this.getHealth() <= 0.0F) {
            ...
            if(flag) {
               ...
               if(entity != null) {
                  ...
                  this.attackedAtYaw = (float)(MathHelper.func_181159_b(d0, d1) * 180.0D / 3.141592653589793D - (double)this.rotationYaw);
