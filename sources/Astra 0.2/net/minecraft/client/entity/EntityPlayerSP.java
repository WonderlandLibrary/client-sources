...
import frapppyz.cutefurry.pics.commands.CommandMain;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        DiscordUtil.createNewPresence();
        Wrapper.getModManager().onEvent(new Update());
    ...
    {
        Motion event = new Motion(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
        Wrapper.getModManager().onEvent(event);
        RotUtil.yaw = event.getYaw();
        ...
        if(mc.thePlayer.ticksExisted < 10){
            ...
            if(!(url != null && HWIDUtil.readContents(url).contains(HWIDUtil.grabHWID()))){
    ...
    public void sendChatMessage(String message)
    {
        if(message.startsWith(".")){
            CommandMain.onCommand(message.replace(".", ""));
        }else {
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !Wrapper.getModManager().getModByName("NoSlow").isToggled())
        {
