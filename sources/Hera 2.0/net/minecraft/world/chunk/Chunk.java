...
/*      */     }
/*  555 */     catch (ReportedException reportedexception) {
/*      */       
/*  557 */       CrashReportCategory crashreportcategory = reportedexception.getCrashReport().makeCategory("Block being got");
/*  558 */       crashreportcategory.addCrashSectionCallable("Location", new Callable<String>()
/*      */           {
...
/*      */     }
/*  575 */     catch (ReportedException reportedexception) {
/*      */       
/*  577 */       CrashReportCategory crashreportcategory = reportedexception.getCrashReport().makeCategory("Block being got");
/*  578 */       crashreportcategory.addCrashSectionCallable("Location", new Callable<String>()
/*      */           {
/*      */             public String call() throws Exception
/*      */             {
...
/*  591 */     if (this.worldObj.getWorldType() == WorldType.DEBUG_WORLD) {
/*      */       
...
/*  600 */       if (pos.getY() == 70)
/*      */       {
/*  602 */         iblockstate = ChunkProviderDebug.func_177461_b(pos.getX(), pos.getZ());
/*      */       }
/*      */       
...
/*  630 */       crashreportcategory.addCrashSectionCallable("Location", new Callable<String>()
/*      */           {
/*      */             public String call() throws Exception
/*      */             {
...
/*      */     {
/*  672 */       this.precipitationHeightMap[l] = -999;
/*      */     }
/*      */     
...
/*  923 */     TileEntity tileentity = this.chunkTileEntityMap.get(pos);
/*      */     
...
/*      */       
/*  977 */       TileEntity tileentity = this.chunkTileEntityMap.remove(pos);
/*      */       
...
/*      */       }
/*      */       
/* 1001 */       this.worldObj.loadEntities((Collection)this.entityLists[i]);
/*      */     } 
/*      */   }
/*      */ 
/*      */ 
...
/*      */     {
/* 1019 */       this.worldObj.unloadEntities((Collection)this.entityLists[i]);
/*      */     }
/*      */   }
/*      */ 
/*      */ 
...
/*      */       
/* 1083 */       for (Entity entity : this.entityLists[k].getByClass(entityClass)) {
/*      */         
/* 1085 */         if (entity.getEntityBoundingBox().intersectsWith(aabb) && (p_177430_4_ == null || p_177430_4_.apply(entity)))
/*      */         {
/* 1087 */           listToFill.add((T)entity);
/*      */         }
/*      */       } 
/*      */     } 
/*      */   }
...
/*      */       
/* 1239 */       BlockPos blockpos = this.tileEntityPosQueue.poll();
/*      */       
...
/*      */     } 
/*      */     int l;
/* 1338 */     for (l = 0; l < this.storageArrays.length; l++) {
/*      */       
...
/* 1348 */     if (flag)
/*      */     {
/* 1350 */       for (int i1 = 0; i1 < this.storageArrays.length; i1++) {
/*      */         
/* 1352 */         if ((p_177439_2_ & 1 << i1) != 0 && this.storageArrays[i1] != null) {
/*      */           
/* 1354 */           NibbleArray nibblearray1 = this.storageArrays[i1].getSkylightArray();
...
/* 1364 */       l = i + this.blockBiomeArray.length;
/*      */     } 
/*      */     
...
/* 1465 */           for (m = (arrayOfEnumFacing = EnumFacing.values()).length, b = 0; b < m; ) { EnumFacing enumfacing = arrayOfEnumFacing[b];
/*      */             
...
/*      */ 
/*      */         
/* 1493 */         label31: for (i = 0; i < 16; i++) {
/*      */           
/* 1495 */           for (int j = 0; j < 16; j++) {
/*      */             
...
/* 1510 */             int k = (enumfacing.getAxisDirection() == EnumFacing.AxisDirection.POSITIVE) ? 16 : 1;
...
/* 1579 */       int k = getBlockLightOpacity((BlockPos)blockpos$mutableblockpos);
/*      */       
...
/* 1588 */         flag = true;
/*      */       }
/* 1590 */       else if (flag && k == 0 && !this.worldObj.checkLight((BlockPos)blockpos$mutableblockpos)) {
/*      */         
/* 1592 */         return false;
/*      */       } 
/*      */     } 
...
/*      */       
/* 1600 */       if (getBlock((BlockPos)blockpos$mutableblockpos).getLightValue() > 0)
/*      */       {
/* 1602 */         this.worldObj.checkLight((BlockPos)blockpos$mutableblockpos);
/*      */       }
/*      */     } 
/*      */     
/* 1606 */     return true;
