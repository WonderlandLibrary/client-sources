...
public class EntityPlayerSP extends AbstractClientPlayer {
   ...
   public void onUpdate() {
      if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
         ...
         super.onUpdate();
         if (AttackUtil.combatTimer.hasReached(600L)) {
   ...
   public void onUpdateWalkingPlayer() {
      ...
      if (RotationUtil.isEnabled) {
         ...
         if (RotationUtil.keepRotationTicks <= 0.0F) {
         ...
         RotationUtil.keepRotationTicks = 0.0F;
         RotationUtil.randomizeAmount = 0.0F;
      }

      MotionEvent event = new MotionEvent(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.lastReportedYaw, this.lastReportedPitch, this.onGround, this.mc.thePlayer);
      event.call();
         ...
         if (this.isCurrentViewEntity()) {
            ...
            if (this.ridingEntity == null) {
               if (flag2 && flag3) {
                  ...
                  this.sendQueue.addToSendQueue(new C03PacketPlayer(event.isGround()));
               }
            } else {
            ...
            if (event.getPitch() != this.mc.thePlayer.rotationPitch) {
               this.renderPitch = event.getPitch();
            }

            if (event.getYaw() != this.mc.thePlayer.rotationYaw) {
   ...
   public void moveEntity(double x, double y, double z) {
      MoveEvent moveEvent = new MoveEvent(x, y, z, false);
      moveEvent.call();
   ...
   public void sendChatMessage(String message) {
      ChatEvent chatEvent = new ChatEvent(message, PacketDirection.OUTGOING);
      if (!chatEvent.call().isCanceled()) {
   ...
   public void onLivingUpdate() {
      ...
      boolean usingItem = this.isUsingItem() || ((KillAura)Slack.getInstance().getModuleManager().getInstance(KillAura.class)).isToggle() && ((KillAura)Slack.getInstance().getModuleManager().getInstance(KillAura.class)).isBlocking;
      if (usingItem && !this.isRiding()) {
         MovementInput var10000;
         if (((NoSlow)Slack.getInstance().getModuleManager().getInstance(NoSlow.class)).isToggle()) {
            var10000 = this.movementInput;
            var10000.moveStrafe *= (Float)((NoSlow)Slack.getInstance().getModuleManager().getInstance(NoSlow.class)).strafeMultiplier.getValue();
            var10000 = this.movementInput;
            var10000.moveForward *= (Float)((NoSlow)Slack.getInstance().getModuleManager().getInstance(NoSlow.class)).forwardMultiplier.getValue();
