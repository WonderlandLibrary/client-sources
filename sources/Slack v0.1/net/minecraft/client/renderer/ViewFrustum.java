...
public class ViewFrustum {
   ...
   protected void createRenderChunks(IRenderChunkFactory renderChunkFactory) {
      ...
      for(k = 0; k < this.countChunksX; ++k) {
         for(int l = 0; l < this.countChunksY; ++l) {
            for(l1 = 0; l1 < this.countChunksZ; ++l1) {
               int j1 = (l1 * this.countChunksY + l) * this.countChunksX + k;
               blockpos1 = new BlockPos(k * 16, l * 16, l1 * 16);
               this.renderChunks[j1] = renderChunkFactory.makeRenderChunk(this.world, this.renderGlobal, blockpos1, j++);
      ...
      for(k = 0; k < this.renderChunks.length; ++k) {
         RenderChunk renderchunk1 = this.renderChunks[k];

         for(l1 = 0; l1 < EnumFacing.VALUES.length; ++l1) {
            EnumFacing enumfacing = EnumFacing.VALUES[l1];
            blockpos1 = renderchunk1.func_181701_a(enumfacing);
   ...
   private void updateVboRegion(RenderChunk p_updateVboRegion_1_) {
      ...
      if (avboregion == null) {
         ...
         for(l = 0; l < aenumworldblocklayer.length; ++l) {
            avboregion[l] = new VboRegion(aenumworldblocklayer[l]);
         }
   ...
   public void deleteVboRegions() {
      Iterator var1 = this.mapVboRegions.keySet().iterator();

      while(var1.hasNext()) {
         ChunkCoordIntPair chunkcoordintpair = (ChunkCoordIntPair)var1.next();
         VboRegion[] avboregion = (VboRegion[])this.mapVboRegions.get(chunkcoordintpair);
