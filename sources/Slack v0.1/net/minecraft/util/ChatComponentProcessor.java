...
public class ChatComponentProcessor {
   public static IChatComponent processComponent(ICommandSender commandSender, IChatComponent component, Entity entityIn) throws CommandException {
      ...
      if (chatstyle != null) {
         ((IChatComponent)ichatcomponent).setChatStyle(chatstyle.createShallowCopy());
      }

      Iterator var13 = component.getSiblings().iterator();

      while(var13.hasNext()) {
         IChatComponent ichatcomponent1 = (IChatComponent)var13.next();
         ((IChatComponent)ichatcomponent).appendSibling(processComponent(commandSender, ichatcomponent1, entityIn));
      }
