...
public class EntityHorse extends EntityAnimal implements IInvBasic {
   ...
   protected EntityHorse getClosestHorse(Entity entityIn, double distance) {
      ...
      Entity entity = null;
      Iterator var7 = this.worldObj.getEntitiesInAABBexcluding(entityIn, entityIn.getEntityBoundingBox().addCoord(distance, distance, distance), horseBreedingSelector).iterator();
   ...
   private void setHorseTexturePaths() {
      ...
      if (i == 0) {
         i1 = j & 255;
         int l = (j & '\uff00') >> 8;
         if (i1 >= horseTextures.length) {
            this.field_175508_bO = false;
            return;
         }

         this.horseTexturesArray[0] = horseTextures[i1];
         this.texturePrefix = this.texturePrefix + HORSE_TEXTURES_ABBR[i1];
      ...
      i1 = this.getHorseArmorIndexSynced();
   ...
   public void readEntityFromNBT(NBTTagCompound tagCompund) {
      ...
      if (tagCompund.hasKey("ArmorItem", 10)) {
         itemstack1 = ItemStack.loadItemStackFromNBT(tagCompund.getCompoundTag("ArmorItem"));
         if (itemstack1 != null && isArmorItem(itemstack1.getItem())) {
            this.horseChest.setInventorySlotContents(1, itemstack1);
         }
      }

      if (tagCompund.hasKey("SaddleItem", 10)) {
         itemstack1 = ItemStack.loadItemStackFromNBT(tagCompund.getCompoundTag("SaddleItem"));
   ...
   public EntityAgeable createChild(EntityAgeable ageable) {
      ...
      if (k == 0) {
         ...
         if (j1 < 2) {
            l |= this.getHorseVariant() & '\uff00';
         } else if (j1 < 4) {
            l |= entityhorse.getHorseVariant() & '\uff00';
         } else {
            l |= this.rand.nextInt(5) << 8 & '\uff00';
         }
