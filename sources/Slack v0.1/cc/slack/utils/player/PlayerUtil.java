   ...
   public static void damagePlayer(double value, boolean groundCheck) {
      if (!groundCheck || getPlayer().onGround) {
         for(int i = 0; (double)i < Math.ceil(getMaxFallDist() / value); ++i) {
            PacketUtil.sendNoEvent(new C04PacketPlayerPosition(getPlayer().posX, getPlayer().posY + value, getPlayer().posZ, false));
            PacketUtil.sendNoEvent(new C04PacketPlayerPosition(getPlayer().posX, getPlayer().posY, getPlayer().posZ, false));
         }
