...
public class GuiCustomizeWorldScreen extends GuiScreen implements GuiSlider.FormatHelper, GuiPageButtonList.GuiResponder
{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (this.field_175339_B != 0)
        {
            ...
            var7.addVertexWithUV(this.width / 2 - 90, 185.0D, 0.0D, 0.0D, 2.65625D);
            var7.addVertexWithUV(this.width / 2 + 90, 185.0D, 0.0D, 5.625D, 2.65625D);
            var7.addVertexWithUV(this.width / 2 + 90, 100.0D, 0.0D, 5.625D, 0.0D);
            var7.addVertexWithUV(this.width / 2 - 90, 100.0D, 0.0D, 0.0D, 0.0D);
            var6.draw();
