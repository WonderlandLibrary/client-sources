...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private float getFOVModifier(float partialTicks, boolean p_78481_2_)
    {
        ...
        else
        {
            ...
            {
            	if (!ModuleManager.get("liquidvision").active) {
    ...
    public void updateCameraAndRender(float partialTicks)
    {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.theWorld != null)
            {
                ...
                if (ModuleManager.get("wallhack").active ||
                		ModuleManager.get("nametags").active ||
                		ModuleManager.get("waypoints").active) {
                    this.setupCameraTransform(partialTicks, 2);
                    
                    ModuleManager.get("wallhack").render3DPost();
                    ModuleManager.get("nametags").render3DPost();
                    ModuleManager.get("waypoints").render3DPost();
                }
    ...
    {
    	if (ModuleManager.get("noweather").active) {
    		return;
    	}
    ...
    private void setupFog(int p_78468_1_, float partialTicks)
    {
        ...
        else
        {
            ...
            else if (var5.getMaterial() == Material.water && !ModuleManager.get("liquidvision").active)
            {
                GlStateManager.setFog(2048);
            ...
            else if (var5.getMaterial() == Material.lava && !ModuleManager.get("liquidvision").active)
            {
                GlStateManager.setFog(2048);
