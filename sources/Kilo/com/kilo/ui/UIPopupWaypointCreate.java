...
public class UIPopupWaypointCreate extends UI {

	private float formOffset;
	
	private float fX, fY, fW, fH;
	
	public UIPopupWaypointCreate(UI parent) {
		super(parent);
	}
	...
	public void init() {
		...
		inters.add(new Button(this, "Create", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, Fonts.ttfRoundedBold14, Colors.GREEN.c, Resources.iconSubmit[1], 16, Align.L, Align.C));
		...
		((TextBox)inters.get(2)).text = "Waypoint";
		((TextBox)inters.get(3)).text = (int)Minecraft.getMinecraft().thePlayer.posX+"";
		((TextBox)inters.get(4)).text = (int)Minecraft.getMinecraft().thePlayer.posY+"";
		((TextBox)inters.get(5)).text = (int)Minecraft.getMinecraft().thePlayer.posZ+"";
		Random rand = new Random();
		((TextBox)inters.get(6)).text = rand.nextInt(255)+"";
		((TextBox)inters.get(7)).text = rand.nextInt(255)+"";
		((TextBox)inters.get(8)).text = rand.nextInt(255)+"";
	}
	...
	public void interact(Inter i) {
		...
		case 1:
			try {
				...
				WaypointManager.addWaypoint(new Waypoint(name, x, y, z, new Color(r, g, b, 255).getRGB()));
				((UIWaypoints)parent).changePopup(null);
