...
public class UIPopupPlaylistEdit extends UI {
	...
	public void init() {
		...
		inters.add(new Button(this, "Edit", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, Fonts.ttfRoundedBold14, Colors.GREEN.c, Resources.iconEdit[1], 16, Align.L, Align.C));
		...
		if (playlist != null) {
			((TextBox)inters.get(2)).text = playlist.name;
		}
	}
	...
	public void update(int mx, int my) {
		...
		((Button)inters.get(1)).text = checking?(String)null:"Edit";
	}
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (ServerUtil.playlistRename(Kilo.kilo().client.clientID, playlist.id, name)) {
