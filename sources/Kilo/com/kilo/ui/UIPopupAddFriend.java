...
public class UIPopupAddFriend extends UI {
	...
	public void init() {
		...
		inters.add(new Button(this, "Request", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, Fonts.ttfRoundedBold14, Colors.GREEN.c, Resources.iconSubmit[1], 16, Align.L, Align.C));
		inters.add(new TextBoxAlt(this, "Enter ingame name...", fX-(fW/2)+48+fH-80, fY-(fH/2)+80, fW-(48+fH-80)-32, 32, Fonts.ttfRoundedBold20, -1, Align.L, Align.C));
		inters.add(new TextBoxAlt(this, "Message...", fX-(fW/2)+48+fH-80, fY-(fH/2)+80+32+16, fW-(48+fH-80)-32, 32, Fonts.ttfRoundedBold20, -1, Align.L, Align.C));
	}
	...
	public void update(int mx, int my) {
		...
		((Button)inters.get(1)).text = checking?(String)null:"Request";
	}
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (ServerUtil.friendAdd(Kilo.kilo().client.clientID, name, msg)) {
						((UIInGameMenu)parent).changePopup(null);
					} else {
						checking = false;
