...
public class GuiOverlayDebug extends Gui {
   ...
   public GuiOverlayDebug(Minecraft mc) {
      this.mc = mc;
      this.fontRenderer = mc.MCfontRenderer;
   }
   ...
   protected List<String> getDebugInfoRight() {
      ...
      long l = j - k;
      List<String> list = Lists.newArrayList(new String[]{String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", OpenGlHelper.func_183029_j()), "", String.format("Display: %dx%d (%s)", Display.getWidth(), Display.getHeight(), GL11.glGetString(7936)), GL11.glGetString(7937), GL11.glGetString(7938)});
      ...
      if (!this.isReducedDebug() && this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.mc.objectMouseOver.getBlockPos() != null) {
         ...
         Entry entry;
         String s1;
         for(UnmodifiableIterator var17 = iblockstate.getProperties().entrySet().iterator(); var17.hasNext(); list.add(((IProperty)entry.getKey()).getName() + ": " + s1)) {
            entry = (Entry)var17.next();
            s1 = ((Comparable)entry.getValue()).toString();
            if (entry.getValue() == Boolean.TRUE) {
               s1 = ChatFormatting.GREEN + s1;
            } else if (entry.getValue() == Boolean.FALSE) {
               s1 = ChatFormatting.RED + s1;
            }
         }
      }
