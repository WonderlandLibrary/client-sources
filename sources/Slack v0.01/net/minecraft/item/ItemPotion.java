...
public class ItemPotion extends Item {
   ...
   public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
      if (stack.getMetadata() != 0) {
         ...
         if (list != null && !list.isEmpty()) {
            ...
            while(var16.hasNext()) {
               ...
               if (map != null && map.size() > 0) {
                  ...
                  while(var12.hasNext()) {
                     Entry<IAttribute, AttributeModifier> entry = (Entry)var12.next();
               ...
               if (potion.isBadEffect()) {
                  ...
                  tooltip.add(ChatFormatting.GRAY + s1);
               }
            }
         } else {
         ...
         if (!multimap.isEmpty()) {
            tooltip.add("");
            tooltip.add(ChatFormatting.DARK_PURPLE + StatCollector.translateToLocal("potion.effects.whenDrank"));
            var16 = multimap.entries().iterator();

            while(var16.hasNext()) {
               Entry<String, AttributeModifier> entry1 = (Entry)var16.next();
               ...
               if (d0 > 0.0D) {
                  tooltip.add(ChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry1.getKey())));
               } else if (d0 < 0.0D) {
                  d1 *= -1.0D;
                  tooltip.add(ChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry1.getKey())));
               }
            }
         }
      }
