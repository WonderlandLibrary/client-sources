...
public class ShadersTex {
   ...
   public static void uploadTexSubForLoadAtlas(int[][] data, int width, int height, int xoffset, int yoffset, boolean linear, boolean clamp) {
      ...
      int[][] aint1;
      if (Shaders.configNormalMap) {
         aint1 = readImageAndMipmaps(iconName + "_n", width, height, data.length, flag, -8421377);
      ...
      if (Shaders.configSpecularMap) {
         aint1 = readImageAndMipmaps(iconName + "_s", width, height, data.length, flag, 0);
      ...
      for(i = 0; w2 > 1 && h2 > 1; o2 = o1) {
         ...
         for(l2 = 0; l2 < h1; ++l2) {
            ...
            for(int k2 = 0; k2 < w1; ++k2) {
               aint[i3 + k2] = blend4Alpha(aint[j3 + k2 * 2], aint[j3 + k2 * 2 + 1], aint[j3 + w2 + k2 * 2], aint[j3 + w2 + k2 * 2 + 1]);
            }
         }
   ...
   public static void genMipmapSimple(int[] aint, int offset, int width, int height) {
      ...
      for(i = 0; w2 > 1 && h2 > 1; o2 = o1) {
         ...
         for(l2 = 0; l2 < h1; ++l2) {
            ...
            for(int k2 = 0; k2 < w1; ++k2) {
               aint[i3 + k2] = blend4Simple(aint[j3 + k2 * 2], aint[j3 + k2 * 2 + 1], aint[j3 + w2 + k2 * 2], aint[j3 + w2 + k2 * 2 + 1]);
            }
         }
