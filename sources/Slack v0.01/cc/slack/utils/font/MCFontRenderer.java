...
public class MCFontRenderer extends CFont {
   ...
   public float drawStringWithShadow(String text, float x, float y, int color) {
      float shadowWidth = this.drawString(text, (double)x + 1.0D, (double)y + 0.5D, color, true);
   ...
   public float drawCenteredString(String text, float x, float y, int color) {
      return this.drawString(text, (double)x - (double)this.getStringWidth(text) / 2.0D, (double)y, color);
   }

   public float drawCenteredString(String text, double x, double y, int color) {
      return this.drawString(text, x - (double)this.getStringWidth(text) / 2.0D, y, color);
   }

   public float drawCenteredStringWithShadow(String text, float x, float y, int color) {
      this.drawString(text, (double)x - (double)this.getStringWidth(text) / 2.0D + 0.45D, (double)y + 0.5D, color, true);
      return this.drawString(text, (double)x - (double)this.getStringWidth(text) / 2.0D, (double)y, color);
   }

   public float drawCenteredStringWithShadow(String text, double x, double y, int color) {
      this.drawString(text, x - (double)this.getStringWidth(text) / 2.0D + 0.45D, y + 0.5D, color, true);
      return this.drawString(text, x - (double)this.getStringWidth(text) / 2.0D, y, color);
   }
      ...
      if ((double)this.getStringWidth(text) > width) {
         ...
         if (currentWord.length() > 0) {
            if ((double)this.getStringWidth(currentWord) < width) {
               ...
               this.formatString(currentWord, width).forEach(finalWords::add);
            }
         }
      } else {
         finalWords.add(text);
