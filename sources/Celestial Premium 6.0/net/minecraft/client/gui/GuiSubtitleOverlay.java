...
implements ISoundEventListener {
    ...
    public void renderSubtitles(ScaledResolution resolution) {
        ...
        if (this.enabled && !this.subtitles.isEmpty()) {
            ...
            Vec3d vec3d1 = new Vec3d(0.0, 0.0, -1.0).rotatePitch(-this.client.player.rotationPitch * ((float)Math.PI / 180)).rotateYaw(-this.client.player.rotationYaw * ((float)Math.PI / 180));
            Vec3d vec3d2 = new Vec3d(0.0, 1.0, 0.0).rotatePitch(-this.client.player.rotationPitch * ((float)Math.PI / 180)).rotateYaw(-this.client.player.rotationYaw * ((float)Math.PI / 180));
