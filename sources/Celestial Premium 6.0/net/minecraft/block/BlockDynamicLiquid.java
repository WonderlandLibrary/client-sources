...
extends BlockLiquid {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (i > 0) {
            ...
            if ((j1 = this.getDepth(worldIn.getBlockState(pos.up()))) >= 0) {
                i1 = j1 >= 8 ? j1 : j1 + 8;
            }
    ...
    private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            BlockPos blockpos;
            if (enumfacing == calculateFlowCost || this.isBlocked(worldIn, blockpos = pos.offset(enumfacing), iblockstate = worldIn.getBlockState(blockpos)) || iblockstate.getMaterial() == this.blockMaterial && iblockstate.getValue(LEVEL) <= 0) continue;
            if (!this.isBlocked(worldIn, blockpos.down(), iblockstate)) {
                return distance;
            }
            if (distance >= this.getSlopeFindDistance(worldIn) || (j = this.getSlopeDistance(worldIn, blockpos, distance + 1, enumfacing.getOpposite())) >= i) continue;
            i = j;
        }
        return i;
    }
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (this.isBlocked(worldIn, blockpos, iblockstate = worldIn.getBlockState(blockpos)) || iblockstate.getMaterial() == this.blockMaterial && iblockstate.getValue(LEVEL) <= 0) continue;
            int j = this.isBlocked(worldIn, blockpos.down(), worldIn.getBlockState(blockpos.down())) ? this.getSlopeDistance(worldIn, blockpos, 1, enumfacing.getOpposite()) : 0;
            if (j < i) {
                set.clear();
            }
            if (j > i) continue;
