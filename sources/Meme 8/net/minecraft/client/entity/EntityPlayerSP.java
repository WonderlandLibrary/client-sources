...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void func_175161_p() {
		...
		if (this.func_175160_A()) {
			...
			if (this.ridingEntity == null) {
				if (var13 && var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(emu.getPosX(), emu.getPosY(), emu.getPosZ(), emu.getYaw(), emu.getPitch(), emu.isOnGround()));
				} else if (var13) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(emu.getPosX(), emu.getPosY(), emu.getPosZ(), emu.getYaw(), emu.getPitch(), emu.isOnGround()));
				} else if (var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(emu.getPosX(), emu.getPosY(), emu.getPosZ(), emu.getYaw(), emu.getPitch(), emu.isOnGround()));
	...
	public void sendChatMessage(String m) {
		if (m.startsWith("-")) {
			for (Command comm : Ruby.getRuby.cmds.cmds) {
				...
				if (m.startsWith("-" + comm.name)) {
					try {
						...
						Ruby.getRuby.chat("Not a command you retard");
					}
				}
			}
			return;
	...
	public void onLivingUpdate() {
		...
		if (!Ruby.getRuby.getModuleManager().getModule(NoSlow.class).isEnabled()) {
