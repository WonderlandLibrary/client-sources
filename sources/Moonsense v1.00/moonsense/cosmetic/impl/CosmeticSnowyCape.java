package moonsense.cosmetic.impl;

import moonsense.cosmetic.CosmeticBoolean;
import moonsense.utils.AnimatedResourceLocation;
...
public class CosmeticSnowyCape implements LayerRenderer<AbstractClientPlayer>
{
    private final RenderPlayer playerRenderer;
    public static AnimatedResourceLocation CAPEANIMATED;
    
    static {
        CosmeticSnowyCape.CAPEANIMATED = new AnimatedResourceLocation("moonsense/capes/snowcape", 10,10);
    }
    
    public CosmeticSnowyCape(final RenderPlayer playerRendererIn) {
    ...
    public void doRenderLayer(final AbstractClientPlayer entitylivingbaseIn, final float p_177141_2_, final float p_177141_3_, final float partialTicks, final float p_177141_5_, final float p_177141_6_, final float p_177141_7_, final float scale) {
        if (CosmeticBoolean.SnowCape() && entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().session.getUsername()) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().session.getUsername())) {
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.playerRenderer.bindTexture(CosmeticSnowyCape.CAPEANIMATED.getTexture());
        ...
        CosmeticSnowyCape.CAPEANIMATED.update();
    }
