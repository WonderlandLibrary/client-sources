...
public final class CommunityScreen extends Screen implements InstanceAccess {
    ...
    private TextBox textBox = new TextBox(new Vector2d(0, 0), FontManager.getNunitoBold(20), Color.WHITE, TextAlign.LEFT, "Message", 230);
    ...
    public void onRender(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        ConcurrentLinkedQueue<Message> messages = new ConcurrentLinkedQueue<>(CommunityComponent.messages);
        ...
        RenderUtil.roundedRectangle(clickGUI.position.x + 40, clickGUI.position.y + clickGUI.scale.y - bottomScale, clickGUI.scale.x - 40, bottomScale, clickGUI.round, color);
        ...
        if (increaseMaxMessages.finished(1000)) {
            this.messages = Math.min(5, this.messages + 1);
...
//                messages--;
                ChatUtil.display(CommunityComponent.messages.size());
