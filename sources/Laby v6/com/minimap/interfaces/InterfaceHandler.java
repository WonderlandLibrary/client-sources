    ...
    public static void load() {
        ...
        InterfaceHandler.minimap = new Minimap(new Interface("gui.xaero_minimap", 128, 128, ModOptions.MINIMAP) {
            ...
            public void drawInterface(final int width, final int height, final int scale, final float partial) {
                ...
                final int scaledX = (int)(super.x * mapScale);
                final int scaledY = (int)(super.y * mapScale);
                ...
                final int specW;
                final int specH = specW = mapW / 2 + 6;
                ...
                if (XaeroMinimap.getSettings().getLockNorth()) {
                    ...
                    InterfaceHandler.mc.ingameGUI.drawTexturedModalRect(2 * scaledX + 18 + (mapW + 1) / 2 - 15, 2 * scaledY + 18 + (mapH + 1) / 2 - 15, 49 + 31 * (arrowRotation % 4), 31 * (arrowRotation / 4), 31, 31);
                    GL11.glPopMatrix();
                }
                ...
                GlStateManager.translate(2 * scaledX + specW - 6 + 18, 2 * scaledY + specH - 6 + 18, 0.0f);
            ...
            public void drawTextUnderMinimap(final int scaledX, final int scaledY, final int height, final float mapScale) {
                ...
                for (int i = 0; i < this.underText.size(); ++i) {
                    ...
                    Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s, scaledX + interfaceSize / 2 - stringWidth / 2, stringY, Minimap.radarPlayers.hashCode());
                }
                this.underText.clear();
            }
            
            private void drawWaypoints(final int specW, final int specH, final double ps, final double pc, final float partial) {
                if (XaeroMinimap.settings.getShowWaypoints() && Minimap.waypoints != null) {
                    final double playerX = Minimap.getEntityX(InterfaceHandler.mc.thePlayer, partial);
                    final double playerZ = Minimap.getEntityZ(InterfaceHandler.mc.thePlayer, partial);
                    this.renderWaypointsList(Minimap.waypoints.list, playerX, playerZ, specW, specH, ps, pc);
                }
            }
            
            private void renderWaypointsList(final ArrayList<Waypoint> waypoints, final double playerX, final double playerZ, final int specW, final int specH, final double ps, final double pc) {
                for (final Waypoint w : waypoints) {
                    if (w != null && !w.disabled) {
                        if (w.type == 1 && !XaeroMinimap.getSettings().getDeathpoints()) {
                            continue;
                        }
                        final double offx = w.x - playerX;
                        final double offy = w.z - playerZ;
            ...
            private void drawCompass(final int specW, final int specH, final double ps, final double pc) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    this.translatePosition(specW, specH, ps, pc, offx, offy);
                    GlStateManager.scale(2.0f, 2.0f, 1.0f);
                    Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(nesw[i], 1 - InterfaceHandler.mc.fontRendererObj.getStringWidth(nesw[i]) / 2, -3.0f, Minimap.radarPlayers.hashCode());
                    GL11.glPopMatrix();
                }
            }
        });
            ...
            if (XaeroMinimap.events.lastGuiOpen instanceof GuiEditMode) {
                ...
                if (i != -1) {
                    ...
                    if (Keyboard.isKeyDown(31)) {
                        ...
                        switch (i) {
                            case 4: {
                                InterfaceHandler.mc.displayGuiScreen(new GuiMinimap(XaeroMinimap.getSettings()));
                                break;
                            }
                        }
                    }
