...
public class ModSettings
{
    ...
    public boolean getShowPlayers() {
        return ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_RADAR && XaeroMinimap.allowPlayerRadar && ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_PLAYER_RADAR && this.showPlayers && !this.minimapDisplayPlayersDisabled();
    }
    
    public boolean getShowMobs() {
        return ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_RADAR && XaeroMinimap.allowRadar && this.showMobs && !this.minimapDisplayMobsDisabled();
    }
    
    public boolean getShowHostile() {
        return ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_RADAR && XaeroMinimap.allowRadar && this.showHostile && !this.minimapDisplayMobsDisabled();
    }
    
    public boolean getShowItems() {
        return ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_RADAR && XaeroMinimap.allowRadar && this.showItems && !this.minimapDisplayItemsDisabled();
    }
    
    public boolean getShowOther() {
        return ServerFilter.getCurrentFilterType() != ServerFilter.FilterType.NO_RADAR && XaeroMinimap.allowRadar && this.showOther && !this.minimapDisplayOtherDisabled();
    }
    ...
    public boolean getSlimeChunks() {
        return this.slimeChunks && (this.serverSlimeSeed != -1L || Minecraft.getMinecraft().getIntegratedServer() != null);
    }
            ...
            try {
                if (args[0].equalsIgnoreCase("world")) {
                    if (!args[1].contains("_")) {
                        ...
                        final int n = 1;
                        array[n] = sb.append(array[n]).append("_null").toString();
                    }
                    ...
                    if (!args[1].contains("_")) {
                        ...
                        final int n2 = 1;
                        array2[n2] = sb2.append(array2[n2]).append("_null").toString();
                    }
                    Minimap.addWorld(args[1]);
                    ...
                    final Waypoint loadWaypoint = new Waypoint(Integer.parseInt(args[4]), Integer.parseInt(args[5]), Integer.parseInt(args[6]), args[2], args[3], Integer.parseInt(args[7]));
                    if (args.length > 8) {
    ...
    public void loadSettings() throws IOException {
        ...
        if (!this.loadWaypoints(XaeroMinimap.waypointsFile)) {
            this.loadWaypoints(XaeroMinimap.optionsFile);
        }
    ...
    public String getKeyBinding(final ModOptions par1EnumOptions) {
        ...
        if (par1EnumOptions.getEnumFloat()) {
            final float f1 = this.getOptionFloatValue(par1EnumOptions);
            return s + f1;
        }
    ...
    public void setOptionValue(final ModOptions par1EnumOptions, final int par2) throws IOException {
        if (par1EnumOptions == ModOptions.ZOOM) {
            ...
            if (this.customSlimeSeedNeeded()) {
                Minecraft.getMinecraft().displayGuiScreen(new GuiSlimeSeed());
                return;
            }
