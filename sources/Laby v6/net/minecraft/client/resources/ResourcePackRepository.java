...
public class ResourcePackRepository
{
    ...
    public void updateRepositoryEntriesAll()
    {
        ...
        this.readAllAndFill(list, this.getResourcePackFiles(), "");
        this.repositoryEntriesAll.removeAll(list);

        for (ResourcePackRepository.Entry resourcepackrepository$entry : this.repositoryEntriesAll)
        {
            resourcepackrepository$entry.closeResourcePack();
        }

        this.repositoryEntriesAll = list;
    }

    private void readAllAndFill(List<ResourcePackRepository.Entry> p_readAllAndFill_1_, List<File> p_readAllAndFill_2_, String p_readAllAndFill_3_)
    {
        for (File file1 : p_readAllAndFill_2_)
        {
            if (file1.isDirectory() && file1.getName().startsWith("dir_") && p_readAllAndFill_3_.isEmpty())
            {
                ...
                {
                    this.readAllAndFill(p_readAllAndFill_1_, Arrays.<File>asList(afile), file1.getName());
                }
            }
            ...
            if (!file1.isDirectory() || !file1.getName().startsWith("dir_"))
            {
                ...
                else
                {
                    ...
                    {
                        p_readAllAndFill_1_.add(this.repositoryEntriesAll.get(i));
                    }
                }

                resourcepackrepository$entry.setDirPath(p_readAllAndFill_3_.replaceFirst("dir_", ""));
            }
        }
    }
    ...
    public class Entry
    {
        ...
        public void setDirPath(String p_setDirPath_1_)
        {
            this.dirPath = p_setDirPath_1_;
        }
