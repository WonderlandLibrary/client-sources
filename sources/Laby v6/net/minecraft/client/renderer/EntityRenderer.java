...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        if (pass == 0 || pass == 2)
        {
            ...
            if (flag)
            {
                ShadersRender.updateChunks(renderglobal, finishTimeNano);
            }
            else
            {
        ...
        if (!this.debugView)
        {
            ...
            if (this.mc.objectMouseOver != null && entity.isInsideOfMaterial(Material.water) && flag1)
            {
                ...
                if ((!flag2 || !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, new Object[] {renderglobal, entityplayer, this.mc.objectMouseOver, Integer.valueOf(0), entityplayer.getHeldItem(), Float.valueOf(partialTicks)})) && !this.mc.gameSettings.hideGUI)
                {
        ...
        if (flag1 && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.water))
        {
            ...
            GlStateManager.disableAlpha();
            this.mc.mcProfiler.endStartSection("outline");
            boolean flag3 = Reflector.ForgeHooksClient_onDrawBlockHighlight.exists();

            if ((!flag3 || !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, new Object[] {renderglobal, entityplayer1, this.mc.objectMouseOver, Integer.valueOf(0), entityplayer1.getHeldItem(), Float.valueOf(partialTicks)})) && !this.mc.gameSettings.hideGUI)
            {
        ...
        boolean flag4 = Reflector.callBoolean(Reflector.ForgeHooksClient_renderFirstPersonHand, new Object[] {this.mc.renderGlobal, Float.valueOf(partialTicks), Integer.valueOf(pass)});

        if (!flag4 && this.renderHand && !Shaders.isShadowPass)
        {
            if (flag)
            {
