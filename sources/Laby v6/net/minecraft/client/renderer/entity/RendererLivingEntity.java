...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        ...
        if (this.canRenderName(entity))
        {
            ...
            if (d0 < (double)(f * f))
            {
                ...
                boolean flag = false;
                String s1 = s;
                s = FriendsLoader.getNick(s, Color.removeColor(entity.getName()));
                flag = !s1.equals(s);

                if (flag)
                {
                    String s2 = entity.getDisplayName().getUnformattedText().replace(Color.removeColor(entity.getName()), "");
                    String s3 = "";

                    if (s2.startsWith(Color.c) && s2.length() >= 2)
                    {
                        s3 = s2.substring(0, 2);
                    }
                ...
                if (entity.isSneaking())
                {
                    ...
                    GlStateManager.pushMatrix();
                    GlStateManager.translate((double)((float)x), (double)((float)y + entity.height + 0.5F - (entity.isChild() ? entity.height / 2.0F : 0.0F)) + LabyMod.getInstance().getCosmeticManager().getNameTagHeight(entity), (double)((float)z));
                ...
                {
                    FontRenderer fontrenderer1 = this.getFontRendererFromRenderManager();
                    GlStateManager.pushMatrix();
                    GlStateManager.translate((double)((float)x), (double)((float)y + entity.height + 0.5F - (entity.isChild() ? entity.height / 2.0F : 0.0F)) + LabyMod.getInstance().getCosmeticManager().getNameTagHeight(entity), (double)((float)z));
                    GL11.glNormal3f(0.0F, 1.0F, 0.0F);
                    GlStateManager.rotate(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
                    ...
                    GlStateManager.disableTexture2D();
                    GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                    int j = fontrenderer1.getStringWidth(s) / 2;
                    Tessellator tessellator1 = Tessellator.getInstance();
                    WorldRenderer worldrenderer1 = tessellator1.getWorldRenderer();
                    worldrenderer1.begin(7, DefaultVertexFormats.POSITION_COLOR);
                    worldrenderer1.pos((double)(-j - 1), -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                    worldrenderer1.pos((double)(-j - 1), 8.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                    worldrenderer1.pos((double)(j + 1), 8.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                    worldrenderer1.pos((double)(j + 1), -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                    tessellator1.draw();
                    GlStateManager.enableTexture2D();
                    ...
                    fontrenderer1.drawString(s, -fontrenderer1.getStringWidth(s) / 2, 0, -1);
                    GlStateManager.enableLighting();
                    GlStateManager.disableBlend();
                    GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                    GlStateManager.popMatrix();
                ...
                else
                {
                    ...
                    if (cosmeticuser != null)
                    {
                        ...
                        if (cosmetic != null)
                        {
                            double d1 = 0.0D;
                            d1 = d1 + 0.25D;
                            this.renderOffsetLivingLabel(entity, x, y - (entity.isChild() ? (double)(entity.height / 2.0F) : 0.0D) + d2 + d1, z, cosmetic.d.replace("&", Color.c), 0.02666667F, d0);
                        }
                    }
                }
            }
    ...
    protected boolean canRenderName(T entity)
    {
        ...
        return Minecraft.isGuiEnabled() && flag && !entity.isInvisibleToPlayer(entityplayersp) && entity.riddenByEntity == null;
    }
