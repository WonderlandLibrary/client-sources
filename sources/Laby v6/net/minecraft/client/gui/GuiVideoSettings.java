...
public class GuiVideoSettings extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        int j1 = this.height / 6 + 21 * (i / 2) - 12;
        int k1 = 0;
        k1 = this.width / 2 - 155 + 0;
        this.buttonList.add(new GuiOptionButton(231, k1, j1, "Shaders..."));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (Math.abs(mouseX - this.lastMouseX) <= 5 && Math.abs(mouseY - this.lastMouseY) <= 5)
        {
            ...
            if (System.currentTimeMillis() >= this.mouseStillTime + (long)j)
            {
                int k = this.width / 2 - 150;
                int l = this.height / 6 - 7;
                ...
                if (guibutton != null)
                {
                    String s3 = this.getButtonName(guibutton.displayString);
                    String[] astring = this.getTooltipLines(s3);
                    ...
                    this.drawGradientRect(k, l, i1, j1, -536870912, -536870912);

                    for (int k1 = 0; k1 < astring.length; ++k1)
                    {
                        String s4 = astring[k1];
                        this.fontRendererObj.drawStringWithShadow(s4, (float)(k + 5), (float)(l + 5 + k1 * 11), 14540253);
                    }
                }
            }
        }
