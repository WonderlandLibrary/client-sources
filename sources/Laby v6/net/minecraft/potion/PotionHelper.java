...
public class PotionHelper
{
    ...
    public static int calcPotionLiquidColor(Collection p_77911_0_)
    {
        ...
        if (p_77911_0_ != null && !p_77911_0_.isEmpty())
        {
            ...
            for (Object potioneffect : p_77911_0_)
            {
                if (((PotionEffect) potioneffect).getIsShowParticles())
                {
                    int j = Potion.potionTypes[((PotionEffect) potioneffect).getPotionID()].getLiquidColor();

                    if (Config.isCustomColors())
                    {
                        j = CustomColorizer.getPotionColor(((PotionEffect) potioneffect).getPotionID(), j);
                    }

                    for (int k = 0; k <= ((PotionEffect) potioneffect).getAmplifier(); ++k)
                    {
        ...
        else
        {
            if (Config.isCustomColors())
            {
                i = CustomColorizer.getPotionColor(0, i);
            }
