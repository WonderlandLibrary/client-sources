...
public abstract class MapGenStructure extends MapGenBase
{
    ...
    public boolean generateStructure(World worldIn, Random randomIn, ChunkCoordIntPair chunkCoord)
    {
        ...
        for (Object structurestart : this.structureMap.values())
        {
            if (((StructureStart) structurestart).isSizeableStructure() && ((StructureStart) structurestart).func_175788_a(chunkCoord) && ((StructureStart) structurestart).getBoundingBox().intersectsWith(i, j, i + 15, j + 15))
            {
                ((StructureStart) structurestart).generateStructure(worldIn, randomIn, new StructureBoundingBox(i, j, i + 15, j + 15));
                ((StructureStart) structurestart).func_175787_b(chunkCoord);
                flag = true;
                this.func_143026_a(((StructureStart) structurestart).getChunkPosX(), ((StructureStart) structurestart).getChunkPosZ(), (StructureStart) structurestart);
            }
        }
    ...
    protected StructureStart func_175797_c(BlockPos pos)
    {
        ...
        for (Object structurestart : this.structureMap.values())
        {
            if (((StructureStart) structurestart).isSizeableStructure() && ((StructureStart) structurestart).getBoundingBox().isVecInside(pos))
            {
                Iterator iterator = ((StructureStart) structurestart).getComponents().iterator();
                ...
                return (StructureStart) structurestart;
            }
        }
    ...
    public BlockPos getClosestStrongholdPos(World worldIn, BlockPos pos)
    {
        ...
        for (Object structurestart : this.structureMap.values())
        {
            if (((StructureStart) structurestart).isSizeableStructure())
            {
                StructureComponent structurecomponent = (StructureComponent)((StructureStart) structurestart).getComponents().get(0);
