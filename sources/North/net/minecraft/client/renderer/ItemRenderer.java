...
public class ItemRenderer {
    ...
    public void renderItem(EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        if (heldStack != null) {
            ...
            GlStateManager.scale(Animations.scale.get().floatValue(), Animations.scale.get().floatValue(), Animations.scale.get().floatValue());
    ...
    public void blockAnimation(float equip, float swingProcess, float partialTicks) {
        float var2 = 0.8F;
        final float funny = MathHelper.sin(MathHelper.sqrt_float(swingProcess) * (float) Math.PI);
        float f = 1.0F - (this.prevEquippedProgress + (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
        float compensation = MathHelper.sin(MathHelper.sqrt_float(swingProcess) * 3.1415927f);
        float var15 = MathHelper.sin(MathHelper.sqrt_float(swingProcess) * (float) Math.PI);
        String mode = Animations.animation.get().getName();
        switch (mode) {
            case "Minecraft":
            ...
            case "Classic":
                ...
                this.transformFirstPersonItem(equip, swingProcess / 1.5f);
                this.func_178103_d();
                break;
            case "Apollo":
                GlStateManager.translate(0.0F, 0.0F, 0.1F);
                GlStateManager.rotate(-var15 * 25.0F, -8.0F, -0.0F, 2.0F);
                transformFirstPersonItem(equip, swingProcess);
                func_178103_d();
                break;
            case "Swang":
                this.transformFirstPersonItem(equip / 2, swingProcess);
                GlStateManager.rotate(funny * 30.0F / 2.0F, -funny, -0.0F, 9.0F);
                GlStateManager.rotate(funny * 40.0F, 1.0F, -funny / 2.0F, -0.0F);
                this.func_178103_d();
                break;
            case "Swong": {
                this.transformFirstPersonItem(equip / 2.0F, swingProcess);
            ...
            case "Exhi":
                this.transformFirstPersonItem(equip / 3, 0.0F);
                GL11.glTranslatef(0.1f, 0.3f, -0.1f);
                GL11.glRotated(-compensation * 20.0f, compensation / 2.0f, 0.0, 9.0);
                GL11.glRotated(-compensation * 50.0f, 0.800000011920929, compensation / 2.0f, 0.0);
                this.func_178103_d();
                break;
            case "Astolfo":
                GlStateManager.rotate(swingProcess * 250, 0, 0, 1);
                this.transformFirstPersonItem(equip, 0);
                this.func_178103_d();
                break;
            case "Spin":
                this.transformFirstPersonItem(equip, 0.0F);
                ...
                GL11.glRotatef(-var15 * 333.0F / -2.0F, -8.0F, 0.0F, 12.0F);
                GL11.glRotatef(-var15 * -166.0F, 77.0F, -2.8F, -0.0F);
                break;
        }
    }
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        ...
        boolean fakeAb = false;
        if (NorthSingleton.INSTANCE.getModules().isEnabled("Animations") && KillAura.target != null) {
            fakeAb = true;
        }
...
//                    NorthSingleton.INSTANCE.getModules().isEnabled("Animations") && KillAura.block) {
//                blockAnimation(f,f1,partialTicks);
//            }

            else if (abstractclientplayer.getItemInUseCount() > 0 || fakeAb) {
                EnumAction enumaction = this.itemToRender.getItemUseAction();
                if (fakeAb) enumaction = EnumAction.BLOCK;
...
//                        this.func_178103_d();
                ...
                if (NorthSingleton.INSTANCE.getModules().isEnabled("Animations")) {
                    if (Animations.betterClick.get()) {
                    } else {
