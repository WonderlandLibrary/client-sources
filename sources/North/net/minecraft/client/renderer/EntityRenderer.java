...
import xyz.northclient.features.events.Render3DEvent;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            NorthSingleton.INSTANCE.getEventBus().post(hurtCamEvent);
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if(NorthSingleton.INSTANCE.auth != null) {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        if(Minecraft.getMinecraft() != null) {
            if(!(Minecraft.getMinecraft().currentScreen instanceof GuiAuth)) {
                if(NorthSingleton.INSTANCE.getAuth().keepAliveDelay == NorthSingleton.INSTANCE.getAuth().marawAutomobile) {
                    if(!Auth.firstLogin) {
                        if(NorthSingleton.INSTANCE.getAuth().uid != -32467324) {

                        } else {
                            System.out.println("uid not passed");
                            while(true) {}
                        }
                    } else {
                        System.out.println("smth not passed");
                        //while(true) {}
                    }
                } else {
                    System.out.println("maraw not passed (" + NorthSingleton.INSTANCE.getAuth().keepAliveDelay + ") (" + NorthSingleton.INSTANCE.getAuth().marawAutomobile + ")");
                    while(true) {}
                }
            }
        }
        ...
        NorthSingleton.INSTANCE.getEventBus().post(new Render3DEvent(partialTicks));
