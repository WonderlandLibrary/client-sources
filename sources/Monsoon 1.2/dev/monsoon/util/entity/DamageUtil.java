...
public class DamageUtil
{
    ...
    public static void damageMethodOne() {
        ...
        for (int i = 0; i < (getMaxFallDist() / (offset - 0.005F)) + 1; i++) {
            mc.getNetHandler().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(x, y + offset, z, false));
            mc.getNetHandler().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(x, y + damageY, z, false));
            mc.getNetHandler().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(x, y + damageYTwo + offset * 0.000001, z, false));
        }
        mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer(true));
    }

    public static void damageVerus() {
        mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 3.2501, mc.thePlayer.posZ, false));
    ...
    public static void damageMethodThree() {
        ...
        for (int i = 0; i < (int) ((minValue / (randomNumber(0.0890D, 0.0849D) - 1.0E-3D - Math.random() * 0.0002F - Math.random() * 0.0002F)) + 18); i++) {
            mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y + randomNumber(0.0655D, 0.0625D) - randomNumber(1.0E-3D, 1.0E-2D) - Math.random() * 0.0002F, z, false));
            mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y + Math.random() * 0.0002F, z, false));
        }
        mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer(true));
    }
