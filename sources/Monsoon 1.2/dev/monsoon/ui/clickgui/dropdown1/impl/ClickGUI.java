package dev.monsoon.ui.clickgui.dropdown1.impl;
...
public class ClickGUI extends ClickHandler {
   ...
   public void doColoring() {
       if(Monsoon.clickGuiMod.theme.is("Discord")) {
    ...
    public void drawModule(int x, int y, int w, int h, int mouseX, int mouseY, Module module) {
       ...
       if (module.isEnabled()) {
        ...
        fr.drawStringWithShadow(module.name, x + 3, y + 3, fontColor);
        GlStateManager.popMatrix();
        if(module.getKey() != 0) {
            fr.drawStringWithShadow(EnumChatFormatting.GRAY + "[" + EnumChatFormatting.RESET + Keyboard.getKeyName(module.key) + EnumChatFormatting.GRAY + "]", x + 3 +  fr.getStringWidth(module.name) + 4, y + 3, fontColor);
        }

        if(!module.settings.isEmpty() && !module.isOpen()) {
           fr.drawStringWithShadow(EnumChatFormatting.DARK_GRAY + "[" + EnumChatFormatting.RESET + "..." + EnumChatFormatting.DARK_GRAY + "]", x+w -  fr.getStringWidth("[...]") - 2, y + 3, fontColor);
    ...
    public void drawSetting(int x, int y, int w, int h, int mouseX, int mouseY, Setting setting)
    {
        ...
        if(setting instanceof BooleanSetting) {
           ...
           fr.drawStringWithShadow(setting.name, x + 3, y + 3, fontColor);
        } else if(setting instanceof ModeSetting) {
            //Gui.drawRect(x, y, x + 2, y+ h, fontColor);
            //Gui.drawRect(x + w, y, x + w - 2, y+ h, fontColor);
            Gui.drawRect(x+ 1, y, x+w - 1, y+h, settingBgColor);
            fr.drawStringWithShadow(setting.name + ": " + ((ModeSetting) setting).getMode(), x + 3, y + 3, fontColor);
           ...
           // Gui.drawRect(x, y, x + 2, y+ h, fontColor);
            ...
            fr.drawStringWithShadow(setting.name + " " + ((NumberSetting) setting).getValue(), x + 3, y + 3, fontColor);
        } else if(setting instanceof PlaceholderSetting) {
            //Gui.drawRect(x, y, x + 2, y+ h, fontColor);
            //Gui.drawRect(x + w, y, x + w - 2, y+ h, fontColor);
            Gui.drawRect(x+ 1, y, x+w - 1, y+h, settingBgColor);
            fr.drawStringWithShadow(setting.name, x + w / 2 - fr.getStringWidth(setting.name) / 2, y + 3, fontColor);
        }
    ...
    public void onClose()
    {
        Monsoon.clickGuiMod.setEnabled(false);
    }
