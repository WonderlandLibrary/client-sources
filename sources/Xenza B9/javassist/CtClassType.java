...
class CtClassType extends CtClass
{
    ...
    @Override
    protected void extendToString(final StringBuilder buffer) {
        if (this.wasChanged) {
    ...
    private void exToString(final StringBuilder buffer, final String msg, CtMember head, final CtMember tail) {
        buffer.append(msg);
        while (head != tail) {
            head = head.next();
            buffer.append(head);
    ...
    public CtClass[] getNestedClasses() throws NotFoundException {
        ...
        final List<CtClass> list = new ArrayList<CtClass>(n);
        for (int i = 0; i < n; ++i) {
    ...
    private static void updateInnerEntry(final int newMod, final String name, final CtClass clazz, final boolean outer) {
        final ClassFile cf = clazz.getClassFile2();
        final InnerClassesAttribute ica = (InnerClassesAttribute)cf.getAttribute("InnerClasses");
        if (ica != null) {
            final int mod = newMod & 0xFFFFFFF7;
            final int i = ica.find(name);
            if (i >= 0) {
                final int isStatic = ica.accessFlags(i) & 0x8;
    ...
    static Object[] toAnnotationType(final boolean ignoreNotFound, final ClassPool cp, final AnnotationsAttribute a1, final AnnotationsAttribute a2) throws ClassNotFoundException {
        ...
        final List<Object> annotations = new ArrayList<Object>();
        for (int i = 0; i < size1; ++i) {
            try {
    ...
    private void makeFieldCache(final CtMember.Cache cache) {
        final List<FieldInfo> fields = this.getClassFile3(false).getFields();
    ...
    private void makeBehaviorCache(final CtMember.Cache cache) {
        final List<MethodInfo> methods = this.getClassFile3(false).getMethods();
    ...
    public CtField[] getFields() {
        final List<CtMember> alist = new ArrayList<CtMember>();
        getFields(alist, this);
        return alist.toArray(new CtField[alist.size()]);
    }
    
    private static void getFields(final List<CtMember> alist, final CtClass cc) {
        if (cc == null) {
            return;
        }
        try {
        ...
        try {
            ...
            final CtClass[] ifs = interfaces = cc.getInterfaces();
            for (final CtClass ctc : interfaces) {
                getFields(alist, ctc);
            }
        }
    ...
    CtField getField2(final String name, final String desc) {
        ...
        try {
            final CtClass[] interfaces;
            final CtClass[] ifs = interfaces = this.getInterfaces();
            for (final CtClass ctc : interfaces) {
                final CtField f = ctc.getField2(name, desc);
                if (f != null) {
                    return f;
                }
            }
    ...
    public CtMethod[] getMethods() {
        final Map<String, CtMember> h = new HashMap<String, CtMember>();
        getMethods0(h, this);
        return h.values().toArray(new CtMethod[h.size()]);
    }
    
    private static void getMethods0(final Map<String, CtMember> h, final CtClass cc) {
        try {
            final CtClass[] interfaces;
            final CtClass[] ifs = interfaces = cc.getInterfaces();
            for (final CtClass ctc : interfaces) {
    ...
    private static CtMethod getMethod0(final CtClass cc, final String name, final String desc) {
        ...
        try {
            ...
            final CtClass[] ifs = interfaces = cc.getInterfaces();
            for (final CtClass ctc : interfaces) {
                final CtMethod i = getMethod0(ctc, name, desc);
                if (i != null) {
                    return i;
                }
            }
    ...
    public CtMethod[] getDeclaredMethods() {
        ...
        CtMember mth = memCache.methodHead();
        final CtMember mthTail = memCache.lastMethod();
        final List<CtMember> methods = new ArrayList<CtMember>();
        while (mth != mthTail) {
            mth = mth.next();
            methods.add(mth);
        }
    ...
    public void instrument(final CodeConverter converter) throws CannotCompileException {
        ...
        for (final MethodInfo minfo : methods.toArray(new MethodInfo[methods.size()])) {
    ...
    public String makeUniqueName(final String prefix) {
        final Map<Object, CtClassType> table = new HashMap<Object, CtClassType>();
        this.makeMemberList(table);
        final Set<Object> keys = table.keySet();
    ...
    private void makeMemberList(final Map<Object, CtClassType> table) {
        ...
        Label_0081: {
            ...
            try {
                final CtClass[] interfaces;
                final CtClass[] ifs = interfaces = this.getInterfaces();
                for (final CtClass ic : interfaces) {
        ...
        final List<MethodInfo> methods = this.getClassFile2().getMethods();
        for (final MethodInfo minfo : methods) {
            table.put(minfo.getName(), this);
        }
        final List<FieldInfo> fields = this.getClassFile2().getFields();
        for (final FieldInfo finfo : fields) {
