...
public class BasicBlock
{
    ...
    static class Mark implements Comparable<Mark>
    {
        ...
        @Override
        public int compareTo(final Mark obj) {
            if (null == obj) {
                return -1;
            }
    ...
    public static class Maker
    {
        ...
        public BasicBlock[] make(final CodeIterator ci, final int begin, final int end, final ExceptionTable et) throws BadBytecode {
            final Map<Integer, Mark> marks = this.makeMarks(ci, begin, end, et);
            final BasicBlock[] bb = this.makeBlocks(marks);
            this.addCatchers(bb, et);
            return bb;
        }
        
        private Mark makeMark(final Map<Integer, Mark> table, final int pos) {
            return this.makeMark0(table, pos, true, true);
        }
        
        private Mark makeMark(final Map<Integer, Mark> table, final int pos, final BasicBlock[] jump, final int size, final boolean always) {
            final Mark m = this.makeMark0(table, pos, false, false);
            m.setJump(jump, size, always);
            return m;
        }
        
        private Mark makeMark0(final Map<Integer, Mark> table, final int pos, final boolean isBlockBegin, final boolean isTarget) {
            final Integer p = pos;
            Mark m = table.get(p);
            if (m == null) {
                m = new Mark(pos);
        ...
        private Map<Integer, Mark> makeMarks(final CodeIterator ci, final int begin, final int end, final ExceptionTable et) throws BadBytecode {
            ci.begin();
            ci.move(begin);
            final Map<Integer, Mark> marks = new HashMap<Integer, Mark>();
            while (ci.hasNext()) {
        ...
        private void makeGoto(final Map<Integer, Mark> marks, final int pos, final int target, final int size) {
        ...
        protected void makeJsr(final Map<Integer, Mark> marks, final int pos, final int target, final int size) throws BadBytecode {
            throw new JsrBytecode();
        }
        
        private BasicBlock[] makeBlocks(final Map<Integer, Mark> markTable) {
            final Mark[] marks = markTable.values().toArray(new Mark[markTable.size()]);
            Arrays.sort(marks);
