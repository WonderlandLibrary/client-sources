...
public class TypeChecker extends Visitor implements Opcode, TokenId
{
    ...
    protected static StringBuilder typeToString(final StringBuilder sbuf, final int type, int dim, final String cname) {
        String s;
        if (type == 307) {
    ...
    protected CtField fieldAccess(final ASTree expr) throws CompileError {
        ...
        if (expr instanceof Expr) {
            ...
            final int op = e2.getOperator();
            if (op == 35) {
                final Member mem2 = (Member)e2.oprand2();
                final CtField f2 = this.resolver.lookupField(((Symbol)e2.oprand1()).get(), mem2);
                mem2.setField(f2);
                return f2;
            }
            if (op == 46) {
                ...
                catch (final CompileError ce) {
                    err = ce;
                }
                final ASTree oprnd1 = e2.oprand1();
                if (oprnd1 instanceof Symbol) {
                    return this.fieldAccess2(e2, ((Symbol)oprnd1).get());
                }
                if (err != null) {
                    throw err;
                }
