...
public abstract class CodeGen extends Visitor implements Opcode, TokenId
{
    ...
    private void atWhileStmnt(final Stmnt st, final boolean notDo) throws CompileError {
        final List<Integer> prevBreakList = this.breakList;
        final List<Integer> prevContList = this.continueList;
    ...
    protected void patchGoto(final List<Integer> list, final int targetPc) {
        for (final int pc : list) {
    ...
    private void atSwitchStmnt(final Stmnt st) throws CompileError {
        ...
        final ArrayList<Integer> gotoDefaults = new ArrayList<Integer>();
        int ipairs = 0;
        int defaultPc = -1;
        for (ASTList list2 = body; list2 != null; list2 = list2.tail()) {
            ...
            if (op == 310) {
                ...
                final int curPos = this.bytecode.currentPc();
                long caseLabel;
                if (isString) {
                    caseLabel = this.computeStringLabel(label.head(), tmpVar, gotoDefaults);
                }
                else {
                    caseLabel = this.computeLabel(label.head());
                }
        ...
        for (final int addr : gotoDefaults) {
    ...
    private int computeStringLabel(ASTree expr, final int tmpVar, final List<Integer> gotoDefaults) throws CompileError {
        this.doTypeCheck(expr);
        expr = TypeChecker.stripPlusExpr(expr);
        if (expr instanceof StringL) {
            final String label = ((StringL)expr).get();
            this.bytecode.addAload(tmpVar);
            ...
            final Integer pc = this.bytecode.currentPc();
            this.bytecode.addIndex(0);
            gotoDefaults.add(pc);
    ...
    private static int getListSize(final List<Integer> list) {
