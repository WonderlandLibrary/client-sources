...
public abstract class TypeData
{
    ...
    public int dfs(final List<TypeData> order, final int index, final ClassPool cp) throws NotFoundException {
        return index;
    }
    ...
    protected static class BasicType extends TypeData
    {
        ...
        @Override
        String toString2(final Set<TypeData> set) {
            return this.name;
        }
    }
    ...
    public static class TypeVar extends AbsTypeVar
    {
        ...
        @Override
        public int dfs(final List<TypeData> preOrder, int index, final ClassPool cp) throws NotFoundException {
            if (this.visited > 0) {
                return index;
            }
            final int n2 = ++index;
            ...
            if (this.visited == this.smallest) {
                final List<TypeData> scc = new ArrayList<TypeData>();
                TypeVar cv;
                do {
        ...
        private void fixTypes(final List<TypeData> scc, final ClassPool cp) throws NotFoundException {
            final Set<String> lowersSet = new HashSet<String>();
            boolean isBasicType = false;
            TypeData kind = null;
            for (int size = scc.size(), i = 0; i < size; ++i) {
                final TypeVar tvar = scc.get(i);
                final List<TypeData> tds = tvar.lowers;
        ...
        private void fixTypes1(final List<TypeData> scc, final TypeData kind) throws NotFoundException {
            for (int size = scc.size(), i = 0; i < size; ++i) {
        ...
        private String fixTypes2(final List<TypeData> scc, final Set<String> lowersSet, final ClassPool cp) throws NotFoundException {
            final Iterator<String> it = lowersSet.iterator();
            if (lowersSet.size() == 0) {
                return null;
            }
            if (lowersSet.size() == 1) {
        ...
        private CtClass fixByUppers(final List<TypeData> users, final ClassPool cp, final Set<TypeData> visited, CtClass type) throws NotFoundException {
            if (users == null) {
                return type;
            }
        ...
        @Override
        String toString2(final Set<TypeData> hash) {
            hash.add(this);
