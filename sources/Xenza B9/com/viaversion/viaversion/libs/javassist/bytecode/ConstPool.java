...
public final class ConstPool
{
    ...
    public int getDynamicBootstrap(final int index) {
        final DynamicInfo iv = (DynamicInfo)this.getItem(index);
        return iv.bootstrap;
    }
    
    public int getDynamicNameAndType(final int index) {
        final DynamicInfo iv = (DynamicInfo)this.getItem(index);
        return iv.nameAndType;
    }
    
    public String getDynamicType(final int index) {
        final DynamicInfo iv = (DynamicInfo)this.getItem(index);
        if (iv == null) {
            return null;
        }
    ...
    public String getModuleInfo(final int index) {
        final ModuleInfo mi = (ModuleInfo)this.getItem(index);
        return this.getUtf8Info(mi.name);
    }
    
    public String getPackageInfo(final int index) {
        final PackageInfo mi = (PackageInfo)this.getItem(index);
        return this.getUtf8Info(mi.name);
    }
    ...
    public int copy(final int n, final ConstPool dest, final Map<String, String> classnames) {
        if (n == 0) {
            return 0;
        }
    ...
    public int addDynamicInfo(final int bootstrap, final int nameAndType) {
        return this.addItem(new DynamicInfo(bootstrap, nameAndType, this.numOfItems));
    }
    
    public int addModuleInfo(final int nameIndex) {
        return this.addItem(new ModuleInfo(nameIndex, this.numOfItems));
    }
    
    public int addPackageInfo(final int nameIndex) {
    ...
    private static Map<ConstInfo, ConstInfo> makeItemsCache(final LongVector items) {
        final Map<ConstInfo, ConstInfo> cache = new HashMap<ConstInfo, ConstInfo>();
        int i = 1;
        while (true) {
