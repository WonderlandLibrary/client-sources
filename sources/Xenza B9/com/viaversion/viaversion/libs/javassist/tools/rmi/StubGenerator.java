...
public class StubGenerator implements Translator
{
    ...
    public void start(final ClassPool pool) throws NotFoundException {
        this.classPool = pool;
        final CtClass c = pool.get("com.viaversion.viaversion.libs.javassist.tools.rmi.Sample");
    ...
    private CtClass produceProxyClass(final CtClass orgclass, final Class<?> orgRtClass) throws CannotCompileException, NotFoundException {
        ...
        CtField f = new CtField(this.classPool.get("com.viaversion.viaversion.libs.javassist.tools.rmi.ObjectImporter"), "importer", proxy);
        f.setModifiers(2);
        ...
        for (int i = 0; i < ms.length; ++i) {
            ...
            if (m.getDeclaringClass() != Object.class && !Modifier.isFinal(mod)) {
                if (Modifier.isPublic(mod)) {
                    ...
                    final CtMethod wmethod = CtNewMethod.wrapped(this.toCtClass(m.getReturnType()), m.getName(), this.toCtClass(m.getParameterTypes()), this.exceptionForProxy, body, CtMethod.ConstParameter.integer(i), proxy);
