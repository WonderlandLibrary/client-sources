...
public class ProxyFactory
{
    ...
    public Class<?> createClass(final MethodHandles.Lookup lookup, final MethodFilter filter) {
        this.computeSignature(filter);
        return this.createClass1(lookup);
    }
    
    Class<?> createClass(final MethodHandles.Lookup lookup, final byte[] signature) {
        this.installSignature(signature);
        return this.createClass1(lookup);
    }
    ...
    private void createClass2(final ClassLoader cl, final MethodHandles.Lookup lookup) {
        final String key = this.getKey(this.superClass, this.interfaces, this.signature, this.factoryWriteReplace);
        Map<String, ProxyDetails> cacheForTheLoader = ProxyFactory.proxyCache.get(cl);
        ...
        if (details != null) {
            final Reference<Class<?>> reference = details.proxyClass;
    ...
    private void createClass3(final ClassLoader cl, final MethodHandles.Lookup lookup) {
    ...
    public Object create(final Class<?>[] paramTypes, final Object[] args) throws NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        final Constructor<?> cons = c.getConstructor(paramTypes);
    ...
    private ClassFile make() throws CannotCompileException {
        ...
        if (this.genericSignature != null) {
            final SignatureAttribute sa = new SignatureAttribute(pool, this.genericSignature);
            cf.addAttribute(sa);
        }
        this.makeConstructors(this.classname, cf, pool, this.classname);
        final List<Find2MethodsArgs> forwarders = new ArrayList<Find2MethodsArgs>();
    ...
    private void makeSortedMethodList() {
        ...
        this.hasGetHandler = false;
        final Map<String, Method> allMethods = this.getMethods(this.superClass, this.interfaces);
        Collections.sort(this.signatureMethods = new ArrayList<Map.Entry<String, Method>>(allMethods.entrySet()), ProxyFactory.sorter);
    }
    ...
    private void computeSignature(final MethodFilter filter) {
        ...
        for (int idx = 0; idx < l; ++idx) {
            final Method m = (Method)this.signatureMethods.get(idx).getValue();
    ...
    private static void addClassInitializer(final ClassFile cf, final ConstPool cp, final String classname, final int size, final List<Find2MethodsArgs> forwarders) throws CannotCompileException {
    ...
    private int overrideMethods(final ClassFile cf, final ConstPool cp, final String className, final List<Find2MethodsArgs> forwarders) throws CannotCompileException {
        final String prefix = makeUniqueName("_d", this.signatureMethods);
        final Iterator<Map.Entry<String, Method>> it = this.signatureMethods.iterator();
        int index = 0;
        while (it.hasNext()) {
            final Map.Entry<String, Method> e = it.next();
            if ((ClassFile.MAJOR_VERSION < 49 || !isBridge(e.getValue())) && this.testBit(this.signature, index)) {
                this.override(className, e.getValue(), prefix, index, keyToDesc(e.getKey(), e.getValue()), cf, cp, forwarders);
            }
            ++index;
        }
        return index;
    ...
    private void override(final String thisClassname, final Method meth, final String prefix, final int index, final String desc, final ClassFile cf, final ConstPool cp, final List<Find2MethodsArgs> forwarders) throws CannotCompileException {
    ...
    private static String makeUniqueName(final String name, final List<Map.Entry<String, Method>> sortedMethods) {
    ...
    private static boolean makeUniqueName0(final String name, final Iterator<Map.Entry<String, Method>> it) {
        while (it.hasNext()) {
            if (((String)it.next().getKey()).startsWith(name)) {
                return false;
            }
        }
        return true;
    ...
    private Map<String, Method> getMethods(final Class<?> superClass, final Class<?>[] interfaceTypes) {
        final Map<String, Method> hash = new HashMap<String, Method>();
    ...
    private void getMethods(final Map<String, Method> hash, final Class<?> clazz, final Set<Class<?>> visitedClasses) {
        ...
        for (int j = 0; j < methods.length; ++j) {
            if (!Modifier.isPrivate(methods[j].getModifiers())) {
                ...
                if (null != oldMethod && isBridge(m) && !Modifier.isPublic(oldMethod.getDeclaringClass().getModifiers()) && !Modifier.isAbstract(oldMethod.getModifiers()) && !isDuplicated(j, methods)) {
    ...
    private static boolean isDuplicated(final int index, final Method[] methods) {
    ...
    private static boolean areParametersSame(final Method method, final Method targetMethod) {
        final Class<?>[] methodTypes = method.getParameterTypes();
        final Class<?>[] targetMethodTypes = targetMethod.getParameterTypes();
    ...
    private static MethodInfo makeConstructor(final String thisClassName, final Constructor<?> cons, final ConstPool cp, final Class<?> superClass, final boolean doHandlerInit) {
        ...
        code.addAload(0);
        code.addGetstatic("com.viaversion.viaversion.libs.javassist.util.proxy.RuntimeSupport", "default_interceptor", ProxyFactory.HANDLER_TYPE);
        code.addPutfield(thisClassName, "handler", ProxyFactory.HANDLER_TYPE);
    ...
    private static MethodInfo makeForwarder(final String thisClassName, final Method meth, final String desc, final ConstPool cp, final Class<?> declClass, final String delegatorName, final int index, final List<Find2MethodsArgs> forwarders) {
    ...
    private static MethodInfo makeWriteReplace(final ConstPool cp) {
        ...
        code.addAload(0);
        code.addInvokestatic("com.viaversion.viaversion.libs.javassist.util.proxy.RuntimeSupport", "makeSerializedProxy", "(Ljava/lang/Object;)Ljavassist/util/proxy/SerializedProxy;");
        code.addOpcode(176);
        minfo.setCodeAttribute(code.toCodeAttribute());
        return minfo;
    }
    ...
    static {
        ...
        ProxyFactory.sorter = new Comparator<Map.Entry<String, Method>>() {
            @Override
            public int compare(final Map.Entry<String, Method> e1, final Map.Entry<String, Method> e2) {
                return e1.getKey().compareTo((String)e2.getKey());
            }
        };
    }
