...
final class ComponentFlattenerImpl implements ComponentFlattener
{
    ...
    private <T extends Component> Handler flattener(final T test) {
        final Handler flattener = this.propagatedFlatteners.computeIfAbsent(test.getClass(), key -> {
            final Function<Component, String> value = this.flatteners.get(key);
            if (value != null) {
                ...
                Map.Entry<Class<?>, Function<?, String>> entry = null;
                ...
                final BiConsumer<Component, Consumer<Component>> complexValue = this.complexFlatteners.get(key);
                if (complexValue != null) {
                    ...
                    while (iterator2.hasNext()) {
                        final Map.Entry<Class<?>, BiConsumer<?, Consumer<Component>>> entry2 = iterator2.next();
                        if (entry2.getKey().isAssignableFrom(key)) {
                            return (component, listener, depth) -> entry.getValue().accept(component, c -> this.flatten0(c, listener, depth));
                        }
                    }
                    return Handler.NONE;
                }
