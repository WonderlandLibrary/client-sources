...
public class EntityPackets1_11 extends LegacyEntityRewriter<Protocol1_10To1_11>
{
        ...
        ((AbstractProtocol<ClientboundPackets1_9_3, C2, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_9_3.EFFECT, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(wrapper -> {
                    final int type = wrapper.get((Type<Integer>)Type.INT, 0);
                    if (type == 2002 || type == 2007) {
                        if (type == 2007) {
                            wrapper.set(Type.INT, 0, 2002);
                        }
                        final int mappedData = PotionSplashHandler.getOldData(wrapper.get((Type<Integer>)Type.INT, 1));
                        if (mappedData != -1) {
        ...
        ((AbstractProtocol<ClientboundPackets1_9_3, C2, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_9_3.SPAWN_MOB, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(wrapper -> {
                    ...
                    final EntityType type = EntityPackets1_11.this.tracker(wrapper.user()).entityType(entityId);
                    final List<Metadata> list = wrapper.get(Types1_9.METADATA_LIST, 0);
    ...
    protected void registerRewrites() {
        ...
        this.filter().filterFamily(Entity1_11Types.EntityType.CHESTED_HORSE).index(15).handler((event, meta) -> {
            final StoredEntityData data3 = this.storedEntityData(event);
            final ChestedHorseStorage storage2 = data3.get(ChestedHorseStorage.class);
            final boolean b4 = (boolean)meta.getValue();
            storage2.setChested(b4);
            event.cancel();
            return;
        });
        this.filter().type(Entity1_11Types.EntityType.LIAMA).handler((event, meta) -> {
            final StoredEntityData data4 = this.storedEntityData(event);
            final ChestedHorseStorage storage3 = data4.get(ChestedHorseStorage.class);
