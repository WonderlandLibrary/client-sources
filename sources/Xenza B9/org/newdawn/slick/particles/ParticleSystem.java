...
public class ParticleSystem
{
    ...
    public void reset() {
        final Iterator pools = this.particlesByEmitter.values().iterator();
        while (pools.hasNext()) {
            final ParticlePool pool = pools.next();
            pool.reset(this);
        }
    ...
    public void render(final float x, final float y) {
        ...
        ParticleSystem.GL.glTranslatef(x, y, 0.0f);
        if (this.blendingMode == 1) {
            ParticleSystem.GL.glBlendFunc(770, 1);
        }
        if (this.usePoints()) {
            ParticleSystem.GL.glEnable(2832);
        ...
        for (int emitterIdx = 0; emitterIdx < this.emitters.size(); ++emitterIdx) {
            ...
            if (emitter.useAdditive()) {
                ParticleSystem.GL.glBlendFunc(770, 771);
            }
        }
        if (this.usePoints()) {
            ParticleSystem.GL.glDisable(2832);
        }
        if (this.blendingMode == 1) {
            ParticleSystem.GL.glBlendFunc(770, 771);
        }
    ...
    public void update(final int delta) {
        ...
        final ArrayList removeMe = new ArrayList();
        for (int i = 0; i < this.emitters.size(); ++i) {
            final ParticleEmitter emitter = this.emitters.get(i);
        ...
        if (!this.particlesByEmitter.isEmpty()) {
            final Iterator it = this.particlesByEmitter.values().iterator();
            while (it.hasNext()) {
                final ParticlePool pool = it.next();
    ...
    static {
        ParticleSystem.GL = Renderer.get();
    }
