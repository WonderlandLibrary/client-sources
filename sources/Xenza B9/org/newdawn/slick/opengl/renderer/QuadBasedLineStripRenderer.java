...
public class QuadBasedLineStripRenderer implements LineStripRenderer
{
    ...
    public void start() {
        ...
        this.pts = 0;
        this.cpt = 0;
        QuadBasedLineStripRenderer.GL.flush();
        final float[] col = QuadBasedLineStripRenderer.GL.getCurrentColor();
    ...
    public void renderLines(final float[] points, final int count) {
        if (this.antialias) {
            QuadBasedLineStripRenderer.GL.glEnable(2881);
            this.renderLinesImpl(points, count, this.width + 1.0f);
        }
        QuadBasedLineStripRenderer.GL.glDisable(2881);
    ...
    public void renderLinesImpl(final float[] points, final int count, final float w) {
        ...
        float lastx2 = 0.0f;
        float lasty2 = 0.0f;
        QuadBasedLineStripRenderer.GL.glBegin(7);
        for (int i = 0; i < count + 1; ++i) {
            ...
            if (dx != 0.0f || dy != 0.0f) {
                ...
                if (i != 0) {
                    this.bindColor(prev);
                    QuadBasedLineStripRenderer.GL.glVertex3f(lastx1, lasty1, 0.0f);
                    QuadBasedLineStripRenderer.GL.glVertex3f(lastx2, lasty2, 0.0f);
                    this.bindColor(current);
                    QuadBasedLineStripRenderer.GL.glVertex3f(x1 + tx, y1 + ty, 0.0f);
                    QuadBasedLineStripRenderer.GL.glVertex3f(x1 - tx, y1 - ty, 0.0f);
                }
                lastx1 = x2 - tx;
                lasty1 = y2 - ty;
                ...
                if (i < count - 1) {
                    ...
                    QuadBasedLineStripRenderer.GL.glVertex3f(x1 - tx, y1 - ty, 0.0f);
                    this.bindColor(next);
                    QuadBasedLineStripRenderer.GL.glVertex3f(x2 - tx, y2 - ty, 0.0f);
                    QuadBasedLineStripRenderer.GL.glVertex3f(x2 + tx, y2 + ty, 0.0f);
                }
            }
        }
        QuadBasedLineStripRenderer.GL.glEnd();
        ...
        if (this.lineCaps) {
            ...
            if (dx2 != 0.0f || dy2 != 0.0f) {
                QuadBasedLineStripRenderer.GL.glBegin(6);
                this.bindColor(0);
                QuadBasedLineStripRenderer.GL.glVertex2f(points[0], points[1]);
                for (int j = 0; j < 180.0f + step; j += (int)step) {
                    final float ang = (float)Math.toRadians(fang + j);
                    QuadBasedLineStripRenderer.GL.glVertex2f(points[0] + (float)(Math.cos(ang) * width), points[1] + (float)(Math.sin(ang) * width));
                }
                QuadBasedLineStripRenderer.GL.glEnd();
            }
        }
        if (this.lineCaps) {
            ...
            if (dx2 != 0.0f || dy2 != 0.0f) {
                ...
                QuadBasedLineStripRenderer.GL.glVertex2f(points[count * 2 - 2], points[count * 2 - 1]);
                for (int j = 0; j < 180.0f + step; j += (int)step) {
                    final float ang = (float)Math.toRadians(fang + j);
                    QuadBasedLineStripRenderer.GL.glVertex2f(points[count * 2 - 2] + (float)(Math.cos(ang) * width), points[count * 2 - 1] + (float)(Math.sin(ang) * width));
                }
                QuadBasedLineStripRenderer.GL.glEnd();
            }
        }
    ...
    private void bindColor(final int index) {
        if (index < this.cpt) {
            if (this.renderHalf) {
                QuadBasedLineStripRenderer.GL.glColor4f(this.colours[index * 4] * 0.5f, this.colours[index * 4 + 1] * 0.5f, this.colours[index * 4 + 2] * 0.5f, this.colours[index * 4 + 3] * 0.5f);
            }
            else {
                QuadBasedLineStripRenderer.GL.glColor4f(this.colours[index * 4], this.colours[index * 4 + 1], this.colours[index * 4 + 2], this.colours[index * 4 + 3]);
            }
        }
    }
    ...
    static {
        QuadBasedLineStripRenderer.GL = Renderer.get();
