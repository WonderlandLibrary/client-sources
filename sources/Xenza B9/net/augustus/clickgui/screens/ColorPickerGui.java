...
public class ColorPickerGui extends GuiScreen implements MM, SM
{
    ...
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        if (keyCode == 1) {
            if (ColorPickerGui.mm.clickGUI.mode.getSelected().equalsIgnoreCase("Default")) {
                this.mc.displayGuiScreen(Augustus.getInstance().getClickGui());
            }
            else if (ColorPickerGui.mm.clickGUI.mode.getSelected().equalsIgnoreCase("Plane")) {
                this.mc.displayGuiScreen(Augustus.getInstance().getPlaneGui());
            }
            else if (ColorPickerGui.mm.clickGUI.mode.getSelected().equalsIgnoreCase("CSGO")) {
                this.mc.displayGuiScreen(Augustus.getInstance().csgogui);
            }
            else if (ColorPickerGui.mm.clickGUI.mode.getSelected().equalsIgnoreCase("New")) {
                EventManager.call(new EventClickGui());
                this.mc.displayGuiScreen(new Dark());
            }
            else {
                this.mc.displayGuiScreen(Augustus.getInstance().getClickGui());
            }
