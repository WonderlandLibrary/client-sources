...
public class AltLogin extends GuiScreen
{
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        ...
        if (button.id == 6) {
            final String naem = "Xenza_" + RandomStringUtils.random(10, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789");
            (this.loginThread = new LoginThread(naem, "", true)).start();
        }
        if (button.id == 5) {
            try {
                final File dir = new File(this.mc.mcDataDir, "xenza");
    ...
    private void login() {
        (this.loginThread = new LoginThread(this.username, this.password, false)).start();
    }
    ...
    public static boolean loginToken(final String token) {
        ...
        catch (final MalformedURLException var15) {
        ...
        catch (final ProtocolException var17) {
