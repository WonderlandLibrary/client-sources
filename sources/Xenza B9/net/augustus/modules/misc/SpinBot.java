...
public class SpinBot extends Module
{
    ...
    public SpinBot() {
        ...
        this.mode = new StringValue(613473, "Mode", this, "Spin", new String[] { "Spin", "Random" });
    ...
    private boolean shouldSpin() {
        ...
        if (!this.click.getBoolean() || (!Mouse.isButtonDown(1) && !Mouse.isButtonDown(0))) {
            ...
            if (SpinBot.mm.killAura.isToggled()) {
                final KillAura killAura = SpinBot.mm.killAura;
                if (KillAura.target != null) {
                    return false;
                }
            }
            if (!BlockUtil.isScaffoldToggled() && !SpinBot.mm.teleport.isToggled()) {
                return true;
            }
            return false;
        }
    ...
    public void onEventPreMotion(final EventPreMotion eventPreMotion) {
        if (!this.moveFix.getBoolean() && this.spin) {
            this.uhh();
    ...
    public void onEventMove(final EventMove eventMove) {
        if (this.spin && !this.moveFix.getBoolean()) {
    ...
    private void uhh() {
        if (this.timeHelper2.reached(1000L)) {
            if (this.timeHelper.reached(50L)) {
                if (this.mode.getSelected().equalsIgnoreCase("Spin")) {
                    final float[] rots = this.rots;
                    final int n = 0;
                    rots[n] += 90.0f;
                    this.rots[1] = 90.0f;
                }
                else if (this.mode.getSelected().equalsIgnoreCase("Random")) {
                    this.rots[0] = RandomUtil.nextFloat(-180.0f, 180.0f);
                    this.rots[1] = RandomUtil.nextFloat(-90.0f, 90.0f);
                }
