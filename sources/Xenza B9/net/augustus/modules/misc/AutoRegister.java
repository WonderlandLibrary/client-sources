...
public class AutoRegister extends Module
{
    ...
    public void onEventReadPacket(final EventReadPacket eventReadPacket) {
        ...
        if (packet instanceof S02PacketChat) {
            ...
            if (StringUtils.containsIgnoreCase((CharSequence)text, (CharSequence)"/register") || StringUtils.containsIgnoreCase((CharSequence)text, (CharSequence)"/register password password") || text.equalsIgnoreCase("/register <password> <password>")) {
                AutoRegister.mc.thePlayer.sendChatMessage("/register Hallo1337 Hallo1337");
                this.text = "/register Hallo1337 Hallo1337";
                this.timeHelper.reset();
            }
            else if (StringUtils.containsIgnoreCase((CharSequence)text, (CharSequence)"/login password") || StringUtils.containsIgnoreCase((CharSequence)text, (CharSequence)"/login") || text.equalsIgnoreCase("/login <password>")) {
