...
public class ScaffoldWalk extends Module
{
    ...
    public void onEventClick(final EventClick eventClick) {
        ...
        if (this.b != null && ScaffoldWalk.mc.currentScreen == null && this.rotated) {
    ...
    private boolean buildForward() {
        final float realYaw = MathHelper.wrapAngleTo180_float(ScaffoldWalk.mc.thePlayer.rotationYaw);
        if ((realYaw > 77.5 && realYaw < 102.5) || realYaw > 167.5 || realYaw < -167.0f || (realYaw < -77.5 && realYaw > -102.5) || realYaw <= -12.5 || realYaw < 12.5) {}
        return true;
    }
    ...
    private void getYawBasedPitch(final float yaw) {
        ...
        for (float i = 40.0f; i < 89.0f; i += 0.01f) {
            final MovingObjectPosition m = ScaffoldWalk.mc.thePlayer.customRayTrace(d, 1.0f, yaw, i);
        ...
        movingObjectPositions.sort(Comparator.comparingDouble(mx -> ScaffoldWalk.mc.thePlayer.getDistanceSq(mx.getBlockPos().add(0.5, 0.5, 0.5))));
        MovingObjectPosition j = null;
        if (movingObjectPositions.size() > 0) {
            j = movingObjectPositions.get(0);
        }
        if (ScaffoldWalk.mc.theWorld.getBlockState(bbbb).getBlock().getMaterial() == Material.air && j != null && ScaffoldWalk.mc.thePlayer.hurtTime == 0) {
            final MovingObjectPosition objectPosition = ScaffoldWalk.mc.thePlayer.customRayTrace(d, 1.0f, yaw, this.lastRots[1]);
            if (objectPosition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.isOkBlock(j.getBlockPos()) && objectPosition.sideHit != EnumFacing.DOWN && objectPosition.sideHit != EnumFacing.UP && objectPosition.getBlockPos().getY() <= bbbb.getY()) {
                this.rots[1] = this.lastRots[1];
            }
            else {
                final float[] f = this.rotationUtil.scaffoldRots(j.hitVec.xCoord, j.getBlockPos().getY() + RandomUtil.nextDouble(0.45, 0.55), j.hitVec.zCoord, this.lastRots[0], this.lastRots[1], (float)this.yawSpeed.getValue(), (float)this.pitchSpeed.getValue(), false);
                this.rots[1] = f[1];
            }
        }
