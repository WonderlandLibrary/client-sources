...
public class Fucker extends Module
{
    ...
    public void onEventEarlyTick(final EventEarlyTick eventEarlyTick) {
        ...
        Label_0102: {
            Label_0097: {
                if (Fucker.mm.killAura.isToggled()) {
                    final KillAura killAura = Fucker.mm.killAura;
                ...
                if (this.b != null) {
                    ...
                    if (this.troughWall.getBoolean()) {
                        ...
                        if (movingObjectPosition != null && movingObjectPosition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                            ...
                            final String movingObjectPositionx = selected = this.action.getSelected();
                            switch (selected) {
                                case "Break": {
                        ...
                        if (Fucker.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && !Fucker.mc.objectMouseOver.getBlockPos().equals(this.b)) {
                            ...
                            if (movingObjectPosition2 != null) {
                                ...
                                final String var10 = selected2 = this.action.getSelected();
                                switch (selected2) {
                                    case "Break": {
                                        if (this.instant.getBoolean()) {
    ...
    private Vec3 getBedPos() {
                ...
                for (int z = b.getZ() - d; z < b.getZ() + d; ++z) {
                    if (this.isValidBlock(new BlockPos(x, y, z), this.getId())) {
                        ...
                        if (this.myBed.getBoolean() && Fucker.mc.thePlayer.getSpawnPos() != null) {
                            if (Fucker.mc.thePlayer.getSpawnPos().distanceTo(vec3) > 20.0) {
                                positions.add(vec3);
        ...
        positions.sort(Comparator.comparingDouble(vec3x -> Fucker.mc.thePlayer.getDistance(vec3x.xCoord, vec3x.yCoord, vec3x.zCoord)));
        return positions.isEmpty() ? null : positions.get(0);
    }
    
    private int getId() {
        final String selected;
        final String var2 = selected = this.block.getSelected();
        int id = 0;
        switch (selected) {
            case "Bed": {
                id = 26;
