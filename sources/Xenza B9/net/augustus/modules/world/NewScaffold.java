...
public class NewScaffold extends Module
{
    ...
    public void onEventEarlyTick(final EventEarlyTick eventEarlyTick) {
        if (NewScaffold.mc.thePlayer != null && NewScaffold.mc.theWorld != null) {
            ...
            if (this.blockPos != null) {
                ...
                final String var3 = selected = this.mode.getSelected();
                switch (selected) {
                    case "Legit": {
        ...
        if (this.start) {
            ...
            if (x <= maX && x >= miX && z <= maZ && z >= miZ) {
                ...
                for (float i = Math.max(this.rots[1] - 20.0f, -90.0f); i < Math.min(this.rots[1] + 20.0f, 90.0f); i += 0.05f) {
                    ...
                    final MovingObjectPosition m4 = NewScaffold.mc.thePlayer.customRayTrace(4.5, 1.0f, yaww, f[1]);
                    if (m4.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.isOkBlock(m4.getBlockPos()) && !movingObjectPositions.contains(m4) && m4.getBlockPos().equalsBlockPos(this.blockPos) && m4.sideHit != EnumFacing.DOWN && m4.sideHit != EnumFacing.UP && m4.getBlockPos().getY() <= b.getY()) {
    ...
    public void onEventClick(final EventClick eventClick) {
        ...
        if (this.blockPos != null && NewScaffold.mc.currentScreen == null) {
            ...
            final String selected;
            final String var24 = selected = this.mode.getSelected();
            switch (selected) {
                case "Basic": {
                    final double x = NewScaffold.mc.thePlayer.posX;
                    final double z = NewScaffold.mc.thePlayer.posZ;
