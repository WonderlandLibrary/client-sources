...
public class BlockFly extends Module
{
    ...
    public final BooleanValue snap;
    public final BooleanValue resetRotation;
    public final BooleanValue dynamicFov;
    ...
    public final BooleanValue godBridge;
    ...
    public final BooleanValue flyTest;
    ...
    public final BooleanValue autoJump;
    public final BooleanValue esp;
    public final StringValue towerMode;
    public final BooleanValue towerMove;
    ...
    private final HashMap<S23PacketBlockChange, Long> blockpackets;
    ...
    private TimeHelper placeTimer;
    private int placeDelay;
    private int offGroundTicks;
    private final HashMap<S32PacketConfirmTransaction, Long> transactions;
    ...
    public BlockFly() {
        ...
        this.minDelay = new DoubleValue(123, "MinDelay", this, 0.0, 0.0, 10000.0, 0);
        this.maxDelay = new DoubleValue(124, "MaxDelay", this, 0.0, 0.0, 10000.0, 0);
        this.snap = new BooleanValue(52789782, "Snap", this, false);
        this.resetRotation = new BooleanValue(52789782, "ResetRotation", this, false);
        this.dynamicFov = new BooleanValue(2643, "NoDynamicFov", this, false);
        ...
        this.godBridge = new BooleanValue(6234, "GodBridge", this, false);
        ...
        this.flyTest = new BooleanValue(42069, "FlyTest", this, false);
        ...
        this.autoJump = new BooleanValue(18, "AutoJump", this, false);
        this.esp = new BooleanValue(26, "ESP", this, true);
        this.towerMode = new StringValue(1234, "TowerMode", this, "None", new String[] { "None", "Vanilla", "LowHop", "FastJump", "NCP" });
        this.towerMove = new BooleanValue(25, "TowerMove", this, false);
        this.noSwing = new BooleanValue(25, "NoSwing", this, false);
        this.safewalk = new BooleanValue(7622, "SafeWalk", this, false);
        ...
        this.blockCounter = 0;
        this.blockpackets = new HashMap<S23PacketBlockChange, Long>();
        ...
        this.placeDelay = 0;
        this.transactions = new HashMap<S32PacketConfirmTransaction, Long>();
    }
    
    @Override
    public void onEnable() {
        super.onEnable();
        this.placeDelay = RandomUtil.nextInt((int)this.minDelay.getValue(), (int)this.maxDelay.getValue());
        this.dynamic = BlockFly.mc.gameSettings.ofDynamicFov;
        if (this.dynamicFov.getBoolean()) {
            BlockFly.mc.gameSettings.ofDynamicFov = false;
        }
        this.sneakCounter = 4;
        this.blockCounter = 0;
    ...
    public void onDisable() {
        super.onDisable();
        for (final Map.Entry<S23PacketBlockChange, Long> set : this.blockpackets.entrySet()) {
            BlockFly.mc.thePlayer.sendQueue.handleBlockChange(set.getKey());
        }
        for (final Map.Entry<S32PacketConfirmTransaction, Long> set2 : this.transactions.entrySet()) {
            BlockFly.mc.thePlayer.sendQueue.handleConfirmTransaction(set2.getKey());
        }
        this.transactions.clear();
        this.blockpackets.clear();
        if (this.dynamicFov.getBoolean()) {
            BlockFly.mc.gameSettings.ofDynamicFov = this.dynamic;
        }
        BlockFly.mc.getTimer().timerSpeed = 1.0f;
    ...
    private void checkS23Packets() {
        try {
            ...
            if (toremove != null) {
                this.blockpackets.remove(toremove);
            }
        }
    ...
    private void checkS32Packets() {
        try {
            ...
            for (final Map.Entry<S32PacketConfirmTransaction, Long> set : this.transactions.entrySet()) {
                if (set.getValue() + 1000L < System.currentTimeMillis()) {
                    BlockFly.mc.thePlayer.sendQueue.handleConfirmTransaction(set.getKey());
                    toremove = set.getKey();
                }
            }
            if (toremove != null) {
    ...
    @EventTarget
    public void onSafewalk(final EventSaveWalk eventSaveWalk) {
        if (this.safewalk.getBoolean()) {
            final Block blockUnder1 = BlockFly.mc.theWorld.getBlockState(new BlockPos(BlockFly.mc.thePlayer.posX, BlockFly.mc.thePlayer.posY - 1.0, BlockFly.mc.thePlayer.posZ)).getBlock();
            final Block blockUnder2 = BlockFly.mc.theWorld.getBlockState(new BlockPos(BlockFly.mc.thePlayer.posX, BlockFly.mc.thePlayer.posY - 2.0, BlockFly.mc.thePlayer.posZ)).getBlock();
    ...
    @EventTarget
    public void onPacketRecv(final EventReadPacket eventReadPacket) {
        final Packet packet = eventReadPacket.getPacket();
        if (this.flyTest.getBoolean()) {
            if (packet instanceof S23PacketBlockChange) {
                this.blockpackets.put((S23PacketBlockChange)packet, System.currentTimeMillis());
                eventReadPacket.setCanceled(true);
            }
            if (packet instanceof S32PacketConfirmTransaction) {
                this.transactions.put((S32PacketConfirmTransaction)packet, System.currentTimeMillis());
                eventReadPacket.setCanceled(true);
            }
            this.checkS23Packets();
        }
    }
    
    public void tower() {
        final String selected = this.towerMode.getSelected();
        switch (selected) {
            case "Vanilla": {
                BlockFly.mc.thePlayer.motionY = 0.5;
                break;
            }
            case "LowHop": {
                if (BlockFly.mc.thePlayer.onGround) {
                    BlockFly.mc.thePlayer.motionY = 0.3700000047683716;
                    break;
                }
                break;
            }
            case "FastJump": {
                if (BlockFly.mc.thePlayer.motionY >= 0.0) {
                    break;
                }
                BlockFly.mc.thePlayer.motionY = BlockFly.mc.thePlayer.getJumpUpwardsMotion();
                if (BlockFly.mc.thePlayer.isPotionActive(Potion.jump)) {
                    final EntityPlayerSP thePlayer = BlockFly.mc.thePlayer;
                    thePlayer.motionY += (BlockFly.mc.thePlayer.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1f;
                    break;
                }
                break;
            }
            case "NCP": {
                if (BlockFly.mc.thePlayer.posY % 1.0 <= 0.00153598) {
                    BlockFly.mc.thePlayer.setPosition(BlockFly.mc.thePlayer.posX, Math.floor(BlockFly.mc.thePlayer.posY), BlockFly.mc.thePlayer.posZ);
                    BlockFly.mc.thePlayer.motionY = 0.41998;
                    break;
                }
                if (BlockFly.mc.thePlayer.posY % 1.0 < 0.1 && this.offGroundTicks != 0) {
                    BlockFly.mc.thePlayer.setPosition(BlockFly.mc.thePlayer.posX, Math.floor(BlockFly.mc.thePlayer.posY), BlockFly.mc.thePlayer.posZ);
                    break;
                }
                break;
    ...
    public boolean isTower2() {
        return BlockFly.mc.gameSettings.keyBindJump.pressed && ((!BlockFly.mc.gameSettings.keyBindLeft.pressed && !BlockFly.mc.gameSettings.keyBindRight.pressed && !BlockFly.mc.gameSettings.keyBindForward.pressed && !BlockFly.mc.gameSettings.keyBindBack.pressed) || this.towerMove.getBoolean());
    }
    ...
    public void onEventEarlyTick(final EventEarlyTick eventEarlyTick) {
        ...
        if (this.shouldScaffold()) {
            if (!this.sameY.getBoolean() && this.isTower2()) {
                this.tower();
            }
            if (this.sprint.getBoolean()) {
                BlockFly.mc.gameSettings.keyBindSprint.pressed = true;
            }
            ...
            if (this.shouldBuild() || !this.snap.getBoolean()) {
                this.setRotation();
            }
            else if (this.resetRotation.getBoolean()) {
                (this.rots = this.lastRots)[0] = BlockFly.mc.thePlayer.rotationYaw;
                this.rots[1] = BlockFly.mc.thePlayer.rotationPitch;
                this.lastRots[0] = BlockFly.mc.thePlayer.prevRotationYaw;
                this.lastRots[1] = BlockFly.mc.thePlayer.prevRotationPitch;
                this.startTimeHelper.reset();
            }
            if (this.snap.getBoolean() && this.shouldBuild() && this.resetRotation.getBoolean() && this.shouldSneak()) {
                BlockFly.mc.thePlayer.movementInput.sneak = true;
            }
            else if (this.snap.getBoolean() && this.resetRotation.getBoolean()) {
                BlockFly.mc.thePlayer.movementInput.sneak = false;
            }
        }
    ...
    private float[] getRayCastRots() {
        ...
        if ((BlockFly.mc.thePlayer.motionX != 0.0 || BlockFly.mc.thePlayer.motionZ != 0.0 || !BlockFly.mc.thePlayer.onGround) && this.startTimeHelper.reached(200L)) {
            if (BlockFly.mc.thePlayer.motionX == 0.0 && BlockFly.mc.thePlayer.motionZ == 0.0 && BlockFly.mc.thePlayer.onGround) {
                this.startTimeHelper.reset();
            }
        }
        else {
        ...
        if (this.shouldBuild()) {
            ...
            for (float dist = 0.0f; dist < 180.0f; dist += difference) {
                ...
                if (dist > 10.0f) {
                    difference = (float)(difference + (double)(dist / 500.0f) + 0.01);
                }
            }
    ...
    private float[] getPlayerYawRotation() {
        ...
        final boolean godBridge = this.godBridge.getBoolean();
        float yaw = this.rots[0];
        final float yawSpeed = MathHelper.clamp_float((float)(this.yawSpeed.getValue() + RandomUtil.nextFloat(0.0f, 15.0f)), 0.0f, 180.0f);
        final float pitchSpeed = MathHelper.clamp_float((float)(this.pitchSpeed.getValue() + RandomUtil.nextFloat(0.0f, 15.0f)), 0.0f, 180.0f);
        if (this.isTower()) {
            ...
            if (objectPosition != null) {
                ...
                yaw = this.rotationUtil.rotateToYaw(pitchSpeed, this.rots, moonWalk ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 18.5f) : (godBridge ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 45.0f) : (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f)));
        ...
        if ((BlockFly.mc.thePlayer.motionX != 0.0 || BlockFly.mc.thePlayer.motionZ != 0.0 || !BlockFly.mc.thePlayer.onGround) && this.startTimeHelper.reached(200L)) {
            ...
            yaw = this.rotationUtil.rotateToYaw(yawSpeed, this.rots, moonWalk ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 18.5f) : (godBridge ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 45.0f) : (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f)));
            rotations = new float[] { yaw, pitch };
        }
        ...
        yaw = this.rotationUtil.rotateToYaw(yawSpeed, this.rots[0], moonWalk ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 18.5f) : (godBridge ? (Augustus.getInstance().getYawPitchHelper().realYaw - 180.0f + 45.0f) : (realYaw - 180.0f)));
        rotations[0] = yaw;
        if (this.shouldBuild()) {
    ...
    private EnumFacing getPlaceSide(final BlockPos blockPos) {
        ...
        if (BlockUtil.isAirBlock(blockPos.add(0, 1, 0)) && !blockPos.add(0, 1, 0).equalsBlockPos(playerPos) && !BlockFly.mc.thePlayer.onGround) {
            ...
            final Vec3 vec3 = this.getBestHitFeet(bp);
            positions.add(vec3);
        ...
        positions.sort(Comparator.comparingDouble(vec3x -> BlockFly.mc.thePlayer.getDistance(vec3x.xCoord, vec3x.yCoord, vec3x.zCoord)));
        if (!positions.isEmpty()) {
            final Vec3 vec4 = this.getBestHitFeet(this.b);
            if (BlockFly.mc.thePlayer.getDistance(vec4.xCoord, vec4.yCoord, vec4.zCoord) >= BlockFly.mc.thePlayer.getDistance(positions.get(0).xCoord, positions.get(0).yCoord, positions.get(0).zCoord)) {
    ...
    private BlockPos getBlockPos() {
        ...
        if (!positions.isEmpty()) {
            ...
            return (this.isTower() && hashMap.get(positions.get(0)).getY() != BlockFly.mc.thePlayer.posY - 1.5) ? new BlockPos(BlockFly.mc.thePlayer.posX, BlockFly.mc.thePlayer.posY - 1.5, BlockFly.mc.thePlayer.posZ) : hashMap.get(positions.get(0));
        }
        return null;
    }
