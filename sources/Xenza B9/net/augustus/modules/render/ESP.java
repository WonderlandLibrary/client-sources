...
public class ESP extends Module
{
    ...
    public ESP() {
        ...
        this.mode = new StringValue(7, "Mode", this, "Box", new String[] { "Box", "FakeCorner", "Fake2D", "Vanilla", "Other2D", "Hase" });
    ...
    private void render(final EntityLivingBase entity, float red, float green, float blue, final float alpha) {
        ...
        final Color other2dcolor = new Color(red, green, blue, alpha);
        final String selected;
        final String var11 = selected = this.mode.getSelected();
        switch (selected) {
            case "Hase": {
                final float x = (float)(entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * ESP.mc.getTimer().renderPartialTicks - ESP.mc.getRenderManager().getRenderPosX());
                final float y = (float)(entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * ESP.mc.getTimer().renderPartialTicks - ESP.mc.getRenderManager().getRenderPosY());
                final float z = (float)(entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * ESP.mc.getTimer().renderPartialTicks - ESP.mc.getRenderManager().getRenderPosZ());
                ...
                RenderUtil.drawCustomImage(width, -20.0f, 54, 92, new ResourceLocation("client/hase.png"));
            ...
            case "Box": {
                net.augustus.utils.RenderUtil.drawEntityESP(entity, red, green, blue, alpha, 1.0f, lineWidth);
                break;
            }
            case "FakeCorner": {
                net.augustus.utils.RenderUtil.drawCornerESP(entity, red, green, blue);
                break;
            }
            case "Fake2D": {
                net.augustus.utils.RenderUtil.drawFake2DESP(entity, red, green, blue);
                break;
            }
            case "Other2D": {
                final double x2 = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * ESP.mc.timer.renderPartialTicks - ESP.mc.getRenderManager().renderPosX;
                final double y2 = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * ESP.mc.timer.renderPartialTicks - ESP.mc.getRenderManager().renderPosY;
                final double z2 = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * ESP.mc.timer.renderPartialTicks - ESP.mc.getRenderManager().renderPosZ;
                GL11.glPushMatrix();
                GL11.glTranslated(x2, y2 - 0.2, z2);
