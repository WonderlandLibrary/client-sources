...
public class Trajectories extends Module
{
    ...
    public void onEvent3D(final EventRender3D eventRender3D) {
        ...
        if (itemStack != null && (itemStack.getItem() instanceof ItemSnowball || itemStack.getItem() instanceof ItemEgg || itemStack.getItem() instanceof ItemBow || itemStack.getItem() instanceof ItemEnderPearl)) {
            ...
            final float ff = MathHelper.sqrt_double(motionX * motionX + motionY * motionY + motionZ * motionZ);
            double x = motionX / ff;
            double y = motionY / ff;
            double z = motionZ / ff;
            ...
            motionY = y;
            motionZ = z;
            float prevRotationYaw = (float)(MathHelper.func_181159_b(x, z) * 180.0 / 3.141592653589793);
            float prevRotationPitch = (float)(MathHelper.func_181159_b(y, MathHelper.sqrt_double(x * x + z * z)) * 180.0 / 3.141592653589793);
            boolean b = true;
            int ticksInAir = 0;
            while (b) {
                if (ticksInAir > 300) {
