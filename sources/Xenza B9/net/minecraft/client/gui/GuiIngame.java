...
public class GuiIngame extends Gui implements MM
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1) {
            ...
            if (GuiIngame.mm.crossHair.isToggled()) {
    ...
    protected void renderTooltip(final ScaledResolution sr, final float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if (GuiIngame.mm.hud.isToggled() && GuiIngame.mm.hud.hotBar.getBoolean()) {
                RenderUtil.drawFloatRect(0.0f, (float)sr.getScaledHeight(), (float)sr.getScaledWidth(), (float)(sr.getScaledHeight() - 22), new Color(0, 0, 0, 185).getRGB());
                RenderUtil.drawFloatRect((float)(i - 90.3 + entityplayer.inventory.currentItem * 20), (float)(sr.getScaledHeight() - 22), (float)(i - 68.9 + entityplayer.inventory.currentItem * 20), (float)sr.getScaledHeight(), new Color(222, 222, 222, 185).getRGB());
                RenderUtil.drawFloatRect((float)(sr.getScaledWidth() - 5), (float)(sr.getScaledHeight() - 22), (float)sr.getScaledWidth(), (float)sr.getScaledHeight(), new Color(255, 255, 255, 200).getRGB());
    ...
    private void renderScoreboard(final ScoreObjective scoreObjective, final ScaledResolution sr) {
        if (!GuiIngame.mm.scoreboard.remove.getBoolean() || !GuiIngame.mm.scoreboard.isToggled()) {
            final Scoreboard scoreboard = scoreObjective.getScoreboard();
            final Collection collection = scoreboard.getSortedScores(scoreObjective);
            ...
            if (GuiIngame.mm.scoreboard.isToggled()) {
                if (GuiIngame.mm.scoreboard.stick.getBoolean()) {
                    k1 = (int)(j1 + GuiIngameHook.lastArrayListY + 2.0f);
                }
                else {
                    k1 = (int)((sr.getScaledHeight() - j1) * (GuiIngame.mm.scoreboard.yCord.getValue() / 100.0)) + j1;
                }
            }
            final byte b0 = 3;
            final int l = GuiIngame.mm.scoreboard.isToggled() ? ((int)((sr.getScaledWidth() - i - b0) * (GuiIngame.mm.scoreboard.xCord.getValue() / 100.0))) : (sr.getScaledWidth() - i - b0);
            int m = 0;
            for (final Object score2 : arraylist2) {
                ...
                final String s3 = EnumChatFormatting.RED + "" + ((Score)score2).getScorePoints();
                final int l2 = k1 - m * this.getFontRenderer().FONT_HEIGHT;
                final int i2 = GuiIngame.mm.scoreboard.isToggled() ? (sr.getScaledWidth() - (sr.getScaledWidth() - i - b0 - (int)((sr.getScaledWidth() - i - b0) * (GuiIngame.mm.scoreboard.xCord.getValue() / 100.0)))) : (sr.getScaledWidth() - b0 + 2);
                ...
                if (m == arraylist2.size()) {
                    final String s4 = scoreObjective.getDisplayName();
