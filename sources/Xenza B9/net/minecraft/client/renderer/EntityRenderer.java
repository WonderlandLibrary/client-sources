...
public class EntityRenderer implements IResourceManagerReloadListener, MM
{
    ...
    public void loadShader(final ResourceLocation resourceLocationIn) {
        if (OpenGlHelper.isFramebufferEnabled()) {
            try {
                ...
                EntityRenderer.logger.warn("Failed to load shader: " + resourceLocationIn, var3);
                this.shaderIndex = EntityRenderer.shaderCount;
                this.useShader = false;
            }
            catch (final JsonSyntaxException var4) {
                EntityRenderer.logger.warn("Failed to load shader: " + resourceLocationIn, (Throwable)var4);
                this.shaderIndex = EntityRenderer.shaderCount;
                this.useShader = false;
            }
        }
    ...
    public void getMouseOver(final float partialTicks) {
        Label_1440: {
            if (EntityRenderer.mm.killAura.isToggled()) {
                final KillAura killAura = EntityRenderer.mm.killAura;
                if (KillAura.target != null) {
                    ...
                    if (entity != null && this.mc.theWorld != null) {
                        ...
                        d0 = Math.max(d0, EntityRenderer.mm.killAura.range);
                        ...
                        d0 = Math.max(d0, EntityRenderer.mm.killAura.range);
                        d2 = Math.max(d2, EntityRenderer.mm.killAura.range);
                        ...
                        Vec3 vec6 = null;
                        final float f = 1.0f;
                        final List list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec4.xCoord * d0, vec4.yCoord * d0, vec4.zCoord * d0).expand(f, f, f), Predicates.and((Predicate<? super Entity>)EntitySelectors.NOT_SPECTATING, (Predicate<? super Entity>)new EntityRenderer11(this)));
                        double d3 = d2;
                        ...
                        if (this.pointedEntity != null && flag && vec3.distanceTo(vec6) > EntityRenderer.mm.killAura.range) {
                            this.pointedEntity = null;
            ...
            if (entity != null && this.mc.theWorld != null) {
                ...
                this.mc.pointedEntity = null;
                double d0 = this.mc.playerController.getBlockReachDistance();
                if (EntityRenderer.mm.teleport.isToggled()) {
                    d0 = 200.0;
                }
                this.mc.objectMouseOver = entity.rayTrace(d0, partialTicks);
                double d2 = d0;
                ...
                for (int j = 0; j < list.size(); ++j) {
                    ...
                    final AxisAlignedBB axisalignedbb2 = entity3.getEntityBoundingBox().expand(f3, f3, f3);
                    final MovingObjectPosition movingobjectposition2 = axisalignedbb2.calculateIntercept(vec3, vec5);
                    if (axisalignedbb2.isVecInside(vec3)) {
                        if (d3 >= 0.0) {
                            this.pointedEntity = entity3;
                            vec6 = ((movingobjectposition2 == null) ? vec3 : movingobjectposition2.hitVec);
                            d3 = 0.0;
                        }
                    }
                    else if (movingobjectposition2 != null) {
                        final double d5 = vec3.distanceTo(movingobjectposition2.hitVec);
                        if (d5 < d3 || d3 == 0.0) {
                            boolean flag4 = false;
                            if (Reflector.ForgeEntity_canRiderInteract.exists()) {
                                flag4 = Reflector.callBoolean(entity3, Reflector.ForgeEntity_canRiderInteract, new Object[0]);
                            }
                            if (entity3 != entity.ridingEntity || flag4) {
                                this.pointedEntity = entity3;
                                vec6 = movingobjectposition2.hitVec;
                                d3 = d5;
                            }
                            else if (d3 == 0.0) {
                                this.pointedEntity = entity3;
                ...
                final EventRayCastPost eventRayCastPost = new EventRayCastPost(partialTicks);
                EventHandler.call(eventRayCastPost);
                this.mc.mcProfiler.endSection();
            }
        }
        final EventRayCast eventRayCast = new EventRayCast(partialTicks);
    ...
    public void func_181560_a(final float p_181560_1_, final long p_181560_2_) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            final float f = this.mc.gameSettings.mouseSensitivity * 0.6f + 0.2f;
            final float f2 = f * f * f * 8.0f;
            final EventMouseMoving eventMouseMoving = new EventMouseMoving(this.mc.mouseHelper.deltaX, this.mc.mouseHelper.deltaY);
            EventHandler.call(eventMouseMoving);
            float f3 = eventMouseMoving.getDeltaX() * f2;
            float f4 = eventMouseMoving.getDeltaY() * f2;
            byte b0 = 1;
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    final CrashReport crashreport = CrashReport.makeCrashReport(var16, "Rendering screen");
    ...
    private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
        ...
        GlStateManager.enableBlend();
        final EventRender3D eventRender3D = new EventRender3D();
