...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                RendererLivingEntity.logger.error("Couldn't render entity", var19);
            }
    ...
    protected boolean setScoreTeamColor(final EntityLivingBase entityLivingBaseIn) {
        ...
        GlStateManager.setActiveTexture(OpenGlHelper.defaultTexUnit);
        float f4 = 1.0f;
        if (RendererLivingEntity.mm.esp.isToggled() && !RendererLivingEntity.mm.esp.teamColor.getBoolean()) {
            f1 = (RendererLivingEntity.mm.esp.rainbow.getBoolean() ? (RendererLivingEntity.mm.esp.getRainbowUtil().getColor().getRed() / 255.0f) : (RendererLivingEntity.mm.esp.outlineColor.getColor().getRed() / 255.0f));
            f2 = (RendererLivingEntity.mm.esp.rainbow.getBoolean() ? (RendererLivingEntity.mm.esp.getRainbowUtil().getColor().getGreen() / 255.0f) : (RendererLivingEntity.mm.esp.outlineColor.getColor().getGreen() / 255.0f));
            f3 = (RendererLivingEntity.mm.esp.rainbow.getBoolean() ? (RendererLivingEntity.mm.esp.getRainbowUtil().getColor().getBlue() / 255.0f) : (RendererLivingEntity.mm.esp.outlineColor.getColor().getBlue() / 255.0f));
            f4 = (RendererLivingEntity.mm.esp.rainbow.getBoolean() ? (RendererLivingEntity.mm.esp.getRainbowUtil().getColor().getAlpha() / 255.0f) : (RendererLivingEntity.mm.esp.outlineColor.getColor().getAlpha() / 255.0f));
        }
    ...
    public void renderName(final T entity, final double x, final double y, final double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity)) {
                ...
                final boolean b = entity instanceof EntityPlayer && RendererLivingEntity.mm.nameTags.isToggled();
                if (d0 < f * f || b) {
                    String s = entity.getDisplayName().getFormattedText();
                    if (entity instanceof EntityPlayer && MidClick.friends.contains(entity.getName())) {
                        s = "§7[§2Friend§7] " + entity.getDisplayName().getFormattedText();
                    }
