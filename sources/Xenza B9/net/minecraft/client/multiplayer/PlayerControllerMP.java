...
public class PlayerControllerMP implements MM
{
    ...
    public boolean clickBlock(final BlockPos loc, final EnumFacing face, final ItemStack itemstack) {
        if (this.currentGameType.isAdventure()) {
            if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
                return false;
            }
    ...
    public void resetBlockRemoving(final boolean b) {
        if (b) {
            this.netClientHandler.addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.ABORT_DESTROY_BLOCK, this.currentBlock, EnumFacing.DOWN));
            this.curBlockDamageMP = 0.0f;
            this.mc.theWorld.sendBlockBreakProgress(this.mc.thePlayer.getEntityId(), this.currentBlock, -1);
    ...
    public boolean onPlayerDamageBlock(final BlockPos posBlock, final EnumFacing directionFacing) {
        ...
        if (!PlayerControllerMP.mm.fastBreak.isToggled() || this.curBlockDamageMP < 0.0f) {
            this.curBlockDamageMP += block.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock);
        }
        else if (PlayerControllerMP.mm.fastBreak.instant.getBoolean()) {
            this.curBlockDamageMP = 2.0f;
        }
        else {
            this.curBlockDamageMP += (float)(block.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock) * PlayerControllerMP.mm.fastBreak.multiplier.getValue());
        }
    ...
    public boolean onPlayerDamageBlockCustom(final BlockPos posBlock, final EnumFacing directionFacing, final int id) {
        this.syncCurrentPlayItem();
        if (this.blockHitDelay > 0) {
            --this.blockHitDelay;
            return true;
    ...
    public void syncCurrentPlayItem() {
        ...
        if (PlayerControllerMP.mm.autoTool.isToggled() && PlayerControllerMP.mm.autoTool.getSlot() >= 0) {
            i = PlayerControllerMP.mm.autoTool.getSlot();
        }
        if (PlayerControllerMP.mm.fucker.isToggled() && PlayerControllerMP.mm.fucker.b != null) {
            i = PlayerControllerMP.mm.fucker.getSlotID();
        }
        if (PlayerControllerMP.mm.blockFly.isToggled() && PlayerControllerMP.mm.blockFly.silentMode.getSelected().equals("Spoof")) {
            i = PlayerControllerMP.mm.blockFly.getSlotID();
        }
