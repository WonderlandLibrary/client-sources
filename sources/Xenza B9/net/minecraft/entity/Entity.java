...
public abstract class Entity implements ICommandSender
{
    ...
    public void setAngles(final float yaw, final float pitch) {
        final float f = Augustus.getInstance().getYawPitchHelper().realPitch;
        final float f2 = Augustus.getInstance().getYawPitchHelper().realYaw;
        Augustus.getInstance().getYawPitchHelper().realYaw += (float)(yaw * 0.15);
        Augustus.getInstance().getYawPitchHelper().realPitch -= (float)(pitch * 0.15);
        ...
        final YawPitchHelper yawPitchHelper;
        YawPitchHelper var10000 = yawPitchHelper = Augustus.getInstance().getYawPitchHelper();
        yawPitchHelper.realLastPitch += Augustus.getInstance().getYawPitchHelper().realPitch - f;
        final YawPitchHelper yawPitchHelper2;
        var10000 = (yawPitchHelper2 = Augustus.getInstance().getYawPitchHelper());
        yawPitchHelper2.realLastYaw += Augustus.getInstance().getYawPitchHelper().realYaw - f2;
    ...
    public void moveEntity(double x, double y, double z) {
        final EventSaveWalk eventSaveWalk = new EventSaveWalk(x, y, z, this);
        ...
        if (this.noClip) {
            ...
            if (this.isInWeb) {
                if (Minecraft.getMinecraft().thePlayer == this) {
                    final EventWeb eventWeb = new EventWeb(0.25, 0.05000000074505806, 0.25);
            ...
            final boolean flag = ((this.onGround && sneak) || eventSaveWalk.isSaveWalk()) && this instanceof EntityPlayer;
            if (flag) {
            ...
            final EventPreStep eventPreStep = new EventPreStep(this.stepHeight);
            if (this instanceof EntityPlayerSP) {
                EventHandler.call(eventPreStep);
            }
            if (eventPreStep.getStepHeight() > 0.0f && flag2 && (d4 != x || d6 != z)) {
                final double d8 = x;
                final double d9 = y;
                final double d10 = z;
                final AxisAlignedBB axisalignedbb5 = this.getEntityBoundingBox();
                this.setEntityBoundingBox(axisalignedbb);
                y = eventPreStep.getStepHeight();
                ...
                if (this instanceof EntityPlayerSP) {
                    final EventPostStep eventPostStep = new EventPostStep();
            ...
            catch (final Throwable var55) {
                final CrashReport crashreport = CrashReport.makeCrashReport(var55, "Checking entity block collision");
                ...
                for (int j = blockpos.getY(); j <= blockpos2.getY(); ++j) {
                    for (int k = blockpos.getZ(); k <= blockpos2.getZ(); ++k) {
                        ...
                        try {
                            ...
                            final CrashReport crashreport = CrashReport.makeCrashReport(var11, "Colliding entity with block");
    ...
    public void moveFlying(float strafe, float forward, final float friction) {
        final EventMove eventMove = new EventMove(this.rotationYaw, friction);
        ...
        if (!eventMove.isCanceled()) {
            ...
            if (f >= 1.0E-4f) {
                ...
                strafe *= f;
                forward *= f;
                final float f2 = MathHelper.sin(eventMove.getYaw() * 3.1415927f / 180.0f);
                final float f3 = MathHelper.cos(eventMove.getYaw() * 3.1415927f / 180.0f);
    ...
    public void writeToNBT(final NBTTagCompound tagCompund) {
        try {
            ...
            final CrashReport crashreport = CrashReport.makeCrashReport(var5, "Saving entity NBT");
    ...
    public void readFromNBT(final NBTTagCompound tagCompund) {
        try {
            ...
            final CrashReport crashreport = CrashReport.makeCrashReport(var5, "Loading entity NBT");
