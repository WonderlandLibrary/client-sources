...
public abstract class EntityLivingBase extends Entity implements MM
{
    ...
    protected void damageEntity(final DamageSource damageSrc, float damageAmount) {
        if (!this.isEntityInvulnerable(damageSrc)) {
            ...
            final float var7 = Math.max(damageAmount - this.getAbsorptionAmount(), 0.0f);
    ...
    private int getArmSwingAnimationEnd() {
        int i = this.isPotionActive(Potion.digSpeed) ? (6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1) : (this.isPotionActive(Potion.digSlowdown) ? (6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2) : 6);
        if (EntityLivingBase.mm.blockAnimation.isToggled()) {
            i /= (int)EntityLivingBase.mm.blockAnimation.speed.getValue();
        }
        return i;
    }
    ...
    public void swingItem() {
        if (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2 || this.swingProgressInt < 0) {
            final EventSwingItemClientSide eventSwingItemClientSide = new EventSwingItemClientSide();
            EventHandler.call(eventSwingItemClientSide);
            if (!eventSwingItemClientSide.cancel) {
    ...
    protected void jump() {
        if (this == Minecraft.getMinecraft().thePlayer) {
            final EventJump eventJump = new EventJump(this.rotationYaw);
            EventHandler.call(eventJump);
            this.motionY = this.getJumpUpwardsMotion();
            if (this.isPotionActive(Potion.jump)) {
                this.motionY += (this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1f;
            }
            if (this.isSprinting() || (EntityLivingBase.mm.sprint.allSprint && EntityLivingBase.mm.sprint.isToggled())) {
                final Minecraft mc = Minecraft.getMinecraft();
                if (EntityLivingBase.mm.sprint.allSprint && EntityLivingBase.mm.sprint.isToggled()) {
                    if (mc.thePlayer.moveForward < 0.0f) {
                        eventJump.setYaw(eventJump.getYaw() - 180.0f);
                    }
                    if (mc.thePlayer.moveStrafing != 0.0f && mc.thePlayer.moveForward == 0.0f) {
                        if (mc.thePlayer.moveStrafing > 0.0f) {
                            eventJump.setYaw(eventJump.getYaw() - 45.0f);
                        }
                ...
                final float f = eventJump.getYaw() * 0.017453292f;
                if (!EntityLivingBase.mm.speed.isToggled() || !EntityLivingBase.mm.speed.mode.getSelected().equals("Matrix")) {
                    this.motionX -= MathHelper.sin(f) * 0.2f;
                    this.motionZ += MathHelper.cos(f) * 0.2f;
                }
                else if (mc.thePlayer.movementInput.moveForward > 0.0f && mc.thePlayer.movementInput.moveStrafe != 0.0f) {
                    this.motionX -= MathHelper.sin(f) * 0.22f;
                    this.motionZ += MathHelper.cos(f) * 0.22f;
                }
                else {
                    this.motionX -= MathHelper.sin(f) * 0.202f;
                    this.motionZ += MathHelper.cos(f) * 0.202f;
                }
            }
        }
        else {
            ...
            if (this.isSprinting()) {
                final float f2 = this.rotationYaw * 0.017453292f;
            ...
            if (!this.isInWater() || (this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)) {
                if (!this.isInLava() || (this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)) {
                    ...
                    if (this.onGround) {
                        ...
                        if (this == Minecraft.getMinecraft().thePlayer && Augustus.getInstance().getModuleManager().sprint.isToggled() && Augustus.getInstance().getModuleManager().sprint.allDirection.getBoolean() && EntityLivingBase.mm.sprint.allSprint) {
                            f6 = 0.12999998f;
                        }
                    }
                    else {
                    ...
                    if (this.isCollidedHorizontally && this.isOnLadder()) {
                        final EventLadder eventLadder = new EventLadder(0.2);
                        EventHandler.call(eventLadder);
                        this.motionY = eventLadder.getMotionYSpeed();
                    }
    ...
    public void onUpdate() {
        ...
        if (this instanceof EntityPlayerSP) {
            final YawPitchHelper yp = Augustus.getInstance().getYawPitchHelper();
            while (yp.realYaw - yp.realLastYaw >= 180.0f) {
                final YawPitchHelper yawPitchHelper = yp;
                yawPitchHelper.realLastYaw += 360.0f;
            }
            while (yp.realPitch - yp.realLastPitch >= 180.0f) {
                final YawPitchHelper yawPitchHelper2 = yp;
                yawPitchHelper2.realLastPitch += 360.0f;
            }
        }
        this.worldObj.theProfiler.endSection();
    ...
    public void onLivingUpdate() {
        ...
        if (ViaMCP.getInstance().getVersion() < 335) {
    ...
    public Vec3 getLook(final float partialTicks) {
        if (this instanceof EntityPlayerSP && EntityLivingBase.mm.fixes.isToggled() && EntityLivingBase.mm.fixes.mouseDelayFix.getBoolean()) {
