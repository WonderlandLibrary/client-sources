...
public class EntityTracker extends StoredObject implements ClientEntityIdChangeListener
{
    ...
    public void removeEntity(final int entityId) {
        ...
        if (this.playersByEntityId.containsKey(entityId)) {
            final UUID playerId = this.playersByEntityId.remove(entityId);
            this.playersByUniqueId.remove(playerId);
            this.playerEquipment.remove(playerId);
        }
    }
    ...
    public Item getPlayerEquipment(final UUID uuid, final int slot) {
        final Item[] items = this.playerEquipment.get(uuid);
    ...
    public void setPlayerEquipment(final UUID uuid, final Item equipment, final int slot) {
        final Item[] items = this.playerEquipment.computeIfAbsent(uuid, it -> new Item[5]);
