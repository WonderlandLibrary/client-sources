...
public class ConnectedProperties {
    ...
    private String[] parseTileNames(String str) {
        ...
        iconStrs = Config.tokenize(str, " ,");
        names = 0;
        while (names < iconStrs.length) {
            i = iconStrs[names];
            if (!i.contains("-") || (iconName = Config.tokenize(i, "-")).length != 2) ** GOTO lbl16
            pathBlocks = Config.parseInt(iconName[0], -1);
            max = Config.parseInt(iconName[1], -1);
            if (pathBlocks < 0 || max < 0) ** GOTO lbl16
            if (pathBlocks > max) ** GOTO lbl14
            ** GOTO lbl18
lbl14: // 1 sources:
...
lbl16: // 2 sources:
            ...
            ** GOTO lbl21
lbl18: // 2 sources:
            for (n = pathBlocks; n <= max; ++n) {
                list.add(String.valueOf(n));
            }
lbl21: // 3 sources:
            ++names;
        }
        var10 = list.toArray(new String[list.size()]);
        var11 = 0;
