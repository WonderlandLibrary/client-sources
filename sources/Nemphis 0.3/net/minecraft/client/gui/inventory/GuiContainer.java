...
extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        int var8 = 0;
        while (var8 < this.inventorySlots.inventorySlots.size()) {
    ...
    private Slot getSlotAtPosition(int x, int y) {
        int var3 = 0;
        while (var3 < this.inventorySlots.inventorySlots.size()) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        this.doubleClick = this.lastClickSlot == var5 && var6 - this.lastClickTime < 250 && this.lastClickButton == mouseButton;
            ...
            if (clickedMouseButton == 0 || clickedMouseButton == 1) {
                if (this.draggedStack == null) {
                    ...
                    if (this.currentDragTargetSlot == var6) {
                        if (var8 - this.dragItemDropDelay > 500) {
                            this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, 0, 0);
                            this.handleMouseClick(var6, var6.slotNumber, 1, 0);
                            this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, 0, 0);
                            this.dragItemDropDelay = var8 + 750;
    ...
    protected boolean isPointInRegion(int left, int top, int right, int bottom, int pointX, int pointY) {
        ...
        if ((pointX -= var7) >= left - 1 && pointX < left + right + 1 && pointY >= top - 1 && (pointY -= var8) < top + bottom + 1) {
            return true;
        }
        return false;
    }
