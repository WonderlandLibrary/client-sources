...
public class BlockPumpkin extends BlockDirectional
{
    ...
    protected BlockPattern getSnowmanBasePattern() {
        if (this.snowmanBasePattern == null) {
            this.snowmanBasePattern = FactoryBlockPattern.start().aisle(" ", "#", "#").where('#', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.snow))).build();
        }
    ...
    protected BlockPattern getSnowmanPattern() {
        if (this.snowmanPattern == null) {
            this.snowmanPattern = FactoryBlockPattern.start().aisle("^", "#", "#").where('^', BlockWorldState.hasState(BlockPumpkin.field_181085_Q)).where('#', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.snow))).build();
        }
    ...
    protected BlockPattern getGolemBasePattern() {
        if (this.golemBasePattern == null) {
            this.golemBasePattern = FactoryBlockPattern.start().aisle("~ ~", "###", "~#~").where('#', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.iron_block))).where('~', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.air))).build();
        }
    ...
    protected BlockPattern getGolemPattern() {
        if (this.golemPattern == null) {
            this.golemPattern = FactoryBlockPattern.start().aisle("~^~", "###", "~#~").where('^', BlockWorldState.hasState(BlockPumpkin.field_181085_Q)).where('#', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.iron_block))).where('~', BlockWorldState.hasState((Predicate<IBlockState>)BlockStateHelper.forBlock(Blocks.air))).build();
        }
