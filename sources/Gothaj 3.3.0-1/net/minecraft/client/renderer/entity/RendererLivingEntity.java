...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
         ...
         try {
            ...
            if (entity == Minecraft.getMinecraft().thePlayer) {
               if (!RotationUtils.customRots) {
                  ...
                  float f = MathHelper.wrapAngleTo180_float(event.getYaw() - Minecraft.getMinecraft().thePlayer.renderYawOffset);
                  Minecraft.getMinecraft().thePlayer.renderYawOffset += f * 0.3F;
                  float f1 = MathHelper.wrapAngleTo180_float(event.getYaw() - Minecraft.getMinecraft().thePlayer.prevRenderYawOffset);
                  if (!(f1 < -90.0F) && !(f1 >= 90.0F)) {
                     boolean var32 = false;
                  } else {
                     boolean var31 = true;
            ...
            float fx = yaw;
            float f1x = this.interpolateRotation(entity.prevRotationYawHead, entity.rotationYawHead, partialTicks);
            float f2 = f1x - yaw;
            if (this.mainModel.isRiding && entity.ridingEntity instanceof EntityLivingBase) {
               EntityLivingBase entitylivingbase = (EntityLivingBase)entity.ridingEntity;
               fx = this.interpolateRotation(entitylivingbase.prevRenderYawOffset, entitylivingbase.renderYawOffset, partialTicks);
               f2 = f1x - fx;
            ...
            this.rotateCorpse(entity, f8, fx, partialTicks);
            ...
            if (this.renderOutlines) {
               ...
               if (!entity.getName().equals("")
                  && Client.INSTANCE.getModuleManager().getModule(ChamsModule.class).isEnabled()
                  ...
                  RenderUtils.color(
                     ColorUtils.getColor(
                        ((ChamsModule)Client.INSTANCE.getModuleManager().getModule(ChamsModule.class)).color,
                        (double)((float)System.nanoTime() / 1000000.0F),
                        0.0,
                        5.0
                     )
                  );
         ...
         if (this.canRenderName(entity)) {
            ...
            if (d0 < (double)(f * f)) {
               ...
               if (entity.isSneaking()) {
                  ...
                  GlStateManager.enableTexture2D();
                  GlStateManager.depthMask(true);
                  fontrenderer.drawString(s, (double)(-fontrenderer.getStringWidth(s) / 2), 0.0, 553648127);
                  GlStateManager.enableLighting();
                  GlStateManager.disableBlend();
                  GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                  GlStateManager.popMatrix();
   ...
   public static boolean setShaderBrightness(Color color) {
      ...
      GL11.glTexEnvi(8960, OpenGlHelper.GL_SOURCE0_ALPHA, OpenGlHelper.GL_PREVIOUS);
      GL11.glTexEnvi(8960, OpenGlHelper.GL_OPERAND0_ALPHA, 770);
      ((Buffer)shaderBrightnessBuffer).position(0);
      ...
      ((Buffer)shaderBrightnessBuffer).flip();
