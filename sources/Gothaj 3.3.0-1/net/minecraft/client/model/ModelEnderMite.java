...
public class ModelEnderMite extends ModelBase {
   ...
   public void setRotationAngles(
      ...
      for (int i = 0; i < this.field_178713_d.length; i++) {
         this.field_178713_d[i].rotateAngleY = MathHelper.cos(ageInTicks * 0.9F + (float)i * 0.15F * (float) Math.PI)
            * (float) Math.PI
            * 0.01F
            * (float)(1 + Math.abs(i - 2));
         this.field_178713_d[i].rotationPointX = MathHelper.sin(ageInTicks * 0.9F + (float)i * 0.15F * (float) Math.PI)
            * (float) Math.PI
            * 0.1F
            * (float)Math.abs(i - 2);
      }
