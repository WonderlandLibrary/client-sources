...
public class GuiStreamOptions extends GuiScreen {
   ...
   public void initGui() {
      ...
      for (GameSettings.Options gamesettings$options : field_152312_a) {
         if (gamesettings$options.getEnumFloat()) {
            this.buttonList
               .add(
                  new GuiOptionSlider(
                     gamesettings$options.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), gamesettings$options
                  )
               );
         } else {
            this.buttonList
      ...
      for (GameSettings.Options gamesettings$options1 : field_152316_f) {
         if (gamesettings$options1.getEnumFloat()) {
            this.buttonList
               .add(
                  new GuiOptionSlider(
                     gamesettings$options1.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), gamesettings$options1
                  )
               );
         } else {
            this.buttonList
      ...
      guibutton.enabled = this.mc.getTwitchStream().isReadyToBroadcast() && this.mc.getTwitchStream().func_152925_v().length > 0
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      if (this.field_152315_t) {
         this.drawCenteredString(
            ...
            EnumChatFormatting.RED + I18n.format("options.stream.changes"),
            this.width / 2,
            20 + this.fontRendererObj.FONT_HEIGHT,
            16777215
         );
      }
