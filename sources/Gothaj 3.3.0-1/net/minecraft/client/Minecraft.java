...
public class Minecraft implements IThreadListener, IPlayerUsage {
   ...
   private xyz.cucumber.base.utils.Timer checkTimer = new xyz.cucumber.base.utils.Timer();
   ...
   public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue, int alpha) {
      ...
      worldrenderer.pos((double)(posX + width), (double)posY, 0.0)
         ...
         .color(red, green, blue, alpha)
         .endVertex();
      worldrenderer.pos((double)posX, (double)posY, 0.0).tex((double)((float)texU * f), (double)((float)texV * f1)).color(red, green, blue, alpha).endVertex();
   ...
   private void displayDebugInfo(long elapsedTicksTime) {
      if (this.mcProfiler.profilingEnabled) {
         ...
         int l2 = 16777215;
         this.fontRendererObj.drawStringWithShadow(s, (double)((float)(j - i)), (double)((float)(k - i / 2 - 16)), l2);
         this.fontRendererObj
            .drawStringWithShadow(
               s = decimalformat.format(profiler$result.field_76330_b) + "%",
               (double)((float)(j + i - this.fontRendererObj.getStringWidth(s))),
               (double)((float)(k - i / 2 - 16)),
               l2
            );
         ...
         for (int k2 = 0; k2 < list.size(); k2++) {
            ...
            this.fontRendererObj.drawStringWithShadow(s1, (double)((float)(j - i)), (double)((float)(k + i / 2 + k2 * 8 + 20)), profiler$result2.getColor());
            this.fontRendererObj
               .drawStringWithShadow(
                  s1 = decimalformat.format(profiler$result2.field_76332_a) + "%",
                  (double)((float)(j + i - 50 - this.fontRendererObj.getStringWidth(s1))),
                  (double)((float)(k + i / 2 + k2 * 8 + 20)),
                  profiler$result2.getColor()
               );
            this.fontRendererObj
               .drawStringWithShadow(
                  s1 = decimalformat.format(profiler$result2.field_76330_b) + "%",
                  (double)((float)(j + i - this.fontRendererObj.getStringWidth(s1))),
                  (double)((float)(k + i / 2 + k2 * 8 + 20)),
                  profiler$result2.getColor()
               );
         }
   ...
   public void runTick() throws IOException {
      ...
      if (this.currentScreen == null || this.currentScreen.allowUserInput) {
         ...
         while (Keyboard.next()) {
            ...
            if (Keyboard.getEventKeyState()) {
               ...
               if (this.currentScreen != null) {
                  ...
                  EventKeyPress event = new EventKeyPress(k);
                  Client.INSTANCE.getEventBus().call(event);
                  KeyHook.handle(k);
                  if (k == 1) {
         ...
         EventClick eventx = new EventClick();
         Client.INSTANCE.getEventBus().call(eventx);
   ...
   public static int getGLMaximumTextureSize() {
      for (int i = 16384; i > 0; i >>= 1) {
         GL11.glTexImage2D(32868, 0, 6408, i, i, 0, 6408, 5121, null);
   ...
   public static void stopIntegratedServer() {
      if (theMinecraft != null) {
         IntegratedServer integratedserver = getMinecraft().getIntegratedServer();
