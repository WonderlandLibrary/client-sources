...
public class EntityGuardian extends EntityMob {
   ...
   public void onLivingUpdate() {
            ...
            for (int i = 0; i < 2; i++) {
               this.worldObj
                  .spawnParticle(
                     ...
                     this.posX + (this.rand.nextDouble() - 0.5) * (double)this.width - vec3.xCoord * 1.5,
                     this.posY + this.rand.nextDouble() * (double)this.height - vec3.yCoord * 1.5,
                     this.posZ + (this.rand.nextDouble() - 0.5) * (double)this.width - vec3.zCoord * 1.5,
                     0.0,
                     0.0,
                     0.0
                  );
      ...
      if (this.inWater) {
         ...
         this.motionY += 0.5;
         this.motionX = this.motionX + (double)((this.rand.nextFloat() * 2.0F - 1.0F) * 0.4F);
         this.motionZ = this.motionZ + (double)((this.rand.nextFloat() * 2.0F - 1.0F) * 0.4F);
         ...
         if (!this.theEntity.canEntityBeSeen(entitylivingbase)) {
            ...
            if (this.tickCounter == 0) {
               ...
               entitylivingbase.attackEntityFrom(
                  ...
                  (float)this.theEntity.getEntityAttribute(SharedMonsterAttributes.attackDamage).getAttributeValue()
               );
               this.theEntity.setAttackTarget(null);
   ...
   static class GuardianMoveHelper extends EntityMoveHelper {
      ...
      public void onUpdateMoveHelper() {
         if (this.update && !this.entityGuardian.getNavigator().noPath()) {
            ...
            this.entityGuardian.motionY = this.entityGuardian.motionY + (double)this.entityGuardian.getAIMoveSpeed() * d1 * 0.1;
