...
public abstract class EntityPlayer extends EntityLivingBase {
   ...
   public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
      if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
         ...
         if (f > 0.0F || f2 > 0.0F) {
            ...
            boolean flag3 = targetEntity.attackEntityFrom(DamageSource.causePlayerDamage(this), f);
            if (flag3) {
               ...
               float yaw = RotationUtils.customRots ? RotationUtils.serverYaw : this.rotationYaw;
               if (i > 0) {
                  if (!eventAttackSlowdown.isCancelled()) {
                     for (int power = 0; power < eventAttackSlowdown.getPower(); power++) {
                        targetEntity.addVelocity(
                           (double)(-MathHelper.sin(yaw * (float) Math.PI / 180.0F) * (float)i * 0.5F),
                           eventAttackSlowdown.isReduceY() ? 0.0 : 0.1,
                           (double)(MathHelper.cos(yaw * (float) Math.PI / 180.0F) * (float)i * 0.5F)
                        );
                     }

                     this.motionX = this.motionX * eventAttackSlowdown.getMotion();
                     this.motionZ = this.motionZ * eventAttackSlowdown.getMotion();
   ...
   public static enum EnumChatVisibility {
      ...
      static {
         ...
         for (EntityPlayer.EnumChatVisibility entityplayer$enumchatvisibility : var3 = values()) {
