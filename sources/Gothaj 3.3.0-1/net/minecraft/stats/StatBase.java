...
public class StatBase {
   ...
   public static IStatType timeStatType = new IStatType() {
      ...
      public String format(int number) {
         ...
         double d4 = d3 / 365.0;
         return d4 > 0.5
            ? StatBase.decimalFormat.format(d4) + " y"
            : (
               d3 > 0.5
                  ? StatBase.decimalFormat.format(d3) + " d"
                  : (d2 > 0.5 ? StatBase.decimalFormat.format(d2) + " h" : (d1 > 0.5 ? StatBase.decimalFormat.format(d1) + " m" : d0 + " s"))
            );
      }
   };
   public static IStatType distanceStatType = new IStatType() {
      ...
      public String format(int number) {
         ...
         double d1 = d0 / 1000.0;
         return d1 > 0.5 ? StatBase.decimalFormat.format(d1) + " km" : (d0 > 0.5 ? StatBase.decimalFormat.format(d0) + " m" : number + " cm");
      }
   };
   public static IStatType field_111202_k = new IStatType() {
      @Override
      public String format(int number) {
         return StatBase.decimalFormat.format((double)number * 0.1);
      }
   };
