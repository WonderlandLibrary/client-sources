...
public final class SpawnerAnimals {
   ...
   public int findChunksForSpawning(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs, boolean p_77192_4_) {
      if (!spawnHostileMobs && !spawnPeacefulMobs) {
         ...
         EnumCreatureType[] var46;
         for (EnumCreatureType enumcreaturetype : var46 = EnumCreatureType.values()) {
            if ((!enumcreaturetype.getPeacefulCreature() || spawnPeacefulMobs)
               ...
               int k4 = Reflector.ForgeWorld_countEntities.exists()
                  ? Reflector.callInt(worldServerIn, Reflector.ForgeWorld_countEntities, enumcreaturetype, true)
                        ...
                        for (int k2 = 0; k2 < 3; k2++) {
                           ...
                           for (int l3 = 0; l3 < 4; l3++) {
                              ...
                              if (!worldServerIn.isAnyPlayerWithinRangeAt((double)f, (double)i3, (double)f1, 24.0)
                                 ...
                                 if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biomegenbase$spawnlistentry, blockpos1)
                                    && canCreatureTypeSpawnAtLocation(
                                       EntitySpawnPlacementRegistry.getPlacementForEntity(biomegenbase$spawnlistentry.entityClass), worldServerIn, blockpos1
                                    )) {
                                    ...
                                    boolean flag2 = Reflector.ForgeEventFactory_canEntitySpawn.exists()
                                       ? ReflectorForge.canEntitySpawn(entityliving, worldServerIn, f, (float)i3, f1)
      ...
      if (!worldIn.getWorldBorder().contains(pos)) {
         ...
         if (spawnPlacementTypeIn == EntityLiving.SpawnPlacementType.IN_WATER) {
            ...
            boolean flag = Reflector.ForgeBlock_canCreatureSpawn.exists()
               ? Reflector.callBoolean(iblockstate.getBlock(), Reflector.ForgeBlock_canCreatureSpawn, worldIn, blockpos, spawnPlacementTypeIn)
   ...
   public static void performWorldGenSpawning(
      World worldIn, BiomeGenBase biomeIn, int p_77191_2_, int p_77191_3_, int p_77191_4_, int p_77191_5_, Random randomIn
   ) {
      ...
      if (!list.isEmpty()) {
         while (randomIn.nextFloat() < biomeIn.getSpawningChance()) {
            ...
            int i = biomegenbase$spawnlistentry.minGroupCount
               + randomIn.nextInt(1 + biomegenbase$spawnlistentry.maxGroupCount - biomegenbase$spawnlistentry.minGroupCount);
            IEntityLivingData ientitylivingdata = null;
