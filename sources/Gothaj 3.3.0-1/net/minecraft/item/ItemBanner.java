...
public class ItemBanner extends ItemBlock {
      ...
      if (side == EnumFacing.DOWN) {
         ...
         if (!playerIn.canPlayerEdit(pos, side, stack)) {
            ...
            if (side == EnumFacing.UP) {
               int i = MathHelper.floor_double((double)((playerIn.rotationYaw + 180.0F) * 16.0F / 360.0F) + 0.5) & 15;
   ...
   public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
      ...
      if (nbttagcompound != null && nbttagcompound.hasKey("Patterns")) {
         ...
         for (int i = 0; i < nbttaglist.tagCount() && i < 6; i++) {
            ...
            TileEntityBanner.EnumBannerPattern tileentitybanner$enumbannerpattern = TileEntityBanner.EnumBannerPattern.getPatternByID(
               nbttagcompound1.getString("Pattern")
            );
            if (tileentitybanner$enumbannerpattern != null) {
               tooltip.add(
                  StatCollector.translateToLocal("item.banner." + tileentitybanner$enumbannerpattern.getPatternName() + "." + enumdyecolor.getUnlocalizedName())
               );
            }
         }
      }
   ...
   public void getSubItems(Item itemIn, CreativeTabs tab, List<ItemStack> subItems) {
      EnumDyeColor[] var7;
      for (EnumDyeColor enumdyecolor : var7 = EnumDyeColor.values()) {
