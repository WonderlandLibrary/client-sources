...
public class ItemSlab extends ItemBlock {
   ...
   public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
      if (stack.stackSize == 0) {
         ...
         if (iblockstate.getBlock() == this.singleSlab) {
            ...
            if ((
                  side == EnumFacing.UP && blockslab$enumblockhalf == BlockSlab.EnumBlockHalf.BOTTOM
   ...
   public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack) {
      ...
      BlockPos var11 = pos.offset(side);
      IBlockState iblockstate1 = worldIn.getBlockState(var11);
      return iblockstate1.getBlock() == this.singleSlab && object == iblockstate1.getValue(iproperty)
         ? true
   ...
   private boolean tryPlace(ItemStack stack, World worldIn, BlockPos pos, Object variantInStack) {
      IBlockState iblockstate = worldIn.getBlockState(pos);
      if (iblockstate.getBlock() == this.singleSlab) {
         Comparable comparable = iblockstate.getValue((IProperty<Comparable>)this.singleSlab.getVariantProperty());
