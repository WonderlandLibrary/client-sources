...
public abstract class MinecraftServer implements Runnable, ICommandSender, IThreadListener, IPlayerUsage {
   ...
   public void run() {
      try {
         ...
         if (var44 instanceof ReportedException) {
            crashreport = this.addServerInfoToCrashReport(((ReportedException)var44).getCrashReport());
         } else {
            crashreport = this.addServerInfoToCrashReport(new CrashReport("Exception in server tick loop", var44));
         }
               ...
               this.serverConfigManager
                  .sendPacketToAllPlayersInDimension(
                     new S03PacketTimeUpdate(
                        worldserver.getTotalWorldTime(), worldserver.getWorldTime(), worldserver.getGameRules().getBoolean("doDaylightCycle")
                     ),
   ...
   public List<String> getTabCompletions(ICommandSender sender, String input, BlockPos pos) {
      ...
      if (input.startsWith("/")) {
         ...
         String[] var10;
         for (String s1 : var10 = this.serverConfigManager.getAllUsernames()) {
