...
public class StreamReader {
   ...
   public String prefix(int length) {
      if (length == 0) {
         ...
         return this.ensureEnoughData(length)
            ? new String(this.dataWindow, this.pointer, length)
            : new String(this.dataWindow, this.pointer, Math.min(length, this.dataLength - this.pointer));
      }
   }
