...
public class EntityTypes1_12 {
   public static EntityTypes1_12.EntityType getTypeFromId(int typeID, boolean isObject) {
      Optional<EntityTypes1_12.EntityType> type;
      if (isObject) {
         type = EntityTypes1_12.ObjectType.getPCEntity(typeID);
      } else {
   ...
   public static enum EntityType implements com.viaversion.viaversion.api.minecraft.entities.EntityType {
      ...
      private static final Map<Integer, EntityTypes1_12.EntityType> TYPES = new HashMap<>();
      ...
      private EntityType(int id, EntityTypes1_12.EntityType parent) {
         this.id = id;
         this.parent = parent;
      }
      ...
      static {
         for (EntityTypes1_12.EntityType type : values()) {
   ...
   public static enum ObjectType implements com.viaversion.viaversion.api.minecraft.entities.ObjectType {
      ...
      private static final Map<Integer, EntityTypes1_12.ObjectType> TYPES = new HashMap<>();
      private final int id;
      private final EntityTypes1_12.EntityType type;

      private ObjectType(int id, EntityTypes1_12.EntityType type) {
         this.id = id;
         this.type = type;
      }
      ...
      public static Optional<EntityTypes1_12.EntityType> getPCEntity(int id) {
         Optional<EntityTypes1_12.ObjectType> output = findById(id);
         return !output.isPresent() ? Optional.empty() : Optional.of(output.get().type);
      }

      static {
         for (EntityTypes1_12.ObjectType type : values()) {
            TYPES.put(type.id, type);
         }
      }
   }
