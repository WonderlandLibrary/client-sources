...
public abstract class AbstractProtocol<CU extends ClientboundPacketType, CM extends ClientboundPacketType, SM extends ServerboundPacketType, SU extends ServerboundPacketType>
   ...
   protected AbstractProtocol(
      @Nullable Class<CU> unmappedClientboundPacketType,
      @Nullable Class<CM> mappedClientboundPacketType,
      @Nullable Class<SM> mappedServerboundPacketType,
      @Nullable Class<SU> unmappedServerboundPacketType
   ) {
   ...
   public final void initialize() {
      ...
      if (this.unmappedClientboundPacketType != null
         && this.mappedClientboundPacketType != null
         && this.unmappedClientboundPacketType != this.mappedClientboundPacketType) {
         this.registerPacketIdChanges(
            this.packetTypesProvider.unmappedClientboundPacketTypes(),
            this.packetTypesProvider.mappedClientboundPacketTypes(),
            this::hasRegisteredClientbound,
            this::registerClientbound
         );
      }

      if (this.mappedServerboundPacketType != null
         && this.unmappedServerboundPacketType != null
         && this.mappedServerboundPacketType != this.unmappedServerboundPacketType) {
         this.registerPacketIdChanges(
            this.packetTypesProvider.unmappedServerboundPacketTypes(),
            this.packetTypesProvider.mappedServerboundPacketTypes(),
   ...
   private <U extends PacketType, M extends PacketType> void registerPacketIdChanges(
      ...
      for (Entry<State, PacketTypeMap<M>> entry : mappedPacketTypes.entrySet()) {
         ...
         for (U unmappedType : unmappedPacketTypes.get(entry.getKey()).types()) {
            ...
            if (mappedType == null) {
               Preconditions.checkArgument(
                  registeredPredicate.test(unmappedType),
                  "Packet %s in %s has no mapping - it needs to be manually cancelled or remapped",
                  new Object[]{unmappedType, this.getClass()}
               );
   ...
   protected void addEntityTracker(UserConnection connection, EntityTracker tracker) {
      connection.addEntityTracker((Class<? extends Protocol>)this.getClass(), tracker);
   }

   protected PacketTypesProvider<CU, CM, SM, SU> createPacketTypesProvider() {
      return new SimplePacketTypesProvider<>(
         this.packetTypeMap(this.unmappedClientboundPacketType),
         this.packetTypeMap(this.mappedClientboundPacketType),
         this.packetTypeMap(this.mappedServerboundPacketType),
         this.packetTypeMap(this.unmappedServerboundPacketType)
      );
   }
   ...
   protected SU configurationAcknowledgedPacket() {
      ...
      PacketTypeMap<SU> packetTypeMap = packetTypes.get(State.PLAY);
   ...
   protected CU startConfigurationPacket() {
      ...
      PacketTypeMap<CU> packetTypeMap = packetTypes.get(State.PLAY);
   ...
   public void registerServerbound(State state, int unmappedPacketId, int mappedPacketId, PacketHandler handler, boolean override) {
      ...
      if (!override && this.serverboundMappings.hasMapping(state, unmappedPacketId)) {
         Via.getPlatform()
            ...
            .log(
               Level.WARNING,
               unmappedPacketId + " already registered! If this override is intentional, set override to true. Stacktrace: ",
   ...
   public void registerClientbound(State state, int unmappedPacketId, int mappedPacketId, PacketHandler handler, boolean override) {
      ...
      if (!override && this.clientboundMappings.hasMapping(state, unmappedPacketId)) {
         Via.getPlatform()
            ...
            .log(
               Level.WARNING,
               unmappedPacketId + " already registered! If override is intentional, set override to true. Stacktrace: ",
               (Throwable)(new Exception())
            );
      }
   ...
   public void registerClientbound(CU packetType, @Nullable PacketHandler handler) {
      PacketTypeMap<CM> mappedPacketTypes = this.packetTypesProvider.mappedClientboundPacketTypes().get(packetType.state());
      CM mappedPacketType = mappedPacketType(packetType, mappedPacketTypes, this.unmappedClientboundPacketType, this.mappedClientboundPacketType);
   ...
   public void registerClientbound(CU packetType, @Nullable CM mappedPacketType, @Nullable PacketHandler handler, boolean override) {
      this.register(
         this.clientboundMappings, packetType, mappedPacketType, this.unmappedClientboundPacketType, this.mappedClientboundPacketType, handler, override
      );
   }
   ...
   public void registerServerbound(SU packetType, @Nullable PacketHandler handler) {
      PacketTypeMap<SM> mappedPacketTypes = this.packetTypesProvider.mappedServerboundPacketTypes().get(packetType.state());
      SM mappedPacketType = mappedPacketType(packetType, mappedPacketTypes, this.unmappedServerboundPacketType, this.mappedServerboundPacketType);
   ...
   public void registerServerbound(SU packetType, @Nullable SM mappedPacketType, @Nullable PacketHandler handler, boolean override) {
      this.register(
         this.serverboundMappings, packetType, mappedPacketType, this.unmappedServerboundPacketType, this.mappedServerboundPacketType, handler, override
      );
   }
   ...
   private void register(
      PacketMappings packetMappings,
      PacketType packetType,
      @Nullable PacketType mappedPacketType,
      Class<? extends PacketType> unmappedPacketClass,
      Class<? extends PacketType> mappedPacketClass,
      ...
      if (!override && packetMappings.hasMapping(packetType)) {
         Via.getPlatform()
            .getLogger()
            .log(
               Level.WARNING, packetType + " already registered! If override is intentional, set override to true. Stacktrace: ", (Throwable)(new Exception())
            );
      }
   ...
   private static <U extends PacketType, M extends PacketType> M mappedPacketType(
      U packetType, PacketTypeMap<M> mappedTypes, Class<U> unmappedPacketTypeClass, Class<M> mappedPacketTypeClass
   ) {
      ...
      if (unmappedPacketTypeClass == mappedPacketTypeClass) {
         ...
         Preconditions.checkNotNull(
            mappedTypes, "Mapped packet types not provided for state %s of type class %s", new Object[]{packetType.state(), mappedPacketTypeClass}
         );
         ...
         if (mappedType != null) {
            ...
            throw new IllegalArgumentException(
               "Packet type " + packetType + " in " + packetType.getClass().getSimpleName() + " could not be automatically mapped!"
            );
         }
      }
   }
   ...
   protected void throwRemapError(Direction direction, State state, int unmappedPacketId, int mappedPacketId, InformativeException e) throws InformativeException {
      if (state != State.PLAY && direction == Direction.SERVERBOUND && !Via.getManager().debugHandler().enabled()) {
         ...
         PacketType packetType = (PacketType)(direction == Direction.CLIENTBOUND
            ? this.unmappedClientboundPacketType(state, unmappedPacketId)
            : this.unmappedServerboundPacketType(state, unmappedPacketId));
         if (packetType != null) {
            Via.getPlatform()
               .getLogger()
               .warning("ERROR IN " + this.getClass().getSimpleName() + " IN REMAP OF " + packetType + " (" + toNiceHex(unmappedPacketId) + ")");
         } else {
            Via.getPlatform()
               .getLogger()
               .warning(
                  "ERROR IN "
                     ...
                     + state
                     + " "
                     + toNiceHex(unmappedPacketId)
                     + "->"
                     + toNiceHex(mappedPacketId)
               );
         }
   ...
   private CU unmappedClientboundPacketType(State state, int packetId) {
      PacketTypeMap<CU> map = this.packetTypesProvider.unmappedClientboundPacketTypes().get(state);
      return map != null ? map.typeById(packetId) : null;
   }

   @Nullable
   private SU unmappedServerboundPacketType(State state, int packetId) {
      PacketTypeMap<SU> map = this.packetTypesProvider.unmappedServerboundPacketTypes().get(state);
      return map != null ? map.typeById(packetId) : null;
   }
   ...
   private static void checkPacketType(PacketType packetType, boolean isValid) {
      if (!isValid) {
         throw new IllegalArgumentException(
            "Packet type " + packetType + " in " + packetType.getClass().getSimpleName() + " is taken from the wrong packet types class"
         );
      }
   }
