...
public class SpongePlugin implements ViaPlatform<Player> {
   ...
   public void onServerStart(StartingEngineEvent<Server> event) {
      ((CommandRegistrar)Sponge.server().commandManager().registrar(Raw.class).get())
         .register(this.container, (Raw)Via.getManager().getCommandHandler(), "viaversion", new String[]{"viaver", "vvsponge"});
   ...
   public String getPlatformName() {
      return this.game.platform().container(Component.IMPLEMENTATION).metadata().name().orElse("unknown");
   }
   ...
   public void sendMessage(UUID uuid, String message) {
      this.game.server().player(uuid).ifPresent(player -> player.sendMessage(LEGACY_SERIALIZER.deserialize(message)));
   }
      ...
      for (PluginContainer plugin : this.game.pluginManager().plugins()) {
         ...
         plugins.add(
            new PluginInfo(
               ...
               metadata.contributors().stream().<String>map(PluginContributor::name).collect(Collectors.toList())
            )
         );
      }
