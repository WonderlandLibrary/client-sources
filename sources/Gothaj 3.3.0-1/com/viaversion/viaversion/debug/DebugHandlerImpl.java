...
public final class DebugHandlerImpl implements DebugHandler {
   ...
   public boolean shouldLog(PacketWrapper wrapper, Direction direction) {
      return this.packetTypesToLog.isEmpty() && this.serverboundPacketIdsToLog.isEmpty() && this.clientboundPacketIdsToLog.isEmpty()
         || wrapper.getPacketType() != null && this.packetTypesToLog.contains(wrapper.getPacketType().getName())
         || (direction == Direction.SERVERBOUND ? this.serverboundPacketIdsToLog : this.clientboundPacketIdsToLog).contains(wrapper.getId());
   }
}
