...
public abstract class AbstractViaConfig extends Config implements ViaVersionConfig {
   ...
   protected void loadFields() {
      ...
      Map<String, String> worlds = this.get("map-1_16-world-names", Map.class, new HashMap<>());
      this.map1_16WorldNames = new WorldIdentifiers(
         worlds.getOrDefault("overworld", "minecraft:overworld"),
         worlds.getOrDefault("nether", "minecraft:the_nether"),
         worlds.getOrDefault("end", "minecraft:the_end")
      );
   ...
   private BlockedProtocolVersions loadBlockedProtocolVersions() {
         ...
         if (!s.isEmpty()) {
            ...
            if (c != '<' && c != '>') {
               ...
               if (protocolVersion != null && !blockedProtocols.add(protocolVersion.getVersion())) {
                  Via.getPlatform()
                     .getLogger()
                     .warning("Duplicated blocked protocol version " + protocolVersion.getName() + "/" + protocolVersion.getVersion());
               }
            } else {
      ...
      if (lowerBound != -1 || upperBound != -1) {
         ...
         blockedProtocols.removeIf(
            version -> {
               if ((finalLowerBound == -1 || version >= finalLowerBound) && (finalUpperBound == -1 || version <= finalUpperBound)) {
                  return false;
               } else {
                  ProtocolVersion protocolVersionx = ProtocolVersion.getProtocol(version);
                  Via.getPlatform()
                     .getLogger()
                     .warning(
                        "Blocked protocol version "
                           + protocolVersionx.getName()
                           + "/"
                           + protocolVersionx.getVersion()
                           + " already covered by upper or lower bound"
                     );
                  return true;
               }
            }
   ...
   public void setCheckForUpdates(boolean checkForUpdates) {
      ...
      this.set("checkforupdates", Boolean.valueOf(checkForUpdates));
   }
