...
public class ItemRewriter {
   ...
   private static String fixBookSpaceChars(String str) {
      return !str.startsWith(" ") ? str : "§r" + str;
   }
   ...
   public static void toClient(Item item) {
      if (item != null) {
         if (item.identifier() == 383 && item.data() != 0) {
            ...
            String entityName = ENTITY_ID_TO_NAME.get(Integer.valueOf(item.data()));
            if (entityName != null) {
         ...
         if (item.identifier() == 373) {
            ...
            tagx.put("Potion", potion);
            item.setTag(tagx);
            item.setData((short)0);
         }

         if (item.identifier() == 387) {
            CompoundTag tagxx = item.tag();
            if (tagxx == null) {
               tagxx = new CompoundTag();
            }

            ListTag pages = tagxx.get("pages");
            if (pages == null) {
               pages = new ListTag(Collections.singletonList(new StringTag(Protocol1_9To1_8.fixJson("").toString())));
               tagxx.put("pages", pages);
               item.setTag(tagxx);
               return;
            }
