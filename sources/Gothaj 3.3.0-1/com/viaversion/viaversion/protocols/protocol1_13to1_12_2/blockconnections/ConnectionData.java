               ...
               for (int idx = 0; idx < 4096; idx++) {
                  ...
                  if (handler != null) {
                     Position position = new Position(
                        xOff + ChunkSection.xFromIndex(idx), yOff + ChunkSection.yFromIndex(idx), zOff + ChunkSection.zFromIndex(idx)
                     );
   ...
   public static void init() {
      if (Via.getConfig().isServersideBlockConnections()) {
         ...
         for (int id = 0; id < blockStates.size(); id++) {
            String key = (String)blockStates.<Tag>get(id).getValue();
         ...
         if (!Via.getConfig().isReduceBlockStorageMemory()) {
            ...
            for (Tag blockTag : (ListTag)MappingDataLoader.loadNBT("blockConnections.nbt").get("data")) {
         ...
         for (String key : KEY_TO_ID.keySet()) {
            ...
            for (ConnectionData.ConnectorInitAction action : initActions) {
   ...
   public static final class NeighbourUpdater {
      ...
      private void updateBlock(int x, int y, int z, List<BlockChangeRecord1_8> records) {
         ...
         if (handler != null) {
            ...
            if (blockState != newBlockState || !ConnectionData.blockConnectionProvider.storesBlocks(this.user, null)) {
