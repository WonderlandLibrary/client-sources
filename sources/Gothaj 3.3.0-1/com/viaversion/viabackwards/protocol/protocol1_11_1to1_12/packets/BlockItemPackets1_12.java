...
public class BlockItemPackets1_12 extends LegacyBlockItemRewriter<ClientboundPackets1_12, ServerboundPackets1_9_3, Protocol1_11_1To1_12> {
   ...
   protected void registerPackets() {
      this.protocol.registerClientbound(ClientboundPackets1_12.MAP_DATA, new PacketHandlers() {
         @Override
      ...
      this.protocol.registerClientbound(ClientboundPackets1_12.PLUGIN_MESSAGE, new PacketHandlers() {
         @Override
         public void register() {
      ...
      this.protocol.registerClientbound(ClientboundPackets1_12.CHUNK_DATA, wrapper -> {
         ClientWorld clientWorld = wrapper.user().get(ClientWorld.class);
         ChunkType1_9_3 type = ChunkType1_9_3.forEnvironment(clientWorld.getEnvironment());
         Chunk chunk = wrapper.passthrough(type);
         this.handleChunk(chunk);
      });
      this.protocol.registerClientbound(ClientboundPackets1_12.BLOCK_CHANGE, new PacketHandlers() {
         @Override
         public void register() {
      ...
      this.protocol.registerClientbound(ClientboundPackets1_12.MULTI_BLOCK_CHANGE, new PacketHandlers() {
         @Override
         public void register() {
            this.map(Type.INT);
            this.map(Type.INT);
      ...
      this.protocol.registerClientbound(ClientboundPackets1_12.BLOCK_ENTITY_DATA, new PacketHandlers() {
         @Override
         public void register() {
            this.map(Type.POSITION1_8);
            this.map(Type.UNSIGNED_BYTE);
      ...
      this.protocol.registerServerbound(ServerboundPackets1_9_3.CLIENT_STATUS, new PacketHandlers() {
         @Override
         public void register() {
            this.map(Type.VAR_INT);
            this.handler(wrapper -> {
   ...
   private void handleNbtToServer(CompoundTag compoundTag, CompoundTag backupTag) {
      for (Entry<String, Tag> entry : backupTag) {
         if (entry.getValue() instanceof CompoundTag) {
