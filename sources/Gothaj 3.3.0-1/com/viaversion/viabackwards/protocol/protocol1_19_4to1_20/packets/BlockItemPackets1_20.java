...
public final class BlockItemPackets1_20 extends ItemRewriter<ClientboundPackets1_19_4, ServerboundPackets1_19_4, Protocol1_19_4To1_20> {
   ...
   public void registerPackets() {
      ...
      this.protocol.registerClientbound(ClientboundPackets1_19_4.CHUNK_DATA, new PacketHandlers() {
         @Override
      ...
      this.protocol.registerClientbound(ClientboundPackets1_19_4.MULTI_BLOCK_CHANGE, new PacketHandlers() {
         @Override
         public void register() {
            ...
            this.handler(wrapper -> {
               for (BlockChangeRecord record : wrapper.passthrough(Type.VAR_LONG_BLOCK_CHANGE_RECORD_ARRAY)) {
                  record.setBlockId(BlockItemPackets1_20.this.protocol.getMappingData().getNewBlockStateId(record.getBlockId()));
               }
            });
         }
      });
   ...
   private void handleBlockEntity(BlockEntity blockEntity) {
      if (blockEntity.typeId() == 7 || blockEntity.typeId() == 8) {
         ...
         CompoundTag frontText = tag.remove("front_text");
   ...
   private void writeMessages(CompoundTag frontText, CompoundTag tag, boolean filtered) {
      ListTag messages = frontText.get(filtered ? "filtered_messages" : "messages");
      if (messages != null) {
         int i = 0;
