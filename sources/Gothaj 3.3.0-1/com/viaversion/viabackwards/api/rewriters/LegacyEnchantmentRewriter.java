...
public class LegacyEnchantmentRewriter {
      ...
      for (Tag enchantmentEntry : enchantments.copy()) {
         ...
         if (idTag != null) {
            ...
            if (enchantmentName != null) {
               ...
               short level = ((CompoundTag)enchantmentEntry).<NumberTag>get("lvl").asShort();
   ...
   public void rewriteEnchantmentsToServer(CompoundTag tag, boolean storedEnchant) {
      String key = storedEnchant ? "StoredEnchantments" : "ench";
      ListTag remappedEnchantments = tag.remove(this.nbtTagName + "|" + key);
      ListTag enchantments = tag.get(key);
      ...
      if (!storedEnchant && tag.remove(this.nbtTagName + "|dummyEnchant") != null) {
         for (Tag enchantment : enchantments.copy()) {
            short id = ((CompoundTag)enchantment).<NumberTag>get("id").asShort();
            short level = ((CompoundTag)enchantment).<NumberTag>get("lvl").asShort();
            if (id == 0 && level == 0) {
               enchantments.remove(enchantment);
            }
         }

         IntTag hideFlags = tag.remove(this.nbtTagName + "|oldHideFlags");
         if (hideFlags != null) {
      ...
      for (Tag enchantmentx : remappedEnchantments.copy()) {
         enchantments.add(enchantmentx);
