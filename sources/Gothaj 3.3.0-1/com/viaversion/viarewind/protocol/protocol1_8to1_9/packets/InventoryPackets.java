...
public class InventoryPackets {
   public static void register(final Protocol1_8To1_9 protocol) {
         ...
         public void register() {
            ...
            this.handler(packetWrapper -> {
               ...
               if (windowId == 0 && slot == 45) {
                  ...
                  String type = packetWrapper.user().get(Windows.class).get((short)windowId);
                  if (type != null) {
                     if (type.equalsIgnoreCase("minecraft:brewing_stand")) {
                        if (slot > 4) {
                           ...
                           packetWrapper.cancel();
                           Windows.updateBrewingStand(packetWrapper.user(), packetWrapper.get(Type.ITEM1_8, 0), (short)windowId);
                        } else {
                           packetWrapper.user().get(Windows.class).getBrewingItems((short)windowId)[slot] = packetWrapper.get(Type.ITEM1_8, 0);
                        }
                     }
                  }
               }
      ...
      protocol.registerServerbound(
         ...
         new PacketHandlers() {
            ...
            public void register() {
               ...
               this.map(Type.ITEM1_8);
               this.handler(
                  packetWrapper -> packetWrapper.set(Type.ITEM1_8, 0, protocol.getItemRewriter().handleItemToServer(packetWrapper.get(Type.ITEM1_8, 0)))
               );
               this.handler(packetWrapper -> {
                  short windowId = packetWrapper.get(Type.UNSIGNED_BYTE, 0);
      ...
      protocol.registerServerbound(
         ServerboundPackets1_8.CREATIVE_INVENTORY_ACTION,
         new PacketHandlers() {
            @Override
            public void register() {
               this.map(Type.SHORT);
