...
IInventory {
    ...
    private void func_146000_x() {
        if (this.isComplete && this.levels > 0 && !this.worldObj.isRemote && this.primaryEffect > 0) {
            ...
            for (EntityPlayer var102 : var8) {
                var102.addPotionEffect(new PotionEffect(this.primaryEffect, 180, var3, true, true));
            }
            if (this.levels >= 4 && this.primaryEffect != this.secondaryEffect && this.secondaryEffect > 0) {
                for (EntityPlayer var102 : var8) {
                    var102.addPotionEffect(new PotionEffect(this.secondaryEffect, 180, 0, true, true));
                }
            }
        }
    }
    ...
    private void func_146003_y() {
        ...
        var5 = new BeamSegment(EntitySheep.func_175513_a(EnumDyeColor.WHITE));
        this.field_174909_f.add(var5);
        var6 = true;
        var7 = var3 + 1;
        while (var7 < this.worldObj.getActualHeight()) {
            var8 = new BlockPos(var2, var7, var4);
            var9 = this.worldObj.getBlockState(var8);
            if (var9.getBlock() != Blocks.stained_glass) ** GOTO lbl18
...
lbl26: // 2 sources:
        ...
        if (this.isComplete) {
            ...
            while (var7 <= 4) {
                ...
                var16 = true;
                var17 = var2 - var7;
                while (var17 <= var2 + var7 && var16) {
                    var11 = var4 - var7;
                    while (var11 <= var4 + var7) {
                        var12 = this.worldObj.getBlockState(new BlockPos(var17, var14, var11)).getBlock();
        ...
        if (this.worldObj.isRemote != false) return;
        if (this.levels != 4) return;
        if (var1 >= this.levels) return;
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        if (stack.getItem() != Items.emerald && stack.getItem() != Items.diamond && stack.getItem() != Items.gold_ingot && stack.getItem() != Items.iron_ingot) {
            return false;
        }
        return true;
    }
