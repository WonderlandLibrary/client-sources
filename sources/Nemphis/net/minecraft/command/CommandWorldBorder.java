...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args[0].equals("set")) {
            ...
            long l = var8 = args.length > 2 ? CommandWorldBorder.parseLong(args[2], 0, 9223372036854775L) * 1000 : 0;
            if (var8 > 0) {
                var3.setTransition(var4, var6, var8);
                if (var4 > var6) {
                    CommandWorldBorder.notifyOperators(sender, (ICommand)this, "commands.worldborder.setSlowly.shrink.success", String.format("%.1f", var6), String.format("%.1f", var4), Long.toString(var8 / 1000));
                } else {
                    CommandWorldBorder.notifyOperators(sender, (ICommand)this, "commands.worldborder.setSlowly.grow.success", String.format("%.1f", var6), String.format("%.1f", var4), Long.toString(var8 / 1000));
                }
            } else {
            ...
            long var8 = var3.getTimeUntilTarget() + (args.length > 2 ? CommandWorldBorder.parseLong(args[2], 0, 9223372036854775L) * 1000 : 0);
            if (var8 > 0) {
                var3.setTransition(var4, var6, var8);
                if (var4 > var6) {
                    CommandWorldBorder.notifyOperators(sender, (ICommand)this, "commands.worldborder.setSlowly.shrink.success", String.format("%.1f", var6), String.format("%.1f", var4), Long.toString(var8 / 1000));
