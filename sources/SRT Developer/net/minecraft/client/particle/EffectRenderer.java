...
public class EffectRenderer {
   ...
   public EntityFX spawnEffectParticle(
      int particleId,
      double p_178927_2_,
      double p_178927_4_,
      double p_178927_6_,
      ...
      if (iparticlefactory != null) {
         EntityFX entityfx = iparticlefactory.getEntityFX(
            particleId, this.worldObj, p_178927_2_, p_178927_4_, p_178927_6_, p_178927_8_, p_178927_10_, p_178927_12_, p_178927_14_
         );
         if (entityfx != null) {
   ...
   private void tickParticle(EntityFX p_178923_1_) {
      try {
         ...
         crashreportcategory.addCrashSectionCallable(
            "Particle Type", () -> i == 0 ? "MISC_TEXTURE" : (i == 1 ? "TERRAIN_TEXTURE" : (i == 3 ? "ENTITY_PARTICLE_TEXTURE" : "Unknown - " + i))
         );
            ...
            if (!this.fxLayers[i][j].isEmpty()) {
               ...
               for(int k = 0; k < this.fxLayers[i][j].size(); ++k) {
                  ...
                  try {
                     ...
                     crashreportcategory.addCrashSectionCallable(
                        "Particle Type", () -> i_f == 0 ? "MISC_TEXTURE" : (i_f == 1 ? "TERRAIN_TEXTURE" : "Unknown - " + i_f)
                     );
                     throw new ReportedException(crashreport);
                  }
               }
   ...
   public void addBlockDestroyEffects(BlockPos pos, IBlockState state) {
      ...
      if (Reflector.ForgeBlock_addDestroyEffects.exists() && Reflector.ForgeBlock_isAir.exists()) {
         ...
         flag = !Reflector.callBoolean(block, Reflector.ForgeBlock_isAir, this.worldObj, pos)
            && !Reflector.callBoolean(block, Reflector.ForgeBlock_addDestroyEffects, this.worldObj, pos, this);
      } else {
         ...
         for(int i = 0; i < l; ++i) {
            for(int j = 0; j < l; ++j) {
               for(int k = 0; k < l; ++k) {
                  ...
                  this.addEffect(
                     new EntityDiggingFX(
                           this.worldObj, d0, d1, d2, d0 - (double)pos.getX() - 0.5, d1 - (double)pos.getY() - 0.5, d2 - (double)pos.getZ() - 0.5, state
                        )
                        .func_174846_a(pos)
                  );
               }
   ...
   public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
      ...
      if (block.getRenderType() != -1) {
         ...
         float f = 0.1F;
         double d0 = (double)i
            + this.rand.nextDouble() * (block.getBlockBoundsMaxX() - block.getBlockBoundsMinX() - (double)(f * 2.0F))
            + (double)f
            + block.getBlockBoundsMinX();
         double d1 = (double)j
            + this.rand.nextDouble() * (block.getBlockBoundsMaxY() - block.getBlockBoundsMinY() - (double)(f * 2.0F))
            + (double)f
            + block.getBlockBoundsMinY();
         double d2 = (double)k
            + this.rand.nextDouble() * (block.getBlockBoundsMaxZ() - block.getBlockBoundsMinZ() - (double)(f * 2.0F))
            + (double)f
         ...
         this.addEffect(
            new EntityDiggingFX(this.worldObj, d0, d1, d2, 0.0, 0.0, 0.0, iblockstate).func_174846_a(pos).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F)
         );
      }
   }
