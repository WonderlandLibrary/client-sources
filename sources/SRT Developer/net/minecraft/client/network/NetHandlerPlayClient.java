...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   public void handleJoinGame(S01PacketJoinGame packetIn) {
      ...
      EventSendBrand eventSendBrand = new EventSendBrand("MC|Brand", new PacketBuffer(Unpooled.buffer()).writeString(ClientBrandRetriever.getClientModName()));
      EventBus.processEvent(eventSendBrand);
      if (!eventSendBrand.isCancelled()) {
         this.netManager.sendPacket(new C17PacketCustomPayload(eventSendBrand.getChannelIn(), eventSendBrand.getDataIn()));
         if (Minecraft.getMinecraft().gameSettings.expertMode) {
            Ries.INSTANCE
               .msg(
                  "§4§lALERT! Expert mode is enabled. You can set any slider to any value, §4§luncapped§7. Lag or a crash or hang may occur if values are set too high or too low!"
               );
         }
      }
   }
   ...
   public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
      ...
      if (packetIn.getType() == 10) {
         ...
         entity = new EntityLeashKnot(
            this.clientWorldController, new BlockPos(MathHelper.floor_double(d0), MathHelper.floor_double(d1), MathHelper.floor_double(d2))
         );
         packetIn.func_149002_g(0);
   ...
   public void handleSpawnExperienceOrb(S11PacketSpawnExperienceOrb packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      Entity entity = new EntityXPOrb(
         this.clientWorldController, (double)packetIn.getX() / 32.0, (double)packetIn.getY() / 32.0, (double)packetIn.getZ() / 32.0, packetIn.getXPValue()
      );
      entity.serverPosX = packetIn.getX();
      entity.serverPosY = packetIn.getY();
      entity.serverPosZ = packetIn.getZ();
   ...
   public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
      ...
      EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(
         this.gameController.theWorld, this.getPlayerInfo(packetIn.getPlayer()).getGameProfile()
      );
      ...
      this.netManager
         .sendPacket(
            new C03PacketPlayer.C06PacketPlayerPosLook(
               entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, entityplayer.rotationYaw, entityplayer.rotationPitch, false
            )
         );
   ...
   public void handleMultiBlockChange(S22PacketMultiBlockChange packetIn) {
      ...
      for(S22PacketMultiBlockChange.BlockUpdateData s22packetmultiblockchange$blockupdatedata : packetIn.getChangedBlocks()) {
         this.clientWorldController
            .invalidateRegionAndSetBlock(s22packetmultiblockchange$blockupdatedata.getPos(), s22packetmultiblockchange$blockupdatedata.getBlockState());
      }
   }
   ...
   public void handleChunkData(S21PacketChunkData packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      if (packetIn.func_149274_i() && this.clientWorldController != null) {
      ...
      assert this.clientWorldController != null;

      this.clientWorldController
         .invalidateBlockReceiveRegion(
            packetIn.getChunkX() << 4, 0, packetIn.getChunkZ() << 4, (packetIn.getChunkX() << 4) + 15, 256, (packetIn.getChunkZ() << 4) + 15
         );
   ...
   public void addToSendQueue(Packet p_147297_1_) {
      ...
      EventBus.processEvent(eventPacket = new EventPacket(p_147297_1_));
      if (!eventPacket.isCancelled()) {
         this.netManager.sendPacket(p_147297_1_);
      }
   }

   public void packetNoEvent(Packet p_147297_1_) {
      this.netManager.sendPacket(p_147297_1_);
   }
   ...
   public void handleCollectItem(S0DPacketCollectItem packetIn) {
      ...
      if (entity != null) {
         if (entity instanceof EntityXPOrb) {
            this.clientWorldController
               .playSoundAtEntity(entity, "random.orb", 0.2F, ((this.avRandomizer.nextFloat() - this.avRandomizer.nextFloat()) * 0.7F + 1.0F) * 2.0F);
         } else {
            this.clientWorldController
               .playSoundAtEntity(entity, "random.pop", 0.2F, ((this.avRandomizer.nextFloat() - this.avRandomizer.nextFloat()) * 0.7F + 1.0F) * 2.0F);
         }
   ...
   public void handleEntityAttach(S1BPacketEntityAttach packetIn) {
      ...
      if (packetIn.getLeash() == 0) {
         ...
         if (flag) {
            ...
            this.gameController
               .ingameGUI
               .setRecordPlaying(I18n.format("mount.onboard", GameSettings.getKeyDisplayString(gamesettings.keyBindSneak.getKeyCode())), false);
         }
   ...
   public void handleRespawn(S07PacketRespawn packetIn) {
      ...
      if (packetIn.getDimensionID() != this.gameController.thePlayer.dimension) {
         ...
         this.clientWorldController = new WorldClient(
            this,
            new WorldSettings(0L, packetIn.getGameType(), false, this.gameController.theWorld.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()),
   ...
   public void handleExplosion(S27PacketExplosion packetIn) {
      ...
      Explosion explosion = new Explosion(
         this.gameController.theWorld, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions()
      );
         ...
         this.gameController
            ...
            .addChatMessage(
               new ChatComponentText(
                  "Unable to locate sign at " + packetIn.getPos().getX() + ", " + packetIn.getPos().getY() + ", " + packetIn.getPos().getZ()
               )
            );
      }
   }
      ...
      if (i == 1) {
         ...
         if (f == 0.0F) {
            ...
            this.gameController
               .ingameGUI
               .getChatGUI()
               .printChatMessage(new ChatComponentTranslation("demo.help.jump", GameSettings.getKeyDisplayString(gamesettings.keyBindJump.getKeyCode())));
         } else if (f == 103.0F) {
            this.gameController
               .ingameGUI
               .getChatGUI()
               .printChatMessage(
                  new ChatComponentTranslation("demo.help.inventory", GameSettings.getKeyDisplayString(gamesettings.keyBindInventory.getKeyCode()))
               );
         }
      } else if (i == 6) {
         this.clientWorldController
            .playSound(
               entityplayer.posX, entityplayer.posY + (double)entityplayer.getEyeHeight(), entityplayer.posZ, "random.successful_hit", 0.18F, 0.45F, false
            );
      } else if (i == 7) {
         ...
         this.clientWorldController
            .spawnParticle(EnumParticleTypes.MOB_APPEARANCE, entityplayer.posX, entityplayer.posY, entityplayer.posZ, 0.0, 0.0, 0.0, new int[0]);
   ...
   public void handleSoundEffect(S29PacketSoundEffect packetIn) {
      ...
      this.gameController
         .theWorld
         .playSound(packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getSoundName(), packetIn.getVolume(), packetIn.getPitch(), false);
   }

   @Override
   public void handleResourcePack(S48PacketResourcePackSend packetIn) {
      if (packetIn.getURL() != null && packetIn.getURL().contains("..")) {
         Ries.INSTANCE
            .msg(
               String.format(
                  "§4§lALERT! Server attempted to traverse through directories on your computer. SRT has cancelled the incoming packet. Directory: %s",
                  packetIn.getURL()
               )
            );
      } else {
                  ...
                  new FutureCallback<Object>() {
                     public void onSuccess(Object p_onSuccess_1_) {
                        NetHandlerPlayClient.this.netManager
                           .sendPacket(new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
                     }
   
                     public void onFailure(Throwable p_onFailure_1_) {
                        NetHandlerPlayClient.this.netManager
                           .sendPacket(new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.FAILED_DOWNLOAD));
                     }
                  }
               );
            } else {
         ...
         for(S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot : packetIn.func_149441_d()) {
            ...
            if (iattributeinstance == null) {
               iattributeinstance = baseattributemap.registerAttribute(
                  new RangedAttribute(null, s20packetentityproperties$snapshot.func_151409_a(), 0.0, Double.MIN_NORMAL, Double.MAX_VALUE)
               );
            }
