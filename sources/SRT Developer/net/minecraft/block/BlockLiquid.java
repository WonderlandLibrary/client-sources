...
public abstract class BlockLiquid extends Block {
   ...
   protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos) {
      ...
      for(EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
         ...
         if (j < 0) {
            ...
            int l = j - i;
            vec3 = vec3.addVector(
               (double)((blockpos.getX() - pos.getX()) * l), (double)((blockpos.getY() - pos.getY()) * l), (double)((blockpos.getZ() - pos.getZ()) * l)
            );
         }
      }
      ...
      if (this.blockMaterial == Material.water) {
         ...
         if (i > 0 && i < 8) {
            if (rand.nextInt(64) == 0) {
               worldIn.playSound(d0 + 0.5, d1 + 0.5, d2 + 0.5, "liquid.water", rand.nextFloat() * 0.25F + 0.75F, rand.nextFloat() + 0.5F, false);
            }
   ...
   public static double getFlowDirection(IBlockAccess worldIn, BlockPos pos, Material materialIn) {
      ...
      return vec3.xCoord == 0.0 && vec3.zCoord == 0.0 ? -1000.0 : MathHelper.func_181159_b(vec3.zCoord, vec3.xCoord) - (Math.PI / 2);
   }
