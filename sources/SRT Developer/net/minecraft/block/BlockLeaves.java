            ...
            for(int i2 = -i; i2 <= i; ++i2) {
               for(int j2 = -i; j2 <= i; ++j2) {
                  for(int k2 = -i; k2 <= i; ++k2) {
                     ...
                     int resolved = (i2 + l1) * k1 + (j2 + l1) * j1 + k2 + l1;
                     if (block != Blocks.log && block != Blocks.log2) {
                        if (block.getMaterial() == Material.leaves) {
                           this.surroundings[resolved] = -2;
                        } else {
                           this.surroundings[resolved] = -1;
                        }
                     } else {
                        this.surroundings[resolved] = 0;
                     }
                  }
               }
            }
               ...
               for(int j3 = -i; j3 <= i; ++j3) {
                  for(int k3 = -i; k3 <= i; ++k3) {
                     for(int l3 = -i; l3 <= i; ++l3) {
                        int resolved = (j3 + l1 - 1) * k1 + (k3 + l1) * j1 + l3 + l1;
                        int resolved2 = (j3 + l1 + 1) * k1 + (k3 + l1) * j1 + l3 + l1;
                        int resolved3 = (j3 + l1) * k1 + (k3 + l1 - 1) * j1 + l3 + l1;
                        int resolved4 = (j3 + l1) * k1 + (k3 + l1 + 1) * j1 + l3 + l1;
                        int resolved5 = (j3 + l1) * k1 + (k3 + l1) * j1 + (l3 + l1 - 1);
                        int resolved6 = (j3 + l1) * k1 + (k3 + l1) * j1 + l3 + l1 + 1;
                        if (this.surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + l3 + l1] == i3 - 1) {
                           if (this.surroundings[resolved] == -2) {
                              this.surroundings[resolved] = i3;
                           }

                           if (this.surroundings[resolved2] == -2) {
                              this.surroundings[resolved2] = i3;
                           }

                           if (this.surroundings[resolved3] == -2) {
                              this.surroundings[resolved3] = i3;
                           }

                           if (this.surroundings[resolved4] == -2) {
                              this.surroundings[resolved4] = i3;
                           }

                           if (this.surroundings[resolved5] == -2) {
                              this.surroundings[resolved5] = i3;
                           }

                           if (this.surroundings[resolved6] == -2) {
                              this.surroundings[resolved6] = i3;
                           }
                        }
                     }
                  }
