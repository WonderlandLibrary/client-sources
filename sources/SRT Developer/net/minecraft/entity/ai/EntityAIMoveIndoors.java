...
public class EntityAIMoveIndoors extends EntityAIBase {
   ...
   public boolean shouldExecute() {
      ...
      if ((
            !this.entityObj.worldObj.isDaytime()
               || this.entityObj.worldObj.isRaining() && !this.entityObj.worldObj.getBiomeGenForCoords(blockpos).canSpawnLightningBolt()
         )
   ...
   public void startExecuting() {
      ...
      if (this.entityObj.getDistanceSq(blockpos) > 256.0) {
         Vec3 vec3 = RandomPositionGenerator.findRandomTargetBlockTowards(this.entityObj, 14, 3, new Vec3((double)i + 0.5, (double)j, (double)k + 0.5));
         if (vec3 != null) {
            this.entityObj.getNavigator().tryMoveToXYZ(vec3.xCoord, vec3.yCoord, vec3.zCoord, 1.0);
         }
      } else {
         this.entityObj.getNavigator().tryMoveToXYZ((double)i + 0.5, (double)j, (double)k + 0.5, 1.0);
      }
   }
