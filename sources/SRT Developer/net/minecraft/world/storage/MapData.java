...
public class MapData extends WorldSavedData {
   ...
   public final Map<String, Vec4b> playersVisibleOnMap = Maps.newLinkedHashMap();
      ...
      for(int i = 0; i < this.playersArrayList.size(); ++i) {
         ...
         if (!mapdata$mapinfo1.entityplayerObj.isDead && (mapdata$mapinfo1.entityplayerObj.inventory.hasItemStack(mapStack) || mapStack.isOnItemFrame())) {
            if (!mapStack.isOnItemFrame() && mapdata$mapinfo1.entityplayerObj.dimension == this.dimension) {
               this.updatePlayersVisibleOnMap(
                  0,
                  mapdata$mapinfo1.entityplayerObj.worldObj,
                  mapdata$mapinfo1.entityplayerObj.getCommandSenderName(),
                  mapdata$mapinfo1.entityplayerObj.posX,
   ...
   private void updatePlayersVisibleOnMap(int type, World worldIn, String entityIdentifier, double worldX, double worldZ, double rotation) {
      ...
      if (f >= (float)(-j) && f1 >= (float)(-j) && f <= (float)j && f1 <= (float)j) {
         ...
         b2 = 0;
         if (f <= (float)(-j)) {
            b0 = (byte)((int)((double)(j * 2) + 2.5));
         }

         if (f1 <= (float)(-j)) {
            b1 = (byte)((int)((double)(j * 2) + 2.5));
         }
      ...
      public Packet getPacket(ItemStack stack) {
         if (this.field_176105_d) {
            ...
            return new S34PacketMaps(
               stack.getMetadata(),
               MapData.this.scale,
               MapData.this.playersVisibleOnMap.values(),
               MapData.this.colors,
               this.minX,
               this.minY,
            ...
            return this.field_176109_i++ % 5 == 0
               ? new S34PacketMaps(stack.getMetadata(), MapData.this.scale, MapData.this.playersVisibleOnMap.values(), MapData.this.colors, 0, 0, 0, 0)
               : null;
         }
      }
