...
public abstract class World implements IBlockAccess {
   ...
   public int getLight(BlockPos pos, boolean checkNeighbors) {
      if (pos.getX() < -30000000 || pos.getZ() < -30000000 || pos.getX() >= 30000000 || pos.getZ() >= 30000000) {
         return 15;
      } else if (checkNeighbors && this.getBlockState(pos).getBlock().getUseNeighborBrightness()) {
   ...
   public MovingObjectPosition rayTraceBlocks(
      Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock
   ) {
      if (Double.isNaN(vec31.xCoord) || Double.isNaN(vec31.yCoord) || Double.isNaN(vec31.zCoord)) {
         return null;
      } else if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
   ...
   public void spawnParticle(
      EnumParticleTypes particleType, double xCoord, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int... p_175688_14_
   ) {
   ...
   public void spawnParticle(
      ...
      this.spawnParticle(
         particleType.getParticleID(), particleType.getShouldIgnoreRange() | p_175682_2_, xCoord, yCoord, zCoord, xOffset, yOffset, zOffset, p_175682_15_
      );
   }

   private void spawnParticle(
      int particleID, boolean p_175720_2_, double xCood, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int... p_175720_15_
   ) {
      for(IWorldAccess worldAccess : this.worldAccesses) {
   ...
   public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn) {
      for(Entity entity : this.getEntitiesWithinAABBExcludingEntity(null, bb)) {
         if (!entity.isDead
            ...
            block instanceof BlockStairs
               ...
               : (
                  block instanceof BlockSlab
                     ...
                     : block instanceof BlockHopper || block instanceof BlockSnow && iblockstate.getValue(BlockSnow.LAYERS) == 7
               )
         );
   }
   ...
   protected void playMoodSoundAndCheckLight(int p_147467_1_, int p_147467_2_, Chunk chunkIn) {
      ...
      if (this.ambientTickCountdown == 0 && !this.isRemote) {
         ...
         if (block.getMaterial() == Material.air && this.getLight(blockpos) <= this.rand.nextInt(8) && this.getLightFor(EnumSkyBlock.SKY, blockpos) <= 0) {
            EntityPlayer entityplayer = this.getClosestPlayer((double)j + 0.5, (double)l + 0.5, (double)k + 0.5, 8.0);
            if (entityplayer != null && entityplayer.getDistanceSq((double)j + 0.5, (double)l + 0.5, (double)k + 0.5) > 4.0) {
   ...
   public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean p_175716_3_, EnumFacing side, Entity entityIn) {
      ...
      return (axisalignedbb == null || this.checkNoEntityCollision(axisalignedbb, entityIn))
         && (block.getMaterial() == Material.circuits && blockIn == Blocks.anvil || block.getMaterial().isReplaceable() && blockIn.canReplace(this, pos, side));
   }
         ...
         if (i >= 15) {
            ...
            if (i >= 15) {
               ...
               if (i >= 15) {
                  ...
                  return i >= 15 ? i : Math.max(i, this.getStrongPower(pos.east(), EnumFacing.EAST));
               }
            }
         }
      }
   ...
   public float getRainStrength(float delta) {
      EventWeatherRender eventWeatherRender = new EventWeatherRender(this.prevRainingStrength + (this.rainingStrength - this.prevRainingStrength) * delta);
      EventBus.processEvent(eventWeatherRender);
      return eventWeatherRender.getRainStrength();
   }
