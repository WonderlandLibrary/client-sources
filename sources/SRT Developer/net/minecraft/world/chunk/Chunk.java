...
public class Chunk {
   ...
   public Block getBlock(int x, int y, int z) {
      try {
         ...
         crashreportcategory.addCrashSectionCallable(
            "Location", () -> CrashReportCategory.getCoordinateInfo(new BlockPos(this.xPosition * 16 + x, y, this.zPosition * 16 + z))
         );
         throw var6;
      }
   }
               ...
               if (entity.getEntityBoundingBox().intersectsWith(aabb) && entity != entityIn) {
                  ...
                  if (aentity != null) {
                     for(Entity value : aentity) {
                        if (value != entityIn && value.getEntityBoundingBox().intersectsWith(aabb) && (p_177414_4_ == null || p_177414_4_.apply(value))) {
   ...
   public <T extends Entity> void getEntitiesOfTypeWithinAAAB(
      Class<? extends T> entityClass, AxisAlignedBB aabb, List<T> listToFill, Predicate<? super T> p_177430_4_
   ) {
      int i = MathHelper.floor_double((aabb.minY - 2.0) / 16.0);
      int j = MathHelper.floor_double((aabb.maxY + 2.0) / 16.0);
      i = MathHelper.clamp_int(i, 0, this.entityLists.length - 1);
   ...
   public Random getRandomWithSeed(long seed) {
      return new Random(
         this.worldObj.getSeed()
               + (long)this.xPosition * (long)this.xPosition * 4987142L
               + (long)this.xPosition * 5947611L
               + (long)this.zPosition * (long)this.zPosition * 4392871L
               + (long)this.zPosition * 389711L
            ^ seed
      );
   }
   ...
   public void fillChunk(byte[] p_177439_1_, int p_177439_2_, boolean p_177439_3_) {
      ...
      if (p_177439_3_) {
         ...
         int var11 = i + this.blockBiomeArray.length;
      }
   ...
   public void enqueueRelightChecks() {
      ...
      for(int i = 0; i < 8; ++i) {
         ...
         for(int i1 = 0; i1 < 16; ++i1) {
            ...
            if (this.storageArrays[j] == null && flag
               || this.storageArrays[j] != null && this.storageArrays[j].getBlockByExtId(k, i1, l).getMaterial() == Material.air) {
