...
public class WorldGenBigTree extends WorldGenAbstractTree {
   ...
   int heightLimit;
   int height;
   final double heightAttenuation = 0.618;
   final double branchSlope = 0.381;
   final double scaleWidth = 1.0;
   final double leafDensity = 1.0;
   ...
   void generateLeafNodeList() {
      ...
      int i = (int)(1.382 + Math.pow(1.0 * (double)this.heightLimit / 13.0, 2.0));
      if (i < 1) {
         i = 1;
      }
      ...
      for(; k >= 0; --k) {
         ...
         if (f >= 0.0F) {
            for(int l = 0; l < i; ++l) {
               double d0 = 1.0 * (double)f * ((double)this.rand.nextFloat() + 0.328);
               ...
               if (this.checkBlockLine(blockpos, blockpos1) == -1) {
                  ...
                  double d4 = (double)blockpos.getY() - Math.sqrt((double)(i1 * i1 + j1 * j1)) * 0.381;
   ...
   public boolean generate(World worldIn, Random rand, BlockPos position) {
      ...
      if (this.heightLimit == 0) {
         this.heightLimit = 5 + this.rand.nextInt(12);
      }
