...
public class WorldServer extends World implements IThreadListener {
   ...
   protected BlockPos adjustPosToNearbyEntity(BlockPos pos) {
      ...
      List<EntityLivingBase> list = this.getEntitiesWithinAABB(
         EntityLivingBase.class, axisalignedbb, p_apply_1_ -> p_apply_1_ != null && p_apply_1_.isEntityAlive() && this.canSeeSky(p_apply_1_.getPosition())
      );
   ...
   public void updateBlockTick(BlockPos pos, Block blockIn, int delay, int priority) {
      ...
      if (this.scheduledUpdatesAreImmediate && blockIn.getMaterial() != Material.air) {
         if (blockIn.requiresUpdates()) {
            int var8 = 8;
            if (this.isAreaLoaded(nextticklistentry.position.add(-var8, -var8, -var8), nextticklistentry.position.add(var8, var8, var8))) {
   ...
   public boolean addWeatherEffect(Entity entityIn) {
      if (super.addWeatherEffect(entityIn)) {
         this.mcServer
            ...
            .sendToAllNear(entityIn.posX, entityIn.posY, entityIn.posZ, 512.0, this.provider.getDimensionId(), new S2CPacketSpawnGlobalEntity(entityIn));
         return true;
      } else {
         return false;
      }
   ...
   protected void updateWeather() {
      ...
      if (this.prevRainingStrength != this.rainingStrength) {
         this.mcServer
            .getConfigurationManager()
            .sendPacketToAllPlayersInDimension(new S2BPacketChangeGameState(7, this.rainingStrength), this.provider.getDimensionId());
      }

      if (this.prevThunderingStrength != this.thunderingStrength) {
         this.mcServer
            .getConfigurationManager()
            .sendPacketToAllPlayersInDimension(new S2BPacketChangeGameState(8, this.thunderingStrength), this.provider.getDimensionId());
      }
   ...
   public void spawnParticle(
      ...
      Packet packet = new S2APacketParticles(
         particleType,
         longDistance,
         (float)xCoord,
         (float)yCoord,
