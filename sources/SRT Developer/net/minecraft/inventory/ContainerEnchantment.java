...
public class ContainerEnchantment extends Container {
   public final IInventory tableInventory = new InventoryBasic("Enchant", true, 2) {
      @Override
   ...
   public final int[] field_178151_h = new int[]{-1, -1, -1};
         ...
         if (itemstack != null && itemstack.isItemEnchantable()) {
            if (!this.worldPointer.isRemote) {
               ...
               for(int j = -1; j <= 1; ++j) {
                  for(int k = -1; k <= 1; ++k) {
                     if ((j != 0 || k != 0)
                        && this.worldPointer.isAirBlock(this.position.add(k, 0, j))
   ...
   public boolean canInteractWith(EntityPlayer playerIn) {
      return this.worldPointer.getBlockState(this.position).getBlock() != Blocks.enchanting_table
         || !(playerIn.getDistanceSq((double)this.position.getX() + 0.5, (double)this.position.getY() + 0.5, (double)this.position.getZ() + 0.5) <= 64.0);
   }
