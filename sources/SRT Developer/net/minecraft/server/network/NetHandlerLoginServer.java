...
public class NetHandlerLoginServer implements INetHandlerLoginServer, ITickable {
         ...
         if (this.server.getNetworkCompressionTreshold() >= 0 && !this.networkManager.isLocalChannel()) {
            this.networkManager
               .sendPacket(
                  ...
                  (ChannelFutureListener)p_operationComplete_1_ -> this.networkManager.setCompressionTreshold(this.server.getNetworkCompressionTreshold())
               );
         }
   ...
   public String getConnectionInfo() {
      return this.loginGameProfile != null
         ? this.loginGameProfile + " (" + this.networkManager.getRemoteAddress().toString() + ")"
   ...
   public void processLoginStart(C00PacketLoginStart packetIn) {
      ...
      if (this.server.isServerInOnlineMode() && !this.networkManager.isLocalChannel()) {
         ...
         this.networkManager.sendPacket(new S01PacketEncryptionRequest("", this.server.getKeyPair().getPublic(), this.verifyToken));
      } else {
         this.currentLoginState = NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
      }
   }
      ...
      if (!Arrays.equals(this.verifyToken, packetIn.getVerifyToken(privatekey))) {
         ...
         (new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
               ...
               public void run() {
                  ...
                  } catch (AuthenticationUnavailableException var31) {
