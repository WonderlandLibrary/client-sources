...
public class ConnectedTextures {
   ...
   public static BakedQuad[] getConnectedTexture(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, BakedQuad quad, RenderEnv renderEnv) {
      ...
      } else if (skipConnectedTexture(blockAccess, blockState, blockPos, quad)) {
      ...
      if (block instanceof BlockPane) {
         ...
         if (enumfacing != EnumFacing.UP && enumfacing != EnumFacing.DOWN) {
            ...
            if (iblockstate.getBlock() != block) {
               ...
               && iblockstate.getValue(BlockStainedGlassPane.COLOR) != blockState.getValue(BlockStainedGlassPane.COLOR)) {
               return false;
            } else {
               ...
               if (d0 < 0.4) {
                  ...
                  if (d1 < 0.4) {
                     ...
                     return d1 <= 0.6 ? true : iblockstate.getValue(BlockPane.SOUTH);
                  }
               }
            }
         }
   ...
   private static BakedQuad[] getConnectedTextureMultiPass(
      IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, EnumFacing side, BakedQuad quad, RenderEnv renderEnv
   ) {
   ...
   private static BakedQuad[] getConnectedTexture(
      ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, BlockPos blockPos, int side, BakedQuad quad, int pass, RenderEnv renderEnv
   ) {
      int i = 0;
      ...
      if (cp.matchesBlock(blockState.getBlockId(), k)) {
         ...
         if (cp.heights != null && !cp.heights.isInRange(i1)) {
            ...
            switch(cp.method) {
               ...
               case 8:
                  return getQuads(
                     getConnectedTextureHorizontalVertical(cp, blockAccess, blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv
                  );
               case 9:
                  return getQuads(
                     getConnectedTextureVerticalHorizontal(cp, blockAccess, blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv
                  );
               case 10:
                  if (pass == 0) {
   ...
   private static TextureAtlasSprite getConnectedTextureRandom(
      ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, BlockPos blockPos, int side
   ) {
      if (cp.tileIcons.length == 1) {
         ...
         if (cp.linked) {
            ...
            for(IBlockState iblockstate = blockAccess.getBlockState(blockpos);
               iblockstate.getBlock() == blockState.getBlock();
         ...
         int l = Config.getRandom(blockPos, i) & 2147483647;
      ...
      if (quad.isFullQuad()) {
         ...
         try {
            ...
            if (!aboolean[0] || !aboolean[3]) {
               ...
               for(int k = 0; k < 4; ++k) {
                  aboolean2[k] = isNeighbourMatching(cp, blockAccess, ablockdir[k].offset(blockPos), side, textureatlassprite);
               }
   ...
   private static BakedQuad[] getConnectedTextureOverlayRandom(
      ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, BlockPos blockPos, int side, BakedQuad quad, RenderEnv renderEnv
   ) {
      if (quad.isFullQuad()) {
         ListQuadsOverlay listquadsoverlay = renderEnv.getListQuadsOverlay(cp.layer);
   ...
   private static BakedQuad[] getConnectedTextureOverlayCtm(
      ...
      if (quad.isFullQuad()) {
         ...
         try {
            TextureAtlasSprite textureatlassprite = getConnectedTextureCtm(
               cp, blockAccess, blockState, blockPos, vertAxis, side, quad.getSprite(), metadata, renderEnv
            );
            if (textureatlassprite != null) {
               listquadsoverlay.addQuad(getQuadFull(textureatlassprite, quad, cp.tintIndex), cp.tintBlockState);
            }
   ...
   private static boolean isNeighbourOverlay(
      ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata
   ) {
      ...
      if (!isFullCubeModel(iblockstate)) {
         ...
         if (cp.connectTileIcons != null) {
            TextureAtlasSprite textureatlassprite = getNeighbourIcon(iblockaccess, blockPos, iblockstate, side);
   ...
   private static boolean isNeighbour(
      ...
      if (blockState == neighbourState) {
         ...
         if (neighbourState == null) {
            ...
            TextureAtlasSprite textureatlassprite = getNeighbourIcon(iblockaccess, blockPos, neighbourState, side);
            return textureatlassprite == icon;
         }
      } else if (cp.connect == 3) {
         return neighbourState != null
            && neighbourState != AIR_DEFAULT_STATE
   ...
   private static TextureAtlasSprite getNeighbourIcon(IBlockAccess iblockaccess, BlockPos blockPos, IBlockState neighbourState, int side) {
      ...
      if (ibakedmodel == null) {
         ...
         if (Reflector.ForgeBlock_getExtendedState.exists()) {
            neighbourState = (IBlockState)Reflector.call(
               neighbourState.getBlock(), Reflector.ForgeBlock_getExtendedState, neighbourState, iblockaccess, blockPos
            );
         }
   ...
   private static TextureAtlasSprite getConnectedTextureHorizontalVertical(
      ...
      if (textureatlassprite != null && textureatlassprite != icon && textureatlassprite != atextureatlassprite[3]) {
         ...
         return textureatlassprite1 == atextureatlassprite[0]
            ? atextureatlassprite[4]
            : (
               textureatlassprite1 == atextureatlassprite[1]
                  ? atextureatlassprite[5]
                  : (textureatlassprite1 == atextureatlassprite[2] ? atextureatlassprite[6] : textureatlassprite1)
            );
      }
   }
