...
public class ExpressionParser {
   ...
   private static IExpression makeConstantFloat(Token token) {
   ...
   private FunctionType getFunctionType(Token tokens, Deque<Token> deque) throws ParseException {
      ...
      if (tokens != null && Objects.requireNonNull(token).getType() == TokenType.BRACKET_OPEN) {
         FunctionType functiontype1 = FunctionType.parse(token.getText());
   ...
   private static IExpression makeFunction(FunctionType type, IExpression[] args) throws ParseException {
      ...
      if (args.length != aexpressiontype.length) {
         throw new ParseException(
            "Invalid number of arguments, function: \"" + type.getName() + "\", count arguments: " + args.length + ", should be: " + aexpressiontype.length
         );
      } else {
         for(int i = 0; i < args.length; ++i) {
            ...
            if (expressiontype != expressiontype1) {
               throw new ParseException(
                  "Invalid argument type, function: \"" + type.getName() + "\", index: " + i + ", type: " + expressiontype + ", should be: " + expressiontype1
               );
            }
         }
