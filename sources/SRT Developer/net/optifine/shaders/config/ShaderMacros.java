...
public class ShaderMacros {
   ...
   public static String getVendor() {
      ...
      if (s == null) {
         ...
         s = s.toLowerCase();
         return s.startsWith("ati")
            ? "MC_GL_VENDOR_ATI"
            : (
               s.startsWith("intel")
                  ? "MC_GL_VENDOR_INTEL"
                  : (s.startsWith("nvidia") ? "MC_GL_VENDOR_NVIDIA" : (s.startsWith("x.org") ? "MC_GL_VENDOR_XORG" : "MC_GL_VENDOR_OTHER"))
            );
      }
   }
                                                ...
                                                : (
                                                   s.startsWith("quadro")
                                                      ...
                                                      : (
                                                         s.startsWith("nvs")
                                                            ? "MC_GL_RENDERER_QUADRO"
                                                            : (s.startsWith("mesa") ? "MC_GL_RENDERER_MESA" : "MC_GL_RENDERER_OTHER")
                                                      )
                                                )
                                          )
                                    )
   ...
   public static ShaderMacro[] getExtensions() {
      if (extensionMacros == null) {
         ...
         for(int i = 0; i < astring.length; ++i) {
            ashadermacro[i] = new ShaderMacro("MC_" + astring[i], "");
         }
