...
public class TextBox extends Gui {
    ...
    public TextBox(int componentId, double x, double y, double par5Width, double par6Height) {
        this.id = componentId;
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            if (this.getEnableBackgroundDrawing()) {
                RenderUtils.drawRoundedRect(this.xPosition, this.yPosition, this.width, this.height, 3, backgroundColor);
                RenderUtils.drawRoundedRectOutline(
                        this.xPosition, this.yPosition, this.width, this.height, 3, Color.WHITE
                );
            }
            ...
            String s = this.fontRendererInstance.trimStringToWidth(this.text.substring(this.lineScrollOffset), (int) this.getWidth());
            boolean flag = j >= 0 && j <= s.length();
            boolean flag1 = this.isFocused && this.cursorCounter / 6 % 2 == 0 && flag;
            double l = this.enableBackgroundDrawing ? this.xPosition + 4 : this.xPosition;
            double i1 = this.enableBackgroundDrawing ? this.yPosition + (this.height - 8) / 2 : this.yPosition;
            double j1 = l;
            ...
            if (k != j) {
                double l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        boolean flag = mouseX >= this.xPosition + 1 && mouseX < this.xPosition + this.width + 1 && mouseY >= this.yPosition + 2 && mouseY < this.yPosition + this.height + 2;

        if (this.canLoseFocus) {
            this.setFocused(!isFocused && flag);
        }

        if (this.isFocused && flag && !firstClick) {
            double i = mouseX - this.xPosition;
     ...
     */
    private void drawCursorVertical(double startX, double startY, double endX, double endY) {
        if (startX < endX) {
            double i = startX;
            startX = endX;
            endX = i;
    ...
    public void setSelectionPos(int position) {
        ...
        if (this.fontRendererInstance != null) {
            ...
            String s = this.fontRendererInstance.trimStringToWidth(this.text.substring(this.lineScrollOffset), (int) j);
            int k = s.length() + this.lineScrollOffset;

            if (position == this.lineScrollOffset) {
                this.lineScrollOffset -= this.fontRendererInstance.trimStringToWidth(this.text, (int) j, true).length();
            }
