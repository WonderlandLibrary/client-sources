...
public class HazelMCDisabler extends ModuleMode<DisablerModule> {
    public HazelMCDisabler(String name, DisablerModule parent) {
        super(name, parent);
    }
    ...
    private final EventCallback<PacketEvent> packetEventEventCallback = event -> {
        ...
        if (event.getPacket() instanceof S08PacketPlayerPosLook && this.expectedTeleport) {
            ...
            if (mc.thePlayer.ticksExisted > 50) {
                event.cancel();
                mc.thePlayer.sendQueue.getNetworkManager().sendPacketNoEvent(new C03PacketPlayer.C06PacketPlayerPosLook(packet.getX(), packet.getY(), packet.getZ(), packet.getYaw(), packet.getPitch(), true));
            }
        }
