...
public class VulcanStep extends ModuleMode<StepModule> {
    ...
    EventCallback<MotionEvent> onMotion = event -> {
        ...
        if (this.offsets.isEmpty()) {
            this.offsets.put(1f, new float[]{0.41999998688698f, 0.753f});
    ...
    EventCallback<StepEvent> onStep = event -> {
        ...
        if (!mc.thePlayer.isCollidedHorizontally || PlayerUtils.inLiquid() || mc.thePlayer.isOnLadder()) return;
        ...
        if ((System.currentTimeMillis() - lastPacket) > (stepHeight > 1.00 ? 50L : 100L)) {
            offsets.forEach((a, b) -> {
                if (a == stepHeight) {
                    for (double ab : b) {
