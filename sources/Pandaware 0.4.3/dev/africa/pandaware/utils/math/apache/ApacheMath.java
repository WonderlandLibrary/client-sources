 ...
 */
package dev.africa.pandaware.utils.math.apache;
 ...
 * methods as is (using {@code IronMath.sin(x)} or {@code IronMath.cbrt(y)}
...
public class ApacheMath {
    ...
    public static double acosh(final double a) {
        return ApacheMath.log(a + ApacheMath.sqrt(a * a - 1));
    }
    ...
    public static double asinh(double a) {
        ...
        double absAsinh;
        if (a > 0.167) {
            absAsinh = ApacheMath.log(ApacheMath.sqrt(a * a + 1) + a);
        } else {
            final double a2 = a * a;
            if (a > 0.097) {
    ...
    public static double atanh(double a) {
        ...
        double absAtanh;
        if (a > 0.15) {
            absAtanh = 0.5 * ApacheMath.log((1 + a) / (1 - a));
        } else {
            final double a2 = a * a;
            if (a > 0.087) {
                ...
                if (yRawExp >= 1023) {
                    ...
                    if (yRawExp < 1075) {
                        ...
                        if ((yFullMantissa & integralMask) == yFullMantissa) {
                            ...
                            return ApacheMath.pow(x, (y < 0) ? -l : l);
                        }
                    } else {
        ...
        private Split pow(final long e) {
            ...
            if (Double.isNaN(result.full)) {
                if (Double.isNaN(full)) {
                    ...
                    if (ApacheMath.abs(full) < 1) {
                        return new Split(ApacheMath.copySign(0.0, full), 0.0);
    ...
    private static class ExpIntTable {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                EXP_INT_TABLE_A = new double[ApacheMath.EXP_INT_TABLE_LEN];
                EXP_INT_TABLE_B = new double[ApacheMath.EXP_INT_TABLE_LEN];

                final double tmp[] = new double[2];
                final double recip[] = new double[2];

                // Populate expIntTable
                for (int i = 0; i < ApacheMath.EXP_INT_TABLE_MAX_INDEX; i++) {
                   // ApacheMath.expint(i, tmp);
                    EXP_INT_TABLE_A[i + ApacheMath.EXP_INT_TABLE_MAX_INDEX] = tmp[0];
                    EXP_INT_TABLE_B[i + ApacheMath.EXP_INT_TABLE_MAX_INDEX] = tmp[1];

                    if (i != 0) {
                        // Negative integer powers
                   //     FastMathCalc.splitReciprocal(tmp, recip);
                        EXP_INT_TABLE_A[ApacheMath.EXP_INT_TABLE_MAX_INDEX - i] = recip[0];
                        EXP_INT_TABLE_B[ApacheMath.EXP_INT_TABLE_MAX_INDEX - i] = recip[1];
                    }
                }
            } else {
    ...
    private static class ExpFracTable {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                EXP_FRAC_TABLE_A = new double[ApacheMath.EXP_FRAC_TABLE_LEN];
                EXP_FRAC_TABLE_B = new double[ApacheMath.EXP_FRAC_TABLE_LEN];
    ...
    private static class lnMant {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                LN_MANT = new double[ApacheMath.LN_MANT_LEN][];
