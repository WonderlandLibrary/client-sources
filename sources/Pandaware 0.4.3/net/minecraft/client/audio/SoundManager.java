...
public class SoundManager {
        ...
        while (iterator.hasNext()) {
            ...
            if (!this.sndSystem.playing(s1)) {
                ...
                if (i <= this.playTime) {
                    ...
                    iterator.remove();
                    Logger.debug("Removed channel {} because it's not playing anymore", new Object[]{s1});
    ...
    public void playSound(ISound sound) {
        if (this.loaded) {
            if (this.sndSystem.getMasterVolume() <= 0.0F) {
                Logger.debug("Skipped playing soundEvent: {}, master volume was zero", new Object[]{sound.getSoundLocation()});
            } else {
                SoundEventAccessorComposite soundeventaccessorcomposite = this.sndHandler.getSound(sound.getSoundLocation());

                if (soundeventaccessorcomposite == null) {
                    Logger.warn("Unable to play unknown soundEvent: {}", new Object[]{sound.getSoundLocation()});
                } else {
                    SoundPoolEntry soundpoolentry = soundeventaccessorcomposite.cloneEntry();

                    if (soundpoolentry == SoundHandler.missing_sound) {
                        Logger.warn("Unable to play empty soundEvent: {}", new Object[]{soundeventaccessorcomposite.getSoundEventLocation()});
                    } else {
                        ...
                        if (f2 == 0.0F) {
                            Logger.debug("Skipped playing sound {}, volume was zero.", new Object[]{resourcelocation});
                        } else {
                            ...
                            Logger.debug("Playing sound {} for event {} as channel {}", new Object[]{soundpoolentry.getSoundPoolEntryLocation(), soundeventaccessorcomposite.getSoundEventLocation(), s});
    ...
    public void pauseAllSounds() {
        for (String s : this.playingSounds.keySet()) {
            Logger.debug("Pausing channel {}", new Object[]{s});
    ...
    public void resumeAllSounds() {
        for (String s : this.playingSounds.keySet()) {
            Logger.debug("Resuming channel {}", new Object[]{s});
            this.sndSystem.play(s);
        }
    }
