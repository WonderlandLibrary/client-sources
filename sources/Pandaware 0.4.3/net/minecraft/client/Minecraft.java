...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                this.freeMemory();
                Logger.error("Reported exception thrown!", reportedexception);
    ...
    private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
        ...
        worldrenderer.pos(0.0D, this.displayHeight, 0.0D).tex(0.0D, 0.0D).func_181669_b(255, 255, 255, 255).endVertex();
        worldrenderer.pos(this.displayWidth, this.displayHeight, 0.0D).tex(0.0D, 0.0D).func_181669_b(255, 255, 255, 255).endVertex();
        worldrenderer.pos(this.displayWidth, 0.0D, 0.0D).tex(0.0D, 0.0D).func_181669_b(255, 255, 255, 255).endVertex();
    ...
    private void runGameLoop() throws IOException {
        ...
        Client.getInstance().setRenderDeltaTime(deltaTime);

        Client.getInstance().getEventDispatcher().dispatch(new GameLoopEvent());
        ...
        synchronized (this.scheduledTasks) {
            while (!this.scheduledTasks.isEmpty()) {
                Util.func_181617_a((FutureTask) this.scheduledTasks.poll());
            }
        }
...
//        this.mcProfiler.clearProfiling();
    ...
    public void runTick() throws IOException {
        Client.getInstance().getEventDispatcher().dispatch(new TickEvent());
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        Client.getInstance().getEventDispatcher().dispatch(new KeyEvent(k, KeyEvent.Type.NULL_SCREEN));
