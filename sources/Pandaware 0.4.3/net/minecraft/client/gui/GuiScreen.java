...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    protected List<CustomButton> customButtons = Lists.<CustomButton>newArrayList();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < this.customButtons.size(); ++i) {
            ((CustomButton) this.customButtons.get(i)).drawButton(this.mc, mouseX, mouseY);
        }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (keyCode == 1 && !(mc.currentScreen instanceof ClickGUI)) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton == 0) {
            ...
            for (int i = 0; i < this.customButtons.size(); ++i) {
                CustomButton customButton = this.customButtons.get(i);
    ...
    public void handleMouseInput() throws IOException {
        ...
        if (Mouse.getEventButtonState()) {
            ...
            Client.getInstance().getEventDispatcher().dispatch(new MouseEvent(i, j, this.eventButton, MouseEvent.Type.CLICK));
        } else if (k != -1) {
            if (this.mc.gameSettings.touchscreen && --this.touchValue > 0) {
                return;
            }
            ...
            Client.getInstance().getEventDispatcher().dispatch(new MouseEvent(i, j, this.eventButton, MouseEvent.Type.RELEASED));
        } else if (this.eventButton != -1 && this.lastMouseEvent > 0L) {
            long l = Minecraft.getSystemTime() - this.lastMouseEvent;
            this.mouseClickMove(i, j, this.eventButton, l);
            Client.getInstance().getEventDispatcher().dispatch(new MouseEvent(i, j, this.eventButton, MouseEvent.Type.CLICK_MOVE));
        }
    }
