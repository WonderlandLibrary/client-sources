...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    @Setter
    @Getter
    private boolean isBlockingSword;

    @Setter
    @Getter
    private boolean isAnimateBlocking;
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            Client.getInstance().getEventDispatcher().dispatch(new UpdateEvent());
            ...
            if (this.isRiding()) {
                var motionEvent = new MotionEvent(this.posX, this.posY, this.posZ,
                        this.rotationYaw, this.rotationPitch, this.onGround, Event.EventState.PRE);
                Client.getInstance().getEventDispatcher().dispatch(motionEvent);
    ...
    public void onUpdateWalkingPlayer() {
        ...
        motionEvent.setEventState(Event.EventState.POST);
        Client.getInstance().getEventDispatcher().dispatch(motionEvent);
    }
    ...
    void setThirdPersonRotations(MotionEvent event) {
    ...
    public void sendChatMessage(String message) {
        var chatEvent = new ChatEvent(message);
        Client.getInstance().getEventDispatcher().dispatch(chatEvent);
    ...
    public float getCooldownPeriod() {
        ...
        if (this.getHeldItem() != null) {
            ...
            if (item instanceof ItemSword) {
                ...
                String name = ((ItemAxe) item).getToolMaterial().getRepairItem().getUnlocalizedName();
    ...
    public void moveEntity(double x, double y, double z) {
        var moveEvent = new MoveEvent(x, y, z);
        Client.getInstance().getEventDispatcher().dispatch(moveEvent);

        if (moveEvent.isCancelled()) {
            return;
        }

        TargetStrafeModule targetStrafeModule = Client.getInstance().getModuleManager()
                .getByClass(TargetStrafeModule.class);
        if (targetStrafeModule.getData().isEnabled()) {
            moveEvent = targetStrafeModule.editMovement(moveEvent.x, moveEvent.y, moveEvent.z);
        }
