...
import dev.africa.pandaware.impl.module.render.AnimationsModule;
...
public class ItemRenderer {
    ...
    public void renderItem(EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        if (heldStack != null) {
            AnimationsModule animationsModule = Client.getInstance().getModuleManager().getByClass(AnimationsModule.class);
            if (animationsModule.getData().isEnabled()) {
                float scale = animationsModule.getItemScale().getValue().floatValue();
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            AnimationsModule animationsModule = Client.getInstance().getModuleManager().getByClass(AnimationsModule.class);
            float offset = animationsModule.getData().isEnabled() ? animationsModule.getItemOffset().getValue().floatValue() : 0;

            float f = (1.0F - offset) - (this.prevEquippedProgress + (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
            ...
            if (animationsModule.getData().isEnabled()) {
                GlStateManager.scale(1, 1, animationsModule.getItemDistance().getValue().floatValue());
            }
            ...
            if (this.itemToRender != null) {
                ...
                } else if (abstractclientplayer.getItemInUseCount() > 0 || blocking) {
                    EnumAction enumaction = (blocking ? EnumAction.BLOCK : this.itemToRender.getItemUseAction());
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            if (!animationsModule.getData().isEnabled()) {
                                ...
                                switch (animationsModule.getMode().getValue()) {
                                    case VANILLA:
                                        this.transformFirstPersonItem(f, 0.0F);
                                        this.func_178103_d();
                                        break;
...
//                                        this.func_178103_d();
                                    ...
                                    case SWANG:
                                        this.transformFirstPersonItem(f / 2.0F - 0.2f, f1);
                ...
                ChamsModule chamsModule = Client.getInstance().getModuleManager().getByClass(ChamsModule.class);

                if (chamsModule.getData().isEnabled()) {
                    GL11.glPushMatrix();
                    ...
                    ColorUtils.glColor(chamsModule.getVisibleColor().getValue());
                    GL11.glLineWidth(1);
