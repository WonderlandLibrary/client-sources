...
extends GuiScreen {
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if (keyCode == 1) {
            ...
            this.mc.displayGuiScreen(null);
            if (s2.length() > 0) {
                ChatEvent event = new ChatEvent(s2);
                EventManager.callEvent(event);
                if (event.isCanceled()) {
                    if (!event.isCanceledLogging()) {
                        this.mc.ingameGUI.getChatGUI().sentMessages.add(event.getMessage());
                    }
                    return;
                }
                this.sendChatMessage(event.getMessage());
            }
        }
    }
