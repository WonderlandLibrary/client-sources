...
ITickable {
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        ...
        for (String s2 : resourceManager.getResourceDomains()) {
            try {
                for (IResource iresource : resourceManager.getAllResources(new ResourceLocation(s2, "sounds.json"))) {
                    try {
                        Map<String, SoundList> map = this.getSoundMap(iresource.getInputStream());
                        for (Map.Entry<String, SoundList> entry : map.entrySet()) {
                            this.loadSoundResource(new ResourceLocation(s2, entry.getKey()), entry.getValue());
                        }
                    }
    ...
    private void loadSoundResource(ResourceLocation location, SoundList sounds) {
        ...
        block10: for (final SoundList.SoundEntry soundlist$soundentry : sounds.getSoundList()) {
            ...
            String s2 = soundlist$soundentry.getSoundEntryName();
            ResourceLocation resourcelocation = new ResourceLocation(s2);
            final String s1 = s2.contains(":") ? resourcelocation.getResourceDomain() : location.getResourceDomain();
