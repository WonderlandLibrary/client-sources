...
IStream {
            ...
            Thread thread = new Thread("Twitch authenticator"){
                ...
                public void run() {
                    try {
                        ...
                        JsonObject jsonobject = JsonUtils.getJsonObject(new JsonParser().parse(s2), "Response");
    ...
    public void func_152935_j() {
        boolean flag1;
        int i2 = this.mc.gameSettings.streamChatEnabled;
    ...
    protected void func_152942_I() {
        ...
        this.field_176029_e = s2 = this.broadcastController.getChannelInfo().name;
        if (chatcontroller$chatstate != ChatController.ChatState.Initialized) {
            ...
            this.chatController.func_152986_d(s2);
        } else {
            LOGGER.warn("Invalid twitch chat state {}", new Object[]{chatcontroller$chatstate});
        }
    }
    ...
    public void func_152922_k() {
        if (this.broadcastController.isBroadcasting() && !this.broadcastController.isBroadcastPaused()) {
            ...
            long k2 = i2 - this.field_152959_k;
            boolean bl = flag = k2 >= (j2 = (long)(1000000000 / this.targetFPS));
            if (flag) {
                ...
                float f2 = this.framebuffer.framebufferWidth;
                float f1 = this.framebuffer.framebufferHeight;
                float f22 = (float)framebuffer1.framebufferWidth / (float)framebuffer1.framebufferTextureWidth;
                ...
                worldrenderer.pos(f2, f1, 0.0).tex(f22, f3).endVertex();
                worldrenderer.pos(f2, 0.0, 0.0).tex(f22, 0.0).endVertex();
    ...
    public void func_152911_a(Metadata p_152911_1_, long p_152911_2_) {
        if (this.isBroadcasting() && this.field_152957_i) {
            long i2 = this.broadcastController.func_152844_x();
            if (!this.broadcastController.func_152840_a(p_152911_1_.func_152810_c(), i2 + p_152911_2_, p_152911_1_.func_152809_a(), p_152911_1_.func_152806_b())) {
                LOGGER.warn(STREAM_MARKER, "Couldn't send stream metadata action at {}: {}", i2 + p_152911_2_, p_152911_1_);
            } else {
                LOGGER.debug(STREAM_MARKER, "Sent stream metadata action at {}: {}", i2 + p_152911_2_, p_152911_1_);
            }
        }
    }
    ...
    public void func_176026_a(Metadata p_176026_1_, long p_176026_2_, long p_176026_4_) {
        if (this.isBroadcasting() && this.field_152957_i) {
            long i2 = this.broadcastController.func_152844_x();
            String s2 = p_176026_1_.func_152809_a();
            String s1 = p_176026_1_.func_152806_b();
            long j2 = this.broadcastController.func_177946_b(p_176026_1_.func_152810_c(), i2 + p_176026_2_, s2, s1);
            if (j2 < 0L) {
                LOGGER.warn(STREAM_MARKER, "Could not send stream metadata sequence from {} to {}: {}", i2 + p_176026_2_, i2 + p_176026_4_, p_176026_1_);
            } else if (this.broadcastController.func_177947_a(p_176026_1_.func_152810_c(), i2 + p_176026_4_, j2, s2, s1)) {
                LOGGER.debug(STREAM_MARKER, "Sent stream metadata sequence from {} to {}: {}", i2 + p_176026_2_, i2 + p_176026_4_, p_176026_1_);
            } else {
                LOGGER.warn(STREAM_MARKER, "Half-sent stream metadata sequence from {} to {}: {}", i2 + p_176026_2_, i2 + p_176026_4_, p_176026_1_);
            }
        }
    }
    ...
    public void updateStreamVolume() {
        if (this.isBroadcasting()) {
            float f2 = this.mc.gameSettings.streamGameVolume;
            boolean flag = this.field_152962_n || f2 <= 0.0f;
            this.broadcastController.setPlaybackDeviceVolume(flag ? 0.0f : f2);
