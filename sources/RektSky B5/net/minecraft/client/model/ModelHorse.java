...
extends ModelBase {
    ...
    public void render(Entity entityIn, float p_78088_2_, float p_78088_3_, float p_78088_4_, float p_78088_5_, float p_78088_6_, float scale) {
        ...
        int i2 = entityhorse.getHorseType();
        float f2 = entityhorse.getGrassEatingAmount(0.0f);
        ...
        if (!flag) {
            GlStateManager.popMatrix();
            GlStateManager.pushMatrix();
            float f22 = 0.5f + f1 * f1 * 0.5f;
            GlStateManager.scale(f22, f22, f22);
            if (f2 <= 0.0f) {
                GlStateManager.translate(0.0f, 1.35f * (1.0f - f1), 0.0f);
            } else {
                GlStateManager.translate(0.0f, 0.9f * (1.0f - f1) * f2 + 1.35f * (1.0f - f1) * (1.0f - f2), 0.15f * (1.0f - f1) * f2);
            }
        }
        if (flag3) {
    ...
    private float updateHorseRotation(float p_110683_1_, float p_110683_2_, float p_110683_3_) {
        float f2;
        for (f2 = p_110683_2_ - p_110683_1_; f2 < -180.0f; f2 += 360.0f) {
        }
        while (f2 >= 180.0f) {
            f2 -= 360.0f;
        }
        return p_110683_1_ + p_110683_3_ * f2;
    }

    @Override
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float p_78086_2_, float p_78086_3_, float partialTickTime) {
        super.setLivingAnimations(entitylivingbaseIn, p_78086_2_, p_78086_3_, partialTickTime);
        float f2 = this.updateHorseRotation(entitylivingbaseIn.prevRenderYawOffset, entitylivingbaseIn.renderYawOffset, partialTickTime);
        ...
        float f22 = entitylivingbaseIn.prevRotationPitch + (entitylivingbaseIn.rotationPitch - entitylivingbaseIn.prevRotationPitch) * partialTickTime;
        float f3 = f1 - f2;
        float f4 = f22 / 57.295776f;
        if (f3 > 20.0f) {
            f3 = 20.0f;
        }
        if (f3 < -20.0f) {
