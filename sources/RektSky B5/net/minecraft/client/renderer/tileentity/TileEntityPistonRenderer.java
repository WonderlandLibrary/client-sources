...
extends TileEntitySpecialRenderer<TileEntityPiston> {
    ...
    @Override
    public void renderTileEntityAt(TileEntityPiston te, double x2, double y2, double z2, float partialTicks, int destroyStage) {
        ...
        if (block.getMaterial() != Material.air && te.getProgress(partialTicks) < 1.0f) {
            ...
            worldrenderer.setTranslation((float)x2 - (float)blockpos.getX() + te.getOffsetX(partialTicks), (float)y2 - (float)blockpos.getY() + te.getOffsetY(partialTicks), (float)z2 - (float)blockpos.getZ() + te.getOffsetZ(partialTicks));
