...
extends TileEntitySpecialRenderer<TileEntityEndPortal> {
    ...
    @Override
    public void renderTileEntityAt(TileEntityEndPortal te, double x2, double y2, double z2, float partialTicks, int destroyStage) {
        float f2 = (float)this.rendererDispatcher.entityX;
        float f1 = (float)this.rendererDispatcher.entityY;
        float f22 = (float)this.rendererDispatcher.entityZ;
        ...
        for (int i2 = 0; i2 < 16; ++i2) {
            ...
            GlStateManager.rotate((float)(i2 * i2 * 4321 + i2 * 9) * 2.0f, 0.0f, 0.0f, 1.0f);
            GlStateManager.translate(-0.5f, -0.5f, 0.0f);
            GlStateManager.translate(-f2, -f22, -f1);
            ...
            worldrenderer.pos(x2, y2 + (double)f3, z2).color(f11, f12, f13, 1.0f).endVertex();
            worldrenderer.pos(x2, y2 + (double)f3, z2 + 1.0).color(f11, f12, f13, 1.0f).endVertex();
            worldrenderer.pos(x2 + 1.0, y2 + (double)f3, z2 + 1.0).color(f11, f12, f13, 1.0f).endVertex();
            worldrenderer.pos(x2 + 1.0, y2 + (double)f3, z2).color(f11, f12, f13, 1.0f).endVertex();
            tessellator.draw();
            GlStateManager.popMatrix();
