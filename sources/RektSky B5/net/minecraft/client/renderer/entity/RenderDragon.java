...
extends RenderLiving<EntityDragon> {
    ...
    protected void rotateCorpse(EntityDragon bat, float p_77043_2_, float p_77043_3_, float partialTicks) {
        float f2 = (float)bat.getMovementOffsets(7, partialTicks)[0];
        ...
        if (bat.deathTime > 0) {
            float f22 = ((float)bat.deathTime + partialTicks - 1.0f) / 20.0f * 1.6f;
            if ((f22 = MathHelper.sqrt_float(f22)) > 1.0f) {
                f22 = 1.0f;
            }
            GlStateManager.rotate(f22 * this.getDeathMaxRotation(bat), 0.0f, 0.0f, 1.0f);
        }
    }
    ...
    protected void renderModel(EntityDragon entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        if (entitylivingbaseIn.deathTicks > 0) {
            float f2 = (float)entitylivingbaseIn.deathTicks / 200.0f;
            GlStateManager.depthFunc(515);
            GlStateManager.enableAlpha();
            GlStateManager.alphaFunc(516, f2);
    ...
    @Override
    public void doRender(EntityDragon entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        BossStatus.setBossStatus(entity, false);
        super.doRender(entity, x2, y2, z2, entityYaw, partialTicks);
        if (entity.healingEnderCrystal != null) {
            this.drawRechargeRay(entity, x2, y2, z2, partialTicks);
        }
    }
    ...
    protected void drawRechargeRay(EntityDragon dragon, double p_180574_2_, double p_180574_4_, double p_180574_6_, float p_180574_8_) {
        float f2 = (float)dragon.healingEnderCrystal.innerRotation + p_180574_8_;
        float f1 = MathHelper.sin(f2 * 0.2f) / 2.0f + 0.5f;
        f1 = (f1 * f1 + f1) * 0.2f;
        float f22 = (float)(dragon.healingEnderCrystal.posX - dragon.posX - (dragon.prevPosX - dragon.posX) * (double)(1.0f - p_180574_8_));
        ...
        float f5 = MathHelper.sqrt_float(f22 * f22 + f4 * f4);
        float f6 = MathHelper.sqrt_float(f22 * f22 + f3 * f3 + f4 * f4);
        GlStateManager.pushMatrix();
        GlStateManager.translate((float)p_180574_2_, (float)p_180574_4_ + 2.0f, (float)p_180574_6_);
        GlStateManager.rotate((float)(-Math.atan2(f4, f22)) * 180.0f / (float)Math.PI - 90.0f, 0.0f, 1.0f, 0.0f);
        ...
        float f8 = MathHelper.sqrt_float(f22 * f22 + f3 * f3 + f4 * f4) / 32.0f - ((float)dragon.ticksExisted + p_180574_8_) * 0.01f;
        worldrenderer.begin(5, DefaultVertexFormats.POSITION_TEX_COLOR);
        int i2 = 8;
        for (int j2 = 0; j2 <= 8; ++j2) {
            float f9 = MathHelper.sin((float)(j2 % 8) * (float)Math.PI * 2.0f / 8.0f) * 0.75f;
            float f10 = MathHelper.cos((float)(j2 % 8) * (float)Math.PI * 2.0f / 8.0f) * 0.75f;
            float f11 = (float)(j2 % 8) * 1.0f / 8.0f;
