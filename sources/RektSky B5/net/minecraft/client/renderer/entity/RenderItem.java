...
implements IResourceManagerReloadListener {
    ...
    private void renderQuads(WorldRenderer renderer, List<BakedQuad> quads, int color, ItemStack stack) {
        ...
        for (int i2 = 0; i2 < j2; ++i2) {
            BakedQuad bakedquad = quads.get(i2);
            int k2 = color;
            if (flag && bakedquad.hasTintIndex()) {
                k2 = stack.getItem().getColorFromItemStack(stack, bakedquad.getTintIndex());
                if (EntityRenderer.anaglyphEnable) {
                    k2 = TextureUtil.anaglyphColor(k2);
                }
                k2 |= 0xFF000000;
            }
            this.renderQuad(renderer, bakedquad, k2);
        }
    }
        ...
        if (stack != null && entityToRenderFor != null) {
            ...
            if (entityToRenderFor instanceof EntityPlayer) {
                ...
                if (item == Items.fishing_rod && entityplayer.fishEntity != null) {
                    ...
                    int i2 = stack.getMaxItemUseDuration() - entityplayer.getItemInUseCount();
                    if (i2 >= 18) {
    ...
    public void renderItemIntoGUI(ItemStack stack, int x2, int y2) {
        IBakedModel ibakedmodel = this.itemModelMesher.getItemModel(stack);
        GlStateManager.pushMatrix();
        this.textureManager.bindTexture(TextureMap.locationBlocksTexture);
        this.textureManager.getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false, false);
    ...
    public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, String text) {
        if (stack != null) {
            if (stack.stackSize != 1 || text != null) {
                ...
                GlStateManager.disableBlend();
                fr.drawStringWithShadow(s2, xPosition + 19 - 2 - fr.getStringWidth(s2), yPosition + 6 + 3, 0xFFFFFF);
                GlStateManager.enableLighting();
            ...
            if (stack.isItemDamaged()) {
                ...
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, 12, 1, (255 - i2) / 4, 64, 0, 255);
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, j2, 1, 255 - i2, i2, 0, 255);
                GlStateManager.enableBlend();
                GlStateManager.enableAlpha();
