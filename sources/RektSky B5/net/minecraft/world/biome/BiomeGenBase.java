...
public abstract class BiomeGenBase {
    ...
    public final float getFloatTemperature(BlockPos pos) {
        if (pos.getY() > 64) {
            float f2 = (float)(temperatureNoise.func_151601_a((double)pos.getX() * 1.0 / 8.0, (double)pos.getZ() * 1.0 / 8.0) * 4.0);
            return this.temperature - (f2 + (float)pos.getY() - 64.0f) * 0.05f / 30.0f;
        }
    ...
    public final void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int p_180628_4_, int p_180628_5_, double p_180628_6_) {
        ...
        int j2 = -1;
        int k2 = (int)(p_180628_6_ / 3.0 + 3.0 + rand.nextDouble() * 0.25);
        int l2 = p_180628_4_ & 0xF;
        int i1 = p_180628_5_ & 0xF;
        BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
        for (int j1 = 255; j1 >= 0; --j1) {
            if (j1 <= rand.nextInt(5)) {
                chunkPrimerIn.setBlockState(i1, j1, l2, Blocks.bedrock.getDefaultState());
                continue;
            }
            IBlockState iblockstate2 = chunkPrimerIn.getBlockState(i1, j1, l2);
            ...
            if (j2 == -1) {
                ...
                j2 = k2;
                if (j1 >= i2 - 1) {
                    chunkPrimerIn.setBlockState(i1, j1, l2, iblockstate);
                    continue;
                }
                if (j1 < i2 - 7 - k2) {
                    iblockstate = null;
                    iblockstate1 = Blocks.stone.getDefaultState();
                    chunkPrimerIn.setBlockState(i1, j1, l2, Blocks.gravel.getDefaultState());
                    continue;
                }
                chunkPrimerIn.setBlockState(i1, j1, l2, iblockstate1);
                continue;
            }
            if (j2 <= 0) continue;
            chunkPrimerIn.setBlockState(i1, j1, l2, iblockstate1);
            if (--j2 != 0 || iblockstate1.getBlock() != Blocks.sand) continue;
            j2 = rand.nextInt(4) + Math.max(0, j1 - 63);
