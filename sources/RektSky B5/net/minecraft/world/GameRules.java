...
public class GameRules {
    ...
    public NBTTagCompound writeToNBT() {
        ...
        for (Object s2 : this.theGameRules.keySet()) {
            Value gamerules$value = (Value)this.theGameRules.get(s2);
            nbttagcompound.setString((String)s2, gamerules$value.getString());
        }
        return nbttagcompound;
    }

    public void readFromNBT(NBTTagCompound nbt) {
        for (String s2 : nbt.getKeySet()) {
            String s1 = nbt.getString(s2);
            this.setOrCreateGameRule(s2, s1);
        }
    }
