...
extends MapGenBase {
    ...
    public boolean generateStructure(World worldIn, Random randomIn, ChunkCoordIntPair chunkCoord) {
        this.func_143027_a(worldIn);
        int i2 = (chunkCoord.chunkXPos << 4) + 8;
        int j2 = (chunkCoord.chunkZPos << 4) + 8;
        boolean flag = false;
        for (Object structurestart0 : this.structureMap.values()) {
            StructureStart structurestart = (StructureStart)structurestart0;
            if (!structurestart.isSizeableStructure() || !structurestart.func_175788_a(chunkCoord) || !structurestart.getBoundingBox().intersectsWith(i2, j2, i2 + 15, j2 + 15)) continue;
            structurestart.generateStructure(worldIn, randomIn, new StructureBoundingBox(i2, j2, i2 + 15, j2 + 15));
    ...
    public BlockPos getClosestStrongholdPos(World worldIn, BlockPos pos) {
        ...
        long i2 = this.rand.nextLong();
        long j2 = this.rand.nextLong();
        long k2 = (long)(pos.getX() >> 4) * i2;
        long l2 = (long)(pos.getZ() >> 4) * j2;
        this.rand.setSeed(k2 ^ l2 ^ worldIn.getSeed());
    ...
    private void func_143027_a(World worldIn) {
        if (this.structureData == null) {
            ...
            if (this.structureData == null) {
                ...
                for (String s2 : nbttagcompound1.getKeySet()) {
                    NBTTagCompound nbttagcompound;
                    NBTBase nbtbase = nbttagcompound1.getTag(s2);
                    if (nbtbase.getId() != 10 || !(nbttagcompound = (NBTTagCompound)nbtbase).hasKey("ChunkX") || !nbttagcompound.hasKey("ChunkZ")) continue;
                    int i2 = nbttagcompound.getInteger("ChunkX");
                    int j2 = nbttagcompound.getInteger("ChunkZ");
                    StructureStart structurestart = MapGenStructureIO.getStructureStart(nbttagcompound, worldIn);
                    if (structurestart == null) continue;
                    this.structureMap.put(ChunkCoordIntPair.chunkXZ2Int(i2, j2), structurestart);
                    this.structureLongMap.add(ChunkCoordIntPair.chunkXZ2Int(i2, j2), structurestart);
                }
            }
        }
    }
