...
import tk.rektsky.module.impl.render.NoRender;
...
implements IBlockAccess {
    ...
    private boolean isAreaLoaded(int xStart, int yStart, int zStart, int xEnd, int yEnd, int zEnd, boolean allowEmpty) {
        if (yEnd >= 0 && yStart < 256) {
            ...
            xEnd >>= 4;
            zEnd >>= 4;
            for (int i2 = xStart >>= 4; i2 <= xEnd; ++i2) {
                for (int j2 = zStart; j2 <= zEnd; ++j2) {
                    if (this.isChunkLoaded(i2, j2, allowEmpty)) continue;
                    return false;
                }
            }
            return true;
    ...
    public BlockPos getHeight(BlockPos pos) {
        int i2 = pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000 ? (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true) ? this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4).getHeightValue(pos.getX() & 0xF, pos.getZ() & 0xF) : 0) : this.func_181545_F() + 1;
    ...
    private void spawnParticle(int particleID, boolean p_175720_2_, double xCood, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int ... p_175720_15_) {
        if (!ModulesManager.getModuleByClass(NoRender.class).isToggled()) {
            for (int i2 = 0; i2 < this.worldAccesses.size(); ++i2) {
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        ...
        int i2 = MathHelper.floor_double(bb.minX);
        int j2 = MathHelper.floor_double(bb.maxX + 1.0);
        int k2 = MathHelper.floor_double(bb.minY);
        int l2 = MathHelper.floor_double(bb.maxY + 1.0);
        ...
        for (int k1 = i2; k1 < j2; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                ...
                for (int i22 = k2 - 1; i22 < l2; ++i22) {
                    blockpos$mutableblockpos.func_181079_c(k1, i22, l1);
                    if (flag && flag1) {
        ...
        for (int j22 = 0; j22 < list1.size(); ++j22) {
            ...
            if ((axisalignedbb = entityIn.getCollisionBox(list1.get(j22))) == null || !axisalignedbb.intersectsWith(bb)) continue;
            list.add(axisalignedbb);
        }
        return list;
    }
    ...
    public void updateEntities() {
        ...
        for (int k2 = 0; k2 < this.unloadedEntityList.size(); ++k2) {
            ...
            if (!entity1.addedToChunk || !this.isChunkLoaded(j2, l1, true)) continue;
    ...
    public boolean checkBlockCollision(AxisAlignedBB bb) {
        int i2 = MathHelper.floor_double(bb.minX);
        int j2 = MathHelper.floor_double(bb.maxX);
        int k2 = MathHelper.floor_double(bb.minY);
        int l2 = MathHelper.floor_double(bb.maxY);
        int i1 = MathHelper.floor_double(bb.minZ);
        ...
        for (int k1 = i2; k1 <= j2; ++k1) {
            for (int l1 = k2; l1 <= l2; ++l1) {
                for (int i22 = i1; i22 <= j1; ++i22) {
                    Block block = this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i22)).getBlock();
    ...
    public boolean isFlammableWithin(AxisAlignedBB bb) {
        ...
        int l2 = MathHelper.floor_double(bb.maxY + 1.0);
        int i1 = MathHelper.floor_double(bb.minZ);
        if (this.isAreaLoaded(i2, k2, i1, j2, l2, j1 = MathHelper.floor_double(bb.maxZ + 1.0), true)) {
            BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
            for (int k1 = i2; k1 < j2; ++k1) {
                for (int l1 = k2; l1 < l2; ++l1) {
                    for (int i22 = i1; i22 < j1; ++i22) {
    ...
    public boolean handleMaterialAcceleration(AxisAlignedBB bb, Material materialIn, Entity entityIn) {
        ...
        for (int k1 = i2; k1 < j2; ++k1) {
            for (int l1 = k2; l1 < l2; ++l1) {
                for (int i22 = i1; i22 < j1; ++i22) {
                    double d0;
                    blockpos$mutableblockpos.func_181079_c(k1, l1, i22);
                    IBlockState iblockstate = this.getBlockState(blockpos$mutableblockpos);
                    Block block = iblockstate.getBlock();
                    if (block.getMaterial() != materialIn || !((double)l2 >= (d0 = (double)((float)(l1 + 1) - BlockLiquid.getLiquidHeightPercent(iblockstate.getValue(BlockLiquid.LEVEL)))))) continue;
                    flag = true;
    ...
    public boolean isMaterialInBB(AxisAlignedBB bb, Material materialIn) {
        ...
        for (int k1 = i2; k1 < j2; ++k1) {
            for (int l1 = k2; l1 < l2; ++l1) {
                for (int i22 = i1; i22 < j1; ++i22) {
                    if (this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i22)).getBlock().getMaterial() != materialIn) continue;
                    return true;
                }
            }
        }
    ...
    public boolean isAABBInMaterial(AxisAlignedBB bb, Material materialIn) {
        ...
        for (int k1 = i2; k1 < j2; ++k1) {
            for (int l1 = k2; l1 < l2; ++l1) {
                for (int i22 = i1; i22 < j1; ++i22) {
                    IBlockState iblockstate = this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i22));
                    Block block = iblockstate.getBlock();
        ...
        if (d0 >= 0.0 && d1 >= 0.0 && d2 >= 0.0) {
            ...
            while (f2 <= 1.0f) {
                ...
                while (f1 <= 1.0f) {
                    float f22 = 0.0f;
                    while (f22 <= 1.0f) {
                        double d5 = bb.minX + (bb.maxX - bb.minX) * (double)f2;
                        double d6 = bb.minY + (bb.maxY - bb.minY) * (double)f1;
                        double d7 = bb.minZ + (bb.maxZ - bb.minZ) * (double)f22;
    ...
    public TileEntity getTileEntity(BlockPos pos) {
        ...
        if (this.processingLoadedTiles) {
            for (int i2 = 0; i2 < this.addedTileEntityList.size(); ++i2) {
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos) {
        ...
        if (l2 > k2) {
            ...
            while (i2 < j2) {
                ...
                for (EnumFacing enumfacing : EnumFacing.values()) {
                    ...
                    if (i3 != l22 - l4 || j2 >= this.lightUpdateBlockList.length) continue;
        ...
        while (i2 < j2) {
            ...
            boolean bl = flag = j2 < this.lightUpdateBlockList.length - 6;
