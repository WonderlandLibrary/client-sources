...
ITickable {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
            ...
            this.inventory[j2] = ItemStack.loadItemStackFromNBT(nbttagcompound);
        }
    }
    ...
    private boolean transferItemsOut() {
        ...
        for (int i2 = 0; i2 < this.getSizeInventory(); ++i2) {
            if (this.getStackInSlot(i2) == null) continue;
            ItemStack itemstack = this.getStackInSlot(i2).copy();
            ItemStack itemstack1 = TileEntityHopper.putStackInInventoryAllSlots(iinventory, this.decrStackSize(i2, 1), enumfacing);
            if (itemstack1 == null || itemstack1.stackSize == 0) {
                iinventory.markDirty();
                return true;
            }
            this.setInventorySlotContents(i2, itemstack);
        }
        return false;
    }
    ...
    private boolean isInventoryFull(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int k2 = 0; k2 < aint.length; ++k2) {
                ItemStack itemstack1 = isidedinventory.getStackInSlot(aint[k2]);
                if (itemstack1 != null && itemstack1.stackSize == itemstack1.getMaxStackSize()) continue;
                return false;
            }
        } else {
            int i2 = inventoryIn.getSizeInventory();
            for (int j2 = 0; j2 < i2; ++j2) {
                ItemStack itemstack = inventoryIn.getStackInSlot(j2);
                if (itemstack != null && itemstack.stackSize == itemstack.getMaxStackSize()) continue;
                return false;
            }
        }
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int i2 = 0; i2 < aint.length; ++i2) {
                if (isidedinventory.getStackInSlot(aint[i2]) == null) continue;
                return false;
            }
        } else {
            int j2 = inventoryIn.getSizeInventory();
            for (int k2 = 0; k2 < j2; ++k2) {
                if (inventoryIn.getStackInSlot(k2) == null) continue;
                return false;
            }
        }
        return true;
    ...
    public static boolean captureDroppedItems(IHopper p_145891_0_) {
        ...
        if (iinventory != null) {
            ...
            if (iinventory instanceof ISidedInventory) {
                ...
                for (int i2 = 0; i2 < aint.length; ++i2) {
                    if (!TileEntityHopper.pullItemFromSlot(p_145891_0_, iinventory, aint[i2], enumfacing)) continue;
                    return true;
                }
            } else {
                int j2 = iinventory.getSizeInventory();
                for (int k2 = 0; k2 < j2; ++k2) {
                    if (!TileEntityHopper.pullItemFromSlot(p_145891_0_, iinventory, k2, enumfacing)) continue;
                    return true;
                }
            }
        } else {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, ItemStack stack, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory && side != null) {
            ISidedInventory isidedinventory = (ISidedInventory)inventoryIn;
            int[] aint = isidedinventory.getSlotsForFace(side);
            for (int k2 = 0; k2 < aint.length && stack != null && stack.stackSize > 0; ++k2) {
                stack = TileEntityHopper.insertStack(inventoryIn, stack, aint[k2], side);
            }
        } else {
            int i2 = inventoryIn.getSizeInventory();
            for (int j2 = 0; j2 < i2 && stack != null && stack.stackSize > 0; ++j2) {
                stack = TileEntityHopper.insertStack(inventoryIn, stack, j2, side);
            }
        }
    ...
    private static ItemStack insertStack(IInventory inventoryIn, ItemStack stack, int index, EnumFacing side) {
        ...
        if (TileEntityHopper.canInsertItemInSlot(inventoryIn, stack, index, side)) {
            ...
            if (itemstack == null) {
                ...
                int i2 = stack.getMaxStackSize() - itemstack.stackSize;
                int j2 = Math.min(stack.stackSize, i2);
                stack.stackSize -= j2;
                itemstack.stackSize += j2;
                boolean bl = flag = j2 > 0;
            }
            if (flag) {
    ...
    public static IInventory getInventoryAtPosition(World worldIn, double x2, double y2, double z2) {
        List<Entity> list;
        ...
        BlockPos blockpos = new BlockPos(i2, j2 = MathHelper.floor_double(y2), k2 = MathHelper.floor_double(z2));
        ...
        if (iinventory == null && (list = worldIn.getEntitiesInAABBexcluding(null, new AxisAlignedBB(x2 - 0.5, y2 - 0.5, z2 - 0.5, x2 + 0.5, y2 + 0.5, z2 + 0.5), EntitySelectors.selectInventories)).size() > 0) {
