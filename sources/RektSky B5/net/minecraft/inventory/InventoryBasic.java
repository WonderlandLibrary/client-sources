...
implements IInventory {
    ...
    public ItemStack func_174894_a(ItemStack stack) {
        ...
        for (int i2 = 0; i2 < this.slotsCount; ++i2) {
            int j2;
            int k2;
            ItemStack itemstack1 = this.getStackInSlot(i2);
            if (itemstack1 == null) {
                this.setInventorySlotContents(i2, itemstack);
                this.markDirty();
                return null;
            }
            if (!ItemStack.areItemsEqual(itemstack1, itemstack) || (k2 = Math.min(itemstack.stackSize, (j2 = Math.min(this.getInventoryStackLimit(), itemstack1.getMaxStackSize())) - itemstack1.stackSize)) <= 0) continue;
            itemstack1.stackSize += k2;
    ...
    public void markDirty() {
        if (this.field_70480_d != null) {
            for (int i2 = 0; i2 < this.field_70480_d.size(); ++i2) {
                this.field_70480_d.get(i2).onInventoryChanged(this);
            }
        }
    }
    ...
    public void clear() {
        for (int i2 = 0; i2 < this.inventoryContents.length; ++i2) {
            this.inventoryContents[i2] = null;
        }
    }
}
