...
extends Entity {
    ...
    public void setPositionAndRotation2(double x2, double y2, double z2, float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_) {
        if (p_180426_10_ && this.riddenByEntity != null) {
            ...
            if (this.isBoatEmpty) {
                ...
                double d0 = x2 - this.posX;
                double d1 = y2 - this.posY;
                double d2 = z2 - this.posZ;
    ...
    public void onUpdate() {
        ...
        for (int j2 = 0; j2 < i2; ++j2) {
            double d1 = this.getEntityBoundingBox().minY + (this.getEntityBoundingBox().maxY - this.getEntityBoundingBox().minY) * (double)(j2 + 0) / (double)i2 - 0.125;
            double d3 = this.getEntityBoundingBox().minY + (this.getEntityBoundingBox().maxY - this.getEntityBoundingBox().minY) * (double)(j2 + 1) / (double)i2 - 0.125;
        ...
        if (this.worldObj.isRemote && this.isBoatEmpty) {
            ...
            if (this.riddenByEntity instanceof EntityLivingBase) {
                ...
                float f2 = this.riddenByEntity.rotationYaw + -entitylivingbase.moveStrafing * 90.0f;
                this.motionX += -Math.sin(f2 * (float)Math.PI / 180.0f) * this.speedMultiplier * (double)entitylivingbase.moveForward * (double)0.05f;
                this.motionZ += Math.cos(f2 * (float)Math.PI / 180.0f) * this.speedMultiplier * (double)entitylivingbase.moveForward * (double)0.05f;
            }
            ...
            for (int i1 = 0; i1 < 4; ++i1) {
                ...
                int i22 = MathHelper.floor_double(this.posZ + ((double)(i1 / 2) - 0.5) * 0.8);
                for (int j2 = 0; j2 < 2; ++j2) {
                    int l2 = MathHelper.floor_double(this.posY) + j2;
                    BlockPos blockpos = new BlockPos(l1, l2, i22);
    ...
    protected void updateFallState(double y2, boolean onGroundIn, Block blockIn, BlockPos pos) {
        ...
        } else if (this.worldObj.getBlockState(new BlockPos(this).down()).getBlock().getMaterial() != Material.water && y2 < 0.0) {
