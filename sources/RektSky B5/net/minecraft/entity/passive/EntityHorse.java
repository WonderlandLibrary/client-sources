...
implements IInvBasic {
    ...
    private void setHorseWatchableBoolean(int p_110208_1_, boolean p_110208_2_) {
        ...
        if (p_110208_2_) {
            this.dataWatcher.updateObject(16, i2 | p_110208_1_);
        } else {
            this.dataWatcher.updateObject(16, i2 & ~p_110208_1_);
        }
    }
    ...
    public int increaseTemper(int p_110198_1_) {
        int i2 = MathHelper.clamp_int(this.getTemper() + p_110198_1_, 0, this.getMaxTemper());
        this.setTemper(i2);
        return i2;
    }
    ...
    public boolean prepareChunkForSpawn() {
        ...
        this.worldObj.getBiomeGenForCoords(new BlockPos(i2, 0, j2));
        return true;
    }
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if ((i2 = MathHelper.ceiling_float_int((distance * 0.5f - 3.0f) * damageMultiplier)) > 0) {
            Block block;
            this.attackEntityFrom(DamageSource.fall, i2);
            if (this.riddenByEntity != null) {
                this.riddenByEntity.attackEntityFrom(DamageSource.fall, i2);
            }
    ...
    private int getChestSize() {
        int i2 = this.getHorseType();
        return !this.isChested() || i2 != 1 && i2 != 2 ? 2 : 17;
    }
    ...
    private void initHorseChest() {
        ...
        if (animalchest != null) {
            ...
            int i2 = Math.min(animalchest.getSizeInventory(), this.horseChest.getSizeInventory());
            for (int j2 = 0; j2 < i2; ++j2) {
                ItemStack itemstack = animalchest.getStackInSlot(j2);
                if (itemstack == null) continue;
                this.horseChest.setInventorySlotContents(j2, itemstack.copy());
            }
        }
    ...
    public void onInventoryChanged(InventoryBasic p_76316_1_) {
        int i2 = this.getHorseArmorIndexSynced();
        boolean flag = this.isHorseSaddled();
        this.updateHorseSlots();
        if (this.ticksExisted > 20) {
            if (i2 == 0 && i2 != this.getHorseArmorIndexSynced()) {
                this.playSound("mob.horse.armor", 0.5f, 1.0f);
            } else if (i2 != this.getHorseArmorIndexSynced()) {
                this.playSound("mob.horse.armor", 0.5f, 1.0f);
            }
    ...
    protected Item getDropItem() {
        ...
        int i2 = this.getHorseType();
        return i2 == 4 ? Items.bone : (i2 == 3 ? (flag ? null : Items.rotten_flesh) : Items.leather);
    }
    ...
    protected String getHurtSound() {
        ...
        return (i2 = this.getHorseType()) == 3 ? "mob.horse.zombie.hit" : (i2 == 4 ? "mob.horse.skeleton.hit" : (i2 != 1 && i2 != 2 ? "mob.horse.hit" : "mob.horse.donkey.hit"));
    }
    ...
    protected String getLivingSound() {
        ...
        return (i2 = this.getHorseType()) == 3 ? "mob.horse.zombie.idle" : (i2 == 4 ? "mob.horse.skeleton.idle" : (i2 != 1 && i2 != 2 ? "mob.horse.idle" : "mob.horse.donkey.idle"));
    }
    ...
    private void setHorseTexturePaths() {
        ...
        int i2 = this.getHorseType();
        int j2 = this.getHorseVariant();
        if (i2 == 0) {
            int k2 = j2 & 0xFF;
            int l2 = (j2 & 0xFF00) >> 8;
            if (k2 >= horseTextures.length) {
                this.field_175508_bO = false;
                return;
            }
            this.horseTexturesArray[0] = horseTextures[k2];
            this.texturePrefix = this.texturePrefix + HORSE_TEXTURES_ABBR[k2];
            if (l2 >= horseMarkingTextures.length) {
                this.field_175508_bO = false;
                return;
            }
            this.horseTexturesArray[1] = horseMarkingTextures[l2];
            this.texturePrefix = this.texturePrefix + HORSE_MARKING_TEXTURES_ABBR[l2];
        } else {
            this.horseTexturesArray[0] = "";
            this.texturePrefix = this.texturePrefix + "_" + i2 + "_";
        }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null) {
            ...
            if (!flag && !this.isUndead()) {
                ...
                if (!this.isAdultHorse() && j2 > 0) {
                    this.addGrowth(j2);
                    flag = true;
                }
                if (k2 > 0 && (flag || !this.isTame()) && k2 < this.getMaxTemper()) {
                    flag = true;
                    this.increaseTemper(k2);
                }
                if (flag) {
                    this.func_110266_cB();
                }
    ...
    public void makeHorseRearWithSound() {
        this.makeHorseRear();
        String s2 = this.getAngrySoundName();
        if (s2 != null) {
    ...
    private void dropItemsInChest(Entity entityIn, AnimalChest animalChestIn) {
        if (animalChestIn != null && !this.worldObj.isRemote) {
            for (int i2 = 0; i2 < animalChestIn.getSizeInventory(); ++i2) {
                ItemStack itemstack = animalChestIn.getStackInSlot(i2);
                if (itemstack == null) continue;
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        if (this.isChested()) {
            ...
            for (int i2 = 2; i2 < this.horseChest.getSizeInventory(); ++i2) {
                ItemStack itemstack = this.horseChest.getStackInSlot(i2);
                if (itemstack == null) continue;
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (this.isChested()) {
            ...
            for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
                ...
                if (j2 < 2 || j2 >= this.horseChest.getSizeInventory()) continue;
                this.horseChest.setInventorySlotContents(j2, ItemStack.loadItemStackFromNBT(nbttagcompound));
            }
        }
    ...
    public boolean canMateWith(EntityAnimal otherAnimal) {
        ...
        if (this.canMate() && entityhorse.canMate()) {
            int j2;
            int i2 = this.getHorseType();
            return i2 == (j2 = entityhorse.getHorseType()) || i2 == 0 && j2 == 1 || i2 == 1 && j2 == 0;
        }
        return false;
    }
    ...
    public EntityAgeable createChild(EntityAgeable ageable) {
        ...
        int i2 = this.getHorseType();
        int j2 = entityhorse.getHorseType();
        int k2 = 0;
        if (i2 == j2) {
            k2 = i2;
        ...
        if (k2 == 0) {
            ...
            int l2 = i1 < 4 ? this.getHorseVariant() & 0xFF : (i1 < 8 ? entityhorse.getHorseVariant() & 0xFF : this.rand.nextInt(7));
            int j1 = this.rand.nextInt(5);
            l2 = j1 < 2 ? (l2 |= this.getHorseVariant() & 0xFF00) : (j1 < 4 ? (l2 |= entityhorse.getHorseVariant() & 0xFF00) : (l2 |= this.rand.nextInt(5) << 8 & 0xFF00));
            entityhorse1.setHorseVariant(l2);
        }
        entityhorse1.setHorseType(k2);
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        if (livingdata instanceof GroupData) {
            i2 = ((GroupData)livingdata).horseType;
            j2 = ((GroupData)livingdata).horseVariant & 0xFF | this.rand.nextInt(5) << 8;
        } else {
            if (this.rand.nextInt(10) == 0) {
                i2 = 1;
            } else {
                int k2 = this.rand.nextInt(7);
                int l2 = this.rand.nextInt(5);
                i2 = 0;
                j2 = k2 | l2 << 8;
            }
            livingdata = new GroupData(i2, j2);
        }
        this.setHorseType(i2);
        this.setHorseVariant(j2);
    ...
    public void updateRiderPosition() {
        ...
        if (this.prevRearingAmount > 0.0f) {
            float f2 = MathHelper.sin(this.renderYawOffset * (float)Math.PI / 180.0f);
            float f1 = MathHelper.cos(this.renderYawOffset * (float)Math.PI / 180.0f);
            float f22 = 0.7f * this.prevRearingAmount;
            float f3 = 0.15f * this.prevRearingAmount;
            this.riddenByEntity.setPosition(this.posX + (double)(f22 * f2), this.posY + this.getMountedYOffset() + this.riddenByEntity.getYOffset() + (double)f3, this.posZ - (double)(f22 * f1));
    ...
    public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
        ...
        if ((i2 = inventorySlot - 400) >= 0 && i2 < 2 && i2 < this.horseChest.getSizeInventory()) {
            if (i2 == 0 && itemStackIn != null && itemStackIn.getItem() != Items.saddle) {
                return false;
            }
            if (i2 != 1 || (itemStackIn == null || EntityHorse.isArmorItem(itemStackIn.getItem())) && this.canWearArmor()) {
                this.horseChest.setInventorySlotContents(i2, itemStackIn);
                this.updateHorseSlots();
                return true;
            }
            return false;
        }
        int j2 = inventorySlot - 500 + 2;
        if (j2 >= 2 && j2 < this.horseChest.getSizeInventory()) {
            this.horseChest.setInventorySlotContents(j2, itemStackIn);
            return true;
        }
        return false;
    }
