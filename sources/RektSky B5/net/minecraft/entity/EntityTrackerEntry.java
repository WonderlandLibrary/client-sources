...
public class EntityTrackerEntry {
    ...
    public void updatePlayerList(List<EntityPlayer> p_73122_1_) {
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged()) {
            if (this.trackedEntity.ridingEntity == null) {
                ...
                int k2 = MathHelper.floor_double(this.trackedEntity.posX * 32.0);
                ...
                int j2 = k2 - this.encodedPosX;
                int k22 = j1 - this.encodedPosY;
                int i3 = k1 - this.encodedPosZ;
                Packet<INetHandlerPlayClient> packet1 = null;
                boolean flag = Math.abs(j2) >= 4 || Math.abs(k22) >= 4 || Math.abs(i3) >= 4 || this.updateCounter % 60 == 0;
                boolean bl = flag1 = Math.abs(l1 - this.encodedRotationYaw) >= 4 || Math.abs(i2 - this.encodedRotationPitch) >= 4;
                if (this.updateCounter > 0 || this.trackedEntity instanceof EntityArrow) {
                    if (j2 >= -128 && j2 < 128 && k22 >= -128 && k22 < 128 && i3 >= -128 && i3 < 128 && this.ticksSinceLastForcedTeleport <= 400 && !this.ridingEntity && this.onGround == this.trackedEntity.onGround) {
                        if (!(flag && flag1 || this.trackedEntity instanceof EntityArrow)) {
                            if (flag) {
                                packet1 = new S14PacketEntity.S15PacketEntityRelMove(this.trackedEntity.getEntityId(), (byte)j2, (byte)k22, (byte)i3, this.trackedEntity.onGround);
                            } else if (flag1) {
                            ...
                            packet1 = new S14PacketEntity.S17PacketEntityLookMove(this.trackedEntity.getEntityId(), (byte)j2, (byte)k22, (byte)i3, (byte)l1, (byte)i2, this.trackedEntity.onGround);
                        }
                    } else {
                        this.onGround = this.trackedEntity.onGround;
                        this.ticksSinceLastForcedTeleport = 0;
                        packet1 = new S18PacketEntityTeleport(this.trackedEntity.getEntityId(), k2, j1, k1, (byte)l1, (byte)i2, this.trackedEntity.onGround);
                    }
                }
                ...
                boolean flag2;
                int j2 = MathHelper.floor_float(this.trackedEntity.rotationYaw * 256.0f / 360.0f);
                int i1 = MathHelper.floor_float(this.trackedEntity.rotationPitch * 256.0f / 360.0f);
                boolean bl = flag2 = Math.abs(j2 - this.encodedRotationYaw) >= 4 || Math.abs(i1 - this.encodedRotationPitch) >= 4;
                if (flag2) {
                    this.sendPacketToTrackedPlayers(new S14PacketEntity.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)j2, (byte)i1, this.trackedEntity.onGround));
                    this.encodedRotationYaw = j2;
            ...
            int l2 = MathHelper.floor_float(this.trackedEntity.getRotationYawHead() * 256.0f / 360.0f);
                ...
                if (!this.trackingPlayers.contains(playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn)) {
                    ...
                    if (this.trackedEntity instanceof EntityLivingBase) {
                        for (int i2 = 0; i2 < 5; ++i2) {
                            ItemStack itemstack = ((EntityLivingBase)this.trackedEntity).getEquipmentInSlot(i2);
                            if (itemstack == null) continue;
                            playerMP.playerNetServerHandler.sendPacket(new S04PacketEntityEquipment(this.trackedEntity.getEntityId(), i2, itemstack));
                        }
                    }
    ...
    public void updatePlayerEntities(List<EntityPlayer> p_73125_1_) {
        for (int i2 = 0; i2 < p_73125_1_.size(); ++i2) {
            this.updatePlayerEntity((EntityPlayerMP)p_73125_1_.get(i2));
        }
    }
    ...
    private Packet func_151260_c() {
        ...
        if (this.trackedEntity instanceof EntityFireball) {
            ...
            s0epacketspawnobject2 = entityfireball.shootingEntity != null ? new S0EPacketSpawnObject(this.trackedEntity, i2, ((EntityFireball)this.trackedEntity).shootingEntity.getEntityId()) : new S0EPacketSpawnObject(this.trackedEntity, i2, 0);
