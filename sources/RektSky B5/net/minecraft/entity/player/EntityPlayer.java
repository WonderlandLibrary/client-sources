...
import tk.rektsky.module.impl.movement.CombatSprintFix;
...
extends EntityLivingBase {
    ...
    protected void onItemUseFinish() {
        if (this.itemInUse != null) {
            ...
            int i2 = this.itemInUse.stackSize;
            ItemStack itemstack = this.itemInUse.onItemUseFinish(this.worldObj, this);
            if (itemstack != this.itemInUse || itemstack != null && itemstack.stackSize != i2) {
    ...
    public void addScore(int p_85039_1_) {
        ...
        this.dataWatcher.updateObject(18, i2 + p_85039_1_);
    }
    ...
    private Collection<ScoreObjective> func_175137_e(Entity p_175137_1_) {
        ...
        if (scoreplayerteam != null && (i2 = scoreplayerteam.getChatFormat().getColorIndex()) >= 0 && i2 < IScoreObjectiveCriteria.field_178793_i.length) {
            for (ScoreObjective scoreobjective : this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.field_178793_i[i2])) {
                Score score = this.getWorldScoreboard().getValueFromObjective(p_175137_1_.getName(), scoreobjective);
                score.func_96648_a();
            }
        }
        if ((scoreplayerteam1 = this.getWorldScoreboard().getPlayersTeam(p_175137_1_.getName())) != null && (j2 = scoreplayerteam1.getChatFormat().getColorIndex()) >= 0 && j2 < IScoreObjectiveCriteria.field_178792_h.length) {
            return this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.field_178792_h[j2]);
        }
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
        ...
        if (dropAround) {
            ...
            entityitem.motionX = -MathHelper.sin(f1) * f2;
            entityitem.motionZ = MathHelper.cos(f1) * f2;
    ...
    public float getToolDigEfficiency(Block p_180471_1_) {
        float f2 = this.inventory.getStrVsBlock(p_180471_1_);
        if (f2 > 1.0f) {
            int i2 = EnchantmentHelper.getEfficiencyModifier(this);
            ItemStack itemstack = this.inventory.getCurrentItem();
        ...
        if (this.isPotionActive(Potion.digSpeed)) {
            f2 *= 1.0f + (float)(this.getActivePotionEffect(Potion.digSpeed).getAmplifier() + 1) * 0.2f;
        }
    ...
    public float getArmorVisibility() {
        ...
        return (float)i2 / (float)this.inventory.armorInventory.length;
    }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
            ...
            if (f2 > 0.0f || f1 > 0.0f) {
                ...
                f2 += f1;
                boolean flag1 = false;
                int j2 = EnchantmentHelper.getFireAspectModifier(this);
                if (targetEntity instanceof EntityLivingBase && j2 > 0 && !targetEntity.isBurning()) {
                    flag1 = true;
                ...
                boolean flag2 = targetEntity.attackEntityFrom(DamageSource.causePlayerDamage(this), f2);
                if (flag2) {
                    IEntityMultiPart ientitymultipart;
                    if (i2 > 0) {
                        targetEntity.addVelocity(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0f) * (float)i2 * 0.5f, 0.1, MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0f) * (float)i2 * 0.5f);
                        if (!ModulesManager.getModuleByClass(CombatSprintFix.class).isToggled()) {
                            this.motionX *= 0.6;
                            this.motionZ *= 0.6;
                    ...
                    if (targetEntity instanceof EntityLivingBase) {
                        this.addStat(StatList.damageDealtStat, Math.round(f2 * 10.0f));
                        if (j2 > 0) {
    ...
    public EnumStatus trySleep(BlockPos bedLocation) {
        ...
        if (this.worldObj.isBlockLoaded(bedLocation)) {
            ...
            this.setPosition((float)bedLocation.getX() + f2, (float)bedLocation.getY() + 0.6875f, (float)bedLocation.getZ() + f1);
        } else {
    ...
    public void wakeUpPlayer(boolean p_70999_1_, boolean updateWorldFlag, boolean setSpawn) {
        ...
        int n2 = this.sleepTimer = p_70999_1_ ? 0 : 100;
        if (setSpawn) {
    ...
    public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
        if (this.ridingEntity == null) {
            if (this.isInsideOfMaterial(Material.water)) {
                int i2 = Math.round(MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (i2 > 0) {
                    this.addStat(StatList.distanceDoveStat, i2);
                    this.addExhaustion(0.015f * (float)i2 * 0.01f);
                }
            } else if (this.isInWater()) {
                int j2 = Math.round(MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (j2 > 0) {
                    this.addStat(StatList.distanceSwumStat, j2);
                    this.addExhaustion(0.015f * (float)j2 * 0.01f);
                }
                ...
                int k2 = Math.round(MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (k2 > 0) {
                    this.addStat(StatList.distanceWalkedStat, k2);
                    if (this.isSprinting()) {
                        this.addStat(StatList.distanceSprintedStat, k2);
                        this.addExhaustion(0.099999994f * (float)k2 * 0.01f);
                    } else {
                        if (this.isSneaking()) {
                            this.addStat(StatList.distanceCrouchedStat, k2);
                        }
                        this.addExhaustion(0.01f * (float)k2 * 0.01f);
                    }
                }
            } else {
                int l2 = Math.round(MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (l2 > 25) {
    ...
    private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
        int i2;
        if (this.ridingEntity != null && (i2 = Math.round(MathHelper.sqrt_double(p_71015_1_ * p_71015_1_ + p_71015_3_ * p_71015_3_ + p_71015_5_ * p_71015_5_) * 100.0f)) > 0) {
            if (this.ridingEntity instanceof EntityMinecart) {
                ...
                this.addStat(StatList.distanceByHorseStat, i2);
            }
        }
    }
    ...
    public void addExperienceLevel(int levels) {
        ...
        if (levels > 0 && this.experienceLevel % 5 == 0 && (float)this.lastXPSound < (float)this.ticksExisted - 100.0f) {
            ...
            this.worldObj.playSoundAtEntity(this, "random.levelup", f2 * 0.75f, 1.0f);
    ...
    public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
        ...
        int i2 = inventorySlot - 100;
        if (i2 >= 0 && i2 < this.inventory.armorInventory.length) {
            int k2 = i2 + 1;
            if (itemStackIn != null && itemStackIn.getItem() != null && (itemStackIn.getItem() instanceof ItemArmor ? EntityLiving.getArmorPosition(itemStackIn) != k2 : k2 != 4 || itemStackIn.getItem() != Items.skull && !(itemStackIn.getItem() instanceof ItemBlock))) {
                return false;
            }
            this.inventory.setInventorySlotContents(i2 + this.inventory.mainInventory.length, itemStackIn);
            return true;
        }
        int j2 = inventorySlot - 200;
        if (j2 >= 0 && j2 < this.theInventoryEnderChest.getSizeInventory()) {
            this.theInventoryEnderChest.setInventorySlotContents(j2, itemStackIn);
            return true;
        }
        return false;
    }
    ...
    public static enum EnumChatVisibility {
        ...
        static {
            ...
            int n2 = enumChatVisibilityArray.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                EnumChatVisibility entityplayer$enumchatvisibility;
                EnumChatVisibility.ID_LOOKUP[entityplayer$enumchatvisibility.chatVisibility] = entityplayer$enumchatvisibility = enumChatVisibilityArray[i2];
            }
        }
    }
}
