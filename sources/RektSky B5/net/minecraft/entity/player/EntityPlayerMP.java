...
implements ICrafting {
    ...
    public EntityPlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, ItemInWorldManager interactionManager) {
        ...
        if (!worldIn.provider.getHasNoSky() && worldIn.getWorldInfo().getGameType() != WorldSettings.GameType.ADVENTURE) {
            int i2 = Math.max(5, server.getSpawnProtectionSize() - 6);
            int j2 = MathHelper.floor_double(worldIn.getWorldBorder().getClosestDistance(blockpos.getX(), blockpos.getZ()));
            if (j2 < i2) {
                i2 = j2;
            }
            if (j2 <= 1) {
            ...
            blockpos = worldIn.getTopSolidOrLiquidBlock(blockpos.add(this.rand.nextInt(i2 * 2) - i2, 0, this.rand.nextInt(i2 * 2) - i2));
        }
    ...
    public void onUpdateEntity() {
        try {
            ...
            for (int i2 = 0; i2 < this.inventory.getSizeInventory(); ++i2) {
                Packet packet;
                ItemStack itemstack = this.inventory.getStackInSlot(i2);
    ...
    protected void updateBiomesExplored() {
        ...
        String s2 = biomegenbase.biomeName;
    ...
    public void func_175173_a(Container p_175173_1_, IInventory p_175173_2_) {
        for (int i2 = 0; i2 < p_175173_2_.getFieldCount(); ++i2) {
            this.playerNetServerHandler.sendPacket(new S31PacketWindowProperty(p_175173_1_.windowId, i2, p_175173_2_.getField(i2)));
        }
    }
    ...
    public void setPositionAndUpdate(double x2, double y2, double z2) {
        this.playerNetServerHandler.setPlayerLocation(x2, y2, z2, this.rotationYaw, this.rotationPitch);
    }
    ...
    public String getPlayerIP() {
        String s2 = this.playerNetServerHandler.netManager.getRemoteAddress().toString();
