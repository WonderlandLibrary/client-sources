...
public class Block {
    ...
    public static IBlockState getStateById(int id) {
        int i2 = id & 0xFFF;
        int j2 = id >> 12 & 0xF;
        return Block.getBlockById(i2).getStateFromMeta(j2);
    }
    ...
    public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos) {
        ...
        int i2 = worldIn.getCombinedLight(pos, block.getLightValue());
        if (i2 == 0 && block instanceof BlockSlab) {
            pos = pos.down();
    ...
    public float getPlayerRelativeBlockHardness(EntityPlayer playerIn, World worldIn, BlockPos pos) {
        ...
        return f2 < 0.0f ? 0.0f : (!playerIn.canHarvestBlock(this) ? playerIn.getToolDigEfficiency(this) / f2 / 100.0f : playerIn.getToolDigEfficiency(this) / f2 / 30.0f);
    }
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        if (!worldIn.isRemote) {
            int i2 = this.quantityDroppedWithBonus(fortune, worldIn.rand);
    ...
    public static void spawnAsEntity(World worldIn, BlockPos pos, ItemStack stack) {
        if (!worldIn.isRemote && worldIn.getGameRules().getBoolean("doTileDrops")) {
            ...
            double d1 = (double)(worldIn.rand.nextFloat() * f2) + (double)(1.0f - f2) * 0.5;
    ...
    protected void dropXpOnBlockBreak(World worldIn, BlockPos pos, int amount) {
        if (!worldIn.isRemote) {
            while (amount > 0) {
                int i2 = EntityXPOrb.getXPSplit(amount);
                amount -= i2;
                worldIn.spawnEntityInWorld(new EntityXPOrb(worldIn, (double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, i2));
            }
        }
    }
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te) {
        ...
        if (this.canSilkHarvest() && EnchantmentHelper.getSilkTouchModifier(player)) {
            ...
            int i2 = EnchantmentHelper.getFortuneModifier(player);
            this.dropBlockAsItem(worldIn, pos, state, i2);
        }
    }
    ...
    protected ItemStack createStackedBlock(IBlockState state) {
        ...
        if (item != null && item.getHasSubtypes()) {
            i2 = this.getMetaFromState(state);
        }
        return new ItemStack(item, 1, i2);
    }
    ...
    public static void registerBlocks() {
        ...
        for (Block block14 : blockRegistry) {
            for (IBlockState iblockstate : block14.getBlockState().getValidStates()) {
                int i2 = blockRegistry.getIDForObject(block14) << 4 | block14.getMetaFromState(iblockstate);
                BLOCK_STATE_IDS.put(iblockstate, i2);
            }
        }
    }
