...
extends Block {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess worldIn, BlockPos pos) {
        ...
        switch (worldIn.getBlockState(pos).getValue(FACING)) {
            ...
            case DOWN_Z: {
                f2 = 0.25f;
                this.setBlockBounds(0.5f - f2, 0.4f, 0.5f - f2, 0.5f + f2, 1.0f, 0.5f + f2);
            }
        }
    }
    ...
    public int getMetaFromState(IBlockState state) {
        int i2 = 0;
        i2 |= state.getValue(FACING).getMetadata();
        if (state.getValue(POWERED).booleanValue()) {
            i2 |= 8;
        }
        return i2;
    ...
    public static enum EnumOrientation implements IStringSerializable
    {
        ...
        static {
            ...
            int n2 = enumOrientationArray.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                EnumOrientation blocklever$enumorientation;
                EnumOrientation.META_LOOKUP[blocklever$enumorientation.getMetadata()] = blocklever$enumorientation = enumOrientationArray[i2];
            }
        }
    }
}
