...
extends Block {
    ...
    public boolean func_176306_h(IBlockAccess blockAccess, BlockPos pos) {
        ...
        this.setBlockBounds(f22, f2, f4, f3, f1, f5);
        return flag1;
    }
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end) {
        ...
        IBlockState iblockstate = worldIn.getBlockState(pos);
        int i2 = iblockstate.getValue(FACING).getHorizontalIndex();
        boolean flag = iblockstate.getValue(HALF) == EnumHalf.TOP;
        int[] aint = field_150150_a[i2 + (flag ? 4 : 0)];
        this.hasRaytraced = true;
        for (int j2 = 0; j2 < 8; ++j2) {
            this.rayTracePass = j2;
            if (Arrays.binarySearch(aint, j2) >= 0) continue;
            amovingobjectposition[j2] = super.collisionRayTrace(worldIn, pos, start, end);
        }
        for (int k2 : aint) {
            amovingobjectposition[k2] = null;
        }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        return i2 |= 5 - state.getValue(FACING).getIndex();
    }
