...
implements LinuxDevice {
        ...
        switch (type) {
            ...
            case 2: {
                ...
                if (index < this.getNumAxes() && (axis = this.axes[index]) != null) {
                    ...
                    axis.setValue(value);
                    if (this.povXs.containsKey(index)) {
                        LinuxJoystickPOV pov = this.povXs.get(index);
                        pov.updateValue();
                        this.event.set(pov, pov.getPollData(), joystick_event.getNanos());
                        break;
                    }
                    if (this.povYs.containsKey(index)) {
                        LinuxJoystickPOV pov = this.povYs.get(index);
                        pov.updateValue();
                        this.event.set(pov, pov.getPollData(), joystick_event.getNanos());
                        break;
                    }
    ...
    public final void registerPOV(LinuxJoystickPOV pov) {
        ...
        this.povXs.put(xIndex, pov);
        this.povYs.put(yIndex, pov);
    }
