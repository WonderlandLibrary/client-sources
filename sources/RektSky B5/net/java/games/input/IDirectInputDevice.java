...
final class IDirectInputDevice {
    ...
    public IDirectInputDevice(DummyWindow window, long address, byte[] instance_guid, byte[] product_guid, int dev_type, int dev_subtype, String instance_name, String product_name) throws IOException {
        ...
        try {
            ...
            DirectInputEnvironmentPlugin.log("Failed to create rumblers: " + e2.getMessage());
        }
        boolean all_relative = true;
        boolean has_axis = false;
        for (int i2 = 0; i2 < this.objects.size(); ++i2) {
            DIDeviceObject obj = this.objects.get(i2);
    ...
    private final List<Rumbler> createRumblers() throws IOException {
        ...
        for (int i2 = 0; i2 < this.effects.size(); ++i2) {
            DIEffectInfo info = this.effects.get(i2);
    ...
    private final Rumbler createPeriodicRumbler(DIDeviceObject[] axes, long[] directions, DIEffectInfo info) throws IOException {
        ...
        for (int i2 = 0; i2 < axis_ids.length; ++i2) {
            axis_ids[i2] = axes[i2].getDIIdentifier();
        }
    ...
    private final DIDeviceObject lookupObjectByGUID(int guid_id) {
        for (int i2 = 0; i2 < this.objects.size(); ++i2) {
            DIDeviceObject object = this.objects.get(i2);
    ...
    public final synchronized boolean getNextEvent(DIDeviceObjectData data) {
        DIDeviceObjectData next_event = this.queue.get();
        if (next_event == null) {
            return false;
        }
        data.set(next_event);
    ...
    private static final native int nGetDeviceData(long var0, int var2, DataQueue<DIDeviceObjectData> var3, Object[] var4, int var5, int var6);
    ...
    public final synchronized void setBufferSize(int size) throws IOException {
        ...
        this.queue = new DataQueue<DIDeviceObjectData>(size, DIDeviceObjectData.class);
    ...
    public final synchronized void release() {
        if (!this.released) {
            this.released = true;
            for (int i2 = 0; i2 < this.rumblers.size(); ++i2) {
                IDirectInputEffect effect = (IDirectInputEffect)this.rumblers.get(i2);
                effect.release();
            }
