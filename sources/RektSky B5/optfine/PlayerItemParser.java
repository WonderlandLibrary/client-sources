...
public class PlayerItemParser {
    ...
    public static PlayerItemModel parseItemModel(JsonObject p_parseItemModel_0_) {
        String s2 = Json.getString(p_parseItemModel_0_, "type");
        if (!Config.equals(s2, ITEM_TYPE_MODEL)) {
            throw new JsonParseException("Unknown model type: " + s2);
        }
        int[] aint = Json.parseIntArray(p_parseItemModel_0_.get(ITEM_TEXTURE_SIZE), 2);
        ...
        for (int i2 = 0; i2 < jsonarray.size(); ++i2) {
            ...
            if ((s22 = Json.getString(jsonobject, MODEL_ID)) != null) {
                if (!map.containsKey(s22)) {
                    map.put(s22, jsonobject);
                } else {
                    Config.warn("Duplicate model ID: " + s22);
                }
            }
    ...
    private static ResourceLocation makeResourceLocation(String p_makeResourceLocation_0_) {
        int i2 = p_makeResourceLocation_0_.indexOf(58);
        if (i2 < 0) {
            return new ResourceLocation(p_makeResourceLocation_0_);
        }
        String s2 = p_makeResourceLocation_0_.substring(0, i2);
        String s1 = p_makeResourceLocation_0_.substring(i2 + 1);
    ...
    private static PlayerItemRenderer parseItemRenderer(JsonObject p_parseItemRenderer_0_, Dimension p_parseItemRenderer_1_) {
        String s2 = Json.getString(p_parseItemRenderer_0_, "type");
        ...
        float f2 = Json.getFloat(p_parseItemRenderer_0_, MODEL_SCALE, 1.0f);
        ...
        PlayerItemRenderer playeritemrenderer = new PlayerItemRenderer(i2, f2, modelrenderer);
    ...
    private static ModelRenderer parseModelRenderer(JsonObject p_parseModelRenderer_0_, ModelBase p_parseModelRenderer_1_) {
        ...
        for (int i2 = 0; i2 < afloat1.length; ++i2) {
            afloat1[i2] = afloat1[i2] / 180.0f * (float)Math.PI;
        }
        if (flag) {
            afloat1[0] = -afloat1[0];
        }
        ...
        if ((jsonarray = p_parseModelRenderer_0_.getAsJsonArray(MODEL_BOXES)) != null) {
            for (int j2 = 0; j2 < jsonarray.size(); ++j2) {
                ...
                float f2 = Json.getFloat(jsonobject, BOX_SIZE_ADD, 0.0f);
                modelrenderer.setTextureOffset(aint[0], aint[1]);
                modelrenderer.addBox(afloat2[0], afloat2[1], afloat2[2], (int)afloat2[3], (int)afloat2[4], (int)afloat2[5], f2);
            }
        }
        if ((jsonarray1 = p_parseModelRenderer_0_.getAsJsonArray(MODEL_SPRITES)) != null) {
            for (int k2 = 0; k2 < jsonarray1.size(); ++k2) {
        ...
        if ((jsonarray2 = (JsonArray)p_parseModelRenderer_0_.get(MODEL_SUBMODELS)) != null) {
            for (int l2 = 0; l2 < jsonarray2.size(); ++l2) {
