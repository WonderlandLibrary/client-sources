...
public class TiledMap {
    ...
    private static void setHeadless(boolean h2) {
        headless = h2;
    }
    ...
    public Image getTileImage(int x2, int y2, int layerIndex) {
        Layer layer = (Layer)this.layers.get(layerIndex);
        int tileSetIndex = layer.data[x2][y2][0];
        if (tileSetIndex >= 0 && tileSetIndex < this.tileSets.size()) {
            TileSet tileSet = (TileSet)this.tileSets.get(tileSetIndex);
            int sheetX = tileSet.getTileX(layer.data[x2][y2][1]);
            int sheetY = tileSet.getTileY(layer.data[x2][y2][1]);
    ...
    public int getTileId(int x2, int y2, int layerIndex) {
        Layer layer = (Layer)this.layers.get(layerIndex);
        return layer.getTileID(x2, y2);
    }

    public void setTileId(int x2, int y2, int layerIndex, int tileid) {
        Layer layer = (Layer)this.layers.get(layerIndex);
        layer.setTileID(x2, y2, tileid);
    }
    ...
    public void render(int x2, int y2) {
        this.render(x2, y2, 0, 0, this.width, this.height, false);
    }

    public void render(int x2, int y2, int layer) {
        this.render(x2, y2, 0, 0, this.getWidth(), this.getHeight(), layer, false);
    }

    public void render(int x2, int y2, int sx, int sy, int width, int height) {
        this.render(x2, y2, sx, sy, width, height, false);
    }

    public void render(int x2, int y2, int sx, int sy, int width, int height, int l2, boolean lineByLine) {
        Layer layer = (Layer)this.layers.get(l2);
        switch (this.orientation) {
            case 1: {
                for (int ty = 0; ty < height; ++ty) {
                    layer.render(x2, y2, sx, sy, width, ty, lineByLine, this.tileWidth, this.tileHeight);
                }
                break;
            }
            case 2: {
                this.renderIsometricMap(x2, y2, sx, sy, width, height, layer, lineByLine);
                break;
            }
        }
    }

    public void render(int x2, int y2, int sx, int sy, int width, int height, boolean lineByLine) {
        switch (this.orientation) {
            case 1: {
                for (int ty = 0; ty < height; ++ty) {
                    for (int i2 = 0; i2 < this.layers.size(); ++i2) {
            ...
            case 2: {
                this.renderIsometricMap(x2, y2, sx, sy, width, height, null, lineByLine);
                break;
            }
        }
    }

    protected void renderIsometricMap(int x2, int y2, int sx, int sy, int width, int height, Layer layer, boolean lineByLine) {
    ...
    private void load(InputStream in, String tileSetsLocation) throws SlickException {
        ...
        try {
            ...
            if (this.loadTileSets) {
                ...
                for (int i2 = 0; i2 < setNodes.getLength(); ++i2) {
                    Element current = (Element)setNodes.item(i2);
            ...
            int i3 = 0;
            while (i3 < layerNodes.getLength()) {
                Element current = (Element)layerNodes.item(i3);
            ...
            int i4 = 0;
            while (i4 < objectGroupNodes.getLength()) {
                Element current = (Element)objectGroupNodes.item(i4);
            ...
            Log.error(e2);
            throw new SlickException("Failed to parse tilemap", e2);
        }
    }
    ...
    public TileSet getTileSetByGID(int gid) {
        for (int i2 = 0; i2 < this.tileSets.size(); ++i2) {
            TileSet set = (TileSet)this.tileSets.get(i2);
    ...
    protected class ObjectGroup {
        ...
        public ObjectGroup(Element element) throws SlickException {
            ...
            int i2 = 0;
            while (i2 < objectNodes.getLength()) {
                Element objElement = (Element)objectNodes.item(i2);
