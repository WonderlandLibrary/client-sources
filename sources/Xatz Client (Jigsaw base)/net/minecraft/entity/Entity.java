...
import me.xatzdevelopments.xatz.client.Unused.inEvents.Listeners.inEventStep;
import me.xatzdevelopments.xatz.client.main.Xatz;
import me.xatzdevelopments.xatz.client.modules.Scaffoldold;
...
public abstract class Entity implements ICommandSender {
	...
	public boolean isMovingXZ(double abs) {
		if (Math.abs(this.motionX) <= abs && Math.abs(this.motionZ) <= abs) {
			return false;
		}
		return true;
	}
	 ...
	 */
	   public void setAngles(float yaw, float pitch) {
		      ...
		      if (!Xatz.getModuleByName("MaxRotations").isToggled()) {
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			double d5 = z;
			//TODO Xatz SafeWalk
			boolean flag = this.onGround && (this.isSneaking() || Xatz.getModuleByName("SafeWalk").isToggled()) && this instanceof EntityPlayer;
			...
			if(Xatz.getModuleByName("SafeWalk").isToggled()) {
				flag = false;
			}
...
//            final inEventStep inEvent = new inEventStep();
...
//                }
	...
	public Entity rayTraceEntity(double distance, float partialTicks) {
		Vec3 vec31 = this.getLook(partialTicks);
		for(double dist = 0; dist < (distance * 5); dist+=0.2) {
			double x = (vec31.xCoord * dist) + this.posX;
			double y = (vec31.yCoord * dist) + this.posY + this.getEyeHeight();
			double z = (vec31.zCoord * dist) + this.posZ;
			
			for(Entity en : this.worldObj.loadedEntityList) {
				if(en.isEntityEqual(this)) {
					continue;
				}
				if(en.boundingBox == null) {
					continue;
				}
				if(en.boundingBox.expand(0.3, 0.3, 0.3).isVecInside(new Vec3(x, y, z))) {
					return en;
				}
			}
		}
