...
public final class ItemStack {
	...
	public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
        ...
        if (this.item == null || Item.itemRegistry.getNameForObject(this.item) == null) {
        ...
        s = String.valueOf(s) + (Object)((Object)EnumChatFormatting.RESET);
        if (advanced) {
            String s12 = "";
            if (s.length() > 0) {
                s = String.valueOf(s) + " (";
            ...
            s = this.getHasSubtypes() ? String.valueOf(s) + String.format("#%04d/%d%s", i, this.itemDamage, s12) : String.valueOf(s) + String.format("#%04d%s", i, s12);
        ...
        if (this.hasTagCompound()) {
            ...
            if (this.stackTagCompound.hasKey("display", 10)) {
                ...
                if (nbttagcompound.getTagId("Lore") == 9) {
					NBTTagList nbttaglist11 = nbttagcompound.getTagList("Lore", 8);

					if (nbttaglist11.tagCount() > 0) {
						for (int j1 = 0; j1 < nbttaglist11.tagCount(); ++j1) {
							list.add(EnumChatFormatting.DARK_PURPLE + "" + EnumChatFormatting.ITALIC
									+ nbttaglist11.getStringTagAt(j1));
						}
                }
            }
        }
        if (!(multimap = this.getAttributeModifiers()).isEmpty() && (i1 & 2) == 0) {
            ...
            for (Map.Entry entry : multimap.entries()) {
                ...
                this.attackValue = (float)d0;
                if (d0 > 0.0) {
                    list.add((Object)((Object)EnumChatFormatting.BLUE) + StatCollector.translateToLocalFormatted(new StringBuilder("attribute.modifier.plus.").append(attributemodifier.getOperation()).toString(), DECIMALFORMAT.format(d1), StatCollector.translateToLocal(new StringBuilder("attribute.name.").append((String)entry.getKey()).toString())));
                    continue;
                }
                if (d0 >= 0.0) continue;
                list.add((Object)((Object)EnumChatFormatting.RED) + StatCollector.translateToLocalFormatted(new StringBuilder("attribute.modifier.take.").append(attributemodifier.getOperation()).toString(), DECIMALFORMAT.format(d1 *= -1.0), StatCollector.translateToLocal(new StringBuilder("attribute.name.").append((String)entry.getKey()).toString())));
            }
        }
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (i1 & 8) == 0 && (nbttaglist2 = this.stackTagCompound.getTagList("CanDestroy", 8)).tagCount() > 0) {
            ...
            while (k1 < nbttaglist2.tagCount()) {
                ...
                if (block != null) {
                    ...
                    list.add((Object)((Object)EnumChatFormatting.DARK_GRAY) + "missingyes");
                }
                ++k1;
            }
        }
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanPlaceOn", 9) && (i1 & 16) == 0 && (nbttaglist3 = this.stackTagCompound.getTagList("CanPlaceOn", 8)).tagCount() > 0) {
            list.add("");
        ...
        if (this.hasTagCompound() && Xatz.getModuleByName("NBTViewer").isToggled()) {

            Splitter.fixedLength((int)50).split((CharSequence)this.getTagCompound().toString()).forEach(s1 -> {
                list.add(s1);
            }
            );
	...
	public int getRepairCost() {
		return this.hasTagCompound() && this.stackTagCompound.hasKey("RepairCost", 3)
				? this.stackTagCompound.getInteger("RepairCost") : 0;
	}
