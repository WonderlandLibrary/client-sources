...
public abstract class Render<T extends Entity> {
	...
	protected void renderName(T entity, double x, double y, double z) {
		if (this.canRenderName(entity)) {
			if (!Xatz.getModuleByName("NoRender").isToggled() && !ClientSettings.HOLOGRAM) {
	...
	// TODO Atlas random for nametag render
	/**
	 * For nametags: 
	 */
	...
	protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
		// TODO Atlas healthtags!
		if (entityIn instanceof EntityLivingBase && Xatz.getModuleByName("Tags+").isToggled() && !Xatz.ghostMode) {
			str += " HP: " + Math.round(((EntityLivingBase) entityIn).getHealth()); 
		}
		if (entityIn instanceof EntityPlayer) {
			if(Xatz.devTagNames.get(entityIn.getName()) != null) {
				str = Xatz.devTagNames.get(entityIn.getName()) + str;
			}
		}
		if (Xatz.getModuleByName("NameProtect").isToggled()) {
			NameProtect np = (NameProtect) Xatz.getModuleByName("NameProtect");
			for (String s : np.replacements.keySet()) {
				str = str.replaceAll(s, np.replacements.get(s));
			}
		}
		if (Xatz.getIsTagName(entityIn.getName()) && !Xatz.ghostMode) {
			StringBuilder sb = new StringBuilder();
			sb.append((char) (rand.nextInt(10) + 780));
			sb.append(" " + str + " ");
			sb.append((char) (rand.nextInt(10) + 780));
			str = sb.toString();
		}
		...
		if (d0 <= (double) (maxDistance * maxDistance)) {
			...
			if (Xatz.getModuleByName("NameTags").isToggled() && entityIn instanceof EntityPlayer) {
				float range = (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(entityIn) / 400)
						* ClientSettings.Nametagssize;
			 ...
			 */
			if (Xatz.getIsMouseHead(entityIn.getName()) && !Xatz.ghostMode) {
				b0 = -10;
			}
			...
			if(entityIn instanceof EntityPlayer && !Xatz.ghostMode && Xatz.getModuleByName("NameTags").isToggled()) {
