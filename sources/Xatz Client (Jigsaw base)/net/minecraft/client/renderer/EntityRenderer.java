...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private void setupViewBobbing(float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			//	GlStateManager.translate(MathHelper.sin(f1 * (float) Math.PI) * f2 * 1.5F,
			//			Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2), 0.0F); // -Math.abs
			//	GlStateManager.rotate(MathHelper.sin(f1 * (float) Math.PI) * f2 * 15.0F, 0.0F, 0.0F, 1.0F);
	...
	public void setupCameraTransform(float partialTicks, int pass) {
		...
		// TODO XATZ NoHurtcam!
		if (!Xatz.getModuleByName("NoHurtcam").isToggled()) {
			this.hurtCameraEffect(partialTicks);
		}
		
		if (Xatz.getModuleByName("Australia").isToggled()) {
	...
	public void func_181560_a(float p_181560_1_, long p_181560_2_) {
		...
		if (!flag && this.mc.gameSettings.pauseOnLostFocus && (!this.mc.gameSettings.touchscreen || !Mouse.isButtonDown(1))) {
			if (Xatz.getModuleByName("NoFocusFPS").isToggled()) {
                net.minecraft.client.settings.GameSettings.limitFramerate = (int)me.xatzdevelopments.xatz.client.settings.ClientSettings.minFps;
			...
			if (Xatz.getModuleByName("NoFocusFPS").isToggled()) {
                net.minecraft.client.settings.GameSettings.limitFramerate = (int)me.xatzdevelopments.xatz.client.settings.ClientSettings.defaultFps;
               
            }
		}
		//lol you're at the wrong line I think
		//actually not my bad, code looks different though.. 
		//I dont give a fuck, this time ur wrong my dude, You see, mcp doesnt always deobf correctly so yea Get rekt The func is the same shit as ur shit code, Im in the right this time...
		//lol toxic ok xd 
            // :Sunglasses:			
		//oh shit hahaha 
		// Yes veryy haha
		//I gotta go now farewell brother
            // Bye - Napoleon
			// autist moment lol ok bye - HeyaGlitz 
            // Niger what duel me - Napoleon
			//hvh gay - HeyaGlitz
            // Fite me negro - Napoleon
		// Fuck it dont work - Napoleon 
		// Ok now it works
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		if (!flag2 && this.renderHand && !Shaders.isShadowPass) {
			...
			// TODO XATZ UIRENDERER

			//.getUIRenderer().updateFrames();
	...
	private void updateFogColor(float partialTicks) {
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			...
			Vec3 vec32 = MathHelper.sin(worldclient.getCelestialAngleRadians(partialTicks)) > 0.0F
					? new Vec3(d0, 0.0D, 0.0D) : new Vec3(1.0D, 0.0D, 0.0D);
