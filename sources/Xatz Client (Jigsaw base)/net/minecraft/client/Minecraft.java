...
import me.xatzdevelopments.xatz.client.Unused.inEvents.Listeners.inEventCanInteract;
import me.xatzdevelopments.xatz.client.Unused.inEvents.Listeners.inEventTick;
...
import me.xatzdevelopments.xatz.client.modules.AutoClicker;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	private String fakeIp = "";
	  ...
	  public boolean isUUIDHack = false;
	...
	public void run() {
        //final inEventTick e = new inEventTick();
        //e.setType(inEventType.PRE);
        //Xatz.oninEvent(e);
			...
			try {
				while (this.running) {
					if (this.crashReporter != null) {
						System.out.println("crashReporter NOT NULL");
					}
					if (this.hasCrashed) {
...
//		        Xatz.oninEvent(e);
	...
	private void startGame() throws LWJGLException, IOException {
		...
		this.mcResourceManager.registerReloadListener(this.getModelManager());
		this.renderItem = new RenderItem(this.renderEngine, this.getModelManager());
		...
		this.blockRenderDispatcher = new BlockRendererDispatcher(this.getModelManager().getBlockModelShapes(),this.gameSettings);
		...
		if (this.serverName != null) {
			...
			// TODO Xatz main menu THING
			this.displayGuiScreen(new GuiMainMenu(true));
		}
	...
	private void createDisplay() throws LWJGLException {
		...
		Display.setTitle("[Minecraft 1.8.8] " + "Loading " + Xatz.getClientName() + " " + Xatz.getClientVersion());

		try {
			Display.create((new PixelFormat()).withDepthBits(24));
		} catch (LWJGLException lwjglexception) {
			logger.error((String) "Couldn\'t set pixel format", (Throwable) lwjglexception +" Ok so, hi Napoleon here, this could be fixed if you use java 8 52 or something, just google java-for-minecraft and download that its slightly older java but its stable with minecraft and has fixed my LWLG/Pixel format issues in the past, dont forget to change your minecraft java path to the java installed from that, hope this helps.");
...
//		if(Xatz.getModuleByName("GodMode").isToggled() && GodMode.enabled) {
//			return;
//		}
	...
	public void shutdownMinecraftApplet() {
		//System.out.println("SHUTDOWN Applet");
		try {
	...
	public void clickMouse() {
		// TODO click mouse lol
		new EventAttack();
		EventAttack event2 = new EventAttack();
		Xatz.OnUpdate(event2);
		if (AutoClicker.shouldClick()) {
			this.leftClickCounter = 0;
		}
		if (this.leftClickCounter <= 0) {
			...
			if (this.objectMouseOver == null) {
				...
				case ENTITY:
					final Entity hit = this.objectMouseOver.entityHit;
					if (hit instanceof EntityPlayer) {
						if (Xatz.getFriendsMananger().isFriend((EntityPlayer) hit)
								&& !Xatz.getModuleByName("Friends").isToggled()) {
							break;
						}
					}
					// TODO Xatz friends stuff and entityHitEvent
					EntityHitEvent event = new EntityHitEvent(hit);
					Xatz.onEntityHit(event);
					if (event.isCancelled()) {
						break;
					}
					this.playerController.attackEntity(this.thePlayer, hit);
					break;
		...
		if (!this.playerController.func_181040_m()) {
			...
			if (this.objectMouseOver == null) {
				...
				case ENTITY:
					...
					Xatz.onEntityInteract(event);
					if (event.isCancelled()) {
						return;
					}
					if (this.playerController.useEntity(this.thePlayer, this.objectMouseOver.entityHit,
	...
	public void runTick() throws IOException {
        final inEventTick inEvent = new inEventTick();
        inEvent.setType(inEventType.PRE);
        Xatz.oninEvent(inEvent);
		if (this.rightClickDelayTimer > 0) {
			// TODO ATlas rightclick fast
			if (Xatz.getModuleByName("NoRightDelay").isToggled()) {
		...
		if ((this.currentScreen == null && this.thePlayer != null)
				|| this.currentScreen.allowUserInput) {
            ...
            final inEventCanInteract e = new inEventCanInteract();
            Xatz.oninEvent(e);
        ...
        inEvent.setType(inEventType.POST);
        Xatz.oninEvent(inEvent);
	}
	...
	private String func_181538_aA() {
		return this.theIntegratedServer != null
				...
				: (this.currentServerData != null
						? (this.currentServerData.func_181041_d() ? "playing_lan" : "multiplayer") : "out_of_game");
	}
	...
	public void setSessionService(final boolean generated) {
        this.setSessionService(generated, this.proxy);
    }
    
    public void setSessionService(final boolean generated, final Proxy proxy) {
        this.proxy = proxy;
        //this.sessionService = (MinecraftSessionService)((generated && !(this.sessionService instanceof HYggdrasilMinecraftSessionService)) ? new HYggdrasilMinecraftSessionService(new YggdrasilAuthenticationService(this.proxy, UUID.randomUUID().toString())) : ((!generated && this.sessionService instanceof HYggdrasilMinecraftSessionService) ? new YggdrasilAuthenticationService(this.proxy, UUID.randomUUID().toString()).createMinecraftSessionService() : this.sessionService));
    }
