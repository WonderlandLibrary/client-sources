...
import me.xatzdevelopments.xatz.client.Unused.inEvents.Listeners.inEventUpdate;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdateWalkingPlayer() {
		...
		UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ,
				this.onGround);
		Xatz.OnUpdate(event);
        final inEventUpdate e = new inEventUpdate();
        e.setType(inEventType.PRE);
        Xatz.oninEvent(e);
        final inEventMotion inEvent = new inEventMotion(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
		...
		if (this.isCurrentViewEntity() && !event.isCancelled()) {
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.x, event.y, event.z,
							event.yaw, event.pitch, event.onGround));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(
							new C03PacketPlayer.C04PacketPlayerPosition(event.x, event.y, event.z, event.onGround));
				} else if (flag3) {
					this.sendQueue.addToSendQueue(
							new C03PacketPlayer.C05PacketPlayerLook(event.yaw, event.pitch, event.onGround));
				} else {
	...
	public void sendChatMessage(String message) {
		if (message.startsWith("@")) {
			final IrcManager irc = Xatz.irc;
			if(Xatz.getModuleByName("IRC").isToggled()) {
			if (irc.isConnected()) {
				irc.sendMessage("#Xatz-Client-ImperiumE",  message.replace("@", ""));
				irc.getLines().add(new IrcLine(irc.getChatLine(true), message.replace("@", ""), irc.getName(), false));
				irc.setUnreadMessages(true);
			  }
			} else {
				Wrapper.tellPlayer("§7[§3IRC§7] Not Connected To IRC. Please enable the IRC module to communicate in the irc chata");
			}
			return;
		}
        if (message.startsWith("+")) {
            Xatz.instance.ExcommandManager.call(message.substring(1));
            return;
        }
	...
	public void swingItem() {
		// TODO Rave NoSwing
		if (Xatz.getModuleByName("NoSwing").isToggled()) {
			return;
		}
		super.swingItem();
	...
	public void setSprinting(boolean sprinting) {
		String mode = Xatz.getModuleByName("AutoSprint").getCurrentMode();
		if(Xatz.getModuleByName("AutoSprint").isToggled()) {
	...
	public void onLivingUpdate() {
		...
		// TODO Xatz Noclip and NoSlowdown...
		if (!Xatz.getModuleByName("NoClip").isToggled()
				&& !(Xatz.getModuleByName("Freecam").isToggled())) {
		...
		if (this.isRidingHorse()) {
			...
			// TODO Xatz horsegrej
			if (Xatz.getModuleByName("PerfectHorseJump").isToggled()) {
	...
	public void moveEntity(double x, double y, double z) {
		// TODO Xatz premotion
		MoveEvent event2 = new MoveEvent(x, y, z);
		Xatz.onMotion(event2);
		PreMotionEvent event = new PreMotionEvent(x, y, z);
		Xatz.onPreMotion(event);
		if (event.isCancelled()) {
			return;
		}
		super.moveEntity(event.x, event.y, event.z);
		// TODO Xatz postmotion
		Xatz.onPostMotion();
	}
	//TODO Xat onDeath
		@Override
