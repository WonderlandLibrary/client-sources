...
import me.xatzdevelopments.xatz.alts.GuiAltManager;
...
import me.xatzdevelopments.xatz.gui.GuiXatz;

import me.xatzdevelopments.xatz.gui.GuiXatzChangelog;
import me.xatzdevelopments.xatz.gui.GuiXatzCredits;
import me.xatzdevelopments.xatz.gui.GuiXatzFAQ;
import me.xatzdevelopments.xatz.gui.GuiMCLeaksRedeemToken;
import me.xatzdevelopments.xatz.gui.animations.Animation;
import me.xatzdevelopments.xatz.gui.custom.clickgui.DisplayClickGui;
import me.xatzdevelopments.xatz.gui.main.MenuParticle;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
			...
			if (!list.isEmpty()) {
				while (true) {
					...
					if (Xatz.ghostMode) {
						...
						this.splashText = (String) Xatz.getModules().get(RANDOM.nextInt(Xatz.getModules().size()))
								.getName() + "!";
					}
	...
	public void updateScreen() {
		// TODO Xatz alert thing
		if (Xatz.triedConnectToUpdate && !Xatz.triedConnectToAlert) {
			new Thread(LoadTools.alertInfoFetcher).start();
			Xatz.triedConnectToAlert = true;
		}
		++this.panoramaTimer;
	}
	...
	public void initGui() {
		...
		Xatz.getInstance().getDiscordRP().update("Idle", "Main Menu");
		...
		// TODO Xatz alt button and settings button main menu
		if (!Xatz.ghostMode) {
			//this.buttonList.add(new GuiButton(69, 5, this.height - 25, 98, 20, "Alt Login"));
			//this.buttonList.add(new GuiButton(436, width - 100, 163, 100, 20, "Alt Manager"));
			//this.buttonList.add(new GuiButton(1337, width - 100, 132 + 11 - 66, 100, 20, Xatz.getClientName().split(" ")[0]));
			//this.buttonList.add(new GuiButton(426, width - 170, 0, 170, 20, "Join our discord today!"));
			//this.buttonList.add(new GuiButton(6969, 5, this.height - 85, 98, 20, I18n.format("FAQ", new Object[0])));
			...
			//this.buttonList.add(new GuiButton(427, width - 100, 66 + -44, 100, 20, "Changelog"));
			...
			//this.buttonList.add(new GuiButton(5, width - 100, 154 - 110, 100, 20, "Website"));
			// this.buttonList.add(new GuiButton(425, width - 100, 110 + 22,
			// 100, 20, "Acc Hacker"));
			this.buttonList.add(new GuiButton(428, width - 100, 132, 100, 20, "Modules"));
			this.buttonList.add(new GuiButton(5, width - 100, 154 - 110, 100, 20, "Website"));
			this.buttonList.add(new GuiButton(427, width - 100, 66 + -44, 100, 20, "Changelog"));
			this.buttonList.add(new GuiButton(436, width - 100, 163, 100, 20, "Alt Mgr"));
			this.buttonList.add(new GuiButton(1337, width - 100, 132 + 11 - 66, 100, 20, Xatz.getClientName().split(" ")[0]));
		}
		if (Xatz.ghostMode) {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 5) {
			...
			try {
				this.openWebLink(new URI("https://xatzclient.tk/"));
		...
		//if (button.id == 69) {
		//	this.mc.displayGuiScreen(new GuiXatzAltLogin(this));
		//}
		if (button.id == 1337) {
			this.mc.displayGuiScreen(new GuiXatz(this));
		}
		if (button.id == 420) {
			this.mc.displayGuiScreen(new GuiXatzChangelog(this));
		}
		if (button.id == 6969) {
			this.mc.displayGuiScreen(new GuiXatzFAQ(this));
		}
		if (button.id == 421) {
			try {
				this.openWebLink(new URI("https://xatzclient.weebly.com/submit-a-bug.html"));
			} catch (URISyntaxException e) {
				e.printStackTrace();
			}
		}
		if (button.id == 422) {
			try {
				this.openWebLink(new URI("https://discord.gg/45ypdBMHZR"));
			} catch (URISyntaxException e) {
				e.printStackTrace();
			}
		}
		if (button.id == 423) {
			this.mc.displayGuiScreen(new GuiXatzCredits(this));
		}
		if (button.id == 424) {
			try {
		...
		if (button.id == 425) {
			this.mc.displayGuiScreen(new GuiXatzAccHacker(this));
		}
		if (button.id == 426) {
			try {
				this.openWebLink(new URI("https://discord.gg/45ypdBMHZR"));
	...
	ArrayList<Object> modules = Xatz.filterModulesByCategory(Xatz.getModules(),
...
//			tessellator.draw();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {	
			gifIndex += 1;
			if(gifIndex >= Xatz.gifLocations.size()) {
				gifIndex = 0;
			}
			...
			int k = 30;
			// TODO Xatz flash sreen thing
			if (!(Xatz.ghostMode) && flashScreen) {
				if (wait <= 0) {
			...
			if (!Xatz.ghostMode) {
				float xPlus = (((float)mouseX) - this.width / 2f) / 70f;
				float yPlus = (((float)mouseY) - this.height / 2f) / 70f;
				GlStateManager.pushMatrix();
				GlStateManager.enableBlend();
				GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				this.mc.getTextureManager().bindTexture(Xatz.newWallpaper);
				if(Xatz.gifMenu) {
					this.mc.getTextureManager().bindTexture(Xatz.gifLocations.get(gifIndex));
				}
				GlStateManager.translate(-xPlus / 2f, -yPlus / 2f, 0);
				GlStateManager.color(1.0F, 1.0F, 1.0F, 0.15f /*oh my god sex moment when I changed this value it changed my life -HeyaGlitz*/);
				...
				GlStateManager.enableBlend();
				GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				GL11.glColor4f(0f, 0f, 0.8f, 1f);
				GL11.glScalef(1.1f, 0.7f, 0f);
				this.mc.getTextureManager().bindTexture(Xatz.xatzTextureMainMenu);

				//blue
				/*GL11.glColor4f(0f, 0f, 1f, 1f);
				
				this.drawTexturedModalRect(j + 16 + xPlus-50, this.height / 2 - 100 + yPlus+50, 0, 0, 512 / 2, 512 / 2);*/
				//blue 
				
				//green
				/*GL11.glColor4f(0f, 1f, 0f, 1f);
				
				this.drawTexturedModalRect(j + 14 + xPlus-50, this.height / 2 - 100 + yPlus+50, 0, 0, 512 / 2, 512 / 2);*/
				//green 
				
				//red
				GL11.glColor4f(1f, 0f, 0f, 1f);

				this.drawTexturedModalRect(j + 12 + xPlus-50, this.height / 2 - 100 + yPlus+50, 0, 0, 512 / 2, 512 / 2);
				//red
				
				//white
				GL11.glColor4f(1f, 1f, 1f, 1f);
				
				this.drawTexturedModalRect(j + 10 + xPlus-50, this.height / 2 - 102 + yPlus+50, 0, 0, 512 / 2, 512 / 2);
				//white
...
//			this.drawCenteredString(this.fontRendererObj, this.splashText, 0, -8, -256);
			...
			if (Xatz.ghostMode) {
				...
				s = Xatz.getClientName() + "Client " + Xatz.getClientVersion() + " / by: " + Xatz.getClientAuthor();
			}

			if (this.mc.isDemo()) {
				s = s + " Demo";
			}
			int offset = Xatz.ghostMode ? 10 : 37;
			this.drawString(this.fontRendererObj, s, 2, this.height - offset, -1);
			int scroll = 0;
			if(LoadTools.showupdatetext !=false && Xatz.ghostMode !=true && Xatz.serverVersion !=Xatz.getClientmultiVersion()) {
				...
				drawCenteredString(fontRendererObj, "Latest version is: §6" + Xatz.serverVersion, width / 2, 30 - scroll,
						0xffffff);
			}
			if (!Xatz.ghostMode && !Xatz.firstStart && Xatz.motd != null) {
				this.drawString(this.fontRendererObj, Xatz.motd,
						(this.width / 2) - this.fontRendererObj.getStringWidth(Xatz.motd) / 2, this.height - 15 - offset,
						-1);
			}
			String s2 = "Welcome " + mc.session.getUsername();
			if(!Xatz.ghostMode) {
			this.drawString(this.fontRendererObj, s2, 5, 9, -1);
			}
		
		// TODO XATZ MODULE COUNT
		if (!Xatz.ghostMode) {
//			this.drawString(this.fontRendererObj,
//					Xatz.getClientName() + " currently has " + modules.size() + " usable hacks!", 5, 5, -1);
		...
		if (!Xatz.ghostMode) {
			for (int ii = 0; ii < this.buttonList.size(); ++ii) {
				((GuiButton) this.buttonList.get(ii)).drawXatzButton(this.mc, mouseX, mouseY);
			}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if(mouseX >= width - 10) {
			...
			if(ClientSettings.bgImage >= Xatz.images.length) {
