...
import me.xatzdevelopments.xatz.gui.GuiXatzUseProxy;
...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
	 ...
	 */
	public void initGui() {
		Xatz.getInstance().getDiscordRP().update("Idle", "Multiplayer Menu");
		Keyboard.enableRepeatEvents(true);
		this.buttonList.clear();
		if (!Xatz.ghostMode) {
			//this.buttonList.add(new GuiButton(14654654, 120, 18, 110, 11, "Hack Test (FusionX)"));
			//this.buttonList.add(new GuiButton(85745654, 5, 18, 110, 11, "Hack Test (XPross)"));
			//this.buttonList.add(new GuiButton(85745654, 7, 7, 120, 20, "§cUse a §6§lProxy§c Server"));
		}
		if (!this.initialized) {
	...
	public void createButtons() {
		...
		if (!Xatz.ghostMode) { // Le smort
	    this.buttonList.add(new GuiButton(5, 5, 55, 98, 20,  "Set nick/UUID"));
	    this.buttonList.add(new GuiButton(69, 5, 33, 98, 20,  "ViaVersions"));
	    this.buttonList.add(new GuiButton(420, 5, 77, 98, 20, "SOCKS5 Proxy"));
		}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.enabled) {
			...
			if (button.id == 2 && guilistextended$iguilistentry instanceof ServerListEntryNormal) {
				...
				this.mc.displayGuiScreen(new GuiXatzUseProxySocks5(this));
			}else  if (button.id == 5) {
	            this.mc.displayGuiScreen(new GuiBungeeCord(this));
			...
			if (button.id == 14654654) {
				Minecraft.getMinecraft().addScheduledTask(new Runnable() {
					@Override
					public void run() {
						Minecraft.getMinecraft().displayGuiScreen(new GuiConnecting(new GuiMainMenu(),
								Minecraft.getMinecraft(), new ServerData("", "FusionX.us.to:25641", false)));
					}
				});

			}
			if (button.id == 85745654) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, I18n.format("multiplayer.title", new Object[0]),
				this.width / 2, 20, 16777215);

		if (!Xatz.ghostMode) {
//			drawString(fontRendererObj, "§bMCLeaks §cStatus: " + MCLeaks.getStatus(),
//					width - mc.fontRendererObj.getStringWidth("§bMCLeaks §cStatus: " + MCLeaks.getStatus()) - 10, 8,
//					0xffffffff);
			// drawString(fontRendererObj, "Hack Test Servers:", 6, 6,
			// 0xffffffff);
		}
