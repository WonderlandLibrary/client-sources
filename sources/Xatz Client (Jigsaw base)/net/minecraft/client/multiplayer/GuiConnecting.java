	...
	private void connect(final String ip, final int port) {
		if(ip.endsWith("hypixel.net")) {
			Xatz.getNotificationManager().addNotification(new Notification(Level.WARNING, 
					"Xatz is not meant for Hypixel, if you wish to continue, use autoclicker etc"));
		}
		if(ip.endsWith("mineplex.net")) {
			Xatz.getNotificationManager().addNotification(new Notification(Level.INFO, 
					"Enjoy playing in this dumb of a server!"));
		}
			...
			public void run() {
				...
				try {
					...
					if (MCLeaks.isAltActive()) {
						GuiConnecting.this.networkManager
								.sendPacket(new C00PacketLoginStart(new GameProfile((UUID) null, MCLeaks.getMCName())));
					} else {
						networkManager.sendPacketFinal(
								new C00PacketLoginStart(GuiConnecting.this.mc.getSession().getProfile()));
						Xatz.getInstance().getDiscordRP().update("Playing " + ip , "In Game");
					}
