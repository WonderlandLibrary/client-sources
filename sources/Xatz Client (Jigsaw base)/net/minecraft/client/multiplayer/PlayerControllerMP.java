...
public class PlayerControllerMP {
		...
		if (!this.mc.theWorld.getWorldBorder().contains(loc)) {
			...
			if (this.currentGameType.isCreative()) {
				...
				if (flag && block1.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, loc) >= 1.0F) {
					 if (!Xatz.getModuleByName("Jesus").isToggled()) {
	...
	public float getBlockReachDistance() {
		if (Xatz.getModuleByName("ExtendedReach").isToggled()) { //TODO: extended reach
			return (float) ClientSettings.ExtendedReachrange;
		}
	...
	public boolean onPlayerRightClick(EntityPlayer player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos,
		...
		if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
			...
			BlockPlaceEvent event = new BlockPlaceEvent(heldStack, hitPos, side, hitVec);
			Xatz.onBlockPlace(event);
	...
	public boolean useEntity(EntityPlayer p_178894_1_, Entity p_178894_2_, MovingObjectPosition p_178894_3_) {
		this.syncCurrentPlayItem();
	...
	public boolean extendedReach() {
		if (Xatz.getModuleByName("ExtendedReach").isToggled()) {
			return true;
		}
		return this.currentGameType.isCreative();
	}
