...
public class HighJump extends Module {
	...
	public void onEnable() {
		if (height == 0) {
			this.toggle();
			Xatz.chatMessage("§cHeight can't be 0!");
		}
		if (height > 20) {
			this.toggle();
			Xatz.chatMessage("§cHeight can't be more than 7!");
		}
		if (delay > 0) {
			this.toggle();
			Xatz.chatMessage("§cDelay can't be more than 0!");
		}
		if (delay < 0) {
			this.toggle();
			Xatz.chatMessage("§cDelay can't be less than 0!");
		}
		Xatz.chatMessage("Jumping " + height + " times with delay: " + delay);
		super.onEnable();
	}
