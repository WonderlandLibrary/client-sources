...
import me.xatzdevelopments.xatz.client.tools.BlockUtils;
import me.xatzdevelopments.xatz.client.tools.LiquidUtils;
import net.minecraft.client.Minecraft;
...
//		});
	...
	private boolean shouldSpeedUp()
	  {
	    ...
	    return (!mc.thePlayer.isInWater()) && (!LiquidUtils.isInLiquid()) && (!mc.thePlayer.isOnLadder()) && (!mc.thePlayer.isSneaking()) && (!LiquidUtils.isOnLiquid()) && (moving);
	  }
	...
	public void onPreMotion(PreMotionEvent event) {
		if (Xatz.getModuleByName("BETAFly").isToggled()) {
			jumps = -1;
			this.stage = 2;
			return;
		}
	 ...
	 private boolean canSpeed(boolean groundCheck)
	  {
	    ...
	    List collidingBoundingBoxes = Minecraft.theWorld.getCollidingBoundingBoxes(mc.thePlayer, mc.thePlayer.getEntityBoundingBox().expand(0.25D, 0.0D, 0.25D));
	  ...
	  {
	    return (float)Math.sqrt(mc.thePlayer.motionX * mc.thePlayer.motionX + 
