...
public class ArrowDodge extends Module {
	...
	public void onUpdate() {
		...
		for (Entity e : mc.theWorld.loadedEntityList) {
			...
			if (arrow.shootingEntity instanceof EntityPlayer) {
				...
				if (mc.thePlayer.isOnSameTeam(player) && !Xatz.getModuleByName("Team").isToggled()) {
					continue;
				}
				if (Xatz.getFriendsMananger().isFriend(player) && !Xatz.getModuleByName("Friends").isToggled()) {
					continue;
				}
			}
			...
			// continue;
			// }
			// Xatz.chatMessage(rayTrace.toString());
	...
	private void doBarrier() {
		timer.reset();
		Xatz.chatMessage("Arrow incoming!!");
		int slot = -1;
		for (int i = 0; i < 9; i++) {
