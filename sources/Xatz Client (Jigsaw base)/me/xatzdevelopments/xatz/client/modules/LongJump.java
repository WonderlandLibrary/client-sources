	...
	if (currentMode.equals("New")){
		 boolean moving = (this.mc.gameSettings.keyBindForward.isKeyDown()) || (this.mc.gameSettings.keyBindBack.isKeyDown());
		    ...
		    if ((!mc.thePlayer.isCollidedVertically) && (!mc.thePlayer.onGround))
		    {
		      ...
		      if (!mc.thePlayer.isCollidedVertically)
		      {
		        if ((this.airTicks - 6 >= 0) && (this.airTicks - 6 < glide.length)) {
		          ...
		          mc.thePlayer.motionY *= glide[(this.airTicks - 6)];
		        }
		        if ((mc.thePlayer.motionY < -0.2D) && (mc.thePlayer.motionY > -0.24D)) {
		          mc.thePlayer.motionY *= 0.7D;
		        } else if ((mc.thePlayer.motionY < -0.25D) && (mc.thePlayer.motionY > -0.32D)) {
		          mc.thePlayer.motionY *= 0.8D;
		        } else if ((mc.thePlayer.motionY < -0.35D) && (mc.thePlayer.motionY > -0.8D)) {
		          mc.thePlayer.motionY *= 0.98D;
		        }
		      }
		      if ((this.airTicks - 1 >= 0) && (this.airTicks - 1 < motion.length))
		      {
		        mc.thePlayer.motionX = (forward * motion[(this.airTicks - 1)] * 3.0D * Math.cos(Math.toRadians(yaw + 90.0F)));
		        mc.thePlayer.motionZ = (forward * motion[(this.airTicks - 1)] * 3.0D * Math.sin(Math.toRadians(yaw + 90.0F)));
		      }
		      else
		      {
		    ...
		    else
		    {
		      ...
		      {
		        mc.thePlayer.motionX = (forward * 0.009999999776482582D * Math.cos(Math.toRadians(yaw + 90.0F)));
		        mc.thePlayer.motionZ = (forward * 0.009999999776482582D * Math.sin(Math.toRadians(yaw + 90.0F)));
		      }
		      else if (this.groundTicks > 2)
		      {
		        mc.thePlayer.motionX = (forward * 0.30000001192092896D * Math.cos(Math.toRadians(yaw + 90.0F)));
		        mc.thePlayer.motionZ = (forward * 0.30000001192092896D * Math.sin(Math.toRadians(yaw + 90.0F)));
