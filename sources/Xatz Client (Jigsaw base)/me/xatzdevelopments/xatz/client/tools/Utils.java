...
import me.xatzdevelopments.xatz.client.modules.AutoBlock;
import me.xatzdevelopments.xatz.client.modules.Criticals;
...
//	static ArrayList<Vec3> positionsBack = new ArrayList<Vec3>();
	...
	public static boolean infiniteReach(double range, double maxXZTP, double maxYTP, 
		...
		boolean sneaking = mc.thePlayer.isSneaking() || Xatz.getModuleByName("AutoSneak").isToggled();
		...
		for (int i = 0; i < range; i++) {
			steps++;
			//Xatz.chatMessage(maxXZTP * steps);
		...
		for (int i = 0; i < steps; i++) {
			...
			if (i != steps - 1) {
				...
				if (Math.abs(yDist) < maxYTP && mc.thePlayer.getDistanceToEntity(en) >= 4) {
					...
					z = zPreEn;
					sendPacket(false, positionsBack, positions);
					if (Xatz.getModuleByName("MegaKnockback").isToggled() && en.onGround) {
...
//			return false;
//		}
//		if (en instanceof EntityPlayer && Xatz.getFriendsMananger().isFriend((EntityPlayer) en)) {
//			if (!Xatz.getModuleByName("Friends").isToggled()) {
//				return false;
//			}
//		}
//		if (en.isInvisible()) {
//			if (!Xatz.getModuleByName("Invisible").isToggled()) {
//				return false;
//			}
//		}
//		if (en instanceof EntityPlayer) {
//			if (!Xatz.getModuleByName("Players").isToggled() || en.height < 0.21f) {
//				return false;
//			}
//		}
//		if (Team.isOnTeam(en)) {
//			if (!Xatz.getModuleByName("Team").isToggled()) {
//				return false;
//			}
//		}
//		if (!(en instanceof EntityPlayer)) {
//			if (!Xatz.getModuleByName("NonPlayers").isToggled()) {
//				return false;
//			}
//		}
//		if ((en instanceof EntityPlayer)) {
//			if (Xatz.getBypassManager().getEnabledBypass() != null && Xatz.getBypassManager().getEnabledBypass().getName().equals("AntiGwen")) {
...
//				}
//			}
//			if(Xatz.getBypassManager().getEnabledBypass() != null && Xatz.getBypassManager().getEnabledBypass().getName().equals("AntiWatchdog")) {
...
//	}
	...
	public static boolean validEntity(EntityLivingBase en) {
		...
		if (!(en instanceof EntityPlayer) && en instanceof EntityLiving && Xatz.getModuleByName("Skip Unarmored Mobs").isToggled()) {
		...
		if ((en instanceof EntityPlayer) && Xatz.getModuleByName("Skip Unarmored Players").isToggled()) {
		...
		if ((en instanceof EntityPlayer)) {
			if (Xatz.getBypassManager().getEnabledBypass() != null && Xatz.getBypassManager().getEnabledBypass().getName().equals("AntiGwen") || Xatz.getModuleByName("AntiBot").isToggled()) {
				if (!((EntityPlayer) en).didSwingItem) {
					if (en.onGround) {
						if (en.isSprinting()) {
							return true;
