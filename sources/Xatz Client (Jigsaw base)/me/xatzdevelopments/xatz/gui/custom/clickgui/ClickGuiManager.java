...
public class ClickGuiManager {
		...
		for(Module m : Xatz.getModules()) {
			...
			if(m.isCheckbox()) {
				...
				if(m.getModes().length > 1) {
					ModeButton modeBtn = new ModeButton(m.getModeName(), m);
	...
	public void draw() {
		GlStateManager.scale(2d / Minecraft.getMinecraft().gameSettings.guiScale, 2d / Minecraft.getMinecraft().gameSettings.guiScale, 0);
		for(ModuleWindow wind : windows) {
			wind.draw();
		}
	}
