...
public class ModuleWindow extends Container {
	...
	public void draw() {
		...
		GL11.glColor4f((float)ClientSettings.hudcolorred, (float)ClientSettings.hudcolorgreen, (float)ClientSettings.hudcolorblue, 1f - Xatz.getClickGuiManager().getAlpha());//module category box color -masterjunior24
		//System.out.println((float)ClientSettings.hudcolorgreen+"   "+(float)ClientSettings.hudcolorblue+"   "+(float)ClientSettings.hudcolorred);
		...
		glEnd();
		if(extended) {
			/*GL11.glColor4f(0.8f, 0.2f, 0.2f, 0.4f - Xatz.getClickGuiManager().getAlpha());//line at the bottom of a module category -masterjunior24
			...
			glEnd();*/
			GL11.glColor4f(0.15f, 0.15f, 0.15f, 0.9f - Xatz.getClickGuiManager().getAlpha());//background of module category -masterjunior
			glBegin(GL_QUADS);
			{
				glVertex2d(0, 0 + headHeight);
		...
		if(title != null) {
			titleRenderer.drawString(title, 2, 1, 0xffffffff);//module category name -masterjunior24
		}
	...
	public void onClicked(double x, double y, int button) {
		Xatz.getClickGuiManager().windows.sort(new Comparator<ModuleWindow>() {
