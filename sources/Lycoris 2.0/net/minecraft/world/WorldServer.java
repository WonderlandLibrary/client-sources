...
public class WorldServer extends World implements IThreadListener {
	...
	private ServerBlockEventList[] field_147490_S = new ServerBlockEventList[] { new ServerBlockEventList(),
			...
			for (ChunkCoordIntPair chunkcoordintpair : this.activeChunkSet) {
				...
				if (this.rand.nextInt(100000) == 0 && this.isRaining() && this.isThundering()) {
					...
					BlockPos blockpos = this
							.adjustPosToNearbyEntity(new BlockPos(k + (i1 & 0xF), 0, l + (i1 >> 8 & 0xF)));
					if (this.canLightningStrike(blockpos)) {
						this.addWeatherEffect(
								new EntityLightningBolt(this, blockpos.getX(), blockpos.getY(), blockpos.getZ()));
					}
				}
				...
				if (this.rand.nextInt(16) == 0) {
					...
					BlockPos blockpos2 = this
							.getPrecipitationHeight(new BlockPos(k + (k2 & 0xF), 0, l + (k2 >> 8 & 0xF)));
					...
					for (ExtendedBlockStorage extendedblockstorage : chunk.getBlockStorageArray()) {
						...
						for (int j1 = 0; j1 < l2; ++j1) {
							...
							block.randomTick(this,
									new BlockPos(l1 + k, j2 + extendedblockstorage.getYLocation(), i2 + l), iblockstate,
									this.rand);
						}
					}
				}
	...
	protected BlockPos adjustPosToNearbyEntity(BlockPos pos) {
		...
		AxisAlignedBB axisalignedbb = new AxisAlignedBB(blockpos,
				new BlockPos(blockpos.getX(), this.getHeight(), blockpos.getZ())).expand(3.0, 3.0, 3.0);
		...
		if (this.scheduledUpdatesAreImmediate && blockIn.getMaterial() != Material.air) {
			if (blockIn.requiresUpdates()) {
				...
				if (this.isAreaLoaded(nextticklistentry.position.add(-i, -i, -i),
						...
						&& (iblockstate = this.getBlockState(nextticklistentry.position)).getBlock()
	...
	public List<NextTickListEntry> func_175712_a(StructureBoundingBox structureBB, boolean p_175712_2_) {
		ArrayList list = null;
		for (int i = 0; i < 2; ++i) {
			Iterator<NextTickListEntry> iterator = i == 0 ? this.pendingTickListEntriesTreeSet.iterator()
	...
	protected void createBonusChest() {
		...
		for (int i = 0; i < 10 && !worldgeneratorbonuschest.generate(this, this.rand, blockpos = this
				.getTopSolidOrLiquidBlock(
						new BlockPos(j = this.worldInfo.getSpawnX() + this.rand.nextInt(6) - this.rand.nextInt(6), 0,
								k = this.worldInfo.getSpawnZ() + this.rand.nextInt(6) - this.rand.nextInt(6)))
				.up()); ++i) {
		}
	}
	...
	public boolean addWeatherEffect(Entity entityIn) {
		if (super.addWeatherEffect(entityIn)) {
			this.mcServer.getConfigurationManager().sendToAllNear(entityIn.posX, entityIn.posY, entityIn.posZ, 512.0,
	...
	public Explosion newExplosion(Entity entityIn, double x, double y, double z, float strength, boolean isFlaming,
		...
		for (EntityPlayer entityplayer : this.playerEntities) {
			if (!(entityplayer.getDistanceSq(x, y, z) < 4096.0))
				continue;
			((EntityPlayerMP) entityplayer).playerNetServerHandler.sendPacket(new S27PacketExplosion(x, y, z, strength,
					explosion.getAffectedBlockPositions(), explosion.getPlayerKnockbackMap().get(entityplayer)));
		}
		return explosion;
	}
		...
		while (!this.field_147490_S[this.blockEventCacheIndex].isEmpty()) {
			...
			for (BlockEventData blockeventdata : this.field_147490_S[i]) {
				if (!this.fireBlockEvent(blockeventdata))
					continue;
				this.mcServer.getConfigurationManager().sendToAllNear(blockeventdata.getPosition().getX(),
						blockeventdata.getPosition().getY(), blockeventdata.getPosition().getZ(), 64.0,
						this.provider.getDimensionId(),
	...
	public void spawnParticle(EnumParticleTypes particleType, boolean longDistance, double xCoord, double yCoord,
			...
			int... p_180505_18_) {
		S2APacketParticles packet = new S2APacketParticles(particleType, longDistance, (float) xCoord, (float) yCoord,
