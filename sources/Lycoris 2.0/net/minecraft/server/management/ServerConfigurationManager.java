...
public abstract class ServerConfigurationManager {
	...
	public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn) {
		...
		ChatComponentTranslation chatcomponenttranslation = !playerIn.getName().equalsIgnoreCase(s)
				? new ChatComponentTranslation("multiplayer.player.joined.renamed", playerIn.getDisplayName(), s)
				: new ChatComponentTranslation("multiplayer.player.joined", playerIn.getDisplayName());
		...
		if (nbttagcompound != null && nbttagcompound.hasKey("Riding", 10) && (entity = EntityList
				.createEntityFromNBT(nbttagcompound.getCompoundTag("Riding"), worldserver)) != null) {
	...
	public String allowUserToConnect(SocketAddress address, GameProfile profile) {
		...
		return this.playerEntityList.size() >= this.maxPlayers && !this.func_183023_f(profile) ? "The server is full!"
				: null;
	}
	...
	public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP playerIn, int dimension, boolean conqueredEnd) {
		...
		ItemInWorldManager iteminworldmanager = this.mcServer.isDemo()
				? new DemoWorldManager(this.mcServer.worldServerForDimension(playerIn.dimension))
				: new ItemInWorldManager(this.mcServer.worldServerForDimension(playerIn.dimension));
	...
	public void transferEntityToWorld(Entity entityIn, int p_82448_2_, WorldServer p_82448_3_, WorldServer p_82448_4_) {
		...
		if (entityIn.dimension == -1) {
			d0 = MathHelper.clamp_double(d0 / d2, p_82448_4_.getWorldBorder().minX() + 16.0,
					p_82448_4_.getWorldBorder().maxX() - 16.0);
			d1 = MathHelper.clamp_double(d1 / d2, p_82448_4_.getWorldBorder().minZ() + 16.0,
					p_82448_4_.getWorldBorder().maxZ() - 16.0);
	...
	public void sendToAllNearExcept(EntityPlayer p_148543_1_, double x, double y, double z, double radius,
		...
		for (int i = 0; i < this.playerEntityList.size(); ++i) {
			...
			if (entityplayermp == p_148543_1_ || entityplayermp.dimension != dimension
					|| !((d0 = x - entityplayermp.posX) * d0 + (d1 = y - entityplayermp.posY) * d1
							+ (d2 = z - entityplayermp.posZ) * d2 < radius * radius))
				continue;
