...
public class CustomColorizer {
    ...
    public static void update() {
        ...
        waterColors = CustomColorizer.getCustomColors(s, astring, 65536);
        if (Config.isCustomColors()) {
            String[] astring1 = new String[]{"pine.png", "pinecolor.png"};
            foliagePineColors = CustomColorizer.getCustomColors(s, astring1, 65536);
            String[] astring2 = new String[]{"birch.png", "birchcolor.png"};
            foliageBirchColors = CustomColorizer.getCustomColors(s, astring2, 65536);
            String[] astring3 = new String[]{"swampgrass.png", "swampgrasscolor.png"};
            swampGrassColors = CustomColorizer.getCustomColors(s, astring3, 65536);
            String[] astring4 = new String[]{"swampfoliage.png", "swampfoliagecolor.png"};
            swampFoliageColors = CustomColorizer.getCustomColors(s, astring4, 65536);
            String[] astring5 = new String[]{"sky0.png", "skycolor0.png"};
            skyColors = CustomColorizer.getCustomColors(s, astring5, 65536);
            String[] astring6 = new String[]{"fog0.png", "fogcolor0.png"};
            fogColors = CustomColorizer.getCustomColors(s, astring6, 65536);
            String[] astring7 = new String[]{"underwater.png", "underwatercolor.png"};
            underwaterColors = CustomColorizer.getCustomColors(s, astring7, 65536);
            String[] astring8 = new String[]{"redstone.png", "redstonecolor.png"};
            redstoneColors = CustomColorizer.getCustomColors(s, astring8, 16);
            String[] astring9 = new String[]{"stem.png", "stemcolor.png"};
            stemColors = CustomColorizer.getCustomColors(s, astring9, 8);
            String[] astring10 = new String[]{"myceliumparticle.png", "myceliumparticlecolor.png"};
            myceliumParticleColors = CustomColorizer.getCustomColors(s, astring10, -1);
            int[][] aint = new int[3][];
            lightMapsColorsRgb = new float[3][][];
            lightMapsHeight = new int[3];
            for (int i = 0; i < aint.length; ++i) {
                String s1 = "mcpatcher/lightmap/world" + (i - 1) + ".png";
                aint[i] = CustomColorizer.getCustomColors(s1, -1);
                if (aint[i] != null) {
                    CustomColorizer.lightMapsColorsRgb[i] = CustomColorizer.toRgb(aint[i]);
                }
                CustomColorizer.lightMapsHeight[i] = CustomColorizer.getTextureHeight(s1, 32);
            }
    ...
    private static void readCustomPalettes(Properties p_readCustomPalettes_0_, String p_readCustomPalettes_1_) {
        ...
        for (Object s : p_readCustomPalettes_0_.keySet()) {
            ...
            if (!((String)s).startsWith(s7)) continue;
            map.put(s, s1);
        }
        String[] astring2 = (String[]) map.keySet().toArray(new String[map.size()]);
        ...
        for (int l = 0; l < astring2.length; ++l) {
            ...
            for (int j = 0; j < astring.length; ++j) {
                ...
                if ((i1 = Config.parseInt(s5, -1)) >= 0 && i1 <= 255) {
                    ...
                    if (k == -1) {
                        CustomColorizer.blockPalettes[i1][0] = l;
                        continue;
                    }
                    ...
                    CustomColorizer.blockPalettes[i1][k] = l;
                    continue;
                }
    ...
    private static int readColor(Properties p_readColor_0_, String[] p_readColor_1_) {
        for (int i = 0; i < p_readColor_1_.length; ++i) {
            ...
            int j = CustomColorizer.readColor(p_readColor_0_, s);
            if (j < 0) continue;
            return j;
        }
        return -1;
    ...
    private static int readColor(Properties p_readColor_0_, String p_readColor_1_) {
        ...
        try {
            int i = Integer.parseInt(s, 16) & 0xFFFFFF;
    ...
    private static Vec3 readColorVec3(Properties p_readColorVec3_0_, String p_readColorVec3_1_) {
        int i = CustomColorizer.readColor(p_readColorVec3_0_, p_readColorVec3_1_);
        if (i < 0) {
            return null;
        }
        int j = i >> 16 & 0xFF;
    ...
    private static int[] getCustomColors(String p_getCustomColors_0_, String[] p_getCustomColors_1_, int p_getCustomColors_2_) {
        for (int i = 0; i < p_getCustomColors_1_.length; ++i) {
            ...
            int[] aint = CustomColorizer.getCustomColors(s, p_getCustomColors_2_);
    ...
    public static int getColorMultiplier(BakedQuad p_getColorMultiplier_0_, Block p_getColorMultiplier_1_, IBlockAccess p_getColorMultiplier_2_, BlockPos p_getColorMultiplier_3_, RenderEnv p_getColorMultiplier_4_) {
        ...
        if (flag1) {
            return CustomColorizer.getSmoothColorMultiplier(p_getColorMultiplier_1_, p_getColorMultiplier_2_, p_getColorMultiplier_3_, aint, aint1, l, i1, p_getColorMultiplier_4_);
        }
        ...
        for (int k1 = l - 1; k1 <= l + 1; ++k1) {
            for (int l1 = j1 - 1; l1 <= j1 + 1; ++l1) {
                ...
                if (aint == null) {
                    ...
                    i2 = CustomColorizer.getCustomColor(aint, p_getSmoothColorMultiplier_1_, blockposm);
                }
                i += i2 >> 16 & 0xFF;
                j += i2 >> 8 & 0xFF;
                k += i2 & 0xFF;
    ...
    public static void updateReddustFX(EntityFX p_updateReddustFX_0_, IBlockAccess p_updateReddustFX_1_, double p_updateReddustFX_2_, double p_updateReddustFX_4_, double p_updateReddustFX_6_) {
        ...
        int i;
        int j;
        if (redstoneColors != null && (j = CustomColorizer.getRedstoneColor(i = CustomColorizer.getRedstoneLevel(iblockstate = p_updateReddustFX_1_.getBlockState(new BlockPos(p_updateReddustFX_2_, p_updateReddustFX_4_, p_updateReddustFX_6_)), 15))) != -1) {
            int k = j >> 16 & 0xFF;
            int l = j >> 8 & 0xFF;
            int i1 = j & 0xFF;
    ...
    public static void updateWaterFX(EntityFX p_updateWaterFX_0_, IBlockAccess p_updateWaterFX_1_, double p_updateWaterFX_2_, double p_updateWaterFX_4_, double p_updateWaterFX_6_) {
        if (waterColors != null) {
            int i = CustomColorizer.getFluidColor(Blocks.water, p_updateWaterFX_1_, new BlockPos(p_updateWaterFX_2_, p_updateWaterFX_4_, p_updateWaterFX_6_));
            int j = i >> 16 & 0xFF;
            int k = i >> 8 & 0xFF;
            int l = i & 0xFF;
            float f = (float)j / 255.0f;
    ...
    public static Vec3 getSkyColor(Vec3 p_getSkyColor_0_, IBlockAccess p_getSkyColor_1_, double p_getSkyColor_2_, double p_getSkyColor_4_, double p_getSkyColor_6_) {
        ...
        int i = CustomColorizer.getSmoothColor(skyColors, p_getSkyColor_1_, p_getSkyColor_2_, p_getSkyColor_4_, p_getSkyColor_6_, 7, 1);
        int j = i >> 16 & 0xFF;
        int k = i >> 8 & 0xFF;
        int l = i & 0xFF;
        float f = (float)j / 255.0f;
        ...
        return new Vec3(f *= f3, f1 *= f4, f2 *= f5);
    }
    ...
    public static Vec3 getFogColor(Vec3 p_getFogColor_0_, IBlockAccess p_getFogColor_1_, double p_getFogColor_2_, double p_getFogColor_4_, double p_getFogColor_6_) {
        ...
        int i = CustomColorizer.getSmoothColor(fogColors, p_getFogColor_1_, p_getFogColor_2_, p_getFogColor_4_, p_getFogColor_6_, 7, 1);
        int j = i >> 16 & 0xFF;
        int k = i >> 8 & 0xFF;
        int l = i & 0xFF;
        float f = (float)j / 255.0f;
    ...
    public static Vec3 getUnderwaterColor(IBlockAccess p_getUnderwaterColor_0_, double p_getUnderwaterColor_1_, double p_getUnderwaterColor_3_, double p_getUnderwaterColor_5_) {
        ...
        int i = CustomColorizer.getSmoothColor(underwaterColors, p_getUnderwaterColor_0_, p_getUnderwaterColor_1_, p_getUnderwaterColor_3_, p_getUnderwaterColor_5_, 7, 1);
        int j = i >> 16 & 0xFF;
        int k = i >> 8 & 0xFF;
        int l = i & 0xFF;
        float f = (float)j / 255.0f;
    ...
    public static int getSmoothColor(int[] p_getSmoothColor_0_, IBlockAccess p_getSmoothColor_1_, double p_getSmoothColor_2_, double p_getSmoothColor_4_, double p_getSmoothColor_6_, int p_getSmoothColor_8_, int p_getSmoothColor_9_) {
        ...
        for (int i2 = i - l; i2 <= i + l; i2 += p_getSmoothColor_9_) {
            for (int j2 = k - l; j2 <= k + l; j2 += p_getSmoothColor_9_) {
                ...
                int k2 = CustomColorizer.getCustomColor(p_getSmoothColor_0_, p_getSmoothColor_1_, blockposm);
                i1 += k2 >> 16 & 0xFF;
                j1 += k2 >> 8 & 0xFF;
                k1 += k2 & 0xFF;
                ++l1;
    ...
    public static int mixColors(int p_mixColors_0_, int p_mixColors_1_, float p_mixColors_2_) {
        ...
        int i = p_mixColors_0_ >> 16 & 0xFF;
        int j = p_mixColors_0_ >> 8 & 0xFF;
        int k = p_mixColors_0_ & 0xFF;
        int l = p_mixColors_1_ >> 16 & 0xFF;
    ...
    private static int averageColor(int p_averageColor_0_, int p_averageColor_1_) {
        int i = p_averageColor_0_ >> 16 & 0xFF;
        int j = p_averageColor_0_ >> 8 & 0xFF;
        int k = p_averageColor_0_ & 0xFF;
        int l = p_averageColor_1_ >> 16 & 0xFF;
    ...
    public static boolean updateLightmap(World p_updateLightmap_0_, float p_updateLightmap_1_, int[] p_updateLightmap_2_, boolean p_updateLightmap_3_) {
        ...
        if (i >= -1 && i <= 1) {
            ...
            CustomColorizer.getLightMapColumn(afloat, f1, i1, l, sunRgbs);
            CustomColorizer.getLightMapColumn(afloat, f2, i1 + 16 * l, l, torchRgbs);
