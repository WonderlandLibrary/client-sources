...
import static wtf.shiyeno.util.math.PlayerPositionTracker.isInView;
...
public class ESPFunction extends Function {
    ...
    private void renderPlayerElements(MatrixStack stack) {
        ...
        for (Map.Entry<Vector4d, PlayerEntity> entry : positions.entrySet()) {
            ...
            if (elements.get(2)) {
                BloomHelper.registerRenderCall(() -> {
                    Fonts.sfbold[10].drawStringWithOutline(stack, healthText, position.x - 3 - size.getValue().floatValue() - Fonts.sfbold[10].getWidth(healthText), position.y + (height * (1 - player.animationPerc)) + 1, colors.x);
                });
                Fonts.sfbold[10].drawStringWithOutline(stack, healthText, position.x - 3 - size.getValue().floatValue() - Fonts.sfbold[10].getWidth(healthText), position.y + (height * (1 - player.animationPerc)) + 1, colors.x);
            }
            if (elements.get(3)) {
    ...
    private void renderEffects(PlayerEntity player,
        ...
        for (int i = 0; i < effectCount; i++) {
            ...
            Fonts.sfbold[12].drawStringWithShadow(matrices, effectString, endX + 2.5f, y - 2 + ((i + 1) * 8), -1);
        }
    }
    ...
    private void renderArmorAndEnchantment(List<ItemStack> stacks,
        ...
        for (ItemStack stack : stacks) {
            ...
            int i = 0;
            for (String s : lines) {
                Fonts.sfbold[12].drawCenteredString(matrixStack, s,
                        center,
