package wtf.shiyeno.ui.clickgui.binds;
...
public class BindWindow {
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY) {
        ...
        GlStateManager.translatef(-(x + width / 2f), -(y + height / 2f), 0);
        RenderUtil.Render2D.drawShadow(x - 1f, y - 1f, width + 2, height + 2, 15, ColorUtil.rgba(100, 100, 100, 255 * bindAnimation));
        RenderUtil.Render2D.drawRoundedCorner(x - 1f, y - 1f, width + 2, height + 2, 7f, ColorUtil.rgba(100, 100, 100, 255));
        ...
        Fonts.msBold[13].drawString(stack, "Bind", x + 5, y + height / 2f + 5, ColorUtil.rgba(100, 100, 100, 255));
        RenderUtil.Render2D.drawImage(new ResourceLocation("shiyeno/images/cross.png"), x + width - 12, y + 5, 5, 5, -1);
        ...
        Fonts.msBold[14].drawCenteredString(stack, key.toUpperCase(), x + width - 5 - wwidth + wwidth / 2f, y + 19 + 4, -1);
        ...
        if (openAnimation) {
            ...
            if (binding && button > 2) {
                if (object instanceof ModuleObject m) {
                    ...
                    Managment.NOTIFICATION_MANAGER.add("Module " + TextFormatting.GRAY + m.function.name + TextFormatting.WHITE + " was keyed " + ClientUtil.getKey(-100 + button).toUpperCase(), "Module Info", 2);
                } else if (object instanceof BindObject m) {
                    m.set.setKey(-100 + button);
                    Managment.NOTIFICATION_MANAGER.add("Module " + TextFormatting.GRAY + m.object.function.name + TextFormatting.WHITE + " was binned on the button " + ClientUtil.getKey(-100 + button).toUpperCase(), "Module Info", 2);
                }
                binding = false;
            }
    ...
    public void keyPress(int key) {
        ...
        if (binding) {
            ...
            if (object instanceof ModuleObject m) {
                m.function.bind = key;
                Managment.NOTIFICATION_MANAGER.add("Module " + TextFormatting.GRAY + m.function.name + TextFormatting.WHITE + " was keyed " + ClientUtil.getKey(key).toUpperCase(), "Module Info", 2);

            } else if (object instanceof BindObject m) {
                m.set.setKey(key);
                Managment.NOTIFICATION_MANAGER.add("Module " + TextFormatting.GRAY + m.object.function.name + TextFormatting.WHITE + " was binned on the button " + ClientUtil.getKey(key).toUpperCase(), "Module Info", 2);
