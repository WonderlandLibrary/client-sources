...
import wtf.shiyeno.ui.beta.component.impl.ColorComponent;
import wtf.shiyeno.ui.beta.component.impl.Component;
import wtf.shiyeno.ui.beta.component.impl.ConfigComponent;
import wtf.shiyeno.ui.beta.component.impl.ModuleComponent;
import wtf.shiyeno.ui.beta.component.impl.ThemeComponent;
import wtf.shiyeno.ui.midnight.Style;
import wtf.shiyeno.util.IMinecraft;
import wtf.shiyeno.util.font.Fonts;
import wtf.shiyeno.util.math.KeyMappings;
...
import static wtf.shiyeno.util.render.RenderUtil.isInRegion;

public class ClickGui extends Screen {
    public static float scrollingOut;
    public static float scrolling;
    public ClickGui(StringTextComponent a) {
    ...
    void renderBackground(MatrixStack matrixStack, float x, float y, float width, float height, float leftPanel, int mouseX, int mouseY) {
        RenderUtil.Render2D.drawShadow(x, y, width, height, 20, ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(90), ColorUtil.getColorStyle(180), ColorUtil.getColorStyle(270));
        RenderUtil.Render2D.drawGradientRound(x, y, width, height, 6, ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(90), ColorUtil.getColorStyle(180), ColorUtil.getColorStyle(270));
        ...
        Fonts.woveline[19].drawCenteredString(matrixStack, "SHIYENO", x + leftPanel / 2f, y + 14, -1);
    }
    ...
    void renderCategories(MatrixStack matrixStack, float x, float y, float width, float height, float leftPanel, int mouseX, int mouseY) {
        ...
        for (Type t : Type.values()) {
            ...
            if (t == current)
                RenderUtil.Render2D.drawRoundedCorner(x + (leftPanel / 2f) * animation, y + 32.5f + t.ordinal() * heightCategory, leftPanel * (1 - animation), heightCategory, 20 * animation, new Color(ColorUtil.getColorStyle(90)).getRGB(), RenderUtil.Render2D.Corner.ALL);

            boolean hovered = RenderUtil.isInRegion(mouseX, mouseY, x, y + 32.5f + t.ordinal() * heightCategory, leftPanel, heightCategory);
            ...
            Fonts.gilroy[14].drawString(matrixStack, t.name(), x + 20 + t.anim, y + 41 + t.ordinal() * heightCategory, t == current ? new Color(0, 0, 0).getRGB() : new Color(255, 255, 255).getRGB());
        }
    ...
    void drawComponents(MatrixStack stack, int mouseX, int mouseY) {
        ...
        for (ModuleComponent component : first) {
            ...
            if (!component.components.isEmpty()) {
                for (wtf.shiyeno.ui.beta.component.impl.Component settingComp : component.components) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (ModuleComponent.binding != null) {
            if (keyCode == GLFW.GLFW_KEY_DELETE) {
                ...
                Managment.NOTIFICATION_MANAGER.add("Module " + TextFormatting.GRAY +  ModuleComponent.binding.function.name + TextFormatting.WHITE + " was keyed " + KeyMappings.reverseKeyMap.get(keyCode), "Module Info",2);

                ModuleComponent.binding.function.bind = keyCode;
            }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (Type t : Type.values()) {
            if (RenderUtil.isInRegion(mouseX, mouseY, x, y + 32.5f + t.ordinal() * heightCategory, leftPanel, heightCategory)) {
        ...
        if (RenderUtil.isInRegion(mouseX, mouseY, x + width - 45 - 2, y + 64 / 2F + 15, 35 - 2, 32 / 2f)) {
        ...
        if (RenderUtil.isInRegion(mouseX, mouseY, x + width - 45 - 35 - 2, y + 64 / 2F + 15, 35 - 2, 32 / 2f)) {
            cfg.clear();
            for (String config : Managment.CONFIG_MANAGER.getAllConfigurations()) {
                cfg.add(new ConfigComponent(Managment.CONFIG_MANAGER.findConfig(config)));
            }
        ...
        if (RenderUtil.isInRegion(mouseX, mouseY, x, y + 64 / 2f, width, height - 64 / 2f)) {
            for (ModuleComponent m : objects) {
                if (searchText.isEmpty()) {
                    if (m.function.category != current) continue;
                } else {
