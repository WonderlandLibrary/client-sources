...
public class ItemRenderer extends EntityRenderer<ItemEntity> {
    ...
    public void render(ItemEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        boolean real = Managment.FUNCTION_MANAGER.itemPhysic.state;
        float f2 = ibakedmodel.getItemCameraTransforms().getTransform(TransformType.GROUND).scale.getY();
        if (!real) {
            matrixStackIn.translate(0.0, (double)(f1 + 0.25F * f2), 0.0);
        }
        ...
        if (real) {
            matrixStackIn.rotate(Vector3f.XP.rotationDegrees(entityIn.isOnGround() ? 90.0F : f3 * 300.0F));
        }
        ...
        if (!flag) {
            ...
            f11 = -0.0F * (float)(j - 1) * 0.5F * f5;
            f13 = -0.09375F * (float)(j - 1) * 0.5F * f6;
            matrixStackIn.translate((double)f7, (double)f11, (double)f13);
        }
        ...
        for(int k = 0; k < j; ++k) {
            ...
            if (k > 0) {
                if (flag) {
                    ...
                    matrixStackIn.translate((double)f11, (double)f13, 0.0);
                }
            }

            this.itemRenderer.renderItem(itemstack, TransformType.GROUND, false, matrixStackIn, bufferIn, packedLightIn, OverlayTexture.NO_OVERLAY, ibakedmodel);
            matrixStackIn.pop();
            if (!flag) {
