...
import dev.tenacity.viamcp.impl.MCPVLBPipeline;
import dev.tenacity.viamcp.impl.ViaMCP;
import dev.tenacity.viamcp.loadingbase.ViaLoadingBase;
import dev.tenacity.viamcp.loadingbase.netty.event.CompressionReorderEvent;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception
    {
        if (this.channel.isOpen())
        {
            try
            {
                ...
                Tenacity.getInstance().getEventBus().dispatch(packetReceiveEvent);
    ...
    public void sendPacket(Packet packetIn)
    {
        if (this.isChannelOpen())
        {
            ...
            Tenacity.getInstance().getEventBus().dispatch(packetSendEvent);

            if(!packetSendEvent.isCanceled()) {
                this.flushOutboundQueue();
                this.dispatchPacket(packetSendEvent.getPacket(), (GenericFutureListener <? extends Future <? super Void >> [])null);
            }
        }
        else
        {
