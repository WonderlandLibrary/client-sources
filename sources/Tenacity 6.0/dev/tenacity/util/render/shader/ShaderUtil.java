package dev.tenacity.util.render.shader;

import dev.tenacity.util.io.FileUtil;
...
public final class ShaderUtil {
    ...
    public ShaderUtil(final String fragmentShaderSRC, final boolean notUsed) {
        final int program = glCreateProgram();
        final int fragmentShaderID = createShader(new ByteArrayInputStream(fragmentShaderSRC.getBytes()), GL_FRAGMENT_SHADER);
        int vertexShaderID;
        try {
            vertexShaderID = createShader(MC.getResourceManager().getResource(new ResourceLocation("Tenacity/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (final IOException e) {
    ...
    public ShaderUtil(final String fragmentShaderLoc) {
        this(fragmentShaderLoc, "Tenacity/vertex.vsh");
    }
