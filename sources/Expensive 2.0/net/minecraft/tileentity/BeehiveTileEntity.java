...
public class BeehiveTileEntity extends TileEntity implements ITickableTileEntity
{
    ...
    public void markDirty()
    {
        if (this.isNearFire())
        {
            this.angerBees((PlayerEntity)null, this.world.getBlockState(this.getPos()), State.EMERGENCY);
        }
    ...
    private void tickBees()
    {
        ...
        for (BlockState blockstate = this.getBlockState(); iterator.hasNext(); beehivetileentity$bee.ticksInHive++)
        {
            ...
            {
                State beehivetileentity$state = beehivetileentity$bee.entityData.getBoolean("HasNectar") ? State.HONEY_DELIVERED : State.BEE_RELEASED;
