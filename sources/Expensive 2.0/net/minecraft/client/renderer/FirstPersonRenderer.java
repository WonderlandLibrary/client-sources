...
import wtf.expensive.modules.impl.render.CustomHand;
import wtf.expensive.modules.impl.render.SwingAnimationFunction;

import static wtf.expensive.modules.impl.render.CustomHand.handBuffer;
...
public class FirstPersonRenderer {
    ...
    private void transformSideFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float equippedProg) {
        ...
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        matrixStackIn.translate((float) i * 0.56F, -0.52F + equippedProg * (animationFunction.state
                && animationFunction.swordAnim.is("Swipe") ? -0.2f : animationFunction.state ? 0.0F : -0.6F), -0.72F);
    }
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (animationFunction.state) {
                    if (flag3) {
                        matrixStackIn.translate(animationFunction.right_x.getValue().floatValue(),
                                animationFunction.right_y.getValue().floatValue(), animationFunction.right_z.getValue().floatValue());
                    } else {
                        matrixStackIn.translate(animationFunction.left_x.getValue().floatValue(),
                                animationFunction.left_y.getValue().floatValue(), animationFunction.left_z.getValue().floatValue());
                    }
                }
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    int k = flag3 ? 1 : -1;
