...
public class Shaders {
	...
	public static ItemStack itemToRenderMain;
	public static ItemStack itemToRenderOff;
	...
	private static final String[] programNames = new String[] { "", "gbuffers_basic", "gbuffers_textured", "gbuffers_textured_lit", "gbuffers_skybasic", "gbuffers_skytextured", "gbuffers_clouds",
			"gbuffers_terrain", "gbuffers_terrain_solid", "gbuffers_terrain_cutout_mip", "gbuffers_terrain_cutout", "gbuffers_damagedblock", "gbuffers_water", "gbuffers_block", "gbuffers_beaconbeam",
			"gbuffers_item", "gbuffers_entities", "gbuffers_armor_glint", "gbuffers_spidereyes", "gbuffers_hand", "gbuffers_weather", "composite", "composite1", "composite2", "composite3",
			"composite4", "composite5", "composite6", "composite7", "final", "shadow", "shadow_solid", "shadow_cutout" };
	...
	private static final String[] formatNames = new String[] { "R8", "RG8", "RGB8", "RGBA8", "R8_SNORM", "RG8_SNORM", "RGB8_SNORM", "RGBA8_SNORM", "R16", "RG16", "RGB16", "RGBA16", "R16_SNORM",
			"RG16_SNORM", "RGB16_SNORM", "RGBA16_SNORM", "R32F", "RG32F", "RGB32F", "RGBA32F", "R32I", "RG32I", "RGB32I", "RGBA32I", "R32UI", "RG32UI", "RGB32UI", "RGBA32UI", "R3_G3_B2", "RGB5_A1",
			"RGB10_A2", "R11F_G11F_B10F" };
	private static final int[] formatIds = new int[] { 33321, 33323, 32849, 32856, 36756, 36757, 36758, 36759, 33322, 33324, 32852, 32859, 36760, 36761, 36762, 36763, 33326, 33328, 34837, 34836,
			33333, 33339, 36227, 36226, 33334, 33340, 36209, 36208, 10768, 32855, 32857, 35898 };
	...
	private static ShaderOption[] loadShaderPackOptions() {
		try {
			...
			for (ShaderOption element : ashaderoption) {
				ShaderOption shaderoption = element;
	...
	public static void startup(Minecraft mc) {
		...
		SMCLog.info("Capabilities: " + (capabilities.OpenGL20 ? " 2.0 " : " - ") + (capabilities.OpenGL21 ? " 2.1 " : " - ") + (capabilities.OpenGL30 ? " 3.0 " : " - ")
	...
	public static void useProgram(int program) {
		...
		if (activeProgram != program) {
			...
			if (programsID[program] == 0) {
				...
				ItemStack itemstack = mc.thePlayer != null ? mc.thePlayer.getHeldItemMainhand() : null;
				...
				ItemStack itemstack1 = mc.thePlayer != null ? mc.thePlayer.getHeldItemOffhand() : null;
	...
	private static int getBufferIndexFromString(String name) {
		return !name.equals("colortex0")
				&& !name.equals(
						"gcolor")
								? (!name.equals("colortex1")
										&& !name.equals("gdepth")
												? (!name.equals("colortex2") && !name.equals("gnormal")
														? (!name.equals("colortex3") && !name.equals("composite") ? (!name.equals("colortex4") && !name.equals("gaux1")
																? (!name.equals("colortex5") && !name.equals("gaux2")
																		? (!name.equals("colortex6") && !name.equals("gaux3") ? (!name.equals("colortex7") && !name.equals("gaux4") ? -1 : 7) : 6) : 5)
																: 4) : 3)
														: 2)
												: 1)
								: 0;
	...
	private static void checkWorldChanged(World world) {
		if (currentWorld != world) {
			...
			if ((prevWorld != null) && (world != null)) {
				int i = prevWorld.provider.getDimensionType().getId();
	...
	public static void drawHorizon() {
		...
		vertexbuffer.pos(d2, d5, d3).endVertex();
		vertexbuffer.pos(d2, d4, d3).endVertex();
		vertexbuffer.pos(d3, d4, d2).endVertex();
		vertexbuffer.pos(d3, d5, d2).endVertex();
		vertexbuffer.pos(d3, d5, d2).endVertex();
		vertexbuffer.pos(d3, d4, d2).endVertex();
		vertexbuffer.pos(d3, d4, d1).endVertex();
		vertexbuffer.pos(d3, d5, d1).endVertex();
		vertexbuffer.pos(d3, d5, d1).endVertex();
		vertexbuffer.pos(d3, d4, d1).endVertex();
		vertexbuffer.pos(d2, d4, d1).endVertex();
		vertexbuffer.pos(d2, d5, d1).endVertex();
		vertexbuffer.pos(d2, d5, d1).endVertex();
		vertexbuffer.pos(d2, d4, d1).endVertex();
		vertexbuffer.pos(d1, d4, d0).endVertex();
		vertexbuffer.pos(d1, d5, d0).endVertex();
		vertexbuffer.pos(d1, d5, d0).endVertex();
		vertexbuffer.pos(d1, d4, d0).endVertex();
		vertexbuffer.pos(d0, d4, d1).endVertex();
		vertexbuffer.pos(d0, d5, d1).endVertex();
		vertexbuffer.pos(d0, d5, d1).endVertex();
		vertexbuffer.pos(d0, d4, d1).endVertex();
		vertexbuffer.pos(d0, d4, d2).endVertex();
		vertexbuffer.pos(d0, d5, d2).endVertex();
		vertexbuffer.pos(d0, d5, d2).endVertex();
		vertexbuffer.pos(d0, d4, d2).endVertex();
		vertexbuffer.pos(d1, d4, d3).endVertex();
		vertexbuffer.pos(d1, d5, d3).endVertex();
		vertexbuffer.pos(d1, d5, d3).endVertex();
		vertexbuffer.pos(d1, d4, d3).endVertex();
		vertexbuffer.pos(d2, d4, d3).endVertex();
		vertexbuffer.pos(d2, d5, d3).endVertex();
	...
	public static void setFog(GlStateManager.FogMode fogMode) {
		...
		Shaders.fogMode = fogMode.capabilityId;
