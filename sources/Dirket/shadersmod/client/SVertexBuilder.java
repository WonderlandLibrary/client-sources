...
public class SVertexBuilder {
	...
	public static void initVertexBuilder(VertexBuffer wrr) {
	...
	public static void pushEntity(IBlockState blockState, BlockPos blockPos, IBlockAccess blockAccess, VertexBuffer wrr) {
		...
		int j = block.getRenderType(block.getDefaultState()).ordinal();
		int k = block.getMetaFromState(blockState);
		int l = ((j & 65535) << 16) + (i & 65535);
		int i1 = k & 65535;
		wrr.sVertexBuilder.pushEntity(((long) i1 << 32) + l);
	}

	public static void popEntity(VertexBuffer wrr) {
		wrr.sVertexBuilder.popEntity();
	}

	public static boolean popEntity(boolean value, VertexBuffer wrr) {
		wrr.sVertexBuilder.popEntity();
		return value;
	}
	...
	public static void beginAddVertex(VertexBuffer wrr) {
	...
	public static void beginAddVertexData(VertexBuffer wrr, int[] data) {
		if (wrr.vertexCount == 0) {
			endSetVertexFormat(wrr);
		}
	...
	public static void endAddVertexData(VertexBuffer wrr) {
		SVertexBuilder svertexbuilder = wrr.sVertexBuilder;
	...
	public void calcNormal(VertexBuffer wrr, int baseIndex) {
