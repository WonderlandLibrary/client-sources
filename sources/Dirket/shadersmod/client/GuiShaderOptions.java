...
public class GuiShaderOptions extends GuiScreen {
	...
	private String[] makeTooltipLines(ShaderOption so, int width) {
		if (so instanceof ShaderOptionProfile) {
			...
			if (so.getPaths() != null) {
				s3 = Lang.get("of.general.from") + ": " + Config.arrayToString(so.getPaths());
			}
	...
	private GuiButton getSelectedButton(int x, int y) {
		for (int i = 0; i < this.buttonList.size(); ++i) {
			...
			if ((x >= guibutton.xPosition) && (y >= guibutton.yPosition) && (x < (guibutton.xPosition + j)) && (y < (guibutton.yPosition + k))) { return guibutton; }
		}
