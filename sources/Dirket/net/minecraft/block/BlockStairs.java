...
public class BlockStairs extends Block {
	...
	/**
	 * Returns a bounding box representing a quarter of a block (two eight-size cubes back to back). Used in all stair shapes except OUTER.
	 */
	...
	/**
	 * Returns a bounding box representing an eighth of a block (a block whose three dimensions are halved). Used in all stair shapes except STRAIGHT (gets added alone in the case of OUTER; alone with a quarter block in case of INSIDE).
	 */
	...
	public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		...
		return (facing != EnumFacing.DOWN) && ((facing == EnumFacing.UP) || (hitY <= 0.5D)) ? iblockstate.withProperty(HALF, BlockStairs.EnumHalf.BOTTOM)
