...
public class JsonToNBT {
		...
		for (int j = 0; j < str.length(); ++j) {
			...
			if (c0 == 34) {
				if (isCharEscaped(str, j)) {
					if (!flag) { throw new NBTException("Illegal use of \\\": " + str); }
				} else {
					flag = !flag;
				}
			} else if (!flag) {
				if ((c0 != 123) && (c0 != 91)) {
					if ((c0 == 125) && (stack.isEmpty() || (stack.pop().charValue() != 123))) { throw new NBTException("Unbalanced curly brackets {}: " + str); }

					if ((c0 == 93) && (stack.isEmpty() || (stack.pop().charValue() != 91))) { throw new NBTException("Unbalanced square brackets []: " + str); }
				} else {
					if (stack.isEmpty()) {
						++i;
					}
	...
	static JsonToNBT.Any nameValueToNBT(String key, String value) throws NBTException {
		...
		if (value.startsWith("{")) {
			...
			for (jsontonbt$compound = new JsonToNBT.Compound(key); value.length() > 0; value = value.substring(s1.length() + 1)) {
				...
				if ((c1 != 44) && (c1 != 123) && (c1 != 125) && (c1 != 91) && (c1 != 93)) { throw new NBTException("Unexpected token \'" + c1 + "\' at: " + value.substring(s1.length())); }
			}
			...
			for (jsontonbt$list = new JsonToNBT.List(key); value.length() > 0; value = value.substring(s.length() + 1)) {
				...
				if ((c0 != 44) && (c0 != 123) && (c0 != 125) && (c0 != 91) && (c0 != 93)) { throw new NBTException("Unexpected token \'" + c0 + "\' at: " + value.substring(s.length())); }
			}
	...
	private static String nextNameValuePair(String str, boolean isCompound) throws NBTException {
		...
		if (isCompound) {
			if (i == -1) { throw new NBTException("Unable to locate name/value separator for string: " + str); }

			if ((j != -1) && (j < i)) { throw new NBTException("Name error at: " + str); }
		} else if ((i == -1) || (i > j)) {
			i = -1;
		}
		...
		for (int j = 0; i < str.length(); ++i) {
			...
			if (c0 == 34) {
				...
				if ((c0 != 123) && (c0 != 91)) {
					...
					if ((c0 == 44) && stack.isEmpty()) { return str.substring(0, i); }
				} else {
					stack.push(Character.valueOf(c0));
				}
			}
			...
			if (!Character.isWhitespace(c0)) {
				if (!flag && flag1 && (j != i)) { return str.substring(0, j + 1); }
	...
	static class Primitive extends JsonToNBT.Any {
		...
		public NBTBase parse() throws NBTException {
			try {
				if (DOUBLE.matcher(this.jsonValue).matches()) { return new NBTTagDouble(Double.parseDouble(this.jsonValue.substring(0, this.jsonValue.length() - 1))); }

				if (FLOAT.matcher(this.jsonValue).matches()) { return new NBTTagFloat(Float.parseFloat(this.jsonValue.substring(0, this.jsonValue.length() - 1))); }

				if (BYTE.matcher(this.jsonValue).matches()) { return new NBTTagByte(Byte.parseByte(this.jsonValue.substring(0, this.jsonValue.length() - 1))); }

				if (LONG.matcher(this.jsonValue).matches()) { return new NBTTagLong(Long.parseLong(this.jsonValue.substring(0, this.jsonValue.length() - 1))); }

				if (SHORT.matcher(this.jsonValue).matches()) { return new NBTTagShort(Short.parseShort(this.jsonValue.substring(0, this.jsonValue.length() - 1))); }

				if (INTEGER.matcher(this.jsonValue).matches()) { return new NBTTagInt(Integer.parseInt(this.jsonValue)); }

				if (DOUBLE_UNTYPED.matcher(this.jsonValue).matches()) { return new NBTTagDouble(Double.parseDouble(this.jsonValue)); }

				if ("true".equalsIgnoreCase(this.jsonValue) || "false".equalsIgnoreCase(this.jsonValue)) { return new NBTTagByte((byte) (Boolean.parseBoolean(this.jsonValue) ? 1 : 0)); }
