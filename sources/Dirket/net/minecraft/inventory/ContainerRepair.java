...
public class ContainerRepair extends Container {
	...
	public boolean canInteractWith(EntityPlayer playerIn) {
		return this.theWorld.getBlockState(this.selfPosition).getBlock() != Blocks.ANVIL ? false
				: playerIn.getDistanceSq(this.selfPosition.getX() + 0.5D, this.selfPosition.getY() + 0.5D, this.selfPosition.getZ() + 0.5D) <= 64.0D;
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if ((slot != null) && slot.getHasStack()) {
			...
			if (index == 2) {
				...
				if ((index >= 3) && (index < 39) && !this.mergeItemStack(itemstack1, 0, 2, false)) { return null; }
