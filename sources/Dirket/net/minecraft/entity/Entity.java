...
public abstract class Entity implements ICommandSender {
		...
		if (this.noClip) {
			...
			if (this.canTriggerWalking() && !flag && !this.isRiding()) {
				...
				if ((this.distanceWalkedOnStepModified > this.nextStepDistance) && (iblockstate.getMaterial() != Material.AIR)) {
					...
					if (this.isInWater()) {
						float f = MathHelper.sqrt_double((this.motionX * this.motionX * 0.20000000298023224D) + (this.motionY * this.motionY) + (this.motionZ * this.motionZ * 0.20000000298023224D))
								* 0.35F;
	...
	public boolean func_189652_ae() {
		return this.dataManager.get(field_189655_aD).booleanValue();
	}
	...
	public boolean isWet() {
		if (this.inWater) {
			...
			if (!this.worldObj.isRainingAt(blockpos$pooledmutableblockpos) && !this.worldObj.isRainingAt(blockpos$pooledmutableblockpos.func_189532_c(this.posX, this.posY + this.height, this.posZ))) {
				blockpos$pooledmutableblockpos.release();
				return false;
			} else {
				blockpos$pooledmutableblockpos.release();
	...
	protected void createRunningParticles() {
		...
		if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
			this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((this.rand.nextFloat() - 0.5D) * this.width), this.getEntityBoundingBox().minY + 0.1D,
					this.posZ + ((this.rand.nextFloat() - 0.5D) * this.width), -this.motionX * 4.0D, 1.5D, -this.motionZ * 4.0D, new int[] { Block.getStateId(iblockstate) });
		}
	}
...
//        TODO: Direkt: NoRender
        final ESP esp = Direkt.getInstance().getModuleManager().getModule(ESP.class);
        boolean isEntityESP = esp.isRunning() && esp.doesQualify(this);
        if (Direkt.getInstance().getModuleManager().getModule(NoRender.class).isRunning()) {
            if (!Wrapper.getPlayer().canEntityBeSeen(this) && !isEntityESP && Direkt.getInstance().getModuleManager().getModule(NoRender.class).shouldDoSmartVisGraphing() && !(this instanceof EntityPlayer))
            	return false;
            return Direkt.getInstance().getModuleManager().getModule(NoRender.class).shouldDoSmartRendering() && this.isInRangeToRenderDist(d3 * 10);
        }
	...
	/**
	 * Attempts to write this Entity to the given NBTTagCompound. Returns false if the entity is dead or its string representation is null. In this event, the given NBTTagCompound is not modified. Similar to writeToNBTOptional, but does not check whether this Entity is a passenger of another.
	 */
	...
	public EntityItem entityDropItem(ItemStack stack, float offsetY) {
		if ((stack.stackSize != 0) && (stack.getItem() != null)) {
			EntityItem entityitem = new EntityItem(this.worldObj, this.posX, this.posY + offsetY, this.posZ, stack);
	...
	public boolean isEntityInsideOpaqueBlock() {
		if (this.noClip) {
			...
			for (int i = 0; i < 8; ++i) {
				int j = MathHelper.floor_double(this.posY + ((i >> 0) % 2 - 0.5F) * 0.1F + this.getEyeHeight());
	...
	public void setPortal(BlockPos pos) {
		if (this.timeUntilPortal > 0) {
			...
			if (!this.worldObj.isRemote && !pos.equals(this.lastPortalPos)) {
				...
				d1 = Math.abs(MathHelper.pct(d1 - (blockpattern$patternhelper.getForwards().rotateY().getAxisDirection() == EnumFacing.AxisDirection.NEGATIVE ? 1 : 0), d0,
						d0 - blockpattern$patternhelper.getWidth()));
				double d2 = MathHelper.pct(this.posY - 1.0D, blockpattern$patternhelper.getFrontTopLeft().getY(),
	...
	public String toString() {
		return String.format("%s[\'%s\'/%d, l=\'%s\', x=%.2f, y=%.2f, z=%.2f]", new Object[] { this.getClass().getSimpleName(), this.getName(), Integer.valueOf(this.entityId),
	...
	public void addEntityCrashInfo(CrashReportCategory category) {
		...
		category.addCrashSection("Entity\'s Block location",
				CrashReportCategory.getCoordinateInfo(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)));
	...
	public boolean isRidingOrBeingRiddenBy(Entity entityIn) {
		for (Entity entity : this.getPassengers()) {
			...
			if (entity.isRidingOrBeingRiddenBy(entityIn)) { return true; }
		}
