...
public class EntityRabbit extends EntityAnimal {
	...
	protected float getJumpUpwardsMotion() {
		if (!this.isCollidedHorizontally && (!this.moveHelper.isUpdating() || (this.moveHelper.getY() <= (this.posY + 0.5D)))) {
			...
			if ((path != null) && (path.getCurrentPathIndex() < path.getCurrentPathLength())) {
				...
				if (vec3d.yCoord > this.posY) { return 0.5F; }
			}
	...
	protected void createEatingParticles() {
		...
		this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_DUST, (this.posX + this.rand.nextFloat() * this.width * 2.0F) - this.width, this.posY + 0.5D + this.rand.nextFloat() * this.height,
				(this.posZ + this.rand.nextFloat() * this.width * 2.0F) - this.width, 0.0D, 0.0D, 0.0D, new int[] { Block.getStateId(iblockstate) });
		this.carrotTicks = 40;
	}
	...
	static class AIRaidFarm extends EntityAIMoveToBlock {
		...
		public boolean shouldExecute() {
			if (this.runDelay <= 0) {
				if (!this.rabbit.worldObj.getGameRules().getBoolean("mobGriefing")) { return false; }
		...
		public void updateTask() {
			super.updateTask();
			this.rabbit.getLookHelper().setLookPosition(this.destinationBlock.getX() + 0.5D, this.destinationBlock.getY() + 1, this.destinationBlock.getZ() + 0.5D, 10.0F,
