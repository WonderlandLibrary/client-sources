...
public class EntityDragon extends EntityLiving implements IEntityMultiPart, IMob {
	...
	public EntityDragon(World worldIn) {
		super(worldIn);
		this.dragonPartArray = new EntityDragonPart[] { this.dragonPartHead, this.dragonPartNeck, this.dragonPartBody, this.dragonPartTail1, this.dragonPartTail2, this.dragonPartTail3,
	...
	public void onLivingUpdate() {
		...
		if (this.getHealth() <= 0.0F) {
			...
			this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + f13, this.posY + 2.0D + f15, this.posZ + f17, 0.0D, 0.0D, 0.0D, new int[0]);
		} else {
			...
			if (this.isAIDisabled()) {
				...
				if (this.worldObj.isRemote) {
					...
					if (vec3d != null) {
						...
						d7 = MathHelper.clamp_double(d7 / MathHelper.sqrt_double((d6 * d6) + (d8 * d8)), (-f6), f6);
						this.motionY += d7 * 0.10000000149011612D;
						this.rotationYaw = MathHelper.wrapDegrees(this.rotationYaw);
						double d4 = MathHelper.clamp_double(MathHelper.wrapDegrees(180.0D - (MathHelper.atan2(d6, d8) * (180D / Math.PI)) - this.rotationYaw), -50.0D, 50.0D);
				...
				this.dragonPartBody.setLocationAndAngles(this.posX + f19 * 0.5F, this.posY, this.posZ - f3 * 0.5F, 0.0F, 0.0F);
				this.dragonPartWing1.onUpdate();
				this.dragonPartWing1.setLocationAndAngles(this.posX + f3 * 4.5F, this.posY + 2.0D, this.posZ + f19 * 4.5F, 0.0F, 0.0F);
				this.dragonPartWing2.onUpdate();
				this.dragonPartWing2.setLocationAndAngles(this.posX - f3 * 4.5F, this.posY + 2.0D, this.posZ - f19 * 4.5F, 0.0F, 0.0F);
				...
				this.dragonPartHead.setLocationAndAngles(this.posX + f4 * 6.5F * f16, this.posY + f5 + f18 * 6.5F, this.posZ - f20 * 6.5F * f16, 0.0F, 0.0F);
				this.dragonPartNeck.setLocationAndAngles(this.posX + f4 * 5.5F * f16, this.posY + f5 + f18 * 5.5F, this.posZ - f20 * 5.5F * f16, 0.0F, 0.0F);
				...
				for (int j = 0; j < 3; ++j) {
					...
					entitydragonpart.setLocationAndAngles(this.posX - ((f19 * 1.5F) + (f22 * f24)) * f16, ((this.posY + (adouble1[1] - adouble[1])) - (f24 + 1.5F) * f18) + 1.5D,
							this.posZ + ((f3 * 1.5F) + (f7 * f24)) * f16, 0.0F, 0.0F);
				}

				if (!this.worldObj.isRemote) {
					this.slowed = this.destroyBlocksInAABB(this.dragonPartHead.getEntityBoundingBox()) | this.destroyBlocksInAABB(this.dragonPartNeck.getEntityBoundingBox())
				...
				for (int i2 = k; i2 <= j1; ++i2) {
					...
					if ((iblockstate.getMaterial() != Material.AIR) && (iblockstate.getMaterial() != Material.FIRE)) {
						if (!this.worldObj.getGameRules().getBoolean("mobGriefing")) {
							flag = true;
						} else if ((block != Blocks.BARRIER) && (block != Blocks.OBSIDIAN) && (block != Blocks.END_STONE) && (block != Blocks.BEDROCK) && (block != Blocks.END_PORTAL)
								&& (block != Blocks.END_PORTAL_FRAME)) {
							if ((block != Blocks.COMMAND_BLOCK) && (block != Blocks.REPEATING_COMMAND_BLOCK) && (block != Blocks.CHAIN_COMMAND_BLOCK) && (block != Blocks.IRON_BARS)
	...
	protected void onDeathUpdate() {
		...
		if (!this.worldObj.isRemote) {
			if ((this.deathTicks > 150) && ((this.deathTicks % 5) == 0) && flag) {
				this.dropExperience(MathHelper.floor_float(i * 0.08F));
			}
		...
		if ((this.deathTicks == 200) && !this.worldObj.isRemote) {
			if (flag) {
				this.dropExperience(MathHelper.floor_float(i * 0.2F));
			}
