...
public class EntityWither extends EntityMob implements IRangedAttackMob {
		...
		for (int l = 0; l < 3; ++l) {
			...
			if (flag && (this.worldObj.rand.nextInt(4) == 0)) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, d10 + (this.rand.nextGaussian() * 0.30000001192092896D), d2 + (this.rand.nextGaussian() * 0.30000001192092896D),
						d4 + (this.rand.nextGaussian() * 0.30000001192092896D), 0.699999988079071D, 0.699999988079071D, 0.5D, new int[0]);
			}
		}

		if (this.getInvulTime() > 0) {
			for (int i1 = 0; i1 < 3; ++i1) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + this.rand.nextGaussian(), this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian(),
			...
			for (int i = 1; i < 3; ++i) {
				if (this.ticksExisted >= this.nextHeadUpdate[i - 1]) {
					...
					if (k1 > 0) {
						...
						List<EntityLivingBase> list = this.worldObj.<EntityLivingBase> getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D),
	...
	public static boolean canDestroyBlock(Block blockIn) {
		return (blockIn != Blocks.BEDROCK) && (blockIn != Blocks.END_PORTAL) && (blockIn != Blocks.END_PORTAL_FRAME) && (blockIn != Blocks.COMMAND_BLOCK) && (blockIn != Blocks.REPEATING_COMMAND_BLOCK)
