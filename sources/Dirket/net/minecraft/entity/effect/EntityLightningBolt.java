...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World worldIn, double x, double y, double z, boolean effectOnlyIn) {
		...
		if (!effectOnlyIn && !worldIn.isRemote && worldIn.getGameRules().getBoolean("doFireTick")
				&& ((worldIn.getDifficulty() == EnumDifficulty.NORMAL) || (worldIn.getDifficulty() == EnumDifficulty.HARD)) && worldIn.isAreaLoaded(blockpos, 10)) {
		...
		if (this.lightningState < 0) {
			if (this.boltLivingTime == 0) {
				...
				if (!this.effectOnly && !this.worldObj.isRemote) {
					...
					if (this.worldObj.getGameRules().getBoolean("doFireTick") && this.worldObj.isAreaLoaded(blockpos, 10) && (this.worldObj.getBlockState(blockpos).getMaterial() == Material.AIR)
		...
		if (this.lightningState >= 0) {
			if (this.worldObj.isRemote) {
				...
				List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
						new AxisAlignedBB(this.posX - 3.0D, this.posY - 3.0D, this.posZ - 3.0D, this.posX + 3.0D, this.posY + 6.0D + 3.0D, this.posZ + 3.0D));
