...
public abstract class EntityAgeable extends EntityCreature {
	...
	public int getGrowingAge() {
		return this.worldObj.isRemote ? (this.dataManager.get(BABY).booleanValue() ? -1 : 1) : this.growingAge;
	}
		...
		if (this.worldObj.isRemote) {
			if (this.forcedAgeTimer > 0) {
				if ((this.forcedAgeTimer % 4) == 0) {
					this.worldObj.spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, (this.posX + this.rand.nextFloat() * this.width * 2.0F) - this.width,
							this.posY + 0.5D + this.rand.nextFloat() * this.height, (this.posZ + this.rand.nextFloat() * this.width * 2.0F) - this.width, 0.0D, 0.0D, 0.0D, new int[0]);
				}
