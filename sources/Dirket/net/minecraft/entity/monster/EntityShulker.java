...
public class EntityShulker extends EntityGolem implements IMob {
	...
	public void onUpdate() {
		...
		if (blockpos != null) {
			...
			double d3 = 0.5D - (MathHelper.sin((0.5F + this.currentPeekAmount) * (float) Math.PI) * 0.5D);
			double d4 = 0.5D - (MathHelper.sin((0.5F + this.currentPeekAmount0) * (float) Math.PI) * 0.5D);
			double d5 = d3 - d4;
			double d0 = 0.0D;
			double d1 = 0.0D;
			double d2 = 0.0D;
	...
	protected boolean tryTeleportToNewPosition() {
		if (!this.isAIDisabled() && this.isEntityAlive()) {
			...
			for (int i = 0; i < 5; ++i) {
				...
				if ((blockpos1.getY() > 0) && this.worldObj.isAirBlock(blockpos1) && this.worldObj.isInsideBorder(this.worldObj.getWorldBorder(), this)
			...
			if (EntityShulker.this.worldObj.getDifficulty() != EnumDifficulty.PEACEFUL) {
				...
				if (d0 < 400.0D) {
					if (this.attackTime <= 0) {
						...
						EntityShulkerBullet entityshulkerbullet = new EntityShulkerBullet(EntityShulker.this.worldObj, EntityShulker.this, entitylivingbase,
								EntityShulker.this.getAttachmentFacing().getAxis());
	...
	class AIAttackNearest extends EntityAINearestAttackableTarget<EntityPlayer> {
		...
		protected AxisAlignedBB getTargetableArea(double targetDistance) {
			...
			return enumfacing.getAxis() == EnumFacing.Axis.X ? this.taskOwner.getEntityBoundingBox().expand(4.0D, targetDistance, targetDistance)
					: (enumfacing.getAxis() == EnumFacing.Axis.Z ? this.taskOwner.getEntityBoundingBox().expand(targetDistance, targetDistance, 4.0D)
