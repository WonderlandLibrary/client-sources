...
public class GuiWinGame extends GuiScreen {
		...
		if (this.lines == null) {
			...
			try {
				...
				while ((s1 = bufferedreader.readLine()) != null) {
					...
					for (s1 = s1.replaceAll("PLAYERNAME", this.mc.getSession().getUsername()); s1
							.contains(s); s1 = s2 + TextFormatting.WHITE + TextFormatting.OBFUSCATED + "XXXXXXXX".substring(0, random.nextInt(4) + 3) + s3) {
						int j = s1.indexOf(s);
						s2 = s1.substring(0, j);
	...
	private void drawWinGameScreen(int p_146575_1_, int p_146575_2_, float p_146575_3_) {
		...
		int i = this.width;
		float f = 0.0F - ((this.time + p_146575_3_) * 0.5F * 0.5F);
		float f1 = this.height - ((this.time + p_146575_3_) * 0.5F * 0.5F);
		float f2 = 0.015625F;
		float f3 = ((this.time + p_146575_3_) - 0.0F) * 0.02F;
		float f4 = (this.totalScrollLength + this.height + this.height + 24) / 0.5F;
		float f5 = (f4 - 20.0F - (this.time + p_146575_3_)) * 0.005F;
		...
		f3 = f3 * f3;
		f3 = (f3 * 96.0F) / 255.0F;
		vertexbuffer.pos(0.0D, this.height, this.zLevel).tex(0.0D, f * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
		vertexbuffer.pos(i, this.height, this.zLevel).tex(i * 0.015625F, f * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
		vertexbuffer.pos(i, 0.0D, this.zLevel).tex(i * 0.015625F, f1 * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
		vertexbuffer.pos(0.0D, 0.0D, this.zLevel).tex(0.0D, f1 * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
		tessellator.draw();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		float f = -(this.time + partialTicks) * 0.5F;
		...
		for (int i1 = 0; i1 < this.lines.size(); ++i1) {
			...
			if (((l + f + 12.0F + 8.0F) > 0.0F) && ((l + f) < this.height)) {
				...
				if (s.startsWith("[C]")) {
					...
					this.fontRendererObj.fontRandom.setSeed((i1 * 4238972211L) + this.time / 4);
		...
		int k1 = this.height;
		vertexbuffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		vertexbuffer.pos(0.0D, k1, this.zLevel).tex(0.0D, 1.0D).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
		vertexbuffer.pos(j1, k1, this.zLevel).tex(1.0D, 1.0D).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
		vertexbuffer.pos(j1, 0.0D, this.zLevel).tex(1.0D, 0.0D).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
		vertexbuffer.pos(0.0D, 0.0D, this.zLevel).tex(0.0D, 0.0D).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
		tessellator.draw();
