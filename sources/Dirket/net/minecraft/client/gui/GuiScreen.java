...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
	...
	public static String getClipboardString() {
		try {
			...
			if ((transferable != null) && transferable.isDataFlavorSupported(DataFlavor.stringFlavor)) { return (String) transferable.getTransferData(DataFlavor.stringFlavor); }
			...
			if (isShiftKeyDown()) {
				...
				if (clickevent.getAction() == ClickEvent.Action.OPEN_URL) {
					...
					try {
						...
						if (!PROTOCOLS.contains(s.toLowerCase())) { throw new URISyntaxException(clickevent.getValue(), "Unsupported protocol: " + s.toLowerCase()); }
	...
	public void drawBackground(int tint) {
		...
		vertexbuffer.pos(0.0D, this.height, 0.0D).tex(0.0D, (this.height / 32.0F) + tint).color(64, 64, 64, 255).endVertex();
		vertexbuffer.pos(this.width, this.height, 0.0D).tex(this.width / 32.0F, (this.height / 32.0F) + tint).color(64, 64, 64, 255).endVertex();
