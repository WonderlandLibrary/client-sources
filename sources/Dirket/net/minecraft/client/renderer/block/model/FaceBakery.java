...
public class FaceBakery {
	...
	public BakedQuad makeBakedQuad(Vector3f p_makeBakedQuad_1_, Vector3f p_makeBakedQuad_2_, BlockPartFace p_makeBakedQuad_3_, TextureAtlasSprite p_makeBakedQuad_4_, EnumFacing p_makeBakedQuad_5_, ModelRotation p_makeBakedQuad_6_, BlockPartRotation p_makeBakedQuad_7_, boolean p_makeBakedQuad_8_, boolean p_makeBakedQuad_9_) {
		...
		int[] aint = this.makeQuadVertexData(blockfaceuv, p_makeBakedQuad_4_, p_makeBakedQuad_5_, this.getPositionsDiv16(p_makeBakedQuad_1_, p_makeBakedQuad_2_), p_makeBakedQuad_6_,
				p_makeBakedQuad_7_, flag);
	...
	private int[] makeQuadVertexData(BlockFaceUV p_makeQuadVertexData_1_, TextureAtlasSprite p_makeQuadVertexData_2_, EnumFacing p_makeQuadVertexData_3_, float[] p_makeQuadVertexData_4_, ModelRotation p_makeQuadVertexData_5_, @Nullable BlockPartRotation p_makeQuadVertexData_6_, boolean p_makeQuadVertexData_7_) {
		int i = 28;
	...
	public static float getFaceBrightness(EnumFacing p_178412_0_) {
		switch (p_178412_0_) {
		case DOWN:
			if (Config.isShaders()) { return Shaders.blockLightLevel05; }
		...
		case NORTH:
		case SOUTH:
			if (Config.isShaders()) { return Shaders.blockLightLevel08; }

			return 0.8F;

		case WEST:
		case EAST:
			if (Config.isShaders()) { return Shaders.blockLightLevel06; }
	...
	private void fillVertexData(int[] p_fillVertexData_1_, int p_fillVertexData_2_, EnumFacing p_fillVertexData_3_, BlockFaceUV p_fillVertexData_4_, float[] p_fillVertexData_5_,
			TextureAtlasSprite p_fillVertexData_6_, ModelRotation p_fillVertexData_7_, @Nullable BlockPartRotation p_fillVertexData_8_, boolean p_fillVertexData_9_) {
		EnumFacing enumfacing = p_fillVertexData_7_.rotateFace(p_fillVertexData_3_);
		...
		Vector3f vector3f = new Vector3f(p_fillVertexData_5_[enumfacedirection$vertexinformation.xIndex], p_fillVertexData_5_[enumfacedirection$vertexinformation.yIndex],
	...
	private void storeVertexData(int[] faceData, int storeIndex, int vertexIndex, Vector3f position, int shadeColor, TextureAtlasSprite sprite, BlockFaceUV faceUV) {
		...
		faceData[j + 4] = Float.floatToRawIntBits(sprite.getInterpolatedU((faceUV.getVertexU(vertexIndex) * 0.999D) + (faceUV.getVertexU((vertexIndex + 2) % 4) * 0.001D)));
		faceData[j + 4 + 1] = Float.floatToRawIntBits(sprite.getInterpolatedV((faceUV.getVertexV(vertexIndex) * 0.999D) + (faceUV.getVertexV((vertexIndex + 2) % 4) * 0.001D)));
	}
	...
	public int rotateVertex(Vector3f p_rotateVertex_1_, EnumFacing p_rotateVertex_2_, int p_rotateVertex_3_, ModelRotation p_rotateVertex_4_) {
		if (p_rotateVertex_4_ == ModelRotation.X0_Y0) {
			...
			return p_rotateVertex_4_.rotateVertex(p_rotateVertex_2_, p_rotateVertex_3_);
		}
	}
