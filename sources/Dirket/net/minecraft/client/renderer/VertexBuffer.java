...
public class VertexBuffer {
	...
	public void sortVertexData(float p_181674_1_, float p_181674_2_, float p_181674_3_) {
		...
		for (int j = 0; j < i; ++j) {
			afloat[j] = getDistanceSq(this.rawFloatBuffer, (float) (p_181674_1_ + this.xOffset), (float) (p_181674_2_ + this.yOffset), (float) (p_181674_3_ + this.zOffset),
	...
	public void putColorMultiplier(float red, float green, float blue, int p_178978_4_) {
        ...
        if(Direkt.getInstance().getModuleManager().getModule(Xray.class).isRunning()){
        	this.rawIntBuffer.put(i, Direkt.getInstance().getModuleManager().getModule(Xray.class).getProperty(Integer.class, "Opacity").getValue() << 24 | (j & 0x00FFFFFF));
        }
        else
	...
	private int drawForIcon(TextureAtlasSprite p_drawForIcon_1_, int p_drawForIcon_2_) {
		...
		for (int l = p_drawForIcon_2_; l < k; ++l) {
			...
			if (textureatlassprite == p_drawForIcon_1_) {
				...
				if (this.blockLayer == BlockRenderLayer.TRANSLUCENT) { return l; }
	...
	public class State {
		...
		public State(int[] p_i6_2_, VertexFormat p_i6_3_, TextureAtlasSprite[] p_i6_4_) {
			this.stateRawBuffer = p_i6_2_;
			this.stateVertexFormat = p_i6_3_;
			this.stateQuadSprites = p_i6_4_;
		}
