	...
	public void draw(VertexBuffer vertexBufferIn) {
		if (vertexBufferIn.getVertexCount() > 0) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (flag) {
					Reflector.callVoid(vertexformatelement$enumusage, Reflector.ForgeVertexFormatElementEnumUseage_preDraw,
			...
			for (int k1 = list.size(); j1 < k1; ++j1) {
				...
				if (flag1) {
					Reflector.callVoid(vertexformatelement$enumusage1, Reflector.ForgeVertexFormatElementEnumUseage_postDraw,
							new Object[] { vertexformat, Integer.valueOf(j1), Integer.valueOf(i), bytebuffer });
				} else {
