...
public class BlockFluidRenderer {
	...
	public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn, VertexBuffer worldRendererIn) {
		...
		try {
            ...
            if(Direkt.getInstance().getModuleManager().getModule(Xray.class).isRunning() && !(Xray.getXrayBlocks().contains(blockStateIn.getBlock())))
            	opacity = Direkt.getInstance().getModuleManager().getModule(Xray.class).getProperty(Integer.class, "Opacity").getValue() / 255F;
			...
			if (flag1 || flag2 || aboolean[0] || aboolean[1] || aboolean[2] || aboolean[3]) {
				...
				if (flag2) {
					...
					int i2 = (l1 >> 16) & 65535;
					int j2 = l1 & 65535;
					worldRendererIn.pos(d0, d1, d2 + 1.0D).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F).tex(f38, f41).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0, d1, d2).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F).tex(f38, f40).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F).tex(f39, f40).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2 + 1.0D).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F).tex(f39, f41).lightmap(i2, j2).endVertex();
					flag3 = true;
				}
				...
				for (int i1 = 0; i1 < 4; ++i1) {
					...
					if (aboolean[i1]) {
						...
						if ((f44 > f42) || (f45 > f43)) {
							...
							worldRendererIn.pos(d3, d1 + f44, d4).color(f35, f36, f37, 1.0F).tex(f27, f29).lightmap(k, l).endVertex();
							worldRendererIn.pos(d5, d1 + f45, d6).color(f35, f36, f37, 1.0F).tex(f28, f30).lightmap(k, l).endVertex();
							worldRendererIn.pos(d5, d1 + f43, d6).color(f35, f36, f37, 1.0F).tex(f28, f33).lightmap(k, l).endVertex();
							worldRendererIn.pos(d3, d1 + f42, d4).color(f35, f36, f37, 1.0F).tex(f27, f32).lightmap(k, l).endVertex();
	...
	private float getFluidHeight(IBlockAccess blockAccess, BlockPos blockPosIn, Material blockMaterial) {
		...
		for (int j = 0; j < 4; ++j) {
			...
			if (blockAccess.getBlockState(blockpos.up()).getMaterial() == blockMaterial) { return 1.0F; }
