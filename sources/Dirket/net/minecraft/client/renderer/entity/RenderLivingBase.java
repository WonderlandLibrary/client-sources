...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
				|| !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] { entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z) })) {
			...
			if (Reflector.ForgeEntity_shouldRiderSit.exists()) {
				this.mainModel.isRiding = entity.isRiding() && (entity.getRidingEntity() != null)
			...
			try {
                ...
                if (esp.isRunning() && esp.getProperty(ESP.Mode.class, "Mode").getValue().equals(ESP.Mode.BFCE) && Direkt.getInstance().getModuleManager().getModule(ESP.class).doesQualify(entity)) {
                	...
                	GL11.glPolygonMode(GL11.GL_FRONT, GL11.GL_LINE);
                	if(entity instanceof EntityPlayer && Direkt.getInstance().getFriendManager().isFriend((EntityPlayer) entity))
                    GL11.glColor3f(0.27F, 0.7F, 1F);
                	else
                    GL11.glColor3f(0F, 1, 0.0F);
...
/*
                    if (Direkt.getInstance().getModuleManager().getModule(ESP.class).doesQualify(entity)) {
    					...
    					else if(entity instanceof EntityPlayer && Direkt.getInstance().getFriendManager().isFriend((EntityPlayer) entity))
    						GL11.glColor3f(0.25F, 0.65F, ClientColors.getFadingHue());
    					else
    						GL11.glColor3f(0F, ClientColors.getFadingHue(), 0F);

                        GL11.glPolygonMode(GL11.GL_FRONT, GL11.GL_LINE);
                        this.renderModel(entity, f6, f5, f8, f2, f7, 0.0625F);
                        GL11.glPolygonMode(GL11.GL_FRONT, GL11.GL_FILL);
                        GL11.glColor4d(0.175, 0.175, 0.175, 1);
	...
	public void renderName(T entity, double x, double y, double z) {
    	//TODO: Direkt: NameTags event.
        final EventRenderNametag event = new EventRenderNametag(this.renderManager, entity, entity.getDisplayName());
        Direkt.getInstance().getEventManager().call(event);
        if (event.isCancelled()) {
            return;
        }
		if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists()
				|| !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, new Object[] { entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z) })) {
