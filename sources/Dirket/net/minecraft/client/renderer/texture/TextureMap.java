...
public class TextureMap extends AbstractTexture implements ITickableTextureObject {
	...
	public TextureMap(String p_i10_1_, boolean p_i10_2_) {
		this(p_i10_1_, (ITextureMapPopulator) null, p_i10_2_);
	}
	...
	public TextureMap(String p_i11_1_, ITextureMapPopulator p_i11_2_, boolean p_i11_3_) {
		this.iconGrid = null;
		this.iconGridSize = -1;
	...
	public void loadTextureAtlas(IResourceManager resourceManager) {
		...
		int j1 = Minecraft.getGLMaximumTextureSize();
		...
		for (Entry<String, TextureAtlasSprite> entry : this.mapRegisteredSprites.entrySet()) {
			...
			if ((k2 >= 1) && (l2 >= 1)) {
				...
				if (i3 < k) {
					LOGGER.warn("Texture {} with size {}x{} limits mip level from {} to {}", new Object[] { resourcelocation, Integer.valueOf(textureatlassprite1.getIconWidth()),
							Integer.valueOf(textureatlassprite1.getIconHeight()), Integer.valueOf(MathHelper.calculateLogBaseTwo(k)), Integer.valueOf(MathHelper.calculateLogBaseTwo(i3)) });
					k = i3;
				}
		...
		int k1 = Math.min(i, k);
		int l1 = MathHelper.calculateLogBaseTwo(k1);
		...
		if (l1 < this.mipmapLevels) {
			LOGGER.warn("{}: dropping miplevel from {} to {}, because of minimum power of two: {}",
					new Object[] { this.basePath, Integer.valueOf(this.mipmapLevels), Integer.valueOf(l1), Integer.valueOf(k1) });
		...
		for (TextureAtlasSprite textureatlassprite2 : stitcher.getStichSlots()) {
			...
			if ((textureatlassprite2 == this.missingImage) || this.generateMipmaps(resourceManager, textureatlassprite2)) {
				...
				try {
					if (Config.isShaders()) {
						ShadersTex.uploadTexSubForLoadAtlas(textureatlassprite2.getFrameTextureData(0), textureatlassprite2.getIconWidth(), textureatlassprite2.getIconHeight(),
								textureatlassprite2.getOriginX(), textureatlassprite2.getOriginY(), false, false);
					} else {
						TextureUtil.uploadTextureMipmap(textureatlassprite2.getFrameTextureData(0), textureatlassprite2.getIconWidth(), textureatlassprite2.getIconHeight(),
								textureatlassprite2.getOriginX(), textureatlassprite2.getOriginY(), false, false);
					}
		...
		if (Config.isMultiTexture()) {
			...
			for (TextureAtlasSprite textureatlassprite4 : stitcher.getStichSlots()) {
				...
				if (textureatlassprite5 != null) {
					...
					try {
						TextureUtil.uploadTextureMipmap(textureatlassprite5.getFrameTextureData(0), textureatlassprite5.getIconWidth(), textureatlassprite5.getIconHeight(),
								textureatlassprite5.getOriginX(), textureatlassprite5.getOriginY(), flag2, flag1);
		...
		if (texture.hasCustomLoader(resourceManager, resourcelocation)) {
			...
			label29: {
				...
				try {
					...
					LOGGER.error("Unable to parse metadata from {}", new Object[] { resourcelocation, runtimeexception1 });
					flag1 = false;
				} catch (IOException ioexception) {
	...
	public ResourceLocation completeResourceLocation(ResourceLocation p_completeResourceLocation_1_) {
		return this.isAbsoluteLocation(p_completeResourceLocation_1_)
				? new ResourceLocation(p_completeResourceLocation_1_.getResourceDomain(), p_completeResourceLocation_1_.getResourcePath() + ".png")
				: new ResourceLocation(p_completeResourceLocation_1_.getResourceDomain(),
						String.format("%s/%s%s", new Object[] { this.basePath, p_completeResourceLocation_1_.getResourcePath(), ".png" }));
	}
	...
	private boolean isTerrainAnimationActive(TextureAtlasSprite p_isTerrainAnimationActive_1_) {
		return (p_isTerrainAnimationActive_1_ != TextureUtils.iconWaterStill) && (p_isTerrainAnimationActive_1_ != TextureUtils.iconWaterFlow)
				? ((p_isTerrainAnimationActive_1_ != TextureUtils.iconLavaStill) && (p_isTerrainAnimationActive_1_ != TextureUtils.iconLavaFlow)
						? ((p_isTerrainAnimationActive_1_ != TextureUtils.iconFireLayer0) && (p_isTerrainAnimationActive_1_ != TextureUtils.iconFireLayer1)
								? (p_isTerrainAnimationActive_1_ == TextureUtils.iconPortal ? Config.isAnimatedPortal()
										: ((p_isTerrainAnimationActive_1_ != TextureUtils.iconClock) && (p_isTerrainAnimationActive_1_ != TextureUtils.iconCompass) ? Config.isAnimatedTerrain()
												: true))
