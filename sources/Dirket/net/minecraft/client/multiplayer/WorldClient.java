...
public class WorldClient extends World {
	...
	private static WorldProvider makeWorldProvider(int p_makeWorldProvider_0_) {
		return Reflector.DimensionManager_createProviderFor.exists()
				? (WorldProvider) Reflector.call(Reflector.DimensionManager_createProviderFor, new Object[] { Integer.valueOf(p_makeWorldProvider_0_) })
				: DimensionType.getById(p_makeWorldProvider_0_).createDimension();
	}
	...
	protected void buildChunkCoordList() {
		int i = MathHelper.floor_double(this.mc.thePlayer.posX / 16.0D);
		int j = MathHelper.floor_double(this.mc.thePlayer.posZ / 16.0D);
		...
		if ((i != this.playerChunkX) || (j != this.playerChunkY)) {
			...
			int l = MathHelper.floor_double(this.mc.thePlayer.posX / 16.0D);
			int i1 = MathHelper.floor_double(this.mc.thePlayer.posZ / 16.0D);
...
//      TODO: Direkt: EventServerDisconnect
      Direkt.getInstance().getEventManager().call(new EventServerDisconnect());
	...
	protected void playMoodSoundAndCheckLight(int p_147467_1_, int p_147467_2_, Chunk chunkIn) {
		...
		if (this.ambienceTicks == 0) {
			...
			if ((iblockstate.getMaterial() == Material.AIR) && (this.getLight(blockpos) <= this.rand.nextInt(8)) && (this.getLightFor(EnumSkyBlock.SKY, blockpos) <= 0) && (this.mc.thePlayer != null)
					&& (this.mc.thePlayer.getDistanceSq(j + 0.5D, l + 0.5D, k + 0.5D) > 4.0D)) {
	...
	public void doVoidFogParticles(int posX, int posY, int posZ) {
		...
		if ((itemstack == null) || (Block.getBlockFromItem(itemstack.getItem()) != Blocks.BARRIER)) {
			itemstack = this.mc.thePlayer.getHeldItemOffhand();
		}
