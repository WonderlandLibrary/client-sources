				...
				if (j < aworldgenspikes$endspike.length) {
					...
					if (flag) {
						...
						for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(
								new BlockPos(worldgenspikes$endspike.getCenterX() - 10, worldgenspikes$endspike.getHeight() - 10, worldgenspikes$endspike.getCenterZ() - 10),
								new BlockPos(worldgenspikes$endspike.getCenterX() + 10, worldgenspikes$endspike.getHeight() + 10, worldgenspikes$endspike.getCenterZ() + 10))) {
							worldIn.setBlockToAir(blockpos$mutableblockpos);
						}

						worldIn.createExplosion((Entity) null, worldgenspikes$endspike.getCenterX() + 0.5F, worldgenspikes$endspike.getHeight(), worldgenspikes$endspike.getCenterZ() + 0.5F, 5.0F,
								true);
