...
public class Chunk {
	...
	private ExtendedBlockStorage getLastExtendedBlockStorage() {
		for (int i = this.storageArrays.length - 1; i >= 0; --i) {
			if (this.storageArrays[i] != NULL_BLOCK_STORAGE) { return this.storageArrays[i]; }
		}
		...
		if (this.worldObj.getWorldType() == WorldType.DEBUG_WORLD) {
			...
			try {
				if ((y >= 0) && ((y >> 4) < this.storageArrays.length)) {
					...
					if (extendedblockstorage != NULL_BLOCK_STORAGE) { return extendedblockstorage.get(x & 15, y & 15, z & 15); }
				}
	...
	public int getLightFor(EnumSkyBlock p_177413_1_, BlockPos pos) {
		...
		int k = pos.getZ() & 15;
		ExtendedBlockStorage extendedblockstorage = this.storageArrays[j >> 4];
		return extendedblockstorage == NULL_BLOCK_STORAGE ? (this.canSeeSky(pos) ? p_177413_1_.defaultLightValue : 0)
				: (p_177413_1_ == EnumSkyBlock.SKY ? (this.worldObj.provider.getHasNoSky() ? 0 : extendedblockstorage.getExtSkylightValue(i, j & 15, k))
						: (p_177413_1_ == EnumSkyBlock.BLOCK ? extendedblockstorage.getExtBlocklightValue(i, j & 15, k) : p_177413_1_.defaultLightValue));
	}
	...
	public void addEntity(Entity entityIn) {
		...
		if ((i != this.xPosition) || (j != this.zPosition)) {
			LOGGER.warn("Wrong location! ({}, {}) should be ({}, {}), {}",
					new Object[] { Integer.valueOf(i), Integer.valueOf(j), Integer.valueOf(this.xPosition), Integer.valueOf(this.zPosition), entityIn, entityIn });
			entityIn.setDead();
		}
	...
	public Random getRandomWithSeed(long seed) {
		return new Random(
				(this.worldObj.getSeed() + this.xPosition * this.xPosition * 4987142 + this.xPosition * 5947611 + (this.zPosition * this.zPosition * 4392871L) + this.zPosition * 389711) ^ seed);
	}
