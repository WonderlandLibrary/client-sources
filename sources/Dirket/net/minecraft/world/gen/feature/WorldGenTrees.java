...
public class WorldGenTrees extends WorldGenAbstractTree {
	...
	private static final IBlockState DEFAULT_LEAF = Blocks.LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.OAK).withProperty(BlockLeaves.CHECK_DECAY,
			...
			for (int j = position.getY(); j <= (position.getY() + 1 + i); ++j) {
				...
				for (int l = position.getX() - k; (l <= (position.getX() + k)) && flag; ++l) {
					for (int i1 = position.getZ() - k; (i1 <= (position.getZ() + k)) && flag; ++i1) {
						if ((j >= 0) && (j < 256)) {
							if (!this.canGrowInto(worldIn.getBlockState(blockpos$mutableblockpos.set(l, j, i1)).getBlock())) {
								flag = false;
							}
						} else {
							flag = false;
