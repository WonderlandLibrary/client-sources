...
public class WorldGenCanopyTree extends WorldGenAbstractTree {
	...
	private static final IBlockState DARK_OAK_LEAVES = Blocks.LEAVES2.getDefaultState().withProperty(BlockNewLeaf.VARIANT, BlockPlanks.EnumType.DARK_OAK).withProperty(BlockLeaves.CHECK_DECAY,
		...
		for (int l = 0; l <= (height + 1); ++l) {
			...
			for (int j1 = -i1; j1 <= i1; ++j1) {
				for (int k1 = -i1; k1 <= i1; ++k1) {
					if (!this.canGrowInto(worldIn.getBlockState(blockpos$mutableblockpos.set(i + j1, j + l, k + k1)).getBlock())) { return false; }
				}
			}
		}
