...
public class WorldGenDesertWells extends WorldGenerator {
	...
	private final IBlockState sandSlab = Blocks.STONE_SLAB.getDefaultState().withProperty(BlockStoneSlab.VARIANT, BlockStoneSlab.EnumType.SAND).withProperty(BlockSlab.HALF,
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (!IS_SAND.apply(worldIn.getBlockState(position))) {
			...
			for (int i = -2; i <= 2; ++i) {
				for (int j = -2; j <= 2; ++j) {
					if (worldIn.isAirBlock(position.add(i, -1, j)) && worldIn.isAirBlock(position.add(i, -2, j))) { return false; }
				}
			}
