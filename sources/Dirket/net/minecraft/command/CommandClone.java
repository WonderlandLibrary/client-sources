								...
								for (int l = structureboundingbox.minY; l <= structureboundingbox.maxY; ++l) {
									for (int i1 = structureboundingbox.minX; i1 <= structureboundingbox.maxX; ++i1) {
										...
										if ((!flag1 || (iblockstate.getBlock() != Blocks.AIR))
												&& ((block == null) || ((iblockstate.getBlock() == block) && ((j < 0) || (iblockstate.getBlock().getMetaFromState(iblockstate) == j))))) {
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
		return (args.length > 0) && (args.length <= 3) ? getTabCompletionCoordinate(args, 0, pos)
				: ((args.length > 3) && (args.length <= 6) ? getTabCompletionCoordinate(args, 3, pos)
						: ((args.length > 6) && (args.length <= 9) ? getTabCompletionCoordinate(args, 6, pos)
								: (args.length == 10 ? getListOfStringsMatchingLastWord(args, new String[] { "replace", "masked", "filtered" })
										: (args.length == 11 ? getListOfStringsMatchingLastWord(args, new String[] { "normal", "force", "move" })
												: ((args.length == 12) && "filtered".equals(args[9]) ? getListOfStringsMatchingLastWord(args, Block.REGISTRY.getKeys())
