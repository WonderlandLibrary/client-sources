	...
	public void execute(final MinecraftServer server, final ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 5) {
			...
			if ("detect".equals(args[4]) && (args.length > 10)) {
				...
				if (!world.isBlockLoaded(blockpos1)) { throw new CommandException("commands.execute.failed", new Object[] { "detect", entity.getName() }); }

				IBlockState iblockstate = world.getBlockState(blockpos1);

				if ((iblockstate.getBlock() != block) || ((j >= 0) && (iblockstate.getBlock().getMetaFromState(iblockstate) != j))) { throw new CommandException("commands.execute.failed",
						new Object[] { "detect", entity.getName() }); }
			...
			try {
				...
				if (k < 1) { throw new CommandException("commands.execute.allInvocationsFailed", new Object[] { s }); }
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
		return args.length == 1 ? getListOfStringsMatchingLastWord(args, server.getAllUsernames())
				: ((args.length > 1) && (args.length <= 4) ? getTabCompletionCoordinate(args, 1, pos)
						: ((args.length > 5) && (args.length <= 8) && "detect".equals(args[4]) ? getTabCompletionCoordinate(args, 5, pos)
								: ((args.length == 9) && "detect".equals(args[4]) ? getListOfStringsMatchingLastWord(args, Block.REGISTRY.getKeys()) : Collections.<String> emptyList())));
	}
