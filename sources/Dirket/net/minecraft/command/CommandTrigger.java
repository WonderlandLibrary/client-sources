	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 3) {
			...
			if (sender instanceof EntityPlayerMP) {
				...
				if (!(entity instanceof EntityPlayerMP)) { throw new CommandException("commands.trigger.invalidPlayer", new Object[0]); }
				...
				if (!scoreboard.entityHasObjective(entityplayermp.getName(), scoreobjective)) {
					...
					if (score.isLocked()) {
						...
						if ("set".equals(args[1])) {
							...
							if (!"add".equals(args[1])) { throw new CommandException("commands.trigger.invalidMode", new Object[] { args[1] }); }
