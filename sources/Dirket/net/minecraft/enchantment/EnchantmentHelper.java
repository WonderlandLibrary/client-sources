...
public class EnchantmentHelper {
		...
		if (stack == null) {
			...
			if (nbttaglist == null) {
				...
				for (int i = 0; i < nbttaglist.tagCount(); ++i) {
					...
					if (enchantment == enchID) { return j; }
				}
	...
	public static List<EnchantmentData> buildEnchantmentList(Random randomIn, ItemStack itemStackIn, int p_77513_2_, boolean allowTreasure) {
		...
		if (i <= 0) {
			...
			p_77513_2_ = MathHelper.clamp_int(Math.round(p_77513_2_ + (p_77513_2_ * f)), 1, Integer.MAX_VALUE);
	...
	public static void removeIncompatible(List<EnchantmentData> p_185282_0_, EnchantmentData p_185282_1_) {
		...
		while (iterator.hasNext()) {
			if (!p_185282_1_.enchantmentobj.canApplyTogether(iterator.next().enchantmentobj)) {
				iterator.remove();
			}
		}
	}
