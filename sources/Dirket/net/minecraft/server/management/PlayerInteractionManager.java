...
public class PlayerInteractionManager {
	...
	public void updateBlockRemoving() {
		...
		if (this.receivedFinishDiggingPacket) {
			...
			if (iblockstate.getMaterial() == Material.AIR) {
				...
				float f = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, this.delayedDestroyPos) * (i + 1);
				int j = (int) (f * 10.0F);
			...
			if (iblockstate1.getMaterial() == Material.AIR) {
				...
				float f1 = iblockstate1.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, this.delayedDestroyPos) * (k + 1);
				int l = (int) (f1 * 10.0F);
	...
	/**
	 * If not creative, it calls sendBlockBreakProgress until the block is broken first. tryHarvestBlock can also be the result of this call.
	 */
	public void onBlockClicked(BlockPos pos, EnumFacing side) {
		if (this.isCreative()) {
			...
			if (this.gameType.isAdventure()) {
				if (this.gameType == GameType.SPECTATOR) { return; }
	...
	public void blockRemoving(BlockPos pos) {
		if (pos.equals(this.destroyPos)) {
			...
			if (iblockstate.getMaterial() != Material.AIR) {
				float f = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, pos) * (i + 1);
	...
	public boolean tryHarvestBlock(BlockPos pos) {
		if (this.gameType.isCreative() && (this.thisPlayerMP.getHeldItemMainhand() != null) && (this.thisPlayerMP.getHeldItemMainhand().getItem() instanceof ItemSword)) {
			...
			if (((block instanceof BlockCommandBlock) || (block instanceof BlockStructure)) && !this.thisPlayerMP.func_189808_dh()) {
				...
				if (this.gameType.isAdventure()) {
					if (this.gameType == GameType.SPECTATOR) { return false; }
	...
	public EnumActionResult processRightClickBlock(EntityPlayer player, World worldIn, @Nullable ItemStack stack, EnumHand hand, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (this.gameType == GameType.SPECTATOR) {
			...
			if (!player.isSneaking() || ((player.getHeldItemMainhand() == null) && (player.getHeldItemOffhand() == null))) {
				...
				if (iblockstate.getBlock().onBlockActivated(worldIn, pos, iblockstate, player, hand, stack, facing, hitX, hitY, hitZ)) { return EnumActionResult.SUCCESS; }
			}
