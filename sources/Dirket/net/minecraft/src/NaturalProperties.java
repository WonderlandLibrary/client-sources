...
public class NaturalProperties {
	...
	public NaturalProperties(String p_i50_1_) {
		if (p_i50_1_.equals("4")) {
			this.rotation = 4;
		} else if (p_i50_1_.equals("2")) {
			this.rotation = 2;
		} else if (p_i50_1_.equals("F")) {
			this.flip = true;
		} else if (p_i50_1_.equals("4F")) {
			this.rotation = 4;
			this.flip = true;
		} else if (p_i50_1_.equals("2F")) {
			this.rotation = 2;
			this.flip = true;
		} else {
			Config.warn("NaturalTextures: Unknown type: " + p_i50_1_);
		}
	}
	...
	private boolean isFullSprite(BakedQuad p_isFullSprite_1_) {
		...
		for (int j = 0; j < 4; ++j) {
			...
			if (!this.equalsDelta(f8, f, f3) && !this.equalsDelta(f8, f1, f3)) { return false; }

			if (!this.equalsDelta(f9, f4, f7) && !this.equalsDelta(f9, f5, f7)) { return false; }
		}
