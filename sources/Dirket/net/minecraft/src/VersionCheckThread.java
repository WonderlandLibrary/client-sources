...
public class VersionCheckThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			URL url = new URL("http://optifine.net/version/1.10.2/HD_U.txt");
			httpurlconnection = (HttpURLConnection) url.openConnection();

			if (Config.getGameSettings().snooperEnabled) {
				httpurlconnection.setRequestProperty("OF-MC-Version", "1.10.2");
				...
				httpurlconnection.setRequestProperty("OF-Release", "C2");
			...
			try {
				...
				if (astring.length >= 1) {
					...
					if (Config.compareRelease(s1, "C2") <= 0) { return; }
