...
public class DynamicLights {
	...
	private static final DataParameter<Optional<ItemStack>> PARAMETER_ITEM_STACK = new DataParameter(6, DataSerializers.OPTIONAL_ITEM_STACK);
	...
	public static int getLightLevel(ItemStack p_getLightLevel_0_) {
		if (p_getLightLevel_0_ == null) {
			...
			if (item instanceof ItemBlock) {
				...
				if (block != null) { return block.getLightValue(block.getDefaultState()); }
			}

			return item == Items.LAVA_BUCKET ? Blocks.LAVA.getLightValue(Blocks.LAVA.getDefaultState())
					: ((item != Items.BLAZE_ROD) && (item != Items.BLAZE_POWDER) ? (item == Items.GLOWSTONE_DUST ? 8
							: (item == Items.PRISMARINE_CRYSTALS ? 8
									: (item == Items.MAGMA_CREAM ? 8 : (item == Items.NETHER_STAR ? Blocks.BEACON.getLightValue(Blocks.BEACON.getDefaultState()) / 2 : 0))))
							: 10);
		}
	}
	...
	public static int getLightLevel(Entity p_getLightLevel_0_) {
		if ((p_getLightLevel_0_ == Config.getMinecraft().getRenderViewEntity()) && !Config.isDynamicHandLight()) {
			...
			if (p_getLightLevel_0_ instanceof EntityPlayer) {
				...
				if (entityplayer.isSpectator()) { return 0; }
			}
			...
			if (p_getLightLevel_0_.isBurning()) {
				...
				if (p_getLightLevel_0_ instanceof EntityCreeper) {
					...
					if (entitycreeper.getCreeperFlashIntensity(0.0F) > 0.001D) { return 15; }
				}
	...
	public static ItemStack getItemStack(EntityItem p_getItemStack_0_) {
		ItemStack itemstack = (ItemStack) ((Optional) p_getItemStack_0_.getDataManager().get(PARAMETER_ITEM_STACK)).orNull();
		return itemstack;
	}
}
