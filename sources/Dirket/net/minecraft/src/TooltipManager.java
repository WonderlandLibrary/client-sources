...
public class TooltipManager {
	...
	public TooltipManager(GuiScreen p_i76_1_) {
		this.guiScreen = p_i76_1_;
	}
		...
		if ((Math.abs(p_drawTooltips_1_ - this.lastMouseX) <= 5) && (Math.abs(p_drawTooltips_2_ - this.lastMouseY) <= 5)) {
			...
			if (System.currentTimeMillis() >= (this.mouseStillTime + i)) {
				...
				if (guibutton instanceof IOptionControl) {
					...
					Gui.drawRect(j, k, l, i1, -536870912);
	...
	private GuiButton getSelectedButton(int p_getSelectedButton_1_, int p_getSelectedButton_2_, List p_getSelectedButton_3_) {
		for (int i = 0; i < p_getSelectedButton_3_.size(); ++i) {
			...
			boolean flag = (p_getSelectedButton_1_ >= guibutton.xPosition) && (p_getSelectedButton_2_ >= guibutton.yPosition) && (p_getSelectedButton_1_ < (guibutton.xPosition + j))
					&& (p_getSelectedButton_2_ < (guibutton.yPosition + k));

			if (flag) { return guibutton; }
		}
