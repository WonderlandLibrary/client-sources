...
public class CustomItemProperties {
	...
	public CustomItemProperties(Properties p_i23_1_, String p_i23_2_) {
		this.name = parseName(p_i23_2_);
		this.basePath = parseBasePath(p_i23_2_);
		this.type = this.parseType(p_i23_1_.getProperty("type"));
		this.items = this.parseItems(p_i23_1_.getProperty("items"), p_i23_1_.getProperty("matchItems"));
		this.mapTextures = parseTextures(p_i23_1_, this.basePath);
		this.texture = parseTexture(p_i23_1_.getProperty("texture"), p_i23_1_.getProperty("tile"), p_i23_1_.getProperty("source"), p_i23_2_, this.basePath, this.type, this.mapTextures);
		String s = p_i23_1_.getProperty("damage");

		if (s != null) {
			this.damagePercent = s.contains("%");
			s = s.replace("%", "");
			this.damage = this.parseRangeListInt(s);
			this.damageMask = this.parseInt(p_i23_1_.getProperty("damageMask"), 0);
		}

		this.stackSize = this.parseRangeListInt(p_i23_1_.getProperty("stackSize"));
		this.enchantmentIds = this.parseRangeListInt(p_i23_1_.getProperty("enchantmentIDs"));
		this.enchantmentLevels = this.parseRangeListInt(p_i23_1_.getProperty("enchantmentLevels"));
		this.nbtTagValues = this.parseNbtTagValues(p_i23_1_);
		this.blend = Blender.parseBlend(p_i23_1_.getProperty("blend"));
		this.speed = this.parseFloat(p_i23_1_.getProperty("speed"), 0.0F);
		this.rotation = this.parseFloat(p_i23_1_.getProperty("rotation"), 0.0F);
		this.layer = this.parseInt(p_i23_1_.getProperty("layer"), 0);
		this.weight = this.parseInt(p_i23_1_.getProperty("weight"), 0);
		this.duration = this.parseFloat(p_i23_1_.getProperty("duration"), 1.0F);
	}
	...
	private NbtTagValue[] parseNbtTagValues(Properties p_parseNbtTagValues_1_) {
		String s = "nbt.";
		Map<String, String> map = getMatchingProperties(p_parseNbtTagValues_1_, s);
