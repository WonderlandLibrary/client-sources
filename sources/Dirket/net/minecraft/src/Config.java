...
public class Config {
	...
	public static final String VERSION = "OptiFine_1.10.2_HD_U_C2";
	...
	public static String getVersion() {
		return "OptiFine_1.10.2_HD_U_C2";
	}
	...
	public static String getVersionDebug() {
		...
		stringbuffer.append("OptiFine_1.10.2_HD_U_C2");
														...
														: (!GLContext.getCapabilities().OpenGL21 ? 20
																: (!GLContext.getCapabilities().OpenGL30 ? 21
																		: (!GLContext.getCapabilities().OpenGL31 ? 30
																				: (!GLContext.getCapabilities().OpenGL32 ? 31 : (!GLContext.getCapabilities().OpenGL33 ? 32
																						: (!GLContext.getCapabilities().OpenGL40 ? 33 : 40)))))))))));
	}
	...
	public static boolean isCloudsFancy() {
		return gameSettings.ofClouds != 0 ? gameSettings.ofClouds == 2
				: (isShaders() && !Shaders.shaderPackClouds.isDefault() ? Shaders.shaderPackClouds.isFancy() : (texturePackClouds != 0 ? texturePackClouds == 2 : gameSettings.fancyGraphics));
	}

	public static boolean isCloudsOff() {
		return gameSettings.ofClouds != 0 ? gameSettings.ofClouds == 3
				: (isShaders() && !Shaders.shaderPackClouds.isDefault() ? Shaders.shaderPackClouds.isOff() : (texturePackClouds != 0 ? texturePackClouds == 3 : false));
	}
	...
	public static ModelManager getModelManager() {
		if (modelManager == null) {
			try {
				Field field = Reflector.getField(Minecraft.class, ModelManager.class);
				field.setAccessible(true);
				modelManager = (ModelManager) field.get(getMinecraft());
			} catch (Exception exception) {
				warn("Error getting ModelManager");
				warn(exception.getClass().getName() + ": " + exception.getMessage());
			}
		}
	...
	private static Method getMethod(Class p_getMethod_0_, String p_getMethod_1_, Object[] p_getMethod_2_) {
		...
		for (Method method : amethod) {
			if (method.getName().equals(p_getMethod_1_) && (method.getParameterTypes().length == p_getMethod_2_.length)) { return method; }
		}
	...
	public static IResourcePack getDefiningResourcePack(ResourceLocation p_getDefiningResourcePack_0_) {
		...
		for (int i = airesourcepack.length - 1; i >= 0; --i) {
			...
			if (iresourcepack.resourceExists(p_getDefiningResourcePack_0_)) { return iresourcepack; }
		}
	...
	private static DisplayMode getDisplayMode(DisplayMode[] p_getDisplayMode_0_, DisplayMode p_getDisplayMode_1_) {
		if (p_getDisplayMode_1_ != null) {
			for (DisplayMode displaymode : p_getDisplayMode_0_) {
				if ((displaymode.getBitsPerPixel() == p_getDisplayMode_1_.getBitsPerPixel()) && (displaymode.getFrequency() == p_getDisplayMode_1_.getFrequency())) { return displaymode; }
			}
		}
	...
	public static DisplayMode getDisplayMode(Dimension p_getDisplayMode_0_) throws LWJGLException {
		...
		for (DisplayMode displaymode : adisplaymode) {
			if ((displaymode.getWidth() == p_getDisplayMode_0_.width) && (displaymode.getHeight() == p_getDisplayMode_0_.height)) { return displaymode; }
		}
	...
	public static boolean equalsOne(Object p_equalsOne_0_, Object[] p_equalsOne_1_) {
		if (p_equalsOne_1_ == null) {
			...
			for (Object object : p_equalsOne_1_) {
				if (equals(p_equalsOne_0_, object)) { return true; }
			}
	...
	public static void checkDisplayMode() {
		try {
			if (minecraft.isFullScreen()) {
				...
				if ((displaymode.getWidth() == dimension.width) && (displaymode.getHeight() == dimension.height)) { return; }

				DisplayMode displaymode1 = getDisplayMode(dimension);

				if (displaymode1 == null) { return; }
