...
public class CustomColormap implements CustomColors.IColorizer {
	...
	public CustomColormap(Properties p_i22_1_, String p_i22_2_, int p_i22_3_, int p_i22_4_) {
		ConnectedParser connectedparser = new ConnectedParser("Colormap");
		this.name = connectedparser.parseName(p_i22_2_);
		this.basePath = connectedparser.parseBasePath(p_i22_2_);
		this.format = this.parseFormat(p_i22_1_.getProperty("format"));
		this.matchBlocks = connectedparser.parseMatchBlocks(p_i22_1_.getProperty("blocks"));
		this.source = parseTexture(p_i22_1_.getProperty("source"), p_i22_2_, this.basePath);
		this.color = ConnectedParser.parseColor(p_i22_1_.getProperty("color"), -1);
		this.yVariance = connectedparser.parseInt(p_i22_1_.getProperty("yVariance"), 0);
		this.yOffset = connectedparser.parseInt(p_i22_1_.getProperty("yOffset"), 0);
		this.width = p_i22_3_;
		this.height = p_i22_4_;
	}
	...
	public boolean isValid(String p_isValid_1_) {
		if ((this.format != 0) && (this.format != 1)) {
			...
			if (this.colors == null) { return false; }
	...
	private MatchBlock[] detectMatchBlocks() {
		...
		if (block != null) {
			...
			if (matcher.matches()) {
				...
				if (i >= 0) { return new MatchBlock[] { new MatchBlock(i) }; }
			}
	...
	private int getColorVanilla(Biome p_getColorVanilla_1_, BlockPos p_getColorVanilla_2_) {
		...
		double d1 = MathHelper.clamp_float(p_getColorVanilla_1_.getRainfall(), 0.0F, 1.0F);
		d1 = d1 * d0;
	...
	private MatchBlock getMatchBlock(int p_getMatchBlock_1_) {
		if (this.matchBlocks == null) {
			...
			for (MatchBlock matchblock : this.matchBlocks) {
				if (matchblock.getBlockId() == p_getMatchBlock_1_) { return matchblock; }
			}
