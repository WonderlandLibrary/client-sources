...
public abstract class PathNavigate {
	...
	public void onUpdateNavigation() {
		...
		if (!this.noPath()) {
			if (this.canNavigate()) {
				...
				if ((vec3d.yCoord > vec3d1.yCoord) && !this.theEntity.onGround && (MathHelper.floor_double(vec3d.xCoord) == MathHelper.floor_double(vec3d1.xCoord))
	...
	protected void pathFollow() {
		...
		if ((MathHelper.abs((float) (this.theEntity.posX - (vec3d1.xCoord + 0.5D))) < this.maxDistanceToWaypoint)
				&& (MathHelper.abs((float) (this.theEntity.posZ - (vec3d1.zCoord + 0.5D))) < this.maxDistanceToWaypoint) && (Math.abs(this.theEntity.posY - vec3d1.yCoord) < 1.0D)) {
			this.currentPath.setCurrentPathIndex(this.currentPath.getCurrentPathIndex() + 1);
		}
