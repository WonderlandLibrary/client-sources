...
public class PathNavigateGround extends PathNavigate {
	...
	public Path getPathToPos(BlockPos pos) {
		if (this.worldObj.getBlockState(pos).getMaterial() == Material.AIR) {
			...
			if (blockpos.getY() > 0) { return super.getPathToPos(blockpos.up()); }
	...
	private int getPathablePosY() {
		if (this.theEntity.isInWater() && this.getCanSwim()) {
			...
			while ((block == Blocks.FLOWING_WATER) || (block == Blocks.WATER)) {
				...
				if (j > 16) { return (int) this.theEntity.getEntityBoundingBox().minY; }
			}
	...
	protected void removeSunnyPath() {
		...
		if (this.shouldAvoidSun) {
			if (this.worldObj.canSeeSky(
					new BlockPos(MathHelper.floor_double(this.theEntity.posX), (int) (this.theEntity.getEntityBoundingBox().minY + 0.5D), MathHelper.floor_double(this.theEntity.posZ)))) { return; }
		...
		if (d2 < 1.0E-8D) {
			...
			if (!this.isSafeToStandAt(i, (int) posVec31.yCoord, j, sizeX, sizeY, sizeZ, posVec31, d0, d1)) {
				...
				while (((k1 * k) > 0) || ((l1 * l) > 0)) {
					...
					if (!this.isSafeToStandAt(i, (int) posVec31.yCoord, j, sizeX, sizeY, sizeZ, posVec31, d0, d1)) { return false; }
				}
			...
			for (int k = i; k < (i + sizeX); ++k) {
				for (int l = j; l < (j + sizeZ); ++l) {
					...
					if (((d0 * p_179683_8_) + (d1 * p_179683_10_)) >= 0.0D) {
						...
						if ((pathnodetype == PathNodeType.DAMAGE_FIRE) || (pathnodetype == PathNodeType.DANGER_FIRE) || (pathnodetype == PathNodeType.DAMAGE_OTHER)) { return false; }
					}
				}
			}
	...
	private boolean isPositionClear(int p_179692_1_, int p_179692_2_, int p_179692_3_, int p_179692_4_, int p_179692_5_, int p_179692_6_, Vec3d p_179692_7_, double p_179692_8_, double p_179692_10_) {
		for (BlockPos blockpos : BlockPos.getAllInBox(new BlockPos(p_179692_1_, p_179692_2_, p_179692_3_),
			...
			if (((d0 * p_179692_8_) + (d1 * p_179692_10_)) >= 0.0D) {
				...
				if (!block.isPassable(this.worldObj, blockpos)) { return false; }
			}
		}
