...
public class TabGui1 extends EventListener {
	...
	private void renderMenu(List<String> items, int currentItem) {
		...
		RenderUtils.drawRect(5, height * 2 - 5, maxItemWidth + 15 + 5, height * (items.size() + 2) - 5, new Color(0, 0, 0, 30).getRGB());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (isCurrentItem) {
				...
				RenderUtils.drawRect(5, height * (i + 2) - 5, maxItemWidth + 15 + 5, height * (i + 3) - 5, new Color(0, 0, 0, 50).getRGB());
				if (indentation == 3)
					item = "&a" + item;
			}
		...
		//mc.getTextureManager().bindTexture(new ResourceLocation("Bpl/neko1.png"));
				//GuiScreen.drawModalRectWithCustomSizedTexture(880, 60, 0, 0, 50, 80, 50, 80);
