...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    public void sendPacket(Packet packetIn) {
        PacketEvent packetEvent = new PacketEvent(PacketEvent.EventState.SENDING, packetIn);
        Pulsive.INSTANCE.getEventBus().call(packetEvent);
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                PacketEvent packetEvent = new PacketEvent(PacketEvent.EventState.RECEIVING, p_channelRead0_2_);
                Pulsive.INSTANCE.getEventBus().call(packetEvent);
                if (packetEvent.getPacket() instanceof S32PacketConfirmTransaction && Minecraft.getMinecraft().currentScreen instanceof GuiContainer) {
                    ((GuiContainer) Minecraft.getMinecraft().currentScreen).onServerTransaction(packetEvent.getPacket());
                }
                if (packetEvent.isCancelled()) return;
