...
import club.pulsive.api.ui.MainMenu;
import club.pulsive.impl.event.client.KeyPressEvent;
import club.pulsive.impl.event.client.StartGameEvent;
import club.pulsive.impl.event.player.WorldLoadEvent;
import club.pulsive.impl.mainmenu.LoadingScreen;
import club.pulsive.impl.mainmenu.MainMenuNew;
...
//        this.drawSplashScreen(this.renderEngine);
        ...
        Pulsive.INSTANCE.init();
        Pulsive.INSTANCE.getEventBus().call(new StartGameEvent());

        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new GuiMainMenu(), this, this.serverName, this.serverPort));
        } else {
            this.displayGuiScreen(new LoadingScreen());
        }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = new MainMenuNew();
        } else if (guiScreenIn == null && this.thePlayer.getHealth() <= 0.0F) {
            guiScreenIn = new GuiGameOver();
        }

        if (guiScreenIn instanceof MainMenuNew || guiScreenIn instanceof LoadingScreen) {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    Pulsive.INSTANCE.getEventBus().call(new KeyPressEvent(k));
                    KeyBinding.onTick(k);
                }
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        Pulsive.INSTANCE.getEventBus().call(new WorldLoadEvent());
        this.systemTime = 0L;
    }
