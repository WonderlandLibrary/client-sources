...
public class FontRenderer extends CFont
{
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        if (camera.isToggled() && camera.prop_elements.isSelected(Camera.ELEMENTS.STREAMING)) {
            if(mc.theWorld != null) {
                for (Object o : mc.theWorld.getLoadedEntityList()) {
                    if (o instanceof EntityPlayer && o != mc.thePlayer) {
                        EntityPlayer entity = (EntityPlayer) o;
                        text = text.replaceAll(entity.getName(), "§k" + entity.getName());
                        
                    }
                }
            }
            if(Minecraft.getMinecraft().session.getUsername() != "Player")
            text = text.replace(Minecraft.getMinecraft().session.getUsername(), "Very Hot");
        }
