...
import club.pulsive.impl.module.impl.visual.waveycapes.util.*;
...
public class SmoothCapeRenderer {
    ...
    private void modifyPoseStackSimulation(CustomCapeRenderLayer layer, PoseStack poseStack, AbstractClientPlayer abstractClientPlayer, float delta, int part) {
        ...
        float partRotation = (float) -ApacheMath.atan2(y, z);
        partRotation = ApacheMath.max(partRotation, 0);
        if (partRotation != 0)
            partRotation = (float) (ApacheMath.PI - partRotation);
