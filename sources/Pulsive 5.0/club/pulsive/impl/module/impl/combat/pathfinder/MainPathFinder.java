...
public class MainPathFinder implements MinecraftUtil {
        ...
        for (final Vec3 pathElm : pathFinderPath) {
            if (i == 0 || i == pathFinderPath.size() - 1) {
                ...
                if (pathElm.squareDistanceTo(lastDashLoc) > 5 * 5) {
                    ...
                    final double smallX = ApacheMath.min(lastDashLoc.getX(), pathElm.getX());
                    final double smallY = ApacheMath.min(lastDashLoc.getY(), pathElm.getY());
                    final double smallZ = ApacheMath.min(lastDashLoc.getZ(), pathElm.getZ());
                    final double bigX = ApacheMath.max(lastDashLoc.getX(), pathElm.getX());
                    final double bigY = ApacheMath.max(lastDashLoc.getY(), pathElm.getY());
                    final double bigZ = ApacheMath.max(lastDashLoc.getZ(), pathElm.getZ());
                    int x = (int) smallX;
                    block1:
                    while (x <= bigX) {
                        int y2 = (int) smallY;
