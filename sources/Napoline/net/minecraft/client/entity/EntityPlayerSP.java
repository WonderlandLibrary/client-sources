...
import me.napoleon.napoline.events.EventPostUpdate;
...
import me.napoleon.napoline.modules.movement.NoSlowDown;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    /**
     * called every tick when the player is on foot. Performs all the things that normally happen during player.
     */
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if(Napoline.moduleManager.getModByClass(TargetStrafe.class).getState() && (Boolean) new TargetStrafe().space.getValue()) {
            double d6;

            for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes((Entity) (Object) this, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty();) {
                if (x < d6 && x >= -d6) {
                    x = 0.0D;
                } else if (x > 0.0D) {
                    x -= d6;
        ...
        EventMove e = (EventMove) EventManager.call(new EventMove(x, y, z, null, hasValidHealth));
    ...
    public void sendChatMessage(String message)
    {
        try {
            if (message.startsWith(".")) {
                if(message.toLowerCase().startsWith(".nocommands")){
                    Command cmd = CommandManager.getCommandByName("nocommands");
                    cmd.run(new String[0]);
                    return;
                }
                if(Napoline.noCommands){
                ...
                for (int i = 0; i < argsWithoutCommand.length; i++) {
                    argsWithoutCommand[i] = args[i + 1];
                }
                Command cmd = CommandManager.getCommandByName(args[0]);
                if (cmd == null) {
                    PlayerUtil.sendMessage("Command \"" + args[0] + "\" not found!");
                    return;
                }
                cmd.run(argsWithoutCommand);
                return;
            }
        }catch(Exception e){
            e.printStackTrace();
    ...
    public void onLivingUpdate()
    {
        ...
        boolean noSlowEnabled = ModuleManager.getModByClass(NoSlowDown.class).getState();
        if (this.isUsingItem() && !this.isRiding() && !noSlowEnabled)
        {
