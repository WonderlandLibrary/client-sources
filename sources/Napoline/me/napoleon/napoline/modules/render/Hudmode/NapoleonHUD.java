...
public class NapoleonHUD extends HUD {
    ...
    public static void renderHud() {
        ...
        Color rainbow2 = new Color(Color.HSBtoRGB(
                (float) ((double) mc.thePlayer.ticksExisted / 50.0 + Math.sin((double) rainbowTick / 50.0 * 1.6))
                        % 1.0f,
                0.5f, 1.0f));
        String text = " \247w" + "Napoli" + ChatFormatting.GREEN + "sense" + "\247w"
                + " | " + Napoline.username + " | " + (mc.isSingleplayer() ? "singleplayer" : mc.getCurrentServerData().serverIP) + " | " + mc.debugFPS + "fps" + " | " + (mc.isSingleplayer() ? "non" : mc.getCurrentServerData().pingToServer + "ms");
