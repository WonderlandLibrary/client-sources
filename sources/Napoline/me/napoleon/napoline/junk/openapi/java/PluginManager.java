...
/**
 * @description: Java插件管理 for base client
 * @author: QianXia
 * @create: 2020/10/5 15:54
 **/
public class PluginManager {
    public List<NapolinePlugin> plugins = new ArrayList<>();
        ...
        try {
            ...
            loading: for (int i = 0; i < urlCL.size(); i++) {
                ...
                try {
                    ...
                    // 防止载入同一个插件的不同版本
                    for (NapolinePlugin oldPlugin : plugins) {
            ...
            // 重载需要重新调用这些函数
            if(reload){
                this.onModuleManagerLoad(Napoline.moduleManager, true);
                this.onCommandManagerLoad(Napoline.commandManager);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    	...
    	try {
			...
			while(entries.hasMoreElements()) {
				...
				if(entry.getName().endsWith(".class")) {
					...
					if("me/napoleon/napoline/openapi/java/NapolinePlugin".equals(cn.superName)) {
    ...
    public boolean isPluginEnabled(NapolinePlugin plugin){
        for (Object value : ModuleManager.pluginModsList.values()) {
            if (value instanceof NapolinePlugin) {
    ...
    public void setPluginState(NapolinePlugin plugin, boolean state) {
                    ...
                    node.methods.forEach(methodNode -> {
                        for (AbstractInsnNode insnNode : methodNode.instructions.toArray()) {
                            if (insnNode instanceof MethodInsnNode) {
                                ...
                                boolean flag = "me/napoleon/napoline/manager/ModuleManager".equals(method.owner) && "addModule".equals(method.name) && "(Lme/superskidder/lune/Modules/Mod;)V".equals(method.desc);
                                if (flag) {
                                    method.name = "addPluginModule";
                                    method.desc = "(Lme/napoleon/napoline/Modules/Mod;Ljava/lang/Object;)V";
