package me.napoleon.napoline.junk.cloud;

import com.mojang.realmsclient.gui.ChatFormatting;

import me.napoleon.napoline.Napoline;
import me.napoleon.napoline.guis.clickgui.IRCgui;
import me.napoleon.napoline.modules.world.IRCThread;
import me.napoleon.napoline.utils.client.DevUtils;
import me.napoleon.napoline.utils.client.HWIDUtil;
import me.napoleon.napoline.utils.irc.IRCMessage;
            ...
            switch (ircMessage) {
                case "CLOSE":
                    if (Minecraft.getMinecraft().theWorld != null) {
                        PlayerUtil.sendMessage("§4IRC Server Shut Down");
                    }
                    return;
                case "你被踢出了IRC":
                    System.exit(99);
                case "HeartPacketNeeded":
                    sendMessage("MSG@" + (Minecraft.getMinecraft().thePlayer == null ? "Null" : Minecraft.getMinecraft().thePlayer.getName()) + "@" + ChatFormatting.BLUE + Napoline.CLIENT_NAME + ChatFormatting.WHITE + "@" + "Heart!Give you,my dear");
...
//            socket = new Socket("43.249.192.204", 41344);
            ...
            pw.println(username + "@" + password + "@" + HWIDUtil.getHWID() + "@" + ChatFormatting.BLUE + "Napoline@" + (Napoline.mc.thePlayer == null ? "Null" : Napoline.mc.thePlayer.getName()));
//            pw.println(username + "@" + password + "@" + HWIDUtil.getHWID() + "@" + ChatFormatting.BLUE + "Napoline@" + Lune.mc.thePlayer.getName());
            new IRCThread().start();
            return true;
        } catch (Exception e) {
        	if(DevUtils.isDev()) {
