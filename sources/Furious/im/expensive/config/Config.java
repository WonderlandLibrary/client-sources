...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\furious\\configs"), name + ".cfg");
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        Furious.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        Furious.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : Furious.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", Furious.getInstance().getStyleManager().getCurrentStyle() == style);
