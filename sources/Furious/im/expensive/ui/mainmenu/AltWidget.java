...
public class AltWidget implements IMinecraft {
    ...
    public void render(MatrixStack stack, int x, int y) {
        scroll = MathUtil.fast(scroll, scrollPre, 15);
        ...
        DisplayUtils.drawShadow(this.x, this.y, width, height, 8, HUD.getColor(0,1));

        DisplayUtils.drawRoundedRect(this.x, this.y, width, height, new Vector4f(3, 3, 3, 3), ColorUtils.rgba(0, 0, 0, 240));
        Scissor.push();
        Scissor.setFromComponentCoordinates(this.x, this.y, width - 16, height);
        Fonts.montserrat.drawText(stack, "Аккаунты", this.x + 50, this.y + 6, -1, 7);
        Scissor.unset();
        Scissor.pop();
