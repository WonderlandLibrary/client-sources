...
public abstract class Function implements IMinecraft {
    ...
    public void onDisable() {
        ...
        Furious.getInstance().getEventBus().unregister(this);
    }
    ...
    public final void toggle() {
        ...
        if (!this.state) {
            this.onDisable();
            Notifications.NOTIFICATION_MANAGER.add("Функция " + this.name + " была выключена", "", 1, NotificationManager.ImageType.FIRST_PHOTO);
        } else {
            this.onEnable();
            Notifications.NOTIFICATION_MANAGER.add("Функция " + this.name + " была включена", "", 1, NotificationManager.ImageType.SECOND_PHOTO);
        }
