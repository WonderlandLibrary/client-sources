...
{
    private static final Noise DEFAULT_NOISE = new Noise(-7, ImmutableList.of(1.0D, 1.0D));
    public static final MapCodec<NetherBiomeProvider> PACKET_CODEC = RecordCodecBuilder.mapCodec((builder) ->
    {
        ...
        }), Noise.CODEC.fieldOf("temperature_noise").forGetter((netherProvider) -> {
            return netherProvider.temperatureNoise;
        }), Noise.CODEC.fieldOf("humidity_noise").forGetter((netherProvider) -> {
            return netherProvider.humidityNoise;
        }), Noise.CODEC.fieldOf("altitude_noise").forGetter((netherProvider) -> {
            return netherProvider.altitudeNoise;
        }), Noise.CODEC.fieldOf("weirdness_noise").forGetter((netherProvider) -> {
    ...
    private Optional<DefaultBuilder> getDefaultBuilder()
    {
        ...
        {
            return new DefaultBuilder(registryPresetPair.getSecond(), registryPresetPair.getFirst(), this.seed);
        });
    }
    ...
    static final class DefaultBuilder
    {
        public static final MapCodec<DefaultBuilder> CODEC = RecordCodecBuilder.mapCodec((builder) ->
        {
            return builder.group(ResourceLocation.CODEC.flatXmap((id) -> {
                return Optional.ofNullable(Preset.PRESETS.get(id)).map(DataResult::success).orElseGet(() -> {
                    return DataResult.error("Unknown preset: " + id);
                });
            }, (preset) -> {
                return DataResult.success(preset.id);
            }).fieldOf("preset").stable().forGetter(DefaultBuilder::getPreset), RegistryLookupCodec.getLookUpCodec(Registry.BIOME_KEY).forGetter(DefaultBuilder::getLookupRegistry), Codec.LONG.fieldOf("seed").stable().forGetter(DefaultBuilder::getSeed)).apply(builder, builder.stable(DefaultBuilder::new));
        });
    ...
    static class Noise
    {
        ...
        {
            return builder.group(Codec.INT.fieldOf("firstOctave").forGetter(Noise::getNumberOfOctaves), Codec.DOUBLE.listOf().fieldOf("amplitudes").forGetter(Noise::getAmplitudes)).apply(builder, Noise::new);
        });
    ...
    public static class Preset
    {
        ...
        public static final Preset DEFAULT_NETHER_PROVIDER_PRESET = new Preset(new ResourceLocation("nether"), (preset, lookupRegistry, seed) ->
        {
