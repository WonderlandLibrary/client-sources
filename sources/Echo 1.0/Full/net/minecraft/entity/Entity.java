...
import dev.echo.listener.event.impl.player.StepConfirmEvent;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            Echo.INSTANCE.getEventProtocol().handleEvent(safeWalkEvent);
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    Echo.INSTANCE.getEventProtocol().handleEvent(new StepConfirmEvent());
                }
            }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (this instanceof EntityPlayerSP) {
            Echo.INSTANCE.getEventProtocol().handleEvent(strafeEvent);
        }
        ...
        float squared = strafe * strafe + forward * forward;

        if (squared >= 1.0E-4F) {
            squared = MathHelper.sqrt_float(squared);

            if (squared < 1.0F) {
                squared = 1.0F;
            }

            squared = friction / squared;

            strafe = strafe * squared;
            forward = forward * squared;

            float xMotion = MathHelper.sin(strafeEvent.getYaw() * (float) Math.PI / 180.0F);
            float zMotion = MathHelper.cos(strafeEvent.getYaw() * (float) Math.PI / 180.0F);

            this.motionX += strafe * zMotion - forward * xMotion;
            this.motionZ += forward * zMotion + strafe * xMotion;
        }
    }
