...
import dev.echo.module.impl.render.CustomModel;
import dev.echo.module.impl.render.ESP2D;
import dev.echo.module.impl.render.TargetHUDMod;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    protected void renderModel(T entitylivingbaseIn, float x, float y, float z, float entityYaw, float entityPitch, float scaleFactor, float partialTicks) {
        ...
        if (flag || flag1) {
            ...
            Echo.INSTANCE.getEventProtocol().handleEvent(renderModelEvent);

            GL11.glEnable(GL11.GL_ALPHA_TEST);

            renderModelEvent.drawModel();

            renderModelEvent.setState(Event.State.POST);
            Echo.INSTANCE.getEventProtocol().handleEvent(renderModelEvent);
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (entity instanceof EntityPlayer) {
            ...
            Echo.INSTANCE.getEventProtocol().handleEvent(nametagRenderEvent);
            if (nametagRenderEvent.isCancelled()) return;
        }
        if (esp2D == null) {
            esp2D = Echo.INSTANCE.getModuleCollection().getModule(ESP2D.class);
        }
