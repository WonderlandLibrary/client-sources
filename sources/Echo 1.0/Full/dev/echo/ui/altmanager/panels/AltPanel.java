...
import dev.echo.ui.altmanager.helpers.Alt;
import dev.echo.ui.altmanager.helpers.AltManagerUtils;
...
public class AltPanel extends Panel {
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (Echo.INSTANCE.getAltManager().isTyping()) return;
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        TextField searchField = Echo.INSTANCE.getAltManager().searchField;
        boolean filterBanned = Echo.INSTANCE.getAltManager().filterBanned.isEnabled();
    ...
    public static class AltRect implements Screen {
        ...
        public void drawScreen(int mouseX, int mouseY) {
            ...
            if (alt.username != null) {
                usernameText = echoBoldFont16.trimStringToWidth(alt.username, (int) (width - (altSize + 10)));
            }
            float totalHeight = echoBoldFont16.getHeight() + 6 + echoFont14.getHeight();
            ...
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0 || ban.getUnbanDate() == 0) {
                    hasBan = true;
                    totalHeight += echoBoldFont14.getHeight() + 4;
                }
            }
            ...
            echoBoldFont16.drawString(usernameText, x + 3 + altSize + 3.5f, usernameY, textColor);

            float typeX = x + 7 + altSize + echoBoldFont16.getStringWidth(usernameText);
            float size = echoBoldFont16.getHeight() + 2;
            switch (alt.getType()) {
                case "Microsoft":
                ...
                case "Mojang":
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("Echo/mojang.png"));
                    Gui.drawModalRectWithCustomSizedTexture(typeX + 2, usernameY - 1, 0, 0, size, size, size, size);
                    RenderUtil.resetColor();
                    break;
                case "Cracked":
                    Color red = Echo.INSTANCE.getSideGui().getRedBadColor();
                    echoBoldFont14.drawString(" Cracked", typeX, usernameY + 1, ColorUtil.applyOpacity(red, 1 - credsAnim));
                    break;
                case "Not logged in":
                    echoBoldFont14.drawString("§bNot logged in", typeX, usernameY + 1, ColorUtil.applyOpacity(-1, 1 - credsAnim));
                    break;
            }

            drawBan(x + altSize + 6.5f, usernameY + echoBoldFont16.getHeight() + 6, 1 - credsAnim);
            if (hasBan) {
                usernameY += echoBoldFont14.getHeight() + 6;
            }
            ...
            hoveringCreds = HoveringUtil.isHovering(x + 3 + altSize + 1.5f, (usernameY + echoBoldFont16.getHeight() + 6) - 2,
                    echoFont14.getStringWidth("§n" + credsText) + 4, echoFont14.getHeight() + 4, mouseX, mouseY);

            float opacity = showCreds ? (hoveringCreds ? .75f : 0.5f) : hoveringCreds ? .5f : .35f;
            echoFont14.drawString("§n" + credsText, x + 3 + altSize + 3.5f,
                    usernameY + echoBoldFont16.getHeight() + 6, ColorUtil.applyOpacity(-1, opacity));
            ...
            if (showCreds || !credsAnimation.isDone()) {
                ...
                if (alt.email != null) {
                    echoFont14.drawString("Email: " + alt.email, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                    yVal += echoFont14.getHeight() + 4;
                }

                echoFont14.drawString("Password: " + alt.password, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                RenderUtil.scaleEnd();
            }


            if (removeShit) return;
            if (Echo.INSTANCE.getAltManager().currentSessionAlt == alt) {
                Color green = Echo.INSTANCE.getSideGui().getGreenEnabledColor();
                echoBoldFont14.drawString("Logged in", x + width - (echoBoldFont14.getStringWidth("Logged in") + 5),
                        y + height - (echoBoldFont14.getHeight() + 5), green);
            } else {
        ...
        public void drawAltHead(float x, float y, float size) {
            Echo.INSTANCE.getAltManager().getUtils().getHead(alt);
            GLUtil.startBlend();
            RenderUtil.setAlphaLimit(0);
            mc.getTextureManager().bindTexture(alt.head != null ? alt.head : new ResourceLocation("Echo/X-Steve.png"));
        ...
        public void drawBan(float x, float y, float alpha) {
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0) {
                    ...
                    echoFont14.drawString("§lBanned §ron Hypixel for §l" + str, x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                } else if (ban.getUnbanDate() == 0) {
                    echoFont14.drawString("§lBanned §ron Hypixel permanently", x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                }
            }
        }
    }
