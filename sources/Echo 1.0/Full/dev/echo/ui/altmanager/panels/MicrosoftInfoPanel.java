...
public class MicrosoftInfoPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        echoBoldFont26.drawCenteredString("How to use Microsoft Login", getX() + getWidth() / 2f, getY() + 4, ColorUtil.applyOpacity(-1, .75f));

        float controlY = getY() + echoBoldFont32.getHeight() + 8;
        for (Pair<String, String> control : steps) {
            if (control.getFirst().equals("INFO")) {
                echoFont16.drawCenteredString("Make sure that you are logged out of all microsoft accounts so that you are",
                        getX() + getWidth() /2f, controlY, ColorUtil.applyOpacity(-1, .75f));

                controlY += echoBoldFont16.getHeight() + 4;

                echoFont16.drawCenteredString("prompted with a new login panel",
                        getX() + getWidth() /2f, controlY, ColorUtil.applyOpacity(-1, .75f));

                controlY += echoBoldFont16.getHeight() + 6;
                continue;
            }

            echoBoldFont16.drawString(control.getFirst() + ". ", getX() + 12, controlY, ColorUtil.applyOpacity(-1, .5f));
            echoFont16.drawString(control.getSecond(), getX() +
                    echoBoldFont16.getStringWidth(control.getFirst() + ". ") + 14, controlY, ColorUtil.applyOpacity(-1, .35f));
