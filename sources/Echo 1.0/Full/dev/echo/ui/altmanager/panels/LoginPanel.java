...
import dev.echo.utils.text.RandomStringGenerator;
...
public class LoginPanel extends Panel {
    ...
    public LoginPanel() {
        ...
        textFields.add(new TextField(echoFont20));
        textFields.add(new TextField(echoFont20));
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        setHeight(180);
        echoBoldFont32.drawCenteredString("Login", getX() + getWidth() / 2f, getY() + 3, ColorUtil.applyOpacity(-1, .75f));
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setFont(echoBoldFont22);

            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        Echo.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        Echo.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                    case "Add":
                        TextField username = textFields.get(0);
                        ...
                        break;
                    case "Gen Cracked":
                        Echo.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Echo.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("Echo/mc.png"));
        ...
        echoBoldFont26.drawString("Microsoft Login", microX + 10, microsoftY + 4, -1);

        echoFont16.drawString("Login to your migrated account", microX + 10, microsoftY + 23, -1);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Echo.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
