...
public class VoteRect implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        Color greenColor = Echo.INSTANCE.getSideGui().getGreenEnabledColor();
        ...
        RenderUtil.scissorStart(x, y + height / 2f - (echoBoldFont16.getHeight() / 2f + 1.5), width, echoBoldFont16.getHeight() + 3);
        drawAnimatedVote(totalVotes, voteColor);
        RenderUtil.scissorEnd();

        //      echoBoldFont16.drawCenteredString(s, x + width / 2f, y + echoBoldFont16.getMiddleOfBox(height), ColorUtil.applyOpacity(voteColor, alpha));
    }
        ...
        if (lastVote != vote) {
            ...
            if (voteAnimation == null) {
                voteAnimation = new DecelerateAnimation(Math.min(500, 200 * Math.abs(additionalVotes)),
                        additionalVotes * (echoBoldFont16.getHeight() + 3));
            }
            ...
            if (additionalVotes > 0) {
                ...
                for (int i = lastVote; i < (vote + 1); i++) {
                    int additionalY = count * (echoBoldFont16.getHeight() + 3);

                    float voteY = y + echoBoldFont16.getMiddleOfBox(height) + additionalY;
                    echoBoldFont16.drawCenteredString(String.valueOf(i), x + width / 2f, voteY - (animation), ColorUtil.applyOpacity(color, alpha));
                    count++;
                }
            ...
            echoBoldFont16.drawCenteredString(String.valueOf(vote), x + width / 2f,
                    y + echoBoldFont16.getMiddleOfBox(height), ColorUtil.applyOpacity(color, alpha));
        }
