...
public class SideGUIHotbar implements Screen {
    ...
    public final TextField searchField = new TextField(echoFont20);
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        echoBoldFont32.drawString("Echo", x + 9.5f, y + echoBoldFont32.getMiddleOfBox(height), textColor);
        echoFont18.drawString(Echo.VERSION, x + 9.5f + echoBoldFont32.getStringWidth("Echo") - 2,
                y + echoBoldFont32.getMiddleOfBox(height) - 2.5f, ColorUtil.applyOpacity(textColor, .5f));
        ...
        if(!searching) {
            ...
            echoFont14.drawString(String.valueOf(Math.round(refreshTimer.getTime() / 1000f)), refreshButton.getX() - 15,
                    y + echoFont14.getMiddleOfBox(height), ColorUtil.applyOpacity(-1, refreshText.getOutput().floatValue()));
            ...
            refreshButton.setClickAction(() -> Multithreading.runAsync(() -> Echo.INSTANCE.getCloudDataManager().refreshData()));

            boolean refreshing = Echo.INSTANCE.getCloudDataManager().isRefreshing();
            if (refreshing) {
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (!Echo.INSTANCE.getCloudDataManager().isRefreshing() && refreshTimer.hasTimeElapsed(12000)) {
