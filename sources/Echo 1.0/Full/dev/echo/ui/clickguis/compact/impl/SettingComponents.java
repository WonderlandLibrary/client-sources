...
public class SettingComponents implements Screen {
    ...
    public SettingComponents(Module module) {
        ...
        for (Setting setting : module.getSettingsList()) {
            ...
            if (setting instanceof StringSetting) {
                ...
                TextField textField = new TextField(echoFont14);
    ...
    public void handle(int mouseX, int mouseY, int button, GuiEvents type) {
        ...
        Color disabledColor = new Color(255, 255, 255,  50);
        for (Setting setting : module.getSettingsList()) {
            if (setting.cannotBeShown()) continue;
            if (setting instanceof KeybindSetting) continue;
            float settingY = y + (count * settingHeight);
            float middleSettingY = (float) (MathUtils.roundToHalf(y + echoFont16.getMiddleOfBox(settingHeight) + (count * settingHeight)));

            if (setting instanceof NumberSetting) {
                NumberSetting numberSetting = (NumberSetting) setting;
                echoFont16.drawString(setting.name, x + 5, middleSettingY, -1);
                ...
                float valueWidth = echoFont14.getStringWidth(maxValue);
                Gui.drawRect2(x + rectWidth - (valueWidth + 7), settingY + 4, valueWidth + 4, 8, disabledColor.getRGB());

                echoFont14.drawCenteredString(value, x + rectWidth - (valueWidth + 5) + valueWidth / 2f, settingY + 6, -1);
            ...
            if (setting instanceof ColorSetting) {
                ...
                if (clickAnimation.isDone() && clickAnimation.getDirection().equals(Direction.FORWARDS) || !clickAnimation.isDone()) {
                    ...
                    float colorInfoWidth = (float) echoFont14.getStringWidth("R: 255") + 6;
                    ...
                    GlStateManager.color(1, 1, 1, 1);
                    echoFont14.drawCenteredString("R§f: " + colorSetting.getColor().getRed(), gradientX + 2.5f + colorWidth / 2f, gradientY + gradientHeight + 6, redColor);
                    GlStateManager.color(1, 1, 1, 1);

                    echoFont14.drawCenteredString("G§f: " + colorSetting.getColor().getGreen(), gradientX + colorInfoWidth + 5 + colorInfoWidth / 2f,
                            gradientY + gradientHeight + 6, greenColor);

                    GlStateManager.color(1, 1, 1, 1);

                    echoFont14.drawCenteredString("B§f: " + colorSetting.getColor().getBlue(), gradientX + (colorInfoWidth * 2) + 12.5f + colorWidth / 2f,
                    ...
                    echoFont14.drawString("Rainbow: ", rainbowX, gradientY + gradientHeight + 6, textColor);

                    float clickAnim = clickAnimation.getOutput().floatValue();

                    RoundedUtil.drawRound(rainbowX + echoFont14.getStringWidth("Rainbow: ") + 4, gradientY + gradientHeight + 5.5f,
                    ...
                    if (type == GuiEvents.CLICK && button == 0) {
                        if (HoveringUtil.isHovering(rainbowX + echoFont14.getStringWidth("Rainbow: ") + 3,
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("Echo/colorpicker2.png"));
                    ...
                    // Hue bar
                    RenderUtil.color(textColor);
                    mc.getTextureManager().bindTexture(new ResourceLocation("Echo/hue2.png"));
            ...
            if (setting instanceof ModeSetting) {
                ...
                float modeRectWidth = (float) echoFont14.getStringWidth(StringUtils.getLongestModeName(modeSetting.modes)) + 10;
                ...
                if (!openAnimation.isDone() || openAnimation.getDirection().equals(Direction.FORWARDS)) {
                    ...
                    for (String mode : modeSetting.modes) {
                        ...
                        float modeY = (realY + 3.5f) + (6 * openAnimation.getOutput().floatValue()) + echoFont14.getMiddleOfBox(modeSize) + seperation;

                        boolean hoveringMode = HoveringUtil.isHovering(x + rectWidth - (modeRectWidth + 5),
                                modeY - echoFont14.getMiddleOfBox(modeSize), modeRectWidth, modeSize, mouseX, mouseY) && openAnimation.isDone();
                        ...
                        GlStateManager.color(1, 1, 1, 1);
                        Gui.drawRect2(x + rectWidth - (modeRectWidth + 5), modeY - echoFont14.getMiddleOfBox(modeSize), modeRectWidth, modeSize,
                                ColorUtil.applyOpacity(hoveringMode ? accentColor : dropdownColor, openAnimation.getOutput().floatValue()).getRGB());

                        GlStateManager.color(1, 1, 1, 1);
                        echoFont14.drawString(mode, x + rectWidth - (modeRectWidth + 3), modeY, ColorUtil.applyOpacity(-1, openAnimation.getOutput().floatValue()));
                ...
                echoFont14.drawString(modeSetting.getMode(), (x + rectWidth - (modeRectWidth + 5)) + 2, realY + echoFont14.getMiddleOfBox(modeSize), -1);
...
//                            (1 + (settingsCount * boolRectSize)) * openingAnimation.getOutput().floatValue(), dropdownColor.getRGB());
                    ...
                    for (BooleanSetting booleanSetting : booleanSettings) {
                        ...
                        float boolSettingY = (realY + 3) + (7 * openingAnimation.getOutput().floatValue()) + echoFont14.getMiddleOfBox(boolRectSize) + seperation;

                        boolean hovered = HoveringUtil.isHovering(x + rectWidth - (width + 5),
                                boolSettingY - echoFont14.getMiddleOfBox(boolRectSize), width, boolRectSize, mouseX, mouseY);

                        if (hovered && button == 0 && type == GuiEvents.CLICK) {
                            booleanSetting.toggle();
                        }

                        Color toggleColor = enabled ? accentColor : disabledColor;
                        Color hoverColor = hovered ? toggleColor.darker() : toggleColor;
                        RenderUtil.resetColor();
                        Gui.drawRect2(x + rectWidth - (width + 5), boolSettingY - echoFont14.getMiddleOfBox(boolRectSize),
                                width, boolRectSize, hoverColor.getRGB());

                        RenderUtil.resetColor();
                        echoFont14.drawString(booleanSetting.name, x + rectWidth - (width + 2), boolSettingY, ColorUtil.applyOpacity(0xFFFFFFFF, openingAnimation.getOutput().floatValue()));
                ...
                echoFont14.drawCenteredString(enabledCount + "/" + settingsCount + " enabled", (x + rectWidth - (width + 5)) + (width / 2f),
                        realY + echoFont14.getMiddleOfBox(boolRectSize), -1);
