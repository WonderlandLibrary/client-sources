...
public class ModeComponent extends SettingComponent<ModeSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        echoFont14.drawString(modeSetting.name, boxX + 1, y + 3, textColor);

        echoFont16.drawString(modeSetting.getMode(), boxX + 5, boxY + echoFont16.getMiddleOfBox(boxHeight), textColor);
        ...
        if(themeSetting){
            ...
            float spacing = 3;
            RoundedUtil.drawRound(boxX + 7.5f + echoFont16.getStringWidth(modeSetting.getMode()), boxY + middleOfRect,
                    width, height, 2.25f, themeColors.getFirst());
            RoundedUtil.drawRound(boxX + 7.5f + echoFont16.getStringWidth(modeSetting.getMode()) + (spacing + width), boxY + middleOfRect,
        ...
        if (opened || !openAnimation.isDone()) {
            ...
            for (String mode : modeSetting.modes) {
                ...
                RenderUtil.resetColor();
                echoFont16.drawString(mode, modeX + 5,
                        modeY + ((echoFont16.getMiddleOfBox(rectHeight) + (rectHeight * rectCount)) * openAnimation.getOutput().floatValue()),
