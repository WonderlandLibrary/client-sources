...
public class NumberComponent extends SettingComponent<NumberSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        float funnyWidth = (echoFont16.getStringWidth(numberSetting.name) - (echoFont16.getStringWidth(": " + value)));
        echoFont16.drawString(": §l" + value, sliderX +
                        funnyWidth + (echoFont16.getStringWidth(": " + value) * firstTextAnim),
                y + 2, ColorUtil.applyOpacity(textColor, firstTextAnim));


        String text = "You can use arrow keys";
        echoFont14.drawCenteredString(text, x + width /2f, sliderY + sliderHeight + 4.5f,
                ColorUtil.applyOpacity(-1, textAnimations.getSecond().getOutput().floatValue() * .25f));

        echoFont16.drawString(numberSetting.name, sliderX, y + 2, textColor);
