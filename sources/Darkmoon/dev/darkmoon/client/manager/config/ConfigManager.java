...
public class ConfigManager extends Manager<Config> {
    public static final File configDirectory = new File(System.getenv("SystemDrive") + "\\DarkMoon\\configs");
    @Getter
    ...
    private static ArrayList<Config> loadConfigs() {
        ...
        if (files != null) {
            for (File file : files) {
                if (FilenameUtils.getExtension(file.getName()).equals("dm"))
    ...
    public void load() {
        ...
        if (configDirectory != null) {
            File[] files = configDirectory.listFiles(f -> !f.isDirectory() && FilenameUtils.getExtension(f.getName()).equals("dm"));
            for (File f : files) {
    ...
    public Config findConfig(String configName) {
        ...
        if (new File(configDirectory, configName + ".dm").exists())
