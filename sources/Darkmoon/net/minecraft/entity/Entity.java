...
import dev.darkmoon.client.event.player.EventThunder;
import dev.darkmoon.client.module.impl.combat.HitBox;
import dev.darkmoon.client.module.impl.player.NoPush;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void move(MoverType type, double x, double y, double z)
    {
        ...
        AxisAlignedBB before = this.getEntityBoundingBox().expand(0, 0, 0);
        if (this.noClip)
        {
        ...
        else
        {
            ...
            if (this instanceof EntityPlayerSP) {
                ...
                boolean isCollidedHorizontally = d2 != x || d4 != z;
                boolean isCollidedVertically = d3 != y;
                predictGround = isCollidedVertically && d3 < 0.0D;
                x = bpMX;
                y = bpMY;
                z = bpMZ;
                EventMove move = new EventMove(new Vec3d(this.posX, this.posY, this.posZ), new Vec3d(predictX, predictY, predictZ), new Vec3d(x, y, z), collisionOffset, predictGround, isCollidedHorizontally, isCollidedVertically, before);
            ...
            list1 = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().expand(x, y, z));
            if (this instanceof EntityPlayerSP) {
                List<AxisAlignedBB> bb = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().expand(x, y, z).offset(collisionOffset.x, collisionOffset.y, collisionOffset.z));
                if (!bb.isEmpty() && collisionOffset.length() > 0) {
                    for (AxisAlignedBB aabb : bb) {
                        ...
                        if (x != 0.0D) {
                            ...
                            for (int l5 = bb.size(); j5 < l5; ++j5) {
                                x = ((AxisAlignedBB) bb.get(j5)).calculateXOffset(aabb2, x);
                            }

                            if (x != 0.0D) {
                                aabb2 = aabb2.offset(x, 0.0D, 0.0D);
                            }
                            d2 = x;
                        }
                        if (z != 0.0D) {
                            ...
                            for (int i6 = bb.size(); k5 < i6; ++k5) {
                                z = ((AxisAlignedBB) bb.get(k5)).calculateZOffset(aabb2, z);
                            }

                            if (z != 0.0D) {
                                aabb2 = aabb2.offset(0.0D, 0.0D, z);
                            }
                            d4 = z;
                        }
                    }
            ...
            if (this.stepHeight > 0.0F && flag && (d2 != x || d4 != z)  && !ignoreHorizontal && !ignoreVertical)
            {
                double d14 = x;
                double d6 = y;
                double d7 = z;
            ...
            if (this instanceof EntityPlayerSP) {
                ...
                EventManager.call(new EventPostMove(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ)));
            }
    ...
    public double sqDistanceToPlayer(Vec3d player) {
        return player.squareDistanceTo(this.posX, this.posY, this.posZ);
    }
    ...
    public boolean isEntityInsideOpaqueBlock()
    {
        ...
        if (DarkMoon.getInstance().getModuleManager().getModule(NoPush.class).isEnabled()) {
            return false;
        } else
        {
    ...
    {
        HitBox hitBox = (HitBox) DarkMoon.getInstance().getModuleManager().getModule(HitBox.class);
        return hitBox.getHitboxSize(this);
    }
    ...
    public Iterable<ItemStack> getArmorAndEquipment()
    {
        return Iterables.<ItemStack>concat(this.getArmorInventoryList(), this.getHeldEquipment());
    }
