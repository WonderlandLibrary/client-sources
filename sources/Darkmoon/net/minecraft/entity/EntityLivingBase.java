...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        ...
        {
            if (DarkMoon.getInstance().getModuleManager().getModule(ItemAnimate.class).isEnabled() && this instanceof EntityPlayerSP) {
                return this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : ItemAnimate.swipeSpeed.getInt();
            } else {
    ...
    protected void jump()
    {
        if (this instanceof EntityPlayerSP && DarkMoon.getInstance().getModuleManager().getModule(JumpCircle.class).isEnabled()) {
            JumpCircle.circles.add(new JumpCircle.Circle(Minecraft.getMinecraft().player.posX, Minecraft.getMinecraft().player.posY + 0.1, Minecraft.getMinecraft().player.posZ));
        }
    ...
    public boolean isActiveItemStackBlocking(int integer) {
        if (this.isHandActive() && !this.activeItemStack.isEmpty()) {
            Item item = this.activeItemStack.getItem();
            if (item.getItemUseAction(this.activeItemStack) != EnumAction.BLOCK) {
                return false;
            } else {
                return item.getMaxItemUseDuration(this.activeItemStack) - this.activeItemStackUseCount >= integer;
            }
        } else {
            return false;
        }
