...
import dev.darkmoon.client.module.impl.util.LightningDetect;
...
import static dev.darkmoon.client.module.impl.util.LightningDetect.lightMode;
...
public abstract class World implements IBlockAccess
{
    ...
    public boolean addWeatherEffect(Entity entityIn)
    {
        ...
        if (entityIn instanceof EntityLightningBolt && DarkMoon.getInstance().getModuleManager().getModule(LightningDetect.class).isEnabled() && lightMode.is("Old")) {
            EntityLightningBolt lightningBolt = (EntityLightningBolt) entityIn;

            // Получаем координаты молнии
            int x = MathHelper.floor(lightningBolt.posX);
            int y = MathHelper.floor(lightningBolt.posY);
            int z = MathHelper.floor(lightningBolt.posZ);

            // Формируем и отправляем сообщение в чат
            if (LightningDetect.autoGps.get()) {
                WayCommand.x = x;
                WayCommand.z = z;
                WayCommand.gps = true;
            }
            String message = "Lightning spawned at: [" + x + ", " + y + ", " + z + "]";
    ...
    {
        EventOverlay event = new EventOverlay(EventOverlay.OverlayType.SKYLIGHT);
