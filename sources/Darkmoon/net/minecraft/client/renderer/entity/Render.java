...
import dev.darkmoon.client.module.impl.render.NameTags;
...
public abstract class Render<T extends Entity> implements IEntityRenderer
{
    ...
    public void doRenderShadowAndFire(Entity entityIn, double x, double y, double z, float yaw, float partialTicks) {
        ...
        if (this.renderManager.options != null) {
            if (this.renderManager.options.entityShadows && this.shadowSize > 0.0F && !entityIn.isInvisible() && this.renderManager.isRenderShadow()) {
                ...
                if (f > 0.0F) {
                    Optimize optimizationModule = (Optimize) DarkMoon.getInstance().getModuleManager().getModule(Optimize.class);
                    if (!(DarkMoon.getInstance().getModuleManager().getModule(Optimize.class).isEnabled() && optimizationModule.booleanSetting.get(1))) {
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        ...
        if (d0 <= (double)(maxDistance * maxDistance))
        {
            ...
            if (DarkMoon.getInstance().getModuleManager().getModule(NameTags.class).isEnabled() && entityIn instanceof EntityPlayer) return;
