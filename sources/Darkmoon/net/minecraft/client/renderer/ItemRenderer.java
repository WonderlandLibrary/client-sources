...
import dev.darkmoon.client.module.impl.render.ItemAnimate;
import dev.darkmoon.client.module.impl.render.ViewModel;
...
public class ItemRenderer
{
    ...
    private void processSpinRotation(EnumHandSide hand)
    {
        float angle = System.currentTimeMillis() / (long) ((int) ItemAnimate.spinSpeed.get()) % 360L;
        if (DarkMoon.getInstance().getModuleManager().getModule(ItemAnimate.class).isEnabled() && ItemAnimate.spin.get() && !(mc.player.isSwingInProgress && ItemAnimate.stopOnHit.get())) {
            if (ItemAnimate.spinHand.is("Left") && hand != EnumHandSide.LEFT || ItemAnimate.spinHand.is("Right") && hand != EnumHandSide.RIGHT && !ItemAnimate.spinHand.is("All")) {
                return;
            }
            GlStateManager.rotate(angle, ItemAnimate.spinMode.is("Vertical") ? -angle : 0.0f, ItemAnimate.spinMode.is("Horizontal") ? 1.0f : 0.0f, 0.0f);
        }
    }
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer player, float p_187457_2_, float p_187457_3_, EnumHand hand, float p_187457_5_, ItemStack stack, float p_187457_7_)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(hand))
        {
            ...
            else
            {
                ...
                if (DarkMoon.getInstance().getModuleManager().getModule(ViewModel.class).isEnabled() && (!ViewModel.onlyAura.get() || KillAura.targetEntity != null)) {
                    if (flag1) {
                        GlStateManager.translate(ViewModel.rightX.get(), ViewModel.rightY.get(), ViewModel.rightZ.get());
                    } else {
                        GlStateManager.translate(ViewModel.leftX.get(), ViewModel.leftY.get(), ViewModel.leftZ.get());
                    }
                }
                ...
                else
                {
                    ...
                    if (hand == EnumHand.MAIN_HAND && DarkMoon.getInstance().getModuleManager().getModule(ItemAnimate.class).isEnabled() && (!ItemAnimate.onlyAura.get() || KillAura.targetEntity != null)) {
                        if (ItemAnimate.mode.get().equals("Smooth")) {
                            float swingSqrt = MathHelper.sqrt(p_187457_5_);
                            GlStateManager.translate(i * 0.56F, -0.52F, -0.72F);
                            GlStateManager.rotate(i * (45 + MathHelper.sin(p_187457_5_ * p_187457_5_ * 3.1415927F) / 4 * -20), 0, 1, 0);
                            GlStateManager.rotate(i * MathHelper.sin(swingSqrt * 3.1415927F) * -20, 0, 0, 1);
                            GlStateManager.rotate(MathHelper.sin(swingSqrt * 3.1415927F) * -ItemAnimate.swipePower.get(), 1, 0, 0);
                            GlStateManager.rotate(i * -45, 0, 1, 0);
                            processSpinRotation(enumhandside);
                        } else if (ItemAnimate.mode.get().equals("Self")) {
                            float anim = (float) Math.sin(p_187457_5_ * (Math.PI / 2) * 2);
                            GlStateManager.translate(i * 0.8f, -0.2f, -1.1f);
                            GlStateManager.rotate(i * 90, 0, 1, 0);
                            GlStateManager.rotate(i * -60, 0, 0, 1);
                            GlStateManager.rotate(-ItemAnimate.angle.get() - ItemAnimate.swipePower.get() * anim, 1, 0, 0);
                            processSpinRotation(enumhandside);
                        } else if (ItemAnimate.mode.get().equals("Block")) {
                            float anim = (float) Math.sin(p_187457_5_ * (Math.PI / 2) * 2);
                            GlStateManager.translate(i * 0.8f, -0.2f, -1.1f);
                            GlStateManager.rotate(i * 90, 0, 1, 0);
                            GlStateManager.rotate(i * -30, 0, 0, 1);
                            GlStateManager.rotate(-ItemAnimate.angle.get() - ItemAnimate.swipePower.get() * anim, 1, 0, 0);
                            processSpinRotation(enumhandside);
                        } else if (ItemAnimate.mode.get().equals("Block Old")) {
                            if (flag1) {
                                ...
                                GlStateManager.translate(0.0f, (equipProgress / 3) * -0.6f, 0.0f);
                                GlStateManager.rotate(45.0f, 0.0f, 1.0f, 0.0f);
                                final float f4 = MathHelper.sin(swingProgress * swingProgress * 3.1415927f);
                                final float f5 = MathHelper.sin(MathHelper.sqrt(swingProgress) * 3.1415927f);
                                GlStateManager.rotate(f4 * -20.0f, 0.0f, 0.0f, 0.0f);
                                GlStateManager.rotate(f5 * -20.0f, 0.0f, 0.0f, 1.0f);
                                GlStateManager.rotate(f5 * -80.0f, 0.01f, 0.0f, 0.0f);
                ...
                if (DarkMoon.getInstance().getModuleManager().getModule(ViewModel.class).isEnabled()) {
                    GlStateManager.scale(ViewModel.size.get(), ViewModel.size.get(), ViewModel.size.get());
                }
