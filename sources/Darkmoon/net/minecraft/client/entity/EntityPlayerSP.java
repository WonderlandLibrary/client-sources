...
import dev.darkmoon.client.event.player.*;
import dev.darkmoon.client.module.impl.movement.Sprint;
import dev.darkmoon.client.module.impl.movement.NoSlow;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        EventMotion eventPre = new EventMotion(rotationYaw, rotationPitch, posX, posY, posZ, onGround);
        ...
        {
            DarkMoon.getInstance().getModuleManager().timer.updateTimer(rotationYaw, rotationPitch, posX, posY, posZ);
    ...
    public void onLivingUpdate()
    {
        ...
        {
            EventItemUsing eventItemUsing = new EventItemUsing();
            EventManager.call(eventItemUsing);
            if (!eventItemUsing.isCancelled()) {
                NoSlow noSlow = (NoSlow) DarkMoon.getInstance().getModuleManager().getModule(NoSlow.class);
                this.movementInput.moveStrafe *= noSlow.isEnabled() && noSlow.isNeeded() ? 1.0F : 0.2F;
                this.movementInput.moveForward *= noSlow.isEnabled() && noSlow.isNeeded() ? 1.0F : 0.2F;
        ...
        if (DarkMoon.getInstance().getModuleManager().getModule(Sprint.class).isEnabled() && Sprint.omnidirectional.get()) {
            this.setSprinting(MovementUtility.isMoving());
        } else {
        ...
        if (this.movementInput.jump && !flag && !this.onGround && this.motionY < 0.0D && !this.isElytraFlying() && !this.capabilities.isFlying)
        {
            ...
            {
                EventStartElytraFlying eventStartElytraFlying = new EventStartElytraFlying();
                EventManager.call(eventStartElytraFlying);
                if (!eventStartElytraFlying.isCancelled()) {
    ...
    public void blockAllMoves(boolean blockX, boolean blockY) {
        this.blockMoving = blockX;
        this.blockMoveY = blockY;
    }
}
