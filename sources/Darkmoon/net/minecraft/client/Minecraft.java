...
import dev.darkmoon.client.event.player.EventInteractWithEntity;
import dev.darkmoon.client.module.impl.player.MultiAction;
import dev.darkmoon.client.module.impl.player.NoInteract;
import dev.darkmoon.client.module.impl.util.BetterChat;
import dev.darkmoon.client.module.impl.util.CPUStabilize;
import dev.darkmoon.client.module.impl.util.FastWorld;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    private void init() throws LWJGLException, IOException
    {
        ...
        DarkMoon.getInstance().start();

        if (this.serverName != null)
        {
            this.displayGuiScreen(new GuiConnecting(new GuiMainMenuElement(), this, this.serverName, this.serverPort));
        }
        else
        {
            this.displayGuiScreen(new GuiMainMenuElement());
        }
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle("DarkMoon Recode " + DarkMoon.VERSION + " -> Best DLC");
    ...
    private void setWindowIcon()
    {
        ...
        if (util$enumos != Util.EnumOS.OSX)
        {
            ...
            try
            {
                inputstream = Minecraft.class.getResourceAsStream("/assets/minecraft/darkmoon/textures/avatar32.png");
                inputstream1 = Minecraft.class.getResourceAsStream("/assets/minecraft/darkmoon/textures/avatar64.png");
    ...
    public void displayGuiScreen(@Nullable GuiScreen guiScreenIn)
    {
        ...
        {
            guiScreenIn = new GuiMainMenuElement();
        }
        ...
        if (guiScreenIn instanceof GuiMainMenuElement || guiScreenIn instanceof GuiMultiplayer)
        {
            this.gameSettings.showDebugInfo = false;
            boolean check = !(DarkMoon.getInstance().getModuleManager().getModule(BetterChat.class).isEnabled() && BetterChat.chatHistory.get());
            this.ingameGUI.getChatGUI().clearChatMessages(check);
        }
    ...
    public int getLimitFramerate() {
        if (DarkMoon.getInstance().getModuleManager().getModule(CPUStabilize.class).isEnabled() && !Display.isActive()) {
            return 1;
        }
    ...
    public void freeMemory() {
        ...
        try {
            if (!DarkMoon.getInstance().getModuleManager().getModule(FastWorld.class).isEnabled()) {
                System.gc();
                this.loadWorld(null);
            }
        }
    ...
    private void sendClickBlockToController(boolean leftClick) {
        ...
        boolean bl = DarkMoon.getInstance().getModuleManager().getModule(MultiAction.class).isEnabled() ? true : (multiActionCheck = !this.player.isHandActive());
            ...
            if (!this.player.isRowingBoat())
            {
                ...
                for (EnumHand enumhand : EnumHand.values())
                {
                    ...
                    if (this.objectMouseOver != null)
                    {
                        ...
                        {
                            case ENTITY:
                                EventInteractWithEntity eiwe = new EventInteractWithEntity(this.objectMouseOver.entityHit);
                                EventManager.call(eiwe);
                                if (!eiwe.isCanceled()) {
                                    if (this.playerController.interactWithEntity(this.player,
                                    ...
                                    if (this.playerController.interactWithEntity(this.player,
                                            this.objectMouseOver.entityHit, enumhand) == EnumActionResult.SUCCESS) {
                                        return;
                                    }
                                }
                                break;
                            ...
                            case BLOCK:
                                ...
                                if (DarkMoon.getInstance().getModuleManager().getModule(NoInteract.class).isEnabled() && NoInteract.allBlocks.get()) {
                                    break;
                                }

                                if (DarkMoon.getInstance().getModuleManager().getModule(NoInteract.class).isEnabled() &&
                                        NoInteract.getBlocks().contains(this.world.getBlockState(blockpos).getBlock())) {
                                    break;
                                }
    ...
    private void runTickMouse() throws IOException
    {
        while (Mouse.next())
        {
            ...
            {
                EventManager.call(new EventMouse(i));
