...
import dev.darkmoon.client.event.misc.EventDisplay;
...
import dev.darkmoon.client.module.impl.render.Crosshair;
import dev.darkmoon.client.module.impl.render.Hud;
import dev.darkmoon.client.module.impl.render.TargetHud;
import dev.darkmoon.client.module.impl.util.BetterTab;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        EventOverlay event = new EventOverlay(EventOverlay.OverlayType.BOSS_BAR);
        ...
        {
            EventOverlay eventOverlay = new EventOverlay(EventOverlay.OverlayType.SCOREBOARD);
        ...
        if (WayCommand.gps) {
            ...
            double yaw = Math.toDegrees(Math.atan2(WayCommand.z - mc.player.posZ, WayCommand.x - mc.player.posX)) - mc.player.rotationYaw
                    - 90;
            double dst = Math.sqrt(Math.pow(WayCommand.x - mc.player.posX, 2) + Math.pow(WayCommand.z - mc.player.posZ, 2));
            GL11.glPushMatrix();
            GL11.glTranslated(DarkMoon.getInstance().getScaleMath().calc(scaledresolution.getScaledWidth_double()) / 2d + 0.5, DarkMoon.getInstance().getScaleMath().calc(scaledresolution.getScaledHeight_double()) / 2d - 115, 0);
            GL11.glTranslated(
                    (Math.cos(Math.toRadians(yaw - 90)) * 1.3)
                            * (Fonts.comfortaa12.getStringWidth("  " + (int) dst + "m") / 2),
                    Math.sin(Math.toRadians(yaw - 90)) * 10, 0);
            GL11.glRotated(yaw, 0, 0, 1);
                RenderUtility.drawTriangle(255, 255, 255);

            GL11.glPopMatrix();
            GL11.glPushMatrix();
            GL11.glTranslated(DarkMoon.getInstance().getScaleMath().calc(scaledresolution.getScaledWidth_double() / 2d), DarkMoon.getInstance().getScaleMath().calc(scaledresolution.getScaledHeight_double()) / 2d - 115, 0);
            Fonts.comfortaa16.drawCenteredString((int) dst + "m", 0, 0, Color.WHITE.getRGB());
            GL11.glPopMatrix();
            DarkMoon.getInstance().getScaleMath().popScale();
        }
        if (DarkMoon.getInstance().getModuleManager().getModule(BetterTab.class).isEnabled() && BetterTab.animation.get()) {
            anim = AnimationMath.fast(anim, (float) (mc.gameSettings.keyBindPlayerList.isKeyDown() ? 1 : 0), 20);
            anim = MathHelper.clamp(anim, 0, 1);
            GL11.glPushMatrix();
            GL11.glTranslated(scaledresolution.getScaledWidth() / 2f, 10, 0);
            GL11.glScaled(anim, anim, anim);
            GL11.glTranslated(-scaledresolution.getScaledWidth() / 2f, -10, 0);
    ...
    private void renderAttackIndicator(float partialTicks, ScaledResolution p_184045_2_)
    {
        ...
        if (gamesettings.thirdPersonView == 0)
        {
            ...
            else
            {
                ...
                if (!DarkMoon.getInstance().getModuleManager().getModule(Crosshair.class).isEnabled()) {
    ...
    public void renderSelectedItem(ScaledResolution scaledRes)
    {
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty())
        {
            ...
            int j = scaledRes.getScaledHeight() - (DarkMoon.getInstance().getModuleManager().getModule(TargetHud.class).isEnabled() ? TargetHud.getTooltipOffset() : 59);
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            boolean check = DarkMoon.getInstance().getModuleManager().getModule(NameProtect.class).isEnabled();
            if (mc.getCurrentServerData() != null && check) {
                ...
                if (s.contains(mc.player.getName())) {
                    s1 = s.replace(mc.player.getName(), ChatFormatting.RED + NameProtect.protectedNick + ChatFormatting.RESET);
                }
            }
