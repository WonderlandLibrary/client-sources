...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn)
    {
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            int k2 = k1 + i5 * 9;
            if (DarkMoon.getInstance().getModuleManager().getModule(BetterTab.class).isEnabled()) {
                drawRect(j2, k2, j2 + i1, k2 + 8, (mc.player == mc.world.getPlayerEntityByUUID(list.get(k4).getGameProfile().getId()) ? new Color(0, 157, 255, 128).getRGB() : 553648127));
            } else {
            ...
            if (k4 < list.size())
            {
                ...
                if (DarkMoon.getInstance().getModuleManager().getModule(NameProtect.class).isEnabled()) {
                    s4 = s4.replace(mc.player.getName(), ChatFormatting.RED + NameProtect.protectedNick + ChatFormatting.RESET);
                }
