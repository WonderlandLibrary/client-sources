...
import dev.darkmoon.client.module.impl.player.InventoryMove;
import dev.darkmoon.client.module.impl.player.ItemScroller;
...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1)
        {
            ...
            {
                if (DarkMoon.getInstance().getModuleManager().getModule(ItemScroller.class).isEnabled()) {
                    if (Mouse.isButtonDown(0) && Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode()) && timer.hasReached(ItemScroller.delay.get())) {
                ...
                else if (!this.dragSplitting)
                {
                    if (this.mc.player.inventory.getItemStack().isEmpty())
                    {
                        ...
                        else
                        {
                            ...
                            if ((MovementUtility.isMoving() && this instanceof GuiInventory)
                                    && (DarkMoon.getInstance().getModuleManager().getModule(InventoryMove.class)
                                    .isEnabled() && InventoryMove.safe.get())) {
                                return;
                            }
