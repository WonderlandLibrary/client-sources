...
import dev.darkmoon.client.module.impl.util.PasswordHider;
...
public class GuiChat extends GuiScreen implements ITabCompleter
{
    ...
    public void onGuiClosed()
    {
        ...
        Timer timerModule = DarkMoon.getInstance().getModuleManager().timer;
        DragManager.getDraggables().values().forEach(dragging -> {
            if (dragging.getModule().isEnabled() || dragging.getModule().equals(timerModule) && Hud.elements.get(4)) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        Vec2i scale = DarkMoon.getInstance().getScaleMath().getMouse(mouseX, mouseY, new ScaledResolution(mc));
        Timer timerModule = DarkMoon.getInstance().getModuleManager().timer;
        DragManager.getDraggables().values().forEach(dragging -> {
            if (dragging.getModule().isEnabled() || dragging.getModule().equals(timerModule) && Hud.elements.get(4)) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (DarkMoon.getInstance().getModuleManager().getModule(PasswordHider.class).isEnabled()) {
            if (((inputField.getText().startsWith("/l") && inputField.getText().split(" ")[0].length() <= 6) || (inputField.getText().startsWith("/reg") && inputField.getText().split(" ")[0].length() <= 9) || (inputField.getText().startsWith("/warp") && inputField.getText().split(" ")[0].length() <= 9)) && inputField.getText().split(" ").length != 1) {
                BlurUtility.drawBlur(10, () -> RenderUtility.drawRectNoWH(mc.fontRenderer.getStringWidth(inputField.getText().split(" ")[0]) + 7, this.height - 14, mc.fontRenderer.getStringWidth(inputField.getText().split(" ")[0]) + 6 + mc.fontRenderer.getStringWidth(inputField.getText().substring(inputField.getText().split(" ")[0].length())), this.height - 2, Integer.MIN_VALUE));
            }
        }
