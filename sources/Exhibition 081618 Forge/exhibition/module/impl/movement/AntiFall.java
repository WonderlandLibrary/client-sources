         ...
         label65: {
            ...
            if ((!this.isBlockUnder() || this.saveMe || !((Module)Client.getModuleManager().get(NoFall.class)).isEnabled()) && !mc.thePlayer.capabilities.isFlying) {
               if ((!this.saveMe || !this.timer.delay(250.0F)) && !mc.thePlayer.isCollidedVertically) {
                  ...
                  if (mc.thePlayer.fallDistance >= (float)dist && mc.theWorld.getBlockState(new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 1.0D, mc.thePlayer.posZ)).getBlock() == Blocks.air && (!((Boolean)((Setting)this.settings.get(this.VOID)).getValue()).booleanValue() || !this.isBlockUnder())) {
                     if (!this.saveMe) {
                        this.saveMe = true;
                        this.timer.reset();
                     } else {
