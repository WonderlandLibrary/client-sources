...
extends Module {
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof EventRender3D) {
            try {
                this.updatePositions(((EventRender3D) event).renderPartialTicks);
            }
                ...
                if ((((Boolean)((Setting)this.settings.get("INVISIBLES")).getValue()).booleanValue() || !ent.isInvisible()) && ent instanceof EntityPlayer && !(ent instanceof EntityPlayerSP)) {
                    try {
                        ...
                        if (!((Module)Client.getModuleManager().get(Tags.class)).isEnabled() && !((Module)Client.getModuleManager().get(Nametags.class)).isEnabled() && ((Boolean)((Setting)this.settings.get(this.NAME)).getValue()).booleanValue()) {
                            ...
                            font.drawStringWithShadow(renderName, (float)(x + (double)meme2) / 2.0f, (float)(y - (double)(font.getHeight(renderName) / 1.5f * 2.0f)) / 2.0f - 3.0f, FriendManager.isFriend(ent.getName()) ? Colors.getColor(temp.red, temp.green, temp.blue) : TargetESP.isPriority(ent) ? Colors.getColor(255, 255, 0) : -1);
                            GlStateManager.popMatrix();
                        }
    ...
    private void updatePositions(final float pt) {
