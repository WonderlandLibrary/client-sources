...
public class Tags extends Module {
      ...
      if (event instanceof EventRender3D) {
         ...
         while(var3.hasNext()) {
            ...
            if (!player.isInvisible() && !(player instanceof EntityPlayerSP)) {
               double x = player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)er.renderPartialTicks - Wrapper.getRenderPosX();
               double y = player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)er.renderPartialTicks - Wrapper.getRenderPosY();
               double z = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)er.renderPartialTicks - Wrapper.getRenderPosZ();
   ...
   public void renderNametag(EntityPlayer player, double x, double y, double z) {
      ...
      GL11.glDisable(3553);
      GL11.glDisable(2929);
      //mc.entityRenderer.setupCameraTransform(Wrapper.getPartialTicks(), 0);
      ...
      GlStateManager.rotate(Wrapper.getMinecraft().getRenderManager().playerViewX, var10001, 0.0F, 0.0F);
   ...
   private void renderEnchantText(ItemStack stack, int x, int y) {
      ...
      if (stack.getEnchantmentTagList() != null && stack.getEnchantmentTagList().tagCount() >= 6) {
         ...
         if (stack.getItem() instanceof ItemArmor) {
            ...
            efficiencyLevel = EnchantmentHelper.getEnchantmentLevel(Enchantment.projectileProtection.effectId, stack);
