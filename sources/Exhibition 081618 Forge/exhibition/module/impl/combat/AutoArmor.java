...
public class AutoArmor extends Module {
    ...
    public void onEvent(Event event) {
        String mode = ((Options) ((Setting) settings.get(MODE)).getValue()).getSelected();
        this.setSuffix(mode);
    	long delay = ((Number) ((Setting) settings.get(DELAY)).getValue()).longValue()*50;
    	...
    	for(int type = 1; type < 5; type++){
    		if(mc.thePlayer.inventoryContainer.getSlot(4 + type).getHasStack()){
    			...
    			if(isBestArmor(is, type)){
    				...
    				if( ((Options) ((Setting) settings.get(MODE)).getValue()).getSelected().equalsIgnoreCase("FakeInv")){
    		...
    		for (int i = 9; i < 45; i++) {
    			if (mc.thePlayer.inventoryContainer.getSlot(i).getHasStack()) {
    				...
    				if(isBestArmor(is, type) && getProtection(is) > 0){
    					shiftClick(i);
    					timer.reset();
    					if(((Number) ((Setting) settings.get(DELAY)).getValue()).longValue() > 0)
    						return;
    				}
    			}
            }
