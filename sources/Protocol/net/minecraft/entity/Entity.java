...
public abstract class Entity implements ICommandSender {
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			double var15 = y;
			double var17 = z;
			boolean var19 = this.onGround && (this.isSneaking() || Protocol.safewalk.isToggled()) && this instanceof EntityPlayer;
			...
			if (canStep) {
				...
				double var33 = y;
				AxisAlignedBB var36;
				if (Protocol.stepModule.isToggled() && Protocol.stepModule.latest.getValue()) {
					Wrapper.mc().timer.timerSpeed = 0.3f;
				}
				...
				if (var33 <= 1 && Protocol.stepModule.isToggled() && Protocol.stepModule.latest.getValue()) {
					Wrapper.getPlayer().sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(Wrapper.getPlayer().posX, Wrapper.getPlayer().posY + 0.42D, Wrapper.getPlayer().posZ, Wrapper.getPlayer().onGround));
					Wrapper.getPlayer().sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(Wrapper.getPlayer().posX, Wrapper.getPlayer().posY + 0.75D, Wrapper.getPlayer().posZ, Wrapper.getPlayer().onGround));
				}
				this.func_174826_a(this.getEntityBoundingBox().offset(0.0D, y, 0.0D));
				...
				if (stepHeight == 1 && Protocol.stepModule.isToggled() && Protocol.stepModule.latest.getValue()) {
			...
			this.onGround = this.isCollidedVertically && var15 < 0.0D || Protocol.flyModule.isToggled();
			...
			this.func_180433_a(y, this.onGround || (Protocol.speed.isToggled() && Protocol.speed.yport.getValue()), var60, var26);
			...
			if (this.worldObj.func_147470_e(this.getEntityBoundingBox().contract(0.001D, 0.001D, 0.001D))) {
				...
				if (!var62 && !Protocol.jesus.isToggled()) {
					++this.fire;
