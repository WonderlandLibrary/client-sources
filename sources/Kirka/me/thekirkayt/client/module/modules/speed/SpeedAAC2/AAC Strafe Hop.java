...
public class AAC Strafe Hop
extends SpeedMode {
    private double moveSpeed;
    private double lastDist;
    public static double stage;

    public AAC Strafe Hop(String string, boolean bl, Module module) {
    ...
    public boolean enable() {
        if (super.enable()) {
            if (ClientUtils.player() != null) {
                this.moveSpeed = SpeedAAC2.getBaseMoveSpeed();
            }
            this.lastDist = 0.3;
            stage = 4.0;
        }
    ...
    public boolean onMove(MoveEvent moveEvent) {
        if (super.onMove(moveEvent)) {
            Object object;
            if (MathUtils.roundToPlace(ClientUtils.player().posY - (double)((int)ClientUtils.player().posY), 3) == MathUtils.roundToPlace(0.1, 3)) {
            ...
            if (stage == 2.0 && (ClientUtils.player().moveForward != 0.0f || ClientUtils.player().moveStrafing != 0.0f)) {
                ...
                double d = 0.66 * (this.lastDist - SpeedAAC2.getBaseMoveSpeed());
            ...
            this.moveSpeed = Math.max(this.moveSpeed, SpeedAAC2.getBaseMoveSpeed());
