...
public class Mini YPort (Latest NCP)
extends SpeedMode {
    private double moveSpeed;
    private double lastDist;
    public static double stage;

    public Mini YPort (Latest NCP)(String string, boolean bl, Module module) {
    ...
    public boolean onMove(MoveEvent moveEvent) {
        if (super.onMove(moveEvent)) {
            Object object;
            if (MathUtils.roundToPlace(ClientUtils.player().posY - (double)((int)ClientUtils.player().posY), 3) == MathUtils.roundToPlace(1.0, 3)) {
                ...
                moveEvent.setY(moveEvent.getY() - 120.0);
            ...
            if (stage == 2.0 && (ClientUtils.player().moveForward != 0.0f || ClientUtils.player().moveStrafing != 0.0f)) {
                ClientUtils.player().motionY = 0.200067677007708;
                moveEvent.setY(0.200067677007708);
                this.moveSpeed *= 0.1;
            } else if (stage == 3.0) {
                double d = 0.77 * (this.lastDist - NewSpeedNCP2.getBaseMoveSpeed());
                ...
                this.moveSpeed = this.lastDist - this.lastDist / 400.444;
            }
