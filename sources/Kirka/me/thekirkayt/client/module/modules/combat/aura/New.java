...
extends AuraMode {
    ...
    public New(String name, boolean value, Module module) {
        ...
        if (super.onUpdate(event) && event.getState().equals((Object)Event.State.PRE)) {
            ...
            if (ClientUtils.player().isEntityAlive()) {
                ...
                if (this.target != null) {
                    ...
                    double dist = Math.sqrt(xDist * xDist + (yDist = (double)RotationUtils.getDistanceBetweenAngles(pitch, ClientUtils.player().rotationPitch % 360.0f)) * yDist);
                    if (dist > auraModule.degrees) {
                        this.target = null;
                    }
                }
                if (ClientUtils.mc().objectMouseOver.entityHit != null && auraModule.isEntityValidType(hit = ClientUtils.mc().objectMouseOver.entityHit)) {
    ...
    private void loadEntity() {
        ...
        for (Entity e : ClientUtils.loadedEntityList()) {
            ...
            float pitch = (float)(Math.atan2(h, h2) * 180.0 / 3.141592653589793);
            double xDist = RotationUtils.getDistanceBetweenAngles(yaw, ClientUtils.player().rotationYaw % 360.0f);
            double angleDistance = Math.sqrt(xDist * xDist + (yDist = (double)RotationUtils.getDistanceBetweenAngles(pitch, ClientUtils.player().rotationPitch % 360.0f)) * yDist);
            if (angleDistance > auraModule.degrees || (double)ClientUtils.player().getDistanceToEntity(e) >= distance) continue;
