...
public class TileEntitySkullRenderer extends TileEntitySpecialRenderer<TileEntitySkull>
{
    ...
    @Override
    public void render(final TileEntitySkull te, final double x, final double y, final double z, final float partialTicks, final int destroyStage, final float alpha) {
        final EnumFacing enumfacing = EnumFacing.byIndex(te.getBlockMetadata() & 0x7);
        final float f = te.getAnimationProgress(partialTicks);
        this.renderSkull((float)x, (float)y, (float)z, enumfacing, te.getSkullRotation() * 360 / 16.0f, te.getSkullType(), te.getPlayerProfile(), destroyStage, f);
    }
    ...
    public void renderSkull(final float x, final float y, final float z, final EnumFacing facing, float rotationIn, final int skullType, @Nullable final GameProfile profile, final int destroyStage, final float animateTicks) {
