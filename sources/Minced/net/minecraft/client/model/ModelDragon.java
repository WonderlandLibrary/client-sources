...
public class ModelDragon extends ModelBase
{
    ...
    public void render(final Entity entityIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        ...
        this.jaw.rotateAngleX = (float)(Math.sin(f * 6.2831855f) + 1.0) * 0.2f;
        float f2 = (float)(Math.sin(f * 6.2831855f - 1.0f) + 1.0);
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            this.spine.rotateAngleY = this.updateRotations(adouble2[0] - adouble[0]) * 0.017453292f * 1.5f;
            this.spine.rotateAngleX = f10 + entitydragon.getHeadPartYOffset(i, adouble, adouble2) * 0.017453292f * 1.5f * 5.0f;
            this.spine.rotateAngleZ = -this.updateRotations(adouble2[0] - f7) * 0.017453292f * 1.5f;
        ...
        this.head.rotateAngleY = this.updateRotations(adouble3[0] - adouble[0]) * 0.017453292f;
        this.head.rotateAngleX = this.updateRotations(entitydragon.getHeadPartYOffset(6, adouble, adouble3)) * 0.017453292f * 1.5f * 5.0f;
        this.head.rotateAngleZ = -this.updateRotations(adouble3[0] - f7) * 0.017453292f;
        ...
        float f12 = -(float)Math.sin(f * 6.2831855f) * 0.0f;
        f8 = f * 6.2831855f;
        f3 = 10.0f;
        f9 = 60.0f;
        f4 = 0.0f;
        ...
        for (int k = 0; k < 12; ++k) {
            ...
            this.spine.rotateAngleY = (this.updateRotations(adouble3[0] - adouble[0]) * 1.5f + 180.0f) * 0.017453292f;
            this.spine.rotateAngleX = f12 + (float)(adouble3[1] - adouble[1]) * 0.017453292f * 1.5f * 5.0f;
            this.spine.rotateAngleZ = this.updateRotations(adouble3[0] - f7) * 0.017453292f * 1.5f;
            this.spine.rotationPointY = f3;
            this.spine.rotationPointZ = f9;
            this.spine.rotationPointX = f4;
            f3 += (float)(Math.sin(this.spine.rotateAngleX) * 10.0);
