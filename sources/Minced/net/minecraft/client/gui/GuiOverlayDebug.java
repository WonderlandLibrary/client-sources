            ...
            if (this.mc.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256) {
                if (!chunk.isEmpty()) {
                    ...
                    if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null) {
                        final PlayerList playerList = this.mc.getIntegratedServer().getPlayerList();
                        final Minecraft mc = this.mc;
                        final EntityPlayerMP entityplayermp = playerList.getPlayerByUUID(Minecraft.player.getUniqueID());
                        if (entityplayermp != null) {
                            final DifficultyInstance difficultyinstance2 = this.mc.getIntegratedServer().getDifficultyAsync(entityplayermp.world, new BlockPos(entityplayermp));
    ...
    protected <T extends Comparable<T>> List<String> getDebugInfoRight() {
        ...
        if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK && this.mc.objectMouseOver.getBlockPos() != null) {
            ...
            for (final Map.Entry<IProperty<?>, Comparable<?>> entry : iblockstate.getProperties().entrySet()) {
                ...
                String s2 = iproperty.getName(t);
                if (Boolean.TRUE.equals(t)) {
                    s2 = TextFormatting.GREEN + s2;
                }
                else if (Boolean.FALSE.equals(t)) {
                    s2 = TextFormatting.RED + s2;
                }
                list.add(iproperty.getName() + ": " + s2);
            }
        }
        return list;
    }
