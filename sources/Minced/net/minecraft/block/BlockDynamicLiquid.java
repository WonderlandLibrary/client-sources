...
public class BlockDynamicLiquid extends BlockLiquid
{
    ...
    private void placeStaticBlock(final World worldIn, final BlockPos pos, final IBlockState currentState) {
        worldIn.setBlockState(pos, BlockLiquid.getStaticBlock(this.material).getDefaultState().withProperty((IProperty<Comparable>)BlockDynamicLiquid.LEVEL, (Comparable)currentState.getValue((IProperty<V>)BlockDynamicLiquid.LEVEL)), 2);
    }
    ...
    public void updateTick(final World worldIn, final BlockPos pos, IBlockState state, final Random rand) {
        ...
        if (i > 0) {
            ...
            if (this.adjacentSourceBlocks >= 2 && this.material == Material.WATER) {
                ...
                else if (iblockstate.getMaterial() == this.material && iblockstate.getValue((IProperty<Integer>)BlockDynamicLiquid.LEVEL) == 0) {
                    i2 = 0;
                }
            }
            if (this.material == Material.LAVA && i < 8 && i2 < 8 && i2 > i && rand.nextInt(4) != 0) {
                k *= 4;
            }
            if (i2 == i) {
    ...
    private int getSlopeDistance(final World worldIn, final BlockPos pos, final int distance, final EnumFacing calculateFlowCost) {
        ...
        for (final EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            if (enumfacing != calculateFlowCost) {
                ...
                if (this.isBlocked(worldIn, blockpos, iblockstate) || (iblockstate.getMaterial() == this.material && iblockstate.getValue((IProperty<Integer>)BlockDynamicLiquid.LEVEL) <= 0)) {
                    continue;
                }
    ...
    private boolean isBlocked(final World worldIn, final BlockPos pos, final IBlockState state) {
        ...
        return block instanceof BlockDoor || block == Blocks.STANDING_SIGN || block == Blocks.LADDER || block == Blocks.REEDS || block.material == Material.PORTAL || block.material == Material.STRUCTURE_VOID || block.material.blocksMovement();
    }
