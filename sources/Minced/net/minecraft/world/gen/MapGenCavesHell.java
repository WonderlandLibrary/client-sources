...
public class MapGenCavesHell extends MapGenBase
{
    ...
    protected void addRoom(final long p_180705_1_, final int p_180705_3_, final int p_180705_4_, final ChunkPrimer p_180705_5_, final double p_180705_6_, final double p_180705_8_, final double p_180705_10_) {
        this.addTunnel(p_180705_1_, p_180705_3_, p_180705_4_, p_180705_5_, p_180705_6_, p_180705_8_, p_180705_10_, 1.0f + this.rand.nextFloat() * 6.0f, 0.0f, 0.0f, -1, -1, 0.5);
    }
    
    protected void addTunnel(final long p_180704_1_, final int p_180704_3_, final int p_180704_4_, final ChunkPrimer p_180704_5_, double p_180704_6_, double p_180704_8_, double p_180704_10_, final float p_180704_12_, float p_180704_13_, float p_180704_14_, int p_180704_15_, int p_180704_16_, final double p_180704_17_) {
        ...
        while (p_180704_15_ < p_180704_16_) {
            final double d3 = 1.5 + MathHelper.sin(p_180704_15_ * 3.1415927f / p_180704_16_) * p_180704_12_;
            ...
            if (!flag1 && p_180704_15_ == j && p_180704_12_ > 1.0f) {
                this.addTunnel(random.nextLong(), p_180704_3_, p_180704_4_, p_180704_5_, p_180704_6_, p_180704_8_, p_180704_10_, random.nextFloat() * 0.5f + 0.5f, p_180704_13_ - 1.5707964f, p_180704_14_ / 3.0f, p_180704_15_, p_180704_16_, 1.0);
                this.addTunnel(random.nextLong(), p_180704_3_, p_180704_4_, p_180704_5_, p_180704_6_, p_180704_8_, p_180704_10_, random.nextFloat() * 0.5f + 0.5f, p_180704_13_ + 1.5707964f, p_180704_14_ / 3.0f, p_180704_15_, p_180704_16_, 1.0);
                return;
            }
            if (flag1 || random.nextInt(4) != 0) {
                ...
                if (p_180704_6_ >= d0 - 16.0 - d3 * 2.0 && p_180704_10_ >= d2 - 16.0 - d3 * 2.0 && p_180704_6_ <= d0 + 16.0 + d3 * 2.0 && p_180704_10_ <= d2 + 16.0 + d3 * 2.0) {
                    int j2 = MathHelper.floor(p_180704_6_ - d3) - p_180704_3_ * 16 - 1;
                    int k = MathHelper.floor(p_180704_6_ + d3) - p_180704_3_ * 16 + 1;
                    int k2 = MathHelper.floor(p_180704_8_ - d4) - 1;
                    int l = MathHelper.floor(p_180704_8_ + d4) + 1;
                    int l2 = MathHelper.floor(p_180704_10_ - d3) - p_180704_4_ * 16 - 1;
                    int i2 = MathHelper.floor(p_180704_10_ + d3) - p_180704_4_ * 16 + 1;
                    if (j2 < 0) {
                        j2 = 0;
                    }
                    if (k > 16) {
                                ...
                                for (int i4 = l; i4 > k2; --i4) {
                                    ...
                                    if (d11 > -0.7 && d9 * d9 + d11 * d11 + d10 * d10 < 1.0) {
                                        ...
                                        if (iblockstate2.getBlock() == Blocks.NETHERRACK || iblockstate2.getBlock() == Blocks.DIRT || iblockstate2.getBlock() == Blocks.GRASS) {
                                            p_180704_5_.setBlockState(i3, i4, j4, MapGenCavesHell.AIR);
                                        }
                                    }
                                }
                            }
    ...
    protected void recursiveGenerate(final World worldIn, final int chunkX, final int chunkZ, final int originalX, final int originalZ, final ChunkPrimer chunkPrimerIn) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                this.addTunnel(this.rand.nextLong(), originalX, originalZ, chunkPrimerIn, d0, d2, d3, f3 * 2.0f, f, f2, 0, 0, 0.5);
            }
        }
    }
