...
public class GenLayerRiver extends GenLayer
{
    ...
    public int[] getInts(final int areaX, final int areaY, final int areaWidth, final int areaHeight) {
        ...
        for (int i2 = 0; i2 < areaHeight; ++i2) {
            for (int j2 = 0; j2 < areaWidth; ++j2) {
                final int k2 = this.riverFilter(aint[j2 + 0 + (i2 + 1) * k]);
                final int l2 = this.riverFilter(aint[j2 + 2 + (i2 + 1) * k]);
                final int i3 = this.riverFilter(aint[j2 + 1 + (i2 + 0) * k]);
                final int j3 = this.riverFilter(aint[j2 + 1 + (i2 + 2) * k]);
                final int k3 = this.riverFilter(aint[j2 + 1 + (i2 + 1) * k]);
                if (k3 == k2 && k3 == i3 && k3 == l2 && k3 == j3) {
                    aint2[j2 + i2 * areaWidth] = -1;
                }
                else {
                    aint2[j2 + i2 * areaWidth] = Biome.getIdForBiome(Biomes.RIVER);
                }
            }
        }
        return aint2;
    ...
    private int riverFilter(final int p_151630_1_) {
