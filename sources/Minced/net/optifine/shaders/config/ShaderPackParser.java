...
public class ShaderPackParser
{
    ...
    public static Map<String, IExpressionBool> parseProgramConditions(final Properties props, final ShaderOption[] shaderOptions) {
        ...
        for (final Object s2 : props.keySet()) {
            ...
            if (matcher.matches()) {
                ...
                final IExpressionBool iexpressionbool = parseOptionExpression(s5, shaderOptions);
                if (iexpressionbool == null) {
                    SMCLog.severe("Error parsing program condition: " + s3);
                }
                else {
                    map.put(s4, iexpressionbool);
                }
            }
        }
        return map;
    ...
    public static CustomUniforms parseCustomUniforms(final Properties props) {
        ...
        for (final Object s5 : props.keySet()) {
            final String s6 = (String)s5;
            final String[] astring = Config.tokenize(s6, ".");
            if (astring.length == 3) {
                final String s7 = astring[0];
                final String s8 = astring[1];
                final String s9 = astring[2];
                final String s10 = props.getProperty(s6).trim();
                if (map.containsKey(s9)) {
                    SMCLog.warning("Expression already defined: " + s9);
                }
                else {
                    if (!s7.equals(s) && !s7.equals(s2)) {
                        continue;
                    }
                    SMCLog.info("Custom " + s7 + ": " + s9);
                    final CustomUniform customuniform = parseCustomUniform(s7, s9, s8, s10, map);
                    if (customuniform == null) {
                        continue;
                    }
                    map.put(s9, customuniform.getExpression());
                    if (s7.equals(s2)) {
                        continue;
                    }
