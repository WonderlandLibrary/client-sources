...
public class SVertexBuilder
{
    ...
    public static void initVertexBuilder(final BufferBuilder wrr) {
    ...
    public static void pushEntity(final IBlockState blockState, final BlockPos blockPos, final IBlockAccess blockAccess, final BufferBuilder wrr) {
        ...
        final int j2 = BlockAliases.getBlockAliasId(i, j);
        if (j2 >= 0) {
            i = j2;
        }
        final int k = block.getRenderType(blockState).ordinal();
        final int l = ((k & 0xFFFF) << 16) + (i & 0xFFFF);
        final int i2 = j & 0xFFFF;
        wrr.sVertexBuilder.pushEntity(((long)i2 << 32) + l);
    }
    
    public static void popEntity(final BufferBuilder wrr) {
        wrr.sVertexBuilder.popEntity();
    }
    
    public static boolean popEntity(final boolean value, final BufferBuilder wrr) {
        wrr.sVertexBuilder.popEntity();
        return value;
    }
    
    public static void endSetVertexFormat(final BufferBuilder wrr) {
    ...
    public static void beginAddVertex(final BufferBuilder wrr) {
        if (wrr.vertexCount == 0) {
            endSetVertexFormat(wrr);
        }
    }
    
    public static void endAddVertex(final BufferBuilder wrr) {
        final SVertexBuilder svertexbuilder = wrr.sVertexBuilder;
        if (svertexbuilder.vertexSize == 14) {
            ...
            final int j = wrr.getBufferSize() - 14 + 12;
            wrr.rawIntBuffer.put(j, (int)i);
            wrr.rawIntBuffer.put(j + 1, (int)(i >> 32));
        }
    }
    
    public static void beginAddVertexData(final BufferBuilder wrr, final int[] data) {
        if (wrr.vertexCount == 0) {
            endSetVertexFormat(wrr);
        }
        final SVertexBuilder svertexbuilder = wrr.sVertexBuilder;
    ...
    public static void beginAddVertexData(final BufferBuilder wrr, final ByteBuffer byteBuffer) {
        if (wrr.vertexCount == 0) {
            endSetVertexFormat(wrr);
        }
        final SVertexBuilder svertexbuilder = wrr.sVertexBuilder;
        if (svertexbuilder.vertexSize == 14) {
            final long i = svertexbuilder.entityData[svertexbuilder.entityDataIndex];
            for (int j = byteBuffer.limit() / 4, k = 12; k + 1 < j; k += 14) {
                final int l = (int)i;
                final int i2 = (int)(i >> 32);
                byteBuffer.putInt(k * 4, l);
                byteBuffer.putInt((k + 1) * 4, i2);
            }
        }
    }
    
    public static void endAddVertexData(final BufferBuilder wrr) {
        final SVertexBuilder svertexbuilder = wrr.sVertexBuilder;
    ...
    public void calcNormal(final BufferBuilder wrr, final int baseIndex) {
        final FloatBuffer floatbuffer = wrr.rawFloatBuffer;
        final IntBuffer intbuffer = wrr.rawIntBuffer;
        final int i = wrr.getBufferSize();
    ...
    public static void calcNormalChunkLayer(final BufferBuilder wrr) {
    ...
    public static void drawArrays(final int drawMode, final int first, final int count, final BufferBuilder wrr) {
        if (count != 0) {
            final VertexFormat vertexformat = wrr.getVertexFormat();
