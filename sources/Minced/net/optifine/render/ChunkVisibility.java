...
public class ChunkVisibility
{
    ...
    public static int getMaxChunkY(final World world, final Entity viewEntity, final int renderDistanceChunks) {
        final int i = MathHelper.floor(viewEntity.posX) >> 4;
        final int j = MathHelper.floor(viewEntity.posY) >> 4;
        final int k = MathHelper.floor(viewEntity.posZ) >> 4;
        final Chunk chunk = world.getChunk(i, k);
        ...
        for (int i3 = l; i3 < i2; ++i3) {
            for (int j3 = j2; j3 < k2; ++j3) {
                final Chunk chunk2 = world.getChunk(i3, j3);
                if (!chunk2.isEmpty()) {
