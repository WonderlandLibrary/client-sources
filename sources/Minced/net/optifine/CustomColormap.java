...
public class CustomColormap implements CustomColors.IColorizer
{
    ...
    public int getColor(final Biome biome, final BlockPos blockPos) {
        if (this.format == 0) {
    ...
    private int getColorVanilla(final Biome biome, final BlockPos blockPos) {
        final double d0 = MathHelper.clamp(biome.getTemperature(blockPos), 0.0f, 1.0f);
        double d2 = MathHelper.clamp(biome.getRainfall(), 0.0f, 1.0f);
        d2 *= d0;
    ...
    private int getColorGrid(final Biome biome, final BlockPos blockPos) {
        final int i = Biome.getIdForBiome(biome);
