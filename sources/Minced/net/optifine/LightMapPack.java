...
public class LightMapPack
{
    ...
    public boolean updateLightmap(final World world, final float torchFlickerX, final int[] lmColors, final boolean nightvision, final float partialTicks) {
        ...
        if (f4 > f3 && this.lightMap.updateLightmap(world, torchFlickerX, aint[i], nightvision)) {
            afloat[i] = f4;
            ++i;
        }
        if (f5 > f3 && this.lightMapRain != null && this.lightMapRain.updateLightmap(world, torchFlickerX, aint[i], nightvision)) {
            afloat[i] = f5;
            ++i;
        }
        if (f2 > f3 && this.lightMapThunder != null && this.lightMapThunder.updateLightmap(world, torchFlickerX, aint[i], nightvision)) {
            afloat[i] = f2;
            ++i;
        }
        if (i == 2) {
