...
public class CustomItems
{
    ...
    private static CustomItemProperties[][] propertyListToArray(final List list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (subList != null) {
                final CustomItemProperties[] acustomitemproperties2 = subList.toArray(new CustomItemProperties[subList.size()]);
    ...
    private static void addToList(final CustomItemProperties cp, final List list, final int id) {
        ...
        ArrayList<CustomItemProperties> subList = list.get(id);
        if (subList == null) {
    ...
    public static boolean bindCustomArmorTexture(final ItemStack itemStack, final EntityEquipmentSlot slot, final String overlay) {
        if (CustomItems.itemProperties == null) {
            return false;
        }
        final ResourceLocation resourcelocation = getCustomArmorLocation(itemStack, slot, overlay);
    ...
    private static ResourceLocation getCustomArmorLocation(final ItemStack itemStack, final EntityEquipmentSlot slot, final String overlay) {
        ...
        final int i = (slot == EntityEquipmentSlot.LEGS) ? 2 : 1;
        final StringBuffer stringbuffer = new StringBuffer();
    ...
    public static ResourceLocation getCustomElytraTexture(final ItemStack itemStack, final ResourceLocation locElytra) {
        if (CustomItems.itemProperties == null) {
            return locElytra;
        }
        final CustomItemProperties customitemproperties = getCustomItemProperties(itemStack, 4);
        if (customitemproperties == null) {
            return locElytra;
        }
    ...
    private static int getItemStackDamage(final ItemStack itemStack) {
        final Item item = itemStack.getItem();
        return (item instanceof ItemPotion) ? getPotionDamage(itemStack) : itemStack.getItemDamage();
    }
    
    private static int getPotionDamage(final ItemStack itemStack) {
        final NBTTagCompound nbttagcompound = itemStack.getTagCompound();
    ...
    private static void addPotion(final String name, int value, final boolean extended, final Map<String, Integer> map) {
        ...
        if (extended) {
            final int i = value | 0x20;
            map.put("minecraft:strong_" + name, i);
            final int j = value | 0x40;
