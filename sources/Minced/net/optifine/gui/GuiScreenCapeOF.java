...
public class GuiScreenCapeOF extends GuiScreenOF
{
    ...
    protected void actionPerformed(final GuiButton button) {
        if (button.enabled) {
            if (button.id == 200) {
                GuiScreenCapeOF.mc.displayGuiScreen(this.parentScreen);
            }
            if (button.id == 210) {
                try {
                    final String s = GuiScreenCapeOF.mc.getSession().getProfile().getName();
                    final String s2 = GuiScreenCapeOF.mc.getSession().getProfile().getId().toString().replace("-", "");
                    final String s3 = GuiScreenCapeOF.mc.getSession().getToken();
                    ...
                    GuiScreenCapeOF.mc.getSessionService().joinServer(GuiScreenCapeOF.mc.getSession().getProfile(), s3, s4);
            ...
            if (button.id == 220) {
                ...
                final Minecraft mc = GuiScreenCapeOF.mc;
                if (Minecraft.player != null) {
                    final long i = 15000L;
                    final long j = System.currentTimeMillis() + i;
                    final Minecraft mc2 = GuiScreenCapeOF.mc;
                    Minecraft.player.setReloadCapeTimeMs(j);
                }
            }
