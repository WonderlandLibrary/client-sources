...
public class ConnectedParser
{
    ...
    public int[] parseBlockMetadatas(final Block block, final String[] params) {
        ...
        final Collection collection = iblockstate.getPropertyKeys();
    ...
    private IBlockState getStateFromMeta(final Block block, final int md) {
        try {
            ...
            if (block == Blocks.OBSERVER && (md & 0x8) != 0x0) {
                iblockstate = iblockstate.withProperty((IProperty<Comparable>)BlockObserver.POWERED, true);
            }
            return iblockstate;
        }
    ...
    public static Comparable getPropertyValue(final String value, final Collection propertyValues) {
        for (final Object comparable1 : propertyValues) {
            final Comparable comparable2 = (Comparable)comparable1;
    ...
    public BlockRenderLayer parseBlockRenderLayer(String str, final BlockRenderLayer def) {
        if (str == null) {
            return def;
        }
    ...
    private static int[] parseCareerIds(final int prof, final String str) {
        final IntSet intset = (IntSet)new IntArraySet();
        final String[] astring = Config.tokenize(str, ",");
        for (int i = 0; i < astring.length; ++i) {
            final String s = astring[i];
    ...
    public int[] parseEntities(String str) {
        ...
        for (int i = 0; i < astring.length; ++i) {
            final String s = astring[i];
            final ResourceLocation resourcelocation = new ResourceLocation(s);
            final Class oclass = this.getEntityClass(resourcelocation);
            if (oclass == null) {
                this.warn("Entity not found: " + s);
            }
            else {
                final int j = this.getEntityTypeId(oclass);
                if (j < 0) {
    ...
    private Class getEntityClass(final ResourceLocation loc) {
        return (Class)(Reflector.ForgeEntityList_getClass.exists() ? Reflector.call(Reflector.ForgeEntityList_getClass, loc) : ((Class<? extends Entity>)EntityList.REGISTRY.getObject(loc)));
    }
    
    private int getEntityTypeId(final Class type) {
