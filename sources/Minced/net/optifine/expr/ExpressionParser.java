...
public class ExpressionParser
{
    ...
    private IExpression[] parseExpressions(final Deque<Token> deque) throws ParseException {
        ...
        IExpression expr;
        while ((expr = this.parseInfix(dequeArg = getGroup(deque, TokenType.COMMA, false))) != null) {
            list.add(expr);
        }
    ...
    private static Deque<Token> getGroup(final Deque<Token> deque, final TokenType tokenTypeEnd, final boolean tokenEndRequired) throws ParseException {
        final ArrayDeque<Token> dequeGroup = new ArrayDeque<Token>();
        int level = 0;
        final Iterator<Token> it = deque.iterator();
        while (it.hasNext()) {
