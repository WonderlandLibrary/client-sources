...
public class RaycastUtility1
{
    ...
    public static boolean isLookingAtEntity(final float yaw, final float pitch, final float xExp, final float yExp, final float zExp, final Entity entity, final double range) {
        final Minecraft mc = RaycastUtility1.mc;
        final Vec3d src = Minecraft.player.getPositionEyes(1.0f);
        final Vec3d vectorForRotation = Entity.getVectorForRotation(pitch, yaw);
        final Vec3d dest = src.add(vectorForRotation.x * range, vectorForRotation.y * range, vectorForRotation.z * range);
        final RayTraceResult rayTraceResult = RaycastUtility1.mc.world.rayTraceBlocks(src, dest, false, false, true);
    ...
    public static RayTraceResult rayTrace(final double blockReachDistance, final float yaw, final float pitch, final boolean walls) {
        ...
        return RaycastUtility1.mc.world.rayTraceBlocks(vec3d, vec3d3, true, true, true);
    }
    ...
    static {
        RaycastUtility1.mc = Minecraft.getMinecraft();
    }
}
