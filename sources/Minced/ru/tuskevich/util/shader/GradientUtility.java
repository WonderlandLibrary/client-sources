...
public class GradientUtility implements Utility
{
    ...
    public static void applyGradientCornerRL(final float x, final float y, final float width, final float height, final float alpha, final Color bottomLeft, final Color topRight, final Runnable content) {
        final Color mixedColor = ColorUtility.interpolateColorC(topRight, bottomLeft, 0.5f);
    ...
    static {
        gradientMaskShader = new ShaderUtility("gradientMask.frag");
        gradientShader = new ShaderUtility("gradient.frag");
    }
}
