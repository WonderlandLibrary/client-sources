...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            double maxStartingReach = 3.0D;
            float expand = 0.0F;

            boolean isPlayer = entity.equals(mc.thePlayer);

            boolean hadNoVec3 = mc.objectMouseOver == null || mc.objectMouseOver.typeOfHit != MovingObjectPosition.MovingObjectType.ENTITY;

            if(isPlayer) {
                Reach reachModule = Flap.instance.getModuleManager().getModule(Reach.class);

                if(reachModule.isEnabled()) {
                    maxReach = reachModule.reach.getValue();
                    maxStartingReach = reachModule.reach.getValue();
                }
            }
            ...
            else if (d0 > (hadNoVec3 ? maxStartingReach : maxReach))
            {
                flag = true;
            }
            ...
            if(this.pointedEntity != null) {
                //Vestige.getInstance().addChatMessage("" + vec3.distanceTo(vec33));
            }

            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > (hadNoVec3 ? maxStartingReach : maxReach))
            {
                this.pointedEntity = null;
    ...
    public void orientCamera(float partialTicks)
    {
        ...
        CameraHandler cameraHandler = Flap.instance.getCameraHandler();
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Flap.instance.getEventManager().post(new Render3DEvent(partialTicks));
