...
public class GuiChat extends GuiScreen
{
    ...
    public void onGuiClosed()
    {
        ...
        for(HUDModule m : Flap.instance.getModuleManager().hudModules) {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        if(state == 0) {
            Flap.instance.getModuleManager().hudModules.forEach(m -> m.setHoldingMouse(false));
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        Flap.instance.getModuleManager().hudModules.forEach(m -> {
            double startX = m.posX.getValue();
            double startY = m.posY.getValue();

            if(m.isHoldingMouse()) {
                if(m.getAlignType() == AlignType.RIGHT) {
                    m.posX.setValue(startX - mouseX + lastMouseX);
                } else if(m.getAlignType() == AlignType.LEFT) {
                    m.posX.setValue(startX + mouseX - lastMouseX);
                }

                m.posY.setValue(startY + mouseY - lastMouseY);
            }
        });
