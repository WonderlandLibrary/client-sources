...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        ...
        Animations animationsModule = Flap.instance.getModuleManager().getModule(Animations.class);

        if(animationsModule.isEnabled()) {
            return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (int) ((this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6) * animationsModule.swingSlowdown.getValue() * Minecraft.getMinecraft().timer.timerSpeed);
        }
