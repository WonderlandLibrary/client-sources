...
public class VestigeFontRenderer extends CFont {
   ...
   public VestigeFontRenderer(Font font, boolean antiAlias, boolean fractionalMetrics) {
   ...
   public float drawStringWithShadow(String text, double x, double y, int color) {
      float shadowWidth = this.drawString(text, x + 1.0D, y + 0.5D, color, true);
      return Math.max(shadowWidth, this.drawString(text, x, y, color, false));
   }

   public float drawCenteredString(String text, float x, float y, int color) {
      return this.drawString(text, (double)x - this.getStringWidth(text) / 2.0D, (double)y, color);
   }

   public float drawCenteredString(String text, double x, double y, int color) {
      return this.drawString(text, x - this.getStringWidth(text) / 2.0D, y, color);
   }

   public float drawCenteredStringWithShadow(String text, float x, float y, int color) {
      this.drawString(text, (double)x - this.getStringWidth(text) / 2.0D + 0.5D, (double)y + 0.5D, color, true);
      return this.drawString(text, (double)x - this.getStringWidth(text) / 2.0D, (double)y, color);
   }
   ...
   public float drawCenteredStringWithShadow(String text, double x, double y, int color) {
      this.drawString(text, x - this.getStringWidth(text) / 2.0D + 0.5D, y + 0.5D, color, true);
      return this.drawString(text, x - this.getStringWidth(text) / 2.0D, y, color);
   }
         ...
         if (render) {
            ...
            for(int i = 0; i < size; ++i) {
               ...
               if (String.valueOf(character).equals("§") && i < size) {
                  ...
                  GL11.glEnd();
                  if (strikethrough) {
                     this.drawLine(x, y + (double)(currentData[character].height / 2), x + (double)currentData[character].width - this.horizontalSpacing, y + (double)(currentData[character].height / 2), 1.0F);
                  }

                  if (underline) {
                     this.drawLine(x, y + (double)currentData[character].height - 2.0D, x + (double)currentData[character].width - this.horizontalSpacing, y + (double)currentData[character].height - 2.0D, 1.0F);
                  }

                  x += (double)currentData[character].width - this.horizontalSpacing + (double)this.charOffset;
               }
            }
      ...
      if (text == null) {
         ...
         for(int i = 0; i < size; ++i) {
            ...
            if (String.valueOf(character).equals("§") && i < size) {
               ...
               width += (double)currentData[character].width - this.horizontalSpacing + (double)this.charOffset;
            }
         }
   ...
   public double getStringWidthCust(String text) {
      if (text == null) {
         ...
         return (width - (double)this.charOffset) / 2.0D;
      }
   }
