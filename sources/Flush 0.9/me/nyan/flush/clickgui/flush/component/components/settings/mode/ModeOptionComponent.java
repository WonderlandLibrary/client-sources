package me.nyan.flush.clickgui.flush.component.components.settings.mode;
...
public class ModeOptionComponent extends Component {
    ...
    public void draw(float x, float y, int mouseX, int mouseY, float partialTicks) {
        int targetColor = setting.getValue().equals(option) ?
                Flush.getInstance().getModuleManager().getModule(ModuleClickGui.class).getClickGUIColor()
                : 0xFF121212;
    ...
    public float getHeight() {
        return FlushPanel.TITLE_HEIGHT - 6;
    }
}
