...
public class ServerCommandManager extends CommandHandler implements IAdminCommand {
    ...
    public void notifyOperators(ICommandSender sender, ICommand command, int flags, String msgFormat, Object... msgParams) {
        ...
        IChatComponent chatComponent = new ChatComponentTranslation("chat.type.admin", sender.getName(), new ChatComponentTranslation(msgFormat, msgParams));
        chatComponent.getChatStyle().setColor(EnumChatFormatting.GRAY);
        chatComponent.getChatStyle().setItalic(Boolean.TRUE);

        if (flag) {
            for (EntityPlayer entityplayer : server.getConfigurationManager().func_181057_v()) {
                if (entityplayer != sender && server.getConfigurationManager().canSendCommands(entityplayer.getGameProfile()) && command.canCommandSenderUseCommand(sender)) {
        ...
        if (sender != server && server.worldServers[0].getGameRules().getBoolean("logAdminCommands"))
            server.addChatMessage(chatComponent);

        boolean flag3 = server.worldServers[0].getGameRules().getBoolean("sendCommandFeedback");
