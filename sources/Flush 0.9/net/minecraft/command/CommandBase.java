...
public abstract class CommandBase implements ICommand {
    ...
    public static int parseInt(String input, int min, int max) throws NumberInvalidException {
        ...
        if (i < min) throw new NumberInvalidException("commands.generic.num.tooSmall", i, min);
        else if (i > max) throw new NumberInvalidException("commands.generic.num.tooBig", i, max);
        else return i;
    }
    ...
    public static BlockPos parseBlockPos(ICommandSender sender, String[] args, int startIndex, boolean centerBlock) throws NumberInvalidException {
        ...
        return new BlockPos(
                parseDouble(blockpos.getX(), args[startIndex], -30000000, 30000000, centerBlock),
                parseDouble(blockpos.getY(), args[startIndex + 1], 0, 256, false),
                parseDouble(blockpos.getZ(), args[startIndex + 2], -30000000, 30000000, centerBlock)
        );
    }
    ...
    public static double parseDouble(String input) throws NumberInvalidException {
        try {
            ...
            if (!Doubles.isFinite(d0)) throw new NumberInvalidException("commands.generic.num.invalid", input);
            else return d0;
    ...
    public static double parseDouble(String input, double min, double max) throws NumberInvalidException {
        ...
        if (d0 < min) throw new NumberInvalidException("commands.generic.double.tooSmall", d0, min);
        else if (d0 > max) throw new NumberInvalidException("commands.generic.double.tooBig", d0, max);
        else return d0;
    }
    ...
    public static EntityPlayerMP getCommandSenderAsPlayer(ICommandSender sender) throws PlayerNotFoundException {
        ...
        else throw new PlayerNotFoundException("You must specify which player you wish to perform this action on.");
    }
    ...
    public static <T extends Entity> T getEntity(ICommandSender commandSender, String p_175759_1_, Class<? extends T> p_175759_2_) throws EntityNotFoundException {
        ...
        if (entity != null && p_175759_2_.isAssignableFrom(entity.getClass())) return (T) entity;
        else throw new EntityNotFoundException();
    }

    public static List<Entity> func_175763_c(ICommandSender sender, String p_175763_1_) throws EntityNotFoundException {
        return PlayerSelector.hasArguments(p_175763_1_) ? PlayerSelector.matchEntities(sender, p_175763_1_, Entity.class) : Lists.newArrayList(func_175768_b(sender, p_175763_1_));
    }
    ...
    public static String getPlayerName(ICommandSender sender, String query) throws PlayerNotFoundException {
        try {
            ...
            if (PlayerSelector.hasArguments(query)) throw playernotfoundexception;
            else return query;
        }
    }
    ...
    public static String getEntityName(ICommandSender p_175758_0_, String p_175758_1_) throws EntityNotFoundException {
        try {
            ...
            try {
                ...
                if (PlayerSelector.hasArguments(p_175758_1_)) throw entitynotfoundexception;
                else return p_175758_1_;
            }
        }
    }
    ...
    public static CommandBase.CoordinateArg parseCoordinate(double p_175767_0_, String p_175767_2_, int min, int max, boolean centerBlock) throws NumberInvalidException {
        ...
        if (flag && Double.isNaN(p_175767_0_))
            ...
            if (min != 0 || max != 0) {
                if (d0 < (double) min) throw new NumberInvalidException("commands.generic.double.tooSmall", d0, min);
                if (d0 > (double) max) throw new NumberInvalidException("commands.generic.double.tooBig", d0, max);
            }
    ...
    public static double parseDouble(double base, String input, int min, int max, boolean centerBlock) throws NumberInvalidException {
        ...
        if (flag && Double.isNaN(base)) {
            ...
            if (!flag || input.length() > 1) {
                ...
                if (flag) input = input.substring(1);
                d0 += parseDouble(input);
                if (!flag1 && !flag && centerBlock) d0 += 0.5D;
            }
    ...
    public static Item getItemByText(String id) throws NumberInvalidException {
        ...
        if (item == null) throw new NumberInvalidException("commands.give.item.notFound", resourcelocation);
        else return item;
    }
    ...
    public static Block getBlockByText(String id) throws NumberInvalidException {
        ...
        if (!Block.blockRegistry.containsKey(resourcelocation))
            ...
            if (block == null) throw new NumberInvalidException("commands.give.block.notFound", resourcelocation);
            else return block;
        }
    }
    ...
    public static String joinNiceString(Object[] elements) {
        ...
        for (int i = 0; i < elements.length; ++i) {
            ...
            if (i > 0) {
                if (i == elements.length - 1) stringbuilder.append(" and ");
    ...
    public static IChatComponent join(List<IChatComponent> components) {
        IChatComponent chatComponents = new ChatComponentText("");

        for (int i = 0; i < components.size(); ++i) {
            if (i > 0) {
                if (i == components.size() - 1) chatComponents.appendText(" and ");
                else chatComponents.appendText(", ");
            }

            chatComponents.appendSibling(components.get(i));
        }
    ...
    public static List<String> func_175771_a(String[] p_175771_0_, int p_175771_1_, BlockPos p_175771_2_) {
        if (p_175771_2_ == null) return null;
        else {
            ...
            if (i == p_175771_1_)
                ...
                if (i != p_175771_1_ + 2) return null;
    ...
    public static List<String> func_181043_b(String[] p_181043_0_, int p_181043_1_, BlockPos p_181043_2_) {
        if (p_181043_2_ == null) return null;
        else {
            ...
            if (i == p_181043_1_)
                ...
                if (i != p_181043_1_ + 1) return null;
