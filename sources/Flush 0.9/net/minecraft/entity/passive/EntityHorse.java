...
public class EntityHorse extends EntityAnimal implements IInvBasic
{
    ...
    public boolean prepareChunkForSpawn()
    {
        ...
        this.world.getBiomeGenForCoords(new BlockPos(i, 0, j));
        return true;
    }
    ...
    private void func_110266_cB()
    {
        ...
        if (!this.isSilent())
        {
            this.world.playSoundAtEntity(this, "eating", 1.0F, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F);
        }
    }
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (i > 0)
        {
            ...
            Block block = this.world.getBlockState(new BlockPos(this.posX, this.posY - 0.2D - (double)this.prevRotationYaw, this.posZ)).getBlock();

            if (block.getMaterial() != Material.air && !this.isSilent())
            {
                Block.SoundType block$soundtype = block.stepSound;
                this.world.playSoundAtEntity(this, block$soundtype.getStepSound(), block$soundtype.getVolume() * 0.5F, block$soundtype.getFrequency() * 0.75F);
            }
        }
    }
    ...
    protected EntityHorse getClosestHorse(Entity entityIn, double distance)
    {
        ...
        for (Entity entity1 : this.world.getEntitiesInAABBexcluding(entityIn, entityIn.getEntityBoundingBox().addCoord(distance, distance, distance), horseBreedingSelector))
        {
    ...
    {
        if (!this.world.isRemote && (this.riddenByEntity == null || this.riddenByEntity == playerEntity) && this.isTame())
        {
            this.horseChest.setCustomName(this.getName());
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.world.isRemote)
        {
            ...
            if (!this.isEatingHaystack() && this.riddenByEntity == null && this.rand.nextInt(300) == 0 && this.world.getBlockState(new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ))).getBlock() == Blocks.grass)
            {
    ...
    public void onUpdate()
    {
        ...
        if (this.world.isRemote && this.dataWatcher.hasObjectChanged())
        {
        ...
        if (!this.world.isRemote && this.jumpRearingCounter > 0 && ++this.jumpRearingCounter > 20)
        {
