...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.getInvulTime() > 0)
        {
            ...
            {
                this.world.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + (double)(this.rand.nextFloat() * 3.3F), this.posZ + this.rand.nextGaussian() * 1.0D, 0.699999988079071D, 0.699999988079071D, 0.8999999761581421D, new int[0]);
            }
        }
    }
    ...
    protected void updateAITasks()
    {
        if (this.getInvulTime() > 0)
        {
            ...
            if (j1 <= 0)
            {
                ...
                this.world.playBroadcastSound(1013, new BlockPos(this), 0);
            }
        ...
        else
        {
            ...
            for (int i = 1; i < 3; ++i)
            {
                if (this.ticksExisted >= this.field_82223_h[i - 1])
                {
                    ...
                    {
                        List<EntityLivingBase> list = this.world.<EntityLivingBase>getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D), Predicates.<EntityLivingBase> and (attackEntitySelector, EntitySelectors.NOT_SPECTATING));
            ...
            if (this.blockBreakCounter > 0)
            {
                ...
                if (this.blockBreakCounter == 0 && this.world.getGameRules().getBoolean("mobGriefing"))
                {
                    ...
                    if (flag)
                    {
                        this.world.playAuxSFXAtEntity((EntityPlayer)null, 1012, new BlockPos(this), 0);
                    }
                }
            }
