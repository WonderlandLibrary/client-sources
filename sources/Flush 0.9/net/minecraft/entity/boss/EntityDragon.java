...
public class EntityDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart, IMob
{
    ...
    public void onLivingUpdate()
    {
        if (this.world.isRemote)
        {
            ...
            {
                this.world.playSound(this.posX, this.posY, this.posZ, "mob.enderdragon.wings", 5.0F, 0.8F + this.rand.nextFloat() * 0.3F, false);
            }
        }
        ...
        if (this.getHealth() <= 0.0F)
        {
            ...
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double)f11, this.posY + 2.0D + (double)f13, this.posZ + (double)f14, 0.0D, 0.0D, 0.0D, new int[0]);
        }
        else
        {
    ...
    private void updateDragonEnderCrystal()
    {
        ...
        {
            float f = 32.0F;
            List<EntityEnderCrystal> list = this.world.<EntityEnderCrystal>getEntitiesWithinAABB(EntityEnderCrystal.class, this.getEntityBoundingBox().expand((double)f, (double)f, (double)f));
    ...
    private void setNewTarget()
    {
        ...
        List<EntityPlayer> list = Lists.newArrayList(this.world.playerEntities);
        ...
        if (!this.world.isRemote)
        {
            if (this.deathTicks > 150 && this.deathTicks % 5 == 0 && flag)
            {
                ...
                while (i > 0)
                {
                    ...
                    i -= k;
                    this.world.spawnEntityInWorld(new EntityXPOrb(this.world, this.posX, this.posY, this.posZ, k));
                }
            }
        ...
        if (this.deathTicks == 200 && !this.world.isRemote)
        {
            if (flag)
            {
                ...
                while (j > 0)
                {
                    ...
                    j -= l;
                    this.world.spawnEntityInWorld(new EntityXPOrb(this.world, this.posX, this.posY, this.posZ, l));
                }
            }
    ...
    private void generatePortal(BlockPos pos)
    {
            ...
            for (int k = -4; k <= 4; ++k)
            {
                for (int l = -4; l <= 4; ++l)
                {
                    ...
                    if (d2 <= 12.25D)
                    {
                        ...
                        {
                            this.world.setBlockState(blockpos, Blocks.end_portal.getDefaultState());
                        }
                    }
                }
            }
        ...
        this.world.setBlockState(pos, Blocks.bedrock.getDefaultState());
        this.world.setBlockState(pos.up(), Blocks.bedrock.getDefaultState());
        BlockPos blockpos1 = pos.up(2);
        this.world.setBlockState(blockpos1, Blocks.bedrock.getDefaultState());
        this.world.setBlockState(blockpos1.west(), Blocks.torch.getDefaultState().withProperty(BlockTorch.FACING, EnumFacing.EAST));
        this.world.setBlockState(blockpos1.east(), Blocks.torch.getDefaultState().withProperty(BlockTorch.FACING, EnumFacing.WEST));
        this.world.setBlockState(blockpos1.north(), Blocks.torch.getDefaultState().withProperty(BlockTorch.FACING, EnumFacing.SOUTH));
        this.world.setBlockState(blockpos1.south(), Blocks.torch.getDefaultState().withProperty(BlockTorch.FACING, EnumFacing.NORTH));
        this.world.setBlockState(pos.up(3), Blocks.bedrock.getDefaultState());
        this.world.setBlockState(pos.up(4), Blocks.dragon_egg.getDefaultState());
    }
