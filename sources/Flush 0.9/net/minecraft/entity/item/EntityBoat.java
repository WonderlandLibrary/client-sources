...
public class EntityBoat extends Entity
{
        ...
        if (d9 > 0.2975D)
        {
            ...
            for (int k = 0; (double)k < 1.0D + d9 * 60.0D; ++k)
            {
                ...
                if (this.rand.nextBoolean())
                {
                    ...
                    this.world.spawnParticle(EnumParticleTypes.WATER_SPLASH, d7, this.posY - 0.125D, d8, this.motionX, this.motionY, this.motionZ, new int[0]);
                }
                else
                {
                    double d24 = this.posX + d2 + d4 * d5 * 0.7D;
                    double d25 = this.posZ + d4 - d2 * d5 * 0.7D;
                    this.world.spawnParticle(EnumParticleTypes.WATER_SPLASH, d24, this.posY - 0.125D, d25, this.motionX, this.motionY, this.motionZ, new int[0]);
                }
            }
        }
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos)
    {
        ...
        else if (this.world.getBlockState((new BlockPos(this)).down()).getBlock().getMaterial() != Material.water && y < 0.0D)
        {
