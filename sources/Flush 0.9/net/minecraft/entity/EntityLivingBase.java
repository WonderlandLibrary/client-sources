...
public abstract class EntityLivingBase extends Entity {
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        ...
        if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
            ...
            if (block.getMaterial() != Material.air) {
                ...
                int i = (int) (150.0D * d0);
                ((WorldServer) this.world).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, Block.getStateId(iblockstate));
            }
        }
    ...
    public void onEntityUpdate() {
        ...
        if (this.isEntityAlive()) {
            if (this.isEntityInsideOpaqueBlock()) {
                ...
                if (d0 < 0.0D) {
                    this.attackEntityFrom(DamageSource.inWall, (float) Math.max(1, MathHelper.floor_double(-d0 * this.world.getWorldBorder().getDamageAmount())));
                }
            }
        }
        ...
        if (this.isEntityAlive()) {
            if (this.isInsideOfMaterial(Material.water)) {
                ...
                if (!this.world.isRemote && this.isRiding() && this.ridingEntity instanceof EntityLivingBase) {
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (tagCompund.hasKey("Attributes", 9) && this.world != null && !this.world.isRemote) {
    ...
    public void renderBrokenItemStack(ItemStack stack) {
        this.playSound("random.break", 0.8F, 0.8F + this.world.rand.nextFloat() * 0.4F);
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord + 0.05D, vec3.zCoord, Item.getIdFromItem(stack.getItem()));
        }
    }
    ...
    private int getArmSwingAnimationEnd() {
        ...
        float speed = animations.isEnabled() ? animations.handSwingSpeed.getValueFloat() : 1;
        return (int) (this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) :
    ...
    public void swingItem() {
        if (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2 || this.swingProgressInt < 0) {
            ...
            if (this.world instanceof WorldServer) {
                ((WorldServer) this.world).getEntityTracker().sendToAllTrackingEntity(this, new S0BPacketAnimation(this, 0));
            }
        }
    }
            ...
            for (int k = -i; k < i; ++k) {
                if (j != 0 || k != 0) {
                    ...
                    if (this.world.func_147461_a(axisalignedbb).isEmpty()) {
                        if (World.doesBlockHaveSolidTopSurface(this.world, new BlockPos(l, (int) this.posY, i1))) {
                            this.setPositionAndUpdate(this.posX + (double) j, this.posY + 1.0D, this.posZ + (double) k);
                            return;
                        }

                        if (World.doesBlockHaveSolidTopSurface(this.world, new BlockPos(l, (int) this.posY - 1, i1)) || this.world.getBlockState(new BlockPos(l, (int) this.posY - 1, i1)).getBlock().getMaterial() == Material.water) {
        ...
        if (this.isServerWorld()) {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    ...
                    if (this.onGround) {
                        f4 = this.world.getBlockState(new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.getEntityBoundingBox().minY) - 1, MathHelper.floor_double(this.posZ))).getBlock().slipperiness * 0.91F;
                    }
        ...
        if (!this.world.isRemote) {
            ...
            for (int j = 0; j < 5; ++j) {
                ...
                if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
                    ((WorldServer) this.world).getEntityTracker().sendToAllTrackingEntity(this, new S04PacketEntityEquipment(this.getEntityId(), j, itemstack1));
    ...
    protected void collideWithNearbyEntities() {
        List<Entity> list = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.20000000298023224D, 0.0D, 0.20000000298023224D), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBePushed));
    ...
    public boolean canEntityBeSeen(Entity entityIn) {
        return this.world.rayTraceBlocks(new Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ),
    ...
    public Team getTeam() {
        return this.world.getScoreboard().getPlayersTeam(this.getUniqueID().toString());
    }
