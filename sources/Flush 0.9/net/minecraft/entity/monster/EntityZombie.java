...
public class EntityZombie extends EntityMob
{
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        if (super.attackEntityFrom(source, amount))
        {
            ...
            if (entitylivingbase != null && this.world.getDifficulty() == EnumDifficulty.HARD && (double)this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue())
            {
                ...
                for (int l = 0; l < 50; ++l)
                {
                    ...
                    if (World.doesBlockHaveSolidTopSurface(this.world, new BlockPos(i1, j1 - 1, k1)) && this.world.getLightFromNeighbors(new BlockPos(i1, j1, k1)) < 10)
                    {
                        entityzombie.setPosition((double)i1, (double)j1, (double)k1);

                        if (!this.world.isAnyPlayerWithinRangeAt((double)i1, (double)j1, (double)k1, 7.0D) && this.world.checkNoEntityCollision(entityzombie.getEntityBoundingBox(), entityzombie) && this.world.getCollidingBoundingBoxes(entityzombie, entityzombie.getEntityBoundingBox()).isEmpty() && !this.world.isAnyLiquid(entityzombie.getEntityBoundingBox()))
                        {
    ...
    public void onKillEntity(EntityLivingBase entityLivingIn)
    {
        ...
        if ((this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD) && entityLivingIn instanceof EntityVillager)
        {
            ...
            this.world.spawnEntityInWorld(entityzombie);
            this.world.playAuxSFXAtEntity((EntityPlayer)null, 1016, new BlockPos((int)this.posX, (int)this.posY, (int)this.posZ), 0);
        }
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata)
    {
        ...
        if (livingdata == null)
        {
            livingdata = new EntityZombie.GroupData(this.world.rand.nextFloat() < 0.05F, this.world.rand.nextFloat() < 0.05F);
        }
    ...
    protected void startConversion(int ticks)
    {
        ...
        this.addPotionEffect(new PotionEffect(Potion.damageBoost.id, ticks, Math.min(this.world.getDifficulty().getDifficultyId() - 1, 0)));
    ...
    public void handleStatusUpdate(byte id)
    {
        if (id == 16)
        {
            if (!this.isSilent())
            {
                this.world.playSound(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "mob.zombie.remedy", 1.0F + this.rand.nextFloat(), this.rand.nextFloat() * 0.7F + 0.3F, false);
            }
        }
        else
        {
    ...
    protected void convertToVillager()
    {
        ...
        this.world.playAuxSFXAtEntity((EntityPlayer)null, 1017, new BlockPos((int)this.posX, (int)this.posY, (int)this.posZ), 0);
    }
        ...
        if (this.rand.nextFloat() < 0.01F)
        {
            ...
            for (int k = (int)this.posX - 4; k < (int)this.posX + 4 && j < 14; ++k)
            {
                for (int l = (int)this.posY - 4; l < (int)this.posY + 4 && j < 14; ++l)
                {
                    ...
                    {
                        Block block = this.world.getBlockState(blockpos$mutableblockpos.func_181079_c(k, l, i1)).getBlock();
