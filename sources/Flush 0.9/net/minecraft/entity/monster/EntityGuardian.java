...
public class EntityGuardian extends EntityMob
{
    ...
    public EntityLivingBase getTargetedEntity()
    {
        ...
        else if (this.world.isRemote)
        {
            ...
            {
                Entity entity = this.world.getEntityByID(this.dataWatcher.getWatchableObjectInt(17));
    ...
    {
        return this.world.getBlockState(pos).getBlock().getMaterial() == Material.water ? 10.0F + this.world.getLightBrightness(pos) - 0.5F : super.getBlockPathWeight(pos);
    }
    ...
    public void onLivingUpdate()
    {
        if (this.world.isRemote)
        {
            ...
            if (!this.isInWater())
            {
                ...
                {
                    this.world.playSound(this.posX, this.posY, this.posZ, "mob.guardian.flop", 1.0F, 1.0F, false);
                }

                this.field_175480_bp = this.motionY < 0.0D && this.world.isBlockNormalCube((new BlockPos(this)).down(), false);
            }
            ...
            if (this.func_175472_n() && this.isInWater())
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3.xCoord * 1.5D, this.posY + this.rand.nextDouble() * (double)this.height - vec3.yCoord * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3.zCoord * 1.5D, 0.0D, 0.0D, 0.0D, new int[0]);
                }
            }
    ...
    {
        return this.world.checkNoEntityCollision(this.getEntityBoundingBox(), this) && this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty();
    }
