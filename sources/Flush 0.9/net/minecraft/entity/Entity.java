...
public abstract class Entity implements ICommandSender {
    ...
    protected void preparePlayerToSpawn() {
        if (this.world != null) {
            while (this.posY > 0.0D && this.posY < 256.0D) {
                ...
                if (this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty()) {
                    break;
                }
    ...
    private boolean isLiquidPresentInAABB(AxisAlignedBB bb) {
        return this.world.getCollidingBoundingBoxes(this, bb).isEmpty() && !this.world.isAnyLiquid(bb);
    }
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            EventSafewalk event = new EventSafewalk().call();
            double d3 = x;
            double d4 = y;
            double d5 = z;
            boolean flag = this.onGround && (isSneaking() || event.isCancelled()) && this instanceof EntityPlayer;

            if (flag) {
                double d6;

                for (d6 = 0.05D; x != 0.0D && this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty(); d3 = x) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
                ...
                for (; z != 0.0D && this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, -1.0D, z)).isEmpty(); d5 = z) {
                    if (z < d6 && z >= -d6) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= d6;
                ...
                for (; x != 0.0D && z != 0.0D && this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0D, z)).isEmpty(); d5 = z) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
            ...
            List<AxisAlignedBB> list1 = this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(x, y, z));
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                EventStep eventStep = new EventStep(stepHeight);
                ...
                List<AxisAlignedBB> list = this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(d3, y, d5));
            ...
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                if (this.onGround) {
                    block1.onEntityCollidedWithBlock(this.world, blockpos, this);
                }
            ...
            if (this.world.isFlammableWithin(this.getEntityBoundingBox().contract(0.001D, 0.001D, 0.001D))) {
    ...
    public void playSound(String name, float volume, float pitch) {
        if (!this.isSilent()) {
            this.world.playSoundAtEntity(this, name, volume, pitch);
        }
    }
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        if (onGroundIn) {
            if (this.fallDistance > 0.0F) {
                if (blockIn != null) {
                    blockIn.onFallenUpon(this.world, pos, this, this.fallDistance);
                } else {
    ...
    public boolean isWet() {
        return this.inWater || this.world.canLightningStrike(new BlockPos(this.posX, this.posY, this.posZ)) || this.world.canLightningStrike(new BlockPos(this.posX, this.posY + (double) this.height, this.posZ));
    }
        ...
        if (this.noClip) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                if (blockpos$mutableblockpos.getX() != k || blockpos$mutableblockpos.getY() != j || blockpos$mutableblockpos.getZ() != l) {
                    ...
                    if (this.world.getBlockState(blockpos$mutableblockpos).getBlock().isVisuallyOpaque()) {
                        return true;
                    }
                }
            }
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_) {
        ...
        this.setRotation(yaw, pitch);
        List<AxisAlignedBB> list = this.world.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().contract(0.03125D, 0.0D, 0.03125D));
    ...
    public void func_181015_d(BlockPos p_181015_1_) {
        if (this.timeUntilPortal > 0) {
            ...
            if (!this.world.isRemote && !p_181015_1_.equals(this.field_181016_an)) {
                this.field_181016_an = p_181015_1_;
                BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.portal.func_181089_f(this.world, p_181015_1_);
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        List<AxisAlignedBB> list = this.world.func_147461_a(this.getEntityBoundingBox());

        if (list.isEmpty() && !this.world.isBlockFullCube(blockpos)) {
            return false;
        } else {
            int i = 3;
            double d3 = 9999.0D;
    ...
    public void travelToDimension(int dimensionId) {
        if (!this.world.isRemote && !this.isDead) {
            ...
            if (entity != null) {
                ...
                if (i == 1 && dimensionId == 1) {
                    BlockPos blockpos = this.world.getTopSolidOrLiquidBlock(worldserver1.getSpawnPoint());
