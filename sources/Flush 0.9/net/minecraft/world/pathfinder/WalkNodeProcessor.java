            ...
            for (int j = y; j < y + sizeY; ++j)
            {
                for (int k = z; k < z + sizeZ; ++k)
                {
                    ...
                    if (block.getMaterial() != Material.air)
                    {
                        ...
                        if (entityIn.world.getBlockState(blockpos$mutableblockpos).getBlock() instanceof BlockRailBase)
                        {
                            if (!(entityIn.world.getBlockState(blockpos).getBlock() instanceof BlockRailBase) && !(entityIn.world.getBlockState(blockpos.down()).getBlock() instanceof BlockRailBase))
                            {
                                return -3;
                            }
                        }
