...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
    ...
    private final IntHashMap<Short> map = new IntHashMap<>();
    ...
    public void kickPlayerFromServer(String reason) {
        ...
        this.netManager.sendPacket(new S40PacketDisconnect(chatcomponenttext), future -> NetHandlerPlayServer.this.netManager.closeChannel(chatcomponenttext));
    ...
    private boolean isInvalidC03(C03PacketPlayer p_183006_1_) {
        ...
        if (this.isInvalidC03(packetIn)) {
            ...
            if (!this.playerEntity.playerConqueredTheEnd) {
                ...
                if (this.hasMoved) {
                    ...
                    //ChatUtils.println("d12: " + d12 + ": " + worldserver.checkBlockCollision(axisalignedbb));
                    if (!this.serverController.isFlightAllowed() && !this.playerEntity.capabilities.allowFlying && !worldserver.checkBlockCollision(axisalignedbb)) {
                        if (d12 >= -0.03125D) {
                            ...
                            //ChatUtils.println("fly flag [" + floatingTickCount + "]");

                            if (this.floatingTickCount > 80) {
                                //ChatUtils.println("kic");
                                logger.warn(this.playerEntity.getName() + " was kicked for floating too long!");
                                this.kickPlayerFromServer("Flying is not enabled on this server");
                                return;
                            }
                        } else {
                            //ChatUtils.println("work");
                        }
                    } else {
                        //ChatUtils.println("fly flag reset");
    ...
    public void processPlayerBlockPlacement(C08PacketPlayerBlockPlacement packetIn) {
        ...
        if (packetIn.getPlacedBlockDirection() == 255) {
            ...
            ChatComponentTranslation component = new ChatComponentTranslation("build.tooHigh", this.serverController.getBuildLimit());
            component.getChatStyle().setColor(EnumChatFormatting.RED);
            this.playerEntity.playerNetServerHandler.sendPacket(new S02PacketChat(component));
            flag = true;
        }
        ...
        if (this.playerEntity.openContainer.windowId == packetIn.getWindowId() && this.playerEntity.openContainer.getCanCraft(this.playerEntity)) {
            if (this.playerEntity.isSpectator()) {
                ...
                if (ItemStack.areItemStacksEqual(packetIn.getClickedItem(), itemstack)) {
                    ...
                    this.map.addKey(this.playerEntity.openContainer.windowId, packetIn.getActionNumber());
    ...
    public void processConfirmTransaction(C0FPacketConfirmTransaction packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.playerEntity.getServerForPlayer());
        Short oshort = this.map.lookup(this.playerEntity.openContainer.windowId);
    ...
    public void processTabComplete(C14PacketTabComplete packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.playerEntity.getServerForPlayer());
        List<String> list = Lists.newArrayList(serverController.getTabCompletions(this.playerEntity, packetIn.getMessage(), packetIn.getTargetBlock()));
        ...
        if ("MC|BEdit".equals(packetIn.getChannelName())) {
            ...
            if (!this.serverController.isCommandBlockEnabled()) {
                ...
                try {
                    ...
                    if (j == 0) {
                        TileEntity tileentity = this.playerEntity.world.getTileEntity(new BlockPos(packetbuffer.readInt(), packetbuffer.readInt(), packetbuffer.readInt()));

                        if (tileentity instanceof TileEntityCommandBlock) {
                            commandblocklogic = ((TileEntityCommandBlock) tileentity).getCommandBlockLogic();
                        }
                    } else if (j == 1) {
                        Entity entity = this.playerEntity.world.getEntityByID(packetbuffer.readInt());
