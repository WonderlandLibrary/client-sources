...
public class ResourcePackRepository {
    ...
    private static final FileFilter resourcePackFilter = file ->
            (file.isFile() && file.getName().endsWith(".zip")) || (file.isDirectory() && (new File(file, "pack.mcmeta")).isFile());
    ...
    public ListenableFuture<?> downloadResourcePack(String url, String hash) {
        ...
        if (hash.matches("^[a-f0-9]{40}$")) s = hash;
        else s = "legacy";
        ...
        try {
            ...
            if (file1.exists() && hash.length() == 40) {
                try {
                    ...
                    if (s1.equals(hash)) return this.setResourcePackInstance(file1);
