...
public class LayerCape implements LayerRenderer
{
    ...
    {
        Cape cape = Flush.getInstance().getModuleManager().getModule(Cape.class);
        ...
        if ((entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible()
                ...
                (entitylivingbaseIn == Minecraft.getMinecraft().thePlayer && !entitylivingbaseIn.isInvisible()
                        && entitylivingbaseIn.getLocationCape() == null && cape.isEnabled())) {
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            if (entitylivingbaseIn == Minecraft.getMinecraft().thePlayer && cape.isEnabled()) {
                if (cape.isCustom()) {
                    Minecraft.getMinecraft().getTextureManager().loadTexture(cape.getImageId(), cape.getCustomImage());
                    this.playerRenderer.bindTexture(cape.getImageId());
                } else {
                    this.playerRenderer.bindTexture(Flush.getCape());
                }
            } else {
