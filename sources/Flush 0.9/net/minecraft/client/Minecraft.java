...
import me.nyan.flush.module.impl.combat.AutoClicker;
import me.nyan.flush.module.impl.player.FastPlace;
import me.nyan.flush.module.impl.render.UnfocusedCpu;
import me.nyan.flush.ui.menu.SplashScreen;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void startGame() throws LWJGLException {
        SplashScreen splashScreen = new SplashScreen(7);
        splashScreen.increment();
        Flush.getInstance().onPreInit();
        ...
        AchievementList.openInventory.setStatStringFormatter(s -> {
            try {
                return String.format(s, GameSettings.getKeyDisplayString(Minecraft.this.gameSettings.keyBindInventory.getKeyCode()));
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("Loading " + Flush.NAME + " " + Flush.VERSION);
    ...
    private void setWindowIcon() {
        ...
        if (os != Util.EnumOS.OSX) {
            ...
            try {
                ...
                //inputstream = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("flush/icons/icon_32x32.png"));
                //inputstream1 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("flush/icons/icon_32x32.png"));
    ...
    private void runGameLoop() throws IOException {
        ...
        this.frameTimer.func_181747_a(k - this.nanoTime);
        this.nanoTime = k;

        while (getSystemTime() >= this.debugUpdateTime + 1000L) {
            debugFPS = this.fpsCounter;
            this.debug = String.format("%d fps (%d chunk update%s) T: %s%s%s%s%s", debugFPS, RenderChunk.renderChunksUpdated,
                    RenderChunk.renderChunksUpdated != 1 ? "s" : "", (float) this.gameSettings.limitFramerate ==
                            GameSettings.Options.FRAMERATE_LIMIT.getValueMax() ? "inf" : Integer.valueOf(this.gameSettings.limitFramerate),
    ...
    public int getLimitFramerate() {
        return UnfocusedCpu.isUnfocused() ? 5 : this.theWorld == null && this.currentScreen != null ? 30 : this.gameSettings.limitFramerate;
    }
    ...
    public void clickMouse() {
        if (this.leftClickCounter <= 0) {
            ...
            AutoClicker autoClicker = Flush.getInstance().getModuleManager().getModule(AutoClicker.class);
            if (this.objectMouseOver == null) {
                logger.error("Null returned as 'hitResult', this shouldn't happen!");

                if (this.playerController.isNotCreative() && (!autoClicker.isEnabled() || !autoClicker.removeCpsCap.getValue())) {
    ...
    public void rightClickMouse() {
        if (!this.playerController.isHittingBlock()) {
            FastPlace fastPlace = Flush.getInstance().getModuleManager().getModule(FastPlace.class);
            rightClickDelayTimer = fastPlace.isEnabled() ? fastPlace.delay.getValueInt() : 4;
    ...
    public void runTick() throws IOException {
        ...
        eventTick.setState(Event.State.POST).call();
    }
    ...
    public void launchIntegratedServer(String folderName, String worldName, WorldSettings worldSettingsIn) {
        ...
        Flush.getInstance().getDiscordRP().update("Playing Singleplayer", "In Game");
    }
    ...
    public MusicTicker.MusicType getAmbientMusicType() {
        return this.thePlayer != null ? (this.thePlayer.world.provider instanceof WorldProviderHell ? MusicTicker.MusicType.NETHER : (this.thePlayer.world.provider instanceof WorldProviderEnd ? (BossStatus.bossName != null && BossStatus.statusBarTime > 0 ? MusicTicker.MusicType.END_BOSS : MusicTicker.MusicType.END) : (this.thePlayer.capabilities.isCreativeMode && this.thePlayer.capabilities.allowFlying ? MusicTicker.MusicType.CREATIVE : MusicTicker.MusicType.GAME))) : MusicTicker.MusicType.MENU;
    }
                    ...
                    if (i == this.gameSettings.keyBindStreamStartStop.getKeyCode()) {
                        if (this.getTwitchStream().isBroadcasting()) {
                            ...
                            this.displayGuiScreen(new GuiYesNo((result, id) -> {
                                if (result) {
                                    Minecraft.this.getTwitchStream().startBroadcasting();
                                }
