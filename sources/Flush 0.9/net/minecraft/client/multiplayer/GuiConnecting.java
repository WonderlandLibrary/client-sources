...
public class GuiConnecting extends GuiScreen {
    ...
    public GuiConnecting(GuiScreen previousScreen, Minecraft mc, ServerData serverData) {
        ...
        new EventConnection(serveraddress.getIP(), serveraddress.getPort()).call();
        Flush.getInstance().getDiscordRP().update("Playing " + serveraddress.getIP() +
                (serveraddress.getPort() != 25565 ? ':' + serveraddress.getPort() : ""), "In Game");

        Flush flush = Flush.getInstance();
        if (serverData.serverIP.toLowerCase().contains("funcraft.net")) {
            Flush.getInstance().setViaVersion(ProtocolCollection.R1_9_3);

            if (mc.getSession().getToken().isEmpty() && flush.getGenerator() != null) {
                try {
                    serverData = new ServerData(serverData.serverName,
                            (String) flush.getGenerator().invoke(null, mc.getSession().getUsername()),
        ...
        Flush.currentServer = serverData;
    }
    ...
    public GuiConnecting(GuiScreen previousScreen, Minecraft mcIn, String hostName, int port) {
        ...
        Flush.currentServer = new ServerData(hostName + ":" + port, hostName + ":" + port, false);
    }
    ...
    public void initGui() {
        ...
        bar = new ProgressBar(width / 2f - 100, height / 4F + 120 + 12 + 20 + 6 + 1, 200);
    }
