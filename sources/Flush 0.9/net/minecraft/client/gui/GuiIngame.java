...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        new Event2D(scaledresolution).call();
    ...
    public void renderExpBar(ScaledResolution resolution, int p_175176_2_) {
        ...
        if (i > 0) {
            ...
            int j = resolution.getScaledHeight() - 32 + 3;
        ...
        if (this.mc.thePlayer.experienceLevel > 0) {
            ...
            int i1 = (resolution.getScaledWidth() - this.getFontRenderer().getStringWidth(s)) / 2;
            int l = resolution.getScaledHeight() - 31 - 4;
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution resolution) {
        ...
        ArrayList<Score> arraylist = collection.stream().filter(score -> score.getPlayerName() != null && !score.getPlayerName().startsWith("#")).collect(Collectors.toCollection(Lists::newArrayList));
        ...
        int height = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        float x = resolution.getScaledWidth() - i - 3;
        float y = resolution.getScaledHeight() / 2f + height / 3f;
        int k = 0;
        for (Object score1 : arraylist1) {
            ...
            float l = y - k * this.getFontRenderer().FONT_HEIGHT;
            int i1 = resolution.getScaledWidth() - 1;
            drawRect(x - 2, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            this.getFontRenderer().drawString(s1, (int) x, (int) l, 553648127);
            this.getFontRenderer().drawString(s2, i1 - this.getFontRenderer().getStringWidth(s2), (int) l, 553648127);

            if (k == arraylist1.size()) {
                String s3 = objective.getDisplayName();
                drawRect(x - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1, 1610612736);
                drawRect(x - 2, l - 1, i1, l, 1342177280);
                this.getFontRenderer().drawString(s3, (int) (x + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2), (int) (l -
                        this.getFontRenderer().FONT_HEIGHT), 553648127);
            }
        }
    }
    ...
    public void setRecordPlayingMessage(String recordPlayingMessage) {
        this.setRecordPlaying(I18n.format("record.nowPlaying", recordPlayingMessage), true);
    }
