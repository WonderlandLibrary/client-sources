...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    protected boolean renderShader = true;

    /**
     * Draws the screen and all the components in it. Args : mouseX, mouseY, renderPartialTicks
     */
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        EventDrawGuiScreen e = new EventDrawGuiScreen(this, width, height, mouseX, mouseY).call();
        ...
        for (int i = 0; i < this.buttonList.size(); ++i) {
            this.buttonList.get(i).drawButton(mc, mouseX, mouseY);
        }
    ...
    public void drawFlush(int mouseX, int mouseY) {
        if (Flush.getInstance().getShader() == null) {
            try {
                Flush.getInstance().setShader(new GLSLSandboxShader("flush/shaders/shader.fsh",
                        "color1", "color2"));
            } catch (IOException e) {
        ...
        RenderUtil.drawImage(new ResourceLocation("flush/background.jpg"), 0, 0, width, height);
         */

        if (!Display.isActive() && !renderShader) {
            return;
        }

        GLSLSandboxShader shader = Flush.getInstance().getShader();

        GlStateManager.disableCull();
        shader.useShader(width, height, mouseX, mouseY, (System.currentTimeMillis() - initTime) / 1000F);

        /*
        glUniform3f(shader.getUniform("color1"), 221 / 255F, 26 / 255F, 82 / 255F);
        glUniform3f(shader.getUniform("color2"), 167 / 255F, 88 / 255F, 212 / 255F);
         */

        glUniform3f(shader.getUniform("color1"), 160 / 255F, 10 / 255F, 170 / 255F);
        glUniform3f(shader.getUniform("color2"), 0 / 255F, 60 / 255F, 255 / 255F);
