...
public class ShaderUtil implements IAccess {
    ...
    public ShaderUtil(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = createShader(mc.getResourceManager().getResource(new ResourceLocation("reality/Shaders/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public ShaderUtil(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "actinium/Shaders/vertex.vsh");
    }
    ...
    private String kawaseUpBloom = "#version 120\n" +
            ...
            "    sum += smp7 * 2.0;\n" +
            "    vec4 result = sum / 12.0;\n" +
            "    gl_FragColor = vec4(result.rgb / result.a, mix(result.a, result.a * (1.0 - texture2D(textureToCheck, gl_TexCoord[0].st).a),check) * 1.1);\n" +
            "}";
