...
public class ReflectionUtil {
    ...
    public static boolean exist(String packageName) {
    ...
    private static Class<?>[] getClassesInPackage(String packageName, File directory) {
        ...
        for (String filename : Objects.requireNonNull(directory.list())) {
            if (filename.endsWith(".class")) {
                String classname = buildClassname(packageName.toLowerCase(), filename);
                try {
    ...
    public static String path() throws URISyntaxException {
        return new File(Actinium.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getPath();
    }
}