...
import net.minecraft.viamcpFix.vialoadingbase.ViaLoadingBase;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
        AnimationModule animationModule = Actinium.INSTANCE.getModuleManager().get(AnimationModule.class);
        return animationModule != null && !animationModule.isEnabled() ?
                this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 :
                        (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6) :
                animationModule.swingSpeed.getValue().intValue();
    }
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            JumpCorrectionEvent Niggaisjumping = new JumpCorrectionEvent(this.rotationYaw);
            if(this instanceof EntityPlayerSP) {
                Actinium.INSTANCE.getEventManager().publish(Niggaisjumping);
            }

            float f = Niggaisjumping.getYaw()  * 0.017453292F;
    ...
    public void onLivingUpdate()
    {
        ...
        double value = ViaLoadingBase.getInstance().getTargetVersion().getVersion() > 47 ? 0.003 : 0.005;

        if (Math.abs(this.motionX) < value)
        {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < value)
        {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < value)
        {
            this.motionZ = 0.0D;
        }
    ...
    /**
     * @Author Esound Thanks uwu esound i lil
     * Positions from BackTrack uwu
     */
    public int realPosX;
    public int realPosY;
    public int realPosZ;
