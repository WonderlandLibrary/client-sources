...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (this.isInWeb)
            {
                ...
                Actinium.INSTANCE.getEventManager().publish(webSlowDownEvent);
                this.isInWeb = false;
                x *= webSlowDownEvent.isCancelled() ? webSlowDownEvent.getSlowDown() : 0.25D;
                y *= webSlowDownEvent.isCancelled() ? webSlowDownEvent.getMotionY() : 0.05000000074505806D;
                z *= webSlowDownEvent.isCancelled() ? webSlowDownEvent.getSlowDown() : 0.25D;

                if(!webSlowDownEvent.isNoMotion()) {
                    this.motionX = webSlowDownEvent.isCancelled() ? webSlowDownEvent.getMotionSpeed() : 0.0D;
                    this.motionY = webSlowDownEvent.isCancelled() ? webSlowDownEvent.getMotionY() : 0.0D;
                    this.motionZ = webSlowDownEvent.isCancelled() ? webSlowDownEvent.getMotionSpeed() : 0.0D;
                }
            }
    ...
    public void moveFlying(float strafe, float forward, float friction)
    {
        ...
        if (player) {
            final StrafeEvent strafeEvent = new StrafeEvent(forward, strafe, friction, rotationYaw);

            Actinium.INSTANCE.getEventManager().publish(strafeEvent);
