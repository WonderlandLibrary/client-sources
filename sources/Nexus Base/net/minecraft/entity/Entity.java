...
import dev.nexus.events.impl.EventLook;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (player) {
            ...
            Nexus.INSTANCE.getEventManager().post(event);
    ...
    public MovingObjectPosition rayTrace(double blockReachDistance, float partialTicks) {
        ...
        if (this != Minecraft.getMinecraft().thePlayer) {
            vec31 = this.getLook(partialTicks);
        } else {
            EventLook eventLook = new EventLook(rotationYaw, rotationPitch);
            Nexus.INSTANCE.getEventManager().post(eventLook);
            if (!eventLook.hasBeenModified())
                vec31 = this.getLook(partialTicks);
            else vec31 = getVectorForRotation(eventLook.getPitch(), eventLook.getYaw());
        }
    ...
    public String toString() {
        return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", new Object[]{this.getClass().getSimpleName(), this.getName(), this.entityId, this.worldObj == null ? "~NULL~" : this.worldObj.getWorldInfo().getWorldName(), this.posX, this.posY, this.posZ});
    }
