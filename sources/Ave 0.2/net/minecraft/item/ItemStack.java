...
public final class ItemStack {
   ...
   public List getTooltip(EntityPlayer playerIn, boolean advanced) {
      ...
      if(advanced) {
         ...
         int multimap = Item.getIdFromItem(this.item);
         if(this.getHasSubtypes()) {
            s = s + String.format("#%04d/%d%s", new Object[]{Integer.valueOf(multimap), Integer.valueOf(this.itemDamage), i1});
         } else {
            s = s + String.format("#%04d%s", new Object[]{Integer.valueOf(multimap), i1});
         }
      ...
      if(this.hasTagCompound()) {
         if((var14 & 1) == 0) {
            ...
            if(var15 != null) {
               for(int nbttaglist3 = 0; nbttaglist3 < var15.tagCount(); ++nbttaglist3) {
                  ...
                  short block1 = var15.getCompoundTagAt(nbttaglist3).getShort("lvl");
                  if(Enchantment.getEnchantmentById(l1) != null) {
                     list.add(Enchantment.getEnchantmentById(l1).getTranslatedName(block1));
                  }
               }
            }
         }
         ...
         if(this.stackTagCompound.hasKey("display", 10)) {
            ...
            if(var16.hasKey("color", 3)) {
               if(advanced) {
                  list.add("Color: #" + Integer.toHexString(var16.getInteger("color")).toUpperCase());
               } else {
            ...
            if(var16.getTagId("Lore") == 9) {
               ...
               if(var18.tagCount() > 0) {
                  for(var20 = 0; var20 < var18.tagCount(); ++var20) {
                     list.add("" + EnumChatFormatting.DARK_PURPLE + EnumChatFormatting.ITALIC + var18.getStringTagAt(var20));
                  }
               }
            }
         }
      ...
      if(!var17.isEmpty() && (var14 & 2) == 0) {
         ...
         while(var21.hasNext()) {
            ...
            if(d0 > 0.0D) {
               list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + var22.getOperation(), new Object[]{DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)var19.getKey())}));
            } else if(d0 < 0.0D) {
               d1 *= -1.0D;
               list.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + var22.getOperation(), new Object[]{DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)var19.getKey())}));
            }
         }
      }
      ...
      if(this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (var14 & 8) == 0) {
         ...
         if(var18.tagCount() > 0) {
            ...
            for(var20 = 0; var20 < var18.tagCount(); ++var20) {
               ...
               if(var23 != null) {
                  list.add(EnumChatFormatting.DARK_GRAY + var23.getLocalizedName());
               } else {
