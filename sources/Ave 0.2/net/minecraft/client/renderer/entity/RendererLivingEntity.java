...
import me.AveReborn.events.EventRendererLivingEntity;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
		...
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			try {
				...
				if (this.renderOutlines) {
					...
					EventManager.call(new EventRendererLivingEntity(this, entity, f6, f5, f8, f2, f8, 0.0625F));
				}
	...
	public void renderName(T entity, double x, double y, double z) {
		if (this.canRenderName(entity) && !ModManager.getModByName("NameTags").isEnabled()) {
				...
				if (this.canRenderName(entity)) {
					...
					if (d0 < (double) (f * f)) {
						...
						if (entity.isSneaking()) {
							...
							worldrenderer.pos((double) (-i - 1), -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F)
									.endVertex();
							...
							this.renderOffsetLivingLabel(entity, x,
									y - (entity.isChild() ? (double) (entity.height / 2.0F) : 0.0D), z, s, 0.02666667F,
									d0);
						}
					}
				}

				if (!Reflector.RenderLivingEvent_Specials_Post_Constructor.exists() || !Reflector.postForgeBusEvent(
						Reflector.RenderLivingEvent_Specials_Post_Constructor,
						new Object[] { entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z) })) {
					;
				}
			}
