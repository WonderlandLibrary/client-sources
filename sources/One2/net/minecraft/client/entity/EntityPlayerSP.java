...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            ...
            if (this.isRiding()) {
                ...
                EventPreUpdate preEvent = new EventPreUpdate();
                EventManager.call(preEvent);
                if (preEvent.isCancelled()) {
                    return;
                }
                this.func_175161_p();
                EventPostUpdate postEvent = new EventPostUpdate();
    ...
    public void sendChatMessage(String p_71165_1_) {
        if (p_71165_1_.startsWith(Twelve.getInstance().getCmdHandler().getPrefix())) {
            for (Cmd cmd : Twelve.getInstance().getCmdHandler().getCmds()) {
                if (p_71165_1_.equals(String.valueOf(Twelve.getInstance().getCmdHandler().getPrefix()) + cmd.getCmd() + " help")) {
                    cmd.runHelp();
                    return;
                }
                if (!p_71165_1_.startsWith(String.valueOf(Twelve.getInstance().getCmdHandler().getPrefix()) + cmd.getCmd())) continue;
                cmd.runCmd(p_71165_1_, p_71165_1_.split(" "));
                return;
            }
            return;
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !Twelve.getInstance().getModHandler().getMod("NoSlowdown").getEnabled()){
