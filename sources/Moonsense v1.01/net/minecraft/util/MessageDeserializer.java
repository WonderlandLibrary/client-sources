...
public class MessageDeserializer extends ByteToMessageDecoder
{
    ...
    protected void decode(final ChannelHandlerContext p_decode_1_, final ByteBuf p_decode_2_, final List p_decode_3_) throws IOException, InstantiationException, IllegalAccessException {
        if (p_decode_2_.readableBytes() != 0) {
            ...
            final Packet var6 = p_decode_1_.channel().attr((AttributeKey<EnumConnectionState>)NetworkManager.attrKeyConnectionState).get().getPacket(this.direction, var5);
            if (var6 == null) {
                throw new IOException("Bad packet id " + var5);
            }
            var6.readPacketData(var4);
            if (var4.readableBytes() > 0) {
                throw new IOException("Packet " + p_decode_1_.channel().attr((AttributeKey<EnumConnectionState>)NetworkManager.attrKeyConnectionState).get().getId() + "/" + var5 + " (" + var6.getClass().getSimpleName() + ") was larger than I expected, found " + var4.readableBytes() + " bytes extra whilst reading packet " + var5);
            }
            p_decode_3_.add(var6);
            if (MessageDeserializer.logger.isDebugEnabled()) {
                MessageDeserializer.logger.debug(MessageDeserializer.RECEIVED_PACKET_MARKER, " IN: [{}:{}] {}", new Object[] { p_decode_1_.channel().attr((AttributeKey<Object>)NetworkManager.attrKeyConnectionState).get(), var5, var6.getClass().getName() });
            }
        }
    }
}
