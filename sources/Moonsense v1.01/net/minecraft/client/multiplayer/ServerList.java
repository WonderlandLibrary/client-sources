...
public class ServerList
{
    ...
    private void loadFeaturedServers() {
        for (final FeaturedServer server : MoonsenseClient.INSTANCE.getFeaturedServerManager().getFeaturedServers()) {
    ...
    public int getFeaturedServerCount() {
        int count = 0;
        for (final Object server : this.servers) {
            if (server instanceof FeaturedServer) {
                ++count;
            }
        }
        return count;
    ...
    public void saveServerList() {
        try {
            ...
            for (final ServerData var3 : this.servers) {
                if (!(var3 instanceof FeaturedServer)) {
