...
import moonsense.features.modules.DabModule;
...
public class ModelPlayer extends ModelBiped
{
    ...
    public void render(final Entity p_78088_1_, final float p_78088_2_, final float p_78088_3_, final float p_78088_4_, final float p_78088_5_, final float p_78088_6_, final float p_78088_7_) {
        ...
        if (this.isChild) {
            ...
            this.field_178732_b.render(p_78088_7_);
            this.field_178730_v.render(p_78088_7_);
            if (p_78088_1_.getName().equals(Minecraft.getMinecraft().getSession().getProfile().getName())) {
                for (final OBJCosmeticHandler handler : CosmeticsManager.cosmetics) {
                    if (handler.getType() == OBJCosmeticHandler.CosmeticsType.HAT) {
                        handler.render(p_78088_1_, this.bipedHead);
                    }
                    else {
                        if (handler.getType() != OBJCosmeticHandler.CosmeticsType.BODY) {
                            continue;
                        }
                        handler.render(p_78088_1_, this.bipedBody);
                    }
                }
            }
        }
    ...
    public void setRotationAngles(final float p_78087_1_, final float p_78087_2_, final float p_78087_3_, final float p_78087_4_, final float p_78087_5_, final float p_78087_6_, final Entity p_78087_7_) {
        ...
        final boolean isOurPlayer = p_78087_7_.getEntityId() == Minecraft.getMinecraft().thePlayer.getEntityId() && DabModule.INSTANCE.isEnabled();
        if (isOurPlayer && DabModule.INSTANCE.getInputEvent().prevDabbingHeld > 0) {
            float heldPercent = (DabModule.INSTANCE.getInputEvent().prevDabbingHeld + (DabModule.INSTANCE.getInputEvent().dabbingHeld - DabModule.INSTANCE.getInputEvent().prevDabbingHeld) * Minecraft.getMinecraft().getTimer().renderPartialTicks) / 8.0f;
            this.bipedRightArm.rotateAngleX = (float)Math.toRadians(-90.0f * heldPercent);
            this.bipedRightArm.rotateAngleY = (float)Math.toRadians(-35.0f * heldPercent);
            this.field_178732_b.rotateAngleX = (float)Math.toRadians(-90.0f * heldPercent);
            this.field_178732_b.rotateAngleY = (float)Math.toRadians(-35.0f * heldPercent);
            this.bipedLeftArm.rotateAngleX = (float)Math.toRadians(15.0f * heldPercent);
            this.bipedLeftArm.rotateAngleY = (float)Math.toRadians(15.0f * heldPercent);
            this.bipedLeftArm.rotateAngleZ = (float)Math.toRadians(-110.0f * heldPercent);
            this.field_178734_a.rotateAngleX = (float)Math.toRadians(15.0f * heldPercent);
            this.field_178734_a.rotateAngleY = (float)Math.toRadians(15.0f * heldPercent);
            this.field_178734_a.rotateAngleZ = (float)Math.toRadians(-110.0f * heldPercent);
            final float rotationX = p_78087_7_.rotationPitch;
            this.bipedHead.rotateAngleX = (float)Math.toRadians(-rotationX * heldPercent) + (float)Math.toRadians(45.0f * heldPercent + rotationX);
            final float rotationY = ((EntityPlayer)p_78087_7_).renderYawOffset - p_78087_7_.rotationYaw;
            this.bipedHead.rotateAngleY = (float)Math.toRadians(rotationY * heldPercent) + (float)Math.toRadians(35.0f * heldPercent - rotationY);
            this.bipedHeadwear.rotateAngleX = (float)Math.toRadians(45.0f * heldPercent);
            this.bipedHeadwear.rotateAngleY = (float)Math.toRadians(35.0f * heldPercent);
            if (isOurPlayer && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0) {
                heldPercent = (DabModule.INSTANCE.getInputEvent().prevDabbingHeld + (DabModule.INSTANCE.getInputEvent().dabbingHeld - DabModule.INSTANCE.getInputEvent().prevDabbingHeld) * DabModule.INSTANCE.getInputEvent().firstPersonPartialTicks) / 8.0f;
