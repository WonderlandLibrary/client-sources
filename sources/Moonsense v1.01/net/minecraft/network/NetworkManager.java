...
public class NetworkManager extends SimpleChannelInboundHandler
{
    ...
    public void setConnectionState(final EnumConnectionState newState) {
        this.channel.attr((AttributeKey<EnumConnectionState>)NetworkManager.attrKeyConnectionState).set(newState);
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener[] futureListeners) {
        ...
        final EnumConnectionState var4 = this.channel.attr((AttributeKey<EnumConnectionState>)NetworkManager.attrKeyConnectionState).get();
        if (var4 != var3) {
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            final ChannelFuture var5 = this.channel.writeAndFlush(inPacket);
            if (futureListeners != null) {
                var5.addListeners((GenericFutureListener<? extends Future<? super Void>>[])futureListeners);
            }
            var5.addListener((GenericFutureListener<? extends Future<? super Void>>)ChannelFutureListener.FIRE_EXCEPTION_ON_FAILURE);
        }
        else {
            this.channel.eventLoop().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    final ChannelFuture var1 = NetworkManager.this.channel.writeAndFlush(inPacket);
                    if (futureListeners != null) {
                        var1.addListeners((GenericFutureListener<? extends Future<? super Void>>[])futureListeners);
                    }
                    var1.addListener((GenericFutureListener<? extends Future<? super Void>>)ChannelFutureListener.FIRE_EXCEPTION_ON_FAILURE);
                }
            });
        }
    }
    ...
    public static NetworkManager provideLanClient(final InetAddress p_150726_0_, final int p_150726_1_) {
        ...
        ((AbstractBootstrap<Bootstrap, C>)((AbstractBootstrap<Bootstrap, C>)new Bootstrap()).group((EventLoopGroup)NetworkManager.CLIENT_NIO_EVENTLOOP.getValue())).handler(new ChannelInitializer() {
            ...
            protected void initChannel(final Channel p_initChannel_1_) {
                ...
                try {
                    p_initChannel_1_.config().setOption(ChannelOption.TCP_NODELAY, SettingsManager.INSTANCE.noTCPDelay.getBoolean());
                }
    ...
    public static NetworkManager provideLocalClient(final SocketAddress p_150722_0_) {
        ...
        ((AbstractBootstrap<Bootstrap, C>)((AbstractBootstrap<Bootstrap, C>)new Bootstrap()).group((EventLoopGroup)NetworkManager.CLIENT_LOCAL_EVENTLOOP.getValue())).handler(new ChannelInitializer() {
