...
public class GuiScreenBook extends GuiScreen {
            ...
            if (this.bookPages != null) {
                ...
                if (publish) {
                    ...
                    for (int i = 0; i < this.bookPages.tagCount(); ++i) {
                        ...
                        final IChatComponent ichatcomponent = new ChatComponentText(s1);
    ...
    private void pageInsertIntoCurrent(final String p_146459_1_) {
        final String s = this.pageGetCurrent();
        final String s1 = s + p_146459_1_;
        final int i = this.fontRendererObj.splitStringWidth(s1 + "" + EnumChatFormatting.BLACK + "_", 118);
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (this.bookGettingSigned) {
            ...
            final String s1 = I18n.format("book.editTitle");
            final int k = this.fontRendererObj.width(s1);
            this.fontRendererObj.draw(s1, i + 36 + (116 - k) / 2, j + 16 + 16, 0);
            final int l = this.fontRendererObj.width(s);
            this.fontRendererObj.draw(s, i + 36 + (116 - l) / 2, j + 48, 0);
            final String s2 = I18n.format("book.byAuthor", this.editingPlayer.getCommandSenderName());
            final int i1 = this.fontRendererObj.width(s2);
            this.fontRendererObj.draw(EnumChatFormatting.DARK_GRAY + s2, i + 36 + (116 - i1) / 2, j + 48 + 10, 0);
            final String s3 = I18n.format("book.finalizeWarning");
            this.fontRendererObj.drawSplitString(s3, i + 36, j + 80, 116, 0);
        } else {
            final String s4 = I18n.format("book.pageIndicator", Integer.valueOf(this.currPage + 1), Integer.valueOf(this.bookTotalPages));
            String s5 = "";
            ...
            if (this.bookIsUnsigned) {
                ...
                if (ItemEditableBook.validBookTagContents(this.bookObj.getTagCompound())) {
                    try {
                        final IChatComponent ichatcomponent = IChatComponent.Serializer.jsonToComponent(s5);
                    ...
                    final ChatComponentText chatcomponenttext = new ChatComponentText(EnumChatFormatting.DARK_RED + "* Invalid book tag *");
            ...
            final int j1 = this.fontRendererObj.width(s4);
            this.fontRendererObj.draw(s4, i - j1 + this.bookImageWidth - 44, j + 16, 0);

            if (this.field_175386_A == null) {
                this.fontRendererObj.drawSplitString(s5, i + 36, j + 16 + 16, 116, 0);
            } else {
                final int k1 = Math.min(128 / this.fontRendererObj.FONT_HEIGHT, this.field_175386_A.size());

                for (int l1 = 0; l1 < k1; ++l1) {
                    final IChatComponent ichatcomponent2 = this.field_175386_A.get(l1);
                    this.fontRendererObj.draw(ichatcomponent2.getUnformattedText(), i + 36, j + 16 + 16 + l1 * this.fontRendererObj.FONT_HEIGHT, 0);
                }

                final IChatComponent ichatcomponent1 = this.func_175385_b(mouseX, mouseY);
     ...
     */
    public boolean handleComponentClick(final IChatComponent p_175276_1_) {
            ...
            if (i >= 0 && j >= 0) {
                ...
                if (i <= 116 && j < this.mc.fontRendererObj.FONT_HEIGHT * k + k) {
                    ...
                    if (l >= 0 && l < this.field_175386_A.size()) {
                        ...
                        for (final IChatComponent ichatcomponent1 : ichatcomponent) {
                            if (ichatcomponent1 instanceof ChatComponentText) {
                                i1 += this.mc.fontRendererObj.width(((ChatComponentText) ichatcomponent1).getChatComponentText_TextValue());
