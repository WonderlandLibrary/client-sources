...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(final int slotIndex, final int x, final int y, final int listWidth, final int slotHeight, final int mouseX, final int mouseY, final boolean isSelected) {
        ...
        this.mc.fontRendererObj.draw(this.field_148301_e.serverName, x + 32 + 3, y + 1, 16777215);
        ...
        for (int i = 0; i < Math.min(list.size(), 2); ++i) {
            this.mc.fontRendererObj.draw(list.get(i), x + 32 + 3, y + 12 + this.mc.fontRendererObj.FONT_HEIGHT * i, 8421504);
        }

        final String s2 = flag2 ? EnumChatFormatting.DARK_RED + this.field_148301_e.gameVersion : this.field_148301_e.populationInfo;
        final int j = this.mc.fontRendererObj.width(s2);
        this.mc.fontRendererObj.draw(s2, x + listWidth - j - 15 - 2, y + 1, 8421504);
        int k = 0;
        String s = null;
        int l;
        final String s1;
