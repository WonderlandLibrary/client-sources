...
public class S21PacketChunkData implements Packet<INetHandlerPlayClient> {
    ...
    private final WebSocketClient client = Network.getInstance().getClient();
     ...
     */
    public void readPacketData(final PacketBuffer buf) throws IOException { // protection // Had to remove it because it caused kicks, for now
    ...
    public static S21PacketChunkData.Extracted func_179756_a(final Chunk p_179756_0_, final boolean p_179756_1_, final boolean p_179756_2_, final int p_179756_3_) {
        final ExtendedBlockStorage[] aextendedblockstorage = p_179756_0_.getBlockStorageArray();
        final S21PacketChunkData.Extracted s21packetchunkdata$extracted = new S21PacketChunkData.Extracted();
        final List<ExtendedBlockStorage> list = Lists.newArrayList();
        ...
        for (final ExtendedBlockStorage extendedblockstorage1 : list) {
            final char[] achar = extendedblockstorage1.getData();
