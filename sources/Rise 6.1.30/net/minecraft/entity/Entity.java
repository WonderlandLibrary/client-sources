...
public abstract class Entity implements ICommandSender {
    ...
    public float prevPrevRotationYaw;
    ...
    public void onUpdate() {
        if (this.isInvisible() || !this.inView) {
            this.ticksVisible = 0;
        }
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (flag || safeWalk) {
                ...
                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, height, 0.0D)).isEmpty(); d3 = x) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
                ...
                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, height, z)).isEmpty(); d5 = z) {
                    if (z < d6 && z >= -d6) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= d6;
                ...
                for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, height, z)).isEmpty(); d5 = z) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
...
//        if (player) ChatUtil.display(f);
        ...
        if (f >= 1.0E-4F) {
            ...
            if (MoveUtil.isMoving() && player) {
//                ChatUtil.display("REal " + Math.hypot((double) (strafe * f2 - forward * f1), (double) (forward * f2 + strafe * f1)));
            }
        }
    ...
    public double[] moveFlyingValue(float strafe, float forward, float friction) {
        float yaw = this.rotationYaw;
