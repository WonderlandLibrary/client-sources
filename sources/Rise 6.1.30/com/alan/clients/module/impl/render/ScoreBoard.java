...
public final class ScoreBoard extends Module {
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        getLayer(REGULAR).add(() -> {
            ...
            if (outline.getValue()) {
                RenderUtil.roundedOutlineGradientRectangle(position.x - 1, position.y - 1, (maxWidth + padding * 4) + 2, (height + fontHeight + padding) + 2, round, 1f, ColorUtil.withAlpha(getTheme().getFirstColor(), 200), ColorUtil.withAlpha(getTheme().getSecondColor(), 200));
            }
        });
        ...
        getLayer(REGULAR, 1).add(() -> this.renderScoreboard(position.x, position.y, blur_color.getValue() ? new Color(getTheme().getSecondColor().getRed(), getTheme().getSecondColor().getGreen(), getTheme().getSecondColor().getBlue(), 60) : new Color(0, 0, 0, 120), true, round, false));
    };
...
//            GuiIngameCache.dirty = true;
        ...
        for (final Score score1 : collection) {
            ...
            fontRenderer.draw(s1, x, y, fontColor);
        }
    }
}