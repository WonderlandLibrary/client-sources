...
public final class Chat extends Module {
    ...
    private TextBox textField = new TextBox(new Vector2d(0, 0), inputFont, Color.WHITE, TextAlign.LEFT, "Start Typing...", 1000f, "abcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*()-_+=[{]};:.>,</?|  ' \" ` ~");
    private int lineCount, recentLineCount;
    private float lineHeight;
    private boolean clicked, open;
        ...
        switch (event.getKeyCode()) {
            ...
            case 15:
                if (matches == null) {
                    ...
                    int indexof = matches.indexOf(textField.getText());

                    if (matches.size() > indexof + 1) {
                        textField.text = matches.get(indexof + 1);
                    } else {
                        textField.text = matches.get(0);
                    }
    ...
    public void renderChat() {
        ...
        structure.scale = new Vector2d(mc.scaledResolution.getScaledWidth() / 3f - 20, scaleAnimation.getValue());
        ...
        List<ChatLine> lines = new ArrayList<>(mc.ingameGUI.getChatGUI().getChatLines());

        // Background
        double round = Math.min(structure.scale.y / 5f, 12);

        if (background.getValue()) {
            getLayer(REGULAR, 1).add(() -> RenderUtil.roundedRectangle(structure.position.x, structure.position.y, structure.scale.x, structure.scale.y, round, getTheme().getBackgroundShade()));
        ...
        getLayer(REGULAR, 1).add(() -> {
            ...
            if (lines.size() > 150) {
                List<ChatLine> linesPointer = mc.ingameGUI.getChatGUI().getChatLines();

                while (linesPointer.size() > 100) {
                    linesPointer.remove(linesPointer.get(linesPointer.size() - 1));
                }
            }
            ...
            update:
            {
                if (lineCount < lines.size()) {
                    if (hidePlayers.getValue()) {
                        mc.ingameGUI.getChatGUI().getChatLines().removeIf(line -> line.getChatComponent().getFormattedText().contains(":"));

                        if (lineCount >= mc.ingameGUI.getChatGUI().getChatLines().size()) {
                            break update;
                        }
                    }
            ...
            for (ChatLine chatLine : lines) {
                ...
                for (IChatComponent chatComponent : chatLine.getChatComponent()) {
                    ...
                    if (open && MouseUtil.isHovered(position.x, position.y, componentWidth, lineHeight)) {
                        ...
                        mc.currentScreen.handleComponentHover(chatComponent, (int) mouse.getX(), (int) mouse.getY());

                        if (!clicked && Mouse.isButtonDown(0)) mc.currentScreen.handleComponentClick(chatComponent);
    ...
    public void renderInput() {
        ...
        textField.setEmptyText("§7Start Typing...");
        ...
        getLayer(REGULAR, 1).add(() -> {
            glOpen.run();
            RenderUtil.roundedRectangle(inputPosition.x, inputPosition.y, inputScale.x, inputScale.y, 6, ColorUtil.withAlpha(getTheme().getBackgroundShade(), Math.min((int) inputOpacity.getValue(), getTheme().getBackgroundShade().getAlpha())));
