...
public class TextBox {
    ...
    public StopWatch drawn = new StopWatch();
    ...
    public TextBox(final Vector2d position, final Font fontRenderer, final Color color, final TextAlign textAlign, final String emptyText, final float width, final String allowedCharacters) {
        this.position = position;
        this.fontRenderer = fontRenderer;
        this.color = color;
        this.textAlign = textAlign;
    ...
    public void draw() {
        ...
        /* Renders Text */
        if (this.isEmpty()) {
            this.fontRenderer.draw(this.emptyText, posX, position.y, new Color(this.color.getRed(),
                    this.color.getBlue(), this.color.getGreen(), (int) (this.color.getAlpha() * (this.selected ? 0.3F : 0.2F))).hashCode());
        } else {
            this.fontRenderer.draw(drawnString.toString(), posX, position.y, this.color.hashCode());
        }
    ...
    public void key(final char typedChar, final int keyCode) {
        if (!this.selected || keyCode == Keyboard.KEY_RETURN) {
            return;
        }
        ...
        } else if (allowedCharacters.contains(character.toLowerCase())) {
