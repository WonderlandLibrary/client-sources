...
public class IRCInfoComponent extends Component implements Accessor {
    ...
    public static String formatNick(String nickname, String name) {
        UserInfo vantageUser = usersMap.get(name);
        if (vantageUser != null) {
            if (!nickname.contains("§" + vantageUser.getColorNick() + vantageUser.getMinecraftNickname() + " §7(" + name + "§7)§r")) {
                return nickname.replaceAll(name, "§" + vantageUser.getColorNick() + vantageUser.getMinecraftNickname() + " §7(" + name + "§7)§r");
            }
        } else {
            ...
            for (IChatComponent chatComponent : new ArrayList<>(wrapper.getChatComponent().getSiblings())) {
                if (chatComponent instanceof ChatComponentText) {
                    ...
                    for (Map.Entry<String, UserInfo> entry : usersMap.entrySet()) {
                        ...
                        newMessage = formatNick(newMessage, key);
                    }
