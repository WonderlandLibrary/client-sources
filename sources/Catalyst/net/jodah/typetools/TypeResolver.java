...
public final class TypeResolver
{
    ...
    public static <T, S extends T> Type reify(final Class<T> type, final Class<S> context) {
        return reify(resolveGenericType(type, context), getTypeVariableMap(context, null));
    }
    
    public static Type reify(final Type type, final Class<?> context) {
    ...
    private static Type reify(Type genericType, final Map<TypeVariable<?>, Type> typeVariableMap, Map<Type, Type> cache) {
        ...
        if (genericType instanceof ParameterizedType) {
            ...
            final Type[] upperBounds2 = wildcardType.getUpperBounds();
            final Type[] lowerBounds = wildcardType.getLowerBounds();
            if (upperBounds2.length == 1 && lowerBounds.length == 0) {
                return reify(upperBounds2[0], typeVariableMap, cache);
            }
            throw new UnsupportedOperationException("Attempted to reify wildcard type with name '" + wildcardType + "' which has " + upperBounds2.length + " upper bounds and " + lowerBounds.length + " lower bounds. Reification of wildcard types is only supported for the trivial case of exactly one upper bound and no lower bounds.");
        }
    }
